...
final class iTermSubStringTests: XCTestCase {
    ...
    func testSubstringRangeAndCount_characterAt() {
        ...
        guard let sub2 = iTermSubString(base: base, range: nsRange) else {
            XCTFail("Failed to create iTermSubString with NSRange")
            return
        }
    ...
    func testHasEqual_usedLength_isEmpty() {
        ...
        let eq = buf.withUnsafeBufferPointer { ptr in
            ...
            return sub.hasEqual(range: NSRange(location: 0, length: 2), to: baseAddress)
        }
        XCTAssertTrue(eq)
    ...
    func testClone_mutableClone() throws {
        let text = "1234"
        let data = Data(text.utf8)
        let style = makeStyle()
        let base = iTermASCIIString(data: data, style: style, ea: nil)
        ...
        let m = try XCTUnwrap(sub.mutableClone() as? iTermMutableStringProtocolSwift)
    ...
    func testExternalAttributesIndex_isSparse() {
        ...
        guard let mutable = base.mutableClone() as? iTermMutableStringProtocolSwift else {
            XCTFail("Expected mutableClone to return iTermMutableStringProtocolSwift")
            return
        }
    ...
    func testRoundTrip_uniform() throws {
        ...
        let original = iTermSubString(base: base, range: 1..<3)
        ...
        let decoded: iTermSubString = try iTermSubString.create(efficientDecoder: &decoder)
