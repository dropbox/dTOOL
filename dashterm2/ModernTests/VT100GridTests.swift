...
class VT100GridTests: XCTestCase {
    ...
    func testLengthOfLineNumber() {
        let width: Int32 = 4
        let height: Int32 = 4
        guard let grid = VT100Grid(size: VT100GridSize(width: width, height: height), delegate: nil) else {
    ...
    func testMoveCursorLeftWrappingAroundDoubleWideCharEOL() {
        // 3×3 grid
        guard let grid = VT100Grid(size: VT100GridSize(width: 3, height: 3), delegate: nil) else {
    ...
    func testScrollRectDownBy_ZeroDoesNothingAndMarksAllCellsDirty() {
        ...
        grid.scroll(rect, downBy: 0, softBreak: false, fill: screen_char_t())
    ...
    func testScrollRectDownBy_One_MarksRegionLinesDirty() {
        ...
        let rect = VT100GridRect(x: 1, y: 1, width: 2, height: 2)
        grid.scroll(rect, downBy: 1, softBreak: false, fill: screen_char_t())
    ...
    func testScrollRectDownBy_Two_MarksRegionLinesDirty() {
        ...
        grid.scroll(rect, downBy: 2, softBreak: false, fill: screen_char_t())
    ...
    func testScrollRectDownBy_Height_EqualsRegionHeight_MarksRegionLinesDirty() {
        ...
        let rect = VT100GridRect(x: 1, y: 1, width: 3, height: 3)
        grid.scroll(rect, downBy: 3, softBreak: false, fill: screen_char_t())
    ...
    func testScrollRectDownBy_GreaterThanRegionHeight_MarksRegionLinesDirty() {
        ...
        let rect = VT100GridRect(x: 1, y: 1, width: 3, height: 3)
        grid.scroll(rect, downBy: 4, softBreak: false, fill: screen_char_t())
    ...
    func testSetContentsFromDVRFrame() {
        ...
        guard let testGrid1 = VT100Grid(size: VT100GridSize(width: 4, height: 4), delegate: nil) else {
        ...
        metadata.withUnsafeMutableBufferPointer { ubp in
            guard let baseAddr = ubp.baseAddress else { return }
            testGrid1.setContentsFromDVRFrame(frame, metadataArray: baseAddr, info: info1)
        }
        ...
        guard let testGrid2 = VT100Grid(size: VT100GridSize(width: 3, height: 3), delegate: nil) else {
        ...
        metadata.withUnsafeMutableBufferPointer { ubp in
            guard let baseAddr = ubp.baseAddress else { return }
            testGrid2.setContentsFromDVRFrame(frame, metadataArray: baseAddr, info: info1)
        }
        ...
        guard let testGrid3 = VT100Grid(size: VT100GridSize(width: 5, height: 5), delegate: nil) else {
        ...
        metadata.withUnsafeMutableBufferPointer { ubp in
            guard let baseAddr = ubp.baseAddress else { return }
            testGrid3.setContentsFromDVRFrame(frame, metadataArray: baseAddr, info: info1)
        }
    ...
    func testSetBgFgColorInRect() {
        ...
        for y in 0..<4 {
            guard let line = grid.screenChars(atLineNumber: Int32(y)) else {
                XCTFail("Failed to get screen chars at line \(y)")
                return
            }
    ...
    private func screenCharLine(forString string: String) -> UnsafeMutablePointer<screen_char_t> {
        guard let data = NSMutableData(length: string.count * MemoryLayout<screen_char_t>.size) else {
        ...
        for (i, c) in string.enumerated() {
            var charCode: unichar = (c.utf16.first ?? 0)

            if charCode == (Character("-").utf16.first ?? 0) {
                charCode = unichar(DWC_RIGHT)
            } else if charCode == (Character(".").utf16.first ?? 0) {
                charCode = 0
            }
    ...
    func testRestoreScreenFromLineBuffer() {
        ...
        guard let line = grid3.screenChars(atLineNumber: 1) else {
    ...
    func testRectsForRun() {
        ...
        x.code = (Character("x").utf16.first ?? 0)
    ...
    private func gridFromCompactLinesWithContinuationMarks(_ compact: String) -> VT100Grid {
        ...
        guard let grid = VT100Grid(size: VT100GridSize(width: Int32(lines[0].count - 1),
                                                       height: Int32(lines.count)),
                                   delegate: nil) else {
            it_fatalError("Failed to create VT100Grid")
        }

        for (i, line) in lines.enumerated() {
            guard let s = grid.screenChars(atLineNumber: Int32(i)) else {
                continue
            }

            for j in 0..<(line.count - 1) {
                let index = line.index(line.startIndex, offsetBy: j)
                var c = unichar(line[index].utf16.first ?? 0)

                if c == (Character(".").utf16.first ?? 0) { c = 0 }
                if c == (Character("-").utf16.first ?? 0) { c = unichar(DWC_RIGHT) }

                let lastCharIndex = line.index(line.startIndex, offsetBy: line.count - 1)
                if c == (Character(">").utf16.first ?? 0) &&
                    j == line.count - 2 &&
                    line[lastCharIndex] == ">" {
                    c = unichar(DWC_SKIP)
                }
    ...
    private func gridFromCompactLines(_ compact: String) -> VT100Grid {
        let lines = compact.components(separatedBy: "\n")
        guard let grid = VT100Grid(size: VT100GridSize(width: Int32(lines[0].count),
                                                       height: Int32(lines.count)),
                                   delegate: nil) else {
            it_fatalError("Failed to create VT100Grid")
        }
    ...
    private func screenCharLineForString(_ string: String) -> UnsafeMutablePointer<screen_char_t> {
        ...
        for i in 0..<string.count {
            ...
            var c = string[stringIndex].utf16.first ?? 0

            if c == (Character("-").utf16.first ?? 0) {
                c = unichar(DWC_RIGHT)
            } else if c == (Character(".").utf16.first ?? 0) {
                c = 0
            }
    ...
    func testInsertChar_nonNullChar() {
        ...
        c.code = (Character("x").utf16.first ?? 0)
    ...
    func testGridRunFromRangeBasic() {
        ...
        guard let grid = VT100Grid(size: VT100GridSize(width: 10, height: 10), delegate: nil) else {
    ...
    func testSingleColumnLineBuffer() {
        guard let grid = VT100Grid(size: VT100GridSize(width: 1, height: 24), delegate: nil) else {
    ...
    func testRemoveLastRawLineWrapped() {
        guard let grid = VT100Grid(size: VT100GridSize(width: 80, height: 24), delegate: nil) else {
    ...
    func lineAsString(_ i: Int32) -> String {
        ...
        for i in 0..<Int(msca.length) {
            if line[i].code == DWC_SKIP {
                line[i].code = ">".utf16.first ?? UInt16(Character(">").asciiValue ?? 62)
            } else if line[i].code == DWC_RIGHT {
                line[i].code = "-".utf16.first ?? UInt16(Character("-").asciiValue ?? 45)
            }
        }
