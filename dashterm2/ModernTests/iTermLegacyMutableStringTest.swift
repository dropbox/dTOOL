...
final class iTermLegacyMutableStringTests: XCTestCase {
    ...
    func testIsEmpty_andUsedLength() {
        ...
        var style = makeStyle()
        style.code = UInt16(("X".utf16.first ?? 0))
    ...
    func testCloneAndMutableCloneIndependence() {
        ...
        guard let mut = m.mutableClone() as? iTermMutableStringProtocolSwift else {
            XCTFail("mutableClone should return iTermMutableStringProtocolSwift")
            return
        }
    ...
    func testEAIndexCreateAndSetAndExternalAttribute() {
        ...
        guard let idx = m.eaIndex(createIfNeeded: true) else {
            XCTFail("eaIndex should be created")
            return
        }
    ...
    func testSetExternalAttributesSetterNilAndNonNil() {
        ...
        guard let idx = m.externalAttributesIndex() else {
            XCTFail("externalAttributesIndex should exist")
            return
        }
    ...
    func testTLVEncoder() throws {
        ...
        guard var metadataDecoder = dict[.metadata] else {
            XCTFail("metadata key should exist")
            return
        }
        let decoded = try Data.create(efficientDecoder: &metadataDecoder)
