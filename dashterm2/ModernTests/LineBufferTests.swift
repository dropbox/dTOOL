...
class LineBufferTests: XCTestCase {
    ...
    func testFindSubstringMultiLineAcrossBlocks() {
        let buffer = LineBuffer(blockSize: 32)
        let width: Int32 = 80
        buffer.append(screenCharArrayWithDefaultStyle("foo", eol: EOL_HARD), width: width)
        buffer.forceSeal()
        buffer.append(screenCharArrayWithDefaultStyle("bar", eol: EOL_HARD), width: width)

        let context = FindContext()
        buffer.prepareToSearch(for: "foo\nbar",
        ...
        repeat {
            ...
            attempts += 1
            XCTAssertLessThan(attempts, 8, "Search should converge quickly")
        } while context.status == .Searching

        XCTAssertEqual(context.status, .Matched, "Expected to find a cross-block multi-line match")
        guard let match = context.results?.firstObject as? ResultRange else {
            XCTFail("Expected a result range")
            return
        }
        XCTAssertEqual(match.position, 0, "Match should start at the beginning of the buffer")
        XCTAssertEqual(match.length, 6, "Match should span both raw lines without inserting newline characters")
        XCTAssertFalse(context.includesPartialLastLine, "Hard-EOL lines should not mark partial last line")
    }
    ...
    func testPackingStatsTracksSavings() throws {
        let buffer = LineBuffer(blockSize: 512)
        let width: Int32 = 80
        appendLines(count: 32, width: width, to: buffer)
        buffer.seal()
        appendLines(count: 32, width: width, to: buffer)
        buffer.seal()

        let stats = buffer.packingStats()
        XCTAssertGreaterThan(stats.totalBlocks, 0)
        XCTAssertGreaterThan(stats.packedBlocks, 0)
        XCTAssertEqual(stats.totalRawLines, 64)
        XCTAssertGreaterThan(stats.estimatedBytesSaved, 0)
        XCTAssertEqual(stats.estimatedUnpackedBytes - stats.currentBytes,
                       stats.estimatedBytesSaved)
    }

    private func appendLines(count: Int, width: Int32, to buffer: LineBuffer) {
        let content = String(repeating: "A", count: Int(width))
        for _ in 0..<count {
            let line = screenCharArrayWithDefaultStyle(content, eol: EOL_HARD)
...
extension ScreenCharArray {
    static func create(string: String,
        ...
        guard let malloced = malloc(3 * augmentedString.utf16.count * MemoryLayout<screen_char_t>.size) else {
            it_fatalError("Failed to allocate memory for screen chars")
        }
...
extension screen_char_t {
    ...
    func with(code: unichar) -> screen_char_t {
        return screen_char_t(code: code,
                             ...
                             underlineStyle0: underlineStyle0,
                             invisible: invisible,
                             inverse: inverse,
                             guarded: guarded,
                             virtualPlaceholder: 0,
                             rtlStatus: .unknown,
                             underlineStyle1: underlineStyle1,
                             unused: unused)
    }
}
...
func screenCharArrayWithDefaultStyle(_ string: String, eol: Int32) -> ScreenCharArray {
    ...
    guard let msca = sca.mutableCopy() as? MutableScreenCharArray else {
        it_fatalError("Failed to create mutable copy of ScreenCharArray")
    }
    let line = msca.mutableLine
    for i in 0..<Int(msca.length) {
        if line[i].code == ("-".utf16.first ?? 0) {
            line[i].code = UInt16(DWC_RIGHT)
        } else if line[i].code == (">".utf16.first ?? 0) && i == Int(msca.length - 1) {
