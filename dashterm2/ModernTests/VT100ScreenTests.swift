...
class VT100ScreenTests: XCTestCase {
    ...
    private func showAltAndUppercase(_ screen: VT100Screen) {
        screen.performBlock(joinedThreads: { _, mutableState, _ in
            guard let temp = mutableState?.currentGrid.copy() as? VT100Grid else { return }
            mutableState?.showAltBuffer()
            for y in 0..<screen.height() {
                guard let lineIn = temp.screenChars(atLineNumber: y),
                      let lineOut = mutableState?.currentGrid.screenChars(atLineNumber: y) else { continue }
    ...
    private func commonNoteResizeRegressionTest(initialRange range1: VT100GridCoordRange,
        ...
        guard let entry1 = restoredNote1.entry else {
            XCTFail("Expected entry")
            return
        }
        let rangeAfterResize1 = screen.coordRange(for: entry1.interval)
        ...
        guard let entry2 = restoredNote2.entry else {
            XCTFail("Expected entry")
            return
        }
        let rangeAfterResize2 = screen.coordRange(for: entry2.interval)
    ...
    private func makeMixedToken(_ string: String) -> VT100Token {
        ...
        data.withUnsafeMutableBytes { umrbp -> Void in
            ...
            guard let baseAddr = umbp.baseAddress else { return }
            token.setAsciiBytes(baseAddr,
    ...
    private func performRandomGangTest(prng: inout SeededGenerator) -> Bool {
        ...
        let initialLines = (0..<numInitialLines).map { i in
            ...
            return String(repeating: Character(scalar), count: prng.next(in: 0..<80))
        }
        var tokens = [String]()
        var letter = 65 + 32
        ...
        for _ in 0..<numTokens {
            ...
            var token = ""
            for j in 0..<numLines {
                let letterScalar = UnicodeScalar(letter)!
                token.append(String(repeating: Character(letterScalar),
...
fileprivate class FakeSession: NSObject, VT100ScreenDelegate {
    ...
    func screenHandleFramerRecoveryString(_ string: String) -> DashTerm2SharedARC.ConductorRecovery? {
        nil
    }
    ...
    func screenSwitchToSharedState() -> VT100ScreenState {
        guard let screen = screen else {
            it_fatalError("Screen is nil in screenSwitchToSharedState")
        }
        return screen.switchToSharedState()
    }
    ...
    func screenDidAdd(_ porthole: any DashTerm2SharedARC.ObjCPorthole) {
    ...
    // MARK: - iTermTriggerSideEffectExecutor (added method)
