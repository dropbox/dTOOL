...
final class iTermRopeTests: XCTestCase {
    ...
    private func legacy(string: String) -> iTermLegacyStyleString {
        ...
        return buffer.withUnsafeMutableBufferPointer { umbp in
            guard let baseAddress = umbp.baseAddress else {
                XCTFail("Failed to get buffer base address")
                return iTermLegacyStyleString(chars: UnsafeMutablePointer<screen_char_t>.allocate(capacity: 0), count: 0, eaIndex: nil)
            }
            var len = Int32(umbp.count)
            StringToScreenChars(string, baseAddress, screen_char_t(), screen_char_t(), &len, false, nil, nil, iTermUnicodeNormalization.none, 9, false, nil)
            return iTermLegacyStyleString(chars: baseAddress, count: Int(len), eaIndex: nil)
        }
    }
    ...
    func testHasEqualAcrossSegments() {
        ...
        let eq = buf.withUnsafeBufferPointer { ptr in
            ...
            return rope.hasEqual(range: NSRange(location: 0, length: 4), to: baseAddress)
        }
        XCTAssertTrue(eq)
        buf[2].code = 0
    ...
    func testCloneIndependence() throws {
        ...
        let m = try XCTUnwrap(rope.mutableClone() as? iTermMutableStringProtocolSwift)
        m.delete(range: 0..<1)
    ...
    func testCopyConstructorAndDeepClone() throws {
        let a = makeASCII("ab")
        let b = makeASCII("cd")
        ...
        let m1 = try XCTUnwrap(rope1.mutableClone() as? iTermMutableStringProtocolSwift)
        m1.delete(range: 0..<1)
        // rope2 unaffected
        XCTAssertEqual(rope2.cellCount, 4)
        // NSCopying
        let rope3 = try XCTUnwrap(rope1.copy() as? iTermRope)
        XCTAssertTrue(rope3.isEqual(to: rope1))
        // mutableCopy
        let m3 = try XCTUnwrap(rope1.mutableCopy() as? iTermMutableStringProtocolSwift)
    ...
    func testUsedLengthMultipleSegmentsTraversed() {
        ...
        var c = screen_char_t()
        c.code = UInt16(("C".utf16.first ?? 0))     // 'C'
    ...
    func testHasEqualWithOffsetSegments() {
        ...
        let eq = buf.withUnsafeBufferPointer { ptr in
            guard let baseAddress = ptr.baseAddress else { return false }
            return rope.hasEqual(range: NSRange(location: 2, length: 2), to: baseAddress)
        }
        XCTAssertTrue(eq)
    ...
    func testRoundTrip() throws {
        ...
        let substring = {
            ...
            return iTermSubString(base: base, range: 1..<3)
        }()
        let segments: [iTermString] = [uniform, legacyStyle, ascii, nonAscii, substring]
        let original = iTermRope(segments)
