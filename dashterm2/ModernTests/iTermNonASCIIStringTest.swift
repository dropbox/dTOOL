...
final class iTermNonASCIIStringTest: XCTestCase {
    ...
    func testHydrateIntoMutable_andHasEqual() {
        ...
        let eq = buf.withUnsafeBufferPointer { ptr in
            ...
            return s.hasEqual(range: NSRange(location: 0, length: 3), to: baseAddress)
        }
        XCTAssertTrue(eq)
    }
    ...
    func testClone_mutableClone_andMutation() throws {
        ...
        let m = try XCTUnwrap(s.mutableClone() as? iTermMutableStringProtocolSwift)
    ...
    func testHydrateIntoCopiesExtendedAttributes() {
        ...
        guard let urlObj = URL(string: "https://x") else {
            XCTFail("Failed to create test URL")
            return
        }
        let url = iTermURL(url: urlObj, identifier: nil, target: nil)
