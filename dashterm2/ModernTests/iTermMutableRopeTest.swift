...
final class iTermMutableRopeTests: XCTestCase {
    func testCellCountAndCharacter_atIndex() {
        ...
        let firstCharCode = UInt16(("H".utf16.first ?? 0))
        var char = style
    ...
    func testSubstringAndIsEqualRanges() {
        ...
        m.erase(defaultChar: makeStyleChar(letter: UInt16(("A".utf16.first ?? 0))))
    ...
    func testCloneAndMutableCloneIndependence() throws {
        let style = makeStyle()
        let m = iTermMutableRope(iTermLegacyMutableString(width: 2))
        var char = style
        char.code = UInt16(("X".utf16.first ?? 0))
        m.erase(defaultChar: char)
        let clone = m.clone()
        XCTAssertTrue(clone.isEqual(to: m))
        let mut = try XCTUnwrap(m.mutableClone() as? iTermMutableStringProtocolSwift)
    ...
    func testResetRTLStatusAndSetRTLIndexes() {
        let style = makeStyle()
        var sct = style
        sct.code = UInt16(("Z".utf16.first ?? 0))
        let m = iTermMutableRope(iTermLegacyMutableString(width: 3))
    ...
    func testInitWidthAndCharacter() {
        ...
        var style = makeStyle()
        style.code = UInt16(("Z".utf16.first ?? 0))
    ...
    func testReplaceAndInsertAndAppend() {
        ...
        defaultChar.code = UInt16(("A".utf16.first ?? 0))
    ...
    func testResetRTLAndSetRTLIndexes() {
        let style = makeStyle()
        var styleA = style
        styleA.code = UInt16(("A".utf16.first ?? 0))
    ...
    func testStringBySettingRTL_NilAndNonNil() {
        ...
        var ch = charStyle
        ch.code = UInt16(("X".utf16.first ?? 0))
    ...
    func testHasEqualTrueAndFalse() {
        ...
        let eq = buf.withUnsafeBufferPointer { ptr in
            ...
            return m.hasEqual(range: NSRange(location: 0, length: 3), to: baseAddress)
        }
        XCTAssertTrue(eq)
        // mutate one entry
        buf[1].code = 0
    ...
    private func makeStyle() -> screen_char_t {
        ...
        ScreenCharSetUnderlineStyle(&s, .single); s.invisible = 0; s.inverse = 1
    ...
    func testHeterogeneousAppendDeltaAndHydrate() {
        ...
        // Legacy segment "C"
        var c = style
        c.code = UInt16(Character("C").utf16.first ?? 0)
        ...
        let D: UInt16 = UInt16(Character("D").utf16.first ?? 0)
        let E: UInt16 = UInt16(Character("E").utf16.first ?? 0)
        let codes = [D, E]
    ...
    func testCrossSegmentDeleteFromStartAndEnd() {
        ...
        var c = style; c.code = UInt16(Character("C").utf16.first ?? 0)
    ...
    func testCopyAndMutableCloneIndependence() throws {
        let style = makeStyle()
        let ascii = iTermASCIIString(data: Data("ABC".utf8), style: style, ea: nil)
        let original = iTermMutableRope([ascii])
        let clone = try XCTUnwrap(original.mutableClone() as? iTermMutableRope)
        ...
        // mutate clone
        let clone2 = try XCTUnwrap(original.mutableClone() as? iTermMutableRope)
