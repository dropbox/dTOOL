...
class LineBlockTests: XCTestCase {
        ...
        let content = { () -> any iTermString in
            if let data = string.data(using: .ascii) {
                ...
                return buffer.withUnsafeMutableBufferPointer { umbp in
                    ...
                    StringToScreenChars(string, umbp.baseAddress ?? UnsafeMutablePointer<screen_char_t>.allocate(capacity: 0), screen_char_t(), screen_char_t(), &len, false, nil, nil, iTermUnicodeNormalization.none, 9, false, nil)
                    let eaIndex = externalAttribute.map { iTermUniformExternalAttributes.withAttribute($0) }
                    return iTermLegacyStyleString(chars: umbp.baseAddress ?? UnsafeMutablePointer<screen_char_t>.allocate(capacity: 0), count: Int(len), eaIndex: eaIndex)
                }
            }
        }()
    ...
    func testAppendLineStringSucceedsWithinCapacityHardEOL() {
        ...
        guard let block = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 1) else {
    ...
    func testIsEmptyAndAllLinesAreEmptyWhenNoEntries() {
        ...
        let capacity: Int32 = 10
        guard let block = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 0) else {
    ...
    func testDictionaryRoundTripPreservesContents() {
        ...
        guard let originalBlock = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 123) else {
    ...
    func testCopyDeepProducesIndependentBlock() {
        ...
        let capacity: Int32 = 20
        guard let original = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 42) else {
        ...
        guard let copy = original.copy(withAbsoluteBlockNumber: 0) else {
    ...
    func testCowCopySetsHasBeenCopiedAndProgenitor() {
        ...
        guard let original = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 0) else {
        ...
        guard let copy = original.cowCopy() else {
            XCTFail("Failed to COW copy LineBlock")
            return
        }
    ...
    func testOffsetOfWrappedLineClampsOutOfRangeSegmentRequests() {
        // BUG-1435: OffsetOfWrappedLine previously trusted asserts to prevent out-of-range
        // accesses. In release builds an oversized segment index would attempt to read past
        // the end of the buffer. Verify that we now clamp to the end of the raw line.
        ...
        XCTAssertEqual(wrappedCount, 5,
                       "Raw length 10 at width 2 should wrap into five segments")

        let rawLength = Int32(block.length(ofRawLine: 0))
        let startOffset = block.offset(ofRawLine: 0)

        let invalidSegmentIndex = wrappedCount  // first index beyond the final wrapped line
        let clampedOffset = OffsetOfWrappedLine(block.rawLine(0),
                                                invalidSegmentIndex,
                                                rawLength,
                                                wrapWidth,
                                                true)

        XCTAssertEqual(clampedOffset, startOffset + rawLength,
                       "Out-of-range segment requests should clamp to the end of the line")
    }
    ...
    func testNumberOfFullLinesFromOffsetSimpleDivision() {
        ...
        guard let block = LineBlock(rawBufferSize: 0, absoluteBlockNumber: 0) else {
    ...
    func testRandomMatchesGoldenData() throws {
        ...
        guard let block = LineBlock(rawBufferSize: 1000, absoluteBlockNumber: 0) else {
    ...
    func testFindSubstringPlainForwardSingleResult() {
        ...
        guard let block = LineBlock(rawBufferSize: 100, absoluteBlockNumber: 0) else {
        ...
        guard let match = results[0] as? ResultRange else {
            XCTFail("Failed to cast to ResultRange")
            return
        }
    ...
    func testRawLineNumberAtWrappedLineOffsetOutParameter() {
        let capacity: Int32 = 32
        guard let block = LineBlock(rawBufferSize: capacity, absoluteBlockNumber: 1) else {
        ...
        // Append two hard-EOL lines whose wrapped segments span multiple rows.
        XCTAssertTrue(block.appendLineString(makeLineString("ABC", eol: EOL_HARD), width: wrapWidth))
        XCTAssertTrue(block.appendLineString(makeLineString("DEFG", eol: EOL_HARD), width: wrapWidth))

        var rawLine: Int32 = -1
        XCTAssertTrue(block.rawLineNumber(atWrappedLineOffset: 0, width: wrapWidth, rawLineNumber: &rawLine))
        XCTAssertEqual(rawLine, 0)

        XCTAssertTrue(block.rawLineNumber(atWrappedLineOffset: 2, width: wrapWidth, rawLineNumber: &rawLine))
        XCTAssertEqual(rawLine, 1)

        rawLine = 99
        XCTAssertFalse(block.rawLineNumber(atWrappedLineOffset: 5, width: wrapWidth, rawLineNumber: &rawLine))
        XCTAssertEqual(rawLine, 99)
    }
    ...
    func testRawLineWithMetadataAtWrappedLineOffsetReturnsCorrectMetadata() {
        ...
        guard let exampleURL = URL(string: "https://example.com"),
              let swiftURL = URL(string: "https://swift.org") else {
            XCTFail("Failed to create URL")
            return
        }
        let url1 = iTermURL(url: exampleURL, identifier: nil, target: nil)
        let url2 = iTermURL(url: swiftURL, identifier: nil, target: nil)
    ...
    func testDropMirroringProgenitorDropsLeadingLinesToMatchProgenitor() {
        ...
        // GIVEN
        guard let progenitor = LineBlock(rawBufferSize: 100, absoluteBlockNumber: 0) else {
        ...
        guard let client = progenitor.cowCopy() else {
            XCTFail("Failed to COW copy LineBlock")
            return
        }
    ...
    func testFindSubstringFindOneResultPerRawLine() {
        ...
        guard let firstMatch = results[0] as? ResultRange else {
            XCTFail("Failed to cast to ResultRange")
            return
        }
        ...
        guard let secondMatch = results[1] as? ResultRange else {
            XCTFail("Failed to cast to ResultRange")
            return
        }
    ...
    func testConvertPositionDoubleWidthBranch() {
        ...
        guard let block = LineBlock(rawBufferSize: 10, absoluteBlockNumber: 0) else {
    ...
    // MARK: - Binary Search Wrapped Line Optimization Tests

    /// Test that binary search finds wrapped lines correctly for a single raw line.
    func testBinarySearchSingleRawLine() {
        guard let block = LineBlock(rawBufferSize: 100, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }

        // Create a line that will wrap: 10 characters at width 3 = 4 wrapped lines
        // "abcdefghij" -> "abc", "def", "ghi", "j"
        let lineString = makeLineString("abcdefghij")
        _ = block.appendLineString(lineString, width: 3)

        // Verify we can get all 4 wrapped lines
        let width: Int32 = 3
        XCTAssertEqual(block.getNumLines(withWrapWidth: width), 4, "Should have 4 wrapped lines")

        // Get each wrapped line by index
        for i: Int32 in 0..<4 {
            let sca = block.screenCharArrayForWrappedLine(withWrapWidth: width,
                                                         lineNum: i,
                                                         paddedTo: width,
                                                         eligibleForDWC: false)
            XCTAssertNotNil(sca, "Should get wrapped line \(i)")
        }
    }

    /// Test binary search with multiple raw lines that wrap differently.
    func testBinarySearchMultipleRawLines() {
        guard let block = LineBlock(rawBufferSize: 200, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }

        // Line 1: "abc" at width 10 = 1 wrapped line
        // Line 2: "defghijklmnop" at width 10 = 2 wrapped lines (10 + 3)
        // Line 3: "qrs" at width 10 = 1 wrapped line
        // Total: 4 wrapped lines
        _ = block.appendLineString(makeLineString("abc"), width: 10)
        _ = block.appendLineString(makeLineString("defghijklmnop"), width: 10)
        _ = block.appendLineString(makeLineString("qrs"), width: 10)

        let width: Int32 = 10
        XCTAssertEqual(block.getNumLines(withWrapWidth: width), 4, "Should have 4 wrapped lines")
        XCTAssertEqual(block.numRawLines(), 3, "Should have 3 raw lines")

        // Verify each wrapped line can be retrieved
        for i: Int32 in 0..<4 {
            let sca = block.screenCharArrayForWrappedLine(withWrapWidth: width,
                                                         lineNum: i,
                                                         paddedTo: width,
    ...
    /// Test binary search with empty lines interspersed.
    func testBinarySearchWithEmptyLines() {
        guard let block = LineBlock(rawBufferSize: 200, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }
        ...
        // Line 3: "def"
        _ = block.appendLineString(makeLineString("abc"), width: 10)
        _ = block.appendLineString(makeLineString(""), width: 10)
        _ = block.appendLineString(makeLineString("def"), width: 10)

        let width: Int32 = 10
        XCTAssertEqual(block.numRawLines(), 3, "Should have 3 raw lines")
        XCTAssertEqual(block.getNumLines(withWrapWidth: width), 3, "Should have 3 wrapped lines")
    ...
    /// Test binary search with many raw lines (stress test for O(log n) performance).
    func testBinarySearchManyRawLines() {
        guard let block = LineBlock(rawBufferSize: 10000, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }

        // Add 100 raw lines, each 5 characters
        for _ in 0..<100 {
            _ = block.appendLineString(makeLineString("abcde"), width: 80)
        }

        let width: Int32 = 80
        XCTAssertEqual(block.numRawLines(), 100, "Should have 100 raw lines")
        XCTAssertEqual(block.getNumLines(withWrapWidth: width), 100, "Should have 100 wrapped lines")

        // Verify first, middle, and last wrapped lines can be retrieved
        XCTAssertNotNil(block.screenCharArrayForWrappedLine(withWrapWidth: width,
                                                           lineNum: 0,
                                                           paddedTo: width,
                                                           eligibleForDWC: false))
        XCTAssertNotNil(block.screenCharArrayForWrappedLine(withWrapWidth: width,
    ...
    /// Test that binary search cache is invalidated when lines are modified.
    func testBinarySearchCacheInvalidation() {
        guard let block = LineBlock(rawBufferSize: 200, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }
        ...
        _ = block.screenCharArrayForWrappedLine(withWrapWidth: width,
                                               lineNum: 0,
                                               paddedTo: width,
                                               eligibleForDWC: false)

        // Add another line - should invalidate cache
        _ = block.appendLineString(makeLineString("ghi"), width: 10)

        XCTAssertEqual(block.getNumLines(withWrapWidth: width), 3, "Should have 3 wrapped lines after append")
        ...
        for i: Int32 in 0..<3 {
            let sca = block.screenCharArrayForWrappedLine(withWrapWidth: width,
                                                         ...
                                                         paddedTo: width,
                                                         eligibleForDWC: false)
            XCTAssertNotNil(sca, "Should get wrapped line \(i) after cache invalidation")
        }
    }

    /// Test binary search with different widths (cache should rebuild).
    func testBinarySearchDifferentWidths() {
        guard let block = LineBlock(rawBufferSize: 200, absoluteBlockNumber: 0) else {
            XCTFail("Failed to create LineBlock")
            return
        }

        // "abcdefghij" at width 5 = 2 wrapped lines, at width 3 = 4 wrapped lines
        _ = block.appendLineString(makeLineString("abcdefghij"), width: 10)

        // At width 5
        let width5: Int32 = 5
        XCTAssertEqual(block.getNumLines(withWrapWidth: width5), 2, "Should have 2 wrapped lines at width 5")
        XCTAssertNotNil(block.screenCharArrayForWrappedLine(withWrapWidth: width5,
                                                           lineNum: 0,
                                                           paddedTo: width5,
                                                           eligibleForDWC: false))
        XCTAssertNotNil(block.screenCharArrayForWrappedLine(withWrapWidth: width5,
        ...
        // At width 3 - cache should rebuild
        let width3: Int32 = 3
        XCTAssertEqual(block.getNumLines(withWrapWidth: width3), 4, "Should have 4 wrapped lines at width 3")
        for i: Int32 in 0..<4 {
            XCTAssertNotNil(block.screenCharArrayForWrappedLine(withWrapWidth: width3,
                                                               lineNum: i,
                                                               paddedTo: width3,
                                                               eligibleForDWC: false))
        }
...
extension LineBlock {
    ...
    func popLastLine(width: Int32) -> ScreenCharArray? {
        ...
        guard let linePtr = line else {
            return nil
        }
        return ScreenCharArray(line: linePtr,
                               length: length,
                               metadata: metadata,
                               continuation: continuation)
    }
