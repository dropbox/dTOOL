...
class iTermBrowserPageSaverTestHelper {
    ...
    private func createTestHTML() {
        ...
        do { try html.write(to: htmlFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    }
    ...
    private func createTestCSS() {
        ...
        do { try css.write(to: cssFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try importedCSS.write(to: importedCSSFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    }
    ...
    private func createTestImage() {
        ...
        do { try pngData.write(to: imageFile) } catch { XCTFail("Failed to write image: \(error)"); return }

        // Create background image
        let bgImageFile = tempDirectory.appendingPathComponent("test-bg.png")
        do { try pngData.write(to: bgImageFile) } catch { XCTFail("Failed to write bg image: \(error)"); return }
    }
    ...
    private func createTestJS() {
        ...
        do { try js.write(to: jsFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    }
    ...
    private func setupWebView() {
        ...
        configuration.userContentController.add(loggingHandler, name: "DashTerm2ConsoleLog")
    ...
    func loadTestPage() async throws {
        guard let url = URL(string: "http://localhost:\(serverPort)/") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
    ...
    func savePageAndVerify() async throws -> (originalHTML: String, savedHTML: String, savedFolder: URL) {
        ...
        guard let baseURL = webView.url else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "WebView has no URL"])
        }
    ...
    func createAndLoadMinimalPage() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/minimal.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
    ...
    func createAndLoadTextOnlyPage() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/textonly.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
        try await loadPageWithRequest(request)
    ...
    func createAndLoadPageWithBrokenLinks() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/broken.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
        try await loadPageWithRequest(request)
    ...
    func createAndLoadComplexPage() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/complex.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
        try await loadPageWithRequest(request)
    ...
    func createAndLoadPageWithComplexCSS() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/complex-css.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
        try await loadPageWithRequest(request)
    ...
    func createAndLoadPageWithNestedCSS() async throws {
        ...
        guard let url = URL(string: "http://localhost:\(serverPort)/nested-css.html") else {
            throw NSError(domain: "TestError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create URL"])
        }
        let request = URLRequest(url: url)
        try await loadPageWithRequest(request)
    ...
    func createComplexTestPage() {
        ...
        do { try complexHTML.write(to: complexFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try iframeHTML.write(to: iframeFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try complexCSS.write(to: complexCSSFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    }
    ...
    func createPageWithComplexCSS() {
        ...
        do { try pageHTML.write(to: pageFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try complexCSS.write(to: cssFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    ...
    func createPageWithNestedCSS() {
        ...
        do { try mainCSS.write(to: mainCSSFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try secondaryCSS.write(to: secondaryCSSFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
        ...
        do { try tertiaryCSS.write(to: tertiaryCSSFile, atomically: true, encoding: .utf8) } catch { XCTFail("Failed to write file: \(error)"); return }
    ...
    private func createDummyImageFile(name: String) {
        ...
        do { try pngData.write(to: imageFile) } catch { XCTFail("Failed to write dummy image: \(error)"); return }
    }
...
private class iTermTestHTTPServer {
    ...
    func start() {
        ...
        do { try task.run() } catch { XCTFail("Failed to start HTTP server: \(error)"); return }
        server = task
