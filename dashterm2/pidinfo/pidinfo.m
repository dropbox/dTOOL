...
- (instancetype)init {
    self = [super init];
    if (self) {
        _queue = dispatch_queue_create("com.dashterm.dashterm2.pidinfo", DISPATCH_QUEUE_CONCURRENT);
    }
    return self;
}
...
- (void)reallyRunShellScript:(NSString *)script shell:(NSString *)shell completion:(void (^)(NSData * _Nullable, NSData * _Nullable, int))completion {
    @try {
        ...
        NSString *tempfile = [self temporaryFileNameWithPrefix:@"DashTerm2-script" suffix:@"sh"];
        NSError *error = nil;
