        ...
        for node in receiveNodes {
            ...
            do {
                ...
                guard let dict = jsResult as? [String: Any],
                      ...
                      let listenersInvoked = dict["listenersInvoked"] as? Int else {
                    logger.error("Failed to parse JavaScript result: \(jsResult ?? "nil")")
                    continue
                }
    ...
    func setStorageAreaAllowedInUntrustedContexts(allowed: Bool, in area: BrowserExtensionStorageArea, extensionId: ExtensionID) async {
        for node in network.nodes(for: extensionId.stringValue) {
            ...
            if !node.trusted {
                ...
                case .managed: fatalError("Managed storage area should not be enabled directly")
                }
...
extension Encodable {
    ...
    func toJSONFragment() throws -> String {
        ...
        guard let string = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "BrowserExtensionRouter", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to encode JSON as UTF-8"])
        }
        return string
    }
}
