...
public class BrowserExtensionContentScriptInjectionGenerator: BrowserExtensionContentScriptInjectionGeneratorProtocol {
    ...
    private func generateInjectionScriptSource(for extensionId: String, scripts: [[String: Any]]) -> String {
        guard let scriptsJSON = try? JSONSerialization.data(withJSONObject: scripts, options: .prettyPrinted),
              let scriptsJSONString = String(data: scriptsJSON, encoding: .utf8) else {
            logger.error("Failed to serialize scripts JSON for extension \(extensionId)",
