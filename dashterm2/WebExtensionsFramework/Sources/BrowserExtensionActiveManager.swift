...
class WebViewState {
    ...
    var contentWorldsWithHandlers: Set<ObjectIdentifier> = []

    /// Track whether we've already injected background script support for this webview
    var hasBackgroundScriptSupport: Bool = false
    /// Track whether we've explicitly executed the DOM nuke helper for this webview's page world
    var hasExecutedDomNuke: Bool = false
    var setAccessLevelToken: String

    /// Tab ID for Chrome extension context (nil for background scripts)
    var tabId: Int?
    /// Frame ID for Chrome extension context (0 = main frame, >0 = iframe)
    var frameId: Int?
...
fileprivate extension WKContentWorld {
    var be_handlerIdentifier: ObjectIdentifier {
        ObjectIdentifier(self)
    }

    var be_debugName: String {
        if let value = self.name, !value.isEmpty {
            return value
        }
        if self === WKContentWorld.page {
            return "page"
        }
        if self === WKContentWorld.defaultClient {
...
public protocol BrowserExtensionActiveManagerProtocol: AnyObject {
    ...
    ///   - tabId: Optional tab ID for Chrome extension messaging context
    ///   - frameId: Optional frame ID (0 = main frame, >0 = iframe)
    func registerWebView(_ webView: BrowserExtensionWKWebView,
                         userContentManager: BrowserExtensionUserContentManager,
                         role: WebViewRole,
                         tabId: Int?,
                         frameId: Int?) async throws
...
public class BrowserExtensionActiveManager: BrowserExtensionActiveManagerProtocol, BrowserExtensionRouterDataSource {
    ...
    private func removeScriptsForExtension(_ extensionId: ExtensionID) {
        ...
        let extensionWorldIdentifier = activeExtensions[extensionId]?.contentWorld.be_handlerIdentifier
        ...
        for webViewState in webViewStates.values {
            ...
            if let identifier = extensionWorldIdentifier {
                webViewState.contentWorldsWithHandlers.remove(identifier)
            }

            if case .backgroundScript(let bgExtensionId) = webViewState.role,
               bgExtensionId == extensionId {
                webViewState.contentWorldsWithHandlers.remove(WKContentWorld.page.be_handlerIdentifier)
                webViewState.hasBackgroundScriptSupport = false
                webViewState.hasExecutedDomNuke = false
            }
        }
    }
    ...
    public func registerWebView(_ webView: BrowserExtensionWKWebView,
                                ...
                                role: WebViewRole,
                                tabId: Int? = nil,
                                frameId: Int? = nil) async throws {
        await logger.inContext("Register webview \(webView)") {
            logger.debug("Registering webview \(webView) for injection script updates (tabId: \(String(describing: tabId)), frameId: \(String(describing: frameId)))")
            ...
            if let existing = webViewStates[id] {
                state = existing
                // Update tab/frame IDs if provided (they may change if webview is reused)
                if tabId != nil { state.tabId = tabId }
                if frameId != nil { state.frameId = frameId }
            } else {
            ...
            case .backgroundScript(let extensionId):
                guard let activeExt = activeExtension(for: extensionId) else {
                    logger.error("Active extension not found for extensionId: \(extensionId)")
                    return
                }
                network.add(webView: webView,
                            world: .page,
                            browserExtension: activeExt.browserExtension,
                            trusted: true,
                            role: role,
    ...
    private func reallyUpdateInjections(webView: BrowserExtensionWKWebView,
        ...
        case .backgroundScript(let extensionId):
            ...
            if !webViewState.hasBackgroundScriptSupport {
                ...
                guard let activeExt = activeExtensions[extensionId] else {
                    logger.error("Active extension not found for extensionId: \(extensionId)")
                    return
                }
                let browserExtension = activeExt.browserExtension
                assert(!webViewState.setAccessLevelToken.isEmpty)
                let shouldRunDomNuke = !webViewState.hasExecutedDomNuke
                await injectChromeAPIs(browserExtension,
                                       ...
                                       executeDomNuke: shouldRunDomNuke)
                if shouldRunDomNuke {
                    webViewState.hasExecutedDomNuke = true
                }
                ...
                webViewState.contentWorldsWithHandlers.insert(WKContentWorld.page.be_handlerIdentifier)
                if debug {
                    await installDebugInstrumentation(on: webView,
                                                       in: .page,
                                                       userContentManager: userContentManager,
                                                       label: "Background script page world",
                                                       immediate: false)
                }
                return
            }
        ...
        for (extensionId, activeExtension) in activeExtensions {
            ...
            let contentWorldKey = contentWorld.be_handlerIdentifier
            let contentWorldName = contentWorld.be_debugName
            if !webViewState.contentWorldsWithHandlers.contains(contentWorldKey) {
                ...
                webViewState.contentWorldsWithHandlers.insert(contentWorldKey)
            } else {
            ...
            if debug {
                await installDebugInstrumentation(on: webView,
                                                   ...
                                                   userContentManager: userContentManager,
                                                   label: "User-facing in content-script world",
                                                   immediate: false)
            }
        ...
        for script in debugScripts {
            ...
            if immediate {
                do {
                    ...
                    logger.error("Failed to evaluate debug script in \(label): \(error)",
    ...
    /// Consistently install console logging overrides and debug scripts for a specific world.
    private func installDebugInstrumentation(on webView: BrowserExtensionWKWebView,
    ...
    private func addMessageHandlersToWebView(_ webView: BrowserExtensionWKWebView,
        ...
        logger.debug("Registering message handlers for content world: \(contentWorld.be_debugName)")
        ...
        webView.be_configuration.be_userContentController.be_add(
            BrowserExtensionAPIRequestMessageHandler(
                ...
                contextProvider: { [weak self, weak webView] in
                    ...
                    // Look up stored tab/frame IDs from webview state
                    let webViewId = ObjectIdentifier(webView)
                    let state = self.webViewStates[webViewId]
                    let tab: BrowserExtensionContext.MessageSender.Tab?
                    if let tabId = state?.tabId {
                        tab = BrowserExtensionContext.MessageSender.Tab(id: tabId, url: nil)
                    } else {
                        tab = nil
                    }
                    return self.context(for: webView, browserExtension: browserExtension, tab: tab, frameId: state?.frameId)
                }
            ),
    ...
    private func injectChromeAPIs(_ browserExtension: BrowserExtension,
                                  ...
                                  setAccessLevelToken: String,
                                  executeDomNuke: Bool = false) async {
        ...
        logger.debug("Injecting Chrome APIs for extension \(browserExtension.id) into webview \(webView) in content world: \(contentWorld.be_debugName)")
        
        if isBackgroundScript {
            // For background scripts, ensure DOM globals are nuked exactly once before injecting APIs
            if executeDomNuke {
            ...
            contentManager.add(userScript: .init(
                ...
                worlds: [contentWorld],
                identifier: UserScripts.chromeAPIs.identifier))
            logger.debug("Added Chrome API user script to webview \(webView) in content world: \(contentWorld.be_debugName)")
        }
    }
    ...
    public func contentWorld(for extensionId: String) async -> WKContentWorld? {
        ...
        let contentWorld = activeExtension.contentWorld
        logger.debug("ActiveManager returning content world: \(contentWorld.be_debugName)")
        return contentWorld
    }
    ...
    private func executeDOMNukeInContentWorld(_ webView: BrowserExtensionWKWebView, contentWorld: WKContentWorld) async {
        logger.debug("About to execute DOM nuke script in content world: \(contentWorld.be_debugName)")
