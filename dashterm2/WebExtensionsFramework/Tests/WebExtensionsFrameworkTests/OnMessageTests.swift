...
class OnMessageTests: XCTestCase {
    ...
    func testOnMessageAddListener() async throws {
        ...
        guard let activeExtension = activeManager.activeExtension(for: mockBrowserExtension.id) else {
            XCTFail("Active extension not found")
            return
        }
        let result = try await receiverWebView.callAsyncJavaScript("""
            ...
            return { success: true };
        """, contentWorld: activeExtension.contentWorld) as? [String: Any]
    ...
    func testSimpleMessageExchange() async throws {
        ...
        let extensionContentWorld = activeExtension.contentWorld
