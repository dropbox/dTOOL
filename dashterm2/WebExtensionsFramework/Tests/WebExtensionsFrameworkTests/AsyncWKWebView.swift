...
class TestIframeSchemeHandler: NSObject, WKURLSchemeHandler {
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        guard let data = html.data(using: .utf8),
              let taskUrl = urlSchemeTask.request.url else {
            urlSchemeTask.didFailWithError(NSError(domain: "TestIframeSchemeHandler", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid request or encoding"]))
            return
        }
        let response = URLResponse(url: taskUrl, mimeType: "text/html", expectedContentLength: data.count, textEncodingName: "utf-8")
