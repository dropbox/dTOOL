...
final class RedBoxExtensionE2ETests: XCTestCase {
    ...
    func testRedBoxExtensionE2E() async throws {
        ...
        guard let browserExtension = extensions.first else {
            XCTFail("Failed to load test browser extension")
            return
        }
        ...
        guard let resultDict = result as? [String: Any] else {
            XCTFail("Expected dictionary result from red box verification script")
            return
        }
        
        // 10. Assert the red box exists and has correct properties
        guard let exists = resultDict["exists"] as? Bool else {
            XCTFail("Missing exists flag in red box result")
            return
        }
        XCTAssertTrue(exists, "Red box should exist in DOM")
        ...
        guard let consoleScriptRan = logResult as? Bool else {
            XCTFail("Expected boolean result verifying console script execution")
            return
        }
        XCTAssertTrue(consoleScriptRan, "Extension script should have executed")
    }
    ...
    func testRedBoxExtensionURLMatching() async throws {
        ...
        guard let hasRedBox = try await webView.evaluateJavaScript(checkScript) as? Bool else {
            XCTFail("Expected boolean result for red box existence check")
            return
        }
    ...
    func testMultipleExtensionsIsolation() async throws {
        ...
        guard let result = try await webView.evaluateJavaScript(isolationCheckScript) as? [String: Any] else {
            XCTFail("Expected dictionary result for isolation check")
            return
        }
        guard let redBoxExists = result["redBoxExists"] as? Bool,
              let pageUnaffected = result["pageUnaffected"] as? Bool else {
            XCTFail("Isolation check returned unexpected payload")
            return
        }
        XCTAssertTrue(redBoxExists, "Red box should exist")
        XCTAssertTrue(pageUnaffected, "Page content should be unaffected")
    }
}
