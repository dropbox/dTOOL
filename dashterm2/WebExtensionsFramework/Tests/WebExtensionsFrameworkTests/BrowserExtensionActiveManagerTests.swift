...
final class BrowserExtensionActiveManagerTests: XCTestCase {
    ...
    func testActivationTimestamp() async throws {
        ...
        let activatedAtValue = try XCTUnwrap(activatedAt, "activatedAt should not be nil")
        XCTAssertGreaterThanOrEqual(activatedAtValue, beforeActivation)
        XCTAssertLessThanOrEqual(activatedAtValue, afterActivation)
    }
    ...
    /// Background script handler registration should re-run after deactivation
    func testBackgroundScriptHandlersReRegisterAfterDeactivate() async throws {
        await manager.activate(testExtension)

        let mockWebView = MockUserContentWebView()
        ...
        let initialHandlerCount = mockWebView.mockUserContentController.addScriptMessageHandlerCalls.count
        XCTAssertGreaterThan(initialHandlerCount, 0)

        await manager.deactivate(testExtension.id)
        await manager.activate(testExtension)
        ...
        let updatedHandlerCount = mockWebView.mockUserContentController.addScriptMessageHandlerCalls.count
        XCTAssertGreaterThan(updatedHandlerCount, initialHandlerCount)
    }
    ...
    func testChromeRuntimeId() async throws {
        ...
        let activeExtension = try XCTUnwrap(manager.activeExtension(for: testExtension.id), "Active extension should exist")
    ...
    func testChromeRuntimeIdMultipleExtensions() async throws {
        ...
        // Get content worlds
        let activeExtension1 = try XCTUnwrap(manager.activeExtension(for: extension1.id), "Active extension 1 should exist")
        let activeExtension2 = try XCTUnwrap(manager.activeExtension(for: extension2.id), "Active extension 2 should exist")
