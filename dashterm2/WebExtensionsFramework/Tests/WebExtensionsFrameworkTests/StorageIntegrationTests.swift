...
final class StorageIntegrationTests: XCTestCase {
    ...
    func testStorageAPIIntegration() async throws {
        ...
        guard let browserExtension = registry.extensions.first else {
            XCTFail("Failed to load storage integration test extension")
            return
        }
        ...
        for _ in 0..<50 { // Wait up to 5 seconds
            let storedData = try await storageProvider.get(keys: nil,
                                                          area: .local,
        ...
        let finalStoredData = try await storageProvider.get(keys: nil,
                                                           area: .local,
                                                           extensionId: browserExtension.id)
