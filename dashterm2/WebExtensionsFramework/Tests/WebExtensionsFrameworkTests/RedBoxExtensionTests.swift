...
final class RedBoxExtensionTests: XCTestCase {
    ...
    func testRedBoxExtensionManifestParsing() throws {
        let json = """
        {
          "manifest_version": 3,
          "name": "Red Box",
        ...
        let data = try XCTUnwrap(json.data(using: .utf8), "Failed to encode JSON to UTF-8 data")
        ...
        let contentScript = try XCTUnwrap(manifest.contentScripts?.first, "Content script should exist")
    ...
    func testActualRedBoxManifestFile() throws {
        ...
        let data = try XCTUnwrap(actualManifestJSON.data(using: .utf8), "Failed to encode actual manifest JSON to UTF-8 data")
        let manifest = try JSONDecoder().decode(ExtensionManifest.self, from: data)
