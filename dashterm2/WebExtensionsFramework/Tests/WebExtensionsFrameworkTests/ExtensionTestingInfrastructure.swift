    ...
    struct TestExtension {
        ...
        init(id: ExtensionID = ExtensionID(), 
            ...
            if !backgroundScripts.isEmpty,
               let firstScript = Array(backgroundScripts.keys).first {
                manifest["background"] = [
    ...
    class TestRunner {
        ...
        func unblockBackgroundScript(extensionId: ExtensionID, blockName: String) async {
            ...
            guard let webView = activeManager.backgroundScriptWebView(in: extensionId) else {
                XCTFail("No background script webview found")
                return
            }
            do {
                _ = try await webView.be_evaluateJavaScript(iife, in: nil, in: .page)
            } catch {
                XCTFail("Failed to unblock background script: \(error)")
            }
        }
        ...
        func unblockContentScript(_ id: ExtensionID, webView: BrowserExtensionWKWebView, name: String) async {
            ...
            true;
            """
            guard let contentWorld = await activeManager.contentWorld(for: id.stringValue) else {
                XCTFail("No content world found for extension \(id.stringValue)")
                return
            }
            do {
                ...
                XCTFail("Failed to unblock content script: \(error)")
            }
        }
        ...
        func waitForContentScriptCompletion(_ id: ExtensionID, webView: BrowserExtensionWKWebView, name: String="Complete") async {
            ...
            do {
                _ = try await webView.be_callAsyncJavaScript(js, arguments: [:], in: nil, in: contentWorld)
            } catch {
                XCTFail("Failed to wait for content script completion: \(error)")
            }
        }
        ...
        func run(_ testExtension: TestExtension) async throws -> AsyncWKWebView {
            ...
            guard let baseURL = URL(string: "chrome-extension://\(extensionId.stringValue)/") else {
                XCTFail("Unable to create base URL for extension \(extensionId)")
                throw TestError.invalidBaseURL(extensionId.stringValue)
            }
            ...
            do {
                ...
                XCTFail("Failed to load extension scripts: \(error)")
                throw error
            }
        ...
        func executeJavaScript(_ script: String,
            ...
            case .backgroundScript:
                guard let webView = activeManager.backgroundScriptWebView(in: extensionId) else {
                    throw TestError.scriptExecutionFailed("No background script webview found")
                }
                return try await webView.be_evaluateJavaScript(script, in: nil, in: .page)
            case .contentScript:
                guard let webView = contentWebView else {
                    throw TestError.scriptExecutionFailed("No content webview provided")
                }
                guard let contentWorld = await activeManager.contentWorld(for: extensionId.stringValue) else {
                    throw TestError.scriptExecutionFailed("No content world found")
                }
                return try await webView.be_evaluateJavaScript(
                    script,
                    in: nil,
                    in: contentWorld)
            case .untrustedWebPage:
        ...
        func verifyAssertions(file: StaticString = #file, line: UInt = #line) {
            ...
            for assertion in assertions {
                ...
                if let actualValue = assertion.actualValue {
                    actualDescription = " (actual: \(actualValue))"
                } else {
                    actualDescription = " (actual: nil)"
                }

                let expectedDescription: String
                if let expectedValue = assertion.expectedValue {
                    expectedDescription = " (expected: \(expectedValue))"
                } else {
                    expectedDescription = " (expected: nil)"
                }

                let message = "JavaScript assertion failed: \(assertion.description)." +
                actualDescription + expectedDescription
                XCTAssertTrue(assertion.passed, message, file: file, line: line)
            }
    ...
    enum TestError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .invalidBaseURL(let baseURLString):
                return "Invalid extension base URL: \(baseURLString)"
            }
        }
    }
}
