...
extension String {
    var jsonEncoded: String {
        guard let data = try? JSONEncoder().encode(self),
            ...
            // Fallback: escape the string manually for JSON
            let escaped = self
...
internal struct AsyncFunction<T>: StringConvertible, JavascriptNamedFunction, JavascriptDependentFunction {
    ...
    private func capitalizedName(context: String?) -> String {
        // BUG-1584: Use guard to safely access .first instead of force unwrap
        guard let firstChar = name.first else {
            return ""
        }
        let baseName = firstChar.uppercased() + String(name.dropFirst())
    ...
    func swiftDispatch(indent: Int, context: String?) -> String? {
        ...
        if T.self is BrowserExtensionJSONRepresentable.Type {
            output += [
                ...
                s + "    // BUG-1055: Use optional cast instead of force cast",
                s + "    guard let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {",
                s + "        throw BrowserExtensionError.internalError(\"Failed to serialize response as dictionary\")",
                s + "    }",
...
internal struct VariableArgsFunction: StringConvertible, JavascriptNamedFunction {
    ...
    func swiftDispatch(indent: Int, context: String?) -> String? {
        ...
        case "\(apiName(context: context))":
            ...
            // BUG-1055: Use optional cast instead of force cast
