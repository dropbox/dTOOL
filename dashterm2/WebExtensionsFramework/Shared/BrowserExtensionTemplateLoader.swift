...
public class BrowserExtensionTemplateLoader {
    // Pre-compiled regex for include patterns - this pattern is validated at compile time
    private static let includeRegex: NSRegularExpression? = {
        try? NSRegularExpression(pattern: "\\{\\{INCLUDE:([^}]+)\\}\\}")
    }()
    ...
    private static func performSubstitutions(template: String, substitutions: [String: String], resourceBundle: Bundle? = nil) -> String {
        ...
        guard let regex = includeRegex else {
            // Regex compilation failed - skip include processing, proceed with substitutions
