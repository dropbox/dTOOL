...
- (BOOL)load:(out NSError **)errorOut {
    ...
    @try {
        ...
        while (offset < length) {
            ...
            SIGArchiveChunk *chunk = [SIGArchiveChunk chunkFromFileHandle:fileHandle
                                                                 atOffset:offset
                                                                    error:&chunkError];
            if (!chunk) {
                primaryError = chunkError ?: [SIGError errorWithCode:SIGErrorCodeInputFileMalformed];
                break;
            }
        ...
        if (!primaryError) {
            if (offset > length) {
                primaryError = [SIGError errorWithCode:SIGErrorCodeInputFileMalformed];
            } else {
                _chunks = [chunks copy];
                success = YES;
            }
        ...
        if (@available(macOS 10.15, *)) {
            ...
            if (![fileHandle closeAndReturnError:&closeError]) {
                if (!primaryError) {
                    primaryError = [SIGError errorWrapping:closeError
                                                       code:SIGErrorCodeIORead
                                                     detail:@"Failed to close input file"];
                }
                success = NO;
            }
        } else {
    ...
    if (!success && errorOut && primaryError) {
        *errorOut = primaryError;
    }
    return success;
}
