...
- (void)dealloc {
    ...
    // BUG-2813: Invalidate timers before releasing to prevent callbacks after dealloc
...
- (void)startDraggingCell:(PSMTabBarCell *)cell
    ...
    [pbItem setString:[@([[control cells] indexOfObject:cell]) stringValue]
              forType:@"com.dashterm.dashterm2.psm.controlitem"];
    ...
    if (_fadeTimer) {
        ...
        if (!_dragViewWindow) {
            ...
            if ([control delegate] &&
                ...
                // BUG-7229 fix: Use cacheDisplayInRect: instead of lockFocus/drawRect:
                // to avoid race conditions with the display system.
                NSView *tabView = [[[self draggedCell] representedObject] view];
                NSRect viewBounds = [tabView bounds];
                NSBitmapImageRep *bitmapRep = [tabView bitmapImageRepForCachingDisplayInRect:viewBounds];
                if (bitmapRep) {
                    [tabView cacheDisplayInRect:viewBounds toBitmapImageRep:bitmapRep];
                    viewImage = [[[NSImage alloc] initWithSize:viewBounds.size] autorelease];
                    [viewImage addRepresentation:bitmapRep];
                } else {
                    // Fallback: create empty image if bitmap rep creation fails
                    viewImage = [[[NSImage alloc] initWithSize:viewBounds.size] autorelease];
                }
            }
            ...
            if (overCell) {
                ...
                if ([overCell isPlaceholder]) {
                    ...
                    if (mouseLoc.x < (overCellRect.origin.x + (overCellRect.size.width / 2.0))) {
                        ...
                        // BUG-9766: Check bounds before accessing array
                        if (overCellIndex > 0) {
                        ...
                        // BUG-9766: Check bounds before accessing array
                        if (overCellIndex + 1 < [cells count]) {
                            [self setTargetCell:[cells objectAtIndex:(overCellIndex + 1)]];
                        } else {
                            [self setTargetCell:overCell];
                        }
...
- (void)distributePlaceholdersInTabBar:(PSMTabBarControl *)control
    ...
    // BUG-9767: Verify cell was found and bounds are valid before array operations
    if (cellIndex == NSNotFound) {
        ILog(@"distributePlaceholdersInTabBar:withDraggedCell: cell not found");
        return;
    }
    NSMutableArray *cells = [control cells];
    NSUInteger cellCount = [cells count];
    // Need at least 3 cells (one before, the cell, one after) for original logic
    if (cellCount < 3 || cellIndex == 0 || cellIndex + 1 >= cellCount) {
        ILog(@"distributePlaceholdersInTabBar:withDraggedCell: invalid bounds cellIndex=%lu count=%lu", (unsigned long)cellIndex, (unsigned long)cellCount);
        return;
    }
...
- (void)distributePlaceholdersInTabBar:(PSMTabBarControl *)control {
    ...
    if (draggedCell) {
        ...
        // BUG-9768: Check if cells array is non-empty before accessing index 0
        NSArray *cells = [control cells];
        if ([cells count] == 0) {
            ILog(@"distributePlaceholdersInTabBar: no cells to distribute");
            return;
        }
        draggedCellFrame = [[cells objectAtIndex:0] frame];
        truncationStyle = [[cells objectAtIndex:0] truncationStyle];
    }
