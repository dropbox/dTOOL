...
- (NSImage *)dragImage {
    ...
    if (!self.indicator.isHidden) {
        // BUG-11178: Render the live progress indicator instead of referencing a missing asset
        PSMProgressIndicator *indicator = self.indicator;
        const NSRect indicatorBounds = indicator.bounds;
        if (NSWidth(indicatorBounds) > 0 && NSHeight(indicatorBounds) > 0) {
            NSBitmapImageRep *indicatorRep =
                [indicator bitmapImageRepForCachingDisplayInRect:indicatorBounds];
            if (indicatorRep) {
                [indicator cacheDisplayInRect:indicatorBounds toBitmapImageRep:indicatorRep];
                NSImage *indicatorImage =
                    [[NSImage alloc] initWithSize:indicatorBounds.size];
                [indicatorImage addRepresentation:indicatorRep];

                [returnImage lockFocus];
                [indicatorImage drawAtPoint:indicator.frame.origin
                                   fromRect:NSZeroRect
                                  operation:NSCompositingOperationSourceOver
                                   fraction:1.0];
                [returnImage unlockFocus];
...
#pragma mark - DashTerm2 Additions
