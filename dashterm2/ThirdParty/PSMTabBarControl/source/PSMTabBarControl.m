...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        [self registerForDraggedTypes:[NSArray arrayWithObjects:@"com.dashterm.dashterm2.psm.controlitem", nil]];
        ...
        // modifier for changing tabs changed (DashTerm2 addon)
...
- (void)dealloc {
    ...
    // Invalidate animation timer to prevent retain cycle
    if (_animationTimer) {
...
- (void)setDelegate:(id<PSMTabBarControlDelegate>)object {
    ...
    NSMutableArray *types = [NSMutableArray arrayWithObject:@"com.dashterm.dashterm2.psm.controlitem"];
...
- (void)otherMouseUp:(NSEvent *)theEvent
{
    ...
    if ([theEvent buttonNumber] == 2 && !_resizing) {
        // BUG-1022: Guard against nil lastMiddleMouseDownEvent
        NSEvent *middleMouseDownEvent = [self lastMiddleMouseDownEvent];
        if (!middleMouseDownEvent) {
            return;
        }
        NSPoint mousePt = [self convertPoint:[theEvent locationInWindow] fromView:nil];
        NSRect cellFrame;
        ...
        PSMTabBarCell *mouseDownCell = [self cellForPoint:[self convertPoint:[middleMouseDownEvent locationInWindow] fromView:nil]
...
- (BOOL)prepareForDragOperation:(id <NSDraggingInfo>)sender {
    ...
    BOOL goodType = [[[sender draggingPasteboard] types] indexOfObject:@"com.dashterm.dashterm2.psm.controlitem"] != NSNotFound;
    if (!goodType && [[self delegate] respondsToSelector:@selector(tabView:shouldAcceptDragFromSender:)] &&
        [[self delegate] tabView:_tabView shouldAcceptDragFromSender:sender]) {
        goodType = YES;
    }
    return goodType ||
...
- (void)changeIdentifier:(id)newIdentifier atIndex:(int)theIndex {
    ...
    if (!tabViewItem) {
        // BUG-f640: Replace assert(tabViewItem) with early return - invalid index should not crash
        DLog(@"BUG-f640: changeIdentifier called with invalid index %d", theIndex);
        return;
    }
    for (PSMTabBarCell *cell in _cells) {
        if ([cell representedObject] == tabViewItem) {
    ...
    // BUG-f641: Replace assert(false) with DLog - cell not found should not crash
    DLog(@"BUG-f641: changeIdentifier could not find cell for tabViewItem at index %d", theIndex);
}
...
- (void)bindPropertiesForCell:(PSMTabBarCell *)cell andTabViewItem:(NSTabViewItem *)item {
    ...
    if (delegate) {
        [delegate tabView:_tabView updateStateForTabViewItem:item];
    }
}
...
- (PSMTabBarCell *)lastVisibleTab
{
    ...
    for(i = 0; i < cellCount; i++){
        if ([[_cells objectAtIndex:i] isInOverflowMenu]) {
            // If the first tab is in overflow, there's no visible tab
            return (i > 0) ? [_cells objectAtIndex:(i-1)] : nil;
        }
    }
