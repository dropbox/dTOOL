...
+ (NSString *)_punycodeEncode:(NSString *)aString;
{
    ...
    // BUG-1160: Pre-validate string length to avoid potential overflow during encoding.
    // Punycode encoding can expand strings significantly. A conservative check ensures
    // we don't overflow the output buffer. Each non-ASCII character can become up to
    // ~5 ASCII characters in the worst case, plus the "xn--" prefix (4 chars).
    // Use MAX_HOSTNAME_LEN/6 as a safe limit for the input to guarantee output fits.
    if (stringLength == 0 || stringLength > (MAX_HOSTNAME_LEN / 6))
        return aString;
...
+ (NSString *)_punycodeDecode:(NSString *)aString;
{
    ...
    /* now use the decoded integers to insert characters into the decoded string */
    {
        ...
        for (deltaIndex = 0; deltaIndex < deltaCount; deltaIndex ++) {
            ...
            // BUG-1158: Handle supplementary plane characters (code points > 0xFFFF)
            // by encoding them as UTF-16 surrogate pairs.
            NSString *insertion;
            if (codeValue <= 0xFFFF) {
                unichar ch = (unichar)codeValue;
                insertion = [[NSString alloc] initWithCharacters:&ch length:1];
            } else {
                // Supplementary plane character - encode as surrogate pair
                uint32_t cp = codeValue - 0x10000;
                unichar surrogatePair[2];
                surrogatePair[0] = (unichar)(0xD800 + (cp >> 10));      // High surrogate
                surrogatePair[1] = (unichar)(0xDC00 + (cp & 0x3FF));    // Low surrogate
                insertion = [[NSString alloc] initWithCharacters:surrogatePair length:2];
                // Adjust decodedLabelLength for the extra character in the surrogate pair
                decodedLabelLength++;
            }
...
// BUG-1174: Properly extract and encode/decode only the hostname component.
// Use NSURLComponents to correctly parse the URL and avoid encoding path/query/fragment.
+ (NSString *)IDNEncodedURL:(NSString *)aURL {
    ...
    if (urlComponents && urlComponents.host) {
        // Use proper URL parsing - only encode the hostname
        NSString *encodedHost = [NSURL IDNEncodedHostname:urlComponents.host];
        if (encodedHost && ![encodedHost isEqualToString:urlComponents.host]) {
            urlComponents.host = encodedHost;
            return urlComponents.string ?: aURL;
        }
        return aURL;
    }

    // Fallback for non-standard URLs: try simple splitting but be more careful
    NSMutableArray *schemeComponents = [[aURL componentsSeparatedByString:@"://"] mutableCopy];
    if ([schemeComponents count] > 1) {
        NSString *afterScheme = schemeComponents[1];
        // Find the end of the hostname (first /, ?, #, or @)
        NSRange endOfHost = [afterScheme rangeOfCharacterFromSet:
            [NSCharacterSet characterSetWithCharactersInString:@"/?#"]];
        NSString *hostPart;
        NSString *restPart = @"";

        if (endOfHost.location != NSNotFound) {
            hostPart = [afterScheme substringToIndex:endOfHost.location];
            restPart = [afterScheme substringFromIndex:endOfHost.location];
        } else {
            hostPart = afterScheme;
        }

        // Check for userinfo@ prefix
        NSRange atRange = [hostPart rangeOfString:@"@"];
        NSString *userinfo = @"";
        if (atRange.location != NSNotFound) {
            userinfo = [hostPart substringToIndex:atRange.location + 1];
            hostPart = [hostPart substringFromIndex:atRange.location + 1];
        }

        // Check for :port suffix
        NSRange portRange = [hostPart rangeOfString:@":" options:NSBackwardsSearch];
        NSString *port = @"";
        if (portRange.location != NSNotFound) {
            port = [hostPart substringFromIndex:portRange.location];
            hostPart = [hostPart substringToIndex:portRange.location];
        }

        NSString *encodedHost = [NSURL IDNEncodedHostname:hostPart];
        schemeComponents[1] = [NSString stringWithFormat:@"%@%@%@%@", userinfo, encodedHost, port, restPart];
        return [schemeComponents componentsJoinedByString:@"://"];
    }
    return aURL;
}

+ (NSString *)IDNDecodedURL:(NSString *)anIDNURL {
    if (!anIDNURL || anIDNURL.length == 0) {
        return anIDNURL;
    }

    NSURLComponents *urlComponents = [NSURLComponents componentsWithString:anIDNURL];
    if (urlComponents && urlComponents.host) {
        // Use proper URL parsing - only decode the hostname
        NSString *decodedHost = [NSURL IDNDecodedHostname:urlComponents.host];
        if (decodedHost && ![decodedHost isEqualToString:urlComponents.host]) {
            urlComponents.host = decodedHost;
            return urlComponents.string ?: anIDNURL;
        }
        return anIDNURL;
    }

    // Fallback for non-standard URLs
    NSMutableArray *schemeComponents = [[anIDNURL componentsSeparatedByString:@"://"] mutableCopy];
    if ([schemeComponents count] > 1) {
        NSString *afterScheme = schemeComponents[1];
        ...
        NSString *decodedHost = [NSURL IDNDecodedHostname:hostPart];
        schemeComponents[1] = [NSString stringWithFormat:@"%@%@%@%@", userinfo, decodedHost, port, restPart];
        return [schemeComponents componentsJoinedByString:@"://"];
    }
    return anIDNURL;
}
