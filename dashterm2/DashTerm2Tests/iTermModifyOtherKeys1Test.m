...
- (void)test_BUG_1948_controlOptionBypassesPreCocoa {
    iTermModifyOtherKeysMapper *mapper = [[iTermModifyOtherKeysMapper alloc] init];
    mapper.delegate = self;
    const unichar controlCode = 0x1d;
    NSString *characters = [NSString stringWithCharacters:&controlCode length:1];
    NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                                      location:NSZeroPoint
    ...
    XCTAssertTrue([mapper keyMapperShouldBypassPreCocoaForEvent:event],
                  @"BUG-1948: Control+option control codes should bypass pre-cocoa handling");
}
...
#pragma mark - RC-010: Empty Characters Handling

// RC-010/BUG-3979: Dead keys and modifier-only events produce empty characters.
// This is expected macOS behavior. The mapper handles empty characters gracefully
// without crashing - that is the requirement being tested.

- (void)test_RC_010_emptyCharactersDoesNotCrash {
    // Dead keys (e.g., ` for accented characters) produce empty strings.
    // Test verifies the mapper handles this without crashing.
    NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                                      location:NSZeroPoint
                                 modifierFlags:0
                                     timestamp:[NSDate timeIntervalSinceReferenceDate]
    ...
    // Call both pre-cocoa and post-cocoa handlers - neither should crash
    NSString *preCocoa = [_mapper keyMapperStringForPreCocoaEvent:event];
    NSData *postCocoa = [_mapper keyMapperDataForPostCocoaEvent:event];

    // We don't care about the specific return values, only that it didn't crash
    XCTAssertTrue(YES, @"RC-010: Empty characters should not crash");
}

- (void)test_RC_010_emptyCharactersWithModifiersDoesNotCrash {
    // Test empty characters with various modifiers - none should crash
    NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                                      location:NSZeroPoint
                                 modifierFlags:C_ | O_
                                     timestamp:[NSDate timeIntervalSinceReferenceDate]
    ...
    XCTAssertTrue(YES, @"RC-010: Empty characters with modifiers should not crash");
}

// RC-010: Verify codePointForEvent handles empty characters in control+option path
- (void)test_RC_010_codePointForEventEmptyCharacters {
    iTermModifyOtherKeysMapper *mapper = [[iTermModifyOtherKeysMapper alloc] init];
    mapper.delegate = self;

    // Create event with empty characters (simulates dead key in control+option context)
    NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                                      location:NSZeroPoint
                                 modifierFlags:(NSEventModifierFlagControl | NSEventModifierFlagOption)
                                     timestamp:[NSDate timeIntervalSinceReferenceDate]
    ...
    // Should not crash - codePointForEvent returns 0 for empty characters
    NSString *result = [mapper keyMapperStringForPreCocoaEvent:event];
    // Result may be nil or a valid string, but should NOT crash
    XCTAssertTrue(YES, @"RC-010: Empty characters should not crash");
}
