...
- (void)testSignature {
    // Use expressionParser (not callParser) because callParser returns a "call sequence"
    // which is always iTermParsedExpressionTypeFunctionCalls (plural), even for single calls.
    // expressionParser returns iTermParsedExpressionTypeFunctionCall (singular) for a single call.
...
- (void)testEvaluateExpressionFunction {
    ...
    evaluator = [[iTermExpressionEvaluator alloc] initWithExpressionString:@"add(x: 1, y: 2)"
                                                                     scope:_scope];
    [evaluator evaluateWithTimeout:0 sideEffectsAllowed:YES completion:^(iTermExpressionEvaluator * _Nonnull evaluator) {
...
- (void)testEvaluateExpressionFunctionComposition {
    ...
    [[[iTermExpressionEvaluator alloc] initWithExpressionString:@"add(x: 1, y: mult(x: 2, y: 3))"
                                                          scope:_scope] evaluateWithTimeout:0 sideEffectsAllowed:YES completion:^(iTermExpressionEvaluator * _Nonnull evaluator) {
        output = evaluator.value;
        XCTAssertNil(evaluator.error);
        XCTAssertEqual(0, evaluator.missingValues.count);
    }];
...
- (void)testEvaluateExpressionUndefinedFunction {
    ...
    [[[iTermExpressionEvaluator alloc] initWithExpressionString:@"add(x: 1, y: bogus(x: 2, y: 3))"
                                                          scope:_scope] evaluateWithTimeout:INFINITY sideEffectsAllowed:YES
...
- (void)testEvaluateExpressionStringVariable {
    ...
    [[[iTermExpressionEvaluator alloc] initWithExpressionString:@"foo"
                                                          scope:_scope] evaluateWithTimeout:0 sideEffectsAllowed:YES
     completion:^(iTermExpressionEvaluator * _Nonnull evaluator) {
         output = evaluator.value;
         XCTAssertNil(evaluator.error);
         XCTAssertEqual(0, evaluator.missingValues.count);
...
- (void)DISABLED_testEvaluateExpressionNestedInterpolatedStringWithUndefinedFunctionCall {
    @autoreleasepool {
        ...
        @"    ) end-outer\"";
        [[[iTermExpressionEvaluator alloc] initWithExpressionString:expression
                                                              scope:_scope] evaluateWithTimeout:INFINITY sideEffectsAllowed:YES completion:^(iTermExpressionEvaluator * _Nonnull evaluator) {
            XCTAssertNil(evaluator.value);
            XCTAssertNotNil(evaluator.error);
...
- (void)testArrayCount {
    __block id result;
    // Use dashterm2 namespace - the function is registered under "dashterm2" not "iterm2"
    [iTermScriptFunctionCall callFunction:@"dashterm2.count(array: a())"
                                  timeout:0
                       sideEffectsAllowed:YES
                                    scope:_scope
                               retainSelf:YES
