...
- (NSString *)compactLineDumpForRectScrolledDownBy:(int)downBy
    ...
    [grid scrollRect:scrollRect downBy:downBy softBreak:NO fillChar:(screen_char_t){0}];
...
- (void)testScrollRectDownBy {
    ...
    VT100Grid *grid = [self gridFromCompactLinesWithContinuationMarks:
                       ...
                       @"mnop+\n"
                       @"qrst!"];
    [grid scrollRect:VT100GridRectMake(0, 1, 4, 3) downBy:1 softBreak:NO fillChar:(screen_char_t){0}];
    ...
    grid = [self gridFromCompactLinesWithContinuationMarks:
            ...
            @"mnop+\n"
            @"qrst!"];
    [grid scrollRect:VT100GridRectMake(0, 1, 4, 3) downBy:2 softBreak:NO fillChar:(screen_char_t){0}];
    XCTAssert([[grid compactLineDumpWithContinuationMarks] isEqualToString:
            @"abcd!\n"
            @"....!\n"
            @"....!\n"
    ...
    grid = [self gridFromCompactLinesWithContinuationMarks:
            ...
            @"mnop+\n"
            @"qrst!"];
    [grid scrollRect:VT100GridRectMake(1, 1, 3, 3) downBy:1 softBreak:NO fillChar:(screen_char_t){0}];
    XCTAssert([[grid compactLineDumpWithContinuationMarks] isEqualToString:
            @"abcd+\n"
            @"e...!\n"
            @"ifgh+\n"
    ...
    grid = [self gridFromCompactLinesWithContinuationMarks:
            ...
            @"mnop+\n"
            @"qrst!"];
    [grid scrollRect:VT100GridRectMake(1, 1, 3, 3) downBy:2 softBreak:NO fillChar:(screen_char_t){0}];
    XCTAssert([[grid compactLineDumpWithContinuationMarks] isEqualToString:
            @"abcd+\n"
            @"e...!\n"
            @"i...!\n"
...
- (void)doTestDefaultLine {
    ...
    // Default lines are zero-initialized (all attributes are 0)
    screen_char_t zeroChar = { 0 };
    for (int i = 0; i < w; i++) {
        XCTAssert(ForegroundAttributesEqual(line[i], zeroChar));
        XCTAssert(BackgroundColorsEqual(line[i], zeroChar));
