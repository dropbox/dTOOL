...
- (NSTextField *)findEditableTextFieldInView:(NSView *)view {
    for (NSView *subview in view.subviews) {
        if ([subview isKindOfClass:[NSTextField class]] &&
            [(NSTextField *)subview isEditable]) {
            return (NSTextField *)subview;
        }
        NSTextField *found = [self findEditableTextFieldInView:subview];
        if (found) {
            return found;
        }
    }
...
- (void)testEmbeddedTabsConvertToSpaces {
    ...
    _warningBlock = ^NSModalResponse(NSAlert *alert, NSString *identifier) {
        ...
        // Search recursively for the editable text field
        NSTextField *textField = [self findEditableTextFieldInView:alert.accessoryView];
        XCTAssertNotNil(textField, @"Should find editable text field in accessory view");
        if (textField) {
