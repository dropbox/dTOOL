...
- (void)testASCIIWordSelection {
    ...
    NSArray *words = @[
        @"word",    @"word",    @"word",    @"word",    @" ",       @"123",     @"123", @"123", @"   ", @"   ",
        @"   ",     @"abc",     @"abc",     @"abc",     @"/",       @"def",     @"def", @"def", @" ",   @"afl-cio",
        @"afl-cio", @"afl-cio", @"afl-cio", @"afl-cio", @"afl-cio", @"afl-cio", @" ",   @"!",   @"@",   @"-"
    ];
    [self performTestForWordSelectionUsingLine:line wordForEachCharacter:words extraWordCharacters:@"-"];
}
    ...
    [iTermSelectorSwizzler swizzleSelector:@selector(standardUserDefaults)
        ...
        forBlock:^{
            ...
            for (int i = 0; i < line.length; i++) {
                range = [extractor rangeForWordAt:VT100GridCoordMake(i, 0) maximumLength:kReasonableMaximumWordLength];
                NSString *actual = [self stringForRange:range];
                XCTAssertEqualObjects(actual, expected[i],
                                      @"For click at %@ got a range of %@ giving “%@”, while I expected “%@”", @(i),
                                      VT100GridWindowedRangeDescription(range), actual, expected[i]);
            }
        }];
}
...
- (void)testDoubleWidthCharacterSmartSelection {
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridWindowedRange range;
    NSDictionary *rule = @{kRegexKey : @"\\S+", kPrecisionKey : kVeryHighPrecision};
...
- (void)testRangeByTrimmingWhitespace_TrimBothEnds {
    _lines = @[ @"  foo  " ];
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridAbsCoordRange actual =
        [extractor rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 7, 0)];
...
- (void)testRangeByTrimmingWhitespace_TrimLeft {
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridAbsCoordRange actual =
        [extractor rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 5, 0)];
    VT100GridAbsCoordRange expected = VT100GridAbsCoordRangeMake(2, 0, 5, 0);
    XCTAssertEqual(actual.start.x, expected.start.x);
    XCTAssertEqual(actual.start.y, expected.start.y);
    XCTAssertEqual(actual.end.x, expected.end.x);
...
- (void)testRangeByTrimmingWhitespace_NothingToTrim {
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridAbsCoordRange actual =
        [extractor rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 3, 0)];
    VT100GridAbsCoordRange expected = VT100GridAbsCoordRangeMake(0, 0, 3, 0);
    XCTAssertEqual(actual.start.x, expected.start.x);
    XCTAssertEqual(actual.start.y, expected.start.y);
    XCTAssertEqual(actual.end.x, expected.end.x);
...
- (void)testRangeByTrimmingWhitespace_MultiLine {
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridAbsCoordRange actual =
        [extractor rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 7, 2)];
...
- (void)testContentInRange_RemoveTabFillers {
    ...
    int len = 0;
    StringToScreenChars(line, data.mutableBytes, color, color, &len, NO, NULL, NULL, iTermUnicodeNormalizationNone,
                        kUnicodeVersion, NO, NULL);
...
- (void)testWrappedString {
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    iTermLocatedString *prefix = [extractor wrappedLocatedStringAt:VT100GridCoordMake(5, 0)
                                                           forward:NO
...
- (void)testRangeForWrappedLine_MaxChars {
    for (int i = 0; i < 10; i++) {
        [self appendWrappedLine:@"1234567890" width:10 eol:i < 9 ? EOL_SOFT : EOL_HARD];
    }
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridWindowedRange range = [extractor rangeForWrappedLineEncompassing:VT100GridCoordMake(5, 5)
...
- (void)testRangeForWrappedLine_EOL_DWC {
    ...
    [self appendWrappedLine:[NSString stringWithFormat:@"111111111111111111111111111中%C%C", (unichar)DWC_RIGHT,
                                                       (unichar)DWC_SKIP]
                      width:30
                        eol:EOL_DWC];
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:self];
    VT100GridWindowedRange range = [extractor rangeForWrappedLineEncompassing:VT100GridCoordMake(5, 1)
                                                         respectContinuations:NO
                                                                     maxChars:1000];
    XCTAssertEqual(range.coordRange.start.x, 0);
...
- (void)testBinarySearch_ExactMatch {
    // Class method - no instance/data source needed
    NSInteger actual = [iTermTextExtractor indexInSortedArray:@[ @10, @20, @30 ]
...
- (void)testBinarySearch_ExactMatchWithMultipleEqualValues {
    // Class method - no instance/data source needed
    NSInteger actual = [iTermTextExtractor indexInSortedArray:@[ @10, @20, @20, @30 ]
                                   withValueLessThanOrEqualTo:20
...
- (int)lengthOfLineAtIndex:(int)theIndex withBuffer:(screen_char_t *)buffer {
    ...
    if ([_lines[0] isKindOfClass:[NSData class]]) {
        ...
        screen_char_t color = {0};
        int len = 0;
        StringToScreenChars(_lines[0], buffer, color, color, &len, NO, NULL, NULL, iTermUnicodeNormalizationNone,
                            kUnicodeVersion, NO, NULL);
        return len;
    }
}
...
- (ScreenCharArray *)screenCharArrayForLine:(int)line {
    ...
    return [[ScreenCharArray alloc] initWithLine:sct length:width continuation:sct[width]];
}
...
- (const screen_char_t *)getLineAtIndex:(int)theIndex {
    ...
    if ([_lines[0] isKindOfClass:[NSData class]]) {
        ...
        screen_char_t color = {0};
        int len = 0;
        StringToScreenChars(_lines[theIndex], _buffer, color, color, &len, NO, NULL, NULL,
                            iTermUnicodeNormalizationNone, kUnicodeVersion, NO, NULL);
