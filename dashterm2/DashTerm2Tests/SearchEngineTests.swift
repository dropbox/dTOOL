...
class SearchEngineTests: XCTestCase {
    ...
    func testUpdateSnapshot2() {
        ...
        guard let op = SearchOperation(snapshot: screen.snapshot(forcingPrimaryGrid: false),
                                        request: request,
                                        queue: queue) else {
            XCTFail("Failed to create SearchOperation")
            return
        }
        ...
        let expected = [9,  // From before the snapshot update, "Test 9"
                        9,  // Test a
                        8, 7, 6, 5, 4, 3, 2, 11, 10].compactMap { y in
    ...
    func testUpdateSnapshot() {
        ...
        let queue = FakeDispatchQueue()
        guard let op = SearchOperation(snapshot: snapshot,
                                        request: request,
                                        queue: queue) else {
            XCTFail("Failed to create SearchOperation")
            return
        ...
        let expected = (5...8).reversed().compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testSpanLines() {
        ...
        let expected = [SearchResult(fromX: 0, y: 2, toX: 5, y: 3)].compactMap { $0 }
        expectEquals(expected, results)
    }
    ...
    func testEmptyQueryMatches() {
        ...
        let request = SearchRequest(direction: .forwards,
                                    ...
                                    initialStart: VT100GridAbsCoord(x: 0, y: 0))
        let results = performTestSearch(screen: screen, request: request)
        let expected = (0..<10).compactMap { y in
    ...
    func testSingleResult() {
        ...
        let request = SearchRequest(direction: .forwards,
                                    ...
                                    initialStart: VT100GridAbsCoord(x: 0, y: 0))
        let results = performTestSearch(screen: screen, request: request)
        let expected = [0].compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 0, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testCaseSensitiveRegex() {
        ...
        let request = SearchRequest(direction: .forwards,
                                    ...
                                    initialStart: VT100GridAbsCoord(x: 0, y: 0))
        let results = performTestSearch(screen: screen, request: request)
        let expected = (5...6).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testRegex() {
        ...
        let request = SearchRequest(direction: .forwards,
                                    ...
                                    initialStart: VT100GridAbsCoord(x: 0, y: 0))
        let results = performTestSearch(screen: screen, request: request)
        let expected = (0...9).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testForwardsWithCursorOutOfBoundsAndLineRange() {
        ...
        let expected = (3..<8).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testBackwardsWithLineRange() {
        ...
        let expected = ([5, 4, 3, 7, 6]).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testForwardsWithLineRange() {
        ...
        let expected = (Array(5...7)+Array(3...4)).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testForwardsFromCursor() {
        ...
        let expected = (Array(5...9)+Array(0...4)).compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testBackwardsFromCursor() {
        ...
        let expected = (Array(6...9)+Array(0...5)).reversed().compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
    ...
    func testBackwards() {
        ...
        let request = SearchRequest(direction: .backwards,
                                    ...
                                    initialStart: VT100GridAbsCoord(x: 5, y: 9))
        let results = performTestSearch(screen: screen, request: request)
        let expected = Array(0...9).reversed().compactMap { y in
            SearchResult(fromX: 0, y: y, toX: 3, y: y)
        }
        expectEquals(expected, results)
    }
