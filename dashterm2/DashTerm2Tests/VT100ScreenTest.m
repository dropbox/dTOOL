...
- (void)screenMoveSelectionUpBy:(int)n inRegion:(VT100GridRect)region {
    if (selection_.allSubSelections.count == 0) {
        return;
    }
    NSArray<iTermSubSelection *> *subs = [selection_.allSubSelections copy];
    [selection_ clearSelection];
    for (iTermSubSelection *sub in subs) {
        ...
        iTermSubSelection *newSub = [iTermSubSelection subSelectionWithAbsRange:range
                                                                           mode:sub.selectionMode
                                                                          width:region.size.width];
        newSub.connected = sub.connected;
        [selection_ addSubSelection:newSub];
    }
}
...
- (void)testSetSizeHeight {
    ...
    XCTAssert([[self selectedStringInScreen:screen] isEqualToString:@"abcdefgh\nijkl\nMNOPQRST\nUVWXYZ\n"]);
    [screen setSize:VT100GridSizeMake(6, 6)];
    XCTAssertEqualObjects([self selectedStringInScreen:screen], @"abcdefgh\nMNOPQRST\nUVWXYZ\n");
...
- (void)testScrollingInAltScreen {
    ...
    XCTAssertEqual(selectionRange.coordRange.start.y, 4);
}
...
// RC-003/BUG-1269: Test that hasMultipleFrames correctly reports when DVR has
// multiple frames with different timestamps (required for instant replay).
- (void)testRC003_BUG_1269_hasMultipleFrames {
    // Create a DVR directly to test the root cause fix
    DVR *dvr = [[DVR alloc] initWithBufferCapacity:10000];

    // Empty DVR should not have multiple frames
    XCTAssertTrue(dvr.empty, @"New DVR should be empty");
    XCTAssertFalse(dvr.hasMultipleFrames, @"Empty DVR cannot have multiple frames");

    // Now test via screen (the normal usage path)
    VT100Screen *screen = [self screenWithWidth:20 height:3];
    screen.delegate = (id<VT100ScreenDelegate>)self;

    // No frames yet
    XCTAssertFalse(screen.dvr.hasMultipleFrames,
                   @"DVR with no frames should return NO for hasMultipleFrames");

    // Add first frame
    [self appendLines:@[ @"Line 1"] toScreen:screen];
    [screen saveToDvr:nil];

    // Single frame - hasMultipleFrames should still be NO
    // (first and last timestamp are the same)
    XCTAssertFalse(screen.dvr.hasMultipleFrames,
                   @"DVR with single frame should return NO - instant replay needs span > 0");

    // Wait a tiny bit to ensure different timestamps, then add second frame
    [NSThread sleepForTimeInterval:0.001];
    [self appendLines:@[ @"Line 2"] toScreen:screen];
    [screen saveToDvr:nil];

    // Two frames with different timestamps - hasMultipleFrames should be YES
    XCTAssertTrue(screen.dvr.hasMultipleFrames,
                  @"DVR with multiple frames should return YES - instant replay can calculate span");

    // Verify the span is non-zero
    long long span = screen.dvr.lastTimeStamp - screen.dvr.firstTimeStamp;
    XCTAssertGreaterThan(span, 0, @"Span should be positive when hasMultipleFrames is YES");
