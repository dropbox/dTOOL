...
// GitLab #12316: Test that leading colons don't break URL detection
- (void)testRangeOfURLInString_leadingColon {
    // A leading colon should be stripped, not treated as part of a URL scheme
    {
        NSString *string = @":foo";
        NSRange range = [string rangeOfURLInString];
        XCTAssertEqual(range.location, 1u, @"Leading colon should be skipped");
        XCTAssertEqual(range.length, 3u, @"'foo' should be detected");
        XCTAssertTrue([[string substringWithRange:range] isEqualToString:@"foo"],
                      @"Should return 'foo', not ':foo'");
    }

    // Just a colon should return NSNotFound
    {
        NSString *string = @":";
        NSRange range = [string rangeOfURLInString];
        XCTAssertEqual(range.location, (NSUInteger)NSNotFound,
                       @"A lone colon should return NSNotFound");
    }

    // Whitespace + colon + text should skip both
    {
        NSString *string = @" :bar";
        NSRange range = [string rangeOfURLInString];
        XCTAssertEqual(range.location, 2u, @"Whitespace and colon should be skipped");
        XCTAssertTrue([[string substringWithRange:range] isEqualToString:@"bar"],
                      @"Should return 'bar'");
    }

    // Valid URL scheme should still work
    {
        NSString *string = @"http://example.com";
        NSRange range = [string rangeOfURLInString];
        XCTAssertTrue([[string substringWithRange:range] isEqualToString:@"http://example.com"],
                      @"Valid URL should still be detected correctly");
    }
}
