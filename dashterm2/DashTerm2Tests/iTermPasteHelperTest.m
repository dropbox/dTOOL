...
- (void)setUp {
    ...
    _warningBlock = [^NSModalResponse(NSAlert *alert, NSString *identifier) {
        if ([identifier
                isEqualToString:[iTermAdvancedSettingsModel noSyncDoNotWarnBeforeMultilinePasteUserDefaultsKey]]) {
            return 1; /* deprecated NSAlertDefaultReturn; */
        }
        XCTAssert(false);
        return 0; // Unreachable but needed for return type
    } copy];
}
...
- (void)testSanitizeIdentity {
    [self sanitizeString:kTestString expect:kTestString flags:0 tabTransform:kTabTransformNone spacesPerTab:0];
}
...
- (void)testSanitizeAllFlagsOn {
    ...
    [self sanitizeString:kTestString
                  expect:expected
                   flags:(kPasteFlagsEscapeSpecialCharacters | kPasteFlagsSanitizingNewlines |
                          kPasteFlagsRemovingUnsafeControlCodes | kPasteFlagsBracket | kPasteFlagsBase64Encode |
...
- (void)testSanitizeTabsToSpaces {
    [self sanitizeString:@"a\tb" expect:@"a    b" flags:0 tabTransform:kTabTransformConvertToSpaces spacesPerTab:4];
}
...
- (void)testBasicPasteString {
    ...
    // Paste history is only saved when Secure Event Input is not enabled
    // (or when saveToPasteHistoryWhenSecureInputEnabled is YES).
    // In test environments (xcodebuild), Secure Event Input is often enabled,
    // so we only check paste history when it's expected to be saved.
...
- (void)testDelegateChangesItsMindAboutBracketingWithQueue {
    ...
    [self runTimer];
    NSString *expected = [[[@"\x1b[200~" stringByAppendingString:test1] stringByAppendingString:@"\x1b[201~"]
        stringByAppendingString:test2];
    XCTAssert([_writeBuffer isEqualToString:expected]);
}
...
- (void)test_BUG_11099_editCurrentSessionDisabledWithoutSession {
    iTermController *controller = [iTermController sharedInstance];
    PseudoTerminal *previousTerminal = controller.currentTerminal;
    controller.currentTerminal = nil;
    @try {
        iTermApplicationDelegate *delegate = [[[iTermApplicationDelegate alloc] init] autorelease];
        NSMenuItem *menuItem = [[[NSMenuItem alloc] initWithTitle:@"Edit Current Session..."
                                                          action:@selector(editCurrentSession:)
                                                   keyEquivalent:@""] autorelease];
        BOOL enabled = [delegate validateMenuItem:menuItem];
        XCTAssertFalse(enabled, @"BUG-11099: Edit Current Session should be disabled without a session");
    } @finally {
        controller.currentTerminal = previousTerminal;
    }
}
