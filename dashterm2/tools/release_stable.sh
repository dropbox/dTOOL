#!/bin/bash
# BUG-128: iterm2.com/downloads URLs kept until DashTerm2 download hosting exists.
...
function SparkleSign {
    ...
    ../../tools/sign_update DashTerm2-${NAME}.zip "$EDPRIVKEY" > /tmp/newsig.txt || die SparkleSignNew
    ...
    cp DashTerm2-${NAME}.zip ~/iterm2-website/downloads/stable/
}
...
function Build {
  ...
  codesign -dv --verbose=4 "build/$BUILDTYPE/DashTerm2.app" > /tmp/signature 2>&1
  ...
  PRENOTARIZED_ZIP=DashTerm2-${NAME}-prenotarized.zip
  ...
  NOTARIZED_ZIP=DashTerm2-${NAME}.zip
  ...
  cp DashTerm2-${NAME}.zip $SVNDIR/downloads/stable/
 
  test -f $SVNDIR/downloads/stable/DashTerm2-${NAME}.summary || (echo "DashTerm2 "$VERSION" ($SUMMARY)" > $SVNDIR/downloads/stable/DashTerm2-${NAME}.summary)
  test -f $SVNDIR/downloads/stable/DashTerm2-${NAME}.description || (echo "$DESCRIPTION" > $SVNDIR/downloads/stable/DashTerm2-${NAME}.description)
  vi $SVNDIR/downloads/stable/DashTerm2-${NAME}.description
  BUILDDATE=$(date +"%B %-d, %Y")
  echo "Version $VERSION of DashTerm2 was built on $BUILDDATE." > $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  echo '' >> $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  shasum -a256 DashTerm2-${NAME}.zip | awk '{print $1}' > /tmp/sum
  gpg --clearsign /tmp/sum
  echo "You can use the following to verify the zip file on https://keybase.io/verify:" >> $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  echo "" >> $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  cat /tmp/sum.asc >> $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  vi $SVNDIR/downloads/stable/DashTerm2-${NAME}.changelog
  pushd $SVNDIR

  echo 'Options +FollowSymlinks' > ~/iterm2-website/downloads/stable/.htaccess
  echo 'Redirect 302 /downloads/stable/latest https://iterm2.com/downloads/stable/DashTerm2-'${NAME}'.zip' >> ~/iterm2-website/downloads/stable/.htaccess
  ...
  git add downloads/stable/DashTerm2-${NAME}.summary downloads/stable/DashTerm2-${NAME}.description downloads/stable/DashTerm2-${NAME}.changelog downloads/stable/DashTerm2-${NAME}.zip source/appcasts/final_modern.xml source/appcasts/full_changes.txt downloads/stable/.htaccess
  popd
