#!/bin/bash
# BUG-126: ~/iterm2-website path kept until DashTerm2 infrastructure exists.
...
function Build {
  ...
  test -f "$PRENOTARIZED_ZIP" || die "Can not find $PRENOTARIZED_ZIP"
  rm -rf iTerm.app
  unzip -x "$PRENOTARIZED_ZIP"
  popd

  codesign --verify --verbose "build/$BUILDTYPE/iTerm.app" || die "Signature not verified"
  codesign -dv --verbose=4 "build/$BUILDTYPE/iTerm.app" > /tmp/signature 2>&1
