...
function SparkleSign {
    ...
    ../../tools/sign_update DashTerm2-${NAME}.zip "$3" > /tmp/newsig.txt || die SparkleSignNew
...
rm -rf build/Nightly/DashTerm2.app
...
cp DashTerm2-${NAME}.zip ~/Dropbox/NightlyBuilds/
set -x
retry scp -vvv -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no DashTerm2-${NAME}.zip gnachman@bryan:iterm2.com/nightly/DashTerm2-${NAME}.zip || die "scp zip"
retry ssh -vvv -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no gnachman@bryan "./newnightly.sh DashTerm2-${NAME}.zip" || die "ssh"
