#!/bin/bash
# BUG-127: Creates DashTerm2-${NAME}.zip (artifact naming updated).
# Run this before uploading.
MINORVERSION=$1
BUGVERSION=$2
NAME=1.$MINORVERSION.$BUGVERSION
cd build/Deployment
zip -r DashTerm2-${NAME}.zip iTerm.app
vi ../../appcasts/full_changes.html
LENGTH=$(ls -l DashTerm2-${NAME}.zip | awk '{print $5}')
ruby "/Users/georgen/Downloads/Sparkle 1.5b6/Extras/Signing Tools/sign_update.rb" DashTerm2-${NAME}.zip $PRIVKEY > /tmp/sig.txt
SIG=$(cat /tmp/sig.txt)
...
echo "Go upload the DashTerm2-${NAME}.zip, then run:"
