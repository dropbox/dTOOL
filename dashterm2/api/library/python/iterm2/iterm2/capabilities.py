...
class AppVersionTooOld(Exception):
    """Raised when you need a newer version of DashTerm2."""
...
def check_supports_coprocesses(connection):
    ...
    if not supports_coprocesses(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to control " +
...
def check_supports_get_default_profile(connection):
    ...
    if not supports_get_default_profile(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to get the " +
...
def check_supports_prompt_id(connection):
    ...
    if not supports_prompt_id(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to fetch a list of " +
...
def check_supports_context_menu_provider(connection):
    ...
    if not supports_context_menu_providers(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to register a " +
...
def check_supports_add_annotation(connection):
    ...
    if not supports_add_annotation(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to add an annotation. " +
            "You should upgrade to run this script.")
...
def check_supports_advanced_key_notifications(connection):
    ...
    if not supports_advanced_key_notifications(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to get advanced keystroke " +
            "notifications. You should upgrade to run this script.")

def supports_file_panels(connection):
    """"Can you use open/save panels?"""
    min_ver = (1, 10)
    return ge(connection.iterm2_protocol_version, min_ver)

def check_supports_file_panels(connection):
    """"Die if you can't use open/save panels."""
    if not supports_file_panels(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to use open/save panels. " +
            "You should upgrade to run this script.")
...
def check_supports_move_session(connection):
    if not supports_move_session(connection):
        raise AppVersionTooOld(
            "This version of DashTerm2 is too old to move sessions to split panes. " +
            "You should upgrade to run this script.")
