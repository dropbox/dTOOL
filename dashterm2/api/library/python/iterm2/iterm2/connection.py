...
# BUG-145: Docstring updated to reference DashTerm2 connection.
"""
...
class Connection:
    """Represents a loopback network connection from the script to DashTerm2.
    ...
    async def async_create() -> 'Connection':
        ...
        :returns: A new connection to DashTerm2.
            ...
            try:
                ...
                if status_code_exception.status_code == 401:
                    ...
                    print("This version of the iterm2 module is too old for " +
                          "the current version of DashTerm2. Please upgrade.")
                    sys.exit(1)
                    raise
                else:
                    raise
    ...
    def iterm2_protocol_version(self):
        ...
        version of DashTerm2 that doesn't report its version or it's unknown.
        """
        key = "X-DashTerm2-Protocol-Version"
    ...
    def _unix_domain_socket_path(self):
        applicationSupport = os.path.expanduser("~/Library/Application Support/DashTerm2")
    ...
    async def async_connect(self, coro, retry=False):
        ...
        setup for you. Connects to DashTerm2 on localhost. Once connected, awaits
        execution of coro.
            ...
            try:
                ...
                if exception.status_code == 401:
                    ...
                    print("This version of the iterm2 module is too old " +
                          "for the current version of DashTerm2. Please upgrade.")
                    sys.exit(1)
                    raise
                else:
                    raise
                ...
                if retry:
                    ...
                    print("""
There was a problem connecting to DashTerm2.

Please check the following:
  * Ensure the Python API is enabled in DashTerm2's preferences
  * Ensure DashTerm2 is running
  * Ensure script is running on the same machine as DashTerm2
...
""", file=sys.stderr)
                    ...
                    exists = os.path.exists(path)
                    if exists:
                        print(f"If you have downgraded from DashTerm2 3.3.12+ to an older version, you must\nmanually delete the file at {path}.\n", file=sys.stderr)
                    done = True
                    raise ConnectionRefusedError("Problem connecting to DashTerm2")
            finally:
                self._remove_auth()
...
def add_disconnect_callback(callback: typing.Callable[[], None]):
    ...
    """
    # BUG-1025: Remove unused global declaration (append doesn't reassign)
