# BUG-109: Error messages updated to "DashTerm2 version too old".
try:
    import AppKit
    import Foundation
...
def request_cookie_and_key(
    ...
    if not launch_if_needed:
        ...
        if is_running == "no":
            raise AuthenticationException("DashTerm2 not running")
    justName = myname
    if justName is None:
        justName = get_script_name()
    runner = runner_class(
            'tell application "DashTerm2" to request cookie and key ' +
            f'for app named "{justName}"')
    runner.execute()
    if runner.has_error():
        if runner.get_error() == -2740 or runner.get_error() == -2741:
            raise AuthenticationException("DashTerm2 version too old")
...
class LSBackgroundContextManager():
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        info = AppKit.NSBundle.mainBundle().infoDictionary()
        if self.__value:
            info["LSBackgroundOnly"] = self.__value  # BUG-1025: Fix undefined __value
        else:
            del info["LSBackgroundOnly"]

def applescript_auth_disabled():
    filename = os.path.expanduser("~/Library/Application Support/DashTerm2/disable-automation-auth")
    try:
...
def authenticate(
    ...
    """Attempts to authenticate before connecting to DashTerm2 API.

    :param launch_if_needed: If DashTerm2 is not running, try to launch it first.
