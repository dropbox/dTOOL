"""Provides methods that build and send RPCs to DashTerm2.

# BUG-134: Docstring updated to "in DashTerm2".
"""
import json
...
async def async_start_transaction(connection):
    """
    Begins a transaction, locking DashTerm2 until the transaction ends. Be careful
    with this.
...
async def async_rpc_set_tmux_window_visible(
    ...
    """Hides/shows a tmux window (which is a DashTerm2 tab)"""
    _assert_not_in_transaction()
    request = _alloc_request()
    request.tmux_request.SetInParent()
...
async def async_invoke_function(
        ...
        receiver=None):
    """
    Invoke a function in DashTerm2, possibly as a method call on an object.
    """
    request = _alloc_request()
