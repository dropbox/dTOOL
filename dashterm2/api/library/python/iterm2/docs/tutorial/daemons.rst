...
A DashTerm2 daemon would ordinarily be an AutoLaunch script that provides some
...
`~/Library/Application Support/DashTerm2/Scripts/AutoLaunch`. When you create a
new one it does not get launched until DashTerm2 is restarted, but you can always
run it by selecting it from the **Scripts** menu.

When you create a new script and choose to make it a "Long-Running Daemon" (as
opposed to a "Simple" script), DashTerm2 will provide a sample program to help you
get started:
...
continue to process custom control sequences until DashTerm2 terminates. This is
