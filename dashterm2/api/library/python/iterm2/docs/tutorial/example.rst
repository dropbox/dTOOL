...
Here's the example script that DashTerm2 provides for you, minus some comments:
...
to communicate with DashTerm2. The underlying implementation uses Google protobuf
...
the link to a running DashTerm2 process. `main` gets called only after a
connection is established.  If the connection terminates (e.g., if you quit
DashTerm2) then any attempt to use it will raise an exception and terminate your
script.
...
remote procedure call over a network. Because DashTerm2 communicates with the
...
response from DashTerm2, other work can happen. For example, handling of
notifications from DashTerm2. We'll see more about that later.
...
It is a singleton that provides access to DashTerm2's windows, and in turn their
tabs and sessions.

Note the use of `await`. Any function that's defined as `async`, which most
functions in the DashTerm2 API are, must be called with `await`. It means it might
not return immediately. While it waits, messages from DashTerm2 or other I/O may
be received and handled.

In this case, it makes an RPC call to DashTerm2 to get its state (such as the list
...
DashTerm2's library follows a naming convention to help you remember to use
...
some other non-terminal window) that receives keyboard input when DashTerm2 is
active.
...
Console. Select *Scripts > Script Console* in DashTerm2 to view the output of
...
This makes a connection to DashTerm2 and invokes your `main` function in an
