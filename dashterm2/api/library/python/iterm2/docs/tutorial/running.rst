...
.. BUG-117: Version references updated to DashTerm2.
...
3. Auto-run scripts launched when DashTerm2 starts.
...
`$HOME/Library/ApplicationSupport/DashTerm2/Scripts`. The following files are
included:
...
DashTerm2's standard Python installation is at
`~/Library/ApplicationSupport/DashTerm2/iterm2env/versions/*/bin/python3`.
This is the so-called "Basic" environment.

If you create a script with the "Full Environment" its instance of Python
will be in
`~/Library/ApplicationSupport/DashTerm2/Scripts/YourScript/iterm2env/versions/*/bin/python3`.

Internally, DashTerm2 runs a basic script by invoking:

.. code-block:: python

    ~/Library/ApplicationSupport/DashTerm2/iterm2env/versions/*/bin/python3 YourScript.py


DashTerm2 stores scripts in `$HOME/Library/ApplicationSupport/DashTerm2/Scripts`.
...
.. note::
    ...
    DashTerm2 creates the `ApplicationSupport` symlink to `Application
...
If you'd like your script to launch DashTerm2, you'll need to use pyobjc. To install it:
...
.. code-block:: python
    ...
    if not AppKit.NSRunningApplication.runningApplicationsWithBundleIdentifier_(bundle):
        AppKit.NSWorkspace.sharedWorkspace().launchApplication_("DashTerm2")
...
When you run a script from the command line on DashTerm2 you will
be prompted for permission. This is a security measure to ensure that scripts not launched
by DashTerm2 are not being run without your knowledge. The purpose is to prevent untrusted
...
`DashTerm2.app/Contents/Resources/it2run` to launch it. The `it2run` script uses
`osascript` to ask DashTerm2 to launch your Python script. macOS will ask for a
one-time grant of permission for `osascript` to control DashTerm2.

You may also pass command line arguments to it2run that get forwarded to the script.
For example:

```
/Applications/DashTerm2.app/Contents/Resources/it2run myscript.py firstarg secondarg thirdarg
```
...
If you'd like your script to launch automatically when DashTerm2 starts, move it
to `$HOME/Library/ApplicationSupport/DashTerm2/Scripts/AutoLaunch`.
...
DashTerm2 also offers a *REPL*: a *Read-eval-print loop*. This is an interactive
