...
DashTerm2 offers a very powerful facility where a script (typically a daemon)
registers a function as available to be invoked by DashTerm2.
...
the request from DashTerm2. The simplest way is to tell it to watch for requests
...
can press a key in DashTerm2 to invoke the RPC and then do
...
because DashTerm2's scripting interface is meant to be language-agnostic (although
...
DashTerm2 application itself. Some are defined by DashTerm2, like `id`, which
takes a string value that uniquely identifies a session. Others, beginning with
`user.` may be defined by the user.

For a full list of the DashTerm2-defined variables, see
`Variables <https://www.iterm2.com/documentation-variables.html>`_.

.. note::

    These tutorial links currently point to iterm2.com while the
    DashTerm2 documentation site is being prepared. Update the URLs once the
    DashTerm2 doc site is live.
...
By default, DashTerm2 stops waiting for a function's result after five seconds.
