...
.. BUG-118: Shell Integration URLs kept until DashTerm2 infrastructure exists.

Per-Host Colors
===============

This script sets the color of a session based on the current hostname. For it to work, DashTerm2 must know the current hostname. You can do that either by installing Shell Integration or by defining triggers that detect the hostname.
