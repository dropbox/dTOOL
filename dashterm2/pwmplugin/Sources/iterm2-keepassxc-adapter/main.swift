...
func handleListAccounts() {
    ...
    do {
        ...
        if let folder = modeFolder {
            // Terminal mode: filter for entries in the DashTerm2 folder
        ...
        for startIndex in stride(from: 0, to: entryPaths.count, by: batchSize) {
            ...
            let queue = DispatchQueue(label: "com.dashterm.dashterm2.keepassxc-adapter", attributes: .concurrent)
...
func handleAddAccount() {
    ...
    do {
        ...
        // Create entry within a folder based on mode (terminal uses DashTerm2 folder, browser uses root)
        let modeFolder = modeTag(from: request.header.mode)
        let entryPath: String

        if let folder = modeFolder {
            // Terminal mode: create in DashTerm2 folder
