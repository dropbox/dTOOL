...
# Create DashTerm2 folder
keepassxc-cli mkdir "$TEST_DB" DashTerm2 <<EOF
$TEST_PASSWORD
EOF

# Add a test entry in DashTerm2 folder
keepassxc-cli add -p -u testuser "$TEST_DB" DashTerm2/TestEntry <<EOF
$TEST_PASSWORD
testpass
testpass
EOF
...
# Check that DashTerm2/TestEntry is no longer in the accounts list
if echo "$LIST_OUTPUT_AFTER" | grep -q '"accountID" : "DashTerm2/TestEntry"'; then
