# dashterm2-keepassxc-adapter

A Swift CLI that wraps `keepassxc-cli` to provide password manager integration for DashTerm2.

## Architecture

This project uses Swift Package Manager and is organized into two modules:

- **PasswordManagerProtocol** - Protocol data structures (shared with DashTerm2)
  - Located in `Sources/PasswordManagerProtocol/`
  - Can be included directly in DashTerm2's Xcode project

- **dashterm2-keepassxc-adapter** - CLI implementation specific to KeePassXC
...
## Integration with DashTerm2

To integrate with DashTerm2:

1. Add the **PasswordManagerProtocol** module to the DashTerm2 Xcode project
...
2. Import and use the protocol structures for JSON encoding/decoding:
   ...
   ```
3. Call the `dashterm2-keepassxc-adapter` executable with appropriate JSON input/output

The protocol structures are shared between the CLI and DashTerm2, ensuring type safety and compatibility.
