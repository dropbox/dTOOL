...

{
    ...
    #
    # Source the user's zshenv before sourcing dashterm2_shell_integration.zsh because the former
    # might set fpath and other things without which dashterm2_shell_integration.zsh won't work.
    #
    ...
    if [[ -o 'interactive' && -n "${ITERM_INJECT_SHELL_INTEGRATION-}" ]]; then
        ...
        'builtin' 'typeset' _it2_file="${${(%):-%x}:A:h}"/dashterm2_shell_integration.zsh
        if [[ -r "$_it2_file" ]]; then
