...
class CompletionsWindow: NSWindow, NSTableViewDataSource, NSTableViewDelegate {
    ...
    static func attributedString(font: NSFont,
        ...
        // BUG-1733: Use as? with fallback to NSMutableAttributedString initializer
        let attributedString: NSMutableAttributedString = (attributedPrefix.mutableCopy() as? NSMutableAttributedString) ?? NSMutableAttributedString(attributedString: attributedPrefix)
    ...
    var selectedItem: Item? {
        ...
        // BUG-f1016: Also check upper bound - selectedRow can be >= items.count after items array changes
    ...
    private func configure(for mode: Mode, animated: Bool) {
        ...
        case .completions(let completionItems):
            // BUG-479: Handle empty completions gracefully instead of crashing
            guard !completionItems.isEmpty else {
                DLog("Warning: configure called with empty completions - ignoring")
                return
            }
            ...
            visualEffectView.autoresizingMask = [.width, .height]
            // BUG-9004: Only add visualEffectView if not already in the view hierarchy.
            // This prevents duplicate subviews when configure() is called multiple times.
            if visualEffectView.superview != contentView {
                contentView?.addSubview(visualEffectView)
            }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        // BUG-f1017: Guard against out-of-bounds access - row may be stale if items changed
        guard row >= 0 && row < items.count else {
            DLog("CompletionsWindow.tableView(_:viewFor:row:): row \(row) out of bounds (items.count=\(items.count))")
            return nil
        }
    ...
    // BUG-1790: Use nil coalescing with fallback instead of force unwrap for all icons
        ...
        case .file:
            if #available(macOS 15, *) {
                return NSImage.it_image(forSymbolName: SFSymbol.document.rawValue,
                                        ...
                                        for: CompletionsWindow.self) ?? NSImage()
            } else {
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        guard let completionsTableView = tableView,
              completionsTableView.selectedRow >= 0,
              completionsTableView.selectedRow < items.count else { return }
    ...
    private func selectRow(_ row: Int) {
        // BUG-f1018: Guard against empty items array - min(items.count - 1, row) is -1 when items is empty
        guard !items.isEmpty else {
            DLog("CompletionsWindow.selectRow: items is empty, cannot select row \(row)")
            return
        }
...
class CompletionCell: NSTableCellView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f839: Return nil instead of crashing for unused coder initializer
        DLog("CompletionsWindow init(coder:) is not supported")
        return nil
    }
}
