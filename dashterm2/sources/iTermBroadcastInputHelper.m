    ...
    switch ([self broadcastMode]) {
        case BROADCAST_TO_ALL_PANES: {
            ...
            NSArray<NSString *> *sessionIDs =
                [self.delegate broadcastInputHelperSessionsInCurrentTab:self includeExited:YES] ?: @[];
            _broadcastDomains = [NSSet setWithArray:@[ [NSSet setWithArray:sessionIDs] ]];
            break;
        }
        case BROADCAST_TO_ALL_TABS: {
            NSArray<NSString *> *sessionIDs =
                [self.delegate broadcastInputHelperSessionsInAllTabs:self includeExited:YES] ?: @[];
            _broadcastDomains = [NSSet setWithArray:@[ [NSSet setWithArray:sessionIDs] ]];
            break;
        }
        case BROADCAST_OFF:
    ...
    if (removed) {
        ...
        if (_broadcastDomains.count == 0) {
            ...
            NSSet<NSString *> *domainToAddTo =
                [_broadcastDomains anyObjectPassingTest:^BOOL(NSSet<NSString *> *_Nonnull element) {
                    return [element containsObject:currentSession];
                }]
                    ?: _broadcastDomains.anyObject;
            // BUG-3774 fix: Use isEqual: instead of pointer comparison since mapWithBlock
            // creates new set objects, making pointer comparison always fail.
            _broadcastDomains = [_broadcastDomains mapWithBlock:^id _Nonnull(NSSet<NSString *> *_Nonnull existingSet) {
                if (![existingSet isEqual:domainToAddTo]) {
                    return existingSet;
                }
...
- (NSSet<NSString *> *)broadcastSessionIDs {
    switch ([self broadcastMode]) {
        ...
        case BROADCAST_TO_ALL_PANES:
            return [NSSet setWithArray:[self.delegate broadcastInputHelperSessionsInCurrentTab:self includeExited:NO]];

        case BROADCAST_TO_ALL_TABS:
            return [NSSet setWithArray:[self.delegate broadcastInputHelperSessionsInAllTabs:self includeExited:NO]];
            break;
        ...
        case BROADCAST_CUSTOM: {
            ...
            NSSet<NSString *> *candidates =
                [NSSet setWithArray:[self.delegate broadcastInputHelperSessionsInAllTabs:self includeExited:NO]];
...
- (NSSet<NSString *> *)allSessions {
    return [NSSet setWithArray:[self.delegate broadcastInputHelperSessionsInAllTabs:self includeExited:YES]];
}
...
- (NSSet<NSSet<NSString *> *> *)sanitizedDomains:(NSSet<NSSet<NSString *> *> *)broadcastDomains {
    ...
    NSMutableDictionary<NSString *, NSNumber *> *sessionToIndex =
        [NSMutableDictionary dictionaryWithCapacity:validIDs.count];
    [broadcastDomains.allObjects
        enumerateObjectsUsingBlock:^(NSSet<NSString *> *_Nonnull domain, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)setBroadcastMode:(BroadcastMode)mode {
    ...
    if (mode != BROADCAST_CUSTOM && mode == self.broadcastMode) {
        ...
        if (mode != BROADCAST_OFF && self.broadcastMode == BROADCAST_OFF) {
            ...
            DLog(@"Warn…");
            if ([iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"Keyboard input will be sent to %@.",
                                                                              [self formatDestinationsForMode:mode]]
                                           actions:@[ @"OK", @"Cancel" ]
...
- (NSString *)formatDestinationsForMode:(BroadcastMode)mode {
    switch (mode) {
        ...
        case BROADCAST_TO_ALL_PANES: {
            // Just this tab
            const NSInteger count = [[self.delegate broadcastInputHelperSessionsInCurrentTab:self
                                                                               includeExited:NO] count];
            if (count < 2) {
