...
- (void)setSize:(VT100GridSize)proposedSize
    ...
    if (gDebugLogging) {
        ...
        for (id<IntervalTreeObject> object in self.intervalTree.allObjects) {
            ...
            DLog(@"Note has coord range %@",
                 VT100GridCoordRangeDescription([self coordRangeForInterval:object.entry.interval]));
        }
        DLog(@"------------ end -----------");
    }
}
...
- (void)destructivelySetScreenWidth:(int)width height:(int)height {
    ...
    [self
        addSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
...
- (BOOL)shouldSetSizeTo:(VT100GridSize)size {
    ...
    if (self.currentGrid.size.width == 0 || self.currentGrid.size.height == 0 ||
        (size.width == self.currentGrid.size.width && size.height == self.currentGrid.size.height)) {
        return NO;
    }
    return YES;
}
...
- (void)sanityCheckIntervalsFrom:(VT100GridSize)oldSize note:(NSString *)note {
    ...
    for (id<IntervalTreeObject> obj in [self.intervalTree allObjects]) {
        ...
        ITBetaAssert(interval.limit >= 0, @"Bogus interval %@ after resizing from %@ to %@. Note: %@", interval,
                     VT100GridSizeDescription(oldSize), VT100GridSizeDescription(self.currentGrid.size), note);
    }
#endif
}
...
- (BOOL)trimSelectionFromStart:(VT100GridCoord)start
    ...
    VT100GridRun run = VT100GridRunFromCoords(VT100GridCoordMake(startX, startY), VT100GridCoordMake(endX, endY),
...
- (LineBufferPositionRange *)positionRangeForCoordRange:(VT100GridCoordRange)range
    ...
    // BUG-3785 fix: Validate input before processing to avoid assertion crashes
    // with invalid coordinates. Return nil for out-of-bounds input.
    if (range.end.y < 0 || range.start.y < 0) {
        VLog(@"positionRangeForCoordRange: Invalid y coordinate (negative)");
        return nil;
    }
    ...
    if (range.end.x == self.width) {
        ...
        const unichar sentinel = line[range.end.x].code;
        if (sentinel == EOL_HARD) {
            VLog(@"positionRangeForCoordRange: hard newline sentinel, set endExtends=YES");
            // The selection goes all the way to a hard line break. Remember this so we can restore
            // the trailing newline even when the last visible cell is populated (for example,
            // after resizing wider and filling the final column with text) and the line buffer can
            // no longer infer that the selection extended to the newline boundary.
            endExtends = YES;
        }
    }
    ...
    if (range.start.x < 0 || range.start.y < 0 || range.end.x < 0 || range.end.y < 0) {
    ...
    BOOL ok = [self trimSelectionFromStart:VT100GridCoordMake(range.start.x, range.start.y)
                                  ...
                                  toStartX:&trimmedStart
                                    toEndX:&trimmedEnd];
    VLog(@"positionRangeForCoordRange: Trimming %@ gives %@ - %@", VT100GridCoordRangeDescription(range),
         VT100GridCoordDescription(trimmedStart), VT100GridCoordDescription(trimmedEnd));
    ...
    positionRange.start = [lineBuffer positionForCoordinate:trimmedStart width:self.currentGrid.size.width offset:0];
    VLog(@"positionRangeForCoordRange: Start of range %@ is at %@", VT100GridCoordDescription(trimmedStart),
         positionRange.start);

    VLog(@"positionRangeForCoordRange: Get position for end of range %@", VT100GridCoordDescription(trimmedEnd));
    positionRange.end = [lineBuffer positionForCoordinate:trimmedEnd width:self.currentGrid.size.width offset:0];
    positionRange.end.extendsToEndOfLine = endExtends;
    VLog(@"positionRangeForCoordRange: End of range %@ is at %@", VT100GridCoordDescription(trimmedEnd),
         positionRange.end);
...
- (NSArray *)subSelectionTuplesWithUsedHeight:(int)usedHeight
    ...
    [self appendScreen:self.currentGrid
        ...
        withUsedHeight:usedHeight
             newHeight:newHeight];
    NSMutableArray *altScreenSubSelectionTuples =
        [[NSMutableArray alloc] initWithCapacity:selection.allSubSelections.count];
    for (iTermSubSelection *sub in selection.allSubSelections) {
        VT100GridAbsCoordRangeTryMakeRelative(
            sub.absRange.coordRange, self.cumulativeScrollbackOverflow, ^(VT100GridCoordRange range) {
                LineBufferPositionRange *positionRange = [self positionRangeForCoordRange:range
...
- (BOOL)intervalTreeObjectMayBeEmpty:(id<IntervalTreeImmutableObject>)note {
    ...
    return ([note isKindOfClass:[VT100RemoteHost class]] || [note isKindOfClass:[VT100WorkingDirectory class]] ||
            [note isKindOfClass:[iTermImageMark class]] || [note isKindOfClass:[PTYAnnotation class]]);
}
...
- (NSArray *)intervalTreeObjectsWithUsedHeight:(int)usedHeight
    ...
    VT100GridCoordRange screenCoordRange =
        VT100GridCoordRangeMake(0, self.numberOfScrollbackLines, 0, self.numberOfScrollbackLines + self.height);
    ...
    [self appendScreen:grid toScrollback:appendOnlyLineBuffer withUsedHeight:usedHeight newHeight:newHeight];

    NSMutableArray *triples = [[NSMutableArray alloc] initWithCapacity:notesAtLeastPartiallyOnScreen.count];
    ...
    for (id<IntervalTreeObject> note in notesAtLeastPartiallyOnScreen) {
        ...
        // BUG-f1291: Replace assert with guard - if removal fails, log and continue
        // This can happen in edge cases where the interval tree state is inconsistent
        if (!removed) {
            DLog(@"BUG-f1291: Failed to remove note from interval tree: %@", note);
        }
        ...
        if (positionRange) {
            DLog(@"Add note on alt screen at %@ (position %@ to %@) to triples", VT100GridCoordRangeDescription(range),
                 positionRange.start, positionRange.end);
            [triples addObject:@[ note, positionRange.start, positionRange.end ]];
        } else {
            DLog(@"Failed to get position range while in alt screen for note %@ with range %@", note,
                 VT100GridCoordRangeDescription(range));
        }
    }
    return triples;
...
- (BOOL)convertRange:(VT100GridCoordRange)range
    ...
    resultPtr->start = [lineBuffer coordinateForPosition:selectionRange.start width:newWidth extendsRight:NO ok:NULL];
...
- (NSArray *)subSelectionsWithConvertedRangesFromSelection:(iTermSelection *)selection newWidth:(int)newWidth {
    NSMutableArray *newSubSelections = [[NSMutableArray alloc] initWithCapacity:selection.allSubSelections.count];
    const long long overflow = self.cumulativeScrollbackOverflow;
    for (iTermSubSelection *sub in selection.allSubSelections) {
        DLog(@"convert sub %@", sub);
        VT100GridAbsCoordRangeTryMakeRelative(sub.absRange.coordRange, overflow, ^(VT100GridCoordRange range) {
            ...
            const BOOL ok = [self convertRange:range
                                 ...
                                 tolerateEmpty:NO];
            if (ok) {
                // BUG-f1292: Replace assert with guard - skip invalid ranges instead of crashing
                // Range coordinates should be >= 0 after successful conversion
                if (range.start.y < 0 || range.end.y < 0) {
                    DLog(@"BUG-f1292: Invalid range coordinates after conversion: start.y=%d, end.y=%d", range.start.y,
                         range.end.y);
                    return; // Skip this invalid selection
                }
                ...
                iTermSubSelection *theSub = [iTermSubSelection subSelectionWithAbsRange:absRange
...
- (VT100GridAbsCoord)absCoordFromAbsCoord:(VT100GridAbsCoord)oldCoord newWidth:(int)newWidth {
    VT100GridAbsCoordRange absRange = VT100GridAbsCoordRangeMake(oldCoord.x, oldCoord.y, oldCoord.x + 1, oldCoord.y);
    const long long overflow = self.cumulativeScrollbackOverflow;
...
- (void)updateIntervalTreeWithWidth:(int)newWidth {
    ...
    NSArray<id<IntervalTreeImmutableEntry>> *entries =
        [objects mapWithBlock:^id(id<IntervalTreeImmutableObject> anObject) {
    ...
    [objects enumerateObjectsUsingBlock:^(id<IntervalTreeObject> _Nonnull note, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        // BUG-f1293: Replace assert with guard - skip invalid note ranges instead of crashing
        // Note ranges should have non-negative y coordinates after conversion
        if (noteRange.start.y < 0 || noteRange.end.y < 0) {
            DLog(@"BUG-f1293: Invalid noteRange coordinates: start.y=%d, end.y=%d for note: %@", noteRange.start.y,
                 noteRange.end.y, note);
            return; // Skip this invalid note
        }
        Interval *newInterval =
            [self intervalForGridAbsCoordRange:VT100GridAbsCoordRangeFromCoordRange(newRange, overflow) width:newWidth];
        ...
        if (screenMark) {
            const VT100GridCoordRange commandRange =
                [self safeCoordRange:VT100GridCoordRangeFromAbsCoordRange(screenMark.commandRange, overflow)];
            if (!VT100GridCoordRangeEqualsCoordRange(commandRange, VT100GridCoordRangeInvalid)) {
                ...
                if ([self convertRange:commandRange
                    ...
                    [self.mutableIntervalTree mutateObject:note
                                                     block:^(id<IntervalTreeObject> mutableMark) {
                                                         [VT100ScreenMark castFrom:mutableMark].commandRange =
                                                             VT100GridAbsCoordRangeFromCoordRange(converted, overflow);
                                                     }];
                }
            } else if (VT100GridAbsCoordRangeIsValid(screenMark.commandRange)) {
                [self.mutableIntervalTree mutateObject:note
                                                 block:^(id<IntervalTreeObject> mutableMark) {
                                                     [VT100ScreenMark castFrom:mutableMark].commandRange =
                                                         VT100GridAbsCoordRangeMake(-1, -1, -1, -1);
                                                 }];
            }

            VT100GridCoordRange promptRange =
                [self safeCoordRange:VT100GridCoordRangeFromAbsCoordRange(screenMark.promptRange, overflow)];
            if (!VT100GridCoordRangeEqualsCoordRange(promptRange, VT100GridCoordRangeInvalid)) {
                ...
                // BUG-8305: Was self-modulo (always 0); should be modulo self.width
                ...
                if ([self convertRange:promptRange
                    ...
                    [self.mutableIntervalTree mutateObject:note
                                                     block:^(id<IntervalTreeObject> mutableMark) {
                                                         [VT100ScreenMark castFrom:mutableMark].promptRange =
                                                             VT100GridAbsCoordRangeFromCoordRange(converted, overflow);
                                                     }];
                }
            } else if (VT100GridAbsCoordRangeIsValid(screenMark.promptRange)) {
                [self.mutableIntervalTree mutateObject:note
                                                 block:^(id<IntervalTreeObject> mutableMark) {
                                                     [VT100ScreenMark castFrom:mutableMark].promptRange =
                                                         VT100GridAbsCoordRangeMake(-1, -1, -1, -1);
            ...
            if (screenMark.outputStart.x != -1) {
                ...
                if (!ok) {
                    ...
                    VT100GridCoordRange converted;
                    if ([self convertRange:VT100GridCoordRangeMake(outputStart.x, outputStart.y, outputStart.x + 1,
                                                                   outputStart.y)
                                   toWidth:newWidth
                                        to:&converted
                              inLineBuffer:self.linebuffer
                        ...
                        [self.mutableIntervalTree mutateObject:note
                                                         block:^(id<IntervalTreeObject> mutableMark) {
                                                             [VT100ScreenMark castFrom:mutableMark].outputStart =
                                                                 VT100GridAbsCoordFromCoord(converted.start, overflow);
                                                         }];
                    }
                }
            }
...
- (VT100GridCoordRange)safeCoordRange:(VT100GridCoordRange)unsafeRange {
    ...
    // If the range lives entirely off-screen on a single line there is nothing usable.
    if (unsafeRange.start.y == unsafeRange.end.y && (unsafeRange.start.x >= width || unsafeRange.end.x <= 0)) {
        return VT100GridCoordRangeInvalid;
    }
    ...
    const int maxStart = MAX(width - 1, 0);
    safe.start.x = clamp(0, maxStart, safe.start.x);
    safe.end.x = clamp(0, width, safe.end.x);
    return safe;
}

- (void)fixUpPrimaryGridIntervalTreeForNewSize:(VT100GridSize)newSize wasShowingAltScreen:(BOOL)wasShowingAltScreen {
    if ([self.mutableIntervalTree count]) {
 ...
 * 2 History      }                    <- originalLimit                                 } equal to
 * _mutableState.linebuffer 3 Line from primary grid            <- limit (pushed into history due to resize)     } 4
 * Line to be lost from alt grid     <- linesMovedUp = 1 because this one line will be lost 5 Line from alt grid } 6
 * Line from alt grid                } These lines will be restored to the alt grid later
 */
- (BOOL)computeRangeFromOriginalLimit:(LineBufferPosition *)originalLimit
    ...
    resultRangePtr->start = [lineBuffer coordinateForPosition:startPos width:newWidth extendsRight:NO ok:NULL];
    ...
    resultRangePtr->end = [lineBuffer coordinateForPosition:endPos width:newWidth extendsRight:YES ok:NULL];
...
- (void)addObjectsToIntervalTreeFromTuples:(NSArray *)altScreenNotes
    ...
    for (NSArray *tuple in altScreenNotes) {
        ...
        if (ok) {
            ...
            Interval *interval = [self intervalForGridAbsCoordRange:VT100GridAbsCoordRangeFromCoordRange(
                                                                        newRange, self.cumulativeScrollbackOverflow)
                                                              width:newSize.width];
...
- (NSArray *)subSelectionsForNewSize:(VT100GridSize)newSize
    ...
    NSMutableArray *newSubSelections = [[NSMutableArray alloc] initWithCapacity:altScreenSubSelectionTuples.count];
    for (int i = 0; i < altScreenSubSelectionTuples.count; i++) {
        ...
        BOOL ok = [self computeRangeFromOriginalLimit:originalLastPos
                                        ...
                                        startPosition:positionRange.start
                                         ...
                                         linesMovedUp:linesMovedUp];
        if (ok) {
            const VT100GridAbsWindowedRange theRange = VT100GridAbsWindowedRangeMake(
                VT100GridAbsCoordRangeFromCoordRange(newSelection, self.cumulativeScrollbackOverflow), 0, 0);
...
- (NSArray *)subSelectionsAfterRestoringPrimaryGridWithCopyOfAltGrid:(VT100Grid *)copyOfAltGrid
    ...
    [self restorePrimaryGridWithLineBuffer:realLineBuffer oldSize:oldSize newSize:newSize];
...
- (void)updateSavedIntervalTreeWithWidth:(int)newWidth
    ...
    for (id<IntervalTreeObject> object in objects) {
        ...
        // BUG-f1294: Replace assert with guard - skip invalid object ranges instead of crashing
        // Object ranges should have non-negative y coordinates after conversion
        if (objectRange.start.y < 0 || objectRange.end.y < 0) {
            DLog(@"BUG-f1294: Invalid objectRange coordinates: start.y=%d, end.y=%d for object: %@",
                 objectRange.start.y, objectRange.end.y, object);
        ...
        DLog(@"  Its new range is %@ including %d lines dropped from top. Remove %@",
             VT100GridCoordRangeDescription(objectRange), numLinesDroppedFromTop, object);
...
- (void)updateAlternateScreenIntervalTreeForNewSize:(VT100GridSize)newSize {
    ...
    [self appendScreen:self.altGrid
        ...
        withUsedHeight:[self.altGrid numberOfLinesUsed]
             newHeight:newSize.height];
    int numLinesThatWillBeRestored = MIN([altScreenLineBuffer numLinesWithWidth:newSize.width], newSize.height);
    ...
    [self updateSavedIntervalTreeWithWidth:newSize.width lineBuffer:altScreenLineBuffer dropped:numLinesDroppedFromTop];
    ...
    // BUG-8308: Use newSize.width (not currentGrid.size.width) since altGrid was resized to newSize
    [self.altGrid restoreScreenFromLineBuffer:altScreenLineBuffer
                              withDefaultChar:[self.altGrid defaultChar]
                            maxLinesToRestore:[altScreenLineBuffer numLinesWithWidth:newSize.width]];
...
- (void)didResizeToSize:(VT100GridSize)newSize
    ...
    if (couldHaveSelection) {
        NSMutableArray *subSelectionsToAdd = [[NSMutableArray alloc] initWithCapacity:newSubSelections.count];
        for (iTermSubSelection *sub in newSubSelections) {
            VT100GridAbsCoordRangeTryMakeRelative(sub.absRange.coordRange, self.cumulativeScrollbackOverflow,
...
- (void)reallySetSize:(VT100GridSize)newSize
    ...
    const VT100GridCoordRange previouslyVisibleLines =
        VT100GridCoordRangeMake(0, previouslyVisibleLineRange.location, 0, previouslyVisibleLineRange.location + 1);
    VT100GridCoord savedCursorPosition = [self.terminal savedCursorPosition];
    savedCursorPosition.y += self.numberOfScrollbackLines;
    const VT100GridAbsCoord absSavedCursorPosition =
        VT100GridAbsCoordFromCoord(savedCursorPosition, self.totalScrollbackOverflow);
    ...
    if (!wasShowingAltScreen && couldHaveSelection) {
        newSubSelections = [self subSelectionsWithConvertedRangesFromSelection:selection newWidth:newSize.width];
    }
    const VT100GridAbsCoord updatedSavedCursorPosition = [self absCoordFromAbsCoord:absSavedCursorPosition
                                                                           newWidth:newSize.width];

    [self fixUpPrimaryGridIntervalTreeForNewSize:newSize wasShowingAltScreen:wasShowingAltScreen];
    ...
    [self.currentGrid restoreScreenFromLineBuffer:wasShowingAltScreen ? altScreenLineBuffer : self.linebuffer
                                ...
                                maxLinesToRestore:[wasShowingAltScreen ? altScreenLineBuffer : self.linebuffer
                                                      numLinesWithWidth:self.currentGrid.size.width]];
    ...
    if (wasShowingAltScreen) {
        ...
        newSubSelections = [self
            subSelectionsAfterRestoringPrimaryGridWithCopyOfAltGrid:copyOfAltGrid
                                                       linesMovedUp:[altScreenLineBuffer
                                                                        numLinesWithWidth:self.currentGrid.size.width]
    ...
    if (updatedSavedCursorPosition.x >= 0) {
        ...
        VT100GridCoord coord =
            VT100GridCoordFromAbsCoord(updatedSavedCursorPosition, self.totalScrollbackOverflow, &ok);
        if (ok) {
...
- (VT100GridAbsCoord)startCoordOfCurrentCommand {
    ...
    return VT100GridAbsCoordMake(lastScreenMark.commandRange.start.x + dx, lastScreenMark.commandRange.start.y + dy);
}
