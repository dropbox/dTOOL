...
- (void)setDelegate:(id<iTermBadgeConfigurationBadgeViewDelegate>)delegate {
    ...
    if (delegate) {
        ...
        _badge.fillColor = [[iTermProfilePreferences colorForKey:KEY_BADGE_COLOR dark:dark
                                                         profile:profile] colorWithAlphaComponent:1];
        _badge.backgroundColor = [NSColor clearColor];
        _loremIpsum.image = [_badge image];
        NSColor *backgroundColor = [iTermProfilePreferences colorForKey:KEY_BACKGROUND_COLOR dark:dark profile:profile];
...
- (iTermBadgeViewEdge)edgeAtPoint:(NSPoint)locationInWindow {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (point.x >= edge.minX && point.x < edge.maxX && point.y >= edge.minY && point.y < edge.maxY) {
            return edge;
        }
    }
...
- (void)mouseDragged:(NSEvent *)event {
    ...
    const NSSize delta = NSMakeSize(point.x - _point.x, point.y - _point.y);
    _point = point;
...
- (void)updateImageViewFrame {
    ...
    // BUG-3136: Guard against division by zero when frame height is zero
    ...
    if (imageAspectRatio > myAspectRatio) {
        ...
        _loremIpsum.frame = NSMakeRect(myFrame.origin.x,
                                       ...
                                       MAX(0, myFrame.size.width), MAX(0, myFrame.size.width / imageAspectRatio));
    } else {
        // image is taller
        _loremIpsum.frame = NSMakeRect(myFrame.size.width - (myFrame.size.height * imageAspectRatio), 0,
                                       myFrame.size.height * imageAspectRatio, myFrame.size.height);
    }
}
...
- (void)layoutSubviews {
    _badge.dirty = YES;
    _badge.viewSize = NSMakeSize(self.superview.bounds.size.width, self.superview.bounds.size.height);
    _loremIpsum.image = [_badge image];
...
- (NSSize)badgeLabelSizeFraction {
    // BUG-3137: Guard against division by zero when superview has zero dimensions
    CGFloat superWidth = self.superview.bounds.size.width;
    CGFloat superHeight = self.superview.bounds.size.height;
    const CGFloat width = (superWidth > 0) ? MIN(0.95, self.bounds.size.width / superWidth) : 0.95;
    const CGFloat height = (superHeight > 0) ? MIN(0.95, self.bounds.size.height / superHeight) : 0.95;
...
- (instancetype)initWithProfile:(Profile *)profile {
    ...
    if (self) {
        ...
        // Badge configuration typically has 5-10 mutation keys
        _profileMutations = [NSMutableDictionary dictionaryWithCapacity:8];
    }
    return self;
}
...
- (Profile *)profileMutations {
    return @{
        KEY_BADGE_MAX_WIDTH : @(MIN(0.95, _maxWidthTextField.doubleValue / 100.0)),
...
- (void)setBadgeFrameFromTextFields {
    ...
    NSRect insetFrame = NSMakeRect(right - width, top - height, width, height);
    _badgeView.frame =
        NSInsetRect(insetFrame, -iTermBadgeConfigurationBadgeViewInset, -iTermBadgeConfigurationBadgeViewInset);
}
...
- (void)badgeViewFrameDidChange:(NSRect)frameIncludingInsets {
    ...
    NSRect frame =
        NSInsetRect(frameIncludingInsets, iTermBadgeConfigurationBadgeViewInset, iTermBadgeConfigurationBadgeViewInset);
    const NSSize containerSize = self.badgeContainerSize;
