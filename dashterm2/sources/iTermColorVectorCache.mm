...
public:
    ...
    // BUG-1592/1597: Helper to safely compare weak colorSpace using isEqual:
    // Returns YES if both are non-nil and equal, NO otherwise.
    bool colorSpaceMatches(NSColorSpace* __weak weakSpace, NSColorSpace* targetSpace) {
        NSColorSpace* strongSpace = weakSpace;  // Promote to strong reference
        if (!strongSpace || !targetSpace) {
            return false;
        }
        return [strongSpace isEqual:targetSpace];
    }
    ...
    bool lookup(iTermColorMapKey key, NSColorSpace* colorSpace, vector_float4& result) {
        ...
        // BUG-1592/1597: Use isEqual: and handle nil weak reference
        if (colorSpaceMatches(slots[0].colorSpace, colorSpace)) {
        ...
        // Try slot 1
        if (colorSpaceMatches(slots[1].colorSpace, colorSpace)) {
    ...
    void store(iTermColorMapKey key, NSColorSpace* colorSpace, vector_float4 value) {
        ...
        } else if (colorSpaceMatches(slots[1].colorSpace, colorSpace)) {
