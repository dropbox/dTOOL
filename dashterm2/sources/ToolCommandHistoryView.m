...
- (instancetype)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        clear_ = [[NSButton alloc]
            initWithFrame:NSMakeRect(0, frame.size.height - kButtonHeight, frame.size.width, kButtonHeight)];
        if (@available(macOS 10.16, *)) {
...
// BUG-397/BUG-3127: Cancel delayed selectors in dealloc to prevent crash if view is
// deallocated while scrollToEndOfDocument: or fixCursor is pending.
- (void)dealloc {
...
- (void)relayout_bigSur {
    ...
    [_scrollView
        setFrame:NSMakeRect(0, searchFieldY, frame.size.width, NSHeight(self.bounds) - searchFieldY - 2 * kMargin)];
...
- (void)relayout_legacy {
    ...
    help_.frame = NSMakeRect(NSMaxX(clear_.frame) + kHelpMargin,
                             ...
                             NSWidth(help_.frame), NSHeight(help_.frame));
...
// BUG-3243: Add bounds check to prevent array index out of bounds crash
- (id)stringOrAttributedStringForColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
    ...
    if ([[aTableColumn identifier] isEqualToString:@"date"]) {
        ...
        return [NSDateFormatter
            compactDateDifferenceStringFromDate:[NSDate dateWithTimeIntervalSinceReferenceDate:commandUse.time
                                                                                                   .doubleValue]];
    } else {
        // Contents
        ...
        if (commandUse.mark && [wrapper.delegate.delegate toolbeltCurrentSessionHasGuid:commandUse.mark.sessionGuid]) {
            return [[NSAttributedString alloc]
                initWithString:value
                    attributes:@{NSFontAttributeName : boldFont_, NSParagraphStyleAttributeName : _paragraphStyle}];
        } else {
            return [[NSAttributedString alloc] initWithString:value
                                                   attributes:@{
                                                       NSFontAttributeName : [NSFont it_toolbeltFont],
...
- (void)doubleClickOnTableView:(id)sender {
    ...
    // BUG-10342: Add upper bounds check before accessing filteredEntries_
...
- (void)computeFilteredEntries {
    ...
    if (searchField_.stringValue.length == 0) {
        ...
        NSMutableArray<iTermCommandHistoryCommandUseMO *> *array =
            [[NSMutableArray alloc] initWithCapacity:entries.count];
