...
- (instancetype)initWithChunkSize:(NSInteger)chunkSize
    ...
    if (self) {
        ...
        // Paste history typically has 5-20 entries
        NSMutableArray *values = [NSMutableArray arrayWithCapacity:16];
        NSMutableArray *labels = [NSMutableArray arrayWithCapacity:16];
...
- (void)getLabels:(NSMutableArray *)labels andValues:(NSMutableArray *)values {
    ...
    for (NSPasteboardItem *item in pasteboard.pasteboardItems) {
        ...
        if (string && ![item stringForType:UTTypeFileURL.identifier]) {
            ...
            NSString *label = [NSString
                stringWithFormat:@"%@: “%@”", [(description ?: @"Unknown Type") stringByCapitalizingFirstLetter],
        ...
        if (!string) {
            ...
            if (!data) {
                for (NSString *typeName in item.types) {
                    if ([typeName hasPrefix:@"public."] && ![typeName isEqualTo:UTTypeFileURL.identifier]) {
    ...
    NSMutableArray *modifiedFilenames = [NSMutableArray arrayWithCapacity:filenames.count];
...
- (void)selectValueAtIndex:(NSInteger)index {
    ...
    NSCharacterSet *theSet = [NSCharacterSet characterSetWithCharactersInString:[NSString shellEscapableCharacters]];
    BOOL containsShellCharacters = [string rangeOfCharacterFromSet:theSet].location != NSNotFound;
    BOOL containsDosNewlines = [string containsString:@"\n"];
    BOOL containsNewlines = containsDosNewlines || [string containsString:@"\r"];
    BOOL containsUnicodePunctuation =
        ([string rangeOfRegex:kPasteSpecialViewControllerUnicodePunctuationRegularExpression].location != NSNotFound);
    BOOL convertValue = [iTermPreferences boolForKey:kPreferenceKeyPasteSpecialConvertDosNewlines];
    BOOL shouldEscape =
        _forceEscapeSymbols || [iTermPreferences boolForKey:kPreferenceKeyPasteSpecialEscapeShellCharsWithBackslash];
    ...
    BOOL shouldBracket = [iTermPreferences boolForKey:kPreferenceKeyPasteSpecialBracketedPasteMode];
    ...
    _pasteSpecialViewController.shouldUseRegexSubstitution =
        !_base64only && [iTermPreferences boolForKey:kPreferencesKeyPasteSpecialUseRegexSubstitution];
    _pasteSpecialViewController.regexString = [iTermPreferences stringForKey:kPreferencesKeyPasteSpecialRegex];
    _pasteSpecialViewController.substitutionString =
        [iTermPreferences stringForKey:kPreferencesKeyPasteSpecialSubstitution];
    _pasteSpecialViewController.enableWaitForPrompt = _canWaitForPrompt;
    _pasteSpecialViewController.shouldWaitForPrompt =
        _isAtShellPrompt && _canWaitForPrompt && [iTermAdvancedSettingsModel advancedPasteWaitsForPromptByDefault];
...
- (void)updatePreview {
    ...
    _statsLabel.stringValue =
        [NSString stringWithFormat:@"%@ byte%@ in %@ line%@.",
                                   [[bytesFormatter stringFromNumber:@(numBytes)] stringWithFirstLetterCapitalized],
                                   numBytes == 1 ? @"" : @"s", [linesFormatter stringFromNumber:@(numberOfLines)],
                                   numberOfLines == 1 ? @"" : @"s"];
    [self updateDuration];
}
...
- (void)saveUserDefaults {
    ...
    if (_pasteSpecialViewController.isUseRegexSubstitutionEnabled) {
        ...
        [iTermPreferences setString:_pasteSpecialViewController.regexString forKey:kPreferencesKeyPasteSpecialRegex];
