...
extension PTYSession: iTermBrowserViewControllerDelegate {
    ...
    func browserViewControllerOnboardingEnableInstantReplay(_ controller: iTermBrowserViewController) {
        ...
        guard let model = isDivorced ? ProfileModel.sessionsInstance() : ProfileModel.sharedInstance() else {
            DLog("Failed to get profile model")
            return
        }
    ...
    func browserViewControllerOnboardingCreateBrowserProfile(_ controller: iTermBrowserViewController) -> String? {
        ...
        // BUG-1645: Use guard instead of force unwrap for GUID generation
        guard let guid = ProfileModel.freshGuid() else {
            DLog("Failed to generate fresh GUID")
            return nil
        }
    ...
    func browserViewControllerOnboardingSwitchToProfile(_ controller: iTermBrowserViewController,
        ...
        guard let origGuid = profile[KEY_GUID] as? String else {
            DLog("Profile missing KEY_GUID")
            return
        }
    ...
    func browserViewController<T>(_ controller: iTermBrowserViewController,
        ...
        return await withCheckedContinuation { (continuation: CheckedContinuation<T?, Never>) in
            ...
            let announcement = iTermAnnouncementViewController.announcement(withTitle: request.message,
                ...
                default:
                    count += 1
                    // BUG-f1413: Validate selection index before accessing options array
                    // selection could be out of bounds if something unexpected happens
                    let index = Int(selection)
                    guard index >= 0 && index < request.options.count else {
                        DLog("BUG-f1413: Invalid selection index \(index) for \(request.options.count) options")
                        continuation.resume(returning: nil)
                        return
                    }
                    continuation.resume(returning: request.options[index].identifier)
                }
            }
...
extension iTermFindMode {
    // BUG-f517: Handle unknown find modes gracefully instead of crashing
    func browserFindMode(query: String) -> iTermBrowserFindMode {
        switch self {
        case .smartCaseSensitivity:
            return (query.rangeOfCharacter(from: CharacterSet.uppercaseLetters) != nil) ? .caseSensitive : .caseInsensitive
        case .caseSensitiveSubstring:
            return .caseSensitive
        case .caseInsensitiveSubstring:
            return .caseInsensitive
        case .caseSensitiveRegex:
            return .caseSensitiveRegex
        case .caseInsensitiveRegex:
            return .caseInsensitiveRegex
        @unknown default:
            // Fall back to case-insensitive for unknown modes
            return .caseInsensitive
        }
    }
}
