...
+ (void)openPartialAttachmentsForArrangement:(NSDictionary *)arrangement
    ...
    if (arrangement[SESSION_ARRANGEMENT_TMUX_PANE] || ![iTermAdvancedSettingsModel runJobsInServers] ||
    ...
    if (generalConnection.type != iTermGeneralServerConnectionTypeMulti) {
        // BUG-463: Replace assert(NO) with ELog and early return
        // Should not happen after getGeneralConnection returns YES, but handle gracefully
        ELog(@"PTYSession.openPartialAttachmentsForArrangement: unexpected connection type %d",
             (int)generalConnection.type);
        completion(@{});
        return;
    }
    ...
    iTermMultiServerJobManager *jobManager = [[iTermMultiServerJobManager alloc] initWithQueue:jobManagerQueue];
    ...
    [jobManager asyncPartialAttachToServer:generalConnection
                             withProcessID:@(generalConnection.multi.pid)
                                completion:^(id<iTermJobManagerPartialResult> partialResult) {
                                    ...
                                    if (!partialResult) {
                                        // BUG-464: Replace assert(NO) with ELog and empty completion
                                        // partialResult can be nil if server died or attachment failed
                                        ELog(@"PTYSession.openPartialAttachmentsForArrangement: partialResult is nil");
                                        completion(@{});
                                        return;
                                    }
                                    DLog(@"PTYSession.openPartialAttachmentsForArrangement: SUCCESS for pid %@",
                                         @(generalConnection.multi.pid));
...
- (iTermJobManagerAttachResults)tryToFinishAttachingToMultiserverWithPartialAttachment:
...
- (void)failWithError:(NSError *)error {
    ...
    NSString *message = [NSString stringWithFormat:@"Cannot start logging to session with profile “%@”: %@",
                                                   self.profile[KEY_NAME], error.localizedDescription];
...
- (void)fetchAutoLogFilenameWithCompletion:(void (^)(NSString *filename))completion {
    ...
    NSString *filenameInterpolatedString = [iTermProfilePreferences stringForKey:KEY_LOG_FILENAME_FORMAT
...
- (NSString *)joinedNameWithFolder:(NSString *)formattedFolder filename:(NSString *)formattedFilename {
...
- (void)watchForPasteBracketingOopsieWithPrefix:(NSString *)prefix andWrite:(NSString *)string {
...
- (void)maybeTurnOffPasteBracketing {
    NSNumber *number =
        [[NSUserDefaults standardUserDefaults] objectForKey:kTurnOffBracketedPasteOnHostChangeUserDefaultsKey];
    if (number.boolValue) {
