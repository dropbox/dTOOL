...
- (void)addChild:(iTermScriptItem *)child {
    if (!_children) {
        _children = [NSMutableArray arrayWithCapacity:8]; // Typical script folder child count
    }
...
- (instancetype)initWithMenu:(NSMenu *)menu {
    self = [super init];
    if (self) {
        _allScripts = [NSMutableArray arrayWithCapacity:32]; // Typical script count
        _scriptsMenu = menu;
...
- (NSInteger)separatorIndex {
    return [_scriptsMenu.itemArray
        indexOfObjectPassingTest:^BOOL(NSMenuItem *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (NSArray<NSString *> *)allScriptsFromMenu {
    ...
    NSMutableArray<NSString *> *result = [NSMutableArray arrayWithCapacity:32]; // Typical script count
    if (i != NSNotFound) {
...
- (void)addMenuItemsIn:(NSMenu *)container
             fromIndex:(NSInteger)fromIndex
               toArray:(NSMutableArray<NSString *> *)result
                  path:(NSString *)path {
    for (NSInteger i = fromIndex; i < container.itemArray.count; i++) {
        NSMenuItem *item = container.itemArray[i];
        if (item.submenu) {
            [self addMenuItemsIn:item.submenu
                       fromIndex:0
                         toArray:result
                            path:[path stringByAppendingPathComponent:item.title]];
...
- (NSArray<iTermScriptItem *> *)scriptItems {
    ...
    [self populateScriptItem:root originalRoot:path clockWatcher:[[iTermClockWatcher alloc] initWithMaxTime:8.0]];
...
- (void)populateScriptItem:(iTermScriptItem *)parentFolderItem
    ...
    for (NSString *file in directoryEnumerator) {
        if (clockWatcher.reachedMaxTime) {
            iTermWarningSelection selection = [iTermWarning
                showWarningWithTitle:
                    [NSString
                        stringWithFormat:@"It is taking a long time to locate all scripts under %@. Avoid storing many "
                                         @"files or using network mounts for the scripts folder.\n\nContinue?",
                                         originalRoot]
            ...
            if (selection == kiTermWarningSelection0) {
                ...
                [iTermWarning
                    showWarningWithTitle:[NSString stringWithFormat:@"Some scripts will not be available until the app "
                                                                    @"has restarted or you change the scripts folder."]
                                 actions:@[ @"OK" ]
                               accessory:nil
        ...
        if (isDirectory) {
            ...
            if ([workspace isFilePackageAtPath:path] || [iTermAPIScriptLauncher environmentForScript:path
                                                                                        checkForMain:NO
                                                                                       checkForSaved:YES]) {
                [parentFolderItem addChild:[[iTermScriptItem alloc] initFullEnvironmentWithPath:path
...
- (void)didFindScriptArchive:(NSString *)file autolaunch:(BOOL)autolaunch {
    ...
    dispatch_once(&onceToken, ^{
        // Scripts are typically a handful per installation
    ...
    // BUG-3763: Validate that the file path doesn't contain path traversal attempts
    NSString *normalizedPath = file.stringByStandardizingPath;
    NSString *scriptsPath = [[NSFileManager defaultManager] scriptsPath].stringByStandardizingPath;
    NSString *scriptsPathNoSpaces = [[NSFileManager defaultManager] scriptsPathWithoutSpaces].stringByStandardizingPath;
    if (![normalizedPath hasPrefix:scriptsPath] && ![normalizedPath hasPrefix:scriptsPathNoSpaces]) {
        DLog(@"BUG-3763: Rejected script archive outside of scripts directory: %@", file);
        return;
    }

    [alreadyFound addObject:file];

    // BUG-3763: Enhanced warning about script execution risks
    NSString *warningTitle =
        [NSString stringWithFormat:@"A script archive named \"%@\" was found in the Scripts directory.\n\n"
                                   @"Warning: Scripts can execute arbitrary code on your system. "
                                   @"Only install scripts from sources you trust.\n\n"
                                   @"Would you like to install it?",
                                   file.lastPathComponent];

    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:warningTitle
                                   actions:@[ @"Install", @"Cancel", @"Move to Trash" ]
                                 accessory:nil
                    ...
                    offerAutoLaunch:autolaunch
                      callbackQueue:dispatch_get_main_queue()
                         ...
                         completion:^(NSString *_Nullable errorMessage, BOOL quiet, NSURL *location) {
                             ...
                             [self importDidFinishWithErrorMessage:errorMessage location:location originalURL:url];
                         }];
            break;
        }
...
- (void)addMenuItemsTo:(NSMenu *)rootMenu {
    [self addMenuItemsForScriptItems:[self scriptItems] toMenu:rootMenu];
}

- (void)addMenuItemsForScriptItems:(NSArray<iTermScriptItem *> *)unsortedScriptItems toMenu:(NSMenu *)containingMenu {
...
- (void)chooseAndExportScript {
    ...
    [iTermScriptChooser
        chooseMultipleWithValidator:^BOOL(NSURL *url) {
            return [url.path.stringByResolvingSymlinksInPath isEqualToString:autoLaunchPath] ||
                   [iTermScriptExporter urlIsScript:url];
        }
        autoLaunchByDefault:NO
        completion:^(NSArray<NSURL *> *urls, SIGIdentity *signingIdentity, BOOL autolaunch) {
            if (!urls) {
                return;
            }
            for (NSURL *url in urls) {
                [iTermScriptExporter
                    exportScriptAtURL:url
...
- (void)importDidFinishWithErrorMessage:(nullable NSString *)errorMessage
    ...
    if (errorMessage) {
        ...
        if (response == NSAlertSecondButtonReturn) {
            [self launchScriptWithAbsolutePath:location.path arguments:@[] explicitUserAction:YES];
        }
    }
}
...
- (NSString *)autoLaunchPathIfFullPathWereMovedToAutoLaunch:(NSString *)fullPath {
    return
        [[iTermScriptsMenuController autolaunchScriptPath] stringByAppendingPathComponent:fullPath.lastPathComponent];
}
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:[iTermScriptsMenuController autolaunchScriptPath]
                              withIntermediateDirectories:YES
                                               attributes:nil
                                                    error:nil];
    NSString *destination = [self autoLaunchPathIfFullPathWereMovedToAutoLaunch:fullPath];
    [[NSFileManager defaultManager] moveItemAtPath:fullPath toPath:destination error:nil];
}
...
- (void)launchOrTerminateScript:(NSMenuItem *)sender {
    ...
    if (entry) {
        ...
        [self launchScriptWithAbsolutePath:fullPath arguments:@[] explicitUserAction:YES];
    }
}
...
- (void)launchScriptWithRelativePath:(NSString *)path
    ...
    NSString *fullPath =
        [[[NSFileManager defaultManager] scriptsPathWithoutSpaces] stringByAppendingPathComponent:path];
    DLog(@"fullPath=%@ args=%@ explicitUserAction=%@", fullPath, arguments, @(explicitUserAction));
    [self launchScriptWithAbsolutePath:fullPath arguments:arguments explicitUserAction:explicitUserAction];
}
...
- (void)launchScriptWithAbsolutePath:(NSString *)fullPath
    ...
    NSString *venv = [iTermAPIScriptLauncher environmentForScript:fullPath checkForMain:YES checkForSaved:YES];
    if (venv) {
        ...
        NSString *mainPyPath = [[[fullPath stringByAppendingPathComponent:name] stringByAppendingPathComponent:name]
    ...
    if ([[fullPath pathExtension] isEqualToString:@"py"]) {
        ...
        [iTermAPIScriptLauncher launchScript:fullPath arguments:arguments explicitUserAction:explicitUserAction];
        return;
    }
    if ([[fullPath pathExtension] isEqualToString:@"scpt"]) {
        ...
        [script executeWithAppleEvent:nil
                    completionHandler:^(NSAppleEventDescriptor *_Nullable result, NSError *_Nullable error) {
                        if (error) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [self showAlertForScript:fullPath error:error];
                            });
    ...
    if ([[NSFileManager defaultManager] itemIsDirectory:fullPath]) {
        iTermWarningSelection selection =
            [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"The script “%@” is malformed.",
...
- (NSString *)pathToTemplateForPicker:(iTermScriptTemplatePickerWindowController *)picker {
    ...
    NSString *templateName =
        [NSString stringWithFormat:@"template_%@_%@", environmentNamePart[picker.selectedEnvironment],
...
- (void)newPythonScript {
    ...
    [downloader
        downloadOptionalComponentsIfNeededWithConfirmation:YES
                                             pythonVersion:nil
...
- (void)reallyCreateNewPythonScript {
    DLog(@"begin");
    iTermScriptTemplatePickerWindowController *picker = [[iTermScriptTemplatePickerWindowController alloc]
        initWithWindowNibName:@"iTermScriptTemplatePickerWindowController"];
                                     ...
                                     minimumEnvironmentVersion:0
                                            requiredToContinue:YES
                                                withCompletion:^(iTermPythonRuntimeDownloaderStatus status) {
                                                    ...
                                                    [self reallyCreateNewPythonScriptAtURL:url
                                                                                    picker:picker
                                                                              dependencies:dependencies
                                                                             pythonVersion:pythonVersion];
                                                }];
...
- (void)reallyCreateNewPythonScriptAtURL:(NSURL *)url
    ...
    DLog(@"url=%@ deps=%@ pythonVersion=%@ selectedEnvironment=%@", url, dependencies, pythonVersion,
         @(picker.selectedEnvironment));
    if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
        ...
        NSURL *existingEnv = [folder URLByAppendingPathComponent:@"dashterm2env"];
                            ...
                            completion:^(NSError *errorStatus) {
                                if (errorStatus != nil) {
                                    ...
                                    alert.informativeText = [NSString
                                        stringWithFormat:@"An error ocurred while installing the Python runtime. "
                                                         @"Remove ~/Library/Application Support/DashTerm2/dashterm2env "
                                                         @"and try again. The error was: %@",
                                                         errorStatus.localizedDescription];
                                    [alert runModal];
                                    return;
                                }
...
- (void)finishInstallingNewPythonScriptForPicker:(iTermScriptTemplatePickerWindowController *)picker url:(NSURL *)url {
    ...
    if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
        [[NSFileManager defaultManager]
                  createDirectoryAtPath:[url.path stringByAppendingPathComponent:url.path.lastPathComponent]
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:nil];
    }
    ...
    NSString *app = nil;
    NSURL *appURL =
        [[NSWorkspace sharedWorkspace] URLForApplicationToOpenURL:[NSURL fileURLWithPath:destinationTemplatePath]];
    if (appURL) {
        app = [[NSFileManager defaultManager] displayNameAtPath:appURL.path];
    }
    if (app) {
        iTermWarningSelection selection =
            [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"Open new script in %@?", app]
...
- (NSView *)newAccessoryViewForSavePanelWithTokenField:(NSTokenField *)tokenField
    ...
    NSView *accessoryView =
        [[NSView alloc] initWithFrame:NSMakeRect(0, 0, NSMaxX(tokenField.frame) + margin + tokenFieldWidth, 32)];
    ...
    NSDictionary *metrics = @{@"sideMargin" : @(sideMargin), @"verticalMargin" : @(verticalMargin)};
    ...
    [accessoryView
        addConstraints:[NSLayoutConstraint
                           constraintsWithVisualFormat:
                               @"V:|-verticalMargin-[label]-verticalMargin-[pythonVersionPopup]-verticalMargin-|"
                                               options:0
                                               metrics:metrics
                                                 views:views]];
    ...
    [accessoryView addConstraint:[NSLayoutConstraint constraintWithItem:pythonVersionPopup
                                                             ...
                                                             multiplier:1
                                                               constant:5]];
    [pythonVersionPopup setContentCompressionResistancePriority:NSLayoutPriorityRequired
                                                 forOrientation:NSLayoutConstraintOrientationHorizontal];
    [tokenField setContentCompressionResistancePriority:NSLayoutPriorityRequired
                                         forOrientation:NSLayoutConstraintOrientationHorizontal];
...
- (NSString *)templateForPicker:(iTermScriptTemplatePickerWindowController *)picker url:(NSURL *)url {
...
- (nullable NSString *)relativePathFrom:(NSString *)possibleSuper
                                 toPath:(NSString *)possibleSub
...
- (NSString *)folderForFullEnvironmentSavePanelURL:(NSURL *)url {
    ...
    NSString *relative = [self relativePathFrom:scriptsRoot toPath:selectedPath];
    if (relative) {
...
- (NSString *)destinationTemplatePathForPicker:(iTermScriptTemplatePickerWindowController *)picker url:(NSURL *)url {
    if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
        ...
        // ~/Library/ApplicationSupport/DashTerm2/Scripts/foo/bar/setup.cfg
        // ~/Library/ApplicationSupport/DashTerm2/Scripts/foo/bar/dashterm2env
        // ~/Library/ApplicationSupport/DashTerm2/Scripts/foo/bar/bar/
        // ~/Library/ApplicationSupport/DashTerm2/Scripts/foo/bar/bar/bar.py
        return [[folder stringByAppendingPathComponent:name]
            stringByAppendingPathComponent:[url.path.lastPathComponent stringByAppendingPathExtension:@"py"]];
    } else {
        return url.path;
    }
}
...
- (NSString *_Nullable)pythonVersionForPicker:(iTermScriptTemplatePickerWindowController *)picker url:(NSURL *)url {
    NSString *raw;
    if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
...
- (void)showAlertForScript:(NSString *)fullPath error:(NSError *)error {
    ...
    alert.informativeText =
        [NSString stringWithFormat:@"The script at “%@” failed:\n\n%@", fullPath, error.localizedFailureReason];
    [alert runModal];
}
...
- (BOOL)panel:(id)sender validateURL:(NSURL *)url error:(NSError **)outError {
    ...
    NSString *message = [NSString
        stringWithFormat:
            @"Full-environment scripts must be located in your Application Support/DashTerm2/Scripts directory:\n%@",
