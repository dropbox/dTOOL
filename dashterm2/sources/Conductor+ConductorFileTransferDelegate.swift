...
extension Conductor: ConductorFileTransferDelegate {
    func beginDownload(fileTransfer: ConductorFileTransfer) {
        ...
        // BUG-1675: Use guard instead of force unwrap for URL path
        guard let remotePath = fileTransfer.path.path else {
            fileTransfer.fail(reason: "Invalid remote path")
            return
        }
    ...
    class DownloadState {
        ...
        var shouldFetchMore: Bool {
            ...
            // BUG-1687: Use guard instead of force unwrap for remoteFile.size
            guard let fileSize = remoteFile.size else {
                return false
            }
        ...
        func addTask(conductor: Conductor) async throws {
                ...
                task: Task { @MainActor in
                    ...
                    counter.perform(.post) {
                        if data.isEmpty {
                            ...
                            // BUG-1687: Use nil-coalescing instead of force unwrap for remoteFile.size
                            let fileSize = remoteFile.size ?? 1
                            progress?.fraction = min(1.0, max(0, Double(offset) / Double(fileSize)))
                        }
                        fileTransfer.didTransferBytes(UInt(data.count))
                    }
                    return data
            ...
            while tasks.count >= maxConcurrency || (!tasks.isEmpty && !shouldFetchMore) {
                ...
                if result.isEmpty && !tasks.isEmpty {
                    // BUG-1687: Use nil-coalescing instead of force unwrap for remoteFile.size
                    let expectedSize = remoteFile.size ?? 0
                    throw ConductorFileTransfer.ConductorFileTransferError(
                        "Download ended prematurely (received \(content.count) of \(expectedSize) byte\(expectedSize == 1 ? "" : "s")")
                }
            }
        }
    }
    ...
    private func reallyBeginUpload(fileTransfer: ConductorFileTransfer,
        ...
        do {
            ...
            // Find a good name
            // BUG-1675: Use guard instead of force unwrap for URL path
            guard let basePath = fileTransfer.path.path else {
                fileTransfer.fail(reason: "Invalid remote path")
                return
            }
            var proposedName = basePath
