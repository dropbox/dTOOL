...
class ComposerTextView: MultiCursorTextView {
    ...
    @objc var suggestion: String? {
        ...
        set {
            // BUG-1627: Use guard instead of precondition - re-entrancy should be prevented gracefully
    ...
    private func showCompletionsWindow(completions: [CompletionItem]?,
        ...
        if let completions, let prefix, let font {
    ...
    private var completionsActions: [Action] {
        ...
        // BUG-f980: Use safe optional binding instead of force unwrap for Unicode.Scalar
        // NSUpArrowFunctionKey (0xF700) and NSDownArrowFunctionKey (0xF701) are valid Unicode scalars,
        // but we handle the edge case gracefully by filtering out nil actions
        var actions: [Action] = []
        if let upArrowScalar = Unicode.Scalar(NSUpArrowFunctionKey) {
            actions.append(Action(modifiers: [],
                   characters: String(upArrowScalar),
                   closure: { [weak self] _, _ in
                       self?.completionsWindowIfVisible?.up()
                       return true
                   }))
        }
        if let downArrowScalar = Unicode.Scalar(NSDownArrowFunctionKey) {
            actions.append(Action(modifiers: [],
                   characters: String(downArrowScalar),
                   closure: { [weak self] _, _ in
    ...
    // BUG-1678: Return optional URL to handle potential failure
    ...
    private func updateLink(with event: NSEvent) {
        ...
        if let (range, command) = characterRangeOfCommand(at: point) {
            if range != linkRange {
                removeLink()
                // BUG-1678: Handle optional URL from url(for:)
                if let linkURL = url(for: command) {
                    makeLink(range, url: linkURL)
                }
            }
        } else {
            removeLink()
    ...
    private func characterRangeOfCommand(atCharacterIndex unsafeIndex: Int) -> (NSRange, String)? {
        ...
        while true {
            ...
            } else if let currentSpan = spanningRange {
                // BUG-1658: Use if let instead of force unwrap for spanningRange
                spanningRange = NSRange(from: currentSpan.location,
        ...
        if let suffixRange = spanningRange, suffixRange.location > 0 {
            ...
            // BUG-1658: Use guard let instead of force unwrap in while condition
            while let currentSpan = spanningRange, currentSpan.location > 0 {
                let string = (textStorage.string as NSString)
                let paragraphRange = string.paragraphRange(for: NSRange(location: characterIndex, length: 0))
                if paragraphRange.location == NSNotFound || paragraphRange.length == 0 {
                    return nil
                ...
                // BUG-1658: Use guard with if let instead of force unwrap
                if let currentSpan = spanningRange {
                    spanningRange = NSRange(from: paragraphRange.location,
                                            to: currentSpan.upperBound)
                }
        ...
        // BUG-1658: Use guard with as? instead of force cast for mutableCopy()
        guard let temp = ((textStorage.string as NSString).substring(with: rangeExcludingSuggestion) as NSString).mutableCopy() as? NSMutableString else {
            return nil
        }
...
extension NSRange {
    ...
    func minus(_ rhs: NSRange) -> NSRange {
        ...
        // BUG-1770: Use guard let instead of force unwrap for intersection
        guard let inter = intersection(rhs), inter.length > 0 else {
            // If no intersection exists (shouldn't happen given previous checks), return self unchanged
            return self
        }
        if self == inter {
            // Remove all of lhs.
...
extension NSAttributedString.Key {
    static let suggestionKey = NSAttributedString.Key(rawValue: "com.dashterm.dashterm2.ComposerTextView.suggestionKey")
    static let promptKey = NSAttributedString.Key(rawValue: "com.dashterm.dashterm2.ComposerTextView.promptKey")
}
...
extension String {
    func index(fromUtf16 i: Int) -> Index? {
        guard let utf16ViewIndex = self.utf16.index(self.utf16.startIndex,
                                                     offsetBy: i,
                                                     limitedBy: self.utf16.endIndex) else {
            return nil
        }
