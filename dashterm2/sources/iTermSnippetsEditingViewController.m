...
static NSString *const iTermSnippetsEditingPasteboardType =
    @"com.dashterm.dashterm2.iTermSnippetsEditingPasteboardType";
...
- (void)awakeFromNib {
    ...
    [menu addItem:[[NSMenuItem alloc] initWithTitle:@"Duplicate"
                                      ...
                                      keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:@"Delete" action:@selector(deleteSnippets:) keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:@"Add Above" action:@selector(addSnippetAbove:) keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:@"Add Below" action:@selector(addSnippetBelow:) keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:@"Edit" action:@selector(editClickedSnippet:) keyEquivalent:@""]];
    _tableView.menu = menu;
}

- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    if (menuItem.action == @selector(duplicateSnippets:) || menuItem.action == @selector(deleteSnippets:)) {
        return [self targetSnippetIndexesForContextMenuAction].count > 0;
    }
    if (menuItem.action == @selector(editClickedSnippet:) || menuItem.action == @selector(addSnippetAbove:) ||
...
- (void)addSnippetAtIndex:(NSInteger)i {
    [self pushUndo];
    _windowController = [[iTermEditSnippetWindowController alloc]
        initWithSnippet:nil
    ...
    [self.view.window beginSheet:_windowController.window
...
// BUG-3105: Add bounds check to prevent array index out of bounds crash
// The data source may change between validateMenuItem and action execution.
- (void)editClickedSnippet:(id)sender {
    const NSInteger i = _tableView.clickedRow;
    NSArray<iTermSnippet *> *snippets = _filteredSnippets;
    if (i < 0 || i >= (NSInteger)snippets.count) {
        return;
    }
    [self editSnippet:snippets[i]];
}
...
- (NSIndexSet *)filteredIndexSet:(NSIndexSet *)input {
    ...
    NSMutableSet<NSString *> *guids = [NSMutableSet setWithCapacity:input.count];
...
- (void)pushUndo {
    // BUG-1201: Deep copy snippets array to prevent undo state corruption from in-place mutations
    NSArray *snippetsCopy = [[NSArray alloc] initWithArray:_allSnippets copyItems:YES];
    [[self undoManager] registerUndoWithTarget:self selector:@selector(setSnippets:) object:snippetsCopy];
}
...
- (void)snippetsDidChange:(iTermSnippetsDidChangeNotification *)notif {
    switch (notif.mutationType) {
            ...
            if (removeIndex != NSNotFound || insertionIndex != NSNotFound) {
                [_tableView it_performUpdateBlock:^{
                    if (removeIndex != NSNotFound) {
                        [_tableView removeRowsAtIndexes:[NSIndexSet indexSetWithIndex:removeIndex] withAnimation:YES];
                    }
                    if (insertionIndex != NSNotFound) {
                        [_tableView insertRowsAtIndexes:[NSIndexSet indexSetWithIndex:insertionIndex] withAnimation:NO];
                    }
                }];
            }
        }
        case iTermSnippetsDidChangeMutationTypeDeletion: {
            ...
            [self load];
            [_tableView it_performUpdateBlock:^{
                [_tableView removeRowsAtIndexes:indexes withAnimation:YES];
            }];
            break;
        }
        case iTermSnippetsDidChangeMutationTypeInsertion: {
            ...
            if ([_filteredSnippets containsObject:_allSnippets[notif.index]]) {
                [_tableView it_performUpdateBlock:^{
                    [_tableView insertRowsAtIndexes:[NSIndexSet indexSetWithIndex:i] withAnimation:YES];
                }];
            }
            break;
        }
...
- (IBAction)add:(id)sender {
    _windowController =
        [[iTermEditSnippetWindowController alloc] initWithSnippet:nil
                                                       completion:^(iTermSnippet *_Nullable snippet) {
                                                           if (!snippet) {
                                                               return;
                                                           }
...
- (IBAction)import:(id)sender {
    ...
    [panel beginWithFallbackWindow:self.view.window
...
- (void)importURL:(NSURL *)url {
    NSError *error = nil;
    NSString *content = [NSString stringWithContentsOfFile:url.path encoding:NSUTF8StringEncoding error:&error];
    if (!content || error) {
        [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"While loading %@: %@", url.path,
    ...
    if (!root) {
        [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"While parsing %@: %@", url.path,
                                                                      error.localizedDescription]
                                   actions:@[ @"OK" ]
                                 accessory:nil
                                identifier:@"NoSyncImportSnippetsFailed"
...
- (void)exportToItem:(iTermSavePanelItem *)item {
    ...
    NSMutableArray<NSDictionary *> *array = [[NSMutableArray alloc] initWithCapacity:indexes.count];
    [indexes enumerateIndexesUsingBlock:^(NSUInteger i, BOOL *_Nonnull stop) {
        // BUG-10368: Bounds check - selection indices may be stale after data changes
        if (i >= _filteredSnippets.count) {
            return;
        }
    ...
    [json writeToSaveItem:item
        completionHandler:^(NSError *error) {
            if (error) {
                [iTermWarning
                    showWarningWithTitle:[NSString stringWithFormat:@"Error saving to %@: %@", item.displayName,
                                                                    error.localizedDescription]
                                 actions:@[ @"OK" ]
                               accessory:nil
...
- (NSView *)viewForTitleColumnOnRow:(NSInteger)row {
    // BUG-10369: Bounds check - row may be stale after data changes
    if (row < 0 || row >= (NSInteger)_filteredSnippets.count) {
        return nil;
    }
    ...
    NSDictionary *attributes = @{NSFontAttributeName : font, NSForegroundColorAttributeName : NSColor.textColor};
    NSAttributedString *titleAttributedString =
        [NSAttributedString attributedStringWithString:[@" " stringByAppendingString:trimmedTitle]
                                            attributes:attributes];
    titleAttributedString = [titleAttributedString highlightMatchesForQuery:self.query phraseIdentifier:@"title:"];

    if (_filteredSnippets[row].tags.count) {
        NSArray<NSAttributedString *> *substrings =
            [_filteredSnippets[row].tags flatMapWithBlock:^id _Nullable(NSString *_Nonnull tag) {
                NSDictionary *attributes = @{
                    NSForegroundColorAttributeName : NSColor.whiteColor,
                    NSBackgroundColorAttributeName : [NSColor colorWithSRGBRed:0.97 green:0.47 blue:0.10 alpha:1.0],
                    NSFontAttributeName : font
                };
                NSAttributedString *tagString =
                    [NSAttributedString attributedStringWithString:[NSString stringWithFormat:@" %@ ", tag]
                                                        attributes:attributes];
                NSAttributedString *space =
                    [NSAttributedString attributedStringWithString:@" " attributes:@{NSFontAttributeName : font}];
...
- (NSView *)viewForValueColumnOnRow:(NSInteger)row {
    // BUG-10370: Bounds check - row may be stale after data changes
    if (row < 0 || row >= (NSInteger)_filteredSnippets.count) {
        return nil;
    }
    NSFont *font = [NSFont systemFontOfSize:[NSFont systemFontSize]];
    ...
    if (self.query.length > 0) {
        const NSRange range =
            [_filteredSnippets[row].value rangeOfString:self.query
    ...
    NSDictionary *attributes = @{NSFontAttributeName : font, NSForegroundColorAttributeName : NSColor.textColor};
    NSAttributedString *unhighlightedAttributedString = [NSAttributedString attributedStringWithString:string
                                                                                            attributes:attributes];
...
- (id<NSPasteboardWriting>)tableView:(NSTableView *)tableView pasteboardWriterForRow:(NSInteger)row {
    // BUG-10371: Bounds check - row may be stale during drag operations
    if (row < 0 || row >= (NSInteger)_filteredSnippets.count) {
        return nil;
    }
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    [self pushUndo];
    NSMutableArray<NSString *> *allGuids = [[NSMutableArray alloc] initWithCapacity:8];
    [info
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *_Nonnull draggingItem, NSInteger idx, BOOL *_Nonnull stop) {
                                   ...
                                   NSArray<NSString *> *guids =
                                       [item propertyListForType:iTermSnippetsEditingPasteboardType];
                                   [allGuids addObjectsFromArray:guids];
                               }];
    [[iTermSnippetsModel sharedInstance] moveSnippetsWithGUIDs:allGuids toIndex:[self unfilteredIndex:row]];
    return YES;
}
