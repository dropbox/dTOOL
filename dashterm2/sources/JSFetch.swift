...
class PluginClient {
    ...
    func call<RequestType: Codable & CustomDebugStringConvertible,
        ...
        if let request {
            ...
            // BUG-1593: Use guard to safely convert data to string instead of force unwrap
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                throw PluginError(reason: "Could not convert request data to UTF8 string")
            }
            args = [jsonString]
        } else {
            args = []
    ...
    private func performHTTPRequest(method: String,
        ...
        // BUG-2001: Invalidate previous session before creating new one to prevent delegate retention leak
    ...
    private func registerFunctions(context: JSContext, stream: ((String) -> ())?) {
        ...
        // BUG-7202: Use weak context reference to avoid retain cycle.
        // The cycle was: context -> performRequest block -> inner closure -> context
        ...
        let performRequest: @convention(block) (String, String, [String: String], Any, JSValue) -> Void = { [weak self]
            ...
            guard let self else {
                // Callback order: (data, error) - nil data with error message
                let noData: String? = nil
                callback.call(withArguments: [noData as Any, "Plugin client deallocated" as Any])
                return
            }
            ...
            guard let bodyData else {
                // Callback order: (data, error) - nil data with error message
                let noData: String? = nil
                callback.call(withArguments: [noData as Any, "Invalid body" as Any])
                return
            }
            self.performHTTPRequest(method: method,
                ...
                if error == nil, let string, stream != nil {
                    if let ctx = weakContext,
                       let streamCallback = ctx.objectForKeyedSubscript("onStream"),
                       !streamCallback.isUndefined {
                        streamCallback.call(withArguments: [string as Any])
                    }
                    return
                }
                // Callback order: (data, error)
                callback.call(withArguments: [string as Any, error as Any])
            }
        }
    ...
    private func callAsync<ResponseType: Codable>(code: String,
        ...
        // BUG-1793: Use guard let instead of force unwrap for JSValue creation
        guard let thenJSValue = JSValue(object: thenClosure, in: context),
              let catchJSValue = JSValue(object: catchClosure, in: context) else {
            throw PluginError(reason: "Failed to create JSValue for promise handlers")
        }
        result.invokeMethod("then", withArguments: [thenJSValue])
        result.invokeMethod("catch", withArguments: [catchJSValue])
