...
extension MessagePrepPipeline {
    ...
    func handleMultipartUserMessage(userMessage: Message,
        ...
        // BUG-f514: Return false for non-multipart content instead of crashing
        default:
            return false
        }
        return true
...
extension MessagePrepPipeline {
        ...
        return parts.compactMap { subpart -> LLM.Message.Attachment.AttachmentType.File? in
            ...
            case .attachment(let attachment):
                ...
                case .code, .statusUpdate:
                    // BUG-442: Replace it_fatalError with nil return - unsupported type should not crash
                    DLog("MessagePrepPipeline: Unsupported user-sent attachment type \(attachment.type). Skipping.")
                    return nil
                case .file(let file):
    ...
    func createVectorStoreAction(chatID: String) throws -> Pipeline<PipelineResult>.Action.Closure {
        return { [weak self] _, completion in
            ...
            delegate?.createVectorStore(
                name: "DashTerm2.\(chatID)") { [weak self] result in
    ...
    func uploadZipAction(zipID: UUID, chatID: String) -> Pipeline<PipelineResult>.Action.Closure {
        return { [weak self] priorValues, completion in
            // BUG-1803: Use guard let to safely unwrap dictionary value instead of force unwrap
            guard let priorValue = priorValues[zipID],
                  case let .zipCreated(url, data) = priorValue else {
                // BUG-443: Replace it_fatalError with completion callback - missing zip should not crash
                DLog("MessagePrepPipeline.uploadZipAction: Missing or invalid zip result for zipID \(zipID). Completing with error.")
                try? completion(.failure(NSError(domain: "MessagePrepPipeline", code: -1, userInfo: [NSLocalizedDescriptionKey: "Missing or invalid zip result"])))
                return
            }
            self?.delegate?.uploadFile(
                name: Self.zipName,
