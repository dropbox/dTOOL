...
#define PACKED_CSI_COMMAND(prefix, intermediate, final) (((prefix) << 16) | ((intermediate) << 8) | (final))
...
static BOOL AdvanceAndEatControlChars(iTermParserContext *context, BOOL support8BitControlCharacters,
                                      CVector *incidentals) {
...
static BOOL ParseCSIPrologue(iTermParserContext *context, BOOL support8BitControlCharacters, CVector *incidentals) {
    if (support8BitControlCharacters && iTermParserPeek(context) == VT100CC_C1_CSI) {
        ...
        // BUG-f1377: Convert assert to guard - malformed input could fail to advance
        if (!iTermParserCanAdvance(context) || iTermParserPeek(context) != '[') {
            return NO;
        }
    }
    return AdvanceAndEatControlChars(context, support8BitControlCharacters, incidentals);
}

static BOOL ParseCSIPrefix(iTermParserContext *context, BOOL support8BitControlCharacters, CVector *incidentals,
                           CSIParam *param) {
...
static BOOL ParseCSIParameters(iTermParserContext *context, BOOL support8BitControlCharacters, CVector *incidentals,
                               CSIParam *param, BOOL *unrecognized) {
    //     2. parse parameters
        ...
        switch (c) {
            ...
            case '9': {
                ...
                if (isSub && param->count > 0) {
                    ...
                    // BUG-f1362: Convert assert to guard - malformed CSI input could cause crash
                    if (paramNum < 0 || paramNum >= VT100CSIPARAM_MAX) {
                        *unrecognized = YES;
                        break;
                    }
...
static BOOL ParseCSIIntermediate(iTermParserContext *context, BOOL support8BitControlCharacters, CVector *incidentals,
                                 CSIParam *param) {
...
static BOOL ParseCSIGarbage(iTermParserContext *context, BOOL support8BitControlCharacters, CVector *incidentals,
                            BOOL *unrecognized) {
    // compatibility HACK:
    //
    // CSI P...P I...I (G...G) F
...
static void ParseCSISequence(iTermParserContext *context, BOOL support8BitControlCharacters, CSIParam *param,
                             CVector *incidentals) {
