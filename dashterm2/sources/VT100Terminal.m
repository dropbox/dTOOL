...
NSString *const kTerminalStateURL_DEPRECATED =
    @"URL"; // This was stored as an NSURL (which didn't work at all) and getting an URL code from an URL is hard
            // (because it depends on the order of state restoration) and ultimately it's not that important to preserve
            // this bit of state so it has been abandoned.
...
- (instancetype)init {
    ...
    if (self) {
        ...
        // Key reporting mode stacks are typically small (1-4 entries)
        _mainKeyReportingModeStack = [[NSMutableArray alloc] initWithCapacity:4];
        _alternateKeyReportingModeStack = [[NSMutableArray alloc] initWithCapacity:4];
        numLock_ = YES;
        mainSavedCursor_.invalid = YES;
        altSavedCursor_.invalid = YES;
        // Unicode version stack is typically small (1-2 entries)
        _unicodeVersionStack = [[NSMutableArray alloc] initWithCapacity:2];
...
- (void)setTermType:(NSString *)termtype {
    ...
    self.isAnsi =
        [_termType rangeOfString:@"ANSI" options:NSCaseInsensitiveSearch | NSAnchoredSearch].location != NSNotFound;
...
- (void)resetForSSH {
    [self resetAllowingResize:NO preservePrompt:YES resetParser:NO preserveSSH:YES modifyContent:NO];
}

- (void)resetForRelaunch {
    [self finishResettingParser:YES preserveSSH:NO preservePrompt:NO modifyContent:NO];
}
...
- (void)resetForTmuxUnpause {
    [self finishResettingParser:YES preserveSSH:YES preservePrompt:NO modifyContent:YES];
}
...
- (void)updateExternalAttributes {
    if (!graphicRendition_.hasUnderlineColor && _currentURL == nil && self.currentBlockIDList == nil &&
        _currentLiteral == nil) {
...
- (screen_char_t)foregroundColorCode {
    ...
    VT100GraphicRenditionUpdateForeground(&graphicRendition_, YES, _protectedMode != VT100TerminalProtectedModeNone,
                                          &result);
    return result;
}
...
- (screen_char_t)foregroundColorCodeReal {
    screen_char_t result = {0};
    VT100GraphicRenditionUpdateForeground(&graphicRendition_, NO, _protectedMode != VT100TerminalProtectedModeNone,
                                          &result);
    return result;
}
...
- (void)executeDecSetReset:(VT100Token *)token {
    // BUG-f972: Use guard instead of assert - invalid token type should return early with warning
    if (token->type != VT100CSI_DECSET && token->type != VT100CSI_DECRST) {
        DLog(@"BUG-f972: executeDecSetReset: called with invalid token type %d - ignoring", token->type);
        return;
    }
    ...
    for (int i = 0; i < token.csi->count; i++) {
        switch (token.csi->p[i]) {
            ...
            case 3: // DECCOLM
                if (self.allowColumnMode) {
                    ...
                    VT100GridCoord coord =
                        VT100GridCoordMake(self.delegate.terminalCursorX, self.delegate.terminalCursorY);
            ...
            case 8:
                ...
                break;
            case 9:
                // X10 mouse compatibility mode (rarely used). Would send mouse x,y on button press.
                // Not implemented - modern apps use 1000/1002/1003 modes instead.
                break;
            case 12:
            ...
            case 95: // DECNCSM
                ...
                break;
            case 1000:
            // case 1001: MOUSE_REPORTING_HIGHLIGHT - rarely used highlighting mouse mode.
            // Not implemented; applications typically use 1000/1002/1003 instead.
            case 1002:
            case 1003:
                if (mode) {
            ...
            // 1047 |              switch        | clear, switch                 | noop           | noop | 1049 | save
            // cursor, switch, clear |        switch, restore cursor | restore cursor | save cursor, clear screen |
            case 47:
...
- (void)executeSGR:(VT100Token *)token {
    self.dirty = YES;
    // BUG-f1379: Convert assert to guard - wrong token type should be no-op
    ...
    if (token.csi->count == 0) {
        ...
        for (i = 0; i < token.csi->count; ++i) {
            const VT100GraphicRenditionSideEffect sideEffect =
                VT100GraphicRenditionExecuteSGR(&graphicRendition_, token.csi, i);
            switch (sideEffect) {
...
- (NSColor *)colorForXtermCCSetPaletteString:(NSString *)argument colorNumberPtr:(int *)numberPtr {
    if ([argument length] == 7) {
        ...
        if (count == 4 && n >= 0 && n <= 22 && r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255) {
        ...
        switch (token.csi->p[0]) {
            ...
            case 6: { // Command from host -- Please report active position
                if (self.originMode) {
                    ...
                    [_delegate
                        terminalSendReport:[self.output reportActivePositionWithX:[_delegate terminalRelativeCursorX]
                                                                                Y:[_delegate terminalRelativeCursorY]
                                                                     withQuestion:withQuestion
                                                                     vt330OrLater:_vtLevel >= iTermEmulationLevel300]];
                } else {
                    [_delegate
                        terminalSendReport:[self.output reportActivePositionWithX:[_delegate terminalCursorX]
            ...
            case 15: // Printer status
                if (withQuestion && _vtLevel >= iTermEmulationLevel200) {
                    [_delegate terminalSendReport:[self.output
                                                      reportDECDSR:13]]; // "No printer" since printing is unsupported.
                } else {
                    DLog(@"vtlevel %@ denied", @(_vtLevel));
                }
                break;
            ...
            case 25:
                if (withQuestion && _vtLevel >= iTermEmulationLevel200) {
                    [_delegate terminalSendReport:[self.output
                                                      reportDECDSR:20]]; //  Locking is unsupported so report unlocked.
                } else {
                    DLog(@"vtlevel %@ denied", @(_vtLevel));
                }
                break;
            ...
            case 55:
                if (withQuestion && _vtLevel >= iTermEmulationLevel300) {
                    [_delegate terminalSendReport:[self.output reportDECDSR:50]]; // Locator unavailable becuase DEC
                                                                                  // locator support unimplemented.
                } else {
                    DLog(@"vtlevel %@ denied", @(_vtLevel));
                }
                break;
            ...
            case 62: // Request DECMSR
                if (withQuestion && _vtLevel >= iTermEmulationLevel400) {
                    [_delegate terminalSendReport:[self.output
                                                      reportMacroSpace:0]]; // Macros are unsupported so report 0 space
                } else {
                    DLog(@"vtlevel %@ denied", @(_vtLevel));
                }
                break;
            ...
            case 63: // Request DECCKSR
                if (withQuestion && _vtLevel >= iTermEmulationLevel400) {
                    [_delegate
                        terminalSendReport:[self.output reportMemoryChecksum:0 id:token.csi->p[1]]]; // Memory checksum
                } else {
                    DLog(@"vtlevel %@ denied", @(_vtLevel));
                }
                break;
            ...
            case 997: // https://github.com/contour-terminal/contour/blob/master/docs/vt-extensions/color-palette-update-notifications.md
                // Send CSI ? 996 n to the terminal to explicitly request the current color preference (dark mode or
                // light mode) by the operating system. The terminal will reply back in either of the two ways: VT
                // sequence      description CSI ? 997 ; 1 n	DSR reply to indicate dark mode CSI ? 997 ; 2 n	DSR
                // reply to indicate light mode
                if (withQuestion) {
                    [self sendDarkModeDSR];
                }
                break;
            case 1337: // DashTerm2 extension
                [_delegate terminalSendReport:[self.output reportDashTerm2Version]];
                break;
...
- (VT100GridRect)rectangleInToken:(VT100Token *)token
    ...
    return VT100GridRectMake(MAX(0, coordRange.start.x - 1), MAX(0, coordRange.start.y - 1),
                             coordRange.end.x - coordRange.start.x + 1, coordRange.end.y - coordRange.start.y + 1);
}

- (BOOL)rectangleIsValid:(VT100GridRect)rect {
    if (self.originMode) {
        VT100GridRect scrollRegion = [_delegate terminalScrollRegion];
        if (rect.origin.y < scrollRegion.origin.y || rect.origin.x < scrollRegion.origin.x ||
...
- (void)sendChecksumReportWithId:(int)identifier rectangle:(VT100GridRect)rect {
    if (![_delegate terminalShouldSendReport:NO]) {
        return;
    }
    if (identifier < 0) {
...
- (NSString *)decodedBase64PasteCommand:(NSString *)commandString query:(NSString **)query {
    ...
    *query = nil;
    ...
    NSString *resultString = [[NSString alloc] initWithData:data encoding:[self encoding]];
    return resultString;
}
...
- (void)saveCursor {
    ...
    savedCursor->position = VT100GridCoordMake([_delegate terminalCursorX] - 1, [_delegate terminalCursorY] - 1);
...
- (void)softReset {
    ...
    const VT100GridSize size = [_delegate terminalSizeInCells];
    [_delegate terminalSetScrollRegionTop:0 bottom:size.height - 1];
...
static BOOL VT100TokenIsTmux(VT100Token *token) {
    return (token->type == TMUX_EXIT || token->type == TMUX_LINE || token->type == DCS_TMUX_HOOK);
}

- (void)executeToken:(VT100Token *)token {
    if (_lastToken != nil && VT100TokenIsTmux(_lastToken) && !VT100TokenIsTmux(token)) {
...
- (void)reallyExecuteToken:(VT100Token *)token {
    ...
    if (receivingFile_) {
        ...
        if (token->type == XTERMCC_MULTITOKEN_BODY) {
            ...
            return;
        } else if (token->type == VT100_ASCIISTRING || token->type == VT100_MIXED_ASCII_CR_LF ||
        ...
        } else if (token->type != SSH_SIDE_CHANNEL && token->type != SSH_BEGIN && token->type != SSH_END &&
                   token->type != SSH_LINE && token->type != SSH_OUTPUT) {
        ...
        if (token->type == XTERMCC_MULTITOKEN_BODY) {
            ...
            _copyMode = VT100TerminalCopyModeNone;
            return;
        } else if (_copyMode != VT100TerminalCopyModeSegmented && token->type != SSH_OUTPUT) {
    ...
    if (token->savingData &&
        ...
        if (token->type != XTERMCC_SET_KVP || ![token.string hasPrefix:@"CopyToClipboard"]) {
    ...
    switch (token->type) {
        ...
        case VT100_ASCIISTRING:
            ...
            break;
        case VT100_MIXED_ASCII_CR_LF:
            [_delegate terminalAppendMixedAsciiCRLFData:token.asciiData crlfs:token.crlfs];
            break;
        case VT100_GANG:
        ...
        case VT100CSI_CUB:
            ...
            break;
        case VT100CSI_CUD:
            [_delegate terminalCursorDown:token.csi->p[0] > 0 ? token.csi->p[0] : 1 andToStartOfLine:NO];
            break;
        case VT100CSI_CUF:
        ...
        case VT100CSI_CHT:
            ...
            break;
        case VT100CSI_CUU:
            [_delegate terminalCursorUp:token.csi->p[0] > 0 ? token.csi->p[0] : 1 andToStartOfLine:NO];
            break;
        case VT100CSI_CNL:
            [_delegate terminalCursorDown:token.csi->p[0] > 0 ? token.csi->p[0] : 1 andToStartOfLine:YES];
            break;
        case VT100CSI_CPL:
            [_delegate terminalCursorUp:token.csi->p[0] > 0 ? token.csi->p[0] : 1 andToStartOfLine:YES];
            break;
        case VT100CSI_DA:
        ...
        case VT100CSI_DA2:
            ...
            break;
        case VT100CSI_DA3:
            // BUG-10195: Fixed inverted logic. DA3 (tertiary device attributes) requires VT400+.
            // The previous code logged "denied" but then sent the report anyway due to fall-through.
            if (_vtLevel < iTermEmulationLevel400) {
                DLog(@"DA3 vtlevel %@ < 400, not sending report", @(_vtLevel));
                break;
            }
            if ([_delegate terminalShouldSendReport:NO]) {
        ...
        case VT100CSI_DECSTBM: {
            ...
            [_delegate terminalSetScrollRegionTop:top bottom:bottom];
        ...
        case VT100_LITERAL:
            ...
            [self updateExternalAttributes];
            [_delegate terminalAppendString:[[NSString stringWithLongCharacter:token->code]
                                                stringByReplacingControlCharactersWithCaretLetter]];
            _currentLiteral = nil;
            [self updateExternalAttributes];
            graphicRendition_.reversed = !graphicRendition_.reversed;
            break;
        ...
        case VT100CSI_DECSLRM: {
             ...
             * In DashTerm2, it is an array of booleans where 0 means normal behavior and 1 means
        ...
        case VT100CSI_DECST8C:
            if (_vtLevel >= iTermEmulationLevel500) {
                if (token.csi->p[0] == 5) {
                    ...
                    // Pre-allocate for expected tab stop count (width / 8)
                    NSMutableArray<NSNumber *> *stops = [NSMutableArray arrayWithCapacity:(width / 8) + 1];
        ...
        case XTERMCC_WIN_TITLE:
            [_delegate terminalSetWindowTitle:[[self sanitizedTitle:[self stringBeforeNewline:token.string]]
                                                  stringByReplacingControlCharactersWithCaretLetter]];
            break;
        case XTERMCC_WINICON_TITLE: {
            NSString *title =
                [[self stringBeforeNewline:token.string] stringByReplacingControlCharactersWithCaretLetter];
            NSString *subtitle =
                [[self subtitleFromIconTitle:token.string] stringByReplacingControlCharactersWithCaretLetter];
        ...
        case XTERMCC_TEXT_FOREGROUND_COLOR:
            [self executeSetDynamicColor:VT100TerminalColorIndexText arg:token.string];
            break;
        ...
        case XTERMCC_TEXT_BACKGROUND_COLOR:
            [self executeSetDynamicColor:VT100TerminalColorIndexBackground arg:token.string];
            break;
        case XTERMCC_SET_TEXT_CURSOR_COLOR:
            [self executeSetDynamicColor:VT100TerminalColorIndexCursor arg:token.string];
            break;
        case XTERMCC_SET_HIGHLIGHT_COLOR:
            [self executeSetDynamicColor:VT100TerminalColorIndexSelectionBackground arg:token.string];
            break;
        case XTERMCC_SET_HIGHLIGHT_FOREGROUND_COLOR:
            [self executeSetDynamicColor:VT100TerminalColorIndexSelectionForeground arg:token.string];
            break;
        ...
        case XTERMCC_ICON_TITLE: {
            ...
            if (!subtitle || token.string.length > 0) {
                [_delegate terminalSetIconTitle:[[self stringBeforeNewline:token.string]
                                                    stringByReplacingControlCharactersWithCaretLetter]];
            }
            if (subtitle) {
                [_delegate terminalSetSubtitle:subtitle];
        ...
        case XTERMCC_DELLN:
            ...
            break;
        case VT100_DECSLPP:
            [_delegate terminalSetRows:MIN(kMaxScreenRows, token.csi->p[0]) andColumns:-1];
            break;
        ...
        case XTERMCC_WINDOWSIZE_PIXEL:
            [_delegate terminalSetPixelWidth:token.csi->p[2] height:token.csi->p[1]];
        ...
        case XTERMCC_REPORT_WIN_STATE: {
            NSString *s = [NSString stringWithFormat:@"\033[%dt", ([_delegate terminalWindowIsMiniaturized] ? 2 : 1)];
        ...
        case XTERMCC_REPORT_WIN_POS: {
            ...
            NSString *s = [NSString stringWithFormat:@"\033[3;%d;%dt", (int)topLeft.x, (int)topLeft.y];
            [_delegate terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
            break;
        }
        case XTERMCC_REPORT_WIN_PIX_SIZE: {
            // TODO: Some kind of adjustment for panes?
            NSString *s = [NSString stringWithFormat:@"\033[4;%d;%dt", [_delegate terminalWindowHeightInPixels],
        ...
        case XTERMCC_REPORT_WIN_SIZE: {
            const VT100GridSize size = [_delegate terminalSizeInCells];
            NSString *s = [NSString stringWithFormat:@"\033[8;%d;%dt", size.height, size.width];
            [_delegate terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
            break;
        }
        case XTERMCC_REPORT_SCREEN_SIZE: {
            NSString *s = [NSString stringWithFormat:@"\033[9;%d;%dt", [_delegate terminalScreenHeightInCells],
        ...
        case XTERMCC_MULTITOKEN_HEADER_SET_KVP:
            DLog(@"Execute MULTITOKEN_HEADER: kvpKey=%@ kvpValue=%@ string=%@", token.kvpKey, token.kvpValue,
                 token.string);
        ...
        case SSH_BEGIN:
            [self.delegate
                terminalDidBeginSSHConductorCommandWithIdentifier:token.string
                                                            depth:token.sshInfo.valid ? token.sshInfo.depth : 0];
            break;
        ...
        case SSH_END: {
            ...
            [self.delegate
                terminalDidEndSSHConductorCommandWithIdentifier:parts[0]
                                                           type:type
                                                         status:status
                                                          depth:token.sshInfo.valid ? token.sshInfo.depth : 0];
            break;
        ...
        case SSH_OUTPUT:
            ...
            [self.delegate terminalDidReadRawSSHData:token.savedData pid:token.csi->p[0] channel:token.csi->p[1]];
            break;
        ...
        default:
            DLog(@"Unexpected token type %d", (int)token->type);
            break;
    }
}
...
- (void)executeRequestTermcapTerminfo:(VT100Token *)token {
    ...
    iTermPromise<NSString *> * (^keyPromise)(unsigned short, NSEventModifierFlags, NSString *, NSString *) =
        ^iTermPromise<NSString *> *(unsigned short keyCode, NSEventModifierFlags flags, NSString *characters,
    ...
    NSArray<NSString *> *keys =
        [[token.string componentsSeparatedByString:@";"] mapWithBlock:^id _Nullable(NSString *_Nonnull hex) {
            NSString *decoded = [NSString stringWithHexEncodedString:hex];
            return decoded ?: @"";
        }];
    NSError * (^error)(NSString *) = ^NSError *(NSString *key) {
        return [NSError errorWithDomain:@"com.dashterm.dashterm2.termcap-terminfo" code:1 userInfo:@{@"code" : key}];
    };


    BOOL (^add)(NSString *, NSMutableArray<iTermPromise<NSString *> *> *, unsigned short, NSEventModifierFlags,
                NSString *, NSString *) =
        ^BOOL(NSString *hexEncodedKey, NSMutableArray<iTermPromise<NSString *> *> *parts, unsigned short keyCode,
              NSEventModifierFlags flags, NSString *characters, NSString *charactersIgnoringModifiers) {
            // First get the value. This is legit async.
            iTermPromise<NSString *> *keyStringPromise =
                keyPromise(keyCode, flags, characters, charactersIgnoringModifiers);

            // Once the value is computed, format it into key=value.
            iTermPromise<NSString *> *promise = [iTermPromise promise:^(id<iTermPromiseSeal> _Nonnull seal) {
    ...
    // Pre-allocate for typical number of XTGETTCAP query parts (8-16)
    NSMutableArray<iTermPromise<NSString *> *> *parts = [NSMutableArray arrayWithCapacity:16];

    NSDictionary<id, BOOL (^)(NSString *, NSMutableArray<iTermPromise<NSString *> *> *)> *handlers = @{@"name" : ^BOOL(
        NSString *hexEncodedKey, NSMutableArray<iTermPromise<NSString *> *> *parts){
        [parts addObject:[iTermPromise promiseValue:[NSString stringWithFormat:kFormat, hexEncodedKey,
...
,
        ...
        @"DashTerm2Profile": ^BOOL(NSString *hexEncodedKey, NSMutableArray<iTermPromise<NSString *> *> *parts) {
            [parts addObject:[iTermPromise promiseValue:[NSString stringWithFormat:kFormat,
                                                         hexEncodedKey,
...
[iTermPromise gather:parts
               queue:[self.delegate terminalQueue]
          completion:^(NSArray<iTermOr<id, NSError *> *> *_Nonnull values) {
...
- (void)finishRequestTermcapTerminfoWithValues:(NSArray<NSString *> *)parts ok:(BOOL)ok {
    if (gDebugLogging) {
        [parts enumerateObjectsUsingBlock:^(NSString *kvp, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            NSMutableString *line = [NSMutableString
                stringWithFormat:@"%@=", [[NSString alloc] initWithData:[tuple.firstObject dataFromHexValues]
    ...
    NSString *s = [NSString stringWithFormat:@"\033P%d+r%@\033\\", ok ? 1 : 0, [parts componentsJoinedByString:@";"]];
    [_delegate terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
}
...
- (NSString *)decrqssSGR {
    NSArray<NSString *> *codes = [[VT100Terminal sgrCodesForGraphicRendition:graphicRendition_].array
...
- (NSString *)decrqssDECSCA {
    return (_protectedMode != VT100TerminalProtectedModeNone &&
            [_delegate terminalProtectedMode] == VT100TerminalProtectedModeDEC)
               ? @"1"
               : @"0";
}
...
+ (NSOrderedSet<NSString *> *)sgrCodesForCharacter:(screen_char_t)c externalAttributes:(iTermExternalAttribute *)ea {
...
+ (NSOrderedSet<NSString *> *)sgrCodesForGraphicRendition:(VT100GraphicRendition)original {
    ...
    switch (graphicRendition.fgColorMode) {
        ...
        case ColorModeAlternate:
            switch (graphicRendition.fgColorCode) {
                case ALTSEM_DEFAULT:
                    break;
                case ALTSEM_REVERSED_DEFAULT: // Not sure quite how to handle this, going with the simplest approach for
                                              // now.
                    [result addObject:@"39"];
                    break;
        ...
        case ColorMode24bit:
            [result addObject:[NSString stringWithFormat:@"38:2:1:%@:%@:%@", @(graphicRendition.fgColorCode),
                                                         @(graphicRendition.fgGreen), @(graphicRendition.fgBlue)]];
            break;
    ...
    switch (graphicRendition.bgColorMode) {
        ...
        case ColorMode24bit:
            [result addObject:[NSString stringWithFormat:@"48:2:1:%@:%@:%@", @(graphicRendition.bgColorCode),
                                                         @(graphicRendition.bgGreen), @(graphicRendition.bgBlue)]];
            break;
    ...
    if (graphicRendition.hasUnderlineColor) {
        switch (graphicRendition.underlineColor.mode) {
            case ColorModeNormal:
                [result addObject:[NSString stringWithFormat:@"58:5:%d", graphicRendition.underlineColor.red]];
                break;
            case ColorMode24bit:
                [result addObject:[NSString stringWithFormat:@"58:2:%d:%d:%d", graphicRendition.underlineColor.red,
        ...
        if ((i % 2) == 0) {
            ...
            if (components) {
                ...
                NSColor *theColor = [NSColor it_colorInDefaultColorSpaceWithRed:components[0].doubleValue
                                                                          ...
                                                                          alpha:1];
                [_delegate terminalSetColorTableEntryAtIndex:theIndex color:theColor];
...
- (void)executeFileCommandWithValue:(NSString *)value {
    ...
    //   type=<string>                     Default: auto-detect; otherwise gives a mime type ("text/plain"), file
    //   extension preceded by dot (".txt"), or language name ("plaintext"). mode=regular|wide                 Default:
    //   regular (only relevant for text documents)
    ...
    if ([dict[@"inline"] boolValue]) {
        NSEdgeInsets inset = {.top = insetTop, .left = insetLeft, .bottom = insetBottom, .right = insetRight};
...
- (void)resetColors:(NSString *)arg {
    ...
    // Pre-allocate for expected size: either parts.count or 256 if resetting all
    NSMutableArray<NSNumber *> *indexes = [NSMutableArray arrayWithCapacity:parts.count > 0 ? parts.count : 256];
    for (NSString *part in parts) {
        ...
        if (components) {
            ...
            NSColor *theColor = [NSColor it_colorInDefaultColorSpaceWithRed:components[0].doubleValue
                                                                      green:components[1].doubleValue
                                                                       blue:components[2].doubleValue
                                                                      alpha:1];
            [_delegate terminalSetColorTableEntryAtIndex:ptyIndex color:theColor];
        }
    }
}
...
- (void)executeXtermSetKvp:(VT100Token *)token {
    ...
    if ([key isEqualToString:@"CursorShape"]) {
        ...
        if ([_delegate terminalShouldSendReport:YES]) {
            ...
            NSString *height =
                [[NSString stringWithFormat:@"%0.2f", size.height] stringByCompactingFloatingPointString];
            NSString *scale = [[NSString stringWithFormat:@"%0.2f", floatScale] stringByCompactingFloatingPointString];
            NSString *s = [NSString stringWithFormat:@"\033]1337;ReportCellSize=%@;%@;%@\033\\", height, width, scale];
            [_delegate terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
        }
        ...
        if ([_delegate terminalIsTrusted]) {
            ...
            if (colon != NSNotFound) {
                ...
                NSMutableDictionary *parameters = [[NSMutableDictionary alloc] initWithCapacity:parts.count];
                ...
                [_delegate terminalCustomEscapeSequenceWithParameters:parameters payload:payload];
            }
        }
...
- (BOOL)wrapChannelIDIsValid:(NSString *)uid {
    ...
    NSCharacterSet *illegalCharacters =
        [NSCharacterSet characterSetWithCharactersInString:@"abcdefABCDEF0123456789"].invertedSet;
...
- (void)executeXtermSetPalette:(VT100Token *)token {
    int n;
    NSColor *theColor = [self colorForXtermCCSetPaletteString:token.string colorNumberPtr:&n];
    if (theColor) {
        switch (n) {
            case 16:
    ...
    if (func) {
        if ([func isEqualToString:@"1"]) {
            ...
            if ([parts count] == 4) {
                ...
                if ([class isEqualToString:@"bg"] && [color isEqualToString:@"*"] &&
                ...
                if ([class isEqualToString:@"bg"] && [attribute isEqualToString:@"brightness"]) {
...
- (void)executeFinalTermToken:(VT100Token *)token {
    ...
    switch ([command characterAtIndex:0]) {
        case 'A':
            ...
            self.softAlternateScreenMode = NO; // We can reasonably assume alternate screen mode has ended if there's a
                                               // prompt. Could be ssh dying, etc.
            self.dirty = YES;
        ...
        case 'D':
            ...
            // BUG-3524/3559: Missing break caused fall-through to case 'E'
            break;
...
- (NSData *)decrpmForMode:(int)mode setting:(iTermDECRPMSetting)setting ansi:(BOOL)ansi {
    NSString *string;
    if (ansi) {
...
- (void)executeDECRSPS_DECCIR:(NSString *)string {
    self.dirty = YES;
    BOOL ok = NO;
    const VT100OutputCursorInformation info =
        VT100OutputCursorInformationFromString(string, _vtLevel >= iTermEmulationLevel500, &ok);
    if (!ok) {
        return;
    }
...
- (void)executeReadSixelGeometry:(VT100Token *)token {
    ...
    [self sendGraphicsAttributeReportForToken:token
                                       status:0
                                        value:[NSString stringWithFormat:@"%@;%@", @(width * cellSize.width * scale),
...
- (void)executeReadMaximumValueOfSixelGeometry:(VT100Token *)token {
    ...
    [self sendGraphicsAttributeReportForToken:token
                                       status:0
                                        value:[NSString stringWithFormat:@"%d;%d", maxDimension, maxDimension]];
}
    ...
    for (int i = 0; i < entry.numElements; i++) {
        switch (entry.elements[i]) {
            ...
            case VT100SGRStackAttributeUnderlined:
                if (entry.graphicRendition.underline &&
                    (entry.graphicRendition.underlineStyle == VT100UnderlineStyleSingle ||
...
- (void)executeDECSCPP:(int)param {
    ...
    [self setWidth:cols preserveScreen:YES updateRegions:NO moveCursorTo:VT100GridCoordMake(-1, -1) completion:nil];
}
...
- (void)executeAPC:(VT100Token *)token {
    ...
    if ([token.string hasPrefix:@"G"]) {
        iTermKittyImageCommand *command =
            [[iTermKittyImageCommand alloc] initWithAPCString:[token.string substringFromIndex:1]];
        if (!command) {
            return;
        }
...
- (void)sendGraphicsAttributeReportForToken:(VT100Token *)token status:(int)status value:(NSString *)value {
    if (![_delegate terminalShouldSendReport:NO]) {
        return;
    }
    [_delegate terminalSendReport:[_output reportGraphicsAttributeWithItem:token.csi->p[0] status:status value:value]];
}
...
- (void)sendDECCIR {
    if ([_delegate terminalShouldSendReport:NO]) {
        ...
        VT100OutputCursorInformation info = VT100OutputCursorInformationCreate(
            [_delegate terminalCursorY], MIN(width, x), self.graphicRendition.reversed, self.graphicRendition.blink,
            self.graphicRendition.underline, self.graphicRendition.bold,
            [_delegate terminalWillAutoWrap] && self.wraparoundMode, lineDrawingMode, self.originMode);
...
- (iTermPromise<NSNumber *> *)promiseOfSettingForDECRequestMode:(int)mode {
    switch (mode) {
        ...
        case 9:
            // X10 mouse mode - not implemented. See case 9 in executeDecSetOrReset.
            break;
        case 12: {
        ...
        case 1004:
            return VT100TerminalPromiseOfDECRPMSettingFromBoolean(self.reportFocus &&
                                                                  [_delegate terminalFocusReportingAllowed]);
...
// BUG-2770: Add limit to unicode version stack to prevent resource exhaustion
// Same limit as key reporting mode stack (1024 entries)
static const NSInteger kMaxUnicodeVersionStackEntries = 1024;
...
- (void)pushUnicodeVersion:(NSString *)label {
    ...
    // BUG-2770: Prune stack if it exceeds maximum size to prevent memory exhaustion
    while (_unicodeVersionStack.count > kMaxUnicodeVersionStackEntries) {
        [_unicodeVersionStack removeObjectAtIndex:0];
    }
}
...
- (void)popUnicodeVersion:(NSString *)label {
    ...
    while (_unicodeVersionStack.count > 0) {
        ...
        if ([entry isKindOfClass:[NSNumber class]]) {
            ...
            // BUG-3977: Validate array has at least 2 elements before accessing indices
            NSArray *entryArray = entry;
            if (entryArray.count >= 2) {
                entryLabel = [entryArray objectAtIndex:0];
                value = [entryArray objectAtIndex:1];
            } else {
                DLog(@"Invalid unicode version stack entry: %@", entry);
                continue;
            }
        } else {
            DLog(@"Unexpected unicode version stack entry type: %@", entry);
            continue;
        }
...
- (NSDictionary *)dictionaryForSavedCursor:(VT100SavedCursor)savedCursor {
    // Pre-allocate for exact charset count (NUM_CHARSETS is typically 4)
    NSMutableArray *lineDrawingArray = [NSMutableArray arrayWithCapacity:NUM_CHARSETS];
    for (int i = 0; i < NUM_CHARSETS; i++) {
        [lineDrawingArray addObject:@(savedCursor.lineDrawing[i])];
    }
    return @{
        kSavedCursorPositionKey : [NSDictionary dictionaryWithGridCoord:savedCursor.position],
...
- (void)setStateFromDictionary:(NSDictionary *)dict {
    ...
    _savedColors = [VT100SavedColors fromData:[NSData castFrom:dict[kTerminalStateSavedColors]]]
                       ?: [[VT100SavedColors alloc] init];
...
- (NSArray<NSDictionary *> *)sgrStack {
    // Pre-allocate for known stack size
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:_sgrStackSize];
    for (int i = 0; i < _sgrStackSize; i++) {
        // Pre-allocate for known number of elements in this stack entry
        NSMutableArray<NSNumber *> *elements = [NSMutableArray arrayWithCapacity:_sgrStack[i].numElements];
        for (int j = 0; j < _sgrStack[i].numElements; j++) {
            [elements addObject:@(_sgrStack[i].elements[j])];
        }
        [result addObject:@{
            @"state" : [self dictionaryForGraphicRendition:_sgrStack[i].graphicRendition],
            @"elements" : elements
        }];
    }
    return result;
