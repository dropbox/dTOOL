...
+ (NSMenu *)findOrCreateTagSubmenuInMenu:(NSMenu *)menu
    ...
    if (parts.count > 1) {
        ...
        if (menuToAddOpenAll && [self menuHasMultipleItemsExcludingAlternates:menuToAddOpenAll fromIndex:0] &&
...
+ (void)addOpenAllToMenu:(NSMenu *)menu
                  params:(iTermProfileModelJournalParams *)params
              identifier:(NSString *)identifier {
    ...
    if (@available(macOS 26, *)) {
        openAll.image = [NSImage imageWithSystemSymbolName:@"person.3.sequence" accessibilityDescription:nil];
    }
...
+ (NSString *)identifierForMenuItem:(NSMenuItem *)item {
    NSString *prefix = item.isAlternate ? iTermProfileModelNewWindowMenuItemIdentifierPrefix
                                        : iTermProfileModelNewTabMenuItemIdentifierPrefix;
...
- (void)addBookmark:(Profile *)b
    ...
    if ([shortcut length]) {
        ...
        [item setKeyEquivalentModifierMask:NSEventModifierFlagCommand | NSEventModifierFlagControl |
                                           (isAlternate ? NSEventModifierFlagOption : 0)];
    } else if (isAlternate) {
...
+ (NSString *)applyAddJournalEntry:(BookmarkJournalEntry *)e
    ...
    NSArray *tags = [NSArray castFrom:b[KEY_TAGS]];
    ...
    NSString *shortcut = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:b];
    return [shortcut uppercaseString];
}

+ (NSArray *)menuItemsForTag:(NSString *)multipartName inMenu:(NSMenu *)menu {
    NSArray *parts = [multipartName componentsSeparatedByString:@"/"];
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:parts.count];
    NSMenuItem *item = nil;
...
+ (NSSet<NSString *> *)applyRemoveAllJournalEntry:(BookmarkJournalEntry *)e
                                   ...
                                   startingAtItem:(int)skip
                                           params:(iTermProfileModelJournalParams *)params {
    NSMutableSet<NSString *> *results = [NSMutableSet setWithCapacity:MAX(0, menu.numberOfItems - skip)];
...
+ (NSDictionary<NSString *, NSNumber *> *)applyJournal:(NSDictionary *)journalDict
    ...
    NSMutableDictionary<NSString *, NSNumber *> *results = [[NSMutableDictionary alloc] initWithCapacity:journal.count];
    for (BookmarkJournalEntry *e in journal) {
        switch (e.action) {
            ...
            case JOURNAL_REMOVE: {
                NSString *shortcut = [self.class applyRemoveJournalEntry:e
                                                                  toMenu:menu
                                                          startingAtItem:skip
                                                                  params:params];
                if (shortcut) {
            ...
            case JOURNAL_REMOVE_ALL: {
                NSSet<NSString *> *shortcuts = [self.class applyRemoveAllJournalEntry:e
                                                                               toMenu:menu
                                                                       startingAtItem:skip
                                                                               params:params];
            ...
            default:
                // BUG-433: Replace assert(false) with DLog - unknown journal entry type should not crash
                DLog(@"Unknown journal entry type %d in applyJournal:toMenu:. Entry skipped.", (int)e.action);
                break;
        }
    }
    return results;
