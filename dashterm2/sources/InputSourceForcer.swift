...
class InputSourceForcer: NSObject {
    ...
    func begin() {
        ...
        // BUG-485: Replace it_assert with guard for proper crash prevention
        guard !begun else {
            DLog("BUG-485: begin() called twice on InputSourceForcer")
            return
        }
    ...
    private func setInputLocale(_ keyboardID: String) {
        ...
        // BUG-486: Replace it_assert with guard for proper crash prevention
        guard forcingEnabled else {
            DLog("BUG-486: setInputLocale called when input source forcing is disabled")
            return
        }
        // BUG-1756: Use as? with empty array fallback instead of as! force cast
