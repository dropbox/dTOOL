...
private let passwordLogger = Logger(subsystem: "com.dashterm2.PasswordManager", category: "default")
*/
...
class CommandLinePasswordDataSource: NSObject {
    ...
    struct InteractiveCommandRequest: CommandLinePasswordDataSourceExecutableCommand {
        ...
        static let ioQueue = DispatchQueue(label: "com.dashterm.dashterm2.pwcmd-io", qos: .userInitiated)
        ...
        func exec() throws -> Output {
            ...
            // BUG-1810: Use guard let instead of force unwrap for output
            guard let output = _output else {
                throw CommandLineRecipeError.unsupported(reason: "Command returned nil output without error")
            }
            return output
        }
    ...
    private class RunningInteractiveCommand: CommandWriting {
        ...
        init(request: InteractiveCommandRequest,
            ...
            // BUG-1795: Use optional chaining instead of force unwrap
            ...
            // BUG-f545: Handle unexpected stdin types gracefully instead of crashing
            if let pipe = stdin as? Pipe {
                process.standardInput = pipe
            } else if let tty = stdin as? TTY {
                process.standardInput = tty.slave
            } else if let fileHandle = stdin as? FileHandle {
                process.standardInput = fileHandle
            } else {
                DLog("WARNING: Unknown stdin type \(type(of: stdin)) - using nil")
                // Leave standardInput as default (nil)
            }
        }
        ...
        private func mainloop() -> Output {
            ...
            group.wait()
            // BUG-f546: Handle nil build result gracefully instead of crashing
            guard let output = builder.tryBuild() else {
                DLog("ERROR: OutputBuilder.canBuild was true but tryBuild() returned nil")
                // Return a fallback output with error indication
                let fallbackOutput = Output(
                    stderr: "Internal error: output builder failed".data(using: .utf8) ?? Data(),
                    stdout: Data(),
                    returnCode: -1,
                ...
                runTerminationHandler(output: fallbackOutput)
                group.wait()
        ...
        func run() throws -> Output {
            // BUG-1619: Use guard instead of precondition - calling run() twice should throw
            guard !ran else {
                throw iTermError("CommandLinePasswordDataSource.run() already called")
            }
            ran = true
            try ObjC.catching {
                self.process.launch()
            }
            // BUG-1796: Use optional chaining instead of force unwrap
            log("Launched \(self.process.executableURL?.path ?? "unknown") with args \(self.process.arguments ?? [])")
    ...
    func standardAccounts(context: RecipeExecutionContext,
        ...
        return configuration.listAccountsRecipe.transformAsync(
            context: context, inputs: ()) { maybeAccounts, maybeError in
                ...
                // BUG-1797: Use guard let instead of force unwrap
                guard let unwrappedAccounts = maybeAccounts else {
                    completion(nil, CommandLineRecipeError.unsupported(reason: "List accounts returned nil without error"))
                    return
                }
                let accounts = unwrappedAccounts.compactMap { account in
    ...
    func standardAdd(_ configuration: Configuration,
        ...
        configuration.addAccountRecipe.transformAsync(context: context, inputs: inputs) { accountIdentifier, maybeError in
            ...
            // BUG-1798: Use guard let instead of force unwrap
            guard let unwrappedIdentifier = accountIdentifier else {
                completion(nil, CommandLineRecipeError.unsupported(reason: "Add account returned nil identifier without error"))
                return
            }
            let account = CommandLineProvidedAccount(identifier: unwrappedIdentifier.value,
                                                     accountName: accountName,
                                                     userName: userName,
                                                     hasOTP: false,
                                                     sendOTP: true,
...
class TTY: NSObject, ReadWriteFileHandleVending {
    ...
    init(term: inout termios,
        ...
        let rc = openpty(&masterFD,
                         ...
                         &size)
        if rc == -1 {
            // BUG-2890: Don't force unwrap - non-standard errno could crash
