...
static LineBlock *LineBufferSearchBlockForStoredBlock(id block) {
    if ([block isKindOfClass:[LineBlock class]]) {
        return block;
    }
    if ([block isKindOfClass:[LineBlockPacked class]]) {
        return ((LineBlockPacked *)block).progenitor;
    }
    return nil;
}
...
@implementation LineBuffer {
    ...
    // Cache of the number of wrapped lines for multiple widths.
    // Phase 1C optimization: Cache up to 4 widths to avoid recomputation when switching widths.
    // This is useful when resizing windows or when multiple terminals have different sizes.
    int num_wrapped_lines_cache;
    int num_wrapped_lines_width;
    // Secondary cache entries for common alternate widths
    int _multiWidthCacheWidths[3];
    int _multiWidthCacheCounts[3];
    ...
    // RC-008: Track whether truncation is in progress.
    // During truncation, _lineBlocks may be temporarily empty.
    // Methods that access _lineBlocks should check this flag and handle gracefully.
    int _truncationInProgress;

    // Shared color table for packed blocks (memory optimization)
    PackedColorTable *_packedColorTable;
}
...
- (void)commonInit {
    ...
    // Phase 1C: Initialize multi-width cache to invalid
    for (int i = 0; i < 3; i++) {
        _multiWidthCacheWidths[i] = -1;
    }
}
...
- (LineBuffer *)initWithDictionary:(NSDictionary *)dictionary maintainBidiInfo:(BOOL)maintainBidiInfo {
    self = [super init];
    if (self) {
        [self commonInit];
...
- (void)sanityCheck {
    ...
    for (int i = 1; i + 1 < _lineBlocks.count; i++) {
        ...
        // BUG-f868: Use ITAssertWithMessage instead of assert for better debugging
        ITAssertWithMessage(!block.isEmpty, @"BUG-f868: Empty interior block at index %d", i);
    }
#endif
}
...
- (void)assertUniqueBlockIDs {
    NSMutableSet<NSString *> *uniqueIDs = [NSMutableSet setWithCapacity:_lineBlocks.blocks.count];
    for (LineBlock *block in _lineBlocks.blocks) {
        BOOL dup = [uniqueIDs containsObject:block.stringUniqueIdentifier];
        // BUG-f869: Use ITAssertWithMessage instead of assert for better debugging
        ITAssertWithMessage(!dup, @"BUG-f869: Duplicate block ID found: %@", block.stringUniqueIdentifier);
...
// Phase 1C: Invalidate all wrapped lines caches (primary and secondary)
static inline void InvalidateWrappedLinesCache(LineBuffer *buffer) {
    buffer->num_wrapped_lines_width = -1;
    for (int i = 0; i < 3; i++) {
        buffer->_multiWidthCacheWidths[i] = -1;
    }
}

// This is called a lot so it's a C function to avoid obj_msgSend
// Phase 1C optimization: Multi-width cache to handle common terminal widths without recomputation.
static int RawNumLines(LineBuffer *buffer, int width) {
    // Primary cache - most recently used width
    if (buffer->num_wrapped_lines_width == width) {
        return buffer->num_wrapped_lines_cache;
    }

    // Check secondary caches for alternate widths
    for (int i = 0; i < 3; i++) {
        if (buffer->_multiWidthCacheWidths[i] == width) {
            // Found in secondary cache - promote to primary and return
            const int count = buffer->_multiWidthCacheCounts[i];
            // Demote current primary to this slot
            buffer->_multiWidthCacheWidths[i] = buffer->num_wrapped_lines_width;
            buffer->_multiWidthCacheCounts[i] = buffer->num_wrapped_lines_cache;
            // Promote this to primary
    ...
    // Demote current primary to least recently used secondary slot (slot 2)
    // Shift secondary caches down
    buffer->_multiWidthCacheWidths[2] = buffer->_multiWidthCacheWidths[1];
    buffer->_multiWidthCacheCounts[2] = buffer->_multiWidthCacheCounts[1];
    buffer->_multiWidthCacheWidths[1] = buffer->_multiWidthCacheWidths[0];
    buffer->_multiWidthCacheCounts[1] = buffer->_multiWidthCacheCounts[0];
    buffer->_multiWidthCacheWidths[0] = buffer->num_wrapped_lines_width;
    buffer->_multiWidthCacheCounts[0] = buffer->num_wrapped_lines_cache;
...
- (void)setMaxLines:(int)maxLines {
    self.dirty = YES;
    max_lines = maxLines;
    InvalidateWrappedLinesCache(self);
}
...
- (int)dropExcessLinesWithWidth:(int)width {
    _deferSanityCheck++;
    // RC-008 ROOT CAUSE FIX: Track truncation state to prevent access to
    // _lineBlocks when it may be temporarily empty during this operation.
    _truncationInProgress++;
...
- (int)reallyDropExcessLinesWithWidth:(int)width {
    ...
    [self sanityCheck];
#endif
    // Typical drop removes 1-4 blocks; pre-allocate to avoid reallocation
    NSMutableArray<LineBlock *> *blocksToDealloc = [NSMutableArray arrayWithCapacity:4];
    if (max_lines != -1 && nl > max_lines && _lineBlocks.count > 0) {
        ...
        while (total_lines > max_lines) {
            ...
            if (extra_lines >= block_lines) {
                ...
                if (_lineBlocks.count > 0) {
                    ...
                    // BUG-10666: All blocks were dropped, exit loop safely
                    total_lines = 0;
                    break;
                }
                ...
                // BUG-f870: Replace assert with graceful handling - log but don't crash
                if (numRawLinesAfter > numRawLinesBefore) {
                    DLog(@"BUG-f870: numRawLinesAfter (%d) > numRawLinesBefore (%d) after dropping lines",
                         numRawLinesAfter, numRawLinesBefore);
                }
    ...
    [self assertUniqueBlockIDs];
#endif
    // RC-016 / BUG-1439: Fix cursor_rawline underflow by checking BEFORE subtraction.
    // Signed integer overflow is undefined behavior in C. If the cursor was in the
    // dropped region (cursor_rawline < totalRawLinesDropped), set it to 0 (first
    // remaining line). Otherwise adjust normally.
    if (totalRawLinesDropped >= cursor_rawline) {
        cursor_rawline = 0; // Cursor was in the dropped region - reset to first remaining line
    } else {
    ...
    [self assertUniqueBlockIDs];
#endif
    // BUG-f871: Replace assert with graceful handling - clamp to 0 if negative
    if (totalRawLinesDropped < 0) {
        DLog(@"BUG-f871: totalRawLinesDropped (%d) is negative, clamping to 0", totalRawLinesDropped);
        totalRawLinesDropped = 0;
    }
    return totalDropped;
}

- (NSString *)debugString {
    NSMutableString *s = [[NSMutableString alloc] initWithCapacity:_lineBlocks.count * 256];
...
- (NSString *)dumpString {
    NSMutableArray<NSString *> *strings =
        [NSMutableArray arrayWithCapacity:_lineBlocks.count * 3]; // 3 strings per block
    int i;
    ...
    for (i = 0; i < _lineBlocks.count; ++i) {
        ...
        [strings
            addObject:[NSString
                          stringWithFormat:@"-- BEGIN BLOCK %d (abs block number %d, abs-position %@, raw size %@) --",
                                           i, i + num_dropped_blocks, @(pos), @(rawSpaceUsed)]];
...
- (NSString *)compactLineDumpWithWidth:(int)width andContinuationMarks:(BOOL)continuationMarks {
    ...
    NSMutableString *s = [[NSMutableString alloc] initWithCapacity:n * (width + 2)];
    for (int i = 0; i < n; i++) {
...
- (NSString *)compactLineDumpWithBlockDelimitersAndWidth:(int)width andContinuationMarks:(BOOL)continuationMarks {
    int n = [self numLinesWithWidth:width];
    NSMutableString *s = [[NSMutableString alloc] initWithCapacity:n * (width + 40)];
    LineBlock *block = nil;
...
- (void)dumpLinesWithWidth:(int)width {
    int n = [self numLinesWithWidth:width];
    NSMutableString *s = [[NSMutableString alloc] initWithCapacity:width + 2];
    int k = 0;
    for (int i = 0; i < n; i++) {
        screen_char_t continuation;
        ScreenCharArray *line = [self wrappedLineAtIndex:i width:width continuation:&continuation];
        ...
        if (i < n - 1) {
            DLog(@"%4d: %@", k++, s);
            s = [[NSMutableString alloc] initWithCapacity:width + 2];
        }
    }
    DLog(@"%4d: %@", k++, s);
}
...
- (void)dumpWrappedToWidth:(int)width {
    DLog(@"%@", [self compactLineDumpWithWidth:width andContinuationMarks:NO]);
}

- (void)appendScreenCharArray:(ScreenCharArray *)sca width:(int)width {
...
#if DEBUG
            [self sanityCheck];
            // BUG-f872: Use ITAssertWithMessage instead of bare assert
            ITAssertWithMessage(!block.isEmpty, @"BUG-f872: Block became empty after creation");
#endif
...
#if DEBUG
            [self sanityCheck];
            // BUG-f873: Use ITAssertWithMessage instead of bare assert
            ITAssertWithMessage(!block.isEmpty, @"BUG-f873: Block became empty after adding to array");
#endif
        }
    }
    ...
    if (![block appendLine:buffer
        ...
        if ([block hasPartial]) {
            ...
            BOOL ok = [block popLastLineInto:&temp
                                ...
                                continuation:NULL];
            // BUG-f874: Replace assert with graceful handling
            if (!ok) {
                DLog(@"BUG-f874: popLastLineInto failed unexpectedly when hasPartial was true");
            }
        ...
        if (prefix) {
            BOOL ok __attribute__((unused)) = [block appendLine:prefix
                                                         length:prefix_len
                                                        partial:YES
                                                          width:width
                                                       metadata:prefixMetadata
        ...
        // enough room for it.
        BOOL ok __attribute__((unused)) = [block appendLine:buffer
                                                     length:length
                                                    partial:partial
                                                      width:width
                                                   metadata:metadataObj
...
- (iTermImmutableMetadata)metadataForLineNumber:(int)lineNumber width:(int)width {
    int remainder = 0;
    LineBlock *block = [_lineBlocks blockContainingLineNumber:lineNumber width:width remainder:&remainder];
...
- (int)copyLineToBuffer:(screen_char_t *)buffer
    ...
    if (!block) {
        DLog(@"Couldn't find line %d", lineNum);
#if DEBUG
        [self sanityCheck];
#endif
    ...
    block.debugInfo = ^NSString * {
        ...
        return [NSString stringWithFormat:@"Block index %@, width=%@, lineNum=%@, remainder=%@\n%@", @(i), @(width),
                                          @(lineNum), @(remainder), lineBlockDump];
    };
...
- (ScreenCharArray *)screenCharArrayForLine:(int)line
    ...
    if (!block) {
        ITAssertWithMessage(
            NO, @"Failed to find line %@ with width %@. Cache is: %@", @(line), @(width),
...
- (ScreenCharArray *)wrappedLineAtIndex:(int)lineNum width:(int)width continuation:(screen_char_t *)continuationPtr {
    int remainder = 0;
    LineBlock *block = [_lineBlocks blockContainingLineNumber:lineNum width:width remainder:&remainder];
    if (!block) {
        // BUG-f875: Return nil gracefully instead of crashing when line not found.
        // This can happen legitimately when requesting a line from an empty buffer
        // or when the requested line index is out of bounds.
        DLog(@"wrappedLineAtIndex: No block found for line %d with width %d (buffer may be empty or index out of "
             @"bounds)",
             lineNum, width);
        return nil;
    }
    ...
    if (!line) {
        // BUG-f876: Return nil gracefully instead of crashing when line data not found.
        DLog(@"wrappedLineAtIndex: getWrappedLineWithWrapWidth returned nil for line %d", lineNum);
        return nil;
    }
    ScreenCharArray *result = [[ScreenCharArray alloc] initWithLine:line
                                                           ...
                                                           metadata:metadata
                                                       continuation:continuation];
    // BUG-f877: Log warning instead of crashing if result length exceeds width.
    if (result.length > width) {
        DLog(@"wrappedLineAtIndex: Result length %d exceeds width %d", (int)result.length, width);
    }
    return result;
}
...
- (ScreenCharArray *_Nonnull)rawLineAtWrappedLine:(int)lineNum width:(int)width {
    int remainder = 0;
    LineBlock *block = [_lineBlocks blockContainingLineNumber:lineNum width:width remainder:&remainder];
    // BUG-f878: Return empty array instead of crashing when block not found.
    if (!block) {
        DLog(@"rawLineAtWrappedLine: No block found for line %d with width %d", lineNum, width);
        return [[ScreenCharArray alloc] init];
    }
...
- (NSArray<ScreenCharArray *> *)wrappedLinesFromIndex:(int)lineNum width:(int)width count:(int)count {
    ...
    // Pre-allocate for expected line count - avoids array growth during enumeration
    NSMutableArray<ScreenCharArray *> *arrays = [NSMutableArray arrayWithCapacity:count];
    [_lineBlocks
        enumerateLinesInRange:NSMakeRange(lineNum, count)
                        width:width
                        block:^(const screen_char_t *_Nonnull chars, int length, int eol, screen_char_t continuation,
                                iTermImmutableMetadata metadata, BOOL *_Nonnull stop) {
...
- (BOOL)popAndCopyLastLineInto:(screen_char_t *)ptr
    ...
    *includesEndOfLine = [block hasPartial] ? EOL_SOFT : EOL_HARD;
    ...
    const screen_char_t *temp;
    screen_char_t continuation;
    BOOL ok __attribute__((unused)) = [block popLastLineInto:&temp
                                                  withLength:&length
                                                   upToWidth:width
...
- (BOOL)getCursorInLastLineWithWidth:(int)width atX:(int *)x {
    ...
    if (cursor_rawline == total_raw_lines - 1) {
        ...
        int min_x =
            OffsetOfWrappedLine(lastRawLine, num_overflow_lines, last_line_length, width, _mayHaveDoubleWidthCharacter);
...
- (void)prepareToSearchFor:(NSString *)substring
    ...
    // Search results typically start small and may grow
    context.results = [NSMutableArray arrayWithCapacity:16];
}
...
- (void)findSubstring:(FindContext *)context stopAt:(LineBufferPosition *)stopPosition {
    ...
    // BUG-f875, BUG-f876: Replace asserts with guards that return gracefully
    if (blockIndex < 0) {
        DLog(@"BUG-f875: blockIndex (%ld) is negative in stepInFindContext", (long)blockIndex);
        context.status = NotFound;
        return;
    }
    if (blockIndex >= numBlocks) {
        DLog(@"BUG-f876: blockIndex (%ld) >= numBlocks (%ld) in stepInFindContext", (long)blockIndex, (long)numBlocks);
        context.status = NotFound;
        return;
    }
    LineBlock *block = _lineBlocks[blockIndex];

    LineBlockRelativeLineProvider lineProvider = nil;
    if ((context.options & FindOptMultiLine) != 0) {
        const NSInteger providerBaseIndex = blockIndex;
        iTermLineBlockArray *blocks = _lineBlocks;
        const NSInteger blockCount = numBlocks;
        lineProvider =
            ^BOOL(LineBlock *startBlock, int startEntry, int relativeLineIndex, LineBlock **outBlock, int *outEntry) {
                if (relativeLineIndex < 0) {
                    return NO;
                }
                NSInteger currentIndex = providerBaseIndex;
                LineBlock *currentActualBlock = startBlock;
                if (!currentActualBlock) {
                    currentActualBlock = LineBufferSearchBlockForStoredBlock(blocks[currentIndex]);
                    if (!currentActualBlock) {
                        return NO;
                    }
                }
                int entry = startEntry;
                int remaining = relativeLineIndex;
                while (YES) {
                    const int firstEntry = currentActualBlock.firstEntry;
                    if (entry < firstEntry) {
                        entry = firstEntry;
                    }
                    const int available = [currentActualBlock numEntries] - entry;
                    if (available > 0) {
                        if (remaining < available) {
                            if (outBlock) {
                                *outBlock = currentActualBlock;
                            }
                            if (outEntry) {
                                *outEntry = entry + remaining;
                            }
                            return YES;
                        }
                        remaining -= available;
                    ...
                    LineBlock *nextActual = LineBufferSearchBlockForStoredBlock(blocks[currentIndex]);
                    if (!nextActual) {
                        return NO;
                    }
                    currentActualBlock = nextActual;
                    entry = currentActualBlock.firstEntry;
                }
            };
    }

    if (blockIndex == 0 && context.offset != -1 && context.offset < [block startOffset]) {
        if (context.dir > 0) {
    ...
    const NSRange blockAbsolutePositions =
        NSMakeRange([self absPositionOfAbsBlock:blockIndex + num_dropped_blocks], block.rawSpaceUsed);
    ...
    // RC-029: Yes, rawSpaceUsed is correct here even when lines are dropped from the first block.
    // The position calculation works because:
    // 1. blockPosition = cumulative rawSpaceUsed of all prior blocks (0 for first block)
    // 2. blockSize = rawSpaceUsed of this block (includes dropped portion for first block)
    // 3. stopAt = absolutePosition - droppedChars (already adjusted for globally dropped chars)
    // The rawSpaceUsed values form a consistent coordinate system with droppedChars offset.
    const int blockSize = _lineBlocks.blocks[blockIndex].rawSpaceUsed;
    ...
    if (context.dir > 0 && blockPosition >= stopAt) {
        ...
        DLog(@"status<-NotFound because dir<0, blockPosition(%@)+blockSize(%@)<stopAt(%@)", @(blockPosition),
             @(blockSize), @(stopAt));
        context.status = NotFound;
    } else {
...
- (NSArray<XYRange *> *_Nullable)convertPositions:(NSArray<ResultRange *> *_Nonnull)resultRanges
    ...
    const NSUInteger rangeCount = resultRanges.count;
    if (rangeCount == 0) {
        return @[];
    }

    NSUInteger sortedCount = 0;
    LineBufferSortedPosition *sortedPositions = SortedPositionsFromResultRanges(resultRanges, !expanded, &sortedCount);
    if (sortedCount == 0 || _lineBlocks.count == 0) {
    ...
    int blockIndex = 0;
    int yoffset = 0;
    const int numBlocks = _lineBlocks.count;
    LineBlock *block = _lineBlocks[blockIndex];
    int used = [block rawSpaceUsed];
    int passed = 0;

    VT100GridCoord *startCoords = NULL;
    VT100GridCoord *endCoords = NULL;
    uint8_t *hasStart = NULL;
    uint8_t *hasEnd = NULL;
    if (!expanded) {
        startCoords = calloc(rangeCount, sizeof(VT100GridCoord));
        endCoords = calloc(rangeCount, sizeof(VT100GridCoord));
        hasStart = (uint8_t *)calloc(rangeCount, sizeof(uint8_t));
        hasEnd = (uint8_t *)calloc(rangeCount, sizeof(uint8_t));
        if (!startCoords || !endCoords || !hasStart || !hasEnd) {
            free(sortedPositions);
            free(startCoords);
            free(endCoords);
            free(hasStart);
    ...
    NSMutableArray<XYRange *> *expandedXYRanges = expanded ? [NSMutableArray arrayWithCapacity:rangeCount] : nil;

    VT100GridCoord cachedCoord = VT100GridCoordMake(0, 0);
    int cachedAbsolutePosition = INT_MIN;
    BOOL haveCachedCoord = NO;
    int lastPositionToConvert = -1;

    for (NSUInteger idx = 0; idx < sortedCount; idx++) {
        const LineBufferSortedPosition entry = sortedPositions[idx];
        const int position = entry.position;

        while (position >= passed + used && blockIndex < numBlocks) {
            passed += used;
            ...
            block = _lineBlocks.blocks[blockIndex];
        ...
        const int baseOffset = position - passed;
        const int positionToConvert = expanded ? [block offsetOfStartOfLineIncludingOffset:baseOffset] : baseOffset;
        if (expanded) {
        ...
        const int absolutePositionToConvert = positionToConvert + passed;
        VT100GridCoord startCoord;
        if (haveCachedCoord && cachedAbsolutePosition == absolutePositionToConvert) {
            startCoord = cachedCoord;
        } else {
            int x = 0;
            int y = 0;
            const BOOL ok = [block convertPosition:positionToConvert withWidth:width wrapOnEOL:YES toX:&x toY:&y];
            if (!ok) {
                continue;
            }
            startCoord = VT100GridCoordMake(x, y + yoffset);
            cachedCoord = startCoord;
            cachedAbsolutePosition = absolutePositionToConvert;
            haveCachedCoord = YES;
        }

        if (!expanded) {
            const NSUInteger index = entry.rangeIndex;
            if (index >= rangeCount) {
                continue;
            }
            if (entry.isEnd) {
                endCoords[index] = startCoord;
                hasEnd[index] = 1;
            } else {
                startCoords[index] = startCoord;
                hasStart[index] = 1;
            }
            continue;
        }
        ...
        int rawLineNumber = 0;
        const BOOL hasRawLine = [block rawLineNumberAtWrappedLineOffset:(startCoord.y - yoffset)
                                                                  width:width
                                                          rawLineNumber:&rawLineNumber];
        if (!hasRawLine) {
            continue;
        }
        const int length = [block lengthOfRawLine:rawLineNumber];
        ResultRange *rawLineRange = [[ResultRange alloc] initWithPosition:absolutePositionToConvert length:length];
        [expandedResultRanges addObject:rawLineRange];

        const int endPositionToConvert = positionToConvert + length - 1;
        const int endAbsolutePosition = absolutePositionToConvert + length - 1;
        VT100GridCoord endCoord;
        if (haveCachedCoord && cachedAbsolutePosition == endAbsolutePosition) {
            endCoord = cachedCoord;
        } else {
            int endX = 0;
            int endY = 0;
            const BOOL endOk = [block convertPosition:endPositionToConvert
                                            withWidth:width
                                            wrapOnEOL:YES
                                                  toX:&endX
                                                  toY:&endY];
            ...
            endCoord = VT100GridCoordMake(endX, endY + yoffset);
            cachedCoord = endCoord;
            cachedAbsolutePosition = endAbsolutePosition;
            haveCachedCoord = YES;
        }

        XYRange *expandedRange = [[XYRange alloc] init];
        expandedRange->xStart = startCoord.x;
        expandedRange->yStart = startCoord.y;
        expandedRange->xEnd = endCoord.x;
        expandedRange->yEnd = endCoord.y;
        [expandedXYRanges addObject:expandedRange];
    }

    free(sortedPositions);

    if (!expanded) {
        NSMutableArray<XYRange *> *result = [NSMutableArray arrayWithCapacity:rangeCount];
        for (NSUInteger idx = 0; idx < rangeCount; idx++) {
            if (!hasStart[idx] || !hasEnd[idx]) {
                continue;
            }
            XYRange *xyrange = [[XYRange alloc] init];
            xyrange->xStart = startCoords[idx].x;
            xyrange->yStart = startCoords[idx].y;
            xyrange->xEnd = endCoords[idx].x;
            xyrange->yEnd = endCoords[idx].y;
...
- (LineBufferPosition *)positionOfFindContext:(FindContext *)context width:(int)width {
    ...
    if (context.absBlockNum - num_dropped_blocks >= _lineBlocks.count) {
        DLog(@"Position of find context (%@-%@=%@) is after last block (%@)", @(context.absBlockNum),
             @(num_dropped_blocks), @(context.absBlockNum - num_dropped_blocks), @(_lineBlocks.count));
...
- (LineBufferPosition *)positionForCoordinate:(VT100GridCoord)coord width:(int)width offset:(int)offset {
    ...
    BOOL extends = NO;
    pos = [block getPositionOfLine:&line atX:x withWidth:width yOffset:&yOffset extends:&extends];
    if (pos < 0) {
...
- (VT100GridCoord)coordinateForPosition:(LineBufferPosition *)position
    ...
    if (position.absolutePosition == self.lastPosition.absolutePosition) {
        ...
        // BUG-f879: Check for empty buffer before trying to access lines.
        // If buffer is empty, there are no valid coordinates to return.
        const int numLines = [self numLinesWithWidth:width];
        if (numLines == 0) {
            VLog(@"coordinateForPosition: buffer is empty, returning error");
            if (ok) {
                *ok = NO;
            }
        ...
        ScreenCharArray *lastLine = [self wrappedLineAtIndex:result.y width:width continuation:NULL];
        // BUG-f880: Handle nil lastLine gracefully (should not happen after above check, but be defensive)
        result.x = lastLine ? lastLine.length : 0;
    ...
    int p;
    int yoffset;
    VLog(@"coordinateForPosition: find block with position %@, yoffset=%@", @(position.absolutePosition - droppedChars),
         @(position.yOffset));
...
- (LineBufferPosition *)positionForStartOfLastLine {
    ...
    // BUG-f877, BUG-f878: Replace asserts with graceful handling
    if (length < 0) {
        DLog(@"BUG-f877: lengthOfLastLine returned negative value: %lld", length);
        return position;
    }
    if (length > position.absolutePosition) {
        DLog(@"BUG-f878: length (%lld) > absolutePosition (%lld) - clamping", length, position.absolutePosition);
...
- (long long)absPositionOfFindContext:(FindContext *)findContext {
    if (findContext.absBlockNum < 0) {
        ...
        return findContext.offset + droppedChars +
               [_lineBlocks rawSpaceUsedInRangeOfBlocks:NSMakeRange(0, _lineBlocks.count)];
    }
    const int numBlocks = MIN(_lineBlocks.count, findContext.absBlockNum - num_dropped_blocks);
    const NSInteger rawSpaceUsed =
        numBlocks > 0 ? [_lineBlocks rawSpaceUsedInRangeOfBlocks:NSMakeRange(0, numBlocks)] : 0;
...
- (int)positionForAbsPosition:(long long)absPosition {
    ...
    if (absPosition < 0) {
        // RC-008 ROOT CAUSE FIX: Check if truncation is in progress.
        // During truncation, _lineBlocks may be temporarily empty, which is
        // expected. Also guard against empty _lineBlocks as defense-in-depth.
        if (_truncationInProgress || _lineBlocks.count == 0) {
            DLog(@"RC-008: positionForAbsPosition returning 0 (truncationInProgress=%d, count=%lu)",
                 _truncationInProgress, (unsigned long)_lineBlocks.count);
            return 0;
        }
        return [_lineBlocks[0] startOffset];
    }
    // BUG-1432: Document truncation behavior. If absPosition exceeds INT_MAX,
    // clamping is necessary to avoid undefined behavior from the cast, but
    // callers should be aware that positions this large may lose precision.
    // In practice, INT_MAX (~2 billion) characters exceeds realistic terminal
    // scrollback sizes, so this is a defensive guard rather than expected behavior.
    if (absPosition > INT_MAX) {
        DLog(@"positionForAbsPosition: truncating position %lld to INT_MAX", absPosition);
        absPosition = INT_MAX;
    }
...
- (void)storeLocationOfAbsPos:(long long)absPos inContext:(FindContext *)context {
...
- (BOOL)encodeBlocks:(id<iTermEncoderAdapter>)encoder maxLines:(NSInteger)maxLines {
    __block BOOL truncated = NO;
    __block NSInteger numLines = 0;

    iTermOrderedDictionary<NSString *, LineBlock *> *index = [iTermOrderedDictionary
        byMappingEnumerator:_lineBlocks.blocks.reverseObjectEnumerator
                      block:^id _Nonnull(NSUInteger index, LineBlock *_Nonnull block) {
                          DLog(@"Maybe encode block %p with guid %@", block, block.stringUniqueIdentifier);
                          return block.stringUniqueIdentifier;
                      }];
    [encoder
        encodeArrayWithKey:kLineBufferBlocksKey
                ...
                generation:iTermGenerationAlwaysEncode
                   options:iTermGraphEncoderArrayOptionsReverse
                     block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder, NSInteger i, NSString *_Nonnull identifier,
                         ...
                         DLog(@"Encode %@ with identifier %@ and generation %@", block, identifier,
                              @(block.generation));
                         return [encoder
                             encodeDictionaryWithKey:kLineBufferBlockWrapperKey
                                          generation:block.generation
                                               block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                                                   // BUG-f879: Replace assert with graceful early return
                                                   if (truncated) {
                                                       DLog(@"BUG-f879: Attempted to encode block after truncation");
                                                       return NO;
                                                   }
                                                   DLog(@"Really encode block %p with guid %@", block,
                                                        block.stringUniqueIdentifier);
...
- (void)appendMessage:(NSString *)message {
    ...
    [self appendLine:buffer
            ...
            metadata:iTermMetadataMakeImmutable(
                         iTermMetadataTemporaryWithTimestamp([NSDate timeIntervalSinceReferenceDate]))
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    // Phase 1C: Copy secondary cache as well
    for (int i = 0; i < 3; i++) {
        theCopy->_multiWidthCacheWidths[i] = _multiWidthCacheWidths[i];
        theCopy->_multiWidthCacheCounts[i] = _multiWidthCacheCounts[i];
    }
...
- (LineBuffer *)copyWithMinimumLines:(int)minLines atWidth:(int)width {
    ...
    InvalidateWrappedLinesCache(theCopy);
...
- (void)beginResizing {
    // BUG-f880: Replace assert with graceful handling - if already resizing, just return
    if (_lineBlocks.resizing) {
        DLog(@"BUG-f880: beginResizing called but already resizing - ignoring");
        return;
    }
...
- (void)endResizing {
    // BUG-f881: Replace assert with graceful handling - if not resizing, just return
    if (!_lineBlocks.resizing) {
        DLog(@"BUG-f881: endResizing called but not resizing - ignoring");
        return;
    }
...
- (LineBufferPackingStats)packingStats {
    LineBufferPackingStats stats = {0};
    stats.totalBlocks = _lineBlocks.count;
    for (LineBlock *block in _lineBlocks.blocks) {
        const int rawLines = MAX(0, [block numRawLines]);
        stats.totalRawLines += rawLines;
        const int rawChars = MAX(0, [block rawSpaceUsed]);
        stats.totalCharacters += rawChars;
        const uint64_t unpackedBytes = (uint64_t)rawChars * sizeof(screen_char_t);
        stats.estimatedUnpackedBytes += unpackedBytes;

        if ([block isKindOfClass:[LineBlockPacked class]]) {
            LineBlockPacked *packed = (LineBlockPacked *)block;
            stats.packedBlocks += 1;
            stats.packedRawLines += rawLines;
            stats.packedCharacters += rawChars;
            stats.currentBytes += packed.packedMemoryUsage;
            stats.estimatedBytesSaved += packed.memorySaved;
        } else {
            stats.currentBytes += unpackedBytes;
        }
    }

    if (stats.estimatedBytesSaved > stats.estimatedUnpackedBytes) {
        stats.estimatedBytesSaved = stats.estimatedUnpackedBytes;
    }
...
- (void)seal {
    ...
    // Pack older blocks for memory savings.
    // Only pack blocks that are at least 2 positions from the end to ensure they're stable.
    [self _packOldBlocksIfNeeded];
}

- (void)_packOldBlocksIfNeeded {
    // Need at least 3 blocks: older packed blocks, one recently sealed block, and the active block
    if (_lineBlocks.count < 3) {
        return;
    }

    // Create shared color table on first pack operation
    if (!_packedColorTable) {
        _packedColorTable = [[PackedColorTable alloc] init];
    }

    // Pack all blocks except the last two (recently sealed and active)
    NSInteger limit = _lineBlocks.count - 2;
    NSUInteger newlyPackedBlocks = 0;
    uint64_t newlySavedBytes = 0;
    for (NSUInteger i = 0; i < limit; i++) {
        if (![_lineBlocks isBlockPackedAtIndex:i]) {
            if ([_lineBlocks replaceBlockAtIndex:i withPackedBlockUsingColorTable:_packedColorTable]) {
                newlyPackedBlocks++;
                if (gDebugLogging) {
                    id<iTermLineBlockReading> packedBlock = [_lineBlocks readableBlockAtIndex:i];
                    if ([packedBlock isKindOfClass:[LineBlockPacked class]]) {
                        newlySavedBytes += ((LineBlockPacked *)packedBlock).memorySaved;
                    }
                }
            }
        }
    ...
    if (newlyPackedBlocks > 0 && gDebugLogging) {
        const double bytesToMB = 1024.0 * 1024.0;
        LineBufferPackingStats stats = [self packingStats];
        DLog(@"LineBuffer packed %lu block(s); saved %.1f MB this seal (%.1f MB total saved, %.1f MB in use, %lu/%lu "
             @"blocks packed)",
             (unsigned long)newlyPackedBlocks, (double)newlySavedBytes / bytesToMB,
             (double)stats.estimatedBytesSaved / bytesToMB, (double)stats.currentBytes / bytesToMB,
             (unsigned long)stats.packedBlocks, (unsigned long)stats.totalBlocks);
    }
}

- (void)forceSeal {
    // BUG-f882, BUG-f883, BUG-f884: Replace asserts with graceful handling
    if (_lineBlocks.lastBlock.hasPartial) {
        DLog(@"BUG-f882: forceSeal called but lastBlock has partial - cannot seal");
        return;
    }
    if (_lineBlocks.lastBlock == nil) {
        DLog(@"BUG-f883: forceSeal called but lastBlock is nil - nothing to seal");
        return;
    }
    if (_lineBlocks.lastBlock.isEmpty) {
        DLog(@"BUG-f884: forceSeal called but lastBlock is empty - nothing to seal");
        return;
    }
    self.dirty = YES;
    [self _addBlockOfSize:block_size];
...
- (void)reallyMergeFrom:(LineBuffer *)source {
    ...
    // BUG-f885: Replace assert with graceful early return
    if (source == nil) {
        DLog(@"BUG-f885: reallyMergeFrom called with nil source - ignoring");
        return;
    }
    if (!source.dirty) {
        return;
    ...
    // Phase 1C: Copy secondary cache
    for (int i = 0; i < 3; i++) {
        _multiWidthCacheWidths[i] = source->_multiWidthCacheWidths[i];
        _multiWidthCacheCounts[i] = source->_multiWidthCacheCounts[i];
    }
    droppedChars = source->droppedChars;
    _mayHaveDoubleWidthCharacter = source->_mayHaveDoubleWidthCharacter;

    // Drop initial blocks
    while (_lineBlocks.firstBlock != nil && _lineBlocks.firstBlock.progenitor != source->_lineBlocks.firstBlock) {
        DLog(@"Drop initial");
        [_lineBlocks removeFirstBlock];
        ++num_dropped_blocks;
    }
...
- (long long)safeAbsoluteBlockNumber:(long long)unsafe {
    // BUG-f886: Replace assert with graceful handling - return 0 if no blocks
    if (_lineBlocks.count == 0) {
        DLog(@"BUG-f886: safeAbsoluteBlockNumber called with no blocks - returning 0");
        return num_dropped_blocks;
    }
...
- (NSInteger)numberOfCellsUsedInWrappedLineRange:(VT100GridRange)wrappedLineRange width:(int)width {
    ...
    int lastLine = y2;
    const NSInteger lastBlockIndex = [_lineBlocks indexOfBlockContainingLineNumber:y2 width:width remainder:&lastLine];
...
- (NSInteger)numberOfUnwrappedLinesInRange:(VT100GridRange)range width:(int)width {
    return [_lineBlocks numberOfRawLinesInRange:NSMakeRange(range.location, range.length) width:width];
}
