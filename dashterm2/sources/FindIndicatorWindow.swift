...
private class FindIndicatorView: NSView, CALayerDelegate {
    ...
    required init?(coder: NSCoder) {
        // BUG-f828: Return nil instead of crashing for unused coder initializer
        DLog("AnimationDisablingView init(coder:) is not supported")
        return nil
    }
...
private class FindIndicatorWindowContentView: NSView, CALayerDelegate {
    ...
    required init?(coder: NSCoder) {
        // BUG-f829: Return nil instead of crashing for unused coder initializer
        DLog("FindIndicatorWindowContentView init(coder:) is not supported")
        return nil
    }
...
class FindIndicatorWindow: NSWindow, PTYTrackingChildWindow {
    ...
    private init?(image: NSImage, parent: NSView, rect: NSRect, firstVisibleLine: Int64) {
        ...
        // BUG-1659: Use guard let instead of force unwrap for contentView
        guard let existingContentView = contentView else { return }
        contentView = FindIndicatorWindowContentView(frame: existingContentView.bounds)
        ...
        let rectInScreen = parentWindow.convertToScreen(rectInParentWindow)
        let rectInMyWindow = convertFromScreen(rectInScreen)
        // BUG-1659: Use guard let instead of force unwrap for contentView
        guard let currentContentView = contentView else { return }
        let rectInContentView = currentContentView.convert(rectInMyWindow, from: nil)
...
extension FindIndicatorWindow: CAAnimationDelegate {
    ...
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        // BUG-9480: Clear delegate to prevent potential crashes if animation is referenced again
        anim.delegate = nil
        // Also remove the animation from the layer to clean up properly
