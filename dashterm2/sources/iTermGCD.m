...
static const char *iTermGCDMutationQueueLabel = "com.dashterm.dashterm2.mutation";
...
+ (void)assertMainQueueSafe {
    ...
    // BUG-f1012: Log error instead of crashing in production
    if (addr != &iTermGCDSpecificMainQueueSafe_Yes) {
        ELog(@"BUG-f1012: assertMainQueueSafe called from non-main-queue-safe context");
    }
}
...
+ (void)assertMutationQueueSafe {
    ...
    // BUG-f1013: Log error instead of crashing in production
    if (addr != &iTermGCDSpecificMutationQueueSafe_Yes) {
        ELog(@"BUG-f1013: assertMutationQueueSafe called from non-mutation-queue-safe context");
    }
}
