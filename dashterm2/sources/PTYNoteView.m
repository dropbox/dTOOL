...
- (NSColor *)defaultBackgroundColor {
    return [NSColor colorWithCalibratedRed:252.0 / 255.0 green:250.0 / 255.0 blue:198.0 / 255.0 alpha:1];
}

- (NSColor *)borderColor {
    return [NSColor colorWithCalibratedRed:255.0 / 255.0 green:229.0 / 255.0 blue:114.0 / 255.0 alpha:0.95];
}
...
- (NSPoint)leadingCornerOfRoundedRect:(NSRect)frame radius:(CGFloat)radius dx:(int)dx dy:(int)dy {
    // BUG-f1363: Convert assert to guard - invalid dx/dy should return safe default
    if (!((dx == 0 || dy == 0) && ((dx == 0) ^ (dy == 0)))) {
        // Invalid parameters - return frame origin as safe default
        return frame.origin;
    }
...
- (NSPoint)trailingCornerOfRoundedRect:(NSRect)frame radius:(CGFloat)radius dx:(int)dx dy:(int)dy {
    // BUG-f1364: Convert assert to guard - invalid dx/dy should return safe default
    if (!((dx == 0 || dy == 0) && ((dx == 0) ^ (dy == 0)))) {
        // Invalid parameters - return frame origin as safe default
        return frame.origin;
    }
...
- (NSPoint)controlPointOfRect:(NSRect)rect forDx:(int)dx dy:(int)dy {
    if (dx == 0 && dy == -1) {
        ...
        // BUG-f642: Replace assert(false) with DLog and fallback - invalid dx/dy should not crash
        DLog(@"BUG-f642: controlPointOfRect called with unexpected dx=%d dy=%d", dx, dy);
        return rect.origin; // Fallback to origin
    }
}

- (NSPoint)projectionOfPoint:(NSPoint)p ontoEdge:(PTYNoteViewTipEdge)edge ofFrame:(NSRect)frame {
    switch (edge) {
        ...
        default:
            // BUG-f643: Replace assert(false) with DLog and fallback - unknown edge should not crash
            DLog(@"BUG-f643: projectionOfPoint called with unknown edge %d", (int)edge);
            return p; // Fallback to original point
    }
}
...
- (NSBezierPath *)roundedRectangleWithPointerInRect:(NSRect)frame
    ...
    NSRect bubbleFrame = [self bubbleFrameInRect:frame inset:inset pointerLength:pointerLength tipOnEdge:tipEdge];
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = [self leadingCornerOfRoundedRect:bubbleFrame radius:radius dx:dx dy:dy];

        if (edge == tipEdge) {
            // This edge has the arrow. Compute the points where the arrow's base intersects
            // the edge.
            NSPoint baseCenter = [self projectionOfPoint:tipPoint ontoEdge:edge ofFrame:bubbleFrame];
            NSPoint bases[2];
            bases[0] = NSMakePoint(baseCenter.x - dx * pointerBase / 2, baseCenter.y - dy * pointerBase / 2);
            bases[1] = NSMakePoint(baseCenter.x + dx * pointerBase / 2, baseCenter.y + dy * pointerBase / 2);
        ...
        p = [self trailingCornerOfRoundedRect:bubbleFrame radius:radius dx:dx dy:dy];
...
- (void)mouseDown:(NSEvent *)theEvent {
    ...
    struct {
        ...
        BOOL horizontal;
        BOOL bottom;
    } regions[] = {{rightDragRegion, YES, NO}, {bottomRightDragRegion, YES, YES}, {bottomDragRegion, NO, YES}};
...
- (void)mouseDragged:(NSEvent *)theEvent {
    if (dragRight_ || dragBottom_) {
        ...
        self.frame =
            NSMakeRect(self.frame.origin.x, self.frame.origin.y, MAX(kMinWidth, ceil(originalSize_.width + dw)),
...
- (NSRect)rightDragRect {
    NSRect bubbleFrame = [self bubbleFrameInRect:self.bounds
                                   ...
                                   pointerLength:kPointerLength
                                       tipOnEdge:self.tipEdge];
    return FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kDragAreaSize, NSMinY(bubbleFrame) + kDragAreaSize, kDragAreaSize,
...
- (NSRect)bottomRightDragRect {
    NSRect bubbleFrame = [self bubbleFrameInRect:self.bounds
                                   ...
                                   pointerLength:kPointerLength
                                       tipOnEdge:self.tipEdge];
    return FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kDragAreaSize, NSMaxY(bubbleFrame) - kDragAreaSize, kDragAreaSize,
                               kDragAreaSize),
                    self.frame.size.height);
}
...
- (NSRect)bottomDragRect {
    NSRect bubbleFrame = [self bubbleFrameInRect:self.bounds
                                   ...
                                   pointerLength:kPointerLength
                                       tipOnEdge:self.tipEdge];
    return FlipRect(NSMakeRect(NSMinX(bubbleFrame), NSMaxY(bubbleFrame) - kDragAreaSize,
                               NSWidth(bubbleFrame) - kDragAreaSize, kDragAreaSize),
                    self.frame.size.height);
}
...
- (void)layoutSubviews {
    ...
    self.contentView.frame = FlipRect(NSMakeRect(NSMinX(bubbleFrame) + kLeftMargin, NSMinY(bubbleFrame) + kTopMargin,
    ...
    killButton_.frame = FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kButtonSize,
                                            NSMinY(bubbleFrame) + kKillButtonTopMargin, kButtonSize, kButtonSize),
                                 self.frame.size.height);
...
- (NSSize)sizeThatFitsContentView {
    switch (tipEdge_) {
        ...
        default:
            // BUG-f644: Replace assert(false) with DLog and fallback - unknown edge should not crash
            DLog(@"BUG-f644: sizeThatFitsContentView called with unknown tipEdge %d", (int)tipEdge_);
            return NSMakeSize(contentView_.frame.size.width + kLeftMargin + kRightMargin + kInset,
                              contentView_.frame.size.height + kBottomMargin + kTopMargin + kInset);
    }
}
