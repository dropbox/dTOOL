...
struct RemoteCommand: Codable {
    ...
    func isSafe() async -> Bool {
        ...
        case .executeCommand(let command):
                ...
                if AIAvailabilityProbe.check() {
                    ...
                    if UserDefaults.standard.object(forKey: kPreferenceKeyAISafetyCheck) == nil &&
                        ...
                        let selection = iTermWarning.show(
                            withTitle: "DashTerm2 can use Apple Intelligence to check the safety of commands suggested by your AI agent. Would you like to enable safety checking?\n\nWhen enabled, commands may be sent to Apple's servers for safety checking.",
                            actions: ["OK", "Cancel"],
                            accessory: nil,
                            identifier: nil,
            ...
            #endif
            _ = command  // Silence unused variable warning when FoundationModels not available
            return true
        }
    }
...
extension RemoteCommand.Content {
    ...
    // BUG-1701: Use as? instead of as! to prevent crashes on type mismatch
    func withValue(_ value: Any) -> RemoteCommand.Content? {
        switch self {
        case .isAtPrompt: (value as? RemoteCommand.IsAtPrompt).map { .isAtPrompt($0) }
        case .executeCommand: (value as? RemoteCommand.ExecuteCommand).map { .executeCommand($0) }
        case .getLastExitStatus: (value as? RemoteCommand.GetLastExitStatus).map { .getLastExitStatus($0) }
        case .getCommandHistory: (value as? RemoteCommand.GetCommandHistory).map { .getCommandHistory($0) }
        case .getLastCommand: (value as? RemoteCommand.GetLastCommand).map { .getLastCommand($0) }
        case .getCommandBeforeCursor: (value as? RemoteCommand.GetCommandBeforeCursor).map { .getCommandBeforeCursor($0) }
        case .searchCommandHistory: (value as? RemoteCommand.SearchCommandHistory).map { .searchCommandHistory($0) }
        case .getCommandOutput: (value as? RemoteCommand.GetCommandOutput).map { .getCommandOutput($0) }
        case .getTerminalSize: (value as? RemoteCommand.GetTerminalSize).map { .getTerminalSize($0) }
        case .getShellType: (value as? RemoteCommand.GetShellType).map { .getShellType($0) }
        case .detectSSHSession: (value as? RemoteCommand.DetectSSHSession).map { .detectSSHSession($0) }
        case .getRemoteHostname: (value as? RemoteCommand.GetRemoteHostname).map { .getRemoteHostname($0) }
        case .getUserIdentity: (value as? RemoteCommand.GetUserIdentity).map { .getUserIdentity($0) }
        case .getCurrentDirectory: (value as? RemoteCommand.GetCurrentDirectory).map { .getCurrentDirectory($0) }
        case .setClipboard: (value as? RemoteCommand.SetClipboard).map { .setClipboard($0) }
        case .insertTextAtCursor: (value as? RemoteCommand.InsertTextAtCursor).map { .insertTextAtCursor($0) }
        case .deleteCurrentLine: (value as? RemoteCommand.DeleteCurrentLine).map { .deleteCurrentLine($0) }
        case .getManPage: (value as? RemoteCommand.GetManPage).map { .getManPage($0) }
        case .createFile: (value as? RemoteCommand.CreateFile).map { .createFile($0) }
        case .searchBrowser: (value as? RemoteCommand.SearchBrowser).map { .searchBrowser($0) }
        case .loadURL: (value as? RemoteCommand.LoadURL).map { .loadURL($0) }
        case .webSearch: (value as? RemoteCommand.WebSearch).map { .webSearch($0) }
        case .getURL: (value as? RemoteCommand.GetURL).map { .getURL($0) }
        case .readWebPage: (value as? RemoteCommand.ReadWebPage).map { .readWebPage($0) }
        }
    }
}
