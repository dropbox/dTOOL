...
- (instancetype)init {
  self = [super init];
  if (self) {
      _guidToPathMap = [[NSMutableDictionary alloc] initWithCapacity:16];
      NSString *path = [self dynamicProfilesPath];
      if (path == nil) {
          ELog(@"Dynamic profiles path is nil");
          return nil;
      }
      _pendingChanges = [NSMutableSet setWithCapacity:4];
...
- (iTermFilesAndFolders *)pathsToWatch {
    NSMutableSet<NSString *> *files = [NSMutableSet setWithCapacity:8];
    NSMutableSet<NSString *> *folders = [NSMutableSet setWithCapacity:4];
...
// Returns ~/Library/Application Support/DashTerm2/DynamicProfiles
...
- (void)reloadDynamicProfiles {
    // BUG-1144: Use early return instead of assert to avoid crash on recursive call
    if (_loading) {
...
- (void)reallyReloadDynamicProfiles {
    ...
    NSMutableArray *newProfiles = [NSMutableArray arrayWithCapacity:32];
    NSMutableSet *guids = [NSMutableSet setWithCapacity:32];
    NSMutableArray *fileNames = [NSMutableArray arrayWithCapacity:16];
...
- (NSArray<Profile *> *)profilesInFile:(NSString *)filename fileType:(iTermDynamicProfileFileType *)fileType {
    ...
    NSMutableArray *profiles = [NSMutableArray arrayWithCapacity:entries.count];
...
- (Profile *)prototypeForDynamicProfile:(Profile *)profile {
    ...
    // Failed. Show an error and fall back to default profile.
    // BUG-7297: Always return a valid profile to prevent nil dereference in caller
    NSString *feature;
    NSString *value;
    if (parentGUID) {
        value = parentGUID;
...
- (Profile *)profileForRewritingBasedOn:(Profile *)updatedProfile {
    ...
    if ([iTermAdvancedSettingsModel addDynamicTagToDynamicProfiles]) {
        ...
        NSArray<NSString *> *tags = [NSArray castFrom:stripped[KEY_TAGS]];
        if (tags) {
...
- (void)writeModifiedProfile:(Profile *)profile
    ...
    // BUG-f933: Guard against non-rewritable profile instead of crashing
    if (![[NSNumber castFrom:profile[KEY_DYNAMIC_PROFILE_REWRITABLE]] boolValue]) {
        ELog(@"iTermDynamicProfileManager: writeModifiedProfile called with non-rewritable profile");
        return;
    }
    if (!filename) {
        return;
