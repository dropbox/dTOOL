...
- (int)intForKey:(NSString *)key {
    if ([self keyHasSyntheticGetter:key]) {
        // BUG-1919: Use safe cast to NSNumber with fallback to iTermProfilePreferences
        NSNumber *value = [NSNumber castFrom:[self syntheticObjectForKey:key]];
        if (value) {
            return [value intValue];
        }
        // Fall through to use profile preferences if synthetic getter returns nil or wrong type
    }
    Profile *profile = [_delegate profilePreferencesCurrentProfile];
...
- (PreferenceInfo *)defineControl:(NSControl *)control
    ...
    // BUG-f1116: Replace assert with guard - log warning if delegate is nil
    if (!self.delegate) {
        DLog(@"Warning: defineControl called with nil delegate for key '%@'", key);
    }
...
- (PreferenceInfo *)defineControl:(NSControl *)control
                      ...
                      displayName:(NSString *)displayName // for search
                             type:(PreferenceInfoType)type {
    // BUG-f1117: Replace assert with guard - log warning if delegate is nil
    if (!self.delegate) {
        DLog(@"Warning: defineControl called with nil delegate for key '%@'", key);
    }
...
- (PreferenceInfo *)defineUnsearchableControl:(NSControl *)control key:(NSString *)key type:(PreferenceInfoType)type {
    // BUG-f1118: Replace assert with guard - log warning if delegate is nil
    if (!self.delegate) {
        DLog(@"Warning: defineUnsearchableControl called with nil delegate for key '%@'", key);
    }
...
- (PreferenceInfo *)defineControl:(NSControl *)control
    ...
    // BUG-f1119: Replace assert with guard - log warning if delegate is nil
    if (!self.delegate) {
        DLog(@"Warning: defineControl called with nil delegate for key '%@'", key);
    }
...
- (PreferenceInfo *)defineControl:(NSControl *)control
                   ...
                   settingChanged:(void (^)(id))settingChanged
                           update:(BOOL (^)(void))update {
    // BUG-f1120: Replace assert with guard - log warning if delegate is nil
    if (!self.delegate) {
        DLog(@"Warning: defineControl called with nil delegate for key '%@'", key);
    }
    return [super defineControl:control
...
- (BOOL)valueOfKeyEqualsDefaultValue:(NSString *)key {
    ...
    dispatch_once(&onceToken, ^{
        NSString *plistFile = [[NSBundle bundleForClass:[self class]] pathForResource:@"DefaultBookmark"
                                                                               ofType:@"plist"];
