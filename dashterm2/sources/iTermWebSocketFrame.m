...
+ (instancetype)frameWithDataSource:(unsigned char * (^)(int64_t))dataSource {
    ...
    if (payloadLength == 126) {
        ...
        memmove(&networkLength, data, sizeof(networkLength));
        uint64_t length64 = ntohll(networkLength);
        // Reject excessively large payloads (limit to 256MB to prevent memory exhaustion)
        const uint64_t kMaxPayloadLength = 256 * 1024 * 1024;
        if (length64 > kMaxPayloadLength) {
            XLog(@"WebSocket frame payload too large: %llu bytes (max %llu)", length64, kMaxPayloadLength);
            return nil;
        }
        payloadLength = (NSInteger)length64;
    }
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<%@: %p opcode=%@ fin=%@ payloadLength=%@>", NSStringFromClass([self class]),
                                      self, opcode, _fin ? @"YES" : @"NO", @(self.payload.length)];
}
...
- (NSData *)data {
    ...
    if (!_data) {
        ...
        // WebSocket frame: 2-10 byte header + payload
        NSMutableData *data = [NSMutableData dataWithCapacity:10 + self.payload.length];
        uint8_t byte = 0;
        if (self.fin) {
            DLog(@"Set fin bit");
            byte |= 0x80;
...
- (uint16_t)closeFrameCode {
    ITAssertWithMessage(self.opcode == iTermWebSocketOpcodeConnectionClose, @"Not a close frame");
...
- (NSString *)text {
    ITAssertWithMessage(self.opcode == iTermWebSocketOpcodeText, @"Not a text frame");
