...
extension AITermController.Message {
...
class ChatAgent {
    // BUG-405: Use late-initialized property pattern instead of implicitly unwrapped optional
    // to prevent crash if conversation is accessed before init completes
    ...
    // BUG-405: Use non-optional with initial value instead of implicitly unwrapped optional
    ...
    init(_ chatID: String,
        ...
        self.conversation = AIConversation(registrationProvider: registrationProvider)
    ...
    func fetchCompletion(userMessage: Message,
        ...
        case .setPermissions(let allowedCategories):
            ...
            return
        case .userCommand:
            // BUG-3022: User commands should not reach fetchCompletion - log and return nil
            DLog("Error: User commands should not reach fetchCompletion - ignoring")
            completion(nil)
            return
        case .vectorStoreCreated:
            // BUG-3023: User should not create vector store - log and return nil
            DLog("Error: User should not create vector store - ignoring")
            completion(nil)
            return
        case .appendAttachment:
            // BUG-3024: User-sent attachments not supported - log and return nil
            DLog("Error: User-sent attachments not supported - ignoring")
            completion(nil)
            return
        }
    ...
    private func requestRenaming() {
        ...
        renameConversation?.complete { [weak self] (result: Result<AIConversation, Error>) in
            ...
            // Always clean up the rename conversation when complete (success or failure)
    ...
    private static func message(completionText text: String,
        ...
        case .remoteCommandRequest, .selectSessionRequest, .clientLocal, .renameChat, .append,
            ...
            // BUG-3025: Unexpected message type for text response - return nil
            DLog("Error: Unexpected message type \(userMessage.content) for text response")
            return nil
        }
    }
    ...
    private static func message(attachment: LLM.Message.Attachment,
        ...
        case .remoteCommandRequest, .selectSessionRequest, .clientLocal, .renameChat, .append,
                .commit, .setPermissions, .appendAttachment, .vectorStoreCreated, .userCommand:
            // BUG-3026: Unexpected message type for attachment response - return nil
            DLog("Error: Unexpected message type \(userMessage.content) for attachment response")
            return nil
        }
    }
}
    ...
    private func define<T: Codable>(in conversation: inout AIConversation, content: RemoteCommand.Content, prototype: T) {
        ...
        conversation.define(
            ...
            arguments: argsType) { [weak self] llmMessage, command, completion in
                // BUG-1701: Handle optional from withValue to prevent crashes on type mismatch
                guard let contentValue = content.withValue(command) else {
                    DLog("Error: Failed to cast command value to expected type for \(content)")
                    try? completion(.failure(RemoteCommandTypeMismatchError()))
                    return
                }
