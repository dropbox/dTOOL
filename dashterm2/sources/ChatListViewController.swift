...
class ChatListViewController: NSViewController {
    ...
    private let newChatButton: NSButton = {
        // BUG-1759: Use guard let with fallback instead of force unwrap
        let image = NSImage(systemSymbolName: SFSymbol.plus.rawValue, accessibilityDescription: nil) ?? NSImage()
        let button = NSButton(image: image, target: nil, action: nil)
        // macOS 26+ glass bezel style requires SDK 26
    ...
    override func loadView() {
        ...
        // BUG-1975: Store observer token to remove in deinit
    ...
    // BUG-3238: Ensure reloadData always runs on main thread to prevent UI crashes
    func reloadData() {
    ...
    func select(chatID: String) {
        ...
        // BUG-f611: Handle optional return from chatAt
...
class ChatTableRowView: NSTableRowView {
    ...
    // BUG-9474: Observe window key state changes to update emphasis appearance
...
extension ChatListViewController: NSTableViewDelegate {
    ...
    private func view(forRow row: Int) -> NSView {
        ...
        // BUG-1809/BUG-f533: Use guard let and return empty view instead of crashing
        guard let cell else {
            DLog("ChatListViewController: Cell creation failed for row \(row)")
            // Return an empty view as fallback instead of crashing
            return NSView()
        }
