...
@interface iTermGlobalSearchWindowController () <iTermFocusReportingSearchFieldDelegate, NSOutlineViewDelegate,
                                                 NSOutlineViewDataSource, NSWindowDelegate>
@end
...
- (instancetype)init {
    ...
    if (self) {
        _state = [[NSMutableDictionary alloc] initWithCapacity:8];
    }
    return self;
}
...
- (void)windowDidLoad {
    ...
    // Make never-before-made-visible webviews load their URLs or interaction state so they will be searchable. This is
    // a race because they won't be searchable until the document starts and they won't be usefully searchable until
    // "enough" of the document loads (which could take infinite time, oh well)
    [[[iTermController sharedInstance] terminals]
        enumerateObjectsUsingBlock:^(PseudoTerminal *term, NSUInteger idx, BOOL *stop) {
...
- (void)addResults:(NSArray<id<iTermGlobalSearchResultProtocol>> *)results
    ...
    if (!_results) {
        _results = [[NSMutableDictionary alloc] initWithCapacity:16];
    }
    NSMutableArray<id<iTermGlobalSearchResultProtocol>> *sessionResults = _results[session.guid];
    [_outlineView beginUpdates];
    BOOL isNew = (sessionResults == nil);
    if (!sessionResults) {
        sessionResults = [[NSMutableArray alloc] initWithCapacity:results.count];
    ...
    if (isNew) {
        ...
        if (i != NSNotFound) {
            [_outlineView insertItemsAtIndexes:[NSIndexSet indexSetWithIndex:i] inParent:nil withAnimation:NO];
            expand = YES;
        }
    }
    [_outlineView insertItemsAtIndexes:indexes inParent:session.guid withAnimation:NO];
    if (expand) {
...
- (void)revealSelection {
    ...
    [result revealWithState:_state
                 completion:^(NSRect hull) {
...
- (void)movePanelAwayFromRect:(NSRect)hull {
    ...
    NSRect candidates[] = {
        NSMakeRect(NSMinX(hull) - NSWidth(windowRect), // left
                   NSMinY(windowRect), NSWidth(windowRect), NSHeight(windowRect)),
        NSMakeRect(NSMaxX(hull), // right
                   NSMinY(windowRect), NSWidth(windowRect), NSHeight(windowRect)),
        NSMakeRect(NSMinX(windowRect), // down
                   NSMaxY(hull), NSWidth(windowRect), NSHeight(windowRect)),
        NSMakeRect(NSMinX(windowRect), // up
                   NSMinY(hull) - NSHeight(windowRect), NSWidth(windowRect), NSHeight(windowRect)),
    };
...
- (IBAction)search:(id)sender {
    ...
    __weak __typeof(self) weakSelf = self;
    _engine = [[iTermGlobalSearchEngine alloc]
        initWithQuery:_searchField.stringValue
             sessions:self.sessions
                 mode:self.mode
              handler:^(PTYSession *session, NSArray<id<iTermGlobalSearchResultProtocol>> *results, double progress) {
...
- (NSInteger)focusReportingSearchFieldNumberOfResults:(iTermFocusReportingSearchField *)sender {
    ...
    [_results enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
                                                  NSMutableArray<id<iTermGlobalSearchResultProtocol>> *_Nonnull obj,
                                                  BOOL *_Nonnull stop) {
        count += obj.count;
    }];
    return count;
...
- (nullable NSView *)outlineView:(NSOutlineView *)outlineView
    ...
    if (guid) {
        ...
        view.textField.stringValue =
            [[[[iTermController sharedInstance] sessionWithGUID:guid] name] removingHTMLFromTabTitleIfNeeded]
                ?: @"Session";
        return view;
    }
...
- (NSDictionary<NSString *, NSMutableArray<id<iTermGlobalSearchResultProtocol>> *> *)nonEmptyResults {
    return
        [_results filteredWithBlock:^BOOL(NSString *key, NSMutableArray<id<iTermGlobalSearchResultProtocol>> *value) {
...
- (id)outlineView:(NSOutlineView *)outlineView child:(NSInteger)index ofItem:(nullable id)item {
    ...
    if ([item isKindOfClass:[NSString class]]) {
        ...
        // BUG-410: Return nil instead of crashing for invalid results state
        if (!results || index < 0 || (NSUInteger)index >= results.count) {
            return nil;
        }
        return results[index];
    }
    // BUG-410: Return nil for unexpected item type instead of crashing
    return nil;
}
