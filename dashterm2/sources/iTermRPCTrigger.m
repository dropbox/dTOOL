...
- (NSArray<NSString *> *)allPaths {
    return @[
        iTermRPCTriggerPathCapturedStrings, iTermRPCTriggerPathCapturedRanges, iTermRPCTriggerPathInput,
...
- (BOOL)performActionWithCapturedStrings:(NSArray<NSString *> *)stringArray
    ...
    NSMutableArray<NSArray<NSNumber *> *> *captureRangeArray = [NSMutableArray arrayWithCapacity:stringArray.count];
    for (NSInteger i = 0; i < stringArray.count; i++) {
        [captureRangeArray addObject:@[ @(capturedRanges[i].location), @(capturedRanges[i].length) ]];
    }

    NSDictionary *temporaryVariables = @{
        iTermRPCTriggerPathCapturedStrings : stringArray,
...
- (id<NSTextFieldDelegate>)newParameterDelegateWithPassthrough:(id<NSTextFieldDelegate>)passthrough {
    return [[iTermFunctionCallTextFieldDelegate alloc]
