...
int iTermFileDescriptorServerSocketBindListen(const char *path) {
    ...
    if (pathLength >= sizeof(local.sun_path)) {
        FDLog(LOG_NOTICE,
              "Socket path is too long (%zu bytes, max %zu)",
              pathLength,
    ...
    socklen_t len = SizeTToSockLenT(pathLength) + SizeTToSockLenT(sizeof(local.sun_family)) + 1;
    if (bind(socketFd, (struct sockaddr *)&local, len) == -1) {
        FDLog(LOG_NOTICE, "bind() failed: %s", strerror(errno));
        umask(oldMask);
        close(socketFd);  // BUG-1014: Close socket on bind failure
        return -1;
    }
    ...
    if (listen(socketFd, kMaxConnections) == -1) {
        ...
        umask(oldMask);
        close(socketFd);  // BUG-1014: Close socket on listen failure
        return -1;
    }
    umask(oldMask);
    return socketFd;
