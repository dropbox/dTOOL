...
- (NSString *)description {
    ...
    indent += 2;
    NSString *result =
        [NSString stringWithFormat:@"<%@: %p key=%@ data=%@\n%@left=%p\n%@right=%p>", self.class, self, self.key,
                                   self.data, [self spaces:indent], self.left, [self spaces:indent], self.right];
    indent -= 2;
    return result;
}
...
- (void)printWithIndent:(int)indent {
    ...
    if (right)
        [right printWithIndent:(indent + 1)];
    ...
    if (left)
        [left printWithIndent:(indent + 1)];
}

- (NSString *)stringWithIndent:(int)indent dataFormatter:(NSString * (^NS_NOESCAPE)(NSString *, id data))dataFormatter {
    ...
    [parts addObject:[NSString stringWithFormat:@"%@%@- (%i)\n%@", pre, key, level,
                                                dataFormatter([pre stringByAppendingString:@" |-"], data)]];
    [pre release];
