...
- (id)init {
    ...
    @throw [NSException exceptionWithName:@"MethodNotAllowedException"
                                   reason:@"Initialize an AATree with the 'initWithKeyComparator:' method."
                                 userInfo:nil];
}
...
- (NSString *)dumpInterior:(AATreeNode *)node verbose:(BOOL)verbose {
    ...
    return
        [NSString stringWithFormat:@"obj=%@ node=%p value=%p [%@-%@] max=%@", [classes componentsJoinedByString:@","],
                                   node, node.data, @(location), @(imax), @(value.maxLimitAtSubtree)];
}

- (void)removeObjectForKey:(id)aKey {

    [self __lockForWriting];
    changedNodes = [[NSMutableSet alloc] initWithCapacity:8]; // Changed tree nodes during removal
#if DEBUG
    NSString *before = [self dump];

    @synchronized([AATree class]) {
        _debugJournal = [[NSMutableArray alloc] initWithCapacity:32]; // Debug journal entries
    }
#endif
...
- (void)setObject:(id)anObject forKey:(id)aKey {
    ...
    ITAssertWithMessage([aKey conformsToProtocol:@protocol(NSCopying)],
                        @"The supplied key does not conform to the NSCopying protocol.");

    [self __lockForWriting];
    changedNodes = [[NSMutableSet alloc] initWithCapacity:8]; // Changed tree nodes during insertion
...
- (NSArray *)pathFromNode:(AATreeNode *)node {
    NSMutableArray *parents = [[NSMutableArray alloc] initWithCapacity:16];
    ...
    if (aRoot) {
        ...
        if (compareResult == NSOrderedSame) {
            ...
            if (aRoot.left && aRoot.right) {
                ...
                // BUG-f1387: Replace assert with safe guard - count underflow should not crash
                if (count > 0) {
                    count--;
                } else {
                    ELog(@"AATree: Attempted to decrement count when already 0, tree state corrupted");
                }
            }
...
- (AATreeNode *)__nodeClosestToKey:(id)aKey atRoot:(AATreeNode *)aRoot {
    ...
    if (aRoot) {
        ...
        // Otherwise, travel left or right until a leaf node is surpassed.
            ...
            result = [self __nodeClosestToKey:aKey atRoot:aRoot.left];
        else
            result = [self __nodeClosestToKey:aKey atRoot:aRoot.right];

        // If no result has been found lower in the tree, test whether this node
        // is the closest.
        if (!result && compareResult == NSOrderedDescending)
            result = aRoot;
    }
