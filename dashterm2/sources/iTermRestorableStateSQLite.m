...
- (NSKeyedUnarchiver * _Nullable)unarchiver {
    // Use secure coding for defense-in-depth (only primitive types are archived)
    ...
    NSKeyedUnarchiver *unarchiver = [[NSKeyedUnarchiver alloc] initForReadingFromData:archiver.encodedData
                                                                                error:&error];
    if (error || !unarchiver) {
        DLog(@"Failed to create unarchiver: %@", error);
        return nil;
    }
...
- (void)initializeDb {
    ...
    sqliteDb.timeoutHandler = ^BOOL {
        ...
        // Run alert on the main queue without synchronously blocking it.
...
- (void)restoreWindowWithRecord:(id<iTermRestorableStateRecord>)record
    ...
    // BUG-f1173: Replace assert() with guard - negative index should call completion with nil, not crash
    if (i < 0) {
        DLog(@"WARNING: Invalid index %ld, cannot restore window", (long)i);
        completion(nil, nil);
        return;
    }
    ...
    // BUG-f1174: Replace assert() with guard - empty identifier should call completion with nil, not crash
    if (identifier.length == 0) {
        DLog(@"WARNING: Empty window identifier, cannot restore window");
        completion(nil, nil);
        return;
    }
...
- (BOOL)saveSynchronously:(BOOL)sync withCompletion:(void (^)(void))completion {
    ...
    // BUG-f1175: Replace assert() with guard - off-main-thread call should be dispatched to main, not crash
    if (![NSThread isMainThread]) {
        DLog(@"WARNING: saveSynchronously called off main thread, dispatching to main");
        __block BOOL result = NO;
        if (sync) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                result = [self saveSynchronously:sync withCompletion:completion];
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self saveSynchronously:sync withCompletion:completion];
            });
        }
        return result;
    }
