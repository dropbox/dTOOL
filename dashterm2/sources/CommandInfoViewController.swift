...
class CommandInfoViewController: NSViewController {
    ...
    required init?(coder: NSCoder) {
        // BUG-f821: Return nil instead of crashing for unused coder initializer
        DLog("CommandInfoViewController init(coder:) is not supported")
        return nil
    }
    ...
    deinit {
        ...
        timer?.invalidate()
        // BUG-1938: Remove output progress observer to prevent leaks and dangling references
    ...
    private func timerDidFire() {
        ...
        if let runtimeConstraint {
            ...
            guard let runningTimeView = runningTime else {
                return
            }
            let constraint = NSLayoutConstraint(item: runningTimeView,
                                                   attribute: .width,
            ...
            runningTimeView.addConstraint(constraint)
        }
    }
    ...
    // BUG-975: Use localized date/time formatting instead of hardcoded English format
    ...
    private var largestWidth: CGFloat {
        ...
        // BUG-1695: Use nil-coalescing instead of force unwrap for max()
        return xs.max() ?? 0
    }
    ...
    private func showCopiedToast(_ view: NSView) {
        guard let superview = copiedView.superview else { return }
        var frame = copiedView.frame
        let viewFrame = superview.convert(view.bounds, from: view)
