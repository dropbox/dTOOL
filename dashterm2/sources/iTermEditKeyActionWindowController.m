...
//
// BUG-285: Key action help text strings updated to DashTerm2
// BUG-451: Modifier remapping help updated to DashTerm2
    ...
    if (self) {
        ...
        switch (tag) {
            ...
            case KEY_ACTION_DO_NOT_REMAP_MODIFIERS:
                ...
                _parameterValue = @"";
                _helpString = @"This action lets you exempt a keystroke from modifier remapping. For example, if you "
                              @"remap ⌘ to ⌥ but you want ⌘-Tab to work as though ⌘ were unmapped just for that "
                              @"keystroke, you would use this action and set the keyboard shortcut to ⌘-Tab";
                _parameterValue = @"";
                break;
            ...
            case KEY_ACTION_REMAP_LOCALLY:
                ...
                _parameterValue = @"";
                _helpString =
                    @"This action applies modifier remapping but prevents other programs from seeing the keystroke. "
                    @"For example, if you've swapped ⌘ and ^ and want physical ^-tab to switch tabs in DashTerm2 "
                    @"instead of triggering the app switcher: bind ^-tab to this action. The system won't see the "
                    @"remapped ⌘-tab (so no app switcher), but DashTerm2 receives it and can switch tabs.";
                _parameterValue = @"";
                break;

            case KEY_ACTION_BYPASS:
                _helpString =
                    @"Prevents the keystroke from being sent to the terminal while allowing macOS to handle it "
                    @"normally. For example, if F1 triggers a  macOS Shortcut, binding F1 to Bypass Terminal stops it "
                    @"from sending a control sequence to the terminal but still lets the system shortcut work.";
                break;

            case KEY_ACTION_IGNORE:
                _helpString = @"Prevents the keystroke from having any effect within DashTerm2. Modifier remapping "
                              @"remains unaffected.";
                break;
            ...
            case KEY_ACTION_COPY_MODE:
                ...
                _helpString = @"Enter copy mode commands to move cursor, toggle selection, and so on. This key binding "
                              @"enters Copy Mode and then acts as though you had pressed the keys listed here. [See a "
                              @"list of all the commands](https://dashterm.com/documentation-copymode.html). Use vim "
                              @"syntax for control, option, and function keys (e.g., `<C-x>` or `<Up>`.";
                break;
            ...
            case KEY_ACTION_INVOKE_SCRIPT_FUNCTION:
                ...
                if (functionCallDelegate) {
                    ...
                    _parameterDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
                        initWithPathSource:[iTermVariableHistory pathSourceForContext:context]
                               passthrough:nil
                             functionsOnly:YES];
                }
                _applyHidden = NO;
            ...
            case KEY_ACTION_COPY_INTERPOLATED_STRING:
                ...
                // BUG-1: Documentation URL updated to dashterm.com
                _helpString =
                    @"You can use this to copy information about the current session to the clipboard. [Learn more "
                    @"about interpolated strings](https://dashterm.com/documentation-scripting-fundamentals.html)";
                break;
...
@interface iTermEditKeyActionWindowController () <
    iTermKeyActionSequenceTableViewControllerDelegate, iTermPasteSpecialViewControllerDelegate,
    iTermSearchableComboViewDelegate, iTermShortcutInputViewDelegate, NSTextFieldDelegate>
...
- (NSArray<iTermSearchableComboViewGroup *> *)groupsForPrimary:(BOOL)primary {
    NSArray<iTermSearchableComboViewGroup *> *groups = @[ [[iTermSearchableComboViewGroup alloc]
        initWithLabel:@"General"
                items:[@[
                    primary ? [[iTermSearchableComboViewItem alloc] initWithLabel:@"Ignore" tag:KEY_ACTION_IGNORE]
                            : [NSNull null],
                    primary ? [[iTermSearchableComboViewItem alloc] initWithLabel:@"Bypass Terminal"
                                                                              tag:KEY_ACTION_BYPASS]
                            : [NSNull null],
                    [[iTermSearchableComboViewItem alloc] initWithLabel:@"Select Menu Item..."
                                                                    tag:KEY_ACTION_SELECT_MENU_ITEM],
                ] arrayByRemovingNulls]] ];
    if (self.mode == iTermEditKeyActionWindowControllerModeKeyboardShortcut) {
        groups = [groups
            arrayByAddingObjectsFromArray:[@[
                primary ? [[iTermSearchableComboViewGroup alloc]
                              initWithLabel:@"Modifier Remapping"
                                      items:@[
                                          [[iTermSearchableComboViewItem alloc]
                                              initWithLabel:@"Do Not Remap Modifiers"
                                                        tag:KEY_ACTION_DO_NOT_REMAP_MODIFIERS],
                                          [[iTermSearchableComboViewItem alloc]
                                              initWithLabel:@"Remap Modifiers in DashTerm2 Only"
                                                        tag:KEY_ACTION_REMAP_LOCALLY],
                                      ]]
                        : [NSNull null],
                [[iTermSearchableComboViewGroup alloc]
                    initWithLabel:@"Cycle"
                            items:@[
                                [[iTermSearchableComboViewItem alloc] initWithLabel:@"Cycle Tabs Forward"
                                                                                tag:KEY_ACTION_NEXT_MRU_TAB],
                                [[iTermSearchableComboViewItem alloc] initWithLabel:@"Cycle Tabs Backward"
                                                                                tag:KEY_ACTION_PREVIOUS_MRU_TAB],
                            ]],
            ] arrayByRemovingNulls]];
    }
    ...
    groups = [groups
        arrayByAddingObjectsFromArray:[@[
            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Miscellaneous"
                        items:[@[
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Run Coprocess"
                                                                                  tag:KEY_ACTION_RUN_COPROCESS],
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Start Instant Replay"
                                                                                  tag:KEY_ACTION_IR_BACKWARD],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Undo" tag:KEY_ACTION_UNDO],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send tmux Command"
                                                                            tag:KEY_ACTION_SEND_TMUX_COMMAND],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Alert on Next Mark"
                                                                            tag:KEY_ACTION_ALERT_ON_NEXT_MARK],
                        ] arrayByRemovingNulls]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"New Tab or Window"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"New Window with Profile"
                                                                            tag:KEY_ACTION_NEW_WINDOW_WITH_PROFILE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"New Tab with Profile"
                                                                            tag:KEY_ACTION_NEW_TAB_WITH_PROFILE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Duplicate Tab"
                                                                            tag:KEY_ACTION_DUPLICATE_TAB],
                        ]],
            ...
            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Split"
                        items:@[
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Split Horizontally with Profile"
                                          tag:KEY_ACTION_SPLIT_HORIZONTALLY_WITH_PROFILE],
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Split Vertically with Profile"
                                          tag:KEY_ACTION_SPLIT_VERTICALLY_WITH_PROFILE],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Profile"
                        items:[@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Change Profile"
                                                                            tag:KEY_ACTION_SET_PROFILE],
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Load Color Preset"
                                                                                  tag:KEY_ACTION_LOAD_COLOR_PRESET],
                        ] arrayByRemovingNulls]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Navigate Tabs"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Next Tab"
                                                                            tag:KEY_ACTION_NEXT_SESSION],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Previous Tab"
                                                                            tag:KEY_ACTION_PREVIOUS_SESSION],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Reorder Tabs"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Move Tab Left"
                                                                            tag:KEY_ACTION_MOVE_TAB_LEFT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Move Tab Right"
                                                                            tag:KEY_ACTION_MOVE_TAB_RIGHT],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Navigate Windows"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Next Window"
                                                                            tag:KEY_ACTION_NEXT_WINDOW],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Previous Window"
                                                                            tag:KEY_ACTION_PREVIOUS_WINDOW],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Navigate Panes"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Next Pane" tag:KEY_ACTION_NEXT_PANE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Previous Pane"
                                                                            tag:KEY_ACTION_PREVIOUS_PANE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Select Split Pane Above"
                                                                            tag:KEY_ACTION_SELECT_PANE_ABOVE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Select Split Pane Below"
                                                                            tag:KEY_ACTION_SELECT_PANE_BELOW],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Select Split Pane On Left"
                                                                            tag:KEY_ACTION_SELECT_PANE_LEFT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Select Split Pane On Right"
                                                                            tag:KEY_ACTION_SELECT_PANE_RIGHT],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Resize Pane"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Decrease Height"
                                                                            tag:KEY_ACTION_DECREASE_HEIGHT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Increase Height"
                                                                            tag:KEY_ACTION_INCREASE_HEIGHT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Decrease Width"
                                                                            tag:KEY_ACTION_DECREASE_WIDTH],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Increase Width"
                                                                            tag:KEY_ACTION_INCREASE_WIDTH],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Scroll"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll to End"
                                                                            tag:KEY_ACTION_SCROLL_END],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll to Top"
                                                                            tag:KEY_ACTION_SCROLL_HOME],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll One Line Down"
                                                                            tag:KEY_ACTION_SCROLL_LINE_DOWN],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll One Line Up"
                                                                            tag:KEY_ACTION_SCROLL_LINE_UP],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll One Page Down"
                                                                            tag:KEY_ACTION_SCROLL_PAGE_DOWN],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Scroll One Page Up"
                                                                            tag:KEY_ACTION_SCROLL_PAGE_UP],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Split Panes"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Split Pane Above"
                                                                            tag:KEY_ACTION_SWAP_PANE_ABOVE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Split Pane Below"
                                                                            tag:KEY_ACTION_SWAP_PANE_BELOW],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Split Pane on Left"
                                                                            tag:KEY_ACTION_SWAP_PANE_LEFT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Split Pane on Right"
                                                                            tag:KEY_ACTION_SWAP_PANE_RIGHT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Next Pane"
                                                                            tag:KEY_ACTION_SWAP_WITH_NEXT_PANE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Swap With Previous Pane"
                                                                            tag:KEY_ACTION_SWAP_WITH_PREVIOUS_PANE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Move Session to Split Pane"
                                                                            tag:KEY_ACTION_MOVE_TO_SPLIT_PANE],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Send Keystrokes"
                        items:[@[
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send ^H Backspace"
                                                                                  tag:KEY_ACTION_SEND_C_H_BACKSPACE],
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send ^? Backspace"
                                                                                  tag:KEY_ACTION_SEND_C_QM_BACKSPACE],
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send Escape Sequence"
                                                                                  tag:KEY_ACTION_ESCAPE_SEQUENCE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send Hex Code"
                                                                            tag:KEY_ACTION_HEX_CODE],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send Text" tag:KEY_ACTION_TEXT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send Text with “vim” Special Chars"
                                                                            tag:KEY_ACTION_VIM_TEXT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Send Snippet"
                                                                            tag:KEY_ACTION_SEND_SNIPPET],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Compose…" tag:KEY_ACTION_COMPOSE],
                        ] arrayByRemovingNulls]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Search"
                        items:[@[
                            hideTerminalOnlyItems
                                ? [NSNull null]
                                : [[iTermSearchableComboViewItem alloc] initWithLabel:@"Find Regular Expression…"
                                                                                  tag:KEY_ACTION_FIND_REGEX],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Find Again Down"
                                                                            tag:KEY_FIND_AGAIN_DOWN],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Find Again Up" tag:KEY_FIND_AGAIN_UP],
                        ] arrayByRemovingNulls]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Pasteboard"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Paste…" tag:KEY_ACTION_PASTE_SPECIAL],
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Paste from Selection…"
                                          tag:KEY_ACTION_PASTE_SPECIAL_FROM_SELECTION],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Copy or Send ^C"
                                                                            tag:KEY_ACTION_COPY_OR_SEND],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Paste or Send ^V"
                                                                            tag:KEY_ACTION_PASTE_OR_SEND],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Copy Interpolated String"
                                                                            tag:KEY_ACTION_COPY_INTERPOLATED_STRING],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Copy Mode Commands"
                                                                            tag:KEY_ACTION_COPY_MODE],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Toggles"
                        items:[@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Toggle Fullscreen"
                                                                            tag:KEY_ACTION_TOGGLE_FULLSCREEN],
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Toggle Pin Hotkey Window"
                                          tag:KEY_ACTION_TOGGLE_HOTKEY_WINDOW_PINNING],
                            hideTerminalOnlyItems ? [NSNull null]
                                                  : [[iTermSearchableComboViewItem alloc]
                                                        initWithLabel:@"Toggle Mouse Reporting"
                                                                  tag:KEY_ACTION_TOGGLE_MOUSE_REPORTING],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Toggle Setting"
                                                                            tag:KEY_ACTION_TOGGLE_SETTING],
                        ] arrayByRemovingNulls]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Selection"
                        items:@[
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Move Start of Selection Back"
                                          tag:KEY_ACTION_MOVE_START_OF_SELECTION_LEFT],
                            [[iTermSearchableComboViewItem alloc]
                                initWithLabel:@"Move Start of Selection Forward"
                                          tag:KEY_ACTION_MOVE_START_OF_SELECTION_RIGHT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Move End of Selection Back"
                                                                            tag:KEY_ACTION_MOVE_END_OF_SELECTION_LEFT],
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Move End of Selection Forward"
                                                                            tag:KEY_ACTION_MOVE_END_OF_SELECTION_RIGHT],
                        ]],

            [[iTermSearchableComboViewGroup alloc]
                initWithLabel:@"Scripting"
                        items:@[
                            [[iTermSearchableComboViewItem alloc] initWithLabel:@"Invoke Script Function…"
                                                                            tag:KEY_ACTION_INVOKE_SCRIPT_FUNCTION],
                        ]],

            primary ? [[iTermSearchableComboViewGroup alloc]
                          initWithLabel:@"Composition"
                                  items:@[
                                      [[iTermSearchableComboViewItem alloc] initWithLabel:@"Sequence…"
                                                                                      tag:KEY_ACTION_SEQUENCE],
                                  ]]
                    : [NSNull null],
        ] arrayByRemovingNulls]];
    return groups;
...
- (void)windowDidLoad {
    ...
    [self loadParameter:self.parameterValue action:self.action applyMode:self.applyMode secondary:NO];
}
...
- (void)loadParameter:(NSString *)parameterValue
    ...
    if (!_profilePopup.isHidden) {
        [_profilePopup populateWithProfilesSelectingGuid:parameterValue ?: @"" profileTypes:_profileType];
    }
    ...
    if (self.action == KEY_ACTION_PASTE_SPECIAL || self.action == KEY_ACTION_PASTE_SPECIAL_FROM_SELECTION) {
        [_pasteSpecialViewController loadSettingsFromString:parameterValue ?: @""];
    } else {
        // Set a few defaults; otherwise everything is reasonable.
        _pasteSpecialViewController.numberOfSpacesPerTab =
            [iTermPreferences intForKey:kPreferenceKeyPasteSpecialSpacesPerTab];
...
- (void)setMode:(iTermEditKeyActionWindowControllerMode)mode {
    // BUG-13014: Replace assert(NO) with ELog - setMode should be overridden in subclasses
    // This base implementation should not be called directly
    ELog(@"setMode: called on base class iTermEditKeyActionWindowController - should be overridden in subclass");
}
...
- (void)updateViewsAnimated:(BOOL)animated secondary:(BOOL)secondary {
    switch (self.mode) {
        case iTermEditKeyActionWindowControllerModeUnbound:
            ...
            if (self.titleIsInterpolated) {
                if (!_labelDelegate) {
                    _labelDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
                        initWithPathSource:[iTermVariableHistory
                                               pathSourceForContext:iTermVariablesSuggestionContextSession]
                               passthrough:self
                             functionsOnly:NO];
                }
    ...
    iTermEditKeyActionWindowConfiguration *config =
        [[iTermEditKeyActionWindowConfiguration alloc] initWithTag:tag
    ...
    if (!config.profilePopupHidden) {
        [_profilePopup populateWithProfilesSelectingGuid:config.parameterValue ?: @"" profileTypes:_profileType];
    }
...
- (BOOL)anyAccessoryVisible {
    return (!_sequenceContainer.isHidden || [self anyNonSequenceAccessoryVisible]);
}

- (BOOL)anyNonSequenceAccessoryVisible {
    return (!_parameter.isHidden || !_profilePopup.isHidden || !_menuToSelectPopup.isHidden ||
            !_settingToTogglePopup.isHidden || !_colorPresetsPopup.isHidden || !_snippetsPopup.isHidden ||
            !_pasteSpecialViewContainer.isHidden || !_parameterLabel.isHidden || !_selectionMovementUnit.isHidden);
}
...
- (NSSize)desiredSize {
    return NSMakeSize(self.desiredWidthExcludingMargins + sideMarginWidth, self.desiredHeight);
}
...
- (CGFloat)desiredWidthExcludingMargins {
    ...
    if (!_pasteSpecialViewContainer.hidden) {
        return MAX(normalWidthExcludingMargins, _pasteSpecialViewController.view.frame.size.width);
    }
    ...
    switch (action) {
        case KEY_ACTION_SELECT_MENU_ITEM:
            if (_menuToSelectPopup.selectedIdentifier.length) {
                return [NSString stringWithFormat:@"%@\n%@", _menuToSelectPopup.selectedTitle,
                                                  _menuToSelectPopup.selectedIdentifier ?: @""];
            } else {
...
- (void)searchableComboView:(iTermSearchableComboView *)view
              didSelectItem:(iTermSearchableComboViewItem *)didSelectItem {
    if (view == _comboView) {
        _parameterValue = @"";
    }
    if (view == _comboView || view == _secondaryComboView) {
        NSString *guid = [[_profilePopup selectedItem] representedObject];
        [_profilePopup populateWithProfilesSelectingGuid:guid profileTypes:_profileType];
...
- (void)keyActionSequenceTableViewController:(iTermKeyActionSequenceTableViewController *)sender
    ...
    [self loadParameter:action.parameter action:action.keyAction applyMode:action.applyMode secondary:YES];
}
...
- (void)keyActionSequenceTableViewControllerDidChange:(iTermKeyActionSequenceTableViewController *)sender
    ...
    if (sender.hasSelection) {
        ...
        [self loadParameter:action.parameter action:action.keyAction applyMode:_applyMode secondary:YES];
    } else {
        [self updateViewsAnimated:NO secondary:YES];
    }
}
