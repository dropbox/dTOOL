      ...
      return YES;
        ...
        if ([[[iTermController sharedInstance] currentTerminal] inInstantReplay]) {
            ...
            return NO;
               ...
               [menuItem action] == @selector(editCurrentSession:)) {
        // BUG-11099: Edit Current Session should be disabled when there are no sessions
    ...
    if (filename) {
        ...
        if (isDir) {
            ...
            if (filename) {
                ...
                NSString *initialText = [iTermProfilePreferences stringForKey:KEY_INITIAL_TEXT inProfile:bookmark];
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSNotification *)theNotification {
    ...
    if (reason.hasReason) {
        ...
        if ([[iTermController sharedInstance] shouldLeaveSessionsRunningOnQuit]) {
            message = @"Sessions will be restored automatically when DashTerm2 is relaunched.";
        } else {
            message = @"All sessions will be closed.";
        }
        [NSApp activateIgnoringOtherApps:YES];
        NSAlert *alert = [[[NSAlert alloc] init] autorelease];
        alert.messageText = @"Quit DashTerm2?";
...
// Track when willFinishLaunching starts for startup timing
static CFAbsoluteTime sWillFinishLaunchingStartTime = 0;

- (void)applicationWillFinishLaunching:(NSNotification *)aNotification {
    // Show placeholder window immediately for instant visual feedback.
    // Must be shown here (not in main()) to ensure iTermApplication is properly initialized.
    // Moving this to main.m would cause [NSApplication sharedApplication] to create a vanilla
    // NSApplication before NSApplicationMain reads the principal class from Info.plist,
    // leading to crashes (BUG-f899) and missing iTermApplication features.
    BOOL isUnitTesting = (NSClassFromString(@"XCTest") != nil) ||
    ...
    sWillFinishLaunchingStartTime = CFAbsoluteTimeGetCurrent();
    DLog(@"Begin");
    // BUG-f899: Guard updateAppearance call - only iTermApplication has this method
    iTermApplication *app = [iTermApplication sharedApplication];
    if ([app isKindOfClass:[iTermApplication class]]) {
        [app updateAppearance];
    }
    if ([iTermUserDefaults importPath]) {
        [DashTerm2ImportExport finishImporting];
        assert(NO);
    }
    ...
    // Defer script menu building to not block startup.
    dispatch_async(dispatch_get_main_queue(), ^{
        DLog(@"buildScriptMenu (deferred)");
    ...
    // Fast startup: Use minimal profile init (no disk I/O)
    // Full profile loading happens async in applicationDidFinishLaunching
    DLog(@"Make ITAddressBookMgr (minimal)");
    ...
    // Defer software update to after window visible for faster startup
    DLog(@"Defer refreshSoftwareUpdateUserDefaults");
    dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // Defer secret server to after window visible for faster startup
    DLog(@"Defer iTermSecretServer");
    dispatch_async(dispatch_get_main_queue(), ^{
        NSError *error = nil;
    ...
    if (_restorableStateController) {
        ...
        // No window restoration - dismiss placeholder after a brief delay to allow
        // the new untitled window to appear
        dispatch_async(dispatch_get_main_queue(), ^{
            [iTermStartupPlaceholderWindow dismissPlaceholder];
            // Ensure the app and its key window are in front
            [NSApp activateIgnoringOtherApps:YES];
            [[NSApp keyWindow] makeKeyAndOrderFront:nil];
            [ITAddressBookMgr loadRemainingProfilesAsync];
        });
    }
    ...
    // Note: applicationDidFinishLaunching is not called in this codebase due to
    // restorable state handling, so we log here at end of willFinishLaunching.
    CFAbsoluteTime startupDuration = CFAbsoluteTimeGetCurrent() - sWillFinishLaunchingStartTime;
    NSLog(@"[STARTUP] Initialization complete: %.3fs", startupDuration);
}

- (void)restoreWindows {
    DLog(@"restoreWindows");
    CFAbsoluteTime restoreWindowsStart = CFAbsoluteTimeGetCurrent();
    NSLog(@"[STARTUP] restoreWindows starting");
    [_restorableStateController restoreWindowsWithCompletion:^{
        NSLog(@"[STARTUP] restoreWindowsWithCompletion finished in %.3fs", CFAbsoluteTimeGetCurrent() - restoreWindowsStart);
        DLog(@"Window restoration is totally complete");
        [_untitledWindowStateMachine didFinishRestoringWindows];
        ScreenCharGarbageCollectImages();

        // Dismiss startup placeholder now that real window is visible
        [iTermStartupPlaceholderWindow dismissPlaceholder];

        // Ensure the app and its key window are in front
        [NSApp activateIgnoringOtherApps:YES];
        [[NSApp keyWindow] makeKeyAndOrderFront:nil];
        NSLog(@"[STARTUP] Placeholder dismissed, window should be visible now");

        // Load remaining profiles async (window is visible, user can start typing)
        [ITAddressBookMgr loadRemainingProfilesAsync];
    }];
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    // Note: This method is currently not called due to restorable state handling.
    // Startup timing is logged at the end of applicationWillFinishLaunching instead.

    // Dismiss the startup placeholder window now that initialization is complete.
    [iTermStartupPlaceholderWindow dismissPlaceholder];

    // Load remaining profiles async (window is visible, user can type)
    [ITAddressBookMgr loadRemainingProfilesAsync];
    ...
    [[iTermLaunchServices sharedInstance] registerForDashTerm2Scheme];
    ...
    if ([iTermAdvancedSettingsModel runJobsInServers] &&
        ...
        [PseudoTerminalRestorer setPostRestorationCompletionBlock:^{
            ...
            if ([[iTermController sharedInstance] numberOfDecodesPending] == 0) {
                _orphansAdopted = YES;
                [[iTermOrphanServerAdopter sharedInstance] openWindowWithOrphansWithCompletion:^{
                    // Ensure app comes to front after orphan adoption creates windows
                    [NSApp activateIgnoringOtherApps:YES];
                    [[NSApp keyWindow] makeKeyAndOrderFront:nil];
                }];
            } else {
    ...
    // Add "Review Crash Report..." menu item (always available, auto-enables when crash detected)
    {
        NSMenu *appMenu = [[[[NSApp mainMenu] itemArray] firstObject] submenu];
        [appMenu addItem:[NSMenuItem separatorItem]];

        NSMenuItem *crashItem = [[[NSMenuItem alloc] initWithTitle:@"Review Crash Report…"
                                                            action:@selector(showCrashReporter)
                                                     keyEquivalent:@""] autorelease];
        crashItem.target = [iTermCrashReportIconController shared];
        [appMenu addItem:crashItem];

        // Add "Build Info" menu item for development
        NSMenuItem *buildInfoItem = [[[NSMenuItem alloc] initWithTitle:@"Build Info…"
                                                                action:@selector(showBuildInfo:)
                                                         keyEquivalent:@"b"] autorelease];
        buildInfoItem.keyEquivalentModifierMask = NSEventModifierFlagCommand | NSEventModifierFlagOption;
        [appMenu addItem:buildInfoItem];
    }
...
- (NSMenu *)statusBarMenu {
    ...
    item = [[[NSMenuItem alloc] initWithTitle:@"Quit DashTerm2"
...
- (IBAction)showBuildInfo:(id)sender {
    [BuildInfoWindowController showBuildInfo];
}
...
// BUG-11099: Disable "Edit Current Session..." when there are no current sessions.
// Validation handled in -validateMenuItem:
...
- (IBAction)sendFeedback:(id)sender {
    [[DTermFeedbackReporter shared] showFeedbackWindowWith:DTermFeedbackTypeFeedback];
}

- (IBAction)reportCrash:(id)sender {
    [[DTermFeedbackReporter shared] showFeedbackWindowWith:DTermFeedbackTypeCrash];
}
...
- (void)updateProcessType {
    // BUG-f899: Guard setIsUIElement: call - only iTermApplication has this method
    iTermApplication *app = [iTermApplication sharedApplication];
    if (![app isKindOfClass:[iTermApplication class]]) {
        return;
    }
...
- (void)orphanServerAdopterOpenSessionForConnection:(iTermGeneralServerConnection)generalConnection
    ...
    // BUG-f900: Replace assert with guard - return early if runJobsInServers is disabled
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
        ELog(@"BUG-f900: orphanServerAdopterOpenSessionForConnection called but runJobsInServers is disabled");
        if (completion) {
            completion(nil);
        }
        return;
...
- (void)orphanServerAdopterOpenSessionForPartialAttachment:(id<iTermPartialAttachment>)partialAttachment
    ...
    // BUG-f901: Replace assert with guard - return early if runJobsInServers is disabled
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
        ELog(@"BUG-f901: orphanServerAdopterOpenSessionForPartialAttachment called but runJobsInServers is disabled");
        if (completion) {
            completion(nil);
        }
        return;
...
- (void)restorableStateRestoreWithRecord:(nonnull iTermEncoderGraphRecord *)record
    ...
    [PseudoTerminalRestorer restoreWindowWithIdentifier:identifier
        ...
        CFAbsoluteTime asyncRestoreStart = CFAbsoluteTimeGetCurrent();
        NSLog(@"[STARTUP] Starting asyncRestoreState for %@", identifier);
        [term asyncRestoreState:state
                     ...
                     completion: ^{
            NSLog(@"[STARTUP] asyncRestoreState for %@ completed in %.3fs", identifier, CFAbsoluteTimeGetCurrent() - asyncRestoreStart);
...
- (BOOL)encodeGraphWithEncoder:(iTermGraphEncoder *)encoder {
    ...
    // BUG-f899: Guard it_restorableStateInvalid access - only iTermApplication has this property
    iTermApplication *app = [iTermApplication sharedApplication];
    BOOL appInvalidState = NO;
    if ([app isKindOfClass:[iTermApplication class]]) {
        appInvalidState = app.it_restorableStateInvalid;
        app.it_restorableStateInvalid = NO;
    }
    if (appInvalidState ||
...
- (void)untitledWindowStateMachineCreateNewWindow:(iTermUntitledWindowStateMachine *)sender {
    ...
    [self newWindow:nil];
    // Note: Window activation is handled by iTermSessionLauncher's async completion
    // to ensure proper window ordering after the window is created.
}
