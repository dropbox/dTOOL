...
+ (void)initialize {
    gRegisteredTools = [[NSMutableDictionary alloc] initWithCapacity:16];  // Registered toolbelt tools
    ...
    NSDictionary<NSString *, NSDictionary *> *dynamicTools =
        [[NSUserDefaults standardUserDefaults] objectForKey:kDynamicToolsKey];
    [dynamicTools enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull identifier, NSDictionary *_Nonnull dict,
                                                      BOOL *_Nonnull stop) {
...
+ (void)registerDynamicToolWithIdentifier:(NSString *)identifier
    ...
    if ([registry[identifier][kDynamicToolURL] isEqualToString:url] &&
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:iTermToolbeltDidRegisterDynamicToolNotification
                                                            object:identifier];
        return;
    }
    NSMutableDictionary *mutableRegistry =
        [registry mutableCopy] ?: [[NSMutableDictionary alloc] initWithCapacity:4]; // Dynamic tools registry
    mutableRegistry[identifier] = @{kDynamicToolName : name, kDynamicToolURL : url};
...
+ (NSArray *)configuredTools {
    ...
    NSMutableArray *vettedTools = [[NSMutableArray alloc] initWithCapacity:tools.count]; // All valid tools
...
+ (void)addToolsToMenu:(NSMenu *)menu {
    ...
    for (NSString *theName in names) {
        ...
        [i setState:[[iTermToolbeltView configuredTools] containsObject:theName] ? NSControlStateValueOn
...
+ (void)toggleShouldShowTool:(NSString *)theName {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"iTermToolToggled" object:theName userInfo:nil];
}
...
+ (NSDictionary *)toolsDictionary {
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithCapacity:gRegisteredTools.count];
    for (NSString *toolName in gRegisteredTools) {
        [dict setObject:[[[gRegisteredTools objectForKey:toolName] alloc] init] forKey:toolName];
    }
    return dict;
}
...
- (instancetype)initWithFrame:(NSRect)frame delegate:(id<iTermToolbeltViewDelegate>)delegate {
    ...
    if (self) {
        ...
        _splitter =
            [[iTermToolbeltSplitView alloc] initWithFrame:NSMakeRect(0, 0, frame.size.width, frame.size.height)];
        ...
        _tools = [[NSMutableDictionary alloc] initWithCapacity:8];  // Active toolbelt tools
...
- (void)dealloc {
    // BUG-3202: Clear layer delegate to prevent use-after-free if layer outlives view
...
- (NSRect)menuButtonFrame {
    ...
    return NSMakeRect(self.bounds.size.width - hamburger.size.width - 4, 4.5, hamburger.size.width,
...
- (void)addToolWithName:(NSString *)toolName index:(NSInteger)index {
    ...
    iTermToolWrapper *wrapper = [[iTermToolWrapper alloc]
        initWithFrame:NSMakeRect(
                          0, 0, self.frame.size.width,
                          self.frame.size.height /
                              MAX(1, [iTermToolbeltView numberOfVisibleToolsForProfileType:self.profileType] - 1))];
    wrapper.name = toolName;
    wrapper.delegate = self;
    Class c = [gRegisteredTools objectForKey:toolName];
    if (c) {
        NSView<ToolbeltTool> *theTool = nil;
        NSRect frame = NSMakeRect(0, 0, wrapper.container.frame.size.width, wrapper.container.frame.size.height);

        if ([self classIsDynamic:c]) {
            NSDictionary *registry = [[NSUserDefaults standardUserDefaults] objectForKey:kDynamicToolsKey];
            NSString *identifier =
                [registry.allKeys objectPassingTest:^BOOL(NSString *key, NSUInteger index, BOOL *stop) {
...
- (void)updateForProfileType {
    ...
    [[iTermToolbeltView availableConfiguredToolsForProfileType:profileType]
        enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx, BOOL *_Nonnull stop) {
            if (_tools[key] == nil) {
...
- (NSDictionary *)restorableState {
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:_splitter.subviews.count];
    for (iTermToolWrapper *wrapper in [_splitter subviews]) {
...
- (void)hideToolbelt {
    // BUG-1042: Include delegate in notification so only the correct window responds
    [[NSNotificationCenter defaultCenter] postNotificationName:kToolbeltShouldHide object:_delegate userInfo:nil];
}
...
// The minimum coordinate gives the smallest allowed distance between the top of the splitview and the `dividerIndex`th
// divider.
- (CGFloat)splitView:(NSSplitView *)splitView
    ...
    DLog(@"Constrain divider %@ (between %@ and %@) currently at %@ to minY >= %@. proposed=%@", @(dividerIndex),
         wrappers[dividerIndex].name, wrappers[dividerIndex + 1].name, @(wrappers[dividerIndex + 1].frame.origin.y),
         @(y), @(proposedMinimumPosition));
    return y;
}

// The maximum coordinate gives the largest allowed distance between the top of the splitview and the `dividerIndex`th
// divider.
- (CGFloat)splitView:(NSSplitView *)splitView
    ...
    DLog(@"Constrain divider %@ (between %@ and %@) currently at %@ to maxY <= %@. proposed=%@", @(dividerIndex),
         wrappers[dividerIndex].name, wrappers[dividerIndex + 1].name, @(wrappers[dividerIndex + 1].frame.origin.y),
         @(y), @(proposedMaximumPosition));
    return y;
}
...
- (void)splitView:(NSSplitView *)splitView resizeSubviewsWithOldSize:(NSSize)oldSize {
    ...
    if (!collapsedAny && splitView.frame.size.height > oldSize.height) {
        for (iTermToolWrapper *wrapper in _splitter.subviews) {
            if (wrapper.collapsed) {
                if ([self minimumHeightForSplitterSubviews] + wrapper.minimumHeightWhenExpanded <=
                    _splitter.frame.size.height) {
...
- (NSDictionary *)proportions {
    ...
    NSArray<NSDictionary<NSString *, id> *> *proportions = [[names zip:fractions] mapWithBlock:^id(iTermTuple *tuple) {
        return @{@"name" : tuple.firstObject, @"heightAsFraction" : tuple.secondObject};
    }];
...
- (void)setProportions:(NSDictionary *)dict {
    ...
    const BOOL consistent =
        [[tuples zip:names] allWithBlock:^BOOL(iTermTuple<iTermTuple<NSString *, NSNumber *> *, NSString *> *tuple) {
            return [tuple.firstObject.firstObject isEqualToString:tuple.secondObject] &&
                   tuple.firstObject.secondObject.doubleValue > 0;
        }];
    if (!consistent) {
        return;
    }
    ...
    if (addedHeight > 0) {
        NSArray<NSNumber *> *wiggleRoom = [[tuples zip:desiredHeights]
            mapWithBlock:^id(iTermTuple<iTermTuple<NSString *, NSNumber *> *, NSNumber *> *tuple) {
...
- (void)splitView:(PTYSplitView *)splitView draggingDidEndOfSplit:(int)clickedOnSplitterIndex pixels:(NSSize)changePx {
    _proportions = [self proportions];
    [[NSUserDefaults standardUserDefaults] setObject:_proportions forKey:iTermToolbeltProportionsUserDefaultsKey];
}
