...
- (NSString *)debugInfoForDirectory:(NSString *)path {
    ...
    return [NSString stringWithFormat:@"Cache status: %@\nPending calls: %@\n",
                                      existing
                                          ? [NSString stringWithFormat:@"Have cached value of age %@", @(existing.age)]
                                          : @"No cached value",
                                      @(pending.count)];
}
...
- (void)didFetchState:(iTermGitState *)state path:(NSString *)path {
    ...
    if (cached != nil && !isnan(cached.creationTime) && // just paranoia to avoid unbounded recursion
    ...
    [blocks enumerateObjectsUsingBlock:^(iTermGitPollWorkerCompletionBlock _Nonnull block, NSUInteger idx,
                                         BOOL *_Nonnull stop) {
