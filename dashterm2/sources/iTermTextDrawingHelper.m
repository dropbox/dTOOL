...
// Optimization: Cache NSNumber objects for iTermMarkIndicatorType values (0-5).
// The enum has 6 values (Success, Error, Other, FoldedSuccess, FoldedError, FoldedOther).
// Caching avoids NSNumber boxing when looking up/storing cached mark images.
static const int kCachedMarkIndicatorTypeCount = 6;
static NSNumber *sMarkIndicatorTypeCache[kCachedMarkIndicatorTypeCount];

NS_INLINE NSNumber *iTermMarkIndicatorTypeToNumber(iTermMarkIndicatorType type) {
    if (type < kCachedMarkIndicatorTypeCount) {
        return sMarkIndicatorTypeCache[type];
    }
    return @(type);
}

__attribute__((constructor)) static void iTermTextDrawingHelperInitializeMarkIndicatorTypeCache(void) {
    for (int i = 0; i < kCachedMarkIndicatorTypeCount; i++) {
        sMarkIndicatorTypeCache[i] = @(i);
    }
}
...
static CGFloat iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(BOOL hasBackgroundImage, BOOL enableBlending,
                                                                         BOOL reverseVideo, CGFloat transparencyAlpha,
                                                                         CGFloat blend);
@implementation iTermTextDrawingHelper {
    ...
    NSDictionary<NSNumber *, NSArray<NSValue *> *> *_noteRangesByLine;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _missingImages = [[NSMutableSet alloc] initWithCapacity:4];
        _lineRefCache = [[NSMutableDictionary alloc] initWithCapacity:64];
        _replacementLineRefCache = [[NSMutableDictionary alloc] initWithCapacity:64];
        _cachedMarks = [[NSMutableDictionary alloc] initWithCapacity:16];
...
- (void)drawTextViewContentInRect:(NSRect)rect
    ...
    for (int i = 0; i < rectCount; i++) {
        ...
        for (int j = 0; j < numRowsInRect; j++) {
            ...
            //            NSLog(@"Set range on line %d to %@", j + boundingCoordRange.start.y,
    ...
    iTermPreciseTimerPeriodicLog(@"drawRect", _stats, sizeof(_stats) / sizeof(*_stats), 5,
                                 [iTermAdvancedSettingsModel logDrawingPerformance], nil, nil);
    ...
    _replacementLineRefCache = [[NSMutableDictionary alloc] initWithCapacity:_lineRefCache.count ?: 64];

    DLog(@"end drawRect:%@ in view %@", [NSValue valueWithRect:rect], _delegate);
}

- (void)cacheNoteRangesForOrigin:(VT100GridCoord)origin count:(NSInteger)numRanges visibleLines:(NSRange)visibleLines {
    _noteRangesByLine = nil;
    if (![_delegate respondsToSelector:@selector(drawingHelperAnnotationsInRange:)] ||
        ![_delegate respondsToSelector:@selector(drawingHelperCoordRangeForAnnotation:)]) {
        return;
    }

    const NSInteger firstLine = MAX((NSInteger)origin.y, (NSInteger)visibleLines.location);
    const NSInteger pastLastLine = MIN((NSInteger)origin.y + numRanges, (NSInteger)NSMaxRange(visibleLines));
    if (pastLastLine <= firstLine) {
        return;
    }

    const VT100GridCoordRange queryRange =
        VT100GridCoordRangeMake(0, (int)firstLine, _gridSize.width, (int)pastLastLine);
    NSArray<id<PTYAnnotationReading>> *annotations = [_delegate drawingHelperAnnotationsInRange:queryRange];
    if (annotations.count == 0) {
        return;
    }

    NSMutableIndexSet *rowsToQuery = [NSMutableIndexSet indexSet];
    for (id<PTYAnnotationReading> annotation in annotations) {
        const VT100GridCoordRange coordRange = [_delegate drawingHelperCoordRangeForAnnotation:annotation];
        const NSInteger annotationStart = MAX((NSInteger)coordRange.start.y, firstLine);
        const NSInteger annotationEnd = MIN((NSInteger)coordRange.end.y, pastLastLine - 1);
        if (annotationEnd < annotationStart) {
            continue;
        }
        const NSUInteger length = (NSUInteger)(annotationEnd - annotationStart + 1);
        [rowsToQuery addIndexesInRange:NSMakeRange((NSUInteger)annotationStart, length)];
    }

    if (rowsToQuery.count == 0) {
        return;
    }

    NSMutableDictionary<NSNumber *, NSArray<NSValue *> *> *noteRanges =
        [[NSMutableDictionary alloc] initWithCapacity:rowsToQuery.count];
    [rowsToQuery enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
        NSArray<NSValue *> *ranges = [self.delegate drawingHelperCharactersWithNotesOnLine:(int)idx];
        if (ranges.count > 0) {
            noteRanges[@(idx)] = ranges;
        }
    }];

    if (noteRanges.count > 0) {
        _noteRangesByLine = noteRanges;
    }
}

- (NSInteger)numberOfEquivalentBackgroundColorLinesInRunArrays:
                 (NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
...
- (void)drawRanges:(NSRange *)ranges
    ...
    [self cacheNoteRangesForOrigin:origin count:numRanges visibleLines:visibleLines];

    // Pre-allocate for number of lines being drawn to avoid array growth during rendering
    NSMutableArray<iTermBackgroundColorRunsInLine *> *backgroundRunArrays =
        [NSMutableArray arrayWithCapacity:numRanges];
    ...
    for (NSInteger i = 0; i < numRanges; i++) {
        ...
        if ([self canDrawLine:line]) {
            ...
            iTermBackgroundColorRunsInLine *runsInLine = [iTermBackgroundColorRunsInLine
                backgroundRunsInLine:theLine
    ...
    if ([self haveAnyImagesUnderText]) {
        ...
        [self drawKittyImagesInRange:iTermSignedRangeWithBounds(NSIntegerMin, -1073741824) virtualOffset:virtualOffset];
    ...
    [self drawKittyImagesInRange:iTermSignedRangeWithBounds(-1073741824, 1) virtualOffset:virtualOffset];
    ...
    [self drawAccessoriesInRect:boundingRect virtualOffset:virtualOffset];
    const BOOL shouldDrawCursorGuideBelowText =
        !self.useNativePowerlineGlyphs || _cursorGuideColor.alphaComponent > iTermCursorGuideAlphaThreshold;
    if (shouldDrawCursorGuideBelowText) {
        [self drawCursrGuideIfNeededInRect:boundingRect virtualOffset:virtualOffset];
    }
    ...
    if (drawCursorBeforeText) {
        cursor = [self drawCursor:NO cursorBackgroundColor:cursorBackgroundColor virtualOffset:virtualOffset];
    }
    ...
    [self drawKittyImagesInRange:iTermSignedRangeWithBounds(1, NSIntegerMax) virtualOffset:virtualOffset];
    [self drawMarksWithBackgroundRunArrays:backgroundRunArrays virtualOffset:virtualOffset];
    ...
    if (drawCursorBeforeText) {
        if ([iTermAdvancedSettingsModel drawOutlineAroundCursor]) {
            [self drawCursor:YES cursorBackgroundColor:cursorBackgroundColor virtualOffset:virtualOffset];
        }
    } else {
        cursor = [self drawCursor:NO cursorBackgroundColor:cursorBackgroundColor virtualOffset:virtualOffset];
    }
    ...
    if (self.copyMode) {
        [self drawCopyModeCursorWithBackgroundColor:cursorBackgroundColor virtualOffset:virtualOffset];
    }
...
- (void)clipOutSuppressedBottomIfNeeded:(CGFloat)virtualOffset {
    if (_pointsOnBottomToSuppressDrawing > 0) {
        NSRect clipRect = NSMakeRect(
            0, _visibleRectIncludingTopMargin.origin.y, _visibleRectIncludingTopMargin.size.width,
...
- (NSColor *)drawBackgroundRunArrays:(NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
    ...
    for (NSInteger i = 0; i < backgroundRunArrays.count;) {
        ...
        if (cursorY >= runArray.line && cursorY < runArray.line + runArray.numberOfEquivalentRows) {
            NSColor *color = [self
                unprocessedColorForBackgroundRun:[runArray runAtVisualIndex:self.cursorCoord.x] ?: runArray.lastRun
                                  enableBlending:NO];
        ...
        for (NSInteger j = i; j < MIN(i + rows, backgroundRunArrays.count); j++) {
            NSColor *leftColor =
                (!extend || runArray.array[0].isDefault) ? defaultColor : runArray.array[0].backgroundColor;
            NSColor *rightColor = (!extend || runArray.array.lastObject.isDefault)
                                      ? defaultColor
                                      : runArray.array.lastObject.backgroundColor;
...
- (void)drawBackgroundForLine:(int)line
    ...
    for (iTermBoxedBackgroundColorRun *box in runs) {
        ...
        NSRect rect = NSMakeRect(
            floor([iTermPreferences intForKey:kPreferenceKeySideMargins] + run->visualRange.location * _cellSize.width),
            yOrigin, ceil(run->visualRange.length * _cellSize.width), _cellSize.height * rows);
        ...
        NSColor *color = [self unprocessedColorForBackgroundRun:run enableBlending:enableBlending];
        ...
        if (draw) {
            [self drawBackgroundColor:color inRect:rect enableBlending:enableBlending virtualOffset:virtualOffset];
        }
        if (_debug) {
...
- (void)drawBackgroundColor:(NSColor *)color
    ...
    [color set];
    iTermRectFillUsingOperation(rect, enableBlending ? NSCompositingOperationSourceOver : NSCompositingOperationCopy,
                                virtualOffset);
    if (_debug) {
...
- (NSColor *)unprocessedColorForBackgroundRun:(const iTermBackgroundColorRun *)run enableBlending:(BOOL)enableBlending {
    NSColor *color;
    ...
    if (run->selected) {
        ...
        const BOOL defaultBackground = (run->bgColor == ALTSEM_DEFAULT && run->bgColorMode == ColorModeAlternate);
        ...
        if (defaultBackground) {
            alpha = iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(_hasBackgroundImage, enableBlending,
                                                                              _reverseVideo, alpha, _blend);
        }
    }
...
- (void)drawMarkForLine:(int)line y:(CGFloat)y virtualOffset:(CGFloat)virtualOffset {
    NSRect leftMargin = [self leftMarginRectAt:y];
    ...
    if (eaIndex.attributes[@0].blockIDList) {
        if ([self canDrawLine:line]) {
            ...
            const NSRect rect = NSMakeRect(1, y, MAX(1, leftMargin.size.width - 2), _cellSize.height);
        ...
        [self drawMarkIfNeededOnLine:line leftMarginRect:leftMargin virtualOffset:virtualOffset];
    }
}
...
- (void)drawShadeOverNonSelectedCommands:(CGFloat)virtualOffset {
    ...
    const CGFloat topHeight = visibleSelectedRange.location * _cellSize.height - commandRegionOutlineThickness -
                              _scrollViewDocumentVisibleRect.origin.y;
    CGFloat y = _scrollViewDocumentVisibleRect.origin.y;

    NSRect rect = NSMakeRect(0, y, _scrollViewDocumentVisibleRect.size.width, MAX(0, topHeight));
    y += topHeight;
    iTermRectFillUsingOperation(rect, NSCompositingOperationSourceOver, virtualOffset);

    const CGFloat selectedHeight =
        MAX(0, visibleSelectedRange.length * _cellSize.height + commandRegionOutlineThickness * 2);
    y += selectedHeight;
    ...
    rect = NSMakeRect(0, y, _scrollViewDocumentVisibleRect.size.width,
    ...
    DLog(@"rect.origin.y=%@ because document origin=%@ + topHeight=%@ + selectedHeight=%@. Cell height is %@",
         @(rect.origin.y - virtualOffset), @(_scrollViewDocumentVisibleRect.origin.y - virtualOffset), @(topHeight),
         @(selectedHeight), @(self.cellSize.height));
}
...
- (void)drawOutlineAroundSelectedCommand:(CGFloat)virtualOffset {
    ...
    NSRect rect = NSMakeRect(0, y - commandRegionOutlineThickness, _scrollViewDocumentVisibleRect.size.width,
...
- (void)drawStripesInRect:(NSRect)rect virtualOffset:(CGFloat)virtualOffset {
    ...
    [[NSGraphicsContext currentContext]
        setPatternPhase:NSMakePoint([iTermPreferences intForKey:kPreferenceKeySideMargins], 0)];
    iTermRectFillUsingOperation(rect, NSCompositingOperationSourceOver, virtualOffset);
    [NSGraphicsContext restoreGraphicsState];
}
...
- (VT100GridCoordRange)safeCoordRange:(VT100GridCoordRange)range {
    ...
    return VT100GridCoordRangeMake(MEDIAN(0, range.start.x, width), MEDIAN(0, range.start.y, height),
                                   MEDIAN(0, range.end.x, width), MEDIAN(0, range.end.y, height));
}

- (void)drawAccessoriesInRect:(NSRect)bgRect virtualOffset:(CGFloat)virtualOffset {
    [self drawBadgeInRect:bgRect margins:self.badgeMargins virtualOffset:virtualOffset];
...
- (void)drawCursrGuideIfNeededInRect:(NSRect)bgRect virtualOffset:(CGFloat)virtualOffset {
    ...
    const BOOL drawCursorGuide =
        (self.highlightCursorLine && cursorLine >= coordRange.start.y && cursorLine <= coordRange.end.y);
    if (drawCursorGuide) {
        CGFloat y = cursorLine * _cellSize.height;
        [self drawCursorGuideForColumns:NSMakeRange(coordRange.start.x, coordRange.end.x - coordRange.start.x)
                                      y:y
                          virtualOffset:virtualOffset];
    }
}

- (void)drawCursorGuideForColumns:(NSRange)range y:(CGFloat)yOrigin virtualOffset:(CGFloat)virtualOffset {
    if (!_isCursorVisible) {
        return;
    }
    [_cursorGuideColor set];
    NSPoint textOrigin =
        NSMakePoint([iTermPreferences intForKey:kPreferenceKeySideMargins] + range.location * _cellSize.width, yOrigin);
    NSRect rect = NSMakeRect(0, textOrigin.y, _scrollViewDocumentVisibleRect.size.width, _cellSize.height);
    iTermRectFillUsingOperation(rect, NSCompositingOperationSourceOver, virtualOffset);
...
+ (NSRect)frameForMarkContainedInRect:(NSRect)container
               ...
               cellSizeWithoutSpacing:(CGSize)cellSizeWithoutSpacing
                                scale:(CGFloat)scale {
    const CGFloat verticalSpacing =
        MAX(0, scale * round((cellSize.height / scale - cellSizeWithoutSpacing.height / scale) / 2.0));
    DLog(@"verticalSpacing=%@", @(verticalSpacing));
    CGRect rect = NSMakeRect(container.origin.x, container.origin.y + verticalSpacing, container.size.width,
...
+ (NSColor *)successMarkColor {
    return [[NSColor colorWithSRGBRed:0.53846 green:0.757301 blue:1
                                alpha:1] colorUsingColorSpace:[NSColorSpace it_defaultColorSpace]];
}

+ (NSColor *)errorMarkColor {
    return [[NSColor colorWithSRGBRed:0.987265 green:0.447845 blue:0.426244
                                alpha:1] colorUsingColorSpace:[NSColorSpace it_defaultColorSpace]];
}

+ (NSColor *)otherMarkColor {
    return [[NSColor colorWithSRGBRed:0.856645 green:0.847289 blue:0.425771
                                alpha:1] colorUsingColorSpace:[NSColorSpace it_defaultColorSpace]];
}

+ (NSImage *)newImageWithMarkOfColor:(NSColor *)color pixelSize:(CGSize)pixelSize folded:(BOOL)folded {
...
+ (NSImage *)newImageWithMarkOfColor:(NSColor *)color size:(CGSize)size folded:(BOOL)folded {
    if (size.width < 1 || size.height < 1) {
        return [self newImageWithMarkOfColor:color
                                        size:CGSizeMake(MAX(1, size.width), MAX(1, size.height))
                                      folded:folded];
    }
    NSImage *img = [NSImage imageOfSize:size
                              drawBlock:^{
...
+ (iTermMarkIndicatorType)markIndicatorTypeForMark:(id<iTermMark>)genericMark folded:(BOOL)folded {
    ...
    if ([iTermAdvancedSettingsModel showYellowMarkForJobStoppedBySignal] && mark.code >= 128 && mark.code <= 128 + 32) {
...
- (BOOL)canDrawLine:(int)line {
    return (line < _linesToSuppress.location || line >= _linesToSuppress.location + _linesToSuppress.length);
}
...
- (void)drawMarksWithBackgroundRunArrays:(NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
    ...
    for (NSInteger i = 0; i < backgroundRunArrays.count; i += 1) {
        [self drawMarkForLine:backgroundRunArrays[i].line y:backgroundRunArrays[i].y virtualOffset:virtualOffset];
    }
}

- (void)drawMarkIfNeededOnLine:(int)line leftMarginRect:(NSRect)leftMargin virtualOffset:(CGFloat)virtualOffset {
    if (![self canDrawLine:line]) {
        return;
    }
    ...
    if (mark != nil && (self.drawMarkIndicators || mark.name.length > 0)) {
        if (mark.lineStyle) {
            ...
            NSColor *merged = [iTermTextDrawingHelper
                colorForLineStyleMark:[iTermTextDrawingHelper markIndicatorTypeForMark:mark folded:folded]
    ...
    if (shouldDrawRegularMark) {
        ...
        NSRect rect = [iTermTextDrawingHelper frameForMarkContainedInRect:insetLeftMargin
                                                   ...
                                                   cellSizeWithoutSpacing:_cellSizeWithoutSpacing
                                                                    scale:1];
        const iTermMarkIndicatorType type = [iTermTextDrawingHelper markIndicatorTypeForMark:mark folded:folded];
        NSNumber *typeKey = iTermMarkIndicatorTypeToNumber(type);
        NSImage *image = _cachedMarks[typeKey];
        if (!image || !NSEqualSizes(image.size, rect.size)) {
            NSColor *markColor = [iTermTextDrawingHelper colorForMark:mark];
            image = [iTermTextDrawingHelper newImageWithMarkOfColor:markColor size:rect.size folded:folded];
            _cachedMarks[typeKey] = image;
        }
...
- (void)drawNoteRangesOnLine:(int)line virtualOffset:(CGFloat)virtualOffset {
    if (![self canDrawLine:line]) {
        return;
    }
    NSArray<NSValue *> *noteRanges = _noteRangesByLine[@(line)];
    if (noteRanges.count == 0) {
        return;
    }
    for (NSValue *value in noteRanges) {
        ...
        CGFloat maxX = MIN(_frame.size.width - [iTermPreferences intForKey:kPreferenceKeySideMargins],
                           range.length * _cellSize.width + x);
        CGFloat w = maxX - x;
...
- (void)createTimestampDrawingHelperWithFontInfo:(PTYFontInfo *)fontInfo {
    ...
    for (int y = _scrollViewDocumentVisibleRect.origin.y / _cellSize.height;
         y < NSMaxY(_scrollViewDocumentVisibleRect) / _cellSize.height && y < _numberOfLines; y++) {
...
- (void)drawTimestampsWithVirtualOffset:(CGFloat)virtualOffset {
    ...
    [_timestampDrawHelper drawInContext:[NSGraphicsContext currentContext] frame:_frame virtualOffset:virtualOffset];
    if (!self.isRetina) {
...
+ (NSRect)rectForBadgeImageOfSize:(NSSize)imageSize
    ...
    NSRect destination =
        NSMakeRect(textViewSize.width - imageSize.width - margins.right,
                   kiTermIndicatorStandardHeight + margins.top + verticalOffset, imageSize.width, imageSize.height);
...
- (NSSize)drawBadgeInRect:(NSRect)rect margins:(NSEdgeInsets)margins virtualOffset:(CGFloat)virtualOffset {
    NSRect source = NSZeroRect;
    const NSRect intersection = [iTermTextDrawingHelper
        rectForBadgeImageOfSize:_badgeImage.size
...
- (void)drawButtons:(CGFloat)virtualOffset {
    NSColor *background = [[self.delegate drawingHelperColorForCode:ALTSEM_DEFAULT
                                                              green:0
                                                               blue:0
                                                          colorMode:ColorModeAlternate
                                                               bold:NO
                                                              faint:NO
                                                       isBackground:YES] colorWithAlphaComponent:_transparencyAlpha];
    NSColor *foreground = [self.delegate drawingHelperColorForCode:ALTSEM_DEFAULT
                                                             green:0
                                                              blue:0
                                                         colorMode:ColorModeAlternate
                                                              bold:NO
...
- (iTermRectArray *)buttonsBackgroundRects NS_AVAILABLE_MAC(11) {
    ...
    // Pre-allocate based on button count - typically one entry per button's Y coordinate
    NSMutableDictionary<NSNumber *, NSValue *> *dict = [NSMutableDictionary dictionaryWithCapacity:buttons.count];
...
- (NSArray<iTermTerminalButton *> *)updateButtonFrames NS_AVAILABLE_MAC(11) {
    ...
    int floatingCount = 0;
    NSArray<iTermTerminalButton *> *allButtons = [self.delegate drawingHelperTerminalButtons];
    // Pre-allocate based on total button count - typically few buttons per line
    NSMutableDictionary<NSNumber *, NSMutableIndexSet *> *usedDict =
        [NSMutableDictionary dictionaryWithCapacity:allButtons.count];
    NSMutableArray<iTermTerminalButton *> *buttons = [NSMutableArray arrayWithCapacity:allButtons.count];
    for (iTermTerminalButton *button in allButtons) {
        CGFloat x;
        ...
        if (_selectedCommandRegion.length > 0 &&
            absCoord.y - self.totalScrollbackOverflow == NSMaxRange(_selectedCommandRegion)) {
        ...
        DLog(@"Set desired frame of %@ to %@ from minAbsLine:%@ = (%@ + %@) visibleRect:%@ cumulativeOffset:%@ "
             @"cellSize.height:%@",
             button, NSStringFromRect(button.desiredFrame), @(drawableCoordRange.start.y + _totalScrollbackOverflow),
             @(drawableCoordRange.start.y), @(_totalScrollbackOverflow),
             NSStringFromRect(_visibleRectExcludingTopMargin), @(_totalScrollbackOverflow), @(_cellSize.height));
    }
    return buttons;
}
...
- (void)drawDropTargetsWithVirtualOffset:(CGFloat)virtualOffset {
    ...
    [self enumerateDropTargets:^(NSString *label, NSRange range) {
        NSRect rect = NSMakeRect(0, range.location * _cellSize.height, _scrollViewDocumentVisibleRect.size.width,
...
- (void)enumerateDropTargets:(void (^NS_NOESCAPE)(NSString *, NSRange))block {
    ...
    // Pre-allocate based on visible lines - typically 1-2 drop targets
    const NSInteger visibleLines = coordRange.end.y - coordRange.start.y + 1;
    NSMutableArray *labels = [NSMutableArray arrayWithCapacity:MIN(visibleLines, 8)];
    NSMutableArray *lineRanges = [NSMutableArray arrayWithCapacity:MIN(visibleLines, 8)];
...
// Draw assuming no foreground color processing. Keeps glyphs together in a single background color run across different
// background colors.
- (void)drawUnprocessedForegroundForBackgroundRunArrays:(NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
    ...
    // color separately.
    NSArray<iTermBackgroundColorRunsInLine *> *fakeRunArrays =
        [backgroundRunArrays mapWithBlock:^id(iTermBackgroundColorRunsInLine *runs) {
            // Pre-allocate with input count as upper bound - combined runs <= input runs
            NSMutableArray<iTermBoxedBackgroundColorRun *> *combinedRuns =
                [NSMutableArray arrayWithCapacity:runs.array.count];
            ...
            for (iTermBoxedBackgroundColorRun *run in runs.array) {
                if (!havePreviousRun) {
                    ...
                    previousRun = *run.valuePointer;
                           ...
                           run.valuePointer->isMatch == previousRun.isMatch && !run.valuePointer->beneathFaintText &&
                    ...
                    [combinedRuns
                        addObject:[iTermBoxedBackgroundColorRun boxedBackgroundColorRunWithValue:previousRun]];
                    previousRun = *run.valuePointer;
                }
            }
            if (havePreviousRun) {
...
- (void)drawForegroundForSourceLineNumber:(int)sourceLineNumber
    ...
    if (sourceLineNumber == displayLineNumber) {
        [self drawNoteRangesOnLine:sourceLineNumber virtualOffset:virtualOffset];
    }
...
- (void)drawCharactersForDisplayLine:(int)displayLine
    ...
    for (iTermBoxedBackgroundColorRun *box in backgroundRuns) {
        iTermBackgroundColorRun *run = box.valuePointer;
        NSPoint textOrigin = NSMakePoint([iTermPreferences intForKey:kPreferenceKeySideMargins], y);
...
- (void)constructAndDrawRunsForLine:(const screen_char_t *)theLine
    ...
    DLog(@"row %f: %@", (initialPoint.y - virtualOffset) / self.cellSize.height,
         ScreenCharArrayToStringDebug(theLine, _gridSize.width));

    iTermPreciseTimerStatsStartTimer(&_stats[TIMER_STAT_CONSTRUCTION]);
    NSArray<id<iTermAttributedString>> *attributedStrings = [_attributedStringBuilder
        attributedStringsForLine:theLine
                        bidiInfo:bidiInfo
              externalAttributes:eaIndex
                           range:indexRange
...
- (void)drawMultipartAttributedString:(NSArray<id<iTermAttributedString>> *)attributedStrings
    ...
    for (id<iTermAttributedString> singlePartAttributedString in attributedStrings) {
        ...
        //                         alpha:1] set];
        //        iTermFrameRect(NSMakeRect(point.x + subpositions[0], point.y, numCellsDrawn * _cellSize.width,
        //        _cellSize.height), virtualOffset);
...
- (int)drawFastPathString:(iTermCheapAttributedString *)cheapString
    ...
    if ([cheapString.attributes[iTermIsBoxDrawingAttribute] boolValue]) {
        ...
        for (NSUInteger i = 0; i < cheapString.length; i++) {
            ...
            [self drawBoxDrawingCharacter:c withAttributes:cheapString.attributes at:p virtualOffset:virtualOffset];
        }
...
- (int)drawFastPathStringWithoutUnderlineOrStrikethrough:(iTermCheapAttributedString *)cheapString
    ...
    if (attributes[iTermImageCodeAttribute]) {
        ...
        const int displayColumn = [attributes[iTermImageDisplayColumnAttribute] intValue];
        [self
            drawKittyImageInPlaceholderWithCoord:coord
                                          origin:VT100GridCoordMake(displayColumn, origin.y)
                                         atPoint:NSMakePoint(positions[0] + point.x, point.y)
    ...
    BOOL ok = CTFontGetGlyphsForCharacters((CTFontRef)font, cheapString.characters, glyphs, cheapString.length);
    if (!ok) {
        NSString *string = [NSString stringWithCharacters:cheapString.characters length:cheapString.length];
        [self
            drawTextOnlyAttributedStringWithoutUnderlineOrStrikethrough:[NSAttributedString
                                                                            attributedStringWithString:string
                                                                                            attributes:cheapString
                                                                                                           .attributes]
                                                                atPoint:point
                                                              positions:positions
                                                        backgroundColor:backgroundColor
    ...
    CGColorRef const color =
        (__bridge CGColorRef)cheapString.attributes[(__bridge NSString *)kCTForegroundColorAttributeName];
    ...
    const BOOL useThinStrokes = [self useThinStrokesAgainstBackgroundColor:backgroundColor foregroundColor:color];
    ...
    CGContextSetTextMatrix(ctx, CGAffineTransformMake(1.0, 0.0, m21, -1.0, x, y));
    ...
    if (smear) {
        ...
        if (fakeBold) {
            ...
            CGContextSetTextMatrix(
                ctx, CGAffineTransformMake(1.0, 0.0, m21, -1.0, x + (antiAlias ? _antiAliasedShift : 1), y));
...
- (void)drawUnderlineOrStrikethroughForFastPathString:(iTermCheapAttributedString *)cheapString
    ...
    const NSSize size =
        NSMakeSize([attributes[iTermUnderlineLengthAttribute] intValue] * _cellSize.width, _cellSize.height * 2);
    const CGFloat xOrigin = origin.x + stringPositions[range.location];
    const NSRect rect = NSMakeRect(xOrigin, origin.y, size.width, size.height);
    NSColor *underlineColor = [self underlineColorForAttributes:attributes];
    [self
        drawUnderlinedOrStruckthroughTextWithContext:underlineContext
                                      ...
                                      underlineColor:underlineColor
                                       ...
                                       virtualOffset:virtualOffset
                                               block:^(CGContextRef ctx) {
                                                   ...
                                                   iTermCheapAttributedString *blackCopy =
                                                       [cheapString copyWithAttributes:attrs];
                                                   [self
                                                       drawFastPathStringWithoutUnderlineOrStrikethrough:blackCopy
                                                                                                 atPoint:
                                                                                                     NSMakePoint(
                                                                                                         -stringPositions
                                                                                                             [0],
...
- (int)drawSinglePartAttributedString:(NSAttributedString *)attributedString
    ...
    if (attributes[iTermImageCodeAttribute]) {
        ...
        [attributedString.string
            enumerateComposedCharacters:^(NSRange range, unichar simple, NSString *complexString, BOOL *stop) {
                ...
                [self drawBoxDrawingCharacter:simple ?: [complexString firstCharacter]
                               withAttributes:[attributedString attributesAtIndex:range.location effectiveRange:nil]
                                           at:p
                                virtualOffset:virtualOffset];
            }];
        return attributedString.length;
...
- (void)drawTextOnlyAttributedStringWithoutUnderlineOrStrikethrough:(NSAttributedString *)attributedString
    ...
    BOOL useThinStrokes = [self useThinStrokesAgainstBackgroundColor:backgroundColor foregroundColor:cgColor];
    int savedFontSmoothingStyle = 0;
    ...
    CGAffineTransform textMatrix = CGAffineTransformMake(1.0, 0.0, c, -1.0, origin.x, ty);
    ...
    iTermCoreTextLineRenderingHelper *renderingHelper =
        [[iTermCoreTextLineRenderingHelper alloc] initWithLine:lineRef
                                                        string:attributedString.string
                                               drawInCellIndex:drawInCellIndex];
    [renderingHelper
        enumerateGridAlignedRunsWithColumnPositions:xOriginsForCharacters
                                        alignToZero:NO
                                            closure:^(CTRunRef run, CTFontRef runFont, const CGGlyph *buffer,
                                                      const NSPoint *positions,
                                                      const CFIndex *glyphIndexToCharacterIndex, size_t length,
                                                      BOOL *stop) {
                                                if (!smear) {
                                                    CTFontDrawGlyphs(runFont, buffer, (NSPoint *)positions, length,
                                                                     cgContext);

                                                    if (bold && !fakeBold) {
                                                        // If this text is supposed to be bold, but the font is not, use
                                                        // double-struck text. Issue 4956.
                                                    ...
                                                    if (fakeBold && _boldAllowed) {
                                                        CGContextTranslateCTM(cgContext,
                                                                              antiAlias ? _antiAliasedShift : 1, 0);
                                                        CTFontDrawGlyphs(runFont, buffer, (NSPoint *)positions, length,
                                                                         cgContext);
                                                        CGContextTranslateCTM(cgContext,
                                                                              antiAlias ? -_antiAliasedShift : -1, 0);
...
- (void)drawUnderlineAndStrikethroughForAttributedString:(NSAttributedString *)attributedString
    ...
    [attributedString
        enumerateAttribute:wantUnderline ? NSUnderlineStyleAttributeName : NSStrikethroughStyleAttributeName
                        ...
                        drawUnderlinedOrStruckthroughTextWithContext:underlineContext
                                                      ...
                                                      underlineColor:underlineColor
                                                       ...
                                                       virtualOffset:virtualOffset
                                                               block:^(CGContextRef ctx) {
                                                                   if (!wantUnderline) {
                                                                       // This is a shortcut to prefer simplicity over
                                                                       // speed. Underline and strikethrough are very
                                                                       // similar except for masking. For strikethrough,
                                                                       // we draw an empty mask. If this becomes a
                                                                       // performance issue, we can skip drawing the
                                                                       // empty mask.
                                                                       return;
                                                                   }
                                                                   [self
                                                                       drawAttributedStringForMask:attributedString
                                                                                            origin:
                                                                                                NSMakePoint(
                                                                                                    -stringPositions
                                                                                                        [range
...
- (void)drawUnderlinedOrStruckthroughTextWithContext:(iTermUnderlineContext *)underlineContext
    ...
    if (!underlineContext->maskGraphicsContext) {
        // Create a mask image.
        [self initializeUnderlineContext:underlineContext ofSize:rect.size block:block];
    }
...
- (void)drawAttributedStringForMask:(NSAttributedString *)attributedString
    ...
    [self drawTextOnlyAttributedStringWithoutUnderlineOrStrikethrough:modifiedAttributedString
                                                              atPoint:origin
                                                            positions:stringPositions
                                                      backgroundColor:[NSColor it_colorInDefaultColorSpaceWithRed:1
                                                                                                            green:1
                                                                                                             blue:1
                                                                                                            alpha:1]
                                                      graphicsContext:[NSGraphicsContext currentContext]
...
- (void)initializeUnderlineContext:(iTermUnderlineContext *)underlineContext
    ...
    [NSGraphicsContext
        setCurrentContext:[NSGraphicsContext graphicsContextWithCGContext:underlineContext->maskGraphicsContext
                                                                  flipped:NO]];
...
- (void)drawInContext:(CGContextRef)cgContext
    ...
    CGContextClipToMask(cgContext, NSRectSubtractingVirtualOffset(rect, virtualOffset), alphaMask);
...
- (CGContextRef)newGrayscaleContextOfSize:(NSSize)size {
    ...
    CGContextRef maskContext = CGBitmapContextCreate(NULL, size.width * scale, size.height * scale,
                                                     8, // bits per component
                                                     size.width * scale, colorspace,
                                                     0); // bitmap info
...
NSColor *iTermTextDrawingHelperTextColorForMatch(NSColor *bgColor) {
    return bgColor.isDark ? [NSColor colorWithCalibratedRed:1 green:1 blue:1 alpha:1]
                          : [NSColor colorWithCalibratedRed:0 green:0 blue:0 alpha:1];
}

static unsigned int VT100TerminalColorValueToKittyImageNumber(VT100TerminalColorValue colorValue, int msb) {
    ...
    if (colorValue.mode == ColorMode24bit) {
        return (shiftedMSB | (((unsigned int)colorValue.red) << 16) | (((unsigned int)colorValue.green) << 8) |
...
BOOL iTermDecodeKittyUnicodePlaceholder(const screen_char_t *c, iTermExternalAttribute *ea,
    ...
    VT100TerminalColorValue colorValue = {
        .red = c->foregroundColor, .green = c->fgGreen, .blue = c->fgBlue, .mode = c->foregroundColorMode};
    unsigned int imageID = VT100TerminalColorValueToKittyImageNumber(colorValue, imageMSB);
    unsigned int placementID =
        ea.hasUnderlineColor ? VT100TerminalColorValueToKittyImageNumber(ea.underlineColor, 0) : 0;

    *info = (iTermKittyUnicodePlaceholderInfo){.row = coord.y,
                                               .column = coord.x,
                                               .imageID = imageID,
...
- (BOOL)zippy {
    return (!(_asciiLigaturesAvailable && _asciiLigatures) && !(_nonAsciiLigatures) &&
...
- (BOOL)useThinStrokesAgainstBackgroundColor:(NSColor *)backgroundColor foregroundColor:(CGColorRef)foregroundColor {
...
- (void)drawUnderlineOrStrikethroughOfColor:(NSColor *)color
    ...
    const CGFloat y = [self retinaRound:(wantUnderline ? [self yOriginForUnderlineForFont:font
                                                                                  yOffset:rect.origin.y
                                                                               cellHeight:_cellSize.height]
                                                       : [self yOriginForStrikethroughForFont:font
                                                                                      yOffset:rect.origin.y
                                                                                   cellHeight:_cellSize.height])];
    NSPoint origin = NSMakePoint(rect.origin.x, y);
    ...
    const CGFloat lineWidth =
        wantUnderline ? [self underlineThicknessForFont:font] : [self strikethroughThicknessForFont:font];
    switch (underlineStyle) {
        ...
        case NSUnderlineStyleThick: { // We use this for curly. Cocoa doesn't have curly underlines, so we reprupose
                                      // thick.
...
- (void)drawKittyImagesInRange:(iTermSignedRange)zIndexRange virtualOffset:(CGFloat)virtualOffset {
    [_kittyImageDraws
        enumerateObjectsUsingBlock:^(iTermKittyImageDraw *_Nonnull draw, NSUInteger idx, BOOL *_Nonnull stop) {
            if (draw.virtual) {
...
- (void)drawKittyImage:(iTermKittyImageDraw *)draw virtualOffset:(CGFloat)virtualOffset {
    ...
    [image drawInRect:destination fromRect:draw.sourceFrame operation:NSCompositingOperationSourceOver fraction:1];
...
iTermKittyPlaceholderDrawInstructionsCreate(iTermKittyImageDraw *draw, NSSize cellSize, VT100GridCoord sourceCoord,
                                            VT100GridCoord destCoord, NSPoint point, unsigned int imageID,
                                            unsigned int placementID, CGFloat virtualOffset) {
...
iTermKittyImageDraw *iTermFindKittyImageDrawForVirtualPlaceholder(NSArray<iTermKittyImageDraw *> *draws,
                                                                  unsigned int placementID, unsigned int imageID) {
...
- (void)drawKittyImageInPlaceholderWithCoord:(VT100GridCoord)sourceCoord
    ...
    iTermKittyImageDraw *draw = iTermFindKittyImageDrawForVirtualPlaceholder(_kittyImageDraws, placementID, imageID);
    if (!draw) {
        return;
    }
    if (draw.placementSize.width <= 0 || draw.placementSize.height <= 0) {
    ...
    iTermKittyPlaceholderDrawInstructions instructions = iTermKittyPlaceholderDrawInstructionsCreate(
        draw, _cellSize, sourceCoord, destCoord, point, imageID, placementID, virtualOffset);
    [NSGraphicsContext saveGraphicsState];
    NSAffineTransform *transform = [NSAffineTransform transform];
...
+ (NSRect)sourceRectangleForKittyPlaceholderAtSourceCoord:(VT100GridCoord)sourceCoord
    ...
    CGFloat adjustedSourceX =
        sourceCoord.x * sourceCellSize.width - horizontalPadding * (sourceCellSize.width / cellSize.width);
    CGFloat adjustedSourceY =
        sourceCoord.y * sourceCellSize.height - verticalPadding * (sourceCellSize.height / cellSize.height);

    const NSRect sourceRect =
        NSMakeRect(adjustedSourceX + sourceFrame.origin.x,
                   imageSize.height - (adjustedSourceY + sourceFrame.origin.y) - sourceCellSize.height,
                   sourceCellSize.width, sourceCellSize.height);
    return sourceRect;
}
...
- (void)drawImageWithCode:(unichar)code
    ...
    // BUG-5065: Guard against division by zero when imageInfo.size is zero.
    // This can happen with corrupted or malformed image data.
    if (imageInfo.size.width <= 0 || imageInfo.size.height <= 0) {
        DLog(@"Image has invalid size (width=%g, height=%g), skipping draw", (double)imageInfo.size.width,
             (double)imageInfo.size.height);
        [[NSColor brownColor] set];
        iTermRectFill(NSMakeRect(point.x, point.y, _cellSize.width * length, _cellSize.height), virtualOffset);
        return;
    }

    NSSize chunkSize = NSMakeSize(image.size.width / imageInfo.size.width, image.size.height / imageInfo.size.height);
    ...
    const NSRect sourceRect = NSMakeRect(chunkSize.width * originInImage.x,
                                         ...
                                         chunkSize.width * length, chunkSize.height);
    DLog(@"Draw %@ -> %@ with source image of size %@", NSStringFromRect(sourceRect), NSStringFromRect(destRect),
         NSStringFromSize(image.size));
    [image drawInRect:destRect fromRect:sourceRect operation:NSCompositingOperationSourceOver fraction:1];
    [NSGraphicsContext restoreGraphicsState];
}
...
+ (NSRect)offscreenCommandLineFrameForVisibleRect:(NSRect)visibleRect
    ...
    const CGFloat vmargin = [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins];
    NSRect rect = NSMakeRect(hmargin, visibleRect.origin.y - vmargin - 1, cellSize.width * gridSize.width,
...
- (void)drawOffscreenCommandLineWithVirtualOffset:(CGFloat)virtualOffset {
    ...
    iTermBackgroundColorRunsInLine *backgroundRuns = [iTermBackgroundColorRunsInLine
        backgroundRunsInLine:_offscreenCommandLine.characters.line
                  lineLength:_gridSize.width
            sourceLineNumber:line
...
- (void)drawOffscreenCommandLineDecorationsInContext:(CGContextRef)ctx virtualOffset:(CGFloat)virtualOffset {
    if (!self.offscreenCommandLine) {
        return;
    }
    ...
    const BOOL enableBlending = !iTermTextIsMonochrome() || [NSView iterm_takingSnapshot];
    const NSCompositingOperation operation =
        enableBlending ? NSCompositingOperationSourceOver : NSCompositingOperationCopy;

    iTermRectFillUsingOperation(NSMakeRect(outline.origin.x, outline.origin.y, outline.size.width, 1), operation,
                                virtualOffset);
    iTermRectFillUsingOperation(NSMakeRect(outline.origin.x, NSMaxY(outline) - 1, outline.size.width, 1), operation,
                                virtualOffset);
}
...
- (BOOL)drawInputMethodEditorTextAt:(int)xStart
    ...
    if ([self hasMarkedText]) {
        ...
        StringToScreenChars(str, buf, fg, bg, &len, _ambiguousIsDoubleWidth, &cursorIndex, NULL, _normalization,
                            self.unicodeVersion, self.softAlternateScreenMode, NULL);
        int cursorX = 0;
        ...
        for (i = 0; i < len;) {
            ...
            int charsInLine = MIN(remainingCharsInLine, remainingCharsInBuffer);
            int skipped = 0;
            if (charsInLine + i < len && ScreenCharIsDWC_RIGHT(buf[charsInLine + i])) {
            ...
            // Draw the background.
            NSRect r = NSMakeRect(x, y, charsInLine * _cellSize.width, _cellSize.height);
            ...
            PTYFontInfo *fontInfo = [_delegate drawingHelperFontForChar:128
                                                           ...
                                                           renderItalic:&unusedItalic
                                                               remapped:&ignore];
            NSRect rect = NSMakeRect(x, y - round((_cellSize.height - _cellSizeWithoutSpacing.height) / 2.0),
                                     charsInLine * _cellSize.width, _cellSize.height);
        ...
        NSRect cursorFrame = NSMakeRect(
            cursorX, cursorY + round((_cellSize.height - _cellSizeWithoutSpacing.height) / 2.0), 2.0, cursorHeight);
...
- (NSRect)dumbFrameForCursorAt:(VT100GridCoord)cursorCoord {
    ...
    if ([iTermAdvancedSettingsModel fullHeightCursor]) {
        ...
        return NSMakeRect(
            floor(cursorCoord.x * _cellSize.width + [iTermPreferences intForKey:kPreferenceKeySideMargins]),
            rowNumber * _cellSize.height, MIN(_cellSize.width, _cellSizeWithoutSpacing.width), height);
    } else {
        ...
        return NSMakeRect(
            ...
            MIN(_cellSize.width, _cellSizeWithoutSpacing.width), height);
    }
}
...
- (iTermCursorInfo)cursorInfoForCoord:(VT100GridCoord)cursorCoord {
    ...
    BOOL isDoubleWidth;
    screen_char_t screenChar = [self charForCursorAtColumn:cursorCoord.x inLine:theLine doubleWidth:&isDoubleWidth];
    ...
    return (iTermCursorInfo){.rect = rect, .isDoubleWidth = isDoubleWidth, .screenChar = screenChar};
}
...
- (void)drawCopyModeCursorWithBackgroundColor:(NSColor *)cursorBackgroundColor virtualOffset:(CGFloat)virtualOffset {
...
- (VT100GridCoord)coordinateByTransformingScreenCoordinateForRTL:(VT100GridCoord)screenCoord {
    iTermBidiDisplayInfo *bidiInfo =
        [self.delegate drawingHelperBidiInfoForLine:screenCoord.y + _numberOfScrollbackLines];
    if (!bidiInfo) {
        return screenCoord;
    }
...
- (NSRect)reallyDrawCursor:(iTermCursor *)cursor
    ...
    const VT100GridCoord cursorCoord =
        coordIsLogical ? [self coordinateByTransformingScreenCoordinateForRTL:nominalCursorCoord] : nominalCursorCoord;
...
- (NSRange)underlinedRangeOnLine:(long long)row {
    ...
    if (row == _underlinedRange.coordRange.start.y && row == _underlinedRange.coordRange.end.y) {
        ...
        const int start = VT100GridAbsWindowedRangeStart(_underlinedRange).x;
        const int end = _underlinedRange.columnWindow.length > 0 ? VT100GridRangeMax(_underlinedRange.columnWindow) + 1
                                                                 : _gridSize.width;
        return NSMakeRange(start, end - start);
    } else if (row == _underlinedRange.coordRange.end.y) {
        // Underline spans multiple lines, ending at this one.
        const int start = _underlinedRange.columnWindow.length > 0 ? _underlinedRange.columnWindow.location : 0;
        const int end = VT100GridAbsWindowedRangeEnd(_underlinedRange).x;
        return NSMakeRange(start, end - start);
...
- (BOOL)shouldShowCursor {
    if (_cursorBlinking && self.isInKeyWindow && _textViewIsActiveSession && _textViewIsFirstResponder &&
...
- (screen_char_t)charForCursorAtColumn:(int)column
    ...
    // BUG-1613: Check bounds BEFORE accessing array to avoid reading past buffer
    // when cursor is in pending-wrap position (column == width)
    screen_char_t screenChar;
    if (column >= width) {
        // Cursor at or past last column (pending wrap state)
        if (width > 0) {
            screenChar = theLine[width - 1];
        } else {
            screenChar = (screen_char_t){0};
        }
        screenChar.code = 0;
        screenChar.complexChar = NO;
    } else if (column < 0) {
        // Defensive: handle negative column
        screenChar = (screen_char_t){0};
    } else {
...
- (BOOL)shouldDrawCursor {
    ...
    BOOL result = (![self hasMarkedText] && _isCursorVisible && shouldShowCursor && column <= width && column >= 0 &&
                   row >= 0 && row < height && !copyMode);
    DLog(@"shouldDrawCursor: hasMarkedText=%d, isCursorVisible=%d, showCursor=%d, column=%d, row=%d"
         @"width=%d, height=%d, copyMode=%@. Result=%@",
         (int)[self hasMarkedText], (int)_isCursorVisible, (int)shouldShowCursor, column, row, width, height,
         @(copyMode), @(result));
    return result;
}
...
- (NSRange)rangeOfVisibleRows {
    int visibleRows =
        floor((_scrollViewContentSize.height - [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) /
              _cellSize.height);
...
- (VT100GridCoordRange)visualCoordRangeForRect:(NSRect)rect {
    ...
    for (int i = minY; i <= maxY; i++) {
        const VT100GridCoordRange logicalRect =
            VT100GridCoordRangeMake(MAX(0, floor((rect.origin.x - sideMargin) / _cellSize.width)), i,
                                    MAX(0, ceil((NSMaxX(rect) - sideMargin) / _cellSize.width)), i);
        VT100GridCoordRange visualRect;
        iTermBidiDisplayInfo *bidi = [self.delegate drawingHelperBidiInfoForLine:i];
        if (bidi) {
            NSRange visualRange = [bidi
                visualRangeForLogicalRange:NSMakeRange(logicalRect.start.x, logicalRect.end.x - logicalRect.start.x)];
...
- (VT100GridCoordRange)coordRangeForRect:(NSRect)rect {
    return VT100GridCoordRangeMake(
        MAX(0, floor((rect.origin.x - [iTermPreferences intForKey:kPreferenceKeySideMargins]) / _cellSize.width)),
...
- (NSRect)rectForCoordRange:(VT100GridCoordRange)coordRange {
    return NSMakeRect(coordRange.start.x * _cellSize.width + [iTermPreferences intForKey:kPreferenceKeySideMargins],
                      coordRange.start.y * _cellSize.height, (coordRange.end.x - coordRange.start.x) * _cellSize.width,
...
- (NSRect)rectByGrowingRect:(NSRect)innerRect {
    ...
    NSRect outerRect = NSMakeRect(minPoint.x, minPoint.y, maxPoint.x - minPoint.x, maxPoint.y - minPoint.y);
    return outerRect;
}
...
- (NSRange)rangeOfColumnsFrom:(CGFloat)x ofWidth:(CGFloat)width {
    ...
    charRange.length = ceil((x + width - [iTermPreferences intForKey:kPreferenceKeySideMargins]) / _cellSize.width) -
                       charRange.location;
    ...
    [self constructAndDrawRunsForLine:line
                   ...
                   externalAttributes:eaIndex
                    ...
                    displayLineNumber:row
                      ...
                      startingAtPoint:NSMakePoint([iTermPreferences intForKey:kPreferenceKeySideMargins],
...
+ (BOOL)cursorUsesBackgroundColorForScreenChar:(screen_char_t)screenChar
    ...
    if (reverseVideo) {
        if (wantBackgroundColor && screenChar.backgroundColorMode == ColorModeAlternate &&
            screenChar.backgroundColor == ALTSEM_DEFAULT) {
            return NO;
        } else if (!wantBackgroundColor && screenChar.foregroundColorMode == ColorModeAlternate &&
