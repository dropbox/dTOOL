...
- (NSString *)description {
    // ~25 chars per sum + ~25 chars per value
    NSMutableString *result = [NSMutableString stringWithCapacity:(_sums.size() + _values.size()) * 25];
    int i = 0;
    for (auto sum_i : _sums) {
...
- (NSInteger)sumOfValuesInRange:(NSRange)range {
    // BUG-f859: Replace assert with guard to prevent crash on invalid range
    if (range.location >= NSIntegerMax) {
        DLog(@"BUG-f859: sumOfValuesInRange called with invalid location (%lu)", (unsigned long)range.location);
        return 0;
    }
    ...
    // BUG-f860: Replace asserts with guard to prevent crash on out-of-bounds access
    if (highIndex >= (int)_sums.size() || lowIndex >= (int)_sums.size() || lowIndex >= (int)_values.size()) {
        DLog(@"BUG-f860: sumOfValuesInRange index out of bounds (highIndex=%d, lowIndex=%d, sums.size=%lu, "
             @"values.size=%lu)",
             highIndex, lowIndex, (unsigned long)_sums.size(), (unsigned long)_values.size());
        return 0;
    }
...
- (NSInteger)indexContainingValue:(NSInteger)value roundUp:(BOOL *)roundUp {
    ...
    // Subtract the offset because the offset is negative and our values are higher than what is exposed by the owner's
    // interface.
...
- (void)setLastValue:(NSInteger)value {
    ...
    // BUG-f861: Replace assert with guard to prevent crash when size < 2
    if (index < 1) {
        DLog(@"BUG-f861: setLastValue called with insufficient sums size (%lu)", (unsigned long)_sums.size());
        return;
    }
    _values[index] = value;
...
- (void)setFirstValue:(NSInteger)value {
    // BUG-f862: Guard against empty _values to prevent crash
    if (_values.empty()) {
        DLog(@"BUG-f862: setFirstValue called with empty values");
        return;
    }
    ...
    if (_sums.size() > 1) {
        ...
        // are dropped, though.
        // BUG-f863: Replace assert with guard to prevent crash on positive delta
        if (delta > 0) {
            DLog(@"BUG-f863: setFirstValue called with positive delta (%ld) when multiple blocks exist", (long)delta);
            return;
        }
    }
    _offset += delta;
