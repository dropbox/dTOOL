...
- (NSString *)stringForRow:(NSInteger)row column:(NSTableColumn *)column {
    // BUG-1726: Validate array bounds before access
    NSArray<iTermScriptHistoryEntry *> *entries = [self filteredEntries];
    if (row < 0 || (NSUInteger)row >= entries.count) {
        return @"";
    }
    iTermScriptHistoryEntry *entry = entries[row];
...
// BUG-392: Add bounds check to prevent array index out of bounds crash
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
    ...
    if (!_tableView.numberOfSelectedRows) {
        ...
        NSInteger row = _tableView.selectedRow;
        NSArray *filteredEntries = [self filteredEntries];
        if (row < 0 || row >= (NSInteger)filteredEntries.count) {
            return;
        }
        iTermScriptHistoryEntry *entry = [filteredEntries objectAtIndex:row];
...
- (void)updateFilteredValue {
    ...
    // BUG-1727: Validate array bounds before access
    NSArray<iTermScriptHistoryEntry *> *entries = [self filteredEntries];
    ...
    iTermScriptHistoryEntry *entry = entries[selectedRow];
...
- (void)connectionAccepted:(NSNotification *)notification {
    ...
    if (key) {
        ...
        // BUG-1718: Replace assert(false) with proper error handling
        DLog(@"BUG-1718: connectionAccepted called with nil key - this should not happen");
        return;
    }
    if (!entry) {
        NSString *name = [NSString castFrom:notification.userInfo[@"job"]];
...
- (void)connectionClosed:(NSNotification *)notification {
    NSString *key = notification.object;
    // BUG-1719: Replace assert(key) with defensive validation
    if (!key) {
        DLog(@"BUG-1719: connectionClosed called with nil key - ignoring");
        return;
    }
