...
class MenuItemTipController: NSObject, NSMenuDelegate {
    @objc static let instance = MenuItemTipController()
    ...
    func registerTip(forMenuItem menuItem: NSMenuItem, image: NSImage?, attributedString: NSAttributedString) {
        // BUG-f656: Replace it_assert with guard - delegate mismatch should not crash
        guard menuItem.parent?.submenu?.delegate === self || menuItem.parent?.submenu?.delegate == nil else {
            DLog("BUG-f656: registerTip called for menuItem with unexpected delegate: \(String(describing: menuItem.parent?.submenu?.delegate))")
            // Continue anyway - we'll override the delegate
    ...
    private func getMenuItemBounds(_ menuItem: NSMenuItem) -> NSRect? {
        ...
        // BUG-1554: Use conditional cast to avoid crash on type mismatch
        guard posResult == .success,
              let positionValue,
              CFGetTypeID(positionValue) == AXValueGetTypeID() else { return nil }
        // BUG-1753: Use unsafeBitCast for CF bridging after type ID check
        let posValue = unsafeBitCast(positionValue, to: AXValue.self)
        ...
        // BUG-1754: Use unsafeBitCast for CF bridging after type ID check
    ...
    private func getAXUIElement(for menuItem: NSMenuItem) -> AXUIElement? {
        ...
        guard AXUIElementCopyAttributeValue(axApp, "AXMenuBar" as CFString, &mainMenu) == .success,
              let mainMenu,
              // BUG-1554: Verify type before force cast
              CFGetTypeID(mainMenu) == AXUIElementGetTypeID() else {
            DLog("No menu")
            return nil
        }
        // Recursively search for the menu item
        // BUG-1755: Use unsafeBitCast for CF bridging after type ID check
        return findAXElement(in: unsafeBitCast(mainMenu, to: AXUIElement.self), withPath: itemPath)
    }
    ...
    private func findAXElement(in element: AXUIElement, withPath path: [String]) -> AXUIElement? {
        ...
        // BUG-1681: Use guard binding instead of force unwrap for path.first
        guard let firstPathElement = path.first else { return nil }
        DLog("Searching \(children.count) children of \(element) for \(firstPathElement)")
        for child in children {
            ...
            // BUG-1681: Use firstPathElement instead of force unwrap
            DLog("Found \(firstPathElement)! Recursing for the rest of \(Array(path.dropFirst()).joined(separator: ", "))…")
