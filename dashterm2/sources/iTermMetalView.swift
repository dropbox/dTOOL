...
/// only useful code paths for DashTerm2. It is more or less tested and works.
@MainActor
@objc(iTermMetalView)
public class iTermMetalView: NSView {
    private static let getDrawableQueue = DispatchQueue(label: "com.dashterm.dashterm2.get-drawable")

    private var metalLayerBox: iTermMetalLayerBox? = nil
    private var frameInterval: Int = 0
    private var sizeDirty: Bool = false
    // BUG-519: Initialize to (1,1) to prevent division by zero before initCommon() completes
    private var drawableScaleFactor: CGSize = CGSize(width: 1.0, height: 1.0)
    ...
    override public func encode(with coder: NSCoder) {
        ...
        let clearColorData = withUnsafeBytes(of: clearColor) { bytes in
            ...
            return Data(bytes: baseAddress, count: MemoryLayout<MTLClearColor>.size)
        }
    ...
    required init?(coder: NSCoder) {
        ...
        if coder.containsValue(forKey: "MTKViewColorPixelFormatArrayCoderKey") {
            ...
            // BUG-f662: Replace it_assert with guard - invalid pixel format length should not crash
            let expectedLength = maxValidAttachmentIndex * 8
            if length != expectedLength {
                DLog("BUG-f662: Invalid pixel format array length: \(length) != \(expectedLength)")
            }
            ...
            if length > 0 && formats.count > maxValidAttachmentIndex {
        ...
        if coder.containsValue(forKey: "MTKViewClearColorCoderKey") {
            // BUG-1751: Use guard with as? instead of as! force cast
            if let data = coder.decodeObject(forKey: "MTKViewClearColorCoderKey") as? Data {
...
extension iTermMetalView {
    ...
    public var drawableSize: CGSize {
        ...
        set {
            ...
            // BUG-520: Default to 1.0 instead of 0 when screen is nil to prevent division by zero
            let backingScaleFactor = if let screen {
                screen.backingScaleFactor
            } else {
                NSScreen.main?.backingScaleFactor ?? 1.0
            }
            // BUG-521: Guard against division by zero when bounds or scale factor is zero
            let divisorWidth = myBounds.width * backingScaleFactor
            let divisorHeight = myBounds.height * backingScaleFactor
            drawableScaleFactor = CGSize(
                width: divisorWidth > 0 ? newValue.width / divisorWidth : 1.0,
                height: divisorHeight > 0 ? newValue.height / divisorHeight : 1.0)
    ...
    private func fetchDrawable(timeout: TimeInterval) -> CAMetalDrawable? {
        ...
        while !timedOut {
            ...
            promise.wait(withTimeout: timeout).whenFirst { pending in
                ...
                // BUG-f663: Replace it_assert with DLog - promise rejection should not crash
                if err.code != iTermPromiseErrorCode.timeout.rawValue {
                    DLog("BUG-f663: Promise rejection with unexpected error code: \(err.code) (expected \(iTermPromiseErrorCode.timeout.rawValue))")
                }
...
extension iTermMetalView {
    ...
    public func currentRenderPassDescriptor(timeout: TimeInterval) -> MTLRenderPassDescriptor? {
        ...
        for i in 0...maxValidAttachmentIndex {
            // BUG-1801: Use guard to safely unwrap texture instead of force unwrap
            guard let texture = _colorTextures[i] else {
                continue
            }
...
extension iTermMetalView {
    ...
    open override func convertFromBacking(_ point: CGPoint) -> CGPoint {
        ...
        // BUG-522: Guard against division by zero if scale is zero
        return CGPoint(
    ...
    open override func convertFromBacking(_ size: CGSize) -> CGSize {
        ...
        let scale = drawableScaleFactor
        // BUG-523: Guard against division by zero if scale is zero
        return CGSize(
