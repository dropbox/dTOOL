...
class IOBuffer: NSObject {
    ...
    private func _handleWriteReady() {
        ...
        while let dataToWrite = writeQueue.first {
            // BUG-f1397: Replace it_assert with guard - in release builds it_assert is stripped
            // which could cause an infinite loop if empty data somehow gets into the queue
            guard !dataToWrite.isEmpty else {
                DLog("BUG-f1397: Empty data in writeQueue - skipping to prevent infinite loop")
