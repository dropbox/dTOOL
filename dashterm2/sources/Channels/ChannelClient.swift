...
class ChannelClient: NSObject {
    ...
    // BUG-f589: Error enum for ChannelClient initialization failures
    enum ChannelClientError: Error, LocalizedError {
        case sshConnectionNotImplemented

        var errorDescription: String? {
            switch self {
            case .sshConnectionNotImplemented:
                return "SSH connection over conductor is not yet implemented"
            }
        }
    }
    ...
    init(uid: String, conductor: Conductor?) throws {
        ...
        mux = if conductor != nil {
            try Self.connectOverSSH()
        } else {
            try Self.connectLocally(uid)
        }
    }

    // BUG-f589: Throw error instead of crashing when SSH connection is attempted
    private static func connectOverSSH() throws -> UnixDomainSocketMux {
        DLog("ERROR: SSH connection over conductor requested but not implemented")
        throw ChannelClientError.sshConnectionNotImplemented
    }
...
fileprivate class MessageFramer {
    ...
    func addSegment(header: Header, data: Data) -> Segment? {
        // BUG-f1412: Validate segment index before accessing array
        // segmentIndex must be < totalSegments to prevent array out of bounds crash
        guard header.segmentIndex < header.totalSegments else {
            DLog("BUG-f1412: Invalid segment index \(header.segmentIndex) >= totalSegments \(header.totalSegments)")
            return nil
        }
        // BUG-f1412: Protect against zero totalSegments creating empty array
        guard header.totalSegments > 0 else {
            DLog("BUG-f1412: Invalid totalSegments = 0")
            return nil
        }
        ...
        // BUG-f1412: Double-check array exists and index is valid before assignment
        guard let existingMessages = messages[key],
              Int(header.segmentIndex) < existingMessages.count else {
            DLog("BUG-f1412: Message array not found or segment index out of bounds")
            return nil
        }
...
class UnixDomainSocketMux {
    ...
    init(baseDir: String = "~/.dashterm2", id: String) throws {
    ...
    deinit {
        close()  // BUG-1018: Ensure socket is closed when object is deallocated
    }
