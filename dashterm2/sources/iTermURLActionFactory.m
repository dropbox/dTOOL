...
+ (instancetype)urlActionAtCoord:(VT100GridCoord)coord
    ...
    dispatch_once(&onceToken, ^{
        sFactories = [NSMutableArray arrayWithCapacity:4]; // Active URL action factories
    });
    DLog(@"Created %@", self);
...
- (URLAction *)urlActionForPrompt {
    ...
    URLAction *action = [URLAction actionToShowCommandInfoForMark:mark coord:self.coord];
...
- (URLAction *)urlActionForHypertextLink {
    ...
    if (url != nil) {
        ...
        action.logicalRange = [extractor
            rangeOfCoordinatesAround:self.coord
                     maximumDistance:1000
                         passingTest:^BOOL(screen_char_t *c, iTermExternalAttribute *ea, VT100GridCoord coord) {
...
- (void)urlActionForExistingFileWithPrefix:(iTermLocatedString *)locatedPrefix
    ...
    NSString *possibleFilePart1 = [locatedPrefix.string substringIncludingOffset:[locatedPrefix.string length] - 1
    ...
    NSString *possibleFilePart2 = [locatedSuffix.string substringIncludingOffset:0
    ...
    _pathFinderCanceler = [self.semanticHistoryController
        pathOfExistingFileFoundWithPrefix:possibleFilePart1
                                   suffix:possibleFilePart2
                         workingDirectory:self.workingDirectory
                           trimWhitespace:NO
                               completion:^(NSString *filename, int prefixChars, int suffixChars,
                                            BOOL workingDirectoryIsLocal) {
                                   DLog(@"Semantic history controller returned filename %@ with %@ prefix and %@ "
                                        @"suffix chars",
                                        filename, @(prefixChars), @(suffixChars));
...
- (URLAction *)urlActionForFilename:(NSString *)filename
    ...
    // pretty common.
    if (filename.length == 0 || [[filename stringByReplacingOccurrencesOfString:@"//"
    ...
    action.fullPath =
        [self.semanticHistoryController cleanedUpPathFromPath:filename
...
- (void)computeURLActionForSmartSelection:(void (^)(URLAction *action))completion {
    ...
    if (actions.count) {
        ...
        URLAction *action = [URLAction urlActionToPerformSmartSelectionRule:smartMatch.rule onString:content];
        ...
        action.representedObject = @{
            iTermSmartSelectionActionContextKeyAction : actions[index],
    ...
    if (self.semanticHistoryController.activatesOnAnyString) {
        ...
        SmartMatch *smartMatch = [self.extractor smartSelectionAt:self.coord
                                                   ...
                                                   actionRequired:NO
                                                            range:&smartRange
                                                 ignoringNewlines:!self.respectHardNewlines];
        if (!VT100GridCoordEquals(smartRange.coordRange.start, smartRange.coordRange.end)) {
...
- (URLAction *)urlActionForURLLike {
    // Guard against nil locatedPrefix/locatedSuffix which would crash stringByAppendingString:
    if (!self.locatedPrefix || !self.locatedSuffix) {
        DLog(@"Cannot look for URL-like things: locatedPrefix=%@ locatedSuffix=%@", self.locatedPrefix,
             self.locatedSuffix);
        return nil;
    }
    NSString *joined = [self.locatedPrefix.string stringByAppendingString:self.locatedSuffix.string];
    DLog(@"Smart selection found nothing. Look for URL-like things in %@ around offset %d", joined,
         (int)[self.locatedPrefix.string length]);
    int prefixChars = 0;
    ...
    DLog(@"Range excluding punctuation is %@. Adjust prefixChars down to %d",
         NSStringFromRange(rangeWithoutNearbyPunctuation), prefixChars);
    ...
    if ([iTermAdvancedSettingsModel conservativeURLGuessing]) {
        ...
        NSString *hostnameRegex = @"(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\\-]*[a-zA-Z0-9])\\.)+([A-Za-z0-9]|[A-Za-z0-9]["
                                  @"A-Za-z0-9\\-]*[A-Za-z0-9])";
        ...
        if ([stringWithoutNearbyPunctuation rangeOfRegex:urlRegex].location != NSNotFound) {
            DLog(@"LGTM, using %@ with range %@ and prefix %d", stringWithoutNearbyPunctuation,
                 NSStringFromRange(rangeWithoutNearbyPunctuation), prefixChars);
    ...
    if (hasColon) {
        ...
        if (looksLikeURL) {
            if (!self.workingDirectoryIsLocal && ![stringWithoutNearbyPunctuation containsString:@"/"]) {
                DLog(@"The working directory is not local and there's no slash in the filename or colon for a scheme, "
                     @"so this might be a file on a remote filesystem. Don't treat it as a URL.");
                return nil;
            }
    ...
    if (looksLikeURL) {
        DLog(@"Looks like a URL. Return %@ with range %@ and prefix %d", stringWithoutNearbyPunctuation,
             NSStringFromRange(rangeWithoutNearbyPunctuation), prefixChars);
...
- (BOOL)stringIsSingleDomainWord:(NSString *)string {
    ...
    return (string.length > 0 && !isdigit([string characterAtIndex:0]) &&
            [string rangeOfCharacterFromSet:nonAlnum].location == NSNotFound);
}

- (URLAction *)urlActionForString:(NSString *)string range:(NSRange)stringRange prefixChars:(int)prefixChars {
    ...
    BOOL openable = (url && [[NSWorkspace sharedWorkspace] URLForApplicationToOpenURL:url] != nil && prefixChars >= 0 &&
    ...
    if (openable) {
        ...
        if (i >= 0 && i < self.locatedSuffix.gridCoords.count) {
            ...
            DLog(@"range.coordRange.end=%@, successor of %@", VT100GridCoordDescription(range.coordRange.end),
        ...
        DLog(@"%@ is not openable (couldn't convert it to a URL [%@] or no scheme handler", string, url);
    }
    return nil;
}
...
- (BOOL)stringLooksLikeURL:(NSString *)s {
    ...
    NSString *hostnameRegex = @"^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\\-]*[a-zA-Z0-9])\\.)+([A-Za-z0-9]|[A-Za-z0-9][A-"
                              @"Za-z0-9\\-]*[A-Za-z0-9])$";
