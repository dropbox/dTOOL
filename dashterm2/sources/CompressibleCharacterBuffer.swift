...
class UnsafeReallocatableMutableBuffer<T: DefaultInitializable & Equatable>: Equatable {
    ...
    init(count: Int) {
        // BUG-f1277: Replace it_fatalError with safe guard - clamp negative count to 0 instead of crashing
        // Negative counts indicate a programming error but shouldn't crash the entire app
        let safeCount: Int
        if count < 0 {
            DLog("BUG-f1277: UnsafeReallocatableMutableBuffer initialized with negative count \(count) - clamping to 0")
            safeCount = 0
        } else {
            safeCount = count
        }
        // BUG-f1278: Replace it_fatalError with empty buffer fallback on allocation failure
        // Memory allocation failures are rare but shouldn't crash - return empty buffer instead
        guard let rawPointer = calloc(max(1, safeCount), MemoryLayout<T>.stride) else {
            DLog("BUG-f1278: Failed to allocate memory for buffer of size \(safeCount) - creating empty buffer")
            buffer = UnsafeMutableBufferPointer<T>(start: nil, count: 0)
            return
        }
        let pointer = UnsafeMutableRawPointer(rawPointer)
    ...
    convenience init(copyingBytesFromAddress fromAddress: UnsafePointer<T>, count: Int) {
        ...
        // BUG-f1279: Return early with empty buffer instead of crashing when baseAddress is nil
        guard let baseAddr = buffer.baseAddress else {
            DLog("BUG-f1279: Buffer baseAddress is nil when copying bytes - leaving buffer empty")
            return
        }
        // BUG-f1280: Return early instead of crashing on integer overflow
        let (byteCount, overflow) = count.multipliedReportingOverflow(by: MemoryLayout<T>.stride)
        if overflow {
            DLog("BUG-f1280: Integer overflow calculating memmove size: \(count) * \(MemoryLayout<T>.stride) - skipping copy")
            return
        }
        memmove(baseAddr, fromAddress, byteCount)
    }

    convenience init(_ data: Data) {
        self.init(count: data.count / MemoryLayout<T>.stride)
        data.withUnsafeBytes { rawBufferPointer in
            // BUG-f1281: Return early instead of crashing when baseAddress is nil
            guard let address = rawBufferPointer.assumingMemoryBound(to: screen_char_t.self).baseAddress,
                  let destAddr = buffer.baseAddress else {
                DLog("BUG-f1281: Buffer baseAddress is nil when initializing from Data - leaving buffer empty")
                return
            }
            // BUG-f1282: Return early instead of crashing on integer overflow
            let (byteCount, overflow) = count.multipliedReportingOverflow(by: MemoryLayout<T>.stride)
            if overflow {
                DLog("BUG-f1282: Integer overflow calculating memmove size: \(count) * \(MemoryLayout<T>.stride) - skipping copy")
                return
            }
            memmove(destAddr, address, byteCount)
        }
    }
    ...
    convenience init(_ decoder: inout EfficientDecoder, placeholder: T) throws {
        ...
        // BUG-f1283: Throw error instead of crashing on integer overflow
        let (byteCount, overflow) = array.count.multipliedReportingOverflow(by: MemoryLayout<T>.stride)
        if overflow {
            DLog("BUG-f1283: Integer overflow calculating memcpy size: \(array.count) * \(MemoryLayout<T>.stride)")
            throw EfficientDecoderError("Integer overflow in buffer size calculation")
        }
        array.withUnsafeBytes { arrayUnsafeBytes in
            // BUG-f1284: Return early instead of crashing when baseAddress is nil
            guard let arrayBase = arrayUnsafeBytes.baseAddress,
                  let bufferBase = buffer.baseAddress else {
                DLog("BUG-f1284: Buffer baseAddress is nil when initializing from decoder - leaving buffer empty")
                return
            }
            arrayBase.withMemoryRebound(to: T.self, capacity: array.count) { arrayPointer in
                bufferBase.withMemoryRebound(to: T.self, capacity: array.count) { destinationBytes in
                    _ = memcpy(destinationBytes, arrayPointer, byteCount)
                }
            }
        }
    }
    ...
    deinit {
        guard let baseAddr = buffer.baseAddress else {
            // Buffer was never allocated or already freed
            return
        }
        let pointer = UnsafeMutableRawPointer(baseAddr)
        Darwin.free(pointer)
    }

    func resize(to count: Int) {
        // BUG-f1285: Return early instead of crashing when baseAddress is nil
        // If buffer has no base address, allocate fresh instead of crashing
        guard let baseAddr = buffer.baseAddress else {
            DLog("BUG-f1285: Buffer baseAddress is nil during resize - allocating new buffer of size \(count)")
            // Allocate new buffer since we can't resize nil
            let safeCount = max(0, count)
            if let rawPointer = calloc(max(1, safeCount), MemoryLayout<T>.stride) {
                let pointer = UnsafeMutableRawPointer(rawPointer)
                buffer = UnsafeMutableBufferPointer<T>(start: pointer.assumingMemoryBound(to: T.self),
                                                       count: safeCount)
            }
        ...
        var pointer = UnsafeMutableRawPointer(baseAddr)
        // BUG-f1286: Return early instead of crashing on integer overflow
        let (byteCount, overflow) = max(1, count).multipliedReportingOverflow(by: MemoryLayout<T>.stride)
        if overflow {
            DLog("BUG-f1286: Integer overflow calculating realloc size: \(count) * \(MemoryLayout<T>.stride) - keeping current size")
            return
        }
        // BUG-f1287: Return early instead of crashing on realloc failure
        guard let newPointer = realloc(pointer, byteCount) else {
            DLog("BUG-f1287: Failed to reallocate memory to size \(count) - keeping current buffer")
            return
        }
        pointer = newPointer
    ...
    func clone() -> UnsafeReallocatableMutableBuffer<T> {
        // BUG-f1288: Return empty buffer instead of crashing when cloning nil buffer
        guard let baseAddr = buffer.baseAddress else {
            DLog("BUG-f1288: Cannot clone buffer with nil baseAddress - returning empty buffer")
            return UnsafeReallocatableMutableBuffer(count: 0)
        }
        return UnsafeReallocatableMutableBuffer(copyingBytesFromAddress: baseAddr,
                                                count: count)
    }
...
struct CompressedScreenCharBuffer: Equatable, CustomDebugStringConvertible {
    ...
    /// BUG-f570: Return early with empty buffer instead of crashing when decompression buffer fails
    func decompressed(capacity: Int) -> UnsafeReallocatableMutableBuffer<screen_char_t> {
        ...
        guard let baseAddr = buffer.buffer.baseAddress else {
            // BUG-f570: Return empty buffer instead of crashing when decompression buffer fails
            DLog("BUG-f570: Buffer baseAddress is nil during decompression - returning empty buffer")
            return buffer  // Return empty buffer as fallback
        }
        var offset = 0
        for run in runs {
            // BUG-f665: Add guard for offset overflow to prevent buffer overrun
            guard count >= offset else {
                DLog("BUG-f665: Decompression offset \(offset) exceeds count \(count) - truncating")
                break
            }
            run.expand(to: baseAddr.advanced(by: offset),
    ...
    /// BUG-f571: Return default screen_char_t instead of crashing for out of bounds access
    subscript(_ i: Int) -> screen_char_t {
        mutating get {
            ...
            // BUG-f571: Return empty character instead of crashing for out of bounds access
            DLog("BUG-f571: Index \(i) out of bounds with \(_lookupCache.i) entries in CompressedScreenCharBuffer - returning empty character")
    ...
    /// BUG-f572: Return nil instead of crashing on integer overflow
    mutating func string(from offset: Int,
        ...
        // BUG-2650: Check for integer overflow in initial capacity calculation
        var (capacity, overflow1) = length.addingReportingOverflow(Int(kMaxParts))
        let overflow2: Bool
        (capacity, overflow2) = capacity.addingReportingOverflow(1)
        if overflow1 || overflow2 {
            // BUG-f572: Return nil instead of crashing on integer overflow
            DLog("BUG-f572: Integer overflow calculating string capacity: \(length) + \(kMaxParts) + 1 - returning nil")
            backingStorePtr.pointee = nil
            deltasPtr.pointee = nil
            return nil
        }
        ...
        for i in 0..<length {
            ...
            if o + Int(kMaxParts) + 1 >= capacity {
                // BUG-2650: Check for integer overflow when doubling capacity
                let (newCapacity, capacityOverflow) = capacity.multipliedReportingOverflow(by: 2)
                if capacityOverflow {
                    // BUG-f573: Return what we have instead of crashing on capacity overflow
                    DLog("BUG-f573: Integer overflow doubling string capacity: \(capacity) * 2 - returning partial string")
...
fileprivate enum Buffer: Equatable {
    ...
    subscript(_ i: Int) -> screen_char_t {
        ...
        case .uncompressed(let innerBuffer):
            // BUG-1252h: Bounds check to prevent crash on invalid index
            guard i >= 0 && i < innerBuffer.count else {
                return screen_char_t()
            }
            return innerBuffer.buffer[i]
        case .compressed(var innerBuffer), .superposition(var innerBuffer, _):
            // BUG-1252h: CompressedScreenCharBuffer handles bounds internally
...
class CompressibleCharacterBuffer: NSObject, UniqueWeakBoxable {
    ...
    private struct State: Equatable {
        ...
        mutating func resize(_ newSize: Int) {
            ...
            case .superposition:
                // BUG-f602: You must have acquired a mutation certificate to resize which guarantees no
                // superposition. Exit superposition first to avoid crash.
                DLog("BUG-f602: resize() called during superposition - exiting superposition first")
                exitSuperpositionIfNeeded(toCompressed: false)
                resize(newSize)
            }
        }
    ...
    init(_ size: Int) {
        ...
        // BUG-f1277: Clamp negative size to 0 to prevent crashes
        // Negative sizes are programming errors but shouldn't crash
        let safeSize = Swift.max(0, size)
        write { state in
            state.size = safeSize
        }
    }
    ...
    init?(encoded data: Data) {
        ...
        do {
            ...
            // BUG-2667: Log decoding errors instead of returning nil silently
            DLog("Failed to decode CompressibleCharacterBuffer from \(data.count) bytes: \(error)")
            return nil
        }
    }
    ...
    /// BUG-f574: Return 0 and log instead of crashing for uncompressed buffer
    @objc
    func numberOfFullLines(offset: Int,
        ...
        read { state in
            ...
            case .uninitialized:
                return 0
            case .uncompressed:
                // BUG-f574: Log and return 0 instead of crashing for uncompressed buffer
                DLog("BUG-f574: numberOfFullLines called on uncompressed buffer - returning 0")
                return 0
    ...
    /// BUG-f575: Return empty array instead of crashing for uninitialized/uncompressed buffer
    ...
    func screenCharArray(offset: Int,
        ...
        read { state in
            switch state.buffer {
            case .uninitialized, .uncompressed:
                // BUG-f575: Log and return empty array instead of crashing for unsupported buffer type
                DLog("BUG-f575: screenCharArray called on uninitialized/uncompressed buffer - returning empty")
                return MutableScreenCharArray.emptyLine(ofLength: Int32(paddedSize))
            case .compressed(var innerBuffer), .superposition(var innerBuffer, _):
    ...
    func string(from offset: Int,
        ...
        read { state in
            switch state.buffer {
            case .uninitialized:
                // BUG-f603: Return nil instead of crashing for uninitialized buffer
                DLog("BUG-f603: string(from:) called on uninitialized buffer")
                return nil
            case .uncompressed(let innerBuffer):
                guard let baseAddr = innerBuffer.buffer.baseAddress else {
                    // BUG-f604: Return nil instead of crashing for nil baseAddress
                    DLog("BUG-f604: Buffer baseAddress is nil in string(from:)")
                    return nil
                }
                return ScreenCharArrayToString(baseAddr.advanced(by: offset),
                                               0,
                                               Int32(length),
                                               backingStorePtr,
                                               deltasPtr)
