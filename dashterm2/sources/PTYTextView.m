...
+ (NSSize)charSizeForFont:(NSFont *)aFont horizontalSpacing:(CGFloat)hspace verticalSpacing:(CGFloat)vspace {
...
- (instancetype)initWithFrame:(NSRect)aRect {
    ...
    if (self) {
        ...
        threeFingerTapGestureRecognizer_ =
            [[ThreeFingerTapGestureRecognizer alloc] initWithTarget:self selector:@selector(threeFingerTap:)];
        ...
        _mouseHandler = [[PTYMouseHandler alloc] initWithSelectionScrollHelper:_selectionScrollHelper
                                               threeFingerTapGestureRecognizer:threeFingerTapGestureRecognizer_
                                                     pointerControllerDelegate:self
                                     mouseReportingFrustrationDetectorDelegate:self];
        _mouseHandler.mouseDelegate = self;
        _notes = [[NSMutableArray alloc] initWithCapacity:4];
        _portholes = [[NSMutableArray alloc] initWithCapacity:4];
        _trackingChildWindows = [[NSMutableArray alloc] initWithCapacity:2];
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<PTYTextView: %p frame=%@ visibleRect=%@ dataSource=%@ delegate=%@ window=%@>",
                                      self, [NSValue valueWithRect:self.frame],
                                      [NSValue valueWithRect:[self visibleRect]], _dataSource, _delegate, self.window];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
    ...
    if ([item action] == @selector(paste:)) {
        ...
        // BUG-1189: Use modern UTType API instead of deprecated kUTTypeUTF8PlainText
        return [[[NSPasteboard generalPasteboard] pasteboardItems] anyWithBlock:^BOOL(NSPasteboardItem *item) {
            return [item stringForType:UTTypeUTF8PlainText.identifier] != nil;
        }];
    }
    ...
    } else if ([item action] == @selector(selectAll:) || [item action] == @selector(installShellIntegration:) ||
    ...
    if ([item action] == @selector(copy:) || [item action] == @selector(copyWithStyles:) ||
    ...
    if (([item action] == @selector(performFindPanelAction:) && item.tag == NSFindPanelActionSetFindString) ||
        ...
        return [[iTermController sharedInstance] lastSelectionPromise] != nil &&
               [[iTermController sharedInstance] lastSelectionPromise].maybeError == nil;
    ...
    if (item.action == @selector(toggleAILock:)) {
        item.state = [_delegate textViewIsAILocked] ? NSControlStateValueOn : NSControlStateValueOff;
        return YES;
    }
    if (item.action == @selector(terminalStateToggleAlternateScreen:) ||
        ...
        item.state =
            [self.delegate textViewTerminalStateForMenuItem:item] ? NSControlStateValueOn : NSControlStateValueOff;
        return YES;
    }
...
- (void)scrollEnd {
    ...
    lastLine.origin.y = (([_dataSource numberOfLines] - 1) * _lineHeight + [self excess] +
...
- (void)flagsChanged:(NSEvent *)theEvent {
    DLog(@"flagsChanged: cmd=%d opt=%d shift=%d ctrl=%d", !!(theEvent.it_modifierFlags & NSEventModifierFlagCommand),
...
- (BOOL)performKeyEquivalent:(NSEvent *)theEvent {
    ...
    if (@available(macOS 15, *)) {
        DLog(@"kPreferenceKeyAllowSymbolicHotKeys=%@",
             @([iTermPreferences boolForKey:kPreferenceKeyAllowSymbolicHotKeys]));
    ...
    // uses cmd-ctrl-F.
    const NSEventModifierFlags mask = (NSEventModifierFlagControl | NSEventModifierFlagCommand |
...
- (void)keyDown:(NSEvent *)event {
    // Record keypress timestamp for input latency measurement
    if ([iTermAdvancedSettingsModel showInputLatencyMeter]) {
        [[iTermInputLatencyTracker sharedInstance] recordKeypressWithTimestamp:event.timestamp];
    }
...
- (int)inputMethodEditorLength {
    ...
    int len;
    StringToScreenChars(str, buf, fg, bg, &len, [_delegate textViewAmbiguousWidthCharsAreDoubleWidth], NULL, NULL,
                        [_delegate textViewUnicodeNormalizationForm], [_delegate textViewUnicodeVersion],
                        self.dataSource.terminalSoftAlternateScreenMode, NULL);
...
- (long long)absoluteScrollPosition {
    ...
    long long localOffset =
        (visibleRect.origin.y + [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]) / [self lineHeight];
...
- (void)scrollToAbsoluteOffset:(long long)absOff height:(int)height {
    NSRect aFrame;
    aFrame.origin.x = 0;
    aFrame.origin.y = (absOff - [_dataSource totalScrollbackOverflow]) * _lineHeight -
...
- (BOOL)withRelativeCoord:(VT100GridAbsCoord)coord block:(void (^NS_NOESCAPE)(VT100GridCoord coord))block {
...
- (BOOL)withRelativeCoordRange:(VT100GridAbsCoordRange)range block:(void (^NS_NOESCAPE)(VT100GridCoordRange))block {
    const long long overflow = _dataSource.totalScrollbackOverflow;
...
- (void)scrollToSelection {
    ...
    [self withRelativeCoordRange:[_selection spanningAbsRange]
                           block:^(VT100GridCoordRange range) {
                               NSRect aFrame;
                               aFrame.origin.x = 0;
                               aFrame.origin.y =
                                   range.start.y * _lineHeight -
                                   [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]; // allow for top margin
                               aFrame.size.width = [self frame].size.width;
...
- (void)scrollToCenterLine:(int)line {
    ...
    int visibleLines =
        (visible.size.height - [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) / _lineHeight;
...
- (NSRange)visibleRelativeRange {
    ...
    ITAssertWithMessage(firstVisibleLine < NSIntegerMax - height, @"Addition would overflow: %@ + %@",
                        @(firstVisibleLine), @(height));
...
- (NSRange)visibleAbsoluteRangeIncludingOffscreenCommandLineIfVisible:(BOOL)includeOffscreenCommandLine {
    ...
    iTermOffscreenCommandLine *offscreenCommandLine =
        [self.dataSource offscreenCommandLineBefore:range.location - _dataSource.totalScrollbackOverflow];
    if (!offscreenCommandLine) {
        return range;
    }
    const NSRect visibleRect = [self adjustedDocumentVisibleRectIncludingTopMargin:NO];
    const NSRect frame = [iTermTextDrawingHelper
        offscreenCommandLineFrameForVisibleRect:visibleRect
...
- (void)scrollLineNumberRangeIntoView:(VT100GridRange)range {
    ...
    if (NSIntersectionRange(desiredRange, currentlyVisibleRange).length ==
        MIN(desiredRange.length, currentlyVisibleRange.length)) {
        // Already visible
        return;
    }
...
- (void)setFrameSize:(NSSize)newSize {
    DLog(@"Set frame size to %@ from\n%@", NSStringFromSize(newSize), [NSThread callStackSymbols]);
    [super setFrameSize:newSize];
    [self recomputeBadgeLabel];
    [self updatePortholeFrames];
    // Invalidate accessibility cache since line breaks may change on resize
    [_accessibilityHelper invalidateCache];
}
...
static NSString *iTermStringForEventPhase(NSEventPhase eventPhase) {
    NSMutableArray<NSString *> *phase = [NSMutableArray arrayWithCapacity:3];
...
- (void)scrollWheel:(NSEvent *)event {
    DLog(@"scrollWheel: momentumPhase=%@, eventPhase=%@, _ignoreMomentumScroll=%@",
         iTermStringForEventPhase(event.momentumPhase), iTermStringForEventPhase(event.phase),
         @(_ignoreMomentumScroll));
...
- (void)rightMouseUp:(NSEvent *)event {
    [_mouseHandler rightMouseUp:event
                    superCaller:^{
...
- (void)touchesBeganWithEvent:(NSEvent *)ev {
    _mouseHandler.numTouches = [[ev touchesMatchingPhase:NSTouchPhaseBegan | NSTouchPhaseStationary inView:self] count];
...
- (void)touchesEndedWithEvent:(NSEvent *)ev {
    _mouseHandler.numTouches = [[ev touchesMatchingPhase:NSTouchPhaseStationary inView:self] count];
...
- (void)mouseEntered:(NSEvent *)event {
    ...
    if ([scrollView hitTest:[scrollView convertPoint:event.locationInWindow fromView:nil]] == nil) {
        DLog(@"hitTest at %@ in view (%@ in window) gives nil",
             NSStringFromPoint([self convertPoint:event.locationInWindow fromView:nil]),
             NSStringFromPoint(event.locationInWindow));
        DLog(@"Event %@ at window coord %@ failed hit test for view with window coords %@", event,
             NSStringFromPoint(event.locationInWindow), NSStringFromRect([self convertRect:self.bounds toView:nil]));
        return;
    }
...
- (NSRect)adjustedDocumentVisibleRectIncludingTopMargin:(BOOL)includeTopMargin {
    ...
    const int firstRow =
        MAX(0, documentVisibleRect.origin.y / _lineHeight - overflow) + _drawingHelper.numberOfIMELines;
    const NSRect result = [self visibleRectExcludingTopMargin:!includeTopMargin startingAtRow:firstRow];
...
- (CGFloat)virtualOffset {
    ...
    const CGFloat virtualOffset = (_dataSource.numberOfLines - height + _drawingHelper.numberOfIMELines) * _lineHeight -
                                  [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins];
    DLog(@"Force draw last rows. numberOfLines=%@ height=%@ lineHeight=%@ bottomMargins=%@ -> virtualOffset=%@",
         @(_dataSource.numberOfLines), @(height), @(_lineHeight),
         @([iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]), @(virtualOffset));
    return virtualOffset;
}
...
- (void)smearCursorIfNeededWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper {
    ...
    if (_previousCursorFrame.size.width > 0 && cursorFrame.size.width > 0 &&
        ...
        [self.delegate textViewSmearCursorFrom:from to:to color:drawingHelper.cursorColor];
    }
...
- (void)drawRect:(NSRect)rect inView:(NSView *)view {
    ...
    DLog(@"drawing document visible rect %@ for %@",
         NSStringFromRect(self.textDrawingHelperVisibleRectExcludingTopMargin), self);
    ...
    [self performBlockWithFlickerFixerGrid:^{
        ...
        [self drawingHelper];
        DLog(@"draw: minY=%@, absLine=%@", @(self.textDrawingHelperVisibleRectExcludingTopMargin.origin.y),
             @([_drawingHelper coordRangeForRect:self.textDrawingHelperVisibleRectExcludingTopMargin].start.y +
        ...
        if (gDebugLogging) {
            DLog(@"DRAW vrect=%@ voff=%@ time=%@ session=%@", NSStringFromRect(virtualRect), @(virtualOffset),
                 @([NSDate timeIntervalSinceReferenceDate]), self.delegate);
        }


        [NSGraphicsContext saveGraphicsState];
        [_drawingHelper drawTextViewContentInRect:virtualRect
                                         rectsPtr:rectArray
                                        rectCount:rectCount
                                    virtualOffset:virtualOffset];

        [NSGraphicsContext restoreGraphicsState];
        const NSRect indicatorsRect =
            NSRectSubtractingVirtualOffset(_drawingHelper.indicatorFrame, MAX(0, virtualOffset));
...
- (NSRect)visibleRectExcludingTopMargin:(BOOL)excludeTopMargin startingAtRow:(int)row {
...
- (void)performBlockWithFlickerFixerGrid:(void(NS_NOESCAPE ^)(void))block {
    ...
    [_dataSource performBlockWithSavedGrid:^(id<PTYTextViewSynchronousUpdateStateReading> _Nullable savedState) {
        if (savedState) {
            ...
            DLog(@"PTYTextView.performBlockWithFlickerFixerGrid: set cusrorVisible=%@",
                 _cursorVisible ? @"true" : @"false");
            [self loadSyncUpdateState:savedState];
        } else {
            DLog(@"PTYTextView.performBlockWithFlickerFixerGrid: (no saved grid) cusrorVisible=%@",
                 _cursorVisible ? @"true" : @"false");
        }
...
- (iTermTextDrawingHelper *)drawingHelper {
    _drawingHelper.showStripes = (_showStripesWhenBroadcastingInput && [_delegate textViewSessionIsBroadcastingInput]);
    ...
    _drawingHelper.cursorCoord = VT100GridCoordMake(_dataSource.cursorX - 1, _dataSource.cursorY - 1);
    ...
    _drawingHelper.shouldDrawFilledInCursor =
        ([self.delegate textViewShouldDrawFilledInCursor] || _focusFollowsMouse.haveStolenFocus);
    ...
    _drawingHelper.passwordInput =
        ([self isInKeyWindow] && [_delegate textViewIsActiveSession] && _delegate.textViewPasswordInput);
    ...
    _drawingHelper.highlightedBlockLineRange =
        _hoverBlockFoldButton ? [self relativeRangeFromAbsLineRange:_hoverBlockFoldButton.absLineRange]
...
- (void)updateScrollerForBackgroundColor {
    ...
    if ([self.delegate textViewTerminalBackgroundColorDeterminesWindowDecorationColor]) {
        ...
        scroller.appearance = isDark ? [NSAppearance appearanceNamed:NSAppearanceNameDarkAqua]
...
- (double)excess {
    ...
    visibleRectExcludingTopAndBottomMargins.size.height -=
        [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2; // Height without top and bottom margins.
    ...
    return MAX(visibleHeightExceptTopMargin - heightOfTextRows,
               bottomMarginHeight); // Never have less than VMARGIN excess, but it can be more (if another tab has a
                                    // bigger font)
}
...
- (BOOL)removeUnderline {
    ...
    _drawingHelper.underlinedRange = VT100GridAbsWindowedRangeMake(VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
...
- (NSRect)configureIndicatorsHelperWithRightMargin:(CGFloat)rightMargin {
    ...
    [_indicatorsHelper setIndicator:kiTermIndicatorCoprocess
                     ...
                     darkBackground:isDark];
    [_indicatorsHelper setIndicator:kiTermIndicatorAlert visible:[_delegate alertOnNextMark] darkBackground:isDark];
    ...
    [_indicatorsHelper setIndicator:kiTermIndicatorZoomedIn
                     ...
                     darkBackground:isDark];
    [_indicatorsHelper setIndicator:kiTermIndicatorCopyMode visible:[_delegate textViewCopyMode] darkBackground:isDark];
    [_indicatorsHelper setIndicator:kiTermIndicatorDebugLogging visible:gDebugLogging darkBackground:isDark];
    [_indicatorsHelper setIndicator:kiTermIndicatorFilter visible:[_delegate textViewIsFiltered] darkBackground:isDark];
...
- (NSRect)offscreenCommandLineFrameForView:(NSView *)view {
    NSRect base = [iTermTextDrawingHelper
        offscreenCommandLineFrameForVisibleRect:self.enclosingScrollView.documentVisibleRect
                                       cellSize:NSMakeSize(self.charWidth, self.lineHeight)
                                       gridSize:VT100GridSizeMake(self.dataSource.width, self.dataSource.height)];
...
- (CGFloat)desiredHeight {
    ...
    return ([_dataSource numberOfLines] * _lineHeight + [self excess] + _drawingHelper.numberOfIMELines * _lineHeight);
}
...
- (NSPoint)locationInTextViewFromEvent:(NSEvent *)event {
    ...
    locationInTextView.y = MIN(self.frame.size.height - 1, MAX(0, locationInTextView.y));
...
- (NSRect)gridRect {
    ...
    return NSMakeRect(visibleRect.origin.x, lineStart * _lineHeight, visibleRect.origin.x + visibleRect.size.width,
...
- (BOOL)mouseIsOverScroller {
    ...
    const CGFloat hotWidth =
        22; // A strip of this width on the right of the scrollview will be considered over the scroller.
    const NSRect scrollerRectInScrollview = NSMakeRect(NSWidth(self.enclosingScrollView.bounds) - hotWidth, 0, hotWidth,
...
- (BOOL)isCursorBlinking {
    if (_blinkingCursor && [self isInKeyWindow] && [_delegate textViewIsActiveSession]) {
        return YES;
    } else {
        return NO;
    }
...
- (BOOL)updateDirtyRects:(BOOL *)foundDirtyPtr haveScrolled:(BOOL)haveScrolled {
    ...
    VT100GridCoord cursorPosition = VT100GridCoordMake([_dataSource cursorX] - 1, [_dataSource cursorY] - 1);
    ...
    if (_previousCursorCoord.x != cursorPosition.x ||
        ...
        DLog(@"Redraw previous cursor line (%d) and current cursor line (%d)",
             (int)(_previousCursorCoord.y - totalScrollbackOverflow), cursorPosition.y);
        ...
        _previousCursorCoord = VT100GridAbsCoordMake(cursorPosition.x, cursorPosition.y + totalScrollbackOverflow);
    }
    ...
    if (allDirty) {
        foundDirty = YES;
        DLog(@"allDirty=YES");
        const NSRange range = NSMakeRange(lineStart + totalScrollbackOverflow, lineEnd - lineStart);
        ...
        for (int y = lineStart; y < lineEnd; y++) {
            ...
            if (range.length > 0) {
                ...
                foundDirty = YES;
                // TODO: It would be more correct to remove all search results from this point down and reset the cursor
                // location to the start of the first dirty line. VT100Screen.savedFindContextAbsPos can be after some
                // of the dirty cells causing them not to be searched.
...
- (VT100MarginColor)desiredMarginColor {
    ...
    if ([self findDominantBackgroundColorInCharacters:values count:numValues threshold:0.8 dominantColor:&dominant]) {
        ...
        return (VT100MarginColor){.enabled = YES,
    ...
    return (VT100MarginColor){.enabled = NO};
}
...
- (void)searchForVisibleBlocks {
    ...
    const NSRange visibleLines = [self visibleRelativeRange];
    for (NSInteger i = 0; i < visibleLines.length; i++) {
        NSDictionary<NSNumber *, iTermExternalAttribute *> *attrs =
            [[self.dataSource externalAttributeIndexForLine:visibleLines.location + i] attributes];
...
- (void)handleScrollbackOverflow:(int)scrollbackOverflow userScroll:(BOOL)userScroll {
    // GitLab #12322: Save selection state BEFORE updating for overflow.
    // We need to compare pre- and post-overflow selection to detect if selection was
    // cleared or changed, which requires a redraw even when canSkipRedraw would be YES.
    const BOOL hadSelectionBefore = _selection.hasSelection;

    // Keep correct selection highlighted
    [_selection scrollbackOverflowDidChange];
    [_oldSelection scrollbackOverflowDidChange];

    // GitLab #12322: If selection was cleared due to scrollback overflow, force redraw
    // to remove stale selection highlights.
    const BOOL selectionCleared = hadSelectionBefore && !_selection.hasSelection;
    ...
    if (userScroll) {
        ...
        [self scrollRectToVisible:scrollRect];
        DLog(@"handleScrollbackOverflow:%@ visibleRect %@ -> %@", @(scrollbackOverflow), NSStringFromRect(previous),
    ...
    // slowly anyway.
    // GitLab #12322: Also force redraw if selection was cleared to remove stale highlights.
    if (!canSkipRedraw || selectionCleared) {
        [self requestDelegateRedraw];
    }
...
- (void)refreshAccessibility {
    // Invalidate the accessibility cache since content has changed.
    ...
    if ([_dataSource cursorX] != _lastAccessibilityCursorX || absCursorY != _lastAccessibiltyAbsoluteCursorY) {
        AccLog(@"Post notification: selected text changed (cursor is now at (%@,%@))", @(_dataSource.cursorX),
               @(_dataSource.cursorY));
        ...
        if (UAZoomEnabled()) {
            CGRect selectionRect = NSRectToCGRect([self.window convertRectToScreen:[self convertRect:[self cursorFrame]
                                                                                              toView:nil]]);
...
- (BOOL)refreshAfterSync:(VT100SyncResult)syncResult {
    ...
    // BUG-f1302: Replace assert with guard - delegate can become nil during frame resize
    // This can happen if the session is closed while resizing, so return early
    if (_delegate == nil) {
        DLog(@"BUG-f1302: Delegate became nil after textViewResizeFrameIfNeeded, returning");
        return YES;
    }
    ...
    [self.dataSource performBlockWithSavedGrid:^(id<PTYTextViewSynchronousUpdateStateReading> _Nullable state) {
        BOOL foundDirty = NO;
        foundBlink =
            [self updateDirtyRects:&foundDirty haveScrolled:syncResult.haveScrolled] || [self isCursorBlinking];
...
- (NSString *)foldableBlockIDOnLine:(int)line {
    NSDictionary<NSNumber *, iTermExternalAttribute *> *attrs =
        [[self.dataSource externalAttributeIndexForLine:line] attributes];
    NSString *block =
        [[attrs[@0].blockIDList componentsSeparatedByString:iTermExternalAttributeBlockIDDelimiter] firstObject];
    return block;
}
...
- (NSString *)updateHoverButtonsForLine:(int)line changed:(out BOOL *)changedPtr {
    ...
    if (VT100GridCoordIsValid(coordRange.start) && VT100GridCoordIsValid(coordRange.end)) {
        const VT100GridAbsCoordRange absCoordRange =
            VT100GridAbsCoordRangeFromCoordRange(coordRange, _dataSource.totalScrollbackOverflow);
        absLineRange = NSMakeRange(absCoordRange.start.y, absCoordRange.end.y - absCoordRange.start.y + 1);
    } else {
        absLineRange = NSMakeRange(NSNotFound, 0);
    }
    // As a side effect update the current blockCopyButton.
    [self updateHoverButtonsForBlockID:block onLine:line absLineRange:absLineRange changed:changedPtr];
    return block;
}
    ...
    *changedPtr = NO;
    if (@available(macOS 11, *)) {
        if (!block) {
            ...
            [self makeOrUpdateBlockFoldButtonForLine:line block:block absLineRange:absLineRange changed:changedPtr];
        }
    }
}
...
- (BOOL)updateHoverBlockFoldButtonWithBlock:(NSString *)block
                               ...
                               absLineRange:(NSRange)absLineRange
                                    changed:(out BOOL *)changedPtr {
    if (![_hoverBlockCopyButton.blockID isEqualToString:block] || _hoverBlockFoldButton.folded != folded ||
...
- (BOOL)_markChangedSelectionAndBlinkDirty:(BOOL)redrawBlink width:(int)width {
    ...
    int lineStart = ([self visibleRect].origin.y + [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]) /
                    _lineHeight; // add VMARGIN because stuff under top margin isn't visible.
    ...
    for (int y = lineStart, i = 0; y < lineEnd; y++, i++) {
        ...
        const BOOL areEmpty = (areSelected.count == 0);
        const BOOL wereEmpty = (wereSelected.count == 0);
        BOOL selectionChanged = NO;
        if (areEmpty && wereEmpty) {
            selectionChanged = NO;
        } else if (!areEmpty && !wereEmpty) {
            selectionChanged = ![areSelected isEqualToIndexSet:wereSelected];
        } else {
...
- (VT100GridCoordRange)rangeByTrimmingNullsFromRange:(VT100GridCoordRange)range trimSpaces:(BOOL)trimSpaces {
    ...
    while (!VT100GridCoordEquals(result.start, range.end)) {
        ...
        BOOL trim = ((code == 0) || (trimSpaces && (code == ' ' || code == '\t' || code == TAB_FILLER)));
        if (trim) {
            result.start.x++;
...
- (IBAction)selectAll:(id)sender {
    ...
    if (_findOnPageHelper.absLineRange.length > 0 && _findOnPageHelper.absLineRange.location != NSNotFound) {
    ...
    [_selection beginSelectionAtAbsCoord:VT100GridAbsCoordMake(0, absRangeToSelect.location)
                                  ...
                                  resume:NO
                                  append:NO];
    [_selection moveSelectionEndpointTo:VT100GridAbsCoordMake([_dataSource width], NSMaxRange(absRangeToSelect) - 1)];
    [_selection endLiveSelection];
...
- (void)selectRange:(VT100GridAbsCoordRange)range {
    ...
    [_selection beginSelectionAtAbsCoord:range.start mode:kiTermSelectionModeCharacter resume:NO append:NO];
...
- (iTermLogicalMovementHelper *)logicalMovementHelperForCursorCoordinate:(VT100GridCoord)relativeCursorCoord {
    const long long overflow = _dataSource.totalScrollbackOverflow;
    VT100GridAbsCoord cursorCoord = VT100GridAbsCoordMake(relativeCursorCoord.x, relativeCursorCoord.y + overflow);
...
- (void)moveSelectionEndpoint:(PTYTextViewSelectionEndpoint)endpoint
    ...
    return [self moveSelectionEndpoint:endpoint inDirection:direction by:unit cursorCoord:[self cursorCoord]];
}
...
- (void)moveSelectionEndpoint:(PTYTextViewSelectionEndpoint)endpoint
    ...
    VT100GridAbsCoordRange newRange = [helper moveSelectionEndpoint:endpoint inDirection:direction by:unit];

    [self withRelativeCoordRange:newRange
                           block:^(VT100GridCoordRange range) {
...
- (BOOL)moveSelectionEndpointToX:(int)x Y:(int)y locationInTextView:(NSPoint)locationInTextView {
    ...
    DLog(@"Move selection endpoint to %d,%d, coord=%@", x, y, [NSValue valueWithPoint:locationInTextView]);
    int width = [_dataSource width];
    if (locationInTextView.y == 0) {
        x = y = 0;
    } else if (locationInTextView.x < [iTermPreferences intForKey:kPreferenceKeySideMargins] &&
               _selection.liveRange.coordRange.start.y < y) {
    ...
    const BOOL hasColumnWindow =
        (_selection.liveRange.columnWindow.location > 0 || _selection.liveRange.columnWindow.length < width);
    if (hasColumnWindow && !VT100GridRangeContains(_selection.liveRange.columnWindow, x)) {
        DLog(@"Mouse has wandered outside columnn window %@",
             VT100GridRangeDescription(_selection.liveRange.columnWindow));
        [_selection clearColumnWindowForLiveSelection];
    }
    const BOOL result =
        [_selection moveSelectionEndpointTo:VT100GridAbsCoordMake(x, y + _dataSource.totalScrollbackOverflow)];
...
- (BOOL)canCopy {
    return [_selection hasSelection] || [self anyPortholeHasSelection] ||
           [self.delegate textViewSelectedCommandMark] != nil;
}
...
- (void)copySelectionWithStyles:(iTermSelection *)selection {
    ...
    NSAttributedString *copyAttributedString = [self selectedAttributedTextWithPad:NO selection:selection];
    if ([iTermAdvancedSettingsModel disallowCopyEmptyString] && copyAttributedString.length == 0) {
        DLog(@"Disallow copying empty string");
        return;
    }
...
- (BOOL)pasteValuesOnPasteboard:(NSPasteboard *)pasteboard cdToDirectory:(BOOL)cdToDirectory {
    ...
    if ([types containsObject:NSPasteboardTypeFileURL]) {
        ...
        if (filenames.count) {
            ...
            NSMutableString *stringToPaste = [[NSMutableString alloc] initWithCapacity:256];
            if (cdToDirectory) {
                ...
                if (filenames.count > 1) {
                    ...
                    filenames = [filenames mapWithBlock:^NSString *(NSString *filename) {
                        if ([[NSFileManager defaultManager]
                                itemIsDirectory:[filename stringByResolvingSymlinksInPath]]) {
                            return filename;
                        }
...
- (NSDictionary * (^)(screen_char_t, iTermExternalAttribute *))attributeProviderUsingProcessedColors:(BOOL)processed
                                                                         elideDefaultBackgroundColor:
                                                                             (BOOL)elideDefaultBackgroundColor {
...
- (id)contentWithAttributes:(BOOL)attributes timestamps:(BOOL)timestamps {
    ...
    VT100GridCoordRange theRange = VT100GridCoordRangeMake(0, 0, [_dataSource width], [_dataSource numberOfLines] - 1);
...
- (void)saveDocumentAs:(id)sender {
    ...
    timestampsButton.state = [[NSUserDefaults standardUserDefaults] boolForKey:userDefaultsKey]
    ...
    // BUG-2730: Use POSIX locale for fixed date formats per Apple TN1480
    NSLocale *posixLocale = [NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];
    dateFormatter.locale = posixLocale;
    dateFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"yyyy-MM-dd hh-mm-ss"
                                                               options:0
                                                                locale:posixLocale];
    ...
    [aSavePanel beginWithFallbackWindow:self.window
                                handler:^(NSModalResponse result, iTermSavePanelItem *item) {
                                    ...
                                    [[NSUserDefaults standardUserDefaults] setBool:wantTimestamps
                                                                            forKey:userDefaultsKey];
                                    [[self dataToSaveWithTimestamps:wantTimestamps]
                                          writeToSaveItem:item
...
- (NSData *)dataToSaveWithTimestamps:(BOOL)timestamps {
    if (!timestamps) {
        ...
        return [string dataUsingEncoding:[_delegate textViewEncoding] allowLossyConversion:YES];
    }
...
- (IBAction)toggleAILock:(id)sender {
    [_delegate textViewToggleAILock];
}

#pragma mark - Marks

- (void)beginFlash:(NSString *)flashIdentifier {
    if ([flashIdentifier isEqualToString:kiTermIndicatorBell] && [iTermAdvancedSettingsModel traditionalVisualBell]) {
...
- (void)highlightMarkOnLine:(int)line hasErrorCode:(BOOL)hasErrorCode {
    ...
    NSView *highlightingView = [[[iTermHighlightRowView alloc]
        initWithFrame:NSMakeRect(0, y, self.frame.size.width, _lineHeight)] autorelease];
    ...
    highlightingView.layer.backgroundColor =
        hasErrorCode ? [[NSColor redColor] CGColor] : [[NSColor blueColor] CGColor];
    ...
    if (!_highlightedRows) {
        _highlightedRows = [[NSMutableArray alloc] initWithCapacity:4];
    }

    iTermHighlightedRow *entry =
        [[iTermHighlightedRow alloc] initWithAbsoluteLineNumber:_dataSource.totalScrollbackOverflow + line
...
- (void)addNote {
    ...
    [self withRelativeCoordRange:_selection.lastAbsRange.coordRange
                           block:^(VT100GridCoordRange range) {
...
- (void)updateNoteViewFrames {
    for (PTYNoteViewController *note in _notes) {
        ...
        if (coordRange.end.y >= 0) {
            [note setAnchor:NSMakePoint(coordRange.end.x * _charWidth +
...
- (VT100GridCoord)moveCursorHorizontallyTo:(VT100GridCoord)target from:(VT100GridCoord)cursor {
    DLog(@"Moving cursor horizontally from %@ to %@", VT100GridCoordDescription(cursor),
    ...
    // is a DWC_RIGHT.
    NSMutableString *stringToSend = [[NSMutableString alloc] initWithCapacity:128];
...
- (void)placeCursorOnCurrentLineWithEvent:(NSEvent *)event verticalOk:(BOOL)verticalOk {
    ...
    VT100GridCoord cursor = VT100GridCoordMake([_dataSource cursorX] - 1, [_dataSource absoluteLineNumberOfCursor] -
                                                                              [_dataSource totalScrollbackOverflow]);
    if (!verticalOk) {
    ...
    if (cursor.x > target.x) {
        ...
        cursor = [self moveCursorHorizontallyTo:VT100GridCoordMake(target.x, cursor.y) from:cursor];
    }
    ...
    while (cursor.y != target.y) {
        if (cursor.y > target.y) {
            [_delegate writeStringWithLatin1Encoding:[[terminalOutput keyArrowUp:0]
                                                         stringWithEncoding:NSISOLatin1StringEncoding]];
            cursor.y--;
        } else {
            [_delegate writeStringWithLatin1Encoding:[[terminalOutput keyArrowDown:0]
                                                         stringWithEncoding:NSISOLatin1StringEncoding]];
            cursor.y++;
        }
    }
...
- (VT100GridCoord)cursorCoord {
    return VT100GridCoordMake(_dataSource.cursorX - 1, _dataSource.numberOfScrollbackLines + _dataSource.cursorY - 1);
}
...
- (BOOL)showCommandInfoForEvent:(NSEvent *)event {
    if (event.buttonNumber == 1) {
        ...
        if (offscreenCommandLine) {
            [self presentCommandInfoForOffscreenCommandLine:offscreenCommandLine
                                                      event:event
                                   fromOffscreenCommandLine:YES];
            return YES;
        }
    }
...
- (BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if (result != YES && [self dragOperationForSender:sender] != NSDragOperationNone) {
        result = YES;
    }
...
- (NSDragOperation)dragOperationForSender:(id<NSDraggingInfo>)sender numberOfValidItems:(int *)numberOfValidItemsPtr {
    ...
    BOOL pasteOK =
        !![[sender draggingPasteboard] availableTypeFromArray:@[ NSPasteboardTypeFileURL, NSPasteboardTypeString ]];
    ...
    if (!optionPressed && pasteOK &&
        (sourceMask & (NSDragOperationGeneric | NSDragOperationCopy | NSDragOperationLink)) != 0) {
...
- (void)_dragImage:(id<iTermImageInfoReading>)imageInfo forEvent:(NSEvent *)theEvent {
    NSImage *icon = [imageInfo imageWithCellSize:NSMakeSize(_charWidth, _lineHeight) scale:1];
    ...
    VT100GridCoord coord = VT100GridCoordMake(
        (dragPoint.x - [iTermPreferences intForKey:kPreferenceKeySideMargins]) / _charWidth, dragPoint.y / _lineHeight);
    const screen_char_t *theLine = [_dataSource screenCharArrayForLine:coord.y].line;
    if (theLine && coord.x < [_dataSource width] && theLine[coord.x].image && !theLine[coord.x].virtualPlaceholder &&
        ...
        VT100GridCoord imageCellOrigin = VT100GridCoordMake(coord.x - pos.x, coord.y - pos.y);

        // Compute the pixel coordinate of the image's top left point
        NSPoint imageTopLeftPoint =
            NSMakePoint(imageCellOrigin.x * _charWidth + [iTermPreferences intForKey:kPreferenceKeySideMargins],
                        imageCellOrigin.y * _lineHeight);

        // Compute the distance from the click location to the image's origin
        NSPoint offset = NSMakePoint(dragPoint.x - imageTopLeftPoint.x, dragPoint.y - imageTopLeftPoint.y);
    ...
    [dragItem setDraggingFrame:NSMakeRect(dragPoint.x, dragPoint.y, icon.size.width, icon.size.height) contents:icon];
...
- (void)_dragText:(NSString *)aString forEvent:(NSEvent *)theEvent {
    ...
    NSDraggingItem *dragItem = [[[NSDraggingItem alloc] initWithPasteboardWriter:pbItem] autorelease];
    [dragItem setDraggingFrame:NSMakeRect(dragPoint.x, dragPoint.y, anImage.size.width, anImage.size.height)
                      contents:anImage];
    NSDraggingSession *draggingSession = [self beginDraggingSessionWithItems:@[ dragItem ] event:theEvent source:self];
...
- (BOOL)confirmUploadOfFiles:(NSArray *)files toPath:(SCPPath *)path {
    ...
    if (files.count == 1) {
        text =
            [NSString stringWithFormat:@"OK to %@\n%@\nto\n%@@%@:%@?", useSSHIntegration ? @"copy" : @"scp",
                                       [files componentsJoinedByString:@", "], path.username, path.hostname, path.path];
    } else {
        text = [NSString stringWithFormat:@"OK to %@ the following files:\n%@\n\nto\n%@@%@:%@?",
                                          useSSHIntegration ? @"copy" : @"scp", [files componentsJoinedByString:@", "],
...
- (BOOL)uploadFilenamesOnPasteboard:(NSPasteboard *)pasteboard location:(NSPoint)windowDropPoint {
    ...
    if ([types containsObject:NSPasteboardTypeFileURL] && filenames.count && dropScpPath) {
        ...
        [self performSelector:@selector(maybeUpload:) withObject:@[ filenames, dropScpPath ] afterDelay:0];
        return YES;
    }
    return NO;
}
    ...
    switch (type) {
        case 0: // visible range
            ...
            VT100GridCoordRange coordRange =
                VT100GridCoordRangeMake(0, lineOffset, [_dataSource width], lineOffset + numLines - 1);
...
- (void)printContent:(id)content {
    ...
    NSTextView *tempView = [[[NSTextView alloc] initWithFrame:frame] autorelease];
    ...
    if ([content isKindOfClass:[NSAttributedString class]]) {
        ...
        attributedString = [[[NSAttributedString alloc] initWithString:content attributes:attributes] autorelease];
    }
    ...
    if (accessory) {
        operation.printPanel.options =
            (NSPrintPanelShowsCopies | NSPrintPanelShowsPaperSize | NSPrintPanelShowsOrientation |
             NSPrintPanelShowsScaling | NSPrintPanelShowsPreview);
...
- (void)insertText:(id)aString {
    // Note: Replaces entire marked text range. This is correct for terminal IME where
    // inputMethodMarkedRange always starts at 0 (set in setMarkedText:selectedRange:replacementRange:).
    [self insertText:aString replacementRange:NSMakeRange(0, [_drawingHelper.markedText length])];
}

// Note: The replacementRange parameter is intentionally not used for terminal emulation.
// Standard IME behavior replaces the entire marked text, which is what we do.
- (void)setMarkedText:(id)aString selectedRange:(NSRange)selRange replacementRange:(NSRange)replacementRange {
    DLog(@"setMarkedText%@ selectedRange%@ replacementRange:%@", aString, NSStringFromRange(selRange),
         NSStringFromRange(replacementRange));
    if ([aString isKindOfClass:[NSAttributedString class]]) {
        _drawingHelper.markedText =
            [[[NSAttributedString alloc] initWithString:[aString string]
                                             attributes:[self markedTextAttributes]] autorelease];
    } else {
        _drawingHelper.markedText =
            [[[NSAttributedString alloc] initWithString:aString attributes:[self markedTextAttributes]] autorelease];
    }
    ...
    do {
        dirtStart = ([_dataSource cursorY] - 1 - _drawingHelper.numberOfIMELines) * [_dataSource width] +
                    [_dataSource cursorX] - 1;
...
- (NSRange)nsrangeOfCursor {
    ...
    VT100GridAbsCoordRange range = VT100GridAbsCoordRangeMake(x, offset + y, x, offset + y);
...
- (NSRange)nsRangeForAbsCoordRange:(VT100GridAbsCoordRange)range {
    ...
    return NSMakeRange(range.start.x + range.start.y * width, VT100GridAbsCoordDistance(range.start, range.end, width));
}

- (NSArray *)validAttributesForMarkedText {
    return @[
        NSForegroundColorAttributeName, NSBackgroundColorAttributeName, NSUnderlineStyleAttributeName,
        NSFontAttributeName
    ];
}
...
- (NSAttributedString *)attributedSubstringForProposedRange:(NSRange)proposedRange
    ...
    NSRange aRange = NSIntersectionRange(proposedRange, NSMakeRange(0, _drawingHelper.markedText.length));
...
- (NSRect)firstRectForCharacterRange:(NSRange)theRange actualRange:(NSRangePointer)actualRange {
    ...
    NSRect rect = NSMakeRect(x * _charWidth + [iTermPreferences intForKey:kPreferenceKeySideMargins],
                             ...
                             _charWidth * theRange.length, _lineHeight);
...
- (void)findOnPageHelperSearchExternallyFor:(NSString *)query mode:(iTermFindMode)mode {
    [_findOnPageHelper addExternalResults:[self searchPortholesFor:query mode:mode] width:self.dataSource.width ?: 1];
}
...
- (void)selectCoordRange:(VT100GridCoordRange)range {
    ...
    iTermSubSelection *sub = [iTermSubSelection subSelectionWithAbsRange:VT100GridAbsWindowedRangeMake(absRange, 0, 0)
                                                                    mode:kiTermSelectionModeCharacter
                                                                   width:_dataSource.width];
...
- (void)selectAbsWindowedCoordRange:(VT100GridAbsWindowedRange)windowedRange {
    ...
    iTermSubSelection *sub = [iTermSubSelection subSelectionWithAbsRange:windowedRange
                                                                   ...
                                                                   width:_dataSource.width];
    [_selection addSubSelection:sub];
    [_delegate textViewDidSelectRangeForFindOnPage:VT100GridWindowedRangeFromVT100GridAbsWindowedRange(
                                                       windowedRange, self.dataSource.totalScrollbackOverflow)
                                                       .coordRange];
}
...
- (NSRect)frameForCoord:(VT100GridCoord)coord {
    return NSMakeRect(MAX(0, floor(coord.x * _charWidth + [iTermPreferences intForKey:kPreferenceKeySideMargins])),
                      MAX(0, coord.y * _lineHeight), _charWidth, _lineHeight);
}
...
- (void)findOnPageRevealRange:(VT100GridCoordRange)range {
    ...
    const VT100GridCoordRange visibleRange =
        VT100GridCoordRangeMake(0, visibleLines.location, _dataSource.width, VT100GridRangeMax(visibleLines));
    if (VT100GridCoordRangeContainsCoord(visibleRange, range.start) &&
        VT100GridCoordRangeContainsCoord(visibleRange, range.end)) {
        DLog(@"Entire range %@ is visible within %@ so don't scroll", VT100GridCoordRangeDescription(range),
...
- (id)validRequestorForSendType:(NSString *)sendType returnType:(NSString *)returnType {
    NSSet *acceptedReturnTypes = [NSSet setWithArray:@[ UTTypeUTF8PlainText.identifier, NSPasteboardTypeString ]];
...
- (BOOL)writeSelectionToPasteboard:(NSPasteboard *)pboard types:(NSArray *)types {
    ...
    NSString *copyString = [self selectedTextCappedAtSize:[iTermAdvancedSettingsModel maximumBytesToProvideToServices]];
...
- (NSRect)rectForCoord:(VT100GridCoord)coord {
    return NSMakeRect([iTermPreferences intForKey:kPreferenceKeySideMargins] + coord.x * _charWidth,
                      coord.y * _lineHeight, _charWidth, _lineHeight);
}
...
- (NSRect)cursorFrame {
    return [self rectForGridCoord:VT100GridCoordMake(_dataSource.cursorX - 1, _dataSource.cursorY - 1)];
}
...
- (void)createFindCursorWindowWithFireworks:(BOOL)forceFireworks {
    ...
    if (forceFireworks) {
        self.findCursorView = [iTermFindCursorView
            newFireworksViewWithFrame:NSMakeRect(0, 0, screenFrame.size.width, screenFrame.size.height)];
    } else {
        self.findCursorView = [[iTermFindCursorView alloc]
...
- (void)selectionDidChange:(iTermSelection *)selection {
    ...
    if (!_selection.live && selection.hasSelection) {
        ...
        [promise onQueue:dispatch_get_main_queue()
                    then:^(NSString *_Nonnull value) {
    ...
    [self requestDelegateRedraw];
    DLog(@"Selection did change: selection=%@. stack=%@", selection, [NSThread callStackSymbols]);
}
...
- (VT100GridRange)selectionRangeOfTerminalNullsOnAbsoluteLine:(long long)absLineNumber {
    ...
    if ([_dataSource bidiInfoForLine:lineNumber] != nil) {
        // TODO: This is a hack. A proper fix would give iTermSelection the ability to extend selection leftwards for
        // lines that are right-justified.
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForParentheticalAt:(VT100GridAbsCoord)absCoord {
    ...
    if (!ok) {
        ...
        return VT100GridAbsWindowedRangeMake(
            VT100GridAbsCoordRangeMake(0, totalScrollbackOverflow, 0, totalScrollbackOverflow), 0, 0);
    }
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:_dataSource];
    [extractor restrictToLogicalWindowIncludingCoord:coord];
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForWordAt:(VT100GridAbsCoord)absCoord {
    ...
    [self getWordForX:coord.x y:coord.y range:&range respectDividers:[self liveSelectionRespectsSoftBoundaries]];
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForWrappedLineAt:(VT100GridAbsCoord)absCoord {
    ...
    const BOOL ok = [self withRelativeCoord:absCoord
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForLineAt:(VT100GridAbsCoord)absCoord {
    ...
    const long long totalScrollbackOverflow = _dataSource.totalScrollbackOverflow;
    const BOOL ok = [self
        withRelativeCoord:absCoord
                    block:^(VT100GridCoord coord) {
                        if (![self liveSelectionRespectsSoftBoundaries]) {
                            result = VT100GridAbsWindowedRangeMake(
                                VT100GridAbsCoordRangeMake(0, absCoord.y, [_dataSource width], absCoord.y), 0, 0);
                            return;
                        }
                        iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:_dataSource];
                        [extractor restrictToLogicalWindowIncludingCoord:coord];
                        result = VT100GridAbsWindowedRangeMake(
                            VT100GridAbsCoordRangeMake(extractor.logicalWindow.location, absCoord.y,
                                                       VT100GridRangeMax(extractor.logicalWindow) + 1, absCoord.y),
                            extractor.logicalWindow.location, extractor.logicalWindow.length);
                    }];
    if (!ok) {
        return VT100GridAbsWindowedRangeMake(
            VT100GridAbsCoordRangeMake(0, totalScrollbackOverflow, 0, totalScrollbackOverflow), 0, 0);
...
- (VT100GridAbsCoord)selectionPredecessorOfAbsCoord:(VT100GridAbsCoord)absCoord {
    ...
    const long long totalScrollbackOverflow = _dataSource.totalScrollbackOverflow;
    const BOOL ok = [self withRelativeCoord:absCoord
                                      block:^(VT100GridCoord relativeCoord) {
...
- (NSIndexSet *)selectionIndexesOnAbsoluteLine:(long long)absLine
    ...
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:_dataSource];
    return [extractor indexesOnLine:absLine - totalScrollbackOverflow containingCharacter:c inRange:range];
}
...
- (void)immutableColorMap:(id<iTermColorMapReading>)colorMap
    ...
    [self updatePortholeColorsWithUseSelectedTextColor:[_delegate textViewShouldUseSelectedTextColor] deferUpdate:YES];
    [self requestDelegateRedraw];
}
...
- (NSArray<id<PTYAnnotationReading>> *)drawingHelperAnnotationsInRange:(VT100GridCoordRange)range {
    return [_dataSource annotationsInRange:range];
}

- (VT100GridCoordRange)drawingHelperCoordRangeForAnnotation:(id<PTYAnnotationReading>)annotation {
    return [_dataSource coordRangeOfAnnotation:annotation];
}
...
- (NSData *)drawingHelperMatchesOnLine:(int)line {
    const long long absoluteLine = line + _dataSource.totalScrollbackOverflow;
    return [_findOnPageHelper highlightDataForAbsoluteLine:absoluteLine];
}

- (BOOL)hasFindOnPageMatches {
    return [_findOnPageHelper hasHighlightedMatches];
}
...
- (VT100GridAbsCoord)absCoordForButton:(iTermTerminalButton *)button API_AVAILABLE(macos(11)) {
    ...
    if (markButton.shouldFloat) {
        ...
        const NSRange markRange =
            NSMakeRange(markAbsCoordRange.start.y, markAbsCoordRange.end.y - markAbsCoordRange.start.y + 1);
        ...
        return VT100GridAbsCoordMake(self.dataSource.width + markButton.dx, intersectionRange.location);
    }
...
- (NSArray<iTermTerminalButton *> *)terminalButtons NS_AVAILABLE_MAC(11) {
    // Pre-allocate for typical button count (hover buttons + command buttons)
    NSMutableArray<iTermTerminalButton *> *updated = [NSMutableArray arrayWithCapacity:8];
    if (_hoverBlockFoldButton) {
...

    {
        ...
        if (!mark.lineStyle && mark.command.length > 0 && [iTermAdvancedSettingsModel showButtonsForSelectedCommand]) {
            const NSRange intersectionRange =
                NSIntersectionRange(self.findOnPageHelper.absLineRange,
            ...
            if (intersectionRange.length > 0) {
                [updated addObjectsFromArray:[self commandButtonsForMark:mark
                                                                    line:intersectionRange.location -
                                                                         _dataSource.totalScrollbackOverflow + 1
                                                             shouldFloat:YES
                                                                offByOne:NO]];
            } else {
    ...
    __weak __typeof(self) weakSelf = self;
    [places
        enumerateObjectsUsingBlock:^(iTermTerminalButtonPlace *_Nonnull place, NSUInteger idx, BOOL *_Nonnull stop) {
            NSInteger i = [_buttons
                indexOfObjectPassingTest:^BOOL(iTermTerminalButton *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            if (i == NSNotFound || !VT100GridAbsCoordEquals([self absCoordForButton:_buttons[i]], place.coord)) {
                if (place.mark.copyBlockID) {
                    iTermTerminalButton *button =
                        [[[iTermTerminalCopyButton alloc] initWithID:place.id
                    ...
                    iTermTerminalButton *button =
                        [[[iTermTerminalRevealChannelButton alloc] initWithPlace:place] autorelease];
...
- (NSArray<iTermTerminalButton *> *)commandButtonsForMark:(id<VT100ScreenMarkReading>)mark
    ...
    __weak __typeof(self) weakSelf = self;
    // Pre-allocate for typical command button count (settings, copy, bookmark, share, run)
    NSMutableArray<iTermTerminalButton *> *updated = [NSMutableArray arrayWithCapacity:5];
    ...
    // Fold / Unfold button.
    id<iTermFoldMarkReading> fold =
        [[self.dataSource foldMarksInRange:VT100GridRangeMake(markLine - (offByOne ? 0 : 1), 1)] firstObject];
    if (fold) {
...
- (void)popCommandSettingsButtonAt:(NSPoint)locationInWindow for:(id<VT100ScreenMarkReading>)mark {
    ...
    __weak __typeof(self) weakSelf = self;
    [menu addItemWithTitle:@"Disable Command Selection"
                    action:^{
    ...
    [menu addItemWithTitle:@"Help"
                    action:^{
                        ...
                        NSString *filePath =
                            [[NSBundle bundleForClass:[weakSelf class]] pathForResource:@"CommandSelectionHelp"
                                                                                 ofType:@"md"];
                        NSError *error = nil;
...
- (iTermSelection *)selectionForCommandAndOutputOfMark:(id<VT100ScreenMarkReading>)mark {
    ...
    [selection beginSelectionAtAbsCoord:absRange.start mode:kiTermSelectionModeLine resume:NO append:NO];
    [selection moveSelectionEndpointTo:absRange.end];
    [selection endLiveSelection];
    return selection;
}

- (iTermRenegablePromise<NSAttributedString *> *)promisedAttributedStringForCommandAndOutputOfMark:
...
- (NSURL *)commandURLForMark:(id<VT100ScreenMarkReading>)mark absLine:(long long)absLine {
    return [iTermCommandURLBuilder urlWithMark:mark absLine:absLine dataSource:self.dataSource];
}

- (void)popShareMenuAt:(NSPoint)locationInWindow absLine:(long long)absLine forMark:(id<VT100ScreenMarkReading>)mark {
    if (![[mark retain] autorelease]) {
        return;
    }

    iTermCommandShareMenuProvider *provider = [[[iTermCommandShareMenuProvider alloc]
                  initWithMark:mark
        ...
        defaultBackgroundColor:[self defaultBackgroundColor]
                    commandURL:[self commandURLForMark:mark absLine:absLine]] autorelease];
...
- (iTermTerminalMarkButton *)cachedTerminalButtonForMark:(id<VT100ScreenMarkReading>)mark
    ...
    return [iTermTerminalMarkButton
        castFrom:[_buttons objectPassingTest:^BOOL(iTermTerminalButton *genericButton, NSUInteger index, BOOL *stop) {
...
- (void)unfoldBlock:(NSString *)blockID {
    ...
    const long long offset = [self.dataSource totalScrollbackOverflow];
    [self unfoldAbsoluteLineRange:NSMakeRange(range.start.y + offset, range.end.y - range.start.y + 1)];
...
- (void)foldBlock:(NSString *)blockID {
    ...
    const long long offset = [self.dataSource totalScrollbackOverflow];
    [self foldRange:NSMakeRange(range.start.y + offset, range.end.y - range.start.y + 1)];
    [self didFoldOrUnfold];
}
...
- (NSAttributedString *)accessibilityAttributedStringForRange:(NSRange)range {
    ...
    AccLog(@"accessibilityAttributedStringForRange:%@ -> “%@”", iTermStringFromRange(range),
           string.string.it_sanitized);
    return string;
}
...
- (VT100GridCoord)accessibilityHelperCoordForPoint:(NSPoint)accessibilityScreenPosition {
    ...
    NSRect visibleRect = [[self enclosingScrollView] documentVisibleRect];
    int x = (locationInTextView.x - [iTermPreferences intForKey:kPreferenceKeySideMargins] - visibleRect.origin.x) /
            _charWidth;
...
- (NSRect)accessibilityHelperFrameForCoordRange:(VT100GridCoordRange)coordRange {
    ...
    NSRect result = NSMakeRect(
        MAX(0, floor(coordRange.start.x * _charWidth + [iTermPreferences intForKey:kPreferenceKeySideMargins])),
        MAX(0, coordRange.start.y * _lineHeight), MAX(0, (coordRange.end.x - coordRange.start.x) * _charWidth),
...
- (VT100GridCoordRange)accessibilityHelperSelectedRange {
    ...
    [self
        withRelativeCoordRange:sub.absRange.coordRange
                         block:^(VT100GridCoordRange initialCoordRange) {
                             coordRange = initialCoordRange;
                             coordRange.start.y = MAX(
                                 0, [self accessibilityHelperAccessibilityLineNumberForLineNumber:coordRange.start.y]);
                             coordRange.end.y = MAX(
                                 0, [self accessibilityHelperAccessibilityLineNumberForLineNumber:coordRange.end.y]);
                         }];
    return coordRange;
}
...
- (const screen_char_t *)accessibilityHelperLineAtIndex:(int)accessibilityIndex
    ...
    ScreenCharArray *sca = [_dataSource
        screenCharArrayForLine:[self accessibilityHelperLineNumberForAccessibilityLineNumber:accessibilityIndex]];
    if (continuation) {
...
- (void)popoverDidClose:(NSNotification *)notification {
    ...
    iTermWebViewWrapperViewController *viewController =
        (iTermWebViewWrapperViewController *)popover.contentViewController;
    [viewController terminateWebView];
}

#pragma mark - iTermKeyboardHandlerDelegate

- (BOOL)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler shouldHandleKeyDown:(NSEvent *)event {
...
- (void)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler interpretKeyEvents:(NSArray<NSEvent *> *)events {
    [self interpretKeyEvents:events];
}

- (void)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler sendEventToController:(NSEvent *)event {
...
- (void)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler insertText:(NSString *)aString {
    if ([self hasMarkedText]) {
    ...
    if (![self hasMarkedText] && keyboardhandler.performsTextReplacement) {
...
- (BOOL)shouldBeAlphaedOut {
    return ([self allAnnotationsAreHidden] && !self.hasPortholes && self.contentNavigationShortcuts.count == 0);
}
...
- (VT100GridCoordRange)rangeOfBlockIncludingLine:(long long)absLine blockID:(NSString *)block {
    int start;
    const long long offset = self.dataSource.totalScrollbackOverflow;
    if (absLine < offset) {
        start = 0;
    ...
    return VT100GridCoordRangeMake(0, start, self.dataSource.width, end);
}
...
- (VT100GridCoord)mouseHandler:(PTYMouseHandler *)handler
    ...
    if (event.type == NSEventTypeLeftMouseUp && event.clickCount == 1) {
        ...
        NSString *message = [_indicatorsHelper
            helpTextForIndicatorAt:enclosingViewPoint
        ...
        if (_drawingHelper.offscreenCommandLine) {
            ...
            NSRect rect = [iTermTextDrawingHelper
                offscreenCommandLineFrameForVisibleRect:[self adjustedDocumentVisibleRect]
                                               cellSize:NSMakeSize(_charWidth, _lineHeight)
                                               gridSize:VT100GridSizeMake(_dataSource.width, _dataSource.height)];
            ...
            if (NSPointInRect(viewPoint, rect)) {
                ...
                [self highlightMarkOnLine:_drawingHelper.offscreenCommandLine.absoluteLineNumber -
                                          _dataSource.totalScrollbackOverflow
                             hasErrorCode:NO];
        ...
        if (foldMark) {
            ...
            return VT100GridCoordMake(-1, -1);
        } else if (pointInSelf.x <
                       [iTermPreferences intForKey:kPreferenceKeySideMargins] + PTYTextViewMarginClickGraceWidth &&
                   pointInSelf.x >= 0 && blockID != nil) {
            ...
            const NSPoint temp = [self clickPoint:event allowRightMarginOverflow:allowRightMarginOverflow];
...
- (void)mouseHandler:(PTYMouseHandler *)sender handleCommandShiftClickAtCoord:(VT100GridCoord)coord {
    if ([iTermPreferences boolForKey:kPreferenceKeyCmdClickOpensURLs] && [self isInUnderlinedRangeAtCoord:coord]) {
        const long long offset = self.dataSource.totalScrollbackOverflow;
        ...
        if (VT100GridAbsCoordRangeLength(absRange.coordRange, self.dataSource.width) > 0) {
            ...
            VT100GridWindowedRange relativeRange =
                VT100GridWindowedRangeFromAbsWindowedRangeSafe(absRange, offset, &valid);
            if (valid) {
                ...
                if (string.stringIsUrlLike && url != nil &&
                    ...
                    [[NSWorkspace sharedWorkspace] it_openURL:url
                                                ...
                                                configuration:[NSWorkspaceOpenConfiguration configuration]
                                                        style:size.width > size.height ? iTermOpenStyleVerticalSplit
...
- (BOOL)mouseHandler:(PTYMouseHandler *)handler coordIsMutable:(VT100GridCoord)coord {
...
- (void)mouseHandlerDidSingleClick:(PTYMouseHandler *)handler event:(NSEvent *)event {
    const VT100GridCoord coord = [self coordForEvent:event];
    const VT100GridCoordRange coordRange = VT100GridCoordRangeMake(coord.x, coord.y, coord.x + 1, coord.y);
...
- (void)mouseHandlerOpenTargetWithEvent:(NSEvent *)event inBackground:(BOOL)inBackground style:(iTermOpenStyle)style {
    [_urlActionHelper openTargetWithEvent:event inBackground:inBackground style:style];
}
...
- (VT100GridCoord)mouseHandlerCoordForPointInView:(NSPoint)point {
    ...
    VT100GridCoord coord =
        VT100GridCoordMake((point.x - liveRect.origin.x) / _charWidth, (point.y - liveRect.origin.y) / _lineHeight);
...
- (NSPoint)mouseHandlerReportablePointForPointInView:(NSPoint)point {
    NSRect liveRect = [self liveRect];
    const NSPoint limit = NSMakePoint(_charWidth * self.dataSource.width - 1, _lineHeight * self.dataSource.height - 1);
...
- (void)mouseHandlerMoveCursorToCoord:(VT100GridCoord)coord forEvent:(NSEvent *)event {
    BOOL verticalOk;
...
- (void)mouseHandlerSetFindOnPageCursorCoord:(VT100GridCoord)clickPoint {
    VT100GridAbsCoord absCoord =
        VT100GridAbsCoordMake(clickPoint.x, [_dataSource totalScrollbackOverflow] + clickPoint.y);
...
- (BOOL)mouseHandler:(PTYMouseHandler *)handler getFindOnPageCursor:(VT100GridCoord *)coord {
    ...
    *coord = VT100GridCoordMake(findOnPageCursor.x, findOnPageCursor.y - [_dataSource totalScrollbackOverflow]);
    return YES;
}
...
- (NSPoint)mouseHandler:(PTYMouseHandler *)handler viewCoordForEvent:(NSEvent *)event clipped:(BOOL)clipped {
    if (!clipped) {
...
- (void)mouseHandler:(PTYMouseHandler *)handler sendString:(NSString *)stringToSend latin1:(BOOL)forceLatin1 {
    if (forceLatin1) {
        [_delegate writeStringWithLatin1Encoding:stringToSend];
    } else {
...
- (BOOL)mouseHandler:(PTYMouseHandler *)mouseHandler
    ...
    return [self moveSelectionEndpointToX:coord.x Y:coord.y locationInTextView:locationInTextView];
}
    ...
    if ([self mouseHandlerAlternateScrollModeIsEnabled:mouseHandler verticalOnly:&verticalOnly]) {
        *forceLatin1 = YES;
        ...
        if (vertical) {
            ...
            data = downOrLeft ? [_dataSource.terminalOutput keyArrowRight:flags]
                              : [_dataSource.terminalOutput keyArrowLeft:flags];
        }
...
- (BOOL)mouseHandlerAlternateScrollModeIsEnabled:(PTYMouseHandler *)handler verticalOnly:(out BOOL *)verticalOnly {
...
- (void)mouseHandler:(PTYMouseHandler *)mouseHandler dragText:(NSString *)text forEvent:(NSEvent *)event {
...
- (CGFloat)scrollDeltaYAdjustedForMouseReporting:(CGFloat)deltaY {
    return [self scrollDeltaWithUnit:self.enclosingScrollView.verticalLineScroll delta:deltaY];
}
...
- (CGSize)mouseHandlerAccumulatedDelta:(PTYMouseHandler *)sender forEvent:(NSEvent *)event {
    ...
    if (event.it_isVerticalScroll) {
        delta.height = [_scrollAccumulator deltaForEvent:event increment:self.enclosingScrollView.verticalLineScroll];
    } else {
        delta.width = [_horizontalScrollAccumulator deltaForEvent:event increment:self.charWidth];
    }
...
- (BOOL)mouseHandlerMouseDownAt:(NSPoint)locationInWindow {
    ...
    if (@available(macOS 11, *)) {
        for (iTermTerminalButton *button in self.terminalButtons) {
            ...
            DLog(@"TextView handling mouseDown: not in %@ with frame %@", button.description,
...
- (void)smartSelectAndMaybeCopyWithEvent:(NSEvent *)event ignoringNewlines:(BOOL)ignoringNewlines {
    [_selection endLiveSelection];
    [_urlActionHelper smartSelectAndMaybeCopyWithEvent:event ignoringNewlines:ignoringNewlines];
}
...
- (void)openContextMenuWithEvent:(NSEvent *)event {
    ...
    NSPoint clickPoint = [self clickPoint:event allowRightMarginOverflow:NO];
    [_contextMenuHelper openContextMenuAt:VT100GridCoordMake(clickPoint.x, clickPoint.y) event:event];
}
...
- (void)selectMenuItemWithIdentifier:(NSString *)identifier title:(NSString *)title event:(NSEvent *)event {
...
- (void)advancedPasteWithConfiguration:(NSString *)configuration
    ...
    [self.delegate textViewPasteSpecialWithStringConfiguration:configuration fromSelection:fromSelection];
}
