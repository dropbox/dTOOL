    ...
    func body() throws -> Data {
        ...
        case .deepSeek:
            try DeepSeekRequestBuilder(messages: messages,
                                       ...
                                       functions: functions,
                                       stream: stream).body()
        // BUG-f509: Handle unknown LLM provider types by throwing an error
        @unknown default:
            throw NSError(domain: "com.dashterm2.llm",
                         code: -1,
                         userInfo: [NSLocalizedDescriptionKey: "Unknown LLM API type"])
        }
    }
