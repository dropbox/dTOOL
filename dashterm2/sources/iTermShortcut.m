...
// Only append to this string! Its indexes count. Never make it longer than 10 characters. backslash -> \0, comma -> \1,
// space -> \2.
...
const NSEventModifierFlags kHotKeyModifierMask =
    (NSEventModifierFlagCommand | NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl |
     iTermLeaderModifierFlag | NSEventModifierFlagNumericPad);
...
+ (NSString *)shortStringForDictionary:(NSDictionary *)dict {
    return [NSString stringWithFormat:@"%@,%@,%@,%@", dict[kKeyCode], dict[kModifiers],
                                      [self escapedString:dict[kCharacters]],
                                      [self escapedString:dict[kCharactersIgnoringModifiers]]];
}

+ (NSDictionary *)dictionaryForShortString:(NSString *)string {
    // Each part is separated by comma; estimate average part length
    NSMutableString *temp = [NSMutableString stringWithCapacity:string.length / 4];
    NSMutableArray *parts = [[NSMutableArray alloc] initWithCapacity:4];
    BOOL esc = NO;
    for (int i = 0; i < string.length; i++) {
        ...
        if (esc) {
            ...
            temp = [NSMutableString stringWithCapacity:string.length / 4];
        } else {
    ...
    return @{
        kKeyCode : @([parts[0] iterm_unsignedIntegerValue]),
...
+ (NSArray<iTermShortcut *> *)shortcutsForProfile:(Profile *)profile {
    ...
    main.charactersIgnoringModifiers = [iTermProfilePreferences stringForKey:KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS
                                                                   inProfile:profile];

    NSArray<NSDictionary *> *additional = (NSArray *)[profile objectForKey:KEY_HOTKEY_ALTERNATE_SHORTCUTS];
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:1 + additional.count];
...
- (NSString *)description {
    return [NSString
        stringWithFormat:
            @"<%@: %p keyCode=%@ modifiers=%@ (%@) characters=“%@” (0x%@) charactersIgnoringModifiers=“%@” (0x%@)>",
...
- (NSUInteger)hash {
    NSArray *components =
        @[ @(self.keyCode), @(self.modifiers), self.characters ?: @"", self.charactersIgnoringModifiers ?: @"" ];
...
- (NSString *)stringValue {
    ...
    // will be empty.
    const BOOL valid = (self.charactersIgnoringModifiers.length > 0 || self.characters.length > 0 || self.keyCode != 0);
...
- (iTermHotKeyDescriptor *)descriptor {
    return _charactersIgnoringModifiers.length > 0
               ? [NSDictionary descriptorWithKeyCode:self.keyCode modifiers:self.modifiers]
               : nil;
}
...
- (NSEventModifierFlags)modifiers {
    // On some keyboards, arrow keys have NSEventModifierFlagNumericPad bit set; manually set it for keyboards that
    // don't.
...
- (BOOL)smellsAccidental {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // All printable keys: 10 digits + 26 letters + ~20 punctuation = ~56 keys
        NSMutableSet *temp = [NSMutableSet setWithCapacity:64];
        ...
        [temp addObjectsFromArray:@[
            ...
            @(kVK_ANSI_Backslash), @(kVK_ANSI_Semicolon), @(kVK_ANSI_Quote), @(kVK_ANSI_Comma), @(kVK_ANSI_Period),
            @(kVK_ANSI_Slash)
        ]];
        // Editing keys (not navigation)
        [temp addObjectsFromArray:@[
            @(kVK_Space), @(kVK_Delete), @(kVK_ForwardDelete), @(kVK_Tab), @(kVK_Return), @(kVK_Escape)
        ]];
        // ISO/JIS keys
        [temp addObjectsFromArray:@[
            @(kVK_ISO_Section), @(kVK_JIS_Yen), @(kVK_JIS_Underscore), @(kVK_JIS_KeypadComma), @(kVK_JIS_Eisu),
            @(kVK_JIS_Kana)
        ]];
