...
DEFINE_BOOL(alwaysShowMenuBarIcon, YES, SECTION_GENERAL @"Always show the DashTerm2 icon in the menu bar?\nWhen enabled, the Dropbox-branded DashTerm2 icon is always shown in the menu bar, providing quick access to About, Settings, and other features. You must restart DashTerm2 after changing this setting.");
...
DEFINE_FLOAT(apiServerConnectionTimeoutSeconds, 30, SECTION_GENERAL @"Time in seconds before API HTTP requests time out while waiting for headers.\nIncrease this if custom tooling connects slowly over forwarded sockets.");
...
// DashTerm2: Increased frame rates for instant terminal I/O feel
DEFINE_FLOAT(metalSlowFrameRate, 60.0, SECTION_DRAWING @"When adaptive framerate is enabled and using the Metal renderer, refresh at this rate during high throughput conditions (FPS).");
DEFINE_FLOAT(activeUpdateCadence, 120.0, SECTION_DRAWING @"Maximum frame rate (FPS) when adaptive framerate is disabled.\nNote: this is doubled on ARM Macs for displays that support at least 120hz. Modifications to this setting will not affect existing sessions.");
DEFINE_INT(adaptiveFrameRateThroughputThreshold, 100000, SECTION_DRAWING @"Throughput threshold for adaptive frame rate.\nIf more than this many bytes per second are received, use the lower frame rate.");
DEFINE_FLOAT(maximumFrameRate, 120.0, SECTION_DRAWING @"Frame rate (FPS) when adaptive framerate is enabled and throughput is low but not 0.");
...
static BOOL iTermAdvancedSettingsUseGCDUpdateTimerDefaultValue(void) {
...
DEFINE_BOOL(useGCDUpdateTimer, iTermAdvancedSettingsUseGCDUpdateTimerDefaultValue(), SECTION_DRAWING @"Use GCD-based update timer instead of NSTimer.\nThis should cause more regular screen updates. Restart iTerm2 after changing this setting.");
...
DEFINE_BOOL(showInputLatencyMeter, NO, SECTION_DRAWING @"Show input latency meter\nDisplays time between key press and frame render. Requires Metal renderer.");
...
DEFINE_BOOL(dontAutoSelectNewTmuxTabs, NO, SECTION_TMUX @"Do not automatically select new tmux tabs.\nWhen enabled, new tmux tabs opened via tmux integration will not steal focus from the current tab.");
...
DEFINE_BOOL(suppressOrphanSessionRestoredWarning, YES, SECTION_WARNINGS @"Suppress warning about restored orphan sessions.");
DEFINE_BOOL(suppressBrowserPluginUpsell, NO, SECTION_WARNINGS @"Suppress browser plugin upsell messages.");
...
DEFINE_BOOL(dtermCoreEnabled, YES, SECTION_EXPERIMENTAL @"Enable dterm-core terminal engine?\nUses dterm-core (Rust) for parsing and terminal state. This is the recommended setting - dterm-core is faster and safer than the legacy VT100Parser.");
DEFINE_BOOL(dtermCoreValidationEnabled, NO, SECTION_EXPERIMENTAL @"Enable dterm-core validation mode?\nPeriodically compares dterm-core state with legacy parser. Only needed for debugging. Disabled by default for performance.");
DEFINE_BOOL(dtermCoreParserComparisonEnabled, NO, SECTION_EXPERIMENTAL @"Enable dterm-core parser comparison?\nRuns BOTH parsers and compares output. WARNING: Doubles parsing overhead. Only enable for debugging. dterm-core has been validated with zero mismatches.");
DEFINE_BOOL(dtermCoreParserOutputEnabled, YES, SECTION_EXPERIMENTAL @"Use dterm-core parser output?\nWhen enabled, uses tokens from dterm-core parser. This is the recommended setting - dterm-core parser is faster and has full box drawing support.");
DEFINE_BOOL(dtermCoreGridEnabled, YES, SECTION_EXPERIMENTAL @"Enable dterm-core grid rendering?\nUses dterm-core cell data for Metal rendering of visible grid lines. dterm-core now has full box drawing character support via dterm_is_box_drawing_character() FFI.");
DEFINE_BOOL(dtermCoreRendererEnabled, YES, SECTION_EXPERIMENTAL @"Enable dterm-core GPU renderer?\nUses the Rust-based hybrid renderer instead of iTermMetalView. Box drawing characters are fully supported. This renderer uses safe frame synchronization that cannot crash with 'unbalanced dispatch_group' errors.");

#pragma mark - Scripting
#define SECTION_SCRIPTING @"Scripting: "

DEFINE_STRING(pythonRuntimeDownloadURL, @"", SECTION_SCRIPTING @"URL to check for new versions of the Python scripting runtime. Empty = disabled.");
DEFINE_STRING(pythonRuntimeBetaDownloadURL, @"", SECTION_SCRIPTING @"URL to check for new Beta versions of the Python scripting runtime. Empty = disabled.");
