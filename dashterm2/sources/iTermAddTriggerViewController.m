...
+ (void)addTriggerForText:(NSString *)text
    ...
    iTermAddTriggerViewController *vc = [[iTermAddTriggerViewController alloc]
                  initWithName:text
        ...
        defaultBackgroundColor:defaultBackgroundColor
                   browserMode:browserMode
                    completion:^(NSDictionary *_Nullable dict, BOOL updateProfile) {
...
- (instancetype)initWithName:(NSString *)name
    ...
    if (self) {
        _regex = [regex copy];
        // BUG-7313: Don't overwrite contentRegex with nil - keep the passed value
...
- (void)setTrigger:(Trigger *)trigger {
    ...
    const NSInteger i =
        [_prototypes indexOfObjectPassingTest:^BOOL(Trigger *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    // i == NSNotFound can happen if you have a trigger of the wrong browser/terminal mode or a trigger from a future
    // version of the app.
    if (i != NSNotFound) {
    ...
    ITAssertWithMessage(
        _currentTrigger != nil, @"Failed with %@",
        trigger
            .dictionaryValue); // If this fails then a trigger is not round-tripping to its dictionary representation.
...
- (void)createViews {
    ...
    // Create help button
    NSString *helpText =
        @"This histogram shows how much CPU time was used evaluating the regular expression for this trigger over the "
        @"lifetime of the current terminal session. The X axis gives time in microseconds while the Y axis gives the "
        @"number of samples which fell in that duration bucket. The vertical red line indicates the mean duration. You "
        @"can use this to diagnose triggers that cause performance problems.";
    ...
    [performanceLabel setContentHuggingPriority:NSLayoutPriorityDefaultHigh
...
- (void)viewDidLoad {
    ...
    _updateProfileButton.state =
        [iTermUserDefaults addTriggerUpdateProfile] ? NSControlStateValueOn : NSControlStateValueOff;
...
- (IBAction)toggleVisualization:(NSButton *)button {
    if (!_popover || !_popover.isShown) {
        ...
        _visualizationViewController =
            [[iTermRegexVisualizationViewController alloc] initWithRegex:_regexTextField.stringValue ?: @""
...
- (IBAction)selectionDidChange:(id)sender {
    ...
    // BUG-f897: Replace assert with guard - return early if trigger creation fails
    if (prototype == nil) {
        ELog(@"BUG-f897: triggerFromUntrustedDict returned nil for dict: %@", dict);
        return;
    }
...
- (IBAction)ok:(id)sender {
    ...
    NSMutableDictionary *mutableTriggerDictionary = [@{
    ...
    NSView *view = [TriggerController viewForParameterForTrigger:trigger
                                             ...
                                             interpolatedStrings:_interpolatedStrings
                                                     ...
                                                     wellFactory:^CPKColorWell *(NSRect frame, NSColor *color) {
                                                         CPKColorWell *well = [[CPKColorWell alloc]
                                                             initWithFrame:frame
...
- (IBAction)toggleContentRegexVisualization:(NSButton *)button {
    if (!_contentRegexPopover || !_contentRegexPopover.isShown) {
        [_contentRegexPopover close];
        _contentRegexVisualizationViewController =
            [[iTermRegexVisualizationViewController alloc] initWithRegex:_contentRegexTextField.stringValue ?: @""
                                                                 maxSize:button.window.screen.visibleFrame.size];
        NSPopover *popover = [[NSPopover alloc] init];
...
- (void)updateButtonLayoutForBrowserMode {
    ...
    for (NSLayoutConstraint *constraint in buttonsRow.constraints) {
        if (constraint.firstItem == _enabledButton && constraint.firstAttribute == NSLayoutAttributeLeading &&
            constraint.secondItem == _instantButton && constraint.secondAttribute == NSLayoutAttributeTrailing) {
    ...
    if (textField == _regexTextField) {
        ...
        if ([textField.identifier isEqual:kTwoPraramNameColumnIdentifier]) {
            iTermTuple<NSString *, NSString *> *pair =
                [iTermTwoParameterTriggerCodec tupleFromString:[NSString castFrom:param]];
