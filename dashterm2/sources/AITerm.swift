...
// BUG-1701: Error type for when RemoteCommand.Content.withValue fails type cast
struct RemoteCommandTypeMismatchError: Error {}
...
class AITermController {
    ...
    private func handle(event: Event) {
        ...
        case .creatingVectorStore:
            ...
            case .begin, .pluginError, .cancel, .word:
                // BUG-f710: Invalid event in creatingVectorStore state - reset to ground
                DLog("WARNING: Invalid event \(event) in creatingVectorStore state - resetting")
                state = .ground
            case .webResponse:
                state = .ground
        ...
        case .uploadingFile:
            switch event {
            case .begin, .pluginError, .cancel, .word:
                // BUG-f711: Invalid event in uploadingFile state - reset to ground
                DLog("WARNING: Invalid event \(event) in uploadingFile state - resetting")
                state = .ground
            case .webResponse:
                state = .ground
            case .error(let error):
        ...
        case .addingFileToVectorStore:
            switch event {
            case .begin, .pluginError, .cancel, .word:
                // BUG-f712: Invalid event in addingFileToVectorStore state - reset to ground
                DLog("WARNING: Invalid event \(event) in addingFileToVectorStore state - resetting")
                state = .ground
            case .webResponse:
                state = .ground
            case .error(let error):
            ...
            case .begin:
                ...
                DispatchQueue.main.async { [weak self] in
                    ...
                    case .completion(let prompt):
                        self.requestCompletion(query: prompt,
                    ...
                    case .createVectorStore(name: let name):
                        self.requestCreateVectorStore(name: name,
                                                 registration: registration)
                    case let .fileUpload(name: name, content: content):
                        self.requestFileUpload(name: name,
                                          content: content,
                                          registration: registration)
                    case let .addFilesToVectorStore(fileIDs: fileIDs, vectorStoreID: vectorStoreID):
                        self.requestAddFilesToVectorStore(fileIDs: fileIDs,
                                                     vectorStoreID: vectorStoreID,
                                                     registration: registration)
                    }
                }
        ...
        case .initializedMessages(messages: let messages, stream: let stream):
            ...
            case .begin:
                ...
                DispatchQueue.main.async { [weak self] in
                    guard let self else { return }
                    self.requestCompletion(messages: messages,
                                registration: registration,
                                stream: stream ? { [weak self] word in
                        self?.handle(event: .word(word))
                    } : nil)
        ...
        case .querySent(messages: let messages, streamParserState: let streamParserState):
            switch event {
            case .begin:
                // BUG-f713: .begin event in querySent state is invalid - reset to ground
                DLog("WARNING: .begin event received in querySent state - resetting to ground")
                state = .ground
            case .webResponse(let response):
                if let error = response.error, !error.isEmpty {
                    ...
                    // BUG-1622: Use nil coalescing instead of force unwrap for data encoding
                    _ = parseStreamingResponse(data: response.data.data(using: .utf8) ?? Data(),
                                               final: true,
                                               parserState: streamParserState)
                } else {
                    // BUG-1622: Use nil coalescing instead of force unwrap for data encoding
                    parseNonStreamingResponse(data: response.data.data(using: .utf8) ?? Data())
                }
            case .pluginError(let error):
            ...
            case .word(let word):
                ...
                let updated = parseStreamingResponse(
                    data: word.data(using: .utf8) ?? Data(),
                    final: false,
    ...
    // BUG-1640: Safe fallback URL for when URL creation fails
    ...
    private var settingsURL: URL {
        ...
        if value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            // BUG-1640: Use safe cast instead of force cast
    ...
    private func parseStreamingResponse(data: Data, final: Bool, parserState: StreamParserState) -> StreamParserState? {
        ...
        guard let parser = llmProvider.streamingResponseParser(stream: true) else {
            handle(event: .error(AIError("Streaming is not supported by this language model in DashTerm2. You can disable streaming in Settings > General > AI.")))
            return nil
        }
        ...
        let drain = {
            ...
            case .functionCall(let call, _):
                ...
                accumulatingMessage.body = .uninitialized
            case .functionOutput:
                // BUG-654: Server should not send function output in streaming - log warning instead of crash
                DLog("Warning: Server unexpectedly sent function output during streaming - ignoring")
            case .multipart(let subparts):
                // BUG-654: Handle multipart messages by processing each subpart individually
                DLog("Draining multipart message with \(subparts.count) subparts")
                for subpart in subparts {
                    switch subpart {
                    case .text(let text):
                        if !text.isEmpty {
                            self.delegate?.aitermController(self, didStreamUpdate: text)
                        }
                    case .attachment(let attachment):
                        self.delegate?.aitermController(self, didStreamAttachment: attachment)
                    case .uninitialized, .functionCall, .functionOutput, .multipart:
                        // Non-text/attachment subparts are informational, skip during drain
                        break
                    }
                }
            }
        ...
        // BUG-1622: Use nil coalescing instead of force unwrap for data encoding
        return StreamParserState(message: accumulatingMessage, buffer: rest.data(using: .utf8) ?? Data())
    }
        ...
        case .querySent(let messages, _):
            ...
            if let impl = functions.first(where: { $0.decl.name == functionCall.name }) {
                ...
                impl.invoke(message: message,
                            json: (functionCall.arguments ?? "").data(using: .utf8) ?? Data()) { [weak self] result in
