...
@objc public class ChatErrorObjc: NSObject {
    @objc static let domain = "com.dashterm.dashterm2.chat"
...
final class ChatWindowController: NSWindowController, DictionaryCodable {
    ...
    @objc(instanceShowingErrors:) static func instance(showErrors: Bool) -> ChatWindowController? {
        ...
        if _instance == nil,
           ...
           let client = ChatClient.instance {
            ...
            // BUG-150: Chat window error messages updated to DashTerm2 branding
            iTermWarning.show(withTitle: "AI Chat could not open because of a problem loading the database. Verify there is only one instance of DashTerm2 running.",
                              actions: ["OK"],
                              accessory: nil,
                              identifier: nil,
    ...
    required init?(coder: NSCoder) {
        // BUG-f837: Return nil instead of crashing for unused coder initializer
        DLog("ChatWindowController init(coder:) is not supported")
        return nil
    }
    ...
    func createNewChatIfNeeded(currentSession: PTYSession?) {
        if model.count == 0 {  // swiftlint:disable:this empty_count
