        ...
        while i < currentLine.count {
            ...
            if (currentChar == "/" || currentChar == "~") && (i == 0 || !characterIsValidInPath(currentLine[i - 1].char)) {
                ...
                let endCoord: VT100GridCoord = {
                    ...
                    // it is safe to access j - 1. Use guard for defense-in-depth.
                    // BUG-f637: Use guard instead of it_assert - assertions stripped in release builds
                    guard j > 0 && j == currentLine.count else {
                        DLog("BUG-f637: PathExtractor invariant violated: j=\(j), count=\(currentLine.count)")
                        return VT100GridCoordMake(0, 0)  // Return safe default
                    }
