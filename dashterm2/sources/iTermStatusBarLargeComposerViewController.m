...
- (NSView *)newBackgroundViewWithFrame:(NSRect)frame {
    ...
    // BUG-9468: Set material and blendingMode for proper visual effect behavior
    myView.material = NSVisualEffectMaterialPopover;
    myView.blendingMode = NSVisualEffectBlendingModeBehindWindow;
    myView.state = NSVisualEffectStateFollowsWindowActiveState;
    return myView;
}
...
- (void)awakeFromNib {
    ...
    _aiCompletionWarning.hidden =
        ![[iTermSecureUserDefaults instance] aiCompletionsEnabled] || ![iTermAdvancedSettingsModel generativeAIAllowed];
...
- (NSMutableDictionary<NSString *, iTermSearchPathsCacheEntry *> *)cache {
    ...
    dispatch_once(&onceToken, ^{
        cache = [[NSMutableDictionary alloc] initWithCapacity:16];
    });
    return cache;
}
...
- (void)setScope:(iTermVariableScope *)scope {
    ...
    [[iTermSlowOperationGateway sharedInstance]
        exfiltrateEnvironmentVariableNamed:@"PATH"
                                     shell:shell
...
- (NSString *)textAfterCursor {
    ...
    // BUG-f1049: Use cached selectedRange.location instead of re-fetching to avoid race conditions
    return [content substringFromIndex:selectedRange.location];
}
...
- (NSString *)textBeforeCursor {
    ...
    // BUG-f1050: Use cached selectedRange.location instead of re-fetching to avoid race conditions
    return [content substringToIndex:selectedRange.location];
}
...
- (NSString *)lineAtCursor {
    ...
    NSInteger upperBound = [content rangeOfCharacterFromSet:[NSCharacterSet newlineCharacterSet]
                                                    options:0
                                                      range:NSMakeRange(lowerBound, content.length - lowerBound)]
                               .location;
...
- (void)openCommandHistory:(id)sender {
    ...
    if ([[iTermShellHistoryController sharedInstance] commandHistoryHasEverBeenUsed]) {
        ...
        if (newlineBefore == NSNotFound) {
            ...
            prefix =
                [content substringWithRange:NSMakeRange(newlineBefore + 1, selectedRange.location - newlineBefore - 1)];
        }
...
- (IBAction)performNaturalLanguageQuery:(id)sender {
    ...
    _aitermController =
        [[AITermControllerObjC alloc] initWithQuery:self.aiPrompt
                                              scope:self.scope
...
- (IBAction)help:(id)sender {
    ...
    style.tabStops = @[
        [[NSTextTab alloc] initWithType:NSLeftTabStopType location:tabStop],
    ...
    NSArray<NSString *> *lines =
        @[ @"^⇧↑\tAdd cursor above", @"^⇧↓\tAdd cursor below", @"^⇧-click\tAdd cursor", @"⌥-drag\tAdd cursors" ];
    if ([iTermAdvancedSettingsModel generativeAIAllowed]) {
        lines = [lines arrayByAddingObject:@"⌘Y\tNatural language AI lookup"];
    }
    lines = [lines arrayByAddingObjectsFromArray:@[
        @"⌘F\tOpen Find bar", @"⌥⌘V\tOpen in Advanced Paste", @"⌘-click\tOpen in explainshell.com",
        @"⇧↩\tSend contents or selection", @"⌥⇧↩\tSend command at cursor", @"⌥↩\tEnqueue command at cursor",
        @"⇧⌘;\tView command history"
    ]];
    [_popoverVC appendString:[lines componentsJoinedByString:@"\n"]];
    [_popoverVC.textView.textStorage addAttribute:NSParagraphStyleAttributeName
                                            value:style
                                            range:NSMakeRange(0, _popoverVC.textView.textStorage.string.length)];
    [_popoverVC sizeToFit];
    [_popoverVC.popover showRelativeToRect:_help.bounds ofView:_help preferredEdge:NSRectEdgeMaxY];
}
...
- (NSArray<iTermCompletionItem *> *)historySuggestionsForPrefix:(NSString *)prefix
    ...
    NSArray<iTermCommandHistoryEntryMO *> *entries =
        [[iTermShellHistoryController sharedInstance] commandHistoryEntriesWithPrefix:prefix onHost:self.host];
    return [[entries
        subarrayToIndex:maxResults] mapWithBlock:^id _Nullable(iTermCommandHistoryEntryMO *_Nonnull entry) {
        NSString *value;
        if (removePrefix) {
        ...
        return [[iTermCompletionItem alloc]
            initWithValue:value
                   detail:[NSString
                              stringWithFormat:@"Last used %@",
                                               [NSDateFormatter
                                                   dateDifferenceStringFromDate:date
...
- (void)textDidChange:(NSNotification *)notification {
    ...
    if (command.length > 0 && ![command endsWithWhitespace] && textAfterCursor.length > 0 &&
    ...
    generation = [self fetchCompletionsForCommand:command
        ...
        earlyResult:^iTermCompletionItem *(NSArray<iTermCompletionItem *> *early,
            ...
            return [weakSelf setEarlyResult:early history:history prefix:command];
        }
        completion:^(BOOL suggestionOnly, NSArray<iTermCompletionItem *> *completions,
...
- (void)fetchCompletions {
    ...
    generation = [self fetchCompletionsForCommand:command
        ...
        completion:^(BOOL _suggestionOnly, NSArray<iTermCompletionItem *> *completions,
                     NSArray<iTermCompletionItem *> *commands) {
            [weakSelf didFetchCompletions:completions
                                 commands:commands
                               generation:generation
...
- (NSInteger)fetchCompletionsForCommand:(NSString *)command
                            ...
                            earlyResult:(iTermCompletionItem * (^)(NSArray<iTermCompletionItem *> *,
                             ...
                             completion:(void (^)(BOOL suggestionOnly, NSArray<iTermCompletionItem *> *,
                                                  NSArray<iTermCompletionItem *> *))completionBlock {
    const BOOL autocompleteSupported =
        [self.delegate largeComposerViewControllerShouldFetchSuggestions:self
                                                                 forHost:self.host
    ...
    if (!autocompleteSupported) {
        ...
        if (historySuggestion) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                iTermCompletionItem *item = [[iTermCompletionItem alloc]
                    initWithValue:[historySuggestion.value substringFromIndex:command.length]
    ...
    // If the command ends with whitespace then we're not in the first word but words will have one element because it
    // ignores trailing whitespce, so append a placeholder character and then count words.
    ...
    iTermSuggestionRequest *request = [[iTermSuggestionRequest alloc] initWithPrefix:prefix
        ...
        completion:^(BOOL suggestionOnly, NSArray<iTermCompletionItem *> *items) {
            DLog(@"iTermStatusBarLargeComposerViewController got suggestions");
            completionBlock(suggestionOnly, items ?: @[], historySuggestions ?: @[]);
        }];
    [self.delegate largeComposerViewController:self fetchSuggestions:request byUserRequest:explicit];
    return generation;
}
...
- (iTermCompletionItem *)setEarlyResult:(NSArray<iTermCompletionItem *> *)files
                                history:(NSArray<iTermCompletionItem *> *)historySuggestions
                                 prefix:(NSString *)prefix {
    NSArray<iTermCompletionItem *> *completions = [[files mapWithBlock:^id _Nullable(iTermCompletionItem *item) {
        NSString *escaped = [item.value stringWithBackslashEscapedShellCharactersIncludingNewlines:YES];
        return [[iTermCompletionItem alloc] initWithValue:escaped detail:item.detail kind:item.kind];
...
- (BOOL)textView:(NSTextView *)textView doCommandBySelector:(SEL)commandSelector {
    if (self.textView.hasSuggestion && commandSelector == @selector(insertTab:)) {
    ...
    if (commandSelector == @selector(deleteForward:) || commandSelector == @selector(deleteBackward:) ||
        ...
        commandSelector == @selector(deleteWordForward:) || commandSelector == @selector(deleteWordBackward:) ||
        commandSelector == @selector(deleteToBeginningOfLine:) || commandSelector == @selector(deleteToEndOfLine:) ||
        commandSelector == @selector(deleteToBeginningOfParagraph:) ||
        commandSelector == @selector(deleteToEndOfParagraph:) || commandSelector == @selector(deleteToMark:)) {
        [self.textView setSuggestion:nil];
        [self.textView doSyntaxHighlighting];
    }
...
- (void)setHideAccessories:(BOOL)hideAccessories {
    ...
    if (hideAccessories) {
        ...
        _scrollView.frame = NSMakeRect(sideMargin, topMargin, self.view.frame.size.width - sideMargin * 2,
                                       self.view.frame.size.height - topMargin * 2);
    } else {
        const CGFloat sideMargin = 5;
        const CGFloat topMargin = 11;
        const CGFloat bottomMargin = 19;
        _scrollView.frame = NSMakeRect(sideMargin, bottomMargin, self.view.frame.size.width - sideMargin * 2,
