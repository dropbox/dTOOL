...
- (instancetype)initWithConfiguration:(NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)configuration
    ...
    if (self) {
        ...
        _childPidRef = [[iTermVariableReference alloc] initWithPath:iTermVariableKeySessionEffectiveSessionRootPid
                                                             vendor:scope];
...
- (NSArray<NSString *> *)newAncestryChainForPid:(int)pid {
    iTermProcessInfo *deepestForegroundJob =
        [[self.delegate statusBarComponentProcessInfoProvider] processInfoForPid:pid];
    int sessionTaskPid =
        [NSNumber castFrom:[self.scope valueForVariableName:iTermVariableKeySessionEffectiveSessionRootPid]].intValue;

    iTermProcessInfo *current = deepestForegroundJob;
    NSMutableArray<NSString *> *chain = [[NSMutableArray alloc] initWithCapacity:8];
    while (current) {
...
- (NSArray<NSString *> *)variantsOfChain:(NSArray<NSString *> *)chain {
    NSMutableArray<NSString *> *temp = [[NSMutableArray alloc] initWithCapacity:chain.count];
...
- (void)statusBarComponentDidClickWithView:(NSView *)view {
    ...
    iTermJobTreeViewController *viewController =
        [[iTermJobTreeViewController alloc] initWithProcessID:pid
    ...
    [popover showRelativeToRect:rect ofView:relativeView preferredEdge:preferredEdge];
