...
+ (instancetype)globalEntry {
    ...
    dispatch_once(&onceToken, ^{
        // BUG-7: Script history app name updated to DashTerm2
        instance = [[self alloc] initWithName:@"DashTerm2 App" fullPath:nil identifier:@"DashTerm2" relaunch:nil];
    });
    return instance;
}
...
+ (instancetype)dynamicProfilesEntry {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[self alloc] initWithName:@"Dynamic Profiles" fullPath:nil identifier:@"__DP" relaunch:nil];
    });
    return instance;
}
...
+ (instancetype)browserEntry:(NSString *)identifier {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        browserEntries = [[NSMutableDictionary alloc] initWithCapacity:8];
    });
...
+ (instancetype)smartSelectionAnctionsEntry {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[self alloc] initWithName:@"Smart Selection Actions" fullPath:nil identifier:@"__SSA" relaunch:nil];
    });
    return instance;
}
...
- (instancetype)initWithName:(NSString *)name
    ...
    if (self) {
        ...
        _logLines = [[NSMutableArray alloc] initWithCapacity:100];    // Script log lines, capped at 1000
        _callEntries = [[NSMutableArray alloc] initWithCapacity:100]; // RPC call history
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            gScriptHistoryDateFormatter = [[NSDateFormatter alloc] init];
            gScriptHistoryDateFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"Ld jj:mm:ssSSS"
                                                                                     options:0
...
- (void)addClientOriginatedRPC:(NSString *)rpc {
    NSString *string = [NSString stringWithFormat:@"Script → DashTerm2 %@:\n%@\n",
...
- (void)kill:(int)signal {
    ...
    // BUG-3758: Verify process still exists before trying to kill it
    // Check if the process exists and belongs to us by sending signal 0
    if (kill(pid, 0) != 0) {
        DLog(@"Process %d no longer exists (errno=%d)", pid, errno);
        return;
    }
    ...
    if (pgid <= 0) {
        ...
        // Process exists but we can't get pgid, try direct kill
        kill(pid, signal);
        return;
    }

    // BUG-3758: Verify the process group leader is still our script process.
    // Only use killpg if the pid is the process group leader (pgid == pid).
    // Otherwise, just kill the individual process to avoid affecting unrelated processes.
    if (pgid == pid) {
        // Safe to kill process group - our process is the leader
        killpg(pgid, signal);
    } else {
        // Process is not the group leader - only kill the individual process
        // to avoid affecting other processes in the group we don't own
        DLog(@"Process %d is not group leader (pgid=%d), killing individual process only", pid, pgid);
        kill(pid, signal);
    }
}
- (void)stopRunning {
...
- (void)appendLogs:(NSString *)rawLogs completion:(void (^)(void))completion {
    ...
    dispatch_once(&onceToken, ^{
        ...
        queue = dispatch_queue_create("com.dashterm.dashterm2.script-history", attr);
    });
    const BOOL continuation = _lastLogLineContinues;
    _lastLogLineContinues = ![rawLogs hasSuffix:@"\n"];
    dispatch_async(queue, ^{
        [self queueAppendLogs:rawLogs timestamp:timestamp continuation:continuation completion:completion];
    });
}
...
- (void)queueAppendLogs:(NSString *)rawLogs
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self didSplitLogIntoLines:newLines continuation:continuation delta:delta completion:completion];
    });
}
...
- (void)didSplitLogIntoLines:(NSArray<NSString *> *)newLines
                continuation:(BOOL)continuation
                  ...
                  completion:(void (^)(void))completion {
    if (continuation) {
        // BUG-3119: Guard against nil access and underflow when arrays are empty
        if (_logLines.count > 0 && newLines.count > 0) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermScriptHistoryEntryDidChangeNotification
                      object:self
                    userInfo:@{
                        iTermScriptHistoryEntryDelta : delta,
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _entries = [[NSMutableArray alloc] initWithCapacity:16]; // Script history entries
        [self addAPSLoggingEntryIfNeeded];
        [_entries addObject:[iTermScriptHistoryEntry globalEntry]];
        _replPIDs = [[NSMutableSet alloc] initWithCapacity:4];   // REPL process IDs
        _browserIDs = [[NSMutableSet alloc] initWithCapacity:4]; // Browser session IDs
    }
    return self;
}
...
- (void)addAPSLoggingEntryIfNeeded {
    ...
    if ([iTermUserDefaults enableAutomaticProfileSwitchingLogging]) {
        ...
        if (_entries.count != 1) {
            [[NSNotificationCenter defaultCenter]
                postNotificationName:iTermScriptHistoryNumberOfEntriesDidChangeNotification
                              object:self];
        }
    }
}
