...
@interface ToolCapturedOutputView () <ToolbeltTool, NSMenuDelegate, NSMenuItemValidation, NSSearchFieldDelegate,
                                      NSTextFieldDelegate>
@end
...
- (instancetype)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        if (@available(macOS 10.16, *)) {
            ...
            _clearButton.image = [NSImage it_imageForSymbolName:SFSymbolGetString(SFSymbolTrash)
        ...
        scrollView_ = [NSScrollView scrollViewWithTableViewForToolbeltWithContainer:self insets:NSEdgeInsetsZero];
...
- (void)updateCapturedOutput {
    ...
    NSMutableArray *temp = [NSMutableArray arrayWithCapacity:allCapturedOutput_.count];
...
- (void)dealloc {
    ...
    // BUG-1697: Cancel pending performSelector calls
...
- (void)relayout {
    ...
    NSRect searchFieldFrame =
        NSMakeRect(0, 0, frame.size.width - help_.frame.size.width - _clearButton.frame.size.width - 2 * kMargin,
    ...
    // Clear button
    {
        ...
        _clearButton.frame = NSMakeRect(help_.frame.origin.x - _clearButton.frame.size.width - kMargin, fudgeFactor,
                                        _clearButton.frame.size.width, _clearButton.frame.size.height);
    }

    // Scroll view
    [scrollView_ setFrame:NSMakeRect(0, searchFieldFrame.size.height + kMargin, frame.size.width,
...
- (void)clear:(id)sender {
    ...
    allCapturedOutput_ = [[NSMutableArray alloc] initWithCapacity:32]; // Cleared captured output entries
    filteredEntries_ = [[NSMutableArray alloc] initWithCapacity:32];   // Filtered view
...
- (NSTableCellView *)newCellViewWithString:(NSString *)string {
    NSTableCellView *cell =
        [tableView_ newTableCellViewWithTextFieldUsingIdentifier:iTermCapturedOutputToolTableViewCellIdentifier
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    // BUG-10341: Add bounds check before accessing filteredEntries_
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)rowIndex {
    ...
    if (rowIndex < 0 || rowIndex >= (NSInteger)filteredEntries_.count) {
        return [NSTableView heightForTextCellUsingFont:[NSFont it_toolbeltFont]]; // Default row height
    }
...
- (void)revealSelection {
    ...
    // BUG-1689: Add upper bounds check
...
- (void)toggleCheckmark:(id)sender {
    ...
    // BUG-1689: Add upper bounds check
    if (index >= 0 && index < (NSInteger)filteredEntries_.count) {
