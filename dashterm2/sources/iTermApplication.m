...
NSNotificationName const iTermApplicationCharacterAccentMenuVisibilityDidChange =
    @"iTermApplicationCharacterAccentMenuVisibilityDidChange";
...
+ (iTermApplication *)sharedApplication {
    ...
    // BUG-f899: Replace assert with log - class type mismatch is critical but shouldn't crash
    // This happens during unit testing when the app is not actually running.
    if (![sharedApplication isKindOfClass:[iTermApplication class]]) {
        ELog(@"BUG-f899: CRITICAL - sharedApplication is not iTermApplication class: %@", [sharedApplication class]);
        return (iTermApplication *)sharedApplication;  // Return early, don't register observers
    }
...
- (void)it_windowDidOrderOffScreen:(NSNotification *)notification {
    ...
    if ([NSStringFromClass(object.class) isEqualToString:@"NSPanelViewBridge"]) {
        _it_imeOpen = NO;
        [[NSNotificationCenter defaultCenter] postNotificationName:iTermApplicationInputMethodEditorDidClose
                                                            object:nil];
    }
}
...
- (BOOL)switchToWindowByNumber:(NSEvent *)event {
    const NSUInteger allModifiers = (NSEventModifierFlagShift | NSEventModifierFlagControl |
                                     NSEventModifierFlagCommand | NSEventModifierFlagOption);
    const NSUInteger maskedValue = ([event it_modifierFlags] & allModifiers);
    const NSUInteger requirement =
        [iTermPreferences maskForModifierTag:[iTermPreferences intForKey:kPreferenceKeySwitchWindowModifier]];
...
- (BOOL)switchToPaneInWindowController:(PseudoTerminal *)currentTerminal byNumber:(NSEvent *)event {
    ...
    if (([event it_modifierFlags] & mask) ==
        [iTermPreferences maskForModifierTag:[iTermPreferences intForKey:kPreferenceKeySwitchPaneModifier]]) {
...
- (BOOL)dispatchHotkeyLocally:(NSEvent *)event {
    if (IsSecureEventInputEnabled() && [[iTermHotKeyController sharedInstance] eventIsHotkey:event]) {
...
- (BOOL)handleKeyDownEvent:(NSEvent *)event {
    DLog(@"Received KeyDown event: %@. Key window is %@. First responder is %@", event, [self keyWindow],
...
- (void)sendEvent:(NSEvent *)event {
    switch (event.type) {
        ...
        case NSEventTypeScrollWheel:
            ...
            event.it_functionModifierPressed = _functionPressed;
            if (event.momentumPhase == NSEventPhaseChanged || event.momentumPhase == NSEventPhaseEnded) {
...
- (void)createStatusBarItemIfNeeded {
    ...
    // Use colored icon (Dropbox blue) instead of template
    image.template = NO;
...
- (void)updateMenuBarIconVisibility {
    // Always show the menu bar icon if the setting is enabled
    if ([iTermAdvancedSettingsModel alwaysShowMenuBarIcon]) {
        [self createStatusBarItemIfNeeded];
    } else if (!_isUIElement) {
        // Only remove if we're not in UI element mode (where the icon is needed for access)
...
- (void)setIsUIElement:(BOOL)uiElement {
    ...
    TransformProcessType(&psn, uiElement ? kProcessTransformToUIElementApplication
    ...
    if (uiElement) {
        ...
        // Show status bar icon when in UI element mode (needed for settings access)
        if ([iTermAdvancedSettingsModel statusBarIcon]) {
            [self createStatusBarItemIfNeeded];
        }
    } else {
        // When not in UI element mode, visibility depends on alwaysShowMenuBarIcon setting
        if (![iTermAdvancedSettingsModel alwaysShowMenuBarIcon]) {
            [self removeStatusBarItem];
        }
    }
...
- (void)updateWindows {
    ...
    if (_characterAccentWindowWasOpen != accentMenuOpen) {
        ...
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermApplicationCharacterAccentMenuVisibilityDidChange
                          object:nil];
...
- (BOOL)shouldCheckForAccentWindowVisibilityChange {
    ...
    if (_lastRepeatTime > 0) {
        ...
        DLog(@"_lastRepeatTime=%@, timesinceboot=%@, delta=%@", @(_lastRepeatTime), @([NSDate it_timeSinceBoot]),
             @(delta));
        if (delta < 5.0) {
...
- (void)orderFrontCharacterPalette:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermApplicationCharacterPaletteWillOpen object:nil];
    [super orderFrontCharacterPalette:sender];
    const NSTimeInterval deadlineToOpen =
        ([NSDate timeIntervalSinceReferenceDate] + [iTermAdvancedSettingsModel timeToWaitForEmojiPanel]);
...
- (void)makeCursorSparkles {
    if (@available(macOS 11.0, *)) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            size.height *= 1.25;
            NSImage *composite =
                [NSImage imageOfSize:NSMakeSize(size.width + 1, size.height + 1)
                           drawBlock:^{
                               for (int dx = 0; dx <= 2; dx++) {
                                   for (int dy = 0; dy <= 2; dy++) {
                                       ...
                                       [white drawInRect:NSMakeRect(dx / 2, dy / 2, size.width, size.height)
                                                fromRect:NSZeroRect
                                               operation:NSCompositingOperationSourceOver
                                                fraction:1];
                                   }
                               }
                               [black drawInRect:NSMakeRect(1.0 / 2, 1.0 / 2, size.width, size.height)
                                        fromRect:NSZeroRect
                                       operation:NSCompositingOperationSourceOver
                                        fraction:1];
                           }];
            sparkles = [[NSCursor alloc] initWithImage:composite
...
- (void)reportException:(NSException *)exception {
    NSString *header = nil;
    @try {
        // BUG-CRASH-2025-12-28: iTermDebugLogHeaderString() accesses AppKit APIs that
        // must be called from the main thread. If we're on a background thread (e.g.,
        // exception thrown in a concurrent test), skip the header to avoid triggering
        // MainThreadChecker abort.
        if ([NSThread isMainThread]) {
            header = iTermDebugLogHeaderString();
        } else {
            header = @"(Header unavailable - exception on background thread)";
        }
        // BUG-4155: @catch must catch NSException, not NSError (NSError isn't thrown)
...
- (NSModalResponse)runModalForWindow:(NSWindow *)window {
    ...
    if (pseudoterminal) {
        iTermProfileHotKey *hotkey =
            [[iTermHotKeyController sharedInstance] profileHotKeyForWindowController:pseudoterminal];
