...
- (void)update {
    ...
    NSMutableArray<NSString *> *lines = [[NSMutableArray alloc] initWithCapacity:8];
    ...
    if (unavailable) {
        ...
        if (self.installUtilities) {
            ...
            if (_busy && stage == 2) {
                ...
                step = [NSString stringWithFormat:@"%@ DashTerm2 utility scripts.", prefix];
            }
            [lines addObject:step];
        }
        ...
        if (_busy && stage == i) {
            ...
            step = [NSString stringWithFormat:@"%@ your shell’s dotfile.", prefix];
        }
        [lines addObject:step];
    ...
    [lines enumerateObjectsUsingBlock:^(NSString *_Nonnull string, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        NSAttributedString *as =
            [[NSAttributedString alloc] initWithString:temp
                                            attributes:idx == indexToBold ? boldAttributes : regularAttributes];
...
- (NSArray<NSButton *> *)previewCommandButtons {
    return @[
        self.previewCommandButton1, self.previewCommandButton2, self.previewCommandButton3, self.previewCommandButton4
    ];
}
