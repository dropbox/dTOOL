...
class AIMetadata: NSObject {
    ...
    // BUG-f551: Return a fallback model instead of crashing when models is empty
    @objc static var defaultModel: AIModel {
        guard let firstModel = instance.models.first else {
            DLog("ERROR: AIMetadata.models is empty - using fallback model")
            // Return a basic fallback model to prevent crash
            let fallback = Model(
                name: "fallback",
            ...
            return AIModel(fallback)
        }
        return AIModel(firstModel)
    }
