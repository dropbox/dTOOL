...
class ChatClient {
    ...
    private func processMessage(_ message: Message,
        ...
        case .userCommand:
            // BUG-3019: Agent should not send userCommand - log and ignore instead of crash
            DLog("Error: Agent unexpectedly sent userCommand - ignoring malformed message")
            return nil
        case .remoteCommandRequest(let request, safe: let safe):
            // BUG-471: Log protocol violation instead of crashing - partial should be false for command requests
            if partial {
                DLog("Warning: Received partial remoteCommandRequest - expected non-partial message")
            }
        ...
        case let .append(string: string, uuid: uuid):
            // BUG-472: Log protocol violation instead of crashing - append should be partial
            if !partial {
                DLog("Warning: Received non-partial append message - expected partial message")
            }
        ...
        case let .appendAttachment(attachment: attachment, uuid: uuid):
            // BUG-473: Log protocol violation instead of crashing - appendAttachment should be partial
            if !partial {
                DLog("Warning: Received non-partial appendAttachment message - expected partial message")
            }
    ...
    private func processAppend(appendMessage: Message,
        ...
        case .plainText, .markdown:
            if let messages = model.messages(forChat: chatID, createIfNeeded: false),
               i < messages.count - 1 {
                ...
                // BUG-3020/BUG-3021: Restructured to handle edge cases without crashing
                switch stringOrAttachment {
                case .string(let string):
                    switch original.content {
                    case .plainText:
                        temp.content = .plainText(string, context: nil)
                    case .markdown:
                        temp.content = .markdown(string)
                    case .explanationRequest, .explanationResponse, .remoteCommandRequest,
                            ...
                            .userCommand:
                        // BUG-3020: Cannot append to these message types - use plainText as fallback
                        DLog("Warning: Cannot append \(stringOrAttachment) to message type \(original.content) - using plainText fallback")
                        temp.content = .plainText(string, context: nil)
                    }
                case .attachment(let attachment):
                    // BUG-3021: Server-sent attachments - pass through as appendAttachment
                    DLog("Note: Server-sent attachment in append - using appendAttachment")
        ...
        case .explanationResponse(var response, _, let accumulatedMarkdown):
            ...
            case .attachment(let attachment):
                ...
                case .fileID(id: let fileID, name: let name):
                    // fileID represents a file on OpenAI's servers that hasn't been downloaded yet.
                    // Append a placeholder indicating the file reference for now.
                    response.append("[File attachment: \(name) (id: \(fileID))]", final: false)
                }
            }
