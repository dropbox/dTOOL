...
class iTermOffscreenDrawable: NSObject, MTLDrawable, CAMetalDrawable {
    ...
    /// BUG-f564: Return nil and log instead of crashing when texture accessed before init
    /// BUG-f617: Safely create placeholder texture instead of force unwrap
    /// BUG-f666: Removed it_assert(false) - using graceful fallback instead
    var texture: MTLTexture {
        guard let tex = _texture else {
            DLog("BUG-f564: Texture accessed before initialization - returning placeholder texture")
            // BUG-f617: Create placeholder texture safely without force unwraps
            // This is a fallback path that should never execute in normal operation
            if let device = _device ?? MTLCreateSystemDefaultDevice(),
               let placeholder = device.makeTexture(descriptor: MTLTextureDescriptor()) {
                return placeholder
            }
            // Last resort: return a minimal valid texture from system device
            // This is extremely unlikely to fail on any Mac with Metal support
            DLog("BUG-f617: Failed to create placeholder texture - using minimal descriptor")
            let descriptor = MTLTextureDescriptor()
            descriptor.width = 1
            descriptor.height = 1
            descriptor.pixelFormat = .bgra8Unorm
            if let systemDevice = MTLCreateSystemDefaultDevice(),
               let minimalTexture = systemDevice.makeTexture(descriptor: descriptor) {
                return minimalTexture
            }
            // If we get here, Metal is fundamentally broken - crash with a clear message
            it_fatalError("BUG-f617: Unable to create any Metal texture - Metal framework unavailable")
        }
        return tex
    }

    /// BUG-f564: Safe accessor that returns nil instead of crashing
    var textureIfReady: MTLTexture? { _texture }
    ...
    /// BUG-f565: Use it_assert instead of fatal error for device access before init
    /// BUG-f615: Safely get system default device instead of force unwrap
    /// BUG-f667: Removed it_assert(false) - using graceful fallback instead
    var device: MTLDevice {
        get {
            guard let dev = _device else {
                DLog("BUG-f565: Device accessed before initialization - returning system default device")
                // BUG-f615: Fall through to get system default device safely
                if let systemDevice = MTLCreateSystemDefaultDevice() {
                    return systemDevice
                }
                // Metal unavailable - crash with clear message
                it_fatalError("BUG-f615: Metal framework unavailable - no GPU device found")
            }
            return dev
        }
    ...
    /// BUG-f565: Safe accessor that returns nil instead of crashing
    var deviceIfReady: MTLDevice? { _device }
    var presentedTime: Double { _presentedTime }
    /// BUG-f566: Use it_assert instead of fatal error for layer access before init
    /// BUG-f668: Removed it_assert(false) - using graceful fallback instead
    var layer: CAMetalLayer {
        guard let lyr = _layer else {
            DLog("BUG-f566: Layer accessed before initialization - returning new layer as fallback")
            // Return a new layer as fallback
            return CAMetalLayer()
        }
        return lyr
    }

    /// BUG-f566: Safe accessor that returns nil instead of crashing
    var layerIfReady: CAMetalLayer? { _layer }
    var size: CGSize {
        get { _size }
        set { _size = newValue }
    }
    ...
    func addPresentedHandler(_ handler: @escaping (() -> ())) {
        // OFFSCREEN_DRAWABLE_STUB: Offscreen drawable doesn't support present handlers.
        // This is a stub implementation for protocol conformance. The offscreen
        // drawable renders to a texture for capture/testing rather than screen display.
    }

    init(device: MTLDevice, pixelFormat: MTLPixelFormat, size: CGSize, drawableID: UInt) {
        // OFFSCREEN_DRAWABLE_STUB: Initializer stub for offscreen drawable.
        // In the offscreen use case, drawables are created via nextDrawable() on the layer,
        // not directly initialized. This exists for protocol conformance.
        super.init()
    }

    func present() {
        // OFFSCREEN_DRAWABLE_STUB: Offscreen drawable doesn't present to display.
        // For offscreen rendering, the texture content is accessed directly.
    }

    func present(at presentationTime: CFTimeInterval) {
        // OFFSCREEN_DRAWABLE_STUB: Offscreen drawable doesn't support timed presentation.
    }

    func present(afterMinimumDuration duration: CFTimeInterval) {
        // OFFSCREEN_DRAWABLE_STUB: Offscreen drawable doesn't support minimum duration presentation.
    }
...
public class iTermMetalView_Full: NSView, CALayerDelegate {
    ...
    // BUG-501: Initialize to (1, 1) instead of zero to prevent division by zero
    // in convertFromBacking methods before initCommon is called
    var drawableScaleFactor: CGSize = CGSize(width: 1.0, height: 1.0)
    ...
    var device: MTLDevice? {
        set {
            ...
            if drawOffscreen, let device = newValue {
                for x23 in 0...3 {
                    offscreenSwapChain[x23]?.device = device
                }
            }
    ...
    public var depthStencilPixelFormat: MTLPixelFormat = .invalid {
        didSet {
            ...
            case .x32_stencil8, .x24_stencil8:
                // BUG-f567, BUG-f669: Log and fall back to invalid instead of crashing
                DLog("BUG-f567: Unsupported depth stencil format \(depthStencilPixelFormat) - falling back to invalid")
                depthStencilPixelFormat = .invalid
                return  // Avoid recursion - already set to invalid
            default:
                break
            }
            renderAttachmentDirtyState |= 0x80000000
    ...
    public var currentRenderPassDescriptor: MTLRenderPassDescriptor? {
        ...
        if sampleCount >= 2 {
            ...
            _ = colorTextures
            for x22 in 0...maxValidAttachmentIndex {
                // BUG-1802: Use guard to safely unwrap texture instead of force unwrap
                guard let texture = _colorTextures[x22] else {
                    continue
                }
                descriptor.colorAttachments[x22].texture = texture
                descriptor.colorAttachments[x22].loadAction = .clear
                descriptor.colorAttachments[x22].clearColor = clearColor
            }
        }
    ...
    public var drawableSize: CGSize {
        ...
        set {
            ...
            let backingScaleFactor = if let screen {
                ...
                NSScreen.main?.backingScaleFactor ?? 1.0  // BUG-502: Default to 1.0 not 0
            }
            // BUG-503: Guard against division by zero when bounds or scale is zero
            let widthDivisor = myBounds.width * backingScaleFactor
            let heightDivisor = myBounds.height * backingScaleFactor
            if widthDivisor > 0 && heightDivisor > 0 {
                drawableScaleFactor = CGSize(width: newValue.width / widthDivisor,
                                             height: newValue.height / heightDivisor)
            }
    ...
    override public func encode(with coder: NSCoder) {
        ...
        let clearColorData = withUnsafeBytes(of: clearColor) { bytes in
            guard let baseAddr = bytes.baseAddress else {
                return Data()
            }
            return Data(bytes: baseAddr, count: MemoryLayout<MTLClearColor>.size)
        }
    ...
    required init?(coder: NSCoder) {
        ...
        if coder.containsValue(forKey: "MTKViewColorPixelFormatArrayCoderKey") {
            ...
            // BUG-f669: Add guard for pixel format array length validation
            let expectedLength = maxValidAttachmentIndex * 8
            guard length == expectedLength else {
                DLog("BUG-f669: Invalid pixel format array length: \(length), expected \(expectedLength) - skipping format restore")
                return
            }
            ...
            // BUG-f670: Add bounds check for formats array access
            if length > 0 && maxValidAttachmentIndex < formats.count {
    ...
    private func callDrawRectIMP(on target: AnyObject, with rect: CGRect, imp: IMP) {
        ...
        // BUG-1680, BUG-7203: This unsafeBitCast is safe because:
        // 1. The IMP is obtained from method(for: #selector(draw(_:))) at initialization
        // 2. #selector(draw(_:)) is NSView.draw(_:) with signature (NSRect) -> Void
        // 3. DrawRectFunction matches the Obj-C method signature with self/SEL prefix
        // 4. The IMP is captured once at init and never refreshed, so swizzling after
        //    initialization is handled correctly (original IMP is still called)
        // 5. The caller (draw()) uses `if let imp = drawRectSubIMP` to safely unwrap
        // Note: Swizzling *before* initialization could theoretically cause issues if the
        // swizzled method has a different signature, but this is an unsupported edge case
        // for which there is no practical runtime validation in Swift.
    ...
    public func draw() {
        ...
        resizeMetalLayerDrawable()
        if subClassOverridesDrawRect, let imp = drawRectSubIMP {
            callDrawRectIMP(on: self, with: bounds, imp: imp)
        } else {
    ...
    open override func convertFromBacking(_ point: CGPoint) -> CGPoint {
        ...
        // BUG-528: Guard against division by zero when scale width/height is zero
        return CGPoint(
            x: scale.width > 0 ? converted.x / scale.width : converted.x,
            y: scale.height > 0 ? converted.y / scale.height : converted.y)
    }
    ...
    open override func convertFromBacking(_ size: CGSize) -> CGSize {
        ...
        let scale = drawableScaleFactor
        // BUG-529: Guard against division by zero when scale width/height is zero
        return CGSize(
            width: scale.width > 0 ? converted.width / scale.width : converted.width,
            height: scale.height > 0 ? converted.height / scale.height : converted.height)
    }
    ...
    private func initCommon() {
        ...
        if drawOffscreen {
            // BUG-f568, BUG-f670: Log and skip instead of crashing for unimplemented feature
            // The disassembly was ambiguous here but this code path isn't taken normally.
            DLog("BUG-f568: Offscreen drawing not implemented - skipping (feature not available)")
    ...
    private func dumpFrameImage(withFilename filename: String) {
        // BUG-f569, BUG-f671: Log and skip instead of crashing for unimplemented debug feature
        DLog("BUG-f569: dumpFrameImage not implemented - skipping dump of \(filename)")
    }

    private func dumpFramerate(_ framerate: Double, withFilename filename: String) {
        // DEBUG_FRAMERATE: Diagnostic function for performance testing.
        // Writes framerate to /tmp for debugging. Not critical for normal operation.
        let path = "/tmp/DashTerm2-\(filename).txt"
        do {
            try String(format: "%.2f fps", framerate).write(toFile: path, atomically: true, encoding: .utf8)
        } catch {
            DLog("Failed to dump framerate to \(path): \(error)")
        }
    }
    ...
    private func exportToTarga(atLocation location: String, width: Int, height: Int, size: Int, bytes: [UInt8]) {
        ...
        guard size >= 0, bytes.count >= size else {
            return
        }
        let (totalLength, overflow) = headerLength.addingReportingOverflow(size)
        guard !overflow else {
            return
        }
        guard let data = NSMutableData(length: totalLength) else {
            return
        }
        guard data.length >= headerLength else {
            return
        }
    ...
    private func calculateRefreshesPerSecond() -> Int {
        if let displayLink {
            ...
            // BUG-504: Guard against division by zero when timeValue is 0
            guard cvtime.timeValue > 0 else {
                return 60
            }
