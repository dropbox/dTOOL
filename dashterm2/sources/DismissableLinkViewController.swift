...
class DismissableLinkViewController: NSViewController {
    ...
    required init?(coder: NSCoder) {
        // BUG-f822: Return nil instead of crashing for unused coder initializer
        DLog("DismissableLinkViewController init(coder:) is not supported")
        return nil
    }
    ...
    private func addHideButton() {
        ...
        // BUG-f579: Return early instead of crashing if button is not initialized
        guard let existingButton = button else {
            DLog("BUG-f579: DismissableLinkViewController.addHideButton() called before button was initialized - skipping")
            return
        }
        view.addConstraint(NSLayoutConstraint(item: imageButton,
                                              attribute: .centerY,
                                              relatedBy: .equal,
                                              toItem: existingButton,
                                              attribute: .centerY,
                                              multiplier: 1,
                                              constant: 1))
        view.addConstraint(NSLayoutConstraint(item: imageButton,
