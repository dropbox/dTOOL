...
static NSString *const iTermBackgroundCommandRunnerDidSelectNotificationNotificationName =
    @"iTermBackgroundCommandRunnerDidSelectNotificationNotificationName";
...
+ (void)maybeNotify:(void (^)(NSInteger))block {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        rateLimit = [[iTermRateLimitedUpdate alloc] initWithName:@"Background command runner" minimumInterval:10];
    });
...
- (instancetype)init {
    ...
    if (self) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            // Typically 0-5 active background command runners
            activeRunners = [NSMutableArray arrayWithCapacity:4];
        });
    }
    return self;
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p command=%@ shell=%@ title=%@ path=%@ running=%@>",
                                      NSStringFromClass(self.class), self, self.command, self.shell, self.title,
                                      self.path, @(_running)];
}
...
- (void)reallyRun {
    DLog(@"%@", self);
    // BUG-f903: Replace assert with guard - prevent double-running but don't crash
    if (_running) {
        ELog(@"BUG-f903: reallyRun called while already running - ignoring");
        return;
    }
    _running = YES;
    iTermCommandRunner *commandRunner =
        [[iTermCommandRunner alloc] initWithCommand:@"/bin/sh"
    ...
    iTermScriptHistoryEntry *entry = [[iTermScriptHistoryEntry alloc] initWithName:self.title
...
- (void)didCompleteWithStatus:(int)status entry:(iTermScriptHistoryEntry *)entry {
    ...
    if (self.notificationTitle && status) {
        ...
        [self.class maybeNotify:^(NSInteger deferCount) {
            ...
            if (deferCount > 1) {
                detail = [detail stringByAppendingFormat:@", plus %@ other error%@ silenced.", @(deferCount - 1),
                                                         deferCount > 2 ? @"s" : @""];
            }
            [[iTermNotificationController sharedInstance]
                   postNotificationWithTitle:self.notificationTitle
                                      detail:detail
