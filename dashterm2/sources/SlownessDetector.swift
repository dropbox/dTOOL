...
class SlownessDetector: NSObject {
    ...
    func measure(event: String, closure: () -> ()) {
        ...
        guard let lastStackValue = stack.last else {
            return
        }
        let duration = durationWithoutDoubleCounting - lastStackValue
        stack.removeLast()
...
// nonisolated(unsafe) because this is initialized once by lazy initialization and never modified
nonisolated(unsafe) private var timebase: Double = {
