...
static id iTermVariablesNewProxy(NSString *name, id value, BOOL isAlias, NSString *debugInfo) {
    ...
    if (nested) {
        iTermVariablesNonterminalProxy *proxy = [[iTermVariablesNonterminalProxy alloc] initWithName:name
                                                                                           variables:nested];
    ...
    if (weak) {
        ...
        iTermVariablesWeakNonterminalProxy *proxy =
            [[iTermVariablesWeakNonterminalProxy alloc] initWithName:name variables:weak.variables isAlias:isAlias];
        proxy.debugInfo = debugInfo;
        return proxy;
    }
...
- (instancetype)initWithName:(NSString *)name variables:(iTermVariables *)variables {
    // BUG-f965: Use guard instead of assert - nil name should return nil with warning
    if (name == nil) {
        DLog(@"BUG-f965: initWithName:variables: called with nil name - returning nil");
        return nil;
    }
    self = [super init];
    if (self) {
...
- (instancetype)initWithScope:(iTermVariableScope *)scope {
    ...
    if (self) {
        _scope = scope;
        NSArray<iTermTuple<NSString *, iTermVariables *> *> *sortedFrames = [scope.frames
            sortedArrayUsingComparator:^NSComparisonResult(iTermTuple<NSString *, iTermVariables *> *_Nonnull obj1,
                                                           iTermTuple<NSString *, iTermVariables *> *_Nonnull obj2) {
        ...
        _children = [sortedFrames flatMapWithBlock:^id(iTermTuple<NSString *, iTermVariables *> *tuple) {
            if (tuple.firstObject) {
                ...
                return [[tuple.secondObject.allNames
                    sortedArrayUsingSelector:@selector(compare:)] mapWithBlock:^id(NSString *name) {
                    ...
                    const BOOL isAlias = (tuple.firstObject == nil && tuple.secondObject == weakVariables.variables);
                    return iTermVariablesNewProxy(name, value, isAlias,
                                                  weakVariables.variables.debugInfo ?: [tuple.secondObject debugInfo]);
                }];
            }
        }]
                        ?: @[];
...
- (NSString *)selectedPathForOutlineView:(NSOutlineView *)outlineView {
    return [self pathForRow:outlineView.selectedRow outlineView:outlineView];
}
...
- (void)copyPath:(id)sender {
    ...
    [pboard setString:[self pathForRow:outlineView.clickedRow outlineView:outlineView] forType:NSPasteboardTypeString];
}
...
- (void)copyValue:(id)sender {
    ...
    NSPasteboard *pboard = [NSPasteboard generalPasteboard];
    [pboard declareTypes:@[ NSPasteboardTypeString ] owner:NSApp];
    [pboard setString:proxy.value forType:NSPasteboardTypeString];
}
