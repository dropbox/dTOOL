...
- (instancetype)init {
    self = [super init];
    if (self) {
        _children = [[NSMutableArray alloc] initWithCapacity:4];  // Close prompt reasons
    }
    return self;
}
...
+ (instancetype)alwaysConfirmQuitPreferenceEnabled {
    return [[[iTermPromptOnCloseMessageReason alloc] initWithMessage:@"“Settings > General > Closing > Confirm Quit DashTerm2” is enabled and there is at least one terminal window." priority:100] autorelease];
}

+ (instancetype)alwaysConfirmQuitPreferenceEvenIfThereAreNoWindowsEnabled {
    return [[[iTermPromptOnCloseMessageReason alloc] initWithMessage:@"“Settings > General > Closing > Confirm Quit DashTerm2” and “Even if there are no windows” is enabled." priority:100] autorelease];
}
