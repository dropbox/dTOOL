...
- (void)setHapticFeedbackForEscEnabled:(BOOL)hapticFeedbackForEscEnabled {
    ...
    [iTermPreferences setBool:hapticFeedbackForEscEnabled forKey:kPreferenceKeyEnableHapticFeedbackForEsc];
}
...
- (void)setSoundForEscEnabled:(BOOL)enabled {
    ...
    [iTermPreferences setBool:enabled forKey:kPreferenceKeyEnableSoundForEsc];
}
...
- (void)setVisualIndicatorForEscEnabled:(BOOL)enabled {
    ...
    [iTermPreferences setBool:enabled forKey:kPreferenceKeyVisualIndicatorForEsc];
}
...
- (void)setPlaceholderView:(NSView *)placeholderView {
    ...
    NSMenuItem *item;
    item = [[NSMenuItem alloc] initWithTitle:@"Import…" action:@selector(importMenuItem:) keyEquivalent:@""];
    item.target = self;
    [_presetsPopup.menu addItem:item];
    item = [[NSMenuItem alloc] initWithTitle:@"Export…" action:@selector(exportMenuItem:) keyEquivalent:@""];
    item.target = self;
    [_presetsPopup.menu addItem:item];
}
...
- (void)addViewsToSearchIndex:(iTermPreferencesBaseViewController *)vc {
    [vc addViewToSearchIndex:_addTouchBarItem displayName:@"Add touch bar item" phrases:@[] key:nil];
    [vc addViewToSearchIndex:_presetsPopup displayName:@"Key binding presets" phrases:@[] key:nil];
    [vc addViewToSearchIndex:_touchBarMitigationsButton
                 displayName:@"Touch bar mitigations"
                     phrases:@[
                         @"Haptic feedback for esc key", @"Key click sound for esc key", @"Visual indicator for esc key"
                     ]
...
- (NSString *)descriptionForKeystroke:(iTermKeystroke *)keystroke bindingDictionary:(NSDictionary *)dict {
...
- (NSString *)stringValueForKeyMappingOnRow:(NSInteger)rowIndex
    ...
    // BUG-10362: Bounds check before accessing sorted keystrokes array
    if (rowIndex < 0 || rowIndex >= (NSInteger)sortedKeystrokes.count) {
        return @"";
    }
    ...
    if (column == _actionColumn) {
        return [self descriptionForKeystroke:keystroke bindingDictionary:dict];
    }
    return nil;
}
...
- (NSString *)stringValueForTouchBarMappingOnRow:(NSInteger)rowIndex column:(NSTableColumn *)column {
    ...
    // BUG-10363: Bounds check before accessing sorted touch bar items array
    if (rowIndex < 0 || rowIndex >= (NSInteger)sortedKeys.count) {
        return @"";
    }
...
- (NSString *)stringValueForColumn:(NSTableColumn *)column row:(NSInteger)rowIndex {
    ...
    if (rowIndex < dict.count) {
        return [self stringValueForKeyMappingOnRow:rowIndex column:column bindingDictionary:dict];
    }

    return [self stringValueForTouchBarMappingOnRow:rowIndex - dict.count column:column];
}
...
- (void)presentEditActionSheet:(iTermEditKeyActionWindowController *)editActionWindowController {
    ...
    [self.view.window beginSheet:editActionWindowController.window
...
- (IBAction)addTouchBarItem:(id)sender {
    ...
    editActionWindowController = [[iTermEditKeyActionWindowController alloc]
        initWithContext:iTermVariablesSuggestionContextSession | iTermVariablesSuggestionContextApp
...
- (IBAction)removeMapping:(id)sender {
    ...
    NSMutableSet<iTermKeystroke *> *regularKeystrokes = [NSMutableSet setWithCapacity:indexes.count];
    NSMutableSet<iTermTouchbarItem *> *touchbarItems = [NSMutableSet setWithCapacity:indexes.count];
    ...
    [indexes enumerateIndexesUsingBlock:^(NSUInteger row, BOOL *_Nonnull stop) {
        if (row < sortedRegularKeystrokes.count) {
            ...
            // BUG-10202: Guard against out of bounds access to touchbar items array
            NSUInteger touchbarIndex = row - sortedRegularKeystrokes.count;
            if (touchbarIndex < sortedTouchbarItems.count) {
                [touchbarItems addObject:sortedTouchbarItems[touchbarIndex]];
            }
        }
    }];
    [_tableView beginUpdates];
    [_delegate keyMapping:self removeKeystrokes:regularKeystrokes touchbarItems:touchbarItems];
...
- (void)doubleClick:(id)sender {
    ...
    if (rowIndex < sortedItems.count) {
        ...
        // BUG-3107: Add bounds check to prevent array index out of bounds crash
        if (rowIndex < 0 || rowIndex >= (int)sortedItems.count) {
            return;
        }
        selectedItem = sortedItems[rowIndex];
        dict = [_delegate keyMappingTouchBarItems];
    }
    _editActionWindowController = [[iTermEditKeyActionWindowController alloc]
        initWithContext:iTermVariablesSuggestionContextSession | iTermVariablesSuggestionContextApp
                   mode:selectedItem.hasSecond ? iTermEditKeyActionWindowControllerModeTouchBarItem
                                               : iTermEditKeyActionWindowControllerModeKeyboardShortcut
            profileType:[self.delegate keyMappingProfileType:self]];
    __block NSDictionary *binding;
    [selectedItem
        whenFirst:^(iTermKeystroke *_Nonnull keystroke) {
            _editActionWindowController.currentKeystroke = keystroke;
            binding = [keystroke valueInBindingDictionary:dict];
        }
        second:^(iTermTouchbarItem *_Nonnull touchbarItem) {
...
- (IBAction)soundForEscToggled:(id)sender {
    [iTermPreferences setBool:_soundForEsc.state == NSControlStateValueOn forKey:kPreferenceKeyEnableSoundForEsc];
}
...
- (void)importMenuItem:(id)sender {
    ...
    [panel beginWithFallbackWindow:self.view.window
                           handler:^(NSModalResponse result, NSArray<NSURL *> *urls) {
...
- (void)importFromOpenPanel:(NSURL *)url {
    ...
    NSError *error = nil;
    NSString *const content = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:&error];
    if (!content) {
        XLog(@"Beep: %@", error);
        NSBeep();
        return;
    ...
    NSSet<iTermKeystroke *> *keystrokesThatWillChange =
        [NSSet setWithArray:[keymappings.allKeys mapWithBlock:^id(id anObject) {
...
- (NSNumber *)removeBeforeLoading:(NSString *)thing {
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection2:
            return nil;
        default:
            // BUG-403: Replace assert(NO) with DLog - unexpected warning selection
            // (e.g., user cancelled or system returned unexpected response)
            DLog(@"iTermKeyMappingViewController: Unexpected warning selection %@", @(selection));
    }
    return nil;
}
...
- (void)exportMenuItem:(id)sender {
    ...
    __weak __typeof(self) weakSelf = self;
    [_savePanel beginWithFallbackWindow:self.view.window
                                handler:^(NSModalResponse response, iTermSavePanelItem *item) {
...
- (void)exportFromSavePanel:(iTermSavePanelItem *)item {
    ...
    NSDictionary *const dict =
        @{INTERCHANGE_KEY_MAPPING_DICT : keymappings ?: @{}, INTERCHANGE_TOUCH_BAR_ITEMS : touchbarItems ?: @[]};
