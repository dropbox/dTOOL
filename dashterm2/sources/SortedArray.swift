...
class SortedArray<T>: CustomDebugStringConvertible {
    ...
    func removeUpTo(location: Int64) {
        // Optimization: Use binary search instead of linear scan since array is sorted.
        // Find the first index where array[i].location > location
        guard !array.isEmpty else { return }

        var start = 0
        var end = array.count

        // Binary search for first element with location > target
        while start < end {
            let mid = (start + end) / 2
            if array[mid].location <= location {
                start = mid + 1
            } else {
                end = mid
            }
        ...
        // start now points to first index with location > target (or array.count if none)
        if start >= array.count {
            array = []
        } else if start > 0 {
            array.removeSubrange(0..<start)
        }
        // If start == 0, nothing to remove
    }
    ...
    private func indexes(entries: [Entry]) -> IndexSet {
        ...
        while i < array.count && entryIndex < sortedEntries.count {
            ...
            // BUG-489: Log warning instead of crashing if location invariant is violated
            if currentLocation >= desiredEntry.location {
                DLog("BUG-489: SortedArray invariant violated: currentLocation \(currentLocation) >= desiredEntry.location \(desiredEntry.location)")
                // Fall through to binary search as recovery
                linearSearchCount = maximumLinearSearchIterations + 1
            }
