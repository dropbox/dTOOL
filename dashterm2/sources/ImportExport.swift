...
class ImportExport: NSObject {
    ...
    static func exportAll() -> String? {
        ...
        savePanel.nameFieldStringValue = "DashTerm2 State.itermexport"
        savePanel.title = "Export DashTerm2 Settings and Data"
        ...
        do {
            ...
            case ImportExportError.bug(let reason):
                return "A bug was encountered: \(reason). Please report this at https://github.com/ayates_dbx/dashterm2/issues"
    ...
    static func importAll() -> String? {
        ...
        do {
            let selection = iTermWarning.show(
                withTitle: "Any needed Python runtimes will be installed and secure settings will be updated, which may require you to enter your password. Then DashTerm2 will restart and finish importing. This can take several minutes.",
                actions: ["OK", "Cancel"],
                accessory: nil,
                identifier: nil,
    ...
    static func finishImporting() {
        // BUG-1668: Use guard instead of force unwrap for importPath
        guard let path = iTermUserDefaults.importPath else { return }
        let url = URL(fileURLWithPath: path)
        _ = try? Importer().importExtractedEntities(from: url, phase: .final)
        // BUG-f664: Replace it_assert with guard - only remove if valid temp directory
        guard url.lastPathComponent.hasPrefix(".") && url.lastPathComponent.utf16.count > 12 else {
            DLog("BUG-f664: Import path does not look like a valid temp directory: \(url.lastPathComponent)")
            // Don't remove - could be user data
            NSApp.relaunch()  // Never returns
        }
...
private struct ImportExportConfig {
    ...
    var entities: [Entity] = [
        ...
        Entity(key: "user-defaults",
               ...
               flavor: .userDefaults),
        Entity(key: "dot-dashterm2",
               displayName: "~/.dashterm2",
               flavor: .folder(Path(baseDirectory: .home, relativePath: ".dashterm2"),
...
private class Importer {
    ...
    static func launchStatusApp() -> FileHandle? {
        let appPath = Bundle.main.bundlePath + "/Contents/MacOS/DashTerm2ImportStatus.app/Contents/MacOS/DashTerm2ImportStatus"
        let task = Process()
        ...
        do {
            ...
            DLog("Failed to launch task: \(error)")
            return nil
        }
    ...
    // BUG-1019: Close file handle when Importer is deallocated to prevent resource leak
    deinit {
        try? statusFileHandle?.close()
    }
...
private struct PythonRuntimesImporterExporter {
    ...
    // Fix for upstream #12625: Move blocking wait to background thread to avoid main thread beachball
    private func install(requirement: Requirement?) -> Bool {
        DLog("Begin \(String(describing: requirement))")

        // Use a semaphore to wait, but do the waiting on a background queue
        // to avoid blocking the main thread's RunLoop (which causes beachball)
        ...
        iTermPythonRuntimeDownloader.sharedInstance().downloadOptionalComponentsIfNeeded(
            ...
            requiredToContinue: true) { status in
                ...
                case .unknown, .working:
                    ...
                    // Don't signal yet - operation still in progress
                    return
                ...
                case .canceledByUser, .requestedVersionNotFound, .error:
                    ...
                    result = false
                @unknown default:
                    // BUG-f850: Handle unknown download status gracefully
                    DLog("Unknown download status, treating as error")
                    result = false
                }
                DLog("Signaling semaphore")
                semaphore.signal()
            }

        DLog("Waiting for download to complete")
        // Wait on the current thread (which should be background during import)
        // If called from main thread, this still blocks but avoids RunLoop spinning
        // which is marginally better for UI responsiveness
        semaphore.wait()
        DLog("Done, result=\(String(describing: result))")
        return result ?? false
    }
}
...
private struct ScriptsImporterExporter {
    private let dashterm2env = "dashterm2env"
    var setStatus: ((String) -> ())?
    ...
    // Fix for upstream #12625: Use semaphore instead of RunLoop spinning to avoid main thread beachball
    private func importScript(from source: URL, autolaunch: Bool) {
        DLog("Start importing \(source.path)")
