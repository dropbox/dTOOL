...
- (instancetype)init {
    self = [super init];
    if (self) {
        _actions = [[[NSArray castFrom:[[NSUserDefaults standardUserDefaults] objectForKey:kPreferenceKeyActions]]
                       mapWithBlock:^id(id anObject) {
...
- (void)moveActionsWithIdentifiers:(NSArray<NSNumber *> *)identifiers toIndex:(NSInteger)row {
    ...
    for (iTermAction *action in actions) {
        ...
        // BUG-f691: Replace assert with guard - action not found should be skipped not crash
        if (i == NSNotFound) {
            DLog(@"ERROR: iTermActionsModel moveActionsWithIdentifiers: action not found - %@", action);
            continue;
        }
        [removals addIndex:i];
...
- (void)save {
    [[NSUserDefaults standardUserDefaults] setObject:[self arrayOfDictionaries] forKey:kPreferenceKeyActions];
}
...
+ (void)subscribe:(NSObject *)owner block:(void (^)(iTermActionsDidChangeNotification *_Nonnull notification))block {
