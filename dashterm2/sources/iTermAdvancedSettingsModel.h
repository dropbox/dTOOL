...
+ (BOOL)doNotSetCtype;
+ (BOOL)dontAutoSelectNewTmuxTabs;
...
+ (BOOL)dtermCoreEnabled;
+ (BOOL)dtermCoreValidationEnabled;
+ (BOOL)dtermCoreParserComparisonEnabled;
+ (BOOL)dtermCoreParserOutputEnabled;
+ (BOOL)dtermCoreGridEnabled;
+ (BOOL)dtermCoreRendererEnabled;
...
+ (double)apiServerConnectionTimeoutSeconds;
...
+ (BOOL)suppressOrphanSessionRestoredWarning;
+ (BOOL)suppressBrowserPluginUpsell;
