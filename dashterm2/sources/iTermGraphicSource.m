...
- (NSDictionary *)it_invertedGraphicDictionary {
    ...
    [self enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull graphicName, NSArray *_Nonnull obj,
                                              BOOL *_Nonnull stop) {
...
- (instancetype)init {
    ...
    if (self) {
        ...
        dispatch_once(&onceToken, ^{
            NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"graphic_colors" ofType:@"json"];
            ...
            path = [[NSBundle bundleForClass:[self class]] pathForResource:@"graphic_icons" ofType:@"json"];
            data = [NSData dataWithContentsOfFile:path options:0 error:nil];
            if (data) {
                sGraphicIconMap = [[NSJSONSerialization JSONObjectWithData:data options:0
                                                                     error:nil] it_invertedGraphicDictionary];
            }
            path = [appSupport stringByAppendingPathComponent:@"graphic_icons.json"];
            data = [NSData dataWithContentsOfFile:path options:0 error:nil];
            if (data) {
                NSDictionary *dict =
                    [[NSDictionary castFrom:[NSJSONSerialization JSONObjectWithData:data options:0
                                                                              error:nil]] it_invertedGraphicDictionary];
...
// Command icon images are loaded from disk and can be regenerated, so they're
// ideal candidates for auto-eviction under memory pressure.
// Limit to 256 entries - typical use involves 10-50 commands at most.
static NSCache<NSString *, NSImage *> *CachedGraphicImages(void) {
    static NSCache<NSString *, NSImage *> *images;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
- (NSImage *)imageForJobName:(NSString *)command enabled:(BOOL)enabled {
    ...
    if (image) {
        [CachedGraphicImages() setObject:image forKey:command];
    }
    return image;
}
...
- (NSImage *)imageForJobName:(NSString *)jobName {
    ...
    NSImage *image = [CachedGraphicImages() objectForKey:command];
    if (image) {
        return image;
    }
    ...
    return
        [NSImage imageOfSize:size
                   drawBlock:^{
                       ...
                       NSPoint textOrigin =
                           NSMakePoint((size.width - textSize.width) / 2.0, (size.height - textSize.height) / 2.0);

                       // Draw the filled circle
                       NSBezierPath *circlePath =
                           [NSBezierPath bezierPathWithOvalInRect:NSMakeRect(1, 1, size.width - 2, size.height - 2)];
...
- (NSString *)randomTintColorForString:(NSString *)string {
    ...
    return
        [NSString stringWithFormat:@"#%02x%02x%02x", (int)(color.r * 255), (int)(color.g * 255), (int)(color.b * 255)];
}
