...
- (instancetype)init {
    ...
    context_ = [[NSMutableArray alloc] initWithCapacity:16];  // Context words for autocomplete
    stack_ = [[NSMutableArray alloc] initWithCapacity:8];  // Stack depth for completion
    findResults_ = [[NSMutableArray alloc] initWithCapacity:128];  // Matches maxConsumeCount
...
- (NSArray<NSString *> *)contextWordsStartingAt:(VT100GridCoord)startCoord
    ...
    // Pre-size based on maxWords parameter
    NSMutableArray<NSString *> *context = [NSMutableArray arrayWithCapacity:maxWords];
    ...
    for (int i = 0; i < kMaxIterations && [context count] < maxWords; ++i) {
        ...
        NSString *s = [self contextWordAt:coord textExtractor:textExtractor start:&start];
        if (s) {
...
- (NSString *)contextWordAt:(VT100GridCoord)coord
    ...
    VT100GridWindowedRange range = [textExtractor rangeForWordAt:coord maximumLength:kReasonableMaximumWordLength];
...
- (void)loadPrefixFromHost:(VT100Screen *)screen {
    const VT100GridCoordRange range =
        VT100GridCoordRangeMake(screen.cursorX - 1, screen.cursorY - 1 + screen.numberOfScrollbackLines,
                                screen.cursorX - 1, screen.cursorY - 1 + screen.numberOfScrollbackLines);
...
- (void)loadPrefixFromScreen:(VT100Screen *)screen {
    ...
    NSArray<NSString *> *context = [self contextWordsStartingAt:range.coordRange.start maxWords:maxWords screen:screen];
...
- (double)scoreResultNumber:(int)resultNumber
        ...
        joiningPrefixLength:(int)joiningPrefixLength
                       word:(NSString *)word {
    AcLog(@"Score result #%d with queryContext:%@ and resultContext:%@", resultNumber,
          [self formatContext:queryContext], [self formatContext:resultContext]);
    ...
    // Strongly penalize very short candidates, because they're not worth completing, unless the context similarity is
    // very strong.
...
- (void)_processPasteboardEntry:(PasteboardEntry *)entry {
    ...
    if (range.location != NSNotFound) {
        if (range.location > 0) {
            if (![[iTermTextExtractor wordSeparatorCharacterSet]
        ...
        // TODO: This is kind of sketchy. We need to look at text before the prefix and lower the score if not much of
        // the prefix was matched.
        double score = [self scoreResultNumber:[self _timestampToResultNumber:[entry timestamp]]
                                  queryContext:context_
                                 resultContext:@[]
                           joiningPrefixLength:[prefix_ length]
                                          word:word];
        PopupEntry *e = [PopupEntry entryWithString:word score:score];
...
- (NSString *)insertableString {
    // BUG-379: Guard against crash when selectedRow is -1 or out of bounds
    NSInteger selectedRow = [table_ selectedRow];
    if (selectedRow < 0) {
        return moreText_ ?: @"";
    }
    NSInteger convertedIndex = [self convertIndex:selectedRow];
    PopupModel *model = [self model];
    if (convertedIndex < 0 || convertedIndex >= (NSInteger)model.count) {
        return moreText_ ?: @"";
    }
    PopupEntry *e = [model objectAtIndex:convertedIndex];
    NSString *result = moreText_ ?: @"";
    return [result stringByAppendingString:e.mainValue];
}

// BUG-384: Add bounds check to prevent array index out of bounds crash
- (void)rowSelected:(id)sender {
    if ([table_ selectedRow] >= 0) {
        NSInteger convertedIndex = [self convertIndex:[table_ selectedRow]];
        PopupModel *model = [self model];
        if (convertedIndex < 0 || convertedIndex >= (NSInteger)model.count) {
            return;
        }
...
- (void)_pushStackObject {
    NSArray *value =
        [NSArray arrayWithObjects:[[moreText_ ? moreText_ : @"" copy] autorelease], [[prefix_ copy] autorelease],
                                  [NSNumber numberWithBool:whitespaceBeforeCursor_], nil];
    // BUG-f673: Replace assert with guard - array creation failure should not crash
    if ([value count] != 3) {
        DLog(@"ERROR: Stack object creation failed, expected 3 elements but got %lu", (unsigned long)[value count]);
        return;
    }
...
- (void)_popStackObject {
    ...
    // BUG-398: Guard against nil value and incorrect count to prevent crash in release builds
    // assert() is stripped in release builds, so we need a runtime check
...
// BUG-385: Add bounds check to prevent array index out of bounds crash
- (void)more {
    AcLog(@"More");
    if ([table_ selectedRow] >= 0) {
        NSInteger convertedIndex = [self convertIndex:[table_ selectedRow]];
...
- (void)_doPopulateMore {
    ...
    if ([findResults_ count] == 0) {
        // BUG-f674: Replace assert with guard - more_ should be YES but crash prevention is critical
        if (!more_) {
            DLog(@"ERROR: Expected more_ to be YES when findResults is empty");
            return;
        }
    ...
    while ([findResults_ count] > 0) {
        ...
        // BUG-1251a: Always remove the result BEFORE continue to prevent infinite loop
        ...
        while ([firstWord length] < [prefix_ length]) {
            range = [extractor rangeForWordAt:range.coordRange.end maximumLength:kReasonableMaximumWordLength];
        ...
        if (wordRange.location == 0) {
            ...
            // Set fullMatch to true if the word we found is equal to prefix, or false if word just has prefix as its
            // prefix.
            ...
            if (fullMatch) {
                ...
                if ([word rangeOfCharacterFromSet:nonWhitespace].location == NSNotFound) {
                    ...
                    if (range.coordRange.end.y < [screen numberOfLines]) {
                        ...
                        AcLog(@"Replacement candidate is at %@: '%@'", VT100GridWindowedRangeDescription(range), word);
                    } else {
            ...
            if ([word rangeOfCharacterFromSet:nonWhitespace].location != NSNotFound) {
                ...
                PopupEntry *e = [PopupEntry entryWithString:word
                                                      score:[self scoreResultNumber:matchCount_++
                                                                       queryContext:context_
...
- (void)addCommandEntries:(NSArray<iTermCommandHistoryEntryMO *> *)entries context:(NSString *)context {
    ...
    for (iTermCommandHistoryEntryMO *entry in entries) {
        ...
        PopupEntry *e = [PopupEntry entryWithString:command score:score];
        if (whitespaceBeforeCursor_) {
            [e setPrefix:[NSString stringWithFormat:@"%@ ", prefix_]];
        } else {
            [e setPrefix:prefix_];
