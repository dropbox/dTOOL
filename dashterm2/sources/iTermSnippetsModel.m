...
NSString *iTermSnippetHelpMarkdown =
    @"Terms in the search query are used to prefix search snippet titles, content, and tags.\n\nYou can use the "
    @"following operators to restrict what attributes of snippets a term searches:\n * `tag:` to search only tags.\n * "
    @"`title:` to search only snippet titles.\n * `text:` to search only the text of a snippet.\nFor example, "
    @"`tag:linux`.\n\nTo search only for snippets that do *not* match a term, use the `-` operator. For example, "
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p title=%@ value=%@ guid=%@ version=%@ escaping=%@>",
                                      NSStringFromClass([self class]), self, _title, _value, _guid, @(_version),
                                      @(_escaping)];
}
...
- (NSString *)trimmedValue:(NSInteger)maxLength includingRange:(NSRange)range {
    // BUG-f1048: Guard against nil/empty value or invalid range
    ...
    // BUG-f1048: Clamp proposed.location to value length to prevent substringFromIndex crash
    if (proposed.location > self.value.length) {
        DLog(@"BUG-f1048: proposed.location %lu exceeds value.length %lu, clamping", (unsigned long)proposed.location,
             (unsigned long)self.value.length);
        proposed.location = self.value.length;
    }
    NSString *end =
        [[self.value substringFromIndex:proposed.location] ellipsizedDescriptionNoLongerThan:proposed.length];
...
+ (void)writeDictionaries:(NSArray<NSDictionary *> *)arrayOfDictionaries {
    ...
    if (error) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:arrayOfDictionaries forKey:kPreferenceKeySnippets];
        [iTermWarning
            showWarningWithTitle:
                [NSString stringWithFormat:@"There was a problem saving snippets to “%@”.\n\nThe error was:\n%@", path,
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _snippets = [[[iTermSnippetsModel freshlyLoadedValues] mapWithBlock:^id(id anObject) {
            ...
            // Pre-allocate for typical number of snippets (usually 4-32)
        }] mutableCopy]
...
- (NSInteger)indexOfSnippetWithGUID:(NSString *)guid {
    return
        [_snippets indexOfObjectPassingTest:^BOOL(iTermSnippet *_Nonnull snippet, NSUInteger idx, BOOL *_Nonnull stop) {
...
+ (BOOL)snippet:(iTermSnippet *)snippet matchesQuery:(NSString *)queryString {
    ...
    iTermProfileStyleSearchEngineQuery *query = [[iTermProfileStyleSearchEngineQuery alloc] initWithQuery:queryString
                                                                                                operators:operators];
    ...
    NSDictionary<NSString *, NSString *> *phrases =
        @{@"title:" : snippet.title ?: @"", @"text:" : snippet.value ?: @""};
    iTermProfileStyleSearchEngineDocument *doc =
        [[iTermProfileStyleSearchEngineDocument alloc] initWithPhrases:phrases tags:snippet.tags];
...
+ (NSArray<iTermSnippet *> *)snippetsMatchingSearchQuery:(NSString *)queryString
    ...
    NSArray<iTermSnippet *> *filteredSnippets = [snippets mapWithBlock:^iTermSnippet *(iTermSnippet *snippet) {
        NSDictionary<NSString *, NSString *> *phrases =
            @{@"title:" : snippet.title ?: @"", @"text:" : snippet.value ?: @""};
        iTermProfileStyleSearchEngineDocument *doc =
            [[iTermProfileStyleSearchEngineDocument alloc] initWithPhrases:phrases tags:snippet.tags];
...
- (void)moveSnippetsWithGUIDs:(NSArray<NSString *> *)guids toIndex:(NSInteger)row {
    ...
    for (iTermSnippet *snippet in snippets) {
        ...
        // BUG-f1267: Replace assert() with guard - snippet not found should skip, not crash
        if (i == NSNotFound) {
            DLog(@"WARNING: BUG-f1267 snippet not found during reorder, skipping");
            continue;
        }
        [removals addIndex:i];
...
+ (instancetype)moveNotificationWithRemovals:(NSIndexSet *)removals destinationIndex:(NSInteger)destinationIndex {
    iTermSnippetsDidChangeNotification *notif = [[self alloc] initPrivate];
...
+ (void)subscribe:(NSObject *)owner block:(void (^)(iTermSnippetsDidChangeNotification *_Nonnull notification))block {
