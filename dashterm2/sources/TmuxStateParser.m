...
- (NSNumber *)paneIdNumberValue {
    if ([self hasPrefix:@"%"] && [self length] > 1) {
        ...
        DLog(@"WARNING: Bogus pane id %@", self);
...
+ (NSString *)format {
    NSMutableString *format = [NSMutableString stringWithCapacity:512];
    NSArray *theModes = @[
        kStateDictPaneId, kStateDictSavedGrid, kStateDictAltSavedCX, kStateDictAltSavedCY, kStateDictCursorX,
        kStateDictCursorY, kStateDictScrollRegionUpper, kStateDictScrollRegionLower, kStateDictTabstops,
        kStateDictCursorMode, kStateDictInsertMode, kStateDictKCursorMode, kStateDictKKeypadMode, kStateDictWrapMode,
        kStateDictMouseStandardMode, kStateDictMouseButtonMode, kStateDictMouseAnyMode, kStateDictMouseUTF8Mode,
        kStateDictMouseSGRMode
    ];
...
+ (NSMutableDictionary *)dictionaryForState:(NSString *)state workAroundTabBug:(BOOL)workAroundTabBug {
    ...
    NSDictionary *fieldTypes = [NSDictionary
        dictionaryWithObjectsAndKeys:intType, kStateDictSavedGrid, intType, kStateDictCursorX, intType,
                                     kStateDictCursorY, intType, kStateDictAltSavedCX, intType, kStateDictAltSavedCY,
                                     uintType, kStateDictCursorMode, uintType, kStateDictInsertMode, uintType,
                                     kStateDictKCursorMode, uintType, kStateDictKKeypadMode, uintType,
                                     kStateDictMouseStandardMode, uintType, kStateDictMouseButtonMode, uintType,
                                     kStateDictMouseAnyMode, uintType, kStateDictMouseUTF8Mode, uintType,
                                     kStateDictMouseSGRMode, uintType, kStateDictWrapMode, uintType,
                                     kStateDictScrollRegionUpper, uintType, kStateDictScrollRegionLower,
                                     paneIdNumberType, kStateDictPaneId, intlistType, kStateDictTabstops, nil];
    ...
    NSMutableDictionary *result = [NSMutableDictionary dictionaryWithCapacity:fields.count];
    for (NSString *kvp in fields) {
        ...
        if (eq.location != NSNotFound) {
            ...
            DLog(@"Bogus result in control command: \"%@\"", kvp);
        }
    }
    return result;
}
...
- (NSMutableDictionary *)parsedStateFromString:(NSString *)stateLines
    ...
    return [[NSMutableDictionary alloc] initWithCapacity:8];
}
