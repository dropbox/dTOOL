...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)draggingInfo {
    ...
    if (theString) {
        ...
        // BUG-1024: Handle plist parsing errors and validate types
        NSError *error = nil;
        id plist = [NSPropertyListSerialization propertyListWithData:data
                                                             options:NSPropertyListImmutable
                                                              format:nil
                                                               error:&error];
        if (error) {
            DLog(@"Failed to parse property list from pasteboard: %@", error);
            return YES;
        }
        // Validate that plist is an array with string elements
        NSArray *filenames = nil;
        if ([plist isKindOfClass:[NSArray class]]) {
            filenames = (NSArray *)plist;
        }
        if (filenames.count > 0 && [filenames[0] isKindOfClass:[NSString class]]) {
