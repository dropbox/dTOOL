...
- (nullable instancetype)initWithComponent:(id<iTermStatusBarComponent>)component {
    ...
    if (self) {
        ...
        _backgroundColor = [component.configuration[iTermStatusBarComponentConfigurationKeyKnobValues]
                                                   [iTermStatusBarSharedBackgroundColorKey] colorValue];
        ...
        if ([component statusBarComponentHandlesClicks]) {
            NSClickGestureRecognizer *recognizer =
                [[NSClickGestureRecognizer alloc] initWithTarget:self action:@selector(clickRecognized:)];
...
- (void)updateIconIfNeeded {
    ...
    if (hasIcon) {
        ...
        NSColor *tintColor =
            [self.component statusBarTextColor] ?: [self.component.delegate statusBarComponentDefaultTextColor];
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p frame=%@ component=%@>", self.class, self, NSStringFromRect(self.frame),
                                      self.component];
}
...
- (void)layoutSubviews {
    ...
    _view.frame =
        NSMakeRect([self retinaRound:self.minX],
                   [self retinaRound:(myHeight - viewHeight) / 2 + _component.statusBarComponentVerticalOffset],
                   [self retinaRoundUp:self.preferredWidthForComponentView], [self retinaRoundUp:viewHeight]);
    CGFloat margin = -2;
    _unreadCountView.frame = NSMakeRect(NSMaxX(self.bounds) - NSWidth(_unreadCountView.frame) - margin,
                                        ...
                                        NSWidth(_unreadCountView.frame), NSHeight(_unreadCountView.frame));
}
...
- (nullable NSView *)hitTest:(NSPoint)point {
    ...
    if (NSPointInRect(point, self.frame)) {
        ...
        if (event.type == NSEventTypeLeftMouseUp || event.type == NSEventTypeLeftMouseDown) {
...
- (void)showContextMenuForEvent:(NSEvent *)event {
    ...
    if (![_component statusBarComponentIsInternal]) {
        if ([[_component statusBarComponentKnobs] count]) {
            [menu addItemWithTitle:[NSString stringWithFormat:@"Configure %@",
                                                              [self.component statusBarComponentShortDescription]]
                            action:@selector(configureComponent:)
                     keyEquivalent:@""];
        }
        [menu
            addItemWithTitle:[NSString stringWithFormat:@"Hide %@", [self.component statusBarComponentShortDescription]]
                      action:@selector(hideComponent:)
               keyEquivalent:@""];
    }
    [menu addItemWithTitle:@"Configure Status Bar" action:@selector(configureStatusBar:) keyEquivalent:@""];
    [menu addItemWithTitle:@"Disable Status Bar" action:@selector(disableStatusBar:) keyEquivalent:@""];
    NSDictionary<NSString *, id> *values = [self.component statusBarComponentKnobValues];
    __block BOOL haveAddedSeparator = NO;
    [[self.component statusBarComponentKnobs]
        enumerateObjectsUsingBlock:^(iTermStatusBarComponentKnob *_Nonnull knob, NSUInteger idx, BOOL *_Nonnull stop) {
            if (knob.type == iTermStatusBarComponentKnobTypeCheckbox) {
                if (!haveAddedSeparator) {
                    haveAddedSeparator = YES;
                    [menu addItem:[NSMenuItem separatorItem]];
                }
                NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:knob.labelText
                                                              action:@selector(toggleKnob:)
                                                       keyEquivalent:@""];
                item.identifier = knob.key;
                item.state =
                    [[NSNumber castFrom:values[knob.key]] boolValue] ? NSControlStateValueOn : NSControlStateValueOff;
                [menu addItem:item];
            }
        }];
...
- (void)toggleKnob:(NSMenuItem *)sender {
    ...
    NSMutableDictionary *knobValues =
        [[self.component statusBarComponentKnobValues] mutableCopy] ?: [[NSMutableDictionary alloc] initWithCapacity:8];
