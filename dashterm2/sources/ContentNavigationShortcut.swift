...
class ContentNavigationShortcut: NSObject {
    ...
    // BUG-404: Use optional instead of implicitly unwrapped optional to prevent crash
    // if action is accessed before init completes or in a subclass scenario
...
class ContentNavigationShortcutView: NSView, ContentNavigationShortcutViewProtocol {
    ...
    required init?(coder: NSCoder) {
        // BUG-f803: Return nil instead of crashing for unused coder initializer
        DLog("ContentNavigationShortcutView init(coder:) is not supported")
        return nil
    }
