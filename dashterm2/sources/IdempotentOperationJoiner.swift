...
class IdempotentOperationJoiner: NSObject {
    ...
    private override init() {
        // BUG-f853, BUG-f689: This init is unavailable - use asyncJoiner(schedulers:) or joiner(scheduler:) instead
        // Log error but initialize with no-op scheduler to prevent crash
        DLog("BUG-f853: IdempotentOperationJoiner.init() should never be called directly - using no-op scheduler")
        self.scheduler = { _ in }
    }
