...
- (instancetype)init {
    self = [super init];
    if (self) {
        _argToExpression = [[NSMutableDictionary alloc] initWithCapacity:8]; // Function call arguments
        _evaluators = [[NSMutableArray alloc] initWithCapacity:8];           // Expression evaluators
    }
    return self;
}
...
- (NSString *)signature {
    return iTermFunctionSignatureFromNamespaceAndNameAndArguments(self.namespace, self.name, _argToExpression.allKeys);
}
...
+ (iTermParsedExpression *)callFunction:(NSString *)invocation
    ...
    dispatch_once(&onceToken, ^{
        array = [[NSMutableArray alloc] initWithCapacity:16]; // Retained expressions
    });
...
+ (iTermParsedExpression *)callFunction:(NSString *)invocation
    ...
    switch (expression.expressionType) {
        ...
        case iTermParsedExpressionTypeArrayLookup: {
            // BUG-1716: Replace assert(false) with proper error handling
            NSString *reason = @"Expected a function call, not a variable reference or array lookup";
            completion(nil,
    ...
    // BUG-1717: Replace assert(NO) with proper error handling for unexpected expression types
    NSString *reason = [NSString stringWithFormat:@"Unexpected expression type: %ld", (long)expression.expressionType];
    completion(nil,
               [NSError errorWithDomain:@"com.dashterm.dashterm2.call"
                                   code:7
                               userInfo:@{NSLocalizedDescriptionKey : reason}],
...
- (void)functionCallDidTimeOutAfter:(NSTimeInterval)timeout
    ...
    if (_remainingArgs.count) {
        reason = [NSString stringWithFormat:@"Timeout (%@ sec) while evaluating invocation “%@”. The timeout occurred "
                                            @"while evaluating the following arguments to %@: %@",
                                            @(timeout), invocation, self.fullyQualifiedName,
                                            [_remainingArgs.allObjects componentsJoinedByString:@", "]];
    } else {
        reason = [NSString stringWithFormat:@"Timeout (%@ sec) while evaluating invocation “%@”. The timeout occurred "
                                            @"while waiting for %@ to return.",
    ...
    iTermScriptHistoryEntry *entry =
        [[iTermAPIHelper sharedInstance] scriptHistoryEntryForConnectionKey:self.connectionKey];
    [entry addOutput:[reason stringByAppendingString:@"\n"]
          completion:^{
          }];

    NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.call" code:2 userInfo:userInfo];
...
- (void)performFunctionCallFromInvocation:(NSString *)invocation
    ...
    if (timeout > 0) {
        ...
        __weak __typeof(self) weakSelf = self;
        timer = [NSTimer it_scheduledTimerWithTimeInterval:timeout
                                                   repeats:NO
                                                     block:^(NSTimer *_Nonnull theTimer) {
                                                         if (timer == nil) {
                                                             // Shouldn't happen
                                                             return;
                                                         }
    ...
    [self callWithScope:scope
     ...
     sideEffectsAllowed:sideEffectsAllowed
             completion:^(id output, NSError *error, NSSet<NSString *> *missing) {
                 if (timeout > 0) {
                     // Not synchronous
                     if (timer == nil) {
                         // Already timed out
...
- (void)callWithScope:(iTermVariableScope *)scope
   ...
   sideEffectsAllowed:(BOOL)sideEffectsAllowed
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        outstandingCalls = [[NSMutableArray alloc] initWithCapacity:32]; // Outstanding function calls
    });
    ...
    [self evaluateParametersWithScope:scope
                   ...
                   sideEffectsAllowed:sideEffectsAllowed
                           completion:^(NSDictionary<NSString *, id> *parameterValues, NSError *depError,
...
- (void)didEvaluateParametersWithScope:(iTermVariableScope *)scope
    ...
    if (receiver) {
        iTermCallMethodByIdentifier(receiver, self.fullyQualifiedName, parameterValues, ^(id object, NSError *error) {
        ...
        NSError *error = [NSError errorWithDomain:iTermAPIHelperErrorDomain
                                         ...
                                         userInfo:@{
                                             NSLocalizedDescriptionKey : [NSString
                                                 stringWithFormat:@"Undefined function with signature “%@”", signature]
                                         }];
        completion(nil, error, nil);
        return;
    }
...
- (void)evaluateParametersWithScope:(iTermVariableScope *)scope
                        ...
                        synchronous:(BOOL)synchronous
                 sideEffectsAllowed:(BOOL)sideEffectsAllowed
                         completion:(void (^)(NSDictionary<NSString *, id> *parameterValues, NSError *depError,
                                              NSSet<NSString *> *missing))completion {
    dispatch_group_t group = dispatch_group_create();
    NSMutableDictionary<NSString *, id> *parameterValues =
        [[NSMutableDictionary alloc] initWithCapacity:_argToExpression.count];
    NSMutableSet<NSString *> *missing = [[NSMutableSet alloc] initWithCapacity:4]; // Missing parameter names
    __block NSError *depError = nil;
    __weak __typeof(self) weakSelf = self;
    // BUG-3762: Track whether we've already called completion to prevent double-calls
    __block BOOL completionCalled = NO;
    // BUG-3762: Track the stop flag separately since we can't access *stop from async callbacks
    __block BOOL shouldStop = NO;
    [_argToExpression
        enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key, iTermParsedExpression *_Nonnull parsedExpression,
                                            BOOL *_Nonnull stop) {
            // BUG-3762: Don't enter group or start evaluation if we're already stopping
            if (shouldStop) {
                return;
            }
                                    ...
                                    sideEffectsAllowed:sideEffectsAllowed
                                            completion:^(NSError *error, id value, NSSet<NSString *> *innerMissing) {
                                                ...
                                                if (error) {
                                                    ...
                                                    // BUG-3762: Only update values if we haven't stopped
                                                    parameterValues[key] = value ?: [NSNull null];
                                                }
                                                dispatch_group_leave(group);
                                            }];
            // BUG-3762: Check if async completion already set shouldStop
            if (shouldStop) {
                *stop = YES;
            }
        }];
    if (synchronous) {
        // BUG-3762: Ensure we only call completion once
        if (!completionCalled) {
...
- (void)evaluateArgumentWithParsedExpression:(iTermParsedExpression *)parsedExpression
                          ...
                          sideEffectsAllowed:(BOOL)sideEffectsAllowed
                                  completion:
                                      (void (^)(NSError *error, id value, NSSet<NSString *> *missing))completion {
    iTermExpressionEvaluator *parameterEvaluator =
        [[iTermExpressionEvaluator alloc] initWithParsedExpression:parsedExpression invocation:invocation scope:scope];
...
- (NSError *)errorForDependentCall:(iTermScriptFunctionCall *)call
    ...
    if (error.code == iTermAPIHelperErrorCodeUnregisteredFunction &&
        [error.domain isEqualToString:iTermAPIHelperErrorDomain]) {
        return error;
    }
    ...
    if (error.userInfo[iTermAPIHelperFunctionCallErrorUserInfoKeyConnection]) {
        userInfo =
            [userInfo dictionaryBySettingObject:error.userInfo[iTermAPIHelperFunctionCallErrorUserInfoKeyConnection]
                                         forKey:iTermAPIHelperFunctionCallErrorUserInfoKeyConnection];
    ...
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.call" code:1 userInfo:userInfo];
}
