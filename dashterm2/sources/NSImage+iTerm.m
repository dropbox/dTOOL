...
+ (NSImage *)it_imageForColorSwatch:(NSColor *)color size:(NSSize)size {
    ...
    NSColor *borderColor =
        [match isEqualToString:NSAppearanceNameDarkAqua] ? [NSColor whiteColor] : [NSColor blackColor];
...
// it.
+ (CGFloat)systemScale {
    NSImage *image = [NSImage imageOfSize:NSMakeSize(1, 1)
                                drawBlock:^{
                                }];
...
+ (NSSize)pointSizeOfGeneratedImageWithPixelSize:(NSSize)pixelSize {
    ...
    NSImage *test = [NSImage imageOfSize:NSMakeSize(1, 1)
                               drawBlock:^{
    ...
    NSSize pointSize = {.width = pixelSize.width / scale, .height = pixelSize.height / scale};
    DLog(@"Pixel size %@ -> point size %@ because %@", NSStringFromSize(pixelSize), NSStringFromSize(pointSize), test);
    return pointSize;
}
...
- (NSImage *)it_imageFillingSize:(NSSize)size {
    ...
    return [NSImage imageOfSize:size
                      drawBlock:^{
...
+ (instancetype)imageWithRawData:(NSData *)data
    ...
    // BUG-f1358: Replace assert() with guard - mismatched data length should return nil, not crash
    const NSUInteger expectedLength = bytesPerRow * size.height;
    if (data.length != expectedLength) {
        ELog(@"imageWithRawData: data.length %lu != expected %lu", (unsigned long)data.length,
...
+ (instancetype)imageWithRawData:(NSData *)data
    ...
    // BUG-f1359: Replace assert() with guard - mismatched data length should return nil, not crash
    const NSUInteger expectedLength = (NSUInteger)(size.width * size.height * bitsPerSample * samplesPerPixel / 8);
    if (data.length != expectedLength) {
        ELog(@"imageWithRawData:size:scaledSize: data.length %lu != expected %lu", (unsigned long)data.length,
             (unsigned long)expectedLength);
        return nil;
    }
    NSBitmapImageRep *bitmapImageRep =
...
+ (NSImage *)it_imageForSymbolName:(NSString *)name
          accessibilityDescription:(NSString *)accessibilityDescription NS_AVAILABLE_MAC(10_16) {
...
- (NSImage *)blurredImageWithRadius:(int)radius {
    ...
    for (int i = 0; i < radius; i++) {
        [image lockFocus];
        [self drawInRect:frame fromRect:frame operation:NSCompositingOperationSourceOver fraction:1];
        [image unlockFocus];
...
- (NSImage *)onePixelBoxBlurOfImage:(NSImage *)image alpha:(CGFloat)alpha {
    ...
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            [image drawInRect:NSMakeRect(dx, dy, size.width, size.height)
...
+ (CGContextRef)newBitmapContextOfSize:(NSSize)size storage:(NSMutableData *)data {
    ...
    CGContextRef context = CGBitmapContextCreate((void *)data.bytes, size.width, size.height, 8, bytesPerRow,
...
- (NSImage *)imageWithColor:(NSColor *)color {
    ...
    NSMutableData *data = [NSMutableData dataWithCapacity:(NSUInteger)(size.width * size.height * 4)];
...
// Using initWithCGImage: instead of -bitmapImageRep preserves the original image's
// color space and pixel format, which is preferred for file export.
...
- (NSData *)rawPixelsInRGBColorSpace {
    // RGBA = 4 bytes per pixel
    NSMutableData *storage = [NSMutableData dataWithCapacity:(NSUInteger)(self.size.width * self.size.height * 4)];
...
- (NSImage *)safelyResizedImageWithSize:(NSSize)unsafeSize
                        destinationRect:(NSRect)destinationRect
                                  scale:(CGFloat)scale {
    DLog(@"safelyResizedImageWithSize:%@ destinationRect:%@ scale:%@", NSStringFromSize(unsafeSize),
         NSStringFromRect(destinationRect), @(scale));
    ...
    const NSRect scaledDestinationRect =
        NSMakeRect(NSMinX(destinationRect) * scale, NSMinY(destinationRect) * scale, NSWidth(destinationRect) * scale,
                   NSHeight(destinationRect) * scale);
    DLog(@"scaledDestinationRect=%@", NSStringFromRect(scaledDestinationRect));
    [self drawInRect:scaledDestinationRect fromRect:NSZeroRect operation:NSCompositingOperationCopy fraction:1.0];
    [NSGraphicsContext restoreGraphicsState];

    NSImage *newImage = [[NSImage alloc] initWithSize:NSMakeSize(newSize.width / scale, newSize.height / scale)];
...
- (NSImage *)it_imageScaledByX:(CGFloat)xScale y:(CGFloat)yScale {
    ...
    const NSSize destSize =
        NSMakeSize(self.size.width * fabs(xScale) * adjustment, self.size.height * fabs(yScale) * adjustment);
...
- (NSImage *)it_imageOfSize:(NSSize)newSize {
    ...
    return [NSImage imageOfSize:newSize
                      drawBlock:^{
...
static NSBitmapImageRep *iTermCreateBitmapRep(NSSize size, NSImage *sourceImage) {
    NSBitmapImageRep *rep = [[NSBitmapImageRep alloc] initWithBitmapDataPlanes:NULL
                                                                    pixelsWide:size.width
                                                                    pixelsHigh:size.height
                                                                 bitsPerSample:8
...
+ (NSImage *)it_imageWithScaledBitmapFromFile:(NSString *)file pointSize:(NSSize)pointSize {
    ...
    const NSSize retinaPixelSize = NSMakeSize(pointSize.width * 2, pointSize.height * 2);
...
- (NSImage *)grayscaleImage {
    ...
    CGContextRef context = CGBitmapContextCreate(nil, size.width, size.height, 8, 0, colorSpace, 0);
    ...
    // BUG-2764: Use CGImageRelease for CGImageRef instead of CFRelease for clarity
...
- (NSImage *)it_subimageWithRect:(NSRect)rect {
    ...
    NSImageRep *representation = [self bestRepresentationForRect:bounds context:nil hints:nil];
...
- (MTLPixelFormat)metalPixelFormat {
    const MTLPixelFormat unsupportedFormatsMask =
        (NSBitmapFormatAlphaNonpremultiplied | NSBitmapFormatSixteenBitBigEndian | NSBitmapFormatThirtyTwoBitBigEndian |
...
- (NSBitmapImageRep *)it_bitmapWithAlphaLast {
    ...
    NSMutableData *storage =
        [NSMutableData dataWithLength:bytesPerSample * samplesPerPixel * self.pixelsWide * self.pixelsHigh];
    ...
    for (NSUInteger i = 0; i < self.bytesPerRow * self.pixelsHigh; i += stride) {
        ...
        memmove(dest, source + i + bytesPerSample, nonAlphaLength);
        // Now move alpha.
        memmove(dest + nonAlphaLength, source + i, bytesPerSample);
    }
