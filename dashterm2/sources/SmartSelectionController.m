...
@interface SmartSelectionController () <NSTableViewDataSource, NSTableViewDelegate, NSTextFieldDelegate,
                                        iTermPlaygroundTextViewDelegate, NSMenuItemValidation>
@end
...
+ (NSArray<NSString *> *)precisionKeys {
    return @[ kVeryLowPrecision, kLowPrecision, kNormalPrecision, kHighPrecision, kVeryHighPrecision ];
}
...
- (void)awakeFromNib {
    ...
    // BUG-1193: Limit undo levels to avoid unbounded memory growth
    _undoManager.levelsOfUndo = 20;
...
+ (NSArray *)defaultRules {
    ...
    if (!rulesArray) {
        ...
        ITCriticalError(rulesDict != nil, @"Failed to parse SmartSelectionRules in %@: %@", plistFile,
                        [NSString stringWithContentsOfFile:plistFile encoding:NSUTF8StringEncoding error:nil]);
...
+ (double)precisionInRule:(NSDictionary *)rule {
    NSDictionary *precisionValues = @{
        kVeryLowPrecision : @(SmartSelectionVeryLowPrecision),
...
- (NSDictionary *)defaultRule {
    return @{kRegexKey : @"", kPrecisionKey : kVeryLowPrecision};
}
...
- (void)setRule:(NSDictionary *)rule forRow:(NSInteger)rowIndex {
    ...
    if (rowIndex < 0) {
        // BUG-f1365: Convert assert to guard - nil rule with negative index should be no-op
        if (!rule) {
            return;
        }
        ...
        // BUG-1252c: Bounds check before accessing array at rowIndex
        if (rowIndex >= (NSInteger)rules.count) {
            return;
        }
        if (rule) {
    ...
    Profile *bookmark = [self bookmark];
    [iTermProfilePreferences setObject:rules
                                forKey:KEY_SMART_SELECTION_RULES
                             inProfile:bookmark
                                 model:[self modelForBookmark:bookmark]
                       withSideEffects:NO];
    if (rowIndex < 0) {
...
- (IBAction)openRegexVisualizer:(NSButton *)button {
    if (!_popover || !_popover.isShown) {
        [_popover close];
        _visualizationViewController =
            [[iTermRegexVisualizationViewController alloc] initWithRegex:_regexTextView.textStorage.string ?: @""
...
- (IBAction)syntaxHelp:(id)sender {
    [[NSWorkspace sharedWorkspace]
        it_openURL:[NSURL URLWithString:@"https://unicode-org.github.io/icu/userguide/strings/regexp.html"]
            target:nil
...
- (IBAction)addRule:(id)sender {
    ...
    [tableView_ selectRowIndexes:[NSIndexSet indexSetWithIndex:tableView_.numberOfRows - 1] byExtendingSelection:NO];
}
...
- (NSString *)precisionKeyWithIndex:(int)i {
    // BUG-f1045: Guard against index out of bounds
    NSArray<NSString *> *keys = [SmartSelectionController precisionKeys];
    if (i < 0 || i >= (int)keys.count) {
        DLog(@"BUG-f1045: precisionKeyWithIndex called with invalid index %d (count=%lu)", i,
             (unsigned long)keys.count);
        return keys.count > 0 ? keys[0] : kNormalPrecision; // Fall back to first key or normal precision
    }
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
    ...
    // BUG-f1061: Bounds check before accessing rules array to prevent crash during table view updates
    if (row < 0 || row >= (NSInteger)rules.count) {
        return 24; // Return default row height
    }
    NSDictionary *rule = rules[row];
    NSAttributedString *attributedString = [self attributedStringForRule:rule];
    // BUG-10357: Bounds check before accessing tableColumns array
...
- (NSView *)tableView:(NSTableView *)tableView
    ...
    NSArray<NSDictionary *> *rules = self.rules;
    // BUG-f1062: Bounds check before accessing rules array to prevent crash during table view updates
    if (rowIndex < 0 || rowIndex >= (NSInteger)rules.count) {
        return nil;
    }
    NSDictionary *rule = rules[rowIndex];
    iTermTableCellViewWithTextField *view =
        [tableView newTableCellViewWithTextFieldUsingIdentifier:@"Smart Selection Tableview Entry"
...
- (NSAttributedString *)attributedStringForRule:(NSDictionary *)rule {
    ...
    NSAttributedString *precisionAttributedString =
        [self precisionAttributedString:rule[kPrecisionKey] actionCount:[[NSArray castFrom:rule[kActionsKey]] count]];
...
- (NSAttributedString *)attributedStringForRegex:(NSString *)regex {
    ...
    return [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"/%@/", regex] attributes:attributes];
}

- (IBAction)changePrecision:(NSPopUpButton *)sender {
    [self pushUndo];
    const NSInteger rowIndex = tableView_.selectedRow;
    // BUG-10358: Bounds check before accessing rules array
    NSArray<NSDictionary *> *rules = self.rules;
    if (rowIndex < 0 || rowIndex >= (NSInteger)rules.count) {
        return;
    }
    const NSInteger index = sender.indexOfSelectedItem;
    NSMutableDictionary *rule = [rules[rowIndex] mutableCopy];
...
- (void)updateDetailView {
    ...
    if (self.hasSelection) {
        ...
        NSArray<NSDictionary *> *rules = self.rules;
        // BUG-10359: Bounds check before accessing rules array
        if (row < 0 || row >= (NSInteger)rules.count) {
            _detailView.hidden = YES;
            _noRuleSelected.hidden = NO;
            return;
        }
        NSDictionary *rule = rules[row];
...
- (IBAction)editActions:(id)sender {
    ...
    [self.window beginSheet:contextMenuPrefsController_.window
...
- (void)contextMenuActionsChanged:(NSArray *)newActions useInterpolatedStrings:(BOOL)useInterpolatedStrings {
    if (self.hasSelection) {
        ...
        // BUG-396: Bounds check before accessing rules array to prevent crash
        if (rowIndex < 0 || rowIndex >= (int)self.rules.count) {
...
- (void)save {
    ...
    NSArray<NSDictionary *> *rules = self.rules;
    // BUG-10360: Bounds check before accessing rules array
    if (rowIndex >= (NSInteger)rules.count) {
        return;
    }
    [self pushUndo];
...
- (void)updatePlayground {
    ...
    iTermTextExtractor *extractor =
        [[iTermTextExtractor alloc] initWithDataSource:_playgroundTextView.textStorage.string ?: @""];
    ...
    [_playgroundTextView
        highlightGridRange:VT100GridCoordRangeMake(result.startX, result.absStartY, result.endX, result.absEndY)];
}
...
- (void)pushUndo {
    [_undoManager registerUndoWithTarget:self selector:@selector(setRules:) object:self.rules];
}

- (void)setRules:(NSArray<NSDictionary *> *)rules {
    Profile *profile = [self bookmark];
    [[self modelForBookmark:profile] setObject:rules forKey:KEY_SMART_SELECTION_RULES inBookmark:profile];
    [self reloadData];
    [delegate_ smartSelectionChanged:nil];
}
...
- (nonnull ScreenCharArray *)screenCharArrayForLine:(int)line {
    ...
    // BUG-10361: Bounds check before accessing lines array
    if (line < 0 || line >= (int)lines.count) {
