...
class ChatSearchResultsViewController: NSViewController {
    ...
    required init?(coder: NSCoder) {
        // BUG-f835: Return nil instead of crashing for unused coder initializer
        DLog("ChatSearchResultsViewController init(coder:) is not supported")
        return nil
    }
...
extension ChatSearchResultsViewController: NSTableViewDataSource {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        // BUG-f1022: Bounds check before configuring cell
        // Row index can be stale if searchResults was modified between numberOfRows and this call
        guard row >= 0 && row < searchResults.count else {
            DLog("ChatSearchResultsViewController tableView(_:viewFor:row:): row \(row) out of bounds for count \(searchResults.count)")
            return nil
        }
    ...
    private func configure(cell: ChatSearchResultCell, row: Int) {
        // BUG-f1022: Bounds check - caller should have checked, but defend in depth
        guard row >= 0 && row < searchResults.count else {
            DLog("ChatSearchResultsViewController configure(cell:row:): row \(row) out of bounds for count \(searchResults.count)")
            return
        }
        // BUG-f1022: Also check tableColumns is non-empty before accessing [0]
        let maxWidth = tableView.tableColumns.first?.width ?? 200
...
extension ChatSearchResultsViewController: NSTableViewDelegate {
    func tableView(_ tableView: NSTableView, heightOfRow row: Int) -> CGFloat {
        // BUG-f1022: Bounds check before accessing searchResults
        guard row >= 0 && row < searchResults.count else {
            DLog("ChatSearchResultsViewController tableView(_:heightOfRow:): row \(row) out of bounds for count \(searchResults.count)")
        ...
        // BUG-474: Handle negative height gracefully instead of crashing
        if height < 0 {
            DLog("Warning: Calculated negative row height \(height), using minimum height")
            return 40  // Default row height from tableView.rowHeight
        }
        return height
    }
