...
- (NSAlert *)makeAlert {
    ...
    // BUG-f1335: Replace assert() with guard - silenceable warnings without identifier should not crash
    // Instead, skip the suppression button if identifier is missing
    if (_warningType == kiTermWarningTypeTemporarilySilenceable) {
        if (!_identifier) {
            ELog(@"iTermWarning: Silenceable warning missing identifier - suppression disabled");
        } else {
