...
@interface ProfilePreferencesViewController () <iTermProfilePreferencesBaseViewControllerDelegate, NSTabViewDelegate,
...
- (NSArray *)tabViewTuples {
    return @[
        @[ _generalTab, _generalViewController.view ], @[ _colorsTab, _colorsViewController.view ],
        @[ _textTab, _textViewController.view ], @[ _webTab, _webViewController.view ],
        @[ _windowTab, _windowViewController.view ], @[ _terminalTab, _terminalViewController.view ],
        @[ _sessionTab, _sessionViewController.view ], @[ _keysTab, _keysViewController.view ],
...
- (NSArray *)tabViewControllerTuples {
    return @[
        @[ _generalTab, _generalViewController ], @[ _colorsTab, _colorsViewController ],
        @[ _textTab, _textViewController ], @[ _webTab, _webViewController ], @[ _windowTab, _windowViewController ],
        @[ _terminalTab, _terminalViewController ], @[ _sessionTab, _sessionViewController ],
        @[ _keysTab, _keysViewController ], @[ _advancedTab, _advancedViewController ]
    ];
}
...
- (void)awakeFromNib {
    ...
    for (NSArray *tuple in tabViewTuples) {
        ...
        if (view.frame.size.height > kMaxHeight) {
            ...
            flippedView.frame = NSMakeRect(flippedView.frame.origin.x, flippedView.frame.origin.y,
                                           view.frame.size.width - scrollbarWidth, flippedView.frame.size.height);
...
- (void)openToProfileWithGuidAndEditHotKey:(NSString *)guid scope:(iTermVariableScope<iTermSessionScope> *)scope {
    self.scope = scope;
...
- (BOOL)confirmProfileDeletion:(Profile *)profile {
    NSMutableString *question = [NSMutableString stringWithFormat:@"Delete profile %@?", profile[KEY_NAME]];
...
- (NSArray<iTermProfilePreferencesBaseViewController *> *)tabViewControllers {
    return @[
        _generalViewController, _colorsViewController, _textViewController, _webViewController, _windowViewController,
        _terminalViewController, _sessionViewController, _keysViewController, _advancedViewController
    ];
}
...
- (id<iTermSearchableViewController>)viewControllerWithOwnerIdentifier:(NSString *)ownerIdentifier {
    return [[self tabViewControllers]
        objectPassingTest:^BOOL(id<iTermSearchableViewController> vc, NSUInteger index, BOOL *stop) {
...
- (void)resizeWindowForView:(iTermSizeRememberingView *)theView animated:(BOOL)animated {
    ...
    CGFloat preferredWidth =
        kSideMarginsWithinInnerTabView + theView.originalSize.width + kSideMarginsWithinInnerTabView;
    ...
    if ([NSScroller preferredScrollerStyle] == NSScrollerStyleLegacy) {
        spaceRight += [NSScroller scrollerWidthForControlSize:NSControlSizeRegular scrollerStyle:NSScrollerStyleLegacy];
    }
    ...
    NSSize contentSize =
        NSMakeSize(spaceLeft + tabViewSize.width + spaceRight, spaceAbove + tabViewSize.height + spaceBelow);
    ...
    NSRect frame =
        [window frameRectForContentRect:NSMakeRect(windowTopLeft.x, 0, contentSize.width, contentSize.height)];
...
- (IBAction)addProfile:(id)sender {
    NSMutableDictionary *newDict = [[NSMutableDictionary alloc] initWithCapacity:32]; // New profile dictionary
...
- (IBAction)copyToProfile:(id)sender {
    ...
    NSString *title = [NSString stringWithFormat:@"Replace profile “%@” with the current session's settings?",
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kReloadAllProfiles object:nil userInfo:nil];

    // Update user defaults
    [[NSUserDefaults standardUserDefaults] setObject:[[ProfileModel sharedInstance] rawData] forKey:@"New Bookmarks"];
}
...
- (IBAction)openCopyBookmarks:(id)sender {
    ...
    [self.view.window beginSheet:_bulkCopyController.window
...
- (IBAction)duplicateProfile:(id)sender {
    ...
    if ([iTermAdvancedSettingsModel addDynamicTagToDynamicProfiles]) {
        ...
        NSArray *existingTags = [NSArray castFrom:newProfile[KEY_TAGS]];
        newProfile[KEY_TAGS] = existingTags ? [existingTags arrayByRemovingObject:kProfileDynamicTag] : @[];
    }
...
- (NSString *)jsonForAllProfilesWithErrorCount:(int *)errorCount {
    ...
    for (Profile *profile in _profilesListView.dataSource.profiles) {
        ...
        if (json) {
            ...
            errors++;
            NSLog(@"Couldn't convert profile %@ to JSON: %@", profile[KEY_NAME], [error localizedDescription]);
        }
    }
    [profiles appendString:@"\n]\n}\n"];
    *errorCount = errors;
...
- (IBAction)importJSONProfiles:(id)sender {
    ...
    [openPanel
        beginWithFallbackWindow:self.view.window
                        handler:^(NSModalResponse response, NSArray<NSURL *> *urls) {
                                    ...
                                    if (![self tryToImportJSONProfileFromURL:url error:&error]) {
                                        ...
                                        iTermWarningSelection selection = [iTermWarning
                                            showWarningWithTitle:[NSString
                                                                     stringWithFormat:@"Import from %@ failed: %@",
                                                                                      url.path,
...
- (BOOL)tryToImportJSONProfileFromURL:(NSURL *)url error:(out NSError **)error {
    ...
    if (!dict) {
        if (error) {
            *error = [[NSError alloc]
                initWithDomain:@"com.dashterm.dashterm2.json-import-profile"
                          code:0
                      userInfo:@{NSLocalizedDescriptionKey : @"Invalid JSON: does not have a top-level dictionary."}];
        }
        return NO;
    }
        ...
        if (error) {
            *error = [[NSError alloc]
                initWithDomain:@"com.dashterm.dashterm2.json-import-profile"
                          code:0
                      userInfo:@{
                          NSLocalizedDescriptionKey : @"Invalid JSON: missing Name or Guid key in top-level dictionary."
                      }];
        }
        return NO;
    }
...
- (BOOL)tryToImportMultipleProfilesFromJSONObject:(NSArray *)profiles error:(out NSError **)error {
    NSMutableArray<NSDictionary *> *dicts = [NSMutableArray arrayWithCapacity:profiles.count];
    __block BOOL ok = YES;
    __block NSError *internalError = nil;
    [profiles enumerateObjectsUsingBlock:^(id _Nonnull object, NSUInteger idx, BOOL *_Nonnull stop) {
        NSDictionary *dict = [NSDictionary castFrom:object];
        if (!dict) {
            internalError = [[NSError alloc]
                initWithDomain:@"com.dashterm.dashterm2.json-import-profile"
                          code:0
                      userInfo:@{NSLocalizedDescriptionKey : @"Invalid JSON: not an array of dictionaries."}];
            ok = NO;
            *stop = YES;
            return;
        }
        ...
        if (!name || !guid) {
            NSString *reason =
                [NSString stringWithFormat:@"Invalid JSON: missing Name or Guid key in profile number %@.", @(idx + 1)];
            internalError = [[NSError alloc] initWithDomain:@"com.dashterm.dashterm2.json-import-profile"
                                                       code:0
              ...
              dispatch_async(dispatch_get_main_queue(), ^{
                  if (error) {
                      ...
                      alert.informativeText =
                          [NSString stringWithFormat:@"Couldn't save to “%@” on %@: %@", item.filename,
                                                     item.host.displayName, [error localizedDescription]];
                      [alert runModal];
                  }
...
- (IBAction)saveProfileAsJSON:(id)sender {
    ...
    [iTermSavePanel asyncShowWithOptions:kSavePanelOptionDefaultToLocalhost
                        ...
                        initialDirectory:NSHomeDirectory()
                         defaultFilename:[self.selectedProfile[KEY_NAME] stringByAppendingPathExtension:@"json"]
                                             ?: @"UnknownProfile.json"
...
- (void)reallySaveProfile:(Profile *)profile asJSON:(iTermModernSavePanel *)savePanel {
    ...
    if (!string) {
        ...
        alert.messageText = @"Error";
        alert.informativeText =
            [NSString stringWithFormat:@"Couldn't convert profile to JSON: %@", [error localizedDescription]];
        [alert runModal];
        return;
    }

    [self saveString:string
              toItem:savePanel.item
          completion:^(BOOL ok) {
              if (ok && savePanel.item.host.isLocalhost) {
                  [[NSWorkspace sharedWorkspace]
                      activateFileViewerSelectingURLs:@[ [NSURL fileURLWithPath:savePanel.item.filename] ]];
              }
          }];
}
...
- (void)reallySaveAllProfilesAsJSON:(iTermModernSavePanel *)savePanel {
    ...
    if (errors) {
        ...
        alert.messageText = @"Error";
        alert.informativeText =
            [NSString stringWithFormat:@"Couldn't convert one or more profiles to JSON. Check Console.app for errors."];
        [alert runModal];
        return;
    }
...
- (void)showDynamicProfileWarning {
    ...
    NSString *message = [NSString
        stringWithFormat:@"The selected profile, “%@”, is a dynamic profile. These are generally only edited by "
                         @"hand.\n\nDashTerm2 is now able to write changes back to dynamic profiles when they are "
                         @"marked as “rewritable“. Rewriting can cause the order of values to change.",
                         profileName];
