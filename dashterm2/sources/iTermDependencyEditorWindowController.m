...
- (void)loadPackageAtIndex:(NSInteger)index completion:(void (^)(BOOL ok, NSString *result))completion {
    ...
    [self fetchVersionOfPackage:tuple.firstObject
                     completion:^(BOOL ok, NSString *result) {
                         const BOOL accepted = [weakSelf didFetchVersionOfPackageAtIndex:index
                                                                           originalTuple:tuple
                                                                               container:container
                                                                                      ok:ok
                                                                                  result:result];
...
- (BOOL)didFetchVersionOfPackageAtIndex:(NSInteger)index
    ...
    if (!ok) {
        ...
        DLog(@"Error running pip3 show %@ in %@: %@", tuple.firstObject, container.path, result);
    } else {
...
- (void)didSelectScriptAtIndex:(NSInteger)index {
    ...
    iTermSetupCfgParser *parser = [[iTermSetupCfgParser alloc]
        initWithPath:[_selectedScriptItem.path stringByAppendingPathComponent:@"setup.cfg"]];
...
- (void)loadPythonVersionsSelecting:(NSString *)selectedVersion {
    NSString *const env = [[_selectedScriptItem.path stringByAppendingPathComponent:@"dashterm2env"]
    ...
    NSArray<NSString *> *versions =
        [[[[iTermPythonRuntimeDownloader pythonVersionsAt:env] mapWithBlock:^id(NSString *version) {
...
- (void)populateScripts {
    _scriptItems = [[NSMutableArray alloc] initWithCapacity:16];
    _checkForUpdate.enabled = NO;
    _remove.enabled = NO;
...
- (void)addScriptItems:(NSArray<iTermScriptItem *> *)scriptItems breadcrumbs:(NSArray<NSString *> *)breadcrumbs {
    for (iTermScriptItem *item in [scriptItems sortedArrayUsingComparator:^NSComparisonResult(
                                                   iTermScriptItem *_Nonnull obj1, iTermScriptItem *_Nonnull obj2) {
        ...
        NSMenuItem *menuItem = [[NSMenuItem alloc]
            initWithTitle:[[breadcrumbs arrayByAddingObject:item.name] componentsJoinedByString:@"/"]
                   action:nil
            keyEquivalent:@""];
    ...
    if (_packageTuples[index] == tuple) {
        [self loadPackageAtIndex:index
                      completion:^(BOOL ok, NSString *result) {
                          ...
                          [iTermDependencyEditorWindowController
                              setDependency:[NSString stringWithFormat:@"%@>=%@", tuple.firstObject, result]
...
- (void)runPip3WithArguments:(NSArray<NSString *> *)arguments completion:(void (^)(void))completion {
    NSURL *container = [NSURL fileURLWithPath:_selectedScriptItem.path];
    NSString *pip3 = [[iTermPythonRuntimeDownloader sharedInstance]
               pip3At:[container.path stringByAppendingPathComponent:@"dashterm2env"]
        pythonVersion:_pythonVersion];
    NSString *command = [[pip3 stringWithBackslashEscapedShellCharactersIncludingNewlines:YES]
...
- (void)pip3InstallDidFinish:(NSString *)selectedScriptPath newDependencyName:(NSString *)newDependencyName {
    ...
    __weak __typeof(self) weakSelf = self;
    [self fetchVersionOfPackage:newDependencyName
                     completion:^(BOOL ok, NSString *result) {
                         [weakSelf installDidFinishSuccessfully:ok
                                             selectedScriptPath:selectedScriptPath
                                              newDependencyName:newDependencyName];
                     }];
}

- (void)uninstallDidFailForPackage:(NSString *)package {
    [iTermWarning
        showWarningWithTitle:[NSString stringWithFormat:@"Uninstall of %@ failed. Check the output of pip3 for errors.",
                                                        package]
                     actions:@[ @"OK" ]
                   accessory:nil
...
- (IBAction)upgrade:(id)sender {
    ...
    NSURL *folder = [[[NSURL fileURLWithPath:_selectedScriptItem.path] URLByDeletingLastPathComponent]
    ...
    __weak __typeof(self) weakSelf = self;
    NSString *pythonVersion = [iTermAPIScriptLauncher inferredPythonVersionFromScriptAt:item.path];
    [[iTermPythonRuntimeDownloader sharedInstance]
                        ...
                        completion:^(NSError *errorStatus) {
                            if (errorStatus != nil) {
                                ...
                                alert.informativeText = [NSString
                                    stringWithFormat:
                                        @"Please file a bug report at https://github.com/ayates_dbx/dashterm2/issues. "
                                        @"The following error occurred while upgrading a dependency: %@",
...
- (void)finishUpgradingScriptItem:(iTermScriptItem *)item toFullEnvironmentAt:(NSURL *)url {
    NSFileManager *fileManager = [NSFileManager defaultManager];

    // Create Scripts/Foo/Foo
    NSString *innerFolder = [url.path stringByAppendingPathComponent:url.path.lastPathComponent];
    NSError *error = nil;
    [fileManager createDirectoryAtPath:innerFolder withIntermediateDirectories:YES attributes:nil error:&error];
    if (error) {
        NSAlert *alert = [[NSAlert alloc] init];
        alert.messageText = @"Installation Failed";
        alert.informativeText =
            [NSString stringWithFormat:@"Error creating %@: %@", innerFolder, error.localizedDescription];
        [alert runModal];
        return;
    }
    ...
    [fileManager moveItemAtPath:item.path toPath:destination error:&error];
    if (error) {
        NSAlert *alert = [[NSAlert alloc] init];
        alert.messageText = @"Installation Failed";
        alert.informativeText = [NSString
            stringWithFormat:@"Error moving %@ to %@: %@", item.path, destination, error.localizedDescription];
        [alert runModal];
        return;
    }

    [self populateScripts];
    NSInteger index = [_scriptItems
        indexOfObjectPassingTest:^BOOL(iTermScriptItem *_Nonnull item, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (IBAction)checkForUpdates:(id)sender {
    ...
    // BUG-381: Also check bounds against _packageTuples array
    if (index < 0 || index >= (NSInteger)_packageTuples.count || !_selectedScriptItem) {
        return;
    }
    iTermScriptItem *selectedScriptItem = _selectedScriptItem;
    // BUG-381: Use cached index instead of reading selectedRow again to avoid race condition
    iTermTuple<NSString *, NSString *> *tuple = _packageTuples[index];
    NSString *selectedDependencyName = tuple.firstObject;
    __weak __typeof(self) weakSelf = self;
    [self runPip3WithArguments:@[ @"install", selectedDependencyName, @"--upgrade" ]
                    completion:^{
                        [weakSelf pip3UpgradeDidFinish:tuple index:index scriptPath:selectedScriptItem.path];
                    }];
}
...
- (void)install:(NSString *)newDependencyName
    selectedScriptPath:(NSString *)selectedScriptPath
            completion:(void (^)(void))completion {
    __weak __typeof(self) weakSelf = self;
    [self runPip3WithArguments:@[ @"install", newDependencyName ]
                    completion:^{
...
- (IBAction)remove:(id)sender {
    const NSInteger index = _tableView.selectedRow;
    // BUG-381: Also check bounds against _packageTuples array
    if (index < 0 || index >= (NSInteger)_packageTuples.count) {
        return;
    }
    // BUG-381: Use cached index instead of reading selectedRow again to avoid race condition
    iTermTuple<NSString *, NSString *> *tuple = _packageTuples[index];
    ...
    iTermScriptItem *selectedScriptItem = _selectedScriptItem;
    __weak __typeof(self) weakSelf = self;
    [self runPip3WithArguments:@[ @"uninstall", selectedDependencyName ]
                    completion:^{
...
- (IBAction)pythonVersionChanged:(id)sender {
    ...
    if ([comparator compareVersion:selectedVersion toVersion:_pythonVersion] == NSOrderedAscending) {
        iTermWarning *warning = [[iTermWarning alloc] init];
        warning.title = @"You have asked to downgrade to an older Python version. Dependencies will need to be "
                        @"reinstalled. This may go badly. Are you sure you want to do this?";
        ...
        iTermWarning *warning = [[iTermWarning alloc] init];
        warning.title = @"You have asked to upgrade to a newer Python version. Dependencies will need to be "
                        @"reinstalled. OK to continue?";
...
- (void)installPackages:(NSArray<NSString *> *)packages selectedScriptPath:(NSString *)selectedScriptPath {
    ...
    [self install:newDependencyName
        selectedScriptPath:selectedScriptPath
                completion:^{
