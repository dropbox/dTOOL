...
extension TextViewPorthole.VisualAttributes {
    ...
    var themeString: String {
        ...
        let subs = [
            ...
            // BUG-1660: Use nil coalescing chain instead of force unwrap for font
            "${fontFamily}": font.familyName ?? NSFont.userFixedPitchFont(ofSize: 12)?.familyName ?? "Menlo",
        ]
...
class TextViewPortholeRenderer {
    ...
    func render(visualAttributes: TextViewPorthole.VisualAttributes) -> NSAttributedString {
        ...
        // BUG-1699: Use guard instead of force unwrap for Highlightr()
        ...
        if let type = type,
           let languages = FileExtensionDB.instance?.languagesForTypeHint(type) {
            ...
            if languages.count == 1, let lang = languages.first {
                // BUG-1630: Use safe optional binding instead of force unwrap
                language = lang
            }
            return
        }
