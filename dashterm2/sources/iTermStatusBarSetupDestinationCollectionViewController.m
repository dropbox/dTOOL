...
@interface iTermStatusBarSetupDestinationCollectionViewController () <
    iTermStatusBarSetupElementDelegate, NSCollectionViewDataSource, NSCollectionViewDelegateFlowLayout>
...
- (void)awakeFromNib {
    _elements = [NSMutableArray arrayWithCapacity:16]; // Status bar destination elements
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[iTermStatusBarSetupCollectionViewItem class] forItemWithIdentifier:@"element"];
}
...
- (void)deleteItemAtIndex:(NSInteger)index {
    NSRect rectInWindowCoords = [self.collectionView convertRect:[self.collectionView frameForItemAtIndex:index]
                                                          toView:nil];
    NSRect rect = [self.collectionView.window convertRectToScreen:rectInWindowCoords];
    NSShowAnimationEffect(NSAnimationEffectPoof, NSMakePoint(NSMidX(rect), NSMidY(rect)), NSZeroSize, nil, nil, NULL);
    [[[self.collectionView itemAtIndex:index] view] setHidden:YES];
    [self.collectionView.animator
        performBatchUpdates:^{
...
- (void)updateAutoRainbowColors {
    ...
    iTermStatusBarAutoRainbowController *controller =
        [[iTermStatusBarAutoRainbowController alloc] initWithStyle:_advancedConfiguration.autoRainbowStyle];
    controller.darkBackground = self.darkBackground;
    _updatingAutoRainbowColors = YES;
    [controller enumerateColorsWithCount:_elements.count
                                   block:^(NSInteger i, NSColor *color) {
                                       id<iTermStatusBarComponent> component = _elements[i].component;
                                       NSMutableDictionary *knobValues =
                                           [component.configuration[iTermStatusBarComponentConfigurationKeyKnobValues]
                                               mutableCopy];
...
- (void)setAdvancedConfiguration:(iTermStatusBarAdvancedConfiguration *)advancedConfiguration {
    ...
    iTermStatusBarLayout *temporaryLayout = [[iTermStatusBarLayout alloc] initWithDictionary:[self layoutDictionary]
                                                                                       scope:nil];
...
- (void)loadElementsFromLayout:(iTermStatusBarLayout *)layout {
    ...
    [layout.components enumerateObjectsUsingBlock:^(id<iTermStatusBarComponent> _Nonnull component, NSUInteger idx,
                                                    BOOL *_Nonnull stop) {
...
- (NSDictionary *)layoutDictionary {
    NSArray<id<iTermStatusBarComponent>> *components =
        [_elements mapWithBlock:^id(iTermStatusBarSetupElement *element) {
...
- (void)configureStatusBarComponentWithIdentifier:(NSString *)identifier {
    NSInteger index = [_elements indexOfObjectPassingTest:^BOOL(iTermStatusBarSetupElement *_Nonnull element,
    ...
    if (_configureButton.enabled) {
        [_configureButton.target it_performNonObjectReturningSelector:_configureButton.action
                                                           withObject:_configureButton];
    }
}
...
- (iTermStatusBarSetupCollectionViewItem *)newItemWithIndexPath:(NSIndexPath *)indexPath {
    iTermStatusBarSetupCollectionViewItem *item = [self.collectionView makeItemWithIdentifier:@"element"
...
- (void)initializeItem:(iTermStatusBarSetupCollectionViewItem *)item atIndexPath:(NSIndexPath *)indexPath {
    ...
    item.textField.attributedStringValue =
        [_elements[index] exemplarWithBackgroundColor:_advancedConfiguration.backgroundColor
...
- (NSSize)collectionView:(NSCollectionView *)collectionView
    ...
    iTermStatusBarSetupCollectionViewItem *item =
        [[iTermStatusBarSetupCollectionViewItem alloc] initWithNibName:@"iTermStatusBarSetupCollectionViewItem"
...
- (void)collectionView:(NSCollectionView *)collectionView
    ...
    NSPoint windowPoint =
        [collectionView.window convertRectFromScreen:NSMakeRect(screenPoint.x, screenPoint.y, 0, 0)].origin;
...
- (id<NSPasteboardWriting>)collectionView:(NSCollectionView *)collectionView
    ...
    [pbItem setData:[NSData it_dataWithArchivedObject:_elements[index]] forType:iTermStatusBarElementPasteboardType];
    return pbItem;
}
...
- (NSImage *)collectionView:(NSCollectionView *)collectionView
    ...
    iTermStatusBarSetupCollectionViewItem *item =
        [iTermStatusBarSetupCollectionViewItem castFrom:[collectionView itemAtIndexPath:indexPaths.anyObject]];
    // BUG-f1275: Replace assert() with guard - nil item should return nil image, not crash
    if (item == nil) {
        DLog(@"WARNING: BUG-f1275 draggingImageForItemsAtIndexPaths got nil item for index paths %@", indexPaths);
        return nil;
    }
    NSPoint locationInItem = [item.view convertPoint:locationInWindow fromView:nil];
    NSPoint center = NSMakePoint(item.view.frame.size.width / 2, item.view.frame.size.height / 2);
    *dragImageOffset = NSMakePoint(center.x - locationInItem.x, center.y - locationInItem.y);
...
- (NSDragOperation)collectionView:(NSCollectionView *)collectionView
    ...
    if (draggingInfo.draggingSource != collectionView && draggingInfo.draggingSource != _sourceCollectionView) {
...
- (BOOL)collectionView:(NSCollectionView *)collectionView
    ...
    // Use it_unarchivedObjectOfClasses which creates its own secure unarchiver
    iTermStatusBarSetupElement *element = [data it_unarchivedObjectOfClasses:@[ [iTermStatusBarSetupElement class] ]];
    if (!element) {
        DLog(@"Reject drop: failed to decode element");
        return NO;
    }
    element.delegate = self;
    [collectionView.animator
        performBatchUpdates:^{
            if (draggingInfo.draggingSource == collectionView) {
                ...
                DLog(@"Move element from %@ to %@", @(fromIndex), @(toIndex));
                if (fromIndex >= toIndex) {
                    [self->_elements removeObjectAtIndex:fromIndex];
                    [collectionView moveItemAtIndexPath:self->_draggingIndexPath toIndexPath:indexPath];
                }
                ...
                DLog(@"Collection move item from %@ to %@", self->_draggingIndexPath, indexPath);
            } else {
...
- (iTermStatusBarSetupKnobsViewController *)viewControllerToConfigureComponent:(id<iTermStatusBarComponent>)component {
    return [[iTermStatusBarSetupKnobsViewController alloc] initWithComponent:component profileType:_profileType];
}
...
- (void)itermStatusBarSetupElementDidChange:(iTermStatusBarSetupElement *)element
    ...
    if (!_updatingAutoRainbowColors && [self keysContainsTextColor:updatedKeys]) {
...
- (IBAction)configureComponent:(id)sender {
    ...
    viewController.view.frame =
        NSMakeRect(0, 0, viewController.preferredContentSize.width, viewController.preferredContentSize.height);
    iTermStatusBarSetupConfigureComponentWindowController *windowController =
        [[iTermStatusBarSetupConfigureComponentWindowController alloc]
            initWithWindowNibName:@"iTermStatusBarSetupConfigureComponentWindowController"];
