...
- (void)openWindowIfWanted {
    ...
    // BUG-1654: Always open a window if there are no windows, even if user declined restore.
    // This ensures the app doesn't quit when user clicks "Don't Reopen".
    NSArray *windows = [[NSApplication sharedApplication] windows];
    BOOL hasTerminalWindow = NO;
    for (NSWindow *window in windows) {
        // Check for actual terminal windows (PseudoTerminal), not utility windows
        if ([NSStringFromClass([window.windowController class]) containsString:@"PseudoTerminal"]) {
            hasTerminalWindow = YES;
            break;
        }
    }

    if (!_wantsWindow && hasTerminalWindow) {
        DLog(@"untitled: doesn't want window and has terminal windows %@", self);
        return;
    }

    if (!_wantsWindow && !hasTerminalWindow) {
        DLog(@"untitled: no terminal windows exist, forcing window creation %@", self);
    }
