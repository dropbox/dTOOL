...
static NSString *const iTermCommandRunnerErrorDomain = @"com.dashterm.dashterm2.command-runner";
...
+ (void)unzipURL:(NSURL *)zipURL
    ...
    iTermCommandRunner *runner = [[self alloc] initWithCommand:@"/usr/bin/unzip"
                                                 withArguments:fullArgs
                                                          path:destination];
    // Start with 4KB, grows as needed up to max 1MB
    NSMutableString *errorText = [NSMutableString stringWithCapacity:4096];
    runner.outputHandler = ^(NSData *data, void (^handled)(void)) {
        ...
        if (errorText.length > maxLength) {
            [errorText replaceCharactersInRange:NSMakeRange(0, errorText.length - maxLength) withString:@""];
        }
        handled();
    };
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _readingQueue = dispatch_queue_create("com.dashterm.dashterm2.crun-reading", DISPATCH_QUEUE_SERIAL);
        _writingQueue = dispatch_queue_create("com.dashterm.dashterm2.crun-writing", DISPATCH_QUEUE_SERIAL);
        _waitingQueue = dispatch_queue_create("com.dashterm.dashterm2.crun-waiting", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)launchTask {
    ...
    @try {
        ...
        NSLog(@"Task failed with %@. launchPath=%@, pwd=%@, args=%@", e, _task.launchPath, _task.currentDirectoryPath,
              _task.arguments);
        DLog(@"Task failed with %@. launchPath=%@, pwd=%@, args=%@", e, _task.launchPath, _task.currentDirectoryPath,
             _task.arguments);
        if (self.completion) {
...
+ (void)runCommandWithPath:(NSString *)path arguments:(NSArray<NSString *> *)arguments window:(NSWindow *)window {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        gCommandRunners = [NSMutableArray arrayWithCapacity:4]; // Active command runners
    });
...
+ (void)runnerDidFinish:(iTermBufferedCommandRunner *)runner withStatus:(int)status {
    ...
    warning.title = [NSString stringWithFormat:@"The following command returned a non-zero exit code:\n\n“%@ %@”",
                                               runner.command, [runner.arguments componentsJoinedByString:@" "]];
    ...
    warning.warningActions = @[
        [iTermWarningAction warningActionWithLabel:@"OK" block:nil],
        [iTermWarningAction warningActionWithLabel:@"View"
                                             block:^(iTermWarningSelection selection) {
                                                 [[iTermController sharedInstance]
                                                     openSingleUseWindowWithCommand:@"/usr/bin/true"
                                                                          arguments:nil
                                                                             inject:inject
                                                                        environment:nil
                                                                                pwd:@"/"
    ...
    NSString *name;
    if ([runner.command isEqualToString:@"/bin/sh"] && [runner.arguments.firstObject isEqualToString:@"-c"] &&
...
- (void)saveData:(NSData *)inData {
    if (!_output) {
        _output = [NSMutableData dataWithCapacity:4096]; // Initial buffer for command output
    }
    if (_truncated) {
        return;
    }
