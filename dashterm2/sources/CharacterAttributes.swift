...
class CharacterAttributesProvider: NSObject {
    ...
    func attributes(_ c: screen_char_t, externalAttributes: iTermExternalAttribute) -> [AnyHashable: Any] {
        ...
        var bgColor = colorMap.color(forCode: Int32(c.backgroundColor),
                                     ...
                                     // BUG-1661: Use nil coalescing instead of force unwrap for colorMap
                                     brightenBold: brightenBold) ?? .textBackgroundColor
        let fgColor: NSColor
        if c.invisible != 0 {
            fgColor = bgColor
        } else {
            // BUG-1661: Use nil coalescing instead of force unwrap for colorMap
            let baseFgColor = colorMap.color(forCode: Int32(c.foregroundColor),
                                     ...
                                     isBackground: false,
                                     useCustomBoldColor: useCustomBoldColor,
                                     brightenBold: brightenBold) ?? .textColor
            fgColor = baseFgColor.premultiplyingAlpha(with: bgColor)
        }
