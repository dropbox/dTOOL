...
void iTermPosixMoveFileDescriptors(int *orig, int count) {
    ...
    int temp[count];
    for (int i = 0; i < count; i++) {
        temp[i] = -1;  // Initialize to invalid fd to satisfy static analyzer
    }
...
static void iTermPosixTTYReplacementLoginTTY(int master,
    ...
    // BUG-7258: Check setsid() return value - failure means no controlling terminal
    if (setsid() == -1) {
        iTermFileDescriptorServerLog("setsid failed: %s", strerror(errno));
        // Continue anyway - this is between fork and exec, limited options
    }
    // BUG-7259: Check TIOCSCTTY ioctl return value
    if (ioctl(slave, TIOCSCTTY, NULL) == -1) {
        iTermFileDescriptorServerLog("TIOCSCTTY failed: %s", strerror(errno));
        // Continue anyway - TTY may still work in degraded mode
    }
...
int iTermPosixTTYReplacementForkPty(int *amaster,
    ...
    switch (pid) {
        case -1:
            ...
            // BUG-7257: Close master and slave fds to prevent leak on fork failure
            close(master);
            close(slave);
            return -1;
...
void iTermExec(const char *argpath,
    ...
    if (errorFd >= 0) {
        ...
        switch (e) {
            ...
            case EINVAL:
            case EFAULT:
                iTermSignalSafeWrite(errorFd, "This appears to be a bug in DashTerm2. Please report it at https://github.com/ayates_dbx/dashterm2/issues.");
                break;
            case EIO:
            ...
            case ETXTBSY:
                ...
                break;
            default:
                iTermSignalSafeWrite(errorFd, "This error code is unexpected. Please report the value of Errno at https://github.com/ayates_dbx/dashterm2/issues.");
        }
    }
