...
// BUG-f1084: Replace assert() with guard to prevent crash on duplicate download call
- (void)download {
    DLog(@"begin %@", self);
    if (_urlSession || _task) {
        DLog(@"BUG-f1084: download called while already downloading - ignoring duplicate call");
        return;  // Already downloading, prevent duplicate requests
    }
...
- (void)cancel {
    ...
    self.downloading = NO;
    [_task cancel];
    // BUG-2000: Properly invalidate session before releasing to prevent delegate retention leak
    [_urlSession invalidateAndCancel];
    _urlSession = nil;
    _task = nil;
    if (wasDownloading) {
        // -999 is the magic number meaning canceled
        _error = [NSError errorWithDomain:@"com.dashterm.dashterm2" code:-999 userInfo:nil];
...
didCompleteWithError:(nullable NSError *)error {
    ...
    if ([error.domain isEqualToString:NSURLErrorDomain] &&
        ...
        if (resumeData.length > 0) {
            ...
            // BUG-2000: Invalidate old session before creating new one to prevent leak
            [_urlSession invalidateAndCancel];
            NSURLSessionConfiguration *sessionConfig = [NSURLSessionConfiguration defaultSessionConfiguration];
            _urlSession = [NSURLSession sessionWithConfiguration:sessionConfig
                                                        delegate:self
    ...
    if (!error) {
        ...
        if (statusCode != 200 && statusCode != 206) {
            error = [NSError errorWithDomain:@"com.dashterm.dashterm2" code:1 userInfo:@{ NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Server returned status code %@: %@", @(statusCode), [NSHTTPURLResponse localizedStringForStatusCode:statusCode] ] }];
        }
    }
    dispatch_async(dispatch_get_main_queue(), ^{
        if (!self.downloading) {
        ...
        self.downloading = NO;
        // BUG-2000: Invalidate session before releasing to prevent delegate retention leak
...
- (NSDictionary *)parsedManifestFromInputStream:(NSInputStream *)stream {
    ...
    [stream close];  // BUG-1017: Close stream after reading to prevent resource leak
    ...
    if (!error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (version < iTermMinimumPythonEnvironmentVersion) {
                ...
                innerError = [NSError errorWithDomain:@"com.dashterm.dashterm2" code:3 userInfo:@{ NSLocalizedDescriptionKey: @"☹️ No usable version found." }];
            } else if (dict) {
                self->_fullComponent =
                ...
                innerError = [NSError errorWithDomain:@"com.dashterm.dashterm2" code:2 userInfo:@{ NSLocalizedDescriptionKey: @"☹️ Malformed manifest." }];
            }
...
- (void)beginPhase:(iTermOptionalComponentDownloadPhase *)phase {
    ...
    _showingMessage = NO;
    // BUG-f1085: Replace assert() with guard to prevent crash on phase overlap
    if (_currentPhase.downloading) {
        DLog(@"BUG-f1085: beginPhase called while current phase is still downloading - canceling current phase first");
        [_currentPhase cancel];  // Cancel current phase before starting new one
    }
    if (!_currentPhase) {
        _firstPhase = phase;
    }
