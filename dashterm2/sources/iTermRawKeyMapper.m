...
- (NSString *)hexForString:(NSString *)string {
    // 2 hex chars per byte of UTF8 string
    NSMutableString *result = [NSMutableString stringWithCapacity:string.length * 4];
...
- (NSString *)rawKeyStringForFlagsChangedEvent:(NSEvent *)event {
    ...
    return [NSString stringWithFormat:@"%c]1337;%@;%@%c", 27, [self nameForEvent:event], @(flags), 7];
}
...
- (NSString *)rawKeyStringForEvent:(NSEvent *)event {
    ...
    return [NSString stringWithFormat:@"%c]1337;%@;%@;%@;%@;%@%c", 27, name, @(flags),
                                      isFunctionKey ? @"" : [self hexForString:event.characters], @(event.keyCode),
                                      isFunctionKey ? @"" : [self hexForString:event.charactersIgnoringModifiers], 7];
}
