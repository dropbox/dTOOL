...
@interface PTYSession () <
    iTermAutomaticProfileSwitcherDelegate, iTermBackgroundDrawingHelperDelegate, iTermBadgeLabelDelegate,
    iTermCoprocessDelegate, iTermCopyModeHandlerDelegate, iTermConductorDelegate, iTermComposerManagerDelegate,
    iTermFilterDestination, iTermHotKeyNavigableSession, iTermImmutableColorMapDelegate, iTermIntervalTreeObserver,
    iTermLogging, iTermMetaFrustrationDetector, iTermMetalGlueDelegate, iTermModernKeyMapperDelegate,
    iTermModifyOtherKeysMapperDelegate, iTermNaggingControllerDelegate, iTermObject, iTermPasteHelperDelegate,
    iTermPasteboardReporterDelegate, iTermSessionNameControllerDelegate, iTermSessionViewDelegate,
    iTermShortcutNavigationModeHandlerDelegate, iTermStandardKeyMapperDelegate, iTermStatusBarViewControllerDelegate,
    iTermTailFindControllerDelegate, iTermTermkeyKeyMapperDelegate, iTermTriggersDataSource, iTermTmuxControllerSession,
    iTermUpdateCadenceControllerDelegate, iTermWorkingDirectoryPollerDelegate, TriggerDelegate> {
    // Changes are made in the main thread to this and it periodically copied to the mutation thread.
    iTermExpect *_expect;
    DTermGridAdapter *_dtermGridAdapter;
    ...
    // BUG-295: Track KVO observer registration to prevent crash on dealloc
    // if init didn't complete through initSynthetic:
...
@property (nonatomic, retain) id<VT100RemoteHostReading>
    lastRemoteHost; // last remote host at time of setting current directory
