...
class ImageRegistry: NSObject {
    ...
    func collectGarbage() {
        mutex.sync {
            ...
            // BUG-4195: Collect keys first to avoid mutating dictionary while iterating
            let keysToRemove = codeToImage.filter { $0.value.provisional }.map { $0.key }
    ...
    var imageMap: NSDictionary {
        mutex.sync {
            // BUG-1741: Use as? with fallback to original dictionary
            let dict = codeToPropertyList as NSDictionary
            return (dict.copy() as? NSDictionary) ?? dict
        }
    }
}
