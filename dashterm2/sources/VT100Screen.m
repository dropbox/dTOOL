...
@implementation VT100Screen {
    ...
    // If positive, _state is a sanitizing adapter of _mutableState. Changes to the current grid are immediately
    // reflected in the mutable state.
    ...
    // Automatically tells objects when a sync is going to happen so they can break any dependencies on the current
    // state.
...
- (instancetype)init {
    ...
    if (self) {
        ...
        dvr_ = [DVR alloc];
        [dvr_ initWithBufferCapacity:[iTermPreferences intForKey:kPreferenceKeyInstantReplayMemoryMegabytes] * 1024 *
                                     1024];
    }
    return self;
}
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<%@: %p grid:%@ mutableState:%p>", [self class], self, _state.currentGrid, _mutableState];
}
...
- (void)userDidPressReturn {
    ...
    [self mutateAsynchronously:^(VT100Terminal *_Nonnull terminal, VT100ScreenMutableState *_Nonnull mutableState,
...
- (void)setTerminalEnabled:(BOOL)enabled {
    ...
    [self performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
...
- (void)destructivelySetScreenWidth:(int)width height:(int)height mutableState:(VT100ScreenMutableState *)mutableState {
    [self.delegate screenEnsureDefaultMode];
...
- (iTermTerminalContentSnapshot *)snapshotForcingPrimaryGrid:(BOOL)forcePrimary {
    if (forcePrimary) {
        return
            [[[iTermTerminalContentSnapshot alloc] initWithLineBuffer:_state.linebuffer
...
- (NSString *)wordBefore:(VT100GridCoord)coord
    ...
    return [self wordWithCoord:coord predecessor:YES additionalWordCharacters:additionalWordCharacters range:rangePtr];
}
...
- (NSString *)wordEndingAt:(VT100GridCoord)coord range:(VT100GridWindowedRange *)rangePtr {
    return [self wordWithCoord:coord predecessor:NO additionalWordCharacters:nil range:rangePtr];
}
...
- (void)replaceRange:(VT100GridAbsCoordRange)range withPorthole:(id<Porthole>)porthole ofHeight:(int)numLines {
    [self.delegate screenEnsureDefaultMode];
    [self mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                 id<VT100ScreenDelegate> delegate) {
...
- (NSDictionary<NSString *, iTermRange *> *)blockMarkDictionaryOnLine:(long long)y {
    ...
    for (iTermBlockMark *blockMark in [_state objectsOnLine:start ofClass:[iTermBlockMark class]]) {
        if (!blockMarks) {
            // Phase 1 optimization: Pre-size for typical block marks per line
            blockMarks = [[NSMutableDictionary alloc] initWithCapacity:4];
        }
...
- (void)replaceRange:(VT100GridAbsCoordRange)range
    ...
    [self mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                 id<VT100ScreenDelegate> delegate) {
        [mutableState replaceRange:range withLines:@[ line ] promptLength:promptLength blockMarks:blockMarks];
    }];
}
...
- (void)replaceRange:(VT100GridAbsCoordRange)range
    ...
    [self mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                 id<VT100ScreenDelegate> delegate) {
        [mutableState replaceRange:range withLines:lines promptLength:promptLength blockMarks:blockMarks];
    }];
}
...
- (void)addSavedIntervalTreeObjects:(NSArray<iTermSavedIntervalTreeObject *> *)savedITOs baseLine:(long long)baseLine {
    [self mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                 id<VT100ScreenDelegate> delegate) {
...
- (NSArray<ScreenCharArray *> *)gridLinesInRange:(const NSRange)range {
    ...
    // Pre-allocate for expected line count - avoids array growth during iteration
    NSMutableArray<ScreenCharArray *> *result = [NSMutableArray arrayWithCapacity:range.length];
    for (NSInteger i = range.location; i < NSMaxRange(range); i++) {
        const screen_char_t *line = [_state.currentGrid immutableScreenCharsAtLineNumber:i - numLinesInLineBuffer];
        ScreenCharArray *array = [[[ScreenCharArray alloc] initWithLine:line length:width
...
- (NSArray<ScreenCharArray *> *)historyLinesInRange:(const NSRange)range {
    return [_state.linebuffer wrappedLinesFromIndex:range.location
...
- (void)saveToDvr:(NSIndexSet *)cleanLines {
    ...
    [dvr_ appendFrame:[_state.currentGrid orderedLines]
               length:sizeof(screen_char_t) * (_state.currentGrid.size.width + 1) *
                      (_state.currentGrid.size.height)metadata:[_state.currentGrid metadataArray]
...
- (BOOL)isAtCommandPrompt {
    id<VT100ScreenMarkReading> mark =
        [self screenMarkAtOrBeforeAbsLine:self.numberOfLines + _state.totalScrollbackOverflow + 1];
    if (!mark) {
...
- (iTermOffscreenCommandLine *)offscreenCommandLineBefore:(int)line {
    DLog(@"offscreenCommandLineBefore:%@ numberOfScrollbackLines=%@ terminalSoftAlternateScreenMode=%@", @(line),
         @(_state.numberOfScrollbackLines), @(_state.terminalSoftAlternateScreenMode));
    ...
    DLog(@"absLine=%@ totalScrollbackOverflow=%@ numberOfScrollbackLines=%@", @(absLine),
         @(_state.totalScrollbackOverflow), @(_state.numberOfScrollbackLines));
    ...
    return [[[iTermOffscreenCommandLine alloc] initWithCharacters:sca absoluteLineNumber:absLine date:date
...
- (NSInteger)numberOfRawLinesInRange:(VT100GridRange)range {
    ...
    [temp seal];
    [_state.currentGrid appendLines:_state.currentGrid.size.height toLineBuffer:temp];
    return [temp numberOfUnwrappedLinesInRange:range width:_state.width];
}

- (NSArray *)charactersWithNotesOnLine:(int)line {
    Interval *interval = [_state intervalForGridCoordRange:VT100GridCoordRangeMake(0, line, 0, line + 1)];
    NSArray *objects = [_state.intervalTree objectsInInterval:interval];
    // Pre-allocate with objects count as upper bound - result <= objects
...
- (NSArray<id<PTYAnnotationReading>> *)annotationsInRange:(VT100GridCoordRange)range {
    ...
    NSArray *objects = [_state.intervalTree objectsInInterval:interval];
    // Pre-allocate with objects count as upper bound - notes <= objects
    NSMutableArray<id<PTYAnnotationReading>> *notes = [NSMutableArray arrayWithCapacity:objects.count];
...
- (void)enumerateObservableMarks:(void (^NS_NOESCAPE)(iTermIntervalTreeObjectType, NSInteger,
                                                      id<IntervalTreeObject>))block {
...
- (void)clearToLastMark {
    ...
    id<VT100ScreenMarkReading> lastMark = [self lastMarkPassingTest:^BOOL(__kindof id<IntervalTreeObject> obj) {
        ...
        const VT100GridAbsCoord cursorAbsCoord =
            VT100GridAbsCoordFromCoord(intervalStart, self.totalScrollbackOverflow);
...
// TODO: This is mighty similar to -[VT100ScreenState rangeOfOutputForCommandMark:] and is implemented quite
// differently!
...
- (id<VT100ScreenMarkReading>)commandMarkAt:(VT100GridCoord)coord
    ...
    return [_state commandMarkAt:coord mustHaveCommand:mustHaveCommand range:range];
}
...
- (NSArray<id<VT100ScreenMarkReading>> *)lastMarks {
    NSEnumerator *enumerator = [_state.intervalTree reverseLimitEnumerator];
    return [self firstMarkBelongingToAnyClassIn:@[ [VT100ScreenMark class] ] usingEnumerator:enumerator];
}
...
- (NSArray<id<PTYAnnotationReading>> *)lastAnnotations {
    NSEnumerator *enumerator = [_state.intervalTree reverseLimitEnumerator];
    return [self firstMarkBelongingToAnyClassIn:@[ [PTYAnnotation class] ] usingEnumerator:enumerator];
}
...
- (id<VT100ScreenMarkReading>)screenMarkAtOrBeforeAbsLine:(long long)absLine {
    ...
    return (id<VT100ScreenMarkReading>)[[_state.markCache findAtOrBeforeLocation:interval.location]
        objectPassingTest:^BOOL(id<iTermMark> element, NSUInteger index, BOOL *stop) {
...
- (id<VT100ScreenMarkReading>)screenMarkBeforeAbsLine:(long long)absLine {
    Interval *interval = [_state intervalForGridAbsCoordRange:VT100GridAbsCoordRangeMake(0, absLine, 0, absLine)];
    return (id<VT100ScreenMarkReading>)[[_state.markCache findBeforeLocation:interval.location]
        objectPassingTest:^BOOL(id<iTermMark> element, NSUInteger index, BOOL *stop) {
            return [element isKindOfClass:[VT100ScreenMark class]];
        }];
}
...
- (NSArray *)marksOfAnyClassIn:(NSArray<Class> *)allowedClasses before:(Interval *)location {
    NSEnumerator *enumerator = [_state.intervalTree reverseLimitEnumeratorAt:location.limit];
    return [self firstObjectsFoundWithEnumerator:enumerator ofAnyClassIn:allowedClasses];
}

- (NSArray *)marksOfAnyClassIn:(NSArray<Class> *)allowedClasses after:(Interval *)location {
    NSEnumerator *enumerator = [_state.intervalTree forwardLimitEnumeratorAt:location.limit];
    return [self firstObjectsFoundWithEnumerator:enumerator ofAnyClassIn:allowedClasses];
}

- (NSArray *)firstObjectsFoundWithEnumerator:(NSEnumerator *)enumerator ofAnyClassIn:(NSArray<Class> *)allowedClasses {
    NSArray *objects;
    do {
        objects = [enumerator nextObject];
        objects = [objects objectsOfClasses:allowedClasses];
...
- (NSArray *)marksOrNotesBefore:(Interval *)location {
    NSArray<Class> *classes = @[ [PTYAnnotation class], [VT100ScreenMark class] ];
...
// Log the screen contents for debugging.
- (void)dumpScreen {
...
- (int)numberOfLinesDroppedWhenEncodingLegacyFormatWithEncoder:(id<iTermEncoderAdapter>)encoder
    ...
    if (gDebugLogging) {
        ...
        for (id<IntervalTreeObject> object in _state.intervalTree.allObjects) {
            ...
            DLog(@"Save note with coord range %@",
                 VT100GridCoordRangeDescription([_state coordRangeForInterval:object.entry.interval]));
        }
    }
    ...
    [encoder encodeDictionaryWithKey:@"LineBuffer"
                          generation:iTermGenerationAlwaysEncode
                               block:^BOOL(id<iTermEncoderAdapter> _Nonnull subencoder) {
                                   linesDropped =
                                       [_state numberOfLinesDroppedWhenEncodingContentsIncludingGrid:NO
                                                                                             encoder:subencoder
                                                                                      intervalOffset:intervalOffsetPtr
                                                                                           unlimited:unlimited];
                                   return YES;
...
- (BOOL)encodeContents:(id<iTermEncoderAdapter>)encoder linesDropped:(int *)linesDroppedOut unlimited:(BOOL)unlimited {
    [self performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                          id<VT100ScreenDelegate> delegate) {
        NSDictionary *extra;
        ...
        if ([iTermAdvancedSettingsModel useNewContentFormat]) {
            ...
            const int linesDroppedForBrevity =
                [self numberOfLinesDroppedWhenEncodingModernFormatWithEncoder:encoder
            ...
            long long intervalOffset = 0;
            const int linesDroppedForBrevity =
                [self numberOfLinesDroppedWhenEncodingLegacyFormatWithEncoder:encoder intervalOffset:&intervalOffset];
            extra = @{
                kScreenStateIntervalTreeKey : [_state.intervalTree dictionaryValueWithOffset:intervalOffset] ?: @{},
        ...
        [encoder
            encodeDictionaryWithKey:kScreenStateKey
                         generation:iTermGenerationAlwaysEncode
                              block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                                  ...
                                  NSDictionary *dict = @{
                                      ...
                                      kScreenStateLineDrawingModeKey : @[
                                          @([_state.charsetUsesLineDrawingMode containsObject:@0]),
                                      ...
                                      kScreenStateSavedIntervalTreeKey :
                                              [_state.savedIntervalTree dictionaryValueWithOffset:0] ?: [NSNull null],
                                      kScreenStateCommandStartXKey : @(_state.commandStartCoord.x),
                                      kScreenStateCommandStartYKey : @(_state.commandStartCoord.y),
                                      kScreenStateNextCommandOutputStartKey :
                                          [NSDictionary dictionaryWithGridAbsCoord:_state.startOfRunningCommandOutput],
                                      ...
                                      kScreenStateLastCommandOutputRangeKey :
                                          [NSDictionary dictionaryWithGridAbsCoordRange:_state.lastCommandOutputRange],
                                      ...
                                      kScreenStateAlternateGridStateKey : _state.altGrid.dictionaryValue
                                          ?: [NSNull null],
...
- (void)performBlockWithJoinedThreadsReentrantSafe:(void (^NS_NOESCAPE)(VT100Terminal *terminal,
    ...
    if (!_mutableState.performingSideEffect || _mutableState.performingPausedSideEffect) {
        block(_mutableState.terminal, _mutableState,
              _mutableState.sideEffectPerformer.sideEffectPerformingScreenDelegate);
    } else {
...
- (void)performBlockWithJoinedThreads:(void (^NS_NOESCAPE)(VT100Terminal *terminal,
    ...
    // BUG-f1299: Replace assert with guard - don't crash on side-effect ordering issues
    // Instead, log the error and return early to prevent out-of-order execution
    if (_mutableState.performingSideEffect && !_mutableState.performingPausedSideEffect) {
        DLog(@"BUG-f1299: Attempted to join threads inside a side-effect - skipping to prevent ordering issues");
        return;
    }
    [_mutableState performBlockWithJoinedThreads:block];
}

- (void)mutateAsynchronously:(void (^)(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                       id<VT100ScreenDelegate> delegate))block {
    [self.delegate screenEnsureDefaultMode];
...
- (void)restoreState:(VT100ScreenState *)state {
    ...
    // This is important during resizing since the search buffer has to be kept in lockstep with the readonly line
    // buffer.
...
- (VT100SyncResult)synchronizeWithConfig:(VT100MutableScreenConfiguration *)sourceConfig
    ...
    if (_sharedStateCount > 0) {
        ...
        return (VT100SyncResult){.overflow = [mutableState scrollbackOverflow] > 0,
    ...
    return (VT100SyncResult){
        .overflow = overflow, .haveScrolled = _state.currentGrid.haveScrolled, .namedMarksChanged = namedMarksChanged};
}
...
- (void)beginEchoProbeWithBackspace:(NSData *)backspace
    ...
    [self performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        [mutableState.echoProbe beginProbeWithBackspace:backspace password:password];
    }];
}
...
- (void)setEchoProbeDelegate:(id<iTermEchoProbeDelegate>)echoProbeDelegate {
    [self performBlockWithJoinedThreadsReentrantSafe:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                                       id<VT100ScreenDelegate> delegate) {
        mutableState.echoProbeDelegate = echoProbeDelegate;
    }];
}
...
- (NSDictionary<NSString *, NSString *> *)exfiltratedEnvironmentVariables:(NSArray<NSString *> *)names {
    // Phase 1 optimization: Pre-size for typical environment variable count
    NSMutableDictionary<NSString *, NSString *> *result =
        [[NSMutableDictionary alloc] initWithCapacity:names.count ?: 16];
    [_state.exfiltratedEnvironment enumerateObjectsUsingBlock:^(iTermTuple<NSString *, NSString *> *_Nonnull tuple,
