...
- (instancetype)init {
    ...
    if (self) {
        ...
        // Typically 1-3 pending permission completions
        _pendingPermissionCompletions = [NSMutableArray arrayWithCapacity:4];
...
- (void)ensureNotificationPermissionsWithCompletion:(void (^)(BOOL granted))completion {
    // BUG-1604: Ensure thread safety by dispatching to main queue
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self ensureNotificationPermissionsWithCompletion:completion];
        });
        return;
    }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            switch (settings.authorizationStatus) {
                ...
                case UNAuthorizationStatusProvisional:
                    // BUG-3846: Provisional authorization allows notifications, don't treat as denied
                    DLog(@"Provisional authorization status - allowing notifications");
                    [self invokeAllPendingCompletionsWithResult:YES];
                    break;
            }
        });
...
- (void)notify:(NSString *)title withDescription:(NSString *)description {
    [self notify:title withDescription:description windowIndex:-1 tabIndex:-1 viewIndex:-1];
}
...
- (BOOL)notifyRich:(NSString *)title
    ...
    withDescription:(NSString *)description
        ...
        windowIndex:(int)windowIndex
           tabIndex:(int)tabIndex
          viewIndex:(int)viewIndex {
    DLog(@"Notify title=%@ description=%@ window=%@ tab=%@ view=%@", title, description, @(windowIndex), @(tabIndex),
         @(viewIndex));
    ...
    [self ensureNotificationPermissionsWithCompletion:^(BOOL granted) {
        ...
        if (windowIndex >= 0) {
            context = @{@"win" : @(windowIndex), @"tab" : @(tabIndex), @"view" : @(viewIndex)};
        }
            ...
            if (data != nil) {
                ...
                if (nsImage) {
                    ...
                    NSString *filename = [NSString
                        stringWithFormat:@"iterm-notification-image-%f.png", [NSDate timeIntervalSinceReferenceDate]];
                    ...
                    // BUG-1600: Clean up temp file after UNNotificationAttachment copies it
        ...
        NSString *identifier =
            [NSString stringWithFormat:@"iterm-notification-%f", [NSDate timeIntervalSinceReferenceDate]];
...
- (BOOL)notify:(NSString *)title
    withDescription:(NSString *)description
        windowIndex:(int)windowIndex
          ...
          viewIndex:(int)viewIndex
             sticky:(BOOL)sticky {
    DLog(@"Notify title=%@ description=%@ window=%@ tab=%@ view=%@ sticky=%@", title, description, @(windowIndex),
         @(tabIndex), @(viewIndex), @(sticky));
...
- (void)postNotificationWithTitle:(NSString *)title
    ...
    [self postNotificationWithTitle:title
                              detail:detail
            callbackNotificationName:name
        callbackNotificationUserInfo:userInfo
...
- (void)postNotificationWithTitle:(NSString *)title
    ...
    [self ensureNotificationPermissionsWithCompletion:^(BOOL granted) {
        ...
        content.title = title ?: @"";
        content.body = detail ?: @"";
        content.userInfo = @{@"CallbackNotificationName" : name ?: @"", @"CallbackUserInfo" : userInfo ?: @{}};
        ...
        if (actionButtonTitle) {
            ...
            UNNotificationCategory *category =
                [UNNotificationCategory categoryWithIdentifier:@"actionable"
                                                       actions:@[ action ]
                                             intentIdentifiers:@[]
...
- (void)notificationWasClicked:(id)clickContext {
    ...
    // BUG-3848: Add nil check for clickContext to prevent accessing keys on nil dictionary
    if (!clickContext || ![clickContext isKindOfClass:[NSDictionary class]]) {
        DLog(@"clickContext is nil or not a dictionary");
        return;
    }
    ...
    // BUG-3848: Validate win/tab/view keys exist before accessing
    if (!clickContext[@"win"] || !clickContext[@"tab"] || !clickContext[@"view"]) {
        DLog(@"clickContext missing required win/tab/view keys");
        return;
    }
