...
static const NSEventModifierFlags iTermHotkeyModifierMask =
...
+ (NSDictionary *)dictionaryWithGridCoord:(VT100GridCoord)coord {
    return @{kGridCoordXKey : @(coord.x), kGridCoordYKey : @(coord.y)};
}

- (VT100GridCoord)gridCoord {
    return VT100GridCoordMake([self[kGridCoordXKey] intValue], [self[kGridCoordYKey] intValue]);
}

+ (NSDictionary *)dictionaryWithGridAbsCoord:(VT100GridAbsCoord)coord {
    return @{kGridCoordXKey : @(coord.x), kGridCoordAbsYKey : @(coord.y)};
}

- (VT100GridAbsCoord)gridAbsCoord {
    return VT100GridAbsCoordMake([self[kGridCoordXKey] intValue], [self[kGridCoordAbsYKey] longLongValue]);
}

+ (NSDictionary *)dictionaryWithGridAbsCoordRange:(VT100GridAbsCoordRange)coordRange {
    return @{
        kGridCoordStartKey : [self dictionaryWithGridAbsCoord:coordRange.start],
...
+ (NSDictionary *)dictionaryWithGridCoordRange:(VT100GridCoordRange)coordRange {
    return @{
        kGridCoordStartKey : [self dictionaryWithGridCoord:coordRange.start],
...
+ (NSDictionary *)dictionaryWithGridWindowedRange:(VT100GridWindowedRange)range {
    return @{
        kGridCoordRange : [NSDictionary dictionaryWithGridCoordRange:range.coordRange],
...
+ (NSDictionary *)dictionaryWithGridAbsWindowedRange:(VT100GridAbsWindowedRange)absRange {
    return @{
        kGridCoordAbsRange : [NSDictionary dictionaryWithGridAbsCoordRange:absRange.coordRange],
...
+ (NSDictionary *)dictionaryWithGridRange:(VT100GridRange)range {
    return @{kGridRangeLocation : @(range.location), kGridRangeLength : @(range.length)};
}

- (VT100GridRange)gridRange {
    return VT100GridRangeMake([self[kGridRangeLocation] intValue], [self[kGridRangeLength] intValue]);
}

+ (NSDictionary *)dictionaryWithGridSize:(VT100GridSize)size {
    return @{kGridSizeWidth : @(size.width), kGridSizeHeight : @(size.height)};
}
...
- (BOOL)isColorValue {
    return (self[kEncodedColorDictionaryRedComponent] != nil && self[kEncodedColorDictionaryGreenComponent] != nil &&
...
- (NSData *)propertyListData {
    NSString *filename = [[NSWorkspace sharedWorkspace] temporaryFileNameWithPrefix:@"DictionaryPropertyList"
                                                                             suffix:@"DashTerm2"];
...
- (NSString *)sizeInfo {
    NSMutableDictionary<NSString *, NSNumber *> *sizes = [NSMutableDictionary dictionaryWithCapacity:self.count * 2];
    NSCountedSet<NSString *> *counts = [[NSCountedSet alloc] initWithCapacity:self.count * 2];
    sizes[@""] = @([self addSizeInfoToSizes:sizes counts:counts keypath:@""]);
    [counts addObject:@""];

    // Each entry is roughly 30-50 chars; allocate for all entries
    NSMutableString *result = [NSMutableString stringWithCapacity:sizes.count * 40];
...
- (NSDictionary<id, NSDictionary *> *)classifyWithBlock:(id (^NS_NOESCAPE)(id key, id object))block {
    NSMutableDictionary<id, NSMutableDictionary *> *result = [NSMutableDictionary dictionaryWithCapacity:4];
    [self enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj, BOOL *_Nonnull stop) {
        id class = block(key, obj);
        if (class) {
            NSMutableDictionary *subdict = result[class];
            if (!subdict) {
                subdict = [NSMutableDictionary dictionaryWithCapacity:self.count / 4];
...
- (id)it_jsonSafeValue {
    NSMutableDictionary *safe = [NSMutableDictionary dictionaryWithCapacity:self.count];
    for (id key in self) {
        id value = self[key];
        id safeKey = nil;
...
+ (NSDictionary *)descriptorWithKeyCode:(NSUInteger)keyCode modifiers:(NSEventModifierFlags)modifiers {
    return @{kHotKeyKeyCode : @(keyCode), kHotKeyModifiers : @(modifiers & iTermHotkeyModifierMask)};
}
...
- (BOOL)isEqualToDictionary:(NSDictionary *)other ignoringKeys:(NSSet *)keysToIgnore {
    NSMutableSet *allKeys = [NSMutableSet setWithCapacity:self.count + other.count];
...
- (NSInteger)removeObjectsPassingTest:(BOOL (^)(id, id))block {
    NSMutableSet *keys = [NSMutableSet setWithCapacity:self.count / 4];
    [self enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj, BOOL *_Nonnull stop) {
        if (block(key, obj)) {
            [keys addObject:key];
        }
...
- (void)it_mergeFrom:(NSDictionary *)other {
    // BUG-f1355: Replace assert() with guard - self-merge should no-op, not crash
    if (self == other) {
        ELog(@"it_mergeFrom: Attempting to merge dictionary with itself, skipping");
        return;
    }
