...
class MainMenuMangler: NSObject {
    ...
    private let iconMap = [
        ...
        "About DashTerm2": "info.circle.fill",
        "Quit DashTerm2": "power",
        ...
        // Help menu
        "DashTerm2 Help": "questionmark.circle",
    ...
    func updateMainMenu() {
        ...
        // Upstream 724e0e692: Use currentTerminal instead of keyWindow delegate
        // The keyWindow approach from 61078a624 didn't work correctly; currentTerminal is more reliable
        if let term = iTermController.sharedInstance().currentTerminal {
            currentSessionIsWeb = term.currentSession()?.isBrowserSession() ?? false
            DLog("updateMainMenu: currentTerminal=\(term), currentSession=\(term.currentSession().debugDescription), isBrowser=\(currentSessionIsWeb)")
        } else {
