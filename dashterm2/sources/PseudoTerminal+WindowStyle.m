...
- (void)updateWindowType {
    ...
    // BUG-f1380: Convert assert to guard - unsupported window types should be no-op
    const BOOL isSupportedInternalType = (_windowType == WINDOW_TYPE_NORMAL ||
                                          _windowType == WINDOW_TYPE_COMPACT ||
                                          _windowType == WINDOW_TYPE_MAXIMIZED ||
                                          _windowType == WINDOW_TYPE_COMPACT_MAXIMIZED);
    const BOOL isSupportedPublicType = (self.windowType == WINDOW_TYPE_NORMAL ||
                                        self.windowType == WINDOW_TYPE_COMPACT ||
                                        self.windowType == WINDOW_TYPE_MAXIMIZED ||
                                        self.windowType == WINDOW_TYPE_COMPACT_MAXIMIZED);
    if (!isSupportedInternalType || !isSupportedPublicType) {
        return;
    }
...
+ (BOOL)shouldHideWindowTitleForWindowType:(iTermWindowType)windowType {
    if (@available(macOS 26, *)) {
        // Upstream fix b28e7a7 - Issue 12656: WINDOW_TYPE_MAXIMIZED should NOT hide title
        switch (iTermThemedWindowType(windowType)) {
            case WINDOW_TYPE_TRADITIONAL_FULL_SCREEN:
            case WINDOW_TYPE_LION_FULL_SCREEN:
            case WINDOW_TYPE_ACCESSORY:
...
- (void)safelySetStyleMask:(NSWindowStyleMask)styleMask {
    // BUG-f1381: Convert assert to guard - prevent reentrant calls
