...
extension NSData {
    ...
    static func styleSheet(fontFamily: String,
        ...
        // BUG-1617: Use nil coalescing instead of force unwrap for data encoding
        return ("<style>body { " + style + "}</style>").data(using: .utf8) ?? Data()
    }
    ...
    var inlineHTMLData: Data {
        ...
        // BUG-1617: Use nil coalescing instead of force unwrap for data encoding
        return "<img src=\"data:image/png;base64, \(base64)\"/>".data(using: .utf8) ?? Data()
    }
}
...
class HTMLEncoder {
    ...
    func encode(_ data: Data,
        ...
        for elt in stack {
            if let tag = elt.tag {
                result.append(("<" + tag).data(using: .utf8) ?? Data())
            }
            if let attributes = elt.attributes {
                result.append((" " + attributes).data(using: .utf8) ?? Data())
            }
            if elt.tag != nil {
                result.append(">".data(using: .utf8) ?? Data())
            }
    ...
    private func styleProperties(foreground: screen_char_t,
        ...
        // BUG-1639: Use optional binding instead of force unwrap for colorMap access
        var fgcolor: NSColor? = fgKey == kColorMapForeground ? nil : colorMap.color(forKey: fgKey)
        ...
        let bgKey = colorMap.key(forColor: Int32(background.backgroundColor),
                                 ...
                                 brightenBold: configuration.brightenBold)
        // BUG-1639: Use optional binding instead of force unwrap for colorMap access
        let bgcolor: NSColor? = bgKey == kColorMapBackground ? nil : colorMap.color(forKey: bgKey)
