...
- (void)awakeFromNib {
    ...
    // BUG-f645: Replace assert(false) with DLog - horizontal spacing uses delegate, not settingChanged
    [self defineControl:_asciiFontPicker.horizontalSpacing.textField
         ...
         settingChanged:^(id sender) { DLog(@"BUG-f645: settingChanged called for horizontal spacing - should use delegate instead"); }
                 update:^BOOL{
                     __strong __typeof(weakSelf) strongSelf = weakSelf;
                     if (!strongSelf) {
                         return NO;
    ...
    // BUG-f646: Replace assert(false) with DLog - vertical spacing uses delegate, not settingChanged
    [self defineControl:_asciiFontPicker.verticalSpacing.textField
                    key:KEY_VERTICAL_SPACING
            relatedView:nil
            displayName:@"Vertical spacing"
                   type:kPreferenceInfoTypeIntegerTextField
         settingChanged:^(id sender) { DLog(@"BUG-f646: settingChanged called for vertical spacing - should use delegate instead"); }
                 update:^BOOL{
                     __strong __typeof(weakSelf) strongSelf = weakSelf;
                     if (!strongSelf) {
                         return NO;
...
- (void)fontPickerCompositeView:(BFPCompositeView *)view didSelectFont:(NSFont *)font {
    ...
    if (view == _asciiFontPicker) {
        key = KEY_NORMAL_FONT;
    } else if (view == _nonASCIIFontPicker) {
        key = KEY_NON_ASCII_FONT;
    } else {
        // BUG-f1372: Convert assert to guard - ignore unknown views instead of crashing
        return;
    }
