...
+ (iTermInitialDirectoryMode)modeForString:(NSString *)modeString
    ...
    if ([modeString isEqualToString:kProfilePreferenceInitialDirectoryCustomValue]) {
        ...
        // BUG-f1066: Replace assert with guard - missing key should use home mode not crash
        if (!key) {
            DLog(@"BUG-f1066: Advanced setting key is nil for objectType %d, falling back to home", (int)objectType);
            return iTermInitialDirectoryModeHome;
        }
    ...
    // BUG-411: Return home directory mode instead of crashing for unexpected object type
    // This can happen with new object types that haven't been added to the switch
    return iTermInitialDirectoryModeHome;
}
