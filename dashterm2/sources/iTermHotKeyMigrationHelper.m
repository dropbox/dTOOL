...
//
// BUG-277: Hot key migration messages updated to DashTerm2
// BUG-417: Hot key window dynamic profile warning updated to DashTerm2
...
- (BOOL)warnAboutChildrenOfHotkeyProfileIfNeeded:(Profile *)profile {
    ...
    NSMutableArray *childrensNames = [NSMutableArray arrayWithCapacity:4]; // Hotkey profile children names
    ...
    if (childrensNames.count) {
        ...
        NSString *title = [NSString
            stringWithFormat:@"You have dynamic profiles whose “Dynamic Profile Parent Name” is set to your hotkey "
                             @"window's profile, “%@.” Because multiple hotkey windows are now supported, the hotkey "
                             @"will now toggle a separate window for each of these profiles. Please update your "
                             @"dynamic profiles appropriately. The affected profiles are:\n%@",
...
- (void)migrateDynamicProfileHotKeySettings:(Profile *)profile {
    ...
    NSString *title = [NSString
        stringWithFormat:@"Your hotkey window‘s profile is a dynamic profile named “%@.” It needs to be updated for "
                         @"this version of DashTerm2 because hotkey settings are now stored in the profile.",
                         profile[KEY_NAME]];
...
- (NSString *)jsonEntriesInDictionary:(NSDictionary *)dictionary {
    NSMutableArray *linesArray = [NSMutableArray arrayWithCapacity:dictionary.count];
...
- (NSString *)xmlEntriesInDictionary:(NSDictionary *)dictionary {
    ...
    [dictionary enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj, BOOL *_Nonnull stop) {
        ...
        // BUG-f1064: Replace assert with guard - skip unsupported value types instead of crashing
        if (!encodedValue) {
            DLog(@"BUG-f1064: Skipping unsupported object type in hot key migration: %@", [obj class]);
            return;
        }
        [linesArray
            addObject:[NSString stringWithFormat:@"<key>%@</key><%@>%@</%@>", encodedKey, type, encodedValue, type]];
    }];
    return [linesArray componentsJoinedByString:@"\n"];
}
...
- (void)showNeededChangesForHotKeyMigrationOfDynamicProfile:(Profile *)profile {
    ...
    alert.informativeText = [NSString stringWithFormat:@"Add these settings to the profile named “%@” in “%@”:\n%@",
                                                       profile[KEY_NAME], filename, lines];
...
- (NSData *)modifiedDynamicProfileFileWithNewHotKeySettingsFromProfile:(Profile *)profile {
    ...
    NSArray<Profile *> *profiles = [[iTermDynamicProfileManager sharedInstance] profilesInFile:filename
                                                                                      fileType:&fileType];
    NSUInteger index =
        [profiles indexOfObjectPassingTest:^BOOL(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
#pragma clang diagnostic pop
    iTermHotKeyDockPreference dockAction =
        dockIconTogglesWindow ? iTermHotKeyDockPreferenceShowIfNoOtherWindowsOpen : iTermHotKeyDockPreferenceDoNotShow;
