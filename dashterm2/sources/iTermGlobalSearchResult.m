...
- (VT100GridCoordRange)internalCoordRange {
    ...
    return VT100GridCoordRangeMake(self.result.internalStartX, startY, self.result.internalEndX + 1, endY);
}
...
- (void)revealWithState:(NSMutableDictionary *)state completion:(void (^)(NSRect))completion {
...
- (void)switchSessionToAlternateScreenIfNeeded:(NSMutableDictionary *)state {
    ...
    if (shouldShowAlternateScreen != self.session.screen.showingAlternateScreen) {
        ...
        if (!sessionGuidsWithChangedScreens) {
            sessionGuidsWithChangedScreens = [[NSMutableDictionary alloc] initWithCapacity:4];
        ...
        [self.session setShowAlternateScreen:shouldShowAlternateScreen announce:YES];
    }
}
...
- (void)revealWithState:(NSMutableDictionary *)state completion:(void (^)(NSRect))completion {
    return [self.session.view.browserViewController revealFindResult:self.findResult completion:completion];
}
