...
- (void)setEndColumn:(NSInteger)endColumn {
    ...
    _attributes = [_attributes dictionaryBySettingObject:value forKey:iTermSourceColumnsAttribute];
}
...
- (void)build {
    ...
    if (_explicitDirectionControls && _attributedString != nil) {
        ...
        // BUG-f1082: Replace assert with guard - castFrom failure should skip processing, not crash
        if (attributedString == nil) {
            DLog(@"WARNING: castFrom failed, skipping direction controls processing");
            return;
        }
...
- (void)flushCharacters {
    ...
    [_string appendString:[NSString stringWithCharacters:_characterData.mutableBytes
                                                  length:_characterData.length / sizeof(unichar)]];
    _canUseFastPath = NO;
    _characterData = nil;
}
