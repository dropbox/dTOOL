...
extension SSHFilePanel {
    ...
    private func defaultPath(for sshIdentity: SSHIdentity) -> String {
        ...
        // BUG-1629: Use nil coalescing instead of force unwrap for first element
    ...
    private func navigateToPath(_ path: SSHFileDescriptor) async -> Bool {
        ...
        do {
            ...
            // BUG-2669: Show navigation errors to user instead of returning false silently
            DLog("Failed to navigate to path \(path): \(error)")
            presentErrorAlert(title: "Navigation Failed", error: error)
        }
        return false
    }
    ...
    @objc private func systemPanelButtonClicked(_ sender: NSButton) {
        if let callback = systemPanelCallback {
            ...
            willClose()
            // BUG-1608: Use if-let to safely unwrap window instead of force unwrap
    ...
    @objc private func locationButtonChanged(_ sender: NSPopUpButton) {
        ...
        if let sshIdentity = locationButton.sshIdentity {
            locationButton.set(path: path, sshIdentity: sshIdentity)
        }
    }
        ...
        if let currentPath, let currentEndpoint {
            ...
            Task { @MainActor [weak self] in
                do {
                    ...
                    // BUG-2668: Cancellation is normal (user started new search), don't show error
                    DLog("Search cancelled")
                } catch {
                    // BUG-2668: Show search errors to user instead of logging silently
                    DLog("Search failed: \(error)")
                    self?.presentErrorAlert(title: "Search Failed", error: error)
                }
            }
        }
    }
    ...
    @objc private func openButtonClicked(_ sender: NSButton?) {
        ...
        if selection.count == 1,
           ...
           firstSelection.isDirectory,
           !canChooseDirectories {
            Task { @MainActor in
                await navigateToPathWithHistory(SSHFileDescriptor(
                    absolutePath: firstSelection.file.absolutePath,
                    isDirectory: true,
                    sshIdentity: currentIdentity) )
            }
            return
        ...
        if isSavePanel {
            ...
            if allowedContentTypes.count == 1,
               let requiredType = allowedContentTypes.first {
    ...
    // BUG-2668/BUG-2669: Helper to present error alerts to users instead of failing silently
    ...
    func runModal() -> NSApplication.ModalResponse {
        // BUG-1608: Use guard to safely unwrap window instead of force unwrap
...
extension SSHFilePanel {
    ...
    func setupKeyboardShortcuts() {
        ...
        let goUpItem = NSMenuItem(title: "Go Up",
                                 ...
                                 keyEquivalent: UnicodeScalar(NSUpArrowFunctionKey).map { String(Character($0)) } ?? "")
        ...
        let backItem = NSMenuItem(title: "Back",
                                 ...
                                 keyEquivalent: UnicodeScalar(NSLeftArrowFunctionKey).map { String(Character($0)) } ?? "")
        ...
        let forwardItem = NSMenuItem(title: "Forward",
                                    ...
                                    keyEquivalent: UnicodeScalar(NSRightArrowFunctionKey).map { String(Character($0)) } ?? "")
    ...
    private func completions(basePath rawBasePath: String,
        ...
        do {
            ...
            // BUG-2670: Log descriptive autocomplete error. Alerting would be disruptive for
            // transient network errors during typing - returning empty is acceptable UX.
            DLog("Autocomplete failed for basePath '\(rawBasePath)': \(error.localizedDescription)")
            return []
        }
    }
...
extension SSHFilePanel {
    ...
    public struct iTermFilePanelError: LocalizedError, CustomStringConvertible, CustomNSError, Codable {
        ...
        public static var errorDomain: String { "com.dashterm.dashterm2.file-panel" }
    ...
    var destinationDescriptor: SSHFileDescriptor? {
        // BUG-483: Replace it_assert with guard for proper crash prevention
        guard isSavePanel else {
            DLog("BUG-483: destinationDescriptor called on non-save panel")
            return nil
        }
