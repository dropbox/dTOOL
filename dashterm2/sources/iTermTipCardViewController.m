...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    [self.color set];
    NSRectFill(NSMakeRect(rect.origin.x, rect.origin.y + 0.5, rect.size.width, 0.5));
}
...
+ (NSColor *)tipTextColor {
    return [NSColor colorWithName:@"iTermTipTextColor"
...
- (void)setBodyText:(NSString *)body {
    ...
    NSColor *sigColor = [NSColor colorWithName:@"iTermTipSignatureColor"
                               dynamicProvider:^NSColor *_Nonnull(NSAppearance *appearance) {
                                   if (appearance.it_isDark) {
    ...
    [attributedString iterm_appendString:body withAttributes:bigTextAttributes];
    [attributedString iterm_appendString:@"\n" withAttributes:bigTextAttributes];
    [attributedString iterm_appendString:@"DashTerm2 tip of the day" withAttributes:signatureAttributes];
...
- (iTermTipCardActionButton *)addActionWithTitle:(NSString *)title
                                        shortcut:(NSString *)shortcut
                                           ...
                                           block:(void (^)(id card))block {
    if (!_actionButtons) {
        _actionButtons = [[NSMutableArray alloc] initWithCapacity:4]; // Tip card action buttons
    }

    iTermTipCardActionButton *button = [[[iTermTipCardActionButton alloc]
        initWithFrame:NSMakeRect(kButtonSideInset, 0, _container.bounds.size.width - kButtonSideInset * 2, 0)]
        autorelease];
...
- (void)combineActionWithTitle:(NSString *)leftTitle andTitle:(NSString *)rightTitle {
    ...
    // BUG-1252a: Check that right is in array before using indexOfObject result
    NSUInteger rightIndex = [_actionButtons indexOfObject:right];
    if (rightIndex == NSNotFound) {
        return;
    }
    [[left retain] autorelease];
    [_actionButtons removeObject:left];
    [_actionButtons insertObject:left atIndex:rightIndex];
}
...
- (NSSize)sizeThatFits:(NSSize)size {
    NSRect desiredRect = [self performLayoutForWidth:size.width dry:YES origin:self.view.frame.origin];
...
- (NSRect)performLayoutForWidth:(CGFloat)width dry:(BOOL)dry origin:(NSPoint)newOrigin {
    ...
    frame.size.height = (kCardTopMargin + kContainerTopBorder + _titleBox.frame.size.height +
                         kMarginBetweenTitleAndBody + bodyFrame.size.height + kMarginBetweenTextAndButtons +
                         totalButtonHeight + kContainerBottomBorder + kCardBottomMargin);
    frame.origin = newOrigin;
    frame = NSIntegralRect(frame);

    // Calculate the container's frame.
    CGFloat containerHeight = (1 + _titleBox.frame.size.height + kMarginBetweenTitleAndBody + bodyFrame.size.height +
                               kMarginBetweenTextAndButtons + totalButtonHeight + 1);
    NSRect containerFrame = NSMakeRect(kContainerSideInset, kCardTopMargin, containerWidth, containerHeight);
    ...
    NSRect coverViewFrame = NSMakeRect(1, NSMaxY(titleFrame), containerWidth - 2,
    ...
    for (iTermTipCardActionButton *actionButton in _actionButtons) {
        ...
        if (!dry) {
            ...
            CGFloat buttonWidth =
                (containerFrame.size.width - 2 * kContainerSideInset) / actionButton.numberOfButtonsInRow;
...
- (void)animateCardWithDuration:(CGFloat)duration
    ...
    NSMutableArray *buttonsToCollapse = [NSMutableArray arrayWithCapacity:4]; // Tip card buttons to collapse
    NSRect frame;
