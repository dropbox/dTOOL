...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<%@: %p %@>", NSStringFromClass([self class]), self, props];
}

- (NSDictionary *)dictionaryValue {
    return @{
        @"fontTable" : _fontTable ?: [NSNull null],
...
+ (instancetype)characterSourceAttributesWithThinStrokes:(BOOL)useThinStrokes bold:(BOOL)bold italic:(BOOL)italic {
...
+ (NSRect)boundingRectForCharactersInRange:(NSRange)range
    ...
    // Section 4.2: NSCache for automatic memory pressure eviction.
    // NSCache is thread-safe, eliminating need for explicit locking.
    // Limit to 128 entries - font metrics are cheap to recompute, expensive to keep in memory.
    static NSCache<NSArray *, NSValue *> *cache;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cache = [[NSCache alloc] init];
        cache.countLimit = 128;  // Bound memory during font exploration
    });
    // fontTable can be nil during window teardown when Metal view is deallocating.
    // Return CGRectZero as a safe fallback - no glyph bounds needed for a closing window.
    if (fontTable == nil) {
        return CGRectZero;
    }
    // During app termination, fontTable may exist but its fonts may be deallocated.
    // Check that the underlying fonts are valid before proceeding.
    if (fontTable.asciiFont == nil || fontTable.asciiFont.font == nil) {
        return CGRectZero;
    }
    // configHash can be nil during teardown - avoid inserting nil into array literal
    if (fontTable.configHash == nil) {
        return CGRectZero;
    }
    CGFloat pointSize;
    if (useNonAsciiFont) {
        pointSize = MAX(fontTable.asciiFont.font.pointSize, fontTable.defaultNonASCIIFont.font.pointSize);
    } else {
        pointSize = fontTable.asciiFont.font.pointSize;
    }
    NSArray *key = @[
        NSStringFromRange(range), fontTable.asciiFont.font.fontName ?: @"",
        fontTable.defaultNonASCIIFont.font.fontName ?: @"", fontTable.configHash, @(pointSize),
        @(fontTable.baselineOffset), @(scale), @(useBoldFont), @(useItalicFont), @(useNonAsciiFont)
    ];
    // NSCache is thread-safe - no explicit locking needed (BUG-1119 fix preserved)
    NSValue *cachedValue = [cache objectForKey:key];
    if (cachedValue) {
        return [cachedValue rectValue];
    }

    const CGSize bigSize = CGSizeMake(round(pointSize * 10), round(pointSize * 10));
    ...
    iTermCharacterSourceAttributes *attributes =
        [iTermCharacterSourceAttributes characterSourceAttributesWithThinStrokes:NO bold:YES italic:YES];
    NSRect unionRect = NSZeroRect;
    for (NSInteger i = 0; i < range.length; i++) {
        @autoreleasepool {
            UTF32Char c = range.location + i;
            iTermCharacterSource *source =
                [[iTermCharacterSource alloc] initWithCharacter:[NSString stringWithLongCharacter:c]
    ...
    // NSCache is thread-safe - no explicit locking needed (BUG-1119 fix preserved)
    [cache setObject:[NSValue valueWithRect:unionRect] forKey:key];
    return unionRect;
}
...
- (instancetype)initWithFont:(NSFont *)font
    ...
    self = [super init];
    if (self) {
        // BUG-2763: Guard against NULL context to avoid undefined behavior
        if (!context) {
            DLog(@"NULL context passed to iTermCharacterSource init");
            return nil;
        }
        _font = font;
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p font=%@>", NSStringFromClass([self class]), self, _font];
}
...
- (void)drawWithOffset:(CGPoint)offset iteration:(NSInteger)iteration {
    ...
    [self drawIteration:iteration atOffset:CGPointMake(offset.x, ty) skew:skew];
    _haveDrawn = YES;
    const NSUInteger length = CGBitmapContextGetBytesPerRow(_context) * CGBitmapContextGetHeight(_context);
    NSMutableData *data = [NSMutableData dataWithBytes:CGBitmapContextGetData(_context) length:length];
    ...
    if (_debug) {
        ...
        NSURL *url = [NSURL fileURLWithPath:[NSString stringWithFormat:@"/tmp/full.%@.%@,%@.%@.png", self.debugName,
                                                                       @(offset.x), @(offset.y), @(iteration)]];
        ...
        if (!CGImageDestinationFinalize(destination)) {
            DLog(@"Failed to write image to /tmp/image.png");
        }
...
- (void)fillBackgroundForIteration:(NSInteger)iteration context:(CGContextRef)context {
    ...
    for (int x = 0; x < self.maxParts; x++) {
        for (int y = 0; y < self.maxParts; y++) {
            CGContextStrokeRect(context, CGRectMake(x * _descriptor.glyphSize.width, y * _descriptor.glyphSize.height,
                                                    _descriptor.glyphSize.width, _descriptor.glyphSize.height));
        }
    }
#endif
}
...
- (void)initializeStateIfNeededWithFont:(CTFontRef)runFont {
    ...
    _isEmoji = ([fontName isEqualToString:@"AppleColorEmoji"] || [fontName isEqualToString:@"Apple Color Emoji"]);
    ...
    // Phase 1 optimization: Pre-allocate for max iterations (1-4 depending on emoji/monochrome)
    _datas = [NSMutableArray arrayWithCapacity:4];
}

- (void)drawBackgroundIfNeededForIteration:(NSInteger)iteration context:(CGContextRef)context {
    if (iteration >= _nextIterationToDrawBackgroundFor) {
        _nextIterationToDrawBackgroundFor = iteration;
        [self fillBackgroundForIteration:iteration context:context];
    }
}
...
- (void)initializeIteration:(NSInteger)iteration
    ...
    [self initializeTextMatrixInContext:context withSkew:skew offset:offset];
}
...
- (void)prepareToDrawIteration:(NSInteger)iteration
    ...
    [self drawBackgroundIfNeededForIteration:iteration context:context];
    [self setTextColorForIteration:iteration context:context];
    if (!haveInitializedThisIteration) {
        [self initializeIteration:iteration offset:offset skew:skew context:context];
    }
}
...
- (void)initializeTextMatrixInContext:(CGContextRef)cgContext withSkew:(CGFloat)skew offset:(CGPoint)offset {
    if (!_isEmoji) {
        // Can't use this with emoji.
        CGAffineTransform textMatrix = CGAffineTransformMake(_descriptor.scale, 0.0, skew * _descriptor.scale,
                                                             _descriptor.scale, offset.x, offset.y);
...
- (iTermCharacterBitmap *)bitmapForPart:(int)part {
    ...
    // BUG-1110: Validate both source and dest bounds to prevent buffer overflow
    for (int i = 0; (destOffset >= 0 && (NSUInteger)(destOffset + destRowSize) <= length && // dest bounds check
                     i < glyphHeight && sourceOffset >= 0 &&                                // source lower bound check
                     sourceOffset + (ssize_t)destRowSize <= sourceLength);
         i++) {
    ...
    if (_debug) {
        ...
        [image saveAsPNGTo:[NSString stringWithFormat:@"/tmp/%@.%@.%@.%@.png", _font.familyName, [@[
                                                          ...
                                                          _attributes.useThinStrokes ? @"Thin" : @""
                                                      ] componentsJoinedByString:@""],
                                                      self.debugName, @(part)]];
...
- (NSArray<NSNumber *> *)newParts {
    ...
    const int radius = _radius;
    const int maxParts = self.maxParts;
    // Phase 1 optimization: Pre-allocate for maxParts*maxParts (typical glyphs span few parts)
    NSMutableArray<NSNumber *> *result = [NSMutableArray arrayWithCapacity:maxParts * maxParts];
    for (int y = 0; y < maxParts; y++) {
        for (int x = 0; x < maxParts; x++) {
            CGRect partRect = CGRectMake(x * _descriptor.glyphSize.width, y * _descriptor.glyphSize.height,
                                         _descriptor.glyphSize.width, _descriptor.glyphSize.height);
            if (CGRectIntersectsRect(partRect, boundingBox)) {
                [result addObject:iTermImagePartToNumber(iTermImagePartFromDeltas(x - radius, y - radius))];
            }
        }
    }
    return [result copy];
...
- (void)logStringRepresentationOfAlphaChannelOfBitmapDataBytes:(unsigned char *)data {
    for (int y = 0; y < _size.height; y++) {
        ...
        NSMutableString *line = [[NSMutableString alloc] initWithCapacity:width];
...
- (NSRect)frameForBoundingRect:(NSRect)frame flipped:(BOOL)flipped {
    ...
    CGPoint min = CGPointMake(floor(CGRectGetMinX(frame)), floor(CGRectGetMinY(frame)));
    CGPoint max = CGPointMake(ceil(CGRectGetMaxX(frame)), ceil(CGRectGetMaxY(frame)));
    frame = CGRectMake(min.x, min.y, max.x - min.x, max.y - min.y);
    DLog(@"%@ Bounding box for character '%@' in font %@ is %@ at scale %@", self, self.debugName, _font,
         NSStringFromRect(frame), @(_descriptor.scale));
