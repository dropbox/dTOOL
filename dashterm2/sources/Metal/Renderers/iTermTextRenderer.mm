...
- (BOOL)isEqual:(id)object {
    ...
    return (CGSizeEqualToSize(_textureSize, other->_textureSize) && CGSizeEqualToSize(_cellSize, other->_cellSize));
}
...
+ (void)enumerateGridOfSubpixelModels:(void (^)(float, float, iTermSubpixelModel *))block {
    ...
    for (float textColor = 0; textColor < 256; textColor += stride) {
        for (float backgroundColor = 0; backgroundColor < 256; backgroundColor += stride) {
            iTermSubpixelModel *model = [[iTermSubpixelModelBuilder sharedInstance]
                modelForForegroundColor:MIN(MAX(0, textColor / 255.0), 1)
...
+ (NSData *)subpixelModelData {
    ...
    dispatch_once(&onceToken, ^{
        // 18 text colors * 18 background colors * 256 samples = ~83KB
        NSMutableData *data = [[NSMutableData alloc] initWithCapacity:18 * 18 * 256];
...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
    ...
    if (self) {
        ...
        // Cache holds at most 8 entries for different cell/texture size combinations
        _quadCache = [NSMutableArray arrayWithCapacity:8];
        ...
        _textInfoPool = [[iTermMetalBufferPool alloc] initWithDevice:device
                                                          bufferSize:sizeof(iTermVertexTextInfoStruct)];
        ...
        const size_t capacity = DashTerm2::PIUArray<iTermTextPIU>::DEFAULT_CAPACITY;
        _piuPool = [[iTermMetalMixedSizeBufferPool alloc]
        ...
        if (iTermTextIsMonochrome()) {
            ...
            [_models replaceRegion:MTLRegionMake2D(0, 0, 18 * 256, 18)
                       ...
                       bytesPerRow:18 * 256];
            [iTermTexture setBytesPerRow:18 * 256 rawDataSize:18 * 256 * 18 samplesPerPixel:1 forTexture:_models];
        }
    }
    return self;
}
...
- (nullable __kindof iTermMetalRendererTransientState *)
    ...
    [self initializeTransientState:transientState commandBuffer:commandBuffer];
...
- (void)initializeTransientState:(iTermTextRendererTransientState *)tState
    ...
    if (!_texturePageCollectionSharedPointer) {
        DashTerm2::TexturePageCollection *collection = new DashTerm2::TexturePageCollection(
            _cellRenderer.device, simd_make_uint2(currentSize.width, currentSize.height), iTermTextAtlasCapacity,
            iTermTextRendererMaximumNumberOfTexturePages);
        _texturePageCollectionSharedPointer =
            [[iTermTexturePageCollectionSharedPointer alloc] initWithObject:collection];
    }
    ...
    [tState preallocatePIUArraysForCellCount:tState.numberOfCells];
}
...
- (id<MTLBuffer>)quadOfSize:(CGSize)size
    ...
    const iTermVertex vertices[] = {
        ...
        {{vw, 0 + _verticalOffset}, {w, 0}}, {{0, 0 + _verticalOffset}, {0, 0}},  {{0, vh + _verticalOffset}, {0, h}},

        {{vw, 0 + _verticalOffset}, {w, 0}}, {{0, vh + _verticalOffset}, {0, h}}, {{vw, vh + _verticalOffset}, {w, h}},
    };
    entry.quad = [_verticesPool requestBufferFromContext:poolContext withBytes:vertices checkIfChanged:YES];
    ...
    // Cache quads for different cell/texture sizes (ascii, non-ascii, emoji, CJK, zoomed, etc.)
    // 8 entries should handle most common combinations without thrashing.
    if (_quadCache.count > 8) {
...
- (id<MTLBuffer>)piuBufferForPIUs:(const iTermTextPIU *)pius
    ...
    piuBuffer = [_piuPool requestBufferFromContext:tState.poolContext size:sizeof(iTermTextPIU) * instances bytes:pius];
    piuBuffer.label = @"Text PIUs";
    ITDebugAssert(piuBuffer);
    return piuBuffer;
}

static NSString *const FragmentFunctionName(const BOOL &underlined, const BOOL &emoji) {
    static NSArray<NSString *> *names = @[

        // Subpixel AA
        @"iTermTextFragmentShaderWithBlending", @"iTermTextFragmentShaderWithBlendingEmoji",
        @"iTermTextFragmentShaderWithBlendingUnderlined", @"iTermTextFragmentShaderWithBlendingUnderlinedEmoji",
        ...
        @"iTermTextFragmentShaderMonochromeUnderlinedEmoji"
    ]; // mono underlined emoji
    int index = (gMonochromeText ? 4 : 0) | (underlined ? 2 : 0) | (emoji ? 1 : 0);
    return names[index];
}

static NSString *const VertexFunctionName(const BOOL &underlined, const BOOL &emoji) {
    static NSArray<NSString *> *names = @[
...
- (void)blitFromTemporaryToIntermediateTexture:(iTermMetalFrameData *_Nonnull)frameData {
    ...
    // Performance: Use static string constant instead of stringWithFormat: which allocates every frame
    static NSString *const blitterLabel = @"Temporary>Intermediate";
    blitter.label = blitterLabel;
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    const simd_float2 cellSize =
        simd_make_float2(frameData.cellConfiguration.cellSize.width, frameData.cellConfiguration.cellSize.height);
    ...
    [tState
        enumerateDraws:^(const iTermTextPIU *pius, NSInteger instances, id<MTLTexture> texture,
                         vector_uint2 textureSize, vector_uint2 glyphSize,
                         iTermMetalUnderlineDescriptor underlineDescriptor,
                         iTermMetalUnderlineDescriptor strikethroughDescriptor, BOOL underlined, BOOL emoji) {
            __block id<MTLBuffer> vertexBuffer;
            [tState measureTimeForStat:iTermTextRendererStatNewQuad
                               ofBlock:^{
            ...
            [tState measureTimeForStat:iTermTextRendererStatNewTextInfo
                               ofBlock:^{
                                   textInfoBuffer = [self textInfoBufferForTransientState:tState
                                                                             numInstances:instances];
                               }];
            __block id<MTLBuffer> piuBuffer;
            [tState measureTimeForStat:iTermTextRendererStatNewPIU
                               ofBlock:^{
                                   piuBuffer = [self piuBufferForPIUs:pius tState:tState instances:instances];
                               }];

            iTermMetalTextureBinding textureBindingsArray[3];
            NSUInteger textureBindingCount = 0;
            textureBindingsArray[textureBindingCount++] =
                iTermMetalTextureBindingMake(iTermTextureIndexPrimary, texture);
            if (!iTermTextIsMonochrome()) {
                textureBindingsArray[textureBindingCount++] =
                    iTermMetalTextureBindingMake(iTermTextureIndexSubpixelModels, self->_models);
                textureBindingsArray[textureBindingCount++] =
                    iTermMetalTextureBindingMake(iTermTextureIndexBackground, tState.backgroundTexture);
            }
            // Use pointer to capture in block (C arrays can't be captured directly)
            const iTermMetalTextureBinding *textureBindings = textureBindingsArray;
            ...
            [tState
                measureTimeForStat:iTermTextRendererStatNewDims
                           ofBlock:^{
                               iTermTextureDimensions textureDimensions = {
                                   ...
                                   .cellSize = cellSize,
                                   .underlineOffset = simd_make_float2(
                                       tState.asciiOffset.width,
                                       MAX(underlineThickness, cellSize.y - (underlineDescriptor.offset * scale))),
                                   .underlineThickness = underlineThickness,
                                   .strikethroughOffset = simd_make_float2(
                                       tState.asciiOffset.width, cellSize.y - strikethroughDescriptor.offset * scale),
                                   .strikethroughThickness = strikethroughThickness,
                                   .scale = scale,
                               };

                               // These tend to get reused so avoid changing the buffer if it is the same as the last
                               // one.
                               if (previousTextureDimensionsBuffer != nil &&
                                   ...
                                   textureDimensionsBuffer =
                                       [self->_dimensionsPool requestBufferFromContext:tState.poolContext
                                   ...
                                   std::memcpy((void *)&previousTextureDimensions, (void *)&textureDimensions,
                                               sizeof(textureDimensions));
                               }
                           }];

            [tState measureTimeForStat:iTermTextRendererStatDraw
                               ofBlock:^{
                                   // Change the pipeline state just before drawing so we get the right underlined/not
                                   // underlined state.
                                   ...
                                   const iTermMetalBufferBinding fragmentBindings[] = {
                                       iTermMetalBufferBindingMake(iTermFragmentInputIndexTextureDimensions,
                                                                   textureDimensionsBuffer),
                                   };
                                   const iTermMetalBufferBinding vertexBindings[] = {
                                       iTermMetalBufferBindingMake(iTermVertexInputIndexVertices, vertexBuffer),
                                       iTermMetalBufferBindingMake(iTermVertexTextInfo, textInfoBuffer),
                                   ...
                                   [self->_cellRenderer
                                       drawWithTransientState:tState
                                         ...
                                         fragmentBindingCount:sizeof(fragmentBindings) / sizeof(fragmentBindings[0])
...
- (void)setASCIICellSize:(CGSize)cellSize
      ...
      creationIdentifier:(id)creationIdentifier
                creation:(NSDictionary<NSNumber *, iTermCharacterBitmap *> * (^)(char,
                                                                                 iTermASCIITextureAttributes))creation {
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
    ...
    [self.class enumerateGridOfSubpixelModels:^(float textColor, float backgroundColor, iTermSubpixelModel *model) {
        NSString *name = [NSString
            stringWithFormat:@"SubpixelModelGridBwGlyph.t_%02x.b_%02x.dat", (int)textColor, (int)backgroundColor];
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer index:(NSUInteger)index toFolder:(NSURL *)folder {
    if (index == iTermVertexInputIndexPerInstanceUniforms) {
        const NSUInteger piuCount = buffer.length / sizeof(iTermTextPIU);
        NSMutableString *s = [[NSMutableString alloc] initWithCapacity:piuCount * 128];
