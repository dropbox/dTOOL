...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
    ...
    [[NSString stringWithFormat:@"descriptors=%@", descriptors]
...
- (nullable instancetype)initWithDevice:(id<MTLDevice>)device {
    ...
    if (self) {
        ...
        _indicatorDescriptors = [[NSMutableArray alloc] initWithCapacity:8];
        _identifierToTextureMap = [[NSMutableDictionary alloc] initWithCapacity:8];
...
- (nullable __kindof iTermMetalRendererTransientState *)
    ...
    __kindof iTermMetalRendererTransientState *_Nonnull transientState =
        [_metalRenderer createTransientStateForConfiguration:configuration commandBuffer:commandBuffer];
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    [tState.indicatorDescriptors enumerateObjectsUsingBlock:^(iTermIndicatorDescriptor *_Nonnull descriptor,
                                                              NSUInteger idx, BOOL *_Nonnull stop) {
        [self drawDescriptor:descriptor withRenderEncoder:frameData.renderEncoder transientState:tState];
    }];
}
...
- (void)drawDescriptor:(iTermIndicatorDescriptor *)descriptor
    ...
    const iTermMetalBufferBinding fragmentBindings[] = {
        iTermMetalBufferBindingMake(iTermFragmentBufferIndexIndicatorAlpha, alphaBuffer),
    };
    const iTermMetalTextureBinding textureBindings[] = {
        iTermMetalTextureBindingMake(iTermTextureIndexPrimary, descriptor.texture),
    };
...
- (id<MTLBuffer>)vertexBufferForFrame:(NSRect)frame
    ...
    CGRect quad = CGRectMake(CGRectGetMinX(frame) * scale, topInset + scale * CGRectGetMinY(frame),
    ...
    return [_metalRenderer.verticesPool requestBufferFromContext:context withBytes:vertices checkIfChanged:YES];
}
