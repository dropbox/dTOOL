...
@implementation iTermBackgroundColorRendererTransientState {
    DashTerm2::PIUArray<iTermBackgroundColorPIU> _pius;
}

- (NSUInteger)sizeOfNewPIUBuffer {
    return sizeof(iTermBackgroundColorPIU) * self.cellConfiguration.gridSize.width *
...
- (void)setColorRLEs:(const iTermMetalBackgroundColorRLE *)rles
    ...
    vector_float2 cellSize =
        simd_make_float2(self.cellConfiguration.cellSize.width, self.cellConfiguration.cellSize.height);
...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
    self = [super init];
    if (self) {
        _suppressedRegionVertexBufferPool = [[iTermMetalBufferPool alloc] initWithDevice:device
                                                                              bufferSize:sizeof(iTermVertex) * 6];
#if ENABLE_TRANSPARENT_METAL_WINDOWS
        if (iTermTextIsMonochrome()) {
            _nonblendingRenderer =
        ...
        _infoPool = [[iTermMetalBufferPool alloc] initWithDevice:device
                                                      bufferSize:sizeof(iTermMetalBackgroundColorInfo)];
    }
    return self;
}
...
- (nullable __kindof iTermMetalRendererTransientState *)
    ...
    __kindof iTermMetalCellRendererTransientState *_Nonnull transientState =
        [renderer createTransientStateForCellConfiguration:configuration commandBuffer:commandBuffer];
...
- (void)initializeTransientState:(iTermBackgroundColorRendererTransientState *)tState {
    tState.vertexBuffer =
        [[self rendererForConfiguration:tState.cellConfiguration] newQuadOfSize:tState.cellConfiguration.cellSize
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    const NSUInteger suppressedBottomPx =
        static_cast<NSUInteger>(tState.suppressedBottomHeight * tState.cellConfiguration.scale - tState.margins.top);
    [tState enumerateSegments:^(const iTermBackgroundColorPIU *pius, size_t numberOfInstances) {
        ...
        const iTermMetalBufferBinding vertexBindings[] = {
            ...
            iTermMetalBufferBindingMake(iTermVertexInputIndexDefaultBackgroundColorInfo, infoBuffer),
        };
    ...
    if (tState.suppressedBottomHeight > 0) {
        ...
        CGRect quad =
            CGRectMake(0, 0, tState.cellConfiguration.cellSize.width * tState.cellConfiguration.gridSize.width,
                       suppressedBottomPx);
        const CGRect textureFrame = CGRectMake(0, 0, 1, 1);
        const iTermVertex bottomRight =
            (iTermVertex){.position = simd_make_float2(NSMaxX(quad), NSMinY(quad)),
                          .textureCoordinate = simd_make_float2(NSMaxX(textureFrame), NSMaxY(textureFrame))};
        const iTermVertex bottomLeft =
            (iTermVertex){.position = simd_make_float2(NSMinX(quad), NSMinY(quad)),
                          .textureCoordinate = simd_make_float2(NSMinX(textureFrame), NSMaxY(textureFrame))};

        const iTermVertex topLeft =
            (iTermVertex){.position = simd_make_float2(NSMinX(quad), NSMaxY(quad)),
                          .textureCoordinate = simd_make_float2(NSMinX(textureFrame), NSMinY(textureFrame))};

        const iTermVertex topRight =
            (iTermVertex){.position = simd_make_float2(NSMaxX(quad), NSMaxY(quad)),
                          .textureCoordinate = simd_make_float2(NSMaxX(textureFrame), NSMinY(textureFrame))};

        iTermVertex vertices[] = {bottomRight, bottomLeft, topLeft, bottomRight, topLeft, topRight};
        id<MTLBuffer> vertexBuffer = [_suppressedRegionVertexBufferPool requestBufferFromContext:tState.poolContext
                                                                                       withBytes:vertices
                                                                                  checkIfChanged:YES];

        iTermBackgroundColorPIU piu = {.offset = simd_make_float2(0, 0),
                                       .runLength = 1,
                                       .numRows = 1,
        ...
        const iTermMetalBufferBinding suppressedVertexBindings[] = {
        ...
        [cellRenderer drawWithTransientState:tState
                            ...
                            numberOfVertices:6
                                numberOfPIUs:1
                              vertexBindings:suppressedVertexBindings
                          vertexBindingCount:sizeof(suppressedVertexBindings) / sizeof(suppressedVertexBindings[0])
                            fragmentBindings:NULL
                        fragmentBindingCount:0
                             textureBindings:NULL
                         textureBindingCount:0];
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer index:(NSUInteger)index toFolder:(NSURL *)folder {
    if (index == iTermVertexInputIndexPerInstanceUniforms) {
        ...
        const NSUInteger count = buffer.length / sizeof(*pius);
        NSMutableString *s = [[NSMutableString alloc] initWithCapacity:count * 96];
        for (int i = 0; i < count; i++) {
            [s appendFormat:@"offset=(%@, %@) runLength=%@ numRows=%@ color=(%@, %@, %@, %@)\n", @(pius[i].offset.x),
                            @(pius[i].offset.y), @(pius[i].runLength), @(pius[i].numRows), @(pius[i].color.x),
                            @(pius[i].color.y), @(pius[i].color.z), @(pius[i].color.w)];
        }
