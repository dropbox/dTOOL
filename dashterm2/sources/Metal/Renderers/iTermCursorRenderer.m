...
// RC-005: Changed from weak to strong to prevent renderer dealloc during render pass.
// TransientState is short-lived (one frame) so no retain cycle risk.
@property (nonatomic, strong) iTermCopyModeCursorRenderer *renderer;
...
// RC-005: Changed from weak to strong to prevent renderer dealloc during render pass.
// TransientState is short-lived (one frame) so no retain cycle risk.
@property (nonatomic, strong) iTermFrameCursorRenderer *renderer;
...
// Use bitmap context instead of lockFocus for thread safety (BUG-3216)
...
- (void)setSelecting:(BOOL)selecting {
    ...
    // RC-005: _renderer is now strong, so it's guaranteed to be valid during the render pass.
    // Still check for nil in case setSelecting is called before renderer is set.
    if (!_renderer) {
        return;
    }
...
// Use bitmap context instead of lockFocus for thread safety (BUG-3217)
- (iTermImageWrapper *)newImage {
    NSSize size = self.cellConfiguration.cellSize;
    if (size.width <= 0 || size.height <= 0) {
        return nil;
    }
...
- (void)setColor:(NSColor *)color {
    ...
    // RC-005: _renderer is now strong, so it's guaranteed to be valid during the render pass.
    // Still check for nil in case setColor is called before renderer is set.
    if (!_renderer) {
        return;
    }
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    const iTermMetalBufferBinding vertexBindings[] = {
        ...
        iTermMetalBufferBindingMake(iTermVertexInputIndexCursorDescription, descriptionBuffer),
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    const iTermMetalTextureBinding textureBindings[] = {
        iTermMetalTextureBindingMake(iTermTextureIndexPrimary, tState.backgroundIsDark ? _lightTexture : _darkTexture),
    };
    [_cellRenderer drawWithTransientState:tState
                            renderEncoder:frameData.renderEncoder
                         numberOfVertices:6
