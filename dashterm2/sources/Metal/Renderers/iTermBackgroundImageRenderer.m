...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    iTermMetalBufferBinding fragmentBindings[2];
    NSUInteger fragmentBindingCount = 0;
#if ENABLE_TRANSPARENT_METAL_WINDOWS
    ...
    if (alpha < 1) {
        ...
        fragmentBindings[fragmentBindingCount++] = iTermMetalBufferBindingMake(iTermFragmentInputIndexAlpha, alphaBuffer);
        fragmentBindings[fragmentBindingCount++] = iTermMetalBufferBindingMake(iTermFragmentInputIndexColor, underlayColorBuffer);
    } else {
    ...
    [_metalRenderer drawWithTransientState:tState
                      ...
                      vertexBindingCount:sizeof(vertexBindings) / sizeof(vertexBindings[0])
                          fragmentBindings:fragmentBindingCount ? fragmentBindings : NULL
                     fragmentBindingCount:fragmentBindingCount
                              textureBindings:textureBindings
                         textureBindingCount:sizeof(textureBindings) / sizeof(textureBindings[0])];

    // BUG-1112: Replace assert with proper nil check that works in release builds
    if (tState.box1 && tState.box2) {
        ...
        tState.pipelineState = _metalRenderer.pipelineState;
        const iTermMetalBufferBinding letterboxFragmentBindings[] = {
            iTermMetalBufferBindingMake(iTermFragmentInputIndexColor, letterboxColorBuffer),
        };
        const iTermMetalBufferBinding box1VertexBindings[] = {
            iTermMetalBufferBindingMake(iTermVertexInputIndexVertices, tState.box1),
        };
        [_metalRenderer drawWithTransientState:tState
                                 renderEncoder:frameData.renderEncoder
                              numberOfVertices:6
                                  numberOfPIUs:0
                                vertexBindings:box1VertexBindings
                           vertexBindingCount:sizeof(box1VertexBindings) / sizeof(box1VertexBindings[0])
                               fragmentBindings:letterboxFragmentBindings
                          fragmentBindingCount:sizeof(letterboxFragmentBindings) / sizeof(letterboxFragmentBindings[0])
                                    textureBindings:NULL
                               textureBindingCount:0];
        const iTermMetalBufferBinding box2VertexBindings[] = {
            iTermMetalBufferBindingMake(iTermVertexInputIndexVertices, tState.box2),
        };
        [_metalRenderer drawWithTransientState:tState
                                 renderEncoder:frameData.renderEncoder
                              numberOfVertices:6
                                  numberOfPIUs:0
                                vertexBindings:box2VertexBindings
                           vertexBindingCount:sizeof(box2VertexBindings) / sizeof(box2VertexBindings[0])
                               fragmentBindings:letterboxFragmentBindings
                          fragmentBindingCount:sizeof(letterboxFragmentBindings) / sizeof(letterboxFragmentBindings[0])
                                    textureBindings:NULL
                               textureBindingCount:0];
