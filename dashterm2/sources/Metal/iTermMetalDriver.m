...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p cursor=%@ range=%@>", NSStringFromClass(self.class), self,
...
@implementation iTermMetalDriver {
    ...
    // Protected by _lock (replaces @synchronized(self))
    ...
    // Row data cache for dirty line optimization.
    // Maps absolute line number (NSNumber) to iTermMetalRowData from previous frame.
    // Only valid if _rowDataCacheGridSize matches current grid size.
    NSMutableDictionary<NSNumber *, iTermMetalRowData *> *_rowDataCache;
    VT100GridSize _rowDataCacheGridSize;

    // Object pool for iTermMetalRowData to reduce per-frame allocation overhead.
    // Dirty rows acquire from pool instead of alloc/init.
    iTermMetalRowDataPool *_rowDataPool;
}
...
- (nullable instancetype)initWithDevice:(nonnull id<MTLDevice>)device {
    ...
    if (self) {
        ...
        _offscreenCommandLineBackgroundColorRenderer =
            [[iTermOffscreenCommandLineBackgroundColorRenderer alloc] initWithDevice:device];
        _backgroundColorRenderer = [[iTermBackgroundColorRenderer alloc] initWithDevice:device];
        _offscreenCommandLineBackgroundRenderer =
            [[iTermOffscreenCommandLineBackgroundRenderer alloc] initWithDevice:device];
        ...
        dispatch_once(&onceToken, ^{
            queue = dispatch_queue_create("com.dashterm.dashterm2.metalDriver", NULL);
            ;
        });
        _queue = queue;
#endif
        ...
        // Pre-allocate for max frames in flight - typically 3
        _currentFrames = [NSMutableArray arrayWithCapacity:iTermMetalDriverMaximumNumberOfFramesInFlight];
        _currentDrawableTime = [[MovingAverage alloc] init];
        _currentDrawableTime.alpha = 0.75;

        // Initialize row data cache for dirty line optimization
        // Typical terminal has 50-100 rows; use conservative initial capacity
        _rowDataCache = [[NSMutableDictionary alloc] initWithCapacity:128];
        _rowDataCacheGridSize = VT100GridSizeMake(0, 0);

        // Initialize row data pool for allocation reduction
        // Pool size: 4 frames at 64 rows = 256 objects
        _rowDataPool = [[iTermMetalRowDataPool alloc] init];

        _fpsMovingAverage = [[MovingAverage alloc] init];
        _fpsMovingAverage.alpha = 0.75;
#if ENABLE_STATS
        iTermMetalFrameDataStatsBundleInitialize(_stats);
        _statHistograms = [[NSArray sequenceWithRange:NSMakeRange(0, iTermMetalFrameDataStatCount)]
            mapWithBlock:^id(NSNumber *anObject) {
...
- (iTermMetalDriverMainThreadState *)mainThreadState {
    // BUG-f1380: Convert assert to guard - ensure main thread but don't crash if not
    if (![NSThread isMainThread]) {
        DLog(@"WARNING BUG-f1380: mainThreadState called from non-main thread");
        dispatch_sync(dispatch_get_main_queue(), ^{
                      }); // Wait for main thread opportunity
    }
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
    ...
    dispatch_async(_queue, ^{
        ...
        [self->_statHistograms
            enumerateObjectsUsingBlock:^(iTermHistogram *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                [obj clear];
            }];
    });
#else
...
- (void)setCellSize:(CGSize)cellSize
    ...
    DLog(@"Cell size is now %@x%@, grid size is now %@x%@", @(cellSize.width), @(cellSize.height), @(gridSize.width),
         @(gridSize.height));
    ...
    self.mainThreadState->extendBackgroundColorIntoMargins =
        [iTermAdvancedSettingsModel extendBackgroundColorIntoMargins];
}
...
- (int)maximumNumberOfFramesInFlight {
    ...
    if (_currentDrawableTime.numberOfMeasurements > 5 && _maxFramesInFlight > 1 &&
        _currentDrawableTime.value > slowEnoughToDowngradeThreshold) {
        DLog(@"Moving average of currentDrawable latency of %0.2fms with %@ measurements with mff of %@ is too high. "
             @"Decrease mff to %@",
             _currentDrawableTime.value * 1000, @(_currentDrawableTime.numberOfMeasurements), @(_maxFramesInFlight),
        ...
        DLog(@"Moving average of currentDrawable latency of %0.2fms with %@ measurements with mff of %@ is low. "
             @"Increase mff to %@",
             _currentDrawableTime.value * 1000, @(_currentDrawableTime.numberOfMeasurements), @(_maxFramesInFlight),
             @(_maxFramesInFlight + 1));
        _maxFramesInFlight += 1;
...
- (BOOL)reallyDrawInMTKView:(nonnull iTermMetalView *)view startToStartTime:(NSTimeInterval)startToStartTime {
    ...
    if (@available(macOS 10.15, *)) {
        self.mainThreadState->maximumExtendedDynamicRangeColorComponentValue =
            view.window.screen.maximumPotentialExtendedDynamicRangeColorComponentValue;
    }

    _total++;
    if (_total % 60 == 0) {
        os_unfair_lock_lock(&_lock);
        DLog(@"fps=%f (%d in flight)", (_total - _dropped) / ([NSDate timeIntervalSinceReferenceDate] - _startTime),
             (int)_currentFrames.count);
        DLog(@"%@", _currentFrames);
        os_unfair_lock_unlock(&_lock);
    }
...
- (iTermMetalFrameData *)newFrameDataForView:(iTermMetalView *)view {
    ...
    [frameData
        measureTimeForStat:iTermMetalFrameDataStatMtExtractFromApp
                   ofBlock:^{
                       frameData.viewportSize = self.mainThreadState->viewportSize;
                       frameData.extendBackgroundColorIntoMargins =
                           self.mainThreadState->extendBackgroundColorIntoMargins;
                       ...
                       // Phase 1 optimization: Get gridSize first so we can pre-allocate rows array
                       frameData.gridSize = frameData.perFrameState.gridSize;
                       frameData.rows = [NSMutableArray arrayWithCapacity:frameData.gridSize.height];
                       ...
                       frameData.extraMargins = NSEdgeInsetsMake(pointInsets.top * scale, pointInsets.left * scale,
                                                                 pointInsets.bottom * scale, pointInsets.right * scale);
                       frameData.vmargin = [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins];
                       if (@available(macOS 10.15, *)) {
                           frameData.maximumExtendedDynamicRangeColorComponentValue =
                               self.mainThreadState->maximumExtendedDynamicRangeColorComponentValue;
                       }
                   }];
    return frameData;
...
- (void)performPrivateQueueSetupForFrameData:(iTermMetalFrameData *)frameData view:(nonnull iTermMetalView *)view {
    DLog(@"Begin private queue setup for frame %@", frameData);
    // Show performance meters (FPS and/or input latency)
    if ([iTermAdvancedSettingsModel showMetalFPSmeter] || [iTermAdvancedSettingsModel showInputLatencyMeter]) {
        NSString *debugString = [self performanceMeterStringForFrameNumber:frameData.frameNumber];
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqBuildRowData
                          ofBlock:^{
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqUpdateRenderers
                          ofBlock:^{
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqCreateTransientStates
                          ofBlock:^{
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqPopulateTransientStates
                          ofBlock:^{
                              [self populateTransientStatesWithFrameData:frameData
                                                                   range:NSMakeRange(0, frameData.rows.count)];
                          }];
    ...
    [frameData enqueueDrawCallsWithBlock:^{
        [self enqueueDrawCallsForFrameData:frameData commandBuffer:commandBuffer];
    }];
}

- (void)addRowDataToFrameData:(iTermMetalFrameData *)frameData {
    // Dirty line optimization: Reuse cached row data for non-dirty rows.
    // This can reduce CPU time by 90%+ for common operations like cursor blink or typing
    // where only 1-2 rows change per frame.

    const VT100GridSize gridSize = frameData.gridSize;
    const int columns = gridSize.width;
    const BOOL allRowsDirty = frameData.perFrameState.allRowsDirty;
    NSIndexSet *dirtyRowIndices = frameData.perFrameState.dirtyRowIndices;

    // Check if cache is valid (same grid size)
    const BOOL cacheValid = !allRowsDirty &&
                            _rowDataCacheGridSize.width == gridSize.width &&
                            _rowDataCacheGridSize.height == gridSize.height &&
                            dirtyRowIndices != nil;

    // If cache is invalid, return all cached rows to pool before we lose track of them
    if (!cacheValid && _rowDataCache.count > 0) {
        [_rowDataPool returnRowDataArray:_rowDataCache.allValues];
        [_rowDataCache removeAllObjects];
    }

    // Prepare new cache for this frame
    NSMutableDictionary<NSNumber *, iTermMetalRowData *> *newCache =
        [[NSMutableDictionary alloc] initWithCapacity:gridSize.height];

    for (int y = 0; y < gridSize.height; y++) {
        const long long absLine = y + frameData.perFrameState.firstVisibleAbsoluteLineNumber;
        NSNumber *absLineKey = @(absLine);

        // Check if we can reuse cached row data
        iTermMetalRowData *cachedRowData = cacheValid ? _rowDataCache[absLineKey] : nil;
        const BOOL rowIsDirty = allRowsDirty || !cachedRowData || [dirtyRowIndices containsIndex:y];

        iTermMetalRowData *rowData;
        if (!rowIsDirty && cachedRowData) {
            // Reuse cached row data - this is the optimization!
            // NOTE: We still need to update screenCharArray as it may have been invalidated
            rowData = cachedRowData;
            rowData.y = y;  // Screen position may have changed due to scroll
            ...
            // Build new row data - acquire from pool instead of alloc
            // Pool pre-allocates data buffers sized for columns
            rowData = [_rowDataPool acquireRowDataWithColumns:columns];
            rowData.y = y;
            rowData.absLine = absLine;
            rowData.screenCharArray = [frameData.perFrameState screenCharArrayForRow:y];
            if (!rowData.screenCharArray) {
                ITDebugAssert(NO);
                rowData.screenCharArray = [ScreenCharArray emptyLineOfLength:columns];
            ...
            rowData.imageRuns = imageRuns;
            rowData.kittyImageRuns = kittyImageRuns;
            ...
            ITConservativeBetaAssert(drawableGlyphs <= rowData.keysData.length / sizeof(iTermMetalGlyphKey),
                                     @"Have %@ drawable glyphs with %@ glyph keys", @(drawableGlyphs),
        ...
        // Store in new cache
        newCache[absLineKey] = rowData;
    }

    // Return evicted row data to pool for reuse
    // These are entries that were in the old cache but not reused in the new cache
    if (_rowDataCache.count > 0) {
        NSMutableArray<iTermMetalRowData *> *evictedRows = [NSMutableArray array];
        for (NSNumber *key in _rowDataCache) {
            if (newCache[key] == nil) {
                // This row data was not reused - return to pool
                [evictedRows addObject:_rowDataCache[key]];
            }
        }
        if (evictedRows.count > 0) {
            [_rowDataPool returnRowDataArray:evictedRows];
        }
    }

    // Update cache for next frame
    _rowDataCache = newCache;
    _rowDataCacheGridSize = gridSize;
}
...
- (void)updateRenderersForNewFrameData:(iTermMetalFrameData *)frameData {
    ...
    [self.cellRenderers
        enumerateObjectsUsingBlock:^(id<iTermMetalCellRenderer> _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)createTransientStatesWithFrameData:(iTermMetalFrameData *)frameData
    ...
    for (id<iTermMetalRenderer> renderer in self.nonCellRenderers) {
        ...
        [frameData measureTimeForStat:renderer.createTransientStateStat
                              ofBlock:^{
...
- (void)populateTransientStatesWithFrameData:(iTermMetalFrameData *)frameData range:(NSRange)range {
...
- (id<MTLTexture>)destinationTextureForFrameData:(iTermMetalFrameData *)frameData {
    if (frameData.debugInfo) {
        ...
        [iTermTexture setBytesPerRow:frameData.destinationDrawable.texture.width * 4
                         rawDataSize:frameData.destinationDrawable.texture.width *
                                     frameData.destinationDrawable.texture.height * 4
                     samplesPerPixel:4
                          forTexture:texture];
        return texture;
    } else {
...
- (void)acquireScarceResources:(iTermMetalFrameData *)frameData view:(iTermMetalView *)view {
    ...
    if (frameData.debugInfo) {
        [frameData measureTimeForStat:iTermMetalFrameDataStatMtGetRenderPassDescriptor
                              ofBlock:^{
                                  frameData.renderPassDescriptor =
                                      [frameData newRenderPassDescriptorWithLabel:@"Offscreen debug texture" fast:NO];
                                  frameData.debugRealRenderPassDescriptor =
                                      [view currentRenderPassDescriptorWithTimeout:timeout];
                              }];
        [frameData measureTimeForStat:iTermMetalFrameDataStatMtGetCurrentDrawable
                              ofBlock:^{
                                  frameData.destinationDrawable = view.currentDrawable;
                                  frameData.destinationTexture =
                                      frameData.renderPassDescriptor.colorAttachments[0].texture;
                              }];

    } else {
#if ENABLE_DEFER_CURRENT_DRAWABLE
        const BOOL synchronousDraw = (_context.group != nil);
        frameData.deferCurrentDrawable =
            ([iTermPreferences boolForKey:kPreferenceKeyMaximizeThroughput] && !synchronousDraw);
#else
        frameData.deferCurrentDrawable = NO;
#endif
        if (!frameData.deferCurrentDrawable) {
            NSTimeInterval duration = [frameData
                measureTimeForStat:iTermMetalFrameDataStatMtGetCurrentDrawable
                           ofBlock:^{
...
#if ENABLE_UNFAMILIAR_TEXTURE_WORKAROUND
                               frameData.textureIsFamiliar =
                                   [self textureIsFamiliar:frameData.destinationDrawable.texture];
                               if (!frameData.textureIsFamiliar) {
                                   [_familiarTextures
                                       addPointer:(__bridge void *)frameData.destinationDrawable.texture];
                               }
        ...
        [frameData
            measureTimeForStat:iTermMetalFrameDataStatMtGetRenderPassDescriptor
                       ofBlock:^{
                           if (frameData.deferCurrentDrawable) {
                               frameData.renderPassDescriptor =
                                   [frameData newRenderPassDescriptorWithLabel:@"RPD for deferred draw" fast:YES];
                           } else {
...
- (void)enqueueDrawCallsForFrameData:(iTermMetalFrameData *)frameData
    ...
    [self updateRenderEncoderForCurrentPass:frameData label:firstLabel];
    ...
    [self drawCellRenderer:_textRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawText];

    _kittyImageRenderer.minZ = 1;
    _kittyImageRenderer.maxZ = INT32_MAX;
    [self drawCellRenderer:_kittyImageRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawImage];

    [self drawCellRenderer:_imageRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawImage];

    [self drawCellRenderer:_lineStyleMarkRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawMarks];

    [self drawCellRenderer:_arrowStyleMarkRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawMarks];

    [self drawCellRenderer:_blockRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawBlocks];
    ...
    [self drawCellRenderer:_rectangleRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawRectangles];

    if (!frameData.perFrameState.haveOffscreenCommandLine) {
        [self drawRenderer:_indicatorRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawIndicators];
        [self drawRenderer:_flashRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawFullScreenFlash];
    }

    [self drawCellRenderer:_timestampsRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawTimestamps];
    ...
    [self finishDrawingWithCommandBuffer:commandBuffer frameData:frameData];
}
...
- (void)configureTextRenderer:(iTermTextRenderer *)textRenderer frameData:(iTermMetalFrameData *)frameData {
    ...
    [textRenderer
          setASCIICellSize:cellSize
                    offset:frameData.asciiOffset
                descriptor:[frameData.perFrameState
                               characterSourceDescriptorForASCIIWithGlyphSize:glyphSize
        ...
        creationIdentifier:[frameData.perFrameState metalASCIICreationIdentifierWithOffset:frameData.asciiOffset]
                  creation:^NSDictionary<NSNumber *, iTermCharacterBitmap *> *(char c,
                                                                               iTermASCIITextureAttributes attributes) {
...
- (NSDictionary<NSNumber *, iTermCharacterBitmap *> *)dictionaryForASCIICharacter:(char)c
    ...
    iTermMetalGlyphKey glyphKey = {.type = iTermMetalGlyphTypeRegular,
...
- (void)updateIndicatorRendererForFrameData:(iTermMetalFrameData *)frameData {
    ...
    NSRect frame = NSMakeRect(0, 0, frameData.viewportSize.x / scale, frameData.viewportSize.y / scale);
    [_indicatorRenderer reset];
    [frameData.perFrameState enumerateIndicatorsInFrame:frame
                                                  block:^(iTermIndicatorDescriptor *_Nonnull indicator) {
                                                      [self->_indicatorRenderer
...
- (void)populateCopyBackgroundRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    iTermCopyBackgroundRendererTransientState *copyState =
        [frameData transientStateForRenderer:_copyBackgroundRenderer];
...
- (void)populateCursorRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    if (_underlineCursorRenderer.rendererDisabled && _barCursorRenderer.rendererDisabled &&
        _blockCursorRenderer.rendererDisabled && _imeCursorRenderer.rendererDisabled) {
        return;
    }
    ...
    if (!cursorInfo.frameOnly && cursorInfo.cursorVisible && cursorInfo.shouldDrawText && cursorInfo.coord.y >= 0 &&
        cursorInfo.coord.y < frameData.gridSize.height && cursorInfo.coord.x < frameData.gridSize.width) {
        iTermMetalRowData *rowWithCursor = frameData.rows[cursorInfo.coord.y];
        iTermMetalGlyphAttributes *glyphAttributes =
            (iTermMetalGlyphAttributes *)rowWithCursor.attributesData.mutableBytes;
        glyphAttributes[cursorInfo.coord.x].foregroundColor = cursorInfo.textColor;
        glyphAttributes[cursorInfo.coord.x].backgroundColor =
            simd_make_float4(cursorInfo.cursorColor.redComponent, cursorInfo.cursorColor.greenComponent,
                             cursorInfo.cursorColor.blueComponent, 1);
    ...
    if (cursorInfo.copyMode) {
        iTermCopyModeCursorRendererTransientState *tState =
            [frameData transientStateForRenderer:_copyModeCursorRenderer];
        ...
        switch (cursorInfo.type) {
            case CURSOR_UNDERLINE: {
                iTermCursorRendererTransientState *tState =
                    [frameData transientStateForRenderer:_underlineCursorRenderer];
                tState.coord = cursorInfo.coord;
                tState.color = cursorInfo.cursorColor;
                tState.doubleWidth = cursorInfo.doubleWidth;

                iTermCursorRendererTransientState *shadowTState =
                    [frameData transientStateForRenderer:_horizontalShadowCursorRenderer];
            ...
            case CURSOR_VERTICAL: {
                ...
                iTermCursorRendererTransientState *shadowTState =
                    [frameData transientStateForRenderer:_verticalShadowCursorRenderer];
                shadowTState.coord = cursorInfo.coord;
                shadowTState.color = cursorInfo.cursorColor;
                break;
            }
...
- (iTermTextRendererTransientState *)_populateTextRenderer:(iTermTextRenderer *)textRenderer
    ...
    [frameData.rows
        enumerateObjectsUsingBlock:^(iTermMetalRowData *_Nonnull rowData, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            if (imeInfo && rowData.y >= imeInfo.markedRange.start.y && rowData.y <= imeInfo.markedRange.end.y &&
            ...
            if (!textRenderer.rendererDisabled) {
                ITConservativeBetaAssert(
                    rowData.numberOfDrawableGlyphs * sizeof(iTermMetalGlyphKey) <= rowData.keysData.length,
                ...
                [textState setGlyphKeysData:rowData.keysData
                     ...
                     backgroundColorRLEData:rowData.backgroundColorRLEData
                          markedRangeOnLine:markedRangeOnLine
                                   ...
                                   creation:^NSDictionary<NSNumber *, iTermCharacterBitmap *> *_Nonnull(int gkIndex,
                                                                                                        BOOL *emoji) {
...
- (void)_populateBackgroundRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (!_backgroundColorRenderer.rendererDisabled) {
        BOOL (^comparator)(iTermMetalRowData *obj1, iTermMetalRowData *obj2) =
            ^BOOL(iTermMetalRowData *obj1, iTermMetalRowData *obj2) {
                const NSUInteger count = obj1.numberOfBackgroundRLEs;
                if (count != obj2.numberOfBackgroundRLEs) {
                    return NO;
                }
                const iTermMetalBackgroundColorRLE *array1 =
                    (const iTermMetalBackgroundColorRLE *)obj1.backgroundColorRLEData.mutableBytes;
                const iTermMetalBackgroundColorRLE *array2 =
                    (const iTermMetalBackgroundColorRLE *)obj2.backgroundColorRLEData.mutableBytes;
                for (int i = 0; i < count; i++) {
                    if (array1[i].color.x != array2[i].color.x || array1[i].color.y != array2[i].color.y ||
                        array1[i].color.z != array2[i].color.z || array1[i].color.w != array2[i].color.w ||
        ...
        [frameData.rows
            enumerateCoalescedObjectsWithComparator:comparator
                                              block:^(iTermMetalRowData *rowData, NSUInteger count) {
                                                  [backgroundState
                                                       setColorRLEs:(const iTermMetalBackgroundColorRLE *)
                                                                        rowData.backgroundColorRLEData.mutableBytes
...
- (void)_populateOffscreenCommandLineBackgroundRendererWithFrameData:(iTermMetalFrameData *)frameData {
    if (!_offscreenCommandLineBackgroundRenderer.rendererDisabled) {
        iTermOffscreenCommandLineBackgroundRendererTransientState *offscreenState =
            [frameData transientStateForRenderer:_offscreenCommandLineBackgroundRenderer];
        if (!frameData.perFrameState.haveOffscreenCommandLine) {
...
- (void)populateTextAndBackgroundRenderersTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (!_offscreenCommandLineBackgroundColorRenderer.rendererDisabled &&
        ...
        iTermBackgroundColorRendererTransientState *osclBottomState =
            [frameData transientStateForRenderer:_offscreenCommandLineBackgroundColorRenderer];
...
- (NSString *)performanceMeterStringForFrameNumber:(int)frameNumber {
    ...
    if ([iTermAdvancedSettingsModel showMetalFPSmeter]) {
        ...
        [result appendFormat:@" %dfps ", (int)round(fps)];
    }

    // Input latency meter
    if ([iTermAdvancedSettingsModel showInputLatencyMeter]) {
        iTermInputLatencyTracker *tracker = [iTermInputLatencyTracker sharedInstance];
        if (tracker.hasValidData) {
            [result appendFormat:@" %@ ", tracker.latencyDisplayString];
        }
    }

    if (result.length > 0) {
        return [NSString
            stringWithFormat:@"[%@]", [result stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
    }
    return @"";
}
...
- (void)populateCursorGuideRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    iTermMetalCursorInfo *cursorInfo = frameData.perFrameState.metalDriverCursorInfo;
    if (cursorInfo.coord.y >= 0 && cursorInfo.coord.y < frameData.gridSize.height) {
...
- (void)populateTimestampsRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (frameData.perFrameState.timestampsEnabled) {
        ...
        if (frameData.perFrameState.haveOffscreenCommandLine) {
            tState.obscured =
                frameData.cellSize.height / frameData.scale + iTermOffscreenCommandLineVerticalPadding * 2;
        } else {
            tState.obscured = 0;
        }
...
- (CGFloat)offscreenCommandLineHeight:(iTermMetalFrameData *)frameData {
    if (!_offscreenCommandLineBackgroundColorRenderer.rendererDisabled &&
        frameData.perFrameState.haveOffscreenCommandLine) {
        return [iTermOffscreenCommandLineBackgroundRendererTransientState
            heightForScale:frameData.scale
                 rowHeight:frameData.cellSize.height
...
- (void)populateMarginRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (frameData.extendBackgroundColorIntoMargins && frameData.perFrameState.softAlternateScreenMode) {
        [frameData.rows enumerateObjectsUsingBlock:^(iTermMetalRowData *rowData, NSUInteger idx, BOOL *_Nonnull stop) {
            const iTermMetalBackgroundColorRLE *rles =
                (const iTermMetalBackgroundColorRLE *)rowData.backgroundColorRLEData.bytes;
...
- (void)populateRectangleRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (frameData.perFrameState.hasSelectedCommand) {
        ...
        if (frameData.perFrameState.pointsOnBottomToSuppressDrawing > 0) {
            ...
            [tState setClipRect:NSMakeRect(0, y, frameData.viewportSize.x, MAX(0, frameData.viewportSize.y - y))];
        }
    }
}
...
- (void)populateTerminalButtonRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData
    ...
    for (iTermTerminalButton *button in frameData.perFrameState.terminalButtons) {
        ...
        if (VT100GridRangeContains(frameData.perFrameState.linesToSuppressDrawing,
                                   (button.absCoordForDesiredFrame.y - firstLine))) {
            continue;
        }
    ...
    if (frameData.perFrameState.buttonsBackgroundRects.count > 0) {
        iTermRectangleRendererTransientState *bgTState =
            [frameData transientStateForRenderer:_buttonsBackgroundRenderer];
...
- (void)populateImageRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    [frameData.rows enumerateObjectsUsingBlock:^(iTermMetalRowData *_Nonnull row, NSUInteger idx, BOOL *_Nonnull stop) {
        [row.imageRuns
            enumerateObjectsUsingBlock:^(iTermMetalImageRun *_Nonnull imageRun, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)populateBackgroundImageRendererTransientStateWithFrameData:(iTermMetalFrameData *)frameData {
    iTermBackgroundImageRendererTransientState *tState = [frameData transientStateForRenderer:_backgroundImageRenderer];
    tState.computedAlpha =
        iTermAlphaValueForBottomView(1 - frameData.perFrameState.transparencyAlpha, frameData.perFrameState.blend);
...
- (void)drawRenderer:(id<iTermMetalRenderer>)renderer
    ...
    [frameData measureTimeForStat:stat
                          ofBlock:^{
...
- (void)drawCellRenderer:(id<iTermMetalCellRenderer>)renderer
    ...
    [frameData measureTimeForStat:stat
                          ofBlock:^{
                              iTermMetalCellRendererTransientState *state =
                                  [frameData transientStateForRenderer:renderer];
                              state.sequenceNumber = frameData.numberOfRenderersDrawn;
                              frameData.numberOfRenderersDrawn = frameData.numberOfRenderersDrawn + 1;
                              if (state != nil && !state.skipRenderer) {
                                  [renderer drawWithFrameData:frameData transientState:state];
...
- (void)saveRenderOutputForDebuggingIfNeeded:(iTermMetalFrameData *)frameData
    ...
    if (frameData.debugInfo && frameData.debugRealRenderPassDescriptor) {
        ...
        id<MTLBuffer> pixelBuffer =
            [self bufferWithContentsOfDestinationTextureInFrameData:frameData label:NSStringFromClass([state class])];
        ...
        [self updateRenderEncoderForCurrentPass:frameData label:@"Save output for debugging"];
    }
}

- (void)updateRenderEncoderForCurrentPass:(iTermMetalFrameData *)frameData label:(NSString *)label {
    ...
    int safePass = pass; // Mutable copy for potential clamping
    if (useTemporaryTexture) {
        // BUG-f1381: Convert assert to guard - clamp pass to valid range instead of crash
        if (pass < 0 || pass > 2) {
            DLog(@"WARNING BUG-f1381: render pass %d out of range [0,2] for temporary texture mode - clamping", pass);
            safePass = MAX(0, MIN(pass, 2));
        }

        descriptors = @[
            frameData.intermediateRenderPassDescriptor ?: frameData.temporaryRenderPassDescriptor,
            frameData.temporaryRenderPassDescriptor, frameData.renderPassDescriptor
        ];
    } else {
        // No temporary texture
        // BUG-f1382: Convert assert to guard - clamp pass to valid range instead of crash
        if (pass < 0 || pass > 1) {
            DLog(@"WARNING BUG-f1382: render pass %d out of range [0,1] for non-temporary mode - clamping", pass);
            safePass = MAX(0, MIN(pass, 1));
        }
        descriptors = @[
            frameData.intermediateRenderPassDescriptor ?: frameData.renderPassDescriptor,
            frameData.renderPassDescriptor, frameData.renderPassDescriptor
        ];
    }

    [frameData updateRenderEncoderWithRenderPassDescriptor:descriptors[safePass] stat:stats[safePass] label:label];
    frameData.destinationTexture = [descriptors[safePass].colorAttachments[0] texture];
}
...
- (void)drawCursorAfterTextWithFrameData:(iTermMetalFrameData *)frameData {
            ...
            switch (cursorInfo.type) {
                case CURSOR_UNDERLINE:
                    if (cursorInfo.cursorShadow) {
                        [self drawCellRenderer:_horizontalShadowCursorRenderer
                                     frameData:frameData
                                          stat:iTermMetalFrameDataStatPqEnqueueDrawCursor];
                    }
                    break;
                case CURSOR_VERTICAL:
                    if (cursorInfo.cursorShadow) {
                        [self drawCellRenderer:_verticalShadowCursorRenderer
                                     frameData:frameData
                                          stat:iTermMetalFrameDataStatPqEnqueueDrawCursor];
                    }
                    break;
    ...
    if (frameData.perFrameState.imeInfo) {
        [self drawCellRenderer:_imeCursorRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawCursor];
    }
}
...
- (void)drawContentBehindTextWithFrameData:(iTermMetalFrameData *)frameData {
    ...
    [self drawCellRenderer:_marginRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueDrawMargin];
    ...
    [self drawRenderer:_badgeRenderer frameData:frameData stat:iTermMetalFrameDataStatPqEnqueueBadge];
    ...
    if (frameData.intermediateRenderPassDescriptor) {
        [frameData measureTimeForStat:iTermMetalFrameDataStatPqEnqueueDrawEndEncodingToIntermediateTexture
                              ofBlock:^{
                                  [frameData.renderEncoder endEncoding];
                              }];
    }
...
- (BOOL)deferredRequestCurrentDrawableWithCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqBlockOnSynchronousGetDrawable
                          ofBlock:^{
                              ...
                              if (!self.waitingOnSynchronousDraw) {
                                  ...
                                  dispatch_async(dispatch_get_main_queue(), ^{
                                      ...
                                      [frameData measureTimeForStat:iTermMetalFrameDataStatMtGetCurrentDrawable
                                                            ofBlock:^{
                                                                ...
                                                                DLog(@"%p DEFERRED PATH: got drawable %@ for frame %@",
                                                                     self, drawable, @(frameData.frameNumber));
                                                                texture = drawable.texture;
                                                                renderPassDescriptor =
                                                                    frameData.view.currentRenderPassDescriptor;
                                                            }];
                                      os_unfair_lock_unlock(&self->_lock);
                                  });
                                  ...
                                  const BOOL semaphoreTimeout =
                                      (dispatch_semaphore_wait(
                                           sema, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC))) != 0);
                                  os_unfair_lock_lock(&self->_lock);
                                  if (semaphoreTimeout) {
...
- (void)finishDrawingWithCommandBuffer:(id<MTLCommandBuffer>)commandBuffer frameData:(iTermMetalFrameData *)frameData {
    ...
    [frameData measureTimeForStat:iTermMetalFrameDataStatPqEnqueueDrawEndEncodingToDrawable
                          ofBlock:^{
    ...
    [frameData
        measureTimeForStat:iTermMetalFrameDataStatPqEnqueueDrawPresentAndCommit
                   ofBlock:^{
                       ...
                       iTermPreciseTimerStats *scheduleWaitStat =
                           &frameData.stats[iTermMetalFrameDataStatGpuScheduleWait];
                       iTermHistogram *scheduleWaitHist =
                           frameData.statHistograms[iTermMetalFrameDataStatGpuScheduleWait];
...
- (BOOL)didComplete:(BOOL)completed withFrameData:(iTermMetalFrameData *)frameData {
    ...
    if (!completed) {
        // Record frame presentation for input latency measurement
        if ([iTermAdvancedSettingsModel showInputLatencyMeter]) {
            [[iTermInputLatencyTracker sharedInstance] recordFramePresented];
        }
...
- (void)complete:(iTermMetalFrameData *)frameData {
    ...
    // Row data objects are released when frameData.rows is cleared/deallocated.
    // Object pooling was removed - see addRowDataToFrameData: for explanation.
    ...
    if (!_offscreenCommandLineTextRenderer.rendererDisabled) {
        iTermTextRendererTransientState *textState =
            [frameData transientStateForRenderer:_offscreenCommandLineTextRenderer];
        [textState didComplete];
    }
...
#else
    [frameData didCompleteWithAggregateStats:nil histograms:nil owner:_identifier additional:@""];
#endif
...
- (NSArray<id<iTermMetalRenderer>> *)nonCellRenderers {
    NSArray *shared = @[
        _backgroundImageRenderer, _offscreenCommandLineBackgroundRenderer, _badgeRenderer, _copyBackgroundRenderer,
        _indicatorRenderer, _flashRenderer
    ];
