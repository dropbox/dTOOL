...
// Dirty line tracking for incremental rendering optimization.
// Returns YES if all visible rows should be considered dirty (e.g., first frame, resize, scroll).
@property (nonatomic, readonly) BOOL allRowsDirty;
// Returns the set of screen-relative row indices (0 to gridSize.height-1) that have changed
// since the last frame. Only valid if allRowsDirty is NO.
@property (nonatomic, readonly) NSIndexSet *dirtyRowIndices;
...
- (void)metalGetGlyphKeysData:(iTermGlyphKeyData *)glyphKeysData
                glyphKeyCount:(out NSUInteger *)glyphKeyCountPtr
                   attributes:(iTermMetalGlyphAttributes *)attributes
                    imageRuns:(NSMutableArray<iTermMetalImageRun *> * __autoreleasing _Nullable *)imageRuns
               kittyImageRuns:(NSMutableArray<iTermKittyImageRun *> * __autoreleasing _Nullable *)kittyImageRuns
...
- (void)metalDidFindImages:(NSSet<NSString *> *)foundImages
             ...
             animatedLines:(NSIndexSet *)animatedLines;  // absolute line numbers
