...
static const NSUInteger kMaxPendingUploads = 32;  // Flush when this many glyphs staged

// Represents a pending glyph upload
typedef struct {
    NSUInteger slice;
    // Offset into _stagingBuffer where this glyph's data starts
...
@implementation iTermTextureArray {
    ...
    // Batched upload state
    NSMutableData *_stagingBuffer;  // CPU buffer for pending glyph data
    iTermPendingUpload _pendingUploads[kMaxPendingUploads];
    NSUInteger _pendingUploadCount;
    NSUInteger _bytesPerCell;  // Cached for staging calculations
}

@synthesize pendingUploadCount = _pendingUploadCount;

+ (CGSize)atlasSizeForUnitSize:(CGSize)unitSize arrayLength:(NSUInteger)length cellsPerRow:(out NSInteger *)cellsPerRowOut {
    // BUG-1126: Validate inputs to prevent integer overflow
    if (unitSize.width <= 0 || unitSize.height <= 0 || length == 0) {
        if (cellsPerRowOut) {
            *cellsPerRowOut = 1;
        }
        return CGSizeMake(1, 1);
    }
    // Use double throughout to avoid overflow
...
- (instancetype)initWithTextureWidth:(uint32_t)width
    ...
    if (self) {
        ...
        // BUG-1115: Check texture creation failure
        if (!_texture) {
            return nil;
        }
        ...
        // Initialize batched upload state
        _bytesPerCell = width * height * 4;  // BGRA8 = 4 bytes per pixel
        _pendingUploadCount = 0;
        // Pre-allocate staging buffer for batch size
        _stagingBuffer = [[NSMutableData alloc] initWithCapacity:_bytesPerCell * kMaxPendingUploads];
    }
...
- (void)dealloc {
    // Flush any pending uploads before releasing texture
    [self flushStagedBitmaps];
    _stagingBuffer = nil;
    _texture = nil;
}
...
#pragma mark - Batched Upload API

- (void)stageBitmapForSlice:(NSUInteger)slice withBitmap:(iTermCharacterBitmap *)bitmap {
    ITDebugAssert(slice < _arrayLength);
    ITDebugAssert(bitmap.data.length <= _bytesPerCell);

    // Auto-flush if batch is full
    if (_pendingUploadCount >= kMaxPendingUploads) {
        [self flushStagedBitmaps];
    }

    // Record the upload info
    _pendingUploads[_pendingUploadCount].slice = slice;
    _pendingUploads[_pendingUploadCount].dataOffset = _stagingBuffer.length;
    _pendingUploads[_pendingUploadCount].dataSize = bitmap.data.length;

    // Copy bitmap data to staging buffer
    [_stagingBuffer appendData:bitmap.data];
...
- (void)flushStagedBitmaps {
    ...
    const unsigned char *stagingBytes = (const unsigned char *)_stagingBuffer.bytes;
    const NSUInteger bytesPerRow = _width * 4;  // BGRA8
    const NSUInteger bytesPerImage = _bytesPerCell;

    // Upload each staged glyph to GPU.
    // The batching benefit comes from deferring CPU work and reducing
    // the frequency of main thread to GPU synchronization points.
    // Future optimization: detect contiguous slices for single replaceRegion call.
    for (NSUInteger i = 0; i < _pendingUploadCount; i++) {
        const NSUInteger slice = _pendingUploads[i].slice;
        const NSUInteger dataOffset = _pendingUploads[i].dataOffset;
        const MTLOrigin origin = [self offsetForIndex:slice];
        const MTLRegion region = MTLRegionMake2D(origin.x, origin.y, _width, _height);
        ...
        [_texture replaceRegion:region
                    mipmapLevel:0
                          slice:0
                      withBytes:stagingBytes + dataOffset
                    bytesPerRow:bytesPerRow
                  bytesPerImage:bytesPerImage];
    }

    // Reset staging state
    _stagingBuffer.length = 0;
    _pendingUploadCount = 0;
}
