...
// Optimization: Pre-allocate NSNumber objects for buffer indices 0-15.
// This eliminates per-draw-call @(index) boxing when setting vertex/fragment
// buffers and textures on the render encoder. Buffer indices in Metal shaders
// typically fall in this range (vertex: 0-7, fragment: 0-7, texture: 0-2).
static const int kCachedBufferIndexCount = 16;
static NSNumber *sBufferIndexCache[kCachedBufferIndexCount];

__attribute__((constructor)) static void iTermMetalRendererInitBufferIndexCache(void) {
    for (int i = 0; i < kCachedBufferIndexCount; i++) {
        sBufferIndexCache[i] = @(i);
    }
}

NSNumber *iTermBufferIndexToNumber(NSUInteger index) {
    if (index < kCachedBufferIndexCount) {
        return sBufferIndexCache[index];
    }
    return @(index);
}
...
- (instancetype)initWithViewportSize:(vector_uint2)viewportSize
    ...
    if (self) {
        ...
        _viewportSizeExcludingLegacyScrollbars = simd_make_uint2(viewportSize.x - legacyScrollbarWidth, viewportSize.y);
        _scale = scale;
...
- (NSString *)debugDescription {
    return [NSString stringWithFormat:@"<%@: %p viewportSize=%@x%@ scale=%@>", NSStringFromClass([self class]), self,
                                      @(self.viewportSize.x), @(self.viewportSize.y), @(self.scale)];
}
...
@implementation iTermMetalRenderer {
    ...
    // Single-entry cache for pipeline state lookup. Most frames use the same
    // pipeline state, so this avoids repeated NSDictionary lookups.
    NSDictionary *_lastPipelineKey;
...
- (nullable instancetype)initWithDevice:(id<MTLDevice>)device
    ...
    if (self) {
        ...
        // Phase 1 optimization: Pre-size for typical pipeline state cache
        _pipelineStates = [[NSMutableDictionary alloc] initWithCapacity:4];
    }
    return self;
}
...
- (NSDictionary *)keyForPipelineState {
    ...
    return @{@"fragment function" : _fragmentFunctionName ?: @"", @"vertex function" : _vertexFunctionName ?: @""};
}

- (id<MTLRenderPipelineState>)pipelineState {
    NSDictionary *key = [self keyForPipelineState];

    // Fast path: check single-entry cache first (most frames use the same pipeline state)
    if (_lastPipelineState && [_lastPipelineKey isEqualToDictionary:key]) {
        return _lastPipelineState;
    }

    id<MTLRenderPipelineState> state = _pipelineStates[key];
    if (state == nil) {
        ...
        state = [self newPipelineWithBlending:_blending vertexFunction:vertexShader fragmentFunction:fragmentShader];
        _pipelineStates[key] = state;
    }
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer index:(NSUInteger)index toFolder:(NSURL *)folder {
    ...
    if (index == iTermVertexInputIndexVertices) {
        const NSUInteger vertexCount = buffer.length / sizeof(iTermVertex);
        NSMutableString *s = [[NSMutableString alloc] initWithCapacity:vertexCount * 64];
        ...
        NSData *data = [NSData dataWithBytesNoCopy:buffer.contents length:buffer.length freeWhenDone:NO];
...
- (nullable __kindof iTermMetalRendererTransientState *)
    createTransientStateForConfiguration:(iTermRenderConfiguration *)configuration
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData transientState:(NSDictionary *)transientState {
    [self doesNotRecognizeSelector:_cmd];
}
...
- (id<MTLBuffer>)newQuadWithFrame:(CGRect)quad
    ...
    const iTermVertex vertices[] = {
        ...
        {{CGRectGetMinX(quad), CGRectGetMaxY(quad)},
         {CGRectGetMinX(textureFrame), textureFrame.origin.y + textureFrame.size.height}},

        {{CGRectGetMaxX(quad), CGRectGetMinY(quad)}, {CGRectGetMaxX(textureFrame), textureFrame.origin.y}},
        {{CGRectGetMinX(quad), CGRectGetMaxY(quad)},
         {CGRectGetMinX(textureFrame), textureFrame.origin.y + textureFrame.size.height}},
        {{CGRectGetMaxX(quad), CGRectGetMaxY(quad)},
         {CGRectGetMaxX(textureFrame), textureFrame.origin.y + textureFrame.size.height}},
    };
    return [_verticesPool requestBufferFromContext:poolContext withBytes:vertices checkIfChanged:YES];
}
...
- (id<MTLBuffer>)newQuadOfSize:(CGSize)size poolContext:(iTermMetalBufferPoolContext *)poolContext {
    const iTermVertex vertices[] = {
        // Pixel Positions             Texture Coordinates
        {{size.width, 0}, {1.f, 0.f}}, {{0, 0}, {0.f, 0.f}},           {{0, size.height}, {0.f, 1.f}},

        {{size.width, 0}, {1.f, 0.f}}, {{0, size.height}, {0.f, 1.f}}, {{size.width, size.height}, {1.f, 1.f}},
    };
    return [_verticesPool requestBufferFromContext:poolContext withBytes:vertices checkIfChanged:YES];
}
...
- (id<MTLBuffer>)newQuadOfSize:(CGSize)size
    ...
    const iTermVertex vertices[] = {
        // Pixel Positions             Texture Coordinates
        {{NSMaxX(rect), NSMinY(rect)}, {1.f, 0.f}}, {{NSMinX(rect), NSMinY(rect)}, {0.f, 0.f}},
        {{NSMinX(rect), NSMaxY(rect)}, {0.f, 1.f}},

        {{NSMaxX(rect), NSMinY(rect)}, {1.f, 0.f}}, {{NSMinX(rect), NSMaxY(rect)}, {0.f, 1.f}},
...
- (id<MTLBuffer>)newFlippedQuadOfSize:(CGSize)size poolContext:(iTermMetalBufferPoolContext *)poolContext {
    const iTermVertex vertices[] = {
        // Pixel Positions, Texture Coordinates
        {{size.width, 0}, {1.f, 1.f}}, {{0, 0}, {0.f, 1.f}},           {{0, size.height}, {0.f, 0.f}},

        {{size.width, 0}, {1.f, 1.f}}, {{0, size.height}, {0.f, 0.f}}, {{size.width, size.height}, {1.f, 0.f}},
    };
    return [_verticesPool requestBufferFromContext:poolContext withBytes:vertices checkIfChanged:YES];
}
...
- (id<MTLRenderPipelineState>)newPipelineWithBlending:(iTermMetalBlending *)blending
    ...
    id<MTLRenderPipelineState> pipeline = [_device newRenderPipelineStateWithDescriptor:pipelineStateDescriptor
                                                                                  error:&error];
    // BUG-1116: Handle pipeline creation failure in release builds
    if (!pipeline) {
        DLog(@"Failed to create pipeline state: %@", error);
        return nil;
    }
    return pipeline;
}

- (nullable id<MTLTexture>)textureFromImage:(iTermImageWrapper *)image
...
- (nullable id<MTLTexture>)textureFromImage:(iTermImageWrapper *)wrapper
    ...
    [self convertWidth:bitmap.size.width // use pixelWidth/pixelHeight if some weird jpegs get clipped
               ...
               toWidth:&width
                height:&height
          notExceeding:4096];
    DLog(@"converted size from %@ to %@", NSStringFromSize(bitmap.size), NSStringFromSize(NSMakeSize(width, height)));
    if (width == 0 || height == 0) {
        return nil;
    }
    ...
    if ([bitmap metalPixelFormat] == MTLPixelFormatInvalid) {
        return [self legacyTextureFromImage:image context:context pool:pool colorSpace:colorSpace];
    }
    ...
    if (pool) {
        texture = [pool requestTextureOfSize:simd_make_uint2(bitmap.size.width, bitmap.size.height)];
    }
    if (!texture) {
...
// Use bitmap context instead of lockFocus to avoid threading issues (BUG-3214)
- (nullable id<MTLTexture>)legacyTextureFromImage:(iTermImageWrapper *)image
    ...
    [image.image drawInRect:NSMakeRect(0, 0, size.width, size.height)
                   fromRect:NSZeroRect
...
- (void)drawWithTransientState:(iTermMetalRendererTransientState *)tState
              ...
              numberOfVertices:(NSInteger)numberOfVertices
                ...
                vertexBindings:(const iTermMetalBufferBinding *)vertexBindings
            vertexBindingCount:(NSUInteger)vertexBindingCount
              fragmentBindings:(const iTermMetalBufferBinding *)fragmentBindings
          fragmentBindingCount:(NSUInteger)fragmentBindingCount
               textureBindings:(const iTermMetalTextureBinding *)textureBindings
           textureBindingCount:(NSUInteger)textureBindingCount {
    ...
    if (tState.suppressedBottomHeight > 0 || tState.suppressedTopHeight > 0) {
        ...
        MTLScissorRect scissorRect = {.x = 0,
                                      .y = suppressedTopPx,
                                      .width = tState.configuration.viewportSize.x,
                                      .height =
                                          tState.configuration.viewportSize.y - suppressedBottomPx - suppressedTopPx};
        [renderEncoder setScissorRect:scissorRect];
    }

    // Optimization: Set viewport buffer directly instead of creating an intermediate dictionary.
    // This avoids one mutableCopy + dictionary allocation per draw call.
    id<MTLBuffer> viewportBuffer = [self vertexBufferForViewportSize:tState.configuration.viewportSize];
    [debugDrawInfo setVertexBuffer:viewportBuffer atIndex:iTermVertexInputIndexViewportSize];
    [renderEncoder setVertexBuffer:viewportBuffer offset:0 atIndex:iTermVertexInputIndexViewportSize];

    // Optimization: Use fast enumeration with cached NSNumber objects.
    // This avoids block capture overhead and repeated @(index) boxing.
    for (NSUInteger i = 0; i < vertexBindingCount; i++) {
        const iTermMetalBufferBinding binding = vertexBindings[i];
        if (!binding.buffer) {
            continue;
        }
        [debugDrawInfo setVertexBuffer:binding.buffer atIndex:binding.index];
        [renderEncoder setVertexBuffer:binding.buffer offset:0 atIndex:binding.index];
    }

    for (NSUInteger i = 0; i < fragmentBindingCount; i++) {
        const iTermMetalBufferBinding binding = fragmentBindings[i];
        if (!binding.buffer) {
            continue;
        }
        [debugDrawInfo setFragmentBuffer:binding.buffer atIndex:binding.index];
        [renderEncoder setFragmentBuffer:binding.buffer offset:0 atIndex:binding.index];
    }

    for (NSUInteger i = 0; i < textureBindingCount; i++) {
        const iTermMetalTextureBinding binding = textureBindings[i];
        if (!binding.texture) {
            continue;
        }
        [debugDrawInfo setFragmentTexture:binding.texture atIndex:binding.index];
        [renderEncoder setFragmentTexture:binding.texture atIndex:binding.index];
    }
...
- (void)drawWithTransientState:(iTermMetalRendererTransientState *)tState
    ...
    const NSUInteger vertexBindingCount = clientVertexBuffers.count;
    iTermMetalBufferBinding *vertexBindings =
        vertexBindingCount ? alloca(sizeof(iTermMetalBufferBinding) * vertexBindingCount) : NULL;
    NSUInteger vertexIndex = 0;
    for (NSNumber *key in clientVertexBuffers) {
        vertexBindings[vertexIndex++] = iTermMetalBufferBindingMake(key.unsignedIntegerValue, clientVertexBuffers[key]);
    }

    const NSUInteger fragmentBindingCount = fragmentBuffers.count;
    iTermMetalBufferBinding *fragmentBindings =
        fragmentBindingCount ? alloca(sizeof(iTermMetalBufferBinding) * fragmentBindingCount) : NULL;
    NSUInteger fragmentIndex = 0;
    for (NSNumber *key in fragmentBuffers) {
        fragmentBindings[fragmentIndex++] = iTermMetalBufferBindingMake(key.unsignedIntegerValue, fragmentBuffers[key]);
    }

    const NSUInteger textureBindingCount = textures.count;
    iTermMetalTextureBinding *textureBindings =
        textureBindingCount ? alloca(sizeof(iTermMetalTextureBinding) * textureBindingCount) : NULL;
    NSUInteger textureIndex = 0;
    for (NSNumber *key in textures) {
        textureBindings[textureIndex++] = iTermMetalTextureBindingMake(key.unsignedIntegerValue, textures[key]);
    }
    ...
    [self drawWithTransientState:tState
            ...
            fragmentBindingCount:fragmentBindingCount
