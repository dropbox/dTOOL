...
} iTermMetalBufferBinding;

typedef struct {
    NSUInteger index;
    __unsafe_unretained id<MTLTexture> texture;
} iTermMetalTextureBinding;

NS_INLINE iTermMetalBufferBinding iTermMetalBufferBindingMake(NSUInteger index,
                                                             id<MTLBuffer> _Nullable buffer) {
    iTermMetalBufferBinding binding = { .index = index, .buffer = buffer };
    return binding;
}

NS_INLINE iTermMetalTextureBinding iTermMetalTextureBindingMake(NSUInteger index,
                                                               id<MTLTexture> _Nullable texture) {
    iTermMetalTextureBinding binding = { .index = index, .texture = texture };
    return binding;
}
...
- (void)drawWithTransientState:(iTermMetalRendererTransientState *)tState
              ...
              numberOfVertices:(NSInteger)numberOfVertices
                  numberOfPIUs:(NSInteger)numberOfPIUs
                vertexBindings:(const iTermMetalBufferBinding *_Nullable)vertexBindings
           vertexBindingCount:(NSUInteger)vertexBindingCount
             fragmentBindings:(const iTermMetalBufferBinding *_Nullable)fragmentBindings
        fragmentBindingCount:(NSUInteger)fragmentBindingCount
               textureBindings:(const iTermMetalTextureBinding *_Nullable)textureBindings
...
// Returns a cached NSNumber for buffer indices 0-15, avoiding per-call @(index) boxing.
// Use this when creating dictionaries for drawWithTransientState: to eliminate allocations.
