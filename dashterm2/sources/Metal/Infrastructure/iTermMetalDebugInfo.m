...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Phase 1 optimization: Pre-size for typical buffer counts in Metal rendering
        _vertexBuffers = [[NSMutableDictionary alloc] initWithCapacity:4];
        _fragmentBuffers = [[NSMutableDictionary alloc] initWithCapacity:4];
        _fragmentTextures = [[NSMutableDictionary alloc] initWithCapacity:4];
    }
    return self;
}
...
- (void)drawWithVertexCount:(NSUInteger)vertexCount instanceCount:(NSUInteger)instanceCount {
...
- (void)writeToFolder:(NSURL *)folder {
    [_vertexBuffers
        enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, id<MTLBuffer> _Nonnull obj, BOOL *_Nonnull stop) {
    ...
    [_fragmentTextures
        enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, id<MTLTexture> _Nonnull obj, BOOL *_Nonnull stop) {
            ...
            [description writeToURL:[folder URLByAppendingPathComponent:file]
                         atomically:NO
                           encoding:NSUTF8StringEncoding
                              error:nil];
        }];
    NSString *description = [NSString
        stringWithFormat:@"vertex count: %@\ninstance count: %@\nFragment function: %@\nVertex function: %@\n",
                         @(_vertexCount), @(_instanceCount), self.fragmentFunctionName, self.vertexFunctionName];
    [description writeToURL:[folder URLByAppendingPathComponent:@"description.txt"]
                 atomically:NO
                   encoding:NSUTF8StringEncoding
                      error:nil];
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Phase 1 optimization: Pre-size for typical terminal rendering
        _rowData = [[NSMutableArray alloc] initWithCapacity:64];         // Typical screen rows
        _transientStates = [[NSMutableArray alloc] initWithCapacity:16]; // Renderer transient states
        _cellRenderers = [[NSMutableArray alloc] initWithCapacity:8];    // Cell renderers
        _draws = [[NSMutableArray alloc] initWithCapacity:16];           // Draw info records
    }
    return self;
}
...
- (void)addRenderOutputData:(NSData *)data size:(CGSize)size transientState:(iTermMetalRendererTransientState *)tState {
...
- (NSData *)newArchive {
    ...
    NSURL *root = [[[NSURL fileURLWithPath:NSTemporaryDirectory()] URLByAppendingPathComponent:uuid]
        URLByAppendingPathExtension:@"dashterm2-metal-frame"];
    NSError *error = nil;
    ...
    [self writeRenderPassDescriptor:_renderPassDescriptor to:[self newFolderNamed:@"RenderPassDescriptor" root:root]];
    if (_postmultipliedRenderPassDescriptor) {
        [self writeRenderPassDescriptor:_postmultipliedRenderPassDescriptor
                                     to:[self newFolderNamed:@"postmultipliedRenderPassDescriptor" root:root]];
    }
    if (_intermediateRenderPassDescriptor) {
        [self writeRenderPassDescriptor:_intermediateRenderPassDescriptor
                                     to:[self newFolderNamed:@"IntermediateRenderPassDescriptor" root:root]];
    }
    if (_temporaryRenderPassDescriptor) {
        [self writeRenderPassDescriptor:_temporaryRenderPassDescriptor
                                     to:[self newFolderNamed:@"TemporaryRenderPassDescriptor" root:root]];
    }
    if (_metalCaptureURL) {
        NSError *error = nil;
        [[NSFileManager defaultManager]
            moveItemAtURL:_metalCaptureURL
    ...
    [_transientStates enumerateObjectsUsingBlock:^(iTermMetalRendererTransientState *_Nonnull obj, NSUInteger idx,
                                                   BOOL *_Nonnull stop) {
        Class theClass = [obj class];
        NSURL *folder = [self newFolderNamed:[NSString stringWithFormat:@"state-%04d-%@", (int)obj.sequenceNumber,
    ...
    [_cellRenderers
        enumerateObjectsUsingBlock:^(id<iTermMetalCellRenderer> _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            Class theClass = [obj class];
            NSURL *folder = [self
                newFolderNamed:[NSString stringWithFormat:@"renderer-%04d-%@", (int)idx, NSStringFromClass(theClass)]
                          root:root];
            [obj writeDebugInfoToFolder:folder];
        }];
