...
typedef struct iTermTextPIU {
    ...
    // Pre-computed alpha vector for text color (optimization to avoid per-vertex computation)
    // Used by monochrome rendering path on macOS 10.14+
    vector_float4 alphaVector;
} iTermTextPIU;
...
typedef struct {
    // Y offset from vertex in pixels (row position)
    float yOffset;
    // Row color (premultiplied alpha)
    vector_float4 color;
} iTermHighlightRowPIU;

typedef struct {
    // Y offset from vertex in pixels (row position)
    float yOffset;
    // X offset from vertex in pixels (used for overflow adjustment)
    float xOffset;
} iTermTimestampPIU;
