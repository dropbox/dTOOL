...
// NOTE: iTermAlphaVectorForTextColor was removed from here and moved to CPU-side computation
// in iTermTextRendererTransientState.mm for performance optimization. The alpha vector is now
// pre-computed on the CPU and stored in the PIU struct to avoid per-vertex GPU computation.
...
iTermTextVertexShader(uint vertexID [[ vertex_id ]],
    ...
    // Use pre-computed alpha vector from CPU to avoid per-vertex computation
    out.alphaVector = perInstanceUniforms[iid].alphaVector;
