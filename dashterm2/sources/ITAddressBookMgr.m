...
iTermWindowType iTermThemedWindowType(iTermWindowType windowType) {
    switch (windowType) {
        ...
        case WINDOW_TYPE_NORMAL:
            ...
            // BUG-3013: Handle unknown tab style gracefully instead of assert(false)
            DLog(@"BUG-3013: Unknown tab style in windowTypeForCompactAndNormal");
            return windowType;
        ...
        case WINDOW_TYPE_MAXIMIZED:
            ...
            // BUG-3014: Handle unknown tab style gracefully instead of assert(false)
            DLog(@"BUG-3014: Unknown tab style in windowTypeForCompactAndMaximized");
            return windowType;
...
@implementation ITAddressBookMgr {
    ...
    BOOL _minimalInitComplete;
    BOOL _fullInitComplete;
}

// Static flag to track whether we did minimal init (need async completion)
static BOOL sMinimalInitUsed = NO;
static ITAddressBookMgr *sSharedInstance = nil;

+ (id)sharedInstance {
    if (!sSharedInstance) {
        sSharedInstance = [[ITAddressBookMgr alloc] init];
    }

    return sSharedInstance;
}

+ (void)initializeMinimalForStartup {
    if (sSharedInstance) {
        return; // Already initialized
    }
    sMinimalInitUsed = YES;
    sSharedInstance = [[ITAddressBookMgr alloc] initMinimal];
}

+ (void)loadRemainingProfilesAsync {
    if (!sMinimalInitUsed || !sSharedInstance) {
        return; // Not using minimal init or not initialized
    }
    if (sSharedInstance->_fullInitComplete) {
        return; // Already loaded
    }
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        [sSharedInstance completeProfileLoadingAsync];
    });
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ([newBookmarks isKindOfClass:[NSArray class]]) {
            [self setBookmarks:newBookmarks defaultGuid:[prefs objectForKey:KEY_DEFAULT_GUID]];
        } else if ([newBookmarks isKindOfClass:[NSString class]]) {
            DLog(@"Loading profiles from %@", newBookmarks);
            NSMutableArray *profiles = [[NSMutableArray alloc] initWithCapacity:64]; // Dynamic profiles
            NSMutableSet *guids = [[NSMutableSet alloc] initWithCapacity:64];        // Profile GUIDs
            if ([_dynamicProfileManager loadDynamicProfilesFromFile:(NSString *)newBookmarks
                ...
                // BUG-ITAddressBookMgr: Gracefully recover instead of exit(1)
                // which can call atexit handlers in potentially inconsistent state.
                // Fall back to default profiles and log error.
                DLog(@"Failed to load profiles from %@, falling back to defaults", newBookmarks);
                // Remove the invalid profile path preference so we don't fail again on next launch
                [prefs removeObjectForKey:KEY_NEW_BOOKMARKS];
            }
        }

        // Make sure there is at least one bookmark.
        if ([[ProfileModel sharedInstance] numberOfBookmarks] == 0) {
            NSMutableDictionary *aDict = [[NSMutableDictionary alloc] initWithCapacity:32];  // Default bookmark
        ...
        _fullInitComplete = YES;
    }

    return self;
}

// Minimal initialization - only creates default profile without disk I/O
- (instancetype)initMinimal {
    self = [super init];
    if (self) {
        // Create menu controller (required for profile model to work)
        iTermProfilesMenuController *menuController = [[iTermProfilesMenuController alloc] init];
        [[ProfileModel sharedInstance] setMenuController:menuController];

        // Create one default profile inline (no disk I/O)
        NSMutableDictionary *defaultProfile = [[NSMutableDictionary alloc] initWithCapacity:32];
        [ITAddressBookMgr setDefaultsInBookmark:defaultProfile];
        [[ProfileModel sharedInstance] addBookmark:defaultProfile];

        _minimalInitComplete = YES;
        _fullInitComplete = NO;
        DLog(@"ITAddressBookMgr: Minimal init complete");
    }
    return self;
}

// Called async after window is visible to complete profile loading
- (void)completeProfileLoadingAsync {
    DLog(@"ITAddressBookMgr: Starting async profile loading");
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];

    // Handle deprecated bookmarks migration
    if ([prefs objectForKey:KEY_DEPRECATED_BOOKMARKS] &&
        [[prefs objectForKey:KEY_DEPRECATED_BOOKMARKS] isKindOfClass:[NSDictionary class]] &&
        ![prefs objectForKey:KEY_NEW_BOOKMARKS]) {
    ...
    NSMutableArray *loadedProfiles = nil;
    NSString *defaultGuid = nil;

    if ([newBookmarks isKindOfClass:[NSArray class]]) {
        loadedProfiles = [newBookmarks mutableCopy];
        defaultGuid = [prefs objectForKey:KEY_DEFAULT_GUID];
    } else if ([newBookmarks isKindOfClass:[NSString class]]) {
        DLog(@"Loading profiles from %@", newBookmarks);
        _dynamicProfileManager = [iTermDynamicProfileManager sharedInstance];
        NSMutableArray *profiles = [[NSMutableArray alloc] initWithCapacity:64];
        NSMutableSet *guids = [[NSMutableSet alloc] initWithCapacity:64];
        if ([_dynamicProfileManager loadDynamicProfilesFromFile:(NSString *)newBookmarks
                                                      intoArray:profiles
                                                          guids:guids] &&
            [profiles count] > 0) {
            loadedProfiles = profiles;
            defaultGuid = profiles[0][KEY_GUID];
            for (Profile *profile in profiles) {
                if ([profile[KEY_DEFAULT_BOOKMARK] isEqualToString:@"Yes"]) {
                    defaultGuid = profile[KEY_GUID];
                    break;
            ...
            DLog(@"Failed to load profiles from %@, using minimal profile", newBookmarks);
            dispatch_async(dispatch_get_main_queue(), ^{
                [prefs removeObjectForKey:KEY_NEW_BOOKMARKS];
            });
        }
    ...
    // Merge loaded profiles with minimal default on main thread
    dispatch_async(dispatch_get_main_queue(), ^{
        if (loadedProfiles && [loadedProfiles count] > 0) {
            // Clear the temporary default profile
            [[ProfileModel sharedInstance] removeAllBookmarks];
            // Load real profiles
            [self setBookmarks:loadedProfiles defaultGuid:defaultGuid];
        } else {
            // Just flush the default profile we already created
            [[ProfileModel sharedInstance] flush];
        }

        // Now do the remaining initialization that was deferred
        if (!_dynamicProfileManager) {
            _dynamicProfileManager = [iTermDynamicProfileManager sharedInstance];
        }
        ...
        _fullInitComplete = YES;
        DLog(@"ITAddressBookMgr: Async profile loading complete");

        // Post notification so UI can update if needed
        [[NSNotificationCenter defaultCenter] postNotificationName:@"iTermProfilesDidFinishLoading"
                                                            object:nil];
    });
}
...
- (void)locateBonjourServices {
    if (!bonjourServices) {
        ...
        bonjourServices = [[NSMutableArray alloc] initWithCapacity:16];  // Typical bonjour services count
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)aNetServiceBrowser
    ...
    if ([self verbose]) {
        NSLog(@"netServiceBrowser:%@ didFindService:%@ moreComing:%d", aNetServiceBrowser, aNetService,
              (int)moreComing);
    }
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)aNetServiceBrowser
    ...
    if ([self verbose]) {
        NSLog(@"netServiceBrowser:%@ didRemoveService:%@ moreComing:%d", aNetServiceBrowser, aNetService,
              (int)moreComing);
    }

    // remove host entry from this group
    NSMutableArray *toRemove = [[NSMutableArray alloc] initWithCapacity:2];  // Usually removing 1-2 services
#ifdef SUPPORT_SFTP
...
+ (void)setDefaultsInBookmark:(NSMutableDictionary *)aDict {
    NSString *plistFile = [[NSBundle bundleForClass:[self class]] pathForResource:@"DefaultBookmark" ofType:@"plist"];
    ...
    [aDict setObject:kProfilePreferenceInitialDirectoryHomeValue forKey:KEY_CUSTOM_DIRECTORY];
    [aDict setObject:NSHomeDirectory() forKey:KEY_WORKING_DIRECTORY];
}

- (BOOL)usernameIsSafe:(NSString *)username {
    NSCharacterSet *unsafeSet =
        [[NSCharacterSet characterSetWithCharactersInString:@"abcdefghijklmnopqrstuvwxyz_1234567890"] invertedSet];
    const NSRange range = [username rangeOfCharacterFromSet:unsafeSet options:NSCaseInsensitiveSearch];
...
- (void)_addBonjourHostProfileWithName:(NSString *)serviceName
    ...
    [newBookmark
        setObject:[NSString stringWithFormat:@"%@ %@%@%@", serviceType, userNameArg, optionalPortArg, destination]
    ...
    [newBookmark setObject:kProfilePreferenceInitialDirectoryHomeValue forKey:KEY_CUSTOM_DIRECTORY];
...
- (void)netServiceDidResolveAddress:(NSNetService *)sender {
    ...
    const char *strAddr =
        inet_ntop(socketAddress->sa_family, [self inaddrFromSockaddr:socketAddress], buffer, sizeof(buffer));
    if (strAddr) {
        if ([self verbose]) {
...
+ (NSString *)sanitizedCustomShell:(NSString *)customShell {
    // BUG-378: Guard against nil customShell crash
    // If customShell is nil, componentsSeparatedByCharactersInSet: returns nil
    // and parts[0] would crash. Also guard against empty parts array.
    if (!customShell || customShell.length == 0) {
        return nil;
    }
    NSArray<NSString *> *parts =
        [customShell componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
...
+ (NSString *)shellLauncherCommandWithCustomShell:(NSString *)customShell {
    ...
    // BUG-2059: Escape shell metacharacters in custom shell path to prevent command injection.
    // Even though sanitizedCustomShell extracts only the first word, paths can contain
    // shell metacharacters like $, `, ', " which must be escaped.
    NSString *customShellArg =
        sanitizedCustomShell
            ? [@" SHELL=" stringByAppendingString:[sanitizedCustomShell
                                                      stringWithBackslashEscapedShellCharactersIncludingNewlines:YES]]
            : @"";
    ...
    return [NSString stringWithFormat:@"/usr/bin/login -f%@pl %@ %@ --launch_shell%@", [self hushlogin] ? @"q" : @"",
...
+ (NSString *)loginShellCommandForBookmark:(Profile *)profile forObjectType:(iTermObjectType)objectType {
    ...
    if ([profile[KEY_CUSTOM_DIRECTORY] isEqualToString:kProfilePreferenceInitialDirectoryAdvancedValue]) {
        switch (objectType) {
            ...
            case iTermPaneObject:
                ...
                break;
            default:
                DLog(@"Bogus object type %d", (int)objectType);
    ...
    if ([customDirectoryString isEqualToString:kProfilePreferenceInitialDirectoryHomeValue] &&
        ...
        // Run DashTerm2's executable with a special flag that makes it run the shell as a login shell
...
+ (NSString *)standardLoginCommand {
    ...
    userName = [userName stringByReplacingOccurrencesOfString:@"\"" withString:@"\\\""];
    return [NSString stringWithFormat:@"login -f%@p \"%@\"", [self hushlogin] ? @"q" : @"", userName];
}
...
+ (void)computeCommandForProfile:(Profile *)profile
    ...
    iTermExpressionEvaluator *evaluator = [[iTermExpressionEvaluator alloc] initWithStrictInterpolatedString:swifty
                                                                                                       scope:scope];
    [evaluator
        evaluateWithTimeout:5
         sideEffectsAllowed:YES
                 completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                     ...
                     if (!string.length) {
                         string = [ITAddressBookMgr loginShellCommandForBookmark:profile forObjectType:objectType];
                     }
...
+ (NSString *)bookmarkCommandSwiftyString:(Profile *)bookmark forObjectType:(iTermObjectType)objectType {
    // BUG-f1652: Use iTermProfilePreferences for type-safe access
    NSString *customCommand = [iTermProfilePreferences stringForKey:KEY_CUSTOM_COMMAND inProfile:bookmark];
    NSString *commandLine = [iTermProfilePreferences stringForKey:KEY_COMMAND_LINE inProfile:bookmark];
    const BOOL browser = [customCommand isEqualToString:kProfilePreferenceCommandTypeBrowserValue];
    if (browser) {
        return commandLine;
    }
    const BOOL custom = [customCommand isEqualToString:kProfilePreferenceCommandTypeCustomValue];
    const BOOL ssh = [customCommand isEqualToString:kProfilePreferenceCommandTypeSSHValue];
    if (custom || ssh) {
        NSString *command = commandLine;
        if ([[command stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] length] > 0) {
            if (ssh) {
                ...
                NSString *wrappedCommand = [NSString stringWithFormat:@"'%@' %@", iTermPathToSSH(), command];
                command = [NSString
                    stringWithFormat:@"/usr/bin/login -fpq %@ %@ -c %@",
                                     [NSUserName() stringWithBackslashEscapedShellCharactersIncludingNewlines:YES],
    ...
    return [ITAddressBookMgr loginShellCommandForBookmark:bookmark forObjectType:objectType];
}

+ (NSString *)customShellForProfile:(Profile *)profile {
    // BUG-f1652: Use iTermProfilePreferences for type-safe access
    NSString *customCommand = [iTermProfilePreferences stringForKey:KEY_CUSTOM_COMMAND inProfile:profile];
    if (![customCommand isEqualToString:kProfilePreferenceCommandTypeCustomShellValue]) {
        return nil;
    }
    NSString *customShell = [iTermProfilePreferences stringForKey:KEY_COMMAND_LINE inProfile:profile];
...
+ (NSString *)_advancedWorkingDirWithOption:(NSString *)option directory:(NSString *)pwd {
...
+ (void)performBlockWithCoalescedNotifications:(void (^)(void))block {
    if (!sDelayedNotifications) {
        sDelayedNotifications = [[NSMutableArray alloc] initWithCapacity:4];  // Typical coalesced notifications
