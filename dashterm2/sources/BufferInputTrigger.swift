...
class BufferInputTrigger: Trigger {
    ...
    override func index(for object: Any?) -> Int {
        // BUG-1697: Use guard instead of force unwrap for menuItemsForPoupupButton()
        guard let menuItems = menuItemsForPoupupButton() else { return -1 }
        return objectsSortedByValue(inDict: menuItems).firstIndex { obj in
    ...
    override func object(at index: Int) -> Any? {
        // BUG-1697: Use guard instead of force unwrap for menuItemsForPoupupButton()
        guard let menuItems = menuItemsForPoupupButton() else { return nil }
        let sorted = objectsSortedByValue(inDict: menuItems)
        // BUG-f613: Add bounds check before accessing sorted array
        guard index >= 0 && index < sorted.count else {
            DLog("BufferInputTrigger: Invalid index \(index) for count \(sorted.count)")
            return nil
        }
        return sorted[index]
    }
