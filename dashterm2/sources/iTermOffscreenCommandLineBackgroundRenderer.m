...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
    self = [super init];
    if (self) {
        _solidColorRenderer = [[iTermMetalRenderer alloc]
        ...
        _colorsPool = [[iTermMetalBufferPool alloc]
            initWithDevice:device
                bufferSize:sizeof(vector_float4) * iTermOffscreenCommandLineBackgroundRendererNumQuads];
        _verticesPool = [[iTermMetalBufferPool alloc]
            initWithDevice:device
                bufferSize:sizeof(iTermVertex) * 6 * iTermOffscreenCommandLineBackgroundRendererNumQuads];
    }
    return self;
}
...
- (id<MTLBuffer>)colorsBufferWithColors:(vector_float4 *)colorsPtr
    ...
    return [_colorsPool requestBufferFromContext:poolContext withBytes:colorsPtr checkIfChanged:YES];
}
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
    ...
    const CGRect hullQuad =
        CGRectMake(0, viewportHeight - tState.rowHeight - padding * 2 + scale - tState.configuration.extraMargins.top,
                   viewportWidth, padding * 2 + tState.rowHeight);
    const CGRect fieldQuad =
        CGRectMake(0, viewportHeight - tState.rowHeight - padding * 2 + scale - tState.configuration.extraMargins.top,
                   viewportWidth, padding * 2 + tState.rowHeight - scale);
    const CGRect lowerLineQuad = CGRectMake(0, NSMinY(fieldQuad), viewportWidth, scale);
    ...
    // BUG-f1083: Use _Static_assert for compile-time check instead of runtime assert
    _Static_assert(sizeof(iTermVertex[iTermOffscreenCommandLineBackgroundRendererNumQuads * 6]) / sizeof(iTermVertex) ==
                       iTermOffscreenCommandLineBackgroundRendererNumQuads * 6,
                   "vertices array size must match expected quad count");

    vector_float4 colors[iTermOffscreenCommandLineBackgroundRendererNumQuads] = {
        iTermPremultiply(iTermBlendColors(simd_make_float4(0, 0, 0, 0), tState.defaultBackgroundColor)),
        tState.backgroundColor, tState.outlineColor};
    // BUG-f1084: Use _Static_assert for compile-time check instead of runtime assert
    _Static_assert(sizeof(vector_float4[iTermOffscreenCommandLineBackgroundRendererNumQuads]) / sizeof(vector_float4) ==
                       iTermOffscreenCommandLineBackgroundRendererNumQuads,
                   "colors array size must match expected quad count");
    ...
    const iTermMetalBufferBinding vertexBindings[] = {
        ...
        iTermMetalBufferBindingMake(iTermVertexColorArray, colorsBuffer),
    };
...
- (nullable __kindof iTermMetalRendererTransientState *)
    createTransientStateForConfiguration:(nonnull iTermRenderConfiguration *)configuration
    ...
    iTermOffscreenCommandLineBackgroundRendererTransientState *_Nonnull tState =
        [_solidColorRenderer createTransientStateForConfiguration:configuration commandBuffer:commandBuffer];
