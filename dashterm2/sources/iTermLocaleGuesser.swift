...
class iTermLocaleGuesser: NSObject {
    struct Config {
        // nonisolated(unsafe) because this is initialized once lazily and never modified
        nonisolated(unsafe) private static var lowerCaseEncodings: [String: Bool] = {
            // BUG-f532: Return empty dictionary instead of crashing if plist is missing
            guard let plistFile = Bundle(for: iTermLocaleGuesser.self).path(forResource: "EncodingsWithLowerCase", ofType: "plist") else {
                ...
                DLog("iTermLocaleGuesser: EncodingsWithLowerCase.plist not found in bundle")
                return [:]
            }
    ...
    // nonisolated(unsafe) because SimpleCache is only accessed from iTermLocaleGuesser methods
    nonisolated(unsafe) private static var titleCache = SimpleCache<String, String>()
    ...
    private func properlyCapitalizedIANAEncoding(for cfEncoding: CFStringEncoding) -> String? {
        // BUG-f657: Replace it_assert with guard - empty config should not crash
        if config.lowerCaseEncodings.isEmpty {
            DLog("BUG-f657: lowerCaseEncodings is empty - using default UTF-8 encoding")
            // Return nil to let caller handle fallback
        }
