...
fileprivate struct Config {
    ...
    init(dictionary: [String: AnyObject]) {
        ...
        // BUG-1590: Use compactMap with guard to prevent index out of bounds crash
        filesToCopy = (dictionary[Keys.filesToCopy.rawValue] as? [[String]] ?? []).compactMap {
...
class SSHConfigurationWindowController: NSWindowController {
    ...
    @IBAction func sshIntegrationHelp(_ sender: Any) {
        ...
        if let url = URL(string: "https://gitlab.com/gnachman/iterm2/-/wikis/SSH-Integration") {
...
extension SSHConfigurationWindowController: NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView,
        ...
        // BUG-f1407: Bounds check before accessing config.filesToCopy array
        // Row index can be stale if array was modified between numberOfRows and this call
        guard row >= 0 && row < config.filesToCopy.count else {
            DLog("SSHConfigurationWindowController.tableView(_:viewFor:row:): row \(row) out of bounds (count=\(config.filesToCopy.count))")
            return nil
        }
...
extension SSHConfigurationWindowController: NSTextDelegate, NSTextFieldDelegate {
    func controlTextDidChange(_ obj: Notification) {
        if obj.object as? NSTokenField === environmentVariablesToCopy {
            ...
            let selectedRow = filesToCopyTable.selectedRow
            guard selectedRow >= 0 && selectedRow < config.filesToCopy.count else {
                return
            }
            if textField.identifier == sourceColumn.identifier {
                config.filesToCopy[selectedRow].0 = textField.stringValue
            } else if textField.identifier == destinationColumn.identifier {
                config.filesToCopy[selectedRow].1 = textField.stringValue
            }
        }
    }
}
...
fileprivate class SSHCopyDestinationView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f841: Return nil instead of crashing for unused coder initializer
        DLog("SSHConfigurationWindowController init(coder:) is not supported")
        return nil
    }
}
