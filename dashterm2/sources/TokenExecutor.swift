...
func CVectorReleaseObjectsAndDestroy(_ vector: CVector) {
    var temp = vector
    // Recycle VT100Tokens back to the pool for reuse instead of releasing them
    CVectorRecycleVT100TokensAndDestroy(&temp)
}
...
private class TokenExecutorImpl {
    ...
    init(_ terminal: VT100Terminal,
        ...
        // DashTerm2: Increased side effect frequency for instant terminal I/O (120 Hz)
        sideEffectScheduler = PeriodicScheduler(DispatchQueue.main, period: 1 / 120.0, action: { [weak self] in
    ...
    func assertSynchronousSideEffectsAreSafe() {
        // BUG-f503: Log warning instead of crashing - this is a programming error but shouldn't crash
        if sideEffects.count != 0 || executingSideEffects.value {  // swiftlint:disable:this empty_count
            DLog("[WARNING] assertSynchronousSideEffectsAreSafe: side effects pending (\(sideEffects.count)) or executing (\(executingSideEffects.value))")
        }
    }
...
extension TokenExecutorImpl: UnpauserDelegate {
    ...
    func unpause() {
        ...
        // BUG-f504: Log error and clamp instead of crashing - pause count should never go negative
        if newCount < 0 {
            DLog("[ERROR] unpause: Pause count went negative (\(newCount)), clamping to 0")
            // Reset to 0 by adding back the magnitude of the negative value
            _ = iTermAtomicInt64Add(pauseCount, -newCount)
            return
        }
...
class PeriodicScheduler: NSObject {
    ...
    @objc func markNeedsUpdate(deferred delay: TimeInterval) {
        ...
        // BUG-981: Logic was inverted - we should schedule if no deferred update is pending
        let shouldSchedule = mutex.sync {
            _needsUpdate = true
            let wasScheduled = scheduledDeferred
            scheduledDeferred = true
            return !wasScheduled  // Schedule if not already scheduled
        }
        if shouldSchedule {
