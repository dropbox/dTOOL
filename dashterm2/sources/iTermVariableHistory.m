...
+ (void)recordBuiltInVariables {
    ...
    [self recordUseOfNonterminalVariableNamed:iTermVariableKeyGlobalScopeName
                                    inContext:iTermVariablesSuggestionContextWindow
                             leadingToContext:iTermVariablesSuggestionContextApp];
    [self recordUseOfVariableNamed:iTermVariableKeyWindowFrame inContext:iTermVariablesSuggestionContextWindow];
    [self recordUseOfVariableNamed:iTermVariableKeyWindowStyle inContext:iTermVariablesSuggestionContextWindow];
    [self recordUseOfVariableNamed:iTermVariableKeyWindowNumber inContext:iTermVariablesSuggestionContextWindow];
    ...
    [self recordUseOfVariableNamed:iTermVariableKeyApplicationLocalhostName
                         inContext:iTermVariablesSuggestionContextApp];
    [self recordUseOfVariableNamed:iTermVariableKeyApplicationEffectiveTheme
                         inContext:iTermVariablesSuggestionContextApp];
}
...
+ (void)recordUseOfVariableNamed:(NSString *)namePossiblyContainingNonterminals
    ...
    NSString *name = [self pathByConsumingNonterminalsInPath:namePossiblyContainingNonterminals
                                                  ...
                                                  contextOut:&context];
    // BUG-f1106: Guard against nil name to prevent crash
    if (!name) {
        ELog(@"iTermVariableHistory recordUseOfVariableNamed: nil name for input %@",
             namePossiblyContainingNonterminals);
        return;
    }
...
+ (NSMutableDictionary<NSNumber *, NSMutableSet<iTermRecordedVariable *> *> *)mutableRecordedNames {
    ...
    dispatch_once(&onceToken, ^{
        ...
        records = [[NSMutableDictionary alloc] initWithCapacity:dict.count];
        for (id key in dict) {
...
+ (void)synchronizeRecordedNames {
    NSDictionary *plist = [[self mutableRecordedNames]
        mapValuesWithBlock:^id(NSNumber *key, NSMutableSet<iTermRecordedVariable *> *object) {
...
+ (NSMutableSet<iTermRecordedVariable *> *)mutableRecordedVariableNamesInContext:
    (iTermVariablesSuggestionContext)context {
    NSMutableSet<iTermRecordedVariable *> *set = self.mutableRecordedNames[@(context)];
    if (!set) {
        set = [[NSMutableSet alloc] initWithCapacity:32]; // Recorded variables per context
...
+ (NSSet<NSString *> *)recordedVariableNamesInContext:(iTermVariablesSuggestionContext)context
    ...
    NSMutableSet<NSString *> *results = [[NSMutableSet alloc] initWithCapacity:terminalCandidates.count];
...
+ (NSString *)pathByConsumingNonterminalsInPath:(NSString *)prefix
    ...
    for (NSString *candidate in candidates) {
        NSString *result = [self pathByConsumingNonterminalsInPath:prefix
                                                         candidate:candidate
                                                           context:context
                                                        contextOut:contextPtr];
        if (result) {
            return result;
        }
    }
...
+ (NSString *)pathByConsumingNonterminalsInPath:(NSString *)prefix
    ...
    NSMutableArray<NSString *> *accum = [[NSMutableArray alloc] initWithCapacity:candidateParts.count];
...
+ (BOOL)pathIsNonterminal:(NSString *)path inContext:(inout iTermVariablesSuggestionContext *)contextPtr {
    ...
    iTermRecordedVariable *record =
        [[vars allObjects] objectPassingTest:^BOOL(iTermRecordedVariable *record, NSUInteger index, BOOL *stop) {
...
+ (NSSet<iTermRecordedVariable *> *)recordedVariablesInContext:(iTermVariablesSuggestionContext)context {
    ...
    if ((context & (iTermVariablesSuggestionContextSession | iTermVariablesSuggestionContextTab)) &&
        ...
        NSSet<iTermRecordedVariable *> *appVariables =
            [self recordedVariablesInContext:iTermVariablesSuggestionContextApp];
        result = [NSSet
            setWithArray:[result.allObjects
                             arrayByAddingObjectsFromArray:[appVariables.allObjects
                                                               mapWithBlock:^id(iTermRecordedVariable *appVariable) {
                                                                   return [appVariable
                                                                       recordByPrependingPath:@"dashterm2."];
                                                               }]]];
    }
    return result;
}
...
+ (NSSet<NSString *> *)recordedTerminalVariableNamesInContext:(iTermVariablesSuggestionContext)context {
    return [NSSet setWithArray:[[[self recordedVariablesInContext:context] allObjects]
                                   mapWithBlock:^id(iTermRecordedVariable *record) {
                                       if (!record.isTerminal) {
                                           return nil;
                                       }
                                       return record.name;
...
+ (void)recordUseOfNonterminalVariableNamed:(NSString *)name
    ...
    iTermRecordedVariable *record = [[iTermRecordedVariable alloc] initNonterminalWithName:name
                                                                                   context:leadingToContext];
    NSMutableSet<iTermRecordedVariable *> *records = [self mutableRecordedVariableNamesInContext:context];
    if (![records containsObject:record]) {
        DLog(@"Record %@ in context %@", name, [iTermVariableHistory stringForContext:context]);
        [records addObject:record];
