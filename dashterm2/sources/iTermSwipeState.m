...
- (instancetype)initWithSwipeHandler:(id<iTermSwipeHandler>)handler {
    // BUG-f1016: Return nil instead of crashing when handler is nil
    if (!handler) {
        DLog(@"BUG-f1016: initWithSwipeHandler: called with nil handler - returning nil");
        return nil;
    }
    self = [super init];
    if (self) {
        ...
        // BUG-f1016: These conditions are already checked above, but verify with guards
        if (_parameters.width <= 0 || _parameters.count == 0 || _parameters.currentIndex >= _parameters.count) {
            DLog(@"BUG-f1016: Invalid parameters (width=%@, count=%@, currentIndex=%@) - returning nil",
                 @(_parameters.width), @(_parameters.count), @(_parameters.currentIndex));
            return nil;
        }
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<%@: %p width=%@ offset=%@ (%@) momentum=%@ state=%@ momentumStage=%@ targetIndex=%@>",
                         NSStringFromClass(self.class), self, @(_parameters.width), @(self.squashedOffset),
                         @(_rawOffset), @(_momentum), @(_state), @(self.momentumStage), _targetIndexNumber];
}
...
- (iTermSwipeStateMomentumStage)desiredMomentumStage {
    // This method decides whether to go to the next/previous tab based on the current momentum (typically how fast the
    // last drag was). Set the threshold higher to require more speed to continue a swipe after the drag ends.
...
- (void)retire {
    DLog(@"Retire %@", self);
    // BUG-f1017: Guard against double retire instead of crashing
    if (_isRetired) {
        DLog(@"BUG-f1017: retire called on already retired state - ignoring");
        return;
    }
...
- (CGFloat)offsetOfTargetIndex {
    // BUG-f1018: Guard against nil _targetIndexNumber instead of crashing
    if (!_targetIndexNumber) {
        DLog(@"BUG-f1018: offsetOfTargetIndex called with nil _targetIndexNumber - returning 0");
        return 0;
    }
...
- (BOOL)offsetIsAfterTargetIndex:(CGFloat)offset {
    // BUG-f1018: Guard against nil _targetIndexNumber instead of crashing
    if (!_targetIndexNumber) {
        DLog(@"BUG-f1018: offsetIsAfterTargetIndex: called with nil _targetIndexNumber - returning NO");
        return NO;
    }
...
- (void)update:(NSTimeInterval)elapsed {
    DLog(@"Update %@", self);
    // BUG-f1019: Guard against update on retired state instead of crashing
    if (_isRetired) {
        DLog(@"BUG-f1019: update: called on retired state - ignoring");
        return;
    }
...
- (BOOL)shouldDrag:(NSEvent *)event state:(iTermScrollWheelStateMachineState)state {
    ...
    if ((_preScrollAccumulatedDeltaY + _preScrollAccumulatedDeltaX) >= 10 &&
        ...
        [self retire];
        DLog(@"Abort: Accumulated pre-scroll deltas are dx=%0.1f dy=%0.1f", _preScrollAccumulatedDeltaX,
             _preScrollAccumulatedDeltaY);
        return NO;
    }
    if (_preScrollAccumulatedDeltaX > 10) {
        DLog(@"Start dragging: Accumulated pre-scroll deltas are dx=%0.1f dy=%0.1f", _preScrollAccumulatedDeltaX,
             _preScrollAccumulatedDeltaY);
        [self startDrag];
        _dragStarted = YES;
    } else {
        DLog(@"Not ready to drag yet: Accumulated pre-scroll deltas are dx=%0.1f dy=%0.1f", _preScrollAccumulatedDeltaX,
             _preScrollAccumulatedDeltaY);
    }
    return _dragStarted;
}
...
- (void)startDrag {
    ...
    if (!_userInfo) {
        ...
        _userInfo = [self.swipeHandler swipeHandlerBeginSessionAtOffset:_initialOffset identifier:_identifier];
    ...
    // BUG-f1020: Guard against nil userInfo instead of crashing - retire gracefully
    if (!_userInfo) {
        DLog(@"BUG-f1020: startDrag failed to create userInfo - retiring");
        [self retire];
        return;
    }
    _momentum = 0;
...
- (void)badStateTransition:(iTermScrollWheelStateMachineStateTransition)transition event:(NSEvent *)event {
    ITBetaAssert(NO, @"Unexpected state transition %@ -> %@: %@", @(transition.before), @(transition.after), event);
    [self retire];
}

- (BOOL)handleEvent:(NSEvent *)event transition:(iTermScrollWheelStateMachineStateTransition)transition {
    DLog(@"handleEvent:%@ before=%@ after=%@ dy=%0.1f", iTermShortEventPhasesString(event), @(transition.before),
         @(transition.after), event.scrollingDeltaY);
        ...
        case iTermScrollWheelStateMachineStateDrag:
            ...
            switch (transition.after) {
                ...
                case iTermScrollWheelStateMachineStateGround:
                    ...
                    // BUG-f1021: Guard against unexpected None momentum stage instead of crashing
                    if (self.momentumStage == iTermSwipeStateMomentumStageNone) {
                        DLog(@"BUG-f1021: Unexpected momentumStage=None after desiredMomentumStage - retiring");
                        [self retire];
                        return NO;
                    }
                    break;
...
// To move by distance d in time T, we will move by m on each frame. Then m will be decreased by gSwipeFriction. This
// function calculates the initial value of m. More formally: v(t) = v(t - 1) * 0.9  (for t > 0) Solve for v(0), which
...
- (NSInteger)indexForOffset:(CGFloat)offset round:(int)roundDirection {
    ...
    DLog(@"Calculate index for offset %@ given width %@ and round direction %@", @(offset), @(_parameters.width),
         @(roundDirection));
...
- (void)setMomentumStage:(iTermSwipeStateMomentumStage)stage {
    ...
    // Clamp it
    _targetIndexNumber =
        @(MAX(MIN(_targetIndexNumber.integerValue, _parameters.currentIndex + 1), _parameters.currentIndex - 1));
    // BUG-f1265: Replace assert() with guard - negative target index should clamp to 0, not crash
    if (_targetIndexNumber.integerValue < 0) {
        DLog(@"WARNING: BUG-f1265 _targetIndexNumber=%@ is negative, clamping to 0", _targetIndexNumber);
        _targetIndexNumber = @0;
    }
    // BUG-f1266: Replace assert() with guard - target index >= count should clamp, not crash
    if (_targetIndexNumber.integerValue >= _parameters.count) {
        DLog(@"WARNING: BUG-f1266 _targetIndexNumber=%@ >= count=%@, clamping",
             _targetIndexNumber, @(_parameters.count));
        _targetIndexNumber = @(MAX(0, _parameters.count - 1));
    }
    _momentum = [self momentumForDistance:[self offsetOfTargetIndex] - _rawOffset];
    DLog(@"Set target index to %@ given offset %@, width %@, stage %@. Set momentum to %@.", _targetIndexNumber,
         @(self.squashedOffset), @(_parameters.width), @(stage), @(_momentum));
}
...
- (CGFloat)squashedOffset {
    ...
    const CGFloat squashedShifted = iTermSquash(shifted, softMovementLimit * 2, wiggle);
    ...
    DLog(@"Squash %0.0f (initial=%0.0f) -> %0.0f (width=%0.0f)", _rawOffset, _initialOffset, squashed,
         _parameters.width);
    return squashed;
}
