...
class RunLengthEncoder<Element, Kind: Hashable> {
    ...
    private func flush() {
        // BUG-f509: Guard against invalid state instead of crashing
        guard !buffer.isEmpty, let currentKind = kind else {
            DLog("flush: invalid state - buffer.isEmpty=\(buffer.isEmpty), kind=\(String(describing: kind))")
            buffer = []
            kind = nil
            return
        }
        emitter(buffer, currentKind)
        buffer = []
        kind = nil
    }
}
