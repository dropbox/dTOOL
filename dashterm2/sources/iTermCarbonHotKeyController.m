    ...
    if (self) {
        ...
        [[[NSWorkspace sharedWorkspace] notificationCenter] addObserver:self
                                                               selector:@selector(sessionDidBecomeActive:)
                                                                 ...
                                                                 object:nil];
        _hotKeys = [[NSMutableArray alloc] initWithCapacity:16];  // Registered carbon hotkeys
        _suspendedHotKeys = [[NSMutableArray alloc] initWithCapacity:8];  // Temporarily suspended hotkeys
    }
    return self;
}
...
- (void)sessionDidBecomeActive:(NSNotification *)notification {
    ...
    for (iTermTuple<NSNumber *, NSNumber *> *key in hotkeysByShortcut) {
        ...
        OSStatus status = RegisterEventHotKey((UInt32)representative.shortcut.keyCode, carbonModifiers,
                                              representative.hotKeyID, GetEventDispatcherTarget(), 0, &eventHotKey);
        if (status != noErr) {
            ELog(@"Failed to register event hotkey when session resumed active: %@", @(status));
            eventHotKey = NULL; // BUG-7325: Set to NULL only on failure, not on success
        }
...
- (iTermHotKey *)registerShortcut:(iTermShortcut *)shortcut
    ...
    iTermHotKey *existingHotKey = [[self hotKeysWithKeyCode:shortcut.keyCode modifiers:shortcut.modifiers] firstObject];
    ...
    if (!existingHotKey) {
        ...
        if (!_suspended) {
            ...
            const OSStatus status = RegisterEventHotKey((UInt32)shortcut.keyCode, carbonModifiers, hotKeyID,
                                                        GetEventDispatcherTarget(), 0, &eventHotKey);
            if (status) {
...
- (EventHotKeyID)nextHotKeyID {
    ...
    EventHotKeyID keyID = {.signature = iTermCarbonHotKeyControllerSignature, .id = ++_nextHotKeyID};

    return keyID;
}

- (NSArray<iTermHotKey *> *)hotKeysWithKeyCode:(NSUInteger)keyCode modifiers:(NSEventModifierFlags)modifiers {
    NSMutableArray<iTermHotKey *> *result = [[NSMutableArray alloc] initWithCapacity:4]; // Matching hotkeys
...
- (NSArray<iTermHotKey *> *)hotKeysWithID:(EventHotKeyID)hotKeyID {
    NSMutableArray<iTermHotKey *> *result = [[NSMutableArray alloc] initWithCapacity:4]; // Hotkeys by ID
...
- (BOOL)handleEvent:(EventRef)event {
    EventHotKeyID hotKeyID;
    if (GetEventParameter(event, kEventParamDirectObject, typeEventHotKeyID, NULL, sizeof(EventHotKeyID), NULL,
                          &hotKeyID)) {
    ...
    if ([NSApp isActive] && [keyWindow.firstResponder isKindOfClass:[iTermShortcutInputView class]]) {
    ...
    NSMutableSet<iTermHotKey *> *handled = [[NSMutableSet alloc] initWithCapacity:hotkeys.count];
