...
- (void)checkIfSystemPythonModuleNeedsUpgrade {
    ...
    [[iTermSlowOperationGateway sharedInstance] runCommandInUserShell:@"pip3 show iterm2" completion:^(NSString * _Nullable value) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *message = [NSString stringWithFormat:@"The system Python's iterm2 module is out of date and won't work with this version of DashTerm2. Run `%@` to fix it.", command];
...
// BUG-20: Nag controller warning messages updated to DashTerm2 branding
- (void)warnAboutChangeToDefaultPasteBehavior {
    [iTermWarning showWarningWithTitle:@"DashTerm2 no longer warns before a multi-line paste, unless you are at the shell prompt."
                               actions:@[ @"OK" ]
                             accessory:nil
                            identifier:nil
                           silenceable:kiTermWarningTypePersistent
