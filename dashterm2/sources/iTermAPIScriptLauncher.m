...
static NSString *const iTermAPIScriptLauncherScriptDidFailUserNotificationCallbackNotification =
    @"iTermAPIScriptLauncherScriptDidFailUserNotificationCallbackNotification";
// Performance optimization: os_unfair_lock has ~10x lower overhead than @synchronized
static os_unfair_lock sQueuesLock = OS_UNFAIR_LOCK_INIT;
...
+ (int)environmentVersionAt:(NSString *)dashterm2env {
    NSString *manifest = [dashterm2env stringByAppendingPathComponent:@"dashterm2env-metadata.json"];
...
+ (void)upgradeFullEnvironmentScriptAt:(NSString *)fullPath
    ...
    NSString *message = [NSString
        stringWithFormat:@"The Python API script “%@” needs a newer version of the runtime environment for security "
                         @"reasons. You must upgrade it before this version of DashTerm2 can launch the script.",
...
+ (void)downloadIfNeededAndUpgradeFullEnvironmentScriptAt:(NSString *)fullPath
    ...
    if ([downloader installedVersionWithPythonVersion:configParser.pythonVersion] >=
        iTermMinimumPythonEnvironmentVersion) {
        [self reallyUpgradeFullEnvironmentScriptAt:fullPath configParser:configParser completion:completion];
        return;
    }
...
+ (void)reallyUpgradeFullEnvironmentScriptAt:(NSString *)fullPath
    ...
    NSURL *existingEnv = [url URLByAppendingPathComponent:@"dashterm2env"];
    NSURL *savedEnv = [url URLByAppendingPathComponent:@"saved-dashterm2env"];
    ...
    [[iTermPythonRuntimeDownloader sharedInstance]
        installPythonEnvironmentTo:url
...
+ (void)upgradeIfNeededFullEnvironmentScriptAt:(NSString *)fullPath
    ...
    NSString *dashterm2env = [fullPath stringByAppendingPathComponent:@"dashterm2env"];
    NSString *saved = [fullPath stringByAppendingPathComponent:@"saved-dashterm2env"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:saved]) {
        ...
        NSError *error = nil;
        [[NSFileManager defaultManager] removeItemAtPath:dashterm2env error:&error];
        DLog(@"Remove unfinished %@: %@", dashterm2env, error);

        [[NSFileManager defaultManager] moveItemAtPath:saved toPath:dashterm2env error:&error];
        DLog(@"Move %@ to %@: %@", saved, dashterm2env, error);
        virtualenv = [self environmentForScript:fullPath checkForMain:YES checkForSaved:NO];
    }

    const int version = [self environmentVersionAt:dashterm2env];
    DLog(@"version=%@", @(version));
    if (version < iTermMinimumPythonEnvironmentVersion) {
        [self upgradeFullEnvironmentScriptAt:fullPath configParser:configParser completion:completion];
        return;
    }
...
+ (void)launchScript:(NSString *)filename
    ...
    explicitUserAction:(BOOL)explicitUserAction {
    DLog(@"launchScript:%@ fullPath:%@ arguments:%@ withVirtualEnv:%@ setupCfgPath:%@ explicitUserAction:%@", filename,
         fullPath, arguments, virtualenv, setupCfgPath, @(explicitUserAction));
...
+ (NSString *)inferredPythonVersionFromScriptAt:(NSString *)path {
    ...
    NSString *candidate = [[line substringFromIndex:expectedPrefix.length]
...
+ (void)reallyLaunchScript:(NSString *)filename
    ...
    DLog(@"reallyLaunchScript:%@ fullPath:%@ arguments:%@ withVirtualEnv:%@ pythonVersion:%@ explicitUserAction:%@",
         filename, fullPath, arguments, virtualenv, pythonVersion, @(explicitUserAction));
    ...
    iTermScriptHistoryEntry *entry =
        [[iTermScriptHistoryEntry alloc] initWithName:name
                                             fullPath:fullPath
...
// BUG-9504: Validate script path is in allowed directory before launching
+ (BOOL)isScriptPathAllowed:(NSString *)filename {
    NSString *normalizedPath = filename.stringByStandardizingPath;
    ...
    // Script must be within the approved scripts directories
    if ([normalizedPath hasPrefix:scriptsPath] || [normalizedPath hasPrefix:scriptsPathNoSpaces]) {
        return YES;
    }

    // Also allow scripts from the app bundle
...
+ (void)tryLaunchScript:(NSString *)filename
         ...
         withVirtualEnv:(NSString *)virtualenv
          pythonVersion:(NSString *)pythonVersion {
    DLog(@"tryLaunchScript:%@ arguments:%@ historyEntry:%@ key:%@ withVirtualEnv:%@ pythonVersion:%@", filename,
         arguments, entry, key, virtualenv, pythonVersion);

    // BUG-9504: Validate script path before launching to prevent arbitrary script execution
    if (![self isScriptPathAllowed:filename]) {
        NSString *errorMessage =
            [NSString stringWithFormat:@"ERROR: Script path '%@' is outside allowed directories", filename];
        [entry addOutput:errorMessage
              completion:^{
              }];
        NSLog(@"%@", errorMessage);
        @throw [NSException exceptionWithName:@"ScriptPathNotAllowed" reason:errorMessage userInfo:nil];
    }
    ...
    NSString *standardEnv =
        [[iTermPythonRuntimeDownloader sharedInstance] pathToStandardPyenvPythonWithPythonVersion:pythonVersion];
    ...
    [entry addOutput:[NSString
                         stringWithFormat:@"%@ %@\n", task.launchPath, [task.arguments componentsJoinedByString:@" "]]
          completion:^{
          }];
    // BUG-3760: Redact sensitive authentication tokens from logs to prevent leakage
    NSMutableDictionary *redactedEnvironment = [task.environment mutableCopy];
    if (redactedEnvironment[@"DASHTERM2_COOKIE"]) {
        redactedEnvironment[@"DASHTERM2_COOKIE"] = @"<REDACTED>";
    }
    if (redactedEnvironment[@"DASHTERM2_KEY"]) {
        redactedEnvironment[@"DASHTERM2_KEY"] = @"<REDACTED>";
    }
    DLog(@"cookie=<REDACTED> standardEnv=%@ searchPath=%@, path=%@, standardPythonVersion=%@ environment=%@",
         standardEnv, searchPath, path, standardPythonVersion, redactedEnvironment);
...
+ (NSDictionary *)environmentFromEnvironment:(NSDictionary *)initialEnvironment
    ...
    environment[@"DASHTERM2_COOKIE"] = cookie;
    environment[@"DASHTERM2_KEY"] = key;
    ...
    environment[@"SSL_CERT_FILE"] = [version stringByAppendingPathComponents:@[
        @"lib", [NSString stringWithFormat:@"python%@", pythonVersion], @"site-packages", @"pip", @"_vendor",
        @"certifi", @"cacert.pem"
    ]];
    environment[@"SSL_CERT_DIR"] = [version stringByAppendingPathComponents:@[ @"openssl", @"ssl", @"certs" ]];
    return environment;
}
...
+ (NSArray *)argumentsToRunScript:(NSString *)filename
    ...
    NSString *command =
        [NSString stringWithFormat:@"%@ %@ %@", [wrapper stringWithEscapedShellCharactersExceptTabAndNewline],
...
+ (void)waitForTask:(NSTask *)task readFromPipe:(NSPipe *)pipe historyEntry:(iTermScriptHistoryEntry *)entry {
    ...
    dispatch_once(&onceToken, ^{
        queues = [NSMutableArray arrayWithCapacity:8]; // Script launcher queues
    });
    dispatch_queue_t q = dispatch_queue_create("com.dashterm.dashterm2.script-launcher", NULL);
    os_unfair_lock_lock(&sQueuesLock);
    [queues addObject:q];
    os_unfair_lock_unlock(&sQueuesLock);
    dispatch_async(q, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!task.isRunning &&
                (task.terminationReason == NSTaskTerminationReasonUncaughtSignal || task.terminationStatus != 0)) {
                ...
                if (!entry.terminatedByUser) {
                    ...
                    [[iTermNotificationController sharedInstance]
                           postNotificationWithTitle:@"Script Failed"
                                              detail:message
                            callbackNotificationName:
                                iTermAPIScriptLauncherScriptDidFailUserNotificationCallbackNotification
...
+ (NSString *)pathToVersionsFolderForPyenvScriptNamed:(NSString *)name {
    NSArray<NSString *> *components = @[ name, @"dashterm2env", @"versions" ];
...
+ (NSString *)environmentForScript:(NSString *)path checkForMain:(BOOL)checkForMain checkForSaved:(BOOL)checkForSaved {
    if (checkForMain) {
        NSString *name = path.lastPathComponent;
        // If path is foo/bar then look for foo/bar/bar/bar.py
        NSString *expectedPath = [[path stringByAppendingPathComponent:name]
    ...
    // foo/bar/dashterm2env
    iTermPythonRuntimeDownloader *downloader = [iTermPythonRuntimeDownloader sharedInstance];
    {
        NSString *pyenvPython = [downloader pyenvAt:[path stringByAppendingPathComponent:@"dashterm2env"]
    ...
    {
        NSString *pyenvPython = [downloader pyenvAt:[path stringByAppendingPathComponent:@"saved-dashterm2env"]
                                      pythonVersion:[iTermAPIScriptLauncher pythonVersionForScript:path]];
        if ([[NSFileManager defaultManager] fileExistsAtPath:pyenvPython isDirectory:nil]) {
            return pyenvPython;
        }
