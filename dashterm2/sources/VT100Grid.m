...
// NEON support for optimized fill operations
...
/**
 * Optimized fill function for screen_char_t arrays.
 *
 * Performance (Apple M3 Max, measured in iteration #142):
 *   - Small fills (10 chars): Uses unrolled loop, ~6x faster than doubling memcpy
 *   - Typical line (80 chars): Uses NEON vectorization, ~3x faster
 *   - Full screen (1920 chars): Uses NEON vectorization, ~1.6x faster
 *   - Overall: 2.12x faster than doubling memcpy baseline
 *
 * The function selects the optimal strategy based on:
 *   - ARM64: NEON vectorized for length >= 8, unrolled loop otherwise
 *   - Other: Unrolled loop for length < 16, doubling memcpy otherwise
 */
NS_INLINE void VT100GridFillRunWithChar(screen_char_t *line, int start, int length, screen_char_t value) {
    if (length <= 0) {
        return;
    }
...
#if VT100_HAS_NEON
    // NEON-optimized path for ARM64 (Apple Silicon)
    // Measured 2.12x faster overall in benchmarks

    if (length < 8) {
        // Small fills: unrolled loop (6x faster than doubling for tiny arrays)
        int i = 0;
        for (; i + 3 < length; i += 4) {
            dest[i] = value;
            dest[i + 1] = value;
    ...
    // Build a 96-byte pattern buffer (8 screen_char_t = 96 bytes)
    // This allows us to use 6 NEON registers for bulk stores
    __attribute__((aligned(16))) screen_char_t pattern[8];
    for (int i = 0; i < 8; i++) {
        pattern[i] = value;
    }

    // Load pattern into NEON registers (96 bytes = 6 x 16-byte vectors)
...
#else
    // Non-NEON path (Intel or other architectures)
    // Use unrolled loop for small, doubling memcpy for large

    if (length < 16) {
        // Small fills: unrolled loop
        int i = 0;
        for (; i + 7 < length; i += 8) {
            dest[i] = value;
            dest[i + 1] = value;
        ...
        // Larger fills: doubling memcpy (leverages optimized memcpy)
        dest[0] = value;
        int filled = 1;
        while (filled < length) {
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary delegate:(id<VT100GridDelegate>)delegate {
    self = [super init];
    if (self) {
        delegate_ = delegate;
        ...
        // BUG-f1005: Gracefully handle invalid grid size instead of crashing
        if (size_.width <= 0 || size_.height <= 0) {
            DLog(@"BUG-f1005: Invalid grid size: width=%d, height=%d", size_.width, size_.height);
            return nil;
        }
        ...
        if (dictionary[@"lines v3"]) {
            ...
            lines_ = [[NSMutableArray alloc] initWithCapacity:legacyLines.count];
            migrationIndexes = [NSMutableDictionary dictionaryWithCapacity:legacyLines.count];
            [legacyLines
                enumerateObjectsUsingBlock:^(NSData *_Nonnull legacyData, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        [[NSArray castFrom:dictionary[@"timestamps"]]
            enumerateObjectsUsingBlock:^(NSNumber *timestamp, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        [[NSArray castFrom:dictionary[@"metadata"]]
            enumerateObjectsUsingBlock:^(NSArray *entry, NSUInteger idx, BOOL *_Nonnull stop) {
                if (idx >= lineInfos_.count) {
                    DLog(@"Too many lineInfos");
                    *stop = YES;
                    return;
        ...
        [migrationIndexes enumerateKeysAndObjectsUsingBlock:^(
                              NSNumber *_Nonnull idx, iTermExternalAttributeIndex *_Nonnull ea, BOOL *_Nonnull stop) {
...
- (iTermExternalAttributeIndex *)externalAttributesOnLine:(int)line createIfNeeded:(BOOL)createIfNeeded {
...
NS_INLINE screen_char_t *VT100GridScreenCharsAtLine(VT100Grid *self, int lineNumber) {
    // BUG-f1400: Guard against out-of-bounds lineNumber access
    // In release builds, assert is stripped, leaving us vulnerable to negative indices
    // which cause NSArray to throw an exception. Also guard against lineNumber >= height.
    if (lineNumber < 0 || lineNumber >= self->size_.height) {
        DLog(@"VT100GridScreenCharsAtLine: lineNumber %d out of bounds [0, %d) - returning first line", lineNumber,
             self->size_.height);
        // Return first line as fallback instead of crashing
        lineNumber = 0;
    }
...
- (NSInteger)numberOfCellsUsedInRange:(VT100GridRange)range {
    ...
    [self enumerateCellsInRect:VT100GridRectMake(0, range.location, self.size.width, range.length)
                         block:^(VT100GridCoord coord, screen_char_t c, iTermExternalAttribute *ea, BOOL *stop) {
...
- (NSArray<VT100LineInfo *> *)metadataArray {
    ...
    NSMutableArray<VT100LineInfo *> *result = [NSMutableArray arrayWithCapacity:height];
...
- (void)markCharsDirty:(BOOL)dirty inRectFrom:(VT100GridCoord)from to:(VT100GridCoord)to {
    DLog(@"Mark rect from %@ to %@ dirty=%@ delegate=%@", VT100GridCoordDescription(from),
         VT100GridCoordDescription(to), @(dirty), delegate_);
    // BUG-f1006: Gracefully handle inverted rect coordinates instead of crashing
    if (from.x > to.x) {
        DLog(@"BUG-f1006: markCharsDirty called with from.x > to.x: from=%@ to=%@", VT100GridCoordDescription(from),
             VT100GridCoordDescription(to));
        return;
    }
    if (!dirty) {
        allDirty_ = NO;
    ...
    for (int y = from.y; y <= to.y; y++) {
        ...
        [lineInfos_[index] setDirty:dirty inRange:xrange updateTimestampTo:dirty ? timestamp : 0];
    }
    _hasChanged = YES;
}
...
- (void)markAllCharsDirty:(BOOL)dirty updateTimestamps:(BOOL)updateTimestamps {
    ...
    if (dirty) {
        ...
        [lineInfos_
            enumerateObjectsUsingBlock:^(VT100LineInfo *_Nonnull lineInfo, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)markCharsDirty:(BOOL)dirty inRun:(VT100GridRun)run {
    DLog(@"Mark chars in run (origin=%@, length=%@) dirty=%@ delegate=%@", VT100GridCoordDescription(run.origin),
         @(run.length), @(dirty), delegate_);
...
- (void)setCursorWithoutInvalidatingDWCFreeLineCount:(VT100GridCoord)coord {
    cursor_.x = MIN(size_.width, MAX(0, coord.x));
    [self setCursorY:MIN(size_.height - 1, MAX(0, coord.y)) resetDWCCount:NO];
}
...
- (int)numberOfNonEmptyLinesIncludingWhitespaceAsEmpty:(BOOL)includeWhitespace {
    ...
    for (; numberOfLinesUsed > 0; numberOfLinesUsed--) {
        ...
        for (i = 0; i < size_.width; i++) {
            if (line[i].complexChar || line[i].image || ![allowedCharacters characterIsMember:line[i].code]) {
                break;
            }
        }
        if (i < size_.width) {
...
- (BOOL)lineIsEmpty:(int)n {
    ...
    for (int i = 0; i < size_.width; i++) {
        if (line[i].complexChar || line[i].image || line[i].code) {
            return NO;
        }
    }
    return YES;
...
- (int)appendLines:(int)numLines toLineBuffer:(LineBuffer *)lineBuffer makeCursorLineSoft:(BOOL)makeCursorLineSoft {
    // BUG-f1007: Gracefully handle excessive numLines instead of crashing
    if (numLines > size_.height) {
        DLog(@"BUG-f1007: appendLines called with numLines=%d > size_.height=%d, clamping", numLines, size_.height);
        numLines = size_.height;
    }
    ...
    for (i = 0; i < numLines; ++i) {
        ...
        } else if ((cursor_.x == 0) && (i == cursor_.y - 1) && (lengthOfNextLine == 0) &&
        ...
        if (makeCursorLineSoft && !isPartial) {
            isPartial = (i + 1 == numLines && self.cursor.y == i && self.cursor.x == [self lengthOfLineNumber:i]);
        }
...
- (void)fastPathScrollLinesAtAndAboveCursorIntoLineBuffer:(LineBuffer *)lineBuffer {
    ...
    for (int i = 0; i < y; i++) {
        ...
        iTermAppendItem item = {.buffer = line,
        ...
        if (!item.partial || item.length > 0) {
            DLog(@"Scroll line %d above cursor into linebuffer: %@", i,
                 ScreenCharArrayToStringDebug(item.buffer, item.length));
    ...
    if (x > 0) {
        ...
        iTermAppendItem item = {.buffer = line,
                                ...
                                .continuation = continuation};
        if (item.length > 0) {
            DLog(@"Scroll partial line %d with cursor into linebuffer: %@", y,
                 ScreenCharArrayToStringDebug(item.buffer, item.length));
            CTVectorAppend(&items, item);
        }
    }
...
- (int)scrollWholeScreenUpIntoLineBuffer:(LineBuffer *)lineBuffer unlimitedScrollback:(BOOL)unlimitedScrollback {
    if (_knownDWCFreeLines < size_.height) {
        [self resetDWCFreeCount];
    }
    ...
    int numLinesDropped = [self appendLineToLineBuffer:lineBuffer unlimitedScrollback:unlimitedScrollback];
    ...
    if (lastLineData) { // This if statement is just to quiet the analyzer.
        // Pass size_.width so EOL marker is written at index width, not past the buffer (BUG-10246)
        [self clearLineDataBytes:lastLineData.mutableBytes count:size_.width];
...
- (int)scrollUpIntoLineBuffer:(LineBuffer *)lineBuffer
    ...
    // BUG-f1008: Gracefully handle invalid scroll region instead of crashing
    if (scrollTop < 0 || scrollTop >= size_.height || scrollBottom < 0 || scrollBottom >= size_.height ||
        scrollTop > scrollBottom) {
        DLog(@"BUG-f1008: Invalid scroll region: scrollTop=%d, scrollBottom=%d, size_.height=%d", scrollTop,
             scrollBottom, size_.height);
        if (sentToLineBuffer) {
            *sentToLineBuffer = NO;
        }
        return 0;
    ...
    if (![self haveScrollRegion]) {
        ...
        return [self scrollWholeScreenUpIntoLineBuffer:lineBuffer unlimitedScrollback:unlimitedScrollback];
    } else {
        // Scroll a region
        ...
        if (scrollTop == 0 && useScrollbackWithRegion && ![self haveColumnScrollRegion]) {
            ...
            numLinesDropped = [self appendLineToLineBuffer:lineBuffer unlimitedScrollback:unlimitedScrollback];
        } else {
            if (sentToLineBuffer) {
                *sentToLineBuffer = NO;
            }
        }
        // Note: scrollTop==scrollBottom case (single-line scroll region) proceeds with scroll.
        // While some terminals treat this as a no-op, the current behavior is intentional.
        [self scrollRect:VT100GridRectMake(scrollLeft, scrollTop, scrollRight - scrollLeft + 1,
...
- (int)resetWithLineBuffer:(LineBuffer *)lineBuffer
    ...
    [self setCharsFrom:topLeft to:bottomRight toChar:[self defaultChar] externalAttributes:nil];
...
- (void)moveCursorUp:(int)n {
    ...
    if (cursor_.y >= scrollTop) {
        [self setCursor:VT100GridCoordMake(x, y < scrollTop ? scrollTop : y)];
    } else {
...
- (void)moveCursorDown:(int)n {
    ...
    int x = MIN(cursor_.x, size_.width - 1);
    if (cursor_.y <= scrollBottom) {
        [self setCursor:VT100GridCoordMake(x, y > scrollBottom ? scrollBottom : y)];
    } else {
        [self setCursor:VT100GridCoordMake(x, y)];
    }
}
...
- (void)mutateCharactersInRange:(VT100GridCoordRange)range
                        dwcFree:(BOOL)dwcFree
                          block:(void (^)(screen_char_t *sct, iTermExternalAttribute **eaOut, VT100GridCoord coord,
                                          BOOL *stop))block {
    if (!dwcFree) {
        [self resetDWCFreeCount];
    }
...
- (void)mutateExtendedAttributesOnLine:(int)y
    ...
    iTermExternalAttributeIndex *eaIndex = [self externalAttributesOnLine:y createIfNeeded:createIfNeeded];
    block(eaIndex);
}
...
- (void)setCharsFrom:(VT100GridCoord)unsafeFrom
    ...
    for (int y = MAX(0, from.y); y <= MIN(to.y, size_.height - 1); y++) {
        ...
        [self erasePossibleDoubleWidthCharOnLine:line lineNumber:y startingAtOffset:from.x - 1 withChar:c];
        [self erasePossibleDoubleWidthCharOnLine:line lineNumber:y startingAtOffset:to.x withChar:c];
        const int minX = MAX(0, from.x);
        const int maxX = MIN(to.x, size_.width - 1);
        VT100GridFillRunWithChar(line, minX, maxX - minX + 1, c);
        if (c.code == 0 && to.x == size_.width - 1) {
            line[size_.width] = c;
            line[size_.width].code = EOL_HARD;
        }
        iTermExternalAttributeIndex *eaIndex = [self externalAttributesOnLine:y createIfNeeded:attrs != nil];
        [eaIndex setAttributes:attrs at:minX count:maxX - minX + 1];
    }
...
- (void)setCharsInRun:(VT100GridRun)run toChar:(unichar)code externalAttributes:(iTermExternalAttribute *)ea {
    ...
    if (y == max.y) {
        ...
        [self setCharsFrom:run.origin to:VT100GridCoordMake(size_.width - 1, y) toChar:c externalAttributes:ea];
        y++;
...
- (void)setBackgroundColor:(screen_char_t)bg
    ...
    // BUG-7249: Clamp coordinates to valid grid bounds to prevent crashes
    const int maxX = self.size.width - 1;
    const int maxY = self.size.height - 1;
    const int clampedFromX = MAX(0, MIN(from.x, maxX));
    const int clampedFromY = MAX(0, MIN(from.y, maxY));
    const int clampedToX = MAX(0, MIN(to.x, maxX));
    const int clampedToY = MAX(0, MIN(to.y, maxY));

    for (int y = clampedFromY; y <= clampedToY; y++) {
        screen_char_t *line = [self screenCharsAtLineNumber:y];
        for (int x = clampedFromX; x <= clampedToX; x++) {
        ...
        [self markCharsDirty:YES inRectFrom:VT100GridCoordMake(clampedFromX, y) to:VT100GridCoordMake(clampedToX, y)];
    }
}

- (iTermExternalAttribute *)applySGR:(CSIParam)csi
                                  to:(screen_char_t *)cPtr
                   externalAttribute:(iTermExternalAttribute *)attr {
...
- (void)setSGR:(CSIParam)csi inRectFrom:(VT100GridCoord)from to:(VT100GridCoord)to {
    for (int y = from.y; y <= to.y; y++) {
        ...
        [self markCharsDirty:YES inRectFrom:VT100GridCoordMake(from.x, y) to:VT100GridCoordMake(to.x, y)];
    }
}

- (void)setURL:(iTermURL *)url inRectFrom:(VT100GridCoord)from to:(VT100GridCoord)to {
    for (int y = from.y; y <= to.y; y++) {
        VT100LineInfo *info = [self lineInfoAtLineNumber:y];
        iTermExternalAttributeIndex *eaIndex = [info externalAttributesCreatingIfNeeded:url != nil];
        [eaIndex mutateAttributesFrom:from.x
                                   to:to.x
...
- (void)setBlockIDList:(NSString *)blockIDList onLine:(int)line {
    ...
    [self markCharsDirty:YES inRectFrom:VT100GridCoordMake(0, line) to:VT100GridCoordMake(self.size.width - 1, line)];
}
...
- (void)copyDirtyFromGrid:(VT100Grid *)otherGrid didScroll:(BOOL)didScroll {
    ...
    for (int i = 0; i < size_.height; i++) {
        ...
        memmove(dest, source, sizeof(screen_char_t) * (size_.width + 1));
...
- (int)appendCharsAtCursor:(const screen_char_t *)buffer
    ...
    useScrollbackWithRegion:(BOOL)useScrollbackWithRegion
     ...
     externalAttributeIndex:(id<iTermExternalAttributeIndexReading>)attributes
                   rtlFound:(BOOL)rtlFound
                    dwcFree:(BOOL)dwcFree {
    int numDropped = 0;
    // BUG-f1090: Guard against nil buffer to prevent crash
    if (!buffer) {
        ELog(@"VT100Grid appendCharsAtCursor: nil buffer passed");
        return 0;
    }
    ...
    int lastY = -1;
    // Cache EA index to avoid repeated lookups when processing multiple chars on same line
    int cachedEALineNumber = -1;
    iTermExternalAttributeIndex *cachedEAIndex = nil;
    if (!dwcFree) {
        [self resetDWCFreeCount];
    }
    ...
    for (idx = 0; idx < len;) {
        ...
        BOOL wrapDwc = NO;
#ifdef VERBOSE_STRING
        NSLog(@"There is %d space left in the line and we are appending %d chars", spaceRemainingInLine,
              charsLeftToAppend);
#endif
        int effective_width;
        ...
        if (spaceRemainingInLine <= charsLeftToAppend) {
            ...
            if (idx + potentialCharsToInsert < len && ScreenCharIsDWC_RIGHT(buffer[idx + potentialCharsToInsert])) {
        ...
        // Use cached EA index if we're still on the same line; otherwise re-fetch.
        // Must re-fetch if we need to create (attributes != nil) but cache holds nil.
        const BOOL needCreate = (attributes != nil);
        iTermExternalAttributeIndex *eaIndex;
        if (lineNumber == cachedEALineNumber && (!needCreate || cachedEAIndex != nil)) {
            eaIndex = cachedEAIndex;
        } else {
            eaIndex = VT100GridGetExternalAttributes(self, lineNumber, needCreate);
            cachedEALineNumber = lineNumber;
            cachedEAIndex = eaIndex;
        }
        ...
        if (newx == size_.width) {
            ...
            mayStompSplitDwc = (!useScrollRegionCols_ && aLine[size_.width].code == EOL_DWC &&
                                ScreenCharIsDWC_SKIP(aLine[size_.width - 1]));
        } else if (!wraparound && rightMargin < size_.width && useScrollRegionCols_ && newx >= rightMargin &&
        ...
        if (charsToInsert > 1 || memcmp(aLine + cursor_.x, buffer + idx, charsToInsert * sizeof(screen_char_t))) {
            // copy charsToInsert characters into the line and set them dirty.
            memcpy(aLine + cursor_.x, buffer + idx, charsToInsert * sizeof(screen_char_t));
            [self markCharsDirty:YES
        ...
        if (mayStompSplitDwc && !ScreenCharIsDWC_SKIP(aLine[size_.width - 1]) && aLine[size_.width].code == EOL_DWC) {
        ...
        // BUG-f1295: Replace assert with guard - skip DWC_RIGHT characters instead of crashing
        // This can occur in edge cases with malformed escape sequences
        if (idx < len && ScreenCharIsDWC_RIGHT(buffer[idx])) {
            DLog(@"BUG-f1295: Unexpected DWC_RIGHT at idx=%d, skipping", idx);
            idx++; // Skip the unexpected DWC_RIGHT
            continue;
        }
    ...
    // BUG-f1296: Replace assert with clamp - numDropped should never be negative
    // If it somehow becomes negative, clamp to 0 to prevent undefined behavior
    if (numDropped < 0) {
        DLog(@"BUG-f1296: numDropped was negative (%d), clamping to 0", numDropped);
        numDropped = 0;
    }
    return numDropped;
}
...
- (void)shiftLine:(int)lineNumber
    ...
    } else if (ScreenCharIsDWC_SKIP(src[elements]) && aLine[size_.width].code == EOL_DWC) {
...
- (void)eraseDWCRightOnLine:(int)lineNumber
                          x:(int)cursorX
...
#endif
    // BUG-1297: Guard against negative cursorX to prevent array bounds crash
    if (cursorX < 0) {
        return;
    }
    screen_char_t *aLine = [self screenCharsAtLineNumber:lineNumber];
    ...
    [self markCharDirty:YES at:VT100GridCoordMake(cursorX, lineNumber) updateTimestamp:YES];
    if (cursorX > 0) {
        [self markCharDirty:YES at:VT100GridCoordMake(cursorX - 1, lineNumber) updateTimestamp:YES];
    }
}

- (void)deleteChars:(int)numberOfCharactersToDelete startingAt:(VT100GridCoord)startCoord {
    ...
    iTermExternalAttributeIndex *eaIndex = [self externalAttributesOnLine:startCoord.y createIfNeeded:NO];
    if (startCoord.x >= leftMargin && startCoord.x < rightMargin && startCoord.y >= 0 && startCoord.y < size_.height) {
        ...
        if (n > 0 && startCoord.x + n <= rightMargin) {
            ...
            [self erasePossibleDoubleWidthCharOnLine:aLine
                                          lineNumber:startCoord.y
            ...
            if (rightMargin == size_.width - 1 && ScreenCharIsDWC_SKIP(aLine[rightMargin])) {
            ...
            if (rightMargin == size_.width - 1 && aLine[size_.width].code == EOL_DWC) {
                // When the previous if statement is true, this one should also always be true.
                aLine[size_.width].code = EOL_HARD;
            }

            memmove(aLine + startCoord.x, aLine + startCoord.x + n, numCharsToMove * sizeof(screen_char_t));
            [self markCharsDirty:YES
...
- (void)scrollRect:(VT100GridRect)rect downBy:(int)distance softBreak:(BOOL)softBreak fillChar:(screen_char_t)fillChar {
    DLog(@"scrollRect:%d,%d %dx%d downBy:%d", rect.origin.x, rect.origin.y, rect.size.width, rect.size.height,
         distance);
    if (distance == 0) {
        return;
    }
    ...
    if (rect.size.width > 0 && rect.size.height > 0) {
        ...
        int sourceIndex = (direction > 0 ? bottomIndex - distance : rect.origin.y - distance);
        ...
        for (int iteration = 0; iteration < rect.size.height; iteration++) {
            ...
            [self erasePossibleDoubleWidthCharOnLine:line
        ...
        // Move lines.
        for (int iteration = 0; (iteration < sourceHeight && sourceIndex < size_.height && destIndex < size_.height &&
            ...
            memmove(targetLine + rect.origin.x, sourceLine + rect.origin.x, length * sizeof(screen_char_t));
        ...
        [self markCharsDirty:YES inRectFrom:rect.origin to:VT100GridCoordMake(rightIndex, bottomIndex)];
        ...
        // the scroll region.
        if (lineNumberAboveScrollRegion >= 0 && lineNumberAboveScrollRegion < size_.height && rect.origin.x == 0) {
            // Affecting continuation marks on line above/below rect.
            screen_char_t *pred = [self screenCharsAtLineNumber:lineNumberAboveScrollRegion];
        ...
        if (rect.origin.x + rect.size.width == size_.width && !softBreak) {
            ...
            if (lastLineOfScrollRegion >= 0 && lastLineOfScrollRegion < size_.height) {
                screen_char_t *lastLine = [self screenCharsAtLineNumber:lastLineOfScrollRegion];
        ...
        if (direction > 0) {
            const VT100GridCoord extent =
                VT100GridCoordMake(rightIndex, MIN(bottomIndex, rect.origin.y + distance - 1));
            [self setCharsFrom:rect.origin to:extent toChar:fillChar externalAttributes:nil];
        } else {
            const VT100GridCoord origin =
                VT100GridCoordMake(rect.origin.x, MAX(rect.origin.y, bottomIndex + distance + 1));
            const VT100GridCoord extent = VT100GridCoordMake(rightIndex, bottomIndex);
            [self setCharsFrom:origin to:extent toChar:fillChar externalAttributes:nil];
        }
...
- (void)setCharactersInLine:(int)line to:(const screen_char_t *)chars length:(int)length {
    [self resetDWCFreeCount];
    // BUG-f1091: Guard against length exceeding width to prevent buffer overflow
    if (length > self.size.width) {
        ELog(@"VT100Grid setCharactersInLine: length %d exceeds width %d, clamping", length, self.size.width);
        length = self.size.width;
    }
    // BUG-f1092: Guard against invalid line number
    if (line < 0 || line >= self.size.height) {
        ELog(@"VT100Grid setCharactersInLine: invalid line %d (height=%d)", line, self.size.height);
        return;
    }
    screen_char_t *destination = [self screenCharsAtLineNumber:line];
    // BUG-f1093: Guard against nil destination
    if (!destination) {
        ELog(@"VT100Grid setCharactersInLine: nil destination for line %d", line);
        return;
    }
    memmove(destination, chars, length * sizeof(screen_char_t));
    [self markCharsDirty:YES inRectFrom:VT100GridCoordMake(0, line) to:VT100GridCoordMake(length - 1, line)];
}
...
- (VT100GridRun)gridRunFromRange:(NSRange)range relativeToRow:(int)row {
    ...
    return VT100GridRunMake(adjustedLocation % size_.width, adjustedLocation / size_.width, adjustedLength);
}
...
- (BOOL)scrollWholeScreenDownPoppingFromLineBuffer:(LineBuffer *)lineBuffer {
    ...
    const BOOL ok = [lineBuffer popAndCopyLastLineInto:line
                                     ...
                                     includesEndOfLine:&eol
                                              metadata:&metadata
                                          continuation:&continuation];
    // BUG-f1094: Guard against popAndCopyLastLineInto failure
    if (!ok) {
        ELog(@"VT100Grid scrollWholeScreenDownPoppingFromLineBuffer: popAndCopyLastLineInto failed");
        return NO;
    }
...
- (BOOL)restoreScreenFromLineBuffer:(LineBuffer *)lineBuffer
    ...
    screen_char_t *defaultLine = [[self lineOfWidth:size_.width filledWithChar:defaultChar] mutableBytes];
    ...
    while (destLineNumber >= 0) {
        ...
        if (!foundCursor) {
            ...
            if (foundCursor) {
                VT100GridCoord newCursorCoord =
                    VT100GridCoordMake(tempCursor % size_.width, destLineNumber + tempCursor / size_.width);
        ...
        iTermImmutableMetadata metadata;
        screen_char_t continuation;
        // BUG-f1095: Guard against popAndCopyLastLineInto failure instead of crash
        BOOL popOk = [lineBuffer popAndCopyLastLineInto:dest
                                                  width:size_.width
                                      includesEndOfLine:&cont
                                               metadata:&metadata
                                           continuation:&continuation];
        if (!popOk) {
            ELog(@"VT100Grid restoreScreenFromLineBuffer: popAndCopyLastLineInto failed at line %d", destLineNumber);
            break;
        }
        ...
        // corresponding line.
        BOOL forceHard = (isFirst && cont == EOL_SOFT && ScreenCharIsNull(dest[size_.width - 1]));
        if (forceHard) {
            cont = EOL_HARD;
...
- (NSMutableData *)resultLineData {
    // BUG-f1096: Guard against invalid width values
    if (size_.width < 0 || size_.width >= INT_MAX) {
        ELog(@"VT100Grid resultLineData: invalid width %d", size_.width);
        // Return empty data to avoid crash
        if (!resultLine_) {
            resultLine_ = [[NSMutableData alloc] initWithLength:sizeof(screen_char_t)];
        }
        return resultLine_;
    }
...
- (NSArray *)rectsForRun:(VT100GridRun)run {
    // Pre-allocate for typical run - most runs span 1-3 lines
    const int estimatedRects = (run.length + size_.width - 1) / size_.width;
    NSMutableArray *rects = [NSMutableArray arrayWithCapacity:MAX(estimatedRects, 1)];
    ...
    // BUG-f1097: Guard against negative length (shouldn't happen but be safe)
    if (length < 0) {
        ELog(@"VT100Grid rectsForRun: negative length %d after loop", length);
    }
    return rects;
}
...
- (VT100GridRect)scrollRegionRect {
    return VT100GridRectMake(self.leftMargin, self.topMargin, self.rightMargin - self.leftMargin + 1,
...
- (NSString *)compactLineDump {
    NSMutableString *dump = [[NSMutableString alloc] initWithCapacity:size_.height * (size_.width + 1)];
    for (int y = 0; y < size_.height; y++) {
        ...
        for (int x = 0; x < size_.width; x++) {
            ...
            if (line[x].code > 127)
                c = '?';
            if (ScreenCharIsDWC_RIGHT(line[x]))
                c = '-';
            if (ScreenCharIsDWC_SKIP(line[x])) {
                // BUG-f1297: Replace assert with check - DWC_SKIP should only be at end of line
                // If found elsewhere, mark with '?' instead of crashing
                if (x != size_.width - 1) {
                    DLog(@"BUG-f1297: DWC_SKIP found at unexpected position x=%d (width=%d)", x, size_.width);
                    c = '?'; // Indicate unexpected state
                } else {
                    c = '>';
                }
            }
...
- (NSString *)compactLineDumpWithTimestamps {
    NSMutableString *dump = [[NSMutableString alloc] initWithCapacity:size_.height * (size_.width + 40)];
    ...
    for (int y = 0; y < size_.height; y++) {
        ...
        NSDate *date =
            [NSDate dateWithTimeIntervalSinceReferenceDate:[[self lineInfoAtLineNumber:y] metadata].timestamp];
...
- (NSString *)compactLineDumpWithContinuationMarks {
    NSMutableString *dump = [[NSMutableString alloc] initWithCapacity:size_.height * (size_.width + 2)];
    for (int y = 0; y < size_.height; y++) {
        const screen_char_t *line = [self immutableScreenCharsAtLineNumber:y];
        for (int x = 0; x < size_.width; x++) {
            char c = line[x].code;
...
- (NSString *)kittyPlaceholderDump {
    NSMutableString *dump = [[NSMutableString alloc] initWithCapacity:size_.height * (size_.width * 8 + 1)];
    for (int y = 0; y < size_.height; y++) {
        const screen_char_t *line = [self immutableScreenCharsAtLineNumber:y];
                ...
                if ([str parseKittyUnicodePlaceholder:&coord imageMSB:&msb]) {
                    ...
                    if (prev.x != -1) {
                        if (coord.x != prev.x + 1 || coord.y != prev.y) {
                            // Discontinuity in Kitty placeholder coordinates - expected to be sequential
                            DLog(@"Kitty placeholder coordinate discontinuity: prev=(%d,%d) curr=(%d,%d)", prev.x,
                                 prev.y, coord.x, coord.y);
                        }
                    }
                    prev = coord;
                } else {
...
- (void)insertChar:(screen_char_t)c
    externalAttributes:(iTermExternalAttribute *)attrs
                    at:(VT100GridCoord)pos
                 times:(int)n {
    ...
    memmove(line + pos.x + n, line + pos.x, charsToMove * sizeof(screen_char_t));
    iTermExternalAttributeIndex *eaIndex = [self externalAttributesOnLine:pos.y createIfNeeded:attrs != nil];
    [eaIndex copyFrom:eaIndex source:pos.x destination:pos.x + n count:charsToMove];

    // Try to clean up DWC_SKIP+EOL_DWC pair, if needed.
    if (self.rightMargin == size_.width - 1 && line[size_.width].code == EOL_DWC) {
    ...
    if (size_.width > 0 && self.rightMargin == size_.width - 1 && line[size_.width].code == EOL_SOFT &&
...
- (NSArray *)orderedLines {
    // Pre-allocate for grid height - exact size known
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:size_.height];
    for (int i = 0; i < size_.height; i++) {
...
- (NSDictionary *)dictionaryValue {
    return @{
        kGridCursorKey : [NSDictionary dictionaryWithGridCoord:cursor_],
...
- (void)restorePreferredCursorPositionIfPossible {
    if (_preferredCursorPosition.x >= 0 && _preferredCursorPosition.y >= 0 &&
        _preferredCursorPosition.x <= size_.width && _preferredCursorPosition.y < size_.height) {
...
- (VT100GridSize)sizeRespectingRegionConditionally {
    if (self.cursor.x >= self.leftMargin && self.cursor.x <= self.rightMargin && self.cursor.y >= self.topMargin &&
        self.cursor.y <= self.bottomMargin) {
        return VT100GridSizeMake(self.rightMargin - self.leftMargin + 1, self.bottomMargin - self.topMargin + 1);
    }
    return self.size;
}

- (void)setContinuationMarkOnLine:(int)line to:(unichar)code {
    // BUG-f1098: Validate line number before accessing
    if (line < 0 || line >= size_.height) {
        ELog(@"VT100Grid setContinuationMarkOnLine: invalid line %d (height=%d)", line, size_.height);
        return;
    }
    screen_char_t *chars = [self screenCharsAtLineNumber:line];
    // BUG-f1099: Guard against nil chars
    if (!chars) {
        ELog(@"VT100Grid setContinuationMarkOnLine: nil chars for line %d", line);
        return;
    }
...
- (void)encode:(id<iTermEncoderAdapter>)encoder {
    ...
    NSArray<NSData *> *legacyLines = [lines mapEnumeratedWithBlock:^id(NSUInteger i, NSData *modernData, BOOL *stop) {
        return [modernData legacyScreenCharArrayWithExternalAttributes:[self externalAttributesOnLine:i
                                                                                       createIfNeeded:NO]];
    }];
...
- (void)mutateCellsInRect:(VT100GridRect)rect
                    block:
                        (void (^NS_NOESCAPE)(VT100GridCoord, screen_char_t *, iTermExternalAttribute **, BOOL *))block {
    [self resetDWCFreeCount];
...
- (NSMutableArray *)linesWithSize:(VT100GridSize)size {
    NSMutableArray *lines = [[NSMutableArray alloc] initWithCapacity:size.height];
...
- (NSMutableArray *)lineInfosWithSize:(VT100GridSize)size {
    NSMutableArray *dirty = [NSMutableArray arrayWithCapacity:size.height];
    for (int i = 0; i < size.height; i++) {
        // BUG-7252: Use size.width (the parameter) instead of size_.width (the ivar)
        [dirty addObject:[[VT100LineInfo alloc] initWithWidth:size.width]];
    }
    return dirty;
}
...
- (NSMutableData *)lineOfWidth:(int)width filledWithChar:(screen_char_t)c {
    ...
    VT100GridFillRunWithChar(line, 0, width + 1, c);
    return data;
}
...
static const screen_char_t *VT100GridDefaultLine(VT100Grid *self, int width) {
    ...
    if (self->cachedDefaultLine_.length == length && length > 0) {
    ...
    // Pass width (not width+1) so EOL marker is written at index width, not past the buffer
    [self clearLineDataBytes:line.mutableBytes count:width];
...
// RC-032: Note that lineBuffer being nil is INTENTIONAL - it means "no scrollback buffer in use".
// In this case, the top line is simply discarded (not appended to any buffer), and 0 is returned
// because no lines were dropped from a non-existent buffer. This is NOT an error condition.
- (int)appendLineToLineBuffer:(LineBuffer *)lineBuffer unlimitedScrollback:(BOOL)unlimitedScrollback {
    if (!lineBuffer) {
        // No scrollback buffer - line will be discarded. Return 0 lines dropped.
        return 0;
    }
...
- (BOOL)haveColumnScrollRegion {
    return (useScrollRegionCols_ && (self.scrollLeft != 0 || self.scrollRight != size_.width - 1));
}
...
- (void)setSize:(VT100GridSize)newSize withSideEffects:(BOOL)withSideEffects {
    if (newSize.width != size_.width || newSize.height != size_.height) {
        ...
        scrollRegionRows_.length = MIN(scrollRegionRows_.length, size_.height - scrollRegionRows_.location);
        // Ensure scroll region has at least length 1 to avoid zero-length region issues
        if (scrollRegionRows_.length == 0 && size_.height > 0) {
            scrollRegionRows_.length = 1;
        }

        scrollRegionCols_.location = MIN(scrollRegionCols_.location, size_.width - 1);
        scrollRegionCols_.length = MIN(scrollRegionCols_.length, size_.width - scrollRegionCols_.location);
        // Ensure scroll region has at least length 1 to avoid zero-length region issues
        if (scrollRegionCols_.length == 0 && size_.width > 0) {
            scrollRegionCols_.length = 1;
        }
...
- (void)erasePossibleSplitDwcAtLineNumber:(int)lineNumber {
    ...
    if (line[size_.width].code == EOL_DWC) {
        ...
        if (ScreenCharIsDWC_SKIP(line[size_.width - 1])) { // This really should always be the case.
            line[size_.width - 1].code = 0;
            [self eraseExternalAttributesAt:VT100GridCoordMake(size_.width - 1, lineNumber) count:1];
        } else {
            DLog(@"Warning! EOL_DWC without DWC_SKIP at line %d", lineNumber);
        }
    }
}

- (void)eraseExternalAttributesAt:(VT100GridCoord)coord count:(int)count {
    iTermExternalAttributeIndex *eaIndex = [self externalAttributesOnLine:coord.y createIfNeeded:NO];
    [eaIndex eraseInRange:VT100GridRangeMake(coord.x, count)];
}

- (void)copyExternalAttributesFrom:(VT100GridCoord)sourceCoord to:(VT100GridCoord)destinationCoord length:(int)length {
    iTermExternalAttributeIndex *source = [self externalAttributesOnLine:sourceCoord.y createIfNeeded:NO];
    iTermExternalAttributeIndex *dest = [self externalAttributesOnLine:destinationCoord.y createIfNeeded:source != nil];
    if (!source && !dest) {
        return;
    }
    if (!dest) {
    ...
    [dest copyFrom:source source:sourceCoord.x destination:destinationCoord.x count:length];
}
...
- (void)erasePossibleDWCSkipOnLine:(int)lineNumber {
    ...
    if (ScreenCharIsDWC_SKIP(aLine[size_.width - 1])) {
        ...
        [self markCharDirty:YES at:VT100GridCoordMake(size_.width - 1, lineNumber) updateTimestamp:YES];
        [self setContinuationMarkOnLine:lineNumber to:EOL_HARD];
    }
}

- (BOOL)erasePossibleDoubleWidthCharOnLine:(screen_char_t *)line
    ...
    if (offset >= 0 && offset < size_.width - 1 && ScreenCharIsDWC_RIGHT(line[offset + 1])) {
        line[offset] = c;
        line[offset + 1] = c;
        [self eraseExternalAttributesAt:VT100GridCoordMake(offset, lineNumber) count:2];
        [self markCharDirty:YES at:VT100GridCoordMake(offset, lineNumber) updateTimestamp:YES];
        [self markCharDirty:YES at:VT100GridCoordMake(offset + 1, lineNumber) updateTimestamp:YES];
...
- (BOOL)erasePossibleDoubleWidthCharInLineNumber:(int)lineNumber
                                startingAtOffset:(int)offset
                                        withChar:(screen_char_t)c {
    screen_char_t *aLine = [self screenCharsAtLineNumber:lineNumber];
    return [self erasePossibleDoubleWidthCharOnLine:aLine lineNumber:lineNumber startingAtOffset:offset withChar:c];
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p size=%d x %d, cursor @ (%d,%d)>", [self class], self, size_.width,
                                      size_.height, cursor_.x, cursor_.y];
}
...
- (id)copyWithZone:(NSZone *)zone {
    VT100Grid *theCopy = [[VT100Grid alloc] initWithSize:size_ delegate:delegate_];
    theCopy->lines_ = [[NSMutableArray alloc] initWithCapacity:lines_.count];
    for (NSObject *line in lines_) {
        [theCopy->lines_ addObject:[line mutableCopy]];
    }
    theCopy->lineInfos_ = [[NSMutableArray alloc] initWithCapacity:lineInfos_.count];
