...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<%@: %p path=%@ variables=%@>", NSStringFromClass([self class]), self, _path, _variables];
}
...
- (BOOL)isEqual:(id)object {
    ...
    return (_variables == other->_variables && [_path isEqual:other->_path]);
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _frames = [NSMutableArray arrayWithCapacity:4]; // Typically session, tab, global frames
...
- (void)enumerateVariables:(void (^)(NSString *_Nonnull, iTermVariables *_Nonnull))block {
    [_frames enumerateObjectsUsingBlock:^(iTermTuple<NSString *, iTermVariables *> *_Nonnull obj, NSUInteger idx,
                                          BOOL *_Nonnull stop) {
...
- (NSDictionary<NSString *, NSString *> *)dictionaryWithStringValues {
    NSMutableDictionary<NSString *, NSString *> *result =
        [NSMutableDictionary dictionaryWithCapacity:_frames.count * 8]; // ~8 vars per frame
...
- (id)valueForPath:(NSString *)firstName, ... {
    ...
    NSMutableArray<NSString *> *parts = [NSMutableArray arrayWithCapacity:4]; // Typical path depth
...
- (nullable iTermVariables *)ownerForKey:(NSString *)key
                              forWriting:(BOOL)forWriting
                                stripped:(out NSString **)stripped {
    ...
    if (parts.count == 1) {
        *stripped = key;
        ...
        return
            [_frames objectPassingTest:^BOOL(iTermTuple<NSString *, iTermVariables *> *element, NSUInteger index,
                                             BOOL *stop) {
                return element.firstObject == nil;
            }].secondObject;
    }
    __block NSString *strippedOut = nil;
    iTermVariables *owner =
        [_frames objectPassingTest:^BOOL(iTermTuple<NSString *, iTermVariables *> *element, NSUInteger index,
                                         BOOL *stop) {
...
- (nullable iTermVariables *)ownerOfTerminalForPath:(NSString *)path
    ...
    // BUG-f1100: Guard against empty parts array (shouldn't happen but be safe)
    if (parts.count == 0) {
        ELog(@"iTermVariableScope ownerOfTerminalNamed: empty parts for stripped path %@", stripped);
        return nil;
    }
    while (parts.count > 1) {
        ...
        if (![value isKindOfClass:[iTermVariables class]]) {
            // You can get here if when checking for cycles in the presence of a bogus path that is a child of a
            // terminal (e.g., currentSession.path.s)
            return nil;
        }
        // BUG-f1101: Redundant assert after type check removed - the check above handles this case
        owner = value;
...
- (void)setValuesFromDictionary:(NSDictionary<NSString *, id> *)dict {
    ...
    NSMutableDictionary<NSValue *, NSMutableDictionary<NSString *, id> *> *valuesByOwner =
    ...
    for (NSString *key in dict) {
        ...
        if (!inner) {
            inner = [NSMutableDictionary dictionaryWithCapacity:4]; // Typical batch size
            valuesByOwner[value] = inner;
        }
        inner[stripped] = object;
    }
    [valuesByOwner
        enumerateKeysAndObjectsUsingBlock:^(NSValue *_Nonnull ownerValue,
                                            NSDictionary<NSString *, id> *_Nonnull setDict, BOOL *_Nonnull stop) {
...
- (BOOL)setValue:(nullable id)value forVariableNamed:(NSString *)name weak:(BOOL)weak {
    // BUG-f1102: Guard against passing iTermVariableScope instead of iTermVariables
    if ([value isKindOfClass:[iTermVariableScope class]]) {
        ELog(@"iTermVariableScope setValue: passed iTermVariableScope instead of iTermVariables for %@", name);
        return NO;
    }
...
- (BOOL)userWritableContainerExistsForPath:(NSString *)path {
    NSString *terminal;
    iTermVariables *vars = [self ownerOfTerminalForPath:path forWriting:YES terminal:&terminal];
...
- (iTermVariableRecordingScope *)recordingCopy {
    ...
    [_frames enumerateObjectsUsingBlock:^(iTermTuple<NSString *, iTermVariables *> *_Nonnull tuple, NSUInteger idx,
                                          BOOL *_Nonnull stop) {
...
- (iTermVariableDesignator *)designatorForPath:(NSString *)path {
    NSString *terminal;
    iTermVariables *owner = [self ownerOfTerminalForPath:path forWriting:NO terminal:&terminal];
...
- (iTermVariableScope *)variableScopeByAddingBackreferences:(NSArray<NSString *> *)backreferences
    ...
    iTermVariables *matchesFrame = [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextNone
                                                                     owner:owner];
...
- (id)valueForVariableName:(NSString *)name {
    if (!_names) {
        _names = [NSMutableSet setWithCapacity:8]; // Typical recording tracks 8-16 variables
    }
