...
+ (void)toggleSizeChangesAffectProfile {
    [iTermPreferences setBool:![iTermPreferences boolForKey:kPreferenceKeySizeChangesAffectProfile]
    ...
    if (reset) {
        ...
        [frontTerminal
            sessionInitiatedResize:session
                             width:MIN(iTermMaxInitialSessionSize, [iTermProfilePreferences intForKey:KEY_COLUMNS
                                                                                            inProfile:abEntry])
                            height:MIN(iTermMaxInitialSessionSize, [iTermProfilePreferences intForKey:KEY_ROWS
                                                                                            inProfile:abEntry])];
    }
}
...
+ (void)adjustProfileFontSizeBy:(int)delta {
    ...
    // Each session may contribute 1-2 GUIDs (original + current)
    NSArray *sessions = [self sessionsToAdjustFontSize];
    NSMutableSet<NSString *> *guids = [NSMutableSet setWithCapacity:sessions.count * 2];
