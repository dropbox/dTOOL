...
- (NSArray<iTermTuple<NSString *, NSString *> *> *)helpfulSynonyms {
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:4];  // hex, scientific, timestamp, utf8
...
- (iTermTuple<NSString *, NSString *> *)utf8Help {
    ...
    NSMutableArray *byteStrings = [NSMutableArray arrayWithCapacity:utf8Len];
    ...
    NSMutableArray *ucs4Strings = [NSMutableArray arrayWithCapacity:numCharacters];
...
- (NSDate *)dateValueFromUnix {
    ...
    // BUG-10735: Changed regexes to accept timestamps starting with 1 or 2.
    // Unix epoch 2000000000 is 2033-05-18, so timestamps will start with "2" starting then.
    // These patterns now work correctly from 1970 through 2286 (when 10-digit timestamps overflow).
    Format formats[] = {
        {
            // 10-digit second timestamps: 1000000000 (2001-09-09) to 9999999999 (2286-11-20)
            .regex = @"^[1-9][0-9]{9}$",
            .divisor = 1
        },
        {
        ...
        {
            // 10-digit second timestamps with decimal fractional seconds
            .regex = @"^[1-9][0-9]{9}\\.[0-9]+$",
            .divisor = 1
        }
    };
