...
- (instancetype)initWithMaxEntries:(int)maxEntries {
    ...
    if (self) {
        ...
        entries_ = [[NSMutableArray alloc] initWithCapacity:maxEntries_];
...
- (NSDictionary*)_entriesToDict {
    NSMutableArray *a = [NSMutableArray arrayWithCapacity:entries_.count];
...
- (void)_writeHistoryToDisk {
    if ([iTermPreferences boolForKey:kPreferenceKeySavePasteAndCommandHistory]) {
        NSError *error = nil;
        // Use secure coding for serialization - the data only contains plist-safe types
        NSData *data =
        ...
        // BUG-1187: Write atomically to a temp file first, then rename to avoid race condition
        // where file is world-readable before chmod. The temp file is created with restrictive
        // permissions by setting umask, and atomicWrite ensures rename-based replacement.
        NSString *tempPath = [path_ stringByAppendingString:@".tmp"];

        // Set restrictive umask before creating temp file
        ...
        if (!writeSuccess) {
            DLog(@"Failed to write pasteboard history: %@", writeError);
            [[NSFileManager defaultManager] removeItemAtPath:tempPath error:nil];
            return;
        }

        // Ensure restrictive permissions on temp file before rename
        NSError *permError = nil;
        if (![[NSFileManager defaultManager] setAttributes:@{ NSFilePosixPermissions: @0600 }
                                              ofItemAtPath:tempPath
                                                     error:&permError]) {
            DLog(@"Failed to set temp file permissions: %@. Removing file.", permError);
            [[NSFileManager defaultManager] removeItemAtPath:tempPath error:nil];
            return;
        }
        ...
        if (![[NSFileManager defaultManager] replaceItemAtURL:[NSURL fileURLWithPath:path_]
                                                withItemAtURL:[NSURL fileURLWithPath:tempPath]
                                               backupItemName:nil
                                                      options:0
                                             resultingItemURL:nil
                                                        error:&renameError]) {
            // replaceItemAtURL may fail if destination doesn't exist, try direct move
            if (![[NSFileManager defaultManager] moveItemAtPath:tempPath toPath:path_ error:&renameError]) {
                DLog(@"Failed to rename temp pasteboard history file: %@. Removing temp file.", renameError);
                [[NSFileManager defaultManager] removeItemAtPath:tempPath error:nil];
            }
        }
    }
...
- (void)_loadHistoryFromDisk {
    ...
    // Use secure coding with explicit allowed classes for deserialization
    ...
    NSSet<Class> *allowedClasses = [NSSet setWithObjects:
                                    ...
                                    [NSNumber class],
                                    nil];
    NSDictionary *dict = [unarchiver decodeObjectOfClasses:allowedClasses
                                                    forKey:NSKeyedArchiveRootObjectKey];
    if (!dict) {
        DLog(@"Failed to decode pasteboard history - may be legacy format");
        return;
    }
...
- (void)save:(NSString*)value
{
    ...
    // Append this value.
    // BUG-1188: Use single timestamp to ensure consistency between score and timestamp
    NSDate *now = [NSDate date];
    PasteboardEntry *entry = [PasteboardEntry entryWithString:value score:[now timeIntervalSince1970]];
    entry.timestamp = now;
    [entries_ addObject:entry];
...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
    // BUG-394: Bounds check before accessing model to prevent crash
    int convertedIndex = [self convertIndex:rowIndex];
    PopupModel *model = [self model];
    if (convertedIndex < 0 || convertedIndex >= (NSInteger)model.count) {
        return nil;
    }
    PasteboardEntry *entry = [model objectAtIndex:convertedIndex];
    if ([[aTableColumn identifier] isEqualToString:@"date"]) {
        // Date
        NSString *formattedDate = [NSDateFormatter dateDifferenceStringFromDate:entry.timestamp];
        // Use same convertedIndex and model - already validated above
...
- (NSString *)insertableString {
    ...
    // BUG-394: Bounds check before accessing model to prevent crash
    int convertedIndex = [self convertIndex:[table_ selectedRow]];
    PopupModel *model = [self model];
    if (convertedIndex < 0 || convertedIndex >= (NSInteger)model.count) {
        return nil;
    }
