...
+ (NSString *)modCharsForMask:(int)modifiers {
    // Max 4 modifier key chars
    NSMutableString *modStr = [NSMutableString stringWithCapacity:4];
...
+ (NSString *)keyForGesture:(NSString *)gestureDescription modifiers:(int)modifiers {
    return [NSString stringWithFormat:@"%@,%@,%@,", kGestureSchema, gestureDescription,
...
// BUG-f1036: Guard against nil components to prevent crash when parsing fails
+ (BOOL)keyIsThreeFingerTap:(NSString *)key {
...
+ (NSDictionary *)gestureNamesDict {
    NSDictionary *names = @{
        kThreeFingerClickGesture : @"Three-finger Tap",
...
+ (int)tagForGestureIdentifier:(NSString *)ident {
    NSArray *keys = @[
        kThreeFingerClickGesture, kThreeFingerSwipeRight, kThreeFingerSwipeLeft, kThreeFingerSwipeUp,
        kThreeFingerSwipeDown, kForceTouchSingleClick
    ];
...
+ (NSDictionary *)localizedActionMap {
    NSDictionary *names = [NSDictionary
        dictionaryWithObjectsAndKeys:@"Ignore", kIgnoreAction, @"Invoke Script Function…", kInvokeScriptFunction,
                                     @"Paste from Clipboard…", kPasteFromClipboardPointerAction,
                                     @"Paste from Selection…", kPasteFromSelectionPointerAction, @"Extend Selection",
                                     kExtendSelectionPointerAction, @"Open URL/Semantic History",
                                     kOpenTargetPointerAction, @"Open URL in background",
                                     kOpenTargetInBackgroundPointerAction, @"Smart Selection",
                                     kSmartSelectionPointerAction, @"Smart Selection Ignoring Newlines",
                                     kSmartSelectionIgnoringNewlinesPointerAction, @"Open Context Menu",
                                     kContextMenuPointerAction, @"Next Tab", kNextTabPointerAction, @"Previous Tab",
                                     kPrevTabPointerAction, @"Next Window", kNextWindowPointerAction,
                                     @"Previous Window", kPrevWindowPointerAction, @"Move Pane", kMovePanePointerAction,
                                     @"Send Escape Sequence…", kSendEscapeSequencePointerAction, @"Send Hex Code…",
                                     kSendHexCodePointerAction, @"Send Text…", kSendTextPointerAction,
                                     @"Select Pane Left", kSelectPaneLeftPointerAction, @"Select Pane Right",
                                     kSelectPaneRightPointerAction, @"Select Pane Above", kSelectPaneAbovePointerAction,
                                     @"Select Pane Below", kSelectPaneBelowPointerAction, @"New Window With Profile…",
                                     kNewWindowWithProfilePointerAction, @"New Tab With Profile…",
                                     kNewWindowWithProfilePointerAction, @"New Tab With Profile…",
                                     kNewTabWithProfilePointerAction, @"New Vertical Split With Profile…",
                                     kNewVerticalSplitWithProfilePointerAction, @"New Horizontal Split With Profile…",
                                     kNewHorizontalSplitWithProfilePointerAction, @"QuickLook", kQuickLookAction,
                                     @"Select Menu Item", kSelectMenuItemPointerAction, @"Select Next Pane",
                                     kSelectNextPanePointerAction, @"Select Previous Pane",
                                     kSelectPreviousPanePointerAction, nil];
    return names;
}

+ (ArgumentType)argumentTypeForAction:(NSString *)action {
    NSDictionary *args =
        [NSDictionary dictionaryWithObjectsAndKeys:@(kEscPlusArg), kSendEscapeSequencePointerAction, @(kHexCodeArg),
                                                   kSendHexCodePointerAction, @(kTextArg), kSendTextPointerAction,
                                                   @(kScriptFunctionArg), kInvokeScriptFunction, @(kProfileArg),
                                                   kNewWindowWithProfilePointerAction, @(kProfileArg),
                                                   kNewTabWithProfilePointerAction, @(kProfileArg),
                                                   kNewVerticalSplitWithProfilePointerAction, @(kProfileArg),
                                                   kNewHorizontalSplitWithProfilePointerAction, @(kAdvancedPasteArg),
                                                   kPasteFromClipboardPointerAction, @(kAdvancedPasteArg),
                                                   kPasteFromSelectionPointerAction, @(kMenuItemArg),
                                                   kSelectMenuItemPointerAction, nil];
        ...
        switch ([PointerPrefsController argumentTypeForAction:action]) {
            ...
            case kProfileArg: {
                NSString *bookmarkName =
                    [[[ProfileModel sharedInstance] bookmarkWithGuid:argument] objectForKey:KEY_NAME];
                if (!bookmarkName) {
                    bookmarkName = @"?";
                }
                return [name stringByReplacingOccurrencesOfString:@"…"
            ...
            case kAdvancedPasteArg: {
                if (argument.length) {
                    return [NSString
                        stringWithFormat:@"%@: %@", [name stringByReplacingOccurrencesOfString:@"…" withString:@""],
...
+ (NSDictionary *)defaultActions {
    ...
    if (!defaultDict) {
        NSMutableDictionary *temp =
            [NSMutableDictionary dictionaryWithDictionary:[PointerPrefsController defaultSettings]];
        if ([iTermPreferences boolForKey:kPreferenceKeyThreeFingerEmulatesMiddle]) {
            ...
            for (NSString *key in temp) {
                if ([PointerPrefsController keyIsButton:key] &&
                    ...
                    NSString *gestureKey =
                        [PointerPrefsController keyForGesture:kThreeFingerClickGesture
                                                    modifiers:[PointerPrefsController modifiersForKey:key]];
                    [tempCopy setObject:middleAction forKey:gestureKey];
                }
            }
            temp = tempCopy;
            int modMasks[] = {NSEventModifierFlagCommand, NSEventModifierFlagOption, NSEventModifierFlagControl,
...
+ (void)setSettings:(NSDictionary *)newSettings {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kPointerPrefsChangedNotification object:nil];
}

+ (NSArray *)sortedKeys {
    //    NSArray *keys = [[PointerPrefsController settings]
    //    keysSortedByValueUsingSelector:@selector(comparePointerPrefsValue:)];
    NSArray *keys =
        [[[PointerPrefsController settings] allKeys] sortedArrayUsingSelector:@selector(comparePointerActions:)];
    return keys;
}
...
- (void)awakeFromNib {
    ...
    NSArray *actions = [[[PointerPrefsController localizedActionMap] allValues]
        sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
    [editAction_ addItemsWithTitles:actions];
}

+ (NSString *)keyForRowIndex:(int)rowIndex {
    // BUG-395: Bounds check before accessing sortedKeys to prevent crash
...
+ (NSString *)argumentWithButton:(int)buttonNumber numClicks:(int)numClicks modifiers:(int)modMask {
    NSString *key = [PointerPrefsController keyForButton:buttonNumber clicks:numClicks modifiers:modMask];
...
+ (BOOL)useCompatibilityEscapingWithButton:(int)buttonNumber numClicks:(int)numClicks modifiers:(int)modMask {
    NSString *key = [PointerPrefsController keyForButton:buttonNumber clicks:numClicks modifiers:modMask];
    NSDictionary *settings = [PointerPrefsController settings];
    NSDictionary *setting = [settings objectForKey:key];
...
+ (NSString *)actionWithButton:(int)buttonNumber numClicks:(int)numClicks modifiers:(int)modMask {
    NSString *key = [PointerPrefsController keyForButton:buttonNumber clicks:numClicks modifiers:modMask];
...
+ (BOOL)useCompatibilityEscapingForTapWithTouches:(int)numTouches modifiers:(int)modMask {
    NSString *gesture = @"";
    if (numTouches == 3) {
        gesture = kThreeFingerClickGesture;
    } else {
...
+ (NSString *)actionForGesture:(NSString *)gesture modifiers:(int)modMask {
    NSString *key;
    key = [PointerPrefsController keyForGesture:gesture modifiers:modMask];
...
+ (BOOL)useCompatibilityEscapingForGesture:(NSString *)gesture modifiers:(int)modMask {
    NSString *key;
    key = [PointerPrefsController keyForGesture:gesture modifiers:modMask];
...
+ (BOOL)compatibilityEscapingForGesture:(NSString *)gesture modifiers:(NSEventModifierFlags)modMask {
    NSString *key;
    key = [PointerPrefsController keyForGesture:gesture modifiers:modMask];
    NSDictionary *settings = [PointerPrefsController settings];
    NSDictionary *setting = [settings objectForKey:key];
...
- (void)setModifierButtons:(int)modMask {
    [editModifiersCommand_
        setState:(modMask & NSEventModifierFlagCommand) ? NSControlStateValueOn : NSControlStateValueOff];
    [editModifiersOption_
        setState:(modMask & NSEventModifierFlagOption) ? NSControlStateValueOn : NSControlStateValueOff];
    [editModifiersShift_
        setState:(modMask & NSEventModifierFlagShift) ? NSControlStateValueOn : NSControlStateValueOff];
    [editModifiersControl_
        setState:(modMask & NSEventModifierFlagControl) ? NSControlStateValueOn : NSControlStateValueOff];
}

- (void)setButtonNumber:(int)buttonNumber clickCount:(int)clickCount modifiers:(int)modMask {
    DLog(@"PointerPrefsController setButtonNumber:%d clickCount:%d modifiers:0x%x", buttonNumber, clickCount, modMask);
...
- (id)tableView:(NSTableView *)aTableView
    ...
    if (aTableColumn == buttonColumn_) {
        if (isButton) {
            ...
            NSString *modifiers =
                [PointerPrefsController localizedModifiers:[PointerPrefsController modifiersForKey:key]];
            if ([modifiers length]) {
                modifiers = [modifiers stringByAppendingString:@" + "];
            }
            NSString *gesture = [PointerPrefsController
                localizedGestureNameForGestureIdentifier:[PointerPrefsController gestureIdentifierForKey:key]];
...
- (void)updateArgumentFieldsForAction:(NSString *)actionIdent argument:(NSString *)currentArg {
    ...
    switch (argType) {
        ...
        case kScriptFunctionArg:
            ...
            [editArgumentField_ setStringValue:currentArg];
            _pasteSpecialViewContainer.hidden = YES;
            _invocationDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
                initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
                       passthrough:nil
                     functionsOnly:YES];
        ...
        case kProfileArg:
            ...
            [editArgumentButton_ populateWithProfilesSelectingGuid:currentArg profileTypes:ProfileTypeAll];
            _pasteSpecialViewContainer.hidden = YES;
            break;
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
    ...
    if (self.hasSelection) {
        ...
        [editModifiersCommand_
            setState:(modflags & NSEventModifierFlagCommand) ? NSControlStateValueOn : NSControlStateValueOff];
        [editModifiersOption_
            setState:(modflags & NSEventModifierFlagOption) ? NSControlStateValueOn : NSControlStateValueOff];
        [editModifiersShift_
            setState:(modflags & NSEventModifierFlagShift) ? NSControlStateValueOn : NSControlStateValueOff];
        [editModifiersControl_
            setState:(modflags & NSEventModifierFlagControl) ? NSControlStateValueOn : NSControlStateValueOff];
    }
...
- (IBAction)ok:(id)sender {
    ...
    NSMutableDictionary *newValue = [NSMutableDictionary dictionaryWithObject:theAction forKey:kActionKey];
    newValue[kVersionKey] = @(version_);
    if (![editArgumentField_ isHidden]) {
        [newValue setObject:[editArgumentField_ stringValue] forKey:kArgumentKey];
        ...
        if ([PointerPrefsController argumentTypeForAction:theAction] == kProfileArg) {
            ...
            [newValue setObject:[[editArgumentButton_ selectedItem] title] forKey:kArgumentKey];
        }
    } else if (!_pasteSpecialViewContainer.isHidden) {
        if ([PointerPrefsController argumentTypeForAction:theAction] == kAdvancedPasteArg) {
            [newValue setObject:[_pasteSpecialViewController stringEncodedSettings] ?: @"" forKey:kArgumentKey];
        }
    } else if (!_menuItemPopupView.isHidden) {
        newValue[kArgumentKey] = [NSString
    ...
    if ([editButton_ selectedTag] >= kMinGestureTag) {
        // Gesture
        newKey = [PointerPrefsController
            keyForGesture:[PointerPrefsController gestureIdentifierForTag:[editButton_ selectedTag]]
                modifiers:modMask];
    } else {
        // Button
...
- (IBAction)actionChanged:(id)sender {
    ...
    [self updateArgumentFieldsForAction:actionIdent argument:@""];
}
...
- (NSComparisonResult)comparePointerActions:(NSString *)other {
    ...
    if (selfIsButton) {
        ...
        // BUG-f1037: Fixed logic error - second condition was duplicate of first
        ...
        result = [[NSNumber numberWithInt:[[selfParts objectAtIndex:1] intValue]]
            compare:[NSNumber numberWithInt:[[otherParts objectAtIndex:1] intValue]]];
        if (result != NSOrderedSame) {
            return result;
        }
        result = [[NSNumber numberWithInt:[[selfParts objectAtIndex:2] intValue]]
            compare:[NSNumber numberWithInt:[[otherParts objectAtIndex:2] intValue]]];
        if (result != NSOrderedSame) {
            return result;
        }
        ...
        // BUG-1924: Fixed - was comparing self to self, should compare to other
        NSArray *otherParts = [PointerPrefsController gestureKeyComponents:other];
        // BUG-f1038/f1039: Guard against nil or malformed arrays to prevent crash
        if (!selfParts && !otherParts) {
            return NSOrderedSame;
        } else if (!selfParts && otherParts) {
            return NSOrderedAscending;
        ...
        // gestureKeyComponents validates count == 4, so indices 1, 2, 3 are safe here
        NSComparisonResult result;
        NSString *selfIdent =
            [PointerPrefsController localizedGestureNameForGestureIdentifier:[selfParts objectAtIndex:1]];
        NSString *otherIdent =
            [PointerPrefsController localizedGestureNameForGestureIdentifier:[otherParts objectAtIndex:1]];
