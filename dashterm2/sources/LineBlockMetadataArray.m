...
@implementation LineBlockMetadataArray {
    ...
    // BUG-1433: Protects _ref from concurrent access during CoW operations.
    // The _ref pointer itself can be read by cowCopy while willMutate is
    // assigning a new value, causing a data race.
    os_unfair_lock _refLock;
}
...
- (LineBlockMetadataArray *)cowCopy {
    // BUG-1433: Lock to prevent racing with willMutate's assignment to _ref.
    os_unfair_lock_lock(&_refLock);
    LineBlockMetadataArray *copy = [[LineBlockMetadataArray alloc] initWithGuts:_guts ref:_ref];
    os_unfair_lock_unlock(&_refLock);
    return copy;
}

- (void)willMutate {
    // BUG-1433: Lock to prevent racing with cowCopy's read of _ref.
    // The ifPluralDecrementThen: call is performed under our _refLock so that
    // concurrent cowCopy operations see a consistent _ref pointer.
    os_unfair_lock_lock(&_refLock);
...
- (const LineBlockMetadata *)metadataAtIndex:(int)i {
    ITAssertWithMessage(i >= 0 && i < _guts->_numEntries, @"i=%@ out of bounds [0, %@)", @(i), @(_guts->_numEntries));
...
- (NSArray *)encodedArray {
    NSMutableArray *metadataArray = [NSMutableArray arrayWithCapacity:_guts->_numEntries];
...
- (void)eraseFirstLineCache {
    ...
    if (_guts->_numEntries > _guts->_first) {
        LineBlockMetadata *metadata = &_guts->_array[_guts->_first];
        metadata->width_for_number_of_wrapped_lines = 0;
        metadata->number_of_wrapped_lines = 0;
        if (_guts->_useDWCCache) {
            metadata->doubleWidthCharacters = nil;
            iTermMetadataSetExternalAttributes(&metadata->lineMetadata, nil);
        }
        metadata->bidi_display_info = nil;
    }
}
