...
+ (NSString *)escapedText:(NSString *)text mode:(iTermSendTextEscaping)escaping {
    ...
    // BUG-13015: Replace assert(NO) with DLog and default return for unknown escaping type
    DLog(@"Unknown escaping type %d in escapedText:mode:, returning original string", (int)escaping);
    return temp;
}
...
+ (instancetype)withAction:(KEY_ACTION)action
    ...
    return [[self alloc] initWithDictionary:@{
        iTermKeyBindingDictionaryKeyAction : @(action),
...
- (NSDictionary *)dictionaryValue {
    ...
    NSDictionary *temp = @{
        iTermKeyBindingDictionaryKeyAction : @(_keyAction),
...
- (NSString *)displayName {
    ...
    switch (_keyAction) {
        ...
        case KEY_ACTION_RUN_COPROCESS:
            actionString = [NSString stringWithFormat:@"Run Coprocess \"%@\"", _parameter];
            break;
        ...
        case KEY_ACTION_NEW_WINDOW_WITH_PROFILE:
            if ([[ProfileModel sharedInstance] bookmarkWithGuid:_parameter]) {
                actionString =
                    [NSString stringWithFormat:@"New Window with \"%@\" Profile", GetProfileName(_parameter)];
            } else {
        ...
        case KEY_ACTION_SPLIT_HORIZONTALLY_WITH_PROFILE:
            if ([[ProfileModel sharedInstance] bookmarkWithGuid:_parameter]) {
                actionString =
                    [NSString stringWithFormat:@"Split Horizontally with \"%@\" Profile", GetProfileName(_parameter)];
            } else {
        ...
        case KEY_ACTION_SPLIT_VERTICALLY_WITH_PROFILE:
            if ([[ProfileModel sharedInstance] bookmarkWithGuid:_parameter]) {
                actionString =
                    [NSString stringWithFormat:@"Split Vertically with \"%@\" Profile", GetProfileName(_parameter)];
            } else {
        ...
        case KEY_ACTION_REMAP_LOCALLY:
            actionString = @"Remap Modifiers in DashTerm2 Only";
            break;
        ...
        case KEY_ACTION_PASTE_SPECIAL_FROM_SELECTION: {
            NSString *pasteDetails = [iTermPasteSpecialViewController descriptionForCodedSettings:_parameter];
        ...
        case KEY_ACTION_MOVE_END_OF_SELECTION_LEFT:
            actionString =
                [NSString stringWithFormat:@"Move End of Selection Left %@",
                                           [self.class stringForSelectionMovementUnit:_parameter.integerValue]];
            break;
        case KEY_ACTION_MOVE_END_OF_SELECTION_RIGHT:
            actionString =
                [NSString stringWithFormat:@"Move End of Selection Right %@",
                                           [self.class stringForSelectionMovementUnit:_parameter.integerValue]];
            break;
        case KEY_ACTION_MOVE_START_OF_SELECTION_LEFT:
            actionString =
                [NSString stringWithFormat:@"Move Start of Selection Left %@",
                                           [self.class stringForSelectionMovementUnit:_parameter.integerValue]];
            break;
        case KEY_ACTION_MOVE_START_OF_SELECTION_RIGHT:
            actionString =
                [NSString stringWithFormat:@"Move Start of Selection Right %@",
                                           [self.class stringForSelectionMovementUnit:_parameter.integerValue]];
            break;
        ...
        case KEY_ACTION_SEQUENCE: {
            NSArray<NSString *> *names = [[_parameter keyBindingActionsFromSequenceParameter]
                mapWithBlock:^id _Nullable(iTermKeyBindingAction *_Nonnull action) {
...
- (BOOL)sendsText {
    switch (self.keyAction) {
        ...
        case KEY_ACTION_SEQUENCE:
            return [[self.parameter keyBindingActionsFromSequenceParameter]
                anyWithBlock:^BOOL(iTermKeyBindingAction *action) {
...
- (NSArray<iTermKeyBindingAction *> *)keyBindingActionsFromSequenceParameter {
    NSArray<NSDictionary *> *dicts =
        [NSJSONSerialization JSONObjectWithData:[self dataUsingEncoding:NSUTF8StringEncoding] options:0 error:nil];
...
+ (NSString *)toggleSettingParameterForKey:(NSString *)key isProfile:(BOOL)isProfile label:(NSString *)label {
    NSDictionary *dict = @{@"key" : key, @"isProfile" : @(isProfile), @"label" : label};
