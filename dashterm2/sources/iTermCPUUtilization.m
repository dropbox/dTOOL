...
- (iTermCPUTicks)sample {
    ...
    // BUG-3300/3392: mach_host_self() returns a new send right each call that must be deallocated.
    // This method is called every second from the timer, so not deallocating causes a port leak.
    mach_port_t host_port = mach_host_self();
    kern_return_t rc = host_statistics(host_port, HOST_CPU_LOAD_INFO, (host_info_t)&cpuinfo, &count);
    // BUG-3300/3392: Always deallocate the host port to prevent mach port leak
...
+ (NSMutableDictionary<NSString *, iTermCPUUtilization *> *)sessionInstances {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        gSessionInstances = [[NSMutableDictionary alloc] initWithCapacity:16];
    });
...
+ (void)setInstance:(iTermCPUUtilization *)instance forSessionID:(NSString *)sessionID {
    // BUG-295: Guard against nil sessionID
    if (!sessionID) {
        return;
    }
...
- (void)setPublisher:(iTermPublisher<NSNumber *> *)publisher {
    ...
    [publisher addSubscriber:self
                       block:^(NSNumber *_Nonnull payload) {
...
- (void)addSubscriber:(id)subscriber block:(iTermCPUUtilizationObserver)block {
    [_router addSubscriber:subscriber
                     block:^(NSNumber *_Nonnull payload) {
