...
extension HorizontalFileListView: NSCollectionViewDataSource {
    ...
    func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
        // BUG-1704: Use guard with as? instead of as! for collection view item cast
        guard let item = collectionView.makeItem(withIdentifier: NSUserInterfaceItemIdentifier("FileItem"), for: indexPath) as? FileItemView else {
            return NSCollectionViewItem()
        }
        // BUG-f1023: Bounds check before accessing files array
        // Index can be stale if files array was modified between numberOfItemsInSection and this call
        let itemIndex = indexPath.item
        guard itemIndex >= 0 && itemIndex < files.count else {
            DLog("HorizontalFileListView collectionView(_:itemForRepresentedObjectAt:): index \(itemIndex) out of bounds for count \(files.count)")
            return NSCollectionViewItem()
        }
        let file = files[itemIndex]
        item.configure(with: file)
        item.setSelected(collectionView.selectionIndexes.contains(itemIndex))

        item.onDeleteRequested = { [weak self] in
            self?.deleteItem(at: itemIndex)
        }
...
class FileItemView: NSCollectionViewItem {
    ...
    required init?(coder: NSCoder) {
        // BUG-f802: Return nil instead of crashing for unused coder initializer
        DLog("FileCollectionViewItem init(coder:) is not supported")
        return nil
    }
    ...
    func configure(with file: HorizontalFileListView.File) {
        ...
        case let .inMemory(filename: _, content: _, mimeType: mimeType):
            ...
            // BUG-1792: Use nil coalescing with fallback instead of force unwrap
            icon = NSWorkspace.shared.icon(for: UTType(mimeType: mimeType) ?? UTType(filenameExtension: ext) ?? .plainText)
            progressIndicator.isHidden = true
    ...
    private func wrapTextForTwoLines(_ words: [SeparatedComponent], font: NSFont, width: CGFloat) -> String {
        // BUG-401: Guard against empty words array to prevent crash on words[0] access
