    ...
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(imageDidLoad:)
                                                   ...
                                                   object:nil];
        _missingImages = [[NSMutableSet alloc] initWithCapacity:8];
        _loadedImages = [[NSMutableSet alloc] initWithCapacity:8];
...
- (nullable id<iTermMetalDriverDataSourcePerFrameState>)metalDriverWillBeginDrawingFrame {
    ...
    iTermAttributedStringBuilder *attributedStringBuilder =
        [[iTermAttributedStringBuilder alloc] initWithStats:statsPointers];
...
- (void)metalDidFindImages:(NSSet<NSString *> *)foundImages
             ...
             animatedLines:(NSIndexSet *)animatedLines {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [animatedLines enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
            const long long absoluteLine = (long long)idx;
            if (absoluteLine >= offset) {
                const int row = (int)(absoluteLine - offset);
...
- (void)metalDriverDidDrawFrame:(id<iTermMetalDriverDataSourcePerFrameState>)perFrameState {
    ...
    if (!VT100GridSizeEquals(state.gridSize,
                             VT100GridSizeMake(_textView.dataSource.width, _textView.dataSource.height))) {
        return;
    }
    if (!CGSizeEqualToSize(state.cellSize, CGSizeMake(_textView.charWidth, _textView.lineHeight))) {
        return;
    }
...
- (void)metalDriverDidProduceDebugInfo:(nonnull NSData *)archive {
    NSString *filename = @"/tmp/DashTerm2-frame-capture.zip";
