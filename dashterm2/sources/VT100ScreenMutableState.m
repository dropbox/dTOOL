...
// Optimization: Cache NSNumber objects for common column/charset indices (0-511) to avoid boxing.
// Terminal columns are typically 80-200, and charsets are 0-3. Tab stop operations use column
// indices extensively. Caching 512 values covers virtually all use cases with zero allocations.
static const int kScreenStateCachedKeyCount = 512;
static NSNumber *sScreenStateCachedKeys[kScreenStateCachedKeyCount];

static inline uint8_t VT100ScreenLineDrawingBitForCharset(int charset) {
    if (charset < 0 || charset >= NUM_CHARSETS) {
        return 0;
    }
    return (uint8_t)(1u << charset);
}

// Copies the terminal's processed default attributes into a run of ASCII screen chars while
// preserving each cell's glyph code that was populated during parsing.
static inline void iTermApplyDefaultCharToAsciiBuffer(screen_char_t *buffer,
                                                      const char *asciiBytes,
                                                      const screen_char_t templateChar,
                                                      const int length) {
    if (length <= 0 || !buffer) {
        return;
    }
    buffer[0] = templateChar;
    int filled = 1;
    while (filled < length) {
        const int chunk = MIN(filled, length - filled);
        memcpy(buffer + filled, buffer, chunk * sizeof(screen_char_t));
        filled += chunk;
    }
    if (!asciiBytes) {
        return;
    ...
    for (int i = 0; i < length; i++) {
        buffer[i].code = (unsigned char)asciiBytes[i];
    }
}

NS_INLINE NSNumber *VT100ScreenStateKeyToNumber(int key) {
    if (key >= 0 && key < kScreenStateCachedKeyCount) {
        return sScreenStateCachedKeys[key];
    }
    return @(key);
}

__attribute__((constructor))
static void VT100ScreenMutableStateInitializeCachedKeys(void) {
    for (int i = 0; i < kScreenStateCachedKeyCount; i++) {
        sScreenStateCachedKeys[i] = @(i);
    }
}
...
@implementation VT100ScreenMutableState {
    ...
    NSArray<NSNumber *> *_cachedSortedTabStops;
    NSMutableIndexSet *_tabStopIndexSet;
    BOOL _tabStopsDirty;
    uint8_t _charsetLineDrawingMask;
    // Pre-allocated mutable string for appendStringAtCursor to avoid per-call allocations
    NSMutableString *_augmentedStringBuffer;
    // dterm-core parser adapter for Phase 7.1 parallel parsing
...
- (instancetype)initWithSideEffectPerformer:(id<VT100ScreenSideEffectPerforming>)performer {
    ...
    if (self) {
        ...
        _postTriggerActions = [NSMutableArray arrayWithCapacity:4];  // Typically few trigger actions
        ...
        _tabStopsDirty = YES;
        _charsetLineDrawingMask = 0;
        // Pre-allocate buffer for appendStringAtCursor string augmentation
        // Typical terminal lines are under 256 chars; this avoids per-call allocations
        _augmentedStringBuffer = [[NSMutableString alloc] initWithCapacity:256];
        // Initialize dterm-core parser adapter for Phase 7.1 parallel parsing
        if ([iTermAdvancedSettingsModel dtermCoreEnabled]) {
...
- (void)setConfig:(VT100MutableScreenConfiguration *)config {
    ...
    // BUG-f1192: Replace assert with guard - setConfig outside joined block should be logged not crash
    if (!VT100ScreenMutableState.performingJoinedBlock) {
        DLog(@"WARNING: VT100ScreenMutableState setConfig called outside of joined block");
    }
...
- (void)incrementOverflowBy:(int)overflowCount {
    ...
    // BUG-f1193: Replace assert with guard - negative overflow should be clamped not crash
    if (self.cumulativeScrollbackOverflow < 0) {
        DLog(@"WARNING: VT100ScreenMutableState incrementOverflowBy: cumulativeScrollbackOverflow is negative (%lld), resetting to 0",
             self.cumulativeScrollbackOverflow);
        self.cumulativeScrollbackOverflow = 0;
    }
    // BUG-f1194: Replace assert with guard - negative overflowCount should be skipped not crash
    if (overflowCount < 0) {
        DLog(@"WARNING: VT100ScreenMutableState incrementOverflowBy: negative overflowCount %d, skipping", overflowCount);
        return;
    }
...
- (void)appendLineFeed {
    ...
    const BOOL atScrollBottom = grid.cursor.y == VT100GridRangeMax(grid.scrollRegionRows);
    // Move selection when scrolling happens without sending to line buffer:
    // 1. When there's a scroll region and we're at its bottom
    // 2. When in alt screen without scrollback and we're at the bottom of the scrollable area
    const BOOL shouldMoveSelectionIfUnsent = atScrollBottom && (haveScrollRegion || noScrollback);
    BOOL sent = NO;
...
- (void)appendStringAtCursor:(NSString *)string {
    ...
    if (3 * len >= kStaticBufferElements) {
        ...
        // BUG-f1195: Remove redundant assert before existing nil check - assert was already guarded
        if (!buffer) {
    ...
    // Reuse pre-allocated buffer for string augmentation to avoid per-call allocations
    [_augmentedStringBuffer setString:@""];
    if (augmented) {
        [_augmentedStringBuffer appendString:predecessorString];
    } else {
        // Prepend a space so we can detect if the first character is a combining mark.
        [_augmentedStringBuffer appendString:@" "];
    }
    [_augmentedStringBuffer appendString:string];

    // BUG-f1196: Replace assert with guard - nil terminal should return not crash
    if (!self.terminal) {
        DLog(@"ERROR: VT100ScreenMutableState appendStringToTriggerLine: terminal is nil");
        if (dynamicBuffer) {
            free(dynamicBuffer);
        }
        return;
    ...
    BOOL dwc = NO;
    BOOL rtlFound = NO;
    StringToScreenChars(_augmentedStringBuffer,
                        buffer,
...
- (void)appendAsciiDataAtCursor:(AsciiData *)asciiData {
    ...
    if (memcmp(&defaultChar, &zero, sizeof(defaultChar))) {
        ...
        iTermApplyDefaultCharToAsciiBuffer(buffer, asciiData->buffer, defaultChar, len);
...
- (BOOL)shouldConvertCharactersToGraphicsCharacterSetInTerminal:(VT100Terminal *)terminal {
    return [self lineDrawingModeEnabledForCharset:terminal.charset];
}
    ...
    switch (self.protectedMode) {
        ...
        case VT100TerminalProtectedModeDEC: {
            ...
            [self.currentGrid setCharsFrom:self.currentGrid.cursor
                        ...
                        externalAttributes:nil];
            // clearTriggerLine sets hard continuation. Whether to conditionally set this only when
            // last char is erased is a minor edge case - current behavior errs on the safe side.
            [self clearTriggerLine];
            break;
        }
        case VT100TerminalProtectedModeISO:
    ...
    if (absCursorCoord.y >= absLine) {
        ...
        if (numberOfLinesRemoved > 0) {
            [marksToMove enumerateObjectsUsingBlock:^(id<IntervalTreeObject>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                // BUG-f1197: Replace assert with warning - failed remove should be logged not crash
                if (!removed) {
                    DLog(@"WARNING: VT100ScreenMutableState clearFromAbsoluteLineToEnd: failed to remove object %@", obj);
                }
...
- (void)rebuildCharsetLineDrawingMask {
    _charsetLineDrawingMask = 0;
    for (NSNumber *charset in self.charsetUsesLineDrawingMode) {
        _charsetLineDrawingMask |= VT100ScreenLineDrawingBitForCharset(charset.intValue);
    }
}

- (void)updateLineDrawingMaskForCharset:(int)charset enabled:(BOOL)enabled {
    uint8_t bit = VT100ScreenLineDrawingBitForCharset(charset);
    if (!bit) {
        return;
    }
    if (enabled) {
        _charsetLineDrawingMask |= bit;
    } else {
        _charsetLineDrawingMask &= ~bit;
    }
}

- (BOOL)lineDrawingModeEnabledForCharset:(int)charset {
    uint8_t bit = VT100ScreenLineDrawingBitForCharset(charset);
    if (!bit) {
        return [self.charsetUsesLineDrawingMode containsObject:VT100ScreenStateKeyToNumber(charset)];
    }
    return (_charsetLineDrawingMask & bit) != 0;
}

- (void)setCharacterSet:(int)charset usesLineDrawingMode:(BOOL)lineDrawingMode {
    NSNumber *charsetKey = VT100ScreenStateKeyToNumber(charset);
    if (lineDrawingMode) {
        if (![self.charsetUsesLineDrawingMode containsObject:charsetKey]) {
            [self.charsetUsesLineDrawingMode addObject:charsetKey];
            [self updateLineDrawingMaskForCharset:charset enabled:YES];
        }
    } else {
        if ([self.charsetUsesLineDrawingMode containsObject:charsetKey]) {
            [self.charsetUsesLineDrawingMode removeObject:charsetKey];
            [self updateLineDrawingMaskForCharset:charset enabled:NO];
        }
    }
}

- (void)setCharsetUsesLineDrawingMode:(NSMutableSet<NSNumber *> *)charsetUsesLineDrawingMode {
    [super setCharsetUsesLineDrawingMode:charsetUsesLineDrawingMode];
    [self rebuildCharsetLineDrawingMask];
}

#pragma mark - Tabs

- (void)markTabStopsDirty {
    _tabStopsDirty = YES;
    _cachedSortedTabStops = nil;
    _tabStopIndexSet = nil;
}

- (NSIndexSet *)tabStopIndexSet {
    if (_tabStopIndexSet) {
        return _tabStopIndexSet;
    }

    NSMutableIndexSet *rebuilt = [NSMutableIndexSet indexSet];
    for (NSNumber *stop in self.tabStops) {
        const NSInteger column = stop.integerValue;
        if (column >= 0) {
            [rebuilt addIndex:(NSUInteger)column];
        }
    }
    _tabStopIndexSet = rebuilt;
    return _tabStopIndexSet;
}

- (NSArray<NSNumber *> *)sortedTabStops {
    if (_cachedSortedTabStops && !_tabStopsDirty) {
        return _cachedSortedTabStops;
    }
    NSIndexSet *indexSet = [self tabStopIndexSet];
    if (indexSet.count == 0) {
        _cachedSortedTabStops = @[];
    } else {
        NSMutableArray<NSNumber *> *sorted = [[NSMutableArray alloc] initWithCapacity:indexSet.count];
        [indexSet enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL * _Nonnull stop) {
            [sorted addObject:VT100ScreenStateKeyToNumber((int)idx)];
        }];
        _cachedSortedTabStops = [sorted copy];
    }
    _tabStopsDirty = NO;
    return _cachedSortedTabStops;
}
...
- (void)setInitialTabStops {
    ...
    for (int i = 0; i < kInitialTabWindow; i += width) {
        [self.tabStops addObject:VT100ScreenStateKeyToNumber(i)];
    }
    DLog(@"Initial tabstops set to %@", self.tabStops);
    [self markTabStopsDirty];
}

- (void)setTabStops:(NSMutableSet<NSNumber *> *)tabStops {
    [super setTabStops:tabStops];
    [self markTabStopsDirty];
}
...
- (int)tabStopAfterColumn:(int)lowerBound {
    const int widthMinusOne = MAX(0, self.width - 1);
    NSIndexSet *indexSet = [self tabStopIndexSet];
    if (indexSet.count == 0) {
        return widthMinusOne;
    }

    const int searchValue = lowerBound + 1;
    const NSUInteger clampedSearchValue = searchValue < 0 ? 0 : (NSUInteger)searchValue;
    NSUInteger nextIndex;
    if ([indexSet containsIndex:clampedSearchValue]) {
        nextIndex = clampedSearchValue;
    } else {
        nextIndex = [indexSet indexGreaterThanIndex:clampedSearchValue];
    }
    if (nextIndex == NSNotFound || nextIndex >= (NSUInteger)widthMinusOne) {
        return widthMinusOne;
    }
...
- (void)setTabStopAtCursor {
    const int column = self.currentGrid.cursorX;
    if (column < 0 || column >= self.currentGrid.size.width) {
        return;
    }
    if ([[self tabStopIndexSet] containsIndex:(NSUInteger)column]) {
        return;
    }
    [self.tabStops addObject:VT100ScreenStateKeyToNumber(column)];
...
- (void)removeTabStopAtCursor {
    ...
    [self.tabStops removeObject:VT100ScreenStateKeyToNumber(column)];
    DLog(@"Remove tabstop at %@", @(column));
    [self markTabStopsDirty];
}
...
- (BOOL)haveTabStopAt:(int)x {
    ...
    return [[self tabStopIndexSet] containsIndex:(NSUInteger)x];
}

// Optimization: Find the tab stop at or before the given column using binary search.
// Returns -1 if no tab stop exists at or before the column.
- (int)tabStopAtOrBeforeColumn:(int)column {
    if (column < 0) {
        return -1;
    }
    NSIndexSet *indexSet = [self tabStopIndexSet];
    ...
    const NSUInteger clampedColumn = (NSUInteger)column;
    if ([indexSet containsIndex:clampedColumn]) {
        candidate = clampedColumn;
    } else {
        candidate = [indexSet indexLessThanIndex:clampedColumn];
    }
    if (candidate == NSNotFound) {
        return -1;
    }
    if (candidate >= (NSUInteger)self.width) {
...
- (void)backTab:(int)n {
    const int leftMargin = self.currentGrid.leftMargin;
    for (int i = 0; i < n; i++) {
        if (self.currentGrid.cursorX > leftMargin) {
            // Optimization: Use binary search to find the previous tab stop
            // instead of iterating backwards and boxing integers on each check.
            int prevTabStop = [self tabStopAtOrBeforeColumn:self.currentGrid.cursorX - 1];
            if (prevTabStop >= leftMargin) {
                self.currentGrid.cursorX = prevTabStop;
            } else {
                self.currentGrid.cursorX = leftMargin;
            }
            [self clearTriggerLine];
        }
    }
...
- (void)removeObjectsFromIntervalTree:(NSArray<id<IntervalTreeObject>> *)objects {
    ...
    [objects enumerateObjectsUsingBlock:^(id<IntervalTreeObject>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        Interval *interval = obj.entry.interval;
        const BOOL removed = [self.mutableIntervalTree removeObject:obj];
        // BUG-f1198: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState removeObjectsFromIntervalTree: failed to remove object %@", obj);
        }
        [self didRemoveObjectFromIntervalTree:obj formerInterval:interval];
    }];
}
...
- (NSMutableArray<id<IntervalTreeObject>> *)removeIntervalTreeObjectsInAbsRange:(VT100GridAbsCoordRange)absCoordRange
    ...
    NSMutableArray<id<IntervalTreeObject>> *marksToMove = [NSMutableArray arrayWithCapacity:4];  // Typically few marks to move
    NSMutableArray<id<IntervalTreeObject>> *marksToRemove = [NSMutableArray arrayWithCapacity:8];  // Usually more removals
...
- (NSArray<id<IntervalTreeObject>> *)intervalTreeObjectsContainedEntirelyInAbsRange:(VT100GridAbsCoordRange)absCoordRange {
    NSMutableArray<id<IntervalTreeObject>> *result = [NSMutableArray arrayWithCapacity:8];  // Typical number of objects in range
...
- (void)removeInaccessibleIntervalTreeObjects {
    ...
    for (id<IntervalTreeObject> obj in [self.intervalTree objectsInInterval:deadInterval]) {
        if ([obj.entry.interval limit] <= lastDeadLocation) {
            ...
            const BOOL removed = [self removeObjectFromIntervalTree:obj];
            // BUG-f1199: Replace assert with warning - failed remove should be logged not crash
            if (!removed) {
                DLog(@"WARNING: VT100ScreenMutableState removeInaccessibleIntervalTreeObjects: failed to remove object %@", obj);
            }
        }
    }
    DLog(@"End");
...
- (NSString *)intervalTreeDump {
    NSMutableArray<NSString *> *lines = [NSMutableArray arrayWithCapacity:16];  // Debug dump, moderate capacity
...
- (void)commandRangeDidChange {
    ...
    if (self.config.wantsCommandChangeNotifications) {
        ...
        [_commandRangeChangeJoiner setNeedsUpdateWithBlock:^{
            ...
            // BUG-f1204: Replace assert with dispatch to main - off-thread should dispatch not crash
            if (![NSThread isMainThread]) {
                DLog(@"WARNING: VT100ScreenMutableState commandRangeChangeJoiner: called off main thread, dispatching");
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf performSideEffect:^(id<VT100ScreenDelegate> delegate) {
                        [weakSelf notifyDelegateOfCommandChange:command
                                            ...
                                            sideEffectPerformer:weakSelf.sideEffectPerformer
                                                       delegate:delegate];
                    } name:@"command range did change (dispatched)"];
                });
                return;
            }
...
- (void)setWorkingDirectory:(NSString *)workingDirectory
    ...
    if (workingDirectory.length) {
        ...
        if ([previousWorkingDirectory.workingDirectory isEqualTo:workingDirectory]) {
            ...
            // BUG-f1200: Replace assert with warning - failed remove should be logged not crash
            if (!removed) {
                DLog(@"WARNING: VT100ScreenMutableState setWorkingDirectory: failed to remove previousWorkingDirectory %@", previousWorkingDirectory);
            }
...
- (NSArray<VT100ScreenMark *> *)promptMarksBelowLine:(int)line {
    ...
    NSMutableArray<VT100ScreenMark *> *marks = [NSMutableArray arrayWithCapacity:8];  // Usually few prompt marks below
...
- (void)removePromptMarksBelowLine:(int)line {
    ...
    [[self promptMarksBelowLine:line] enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        const BOOL removed = [self removeObjectFromIntervalTree:obj];
        // BUG-f1201: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState removePromptMarksBelowLine: failed to remove object %@", obj);
        }
    }];
}
...
- (NSArray<id<IntervalTreeObject>> *)moveNotesOnScreenFrom:(IntervalTree *)source
    ...
    for (id<IntervalTreeObject> obj in objectsMoved) {
        ...
        // BUG-f1202: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState moveNotesOnScreenFrom: failed to remove object %@ from source tree", obj);
        }
...
- (NSArray<iTermTuple<id<IntervalTreeObject>, Interval *> *> *)removeNotesOnScreenFrom:(IntervalTree *)source
    ...
    Interval *sourceInterval = [self intervalForGridCoordRange:screenRange];
    self.lastCommandMark = nil;
    NSMutableArray<iTermTuple<id<IntervalTreeObject>, Interval *> *> *objects = [NSMutableArray arrayWithCapacity:8];  // Typical notes on screen
    for (id<IntervalTreeObject> obj in [source objectsInInterval:sourceInterval]) {
        Interval *interval = obj.entry.interval;
        DLog(@"  found note with interval %@. Remove %@", interval, obj);
        const BOOL removed = [source removeObject:obj];
        // BUG-f1203: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState removeNotes: failed to remove object %@ from source tree", obj);
        }
        Interval *newInterval = [Interval intervalWithLocation:interval.location + offset
                                                        length:interval.length];
        DLog(@"  new interval is %@", interval);
...
- (void)swapOnscreenIntervalTreeObjects {
    ...
    // BUG-10282 fix: Instead of removeAllObjects followed by individual adds (which could
    // leave an inconsistent state during rapid alt/primary buffer switches), build the new
    // saved tree contents first, then perform a single removeAllObjects + batch add operation.
    // The saved tree should now be empty after moveNotesOnScreenFrom removed objects, but
    // verify this and only clear if there are unexpected leftovers.
    if ([self.mutableSavedIntervalTree count] > 0) {
        DLog(@"Warning: saved tree not empty after move, clearing %lu remaining objects",
             (unsigned long)[self.mutableSavedIntervalTree count]);
...
- (void)reloadMarkCache {
    ...
    // Performance optimization: Use forwardLimitEnumerator instead of allObjects to avoid
    // O(n) memory allocation for the array copy. The enumerator iterates in-place.
...
- (void)setPathFromURL:(NSString *)path {
    ...
    [self currentDirectoryDidChangeTo:path completion:^{
        ...
        // BUG-11314: Only create prompt marks from OSC 7 if shell integration is NOT installed.
        // When shell integration is installed (e.g., fish 3.7.0+), the shell sends proper OSC 133
        // sequences to mark prompts. Creating marks from OSC 7 directory changes when shell
        // integration is active causes duplicate/spurious mark indicators to appear.
        if (!strongSelf.shellIntegrationInstalled) {
...
- (void)promptEndedAndCommandStartedAt:(VT100GridCoord)commandStartLocation
    ...
    if (promptRange.end.y - promptRange.start.y > maxPromptLines) {
        ...
        // BUG-3828: Fix prompt range start calculation - was incorrectly setting both to maxPromptLines
...
- (NSArray<ScreenCharArray *> *)contentInRange:(VT100GridAbsCoordRange)range {
    // Pre-allocate based on the line range - exact size is (end.y - start.y + 1) lines
    const NSInteger lineCount = range.end.y - range.start.y + 1;
    NSMutableArray<ScreenCharArray *> *result = [NSMutableArray arrayWithCapacity:lineCount];
...
- (void)removeAnnotation:(PTYAnnotation *)annotation {
    if ([self.intervalTree containsObject:annotation]) {
        ...
        // BUG-f1205: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState removeAnnotation: failed to remove annotation from intervalTree %@", annotation);
        }
        ...
        // BUG-f1206: Replace assert with warning - failed remove should be logged not crash
        if (!removed) {
            DLog(@"WARNING: VT100ScreenMutableState removeAnnotation: failed to remove annotation from savedIntervalTree %@", annotation);
        }
    }
    [self setNeedsRedraw];
}
...
- (void)replaceRange:(VT100GridAbsCoordRange)absRange
    ...
    // Pre-allocate exact capacity for the porthole lines
    NSMutableArray<ScreenCharArray *> *lines = [NSMutableArray arrayWithCapacity:numLines];
...
- (void)changeHeightOfMark:(iTermMark *)mark to:(int)newHeight {
    ...
    // Pre-allocate exact capacity for the new height
    NSMutableArray<ScreenCharArray *> *lines = [NSMutableArray arrayWithCapacity:newHeight];
...
- (VT100GridAbsCoordRange)reallyReplaceRange:(VT100GridAbsCoordRange)absRange
    ...
    const int removedLineCount = MIN(range.end.y - range.start.y + 1, numLinesBeforeFolding - range.start.y);
    NSMutableArray<ScreenCharArray *> *removedSCAs = [NSMutableArray arrayWithCapacity:MAX(0, removedLineCount)];
...
- (void)threadedReadTask:(char *)buffer length:(int)length {
    // Phase 10: Skip legacy parser entirely when dterm-core parser output is enabled
    // This is a performance optimization - no need to run both parsers
    const BOOL useDTermCoreParserOutput = (_dtermCoreParserAdapter != nil &&
                                           [iTermAdvancedSettingsModel dtermCoreParserOutputEnabled]);
    const BOOL runParserComparison = [iTermAdvancedSettingsModel dtermCoreParserComparisonEnabled];

    NSArray<VT100Token *> *dtermTokens = nil;
    CVector vector;
    int nonSignalingLength = length;

    if (useDTermCoreParserOutput) {
        // dterm-core is the primary parser - skip legacy parser entirely
        dtermTokens = [_dtermCoreParserAdapter parseWithBytes:(const uint8_t *)buffer length:length];

        // Only run legacy parser if comparison mode is enabled (for debugging)
        if (runParserComparison) {
            [self.terminal.parser putStreamData:buffer length:length];
            CVectorCreate(&vector, 100);
            nonSignalingLength = [self.terminal.parser addParsedTokensToVector:&vector];
            [self compareITerm2Tokens:&vector withDTermCoreTokens:dtermTokens];
            CVectorReleaseObjects(&vector);
            CVectorDestroy(&vector);
        }

        // Convert dterm-core NSArray to CVector
        CVectorCreate(&vector, (int)dtermTokens.count + 1);
        for (VT100Token *token in dtermTokens) {
            // Retain token and add to vector (CVector stores raw pointers)
            CVectorAppend(&vector, (__bridge_retained void *)token);
        }
    } else {
        // Legacy path: use iTerm2's VT100Parser
        // Run dterm-core in parallel if adapter exists (for comparison/validation)
        if (_dtermCoreParserAdapter != nil) {
            dtermTokens = [_dtermCoreParserAdapter parseWithBytes:(const uint8_t *)buffer length:length];
        }

        // Pass the input stream to iTerm2's parser.
        [self.terminal.parser putStreamData:buffer length:length];
        ...
        // Compare token sequences if parser comparison is enabled
        if (dtermTokens != nil && runParserComparison) {
            [self compareITerm2Tokens:&vector withDTermCoreTokens:dtermTokens];
        }
    }
...
// Phase 7.1/9: Compare token sequences between iTerm2's VT100Parser and dterm-core
- (void)compareITerm2Tokens:(CVector *)itermVector withDTermCoreTokens:(NSArray<VT100Token *> *)dtermTokens {
    NSUInteger itermCount = CVectorCount(itermVector);
    NSUInteger dtermCount = dtermTokens.count;
    BOOL hasMismatch = NO;

    // Quick check: count mismatch
    if (itermCount != dtermCount) {
        NSLog(@"[dterm-core parser] Token count mismatch: iTerm2=%lu, dterm-core=%lu",
              (unsigned long)itermCount, (unsigned long)dtermCount);
        hasMismatch = YES;
    }

    // Detailed comparison (limited to avoid log spam)
    NSUInteger compareCount = MIN(MIN(itermCount, dtermCount), (NSUInteger)10);
    for (NSUInteger i = 0; i < compareCount; i++) {
        VT100Token *itermToken = (__bridge VT100Token *)CVectorGet(itermVector, i);
        VT100Token *dtermToken = dtermTokens[i];

        if (itermToken.type != dtermToken.type) {
            NSLog(@"[dterm-core parser] Token[%lu] type mismatch: iTerm2=%d (%@), dterm-core=%d (%@)",
                  (unsigned long)i,
                  (int)itermToken.type, [self tokenTypeName:itermToken.type],
                  (int)dtermToken.type, [self tokenTypeName:dtermToken.type]);
            hasMismatch = YES;
        } else {
            // Types match - compare content for specific token types
            if (![self compareTokenContent:itermToken with:dtermToken atIndex:i]) {
                hasMismatch = YES;
            }
        }
    }

    if (itermCount > 10 || dtermCount > 10) {
        NSLog(@"[dterm-core parser] (comparison limited to first 10 tokens)");
    }

    // Log summary when no mismatches (useful to know parser is working)
    if (!hasMismatch && itermCount > 0) {
        static NSUInteger successCount = 0;
        successCount++;
        // Log every 1000th success to avoid spam but confirm it's working
        if (successCount % 1000 == 0) {
            NSLog(@"[dterm-core parser] %lu successful comparisons, last batch: %lu tokens",
                  (unsigned long)successCount, (unsigned long)itermCount);
        }
    }
}

// Compare content of tokens that have the same type
- (BOOL)compareTokenContent:(VT100Token *)itermToken with:(VT100Token *)dtermToken atIndex:(NSUInteger)index {
    BOOL match = YES;

    // Compare ASCII string content
    if (itermToken.type == VT100_ASCIISTRING) {
        AsciiData *itermData = itermToken.asciiData;
        AsciiData *dtermData = dtermToken.asciiData;
        if (itermData && dtermData) {
            if (itermData->length != dtermData->length) {
                NSLog(@"[dterm-core parser] Token[%lu] ASCII length mismatch: iTerm2=%d, dterm-core=%d",
                      (unsigned long)index, itermData->length, dtermData->length);
                match = NO;
            } else if (itermData->length > 0 && memcmp(itermData->buffer, dtermData->buffer, itermData->length) != 0) {
                NSLog(@"[dterm-core parser] Token[%lu] ASCII content mismatch", (unsigned long)index);
                match = NO;
            }
        }
    }

    // Compare CSI parameters for CSI sequences
    CSIParam *itermCSI = itermToken.csi;
    CSIParam *dtermCSI = dtermToken.csi;
    if (itermCSI && dtermCSI) {
        if (itermCSI->count != dtermCSI->count) {
            NSLog(@"[dterm-core parser] Token[%lu] CSI param count mismatch: iTerm2=%d, dterm-core=%d",
                  (unsigned long)index, itermCSI->count, dtermCSI->count);
            match = NO;
        } else {
            // Compare individual parameters
            for (int j = 0; j < itermCSI->count && j < VT100CSIPARAM_MAX; j++) {
                int32_t itermParam = ((int32_t *)&itermCSI->p)[j];
                int32_t dtermParam = ((int32_t *)&dtermCSI->p)[j];
                if (itermParam != dtermParam) {
                    NSLog(@"[dterm-core parser] Token[%lu] CSI param[%d] mismatch: iTerm2=%d, dterm-core=%d",
                          (unsigned long)index, j, itermParam, dtermParam);
                    match = NO;
                }
            }
        }
...
// Helper to get human-readable token type name
- (NSString *)tokenTypeName:(VT100TerminalTokenType)type {
    // Common token types for debugging
    switch (type) {
        case VT100_ASCIISTRING: return @"ASCII_STRING";
        case VT100_STRING: return @"STRING";
        case VT100CC_CR: return @"CR";
        case VT100CC_LF: return @"LF";
        case VT100CC_BS: return @"BS";
        case VT100CC_HT: return @"TAB";
        case VT100CC_BEL: return @"BEL";
        case VT100CC_ESC: return @"ESC";
        case VT100CSI_CUU: return @"CSI_CUU";
        case VT100CSI_CUD: return @"CSI_CUD";
        case VT100CSI_CUF: return @"CSI_CUF";
        case VT100CSI_CUB: return @"CSI_CUB";
        case VT100CSI_CUP: return @"CSI_CUP";
        case VT100CSI_ED: return @"CSI_ED";
        case VT100CSI_EL: return @"CSI_EL";
        case VT100CSI_SGR: return @"CSI_SGR";
        case VT100CSI_DECSC: return @"DECSC";
        case VT100CSI_DECRC: return @"DECRC";
        case VT100CSI_IND: return @"IND";
        case VT100CSI_RI: return @"RI";
        case VT100_NOTSUPPORT: return @"NOT_SUPPORT";
        default: return [NSString stringWithFormat:@"TYPE_%d", (int)type];
    }
}
...
- (void)injectData:(NSData *)data {
    // Use dterm-core parser if enabled, otherwise fall back to legacy parser
    const BOOL useDTermCore = (_dtermCoreParserAdapter != nil &&
                               [iTermAdvancedSettingsModel dtermCoreParserOutputEnabled]);

    if (useDTermCore) {
        // dterm-core path: use the adapter directly
        NSArray<VT100Token *> *tokens = [_dtermCoreParserAdapter parseWithBytes:(const uint8_t *)data.bytes
                                                                          length:(int)data.length];
        if (tokens.count == 0) {
            return;
        }
        // Convert NSArray to CVector for addTokens
        CVector vector;
        CVectorCreate(&vector, (int)tokens.count);
...
lengthExcludingInBandSignaling:data.length
        ...
        // Legacy path: create a local VT100Parser
...
- (void)restoreColorsFromSlot:(VT100SavedColorsSlot *)slot {
    ...
    NSMutableDictionary<NSNumber *, id> *dict = [NSMutableDictionary dictionaryWithCapacity:MAX(0, limit - 16)];
...
- (void)willSynchronize {
    // BUG-f1207: Replace assert with warning - willSynchronize outside joined block should warn not crash
    if (!VT100ScreenMutableState.performingJoinedBlock) {
        DLog(@"WARNING: VT100ScreenMutableState willSynchronize called outside of joined block");
    }
...
- (void)performBlockWithJoinedThreads:(void (^ NS_NOESCAPE)(VT100Terminal *terminal,
    ...
    if (gPerformingJoinedBlock) {
        ...
        // BUG-f1208: Replace assert with dispatch to main - off-thread should dispatch not crash
        if (![iTermGCD onMainQueue]) {
            DLog(@"WARNING: VT100ScreenMutableState performBlockWithJoinedThreads called off main queue, dispatching");
            __weak __typeof(self) weakSelf = self;
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf performBlockWithJoinedThreads:block];
            });
            return;
        }
...
- (void)performSynchroDanceWithBlock:(void (^)(void))block {
    // BUG-f1209: Replace assert with dispatch to main - off-thread should dispatch not crash
    if (dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL) != dispatch_queue_get_label(dispatch_get_main_queue())) {
        DLog(@"WARNING: VT100ScreenMutableState performSynchroDanceWithBlock called off main queue, dispatching");
        dispatch_async(dispatch_get_main_queue(), ^{
            [self performSynchroDanceWithBlock:block];
        });
        return;
    }
    DLog(@"begin");
...
- (void)loadConfigIfNeededFromDelegate:(id<VT100ScreenDelegate>)delegate {
    DLog(@"begin");
    // BUG-f1210: Replace assert with warning - loading config outside joined block should warn not crash
    if (!VT100ScreenMutableState.performingJoinedBlock) {
        DLog(@"WARNING: VT100ScreenMutableState loadConfigIfNeededFromDelegate called outside of joined block");
    }
...
- (void)fixUpDeserializedIntervalTree:(iTermEventuallyConsistentIntervalTree *)intervalTree
    ...
    // BUG-f1211: Replace assert with warning - fixup outside joined block should warn not crash
    if (!VT100ScreenMutableState.performingJoinedBlock) {
        DLog(@"WARNING: VT100ScreenMutableState fixUpDeserializedIntervalTree called outside of joined block");
    }
    id<VT100RemoteHostReading> lastRemoteHost = nil;
    NSMutableDictionary<NSString *, id<CapturedOutputReading>> *markGuidToCapturedOutput = [NSMutableDictionary dictionaryWithCapacity:16];  // Captured outputs typically few
...
- (void)setHistory:(TmuxHistory *)history {
    ...
    const int n = [temp numLinesWithWidth:self.currentGrid.size.width];
    NSMutableArray<ScreenCharArray *> *wrappedLines = [NSMutableArray arrayWithCapacity:n];
...
- (void)setAltScreen:(NSArray<NSData *> *)lines {
    ...
    for (int i = 0; o < self.altGrid.size.height && i < MIN(lines.count, self.altGrid.size.height); i++) {
        ...
        do {
            ...
            // Copy attributes (fg/bg color) from last char to EOL position, then set EOL code.
            // This is intentional - EOL markers carry color info for null cells after line end.
            dest[self.altGrid.size.width] = dest[self.altGrid.size.width - 1];
...
- (void)setTmuxState:(NSDictionary *)state {
    ...
    // BUG-10066: Fixed copy-paste bug - was using kStateDictMouseButtonMode for ALL_MOTION.
    // Also fixed duplicate state restoration that occurred later in this method.
...
- (void)setFromFrame:(const screen_char_t *)s
    ...
    // BUG-f1212: Replace assert with guard - frame size mismatch should return not crash
    const int expectedLen = (info.width + 1) * info.height * sizeof(screen_char_t);
    if (len != expectedLen) {
        DLog(@"ERROR: VT100ScreenMutableState setFromFrame: frame size mismatch. len=%d expected=%d (width=%d height=%d)",
             len, expectedLen, info.width, info.height);
        return;
    }
...
- (id<PTYAnnotationReading>)triggerSession:(Trigger *)trigger
                     makeAnnotationInRange:(NSRange)rangeInScreenChars
                                      line:(long long)lineNumber {
    // BUG-f1213: Replace assert with guard - zero-length range should return nil not crash
    if (rangeInScreenChars.length == 0) {
        DLog(@"WARNING: VT100ScreenMutableState makeAnnotationInRange: zero-length range");
        return nil;
    }
...
- (NSInteger)numberOfBytesExecutedTotal {
    os_unfair_lock_lock(&_lock);
    NSInteger result = _numberOfBytesExecutedTotal;
    os_unfair_lock_unlock(&_lock);
    return result;
}
...
- (NSInteger)numberOfBytesExecutedExcludingInBandSignaling {
    os_unfair_lock_lock(&_lock);
    NSInteger result = _numberOfBytesExecutedExcludingInBandSignaling;
    os_unfair_lock_unlock(&_lock);
    return result;
}
...
- (VT100ScreenTokenExecutorUpdate *)fork {
    os_unfair_lock_lock(&_lock);
    VT100ScreenTokenExecutorUpdate *copy = [self copyWithZone_locked];
...
// Internal method called when lock is already held (from fork)
- (id)copyWithZone_locked {
    VT100ScreenTokenExecutorUpdate *copy = [[VT100ScreenTokenExecutorUpdate alloc] init];
    // Directly set ivars to avoid acquiring lock in called methods
    copy->_numberOfBytesExecutedTotal = _numberOfBytesExecutedTotal;
    copy->_numberOfBytesExecutedExcludingInBandSignaling = _numberOfBytesExecutedExcludingInBandSignaling;
    copy->_dirty = (_numberOfBytesExecutedTotal > 0 || _numberOfBytesExecutedExcludingInBandSignaling > 0);
    if (_inputHandled) {
        copy->_inputHandled = YES;
        copy->_dirty = YES;
    }
