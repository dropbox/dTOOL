...
+ (NSString *)dateDifferenceStringFromDate:(NSDate *)date options:(iTermDateDifferenceOptions)options {
...
+ (NSString *)highResolutionCompactRelativeTimeStringFromSeconds:(NSTimeInterval)seconds {
    ...
    // < 10 sec → "X.yyy sec"
    if (interval < 10) {
        return [NSString stringWithFormat:@"%@%0.3f sec", negative ? @"-" : @"+", interval];
    }

    // < 1 min → “X sec”
    if (interval < 60) {
        int sec = (int)interval;
        return [NSString stringWithFormat:@"%@%d sec", negative ? @"-" : @"+", sec];
    }

    // < 1 hr → “X min”
    if (interval < 3600) {
        int mins = (int)(interval / 60);
        return [NSString stringWithFormat:@"%@%d min", negative ? @"-" : @"+", mins];
    }
    ...
    NSMutableArray<NSString *> *comps = [NSMutableArray arrayWithCapacity:2]; // Max 2 time components
    for (int i = 0; i < 7; i++) {
        if (vals[i] > 0) {
            [comps addObject:[NSString stringWithFormat:@"%ld %s", (long)vals[i], units[i]]];
        }
        if (comps.count == 2)
            break;
    }
