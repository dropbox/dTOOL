...
// BUG-403: Cancel delayed selector in dealloc to prevent crash if split view is
// deallocated while forceRedraw is pending.
- (void)dealloc {
...
- (NSString *)description {
    ...
    [d appendFormat:@"<%@:%p frame:%@ splitter:%@ [", [self class], self, [NSValue valueWithRect:[self frame]],
                    [self isVertical] ? @"|" : @"--"];
...
- (void)mouseDown:(NSEvent *)theEvent {
    ...
    [[self delegate] splitView:self draggingDidEndOfSplit:clickedOnSplitterIndex pixels:changePx];
...
- (void)willRemoveSubview:(NSView *)subview {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // BUG-1039: Use strongSelf consistently instead of captured self
        [strongSelf.delegate splitViewDidChangeSubviews:strongSelf];
    });
...
- (NSArray<PTYSplitViewDividerInfo *> *)transitiveDividerLocationsVertical:(BOOL)vertical root:(PTYSplitView *)root {
    const NSPoint originPoint = [root convertRect:self.bounds fromView:self].origin;

    const NSInteger count = self.subviews.count;
    // Estimate capacity: at least one divider per subview pair, plus room for recursive subviews
    NSMutableArray<PTYSplitViewDividerInfo *> *result = [NSMutableArray arrayWithCapacity:count];
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        if ([subview isKindOfClass:[PTYSplitView class]]) {
            ...
            [result addObjectsFromArray:[childSplit transitiveDividerLocationsVertical:vertical root:root]];
        }
