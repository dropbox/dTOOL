...
+ (iTermColorPresetDictionary *)builtInColorPresets {
    NSString *plistFile = [[NSBundle bundleForClass:[self class]] pathForResource:@"ColorPresets" ofType:@"plist"];
    ...
    if (!aDict) {
        ...
        if (dup) {
            ...
            alert.informativeText = [NSString
                stringWithFormat:
                    @"The color preset “%@” is the same as the preset you're trying to add. Really add it?", dup];
        ...
        [self addColorPreset:[self presetNameFromFilename:filename] withColors:aDict];
        return YES;
    }
}
...
+ (void)deletePresetWithName:(NSString *)name {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:newCustom forKey:kCustomColorPresetsKey];
    [[NSNotificationCenter defaultCenter] postNotificationName:kRebuildColorPresetsMenuNotification object:nil];
}
...
- (iTermColorDictionary *)iterm_presetColorWithName:(NSString *)colorName {
    ...
    return [iTermProfilePreferences objectForKey:colorName inProfile:self];
}
...
+ (NSArray<NSString *> *)colorKeysWithModes:(BOOL)modes {
    NSArray *keys = @[
        KEY_ANSI_0_COLOR,  KEY_ANSI_1_COLOR,     KEY_ANSI_2_COLOR,        KEY_ANSI_3_COLOR,  KEY_ANSI_4_COLOR,
        KEY_ANSI_5_COLOR,  KEY_ANSI_6_COLOR,     KEY_ANSI_7_COLOR,        KEY_ANSI_8_COLOR,  KEY_ANSI_9_COLOR,
        KEY_ANSI_10_COLOR, KEY_ANSI_11_COLOR,    KEY_ANSI_12_COLOR,       KEY_ANSI_13_COLOR, KEY_ANSI_14_COLOR,
        KEY_ANSI_15_COLOR, KEY_FOREGROUND_COLOR, KEY_BACKGROUND_COLOR,    KEY_BOLD_COLOR,    KEY_LINK_COLOR,
        KEY_MATCH_COLOR,   KEY_SELECTION_COLOR,  KEY_SELECTED_TEXT_COLOR, KEY_CURSOR_COLOR,  KEY_CURSOR_TEXT_COLOR,
        KEY_TAB_COLOR,     KEY_UNDERLINE_COLOR,  KEY_CURSOR_GUIDE_COLOR,  KEY_BADGE_COLOR
    ];
...
- (BOOL)addColorPresetNamed:(NSString *)presetName
    ...
    // BUG-f913: Guard against nil GUID instead of crashing
    if (!guid) {
        ELog(@"iTermColorPresets: profile has no KEY_GUID");
        return NO;
    }
    ...
    for (NSString *colorName in [ProfileModel colorKeysWithModes:presetUsesModes]) {
        ...
        for (NSString *suffix in suffixesToSet) {
            ...
            if (colorDict) {
                ...
                [newDict removeObjectForKey:key]; // Can happen for tab color, match color, and underline color, which
                                                  // are optional
            }
        }
    }
