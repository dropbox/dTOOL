...
- (NSDictionary *)keyMapperDictionaryValue {
    ...
    return @{
        @"standard" : iTermStandardKeyMapperConfigurationDictionaryValue(_standard.configuration),
...
- (NSString *)escapeString:(NSString *)string {
    NSMutableArray<NSString *> *parts = [NSMutableArray arrayWithCapacity:string.length];
...
- (NSString *)mods:(NSEventModifierFlags)flags {
    NSMutableArray<NSString *> *parts = [NSMutableArray arrayWithCapacity:4]; // Modifier flag strings
...
- (BOOL)shouldModifyOtherKeysForEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    DLog(@"%@", event);
    if (modifiedEvent) {
        *modifiedEvent = event;
    }
    ...
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
...
- (BOOL)shouldModifyOtherKeysForRegularEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    DLog(@"Regular event");
    return NO;
}

- (BOOL)shouldModifyOtherKeysForNumberEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    DLog(@"Number event");
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
    ...
    if ((control && meta && shift) || (control && !meta && shift)) {
        switch (digit) {
            case 2:
            case 6:
                return NO;
    ...
    if ((control && meta) || (control && !meta && !shift)) {
        switch (digit) {
            case 2:
            case 3:
            case 4:
...
- (BOOL)shouldModifyOtherKeysForSymbolEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
    const NSEventModifierFlags flags = event.it_modifierFlags & mask;
    const BOOL control = !!(flags & NSEventModifierFlagControl);
    const BOOL meta = !!(flags & NSEventModifierFlagOption);
    const BOOL shift = !!(flags & NSEventModifierFlagShift);
    DLog(@"control=%@ meta=%@ shirt=%@ charactersIgnoringModifiers=%@", @(control), @(meta), @(shift),
...
- (BOOL)shouldModifyOtherKeysForFunctionEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    DLog(@"function event");
    return NO;
}

- (BOOL)shouldModifyOtherKeysForTabEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
    const NSEventModifierFlags flags = event.it_modifierFlags & mask;
    const BOOL control = !!(flags & NSEventModifierFlagControl);
...
- (BOOL)shouldModifyOtherKeysForEscEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
    const NSEventModifierFlags flags = event.it_modifierFlags & mask;
    const BOOL control = !!(flags & NSEventModifierFlagControl);
...
- (BOOL)shouldModifyOtherKeysForReturnEvent:(NSEvent *)event modifiedEvent:(out NSEvent **)modifiedEvent {
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagShift | NSEventModifierFlagControl);
    const NSEventModifierFlags flags = event.it_modifierFlags & mask;
    const BOOL control = !!(flags & NSEventModifierFlagControl);
