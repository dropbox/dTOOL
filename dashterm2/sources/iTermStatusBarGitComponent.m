...
- (instancetype)initWithConfiguration:(NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)configuration
    ...
    if (self) {
        const NSTimeInterval cadence =
            [self cadenceInDictionary:configuration[iTermStatusBarComponentConfigurationKeyKnobValues]];
        __weak __typeof(self) weakSelf = self;
        iTermGitPoller *gitPoller = [[iTermGitPoller alloc] initWithCadence:cadence
                                                                     update:^{
        ...
        _remoteObserver =
            [[iTermRemoteGitStateObserver alloc] initWithScope:scope
                                                         block:^{
...
// BUG-1492: Add dealloc to terminate log runner and prevent resource leaks
- (void)dealloc {
...
+ (NSDictionary *)statusBarComponentDefaultKnobs {
    ...
    knobs = [knobs dictionaryByMergingDictionary:@{
        iTermStatusBarGitComponentPollingIntervalKey : @(iTermStatusBarGitComponentDefaultCadence)
    }];
...
- (NSArray<NSAttributedString *> *)attributedStringVariants {
    ...
    }] sortedArrayUsingComparator:^NSComparisonResult(NSAttributedString *_Nonnull obj1,
                                                      NSAttributedString *_Nonnull obj2) {
...
- (NSArray<NSString *> *)variantsOfBranch:(NSString *)branch {
    NSIndexSet *dividers =
        [branch indicesOfCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"-_./+,"]];
    NSMutableArray<NSString *> *result = [[NSMutableArray alloc] initWithCapacity:dividers.count + 1];
...
- (nullable NSAttributedString *)attributedStringValueForBranch:(NSString *)branchString {
    ...
    NSAttributedString *upCount = self.currentState.pushArrow.integerValue > 0
                                      ? [self attributedStringWithString:self.currentState.pushArrow]
                                      : nil;
    NSAttributedString *downCount = self.currentState.pullArrow.integerValue > 0
                                        ? [self attributedStringWithString:self.currentState.pullArrow]
                                        : nil;
...
- (void)openMenuWithView:(NSView *)view {
    ...
    if (_session) {
        ...
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:[NSString stringWithFormat:@"Cancel %@", actionName]
                                                      action:@selector(killSession:)
                                               keyEquivalent:@""];
        item.target = self;
        [menu addItem:item];
    ...
    if (self.currentState.xcode.length > 0) {
        [iTermWarning showWarningWithTitle:[self.currentState.xcode stringByReplacingOccurrencesOfString:@"\t"
                                                                                              withString:@"\n"]
                                   actions:@[ @"OK" ]
                                 accessory:nil
    ...
    if (self.currentState.branch.length == 0) {
        NSMenu *menu = [[NSMenu alloc] init];
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:@"Show Debug Info"
    ...
    [self fetchRecentBranchesWithTimeout:0.5
                                   count:maxCount + 1
                              completion:^(NSArray<NSString *> *branches) {
                                  NSMenu *menu = [[NSMenu alloc] init];
                                  iTermGitMenuItemContext * (^addItem)(NSString *, SEL, BOOL) =
                                      ^iTermGitMenuItemContext *(NSString *title, SEL selector, BOOL enabled) {
                                  ...
                                  addItem([NSString stringWithFormat:@"Push origin %@", state.branch], @selector(push:),
                                          state.pushArrow.intValue > 0 || [state.pushArrow isEqualToString:@"error"]);
                                  addItem([NSString stringWithFormat:@"Pull origin %@", state.branch], @selector(pull:),
                                          !state.dirty);
                                  ...
                                  for (NSString *branch in [branches it_arrayByKeepingFirstN:maxCount]) {
                                      ...
                                      addItem([NSString stringWithFormat:@"Check out %@", branch], @selector(checkout:),
                                              YES)
                                          .userData = branch;
                                  }
                                  [menu addItem:[NSMenuItem separatorItem]];
...
- (void)runGitCommandWithArguments:(NSArray<NSString *> *)args
    ...
    iTermBufferedCommandRunner *runner =
        [[iTermBufferedCommandRunner alloc] initWithCommand:[self pathToGit]
                                              withArguments:args
...
- (void)showPopover {
    ...
    frame.size.width = [FontSizeEstimator fontSizeEstimatorForFont:_popoverVC.textView.font].size.width * 60 +
                       iTermTextPopoverViewControllerHorizontalMarginWidth * 2 + inset.width * 2;
    ...
    [_popoverVC.popover showRelativeToRect:rect ofView:view preferredEdge:NSRectEdgeMaxY];
}
...
- (void)debug {
    ...
    alert.informativeText = [NSString
        stringWithFormat:@"Directory: %@\n"
                         @"Polling cadence: %@ sec\n"
                         @"Polling enabled: %@\n"
                         @"Last polled %@ seconds ago\n"
                         @"Repo state: %@\n"
                         @"%@",
                         _gitPoller.currentDirectory, @(_gitPoller.cadence), _gitPoller.enabled ? @"Yes" : @"No",
                         @(-[_gitPoller lastPollTime].timeIntervalSinceNow), [_gitPoller.state prettyDescription],
...
- (void)runGitOnRemoteHost:(NSArray<NSString *> *)args {
    ...
    const iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:[NSString
                                 stringWithFormat:@"Looks like you're sshed somewhere. OK to send the command “%@”?",
                                                  command]
                     actions:@[ @"OK", @"Cancel" ]
                   accessory:nil
...
- (void)runGitInWindowWithArguments:(NSArray<NSString *> *)args
                                pwd:(NSString *)pwd
    ...
    __weak __typeof(self) weakSelf = self;
    iTermSingleUseWindowOptions options =
        (iTermSingleUseWindowOptionsCloseOnTermination | iTermSingleUseWindowOptionsShortLived);
    if (bury) {
    ...
    [[iTermController sharedInstance] openSingleUseWindowWithCommand:gitWrapper
        ...
        pwd:pwd
        options:options
        didMakeSession:^(PTYSession *newSession) {
