...
class StatusBarTriggersComponent: iTermStatusBarTextComponent {
    ...
    override func statusBarComponentIcon() -> NSImage {
        // BUG-f531: Return a fallback icon instead of crashing if image is missing
        guard let image = NSImage.it_cacheableImageNamed("StatusBarIconTriggers", for: Self.self) else {
            ...
            DLog("StatusBarTriggersComponent: Missing StatusBarIconTriggers image, using fallback")
            // Return a system fallback icon instead of crashing
            return NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "Triggers") ?? NSImage()
        }
        return image
    }
    ...
    private func openMenu(_ view: NSView) {
        ...
        // BUG-1628: Use guard to safely access first item instead of force unwrap
        guard let firstItem = menu.items.first else { return }
        menu.popUp(positioning: firstItem, at: NSPoint.zero, in: containingView)
    }
