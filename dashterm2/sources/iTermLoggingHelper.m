...
- (instancetype)initWithRawLogger:(id<iTermLogging>)rawLogger
    ...
    if (self) {
        ...
        _queue = dispatch_queue_create("com.dashterm.dashterm2.logging", DISPATCH_QUEUE_SERIAL);
...
// BUG-2751: Ensure file handle is closed when object is deallocated
- (void)dealloc {
    // Close file handle synchronously on its queue to prevent leak
    if (_fileHandle) {
...
asciicastMetadata:(iTermAsciicastMetadata *)asciicastMetadata
    ...
    if (_style == iTermLoggingStyleAsciicast) {
        // BUG-f1063: Replace assert with guard - missing metadata should be handled gracefully
        if (asciicastMetadata == nil) {
            DLog(@"BUG-f1063: asciicastMetadata is nil for asciicast style, disabling logging");
            _enabled = NO;
            return;
        }
