...
class KittyImageRendererTransientState: iTermMetalCellRendererTransientState {
    ...
    fileprivate func enumerateOperations(zRange: Range<Int>, closure: (Operation) -> ()) {
        ...
        // BUG-511: Guard against division by zero when scale is 0
        guard configuration.scale > 0 else { return }
        ...
        for run in runs {
            ...
            iTermKittyPlaceholderDrawInstructionsCreate(
                ...
                draw.placementID,
                0)
            // BUG-527: Guard against division by zero when scaledSize width/height is zero
            let scaledSize = draw.image.scaledSize
            guard scaledSize.width > 0 && scaledSize.height > 0 else {
                continue
            }
            let quadTextureFrame = instructions.sourceRect.flipped(in: scaledSize.height) / scaledSize
    ...
    private func textureFrame(_ draw: iTermKittyImageDraw) -> NSRect {
        // BUG-526: Guard against division by zero when scaledSize width/height is zero
        let scaledSize = draw.image.scaledSize
        guard scaledSize.width > 0 && scaledSize.height > 0 else {
            return NSRect.zero
        }
        return NSRect(x: draw.sourceFrame.minX / scaledSize.width,
                      y: draw.sourceFrame.minY / scaledSize.height,
                      width: draw.sourceFrame.width / scaledSize.width,
                      height: draw.sourceFrame.height / scaledSize.height).inverted
    }
...
class KittyImageRenderer: NSObject, iTermMetalCellRendererProtocol {
        ...
        tState.enumerateOperations(zRange: zRange) { op in
            ...
            withUnsafeTemporaryAllocation(of: iTermMetalBufferBinding.self, capacity: 1) { vertexBindings in
                vertexBindings[0] = iTermMetalBufferBindingMake(
                    ...
                    op.quad)
                withUnsafeTemporaryAllocation(of: iTermMetalTextureBinding.self, capacity: 1) { textureBindings in
                    textureBindings[0] = iTermMetalTextureBindingMake(
                        UInt(iTermTextureIndexPrimary.rawValue),
                        op.texture)
                    cellRenderer.draw(with: transientState,
                                      ...
                                      textureBindings: textureBindings.baseAddress,
                                      textureBindingCount: UInt(textureBindings.count))
                }
            }
        }
    }
    ...
    func createTransientState(forCellConfiguration configuration: iTermCellRenderConfiguration, commandBuffer: any MTLCommandBuffer) -> iTermMetalRendererTransientState? {
        // BUG-1750: Use guard with as? instead of as! force cast
        guard let transientState = cellRenderer.createTransientState(forCellConfiguration: configuration,
                                                                     commandBuffer: commandBuffer) as? KittyImageRendererTransientState else {
            return nil
        }
