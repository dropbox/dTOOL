...
class PromptStateMachine: NSObject {
    ...
    private func handleFinalTermB() {
        ...
        case .disabled:
            break
        case .receivingPrompt:
            // BUG-978: Previously just broke expecting didCapturePrompt to be called,
            // but if prompt capture fails, the state would remain stuck indefinitely.
            // Now we check for prompt and transition to ground to recover gracefully.
