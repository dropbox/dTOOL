...
+ (NSImage *)imageWithSFSymbol:(NSString *)sfSymbol
    ...
    if (backgroundless) {
        ...
        NSImage *resizedImage = [NSImage
            imageOfSize:imageSize
              drawBlock:^{
                  ...
                  NSRect centeredRect =
                      NSMakeRect((imageSize.width - scaledSize.width) / 2, (imageSize.height - scaledSize.height) / 2,
    ...
    NSImage *background = [NSImage
        imageOfSize:imageSize
          drawBlock:^{
              const CGFloat radiusFraction = 6;

              NSBezierPath *path = [NSBezierPath
                  bezierPathWithRoundedRect:NSInsetRect(NSMakeRect(0, 0, imageSize.width, imageSize.height), 0.5, 0.5)
    ...
    [builder addImage:[tintedImage imageTintedWithColor:darkBackground ? [NSColor whiteColor] : [NSColor blackColor]
                                                   size:[self fillingSizeFor:sfSymbolImage.size filling:insetSize]]];

    return [builder image];
}

+ (NSSize)fillingSizeFor:(NSSize)innerSize filling:(NSSize)outerSize {
...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Phase 1 optimization: Pre-size for typical indicator count
        _visibleIndicators = [[NSMutableDictionary alloc] initWithCapacity:8];
...
// BUG-402: Cancel delayed selector in dealloc to prevent crash if helper is
// deallocated while checkForFlashUpdate is pending.
- (void)dealloc {
...
- (void)setIndicator:(NSString *)identifier visible:(BOOL)visible darkBackground:(BOOL)darkBackground {
    if (visible && (!_visibleIndicators[identifier] || _visibleIndicators[identifier].dark != darkBackground)) {
        ...
        // BUG-f1043: Replace assert with guard - missing image should return early, not crash
        if (!indicator.image) {
            ELog(@"Indicator '%@' has no image - skipping", identifier);
            return;
        }
...
+ (NSArray *)flashingIndicatorIdentifiers {
    return @[ kiTermIndicatorBell, kiTermIndicatorWrapToTop, kiTermIndicatorWrapToBottom ];
}

+ (NSArray *)sequentialIndicatorIdentifiers {
    return @[
        kiTermIndicatorMaximized, kItermIndicatorBroadcastInput, kiTermIndicatorCoprocess, kiTermIndicatorAlert,
        kiTermIndicatorAllOutputSuppressed, kiTermIndicatorZoomedIn, kiTermIndicatorFilter, kiTermIndicatorCopyMode,
        kiTermIndicatorDebugLogging, kiTermIndicatorSecureKeyboardEntry_Forced, kiTermIndicatorSecureKeyboardEntry_User,
        kiTermIndicatorPinned, kiTermIndicatorAIChatLinked, kiTermIndicatorAIChatStreaming, kiTermIndicatorChannel
    ];
}

- (void)enumerateTopRightIndicatorsInFrame:(NSRect)frame
                                   andDraw:(BOOL)shouldDraw
    ...
    NSPoint point = NSMakePoint(frame.origin.x + frame.size.width, frame.origin.y + kIndicatorTopMargin);
...
- (NSString *)helpTextForIndicatorWithName:(NSString *)name sessionID:(NSString *)sessionID {
    ...
    NSDictionary<NSString *, NSString *> *messages = @{
        ...
        kiTermIndicatorCopyMode :
            @"In copy mode. See [documentation](https://dashterm.com/documentation-copymode.html) for details.",
        kiTermIndicatorDebugLogging : @"Debug logging is enabled.",
        kiTermIndicatorSecureKeyboardEntry_User :
            @"Secure Keyboard Entry is enabled. Select DashTerm2 > Secure Keyboard Entry to disable.\n[Disable this "
            @"indicator.](dashterm2:disable-secure-keyboard-entry-indicator)",
        ...
        kiTermIndicatorAIChatLinked :
            [NSString stringWithFormat:@"AI Chats can view or control this session.\n * [Unlink from AI "
                                       @"Chat](dashterm2:unlink-session-chat?s=%@&t=%@)\n * [Reveal AI "
                                       @"Chat](dashterm2:reveal-chat-for-session?s=%@&t=%@)",
                                       sessionID, [[NSWorkspace sharedWorkspace] it_newToken], sessionID,
                                       [[NSWorkspace sharedWorkspace] it_newToken]],
        kiTermIndicatorAIChatStreaming : [NSString
            stringWithFormat:@"Commands run in this session are automatically sent to an AI chat, along with their "
                             @"output. [Stop sending](dashterm2:disable-streaming-session-chat?s=%@&t=%@)",
                             sessionID, [[NSWorkspace sharedWorkspace] it_newToken]],
        kiTermIndicatorChannel :
            [NSString stringWithFormat:@"This command is running within another session.\n * [Return to Enclosing "
                                       @"Session](dashterm2:pop-channel?s=%@&t=%@)",
                                       sessionID, [[NSWorkspace sharedWorkspace] it_newToken]]
    };
...
- (NSString *)helpTextForIndicatorAt:(NSPoint)point sessionID:(NSString *)sessionID {
    ...
    [self enumerateTopRightIndicatorsInFrame:_lastFrame
                                     andDraw:NO
                                       block:^(NSString *name, NSImage *image, NSRect frame, BOOL dark) {
...
- (void)enumerateCenterIndicatorsInFrame:(NSRect)frame
                                   block:(void (^)(NSString *, NSImage *, NSRect, CGFloat, BOOL))block {
    ...
    for (NSString *identifier in centeredIdentifiers) {
        ...
        if (alpha > 0) {
            ...
            NSRect destinationRect =
                NSMakeRect(frame.origin.x + frame.size.width / 2 - size.width / 2,
                           frame.origin.y + frame.size.height / 2 - size.height / 2, size.width, size.height);
...
- (void)drawInFrame:(NSRect)frame {
    ...
    [self enumerateTopRightIndicatorsInFrame:frame
                                     andDraw:YES
                                       block:^(NSString *identifier, NSImage *image, NSRect frame, BOOL dark) {
    ...
    [self enumerateCenterIndicatorsInFrame:frame
                                     block:^(NSString *identifier, NSImage *image, NSRect destinationRect,
...
- (void)beginFlashingIndicator:(NSString *)identifier darkBackground:(BOOL)darkBackground {
    // BUG-f1044: Replace assert with guard - invalid identifier should return early, not crash
    if (![[iTermIndicatorsHelper flashingIndicatorIdentifiers] containsObject:identifier]) {
        ELog(@"beginFlashingIndicator called with invalid identifier: %@", identifier);
        return;
    }
