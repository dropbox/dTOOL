...
- (void)windowDidLoad {
    ...
    _openDashboardIfHiddenWindows.state =
        iTermUserDefaults.openTmuxDashboardIfHiddenWindows ? NSControlStateValueOn : NSControlStateValueOff;
}
...
- (void)linkWindowId:(int)windowId inSessionNumber:(int)sourceSessionNumber toSessionNumber:(int)targetSessionNumber {
...
- (void)moveWindowId:(int)windowId inSessionNumber:(int)sessionNumber toSessionNumber:(int)targetSessionNumber {
    [[self tmuxController] moveWindowId:windowId inSessionNumber:sessionNumber toSessionNumber:targetSessionNumber];
}
...
- (void)setWindows:(TSVDocument *)doc forSession:(NSNumber *)sessionNumber {
    ...
    if ([sessionNumber isEqual:[sessionsTable_ selectedSessionNumber]]) {
        // Pre-allocate based on number of tmux window records
        NSMutableArray *windows = [NSMutableArray arrayWithCapacity:doc.records.count];
        for (NSArray *record in doc.records) {
            [windows addObject:[NSMutableArray arrayWithObjects:[doc valueInRecord:record forField:@"window_name"],
                                                                [[doc valueInRecord:record
                                                                           forField:@"window_id"] substringFromIndex:1],
                                                                nil]];
        }
...
- (void)renameWindowWithId:(int)windowId toName:(NSString *)newName {
    ...
    [[self tmuxController] renameWindowWithId:windowId inSessionNumber:sessionNumber toName:newName];
    [self reloadWindows];
}
...
- (void)tmuxControllerWindowWasRenamed:(NSNotification *)notification {
    if ([[self window] isVisible]) {
        ...
        // BUG-f1426: Guard array bounds - notification object needs at least 3 elements
        // GitLab #11433: Must also verify the notification is from our TmuxController
        if (objects.count < 3) {
            DLog(@"BUG-f1426: Window renamed notification has insufficient elements (count=%lu)",
                 (unsigned long)objects.count);
            return;
        }
        // GitLab #11433: Verify the notification is from the currently selected TmuxController
        // Window IDs are not globally unique - different tmux sessions can have same window IDs
        TmuxController *notificationController = objects[2];
        if (notificationController != [self tmuxController]) {
            // Notification is from a different tmux session - ignore it
            return;
        }
...
- (TmuxController *)tmuxController {
    ...
    return [[TmuxControllerRegistry sharedInstance]
        controllerForClient:[self currentClient]]; // TODO: track the current client when multiples are supported
}
