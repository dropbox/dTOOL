...
@synthesize pooled = _pooled;

// Swift access to the @public code ivar
- (unsigned char)codeValue {
    return code;
}

- (void)setCodeValue:(unsigned char)value {
    code = value;
}
...
void iTermAsciiDataFree(AsciiData *asciiData) {
    ...
    if (asciiData->screenChars && asciiData->screenChars->buffer != asciiData->screenChars->staticBuffer) {
...
void iTermAsciiDataSet(AsciiData *asciiData, const char *bytes, int length, ScreenChars *screenChars) {
    // BUG-f1300: Replace assert with guard - free existing buffer if already set
    // This prevents memory leak if buffer is already allocated
    if (asciiData->buffer != NULL) {
        DLog(@"BUG-f1300: AsciiData buffer already set, freeing existing buffer");
        if (asciiData->buffer != asciiData->staticBuffer) {
            free(asciiData->buffer);
        }
        asciiData->buffer = NULL;
    }
...
- (void)resetForPoolReuse {
    // Reset type and flags
    type = VT100CC_NULL;
    savingData = NO;
    code = 0;

    // Release and nil retained objects
    [_string release];
    _string = nil;
    ...
    // Reset CSI - keep allocation for reuse, just zero it
    if (_csi) {
        memset(_csi, 0, sizeof(*_csi));
    }

    // Free ascii data buffers if they were dynamically allocated.
    // NOTE: iTermAsciiDataFree also frees screenChars->buffer if asciiData->screenChars
    // points to _screenChars, so we must NOT free _screenChars.buffer separately
    // to avoid a double-free crash.
    BOOL screenCharsFreedByAsciiData = (_asciiData.screenChars == &_screenChars);
    iTermAsciiDataFree(&_asciiData);
    memset(&_asciiData, 0, sizeof(_asciiData));

    // Free screen chars buffers if dynamically allocated AND not already freed above
    if (!screenCharsFreedByAsciiData && _screenChars.buffer && _screenChars.buffer != _screenChars.staticBuffer) {
        free(_screenChars.buffer);
    }
    memset(&_screenChars, 0, sizeof(_screenChars));

    // Destroy CRLFs vector if allocated
    if (_crlfs.elements) {
        CTVectorDestroy(&_crlfs);
        memset(&_crlfs, 0, sizeof(_crlfs));
    }
...
- (NSString *)description {
    // Token descriptions can vary in length; 256 is a reasonable starting capacity
...
- (NSString *)stringForAsciiData {
    ...
    return [[[NSString alloc] initWithBytes:_asciiData.buffer length:_asciiData.length
...
- (void)realizeCRLFsWithCapacity:(int)capacity {
    // BUG-f1301: Replace assert with guard - clear existing vector if already created
    // This prevents memory leak and double-allocation issues
    if (_crlfs.elements) {
        DLog(@"BUG-f1301: CRLFs vector already created, destroying existing vector");
        CTVectorDestroy(&_crlfs);
    }
