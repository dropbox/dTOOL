...
class FilteringUpdater: HexAddressFormatting {
    ...
    var progress: Double {
        // BUG-f983: Guard against division by zero when count is 0
    ...
    func update() -> Bool {
        ...
        case .Matched:
            ...
            // BUG-1651: Use as? instead of force cast for NSArray results
            guard let resultRanges = context.results as? [ResultRange] else { break }
            ...
            for (i, range) in positions.enumerated() {
                ...
                // BUG-1651: Use as? instead of force cast for expanded result range
                if !temporary, let expandedRange = expandedResultRanges[i] as? ResultRange {
                    acceptedLines.append(AbsResultRange(expandedRange,
        ...
        case .Searching, .NotFound:
            ...
            break
        @unknown default:
            // BUG-2897: Don't crash on unknown status - treat as no matches yet
            DLog("\(hexAddress): FilteringUpdater: unknown status \(context.status.rawValue), ignoring")
            break
        }
        ...
        case .Matched, .Searching:
            ...
            return true
        @unknown default:
            // BUG-2898: Don't crash on unknown status - conservatively return true to continue searching
            DLog("\(hexAddress): FilteringUpdater: unknown status \(context.status.rawValue), continuing search")
            return true
        }
    }
}
...
class AsyncFilter: NSObject {
    ...
    init(query: String,
        ...
        // BUG-1616: Use nil coalescing to safely convert Range instead of force unwrap
        let safeAbsLineRange: Range<Int64> = Range(absLineRange).map { Int64($0.lowerBound)..<Int64($0.upperBound) } ?? 0..<0
        updater = FilteringUpdater(query: query,
                                   lineBuffer: lineBufferCopy,
                                   count: numberOfLines,
                                   width: width,
                                   mode: mode,
                                   absLineRange: safeAbsLineRange,
                                   cumulativeOverflow: cumulativeOverflow)
    ...
    @objc func start() {
        ...
        // BUG-f505: Guard against multiple starts instead of crashing
        guard !started else {
            DLog("\(it_addressString): AsyncFilter already started, ignoring duplicate start")
            return
        }
        started = true
        progress?(0)
...
extension AsyncFilter: ContentSubscriber {
    ...
    func updateMetadata(selectedCommandRange: NSRange, cumulativeOverflow: Int64) {
        ...
        // BUG-1616: Use guard to safely convert Range instead of force unwrap
        guard let intRange = Range(selectedCommandRange) else { return }
        let swiftRange: Range<Int64> = Int64(intRange.lowerBound)..<Int64(intRange.upperBound)
        updater.updateMetadata(absLineRange: swiftRange,
                               cumulativeOverflow: cumulativeOverflow)
    }
}
