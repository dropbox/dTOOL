...
+ (NSView *)viewAtScreenCoordinate:(NSPoint)point {
    const NSRect mouseRect = {.origin = point, .size = NSZeroSize};
    NSArray<NSWindow *> *frontToBackWindows =
        [[iTermApplication sharedApplication] orderedWindowsPlusVisibleHotkeyPanels];
...
- (void)insertSubview:(NSView *)subview atIndex:(NSInteger)index {
    ...
    // BUG-12625: Guard against index out of bounds - clamp to valid range
    if (index < 0) {
        DLog(@"BUG-12625: insertSubview:atIndex: negative index %ld, using 0", (long)index);
        index = 0;
    }
    if (index >= (NSInteger)subviews.count) {
        DLog(@"BUG-12625: insertSubview:atIndex: index %ld >= count %lu, appending", (long)index, (unsigned long)subviews.count);
        [self addSubview:subview];
        return;
    }
    if (index == 0) {
...
- (void)swapSubview:(NSView *)subview1 withSubview:(NSView *)subview2 {
    ...
    // BUG-f966: Use guards instead of asserts - invalid indices should return early with warning
    if (index1 == NSNotFound) {
        DLog(@"BUG-f966: swapSubview:withSubview: subview1 not found in subviews - ignoring");
        return;
    }
    if (index2 == NSNotFound) {
        DLog(@"BUG-f966: swapSubview:withSubview: subview2 not found in subviews - ignoring");
        return;
    }
    if (index1 == index2) {
        DLog(@"BUG-f966: swapSubview:withSubview: same view passed for both - ignoring");
        return;
    }
...
- (CGRect)retinaRoundRect:(CGRect)rect {
    NSRect result = NSMakeRect([self retinaRound:NSMinX(rect)], [self retinaRound:NSMinY(rect)],
                               [self retinaRoundUp:NSWidth(rect)], [self retinaRoundUp:NSHeight(rect)]);
    return result;
}
...
- (void)it_showWarning:(NSString *)text rect:(NSRect)rect {
    iTermTextPopoverViewController *popoverVC =
        [[iTermTextPopoverViewController alloc] initWithNibName:@"iTermTextPopoverViewController"
    ...
    [popoverVC.popover showRelativeToRect:rect ofView:self preferredEdge:NSRectEdgeMaxY];
...
- (void)it_showWarningWithMarkdown:(NSString *)markdown {
    NSAttributedString *attributedString =
        [NSAttributedString attributedStringWithMarkdown:markdown
                                                    font:[NSFont systemFontOfSize:[NSFont systemFontSize]]
                                          paragraphStyle:[NSParagraphStyle defaultParagraphStyle]];
    [self it_showWarningWithAttributedString:attributedString rect:self.bounds];
}
