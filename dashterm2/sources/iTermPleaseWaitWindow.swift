...
class PleaseWaitWindow: NSWindow {
    ...
    init(owningWindow: NSWindow, message: String, image: NSImage) {
        ...
        if let contentView, let cancelButton, let imageView {
            let views: [NSView] = [cancelButton, progressIndicator, messageLabel, imageView]
            for view in views {
            ...
            // BUG-1644: Use nil coalescing instead of force unwrap for max()
            let contentViewWidth = (views.map { $0.bounds.width }.max() ?? 200) + sideMargin * 2
