...
// Note: vector_float4 (SIMD4<Float>) already conforms to Hashable and Equatable
// as of Swift 5.5+. The redundant conformances from BUG-942 have been removed.
...
class TerminalButtonRenderer: NSObject, iTermMetalCellRendererProtocol {
    ...
    // BUG-f550: Make initializer failable to handle Metal setup failures gracefully
    @objc(initWithDevice:)
    init?(device: MTLDevice) {
        guard let renderer = iTermMetalCellRenderer(
            device: device,
            ...
            piuElementSize: 0,
            transientStateClass: TerminalButtonRendererTransientState.self) else {
            DLog("ERROR: Failed to create TerminalButtonRenderer cell renderer")
            return nil
        }
    ...
    func draw(with frameData: iTermMetalFrameData,
        ...
        guard let renderEncoder = frameData.renderEncoder,
              let tState = transientState as? TerminalButtonRendererTransientState else {
            return
        }
    ...
    private func vertexBuffer(button: TerminalButtonRendererTransientState.Button,
        ...
        // BUG-1683: bindMemory is safe here because vertices is a non-empty array literal
        // (6 elements), so withUnsafeBytes always provides a valid pointer with non-nil baseAddress
    ...
    private func drawButton(_ button: TerminalButtonRendererTransientState.Button,
        ...
        withUnsafeTemporaryAllocation(of: iTermMetalBufferBinding.self, capacity: 1) { vertexBindings in
            guard vertexBindings.count >= 1 else { return }
            ...
            withUnsafeTemporaryAllocation(of: iTermMetalTextureBinding.self, capacity: 1) { textureBindings in
                guard textureBindings.count >= 1 else { return }
