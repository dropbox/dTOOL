...
class SpecialExceptionEntryEditorWindowController: NSWindowController, NSTextFieldDelegate, AffordanceDelegate {
    ...
    func ok(_ sender: AnyObject) {
        ...
        // BUG-1607: Use if-let to safely unwrap window instead of force unwrap
...
final class SpecialExceptionsWindowController: NSWindowController {
    ...
    private var nerdBundleInstalled: Bool {
        guard let nerdConfig = NerdFontInstaller.config else { return false }
        return config == nerdConfig
    }
    ...
    private func importString(_ content: String) {
        guard let newConfig = FontTable.Config(string: content) else {
            showError("This file is not well formed. Is it from a newer version of DashTerm2?")
            return
        }
        guard newConfig.version <= FontTable.Config.latestKnownVersion else {
            showError("This file is from a newer version of DashTerm2 and cannot be loaded.")
            return
        }
    ...
    @IBAction func installNerdFontBundle(_ sender: Any) {
        ...
        NerdFontInstaller.start(window: window) { [weak self] error in
            ...
            if let error {
                ...
                if let configString = NerdFontInstaller.configString {
                    self?.importString(configString)
                }
            }
...
extension SpecialExceptionsWindowController: CRUDTableViewControllerDelegate {
    ...
    func crudTextFieldDidChange(_ sender: CRUDTableViewController<SpecialExceptionsWindowController>,
        ...
        // BUG-f1024: Bounds check before accessing config.entries array
        // Row can be stale if entries were modified asynchronously
        guard row >= 0 && row < config.entries.count else {
            DLog("crudTextFieldDidChange: row \(row) out of bounds for count \(config.entries.count)")
            return
        }
        switch column {
        case 0:
        ...
        default:
            // BUG-f605: Handle unexpected column gracefully instead of crashing
            DLog("BUG-f605: Unexpected column index \(column) in crudTextDidChange")
        }
    }
    ...
    func crudDoubleClick(_ sender: CRUDTableViewController<SpecialExceptionsWindowController>, row: Int, column: Int) {
        ...
        if row == -1 {
            ...
            // BUG-f1026: Bounds check before accessing config.entries array
            guard row >= 0 && row < config.entries.count else {
                DLog("crudDoubleClick: row \(row) out of bounds for count \(config.entries.count)")
                return
            }
        ...
        // BUG-1607: Use guard to safely unwrap windows instead of force unwrap
        guard let sheet = editorWindowController.window, let parentWindow = window else {
            return
        }
        parentWindow.beginSheet(sheet) { [weak self] status in
            if status == .OK {
    ...
    private func acceptEdit(row: Int) {
        rowBeingEdited = -1
        guard let entry = editorWindowController?.entry else {
            DLog("acceptEdit: editorWindowController or entry is nil")
            return
        }
        // BUG-f1025: Bounds check before accessing config.entries array
        // Row can be stale if entries were modified while sheet was open
        guard row >= 0 && row < config.entries.count else {
            DLog("acceptEdit: row \(row) out of bounds for count \(config.entries.count)")
            return
        }
        config.entries[row] = entry
        crud.reload(row: row)
        updateEnabled()
    }
}
...
extension SpecialExceptionsWindowController: CRUDDataProvider {
    private struct Row: CRUDRow {
        ...
        func format(column: Int) -> CRUDFormatted {
            ...
            default:
                // BUG-f606: Handle unexpected column gracefully instead of crashing
                DLog("BUG-f606: Unexpected column index \(column) in format(column:)")
                return .string("")
            }
        }
    }
    ...
    subscript(index: Int) -> CRUDRow {
        get {
            // BUG-f1008: Add bounds check for subscript access to prevent crash
            guard index >= 0 && index < config.entries.count else {
                DLog("SpecialExceptionsDataProvider: Index \(index) out of bounds for count \(config.entries.count)")
                return Row(entry: FontTable.Entry(start: 0, count: 0, fontName: ""))  // Return empty entry as fallback
            }
    ...
    private func acceptAdd(completion: (Int) -> ()) {
        guard let entry = editorWindowController?.entry else {
            DLog("acceptAdd: editorWindowController or entry is nil")
            return
        }
