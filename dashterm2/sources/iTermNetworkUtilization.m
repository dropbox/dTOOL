...
- (void)addSubscriber:(id)subscriber block:(void (^)(double, double))block {
    [_publisher addSubscriber:subscriber
                        block:^(iTermNetworkUtilizationSample *_Nonnull payload) {
                            block(payload.bytesPerSecondRead, payload.bytesPerSecondWrite);
                        }];
...
- (void)update {
    ...
    if (_last.interfaces == nil || [_last.interfaces isEqual:current.interfaces]) {
        ...
        iTermNetworkUtilizationStats diff = {.upbytes = (current.stats.upbytes - last.stats.upbytes) / t,
        ...
        [_publisher publish:_publisher.historicalValues.lastObject
                                ?: [[iTermNetworkUtilizationSample alloc] initWithStats:zeroState]];
    }
    _last = current;
}
...
- (iTermNetworkUtilizationStatsAndInterfaces)currentStatsAndInterfaces {
    ...
    // Typical systems have ~8-16 network interfaces
    NSMutableSet<NSData *> *interfaceAddrs = [NSMutableSet setWithCapacity:16];
    char *lim = buf + len;
    char *next = NULL;
    for (next = buf; next < lim;) {
        ...
        if (interface->ifm_type == RTM_IFINFO2) {
            ...
            // See also: https://opensource.apple.com/source/Libinfo/Libinfo-542.40.3/gen.subproj/getifaddrs.c.auto.html
            // L282
            if (header->ifm_addrs & RTA_IFP) {
                struct sockaddr *sa = (struct sockaddr *)(header + 1);
                if (sa->sa_family == AF_LINK) {
                    struct sockaddr_dl *dl = (struct sockaddr_dl *)sa;
                    // Caches the socket address data (interface name + MAC address), in order to detect interface
                    // change
