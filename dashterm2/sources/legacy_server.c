...
static void ExecChild(int argc, char *const *argv) {
    ...
    // If we get here, execvp failed - child must exit immediately
    // Use _exit() not exit() to avoid running atexit handlers and corrupting
    // shared state with parent process
    int error = errno;
    // Note: printf/strerror are not async-signal-safe but we're past fork
    // so it's acceptable for error reporting (stderr may be corrupted anyway)
...
//
// So, before forking our DashTerm2 Daemon we need to move it from the 'per-session' namespace
...
// ----
//
// This way DashTerm2 daemon is in conformation with current MacOS security rules mechanics,
