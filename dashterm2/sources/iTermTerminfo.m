...
static os_unfair_lock sTerminfoLock = OS_UNFAIR_LOCK_INIT;
...
+ (instancetype)forTerm:(NSString *)term {
    ...
    dispatch_once(&onceToken, ^{
        terms = [[NSMutableDictionary alloc] initWithCapacity:8];
    });
    os_unfair_lock_lock(&sTerminfoLock);
    id instance = terms[term];
    if (!instance) {
        instance = [[iTermTerminfo alloc] initWithTerm:term];
        terms[term] = instance;
    }
    os_unfair_lock_unlock(&sTerminfoLock);
    return instance;
}
...
- (id)sync:(id (^NS_NOESCAPE)(void))block {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _queue = dispatch_queue_create("com.dashterm.dashterm2.terminfo", DISPATCH_QUEUE_SERIAL);
    });
...
- (NSDictionary<NSString *, id> *)computeDatabase {
    return [self sync:^id {
        ...
        NSMutableDictionary<NSString *, id> *result = [[NSMutableDictionary alloc] initWithCapacity:256];
...
- (NSString *_Nullable)stringForStringKey:(NSString *)key {
    ...
    // BUG-450: Replace assert(NO) with ELog - unexpected type should not crash
    // If we get an unexpected type, log it and return nil safely
    ELog(@"iTermTerminfo stringForStringKey got unexpected object type: %@", NSStringFromClass([obj class]));
    return nil;
}
