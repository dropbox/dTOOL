...
//
// BUG-276: Migration helper messages updated to DashTerm2
// BUG-416: Application Support directory message updated to DashTerm2
...
+ (void)migrateOpenAIKeyIfNeeded {
    ...
    const iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            @"Move OpenAI API key into the keychain? It is currently stored in User Defaults, which is not as secure."
...
+ (void)migrateApplicationSupportDirectoryIfNeeded {
    ...
    if ([fileManager itemIsDirectory:modern] && [fileManager itemIsDirectory:legacy]) {
        ...
        if (legacyEmpty) {
            // BUG-3955: Handle errors during migration to avoid inconsistent filesystem state
            NSError *error = nil;
            if (![fileManager removeItemAtPath:legacy error:&error]) {
                DLog(@"Failed to remove empty legacy directory %@: %@", legacy, error);
                return;
            }
            error = nil;
            if (![fileManager createSymbolicLinkAtPath:legacy withDestinationPath:modern error:&error]) {
                DLog(@"Failed to create symlink from %@ to %@: %@", legacy, modern, error);
            }
            return;
        }
        ...
        if (modernEmpty) {
            // BUG-3955: Handle errors during migration to avoid inconsistent filesystem state
            NSError *error = nil;
            if (![fileManager removeItemAtPath:modern error:&error]) {
                DLog(@"Failed to remove empty modern directory %@: %@", modern, error);
                return;
            }
            error = nil;
            if (![fileManager moveItemAtPath:legacy toPath:modern error:&error]) {
                DLog(@"Failed to move %@ to %@: %@", legacy, modern, error);
                return;
            }
            error = nil;
            if (![fileManager createSymbolicLinkAtPath:legacy withDestinationPath:modern error:&error]) {
        ...
        alert.informativeText = @"DashTerm2's Application Support directory has changed.\n\n"
                                @"Previously, both these directories were supported:\n~/Library/Application "
                                @"Support/iTerm\n~/Library/Application Support/DashTerm2.\n\n"
                                @"Now, only the DashTerm2 version is supported. But you have files in both so please "
                                @"move everything from iTerm to DashTerm2.";

        NSMutableArray<NSString *> *files = [NSMutableArray arrayWithCapacity:6]; // Migration files to list
        int over = 0;
        for (NSString *file in [fileManager enumeratorAtPath:legacy]) {
            if (files.count > 5) {
                over++;
        ...
        switch ([alert runModal]) {
            case NSAlertFirstButtonReturn:
                [[NSWorkspace sharedWorkspace] activateFileViewerSelectingURLs:@[
                    [NSURL fileURLWithPath:legacy], [NSURL fileURLWithPath:modern]
                ]];
...
+ (void)copyProfileToBookmark:(NSMutableDictionary *)dict {
    ...
    for (int i = 0; i < [xforms count]; ++i) {
        ...
        // BUG-2054: Guard against nil value to prevent data loss and crash
        // Setting nil in NSDictionary would crash; skipping preserves existing value
        if (value) {
...
+ (void)recursiveMigrateBookmarks:(NSDictionary *)node path:(NSArray *)path {
    ...
    if ([data objectForKey:KEY_COMMAND_LINE]) {
        ...
        if (dir && [dir length] > 0) {
            [temp setObject:kProfilePreferenceInitialDirectoryCustomValue forKey:KEY_CUSTOM_DIRECTORY];
        } else if (dir && [dir length] == 0) {
            [temp setObject:kProfilePreferenceInitialDirectoryRecycleValue forKey:KEY_CUSTOM_DIRECTORY];
        } else {
            [temp setObject:kProfilePreferenceInitialDirectoryHomeValue forKey:KEY_CUSTOM_DIRECTORY];
        }
...
static NSString *const iTermMigrationHelperRemoveDeprecatedKeyMappingsUserDefaultKey =
...
+ (void)setShouldRemoveDeprecatedKeyMappings:(iTermMigrationHelperShouldRemoveDeprecatedKeyMappings)value {
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    [ud setObject:@(value) forKey:iTermMigrationHelperRemoveDeprecatedKeyMappingsUserDefaultKey];
}
    ...
    switch ([self shouldRemoveDeprecatedKeyMappings]) {
        ...
        case iTermMigrationHelperShouldRemoveDeprecatedKeyMappingsDefault:
            ...
            if (![self anyProfileOrArrangementHasDeprecatedKeyMappings]) {
                [self setShouldRemoveDeprecatedKeyMappings:
...
+ (BOOL)askToRemoveDeprecatedKeyMappings:(NSString *)specialReason NS_AVAILABLE_MAC(15) {
    NSString *message = specialReason
                            ?: @"Some profiles have unnecessary key mappings which may interfere with window tiling "
                               @"shortcuts added in macOS Sequoia. These were in the default profile for many years "
                               @"but are no longer needed. Remove the key mappings? It shouldn’t break anything, and "
                               @"it won’t modify the on-disk copy of the dynamic profile.";
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection1: // lean more
            [[NSWorkspace sharedWorkspace]
                it_openURL:[NSURL URLWithString:@"https://gitlab.com/gnachman/iterm2/-/wikis/Deprecated-Key-Mappings"]
                    target:nil
...
+ (BOOL)removeDeprecatedKeyMappingsTestOnly:(BOOL)testOnly {
    const BOOL global = [self removeDeprecatedKeyMappingsInModel:[ProfileModel sharedInstance] testOnly:testOnly];
    const BOOL divorced = [self removeDeprecatedKeyMappingsInModel:[ProfileModel sessionsInstance] testOnly:testOnly];
...
+ (BOOL)removeDeprecatedKeyMappingsFromArrangementsTestOnly:(BOOL)testOnly {
    ...
    for (NSString *name in [WindowArrangements allNames]) {
        ...
        NSArray *modifiedArrangements = [windowArrangements mapWithBlock:^id _Nullable(NSDictionary *arrangement) {
            return [PseudoTerminal repairedArrangement:arrangement
                                        profileMutator:^NSDictionary *(NSDictionary *profile) {
                                            NSDictionary *keyMappings = profile[KEY_KEYBOARD_MAP];
                                            if (!keyMappings) {
                                                return profile;
                                            }
                                            NSDictionary *dict =
                                                [self keyMappingsByRemovingDeprecatedKeyMappingsFrom:keyMappings];
                                            if (dict) {
                                                changed = YES;
                                            }
...
+ (BOOL)removeDeprecatedKeyMappingsInModel:(ProfileModel *)model testOnly:(BOOL)testOnly {
...
+ (NSDictionary *)keyMappingsByRemovingDeprecatedKeyMappingsFrom:(NSDictionary *)input {
    ...
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"DeprecatedProfileKeyMappings"
                                                                      ofType:@"plist"];
...
+ (BOOL)removeDeprecatedKeyMappingsInProfile:(Profile *)profile model:(ProfileModel *)model testOnly:(BOOL)testOnly {
    NSDictionary *mappings = [self keyMappingsByRemovingDeprecatedKeyMappingsFrom:profile[KEY_KEYBOARD_MAP]];
    if (mappings && !testOnly) {
        [iTermProfilePreferences setObject:mappings forKey:KEY_KEYBOARD_MAP inProfile:profile model:model];
    }
    return mappings != nil;
}
...
+ (void)migrateAISettings {
    if ([iTermSecureUserDefaults.instance enableAI]) {
        ...
        if (originalModelName == nil) {
            ...
            [ud setObject:model.name forKey:kPreferenceKeyAIModel];
        } else {
            model = [[iTermAIModel alloc] initWithModelName:originalModelName url:originalURL legacy:originalLegacy];
        }
        if ([ud objectForKey:kPreferenceKeyAITermAPI] == nil) {
            if (originalLegacy) {
                [ud setInteger:iTermAIAPICompletions forKey:kPreferenceKeyAITermAPI];
            } else {
                [ud setInteger:model.api forKey:kPreferenceKeyAITermAPI];
            }
        }
        if (originalURL.length == 0) {
            [ud setObject:model.url forKey:kPreferenceKeyAITermURL];
        }
        if ([ud objectForKey:kPreferenceKeyAIFeatureFunctionCalling] == nil) {
            [ud setBool:model.functionCallingFeatureEnabled forKey:kPreferenceKeyAIFeatureFunctionCalling];
        }
        if ([ud objectForKey:kPreferenceKeyAIFeatureHostedWebSearch] == nil) {
            [ud setBool:model.hostedWebSearchFeatureEnabled forKey:kPreferenceKeyAIFeatureHostedWebSearch];
        }
        if ([ud objectForKey:kPreferenceKeyAIFeatureHostedFileSearch] == nil) {
            [ud setBool:model.hostedFileSearchFeatureEnabled forKey:kPreferenceKeyAIFeatureHostedFileSearch];
        }
        if ([ud objectForKey:kPreferenceKeyAIFeatureHostedCodeInterpreter] == nil) {
            [ud setBool:model.hostedCodeInterpreterFeatureEnabled forKey:kPreferenceKeyAIFeatureHostedCodeInterpreter];
        }
        if ([ud objectForKey:kPreferenceKeyAIFeatureStreamingResponses] == nil) {
            [ud setBool:model.streamingFeatureEnabled forKey:kPreferenceKeyAIFeatureStreamingResponses];
        }
        if ([ud objectForKey:kPreferenceKeyAITokenLimit] == nil) {
            [ud setInteger:model.contextWindowTokens forKey:kPreferenceKeyAITokenLimit];
        }
        if ([ud objectForKey:kPreferenceKeyAIResponseTokenLimit] == nil) {
            [ud setInteger:model.maxResponseTokens forKey:kPreferenceKeyAIResponseTokenLimit];
        }
    }
}
