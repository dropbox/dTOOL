...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Typical number of concurrent command runners
        _commandRunners = [[NSMutableArray alloc] initWithCapacity:4];
    }
    return self;
}
...
- (NSString *)emacsClientInApplicationBundle:(NSBundle *)bundle {
    ...
    // Typically 1-4 bin directories in Emacs app bundle
    NSMutableArray<NSString *> *bindirs = [[NSMutableArray alloc] initWithCapacity:4];
...
- (void)launchVSCodeWithPath:(NSString *)path codium:(BOOL)codium {
    // BUG-f1224: Remove assert since we have a guard - nil path should return not crash
    if (!path) {
        DLog(@"WARNING: iTermSemanticHistoryController launchVSCodeWithPath: nil path");
        return;
    }
...
- (void)launchBobWithPath:(NSString *)path {
    // BUG-f1225: Remove assert since we have a guard - nil path should return not crash
    if (!path) {
        DLog(@"WARNING: iTermSemanticHistoryController launchBobWithPath: nil path");
        return;
    }
    NSString *bundlePath = nil;
...
- (void)openDocumentInNova:(NSString *)path line:(NSString *)line column:(NSString *)column {
    ...
    // Args: open, path, optionally -l with line:column
    NSMutableArray<NSString *> *args = [[NSMutableArray alloc] initWithCapacity:4];
    [args addObjectsFromArray:@[@"open", path]];
    if (line) {
        [args addObject:@"-l"];
        if (column) {
...
- (void)openDocumentInZed:(NSString *)path line:(NSString *)line column:(NSString *)column {
    ...
    // Parts: path, optionally line, optionally column
...
- (void)launchSublimeTextWithBundleIdentifier:(NSString *)bundleId path:(NSString *)path {
    // BUG-f1103: Remove assert() since we have a guard - don't crash in production
    if (!path) {
        ELog(@"launchSublimeTextWithBundleIdentifier: nil path");
        return;
    }
...
- (void)openDocumentInXcode:(NSString *)path line:(NSString *)line {
    // BUG-f1104: Remove assert() since we have a guard - don't crash in production
    if (!path) {
        ELog(@"openDocumentInXcode: nil path");
        return;
    }
...
- (void)openPath:(NSString *)cleanedUpPath
   ...
   substitutions:(NSDictionary *)substitutions
    ...
    if (isRawAction) {
        __weak __typeof(self) weakSelf = self;
        [_expressionEvaluator evaluateWithTimeout:self.evaluationTimeout sideEffectsAllowed:NO completion:^(iTermExpressionEvaluator * _Nonnull evaluator) {
