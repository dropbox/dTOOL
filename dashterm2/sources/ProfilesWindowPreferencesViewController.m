...
- (void)awakeFromNib {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reloadProfile)  // In superclass
                                               ...
                                               object:nil];
    // BUG-1143: Update screen dropdown when displays are connected/disconnected
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)openFilePicker {
    ...
    void (^completion)(NSInteger) = ^(NSInteger result) {
        if (result == NSModalResponseOK) {
            // BUG-f1044: Guard against empty URLs array
            if ([panel URLs].count == 0) {
                DLog(@"BUG-f1044: NSOpenPanel returned OK but URLs array is empty");
                return;
            }
