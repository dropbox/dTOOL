...
class ChatListModel: ChatListDataSource {
    ...
    // nonisolated(unsafe) because access is protected by instanceLock
    nonisolated(unsafe) private static var _instance: ChatListModel?
    ...
    // BUG-f611: Use safe element(at:) method to prevent out of bounds crash
    func chatListViewController(_ chatListViewController: ChatListViewController, chatAt index: Int) -> Chat? {
        return chatStorage.element(at: index)
    }
...
class TypingStatusModel {
    // nonisolated(unsafe) because TypingStatusModel is thread-safe (uses lock internally)
    nonisolated(unsafe) static let instance = TypingStatusModel()
