...
- (instancetype)init {
    self = [super init];
    if (self) {
        records_ = [[NSMutableArray alloc] initWithCapacity:64]; // TSV records
    }
    return self;
}
...
- (NSString *)valueInRecord:(NSArray *)record forField:(NSString *)fieldName {
    if (!map_) {
        map_ = [[[NSMutableDictionary alloc] initWithCapacity:self.columns.count] retain];
        for (int i = 0; i < self.columns.count; i++) {
            [map_ setObject:[NSNumber numberWithInt:i] forKey:[self.columns objectAtIndex:i]];
        }
    }
