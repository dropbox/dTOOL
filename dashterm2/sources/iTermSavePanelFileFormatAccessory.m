...
- (void)viewDidLoad {
    ...
    NSMutableArray<NSNumber *> *ys = [[NSMutableArray alloc] initWithCapacity:views.count];
    const CGFloat margin = 12;
    CGFloat y = margin;
    for (NSView *view in views) {
        [ys addObject:@(y)];
        if ((view == _fileFormat && self.showFileFormat) || (view == _timestampsButton && self.showTimestamps)) {
    ...
    _timestampsButton.state = [[NSUserDefaults standardUserDefaults] boolForKey:iTermSaveWithTimestampsUserDefaultsKey]
...
- (IBAction)timestampsDidChange:(id)sender {
    [[NSUserDefaults standardUserDefaults] setBool:_timestampsButton.state == NSControlStateValueOn
                                            forKey:iTermSaveWithTimestampsUserDefaultsKey];
}
@end
