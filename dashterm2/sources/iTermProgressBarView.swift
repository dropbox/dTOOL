...
class iTermProgressBarView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f820: Return nil instead of crashing for unused coder initializer
        DLog("iTermProgressBarView init(coder:) is not supported")
        return nil
    }

    // BUG-3203: Clear layer delegate to prevent dangling reference
    deinit {
        layer?.delegate = nil
    }

    // MARK: - Accessibility (BUG-3018)
    private func setupAccessibility() {
        setAccessibilityRole(.progressIndicator)
        setAccessibilityLabel(NSLocalizedString("Progress", comment: "Accessibility label for progress bar"))
    }

    private func updateAccessibilityValue() {
        switch mode {
        case .ground:
            setAccessibilityValue(NSLocalizedString("Stopped", comment: "Accessibility value when progress is stopped"))
        case .error:
            setAccessibilityValue(NSLocalizedString("Error", comment: "Accessibility value when progress shows error"))
        case .indeterminate:
            setAccessibilityValue(NSLocalizedString("In progress", comment: "Accessibility value for indeterminate progress"))
        case .determinate(_, let percentage):
    ...
    override func accessibilityValue() -> Any? {
        switch mode {
        case .ground:
            return NSLocalizedString("Stopped", comment: "Accessibility value when progress is stopped")
        case .error:
            return NSLocalizedString("Error", comment: "Accessibility value when progress shows error")
        case .indeterminate:
            return NSLocalizedString("In progress", comment: "Accessibility value for indeterminate progress")
        case .determinate(_, let percentage):
            return "\(percentage)%"
        }
    }
...
extension iTermProgressBarView {
    ...
    private func startIndeterminateAnimation() {
        ...
        // BUG-508: Guard against division by zero when width is zero
        let distance = max(1.0, width + gradientWidth)
