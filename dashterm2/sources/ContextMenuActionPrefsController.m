...
- (instancetype)initWithWindow:(NSWindow *)window {
    ...
    if (self) {
        _model = [[NSMutableArray alloc] initWithCapacity:16]; // Context menu actions
    }
    return self;
}
...
+ (NSString *)parameterValue:(NSString *)parameter encodedForAction:(ContextMenuActions)action {
    switch (action) {
...
+ (void)computeParameterForActionDict:(NSDictionary *)dict
    ...
    if (useInterpolation) {
        iTermSwiftyStringWithBackreferencesEvaluator *evaluator =
            [[iTermSwiftyStringWithBackreferencesEvaluator alloc] initWithExpression:parameter];
    ...
    for (int i = 0; i < 9; i++) {
        NSString *repl = @"";
        if (i < components.count) {
            repl = [self parameterValue:[components objectAtIndex:i] encodedForAction:action];
        }
...
- (void)awakeFromNib {
    ...
    // BUG-1192: Limit undo levels to avoid unbounded memory growth
...
- (IBAction)help:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace]
        it_openURL:[NSURL URLWithString:@"https://dashterm.com/documentation-smart-selection.html"]
            target:nil
...
- (void)updateHelpText {
    if (_useInterpolatedStringsButton.state == NSControlStateValueOn) {
        ...
        NSString *html =
            @"You can use captured strings from the Smart Selection's regular expression in the parameter. Use "
            @"\\(matches[i]) where i=0 for the entire match and i>0 for capture groups. For other values, see <a "
            @"href=\"https://dashterm.com/documentation-scripting-fundamentals.html\">Scripting Fundamentals</a> and "
            @"<a href=\"https://dashterm.com/documentation-variables.html#session-context\">Variables Reference "
            @"(Session Context)</a>.";
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
        paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
        _parameterInfoTextField.attributedStringValue =
        ...
        _parameterInfoTextField.stringValue =
            @"You can use captured strings from the Smart Selection's regular expression in the parameter. Use \\0 for "
            @"match, \\1…\\9 for match groups, \\d for directory, \\u for user, \\h for host.";
    }
}
...
- (IBAction)ok:(id)sender {
    [_delegate contextMenuActionsChanged:_model useInterpolatedStrings:self.useInterpolatedStrings];
}

- (IBAction)add:(id)sender {
    [self pushUndo];
    NSDictionary *defaultAction =
        [NSDictionary dictionaryWithObjectsAndKeys:@"", kTitleKey, [NSNumber numberWithInt:kOpenFileContextMenuAction],
                                                   kActionKey, nil];
    [_model addObject:defaultAction];
    [_tableView reloadData];
    [_tableView selectRowIndexes:[NSIndexSet indexSetWithIndex:_model.count - 1] byExtendingSelection:NO];
    [_title.window makeFirstResponder:_title];
}
...
- (void)setActions:(NSArray *)newActions browser:(BOOL)browser {
    if (!newActions) {
        newActions = [NSMutableArray arrayWithCapacity:8]; // Context menu actions
    }
    _browser = browser;
...
- (NSView *)tableView:(NSTableView *)tableView
    ...
    iTermTableCellViewWithTextField *view =
        [tableView newTableCellViewWithTextFieldUsingIdentifier:@"Smart Selection Action Tableview Entry"
...
- (NSAttributedString *)attributedStringForAction:(NSDictionary *)action {
    ...
    if (actionTag < 0 ||
        actionTag >= sizeof(gContextMenuActionDeclarations) / sizeof(*gContextMenuActionDeclarations)) {
        return nil;
    }
    ...
    return [[@[ nameAttributedString, actionAttributedString, parameterAttributedString ] arrayByRemovingNulls]
        it_componentsJoinedBySeparator:newline];
}
...
- (void)updateDetailView {
    ...
    // BUG-f997: Add bounds check - selectedRow may be out of sync with model during table updates
    NSInteger selectedRow = _tableView.selectedRow;
    if (selectedRow < 0 || selectedRow >= (NSInteger)_model.count) {
        DLog(@"BUG-f997: selectedRow %ld out of bounds for model count %lu", (long)selectedRow,
             (unsigned long)_model.count);
        return;
    }
    NSDictionary *item = _model[selectedRow];
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
    // BUG-f998: Add bounds check - row may be out of bounds during table reload race conditions
    if (row < 0 || row >= (NSInteger)_model.count) {
        DLog(@"BUG-f998: row %ld out of bounds for model count %lu", (long)row, (unsigned long)_model.count);
        return 24.0; // Return default row height
    }
