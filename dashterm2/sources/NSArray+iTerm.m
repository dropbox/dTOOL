...
+ (instancetype)mapIntegersFrom:(NSInteger)min
                             to:(NSInteger)noninclusiveUpperBound
                          block:(id (^NS_NOESCAPE)(NSInteger i))block {
    // BUG-f1352: Replace assert() with guard - invalid range should return empty array, not crash
    if (min > noninclusiveUpperBound) {
        ELog(@"mapIntegersFrom:to:block: min %ld > noninclusiveUpperBound %ld, returning empty array", (long)min,
             (long)noninclusiveUpperBound);
        return @[];
    }
...
+ (NSArray<NSNumber *> *)sequenceWithRange:(NSRange)range {
    NSMutableArray<NSNumber *> *temp = [NSMutableArray arrayWithCapacity:range.length];
...
+ (NSArray<NSString *> *)stringSequenceWithRange:(NSRange)range {
    NSMutableArray<NSString *> *temp = [NSMutableArray arrayWithCapacity:range.length];
    for (NSUInteger i = 0; i < range.length; i++) {
...
- (NSArray *)flatMapWithBlock:(NSArray * (^NS_NOESCAPE)(id anObject))block {
    NSMutableArray *temp = [NSMutableArray arrayWithCapacity:self.count * 2];
...
- (NSArray *)filteredArrayUsingBlock:(BOOL (^NS_NOESCAPE)(id))block {
    NSIndexSet *indexes =
        [self indexesOfObjectsPassingTest:^BOOL(id _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            return block(obj);
        }];
...
- (NSArray *)arrayByRemovingDuplicatesStably {
    NSMutableSet *members = [NSMutableSet setWithCapacity:self.count];
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:self.count];
    [self enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (NSString *)numbersAsHexStrings {
    // Each hex number is ~10 chars; allocate for all numbers
...
- (NSArray *)intersectArray:(NSArray *)other {
    NSMutableArray *intersection = [NSMutableArray arrayWithCapacity:MIN(self.count, other.count)];
    for (id obj in self) {
...
- (NSArray<iTermTuple *> *)zip:(NSArray *)other {
    NSMutableArray<iTermTuple *> *result = [NSMutableArray arrayWithCapacity:MIN(self.count, other.count)];
