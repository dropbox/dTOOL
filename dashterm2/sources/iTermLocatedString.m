...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Located strings are typically terminal line fragments (~128 chars)
        _string = [NSMutableString stringWithCapacity:128];
...
- (void)erase {
    // Maintain same capacity hint as init
    _string = [NSMutableString stringWithCapacity:128];
...
- (void)removeOcurrencesOfString:(NSString *)string {
    [_string reverseEnumerateSubstringsEqualTo:string
                                         block:^(NSRange range) {
                                             [_string replaceCharactersInRange:range withString:@""];
...
- (void)replaceCharactersInRange:(NSRange)range withString:(NSString *)replacement {
    ...
    [_gridCoords resizeRange:range to:NSMakeRange(range.location, replacement.length)];
}
...
- (void)appendString:(NSString *)string withAttributes:(NSDictionary *)attributes at:(VT100GridCoord)coord {
    [_attributedString iterm_appendString:string withAttributes:attributes];
    [self appendCoordsForString:string at:coord];
}

- (void)appendAttributedString:(NSAttributedString *)attributedString at:(VT100GridCoord)coord {
...
- (void)removeOcurrencesOfString:(NSString *)string {
    [_attributedString.string reverseEnumerateSubstringsEqualTo:string
                                                          block:^(NSRange range) {
