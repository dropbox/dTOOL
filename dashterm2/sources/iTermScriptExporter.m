...
+ (void)exportScriptAtURL:(NSURL *)fullURL
    ...
    if (!fullEnvironment) {
        ...
        // BUG-1730: Log directory creation errors
        NSError *mkdirErr = nil;
        [[NSFileManager defaultManager] createDirectoryAtPath:temp withIntermediateDirectories:YES attributes:nil error:&mkdirErr];
        if (mkdirErr) {
            DLog(@"BUG-1730: Error creating temp directory '%@': %@", temp, mkdirErr);
        }
...
+ (void)copySimpleScriptAtURL:(NSURL *)simpleScriptSourceURL
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    // BUG-1730: Log file operation errors
    ...
    [fileManager createDirectoryAtPath:sourceFolder
           withIntermediateDirectories:NO
                            attributes:nil
                                 error:&mkdirError];
    if (mkdirError) {
        DLog(@"BUG-1730: Error creating source folder '%@': %@", sourceFolder, mkdirError);
    }
    ...
    [fileManager copyItemAtURL:simpleScriptSourceURL
                         ...
                         error:&copyError];
    if (copyError) {
        DLog(@"BUG-1730: Error copying script to '%@': %@", destinationPy, copyError);
    }
    ...
    [fileManager setAttributes:@{ NSFilePosixPermissions: @(0744)}
                  ofItemAtPath:destinationPy.path
                         error:&attrError];
    if (attrError) {
        DLog(@"BUG-1730: Error setting attributes on '%@': %@", destinationPy.path, attrError);
    }
}
...
+ (BOOL)urlContainsScript:(NSURL *)url fullEnvironment:(out nullable BOOL *)fullEnvironment {
    ...
    if (isDirectory) {
        // Legal scripts must have a setup.cfg, dashterm2env, and appropriately named source folder and file.
        NSString *setupCfg = [url.path stringByAppendingPathComponent:@"setup.cfg"];
        NSString *dashterm2env = [url.path stringByAppendingPathComponent:@"dashterm2env"];
        ...
        if  ([fileManager fileExistsAtPath:setupCfg] &&
             [fileManager fileExistsAtPath:dashterm2env] &&
