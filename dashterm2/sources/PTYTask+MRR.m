...
int iTermForkAndExecToRunJobInServer(iTermForkState *forkState,
    ...
    if (forkState->connectionFd == -1) {
        // Failed to connect - likely ran out of file descriptors.
        // Signal the semaphore so the background thread can exit, then clean up.
        // The accept thread will fail gracefully when there's nothing to accept.
        iTermFileDescriptorServerLog("Failed to connect - out of file descriptors?");
        DLog(@"iTermFileDescriptorClientConnect failed, likely out of FDs");
        // Close the server socket so the accept thread won't block forever
    ...
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
        // This function should only be called when runJobsInServers is true.
        // If we get here with it false, something is wrong in the caller.
        DLog(@"ERROR: iTermForkAndExecToRunJobInServer called but runJobsInServers is NO");
        iTermFileDescriptorServerLog("ERROR: runJobsInServers is NO but we're in iTermForkAndExecToRunJobInServer");
        forkState->pid = -1;
        return -1;
    }
