...
extension PTYSession {
    ...
    func add(aiAnnotations annotations: [AITermAnnotation],
        ...
        for aiAnnotation in annotations {
            ...
            // BUG-f1001: Guard against division by zero when width is 0
            // This can happen during screen initialization or edge cases
            if width > 0 && end.x >= width {
                end.y += end.x / width
                end.x %= width
            }
            end.x += 1
    ...
    // BUG-1679: Return optional URL to avoid force unwrap crash
    ...
    func explainWithAI(selection: iTermSelection,
        ...
        guard let client = ChatClient.instance else {
            iTermWarning.show(withTitle: "AI Chat could not be opened. Verify you only have one instance of DashTerm2 running.",
                              actions: ["OK"],
                              accessory: nil,
                              identifier: nil,
...
private enum LocalFileValidationError: Error {
    case containsParentTraversal
    case escapesBaseDirectory
}

private extension LocalFileValidationError {
    var userFacingMessage: (text: String, detail: String) {
        switch self {
        case .containsParentTraversal:
            return ("Error: Invalid filename", "Filename cannot contain '..' path traversal sequences")
        case .escapesBaseDirectory:
            return ("Error: Invalid path", "Resolved path escapes base directory")
        }
    }
}

private extension URL {
    func hasDirectoryPrefix(_ ancestor: URL) -> Bool {
        let ancestorComponents = ancestor.standardizedFileURL.pathComponents
        let childComponents = standardizedFileURL.pathComponents
        guard ancestorComponents.count <= childComponents.count else {
            return false
        }
        for (ancestorComponent, childComponent) in zip(ancestorComponents, childComponents) {
            if ancestorComponent != childComponent {
                return false
            }
        }
        return true
...
extension PTYSession {
        ...
        let expectation = addExpectation("^-- FINISHED \(uuid)", after: nil, deadline: nil, willExpect: willExpect) { [weak self] _ in
            ...
            if var content = self?.contentAfter(start) {
                ...
                if ranges.count == 2,
                   ...
                   let secondRange = ranges.dropFirst().first {
                    // Normal case
                    content = String(content[firstRange.upperBound..<secondRange.lowerBound])
    ...
    /// RC-040: Log completion errors since they can't be propagated from async closure.
    ...
    private func createRemoteFile(_ createFile: RemoteCommand.CreateFile,
        ...
        conductor.create(file: createFile.filename, content: createFile.content.lossyData) { error in
            do {
                ...
                DLog("createRemoteFile completion threw: \(error)")
            }
        }
    }
    ...
    private func createLocalFile(_ createFile: RemoteCommand.CreateFile,
                                 completion: @escaping (String, String) throws -> ()) rethrows {
        do {
            let baseDir = currentLocalWorkingDirectory ?? NSHomeDirectory()
            let destinationURL = try sanitizedLocalFileURL(forFilename: createFile.filename,
                                                          baseDirectory: baseDir)

            try createFile.content.write(to: destinationURL,
                                         atomically: false,
                                         encoding: .utf8)
            revealCreatedFile(at: destinationURL)
            try completion("Ok", "Created \(createFile.filename) and revealed in Finder.")
        } catch let error as LocalFileValidationError {
            let message = error.userFacingMessage
            try completion(message.text, message.detail)
        } catch {
            try completion("Error: \(error.localizedDescription)", "Failed to create \(createFile.filename): \(error.localizedDescription)")
        }
    }

    private func sanitizedLocalFileURL(forFilename filename: String,
                                       baseDirectory: String) throws -> URL {
        guard !filename.contains("..") else {
            throw LocalFileValidationError.containsParentTraversal
        }
        ...
        if filename.hasPrefix("/") {
            ...
            absolutePath = baseDirectory.appending(pathComponent: filename)
        }
        ...
        if !filename.hasPrefix("/") {
            ...
            if !resolvedFileURL.hasDirectoryPrefix(baseURL) {
                throw LocalFileValidationError.escapesBaseDirectory
            }
        }
    ...
    @objc func revealCreatedFile(at url: URL) {
    ...
    /// RC-040 ROOT CAUSE FIX: Handle JSON encoding errors instead of returning "[]".
    /// Also log completion errors since they can't be propagated from async closure.
        ...
        view.browserViewController?.findOnPage(query: args.query, maxResults: 20, contextLength: 100) { result in
            ...
            case .success(let results):
                do {
                    ...
                    DLog("JSON encoding failed for search results: \(encodingError)")
                    do {
                        try completion("JSON encoding error: \(encodingError.localizedDescription)", "Find on page failed")
                    } catch {
                        DLog("completion threw after encoding error: \(error)")
                    }
                } catch {
                    DLog("completion threw: \(error)")
                }
    ...
    private func loadURL(args: RemoteCommand.LoadURL, completion: @escaping (String, String) throws -> ()) rethrows {
        ...
        guard let scheme = url.scheme?.lowercased(), allowedSchemes.contains(scheme) else {
            try completion("Only http and https URLs are allowed", "Navigation failed")
            return
        }
        view.browserViewController?.loadURL(url) { error in
            do {
                ...
                DLog("loadURL completion threw: \(error)")
            }
        }
    }
        ...
        view.browserViewController?.doWebSearch(for: args.query) { [weak self] error in
            if let error {
                ...
                do {
                    ...
                    DLog("webSearch completion threw: \(error)")
                }
                return
            }
    ...
    private func readWebPage(args: RemoteCommand.ReadWebPage, completion: @escaping (String, String) throws -> ()) rethrows {
        view.browserViewController?.convertToMarkdown(skipChrome: false) { (result: Result<String, Error>) in
            do {
                ...
                DLog("readWebPage completion threw: \(error)")
            }
        }
    }
}
...
struct SubSelectionSerializationInfo {
    ...
    static func from(queryValue: String) -> SubSelectionSerializationInfo? {
        ...
        guard components.count == 4,
              let modeComponent = components.first,
              let startComponent = components.dropFirst().first,
              let endComponent = components.dropFirst(2).first,
              let rangeComponent = components.dropFirst(3).first else {
            DLog("Invalid queryValue format: expected 4 components, got \(components.count)")
            return nil
        }

        guard let mode = Int(modeComponent) else {
            DLog("Invalid mode in queryValue \(queryValue)")
            return nil
        }

        // Assumes LineBufferPosition can be re-created from its compact string.
        let start = LineBufferPosition.fromCompactStringValue(startComponent)
        let end = LineBufferPosition.fromCompactStringValue(endComponent)

        let windowedRange: Range<Int32>? = {
            if rangeComponent == "nil" { return nil }
            let parts = rangeComponent.split(separator: ":", omittingEmptySubsequences: false).map(String.init)
            guard parts.count == 2 else {
                DLog("Invalid range format: expected 2 parts, got \(parts.count)")
                return nil
            }
...
extension PTYSession {
    // NOTE: The channels branch of DashTerm2-Shell-Integration has the `it2run` server process you need to test this.
    @objc(addChannelClient:command:)
    func add(channelClient: ChannelClient, command: String) {
        // BUG-484: Replace it_assert with guard for proper crash prevention
        guard !isTmuxClient else {
            DLog("BUG-484: Attempted to add channel client to tmux session")
            return
        }

        channelClients.add(channelClient)
        // BUG-1769: Use guard let instead of force unwrap for session creation
        guard let session = newSession(forChannelID: channelClient.uid, command: command) else {
            DLog("Failed to create session for channel \(channelClient.uid)")
        ...
        let iobuffer = IOBuffer(fileDescriptor: channelClient.fd,
            ...
            if var data = channelClient.mux.receive() {
                data.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) in
                    ...
                    session?.threadedReadTask(baseAddress, length: Int32(ptr.count))
                }
            }
        ...
        guard let restorableSession = self.restorableSession else {
            return
        }
    ...
    func removeChannelClient(id: String) {
        // BUG-1587: Use as? to avoid crash on type mismatch
        let indexes = channelClients.indexesOfObjects { obj, _, _ in
            guard let client = obj as? ChannelClient else { return false }
    ...
    func removeChannelClients(for conductor: Conductor) {
        ...
        let indexes = channelClients.indexesOfObjects { obj, _, _ in
            guard let client = obj as? ChannelClient else { return false }
            return client.conductor === conductor
        }
        channelClients.removeObjects(at: indexes)
    }
...
extension PTYSession {
    func smartSelectAllVisible() {
        ...
        guard let visibleRange = Range(visibleAbsLines) else {
            DLog("Invalid visibleAbsLines range: \(visibleAbsLines)")
            return
        }
        var indexes = IndexSet(integersIn: visibleRange)
        findDriver?.closeViewAndDoTemporarySearch(for: regex,
            ...
            // BUG-1582: Use guard to safely convert Range instead of force unwrap
            guard let searchedRange = Range(linesSearched) else {
                return
            }
            indexes.remove(integersIn: searchedRange)
            if indexes.isEmpty {
...
extension PTYSession {
    ...
    func becomeBrowser(configuration: WKWebViewConfiguration,
        ...
        let guid: String
        guard let myGuid = profile[KEY_GUID] as? String else {
            DLog("Profile missing KEY_GUID")
            return false
        }
        // BUG-1689: Use guard instead of force unwrap for ProfileModel instances
        guard let sharedModel = ProfileModel.sharedInstance() else {
            DLog("ProfileModel.sharedInstance() returned nil")
            return false
        }
        if isDivorced {
            if let originalGuid = profile[KEY_ORIGINAL_GUID] as? String,
               sharedModel.bookmark(withGuid: originalGuid) != nil {
                model = sharedModel
                guid = originalGuid
            } else {
                guard let sessionsModel = ProfileModel.sessionsInstance() else {
                    DLog("ProfileModel.sessionsInstance() returned nil")
                    return false
                }
                model = sessionsModel
                guid = myGuid
