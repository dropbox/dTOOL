...
- (instancetype)init {
    ...
    if (self) {
        _count = 0;
        NSString *path;
        path = [[[NSFileManager defaultManager] applicationSupportDirectoryWithoutCreating]
            stringByAppendingPathComponent:@"CopyModeKeyBindings.dict"];
        if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
            path = [[NSBundle bundleForClass:[iTermCopyModeHandler class]] pathForResource:@"iTermCopyModeKeyBindings"
                                                                                    ofType:@"dict"];
        }
...
- (BOOL)shouldAutoEnterWithEventIgnoringPrefs:(NSEvent *)event {
    ...
    const NSEventModifierFlags masks[] = {
        NSEventModifierFlagShift, NSEventModifierFlagShift | NSEventModifierFlagOption,
    ...
    const NSEventModifierFlags mask = (NSEventModifierFlagCommand | NSEventModifierFlagShift |
...
- (void)handleAutoEnteringEvent:(NSEvent *)event {
    ...
    // BUG-f921: Guard against zero direction instead of crashing
    if (direction == 0) {
        ELog(@"iTermCopyModeHandler: handleAutoEnteringEvent called with zero direction");
        return;
    }
                       ...
                       completion:^(BOOL moved) {
                           ...
                           if (moved || (strongSelf->_state.selecting != wasSelecting)) {
                               if (strongSelf.enabled) {
                                   [strongSelf.delegate copyModeHandler:strongSelf
                                               revealCurrentLineInState:strongSelf->_state];
                               }
...
- (int)countAccruedByAction:(iTermCopyModeAction)action event:(NSEvent *)event {
    switch (action) {
        case iTermCopyModeActionAccrueCount:
            // BUG-3978: Check characters length before accessing index 0
...
- (void)performAction:(iTermCopyModeAction)action event:(NSEvent *)event completion:(void (^)(BOOL))completion {
    [_state
        performAsynchronously:^{
            [self performAction:action event:event];
        }
...
- (BOOL)performAction:(iTermCopyModeAction)action event:(NSEvent *)event {
    switch (action) {
        case iTermCopyModeActionAccrueCount:
            // BUG-13001: Replace assert(NO) with early return for AccrueCount
            // This action is handled specially in accrueCountIfPossible and should not reach here.
            // Return NO instead of crashing to allow graceful recovery.
            return NO;
        case iTermCopyModeActionPageDownHalfScreen:
...
- (iTermCopyModeAction)actionForEvent:(NSEvent *)event {
    BOOL pointless = NO;
    // Pre-allocate for typical case (1-2 extra events)
    NSMutableArray<NSEvent *> *extraEvents = [NSMutableArray arrayWithCapacity:2];
    NSString *name = nil;
    ...
    [[iTermNotificationController sharedInstance]
        postNotificationWithTitle:@"Copy Mode"
                           detail:@"Copy Mode lets you make a selection with the keyboard. Click to view the manual."
                              URL:[NSURL URLWithString:@"https://dashterm.com/documentation-copymode.html"]];
}
...
- (iTermSessionModeHandlerHandleEvent)handleEvent:(NSEvent *)event {
    ...
    [self setPrefix:nil];
    // BUG-3978: Check characters length before accessing index 0
    if (event.characters.length > 0 && [event.characters characterAtIndex:0] == 27) {
...
- (BOOL)wouldHandleEvent:(NSEvent *)event {
    ...
    // BUG-13002: Replace assert(NO) with DLog and early return for unexpected mode
    DLog(@"Unexpected session mode %lu in wouldHandleEvent", (unsigned long)_mode);
    return NO;
}
...
- (BOOL)handleEvent:(NSEvent *)event {
    switch (_mode) {
        ...
        case iTermSessionModeShortcutNavigation: {
            ...
            // BUG-13003: Replace assert(NO) with DLog and early return for unexpected handler result
            DLog(@"Unexpected handler result in handleEvent shortcut navigation");
            return NO;
        }
        case iTermSessionModeCopy: {
    ...
    // BUG-13004: Replace assert(NO) with DLog and early return for unexpected mode in handleEvent
    DLog(@"Unexpected session mode %lu in handleEvent", (unsigned long)_mode);
    return NO;
}
...
- (BOOL)shouldAutoEnterWithEvent:(NSEvent *)event {
    ...
    // BUG-13005: Replace assert(NO) with DLog and early return for unexpected mode
    DLog(@"Unexpected session mode %lu in shouldAutoEnterWithEvent", (unsigned long)_mode);
    return NO;
}
