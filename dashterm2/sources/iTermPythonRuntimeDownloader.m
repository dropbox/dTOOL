...
//
// BUG-258: Python runtime downloader strings updated to DashTerm2
// BUG-2, BUG-26-28: REPL banner and Python module branding updated
...
NSString *const iTermPythonRuntimeDownloaderDidInstallRuntimeNotification =
    @"iTermPythonRuntimeDownloaderDidInstallRuntimeNotification";
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _queue = dispatch_queue_create("com.dashterm.dashterm2.python-runtime", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (NSString *)threePartVersionForTwoPartVersion:(NSString *)twoPartVersion at:(NSString *)versionsPath {
    SUStandardVersionComparator *comparator = [[SUStandardVersionComparator alloc] init];
    return [[[iTermPythonRuntimeDownloader pythonVersionsAt:versionsPath]
...
- (NSString *)pathToStandardPyenvPythonWithPythonVersion:(NSString *)pythonVersion {
    return [self pyenvAt:[self pathToStandardPyenvWithVersion:pythonVersion creatingSymlinkIfNeeded:NO]
...
- (NSString *)pathToStandardPyenvWithVersion:(NSString *)pythonVersion creatingSymlinkIfNeeded:(BOOL)createSymlink {
    ...
    if (pythonVersion) {
        return
            [appsupport stringByAppendingPathComponent:[NSString stringWithFormat:@"dashterm2env-%@", pythonVersion]];
    } else {
        return [appsupport stringByAppendingPathComponent:@"dashterm2env"];
    }
}
...
- (NSURL *)pathToMetadataWithPythonVersion:(NSString *)pythonVersion {
    ...
    path = [path stringByAppendingPathComponent:@"dashterm2env-metadata.json"];
...
- (BOOL)shouldDownloadEnvironmentForPythonVersion:(NSString *)pythonVersion
    ...
    return ([self installedVersionWithPythonVersion:pythonVersion] <
            MAX(minimumEnvironmentVersion, iTermMinimumPythonEnvironmentVersion));
}
...
- (BOOL)isPythonRuntimeInstalled {
    ...
    return ![self shouldDownloadEnvironmentForPythonVersion:nil minimumEnvironmentVersion:0];
}
...
- (void)performPeriodicUpgradeCheck {
    // DashTerm2: Python runtime download is disabled
    NSString *downloadURL = [iTermAdvancedSettingsModel pythonRuntimeDownloadURL];
    if (downloadURL.length == 0) {
        return;
    }
    if (!_checkForUpdateRateLimit) {
        const NSTimeInterval day = 24 * 60 * 60;
        _checkForUpdateRateLimit =
            [[iTermPersistentRateLimitedUpdate alloc] initWithName:@"CheckForUpdatedPythonRuntime"
...
- (void)downloadOptionalComponentsIfNeededWithConfirmation:(BOOL)confirm
    ...
    DLog(@"confirm=%@ pythonVersion=%@ minimumEnvironmentVersion=%@ requiredToContinue=%@", @(confirm), pythonVersion,
         @(minimumEnvironmentVersion), @(requiredToContinue));
...
- (void)checkForNewerVersionThan:(int)installedVersion
    ...
    _downloadController = [[iTermOptionalComponentDownloadWindowController alloc]
        initWithWindowNibName:@"iTermOptionalComponentDownloadWindowController"];
    ...
    iTermManifestDownloadPhase *manifestPhase = [[iTermManifestDownloadPhase alloc]
                   initWithURL:url
        requestedPythonVersion:pythonVersion
              nextPhaseFactory:^iTermOptionalComponentDownloadPhase *(
                  iTermOptionalComponentDownloadPhase *currentPhase) {
                  ...
                  iTermDownloadableComponentInfo *const info =
                      [mphase infoGivenExistingFullComponent:latestFullComponent];
                  if (stillNeedsConfirmation) {
                      ...
                      if (requiredToContinue) {
                          alert.informativeText =
                              [NSString stringWithFormat:@"The Python Runtime is used by Python scripts that work with "
                                                         @"DashTerm2. It must be downloaded to complete the requested "
                                                         @"action. The download is about %@. OK to download it now?",
                                                         [NSString it_formatBytes:info.size]];
                      } else {
                          alert.informativeText =
                              [NSString stringWithFormat:@"The Python Runtime is used by Python scripts that work with "
                                                         @"DashTerm2. The download is about %@. OK to download it now?",
                                                         [NSString it_formatBytes:info.size]];
                      }
                  ...
                  return [[iTermPayloadDownloadPhase alloc]
                                 initWithURL:info.URL
                      ...
                      requestedPythonVersion:mphase.requestedPythonVersion
                            ...
                            nextPhaseFactory:^iTermOptionalComponentDownloadPhase *(
                                iTermOptionalComponentDownloadPhase *completedPhase) {
                                DLog(@"Download phase completed");
                                const BOOL shouldContinue = [weakSelf
                                    payloadDownloadPhaseDidComplete:(iTermPayloadDownloadPhase *)completedPhase
                                ...
                                return [[iTermInstallingPhase alloc] initWithURL:nil
                                                                           title:@"Download Finished"
                                                                nextPhaseFactory:nil];
                            }];
              }];
    _downloadController.completion = ^(iTermOptionalComponentDownloadPhase *lastPhase) {
        ...
        if (lastPhase == manifestPhase) {
            iTermPythonRuntimeDownloader *strongSelf = weakSelf;
            [strongSelf didStopCheckAfterReceivingManifestBecauseDeclined:declined raiseOnCompletion:raiseOnCompletion];
        }
    };
    if (!silent) {
        [_downloadController.window makeKeyAndOrderFront:nil];
...
- (void)didStopCheckAfterReceivingManifestBecauseDeclined:(BOOL)declined raiseOnCompletion:(BOOL)raiseOnCompletion {
    if (declined) {
...
- (BOOL)showDownloadFailedAlertWithError:(NSError *)error
    ...
    NSString *reason;
    if (error.code == -999 && [error.domain isEqualToString:@"com.dashterm.dashterm2"]) {
        if (!requiredToContinue) {
            [_downloadController close];
            return YES;
        }
    ...
    if (pythonVersion) {
        alert.informativeText = [NSString
            stringWithFormat:@"A DashTerm2 Python Runtime with Python version %@ must be downloaded to proceed.%@",
                             pythonVersion, reason];
    } else {
        alert.informativeText =
            [NSString stringWithFormat:@"A DashTerm2 Python Runtime must be downloaded to proceed.%@", reason];
    }
    [alert runModal];
    return NO;
}
...
- (BOOL)payloadDownloadPhaseDidComplete:(iTermPayloadDownloadPhase *)payloadPhase
    ...
    // BUG-f1161: Convert assert() to guard - nil group should log and return early, not crash
    if (group == nil) {
        ELog(@"BUG-f1161: payloadDownloadPhaseDidComplete called with nil group");
        return NO;
    }
    ...
    NSString *tempfile = [[NSWorkspace sharedWorkspace] temporaryFileNameWithPrefix:@"dashterm2-pyenv" suffix:@".zip"];
    ...
    NSString *pubkey = [NSString stringWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"rsa_pub"
                                                                                 withExtension:@"pem"]
                                                encoding:NSUTF8StringEncoding
                                                   error:nil];
    NSError *verifyError = [iTermSignatureVerifier validateFileURL:tempURL
                                              withEncodedSignature:payloadPhase.expectedSignature
                                                         publicKey:pubkey];
    if (verifyError) {
        ...
        alert.informativeText = [NSString
            stringWithFormat:@"The Python runtime's signature failed validation: %@", verifyError.localizedDescription];
        [alert runModal];
        _status = iTermPythonRuntimeDownloaderStatusError;
        ...
        void (^completion)(NSError *) = ^(NSError *error) {
            if (!error) {
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:iTermPythonRuntimeDownloaderDidInstallRuntimeNotification
                                  object:nil];
            ...
            alert.informativeText =
                error.localizedDescription ?: @"An error occurred while unzipping the downloaded python environment";
            [alert runModal];
    ...
    NSDirectoryEnumerator *enumerator =
        [NSFileManager.defaultManager enumeratorAtURL:[NSURL fileURLWithPath:path]
                           includingPropertiesForKeys:nil
                                         ...
                                         errorHandler:nil];
    // Pre-allocate for typical number of Python versions (usually 1-4)
    NSMutableArray<NSString *> *result = [NSMutableArray arrayWithCapacity:4];
...
+ (NSString *)latestPythonVersion {
    NSArray<NSString *> *components = @[ @"dashterm2env", @"versions" ];
...
- (void)installSitePackagesFromZip:(NSString *)zip
    ...
    DLog(@"zip=%@ runtimeVersion=%@ pythonVersions=%@ latestFullComponent=%@", zip, @(runtimeVersion), pythonVersions,
         @(latestFullComponent));
    ...
    NSURL *const tempDestination =
        [NSURL fileURLWithPath:[[[NSFileManager defaultManager] spacelessAppSupportCreatingLink]
    ...
    NSURL *overwritableURL = [tempDestination URLByAppendingPathComponent:@"dashterm2env"];
    NSError *error = nil;
    [[NSFileManager defaultManager] createDirectoryAtPath:tempDestination.path
    ...
    [self copyFullEnvironmentFrom:sourceURL
                       ...
                       completion:^(NSError *error) {
                           ...
                           NSString *searchFor1 =
                               [NSString stringWithFormat:@"/dashterm2env-%@/", @(latestFullComponent)];
                           NSString *replaceWith1 = [NSString stringWithFormat:@"/dashterm2env-%@/", @(runtimeVersion)];
                           NSString *searchFor2 =
                               [NSString stringWithFormat:@"/dashterm2env-%@\"", @(latestFullComponent)];
                           NSString *replaceWith2 =
                               [NSString stringWithFormat:@"/dashterm2env-%@\"", @(runtimeVersion)];
                           NSDictionary<NSString *, NSString *> *subs =
                               @{searchFor1 : replaceWith1, searchFor2 : replaceWith2};
                           [self performSubstitutions:subs inFilesUnderFolder:tempDestination];
                           [self unzip:[NSURL fileURLWithPath:zip]
                                       to:tempDestination
                               completion:^(NSError *error) {
                                   if (error) {
                                       DLog(@"Failed with %@", error);
...
- (void)copyFullEnvironmentFrom:(NSURL *)source to:(NSURL *)destination completion:(void (^)(NSError *))completion {
...
- (int)runtimeVersionFromFilename:(NSString *)filename {
    ...
    if (![scanner scanString:@"dashterm2env-" intoString:nil]) {
        return -1;
    }
    int version = -1;
...
- (void)installPythonEnvironmentFromZip:(NSString *)zip completion:(void (^)(NSError *))completion {
    ...
    if (runtimeVersion < 0) {
        completion([NSError
            errorWithDomain:@"com.dashterm.dashterm2"
                       code:-1
                   userInfo:@{
                       NSLocalizedDescriptionKey : @"Invalid filename. Expected `dashterm2env-<version>.zip`."
                   }]);
        return;
    }
    [self installPythonEnvironmentFromZip:zip runtimeVersion:runtimeVersion pythonVersions:nil completion:completion];
}

- (NSArray<NSString *> *)discoveredPythonVersionsAt:(NSString *)base {
    NSURL *url = [NSURL fileURLWithPath:[base stringByAppendingPathComponents:@[ @"dashterm2env", @"versions" ]]];
    NSDirectoryEnumerator *enumerator =
...
- (void)installPythonEnvironmentFromZip:(NSString *)zip
    ...
    NSURL *finalDestination =
        [NSURL fileURLWithPath:[self pathToStandardPyenvWithVersion:[@(runtimeVersion) stringValue]
                                            creatingSymlinkIfNeeded:YES]];
    NSURL *tempDestination = [NSURL fileURLWithPath:[[[NSFileManager defaultManager] spacelessAppSupportCreatingLink]
                                                        stringByAppendingPathComponent:[[NSUUID UUID] UUIDString]]];
    ...
    [self unzip:[NSURL fileURLWithPath:zip]
        ...
        completion:^(NSError *error) {
            ...
            if (!error) {
                NSDictionary<NSString *, NSString *> *subs = @{
                    @"__DASHTERM2_ENV__" : finalDestination.path,
                    @"__DASHTERM2_PYENV__" : [finalDestination.path stringByAppendingPathComponent:@"pyenv"]
                };
                DLog(@"perform subs");
                [self performSubstitutions:subs inFilesUnderFolder:tempDestination];
                ...
                [self finishInstallingRuntimeVersion:runtimeVersion
                                      pythonVersions:pythonVersions
                                                         ?: [self discoveredPythonVersionsAt:tempDestination.path]
                                     tempDestination:tempDestination
                                    finalDestination:finalDestination
                                          completion:completion];
            } else {
...
- (void)finishInstallingRuntimeVersion:(int)runtimeVersion
                      ...
                      finalDestination:(NSURL *)finalDestination
                            completion:(void (^)(NSError *))completion {
    DLog(@"runtimeVersion=%@ pythonVersions=%@ tempDestination=%@ finalDestination=%@", @(runtimeVersion),
         pythonVersions, tempDestination, finalDestination);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray<NSString *> *twoPartVersions = iTermConvertThreePartVersionNumbersToTwoPart(pythonVersions);
        NSArray<NSString *> *extendedPythonVersions =
            [NSSet setWithArray:[pythonVersions arrayByAddingObjectsFromArray:twoPartVersions]].allObjects;
        [self createDeepLinksTo:[tempDestination.path stringByAppendingPathComponent:@"dashterm2env"]
                 runtimeVersion:runtimeVersion
                    forVersions:extendedPythonVersions];
        [self createDeepLinkTo:[tempDestination.path stringByAppendingPathComponent:@"dashterm2env"]
                 pythonVersion:nil
                runtimeVersion:runtimeVersion];
        NSError *error = nil;
        DLog(@"Move %@ to %@", [tempDestination URLByAppendingPathComponent:@"dashterm2env"], finalDestination);
        [[NSFileManager defaultManager] moveItemAtURL:[tempDestination URLByAppendingPathComponent:@"dashterm2env"]
        ...
        for (int i = 1; i < runtimeVersion; i++) {
            ...
            NSError *error = nil;
            DLog(@"Remove %@", [self pathToStandardPyenvWithVersion:[@(i) stringValue] creatingSymlinkIfNeeded:NO]);
...
- (void)createDeepLinkTo:(NSString *)container
           pythonVersion:(NSString *)pythonVersion
          runtimeVersion:(int)runtimeVersion {
    ...
    if (runtimeVersion > existingVersion) {
        NSString *pathToVersionedEnvironment = [self pathToStandardPyenvWithVersion:pythonVersion
                                                            creatingSymlinkIfNeeded:NO];
        [[NSFileManager defaultManager] removeItemAtPath:pathToVersionedEnvironment error:nil];
        NSError *error = nil;
        DLog(@"Will link %@ to %@", container, pathToVersionedEnvironment);
        [[NSFileManager defaultManager] linkItemAtPath:container toPath:pathToVersionedEnvironment error:&error];
...
- (void)installPythonEnvironmentTo:(NSURL *)folder
    ...
    _busy++;
    [[iTermPythonRuntimeDownloader sharedInstance]
        installPythonEnvironmentTo:folder
                  eventualLocation:folder
                     pythonVersion:pythonVersion
                environmentVersion:[[iTermPythonRuntimeDownloader sharedInstance]
                                       installedVersionWithPythonVersion:pythonVersion]
...
+ (NSError *)errorWithCode:(iTermInstallPythonStatus)code reason:(NSString *)reason {
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.python-runtime-downloader"
                               code:code
...
- (void)installPythonEnvironmentTo:(NSURL *)container
                ...
                environmentVersion:(NSInteger)environmentVersion
                    ...
                    createSetupCfg:(BOOL)createSetupCfg
                        completion:(void (^)(NSError *))completion {
    DLog(@"Begin. container=%@ eventualLocation=%@ pythonVersion=%@ environmentVersion=%@ dependencies=%@ "
         @"createSetupCfg=%@",
         container, eventualLocation, pythonVersion, @(environmentVersion), dependencies, @(createSetupCfg));
    NSString *source = [self pathToStandardPyenvWithVersion:pythonVersion creatingSymlinkIfNeeded:NO];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        DLog(@"On bg queue");
        NSError *error = nil;
        NSString *destination = [container URLByAppendingPathComponent:@"dashterm2env"].path;
        BOOL ok;
        ...
        ok = [[NSFileManager defaultManager] linkItemAtPath:source toPath:destination error:&error];
        DLog(@"Done creating hard links. error=%@", error);
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!ok) {
                XLog(@"Failed to link %@ to %@: %@", source, destination, error);
                completion([iTermPythonRuntimeDownloader
                    errorWithCode:iTermInstallPythonStatusGeneralFailure
                           reason:[NSString stringWithFormat:@"Failed to link from %@ to %@: %@", source, destination,
                                                             error.localizedDescription]]);
                return;
            }

            // pip3 must use the python in this environment so it will install new dependencies to the right place.
            NSString *const pathToEnvironment = [container.path stringByAppendingPathComponent:@"dashterm2env"];
            NSString *const pip3 = [self pip3At:pathToEnvironment pythonVersion:pythonVersion];
            NSString *const pathToPython =
                [self executableNamed:@"python3"
                          atPyenvRoot:[eventualLocation.path stringByAppendingPathComponent:@"dashterm2env"]
                        pythonVersion:pythonVersion
                           searchPath:source];
            ...
            [self
                installDependencies:dependencies
                                 to:container
                      pythonVersion:pythonVersion
                         completion:^(NSArray<NSString *> *failures, NSArray<NSData *> *outputs) {
                             DLog(@"Dependency installation finished with these failures: %@", failures);
                             if (failures.count) {
                                 NSAlert *alert = [[NSAlert alloc] init];
                                 alert.messageText = @"Dependency Installation Failed";
                                 NSString *failureList = [[failures sortedArrayUsingSelector:@selector(compare:)]
                                     componentsJoinedByString:@", "];
                                 alert.informativeText = [NSString
                                     stringWithFormat:@"The following dependencies failed to install: %@", failureList];

                                 // Pre-allocate based on number of failures
                                 NSMutableArray<NSString *> *messages =
                                     [NSMutableArray arrayWithCapacity:failures.count];
                                 for (NSInteger i = 0; i < failures.count; i++) {
                                     NSData *output = outputs[i];
                                     NSString *stringOutput = [[NSString alloc] initWithData:output
                                                                                    encoding:NSUTF8StringEncoding];
                                     if (!stringOutput) {
                                 ...
                                 DLog(@"%@", messages);
                                 iTermDisclosableView *accessory = [[iTermDisclosableView alloc]
                                     initWithFrame:NSZeroRect
                                            prompt:@"Output"
                                           message:[messages componentsJoinedByString:@"\n\n"]];
                                 iTermAccessoryViewUnfucker *unfucker =
                                     [[iTermAccessoryViewUnfucker alloc] initWithView:accessory];
                                 accessory.frame = NSMakeRect(0, 0, accessory.intrinsicContentSize.width,
                                 ...
                                 [alert runModal];
                                 completion([iTermPythonRuntimeDownloader
                                     errorWithCode:iTermInstallPythonStatusDependencyFailed
                             ...
                             if (!pythonVersionToUse) {
                                 ...
                                 completion([iTermPythonRuntimeDownloader
                                     errorWithCode:iTermInstallPythonStatusGeneralFailure
                                            reason:@"Dependency installation failed: could not determine Python "
                                                   @"version. Please file a bug report at "
                                                   @"https://github.com/ayates_dbx/dashterm2/issues"]);
                                 return;
                             }
                             if (createSetupCfg) {
                                 [iTermSetupCfgParser
                                        writeSetupCfgToFile:[container.path stringByAppendingPathComponent:@"setup.cfg"]
                                                       name:container.path.lastPathComponent
                                               dependencies:dependencies
                                     ensureDashTerm2Present:YES
...
- (void)installDependencies:(NSArray<NSString *> *)dependencies
                         to:(NSURL *)container
              pythonVersion:(NSString *)pythonVersion
                 completion:(void (^)(NSArray<NSString *> *failures, NSArray<NSData *> *outputs))completion {
...
- (void)runPip3InContainer:(NSURL *)container
    ...
    NSString *pip3 = [self pip3At:[container.path stringByAppendingPathComponent:@"dashterm2env"]
                    pythonVersion:pythonVersion];
    if (!pip3) {
        DLog(@"pip3 not found");
        completion(NO, [[NSString stringWithFormat:@"pip3 not found for python version %@ in %@", pythonVersion,
                                                   container.path] dataUsingEncoding:NSUTF8StringEncoding]);
        return;
    }
    // pip3 output can be substantial; 4KB is a reasonable starting capacity
    NSMutableData *output = [NSMutableData dataWithCapacity:4096];
    ...
    runner.completion = ^(int status) {
        if (status != 0) {
            DLog(@"Runner %@ FAILED with %@", identifier,
...
- (void)performSubstitutions:(NSDictionary *)subs inFilesUnderFolder:(NSURL *)folderURL {
    ...
    NSMutableDictionary *dataSubs = [[NSMutableDictionary alloc] initWithCapacity:subs.count];
