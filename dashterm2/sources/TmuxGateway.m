...
// RC-006: Client-side sequence number for command correlation
static NSString *kCommandClientSequence = @"clientSequence";
...
@implementation TmuxGateway {
    ...
    // RC-006: Client-side sequence counter for command correlation.
    // Commands are assigned incrementing sequence numbers when queued.
    // This enables detection of out-of-order or orphaned server responses.
    NSInteger _nextCommandSequence;
}
...
- (instancetype)initWithDelegate:(id<TmuxGatewayDelegate>)delegate dcsID:(NSString *)dcsID {
    ...
    if (self) {
        delegate_ = delegate;
        commandQueue_ = [[NSMutableArray alloc] initWithCapacity:32];  // Tmux command queue
        strayMessages_ = [[NSMutableString alloc] init];
        _subscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];  // Tmux subscriptions
        _writeQueue = [[NSMutableString alloc] initWithCapacity:256];
        _dcsID = [dcsID copy];
        _sessionID = -1;
        _newline = @"\r";
    }
...
// BUG-3134: Cancel delayed selectors (enableWrites) to prevent crash if gateway is deallocated
// before the 1-second delay completes.
- (void)dealloc {
...
{
    NSMutableData *data = [[NSMutableData alloc] initWithCapacity:strlen(bytes)];
    unsigned char c;
    for (int i = 0; bytes[i]; i++) {
        ...
        if (c == '\\') {
            ...
            // BUG-7288: Check for null terminator before incrementing to prevent buffer overread
            c = 0;
            for (int j = 0; j < 3; j++) {
                // Check for end of string before incrementing
                if (bytes[i + 1] == '\0') {
                    c = '?';
                    break;
                }
                ...
                if (bytes[i] == '\r') {
                    ...
                    j--;  // Don't count \r towards the 3 octal digits
                    continue;
                }
...
// RC-006: Find command by server's commandId if it matches a known pending command.
// The server assigns sequential IDs to commands. If we have multiple commands in flight
// and the server responds out of order, we need to find the matching one.
- (NSUInteger)findCommandIndexForServerID:(NSString *)commandId {
    // In tmux protocol, command IDs are monotonically increasing integers assigned by the server.
    // However, since we don't know the server's starting point, we use FIFO as default.
    // The defense is: if we already have a server ID stored from a previous partial response,
    // we can match against it.

    // For now, since tmux guarantees FIFO processing on a single connection,
    // we use index 0. The key fix is in the defensive handling below.
    return 0;
}

- (void)beginHandlingNextResponseWithID:(NSString *)commandId {
    // RC-006 ROOT CAUSE FIX: Handle state machine synchronization issues
    //
    // The state machine can get out of sync in these scenarios:
    // 1. currentCommand_ is still set when a new %begin arrives (overlapping responses)
    // 2. commandQueue_ is empty when %begin arrives (orphaned response for timed-out command)
    // 3. Server sends responses out of expected order
    //
    // Fix: Properly handle each scenario instead of just logging and continuing.

    if (currentCommand_) {
        // Scenario 1: Overlapping responses - previous command didn't get %end/%error
        // This can happen if:
        // - Network issues caused %end to be lost
        // - Server crashed and restarted mid-response
        // - Protocol violation
        //
        // Root cause fix: Complete the previous command with an error before starting new one
        DLog(@"RC-006: currentCommand_ already set (seq=%@), completing with error before handling new response (id=%@)",
             currentCommand_[kCommandClientSequence], commandId);

        // Invoke callback with error to notify caller their command was interrupted
        const int flags = [currentCommand_[kCommandFlags] intValue];
        if (flags & kTmuxGatewayCommandShouldTolerateErrors) {
            // Silently complete commands that tolerate errors
            [self invokeCurrentCallbackWithError:YES];
        } else {
            // For critical commands, log but still invoke callback
            DLog(@"RC-006: Command '%@' interrupted without completion", currentCommand_[kCommandString]);
            [self invokeCurrentCallbackWithError:YES];
        }
        [self resetCurrentCommand];
    }

    if (commandQueue_.count == 0) {
        // Scenario 2: Orphaned response - command was removed from queue (timeout/cancel)
        // This can happen if:
        // - Client cancelled command but server already started processing
        // - Timeout removed command from queue before response arrived
        //
        // Root cause fix: Create a dummy command to consume this response
        // so the state machine stays in sync with the server.
        DLog(@"RC-006: Orphaned response (id=%@) - commandQueue_ is empty, creating placeholder", commandId);

        // Create placeholder command to consume the response
        currentCommand_ = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                           commandId, kCommandId,
                           @(-1), kCommandClientSequence,  // Negative sequence = orphaned
                           nil];
        currentCommandResponse_ = [[NSMutableString alloc] init];
        currentCommandData_ = [[NSMutableData alloc] init];
        return;
    ...
    // Normal path: Take the first command from queue (FIFO)
    NSUInteger index = [self findCommandIndexForServerID:commandId];
    if (index >= commandQueue_.count) {
        index = 0;  // Fallback to FIFO
    }

    currentCommand_ = commandQueue_[index];
    currentCommand_[kCommandId] = commandId;

    currentCommandResponse_ = [[NSMutableString alloc] init];
    currentCommandData_ = [[NSMutableData alloc] init];
    [commandQueue_ removeObjectAtIndex:index];

    DLog(@"RC-006: Matched response (id=%@) to command (seq=%@): %@",
         commandId, currentCommand_[kCommandClientSequence], currentCommand_[kCommandString]);
}
...
{
    NSMutableString *encoded = [[NSMutableString alloc] initWithCapacity:data.length * 5];
...
- (void)sendCodePoints:(NSArray<NSNumber *> *)codePoints toWindowPane:(int)windowPane {
    ...
    NSMutableArray *commands = [NSMutableArray arrayWithCapacity:codePoints.count];
...
- (void)enqueueCommandDict:(NSDictionary *)dict {
    ...
    // RC-006: Assign client-side sequence number for command correlation
    object[kCommandClientSequence] = @(_nextCommandSequence);
...
- (void)sendCommandList:(NSArray *)commandDicts initial:(BOOL)initial
{
    ...
    NSMutableString *cmd = [[NSMutableString alloc] initWithCapacity:commandDicts.count * 64];
    NSString *sep = @"";
...
- (void)flushWriteQueue {
    ...
    // Defensive check - don't write if not allowed to (upstream #12158)
    if (!_canWrite) {
        DLog(@"flushWriteQueue called but _canWrite is NO, deferring");
        return;
    }
