...
class VT100ConductorParser: NSObject, VT100DCSParserHook {
    ...
    private func parseNextOSC(_ context: UnsafeMutablePointer<iTermParserContext>,
        ...
        // BUG-f981: Use hardcoded ASCII values instead of force unwraps
        // This is safer (no crash possible) and faster (no runtime conversion)
        let esc: UInt8 = 27           // ESC character
        let closeBracket: UInt8 = 93  // ']' ASCII 0x5D
        let zero: UInt8 = 48          // '0' ASCII 0x30
        let nine: UInt8 = 57          // '9' ASCII 0x39
        let semicolon: UInt8 = 59     // ';' ASCII 0x3B
        let backslash: UInt8 = 92     // '\\' ASCII 0x5C
    ...
    private func parseNextLine(_ context: UnsafeMutablePointer<iTermParserContext>) -> Data? {
        ...
        // BUG-2778: Copy data BEFORE advancing the parser context.
        // The pointer from iTermParserPeekRawBytes becomes invalid after
        // iTermParserAdvanceMultiple is called. We must create the Data copy
    ...
    private func consumeUntilStartOfNextOSCOrEnd(_ context: UnsafeMutablePointer<iTermParserContext>) -> DataOrOSC {
        ...
        if count == 0 {
            ...
            // BUG-f659: Replace it_assert with guard - count validation should not crash
            if count <= 0 {
                DLog("BUG-f659: count should be positive when escape found, got \(count)")
                bytesToConsume = 1  // Consume at least one byte as fallback
            } else {
                // stuff before esc, consume up to it
                bytesToConsume = count
            }
        }
        // BUG-f660: Replace it_assert with guard - ensure at least one byte consumed
        if bytesToConsume <= 0 {
            DLog("BUG-f660: bytesToConsume should be positive, got \(bytesToConsume)")
            bytesToConsume = 1  // Must consume at least one byte to make progress
        }
        // BUG-1805: Use guard let instead of force unwrap for parser peek
        guard let rawBytes = iTermParserPeekRawBytes(context, bytesToConsume) else {
            return .data(Data())
        }
        let buffer = UnsafeBufferPointer(start: rawBytes,
    ...
    private func parseGround(_ context: UnsafeMutablePointer<iTermParserContext>,
        ...
        while iTermParserCanAdvance(context) {
            ...
            case .blocked:
                return .blocked
            case .notOSC:
                // BUG-f607: Handle unexpected .notOSC gracefully - should never happen with skipInitialGarbage:true
                // but if it does, log and continue rather than crash
                DLog("BUG-f607: Unexpected .notOSC in parseGround with skipInitialGarbage=true")
                continue
...
extension String {
    // BUG-1680: Use guard instead of force unwrap for utf8.first
    // Precondition: String must not be empty (returns 0 for empty strings as fallback)
...
private class SSHOutputTokenBuilder {
    ...
    init?(_ string: String) {
        self.string = string
        let parts = string.components(separatedBy: " ")
        // BUG-359: Guard against empty parts array crash
        // If string is empty or starts with space, parts[0] or parts[1] access crashes
        guard !parts.isEmpty, let flavor = Flavor(rawValue: parts[0]) else {
            return nil
        }
        self.flavor = flavor
        switch flavor {
        ...
        // BUG-359: The guards above ensure parts.count >= 2 for all valid flavors
...
extension UnsafeMutablePointer where Pointee == iTermParserContext {
    var dump: String {
        ...
        // BUG-1806: Use guard let instead of force unwrap for parser peek
        guard let bytes = iTermParserPeekRawBytes(self, length) else {
            return "count=\(length) (no data)"
        }
