...
+ (void)suppressNotifications:(void (^NS_NOESCAPE)(void))block {
    // BUG-f1039: Replace assert with guard - negative suppression count should reset to 0, not crash
    if (iTermKeyMappingsNotificationSupressionCount < 0) {
        ELog(@"iTermKeyMappingsNotificationSupressionCount was negative (%ld), resetting to 0",
             (long)iTermKeyMappingsNotificationSupressionCount);
        iTermKeyMappingsNotificationSupressionCount = 0;
    }
    iTermKeyMappingsNotificationSupressionCount += 1;
    block();
    iTermKeyMappingsNotificationSupressionCount -= 1;
    // BUG-f1040: Replace assert with guard - negative suppression count should reset to 0, not crash
    if (iTermKeyMappingsNotificationSupressionCount < 0) {
        ELog(@"iTermKeyMappingsNotificationSupressionCount went negative (%ld) after decrement, resetting to 0",
             (long)iTermKeyMappingsNotificationSupressionCount);
        iTermKeyMappingsNotificationSupressionCount = 0;
    }
}
...
+ (iTermKeyBindingAction *)actionForKeystroke:(iTermKeystroke *)keystroke keyMappings:(NSDictionary *)keyMappings {
    if (keyMappings) {
        iTermKeyBindingAction *action = [self localActionForKeystroke:keystroke keyMappings:keyMappings];
        if (action) {
            return action;
        }
    }
    ...
    return [self localActionForKeystroke:keystroke keyMappings:[self globalKeyMap]];
}
...
+ (iTermKeyBindingAction *)localActionForKeystroke:(iTermKeystroke *)keystroke keyMappings:(NSDictionary *)keyMappings {
...
+ (NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *)tuplesOfActionsInProfile:(Profile *)profile {
    ...
    // Pre-size based on keyboard map count
    NSMutableArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *result =
        [NSMutableArray arrayWithCapacity:keyboardMap.count];
    [keyboardMap enumerateKeysAndObjectsUsingBlock:^(id _Nonnull serialized, iTermKeyBindingAction *_Nonnull action,
                                                     BOOL *_Nonnull stop) {
        iTermKeystroke *key = [[iTermKeystroke alloc] initWithSerialized:serialized];
        if (!key) {
            return;
        }
        [result addObject:[iTermTuple tupleWithObject:key andObject:action]];
    }];
...
+ (void)setMappingAtIndex:(int)rowIndex
    ...
    // BUG-f1041: Replace assert(keyStroke) with guard - nil keyStroke should return early, not crash
    if (!keyStroke) {
        ELog(@"setMappingAtIndex called with nil keyStroke at rowIndex %d", rowIndex);
        return;
    }
    iTermKeystroke *originalKeystroke = nil;

    NSArray *allKeys = [[mutableKeyMapping allKeys] sortedArrayUsingSelector:@selector(compareSerializedKeystroke:)];
    if (!newMapping) {
        if (rowIndex >= 0 && rowIndex < [allKeys count]) {
...
+ (void)setMappingAtIndex:(int)rowIndex
    ...
    [self setMappingAtIndex:rowIndex forKeystroke:keystroke action:action createNew:newMapping inDictionary:keyMapping];
    profile[KEY_KEYBOARD_MAP] = keyMapping;
}

+ (void)removeKeystroke:(iTermKeystroke *)keystroke fromProfile:(MutableProfile *)profile {
...
+ (void)removeMappingAtIndex:(int)rowIndex fromProfile:(MutableProfile *)profile {
    profile[KEY_KEYBOARD_MAP] = [self removeMappingAtIndex:rowIndex inDictionary:profile[KEY_KEYBOARD_MAP]];
}
...
+ (NSDictionary *)globalKeyMap {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [observer observeKey:key
                       block:^{
                           gGlobalKeyMapping = [[NSUserDefaults standardUserDefaults] objectForKey:key]
                                                   ?: [iTermPresetKeyMappings defaultGlobalKeyMap];
                           if (iTermKeyMappingsNotificationSupressionCount == 0) {
                               [[NSNotificationCenter defaultCenter]
                                   postNotificationName:kKeyBindingsChangedNotification
                                                 object:nil
                                               userInfo:nil];
                           }
                       }];
...
+ (NSDictionary *)defaultGlobalKeyMap {
    NSString *plistFile = [[NSBundle bundleForClass:[self class]] pathForResource:@"DefaultGlobalKeyMap"
                                                                           ofType:@"plist"];
...
+ (void)setGlobalKeyMap:(NSDictionary *)src {
    ...
    // BUG-f1042: Replace assert with warning log - verification failure should log, not crash
    if (![self.globalKeyMap isEqual:src]) {
        ELog(@"setGlobalKeyMap: globalKeyMap does not match after setting. Expected: %@, Got: %@", src,
             self.globalKeyMap);
    }
}
...
+ (iTermKeystroke *)keystrokeForMappingReferencingProfileWithGuid:(NSString *)guid inProfile:(Profile *)profile {
...
+ (Profile *)removeKeyMappingsReferencingGuid:(NSString *)guid fromProfile:(Profile *)profile {
    if (profile) {
        ...
        while (YES) {
            ...
            NSArray *sortedKeystrokes =
                [iTermKeyMappings sortedKeystrokesForKeyMappingsInProfile:mutableProfile ?: profile];
            ...
            if (i != NSNotFound) {
                ...
                XLog(@"Profile with guid %@ has key mapping referencing guid %@ with keystroke %@ but I can't find it "
                     @"in sorted keys",
                     profile[KEY_GUID], guid, keystrokeToRemove);
                break;
            }
        }
    ...
    do {
        ...
        for (NSInteger i = 0; i < [mutableGlobalKeyMap count]; i++) {
            ...
            if (action.keyAction == KEY_ACTION_NEW_TAB_WITH_PROFILE ||
                ...
                mutableGlobalKeyMap = [self removeMappingAtIndex:i inDictionary:mutableGlobalKeyMap];
