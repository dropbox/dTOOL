...
NSString *const iTermAPIDidRegisterSessionTitleFunctionNotification =
    @"iTermAPIDidRegisterSessionTitleFunctionNotification";
NSString *const iTermAPIDidRegisterStatusBarComponentNotification =
    @"iTermAPIDidRegisterStatusBarComponentNotification";
...
NSString *const iTermAPIHelperErrorDomain = @"com.dashterm.dashterm2.api";
static NSString *const iTermAPIHelperDisableApplescriptAuthMagic = @"61DF88DC-3423-4823-B725-22570E01C027";
NSString *const iTermAPIHelperDidDetectChangeOfPythonAuthMethodNotification =
    @"iTermAPIHelperDidDetectChangeOfPythonAuthMethodNotification";

NSString *const iTermAPIHelperFunctionCallErrorUserInfoKeyConnection =
    @"iTermAPIHelperFunctionCallErrorUserInfoKeyConnection";
...
- (NSString *)invocationOfRegistrationRequest:(ITMRPCRegistrationRequest *)req {
    return [iTermAPIHelper invocationWithFullyQualifiedName:req.it_fullyQualifiedName defaults:req.defaultsArray];
}
...
- (BOOL)it_rpcRegistrationRequestValidWithError:(out NSError **)error {
    ...
    if ([self.name rangeOfCharacterFromSet:invalidIdentifierCharacterSet].location != NSNotFound) {
        if (error) {
            *error = newErrorWithReason([NSString
                stringWithFormat:@"Function name '%@' contains an invalid character. Must match /[A-Za-z0-9_]/",
                                 self.name]);
        }
        return NO;
    }
    NSMutableSet<NSString *> *args = [NSMutableSet setWithCapacity:self.argumentsArray.count];
    for (ITMRPCRegistrationRequest_RPCArgumentSignature *arg in self.argumentsArray) {
        ...
        if ([name rangeOfCharacterFromSet:invalidIdentifierCharacterSet].location != NSNotFound) {
            if (error) {
                *error = newErrorWithReason([NSString
                    stringWithFormat:@"Argument name '%@' contains an invalid character. Must match /[A-Za-z0-9_]/",
                                     name]);
            }
            return NO;
        }
        if ([args containsObject:name]) {
            if (error) {
                *error = newErrorWithReason([NSString
                    stringWithFormat:@"Two arguments share the name '%@'. Argument names must be unique.", name]);
            }
            return NO;
        }
        [args addObject:name];
...
- (NSString *)it_namespace {
    switch (self.roleSpecificAttributesOneOfCase) {
        ...
        case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_SessionTitleAttributes:
            return [NSString stringWithFormat:@"title.%@", [self.sessionTitleAttributes.uniqueIdentifier
                                                               stringByReplacingOccurrencesOfString:@"-"
                                                                                         withString:@"_"]];
        case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_StatusBarComponentAttributes:
            return [NSString stringWithFormat:@"statusbar.%@", [self.statusBarComponentAttributes.uniqueIdentifier
                                                                   stringByReplacingOccurrencesOfString:@"-"
                                                                                             withString:@"_"]];
        case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_ContextMenuAttributes:
            return [NSString stringWithFormat:@"contextMenu.%@", [self.contextMenuAttributes.uniqueIdentifier
                                                                     stringByReplacingOccurrencesOfString:@"-"
                                                                                               withString:@"_"]];
    }
}
...
- (NSString *)it_stringRepresentation {
    NSArray<NSString *> *argNames =
        [self.argumentsArray mapWithBlock:^id(ITMRPCRegistrationRequest_RPCArgumentSignature *anObject) {
...
- (NSSet<NSString *> *)it_allArgumentNames {
    return [NSSet
        setWithArray:[self.argumentsArray mapWithBlock:^id(ITMRPCRegistrationRequest_RPCArgumentSignature *anObject) {
            return anObject.name;
        }]];
}
...
- (NSString *)it_stringRepresentationWithNamespace:(NSString *)namespace {
    NSArray<NSString *> *argNames =
        [self.argumentsArray mapWithBlock:^id(ITMServerOriginatedRPC_RPCArgument *anObject) {
            return anObject.name;
        }];
    return iTermFunctionSignatureFromNamespaceAndNameAndArguments(namespace, self.name, argNames);
}
...
- (void)attachToOwner:(NSObject *)owner failure:(void (^)(void))failure {
    // BUG-f692: Replace assert with guard - already attached should be no-op not crash
    if (_associatedObjectKey) {
        DLog(@"ERROR: iTermAllObjectsSubscription attachToOwner: already attached");
        return;
    }
...
@implementation iTermAPIHelper {
    ...
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_appVariableSubscriptions;
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_tabVariableSubscriptions;
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_windowVariableSubscriptions;
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_sessionVariableSubscriptions;

    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_allSessionVariableSubscriptions; // Will have one entry per session
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *_allWindowVariableSubscriptions; // Will have one entry per window
    // signature -> ( connection, request )
    NSMutableDictionary<NSString *, iTermTuple<id, ITMNotificationRequest *> *>
        *_internalServerOriginatedRPCSubscriptions;
    NSMutableArray<iTermAllObjectsSubscription *>
        *_allSessionsSubscriptions; // Has one entry per "all" subscription for sessions
    NSMutableArray<iTermAllObjectsSubscription *>
        *_allWindowsSubscriptions; // Has one entry per "all" subscription for windows
...
// BUG-10387: Thread-safe singleton initialization using dispatch_once.
// Note: explicitUserAction is stored separately since it can only be set once.
static BOOL sAPIHelperExplicitUserAction = NO;

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sAPIHelperInstance = [[self alloc] initWithExplicitUserAction:sAPIHelperExplicitUserAction];
    });
    return sAPIHelperInstance;
}

+ (instancetype)sharedInstanceFromExplicitUserAction {
    // Record that user explicitly requested this, then create instance.
    // If instance already exists, this flag has no effect (which is correct behavior).
    sAPIHelperExplicitUserAction = YES;
...
+ (NSString *)noauthPath {
    return [[[NSFileManager defaultManager] applicationSupportDirectory]
        stringByAppendingPathComponent:@"disable-automation-auth"];
}

+ (NSString *)noauthMagic {
    return
        [NSString stringWithFormat:@"%@ %@", [[[self noauthPath] dataUsingEncoding:NSUTF8StringEncoding] it_hexEncoded],
...
+ (BOOL)requireApplescriptAuth {
    ...
    if (changed) {
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermAPIHelperDidDetectChangeOfPythonAuthMethodNotification
                          object:nil];
    }
    return result;
}
...
+ (iTermNoAuthStatus)noAuthStatus:(out NSString **)contentsPtr {
    ...
    NSDictionary<NSFileAttributeKey, id> *attributes =
        [[NSFileManager defaultManager] attributesOfItemAtPath:[self noauthPath] error:&error];
    if (!attributes || error) {
    ...
    NSString *actualContents = [NSString stringWithContentsOfFile:[self noauthPath]
...
+ (BOOL)internalRequireApplescriptAuth {
    ...
    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:
                          @"The location of your Application Support directory appears to have moved or its contents "
                          @"have changed unexpectedly. As a precaution, the authentication mechanism for Python API "
                          @"scripts for DashTerm2 has been reverted to always require Automation permission."
...
+ (BOOL)createNoAuthFile:(NSWindow *)window {
    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:@"Do you want to allow all apps running on this machine to use the Python "
                                           @"API?\n\nThis will disable the check for Automation permission. If you "
                                           @"agree, you’ll be prompted for administrator access to make the change."
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection2:
            ...
            [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/python-api-auth.html"]
                                               target:nil
                                                style:iTermOpenStyleTab
                                               window:window];
            return NO;
        default:
            // Unexpected selection - treat as cancel
            XLog(@"Unexpected warning selection: %d", (int)selection);
            return NO;
    }
}
...
+ (BOOL)removeNoAuthFile:(NSWindow *)window {
    ...
    const iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:[NSString
                                 stringWithFormat:@"Failed to remove the file “%@”: %@\n\nPlease remove this file "
                                                  @"manually to require Automation permission for the Python "
                                                  @"API.\n\nThe Python API has been disabled for your security.",
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection1:
            ...
            break;
        default:
            // Unexpected selection - log and treat as OK
            XLog(@"Unexpected warning selection in removeNoAuthFile: %d", (int)selection);
            break;
    }
    return NO;
}

+ (void)setRequireApplescriptAuth:(BOOL)requireApplescriptAuth window:(NSWindow *)window {
...
+ (BOOL)reallyCreateNoAuthFile:(NSWindow *)window {
    ...
    NSString *sourceCode = [NSString
        stringWithFormat:@"do shell script \"umask 077; TF=$(mktemp); printf '%%s' '%@' > \\\"$TF\\\" && chmod a+r "
                         @"\\\"$TF\\\" && mv \\\"$TF\\\" \\\"%@\\\" || rm -f \\\"$TF\\\"\" with prompt \"DashTerm2 "
                         @"needs to modify a secure setting.\" with administrator privileges",
                         [self noauthMagic], path];
    ...
    [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"The setting could not be changed: %@",
                                                                  dict[NSAppleScriptErrorBriefMessage]]
                               actions:@[ @"OK" ]
                             accessory:nil
...
+ (void)reportFunctionCallError:(NSError *)error
    ...
    [entry
         addOutput:[NSString stringWithFormat:
                                 @"An error occurred while running the function invocation “%@”:\n%@\n\nTraceback:\n%@",
                                 invocation, error.localizedDescription, traceback]
        completion:^{
        }];
...
- (NSInteger)removeServerOriginatedRPCSubscriptionsPasstingTest:
    (BOOL (^)(NSString *signature, iTermTuple<id, ITMNotificationRequest *> *tuple))block {
...
+ (BOOL)confirmShouldStartServerAndUpdateUserDefaultsForced:(BOOL)forced {
    ...
    warning.title = @"The Python API allows scripts you run to control DashTerm2 and access all its data.";
    static BOOL showing;
    if (showing) {
        // This can happen because the call to -runModal below starts a runloop and a delayed perform can then call
        // this.
...
- (instancetype)initWithExplicitUserAction:(BOOL)force {
    ...
    if (self) {
        ...
        _serverOriginatedRPCCompletionBlocks = [[NSMutableDictionary alloc] initWithCapacity:16];
        _outstandingRPCs = [[NSMutableDictionary alloc] initWithCapacity:8];
        _allSessionsSubscriptions = [[NSMutableArray alloc] initWithCapacity:4];
        _allWindowsSubscriptions = [[NSMutableArray alloc] initWithCapacity:4];
...
- (void)didCreateTerminalWindow:(NSNotification *)notification {
    ...
    for (iTermAllObjectsSubscription *sub in _allWindowsSubscriptions) {
        ...
        [term handleAPINotificationRequest:sub.request connectionKey:sub.connectionKey];
    }
}
...
- (void)sessionCreated:(NSNotification *)notification {
    ...
    for (iTermAllObjectsSubscription *sub in _allSessionsSubscriptions) {
        ...
        [session handleAPINotificationRequest:sub.request connectionKey:sub.connectionKey];
    }
    [_newSessionSubscriptions enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                                                  BOOL *_Nonnull stop) {
...
- (void)sessionDidTerminate:(NSNotification *)notification {
    PTYSession *session = notification.object;
    [_terminateSessionSubscriptions enumerateKeysAndObjectsUsingBlock:^(
                                        id _Nonnull key, ITMNotificationRequest *_Nonnull obj, BOOL *_Nonnull stop) {
        ITMNotification *notification = [[ITMNotification alloc] init];
...
- (void)handleLayoutChange {
    _layoutChanged = NO;
    [_layoutChangeSubscriptions enumerateKeysAndObjectsUsingBlock:^(
                                    id _Nonnull key, ITMNotificationRequest *_Nonnull obj, BOOL *_Nonnull stop) {
        ITMNotification *notification = [[ITMNotification alloc] init];
...
- (void)profileDidChange:(NSNotification *)notification {
    ...
    for (NSString *guid in guids) {
        [_profileChangeSubscriptions
            enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull request,
                                                BOOL *_Nonnull stop) {
                ITMNotification *notification = [[ITMNotification alloc] init];
...
- (void)handleFocusChange:(ITMFocusChangedNotification *)notif {
    void (^handle)(void) = ^{
        [self->_focusChangeSubscriptions
...
- (void)handleBroadcastChange {
    ITMBroadcastDomainsChangedNotification *broadcastSubNotification =
        [[ITMBroadcastDomainsChangedNotification alloc] init];
    ...
    [_broadcastDomainChangeSubscriptions
        enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                            BOOL *_Nonnull stop) {
            [self postAPINotification:notification toConnectionKey:key];
        }];
...
- (ITMServerOriginatedRPC *)serverOriginatedRPCWithName:(NSString *)fullyQualifiedName
    ...
    for (NSString *argumentName in [arguments.allKeys sortedArrayUsingSelector:@selector(compare:)]) {
        ...
        if ([NSNull castFrom:argumentValue]) {
            ...
            if (!jsonValue) {
                NSString *reason =
                    [NSString stringWithFormat:@"Could not JSON encode value “%@”", arguments[argumentName]];
                NSString *signature =
                    iTermFunctionSignatureFromNamespaceAndNameAndArguments(namespace, relativeName, arguments.allKeys);
...
- (void)dispatchRPCWithName:(NSString *)fullyQualifiedName
    ...
    NSError *error = nil;
    ITMServerOriginatedRPC *rpc = [self serverOriginatedRPCWithName:fullyQualifiedName
                                                          arguments:arguments
                                                              error:&error];
    if (error) {
...
- (NSString *)fullyQualifiedNameFromRelativeName:(NSString *)relativeName namespace:(NSString *)namespace {
    if (!namespace) {
        return relativeName;
    }
...
- (NSString *)signatureOfAnyRegisteredFunctionWithName:(NSString *)name namespace:(NSString *)namespace {
...
- (void)dispatchServerOriginatedRPC:(ITMServerOriginatedRPC *)rpc
    ...
    if (!connectionKey) {
        NSString *reason =
            [NSString stringWithFormat:@"No function registered for invocation “%@”. Ensure the script is running and "
                                       @"the function name and argument names are correct.",
                                       signature];
        NSString *bestMatch = [self signatureOfAnyRegisteredFunctionWithName:rpc.name namespace:namespace];
        if (bestMatch) {
            reason = [reason
                stringByAppendingFormat:
                    @" There is a similarly named function available with a different signature: %@", bestMatch];
        }
...
- (NSError *)rpcDispatchError:(NSString *)reason
    ...
    return [NSError errorWithDomain:iTermAPIHelperErrorDomain
                               code:unregistered ? iTermAPIHelperErrorCodeUnregisteredFunction
...
- (void)dispatchRPC:(ITMServerOriginatedRPC *)rpc
      ...
      connectionKey:(NSString *)connectionKey
         completion:(iTermServerOriginatedRPCCompletionBlock)completion {
    ITMNotification *notification = [[ITMNotification alloc] init];
    notification.serverOriginatedRpcNotification.requestId =
        [self nextServerOriginatedRPCRequestIDWithCompletion:completion];
    NSMutableSet *outstanding = _outstandingRPCs[connectionKey];
    if (!outstanding) {
        outstanding = [NSMutableSet setWithCapacity:8];
    ...
    __weak __typeof(self) weakSelf = self;
    const NSTimeInterval timeoutSeconds =
        handler.rpcRegistrationRequest.hasTimeout ? handler.rpcRegistrationRequest.timeout : 5;
...
- (void)checkForRPCTimeout:(NSString *)requestID connectionKey:(NSString *)connectionKey {
    ...
    completion(nil, [self rpcDispatchError:@"Timeout" detail:nil unregistered:NO connectionKey:connectionKey]);
}
...
- (NSDictionary<NSString *, NSArray<NSString *> *> *)registeredFunctionSignatureDictionary {
    NSMutableDictionary<NSString *, NSArray<NSString *> *> *result =
        [[NSMutableDictionary alloc] initWithCapacity:self.serverOriginatedRPCSubscriptions.count];
    for (NSString *stringSignature in self.serverOriginatedRPCSubscriptions.allKeys) {
        ...
        NSArray<NSString *> *args =
            [sig.argumentsArray mapWithBlock:^id(ITMRPCRegistrationRequest_RPCArgumentSignature *anObject) {
                return anObject.name;
            }];
...
+ (NSString *)invocationWithFullyQualifiedName:(NSString *)fullyQualifiedName
    ...
    NSArray<ITMRPCRegistrationRequest_RPCArgument *> *sortedDefaults = [defaultsArray
        sortedArrayUsingComparator:^NSComparisonResult(ITMRPCRegistrationRequest_RPCArgument *_Nonnull obj1,
...
+ (NSString *)nameOfScriptVendingStatusBarComponentWithUniqueIdentifier:(NSString *)uniqueID {
    return [[NSUserDefaults standardUserDefaults]
        objectForKey:[self userDefaultsKeyForNameOfScriptVendingStatusBarComponentWithID:uniqueID]];
}
...
+ (ITMRPCRegistrationRequest *)registrationRequestForStatusBarComponentWithUniqueIdentifier:
    (NSString *)uniqueIdentifier {
    return [[[self sharedInstance] statusBarComponentProviderRegistrationRequests]
        objectPassingTest:^BOOL(ITMRPCRegistrationRequest *request, NSUInteger index, BOOL *stop) {
...
- (void)logToConnectionHostingFunctionWithSignature:(NSString *)signatureString format:(NSString *)format, ... {
    va_list arguments;
...
- (void)logToConnectionHostingFunctionWithSignature:(NSString *)signatureString string:(NSString *)string {
    NSString *connectionKey = [self.serverOriginatedRPCSubscriptions[signatureString] firstObject];
    [self logToConnectionWithKey:connectionKey string:string];
}

- (void)logToConnectionWithKey:(NSString *)connectionKey format:(NSString *)format, ... {
    va_list arguments;
    va_start(arguments, format);
    NSString *string = [[NSString alloc] initWithFormat:format arguments:arguments];
    va_end(arguments);
...
- (NSMenuItem *)menuItemWithTitleParts:(NSArray<NSString *> *)titleParts
    ...
    for (NSMenuItem *item in [menu itemArray]) {
        if ([item.title isEqualToString:head]) {
            if ([item hasSubmenu]) {
                return [self menuItemWithTitleParts:remainingTitleParts inMenu:item.submenu];
...
- (NSMenuItem *)menuItemWithIdentifier:(NSString *)identifier inMenu:(NSMenu *)menu NS_AVAILABLE_MAC(10_12) {
    for (NSMenuItem *item in [menu itemArray]) {
        if ([item hasSubmenu]) {
...
- (BOOL)apiServerAuthorizeProcesses:(NSArray<NSNumber *> *)pids
    ...
    *displayName = advisoryName ? [@"≈" stringByAppendingString:advisoryName] : @"Unknown";
    ...
    NSString *message = @"Another process is trying to use the DashTerm2 API. The API allows a script to control "
                        @"DashTerm2 and view and modify its contents. Allow the connection?";

    if ([iTermAdvancedSettingsModel setCookie]) {
        message = [NSString stringWithFormat:@"%@\n\nAlthough you have chosen to allow connections automatically, this "
                                             @"script has not presented a valid cookie.",
                                             message];
    }
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection2:
            *reason = @"Denied by user";
            ...
            [[NSWorkspace sharedWorkspace]
                it_openURL:[NSURL URLWithString:@"https://dashterm.com/python-api-security-model"]
                    target:nil
                     style:iTermOpenStyleTab
                    window:nil];
            return NO;
        case kiTermWarningSelection3:
            if ([iTermWarning
                    showWarningWithTitle:@"New sessions will contain an environment variable that allows scripts to "
                                         @"run without confirmation. Are you sure you want to enable this?"
                                 actions:@[ @"OK", @"Cancel" ]
                               accessory:nil
...
- (PseudoTerminal *)windowControllerWithID:(NSString *)windowID {
    return [[[iTermController sharedInstance] terminals]
        objectPassingTest:^BOOL(PseudoTerminal *windowController, NSUInteger index, BOOL *stop) {
...
- (BOOL)rpcNotificationRequestIsValid:(ITMNotificationRequest *)request connectionKey:(NSString *)connectionKey {
    if (request.argumentsOneOfCase != ITMNotificationRequest_Arguments_OneOfCase_RpcRegistrationRequest) {
        [self logToConnectionWithKey:connectionKey
                              format:@"Expected an RPC registration request, but got:\n%@", request];
        return NO;
    }
    NSError *error = nil;
    if (![request.rpcRegistrationRequest it_rpcRegistrationRequestValidWithError:&error]) {
        [self logToConnectionWithKey:connectionKey
                              format:@"Malformed RPC function signature: %@", error.localizedDescription];
...
- (void)createSubscriptionDictionariesIfNeeded {
    if (!_newSessionSubscriptions) {
        _newSessionSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _terminateSessionSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _layoutChangeSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _focusChangeSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _internalServerOriginatedRPCSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:16];
        _broadcastDomainChangeSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _appVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:4];
        _tabVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _windowVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _sessionVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:16];
        _allSessionVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:16];
        _allWindowVariableSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _profileChangeSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:8];
    }
}

- (NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *> *)
    subscriptionsForVariableChangeScope:(ITMVariableScope)scope {
...
- (void)
    monitorVariableChangesForConnectionKey:(NSString *)connectionKey
                                identifier:(NSString *)identifier
                                   request:(ITMNotificationRequest *)request
                                     scope:(iTermVariableScope *)scope
                      mutableVariablesDict:
                          (NSMutableDictionary<
                              id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *> *)
                              dict {
    NSString *name = request.variableMonitorRequest.name;
    iTermVariableReference *ref = [[iTermVariableReference alloc] initWithPath:name vendor:scope];
    __weak __typeof(ref) weakRef = ref;
    __weak __typeof(self) weakSelf = self;
    ref.onChangeBlock = ^{
        ITMNotification *notification =
            [weakSelf variableChangeNotificationWithScope:request.variableMonitorRequest.scope
...
- (ITMNotificationResponse *)handleVariableChangeNotificationRequest:(ITMNotificationRequest *)request
    ...
    if ([identifier isEqualToString:@"all"]) {
        switch (request.variableMonitorRequest.scope) {
            case ITMVariableScope_Session:
                return [self handleSubscriptionRequestForAllSessionsFromConnectionKey:connectionKey request:request];
            case ITMVariableScope_Window:
                return [self handleSubscriptionRequestForAllWindowsFromConnectionKey:connectionKey request:request];
            case ITMVariableScope_App:
            case ITMVariableScope_Tab:
                // TODO
    ...
    NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
        *subscriptions = [self subscriptionsForVariableChangeScope:request.variableMonitorRequest.scope];
    NSMutableArray *array = subscriptions[connectionKey];
    const NSInteger index = [array
        indexOfObjectPassingTest:^BOOL(iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *_Nonnull tuple,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
            return ([NSObject object:tuple.firstObject.variableMonitorRequest
                     isEqualToObject:request.variableMonitorRequest]);
        }];
...
- (ITMNotificationResponse *)handleAPINotificationRequest:(ITMNotificationRequest *)request
    ...
    if (request.notificationType == ITMNotificationType_NotifyOnNewSession) {
        ...
        if (request.subscribe) {
            ...
            switch (request.rpcRegistrationRequest.role) {
                case ITMRPCRegistrationRequest_Role_SessionTitle:
                    [[NSNotificationCenter defaultCenter]
                        postNotificationName:iTermAPIDidRegisterSessionTitleFunctionNotification
        ...
        XLog(@"Unhandled notification type: %d", (int)request.notificationType);
        response.status = ITMNotificationResponse_Status_RequestMalformed;
        return response;
    }
    if (request.subscribe) {
...
- (BOOL)
    unsubscribeFromVariableChangeNotificationsForAllObjects:(NSArray<id<iTermSubscribable>> *)objects
                                                mutableSubs:
                                                    (NSMutableDictionary<
                                                        id, NSMutableArray<iTermTuple<ITMNotificationRequest *,
                                                                                      iTermVariableReference *> *> *> *)
                                                        subs
                                           forConnectionKey:(NSString *)connectionKey
                                                    request:(ITMNotificationRequest *)request {
    NSMutableArray *array = subs[connectionKey];
    NSIndexSet *indexes = [array indexesOfObjectsPassingTest:^BOOL(
                                     iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *_Nonnull tuple,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
        return ([NSObject object:tuple.firstObject.variableMonitorRequest
                 isEqualToObject:request.variableMonitorRequest]);
    }];
...
- (ITMNotificationResponse *)
    handleVariableSubscriptionRequestForAllObjects:(NSArray<id<iTermSubscribable>> *)objects
                              mutableVariablesDict:
                                  (NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *,
                                                                                     iTermVariableReference *> *> *> *)
                                      dict
...
- (ITMNotificationResponse *)
    handleSubscriptionRequestForAllObjects:(NSArray<id<iTermSubscribable>> *)objects
...
- (ITMNotificationResponse *)handleSubscriptionRequestForAllSessionsFromConnectionKey:(NSString *)connectionKey
                                                                              request:
                                                                                  (ITMNotificationRequest *)request {
...
- (void)apiServerNotification:(ITMNotificationRequest *)request
    ...
    if (request.notificationType == ITMNotificationType_NotifyOnVariableChange) {
        handler([self handleVariableChangeNotificationRequest:request connectionKey:connectionKey]);
        return;
    }
    if (request.notificationType == ITMNotificationType_NotifyOnNewSession ||
        ...
        handler([self handleAPINotificationRequest:request connectionKey:connectionKey]);
        ...
        if (!session) {
            ...
            handler([session handleAPINotificationRequest:request connectionKey:connectionKey]);
        }
    }
}
...
- (void)removeAllSubscriptionsForConnectionKey:(id)connectionKey {
    ...
    // RPCs are special.
    NSInteger rpcsRemoved = [self removeServerOriginatedRPCSubscriptionsPasstingTest:^BOOL(
                                      NSString *signature, iTermTuple<id, ITMNotificationRequest *> *tuple) {
        return [tuple.firstObject isEqual:connectionKey];
    }];
    [_allSessionVariableSubscriptions[connectionKey]
        enumerateObjectsUsingBlock:^(iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *_Nonnull tuple,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
            [tuple.secondObject removeAllLinks];
        }];
    [_allWindowVariableSubscriptions[connectionKey]
        enumerateObjectsUsingBlock:^(iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *_Nonnull tuple,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
            [tuple.secondObject removeAllLinks];
        }];
    ...
    NSArray<NSMutableDictionary<id, ITMNotificationRequest *> *> *dicts = @[
        _newSessionSubscriptions ?: empty, _profileChangeSubscriptions ?: empty,
        _terminateSessionSubscriptions ?: empty, _layoutChangeSubscriptions ?: empty,
        _focusChangeSubscriptions ?: empty, _broadcastDomainChangeSubscriptions ?: empty,
        _allSessionVariableSubscriptions ?: empty, _allWindowVariableSubscriptions ?: empty
    ];
...
- (void)apiServerDidCloseConnectionWithKey:(id)connectionKey {
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermRemoveAPIServerSubscriptionsNotification
...
- (void)apiServerRegisterTool:(ITMRegisterToolRequest *)request handler:(void (^)(ITMRegisterToolResponse *))handler {
...
- (void)apiServerSetProfileProperty:(ITMSetProfilePropertyRequest *)request
    ...
    NSMutableArray *objects = [[NSMutableArray alloc] initWithCapacity:16];
    ...
    switch (request.targetOneOfCase) {
        ...
        case ITMSetProfilePropertyRequest_Target_OneOfCase_GuidList: {
            setter =
                ^ITMSetProfilePropertyResponse_Status(id object, NSArray<iTermTuple<NSString *, id> *> *assignments) {
    ...
    assignments = [assignments mapWithBlock:^id(iTermTuple<NSString *, id> *tuple) {
        ...
        if (!value || error) {
            ...
            error = error
                        ?: [NSError errorWithDomain:iTermAPIHelperErrorDomain
                                               code:iTermAPIHelperErrorCodeInvalidJSON
                                           userInfo:nil];
            return nil;
        }
...
- (void)apiServerListSessions:(ITMListSessionsRequest *)request handler:(void (^)(ITMListSessionsResponse *))handler {
...
- (void)apiServerCreateTab:(ITMCreateTabRequest *)request handler:(void (^)(ITMCreateTabResponse *))handler {
    ...
    launcher.makeSession = ^(NSDictionary *_Nonnull profile, PseudoTerminal *_Nonnull term,
                             void (^_Nonnull didMakeSession)(PTYSession *_Nullable)) {
        ...
        [term asyncSplitVertically:vertical
                     ...
                     targetSession:session
                        completion:^(PTYSession *newSession, BOOL ok) {
                            if (newSession &&
                                newSession.guid) { // The test for newSession.guid is just to quiet the analyzer
...
- (Profile *)profileByCustomizing:(Profile *)profile
                   withProperties:(NSArray<ITMProfileProperty *> *)customProfilePropertiesArray {
    ...
    switch (request.identifierOneOfCase) {
        ...
        case ITMSetPropertyRequest_Identifier_OneOfCase_SessionId: {
            if ([request.sessionId isEqualToString:@"all"]) {
                // BUG-3754: Process all sessions, not just the first one
                // Each session gets its own response; use the last status
...
- (void)setPropertyInWindow:(PseudoTerminal *)term
    ...
    NSDictionary<NSString *, void (^)(void)> *handlers = @{@"frame" : setFrame, @"fullscreen" : setFullScreen};
...
- (NSString *)getPropertyFromWindow:(PseudoTerminal *)term name:(NSString *)name {
    ...
    NSDictionary<NSString *, GetWindowPropertyBlock> *handlers = @{@"frame" : getFrame, @"fullscreen" : getFullScreen};
...
- (NSString *)getPropertyFromSession:(PTYSession *)session name:(NSString *)name {
    ...
    if (session.isBrowserSession) {
        getGridSize = ^NSString * {
            NSDictionary *dict = @{@"width" : @0, @"height" : @0};
            return [NSJSONSerialization it_jsonStringForObject:dict];
        };

        getNumberOfLines = ^NSString * {
            NSDictionary *dict = @{@"overflow" : @0, @"grid" : @0, @"history" : @0, @"first_visible" : @0};
            return [NSJSONSerialization it_jsonStringForObject:dict];
        };
    } else {
        getGridSize = ^NSString * {
            NSDictionary *dict = @{@"width" : @(session.screen.width - 1), @"height" : @(session.screen.height - 1)};
            return [NSJSONSerialization it_jsonStringForObject:dict];
        };
...
- (void)apiServerActivate:(ITMActivateRequest *)request handler:(void (^)(ITMActivateResponse *))handler {
    ...
    if (request.identifierOneOfCase == ITMActivateRequest_Identifier_OneOfCase_TabId) {
        ...
        windowController = [[[iTermController sharedInstance] terminals]
            objectPassingTest:^BOOL(PseudoTerminal *element, NSUInteger index, BOOL *stop) {
...
- (void)handleAppScopeVariableRequest:(ITMVariableRequest *)request handler:(void (^)(ITMVariableResponse *))handler {
    ITMVariableResponse *response = [[ITMVariableResponse alloc] init];
...
- (void)handleTabScopeVariableRequest:(ITMVariableRequest *)request handler:(void (^)(ITMVariableResponse *))handler {
...
- (void)handleWindowScopeVariableRequest:(ITMVariableRequest *)request
    ...
    if ([request.sessionId isEqualToString:@"all"]) {
        NSArray<iTermVariableScope *> *scopes =
            [[iTermController sharedInstance].terminals mapWithBlock:^id(PseudoTerminal *anObject) {
...
- (BOOL)handleVariableSetsInRequest:(ITMVariableRequest *)request scope:(iTermVariableScope *)scope {
    ...
    [request.setArray
        enumerateObjectsUsingBlock:^(ITMVariableRequest_Set *_Nonnull setRequest, NSUInteger idx, BOOL *_Nonnull stop) {
            id value;
...
- (void)handleVariableGetsInRequest:(ITMVariableRequest *)request
                           response:(ITMVariableResponse *)response
                              scope:(iTermVariableScope *)scope {
    ...
    switch (result.resultOneOfCase) {
        case ITMServerOriginatedRPCResultRequest_Result_OneOfCase_JsonValue:
            ...
            if (!value || error) {
                value = nil;
                exception =
                    @{@"reason" : [NSString stringWithFormat:@"Undecodable value: %@", error.localizedDescription]};
            }
            break;

        case ITMServerOriginatedRPCResultRequest_Result_OneOfCase_JsonException:
            exception =
                [NSJSONSerialization JSONObjectWithData:[result.jsonException dataUsingEncoding:NSUTF8StringEncoding]
                                                options:NSJSONReadingAllowFragments
                                                  error:&error];
            if (error) {
                exception =
                    @{@"reason" : [NSString stringWithFormat:@"Undecodable exception: %@", error.localizedDescription]};
            } else {
...
- (void)apiServerRestartSession:(ITMRestartSessionRequest *)request
                        handler:(void (^)(ITMRestartSessionResponse *))handler {
    PTYSession *session = [self sessionForAPIIdentifier:request.sessionId includeBuriedSessions:YES];
...
- (void)apiServerGetBroadcastDomains:(ITMGetBroadcastDomainsRequest *)request
                             handler:(void (^)(ITMGetBroadcastDomainsResponse *))handler {
...
- (void)handleTmuxSendCommand:(ITMTmuxRequest_SendCommand *)request handler:(void (^)(ITMTmuxResponse *))handler {
    ...
    iTermBlockTargetActionForwarder *forwarder =
        [[iTermBlockTargetActionForwarder alloc] initWithBlock:^(NSString *result) {
    ...
    [forwarder attachToOwner:controller.gateway
                     failure:^{
                         handler(response);
                     }];
}

- (void)handleTmuxListConnections:(ITMTmuxRequest_ListConnections *)request
                          handler:(void (^)(ITMTmuxResponse *))handler {
    ITMTmuxResponse *response = [[ITMTmuxResponse alloc] init];
    for (NSString *clientName in [[TmuxControllerRegistry sharedInstance] clientNames]) {
        ITMTmuxResponse_ListConnections_Connection *connection =
            [[ITMTmuxResponse_ListConnections_Connection alloc] init];
...
- (void)handleTmuxSetWindowVisible:(ITMTmuxRequest_SetWindowVisible *)request
                           handler:(void (^)(ITMTmuxResponse *))handler {
    ITMTmuxResponse *response = [[ITMTmuxResponse alloc] init];
    TmuxController *controller = [[TmuxControllerRegistry sharedInstance] controllerForClient:request.connectionId];
    if (!controller) {
    ...
    if (request.visible) {
        ...
        [controller openWindowWithId:[[request windowId] intValue] intentional:YES profile:controller.sharedProfile];
        response.status = ITMTmuxResponse_Status_Ok;
        handler(response);
        return;
    }
...
- (ITMReorderTabsResponse_Status)validateReorderTabsRequest:(ITMReorderTabsRequest *)request {
    NSMutableSet<NSString *> *windowIds = [NSMutableSet setWithCapacity:request.assignmentsArray.count];
    NSMutableSet<NSString *> *tabIds = [NSMutableSet setWithCapacity:request.assignmentsArray.count * 4];
...
- (void)performReorderAssignment:(ITMReorderTabsRequest_Assignment *)assignment {
    ...
    // BUG-f693: Replace assert with guard - missing destination should skip not crash
    if (!destination) {
        DLog(@"ERROR: iTermAPIHelper performReorderAssignment: destination window not found - %@", assignment.windowId);
        return;
    }
    ...
    for (NSString *tabId in assignment.tabIdsArray) {
        PTYTab *tab = [self tabWithID:tabId];
        // BUG-f694: Replace assert with guard - missing tab should skip not crash
        if (!tab) {
            DLog(@"ERROR: iTermAPIHelper performReorderAssignment: tab not found - %@", tabId);
            continue;
        }
        ...
        if (source == destination) {
            ...
            // BUG-f695: Replace assert with guard - missing tab in source should skip not crash
            if (sourceIndex == NSNotFound) {
                DLog(@"ERROR: iTermAPIHelper performReorderAssignment: tab not found in source - %@", tabId);
                continue;
            }
...
- (void)apiServerReorderTabsRequest:(ITMReorderTabsRequest *)request
                            handler:(void (^)(ITMReorderTabsResponse *))handler {
...
- (void)apiServerPreferencesRequest:(ITMPreferencesRequest *)request
                            handler:(void (^)(ITMPreferencesResponse *))handler {
    ITMPreferencesResponse *response = [[ITMPreferencesResponse alloc] init];

    [request.requestsArray
        enumerateObjectsUsingBlock:^(ITMPreferencesRequest_Request *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    switch (request.requestOneOfCase) {
        ...
        case ITMPreferencesRequest_Request_Request_OneOfCase_SetPreferenceRequest:
            result.setPreferenceResult =
                [self handleSetPreferenceRequestWithKey:request.setPreferenceRequest.key
        ...
        case ITMPreferencesRequest_Request_Request_OneOfCase_SetDefaultProfileRequest:
            result.setDefaultProfileResult =
                [self handleSetDefaultProfileWithGUID:request.setDefaultProfileRequest.guid];
            break;
...
- (ITMPreferencesResponse_Result_GetPreferenceResult *)handleGetPreferenceRequestForKey:(NSString *)key {
    ITMPreferencesResponse_Result_GetPreferenceResult *result =
        [[ITMPreferencesResponse_Result_GetPreferenceResult alloc] init];
...
- (ITMPreferencesResponse_Result_SetPreferenceResult *)handleSetPreferenceRequestWithKey:(NSString *)key
    ...
    ITMPreferencesResponse_Result_SetPreferenceResult *result =
        [[ITMPreferencesResponse_Result_SetPreferenceResult alloc] init];
    NSError *error = nil;
...
- (ITMPreferencesResponse_Result_GetDefaultProfileResult *)handleGetDefaultProfile {
    ITMPreferencesResponse_Result_GetDefaultProfileResult *result =
        [[ITMPreferencesResponse_Result_GetDefaultProfileResult alloc] init];
...
- (ITMPreferencesResponse_Result_SetDefaultProfileResult *)handleSetDefaultProfileWithGUID:(NSString *)guid {
    ITMPreferencesResponse_Result_SetDefaultProfileResult *result =
        [[ITMPreferencesResponse_Result_SetDefaultProfileResult alloc] init];
    Profile *profile = [[ProfileModel sharedInstance] bookmarkWithGuid:guid];
    if (!profile) {
...
- (void)handleGetPreset:(ITMColorPresetRequest_GetPreset *)request
             completion:(void (^)(ITMColorPresetResponse *))completion {
    ITMColorPresetResponse *message = [[ITMColorPresetResponse alloc] init];
    ...
    for (NSString *key in preset) {
        ITMColorPresetResponse_GetPreset_ColorSetting *colorSetting = [self colorSettingForDictionary:preset[key]
                                                                                                  key:key];
...
- (ITMColorPresetResponse_GetPreset_ColorSetting *)colorSettingForDictionary:(iTermColorDictionary *)dict
                                                                         key:(NSString *)key {
    ITMColorPresetResponse_GetPreset_ColorSetting *setting =
        [[ITMColorPresetResponse_GetPreset_ColorSetting alloc] init];
    setting.key = key;
    NSNumber *obj;
...
- (void)handleListPresets:(ITMColorPresetRequest_ListPresets *)request
               completion:(void (^)(ITMColorPresetResponse *))completion {
    ITMColorPresetResponse *message = [[ITMColorPresetResponse alloc] init];
...
- (void)handleSetSelectionRequest:(ITMSelectionRequest_SetSelectionRequest *)request
    ...
    NSArray<iTermSubSelection *> *subSelections = [request.selection.subSelectionsArray mapWithBlock:^id(
                                                                                            ITMSubSelection *subProto) {
        if (subProto.windowedCoordRange.coordRange.end.x > width || subProto.windowedCoordRange.columns.length < 0 ||
            subProto.windowedCoordRange.columns.location + subProto.windowedCoordRange.columns.length > width ||
            subProto.windowedCoordRange.coordRange.start.x < 0 || subProto.windowedCoordRange.columns.location < 0 ||
            subProto.windowedCoordRange.coordRange.start.y < 0 || subProto.windowedCoordRange.coordRange.end.y < 0) {
            return nil;
        }
        VT100GridCoordRange coordRange =
            VT100GridCoordRangeMake(subProto.windowedCoordRange.coordRange.start.x,
        ...
        VT100GridWindowedRange range = VT100GridWindowedRangeMake(
            coordRange, subProto.windowedCoordRange.columns.location, subProto.windowedCoordRange.columns.length);
...
- (void)handleOpenStatusBarPopoverRequest:(ITMStatusBarComponentRequest_OpenPopover *)request
    ...
    [component statusBarComponentOpenPopoverWithHTML:request.html
                                              ofSize:NSMakeSize(request.size.width, request.size.height)];
...
- (ITMSetBroadcastDomainsResponse *)handleSetBroadcastDomains:(ITMSetBroadcastDomainsRequest *)request {
    ...
    // Check validity
    NSMutableSet<NSString *> *sessionIDs = [NSMutableSet setWithCapacity:request.broadcastDomainsArray.count * 4];
    NSMutableArray<NSArray<PTYSession *> *> *sessionGroups =
        [[NSMutableArray alloc] initWithCapacity:request.broadcastDomainsArray.count];
    for (ITMBroadcastDomain *domain in request.broadcastDomainsArray) {
        NSMutableArray<PTYSession *> *sessions = [[NSMutableArray alloc] initWithCapacity:domain.sessionIdsArray.count];
    ...
    for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
        NSArray<NSArray<PTYSession *> *> *sessionGroupsForWindow =
            [sessionGroups filteredArrayUsingBlock:^BOOL(NSArray<PTYSession *> *sessionGroup) {
...
- (void)apiServerInvokeFunctionRequest:(ITMInvokeFunctionRequest *)request
                               handler:(void (^)(ITMInvokeFunctionResponse *))completion {
    ...
    switch (request.contextOneOfCase) {
        ...
        case ITMInvokeFunctionRequest_Context_OneOfCase_Tab:
            [self invokeFunction:request.invocation
                     inTabWithID:request.tab.tabId
                      completion:completion
                         timeout:request.timeout];
            return;
        case ITMInvokeFunctionRequest_Context_OneOfCase_Window:
            [self invokeFunction:request.invocation
                  inWindowWithID:request.window.windowId
                      completion:completion
                         timeout:request.timeout];
            return;
        case ITMInvokeFunctionRequest_Context_OneOfCase_Session:
            [self invokeFunction:request.invocation
                 inSessionWithID:request.session.sessionId
                      completion:completion
                         timeout:request.timeout];
            return;
        case ITMInvokeFunctionRequest_Context_OneOfCase_Method:
            ...
            [self invokeMethod:request.invocation
                      receiver:request.method.receiver
                    completion:completion
                       timeout:request.timeout];
            return;
...
- (void)invokeFunction:(NSString *)invocation
    inAppContextWithCompletion:(void (^)(ITMInvokeFunctionResponse *))completion
    ...
    [iTermScriptFunctionCall callFunction:invocation
                       ...
                       sideEffectsAllowed:YES
                               ...
                               completion:^(id object, NSError *error, NSSet<NSString *> *missing) {
                                   [self functionInvocationDidCompleteWithObject:object
                                                                           error:error
                                                                      completion:completion];
                               }];
}
...
- (void)invokeFunction:(NSString *)invocation
       inSessionWithID:(NSString *)sessionId
            completion:(void (^)(ITMInvokeFunctionResponse *))completion
               timeout:(NSTimeInterval)timeout {
    PTYSession *session = [self sessionForAPIIdentifier:sessionId includeBuriedSessions:YES];
    if (!session) {
...
- (void)functionInvocationDidCompleteWithObject:(id)object
    ...
    ITMInvokeFunctionResponse *response = [[ITMInvokeFunctionResponse alloc] init];
    if (error) {
        if ([error.domain isEqualToString:@"com.dashterm.dashterm2.call"] && error.code == 2) {
