...
- (NSString *)it_description {
    return [NSString stringWithFormat:@"<%@: %p frame=%@ visibleFrame=%@ hasDock=%@>", NSStringFromClass(self.class),
                                      self, NSStringFromRect(self.frame), NSStringFromRect(self.visibleFrame),
                                      [self hasDock] ? @"YES" : @"NO"];
}
...
+ (double)fractionOfFrameOnAnyScreen:(NSRect)frame recommendedOrigin:(NSPoint *)recommendedOriginPtr {
...
- (NSPoint)improvedOrigin:(NSRect)frame {
    ...
    NSArray<NSNumber *> *xOriginCandidates =
        @[ @(NSMinX(myFrame)), @(NSMaxX(myFrame) - NSWidth(frame)), @(NSMinX(frame)) ];
    NSArray<NSNumber *> *yOriginCandidates =
        @[ @(NSMinY(myFrame)), @(NSMaxY(myFrame) - NSHeight(frame)), @(NSMinY(frame)) ];
    ...
    iTermTuple<NSNumber *, NSNumber *> *best =
        [tuples maxWithBlock:^NSComparisonResult(iTermTuple<NSNumber *, NSNumber *> *v1,
...
- (NSRect)frameExceptMenuBar {
    if ([[NSScreen screens] firstObject] == self || [NSScreen screensHaveSeparateSpaces]) {
        ...
        const CGFloat hackyGuess =
            NSHeight(self.frame) - NSHeight(self.visibleFrame) - NSMinY(self.visibleFrame) + NSMinY(self.frame) - 1;
        const CGFloat notchHeight = [self notchHeight];
...
- (iTermScreenIdentifier)it_identifier {
    ...
    const iTermScreenIdentifier result = {.modelNumber = CGDisplayModelNumber(displayID),
...
- (NSString *)it_uniqueKey {
    ...
    return [NSString stringWithFormat:@"UniqueDisplayKey: %u %u %u", screenID.modelNumber, screenID.vendorNumber,
...
- (NSString *)it_fallbackName {
    ...
    NSInteger index =
        [screens indexOfObjectPassingTest:^BOOL(NSScreen *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (NSArray<NSScreen *> *)it_sortedScreens {
    return [[NSScreen screens]
        sortedArrayUsingComparator:^NSComparisonResult(NSScreen *_Nonnull lhs, NSScreen *_Nonnull rhs) {
...
- (NSString *)it_uniqueName {
    ...
    NSMutableDictionary<NSString *, NSNumber *> *last = [[NSMutableDictionary alloc] initWithCapacity:sorted.count];
    for (NSScreen *screen in sorted) {
...
+ (NSArray<NSDictionary *> *)it_allWindowInfoDictionaries {
    const CGWindowListOption options = (kCGWindowListExcludeDesktopElements | kCGWindowListOptionOnScreenOnly);
    NSArray<NSDictionary *> *windowInfos =
        (__bridge_transfer NSArray *)CGWindowListCopyWindowInfo(options, kCGNullWindowID);
    return windowInfos;
}
...
+ (CGRect)windowBoundsRectFromWindowInfoDictionary:(NSDictionary *)dictionary {
    ...
    BOOL ok = NO;
    if (dictionary) {
        ok = CGRectMakeWithDictionaryRepresentation((CFDictionaryRef)dictionary[(id)kCGWindowBounds], &rect);
    }
    if (!ok) {
...
- (NSNumber *)it_cachedSupportsHighFrameRates {
    iTermTuple<NSNumber *, NSNumber *> *tuple =
        [self it_associatedObjectForKey:&iTermNSScreenSupportsHighFrameRatesCacheKey];
    if (!tuple) {
        return nil;
    }
