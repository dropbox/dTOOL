...
- (IBAction)optionAsMetaHelp:(id)sender {
    [[NSView castFrom:sender] it_showWarningWithMarkdown:@"In most key reporting modes, when reporting special keys like arrows, the ⌥ key may act as either Meta or Alt. Prior to version 3.5.6, DashTerm2 used Meta. The default changed to Alt because some programs like Emacs expect it."];
}
...
- (IBAction)csiuHelp:(id)sender {
    ...
    NSURL *url = [NSURL URLWithString:@"https://dashterm.com/documentation-csiu.html"];
...
- (void)optionKeySendsDidChangeForControl:(NSPopUpButton *)sender {
    ...
    // BUG-f1339: Replace assert() with guard - missing info should not crash
    if (!info) {
        ELog(@"optionKeySendsDidChangeForControl: No info found for sender %@", sender);
        return;
    }
...
- (void)updateOptionKeySendsForControl:(NSPopUpButton *)control {
    ...
    // BUG-f1340: Replace assert() with guard - missing info should not crash
    if (!info) {
        ELog(@"updateOptionKeySendsForControl: No info found for control %@", control);
        return;
    }
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
    ...
    Profile *profile = [self.delegate profilePreferencesCurrentProfile];
    // BUG-f1341: Replace assert() with guard - no profile should not crash
    if (!profile) {
        ELog(@"keyMapping:didChangeItem: No current profile");
        return;
    }
    NSMutableDictionary *dict = [profile mutableCopy];
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  removeKeystrokes:(NSSet<iTermKeystroke *> *)keystrokes
    ...
    Profile *profile = [self.delegate profilePreferencesCurrentProfile];
    // BUG-f1342: Replace assert() with guard - no profile should not crash
    if (!profile) {
        ELog(@"keyMapping:removeKeystrokes: No current profile");
        return;
    }
    ...
    [keystrokes enumerateObjectsUsingBlock:^(iTermKeystroke * _Nonnull keystroke, BOOL * _Nonnull stop) {
        ...
        // BUG-f1343: Replace assert() with guard - keystroke not found should skip, not crash
        if (index == NSNotFound) {
            ELog(@"keyMapping:removeKeystrokes: Keystroke %@ not found in profile", keystroke);
            return;  // continue to next keystroke
        }
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  loadPresetsNamed:(NSString *)presetName {
    Profile *profile = [self.delegate profilePreferencesCurrentProfile];
    // BUG-f1344: Replace assert() with guard - no profile should not crash
    if (!profile) {
        ELog(@"keyMapping:loadPresetsNamed: No current profile");
        return;
    }
