...
- (instancetype)initWithQueue:(dispatch_queue_t)queue {
    ...
    if (self) {
        _children = [NSMutableArray arrayWithCapacity:16];  // Typical session count
        _pendingLaunches = [NSMutableDictionary dictionaryWithCapacity:8];
        _readFD = -1;
        _writeFD = -1;
        _serverPID = -1;
        _writeQueue = [NSMutableArray arrayWithCapacity:64];  // Message queue
        _readQueue = [NSMutableArray arrayWithCapacity:64];
    }
    return self;
}
...
- (void)setWriteFD:(int)writeFD {
    ...
    if (cancelWrites) {
        ...
        // BUG-f980: Guard writeQueue state instead of assert - clear queue if not empty
...
- (void)setReadFD:(int)readFD {
    ...
    if (cancelReads) {
        ...
        // BUG-f981: Guard readQueue state instead of assert - clear queue if not empty
