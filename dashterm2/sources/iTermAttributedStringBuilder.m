...
static NSColor *iTermTextDrawingHelperGetTextColor(screen_char_t *c,
    ...
    if (isMatch) {
        ...
        // BUG-f696: Replace assert with fallback - nil color should use black not crash
        if (!rawColor) {
            rawColor = [NSColor blackColor];
        }
        context->havePreviousCharacterAttributes = NO;
    } else if (inUnderlinedRange) {
        // Blue link text.
        rawColor = [context->colorMap colorForKey:kColorMapLink];
        // BUG-f697: Replace assert with fallback - nil color should use blue not crash
        if (!rawColor) {
            rawColor = [NSColor blueColor];
        }
        context->havePreviousCharacterAttributes = NO;
        ...
        // BUG-f698: Replace assert with fallback - nil color should use black not crash
        if (!rawColor) {
            rawColor = [NSColor blackColor];
        }
        context->havePreviousCharacterAttributes = NO;
        ...
        // BUG-f699: Replace assert with fallback - nil color should use white not crash
        if (!rawColor) {
            rawColor = [NSColor whiteColor];
        }
        context->havePreviousCharacterAttributes = NO;
        ...
        rawColor = [context->delegate colorForCode:c->foregroundColor
                                         ...
                                         colorMode:c->foregroundColorMode
                                             ...
                                             faint:c->faint
                                      isBackground:NO];
        // BUG-f700: Replace assert with fallback - nil color should use gray not crash
        if (!rawColor) {
            rawColor = [NSColor grayColor];
        }
    } else {
        ...
        if (needsProcessing && context->backgroundColor != context->previousBackgroundColor) {
            ...
            // BUG-f701: Replace assert with fallback - nil color should use gray not crash
            if (!rawColor) {
                rawColor = [NSColor grayColor];
            }
        } else {
    ...
    // BUG-f702: Replace assert with fallback - nil result should use rawColor not crash
    if (!result) {
        result = rawColor ?: [NSColor grayColor];
    }
    return result;
}
...
- (NSArray<id<iTermAttributedString>> *)attributedStringsForLine:(const screen_char_t *)line
    ...
    // Most lines have 1-4 attributed string segments (uniform text, maybe some colored portions)
    NSMutableArray<id<iTermAttributedString>> *attributedStrings = [NSMutableArray arrayWithCapacity:4];
...
- (NSString *)statisticsString {
    // Performance optimization: iTermPreciseTimerLock/Unlock use os_unfair_lock with ~10x lower overhead than @synchronized.
    // CRITICAL: Use _locked versions of timer functions to avoid recursive lock attempts on os_unfair_lock,
    // which would crash with _os_unfair_lock_recursive_abort.
    iTermPreciseTimerLock();
    iTermPreciseTimerStatsRecordTimer_locked(_stats.attrsForChar);
    iTermPreciseTimerStatsRecordTimer_locked(_stats.shouldSegment);
    iTermPreciseTimerStatsRecordTimer_locked(_stats.buildMutableAttributedString);
    iTermPreciseTimerStatsRecordTimer_locked(_stats.combineAttributes);
    iTermPreciseTimerStatsRecordTimer_locked(_stats.updateBuilder);
    iTermPreciseTimerStatsRecordTimer_locked(_stats.advances);
    ...
    _statsCount += 1;
    NSString *result = iTermPreciseTimerLogString_locked(NSStringFromClass([self class]),
                                                         array,
    ...
    if (_statsCount % 1000 == 0) {
        iTermPreciseTimerStatsInit_locked(_stats.attrsForChar, NULL);
        iTermPreciseTimerStatsInit_locked(_stats.shouldSegment, NULL);
        iTermPreciseTimerStatsInit_locked(_stats.buildMutableAttributedString, NULL);
        iTermPreciseTimerStatsInit_locked(_stats.combineAttributes, NULL);
        iTermPreciseTimerStatsInit_locked(_stats.updateBuilder, NULL);
        iTermPreciseTimerStatsInit_locked(_stats.advances, NULL);
    }

    iTermPreciseTimerUnlock();
    return result;
}
@end
