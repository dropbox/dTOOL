...
static os_unfair_lock sHistogramsLock = OS_UNFAIR_LOCK_INIT;
...
@property (readonly, strong)
    NSMutableDictionary<NSString *, __kindof iTermMetalRendererTransientState *> *transientStates;
...
- (instancetype)initWithView:(iTermMetalView *)view fullSizeTexturePool:(iTermTexturePool *)fullSizeTexturePool {
    ...
    if (self) {
        ...
        // Pre-allocate for typical renderer count - usually 10-15 renderers per frame
        _transientStates = [NSMutableDictionary dictionaryWithCapacity:16];
#if ENABLE_STATS
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<%@: %p age=%f frameNumber=%@/%@ status=%@>", self.class, self, now - _creation,
                                      @(_frameNumber), @(gNextFrameDataNumber), self.status];
}
...
#if ENABLE_STATS
        const double duration = iTermPreciseTimerStatsMeasureAndRecordTimer(
            &self->_stats[iTermMetalFrameDataStatDispatchToPrivateQueueForCompletion]);
...
- (id<MTLRenderCommandEncoder>)newRenderEncoderWithDescriptor:(MTLRenderPassDescriptor *)renderPassDescriptor
    ...
    MTLViewport viewport = {-(double)viewportSize.x, 0.0, viewportSize.x * 2, viewportSize.y * 2, 0.0, 1.0};
    [renderEncoder setViewport:viewport];
    return renderEncoder;
}

- (MTLRenderPassDescriptor *)newRenderPassDescriptorWithLabel:(NSString *)label fast:(BOOL)fast {
    ...
    // BUG-f1074: Replace assert with guard - texture allocation failure should return nil, not crash
    if (renderPassDescriptor.colorAttachments[0].texture == nil) {
        ELog(@"ERROR: newRenderPassDescriptorWithLabel: texture allocation failed for label: %@", label);
        return nil;
    }
...
- (void)createIntermediateRenderPassDescriptor {
    [self measureTimeForStat:iTermMetalFrameDataStatPqCreateIntermediate
                     ofBlock:^{
                         // BUG-f1075: Replace assert with guard - duplicate call should be idempotent, not crash
                         if (self.intermediateRenderPassDescriptor) {
                             DLog(@"WARNING: createIntermediateRenderPassDescriptor called when descriptor already "
                                  @"exists");
                             return;
                         }

                         self.intermediateRenderPassDescriptor =
                             [self newRenderPassDescriptorWithLabel:@"Intermediate Texture" fast:YES];
...
- (void)createTemporaryRenderPassDescriptor {
    [self
        measureTimeForStat:iTermMetalFrameDataStatPqCreateTemporary
                   ofBlock:^{
                       // BUG-f1076: Replace assert with guard - duplicate call should be idempotent, not crash
                       if (self.temporaryRenderPassDescriptor) {
                           DLog(@"WARNING: createTemporaryRenderPassDescriptor called when descriptor already exists");
                           return;
                       }
...
- (void)didCompleteWithAggregateStats:(iTermPreciseTimerStats *)aggregateStats
    ...
    [self.transientStates
        enumerateKeysAndObjectsUsingBlock:^(
            NSString *_Nonnull key, __kindof iTermMetalRendererTransientState *_Nonnull tState, BOOL *_Nonnull stop) {
    ...
    [self.transientStates
        enumerateKeysAndObjectsUsingBlock:^(
            NSString *_Nonnull key, __kindof iTermMetalRendererTransientState *_Nonnull tState, BOOL *_Nonnull stop) {
            [self mergeHistogram:tState.poolContext.histogram
                            name:[NSString stringWithFormat:@"%@: buffer sizes", NSStringFromClass(tState.class)]];
            [self mergeHistogram:tState.poolContext.textureHistogram
                            name:[NSString stringWithFormat:@"%@: texture sizes", NSStringFromClass(tState.class)]];
            [self mergeHistogram:tState.poolContext.wasteHistogram
                            name:[NSString stringWithFormat:@"%@: wasted space", NSStringFromClass(tState.class)]];
        }];
    ...
    iTermPreciseTimerPeriodicLog([NSString stringWithFormat:@"%@: Metal Frame Data\n", owner], temp,
                                 iTermMetalFrameDataStatCount, 1, [iTermAdvancedSettingsModel logDrawingPerformance],
                                 aggregateHistograms, additional);
#endif // ENABLE_STATS
}
...
- (void)mergeHistogram:(iTermHistogram *)histogramToMerge name:(NSString *)name {
    ...
    dispatch_once(&onceToken, ^{
        sHistograms = [[NSMutableDictionary alloc] initWithCapacity:16]; // Metal frame histograms
    });
    os_unfair_lock_lock(&sHistogramsLock);
    ...
    os_unfair_lock_unlock(&sHistogramsLock);
#endif
}

- (NSString *)histogramsString {
#if ENABLE_STATS
    // Each histogram sparkline is ~100 chars; allocate for estimated histogram count
    NSMutableString *result = [NSMutableString stringWithCapacity:4096];
    os_unfair_lock_lock(&sHistogramsLock);
    NSArray *sortedKeys = [sHistograms.allKeys sortedArrayUsingSelector:@selector(compare:)];
...
- (iTermCellRenderConfiguration *)cellConfiguration {
    if (!_cellConfiguration) {
        _cellConfiguration = [[iTermCellRenderConfiguration alloc]
                                      initWithViewportSize:self.viewportSize
