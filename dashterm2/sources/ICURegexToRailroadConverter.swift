...
class ICURegexToRailroadConverter {
    ...
    private func applyQuantifier(_ base: String) -> String {
        ...
        case "{":
            ...
            if parts.count == 1, let onlyPart = parts.first {
                // Exactly n times
                if let n = Int(onlyPart) {
                    if n == 0 {
                        // {0} means empty
                        return "!"
                    } else if n == 1 {
                      ...
                      let lowerPart = parts.first,
                      let upperPart = parts.dropFirst().first {
                if upperPart.isEmpty {
                    // At least n times {n,}
                    if let n = Int(lowerPart) {
                        if n == 0 {
                            // {0,} is same as *
                    ...
                    if let n = Int(lowerPart), let m = Int(upperPart) {
                        if n == 0 && m == 1 {
                            // {0,1} is same as ?
