...
class LastPassDataSource: CommandLinePasswordDataSource {
    ...
    // BUG-264: Default initializer required for ObjC runtime instantiation
    // (e.g., NSClassFromString("...").init() in tests)
    @objc override init() {
        self.browser = false
        super.init()
    }
    ...
    // BUG-333: Terminal groups use DashTerm2 branding
    ...
    private var setPasswordRecipe: AnyRecipe<SetPasswordRequest, Void> {
        let recipe = LastPassDynamicCommandRecipe<SetPasswordRequest, Void> {
            ...
            var commandRequest = InteractiveCommandRequest(
                ...
                args: args,
                env: LastPassUtils.basicEnvironment)
            // BUG-1619: Use nil coalescing instead of force unwrap for data encoding
    ...
    // BUG-333: Group prefix uses DashTerm2 branding
    private var addAccountRecipe: AnyRecipe<AddRequest, AccountIdentifier> {
        ...
        let addRecipe = LastPassDynamicCommandRecipe<AddRequest, Void> {
            ...
            var commandRequest = InteractiveCommandRequest(
                ...
                env: LastPassUtils.basicEnvironment)
            // BUG-1619: Use nil coalescing instead of force unwrap for data encoding
            let dataToWrite = input.data(using: .utf8) ?? Data()
            commandRequest.callbacks = InteractiveCommandRequest.Callbacks(
                callbackQueue: InteractiveCommandRequest.ioQueue,
                handleStdout: nil,
                handleStderr: nil,
        ...
        let showRecipe = LastPassDynamicCommandRecipe<(AddRequest, Void), AccountIdentifier> { tuple in
            ...
            // BUG-1605: Use guard to safely access .last instead of force unwrap
            guard let idString = lines.last else {
                throw LPError.runtime
            }
...
extension LastPassDataSource: PasswordManagerDataSource {
    ...
    // BUG-f504: Return an error instead of crashing when OTP toggling is not supported
    func toggleShouldSendOTP(context: RecipeExecutionContext,
        ...
        completion(nil, NSError(domain: "com.dashterm2.lastpass",
                               code: -1,
                               userInfo: [NSLocalizedDescriptionKey: "OTP toggling is not supported by LastPass"]))
    }
    ...
    private func _checkAvailability() -> Bool {
        // Use silent path check to avoid prompting during availability checks
        guard let cliPath = LastPassUtils.pathToCLISilent else {
            return false
        }
        do {
            let request = InteractiveCommandRequest(command: cliPath,
...
class LastPassUtils {
    static var basicEnvironment: [String: String] {
        var env = ["HOME": NSHomeDirectory()]
        if let askpass = pathToAskpass {
            env["LPASS_ASKPASS"] = askpass
        }
        return env
    }
    static var pathToAskpass: String? {
        guard let path = Bundle.main.path(forResource: "askpass", ofType: "sh") else {
            DLog("askpass.sh resource not found in bundle")
            return nil
        }
        return path
    }
    ...
    /// Returns path to lpass CLI, prompting user if not found
    static var pathToCLI: String {
        // BUG-f1388: Replace force unwrap with safe guard + fallback
        // When returnNilIfNotFound is false, pathToCLI always returns a value (normalPaths[0] fallback)
        // but using force unwrap is fragile if implementation changes
        guard let path = pathToCLI(promptIfNotFound: true) else {
            DLog("BUG-f1388: pathToCLI returned nil unexpectedly, using default path")
            return "/opt/local/bin/lpass"
        }
        return path
    }

    /// Returns path to lpass CLI without prompting (for availability checks)
    static var pathToCLISilent: String? {
        return pathToCLI(promptIfNotFound: false, returnNilIfNotFound: true)
    }

    private static func pathToCLI(promptIfNotFound: Bool, returnNilIfNotFound: Bool = false) -> String? {
        ...
        // Don't prompt if this is just an availability check
        guard promptIfNotFound else {
            usable = false
            return returnNilIfNotFound ? nil : normalPaths[0]
        }
    ...
    static func showNotLoggedInMessage() {
        ...
        case .alertFirstButtonReturn:
            // BUG-1619: Use nil coalescing instead of force unwrap for data encoding
            let commandData = "lpass login \(email)".data(using: .utf8) ?? Data()
            let window = iTermController.sharedInstance().openSingleUseLoginWindowAndWrite(commandData) { session in
    ...
    // BUG-401: Dialog text uses DashTerm2 branding
    ...
    private static func showCannotFindCLIMessage() -> Bool {
        ...
        alert.informativeText = "In order to use the LastPass integration, DashTerm2 needs to know where to find the CLI app named \"lpass\". Select Locate to provide its location."
        ...
        case .alertThirdButtonReturn:
            // BUG-1600: Use if-let instead of force unwrap for URL creation
            // BUG-1: Documentation URL updated to dashterm.com
            if let url = URL(string: "https://dashterm.com/lastpass-cli") {
