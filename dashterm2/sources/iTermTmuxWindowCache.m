...
- (instancetype)init {
    self = [super init];
    if (self) {
        _clients = [NSMutableArray arrayWithCapacity:2]; // Typically 1-2 tmux clients
...
- (NSArray<iTermTmuxWindowCacheWindowInfo *> *)hiddenWindows {
    NSMutableArray<iTermTmuxWindowCacheWindowInfo *> *results =
        [NSMutableArray arrayWithCapacity:8]; // Typical hidden windows
    [_clients enumerateObjectsUsingBlock:^(iTermTmuxCacheClient *_Nonnull client, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        // BUG-1083: Check for nil controller before dereferencing
        if (!controller) {
            return;
        }
        [client.sessions
            enumerateObjectsUsingBlock:^(iTermTmuxCacheSession *_Nonnull session, NSUInteger idx, BOOL *_Nonnull stop) {
                if (session.sessionObject.number != controller.sessionId) {
                    return;
                }
                [session.mutableWindows enumerateObjectsUsingBlock:^(iTermTmuxCacheWindow *_Nonnull window,
...
- (void)tmuxControllerSessionsDidChange:(NSNotification *)notification {
    ...
    client.sessions = [[controller.sessionObjects mapWithBlock:^id(iTermTmuxSessionObject *sessionObject) {
        ...
        session.mutableWindows = [NSMutableArray arrayWithCapacity:8]; // Populated async
...
- (void)tmuxControllerWindowOpened:(NSNotification *)notification {
    ...
    iTermTmuxCacheWindow *window = [self windowInClient:[self clientNamed:controller.clientName] number:wid];
    if (!window) {
        return;
    }
    [self invalidate];
...
- (void)tmuxControllerWindowClosed:(NSNotification *)notification {
    ...
    TmuxController *controller = params[1];
    iTermTmuxCacheWindow *window = [self windowInClient:[self clientNamed:controller.clientName] number:wid];
    iTermTmuxCacheSession *session = [self sessionInClient:[self clientNamed:controller.clientName] withWindow:window];
...
// BUG-f1419: Add bounds check to prevent crash when notification array is malformed
- (void)tmuxControllerWindowWasRenamed:(NSNotification *)notification {
    NSArray *params = notification.object;
    if (![params isKindOfClass:[NSArray class]] || params.count < 3) {
        DLog(@"tmuxControllerWindowWasRenamed: expected array with 3 elements, got %@", params);
        return;
    }
...
// BUG-f1420: Add bounds check to prevent crash when notification array is malformed
- (void)tmuxControllerSessionWasRenamed:(NSNotification *)notification {
    NSArray *params = notification.object;
    if (![params isKindOfClass:[NSArray class]] || params.count < 3) {
        DLog(@"tmuxControllerSessionWasRenamed: expected array with 3 elements, got %@", params);
        return;
    }
...
- (void)tmuxControllerRegistryDidChange:(NSNotification *)notification {
    NSMutableArray<iTermTmuxCacheClient *> *updated = [NSMutableArray arrayWithCapacity:_clients.count + 1];
    for (NSString *clientName in [[TmuxControllerRegistry sharedInstance] clientNames]) {
        ...
        // BUG-1103: Build sessions array and assign it to novel.sessions
        NSMutableArray<iTermTmuxCacheSession *> *novelSessions =
            [NSMutableArray arrayWithCapacity:controller.sessionObjects.count];
        for (iTermTmuxSessionObject *sessionObject in controller.sessionObjects) {
            ...
            [controller listWindowsInSessionNumber:sessionObject.number
                                          ...
                                          selector:@selector(didListWindows:forSession:)
                                            object:session];
            [novelSessions addObject:session];
        }
        novel.sessions = novelSessions;
        [updated addObject:novel];
    }
    // BUG-1103: Update _clients with the new client list
    _clients = updated;
    [self invalidate];
}
...
- (void)reallyInvalidate {
    _needsInvalidate = NO;
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermTmuxWindowCacheDidChange object:nil];
}
...
- (void)didListWindows:(TSVDocument *)doc forSession:(iTermTmuxCacheSession *)session {
    ...
    NSMutableArray *windows = [NSMutableArray arrayWithCapacity:doc.records.count];
    for (NSArray *record in doc.records) {
        ...
        // BUG-1082: Check length before substringFromIndex
        NSString *windowIdStr = [doc valueInRecord:record forField:@"window_id"];
        if (windowIdStr.length > 1) {
            window.number = [[windowIdStr substringFromIndex:1] intValue];
        } else {
            window.number = -1;
        }
