...
- (void)setCount:(NSInteger)count {
    // BUG-f1026: Guard against negative count instead of crashing - clamp to 0
    if (count < 0) {
        DLog(@"BUG-f1026: setCount: called with negative count %ld - clamping to 0", (long)count);
        count = 0;
    }
    if (count == _count) {
        return;
...
- (NSView *)newBubbleViewWithTextField:(NSTextField *)textField {
    ...
    NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:view.bounds xRadius:radius yRadius:radius];
    ...
    textField.frame = NSMakeRect((NSWidth(view.frame) - NSWidth(textField.frame)) / 2.0,
                                 (NSHeight(view.frame) - NSHeight(textField.frame)) / 2.0, NSWidth(textField.frame),
