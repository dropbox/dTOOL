...
- (NSString *)it_cachesDirectory {
    ...
    NSError *error = nil;
    NSString *path = [self findOrCreateDirectory:NSCachesDirectory
...
- (NSString *)applicationSupportDirectory {
    NSString *executableName = [[[NSBundle mainBundle] infoDictionary] objectForKey:(id)kCFBundleExecutableKey];
    NSError *error;
        ...
        dispatch_once(&onceToken, ^{
            [iTermWarning
                showWarningWithTitle:
                    [NSString
                        stringWithFormat:@"There was a problem finding or creating your application support directory. "
                                         @"DashTerm2 won't work very well until this problem is fixed.\n\nIt should be "
                                         @"at ~/Library/Application Support/DashTerm2.\n\nThe error was:\n%@",
...
- (NSString *)spacelessAppSupportCreatingLink {
    ...
    if (!created) {
        ...
        if ([attributes.fileType isEqual:NSFileTypeSymbolicLink]) {
            ...
            [[NSFileManager defaultManager] createSymbolicLinkAtPath:temp withDestinationPath:realFolder error:nil];
...
- (BOOL)customScriptsFolderIsValid:(NSString *)unexpanded {
    ...
    BOOL dir = NO;
    return (candidate && ![candidate containsString:@" "] && [self fileExistsAtPath:candidate isDirectory:&dir] && dir);
}
...
- (NSString *)scriptsPathWithoutSpaces {
    NSString *modernPath =
        self.customScriptsFolder
            ?: [[self spacelessAppSupportWithoutCreatingLink] stringByAppendingPathComponent:@"Scripts"];
    return modernPath;
}

- (NSString *)scriptsPathWithoutSpacesCreatingLink {
    NSString *modernPath =
        self.customScriptsFolder ?: [[self spacelessAppSupportCreatingLink] stringByAppendingPathComponent:@"Scripts"];
    return modernPath;
}
...
- (NSString *)pathToExistingDirectoryCreatingIfNeeded:(NSString *)dotdir error:(out NSError **)errorPtr {
    ...
    // Try to create ~/.config/dashterm2 if needed
    if (![self fileExistsAtPath:dotdir isDirectory:&isdir]) {
        ...
        if (errorPtr) {
            *errorPtr = [NSError errorWithDomain:@"com.dashterm.dashterm2.createDir"
                                            code:1
                                        userInfo:@{NSLocalizedDescriptionKey : @"File exists but is not a directory."}];
        }
        return nil;
    }
    return dotdir;
...
- (NSString *)pathToFirstDirectoryCreatingIfNeeded:(NSArray<NSString *> *)options error:(NSError **)errorPtr {
    NSMutableArray<NSString *> *errors = [NSMutableArray arrayWithCapacity:options.count];
...
- (NSString *)_homeDirectoryDotDir {
    ...
    NSString *dotConfigDashterm2 = [xdgConfigHome stringByAppendingPathComponent:@"dashterm2"];
    NSString *dotDashterm2 = [homedir stringByAppendingPathComponent:@".dashterm2"];
    NSArray<NSString *> *options = @[ dotConfigDashterm2, dotDashterm2, @".dashterm2-1" ];
        ...
        dispatch_once(&onceToken, ^{
            ...
            [iTermWarning
                showWarningWithTitle:[NSString
                                         stringWithFormat:@"There was a problem finding or creating the config "
                                                          @"directory. You can set “Settings > Advanced > Folder for "
                                                          @"config files“ to set a custom location for this directory. "
                                                          @"Until this is fixed, some features will be disabled.\n%@",
                                                          error.localizedDescription]
                             actions:@[ @"OK" ]
                           accessory:nil
...
- (BOOL)directoryIsWritable:(NSString *)dir {
    ...
    NSError *error = nil;
    [@"test" writeToFile:filename atomically:YES encoding:NSUTF8StringEncoding error:&error];
    if (error) {
        return NO;
    }
...
- (NSArray<NSString *> *)it_itemsInDirectory:(NSString *)path {
    NSMutableArray<NSString *> *results = [NSMutableArray arrayWithCapacity:16]; // Directory items
...
+ (NSString *)pathToSaveFileInFolder:(NSString *)destinationDirectory preferredName:(NSString *)preferredName {
    ...
    int retries = 0;
    do {
        NSString *name = retries > 0
                             ? [NSString stringWithFormat:@"%@ (%d).%@", preferredName.stringByDeletingPathExtension,
...
- (int)it_shmOpen:(NSString *)name oflag:(int)oflag mode:(mode_t)mode {
