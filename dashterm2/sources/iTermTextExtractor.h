...
typedef NS_ENUM(NSInteger, iTermTextExtractorNullPolicy) {
    ...
    kiTermTextExtractorNullPolicyTreatAsSpace,     // Treat midline nulls as spaces and a range of terminal nulls as a
                                                   // single space
...
- (VT100GridWindowedRange)rangeForBigWordAt:(VT100GridCoord)location maximumLength:(NSInteger)maximumLength;
...
- (void)enumerateCharsInRange:(VT100GridWindowedRange)range
                  supportBidi:(BOOL)supportBidi
                    charBlock:(BOOL (^NS_NOESCAPE _Nullable)(const screen_char_t *currentLine, screen_char_t theChar,
...
// See comment at
// -contentInRange:nullPolicy:pad:includeLastNewline:trimTrailingWhitespace:cappedAtSize:continuationChars:coords: If
// |convertNullToSpace| is YES then the string does not stop at a NULL character.
//
// If |coords| is non-nil it will be filled with NSValue*s in 1:1 correspondence with characters in
// the return value, giving VT100GridCoord's with their provenance.
...
- (VT100GridWindowedRange)rangeOfCoordinatesAround:(VT100GridCoord)coord
                                   maximumDistance:(int)maximumDistance
                                       passingTest:(BOOL (^)(screen_char_t *c, iTermExternalAttribute *_Nullable ea,
...
#pragma mark - For tests (class methods - no instance required)
