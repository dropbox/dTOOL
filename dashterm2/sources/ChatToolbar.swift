...
class ChatToolbar {
    ...
    init(dataSource: ChatToolbarDataSource) {
        ...
        do {
            // BUG-1760: Use nil coalescing with fallback instead of force unwrap
            let image = NSImage(systemSymbolName: SFSymbol.infoCircle.rawValue, accessibilityDescription: nil) ?? NSImage()
        ...
        do {
            // BUG-1766: Use nil coalescing with fallback instead of force unwrap
        ...
        do {
            ...
            // BUG-1767: Use nil coalescing with fallback instead of force unwrap
            let image = NSImage(
                systemSymbolName: SFSymbol.lightbulb.rawValue,
                accessibilityDescription: "Enable high-effort reasoning?")?.withSymbolConfiguration(smallerConfig) ?? NSImage()
...
extension ChatToolbar {
    ...
    func createFloatingView() -> NSView {
        ...
        // NSGlassEffectView requires macOS 26 SDK - use containerView as fallback
