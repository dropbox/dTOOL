...
static void SigChildHandler(int arg) {
    pid_t waitResult = Wait();
    // BUG-7262: Handle the case where waitpid returns -1 with ECHILD.
    // This can happen if the child was already reaped (race condition).
    // In this case, treat it the same as successful child exit - wake
    // up the select loop and exit gracefully.
    if (waitResult == gChildPid || (waitResult == -1 && errno == ECHILD)) {
        // Wake up the select loop and exit.
        write(gPipe[1], "", 1);
    } else {
        // Something weird happened.
        unlink(gPath);
        // BUG-2909: exit() is not async-signal-safe; use _exit() in signal handlers
        _exit(1);
    }
}
...
static int Initialize(char *path, pid_t childPid) {
    ...
    gPath = strdup(path);
    // We get this when DashTerm2 crashes. Ignore it.
    FDLog(LOG_DEBUG, "Installing SIGHUP handler.");
    signal(SIGHUP, SIG_IGN);

    // BUG-2915: Check pipe() return value - failed pipe could cause signal handler
    // to write to garbage fd
    if (pipe(gPipe) != 0) {
        FDLog(LOG_ERR, "pipe() failed: %s", strerror(errno));
        return -1;
    }
...
int iTermFileDescriptorServerRun(char *path, pid_t childPid, int connectionFd) {
    ...
    // BUG-2747: Close gPipe to prevent file descriptor leak on exit path.
    // Note: gPipe is only valid if Initialize() succeeded (rc == 0) since that's
    // where pipe() is called. If Initialize() failed early, gPipe may contain garbage.
    if (rc == 0) {
        close(gPipe[0]);
        close(gPipe[1]);
    }
