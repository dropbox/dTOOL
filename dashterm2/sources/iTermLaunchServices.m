...
- (instancetype)init {
    ...
    if (self) {
        ...
        _urlHandlersByGuid = [[NSMutableDictionary alloc] initWithCapacity:16];  // URL scheme handlers
...
// BUG-1170: List of system schemes that should never be overridden by DashTerm2
+ (NSSet<NSString *> *)protectedSchemes {
    ...
    dispatch_once(&onceToken, ^{
        schemes = [NSSet setWithArray:@[
            ...
            @"tel", @"sms",              // Phone/messaging
            @"facetime", @"facetime-audio", // FaceTime
            @"maps",                     // Apple Maps
            @"music", @"itmss", @"itms", // iTunes/Music
...
- (void)connectBookmarkWithGuid:(NSString*)guid toScheme:(NSString*)scheme {
    // BUG-1170: Prevent overriding protected system schemes
    NSString *lowercaseScheme = [scheme lowercaseString];
    if ([[self.class protectedSchemes] containsObject:lowercaseScheme]) {
        NSAlert *alert = [[[NSAlert alloc] init] autorelease];
        alert.messageText = [NSString stringWithFormat:@"Cannot set DashTerm2 as handler for %@", scheme];
        alert.informativeText = @"This scheme is protected and cannot be overridden.";
    ...
    if (appURL == nil) {
        NSAlert *alert = [[[NSAlert alloc] init] autorelease];
        alert.messageText = [NSString stringWithFormat:@"DashTerm2 is not the default handler for %@. "
                             @"Would you like to set DashTerm2 as the default handler?",
                             scheme];
        alert.informativeText = @"There is currently no handler.";
        [alert addButtonWithTitle:@"OK"];
        [alert addButtonWithTitle:@"Cancel"];
        set = ([alert runModal] == NSAlertFirstButtonReturn);
    } else if (![[[NSFileManager defaultManager] displayNameAtPath:[appURL path]] isEqualToString:@"DashTerm2"]) {
        NSString *theTitle = [NSString stringWithFormat:@"DashTerm2 is not the default handler for %@. "
                                                        @"Would you like to set DashTerm2 as the default handler?", scheme];
        NSAlert *alert = [[[NSAlert alloc] init] autorelease];
...
- (void)registerForDashTerm2Scheme {
    LSSetDefaultHandlerForURLScheme((CFStringRef)@"dashterm2",
                                    (CFStringRef)[[NSBundle mainBundle] bundleIdentifier]);
}
