...
static void iTermOrphanServerAdopterFindMonoServers(void (^completion)(NSArray<NSString *> *)) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:8]; // Orphan mono server paths
...
static void iTermOrphanServerAdopterFindMultiServers(void (^completion)(NSArray<NSString *> *)) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSMutableArray<NSString *> *result = [NSMutableArray arrayWithCapacity:8]; // Orphan multi server paths
        ...
        for (NSURL *url in enumerator) {
            if (![url.path.lastPathComponent stringMatchesGlobPattern:@"dashterm2-daemon-*.socket" caseSensitive:YES]) {
                continue;
            }
...
- (void)reallyOpenWindowWithOrphansWithCompletion:(void (^)(void))completion {
    ...
    for (NSString *path in _pathsOfMultiServers) {
        dispatch_group_enter(group);
        [self enqueueAdoptionsOfMultiServerOrphansWithPath:path
                                                completion:^{
                                                    dispatch_group_leave(group);
                                                }];
    }
...
- (void)adoptMonoServerOrphanWithPath:(NSString *)filename completion:(void (^)(PTYSession *))completion {
    ...
    if (serverConnection.ok) {
        DLog(@"Restore it");
        iTermGeneralServerConnection generalConnection = {.type = iTermGeneralServerConnectionTypeMono,
                                                          .mono = serverConnection};
        [self.delegate
            orphanServerAdopterOpenSessionForConnection:generalConnection
                                               inWindow:_window
                                             completion:^(PTYSession *session) {
                                                 // BUG-f1110: Replace assert with defensive check for main queue
                                                 if (dispatch_queue_get_label(dispatch_get_main_queue()) !=
                                                     dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL)) {
                                                     DLog(@"Warning: orphanServerAdopter completion called off main "
                                                          @"queue");
                                                     dispatch_async(dispatch_get_main_queue(), ^{
                                                         if (!self->_window) {
                                                             self->_window = [[iTermController sharedInstance]
                                                                 terminalWithSession:session];
                                                         }
                                                         completion(session);
                                                     });
                                                     return;
                                                 ...
                                                 if (!self->_window) {
                                                     self->_window =
                                                         [[iTermController sharedInstance] terminalWithSession:session];
                                                 }
                                                 completion(session);
                                             }];
    } else {
...
- (void)enqueueAdoptionsOfMultiServerOrphansWithPath:(NSString *)filename completion:(void (^)(void))completion {
    ...
    NSString *const prefix = @"dashterm2-daemon-";
    // BUG-f1111: Replace assert with guard to prevent crash on invalid filename
    if (![basename hasPrefix:prefix]) {
        DLog(@"Warning: filename '%@' does not have expected prefix '%@'", filename, prefix);
        completion();
        return;
    }
    ...
    [iTermMultiServerConnection
        getConnectionForSocketNumber:number
                    createIfPossible:NO
                            callback:[iTermThread.main
                                         newCallbackWithBlock:^(iTermMainThreadState *state,
                                                                iTermResult<iTermMultiServerConnection *> *result) {
                                             [result
                                                 handleObject:^(iTermMultiServerConnection *_Nonnull connection) {
...
- (void)didEstablishMultiserverConnection:(iTermMultiServerConnection *)connection
    ...
    for (iTermFileDescriptorMultiClientChild *child in children) {
        iTermGeneralServerConnection generalConnection = {.type = iTermGeneralServerConnectionTypeMulti,
                                                          .multi = {.pid = child.pid, .number = number}};
        dispatch_group_enter(group);
