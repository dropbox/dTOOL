// BUG-286: Terminal version report updated to DashTerm2
// BUG-74: TERM_PROGRAM environment variable updated to DashTerm2
...
+ (NSSet<NSString *> *)standardTerminals {
    ...
    dispatch_once(&onceToken, ^{
        terms = [NSSet setWithArray:@[
            @"xterm", @"xterm-new", @"xterm-256color", @"xterm+256color", @"xterm-kitty", @"iterm", @"dashterm2"
        ]];
    });
    return terms;
}
...
- (void)setTermType:(NSString *)term {
    ...
    if (ti.isValid) {
        iTermTerminfoString stringKeys[] = {
            iTermTerminfoString_key_left,  iTermTerminfoString_key_right, iTermTerminfoString_key_up,
            iTermTerminfoString_key_down,  iTermTerminfoString_key_home,  iTermTerminfoString_key_end,
            iTermTerminfoString_key_npage, iTermTerminfoString_key_ppage, iTermTerminfoString_key_f0,
            iTermTerminfoString_key_f1,    iTermTerminfoString_key_f2,    iTermTerminfoString_key_f3,
            iTermTerminfoString_key_f4,    iTermTerminfoString_key_f5,    iTermTerminfoString_key_f6,
            iTermTerminfoString_key_f7,    iTermTerminfoString_key_f8,    iTermTerminfoString_key_f9,
            iTermTerminfoString_key_f10,   iTermTerminfoString_key_f11,   iTermTerminfoString_key_f12,
            iTermTerminfoString_key_f13,   iTermTerminfoString_key_f14,   iTermTerminfoString_key_f15,
            iTermTerminfoString_key_f16,   iTermTerminfoString_key_f17,   iTermTerminfoString_key_f18,
            iTermTerminfoString_key_f19,   iTermTerminfoString_key_f20,   iTermTerminfoString_key_f21,
            iTermTerminfoString_key_f22,   iTermTerminfoString_key_f23,   iTermTerminfoString_key_f24,
            iTermTerminfoString_key_f25,   iTermTerminfoString_key_f26,   iTermTerminfoString_key_f27,
            iTermTerminfoString_key_f28,   iTermTerminfoString_key_f29,   iTermTerminfoString_key_f30,
            iTermTerminfoString_key_f31,   iTermTerminfoString_key_f32,   iTermTerminfoString_key_f33,
            iTermTerminfoString_key_f34,   iTermTerminfoString_key_f35,   iTermTerminfoString_key_backspace,
            iTermTerminfoString_key_btab,  iTermTerminfoString_tab,       iTermTerminfoString_key_dc,
            iTermTerminfoString_key_ic,    iTermTerminfoString_key_help};
...
- (NSData *)keyInsert {
    if (_keyStrings[TERMINFO_KEY_INS]) {
        return [NSData dataWithBytes:_keyStrings[TERMINFO_KEY_INS] length:strlen(_keyStrings[TERMINFO_KEY_INS])];
    } else {
...
- (NSData *)keyDelete:(NSEventModifierFlags)flags {
    ...
    if (_keyStrings[TERMINFO_KEY_DEL]) {
        return [NSData dataWithBytes:_keyStrings[TERMINFO_KEY_DEL] length:strlen(_keyStrings[TERMINFO_KEY_DEL])];
    } else {
...
- (NSData *)keyPageUp:(unsigned int)modflag {
    ...
    if (_keyStrings[TERMINFO_KEY_PAGEUP]) {
        ...
        theSuffix = [NSData dataWithBytes:KEY_PAGE_UP length:STATIC_STRLEN(KEY_PAGE_UP)];
    }
    NSMutableData *data = [[NSMutableData alloc] initWithCapacity:theSuffix.length + 1];
...
- (NSData *)keyPageDown:(unsigned int)modflag {
    ...
    if (_keyStrings[TERMINFO_KEY_PAGEDOWN]) {
        ...
        theSuffix = [NSData dataWithBytes:KEY_PAGE_DOWN length:STATIC_STRLEN(KEY_PAGE_DOWN)];
    }
    NSMutableData *data = [[NSMutableData alloc] initWithCapacity:theSuffix.length + 1];
    if (modflag & NSEventModifierFlagOption) {
        char esc = ESC;
...
- (NSData *)mouseReport:(int)button release:(bool)release coord:(VT100GridCoord)coord point:(NSPoint)point {
    switch (self.mouseFormat) {
        case MOUSE_FORMAT_XTERM_EXT: {
            ...
            NSString *string = [NSString stringWithFormat:@"\e[M%@%@%@", [NSString stringWithLongCharacter:32 + button],
                                                          [NSString stringWithLongCharacter:32 + coord.x],
                                                          [NSString stringWithLongCharacter:32 + coord.y]];
            return [string dataUsingEncoding:NSUTF8StringEncoding];
        }
        case MOUSE_FORMAT_URXVT:
            return [[NSString stringWithFormat:@"\033[%d;%d;%dM", 32 + button, coord.x, coord.y]
                dataUsingEncoding:NSUTF8StringEncoding];

        case MOUSE_FORMAT_SGR:
            return [[self reportForSGRButton:button release:release x:coord.x
                                           y:coord.y] dataUsingEncoding:NSUTF8StringEncoding];

        case MOUSE_FORMAT_SGR_PIXEL:
            return [[self reportForSGRButton:button release:release x:point.x
                                           y:point.y] dataUsingEncoding:NSUTF8StringEncoding];

        case MOUSE_FORMAT_XTERM:
        default:
            return [[NSString stringWithFormat:@"\033[M%c%c%c", 32 + button, MIN(255, 32 + coord.x),
                                               MIN(255, 32 + coord.y)] dataUsingEncoding:NSISOLatin1StringEncoding];
    }
    return [NSData data];
}
...
- (NSData *)mousePress:(int)button withModifiers:(unsigned int)modflag at:(VT100GridCoord)coord point:(NSPoint)point {
    ...
    cb = button & 3;
    if (button == MOUSE_BUTTON_SCROLLDOWN || button == MOUSE_BUTTON_SCROLLUP || button == MOUSE_BUTTON_SCROLLLEFT ||
        button == MOUSE_BUTTON_SCROLLRIGHT) {
    ...
    return [self mouseReport:cb
                       coord:VT100GridCoordMake((coord.x + 1), (coord.y + 1))
...
- (NSData *)reportDashTerm2Version {
    ...
    NSString *version = [NSString stringWithFormat:@"%c[DASHTERM2 %@n", ESC,
                                                   [[[[NSBundle mainBundle] infoDictionary]
                                                       objectForKey:@"CFBundleShortVersionString"] uppercaseString]];
...
- (NSData *)reportStatus {
    return [NSData dataWithBytes:REPORT_STATUS length:STATIC_STRLEN(REPORT_STATUS)];
}
...
- (NSData *)reportDeviceAttribute:(VT100OutputOptionalDeviceAttributes)attrs {
    ...
    // 18: Windowing capability (I think this means you need to support rectangle operations and DECSLRM; xterm also
    // mentions: DECSNLS, DECSCPP, DECSLPP) 21: Horizontal scrolling (based on mintty I think this means the SL and SR
    // codes) 22: ANSI color, VT525 (the vt525 was a color version of the vt520) 28: rectangular editing 29: ANSI text
    // locator
    ...
    VT100OutputPrimaryDAFeature vt100Features[] = {VT100OutputPrimaryDAFeature132Columns,
                                                   VT100OutputPrimaryDAFeaturePrinterPort};
    VT100OutputPrimaryDAFeature vt200Features[] = {VT100OutputPrimaryDAFeatureVT220,
    ...
    VT100OutputPrimaryDAFeature vt400Features[] = {VT100OutputPrimaryDAFeatureVT420,
                                                   VT100OutputPrimaryDAFeature132Columns,
                                                   VT100OutputPrimaryDAFeaturePrinterPort,
                                                   VT100OutputPrimaryDAFeatureSixelGraphics,
                                                   VT100OutputPrimaryDAFeatureSelectiveErase,
    ...
    NSMutableArray<NSString *> *values =
        [[[NSArray sequenceWithRange:NSMakeRange(0, count)] mapWithBlock:^id(NSNumber *anObject) {
...
- (NSData *)reportTertiaryDeviceAttribute {
    switch (_emulationLevel) {
        ...
        case iTermEmulationLevel400:
        case iTermEmulationLevel500:
            return [[NSString stringWithFormat:@"\eP!|%02X%02X%02X%02X\e\\", 'i', 'T', 'r', 'm']
                dataUsingEncoding:NSUTF8StringEncoding];
    }
    return nil;
}
...
- (NSData *)reportExtendedDeviceAttribute {
    ...
    NSString *reportString = [NSString stringWithFormat:@"%cP>|DashTerm2 %@%c\\", ESC, versionString, ESC];
...
- (NSData *)reportColor:(NSColor *)color atIndex:(int)index prefix:(NSString *)prefix {
    ...
    if ([iTermAdvancedSettingsModel oscColorReport16Bits]) {
        string =
            [NSString stringWithFormat:@"%c]%@%d;rgb:%04x/%04x/%04x%c\\", ESC, prefix, index,
                                       (int)([color redComponent] * 65535.0), (int)([color greenComponent] * 65535.0),
                                       (int)([color blueComponent] * 65535.0), ESC];
    } else {
        string = [NSString stringWithFormat:@"%c]%@%d;rgb:%02x/%02x/%02x%c\\", ESC, prefix, index,
                                            (int)([color redComponent] * 255.0), (int)([color greenComponent] * 255.0),
                                            (int)([color blueComponent] * 255.0), ESC];
    }
    return [string dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSData *)reportChecksum:(int)checksum withIdentifier:(int)identifier {
    // DCS Pid ! ~ D..D ST
    NSString *string = [NSString stringWithFormat:@"%cP%d!~%04x%c\\", ESC, identifier, (short)checksum, ESC];
    return [string dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSData *)windowResizeNotificationWithGridSize:(VT100GridSize)gridSize pixelSize:(NSSize)pixelSize {
    NSString *string = [NSString stringWithFormat:@"%c[48;%d;%d;%d;%dt", ESC, gridSize.height, gridSize.width,
...
- (NSData *)specialKey:(int)terminfo
    ...
    if (_keyStrings[terminfo] && mod == 0 && !isCursor && self.keypadMode) {
        // Application keypad mode.
        theSuffix = [NSData dataWithBytes:_keyStrings[terminfo] length:strlen(_keyStrings[terminfo])];
    } else {
        if (mod) {
            NSString *format = [NSString stringWithCString:cursorMod encoding:NSUTF8StringEncoding];
            NSString *string = [format stringByReplacingOccurrencesOfString:@"%d" withString:[@(mod) stringValue]];
            theSuffix = [string dataUsingEncoding:NSISOLatin1StringEncoding];
        } else {
            if (self.cursorMode) {
                theSuffix = [NSData dataWithBytes:cursorSet length:strlen(cursorSet)];
            } else {
                theSuffix = [NSData dataWithBytes:cursorReset length:strlen(cursorReset)];
            }
        }
    }
    NSMutableData *data = [[NSMutableData alloc] initWithCapacity:(prefix ? prefix.length : 0) + theSuffix.length];
    if (prefix) {
...
- (NSData *)reportCursorInformation:(VT100OutputCursorInformation)info {
    return [[NSString stringWithFormat:@"\eP1$u%d;%d;%d;%c;%c;%c;%d;%d;%c;%s%s%s%s\e\\", info.pr, info.pc, info.pp,
                                       info.srend, info.satt, info.sflag, info.pgl, info.pgr, info.scss, info.sdesig[0],
                                       info.sdesig[1], info.sdesig[2], info.sdesig[3]]
        dataUsingEncoding:NSUTF8StringEncoding];
}

VT100OutputCursorInformation VT100OutputCursorInformationCreate(int row,    // 1-based
                                                                int column, // 1-based
                                                                BOOL reverseVideo, BOOL blink, BOOL underline,
                                                                BOOL bold, BOOL autowrapPending,
...
VT100OutputCursorInformation VT100OutputCursorInformationFromString(NSString *string, BOOL vt500OrLater, BOOL *ok) {
    ...
    if (parts.count < 10 || [parts[3] length] < 1 || [parts[4] length] < 1 || [parts[5] length] < 1 ||
        [parts[6] length] < 1) {
        *ok = NO;
    ...
    *ok = YES;
    return (VT100OutputCursorInformation){.pr = [parts[0] intValue],
                                          .pc = [parts[1] intValue],
                                          .pp = [parts[2] intValue],
                                          .srend = srend,
                                          .satt = satt,
...
- (NSData *)reportSavedColorsUsed:(int)used largestUsed:(int)last {
...
- (NSData *)reportMemoryChecksum:(int)checksum id:(int)reqid {
    return [[NSString stringWithFormat:@"\eP%d!~%04X\e\\", MAX(1, reqid), checksum]
        dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSData *)reportVariableNamed:(NSString *)name value:(NSString *)variableValue {
    ...
    NSString *report = [NSString stringWithFormat:@"\e]1337;ReportVariable=%@\a", encodedValue ?: @""];
    return [report dataUsingEncoding:NSUTF8StringEncoding];
}

VT100Capabilities VT100OutputMakeCapabilities(BOOL compatibility24Bit, BOOL full24Bit, BOOL clipboardWritable,
                                              BOOL decslrm, BOOL mouse, BOOL DECSCUSR14, BOOL DECSCUSR56,
                                              BOOL DECSCUSR0, BOOL unicode, BOOL ambiguousWide, uint32_t unicodeVersion,
                                              BOOL titleStacks, BOOL titleSetting, BOOL bracketedPaste,
                                              BOOL focusReporting, BOOL strikethrough, BOOL overline, BOOL sync,
                                              BOOL hyperlinks, BOOL notifications, BOOL sixel, BOOL file) {
...
- (NSData *)reportCapabilities:(VT100Capabilities)capabilities {
    NSString *report = [NSString
        stringWithFormat:@"\e]1337;Capabilities=%@\a", [VT100Output encodedTermFeaturesForCapabilities:capabilities]];
    return [report dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSData *)reportPasteboard:(NSString *)pasteboard contents:(NSString *)string {
    NSString *report = [NSString
        stringWithFormat:@"\e]52;%@;%@\e\\", pasteboard, [string base64EncodedWithEncoding:NSUTF8StringEncoding]];
    return [report dataUsingEncoding:NSUTF8StringEncoding];
}

- (NSData *)reportDisplayedExtentOfSize:(VT100GridSize)size {
    NSString *report = [NSString stringWithFormat:@"\e[%@;%@;1;1;1\"w", @(size.height), @(size.width)];
    return [report dataUsingEncoding:NSUTF8StringEncoding];
}
