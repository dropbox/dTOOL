...
// key->NSImage
// BUG-f1429: Use NSCache instead of NSMutableDictionary for automatic memory pressure eviction

static NSCache<NSString *, NSImage *> *gLogoCache;
...
- (NSImage *)generatedImage {
    ...
    // BUG-f1429: NSCache uses objectForKey: instead of subscript
    NSImage *cachedImage = [gLogoCache objectForKey:key];
    if (cachedImage) {
        return cachedImage;
    }
    ...
    if (!gLogoCache) {
        // BUG-f1429: NSCache automatically evicts under memory pressure
        gLogoCache = [[NSCache alloc] init];
        gLogoCache.name = @"iTermLogoCache";
        gLogoCache.countLimit = 64;  // Reasonable limit for logo variants
    }
    [gLogoCache setObject:image forKey:key];
