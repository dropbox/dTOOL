...
class ConductorRegistry {
    ...
    // BUG-f668: Runtime enforcement of @MainActor constraint for Objective-C interop
    // The @MainActor attribute provides compile-time safety for Swift, but ObjC code may bypass this.
    // Log warning if called from wrong thread - the @MainActor annotation handles correctness.
    private func assertMainActor(function: String = #function) {
        if !Thread.isMainThread {
            DLog("BUG-f668: ConductorRegistry.\(function) called off main thread - @MainActor should handle this")
        }
    }
