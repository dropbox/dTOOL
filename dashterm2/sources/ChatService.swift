...
class ChatService {
    ...
    private func newAgent(forChatID chatID: String,
        ...
        // BUG-475: Handle case where agent already exists instead of crashing
        if let existingAgent = agents[chatID] {
            DLog("Warning: Agent already exists for chatID \(chatID), returning existing agent")
