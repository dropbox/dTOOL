...
class SSHFilePanelFileList: NSScrollView {
    ...
    // BUG-3441: Track temp files created during drag operations for cleanup
    ...
    required init?(coder: NSCoder) {
        // BUG-f812: Return nil instead of crashing for unused coder initializer
        DLog("SSHFilePanelFileListViewController init(coder:) is not supported")
        return nil
    }
    ...
    private func iconForFile(_ file: RemoteFile) -> NSImage? {
        ...
        case .file:
            ...
            // BUG-1773: Use nil coalescing with fallback instead of force unwrap for UTType
    ...
    // BUG-974: Use system locale-aware date/time formatting styles instead of US-specific formats.
    // This respects user's locale preferences for date order and 12/24-hour time.
    private func formatDate(_ date: Date?, columnWidth: CGFloat) -> String {
        guard let date = date else { return "--" }

        let formatter = DateFormatter()

        // Use localized styles that adapt to user's locale and preferences
        if columnWidth >= 186 {
            // Wide: full date with time (e.g., "October 31, 2025 at 12:30 PM" or "31 October 2025, 12:30")
            formatter.dateStyle = .long
            formatter.timeStyle = .short
        } else if columnWidth >= 147 {
            // Medium: medium date with time (e.g., "Oct 31, 2025, 12:30 PM" or "31 Oct 2025, 12:30")
            formatter.dateStyle = .medium
            formatter.timeStyle = .short
        } else if columnWidth >= 119 {
            // Narrow: short date with time (e.g., "10/31/25, 12:30 PM" or "31/10/25, 12:30")
            formatter.dateStyle = .short
            formatter.timeStyle = .short
        } else {
            // Very narrow: short date only (e.g., "10/31/25" or "31/10/25")
            formatter.dateStyle = .short
...
extension SSHFilePanelFileList: NSOutlineViewDataSource {
    ...
    // BUG-f523: Return safe fallback instead of crashing for unexpected item type
    func outlineView(_ outlineView: NSOutlineView, child index: Int, ofItem item: Any?) -> Any {
        if item == nil {
            // BUG-f609: Safely handle index out of bounds for root nodes
            guard index >= 0 && index < rootNodes.count else {
                DLog("SSHFilePanelFileList: Invalid rootNodes index \(index) for count \(rootNodes.count)")
                if !rootNodes.isEmpty { return rootNodes[0] }
                return self as Any
            }
            return rootNodes[index]
        } else if let node = item as? FileNode {
            // BUG-f610: Safely handle index out of bounds for sorted children
            guard index >= 0 && index < node.sortedChildren.count else {
                DLog("SSHFilePanelFileList: Invalid sortedChildren index \(index) for count \(node.sortedChildren.count)")
                if !node.sortedChildren.isEmpty { return node.sortedChildren[0] }
                return node as Any
            }
            return node.sortedChildren[index]
        }
        // BUG-487: Log warning but don't crash on unexpected item type
        // Return first root node if available, otherwise return self as a placeholder
        DLog("BUG-487: Unexpected item type in outlineView(_:child:ofItem:): \(type(of: item))")
        if !rootNodes.isEmpty {
            return rootNodes[0]
        }
        // Last resort - return self as Any to satisfy the return type requirement
        // This is a programmer error but we shouldn't crash
        return self as Any
    }
...
extension SSHFilePanelFileList: NSOutlineViewDelegate {
    ...
    func outlineView(_ outlineView: NSOutlineView, viewFor tableColumn: NSTableColumn?, item: Any) -> NSView? {
        ...
        // BUG-1799: Use local non-optional variable to avoid force unwraps
        if cellView == nil {
            let newCellView = NSTableCellView()
            newCellView.identifier = tableColumn?.identifier
            ...
            if columnID == .name {
                ...
                newCellView.imageView = imageView

                NSLayoutConstraint.activate([
                    imageView.leadingAnchor.constraint(equalTo: newCellView.leadingAnchor, constant: 4),
                    imageView.centerYAnchor.constraint(equalTo: newCellView.centerYAnchor),
            ...
            newCellView.addSubview(textField)
            newCellView.textField = textField

            // Set up constraints based on column
            if columnID == .name {
                NSLayoutConstraint.activate([
                    textField.leadingAnchor.constraint(equalTo: newCellView.leadingAnchor, constant: 24),
                    textField.trailingAnchor.constraint(equalTo: newCellView.trailingAnchor, constant: -4),
                    textField.centerYAnchor.constraint(equalTo: newCellView.centerYAnchor)
                ])
            } else {
                NSLayoutConstraint.activate([
                    textField.leadingAnchor.constraint(equalTo: newCellView.leadingAnchor, constant: 4),
                    textField.trailingAnchor.constraint(equalTo: newCellView.trailingAnchor, constant: -4),
                    textField.centerYAnchor.constraint(equalTo: newCellView.centerYAnchor)
                ])
            }
        ...
        case .dateModified:
            // BUG-1800: Use optional chaining instead of force unwrap
            let columnWidth = tableColumn.flatMap { outlineView.tableColumn(withIdentifier: $0.identifier)?.width } ?? 160
    ...
    private func createTemporaryFile(for node: FileNode) -> URL? {
        ...
        do {
            ...
            // BUG-3441: Track temp files for cleanup when drag ends
            dragTempFileURLs.append(tempFileURL)
            return tempFileURL
        } catch {
    ...
    // BUG-3441: Clean up temporary files created during drag operations
    private func cleanupDragTempFiles() {
        for url in dragTempFileURLs {
            do {
                if FileManager.default.fileExists(atPath: url.path) {
                    try FileManager.default.removeItem(at: url)
                    DLog("Cleaned up drag temp file: \(url)")
                }
            } catch {
                DLog("Failed to cleanup drag temp file \(url): \(error)")
            }
        }
    ...
    func outlineView(_ outlineView: NSOutlineView, draggingSession session: NSDraggingSession, endedAt screenPoint: NSPoint, operation: NSDragOperation) {
        ...
        // BUG-3441: Clean up temporary files created during drag operation
...
class SSHFilePanelFileListOutlineView: NSOutlineView {
    ...
    // RC-012: Use NSKeyValueObservation tokens for automatic cleanup instead of legacy KVO.
    // Tokens automatically invalidate when deallocated, preventing observer leaks.
    private var columnWidthObservations: [NSTableColumn: NSKeyValueObservation] = [:]

    init() {
        super.init(frame: .zero)
    }

    // RC-012: deinit no longer needs to manually remove observers.
    // NSKeyValueObservation tokens are automatically invalidated when the dictionary is deallocated.

    override func addTableColumn(_ tableColumn: NSTableColumn) {
        super.addTableColumn(tableColumn)
        // RC-012: Use modern KVO with observation token for automatic cleanup
        let observation = tableColumn.observe(\.width, options: [.new, .old]) { [weak self] column, _ in
            self?.onColumnWidthChange?(column)
        }
        columnWidthObservations[tableColumn] = observation
    }

    // RC-012: Remove observation token when column is removed
    override func removeTableColumn(_ tableColumn: NSTableColumn) {
        // Invalidate the observation token (automatic cleanup, but explicit removal keeps dict clean)
        columnWidthObservations.removeValue(forKey: tableColumn)
        super.removeTableColumn(tableColumn)
    }

    required init?(coder: NSCoder) {
        // BUG-f813: Return nil instead of crashing for unused coder initializer
        DLog("FileEntryTableCellView init(coder:) is not supported")
        return nil
    }
}
