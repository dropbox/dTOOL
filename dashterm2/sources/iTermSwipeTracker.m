...
- (BOOL)handleEvent:(NSEvent *)event {
    ...
    // Pre-allocate for expected number of events during swipe tracking (typically 10-50)
    NSMutableArray<NSString *> *log = [NSMutableArray arrayWithCapacity:32];
...
- (BOOL)internalHandleEvent:(NSEvent *)event {
    ...
    iTermScrollWheelStateMachineStateTransition transition = {.before = _stateMachine.state};
...
- (BOOL)createStateForEventIfNeeded:(NSEvent *)event
    ...
    if (transition.before == iTermScrollWheelStateMachineStateStartDrag ||
        ...
        DLog(@"Can't create state for transition %@ -> %@", @(transition.before), @(transition.after));
        return NO;
    }
...
- (void)updateTimer {
    ...
    DLog(@"Schedule timer");
    _timer = [[iTermGCDTimer alloc] initWithInterval:1.0 / 60 target:self selector:@selector(update:)];
}
