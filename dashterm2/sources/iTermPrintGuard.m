...
- (BOOL)haveTriedToPrintRecently {
    // BUG-10088: Fixed order of operations. Read timestamp first, save to local,
    // update ivar, then compute result. This ensures the check uses the old value.
    // Note: This class is only accessed from the main thread, so thread-safety
    // is not a concern here - the race condition in the backlog refers to
    // a potential semantic issue with the order of operations.
