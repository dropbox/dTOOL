...
static Boolean iTermHighlightMapKeyEqual(const void *value1, const void *value2) {
    return value1 == value2;
}

static CFHashCode iTermHighlightMapKeyHash(const void *value) {
    return (CFHashCode)(uintptr_t)value;
}

static const CFDictionaryKeyCallBacks kHighlightMapKeyCallbacks = {
    0,
    NULL,
    NULL,
    NULL,
    iTermHighlightMapKeyEqual,
    iTermHighlightMapKeyHash
};

static inline const void *iTermHighlightMapKeyForLine(long long line) {
    uint64_t zigZag = ((uint64_t)line << 1) ^ ((uint64_t)(line >> 63));
    return (const void *)(uintptr_t)(zigZag + 1);
}

static inline NSMutableData *iTermHighlightMapGet(CFMutableDictionaryRef map, long long line) {
    const void *key = iTermHighlightMapKeyForLine(line);
    return (__bridge NSMutableData *)CFDictionaryGetValue(map, key);
}

static inline void iTermHighlightMapSet(CFMutableDictionaryRef map,
                                        long long line,
                                        NSMutableData *data) {
    const void *key = iTermHighlightMapKeyForLine(line);
    CFDictionarySetValue(map, key, (__bridge const void *)data);
}

static inline void iTermHighlightMapRemove(CFMutableDictionaryRef map, long long line) {
    const void *key = iTermHighlightMapKeyForLine(line);
...
@implementation iTermFindOnPageHelper {
    ...
    // Maps an absolute line number to an NSData bit array with one bit per
    // cell indicating whether that cell is a match.
    CFMutableDictionaryRef _highlightMap;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _highlightMap = CFDictionaryCreateMutable(kCFAllocatorDefault,
                                                  0,
                                                  &kHighlightMapKeyCallbacks,
    ...
    [_searchResults enumerateObjectsUsingBlock:^(SearchResult * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.isExternal) {
            if (!NSLocationInRange(obj.externalAbsY, absLineRange)) {
                ...
                iTermHighlightMapRemove(_highlightMap, obj.externalAbsY);
            ...
            if (!NSLocationInRange(obj.internalAbsStartY, absLineRange) &&
                ...
                iTermHighlightMapRemove(_highlightMap, obj.internalAbsStartY);
            }
        }
    }];
    if (indexes.count) {
...
- (void)clearHighlights {
    ...
    CFDictionaryRemoveAllValues(_highlightMap);
...
- (BOOL)continueFind:(double *)progress
  ...
  overflowAdjustment:(long long)overflowAdjustment {
    ...
    // BUG-f1014: Gracefully handle when find is not in progress instead of crashing
    if (![self findInProgress]) {
        DLog(@"BUG-f1014: continueFinding called when findInProgress is NO");
        *progress = 1;
        return NO;
    }
    // Typically get multiple results per search iteration
    NSMutableArray<SearchResult *> *newSearchResults = [[NSMutableArray alloc] initWithCapacity:32];
    if (_findInProgress) {
...
- (void)addSearchResult:(SearchResult *)searchResult width:(int)width {
    ...
    if (!searchResult.isExternal) {
        for (long long y = searchResult.internalAbsStartY; y <= searchResult.internalAbsEndY; y++) {
            NSMutableData *data = iTermHighlightMapGet(_highlightMap, y);
            BOOL set = NO;
            if (!data) {
            ...
            if (set) {
                iTermHighlightMapSet(_highlightMap, y, data);
            }
        }
    }
}
...
- (BOOL)selectNextResultForward:(BOOL)forward
    ...
    // BUG-3778 fix: Early return when no valid results to avoid index out of bounds
    ...
    if (start >= NSMaxRange(validRange)) {
        if (stride > 0) {
            ...
            // validRange.length > 0 guaranteed by early return above
            start = (int)(NSMaxRange(validRange) - 1);
        }
    }
    for (int j = 0, i = start; !found && j < validRange.length; j++) {
        ...
        // BUG-f1073: Replace assert with guard - found being true here indicates logic error
        if (found) {
            DLog(@"BUG-f1073: found was unexpectedly true before checking range");
            continue;
        }
        if (_findCursor.type == FindCursorTypeExternal) {
...
- (void)removeHighlightsInRange:(NSRange)range {
    for (NSUInteger o = 0; o < range.length; o++) {
        iTermHighlightMapRemove(_highlightMap, range.location + o);
    }
}

- (NSData *)highlightDataForAbsoluteLine:(long long)absoluteLine {
    return iTermHighlightMapGet(_highlightMap, absoluteLine);
}

- (BOOL)hasHighlightedMatches {
    return CFDictionaryGetCount(_highlightMap) > 0;
}

- (NSInteger)smallestIndexOfLastSearchResultWithYLessThan:(NSInteger)query {
    // Guard against empty search results (BUG-10318)
    ...
    // Protect against underflow and out-of-bounds (BUG-10318)
    if (index < 0 || index >= (NSInteger)_searchResults.count) {
        return NSNotFound;
    }
    while (_searchResults[index].safeAbsStartY >= query) {
        if (index + 1 >= (NSInteger)_searchResults.count) {
            return NSNotFound;
        }
        index++;
    }
