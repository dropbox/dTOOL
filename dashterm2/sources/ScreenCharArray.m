...
+ (instancetype)emptyLineOfLength:(int)length {
    // Pre-allocate exact size for screen_char_t array
    NSMutableData *data = [NSMutableData dataWithCapacity:length * sizeof(screen_char_t)];
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
    ...
    if (self) {
        ...
        self = [self initWithData:dictionary[ScreenCharArrayKeyData]
                                      ?: [NSData dataWithBytes:&placeholder length:sizeof(placeholder)]
        ...
        _bidiInfo = [[iTermBidiDisplayInfo alloc]
            initWithDictionary:[NSDictionary castFrom:dictionary[ScreenCharArrayKeyBidiInfo]]];
    }
    return self;
}
...
- (instancetype)initWithData:(NSData *)data
       includingContinuation:(BOOL)includingContinuation
                    metadata:(iTermImmutableMetadata)metadata
                continuation:(screen_char_t)continuation {
    // BUG-f1001: Gracefully handle nil data instead of crashing
    if (data == nil || data.bytes == nil) {
        DLog(@"BUG-f1001: initWithData called with nil data or nil bytes");
        return nil;
    }
    self = [super init];
    if (self) {
        ...
        // BUG-f1001: Validate data length instead of crashing
        NSUInteger expectedLength = (includingContinuation ? (_length + 1) : _length) * sizeof(screen_char_t);
        if (expectedLength != data.length) {
            DLog(@"BUG-f1001: Data length mismatch: expected %lu, got %lu", (unsigned long)expectedLength,
...
- (instancetype)initWithCopyOfLine:(const screen_char_t *)line
                      ...
                      continuation:(screen_char_t)continuation
                          bidiInfo:(iTermBidiDisplayInfo *)bidiInfo {
    // BUG-2161: Use iTermMemdup for safe allocation with overflow protection
    screen_char_t *copy = iTermMemdup(line, length, sizeof(*line));
...
- (instancetype)initWithCopyOfLine:(const screen_char_t *)line
                      ...
                      continuation:(screen_char_t)continuation
                          bidiInfo:(iTermBidiDisplayInfo *)bidiInfo {
    // BUG-2162: Use iTermMemdup for safe allocation with overflow protection
    screen_char_t *copy = iTermMemdup(line, length, sizeof(*line));
    self = [self initWithLine:copy length:length continuation:continuation];
    if (self) {
        _shouldFreeOnRelease = YES;
...
- (instancetype)initWithLine:(const screen_char_t *)line length:(int)length continuation:(screen_char_t)continuation {
    return [self initWithLine:line length:length metadata:iTermImmutableMetadataDefault() continuation:continuation];
}
...
- (instancetype)initWithLine:(const screen_char_t *)line
                    ...
                    metadata:(iTermImmutableMetadata)metadata
                continuation:(screen_char_t)continuation {
    // BUG-f1002: Gracefully handle nil line instead of crashing
    if (line == nil) {
        DLog(@"BUG-f1002: initWithLine called with nil line");
        return nil;
    }
    self = [super init];
    if (self) {
...
- (instancetype)initWithLine:(const screen_char_t *)line
    ...
    iTermMetadata metadata = {
        .timestamp = [date timeIntervalSinceReferenceDate], .externalAttributes = nil, .rtlFound = rtlFound};
...
- (NSString *)description {
    if (_metadata.externalAttributes) {
        return [NSString stringWithFormat:@"<%@: %p value=\"%@\" eol=%@ ea=%@>", NSStringFromClass([self class]), self,
                                          ScreenCharArrayToStringDebug(_line, _length), @(self.eol),
                                          iTermImmutableMetadataGetExternalAttributesIndex(_metadata)];
    } else {
        return [NSString stringWithFormat:@"<%@: %p value=\"%@\" eol=%@>", NSStringFromClass([self class]), self,
                                          ScreenCharArrayToStringDebug(_line, _length), @(self.eol)];
    }
}
...
- (NSString *)stringValueIncludingEmbeddedNulls {
    ...
    NSMutableString *result = [[NSMutableString alloc] initWithCapacity:usedLength];
    const screen_char_t *line = self.line;
...
- (NSAttributedString *)attributedStringValueWithAttributeProvider:
    ...
    for (int i = 0; i < self.length; i++) {
        ...
        [result iterm_appendString:string withAttributes:attributeProvider(c, eaindex[i])];
    }
    return result;
}
...
- (ScreenCharArray *)screenCharArrayByAppendingScreenCharArray:(ScreenCharArray *)other {
    ...
    // BUG-2161: Use iTermCalloc for safe allocation with overflow protection
    screen_char_t *copy = iTermCalloc(combinedLength, sizeof(screen_char_t));
    ...
    id<iTermExternalAttributeIndexReading> appendage =
        iTermImmutableMetadataGetExternalAttributesIndex(other->_metadata);
    iTermExternalAttributeIndex *eaIndex = [iTermExternalAttributeIndex concatenationOf:originalIndex
                                                                                 length:_length
                                                                                   with:appendage
                                                                                 length:other->_length];
    iTermMetadata combined;
    iTermMetadataInit(&combined, _metadata.timestamp, other->_metadata.rtlFound, eaIndex);
...
- (ScreenCharArray *)paddedToLength:(int)length eligibleForDWC:(BOOL)eligibleForDWC {
    ...
    // BUG-1905: Check for allocation failure (can happen with very large sizes or OOM)
    if (!data) {
        ELog(@"BUG-1905: Failed to allocate data of size %lu in paddedToLength:",
             (unsigned long)(sizeof(screen_char_t) * (length + 1)));
        return self;
    }
    ...
    if (eol == EOL_SOFT && buffer[length - 1].code == 0 && eligibleForDWC) {
...
- (ScreenCharArray *)screenCharArrayBySettingCharacterAtIndex:(int)i to:(screen_char_t)c {
    // BUG-f1003: Gracefully handle out-of-bounds index instead of crashing
    if (i < 0 || i >= self.length) {
        DLog(@"BUG-f1003: screenCharArrayBySettingCharacterAtIndex called with invalid index %d (length=%d)", i,
             self.length);
        return nil;
    }
...
- (void)makeSafe {
    if (_data != nil) {
        // BUG-f1004: Gracefully handle inconsistent state instead of crashing
        if (_line == nil) {
            DLog(@"BUG-f1004: makeSafe called with non-nil _data but nil _line");
        }
        return;
    }
    NSMutableData *mutableData = [[NSMutableData alloc] initWithLength:(_length + 1) * sizeof(screen_char_t)];
    // BUG-1905: Check for allocation failure
    if (!mutableData) {
        ELog(@"BUG-1905: Failed to allocate mutableData of size %lu in makeDataMutable",
             (unsigned long)((_length + 1) * sizeof(screen_char_t)));
        return;
    }
    ...
    _line = _data.bytes;
    _shouldFreeOnRelease = NO;
    // BUG-f1004: Gracefully handle allocation failure instead of crashing
    if (_line == nil) {
        DLog(@"BUG-f1004: makeSafe resulted in nil _line after allocation");
    }
}
...
- (void)appendScreenCharArray:(ScreenCharArray *)sca {
    ...
    // BUG-1905: Check for allocation failure
    if (!data) {
        ELog(@"BUG-1905: Failed to allocate data of size %lu in appendScreenCharArray:",
             (unsigned long)((self.length + sca.length + 1) * sizeof(screen_char_t)));
        return;
    }
...
- (void)ensureContinuationMarkAppended {
    if (_data.length >= (_length + 1) * sizeof(screen_char_t)) {
        ...
        NSData *temp = [NSData dataWithBytesNoCopy:(void *)_line
                                            length:_length * sizeof(screen_char_t)
                                      freeWhenDone:NO];
...
- (void)appendString:(NSString *)string style:(screen_char_t)c continuation:(screen_char_t)continuation {
    ...
    // BUG-1905: Check for allocation failure
    if (!storage) {
        ELog(@"BUG-1905: Failed to allocate storage of size %lu in appendString:style:continuation:",
             (unsigned long)(string.length * 2 * sizeof(screen_char_t)));
        return;
    }
    int length = string.length;
    StringToScreenChars(string, (screen_char_t *)storage.mutableBytes, c, c, &length, NO, NULL, NULL,
...
- (void)appendString:(NSString *)string fg:(screen_char_t)fg bg:(screen_char_t)bg {
    ...
    // BUG-1905: Check for allocation failure
    if (!storage) {
        ELog(@"BUG-1905: Failed to allocate storage of size %lu in appendString:fg:bg:",
             (unsigned long)(sizeof(screen_char_t) * string.length * 3));
        return;
    }
    int len = 0;
    StringToScreenChars(string, (screen_char_t *)storage.mutableBytes, fg, bg, &len, NO, NULL, NULL,
                        iTermUnicodeNormalizationNone, 9, NO, NULL);
...
- (void)deleteRange:(NSRange)range {
    ...
    screen_char_t *line = self.mutableLine;
    memmove(line + range.location, line + NSMaxRange(range), (self.length - NSMaxRange(range)) * sizeof(screen_char_t));
...
- (void)insert:(ScreenCharArray *)sca atIndex:(int)index {
    // BUG-1904: Protect against integer overflow in size calculation
    // Use NSUInteger for intermediate calculation to detect overflow
    NSUInteger combinedLength = (NSUInteger)_length + (NSUInteger)sca.length;
    NSUInteger requiredBytes = combinedLength * sizeof(screen_char_t);
    // Check for overflow: if multiplying by sizeof resulted in a smaller number, overflow occurred
    if (combinedLength > 0 && requiredBytes / sizeof(screen_char_t) != combinedLength) {
        ELog(@"BUG-1904: Integer overflow in insert:atIndex: - combined length %lu would overflow",
             (unsigned long)combinedLength);
        return;
    }
    NSMutableData *data = [NSMutableData uninitializedDataWithLength:requiredBytes];
    // BUG-1905: Check for allocation failure
    if (!data) {
        ELog(@"BUG-1905: Failed to allocate data of size %lu in insert:atIndex:", (unsigned long)requiredBytes);
        return;
    }
    int offset = 0;
    ...
    [eaIndex insertFrom:sca.eaIndex sourceRange:NSMakeRange(0, sca.length) atIndex:index];
}

- (void)setMetadata:(iTermMetadata)metadata {
    // Release old metadata, retain new, then make immutable copy
    // Memory management verified correct: release old before retain new
    iTermImmutableMetadataRelease(_metadata);
...
- (void)copyRange:(NSRange)sourceRange from:(ScreenCharArray *)source destinationIndex:(int)destinationIndex {
    ...
    screen_char_t *line = [self mutableLine];
    memmove(line + destinationIndex, source.line + sourceRange.location, sourceRange.length * sizeof(screen_char_t));
    ...
    if (srcEA) {
        ...
        [destEA copyFrom:srcEA source:sourceRange.location destination:destinationIndex count:sourceRange.length];
    } else if (destEA) {
