                                       ...
                                       windowTouchesRightMargin:(BOOL)windowTouchesRightMargin
{
    ...
    // Typical word length is ~20 characters
    NSMutableArray<NSNumber *> *indexesInSuffix = [[NSMutableArray alloc] initWithCapacity:32];
    // String composed of the characters found to be in the word, excluding private range characters.
    NSMutableString *stringFromLocation = [[NSMutableString alloc] initWithCapacity:64];
    NSMutableArray<NSValue *> *coords = [[NSMutableArray alloc] initWithCapacity:32];
...
- (iTermWordExtractorReverseSearchResult)searchReverseInRange:(VT100GridCoordRange)theRange
    ...
    // Typical word length is ~20 characters
    NSMutableArray *substrings = [[NSMutableArray alloc] initWithCapacity:32];
    ...
    NSMutableArray<NSNumber *> *stringLengthsInPrefix = [[NSMutableArray alloc] initWithCapacity:32];
    NSMutableArray<NSValue *> *coords = [[NSMutableArray alloc] initWithCapacity:32];
    __block NSInteger iterations = 0;
...
- (VT100GridWindowedRange)languageSpecificWindowedRangeWithStringBefore:(NSString *)stringBeforeLocation
    ...
    NSMutableArray<NSNumber *> *indexes = [[NSMutableArray alloc] initWithCapacity:coords.count];
...
- (NSString *)fastStringAt:(VT100GridCoord)location {
    ...
    // Fast string extraction typically gets short strings (maxLength = 20)
    NSMutableString *word = [[NSMutableString alloc] initWithCapacity:maxLength];
...
- (iTermTextExtractorClass)classForCharacter:(screen_char_t)theCharacter
    ...
    if (!theCharacter.complexChar && !theCharacter.image) {
        ...
        } else if (theCharacter.code == DWC_RIGHT || theCharacter.code == DWC_SKIP) {  // BUG-7317: complexChar was wrong property
