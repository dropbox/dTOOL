...
extension Conductor: SSHEndpoint {
    ...
    /// RC-035 ROOT CAUSE FIX: Changed completion signature to include optional error.
    /// Previously callers received empty array on error, which was indistinguishable from
    /// "directory is empty". Now callers can distinguish error (non-nil error) from empty
    /// directory (nil error, empty array).
    @objc(fetchDirectoryListingOfPath:queue:completion:)
    @MainActor
    func objcListFiles(_ path: String,
                       queue: DispatchQueue,
                       completion: @escaping ([iTermDirectoryEntry], Error?) -> ()) {
        Task {
            do {
                ...
                DLog("objcListFiles failed for path '\(path)': \(error.localizedDescription)")
                if let sshError = error as? SSHEndpointException {
                    DLog("SSH endpoint error: \(sshError)")
                } else if let fileProviderError = error as? iTermFileProviderServiceError {
                    DLog("File provider error: \(fileProviderError)")
                }
                queue.async {
                    completion([], error)
                }
    ...
    func stat(_ path: String,
        ...
        Task {
            do {
                ...
                // BUG-2665: Log error details since completion only gets error code.
                // Error code 1 loses error type (connection closed vs file not found vs permission denied).
                DLog("stat failed for path '\(path)': \(error.localizedDescription)")
                if let sshError = error as? SSHEndpointException {
                    DLog("SSH endpoint error type: \(sshError)")
                } else if let fileProviderError = error as? iTermFileProviderServiceError {
                    DLog("File provider error type: \(fileProviderError)")
                }
                queue.async {
                    completion(1, nil)
                }
