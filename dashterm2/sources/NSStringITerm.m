...
NS_INLINE BOOL iTermCharacterSetContainsScalar(NSCharacterSet *set, UTF32Char scalar) {
    if (!set) {
        return NO;
    }
    if (scalar <= 0xffff) {
        return [set characterIsMember:(unichar)scalar];
    }
    return [set longCharacterIsMember:scalar];
}

NS_INLINE UTF32Char iTermScalarEndingAtIndex(NSString *string, NSUInteger end, NSUInteger *utf16Length) {
    if (end == 0) {
        if (utf16Length) {
            *utf16Length = 0;
        }
    ...
    unichar low = [string characterAtIndex:end - 1];
    if (CFStringIsSurrogateLowCharacter(low) && end >= 2) {
        unichar high = [string characterAtIndex:end - 2];
...
NS_INLINE UTF32Char iTermScalarStartingAtIndex(NSString *string, NSUInteger index, NSUInteger length,
    ...
    if (CFStringIsSurrogateHighCharacter(high) && index + 1 < length) {
...
static NSCharacterSet *iTermWhitespaceAndNewlineCharacterSet(void) {
...
static NSCharacterSet *iTermWhitespaceCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
static NSCharacterSet *iTermNewlineCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [NSCharacterSet newlineCharacterSet];
    });
    return set;
}

// Cached inverted character sets - avoid per-call allocation overhead
static NSCharacterSet *iTermNonWhitespaceCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [[NSCharacterSet whitespaceCharacterSet] invertedSet];
    });
    return set;
}

static NSCharacterSet *iTermNonWhitespaceAndNewlineCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
// Cached inverted decimal digit set for isNumeric checks
static NSCharacterSet *iTermNonDecimalDigitCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
// Cached inverted base64 character sets for mayBeBase64Encoded checks
static NSCharacterSet *iTermNonBase64CharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [[NSCharacterSet it_base64Characters] invertedSet];
    });
    return set;
}

static NSCharacterSet *iTermNonURLSafeBase64CharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [[NSCharacterSet it_urlSafeBse64Characters] invertedSet];
    });
    return set;
}

static NSCharacterSet *iTermURLSchemeCharacterSet(void) {
    static NSCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSMutableCharacterSet *mutableSet = [[NSCharacterSet alphanumericCharacterSet] mutableCopy];
        [mutableSet addCharactersInString:@"-"]; // chrome-devtools:, x-man-page:
        set = [mutableSet copy];
    });
    return set;
}

static NSCharacterSet *iTermTrailingPunctuationCharacterSet(void) {
    static os_unfair_lock lock = OS_UNFAIR_LOCK_INIT;
    static NSString *cachedCharacters;
    static NSCharacterSet *cachedSet;
    NSString *characters = [iTermAdvancedSettingsModel trailingPunctuationMarks] ?: @"";
    os_unfair_lock_lock(&lock);
    if (![cachedCharacters isEqualToString:characters]) {
        cachedCharacters = [characters copy];
        cachedSet = [NSCharacterSet characterSetWithCharactersInString:cachedCharacters];
    }
    NSCharacterSet *result = cachedSet;
    os_unfair_lock_unlock(&lock);
    return result;
}

NS_INLINE BOOL iTermClosingCharacterFor(unichar opening, unichar *closing) {
    switch (opening) {
        case '(':
            *closing = ')';
            return YES;
...
static BOOL iTermRemoveEnclosingPunctuationInRange(NSString *string, NSUInteger *start, NSUInteger *end) {
    if (*end <= *start) {
        return NO;
    }
    unichar candidate = [string characterAtIndex:*start];
    unichar closing = 0;
    if (!iTermClosingCharacterFor(candidate, &closing)) {
        return NO;
    }
    for (NSUInteger index = *start + 1; index < *end; index++) {
        if ([string characterAtIndex:index] == closing) {
            *start = *start + 1;
            *end = index;
            return YES;
        }
...
NS_INLINE NSUInteger iTermTrimSingleTrailingCharacter(NSString *string, NSUInteger lowerBound, NSUInteger upperBound,
                                                      NSCharacterSet *characterSet) {
    if (upperBound <= lowerBound || !characterSet) {
        return upperBound;
    }
    NSUInteger scalarLength = 0;
    UTF32Char scalar = iTermScalarEndingAtIndex(string, upperBound, &scalarLength);
    if (scalarLength == 0) {
        return upperBound;
    }
    if (iTermCharacterSetContainsScalar(characterSet, scalar)) {
        return upperBound - scalarLength;
    }
    return upperBound;
}
...
+ (BOOL)isDoubleWidthCharacter:(int)unicode
    ...
    // Use fast lookup table for full-width and ambiguous checks.
    // This is 10-20x faster than NSCharacterSet lookups (~2-5ns vs ~50-100ns).
    if (iTermIsDoubleWidthFast((UTF32Char)unicode, ambiguousIsDoubleWidth ? 1 : 0, (int)version)) {
        return YES;
    }
    // Flag characters still use NSCharacterSet (sparse data not in lookup table)
...
+ (NSString *)stringFromPasteboard {
    ...
    if (gDebugLogging) {
        ...
        for (NSPasteboardItem *item in [[NSPasteboard generalPasteboard] pasteboardItems]) {
            ...
            for (NSPasteboardType type in item.types) {
                DLog(@"  For type %@: plist=%@ data=%@ string=%@", type, [item propertyListForType:type],
                     [item dataForType:type], [item stringForType:type]);
            }
        }
    ...
    const BOOL remote =
        ([board availableTypeFromArray:@[ (NSPasteboardType) @"com.apple.is-remote-clipboard" ]] != nil);
    const BOOL isURL = [bestType isEqualToString:NSPasteboardTypeFileURL];
    if (remote && isURL) {
        DLog(@"Pasteboard has a string from a remote clipboard");
        NSArray<NSURL *> *urls = [board readObjectsForClasses:@[ [NSURL class] ] options:nil];
        NSArray<NSString *> *strings = [urls mapWithBlock:^id _Nullable(NSURL *_Nonnull url) {
            DLog(@"Load %@", url);
            // BUG-1185: Only load local file URLs to prevent blocking on network requests.
            // Remote/network URLs could hang indefinitely with no timeout.
            if (![url isFileURL]) {
                DLog(@"Skipping non-file URL from remote clipboard: %@", url);
                return nil;
            }
            // Additional safety: check if file exists and is readable
            if (![[NSFileManager defaultManager] isReadableFileAtPath:url.path]) {
                DLog(@"Skipping unreadable file from remote clipboard: %@", url.path);
                return nil;
            }
            // Limit file size to prevent OOM (max 10MB for remote clipboard content)
            static const unsigned long long kMaxRemoteClipboardFileSize = 10 * 1024 * 1024;
            NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:url.path error:nil];
            if ([attrs fileSize] > kMaxRemoteClipboardFileSize) {
                DLog(@"Skipping oversized file from remote clipboard: %@ (%llu bytes)", url.path, [attrs fileSize]);
                return nil;
            }
        ...
        NSArray<NSURL *> *urls = [board readObjectsForClasses:@[ [NSURL class] ] options:nil];
        NSMutableArray *escapedFilenames = [NSMutableArray arrayWithCapacity:urls.count];
    ...
    if (!info) {
        DLog(@"Using fallback technique of iterating pasteboard items %@",
             [[NSPasteboard generalPasteboard] pasteboardItems]);
        for (NSPasteboardItem *item in [[NSPasteboard generalPasteboard] pasteboardItems]) {
...
- (NSArray<NSString *> *)componentsBySplittingProfileListQuery {
    ...
    NSMutableString *currentValue = [[NSMutableString alloc] initWithCapacity:32];
    ...
    for (NSInteger i = 0; i <= self.length; i++) {
        ...
        if (!inDoubleQuotes && isWhitespace) {
            if (!isFirstCharacterOfWord) {
                ...
                currentValue = [[NSMutableString alloc] initWithCapacity:32];
...
- (void)enumerateLongCharactersWithIndex:(void (^)(UTF32Char c, NSUInteger i, BOOL *stop))block {
    ...
    BOOL expectingLowSurrogate = NO;
    unichar highSurrogate = 0;
    NSInteger highSurrogateIndex = 0;
    ...
    for (NSInteger i = 0; !stop && i < length; i++) {
        ...
        if (IsHighSurrogate(c)) {
            ...
            expectingLowSurrogate = YES;
            highSurrogate = c;
            highSurrogateIndex = i;
            continue;
        }
        if (IsLowSurrogate(c)) {
            if (expectingLowSurrogate) {
                // BUG-1472: Report the high surrogate index, not the low surrogate index
                block(DecodeSurrogatePair(highSurrogate, c), highSurrogateIndex, &stop);
                expectingLowSurrogate = NO;
            }
            // If the previous character was not a high surrogate, quietly ignore this one.
            continue;
        }
        // Not a surrogate
        block(c, i, &stop);
...
- (NSString *)it_stringByExpandingBackslashEscapedCharacters {
    ...
    NSDictionary *escapes =
        @{nkey : @('\n'), akey : @('\x07'), tkey : @('\t'), rkey : @('\r'), bskey : @('\\'), ekey : @('\e')};
...
- (NSData *)dataFromWhitespaceDelimitedHexValues {
    ...
    NSArray<NSString *> *parts = [self componentsSeparatedByCharactersInSet:iTermWhitespaceAndNewlineCharacterSet()];
    // Each pair of hex chars becomes one byte; rough estimate is self.length/2
...
- (NSString *)stringByReplacingEscapedHexValuesWithChars {
    NSMutableArray *ranges = [NSMutableArray arrayWithCapacity:4];
...
static int maximal_subpart_of_row(const unsigned char *datap, int datalen, int bytesInRow,
...
static int minimal_subpart(const unsigned char *datap, int datalen) {
    ...
    for (int row = 0; wellFormedSequencesTable[row].numBytes > 0; row++) {
        longest =
            MAX(longest, maximal_subpart_of_row(datap, datalen, wellFormedSequencesTable[row].numBytes,
                                                wellFormedSequencesTable[row].min, wellFormedSequencesTable[row].max));
    }
    return MIN(datalen, MAX(1, longest));
}

int decode_utf8_char(const unsigned char *datap, int datalen, int *restrict result) {
...
- (NSString *)initWithUTF8DataIgnoringErrors:(NSData *)data {
    ...
    int theChar = 0;
    NSMutableData *utf16Data = [[NSMutableData alloc] initWithCapacity:data.length * sizeof(UniChar)];
...
- (NSString *)stringWithOnlyDigits {
    NSMutableString *s = [[NSMutableString alloc] initWithCapacity:self.length];
...
- (NSString *)stringByTrimmingLeadingWhitespace {
    NSCharacterSet *whitespace = iTermWhitespaceCharacterSet();
    ...
    while (start < len) {
        NSUInteger scalarLength = 0;
        UTF32Char scalar = iTermScalarStartingAtIndex(self, start, len, &scalarLength);
        if (scalarLength == 0 || !iTermCharacterSetContainsScalar(whitespace, scalar)) {
            break;
        }
        start += scalarLength;
    }
...
- (BOOL)mayBeBase64Encoded {
    if ([self rangeOfCharacterFromSet:iTermNonBase64CharacterSet()].location == NSNotFound) {
        return YES;
    }
    if ([self rangeOfCharacterFromSet:iTermNonURLSafeBase64CharacterSet()].location == NSNotFound) {
        return YES;
    }
    return NO;
}
...
- (NSString *)stringByBase64DecodingStringWithEncoding:(NSStringEncoding)encoding {
    return [[NSString alloc] initWithData:[NSData dataWithBase64EncodedString:self] encoding:encoding];
}

- (NSString *)stringByTrimmingTrailingWhitespace {
    return [self stringByTrimmingTrailingCharactersFromCharacterSet:iTermWhitespaceCharacterSet()];
}

- (NSString *)stringByTrimmingTrailingCharactersFromCharacterSet:(NSCharacterSet *)charset {
    if (!charset || self.length == 0) {
        return self;
    }
    ...
    while (end > 0) {
        NSUInteger scalarLength = 0;
        UTF32Char scalar = iTermScalarEndingAtIndex(self, end, &scalarLength);
        if (scalarLength == 0 || !iTermCharacterSetContainsScalar(charset, scalar)) {
            break;
        }
        trimmed = YES;
        end -= scalarLength;
...
- (NSString *)stringByRemovingEnclosingPunctuationMarks {
    ...
    NSUInteger start = 0;
    NSUInteger end = self.length;
    if (iTermRemoveEnclosingPunctuationInRange(self, &start, &end)) {
        return [self substringWithRange:NSMakeRange(start, end - start)];
    }
    return self;
}
...
- (NSRange)rangeOfURLInString {
    ...
    NSCharacterSet *whitespace = iTermWhitespaceAndNewlineCharacterSet();
    NSUInteger start = 0;
    NSUInteger end = self.length;

    while (start < end && [whitespace characterIsMember:[self characterAtIndex:start]]) {
        start++;
    }
    while (end > start && [whitespace characterIsMember:[self characterAtIndex:end - 1]]) {
        end--;
    }
    if (start == end) {
        return NSMakeRange(NSNotFound, 0);
    ...
    NSRange colonRange = [self rangeOfString:@":" options:0 range:trimmedRange];
    if (colonRange.location != NSNotFound) {
        NSCharacterSet *schemeCharacterSet = iTermURLSchemeCharacterSet();
        NSInteger i = ((NSInteger)colonRange.location) - 1;
        BOOL adjusted = NO;
        while (i >= (NSInteger)trimmedRange.location) {
            unichar c = [self characterAtIndex:(NSUInteger)i];
            if ([schemeCharacterSet characterIsMember:c]) {
                i--;
                continue;
            }
            NSUInteger candidateStart = (NSUInteger)i;
            NSUInteger tempStart = candidateStart;
            NSUInteger tempEnd = end;
            if (!iTermRemoveEnclosingPunctuationInRange(self, &tempStart, &tempEnd)) {
                tempStart = MIN(colonRange.location, candidateStart + 1);
            }
            start = tempStart;
            end = tempEnd;
            adjusted = YES;
        ...
        if (!adjusted) {
            // GitLab #12316: If colon is at the start with no scheme characters before it,
            // skip past the leading colon since it's not part of a valid URL scheme.
            if (colonRange.location == trimmedRange.location) {
                // Leading colon with no scheme - skip it
                start = colonRange.location + 1;
                end = trimmedRange.location + trimmedRange.length;
            } else {
                start = trimmedRange.location;
                end = trimmedRange.location + trimmedRange.length;
            }
        }
    } else {
        iTermRemoveEnclosingPunctuationInRange(self, &start, &end);
    }
    ...
    end = iTermTrimSingleTrailingCharacter(self, start, end, iTermTrailingPunctuationCharacterSet());
...
- (NSString *)stringByRemovingTerminatingPunctuation {
    NSCharacterSet *punctuationCharacterSet = iTermTrailingPunctuationCharacterSet();
    if (!punctuationCharacterSet || self.length == 0) {
        return self;
    }
    NSUInteger newEnd = iTermTrimSingleTrailingCharacter(self, 0, self.length, punctuationCharacterSet);
    if (newEnd == self.length) {
        return self;
    }
    return [self substringToIndex:newEnd];
}
...
- (NSString *)stringByTrimmingLeadingHyphens {
    ...
    NSUInteger index = 0;
    while (index < self.length && [self characterAtIndex:index] == '-') {
        index++;
    }
    if (index == 0) {
        return self;
...
- (NSFont *)fontValueWithLigaturesEnabled:(BOOL)ligaturesEnabled {
    ...
    NSDictionary *dict =
        [NSDictionary castFrom:[NSJSONSerialization JSONObjectWithData:[suffix dataUsingEncoding:NSUTF8StringEncoding]
                                                               options:0
                                                                 error:nil]];
...
+ (NSString *)stringWithHexEncodedString:(NSString *)hexEncodedString {
    NSMutableString *result = [[NSMutableString alloc] initWithCapacity:hexEncodedString.length / 2];
    for (int i = 0; i + 1 < hexEncodedString.length; i += 2) {
        char buffer[3] = {[hexEncodedString characterAtIndex:i], [hexEncodedString characterAtIndex:i + 1], 0};
        int value;
...
static TECObjectRef CreateTECConverterForUTF8Variants(TextEncodingVariant variant) {
    TextEncoding utf16Encoding =
        CreateTextEncoding(kTextEncodingUnicodeDefault, kTextEncodingDefaultVariant, kUnicodeUTF16Format);
    TextEncoding hfsPlusEncoding = CreateTextEncoding(kTextEncodingUnicodeDefault, variant, kUnicodeUTF16Format);
...
- (NSString *)_convertBetweenUTF8AndHFSPlusAsPrecomposition:(BOOL)precompose {
    ...
    if (TECConvertText(precompose ? gHFSPlusComposed : gHFSPlusDecomposed, (TextPtr)in, in_len, &in_len, (TextPtr)out,
                       out_len, &out_len) != noErr) {
        ret = self;
    } else {
...
- (NSString *)stringByExpandingVimSpecialCharacters {
    ...
    while (index != NSNotFound && index < self.length) {
        [result
            appendString:[self substringWithRange:NSMakeRange(haveAppendedUpToIndex, index - haveAppendedUpToIndex)]];
    ...
    index = self.length;
    [result appendString:[self substringWithRange:NSMakeRange(haveAppendedUpToIndex, index - haveAppendedUpToIndex)]];
...
- (BOOL)interpolatedStringContainsNonliteral {
    ...
    [parser enumerateSwiftySubstringsWithBlock:^(NSUInteger index, NSString *_Nonnull substring, BOOL isLiteral,
                                                 BOOL *_Nonnull stop) {
        if (!isLiteral) {
            result = YES;
            *stop = YES;
...
- (NSString *)stringRepeatedTimes:(int)n {
    NSMutableString *result = [[NSMutableString alloc] initWithCapacity:self.length * n];
    for (int i = 0; i < n; i++) {
...
- (NSString *)ellipsizedDescriptionNoLongerThan:(int)maxLength {
    ...
    if (noNewlineSelf.length <= maxLength) {
        return [noNewlineSelf stringByTrimmingCharactersInSet:iTermWhitespaceCharacterSet()];
    }
    NSRange firstNonWhitespaceRange =
        [noNewlineSelf rangeOfCharacterFromSet:iTermNonWhitespaceAndNewlineCharacterSet()];
    ...
    prefix = [prefix stringByTrimmingCharactersInSet:iTermWhitespaceCharacterSet()];
...
+ (NSString *)stringForModifiersWithMask:(NSUInteger)keyMods {
    NSMutableString *theKeyString = [[NSMutableString alloc] initWithCapacity:16];
...
- (NSString *)stringByReplacingControlCharactersWithCaretLetter {
    return [self
        stringByReplacingOccurrencesOfRegex:@"[\x00-\x1f\x7f]"
                                 usingBlock:^NSString *(NSInteger captureCount,
                                     ...
                                     NSMutableString *replacement =
                                         [[NSMutableString alloc] initWithCapacity:capturedRanges[0].length * 2];
                                     const NSInteger index = capturedRanges[0].location;
                                     for (NSInteger i = 0; i < capturedRanges[0].length; i++) {
                                         unichar c =
                                             [[self substringWithRange:NSMakeRange(index + i, 1)] characterAtIndex:0];
                                         if (c == 0x7f) {
...
- (BOOL)stringMatchesGlobPattern:(NSString *)glob caseSensitive:(BOOL)caseSensitive {
    ...
    for (NSString *part in parts) {
        ...
        // BUG-f1356: Replace assert() with guard - invalid start should return NO, not crash
        if (start > self.length) {
            ELog(@"stringMatchesGlobPattern: start %lu > length %lu", (unsigned long)start, (unsigned long)self.length);
            return NO;
        }
...
- (void)enumerateComposedCharacters:(void (^)(NSRange, unichar, NSString *, BOOL *))block {
    ...
    do {
        ...
        if (range.length > 0) {
            NSRange rangeOfFirstException =
                [self rangeOfCharacterFromSet:exceptions
                                      options:NSLiteralSearch
                                        range:NSMakeRange(range.location + 1, range.length - 1)];
            if (rangeOfFirstException.location != NSNotFound) {
                // BUG-1477: Only skip splitting after ZWJ if the exception character
                // is a valid ZWJ sequence continuation (emoji, regional indicator, etc.)
                BOOL shouldSplit = YES;
                if (rangeOfFirstException.location > 0 &&
                    [self characterAtIndex:rangeOfFirstException.location - 1] == 0x200d) {
                    // Check if the character after ZWJ is a valid ZWJ sequence member
                    unichar exceptChar = [self characterAtIndex:rangeOfFirstException.location];
                    // Valid ZWJ continuations: emoji characters (typically >= 0x1F000 in supplementary plane
                    // or certain BMP emoji), regional indicators (0x1F1E6-0x1F1FF), tag chars (0xE0000-0xE007F)
                    // For BMP, check if it's a known emoji presentation character
                    if (exceptChar >= 0x2600 && exceptChar <= 0x27BF) {
                        // Common emoji range in BMP (miscellaneous symbols, dingbats)
                        shouldSplit = NO;
                    } else if (exceptChar >= 0xD800 && exceptChar <= 0xDBFF) {
                        // High surrogate - check if it forms an emoji in supplementary plane
                        // Most emoji ZWJ sequences use supplementary characters
                        shouldSplit = NO;
                    } else if (exceptChar == 0x2640 || exceptChar == 0x2642 || // gender symbols
                               exceptChar == 0x2695 || exceptChar == 0x2696 || // profession symbols
                               exceptChar == 0x2708 || exceptChar == 0x2764) { // common ZWJ emoji
                        shouldSplit = NO;
                    }
                }
                if (shouldSplit) {
...
- (NSString *)byTruncatingComposedCharactersInCenter:(NSInteger)numberToOmit {
    ...
    NSString *suffix = [[characters subarrayWithRange:NSMakeRange(characters.count - suffixLength, suffixLength)]
        componentsJoinedByString:@""];
    return [NSString stringWithFormat:@"%@…%@", prefix, suffix];
}

- (void)reverseEnumerateSubstringsEqualTo:(NSString *)query block:(void (^NS_NOESCAPE)(NSRange range))block {
...
- (NSDictionary *)attributesUsingFont:(NSFont *)font
                          fittingSize:(NSSize)maxSize
                           attributes:(NSDictionary *)baseAttributes {
    ...
    while (points != prevPoints) {
        ...
        if (boundingRect.size.width > maxSize.width || boundingRect.size.height > maxSize.height) {
            max = points;
        } else if (boundingRect.size.width < maxSize.width && boundingRect.size.height < maxSize.height) {
            min = points;
        }
        prevPoints = points;
        points = (min + max) / 2;
...
- (NSString *)stringByCompactingFloatingPointString {
    ...
    NSString *compact = [self
        stringByTrimmingTrailingCharactersFromCharacterSet:[NSCharacterSet characterSetWithCharactersInString:@"0"]];
...
- (NSArray<NSNumber *> *)codePoints {
    NSMutableArray<NSNumber *> *result = [NSMutableArray arrayWithCapacity:self.length];
    for (NSUInteger i = 0; i < self.length; i++) {
        unichar c = [self characterAtIndex:i];
...
- (BOOL)isNumeric {
    ...
    NSRange range = [self rangeOfCharacterFromSet:iTermNonDecimalDigitCharacterSet()];
...
- (NSRange)makeRangeSafe:(NSRange)range {
    ...
    // Guard against range.location out of bounds
    if (range.location >= self.length) {
        return NSMakeRange(NSNotFound, 0);
    }
    // BUG-1476: Check for orphaned low surrogate at start
    unichar firstCharacter = [self characterAtIndex:range.location];
    if (CFStringIsSurrogateLowCharacter(firstCharacter)) {
        if (range.location > 0) {
            ...
            // No preceding character, skip the orphaned low surrogate
            range.location += 1;
            if (range.length > 0) {
                range.length -= 1;
            }
    ...
    // Check for orphaned high surrogate at end
    if (range.length > 0 && NSMaxRange(range) <= self.length) {
...
- (NSRect)it_boundingRectWithSize:(NSSize)bounds attributes:(NSDictionary *)attributes truncated:(BOOL *)truncated {
    ...
    *truncated = NO;
    for (NSString *part in [self componentsSeparatedByString:@"\n"]) {
        NSAttributedString *string = [[NSAttributedString alloc] initWithString:part attributes:attributes];

        CTFramesetterRef framesetter =
            CTFramesetterCreateWithAttributedString((__bridge CFMutableAttributedStringRef)string);
        CFRange fitRange;

        CFRange textRange = CFRangeMake(0, part.length);
        CGSize frameSize =
            CTFramesetterSuggestFrameSizeWithConstraints(framesetter, textRange, NULL, bounds, &fitRange);
...
- (void)it_drawInRect:(CGRect)rect attributes:(NSDictionary *)attributes alpha:(CGFloat)alpha {
    ...
    for (NSString *part in [self componentsSeparatedByString:@"\n"]) {
        ...
        CFRange textRange = CFRangeMake(0, part.length);
        CGSize frameSize =
            CTFramesetterSuggestFrameSizeWithConstraints(framesetter, textRange, NULL, rect.size, &fitRange);
...
- (NSString *)jsonEncodedString {
    ...
    dispatch_once(&onceToken, ^{
        froms = [[NSMutableArray alloc] initWithCapacity:numberOfControlCharacters];
        tos = [[NSMutableArray alloc] initWithCapacity:numberOfControlCharacters];
    ...
    for (int i = 0; i < numberOfControlCharacters; i++) {
        [s replaceOccurrencesOfString:froms[i] withString:tos[i] options:0 range:NSMakeRange(0, [s length])];
    }
...
- (NSString *)stringByTrimmingOrphanedSurrogates {
    ...
    // BUG-1473: Trim both orphaned high AND low surrogates from both ends
    // Orphaned surrogates at start: unpaired high surrogates OR unpaired low surrogates
    NSInteger startIndex = 0;
    while (startIndex < (NSInteger)self.length) {
        const unichar c = [self characterAtIndex:startIndex];
        if (IsHighSurrogate(c)) {
            // High surrogate at start - check if there's a following low surrogate
            if (startIndex + 1 < (NSInteger)self.length && IsLowSurrogate([self characterAtIndex:startIndex + 1])) {
                // Valid surrogate pair, stop trimming
                break;
            }
            // Orphaned high surrogate, skip it
            startIndex++;
        } else if (IsLowSurrogate(c)) {
            // Orphaned low surrogate at start (no preceding high), skip it
            startIndex++;
        } else {
            // Regular character, stop trimming
            break;
        }
    }
    if (startIndex >= (NSInteger)self.length) {
        return @"";
    }
    // Trim from end: orphaned high surrogates OR unpaired low surrogates
    NSInteger endIndex = self.length - 1;
    while (endIndex > startIndex) {
        const unichar c = [self characterAtIndex:endIndex];
        if (IsLowSurrogate(c)) {
            // Low surrogate at end - check if there's a preceding high surrogate
            if (endIndex > startIndex && IsHighSurrogate([self characterAtIndex:endIndex - 1])) {
                // Valid surrogate pair, stop trimming
                break;
            }
            ...
            // Orphaned high surrogate at end (no following low), skip it
            endIndex--;
        } else {
            // Regular character, stop trimming
            break;
    ...
    // BUG-f1357: Replace assert() with guard - invalid indices should return empty string, not crash
    if (endIndex < startIndex) {
        ELog(@"stringByTrimmingOrphanedSurrogates: endIndex %lu < startIndex %lu, returning empty string",
             (unsigned long)endIndex, (unsigned long)startIndex);
        return @"";
    }
...
- (NSString *)it_normalized {
    ...
    return [[self lowercaseString] stringByFoldingWithOptions:NSDiacriticInsensitiveSearch locale:usEnglish];
}

- (NSArray<NSString *> *)it_normalizedTokens {
    NSMutableArray<NSString *> *tokens = [NSMutableArray arrayWithCapacity:8];
...
- (iTermTuple<NSArray<NSString *> *, NSString *> *)queryBySplittingLiteralPhrases {
    ...
    NSMutableArray<NSString *> *literals = [NSMutableArray arrayWithCapacity:matches.count];
    ...
    NSArray<NSString *> *parts =
        [stripped componentsSeparatedByCharactersInSet:iTermWhitespaceAndNewlineCharacterSet()];
    ...
    return [iTermTuple tupleWithObject:literals andObject:remaining];
}

- (double)it_localizedDoubleValue {
    if ([[self stringByTrimmingCharactersInSet:iTermWhitespaceCharacterSet()] isEqualToString:@"∞"]) {
        return INFINITY;
    }
    if ([[self stringByTrimmingCharactersInSet:iTermWhitespaceCharacterSet()] isEqualToString:@"-∞"]) {
        return -INFINITY;
...
- (NSString *)stringByPerformingOrderedSubstitutions:(iTermOrderedDictionary<NSString *, NSString *> *)substitutions {
    return [substitutions.keys reduceWithFirstValue:self
                                              block:^NSString *(NSString *accumulator, NSString *key) {
                                                  NSString *replacement = substitutions[key];
                                                  return [accumulator stringByReplacingOccurrencesOfString:key
                                                                                                withString:replacement];
                                              }];
}
...
- (BOOL)caseInsensitiveHasPrefix:(NSString *)prefix {
    const NSRange prefixRange = [self rangeOfString:prefix options:(NSAnchoredSearch | NSCaseInsensitiveSearch)];
...
- (NSDictionary<NSString *, NSString *> *)it_keyValuePairsSeparatedBy:(NSString *)separator {
    NSArray<iTermTuple<NSString *, NSString *> *> *parts =
        [[self componentsSeparatedByString:separator] mapWithBlock:^id _Nullable(NSString *_Nonnull string) {
            return [string keyValuePair];
        }];
    NSDictionary<NSString *, iTermTuple<NSString *, NSString *> *> *tupleDict =
        [parts classifyUniquelyWithBlock:^id(iTermTuple<NSString *, NSString *> *tuple) {
            return tuple.firstObject;
        }];
    NSDictionary<NSString *, NSString *> *dict =
        [tupleDict mapValuesWithBlock:^id(NSString *key, iTermTuple<NSString *, NSString *> *tuple) {
...
- (BOOL)beginsWithWhitespace {
    ...
    return [iTermWhitespaceAndNewlineCharacterSet() longCharacterIsMember:[self longCharacterAtIndex:0]];
}
...
- (BOOL)endsWithWhitespace {
    ...
    return [iTermWhitespaceAndNewlineCharacterSet() longCharacterIsMember:[self longCharacterAtIndex:i]];
}

- (NSRange)rangeOfLastWordFromIndex:(NSUInteger)index {
    const NSRange searchRange = NSMakeRange(0, index);
    const NSRange nonWhitespaceRange = [self rangeOfCharacterFromSet:iTermNonWhitespaceAndNewlineCharacterSet()
                                                             options:NSBackwardsSearch
                                                               range:searchRange];
    ...
    const NSRange whitespaceRange = [self rangeOfCharacterFromSet:iTermWhitespaceAndNewlineCharacterSet()
                                                          options:NSBackwardsSearch
...
- (NSString *)firstNonEmptyLine {
    const NSInteger indexOfFirstNonWhitespace =
        [self rangeOfCharacterFromSet:iTermNonWhitespaceAndNewlineCharacterSet()].location;
    if (indexOfFirstNonWhitespace == NSNotFound) {
        return @"";
    }

    const NSInteger indexOfSubsequentNewline =
        [self rangeOfCharacterFromSet:iTermNewlineCharacterSet()
                              options:0
                                range:NSMakeRange(indexOfFirstNonWhitespace, self.length - indexOfFirstNonWhitespace)]
            .location;
...
- (NSString *)sanitizedUsername {
    ...
    // BUG-1162: Filter out any shell metacharacters from user-configurable valid chars.
    // Even if user allows certain special chars, we must not allow shell metacharacters
    // to prevent command injection via malicious URLs.
    static NSCharacterSet *shellMetacharacters;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shellMetacharacters = [NSCharacterSet characterSetWithCharactersInString:@";<>&|$`\\!#*?(){}[]'\"\n\r\t "];
    });

    NSMutableCharacterSet *legalCharacters = [NSMutableCharacterSet alphanumericCharacterSet];
    NSString *userConfiguredChars = [iTermAdvancedSettingsModel validCharactersInSSHUserNames];
    // Filter user-configured chars to remove any dangerous shell metacharacters
    NSMutableString *safeUserChars = [[NSMutableString alloc] initWithCapacity:userConfiguredChars.length];
    for (NSUInteger i = 0; i < userConfiguredChars.length; i++) {
        unichar c = [userConfiguredChars characterAtIndex:i];
        if (![shellMetacharacters characterIsMember:c]) {
            [safeUserChars appendFormat:@"%C", c];
        }
    }
    [legalCharacters addCharactersInString:safeUserChars];
...
- (NSString *)sanitizedHostname {
    ...
    {
        NSCharacterSet *legalInitialCharacters = [NSCharacterSet
    ...
    // BUG-1161: Don't allow colons in hostname - they could enable port injection.
    // Colons are only valid for IPv6 addresses which should be bracketed in URLs anyway.
    // The URL parser should handle ports via url.port, not embedded in hostname.
    ...
    // BUG-1172: Validate hostname structure - no consecutive dots, no leading/trailing dots
    if ([self hasPrefix:@"."] || [self hasSuffix:@"."] || [self containsString:@".."]) {
        ELog(@"Hostname %@ has invalid dot structure", self);
        return nil;
    }
    return [self stringWithEscapedShellCharactersIncludingNewlines:YES];
}
...
static NSDictionary<NSString *, NSNumber *> *iTermKittyDiacriticIndex(void) {
    ...
    dispatch_once(&onceToken, ^{
        NSArray<NSString *> *diacritics = @[
            ...
            @"\uA8EE",  @"\uA8EF",  @"\uA8F0",  @"\uA8F1",  @"\uAAB0",  @"\uAAB2",  @"\uAAB3",  @"\uAAB7",
            @"\uAAB8",  @"\uAABE",  @"\uAABF",  @"\uAAC1",  @"\uFE20",  @"\uFE21",  @"\uFE22",  @"\uFE23",
            @"\uFE24",  @"\uFE25",  @"\uFE26",  @"\u10A0F", @"\u10A38", @"\u1D185", @"\u1D186", @"\u1D187",
            @"\u1D188", @"\u1D189", @"\u1D1AA", @"\u1D1AB", @"\u1D1AC", @"\u1D1AD", @"\u1D242", @"\u1D243",
            @"\u1D244"
        ];
        gKittyImageDiacriticIndex = [NSMutableDictionary dictionaryWithCapacity:diacritics.count];
...
- (BOOL)parseKittyUnicodePlaceholder:(out VT100GridCoord *)coord imageMSB:(out int *)imageMSB {
...
- (NSString *)it_sanitized {
    ...
    while (startIndex < self.length) {
        ...
        if (unsafeRange.location > startIndex) {
            [sanitized
                appendString:[self substringWithRange:NSMakeRange(startIndex, unsafeRange.location - startIndex)]];
        }
...
- (ScreenCharArray *)asScreenCharArray {
    ...
    int len = self.length;
    StringToScreenChars(self, buf, (screen_char_t){0}, (screen_char_t){0}, &len, NO, NULL, NULL,
                        iTermUnicodeNormalizationNone, 9, NO, NULL);
    ScreenCharArray *sca = [[ScreenCharArray alloc] initWithLine:buf
                                                          length:len
...
+ (NSData *)dataForHexCodes:(NSString *)codes {
    NSMutableData *data = [[NSMutableData alloc] initWithCapacity:codes.length / 3 + 1];
...
- (NSArray<NSValue *> *)it_rangesOfString:(NSString *)needle {
    ...
    NSMutableArray<NSValue *> *found = [NSMutableArray arrayWithCapacity:4];
    NSRange rangeToSearch = NSMakeRange(0, self.length);
...
- (NSArray<NSValue *> *)it_rangesOfStringsIn:(NSArray<NSString *> *)needles {
    ...
    NSMutableArray<NSValue *> *found = [NSMutableArray arrayWithCapacity:needles.count * 2];
    ...
    NSMutableArray<NSValue *> *merged = [NSMutableArray arrayWithCapacity:found.count];
...
- (NSArray<iTermTuple<NSString *, NSString *> *> *)it_componentsSeparatedAtRanges:(NSArray<NSValue *> *)ranges {
    ...
    NSMutableArray<iTermTuple<NSString *, NSString *> *> *result = [NSMutableArray arrayWithCapacity:ranges.count + 1];
    NSUInteger start = 0;
    for (NSValue *value in ranges) {
...
- (NSArray<iTermTuple<NSString *, NSString *> *> *)it_componentsSeparatedByAnyStringIn:
...
- (void)trimTrailingWhitespace {
    NSRange rangeOfLastWantedCharacter = [self rangeOfCharacterFromSet:iTermNonWhitespaceCharacterSet()
                                                               options:NSBackwardsSearch];
...
- (void)escapeShellCharactersExceptTabAndNewline {
    NSString *charsToEscape = [[NSString shellEscapableCharacters] stringByReplacingOccurrencesOfString:@"\t"
                                                                                             withString:@""];
    return [self escapeCharacters:charsToEscape];
}

- (void)escapeShellCharactersForBash {
    [self escapeShellCharactersWithSingleQuotesIncludingNewlines:YES forBash:YES];
}
...
- (void)escapeShellCharactersWithSingleQuotesIncludingNewlines:(BOOL)includingNewlines {
    return [self escapeShellCharactersWithSingleQuotesIncludingNewlines:includingNewlines forBash:NO];
}

- (void)escapeShellCharactersWithSingleQuotesIncludingNewlines:(BOOL)includingNewlines forBash:(BOOL)forBash {
    // Only need to escape single quote and backslash in a single-quoted string
    NSMutableString *charsToEscape = [@"\\'" mutableCopy];
    NSMutableCharacterSet *charsToSearch =
        [NSMutableCharacterSet characterSetWithCharactersInString:[NSString shellEscapableCharacters]];
...
- (void)escapeCharacters:(NSString *)charsToEscape forBash:(BOOL)forBash {
    for (int i = 0; i < [charsToEscape length]; i++) {
        ...
        [self replaceOccurrencesOfString:before withString:after options:0 range:NSMakeRange(0, [self length])];
    }
}
