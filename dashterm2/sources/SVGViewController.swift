...
class SVGViewController: NSViewController, WKScriptMessageHandler {
    ...
    override func viewDidLoad() {
        // BUG-4068: Must call super.viewDidLoad() to complete view controller lifecycle
        super.viewDidLoad()
        loadSVG()
    }
    ...
    override init(nibName nibNameOrNil: NSNib.Name?, bundle nibBundleOrNil: Bundle?) {
        ...
        let js = """
               ...
               console.log = function() {
                   ...
                   window.webkit.messageHandlers.DashTerm2ConsoleLog.postMessage(
        ...
        configuration.userContentController.add(self, name: "DashTerm2ConsoleLog")
    ...
    required init?(coder: NSCoder) {
        // BUG-f826: Return nil instead of crashing for unused coder initializer
        DLog("SVGViewController init(coder:) is not supported")
        return nil
    }
...
class RegexVisualizationViewController: SVGViewController {
    ...
    private func reload(sync: Bool) {
        ...
        let svg = dsl.withCString { dslPtr -> UnsafeMutablePointer<CChar>? in
            guard let cssPtr = railroad_dsl_css_for_theme(view.effectiveAppearance.it_isDark ? "dark" : "light") else {
                return nil
            }
            defer {
        ...
        if sync {
            let estimator = SVGSizeEstimator(html: html)
            sizeEstimator = estimator
            preferredContentSize = min(maxSize, estimator.desiredSize)
        } else {
    ...
    @MainActor required init?(coder: NSCoder) {
        // BUG-f827: Return nil instead of crashing for unused coder initializer
        DLog("SVGWebView init(coder:) is not supported")
        return nil
    }
}
