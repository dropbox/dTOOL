...
//
// BUG-283: API server upgrade messages updated to DashTerm2
// BUG-479: Invalid request error message updated to DashTerm2
...
// mainthread-blocking DashTerm2-to-script RPC.
...
// Returns YES if signaled, NO if timed out (30 seconds)
- (BOOL)wait;
- (void)signal;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        // Request queue typically holds 0-10 pending requests
        _requests = [NSMutableArray arrayWithCapacity:8];
        _lock = OS_UNFAIR_LOCK_INIT;
    }
    return self;
}

// Returns YES if signaled, NO if timed out
- (BOOL)wait {
    // BUG-11877: Add 30 second timeout to prevent indefinite hang if script disconnects
    dispatch_time_t timeout = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(30 * NSEC_PER_SEC));
    return dispatch_semaphore_wait(_sema, timeout) == 0;
}
...
- (iTermAPIRequest *)dequeueRequestFromAnyConnection:(BOOL)anyConnection {
    os_unfair_lock_lock(&_lock);
    iTermAPIRequest *result = nil;
    if (anyConnection) {
        result = _requests.firstObject;
        if (result) {
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _connections = [[NSMutableDictionary alloc] initWithCapacity:16];  // API connections
        _unixSocket = [iTermSocket unixDomainSocket];
        if (!_unixSocket) {
            XLog(@"Failed to create unix socket");
            return nil;
        }
        // Pending connections queue, typically 0-5 waiting
        _pendingConnections = [NSMutableArray arrayWithCapacity:4];
        _queue = dispatch_queue_create("com.dashterm.dashterm2.apisockets", NULL);
        _executionQueue = dispatch_queue_create("com.dashterm.dashterm2.apiexec", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)listenOnUnixSocket {
    ...
    BOOL ok = [_unixSocket listenWithBacklog:5
                                      accept:^(int fd, iTermSocketAddress *clientAddress, NSNumber *euid) {
                                          [self didAcceptConnectionOnFileDescriptor:fd
...
- (void)stop {
    ...
    dispatch_sync(_queue, ^{
        ...
        [self->_pendingConnections enumerateObjectsUsingBlock:^(iTermHTTPConnection *_Nonnull connection,
...
- (void)queueStop {
    DLog(@"queueStop");
    [_connections enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, iTermWebSocketConnection *_Nonnull conn,
                                                      BOOL *_Nonnull stop) {
...
- (void)didAcceptConnectionOnFileDescriptor:(int)fd
    ...
    dispatch_async(queue, ^{
        ...
        NSTimeInterval timeout = [iTermAdvancedSettingsModel apiServerConnectionTimeoutSeconds];
        if (timeout > 0) {
            __weak typeof(self) weakSelf = self;
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeout * NSEC_PER_SEC)), queue, ^{
                [weakSelf expirePendingConnectionIfNecessary:connection];
            });
        }
...
- (void)expirePendingConnectionIfNecessary:(iTermHTTPConnection *)connection {
    if (![_pendingConnections containsObject:connection]) {
        return;
    }
    DLog(@"Expiring pending API connection due to timeout");
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermAPIServerConnectionRejected
                          object:nil
                        userInfo:@{@"reason" : @"Timed out waiting for API client request"}];
    });
}

// run on _queue
- (void)reallyDidAcceptConnection:(iTermHTTPConnection *)connection retries:(NSInteger)retries {
    ...
    if (!connection.euid || connection.euid.unsignedIntValue != geteuid()) {
        ...
        dispatch_async(connection.queue, ^{
            NSString *reason = [NSString
                stringWithFormat:@"Peer's euid of %@ not equal to my euid of %@", connection.euid, @(geteuid())];
    ...
    [self startRequestOnConnection:connection
                              pids:@[]
...
- (void)reallyStartRequestOnConnection:(iTermHTTPConnection *)connection
    ...
    if (!request) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter]
                postNotificationName:iTermAPIServerConnectionRejected
                              object:nil
                            userInfo:@{@"reason" : @"Failed to read request from connection", @"pids" : pids}];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter]
                postNotificationName:iTermAPIServerConnectionRejected
                              object:nil
                            userInfo:@{
                                @"reason" : [NSString stringWithFormat:@"Bad request. URL path was %@, but should be /",
                                                                       request.URL.path],
                                @"pids" : pids
                            }];
        });
    ...
    iTermWebSocketConnection *webSocketConnection =
        [iTermWebSocketConnection newWebSocketConnectionForRequest:request connection:connection reason:&authReason];
    if (webSocketConnection) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            const BOOL disableAuthUI = request.allHTTPHeaderFields[@"x-dashterm2-disable-auth-ui"] != nil;
            if (connection.clientAddress.addressFamily != AF_UNIX) {
                // Security: Only allow connections from Unix domain sockets
                XLog(@"Rejecting connection from non-Unix socket: address family %d",
                     connection.clientAddress.addressFamily);
                completion(NO, @"Only Unix domain socket connections are allowed");
                return;
            }
            ...
            if (ok) {
                ...
                if (disableAuthUI) {
                    ...
                    [[NSNotificationCenter defaultCenter]
                        postNotificationName:iTermAPIServerConnectionRejected
                                      object:request.allHTTPHeaderFields[@"x-dashterm2-key"]
                                    userInfo:@{
                                        @"reason" : reason ?: @"Unknown reason",
                                        @"job" : displayName ?: [NSNull null],
                                        @"pids" : pids
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter]
                postNotificationName:iTermAPIServerConnectionRejected
                              object:request.allHTTPHeaderFields[@"x-dashterm2-key"]
                            userInfo:@{@"reason" : authReason ?: @"Unknown reason", @"pids" : pids}];
        });
        dispatch_async(connection.queue, ^{
...
// queue
- (void)sendResponse:(ITMServerOriginatedMessage *)response
...
- (void)drainTransaction:(iTermAPITransaction *)transaction {
    while (1) {
        if (![transaction wait]) {
            // BUG-11877: Timed out waiting for script response
            DLog(@"Transaction timed out waiting for script response");
            break;
        }
        ...
        if (transactionRequest.request.submessageOneOfCase ==
                ITMClientOriginatedMessage_Submessage_OneOfCase_TransactionRequest &&
        ...
        [self dispatchRequest:transactionRequest.request connection:transactionRequest.connection];
    }
...
- (void)handleTransactionRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
    response.transactionResponse = [[ITMTransactionResponse alloc] init];
...
- (void)handleGetBufferRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
    ...
    [_delegate apiServerGetBuffer:request.getBufferRequest
                          handler:^(ITMGetBufferResponse *getBufferResponse) {
                              if (handled) {
                                  XLog(@"Double callback in handleGetBufferRequest - ignoring");
                                  return;
                              }
                              handled = YES;
...
- (void)handleGetPromptRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerGetPrompt:request.getPromptRequest
                          handler:^(ITMGetPromptResponse *getPromptResponse) {
                              if (handled) {
                                  XLog(@"Double callback in API request handler - ignoring");
                                  return;
                              }
                              handled = YES;
...
- (void)handleListPromptsRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerListPrompts:request.listPromptsRequest
                            handler:^(ITMListPromptsResponse *listPromptsResponse) {
                                if (handled) {
                                    XLog(@"Double callback in API request handler - ignoring");
                                    return;
                                }
...
- (void)handleNotificationRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleRegisterToolRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleSetProfilePropertyRequest:(ITMClientOriginatedMessage *)request
                             connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
    ...
    [_delegate apiServerSetProfileProperty:request.setProfilePropertyRequest
                                   handler:^(ITMSetProfilePropertyResponse *setProfilePropertyResponse) {
                                       ...
                                       [weakSelf finishHandlingRequestWithResponse:response
                                                                      onConnection:webSocketConnection];
                                   }];
}

- (void)handleGetProfilePropertyRequest:(ITMClientOriginatedMessage *)request
                             connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleListSessionsRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleSendTextRequest:(ITMClientOriginatedMessage *)request
                   connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSendText:request.sendTextRequest
                         handler:^(ITMSendTextResponse *sendTextResponse) {
                             if (handled) {
                                 XLog(@"Double callback in API request handler - ignoring");
                                 return;
                             }
...
- (void)handleCreateTabRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerCreateTab:request.createTabRequest
                          handler:^(ITMCreateTabResponse *createTabResponse) {
                              if (handled) {
                                  XLog(@"Double callback in API request handler - ignoring");
                                  return;
                              }
...
- (void)handleSplitPaneRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSplitPane:request.splitPaneRequest
                          handler:^(ITMSplitPaneResponse *splitPaneResponse) {
                              if (handled) {
                                  XLog(@"Double callback in API request handler - ignoring");
                                  return;
                              }
...
- (void)handleSetPropertyRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSetProperty:request.setPropertyRequest
                            handler:^(ITMSetPropertyResponse *setPropertyResponse) {
                                if (handled) {
                                    XLog(@"Double callback in API request handler - ignoring");
                                    return;
                                }
...
- (void)handleGetPropertyRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerGetProperty:request.getPropertyRequest
                            handler:^(ITMGetPropertyResponse *getPropertyResponse) {
                                if (handled) {
                                    XLog(@"Double callback in API request handler - ignoring");
                                    return;
                                }
...
- (void)handleInjectRequest:(ITMClientOriginatedMessage *)request
                 connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerInject:request.injectRequest
                       handler:^(ITMInjectResponse *injectResponse) {
                           if (handled) {
                               XLog(@"Double callback in API request handler - ignoring");
                               return;
                           }
...
- (void)handleActivateRequest:(ITMClientOriginatedMessage *)request
                   connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerActivate:request.activateRequest
                         handler:^(ITMActivateResponse *activateResponse) {
                             if (handled) {
                                 XLog(@"Double callback in API request handler - ignoring");
                                 return;
                             }
...
- (void)handleVariableRequest:(ITMClientOriginatedMessage *)request
                   connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerVariable:request.variableRequest
                         handler:^(ITMVariableResponse *variableResponse) {
                             if (handled) {
                                 XLog(@"Double callback in API request handler - ignoring");
                                 return;
                             }
...
- (void)handleSavedArrangementRequest:(ITMClientOriginatedMessage *)request
                           connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSavedArrangement:request.savedArrangementRequest
                                 handler:^(ITMSavedArrangementResponse *savedArrangementResponse) {
                                     if (handled) {
                                         XLog(@"Double callback in API request handler - ignoring");
                                         return;
                                     }
...
- (void)handleFocusRequest:(ITMClientOriginatedMessage *)request
                connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleListProfilesRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerListProfiles:request.listProfilesRequest
                             handler:^(ITMListProfilesResponse *listProfilesResponse) {
                                 if (handled) {
                                     XLog(@"Double callback in API request handler - ignoring");
                                     return;
                                 }
...
- (void)handleServerOriginatedRpcResultRequest:(ITMClientOriginatedMessage *)request
                                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleMalformedRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
...
- (void)handleUnhandleableRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
    response.error = @"Not ready. This is a bug! Please report it at https://github.com/ayates_dbx/dashterm2/issues";
    [self finishHandlingRequestWithResponse:response onConnection:webSocketConnection];
}

- (void)handleRestartSessionRequest:(ITMClientOriginatedMessage *)request
                         connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerRestartSession:request.restartSessionRequest
                               handler:^(ITMRestartSessionResponse *restartSessionResponse) {
                                   if (handled) {
                                       XLog(@"Double callback in API request handler - ignoring");
                                       return;
                                   }
...
- (void)handleMenuItemRequest:(ITMClientOriginatedMessage *)request
                   connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerMenuItem:request.menuItemRequest
                         handler:^(ITMMenuItemResponse *menuItemResponse) {
                             if (handled) {
                                 XLog(@"Double callback in API request handler - ignoring");
                                 return;
                             }
...
- (void)handleSetTabLayoutRequest:(ITMClientOriginatedMessage *)request
                       connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSetTabLayout:request.setTabLayoutRequest
                             handler:^(ITMSetTabLayoutResponse *theResponse) {
                                 if (handled) {
                                     XLog(@"Double callback in API request handler - ignoring");
                                     return;
                                 }
...
- (void)handleGetBroadcastDomainsRequest:(ITMClientOriginatedMessage *)request
                              connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerGetBroadcastDomains:request.getBroadcastDomainsRequest
                                    handler:^(ITMGetBroadcastDomainsResponse *theResponse) {
                                        if (handled) {
                                            XLog(@"Double callback in API request handler - ignoring");
                                            return;
                                        }
...
- (void)handleTmuxRequest:(ITMClientOriginatedMessage *)request
               connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerTmuxRequest:request.tmuxRequest
                            handler:^(ITMTmuxResponse *theResponse) {
                                if (handled) {
                                    XLog(@"Double callback in API request handler - ignoring");
                                    return;
                                }
...
- (void)handleReorderTabsRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerReorderTabsRequest:request.reorderTabsRequest
                                   handler:^(ITMReorderTabsResponse *theResponse) {
                                       if (handled) {
                                           XLog(@"Double callback in API request handler - ignoring");
                                           return;
                                       }
...
- (void)handlePreferencesRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerPreferencesRequest:request.preferencesRequest
                                   handler:^(ITMPreferencesResponse *theResponse) {
                                       if (handled) {
                                           XLog(@"Double callback in API request handler - ignoring");
                                           return;
                                       }
...
- (void)handleColorPresetRequest:(ITMClientOriginatedMessage *)request
                      connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerColorPresetRequest:request.colorPresetRequest
                                   handler:^(ITMColorPresetResponse *theResponse) {
                                       if (handled) {
                                           XLog(@"Double callback in API request handler - ignoring");
                                           return;
                                       }
...
- (void)handleSelectionRequest:(ITMClientOriginatedMessage *)request
                    connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSelectionRequest:request.selectionRequest
                                 handler:^(ITMSelectionResponse *theResponse) {
                                     if (handled) {
                                         XLog(@"Double callback in API request handler - ignoring");
                                         return;
                                     }
...
- (void)handleStatusBarComponentRequest:(ITMClientOriginatedMessage *)request
                             connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerStatusBarComponentRequest:request.statusBarComponentRequest
                                          handler:^(ITMStatusBarComponentResponse *theResponse) {
                                              if (handled) {
                                                  XLog(@"Double callback in API request handler - ignoring");
                                                  return;
                                              }
...
- (void)handleSetBroadcastDomainsRequest:(ITMClientOriginatedMessage *)request
                              connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerSetBroadcastDomainsRequest:request.setBroadcastDomainsRequest
                                           handler:^(ITMSetBroadcastDomainsResponse *theResponse) {
                                               if (handled) {
                                                   XLog(@"Double callback in API request handler - ignoring");
                                                   return;
                                               }
...
- (void)handleCloseRequest:(ITMClientOriginatedMessage *)request
                connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerCloseRequest:request.closeRequest
                             handler:^(ITMCloseResponse *theResponse) {
                                 if (handled) {
                                     XLog(@"Double callback in API request handler - ignoring");
                                     return;
                                 }
...
- (void)handleInvokeFunctionRequest:(ITMClientOriginatedMessage *)request
                         connection:(iTermWebSocketConnection *)webSocketConnection {
    ITMServerOriginatedMessage *response = [self newResponseForRequest:request];

    __block BOOL handled = NO;
    __weak __typeof(self) weakSelf = self;
    [_delegate apiServerInvokeFunctionRequest:request.invokeFunctionRequest
                                      handler:^(ITMInvokeFunctionResponse *theResponse) {
                                          if (handled) {
                                              XLog(@"Double callback in API request handler - ignoring");
                                              return;
                                          }
...
- (void)dispatchRequest:(ITMClientOriginatedMessage *)request
             connection:(iTermWebSocketConnection *)webSocketConnection {
...
// Runs on execution queue
- (void)enqueueOrDispatchRequest:(ITMClientOriginatedMessage *)request
                    onConnection:(iTermWebSocketConnection *)webSocketConnection {
    if (self.transaction) {
...
- (void)webSocketConnection:(iTermWebSocketConnection *)webSocketConnection didReadFrame:(iTermWebSocketFrame *)frame {
    ...
    if (frame.opcode == iTermWebSocketOpcodeBinary) {
        ...
        ITMClientOriginatedMessage *request = [ITMClientOriginatedMessage parseFromData:frame.payload
                                                                                  error:&parseError];
        ...
        if (request) {
            ...
            XLog(@"Failed to parse protobuf message: %@", parseError);
        }
    }
