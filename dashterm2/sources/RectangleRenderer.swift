...
class RectangleRenderer: NSObject, iTermMetalCellRendererProtocol {
    ...
    // BUG-f548: Make initializer failable to handle Metal setup failures gracefully
    @objc(initWithDevice:)
    init?(device: MTLDevice) {
        guard let cellRenderer = iTermMetalCellRenderer(device: device,
                                                        ...
                                                        piuElementSize: 1,
                                                        transientStateClass: RectangleRendererTransientState.self) else {
            DLog("ERROR: Failed to create RectangleRenderer cell renderer")
            return nil
        }
    ...
    // BUG-f1011: Mark as unavailable instead of crashing
    ...
    private override init() {
        ...
        it_fatalError("BUG-f1011: Use init(device:) instead")
    }
    ...
    private func vertexBuffer(rectangle: RectangleRendererTransientState.Rectangle,
        ...
        // BUG-1684: bindMemory is safe here because vertices() always returns a non-empty
        // 6-element array, so withUnsafeBytes provides a valid pointer with non-nil baseAddress
            ...
            withUnsafePointer(to: &color) {
                ...
                withUnsafeTemporaryAllocation(of: iTermMetalBufferBinding.self, capacity: 2) { vertexBindings in
                    ...
                    vertexBindings[1] = iTermMetalBufferBindingMake(
                        UInt(iTermVertexColorArray.rawValue),
                        colorBuffer)
    ...
    func createTransientState(forCellConfiguration configuration: iTermCellRenderConfiguration,
        ...
        guard let transientState = renderer.createTransientState(forCellConfiguration: configuration,
                                                                  commandBuffer: commandBuffer) as? RectangleRendererTransientState else {
            return nil
        }
