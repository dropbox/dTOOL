...
@implementation iTermStatusBarClockComponent {
    ...
    // BUG-7354: Track scheduled fetch to prevent dispatch_after accumulation
    NSInteger _fetchGeneration;
}
...
- (void)fetchTimeOffset {
    ...
    // BUG-7354: Increment generation to invalidate any pending dispatch_after
    // This prevents accumulation of infinite dispatch chains when fetchTimeOffset
    // is called multiple times (e.g., on rapid variable changes)
    _fetchGeneration++;
    const NSInteger currentGeneration = _fetchGeneration;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(15 * 60 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        // Only continue if this is still the most recent fetch chain
        if (strongSelf && strongSelf->_fetchGeneration == currentGeneration) {
            [strongSelf fetchTimeOffset];
        }
    });
}
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
    ...
    formatKnob.helpURL = [NSURL URLWithString:@"https://dashterm.com/clock-status-bar-component-help"];
