...
//
// BUG-113: Password manager strings updated to DashTerm2 branding
// BUG-284: Password manager erase warning updated to DashTerm2
...
@interface iTermPasswordManagerWindowController () <
    NSTableViewDataSource, NSTableViewDelegate, NSControlTextEditingDelegate, NSWindowDelegate, NSMenuItemValidation>
...
+ (void)fetchAccountsWithWindow:(NSWindow *)window
    ...
    [[self dataSource]
        fetchAccountsWithContext:[[RecipeExecutionContext alloc] initWithWindow:window]
                      completion:^(NSArray<id<PasswordManagerAccount>> *_Nonnull accounts) {
                          // Sort accounts
                          NSArray<id<PasswordManagerAccount>> *result = [accounts
                          ...
                          [self setCachedCombinedAccountNames:[result mapWithBlock:^id _Nonnull(
                                                                          id<PasswordManagerAccount> _Nonnull account) {
...
- (NSArray<id<PasswordManagerAccount>> *)accounts:(NSArray<id<PasswordManagerAccount>> *)accounts
                                       filteredBy:(NSString *)filter {
...
- (void)awakeFromNib {
    ...
    // NSGlassEffectView requires macOS 26 SDK
    ...
    if ([[self.class dataSourceProvider] authenticated] && ![self.currentDataSource checkAvailability]) {
...
- (NSEvent *)caughtKeyDownEvent:(NSEvent *)event {
    if ([self eventIsTab:event] && [self tabShouldSelectTwoFactorField] && [_twoFactorCode acceptsFirstResponder]) {
...
- (void)updateWithAvailability:(BOOL)available {
    ...
    const BOOL shouldEnableButtons =
        ([_tableView selectedRow] >= 0 && [_delegate iTermPasswordManagerCanEnterPassword]);
    if (_didSendUserName) {
        const BOOL enable = (shouldEnableButtons && self.selectedUserName.length > 0 &&
        ...
        [_secondaryButton setEnabled:(shouldEnableButtons && self.selectedUserName.length > 0 &&
                                      [_delegate iTermPasswordManagerCanEnterUserName])];
    }
...
// BUG-505: Add bounds check for selectedRow against _entries.count
- (id<PasswordManagerAccount>)selectedAccount {
    NSInteger selectedRow = _tableView.selectedRow;
    if (selectedRow < 0 || (NSUInteger)selectedRow >= _entries.count) {
        return nil;
    }
    return _entries[selectedRow];
}
...
- (void)reallySelectAccountName:(NSString *)name {
    ...
    if (index != NSNotFound) {
        ...
        DLog(@"failed to find %@ among %@", name,
             [[_entries mapWithBlock:^id _Nullable(id<PasswordManagerAccount> _Nonnull anObject) {
...
+ (NSString *)randomPassword {
    ...
    // Max 18 chars for password
    NSMutableString *randomPassword = [NSMutableString stringWithCapacity:18];
...
- (IBAction)reloadItems:(id)sender {
    ...
    [self.currentDataSource reload:^{
        [weakSelf ifCancelCountUnchanged:cancelCount
                                 perform:^{
...
- (IBAction)resetIntegrationConfiguration:(id)sender {
    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:@"This will erase DashTerm2's configuration settings for this password "
                                           @"manager. The actual passwords will remain unaffected. You'll have to go "
                                           @"through some setup steps to use it again. This action cannot be undone."
                                   actions:@[ @"OK", @"Cancel" ]
                                 accessory:nil
                                identifier:nil
...
- (IBAction)add:(id)sender {
    ...
    [self.window beginSheet:_newAccountPanel
...
- (IBAction)cancelNewAccount:(id)sender {
    // BUG-1516: Securely clear password by overwriting with zeros before clearing
...
- (IBAction)reallyAdd:(id)sender {
    ...
    if (index != NSNotFound) {
        DLog(@"Already have an account at index %@ for name=%@ user=%@", @(index), _newAccount.stringValue,
             _newUserName.stringValue);
        [_newAccountPanel it_shakeNo];
        return;
    }
...
- (IBAction)appendOTP:(id)sender {
    ...
    if (account) {
        ...
        [self.currentDataSource
            toggleShouldSendOTPWithContext:self.recipeExecutionContext
                                forAccount:account
                                completion:^(id<PasswordManagerAccount> _Nullable replacement, NSError *error) {
                                    [weakSelf reloadItemsWithCompletion:^{
                                        [weakSelf ifCancelCountUnchanged:cancelCount
                                                                 perform:^{
                                                                     [self decrBusy];
                                                                     if (replacement) {
                                                                         [weakSelf selectAccount:replacement
...
- (void)revealPassword:(NSString *)password forAccountName:(NSString *)accountName {
    ...
    // BUG-1517: Use NSSecureTextField in accessory view instead of informativeText
    // to prevent password from being visible in plaintext in the alert
    NSAlert *alert = [[NSAlert alloc] init];
    ...
    NSTextField *passwordField = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 0, 300, 24)];
    passwordField.stringValue = password;
    passwordField.editable = NO;
    passwordField.selectable = YES;
    passwordField.bezeled = YES;
    passwordField.bezelStyle = NSTextFieldSquareBezel;
    passwordField.font = [NSFont monospacedSystemFontOfSize:12 weight:NSFontWeightRegular];
    ...
    [self runModal:alert
        completion:^(NSModalResponse response) {
            // BUG-1516: Clear the password field after dialog closes
...
// BUG-1526: Track clipboard change count to clear only if unchanged
...
- (void)copy:(NSString *)password {
    ...
    // BUG-1526: Schedule clipboard clearing after 60 seconds
    sPasswordClipboardChangeCount = pasteboard.changeCount;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(60 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSPasteboard *pb = [NSPasteboard generalPasteboard];
        // Only clear if the clipboard hasn't been modified since we set it
        if (pb.changeCount == sPasswordClipboardChangeCount) {
            [pb clearContents];
            DLog(@"Password cleared from clipboard after timeout");
        }
    });
}
...
- (void)didFetchPasswordToCopy:(NSString *)password {
    // BUG-1526: Use the copy: method for consistent clipboard clearing behavior
    [self copy:password];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    ...
    if (menuItem.action == @selector(toggleRequireAuthenticationAfterScreenLocks:)) {
        ...
        menuItem.state =
            [iTermUserDefaults requireAuthenticationAfterScreenLocks] ? NSControlStateValueOn : NSControlStateValueOff;
        ...
        return allowed;
    } else if (menuItem.action == @selector(editAccountName:) || menuItem.action == @selector(editUserName:) ||
               menuItem.action == @selector(copyPassword:) || menuItem.action == @selector(revealPassword:)) {
        ...
        menuItem.state =
            [iTermUserDefaults shouldSendReturnAfterPassword] ? NSControlStateValueOn : NSControlStateValueOff;
        return YES;
...
- (IBAction)toggleRequireAuthenticationToOpenPasswordManager:(id)sender {
    iTermSecureUserDefaults.instance.requireAuthToOpenPasswordManager =
        !iTermSecureUserDefaults.instance.requireAuthToOpenPasswordManager;
}

- (IBAction)toggleRequireAuthenticationAfterScreenLocks:(id)sender {
    [iTermUserDefaults
        setRequireAuthenticationAfterScreenLocks:![iTermUserDefaults requireAuthenticationAfterScreenLocks]];
}
...
- (void)fetchPasswordForRow:(NSInteger)index completion:(void (^)(NSString *password, NSString *otp))completion {
    ...
    [_entries[index]
        fetchPasswordWithContext:self.recipeExecutionContext
                      completion:^(NSString *password, NSString *otp, NSError *error) {
                          [weakSelf
                              ifCancelCountUnchanged:cancelCount
                                             perform:^{
                                                 [weakSelf decrBusy];
                                                 if (error) {
                                                     ...
                                                     alert.messageText = [NSString
                                                         stringWithFormat:
                                                             @"Could not get password. Keychain query failed: %@",
...
- (NSUInteger)indexOfDisplayName:(NSString *)name {
    return [_entries
        indexOfObjectPassingTest:^BOOL(id<PasswordManagerAccount> _Nonnull entry, NSUInteger idx, BOOL *_Nonnull stop) {
                                     ...
                                     [weakSelf ifCancelCountUnchanged:cancelCount
                                                              perform:^{
                                                                  ...
                                                                  [weakSelf setAccounts:accounts
                                                                               filtered:[weakSelf accounts:accounts
                                                                                                filteredBy:filter]];
                                                                  completion();
                                                              }];
                                 }];
...
- (void)decrBusy {
    // BUG-2209: Guard against underflow - assert only works in debug builds
    if (_busyCount <= 0) {
        ELog(@"BUG-2209: decrBusy called when _busyCount is %ld (should be > 0)", (long)_busyCount);
        _busyCount = 0; // Reset to valid state
        return;
    }
    _busyCount -= 1;
    if (_busyCount == 0) {
                                                                         ...
                                                                         completion:^(NSError *_Nullable error) {
                                                                             [weakSelf
                                                                                 ifCancelCountUnchanged:cancelCount
                                                                                                perform:^{
                                                                                                    const NSInteger
                                                                                                        cancelCount = [weakSelf
                                                                                                            incrBusy]; // 3
                                                                                                    [weakSelf
                                                                                                        decrBusy]; // (2)
                                                                                                    [[weakSelf
                                                                                                        currentDataSource] addUserName:
                                                                                                                               userName
                                                                                                                           accountName:
                                                                                                                               accountName
                                                                                                                              password:
                                                                                                                               ...
                                                                                                                               context:
                                                                                                                                   context
                                                                                                                            completion:^(
                                                                                                                                id<PasswordManagerAccount> _Nullable replacement,
                                                                                                                                NSError *_Nullable error) {
                                                                                                                                [weakSelf
                                                                                                                                    ifCancelCountUnchanged:
                                                                                                                                        cancelCount
                                                                                                                                                   perform:^{
                                                                                                                                                       DLog(
                                                                                                                                                           @"%@",
                                                                                                                                                           error);
                                                                                                                                                       const NSInteger cancelCount =
                                                                                                                                                           [weakSelf
                                                                                                                                                               incrBusy]; // 4
                                                                                                                                                       [weakSelf
                                                                                                                                                           decrBusy]; // (3)
...
- (void)updateFilter {
    [self setAccounts:_unfilteredEntries
             filtered:[self accounts:_unfilteredEntries filteredBy:[_searchField stringValue]]];
}
