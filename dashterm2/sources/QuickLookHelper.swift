...
class QuickLookHelper: NSResponder, QLPreviewPanelDataSource, QLPreviewPanelDelegate {
    ...
    private let fileURLsLock = NSLock()
    ...
    func showQuickLook(for urls: [URL],
        ...
        fileURLsLock.withLock {
            self.fileURLs = urls
        }
        self.sourceRect = sourceRect
        self.windowHoldingUs = window

        // BUG-1659: Use guard let instead of force unwrap for window.contentView
    ...
    func showQuickLookWithDownloads(for urls: [URL], from sourceRect: NSRect) {
        // BUG-987: Cancel any existing downloads before starting new session
        cancelDownloads()
    ...
    private func cancelDownloads() {
        let tasksToCancel = downloadTasksLock.withLock { () -> [Task<Void, Never>] in
            let copy = downloadTasks
    ...
    private func downloadUrlToTemporaryFile(_ url: URL) async -> URL? {
        ...
        do {
            ...
            // BUG-1048: Validate HTTP status code
            if let httpResponse = response as? HTTPURLResponse,
               !(200..<300).contains(httpResponse.statusCode) {
                DLog("HTTP error \(httpResponse.statusCode) for \(url)")
                return nil
            }
    ...
    func numberOfPreviewItems(in panel: QLPreviewPanel!) -> Int {
        return fileURLsLock.withLock { fileURLs.count }
    }

    func previewPanel(_ panel: QLPreviewPanel!,
                      previewItemAt index: Int) -> QLPreviewItem! {
        return fileURLsLock.withLock {
            guard index < fileURLs.count else { return nil }  // swiftlint:disable:this empty_count
    ...
    func previewPanel(_ panel: QLPreviewPanel!,
        ...
        return fileURLsLock.withLock {
            guard idx >= 0 && idx < fileURLs.count else {  // swiftlint:disable:this empty_count
                return nil
            }
