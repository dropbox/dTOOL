...
func sanitizeMarkdown(_ input: String) -> String {
    ...
    // BUG-1667: Use limitedBy to safely calculate index offset
    if let match = matches.first, match.range.length > 0,
       let endIndex = input.index(input.startIndex, offsetBy: match.range.length, limitedBy: input.endIndex) {
        let result = input.replacingOccurrences(of: " ", with: "", options: [], range: input.startIndex..<endIndex)
        return result
    }
...
func AttributedStringForSystemMessageMarkdown(_ unsafeString: String,
    ...
    // BUG-1597: Use nil coalescing instead of force unwrap for color parsing
    let md = SwiftyMarkdownForMessage(
        string: unsafeString,
        linkColor: linkColor,
        textColor: .it_dynamicColor(forLightMode: NSColor(fromHexString: "#202020") ?? .darkGray,
                                    darkMode: NSColor(fromHexString: "#ffffc0") ?? .systemYellow))
...
private func AttributedStringForMessage(_ md: SwiftyMarkdown,
    ...
    if #available(macOS 11.0, *) {
        // BUG-1763: Use nil coalescing with fallback instead of force unwrap
        let image = NSImage(systemSymbolName: SFSymbol.docOnDoc.rawValue, accessibilityDescription: "Copy") ?? NSImage()
        // BUG-1732: Use as? with fallback to NSMutableAttributedString initializer
        let modified = (attributedString.mutableCopy() as? NSMutableAttributedString) ?? NSMutableAttributedString(attributedString: attributedString)
        var ranges = [NSRange]()
        ...
        attributedString.enumerateAttribute(
            ...
            if let previous = ranges.last {
                // BUG-f994: Use limitedBy to safely calculate String.Index offsets
                // previous.upperBound and range.lowerBound could be out of bounds
                guard let startIndex = utf16String.index(utf16String.startIndex,
                                                         offsetBy: previous.upperBound,
                                                         limitedBy: utf16String.endIndex),
                      let endIndex = utf16String.index(utf16String.startIndex,
                                                       offsetBy: range.lowerBound,
                                                       limitedBy: utf16String.endIndex),
                      startIndex <= endIndex else {
                    // Invalid range - just append this range separately
                    ranges.append(range)
                    return
                }
        ...
        if let didCopy {
            for range in ranges.reversed() {
                ...
                // BUG-f995: Guard against out-of-bounds index when accessing attributes
                // After insertButton, modified.length has increased but range.location + 1 could still
                // be out of bounds if range was at the very end of the original string
                let attrIndex = range.location + 1
                if attrIndex < modified.length {
                    modified.insert(
                        NSAttributedString(
                            string: " ",
                    ...
                    // Fall back to inserting a plain space if index is out of bounds
                    modified.insert(NSAttributedString(string: " "), at: min(attrIndex, modified.length))
                }
            }
        }
        return modified
