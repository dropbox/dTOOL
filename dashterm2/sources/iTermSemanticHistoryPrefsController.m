...
static NSString *const iTermSemanticHistoryPrefsControllerCaveatTextFieldDidClickOnLink =
    @"iTermSemanticHistoryPrefsControllerCaveatTextFieldDidClickOnLink";
...
- (BOOL)textView:(NSTextView *)textView clickedOnLink:(id)link atIndex:(NSUInteger)charIndex {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermSemanticHistoryPrefsControllerCaveatTextFieldDidClickOnLink
                      object:link];
    return YES;
}
@end
...
+ (BOOL)applicationExists:(NSString *)bundleId {
    ...
    if (count > 0) {
        if ([bundleId isEqualToString:kSublimeText2Identifier] || [bundleId isEqualToString:kSublimeText3Identifier] ||
...
+ (NSArray *)editorsInPreferenceOrder {
    ...
    return @[
        kSublimeText4Identifier,   kSublimeText3Identifier, kSublimeText2Identifier,
        kMacVimIdentifier,         kTextmateIdentifier,     kTextmate2Identifier,
        kBBEditIdentifier,         kNovaAppIdentifier,      kAtomIdentifier,
        kVSCodeIdentifier,         kVSCodiumIdentifier1,    kVSCodiumIdentifier2,
        kVSCodeInsidersIdentifier, kEmacsAppIdentifier,     kIntelliJIDEAIdentifierUE,
        kIntelliJIDEAIdentifierCE, kXcodeAppIdentifier,     kWebStormIdentifier,
        kRiderIdentifier,          kCursorAppIdentifier,    kZedAppIdentifier,
        kWindsurfIdentifier,       kBobIdentifier
    ];
}
...
+ (NSDictionary *)identifierToTagMap {
    NSDictionary *tags = @{
        kSublimeText4Identifier : @(kSublimeText4Tag),
...
- (void)awakeFromNib {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showPopover)
                                               ...
                                               object:nil];
    NSSet<NSString *> * (^fallbackSource)(NSString *) =
        [iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession];
    NSArray *mine = @[
        kSemanticHistoryPathSubstitutionKey, kSemanticHistoryPrefixSubstitutionKey,
        kSemanticHistorySuffixSubstitutionKey, kSemanticHistoryWorkingDirectorySubstitutionKey,
        kSemanticHistoryLineNumberKey, kSemanticHistoryColumnNumberKey
    ];
    _textFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:^NSSet<NSString *> *(NSString *prefix) {
    ...
    NSDictionary *names = @{
        kSublimeText4Identifier : @"Sublime Text 4",
    ...
    NSMutableDictionary *items = [[NSMutableDictionary alloc] initWithCapacity:16];
...
+ (NSDictionary *)tagToActionMap {
    return @{
        @1 : kSemanticHistoryBestEditorAction,
...
- (NSString *)editorIdentifier {
    NSDictionary *map = @{
        @(kSublimeText4Tag) : kSublimeText4Identifier,
...
- (NSAttributedString *)attributedStringWithLearnMoreLinkAfterText:(NSString *)text {
    NSDictionary *attributes =
        @{NSFontAttributeName : caveat_.font ?: [NSFont systemFontOfSize:[NSFont smallSystemFontSize]]};
    NSAttributedString *legacy = [NSAttributedString attributedStringWithString:text attributes:attributes];
    NSAttributedString *learnMore =
        [NSAttributedString attributedStringWithLinkToURL:@"dashterm2-private://semantic-history-learn-more/"
                                                   string:@"Learn more"];
...
- (NSString *)detailTextForCurrentMode {
    NSString *subs = @"You can provide substitutions as follows:\n"
                     ...
                     @"\n"
                     @"This is also an interpolated string evaluated in the context of the current session. In "
                     @"addition to the usual variables, the following substitutions are available:\n"
    ...
    switch ([[action_ selectedItem] tag]) {
        ...
        case 5:
            return [@"In this mode semantic history will be activated on any click, even if you click on something "
...
- (IBAction)actionChanged:(id)sender {
    ...
    switch ([[action_ selectedItem] tag]) {
        case 1:
            [caveat_
                setStringValue:@"When you activate Semantic History on a filename, the associated app loads the file."];
            hideCaveat = NO;
            break;

        case 2: {
            [[text_ cell] setPlaceholderString:@"Enter URL."];
            NSString *text = @"When you activate Semantic History on a filename, the browser opens a URL.\n"
        ...
        case 3:
            hideEditors = NO;
            [caveat_ setStringValue:@"When you activate Semantic History on a text file, the specified editor opens "
        ...
        case 4: {
            ...
            NSString *text = @"Command runs when you activate Semantic History on any filename. "
                             @"Use \\1 for filename, \\2 for line number, \\3 for text before click, \\4 for text "
                             @"after click, \\5 for pwd. "
        ...
        case 5: {
            ...
            NSString *text =
                ...
                @"Use \\1 for filename, \\2 for line number, \\3 for text before click, \\4 for text after click, \\5 "
                @"for pwd. ";
        ...
        case 6: {
            [[text_ cell] setPlaceholderString:@"Enter command"];
            NSString *text = @"Coprocess runs when you activate Semantic History on any filename. "
                             @"Use \\1 for filename, \\2 for line number, \\3 for text before click, \\4 for text "
                             @"after click, \\5 for pwd. ";
            caveat_.attributedStringValue = [self attributedStringWithLearnMoreLinkAfterText:text];
            hideCaveat = NO;
...
- (NSDictionary *)prefs {
    return @{
        kSemanticHistoryActionKey : [self actionIdentifier],
...
- (void)showPopover {
    ...
    [_popoverVC.popover showRelativeToRect:text_.bounds ofView:text_ preferredEdge:NSRectEdgeMaxY];
}
