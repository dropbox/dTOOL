...
- (instancetype)initWithTransferrableFile:(TransferrableFile *)transferrableFile {
    ...
    if (self) {
        ...
        _effectView = [[NSVisualEffectView alloc] initWithFrame:NSMakeRect(5, 0, kWidth - 10, kHeight)];
        ...
        _contentView = [[TransferrableFileMenuItemView alloc] initWithFrame:NSMakeRect(0, 0, kWidth, kHeight)
...
- (void)loadView {
    self.view = [[NSView alloc] initWithFrame:NSMakeRect(0, 0, kWidth, kHeight)];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    ...
    if ([menuItem action] == @selector(stop:)) {
        return (status == kTransferrableFileStatusStarting || status == kTransferrableFileStatusTransferring);
    }
    ...
    if ([menuItem action] == @selector(removeFromList:)) {
        return (status == kTransferrableFileStatusFinishedSuccessfully ||
                status == kTransferrableFileStatusFinishedWithError || status == kTransferrableFileStatusCancelled);
    }
...
- (void)getInfo:(id)sender {
    ...
    if (_transferrableFile.destination) {
        extra = [NSString stringWithFormat:@"\nDestination: %@", _transferrableFile.destination];
    } else if (_transferrableFile.localPath) {
        extra = [NSString stringWithFormat:@"\nLocal path: %@", _transferrableFile.localPath];
    }
    NSString *text = [NSString stringWithFormat:@"%@\n\nStatus: %@%@", [_transferrableFile displayName],
                                                [self stringForStatus:_transferrableFile.status], extra];
