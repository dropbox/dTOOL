...
@class DashTerm2Application, DashTerm2Window, DashTerm2Tab, DashTerm2Session;

enum DashTerm2SaveOptions {
	DashTerm2SaveOptionsYes = 'yes ' /* Save the file. */,
	DashTerm2SaveOptionsNo = 'no  ' /* Do not save the file. */,
	DashTerm2SaveOptionsAsk = 'ask ' /* Ask the user whether or not to save the file. */
};
typedef enum DashTerm2SaveOptions DashTerm2SaveOptions;

@protocol DashTerm2GenericMethods
...
- (DashTerm2Tab *) createTabWithProfile:(NSString *)withProfile command:(NSString *)command;  // Create a new tab
- (DashTerm2Tab *) createTabWithDefaultProfileCommand:(NSString *)command;  // Create a new tab with the default profile
...
- (DashTerm2Session *) splitVerticallyWithProfile:(NSString *)withProfile command:(NSString *)command;  // Split a session vertically.
- (DashTerm2Session *) splitVerticallyWithDefaultProfileCommand:(NSString *)command;  // Split a session vertically, using the default profile for the new session
- (DashTerm2Session *) splitVerticallyWithSameProfileCommand:(NSString *)command;  // Split a session vertically, using the original session's profile for the new session
- (DashTerm2Session *) splitHorizontallyWithProfile:(NSString *)withProfile command:(NSString *)command;  // Split a session horizontally.
- (DashTerm2Session *) splitHorizontallyWithDefaultProfileCommand:(NSString *)command;  // Split a session horizontally, using the default profile for the new session
- (DashTerm2Session *) splitHorizontallyWithSameProfileCommand:(NSString *)command;  // Split a session horizontally, using the original session's profile for the new session
...
@interface DashTerm2Application : SBApplication

- (SBElementArray<DashTerm2Window *> *) windows;

@property (copy) DashTerm2Window *currentWindow;  // The frontmost window
@property (copy, readonly) NSString *name;  // The name of the application.
@property (readonly) BOOL frontmost;  // Is this the frontmost (active) application?
@property (copy, readonly) NSString *version;  // The version of the application.

- (DashTerm2Window *) createWindowWithProfile:(NSString *)x command:(NSString *)command;  // Create a new window
- (DashTerm2Window *) createWindowWithDefaultProfileCommand:(NSString *)command;  // Create a new window with the default profile

@end

// A window.
@interface DashTerm2Window : SBObject <DashTerm2GenericMethods>

- (SBElementArray<DashTerm2Tab *> *) tabs;
...
@property (copy) DashTerm2Tab *currentTab;  // The currently selected tab
@property (copy) DashTerm2Session *currentSession;  // The current session in a window
...
// A terminal tab
@interface DashTerm2Tab : SBObject <DashTerm2GenericMethods>

- (SBElementArray<DashTerm2Session *> *) sessions;

@property (copy) DashTerm2Session *currentSession;  // The current session in a tab
...
// A terminal session
@interface DashTerm2Session : SBObject <DashTerm2GenericMethods>
