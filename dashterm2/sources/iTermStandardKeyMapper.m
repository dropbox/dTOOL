...
- (NSString *)preCocoaString {
    ...
    const unichar characterIgnoringModifiers =
        [charactersIgnoringModifiers length] > 0 ? [charactersIgnoringModifiers characterAtIndex:0] : 0;
    ...
    const BOOL onlyControlPressed =
        (modflag & (NSEventModifierFlagControl | NSEventModifierFlagCommand | NSEventModifierFlagOption)) ==
...
- (NSData *)dataForRegularKeypress {
    ...
    const unichar characterIgnoringModifier =
        [charactersIgnoringModifiers length] > 0 ? [charactersIgnoringModifiers characterAtIndex:0] : 0;
    ...
    if ([_event it_isNumericKeypadKey]) {
        DLog(@"PTYSession keyDown numeric keypad");
        return [_configuration.outputFactory keypadDataForString:characters modifiers:_event.it_modifierFlags];
    }
...
- (iTermOptionKeyBehavior)optionKeyBehavior {
    ...
    // BUG-f1348: Replace assert() with guard - default to left option key if neither is pressed
    // This can happen in edge cases with synthetic events or unusual keyboard configurations
    if (!leftAltPressed && !rightAltPressed) {
        ELog(@"optionKeyBehavior: Neither left nor right alt pressed, defaulting to leftOptionKey");
...
- (NSData *)dataForOptionModifiedKeypress {
    ...
    // BUG-411: Replace assert(NO) with DLog - this should never be reached
    // since switch exhausts all OPT_* enum values, but handle gracefully
    // if new option key behavior is added without updating this switch
    DLog(@"iTermStandardKeyMapper: Unexpected option key behavior - returning data as-is");
    return data;
}
...
- (NSData *)dataForFunctionKeyPress {
    ...
    switch (unicode) {
        ...
        case NSF35FunctionKey:
            return [_configuration.outputFactory keyFunction:unicode - NSF1FunctionKey + 1 modifiers:modifiers];
    }
...
- (BOOL)shouldSquelchKeystrokeWithString:(NSString *)keystr modifiers:(NSEventModifierFlags)modflag {
...
NSDictionary *iTermStandardKeyMapperConfigurationDictionaryValue(iTermStandardKeyMapperConfiguration *config) {
    return @{
        @"output" : config.outputFactory.configDictionary ?: @{},
