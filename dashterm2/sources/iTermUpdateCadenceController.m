...
- (void)didHandleInputWithThroughput:(NSInteger)estimatedThroughput {
    DLog(@"didHandleInput");
    // DashTerm2: Always update immediately after keystroke for instant feel.
    // Removed throughput check - we want instant response regardless of bandwidth.
    if ([self lastKeystrokeWasRecent]) {
        DLog(@"  recent keystroke so call updateDisplay immediately");
        [self updateDisplay];
    }
}
