...
- (instancetype)initWithDatabase:(FMDatabase *)db lockName:(NSString *)lockName NS_DESIGNATED_INITIALIZER;
- (instancetype)init NS_UNAVAILABLE;
@end

// Context for integrity check coordination between main thread and timeout block
typedef enum {
...
@interface iTermDatabaseIntegrityCheckContext : NSObject {
  @public
    os_unfair_lock _lock;
}
@property (nonatomic) iTermDatabaseIntegrityCheckState state;
@end

@implementation iTermDatabaseIntegrityCheckContext
- (instancetype)init {
    self = [super init];
    if (self) {
        _state = iTermDatabaseIntegrityCheckStateQuerying;
...
- (void)unlink {
    // BUG-f923: Guard against unlinking while database is open
    if (_db.isOpen) {
        ELog(@"iTermDatabase: Attempt to unlink while database is still open");
        return;
    }
    if (!self.url) {
        return;
...
- (void)logStatement:(NSString *)sql vaList:(va_list)args {
    ...
    DebugLogImpl(__FILE__, __LINE__, __FUNCTION__, statement);
}
...
- (void)integrityCheckDidTimeOut:(iTermDatabaseIntegrityCheckContext *)context {
    ...
    os_unfair_lock_lock(&context->_lock);
    if (context.state != iTermDatabaseIntegrityCheckStateQuerying) {
        os_unfair_lock_unlock(&context->_lock);
    ...
    context.state = iTermDatabaseIntegrityCheckStateInterrupted;
    os_unfair_lock_unlock(&context->_lock);
}

- (BOOL)passesIntegrityCheck {
    // BUG-f924: Guard lock state check instead of crashing
    if (_lockName && !_advisoryLock) {
        ELog(@"iTermDatabase: passesIntegrityCheck called without advisory lock when lockName is set");
        return NO;
    }
    ...
    iTermDatabaseIntegrityCheckContext *context = [[iTermDatabaseIntegrityCheckContext alloc] init];
    __weak __typeof(self) weakSelf = self;
    const NSTimeInterval timeout = 10;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeout * NSEC_PER_SEC)),
                   dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                       [weakSelf integrityCheckDidTimeOut:context];
                   });
    DLog(@"Begin pragma");
    ...
    switch (context.state) {
        case iTermDatabaseIntegrityCheckStateQuerying:
            DLog(@"state<-finished");
            context.state = iTermDatabaseIntegrityCheckStateFinished;
            break;
        case iTermDatabaseIntegrityCheckStateFinished:
            // BUG-13007: Replace assert(NO) with ELog and failed=YES for invalid state transition
            // This indicates integrityCheckDidFinish was called twice which shouldn't happen
            ELog(@"Unexpected state: integrityCheckDidFinish called while already finished");
            failed = YES;
            break;
...
- (BOOL)open {
    DLog(@"Open");
    // BUG-f925: Guard lock state check instead of crashing
    if (_lockName && !_advisoryLock) {
        ELog(@"iTermDatabase: open called without advisory lock when lockName is set");
        return NO;
    }
