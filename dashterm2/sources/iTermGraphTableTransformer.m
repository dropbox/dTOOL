...
static iTermEncoderGraphRecord *iTermGraphDeltaEncoderMakeGraphRecord(NSNumber *nodeID, NSDictionary *nodes,
    ...
    NSArray<iTermEncoderGraphRecord *> *childGraphRecords = [childNodeIDs mapWithBlock:^id(NSNumber *childNodeID) {
    ...
    DLog(@"key=%@ id=%@ rowid=%@ children=%@ pod=%@", nodeDict[@"key"], nodeDict[@"identifier"], nodeDict[@"rowid"],
...
- (NSDictionary<NSNumber *, NSMutableDictionary *> *)nodes:(out NSNumber **)rootNodeIDOut {
    // Create nodes
    NSMutableDictionary<NSNumber *, NSMutableDictionary *> *nodes =
        [[NSMutableDictionary alloc] initWithCapacity:_nodeRows.count];
    for (NSArray *row in _nodeRows) {
        if (row.count != 5) {
            DLog(@"Wrong number of items in row: %@", row);
            _lastError = [NSError errorWithDomain:@"com.dashterm.dashterm2.graph-transformer"
                                             code:1
                                         userInfo:@{
                                             NSLocalizedDescriptionKey :
                                                 [NSString stringWithFormat:@"Wrong number of items in row: %@", row]
                                         }];
            return nil;
        }
        ...
        if (!row || !key || !identifier || !parent || !data) {
            DLog(@"Bad row: %@", row);
            _lastError = [NSError
                errorWithDomain:@"com.dashterm.dashterm2.graph-transformer"
                           code:1
        ...
        nodes[rowid] = [@{
            @"pod" : [NSMutableDictionary dictionaryWithCapacity:8],
            @"key" : key,
            @"identifier" : identifier,
            @"parent" : parent,
            @"children" : [NSMutableArray arrayWithCapacity:4], // Child node references
            @"rowid" : rowid,
            @"data" : data
        } mutableCopy];
    }
...
- (BOOL)attachChildrenToParents:(NSDictionary<NSNumber *, NSMutableDictionary *> *)nodes
    ...
    __block BOOL ok = YES;
    [nodes enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull nodeid, NSMutableDictionary *_Nonnull nodeDict,
                                               BOOL *_Nonnull stop) {
                ...
                errorWithDomain:@"com.dashterm.dashterm2.graph-transformer"
                       ...
                       userInfo:@{
                           NSLocalizedDescriptionKey : [NSString
                               stringWithFormat:@"Dangling parent pointer %@ from %@", nodeDict[@"parent"], nodeid]
                       }];
            *stop = YES;
        }
