...
- (void)dealloc {
    // iTermSnippetsDidChangeNotification cleanup is automatic via associated object.
...
- (void)reload {
    ...
    NSMutableArray *tagTree = [NSMutableArray arrayWithCapacity:16]; // Snippet tag tree nodes
...
- (void)addTags:(NSArray<NSString *> *)tags snippet:(iTermSnippet *)snippet toTree:(NSMutableArray *)tree {
    [tags enumerateObjectsUsingBlock:^(NSString *_Nonnull tag, NSUInteger idx, BOOL *_Nonnull stop) {
        [self addTagComponents:[tag componentsSeparatedByString:@"/"] snippet:snippet toTree:tree];
    }];
}

- (id)nodeForFolderNamed:(NSString *)name {
    return @{@"tag" : name, @"container" : [NSMutableArray arrayWithCapacity:4]}; // Child nodes
}

- (id)nodeForTagNamed:(NSString *)name snippet:(iTermSnippet *)snippet {
    return @{
        @"tag" : name,
        @"snippets" : @[ snippet ].mutableCopy,
        @"container" : [NSMutableArray arrayWithCapacity:4]
    }; // Child nodes
}
...
- (void)addTagComponents:(NSArray<NSString *> *)components
    ...
    if (components.count == 1) {
        ...
        [self addTagComponents:[components subarrayFromIndex:1] snippet:snippet toTree:container];
    }
}
