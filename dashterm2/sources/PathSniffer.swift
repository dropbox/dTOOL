...
class PathSniffer: NSObject {
    ...
    // RC-018: Use a serial dispatch queue for all mutable state access.
    // The `stateLock` protects `_count` and `acceptedRanges` from data races
    // when accessed from multiple async callbacks.
    private let stateLock = NSLock()
    private var _count = 0

    var count: Int {
        get {
            stateLock.withLock { _count }
        }
        set {
            let (oldValue, newValue) = stateLock.withLock { () -> (Int, Int) in
                let old = _count
                _count = newValue
                return (old, newValue)
            }
            // Update static instances outside the state lock to avoid lock ordering issues
            if oldValue == 0 && newValue > 0 {  // swiftlint:disable:this empty_count
                Self.instancesLock.withLock {
                    Self.instances.append(self)
                }
            } else if oldValue > 0 && newValue == 0 {  // swiftlint:disable:this empty_count
                Self.instancesLock.withLock {
...
extension PathSniffer {
    ...
    private func accept(_ candidate: PathExtractor.Candidate) {
        ...
        // RC-018 / BUG-1451: acceptedRanges is accessed from async callbacks, use stateLock.
        let shouldAccept = stateLock.withLock {
