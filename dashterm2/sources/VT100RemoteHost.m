...
// Class-level lock for doppelganger creation (replaces @synchronized([VT100RemoteHost class]))
static os_unfair_lock gVT100RemoteHostLock = OS_UNFAIR_LOCK_INIT;
...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
    return [self initWithUsername:dict[kRemoteHostUserNameKey] hostname:dict[kRemoteHostHostNameKey]];
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p hostname=%@ username=%@ doppelganger=%p (%@) progenitor=%p>",
                                      self.class, self, self.hostname, self.username, _doppelganger,
                                      _isDoppelganger ? @"IsDop" : @"NotDop", _progenitor];
}

- (BOOL)isEqualToRemoteHost:(id<VT100RemoteHostReading>)other {
    return ([_hostname isEqualToString:other.hostname] && [_username isEqualToString:other.username]);
}
...
- (NSDictionary *)dictionaryValue {
    NSDictionary *dict =
        @{kRemoteHostHostNameKey : _hostname ?: [NSNull null], kRemoteHostUserNameKey : _username ?: [NSNull null]};
...
- (id<IntervalTreeObject>)doppelganger {
    os_unfair_lock_lock(&gVT100RemoteHostLock);
    // BUG-f1377: Replace assert with guard - doppelganger of doppelganger should return nil, not crash
    if (_isDoppelganger) {
        os_unfair_lock_unlock(&gVT100RemoteHostLock);
        DLog(@"WARNING: Attempted to get doppelganger of a doppelganger (VT100RemoteHost)");
        return nil;
    }
    if (!_doppelganger) {
