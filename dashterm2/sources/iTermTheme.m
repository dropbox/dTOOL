...
- (id<PSMTabStyle>)tabStyleWithDelegate:(id<PSMMinimalTabStyleDelegate>)delegate
    ...
    // Note that it_tabStyle: maps preferredStyle, which comes from settings, onto dark/light/dark high contrast/light
    // high contrast.
    switch (tabStyle) {
        case TAB_STYLE_AUTOMATIC:
        case TAB_STYLE_COMPACT:
        case TAB_STYLE_MINIMAL:
            // BUG-451: Replace assert(NO) with DLog and fallthrough to TAB_STYLE_LIGHT
            // These styles should have been resolved to concrete styles by it_tabStyle but
            // if they weren't, fall through to light style as a safe default
            DLog(@"iTermTheme tabStyleWithAppearance got unexpected tab style: %d", (int)tabStyle);
            // Intentional fallthrough to TAB_STYLE_LIGHT
        case TAB_STYLE_LIGHT:
            if (preferredStyle == TAB_STYLE_COMPACT) {
                return [[PSMYosemiteTabStyle alloc] init];
            }
            // PSMTahoeTabStyle requires macOS 26 SDK
        ...
        case TAB_STYLE_DARK:
            ...
            // PSMTahoeDarkTabStyle requires macOS 26 SDK
            // TODO: Re-enable when building with macOS 26 SDK
            return [[PSMDarkTabStyle alloc] init];
        case TAB_STYLE_LIGHT_HIGH_CONTRAST:
            // PSMTahoeLightHighContrastTabStyle requires macOS 26 SDK
            // TODO: Re-enable when building with macOS 26 SDK
            return [[PSMLightHighContrastTabStyle alloc] init];
        case TAB_STYLE_DARK_HIGH_CONTRAST:
            // PSMTahoeDarkHighContrastTabStyle requires macOS 26 SDK
            // TODO: Re-enable when building with macOS 26 SDK
            return [[PSMDarkHighContrastTabStyle alloc] init];
    }
    // BUG-452: Replace assert(NO) with ELog and default return
    // This should be unreachable but if we get here, return a safe default
    ELog(@"iTermTheme tabStyleWithAppearance reached unreachable code with tabStyle=%d, preferredStyle=%d",
         (int)tabStyle, (int)preferredStyle);
    return [[PSMYosemiteTabStyle alloc] init];
}
...
- (NSColor *)backgroundColorForDecorativeSubviewsInSessionWithTabColor:(NSColor *)tabColor
    ...
    undimmedColor =
        [self backgroundColorForDecorativeSubviewsForTabColor:undimmedColor
                                                     tabStyle:[effectiveAppearance it_tabStyle:preferredStyle]
                                       sessionBackgroundColor:sessionBackgroundColor];
    if (isFirstResponder) {
...
- (nullable NSColor *)statusBarContainerBackgroundColorForTabColor:(NSColor *)tabColor
    ...
    if ([iTermPreferences boolForKey:kPreferenceKeySeparateStatusBarsPerPane]) {
        ...
        return [self tabBarBackgroundColorForTabColor:tabColor style:tabStyle transparencyAlpha:transparencyAlpha];
    }
}
...
- (NSColor *)statusBarTextColorForEffectiveAppearance:(NSAppearance *)effectiveAppearance
    ...
    if (self.useMinimalStyle) {
        NSColor *bgColor =
            marginColor
                ?: [colorMap processedBackgroundColorForBackgroundColor:[colorMap colorForKey:kColorMapBackground]];
...
- (NSColor *)terminalWindowDecorationTextColorForBackgroundColor:(NSColor *)backgroundColor
    ...
    if (self.useMinimalStyle) {
        ...
        switch ([effectiveAppearance it_tabStyle:preferredStyle]) {
            ...
            case TAB_STYLE_COMPACT:
            case TAB_STYLE_MINIMAL:
                // BUG-453: Replace assert(NO) with DLog and fallthrough
                // These styles should have been resolved by it_tabStyle
                DLog(@"terminalWindowDecorationTextColorForBackgroundColor got unresolved style");
...
- (NSColor *)backgroundColorForDecorativeSubviewsForTabColor:(NSColor *)tabColor
    ...
    // BUG-454: Replace assert(NO) with ELog and default return
    // This handles unexpected tab styles safely by returning the tab color
    ELog(@"backgroundColorForDecorativeSubviewsForTabColor got unexpected tabStyle=%d", (int)tabStyle);
    return tabColor;
}
...
- (NSColor *)dimmedBackgroundColorWithAppearance:(NSAppearance *)appearance
    ...
    if (self.useMinimalStyle) {
        ...
        if (inactive && !dimOnlyText) {
            if (PSMShouldExtendTransparencyIntoMinimalTabBar()) {
                ...
                return [color colorDimmedBy:adjustedDimmingAmount towardsGrayLevel:0.5];
            }
        } else {
            if (PSMShouldExtendTransparencyIntoMinimalTabBar()) {
                return [color colorWithAlphaComponent:transparencyAlpha];
    ...
    switch ([appearance it_tabStyle:preferredStyle]) {
        ...
        case TAB_STYLE_COMPACT:
        case TAB_STYLE_MINIMAL:
            // BUG-455: Replace assert(NO) with DLog and fallthrough
            // These styles should have been resolved by it_tabStyle
            DLog(@"dimmedBackgroundColorWithAppearance got unresolved style");
            // Intentional fallthrough
        case TAB_STYLE_LIGHT:
            if (inactive) {
                // Not selected
