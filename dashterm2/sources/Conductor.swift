...
class Conductor: NSObject {
    ...
    private var suggestionTask: Task<Void, Never>?  // BUG-980: Track suggestion task for cancellation
    ...
    // BUG-f543: Handle nil framedPID gracefully instead of crashing
    lazy var _processInfoProvider: SSHProcessInfoProvider? = {
        ...
        guard let pid = framedPID else {
            DLog("WARNING: framedPID is nil when creating process info provider")
            return nil
        }
        let provider = SSHProcessInfoProvider(rootPID: pid, runner: self)
        provider.register(trackedPID: pid)
...
extension Conductor {
    ...
    func fetchTimeOffset(completion: @escaping (TimeInterval, String?, NSError?) -> ()) {
        ...
        framerExecPythonStatements(statements: script) { [weak self] ok, output in
            ...
            guard let diff = Double(lines[0]) else {
                completion(0, nil, iTermError("Invalid time delta: \(output)") as NSError)
                return
            }
            guard let tzName = lines.dropFirst().first else {
                completion(0, nil, iTermError("Invalid timezone output: \(output)") as NSError)
                return
            }
...
extension Conductor {
    ...
    enum Command: Equatable, Codable, CustomDebugStringConvertible {
        ...
        var stringValue: String {
            ...
            case .framerSave(let dict):
                // BUG-1635: Use nil coalescing instead of force unwrap for dictionary access
    ...
    struct ExecutionContext: Codable, CustomDebugStringConvertible {
        ...
        var size: Int {
            // BUG-f666: Add guard for pipelining check - return max size if not pipelining
            // Note: DLog not available in nested struct context
            ...
            case .framerSend:
                ...
                // BUG-f1415: Use overflow-safe multiplication to prevent crash on very large strings
                // For strings > ~1.6 billion characters, count * 4 would overflow Int on 64-bit
                let count = command.stringValue.count
                let (product, overflow) = count.multipliedReportingOverflow(by: 4)
                return overflow ? .max : product / 3
            case .framerFile(let sub):
                switch sub {
                case .fetch:
                    // BUG-f1415: Use overflow-safe multiplication
                    let count = command.stringValue.count
                    let (product, overflow) = count.multipliedReportingOverflow(by: 4)
                    return overflow ? .max : product / 3
                default:
    ...
    @objc(startJumpingTo:) func startJumping(to jumps: [SSHReconnectionInfoObjC]) {
        // BUG-1581: Use guard instead of precondition + force unwrap
        guard let firstJump = jumps.first else {
            DLog("startJumping called with empty jumps array")
            return
        }
        myJump = firstJump.state
    ...
    // BUG-f541: Return optional to handle missing resources gracefully
    private var jumpScript: String? {
        defer {
            myJump = nil
        }
        guard let jump = myJump else {
            DLog("ERROR: jumpScript accessed without myJump being set")
            return nil
        }
        guard let path = Bundle(for: Conductor.self).path(forResource: "utilities/it2ssh", ofType: nil) else {
            DLog("ERROR: Missing required resource: utilities/it2ssh")
            return nil
        }
        guard let it2ssh = try? String(contentsOfFile: path) else {
            DLog("ERROR: Failed to read resource: utilities/it2ssh")
            return nil
        }
        let code = """
        #!/usr/bin/env bash
        ...
        it2ssh_wrapper \(jump.sshargs)
        """
        return code
    }

    private func jumpWithEval() {
        guard let script = jumpScript else {
            DLog("ERROR: Failed to generate jump script - aborting jump")
            return
        }
        eval(code: script)
    }
    ...
    @objc func sendKeys(_ data: Data) {
        guard let pid = framedPID else {
            ...
            // BUG-1592: Use guard to safely convert data to string instead of force unwrap
            guard let string = String(data: data, encoding: .isoLatin1) else {
                DLog("Failed to convert data to isoLatin1 string")
                return
            }
            delegate?.conductorWrite(string: string)
            return
        }
    ...
    func fetchSuggestions(_ request: SuggestionRequest, suggestionOnly: Bool) {
        ...
        // BUG-980: Cancel previous task to prevent stale results from being delivered
        ...
        suggestionTask = Task { [weak self] in
            ...
            do {
                ...
                // Check if task was cancelled before delivering results
    ...
    private func framerJump() {
        // BUG-f541: Handle nil jumpScript gracefully
        guard let script = jumpScript else {
            DLog("ERROR: jumpScript is nil, cannot execute framer jump")
            return
        }
        send(.framerEval(script), .handleJump(StringArray()))
    }
    ...
    private func execFramer() {
        // BUG-1688: Use guard instead of force unwrap for bundle resource URL
        guard var path = Bundle(for: Self.self).url(forResource: "framer", withExtension: "py") else {
            DLog("ERROR: Could not find framer.py resource")
            return
        }
#if DEBUG
        // BUG-1688: Use if let instead of force unwrap for alternate script path
        if let alt = iTermAdvancedSettingsModel.alternateSSHIntegrationScript(), !alt.isEmpty {
        ...
        // BUG-f542: Handle missing resource gracefully instead of crashing
        guard let pythonCode = try? String(contentsOf: path) else {
            DLog("ERROR: Failed to read resource: \(path.path)")
            return
        }
        runPython(pythonCode.replacingOccurrences(of: "#{SUB}", with: customCode))
        framerVersion = .v2
    }
    ...
    private func update(executionContext: ExecutionContext, result: PartialResult) throws {
        ...
        case .handlePoll(let output, let completion):
            ...
            case .sideChannelLine(_, _, _), .abort, .canceled:
                break
            case .end(_):
                // BUG-1498: Use nil-coalescing to ensure completion is always called,
                // even if UTF-8 encoding fails (e.g., invalid byte sequences in output).
                let data = output.strings.joined(separator: "\n").data(using: .utf8) ?? Data()
            ...
            case .end(let status):
                ...
                if !shell.isEmpty &&
                    ...
                    (modifiedVars, modifiedCommandArgs) = ShellIntegrationInjector.instance.modifyRemoteShellEnvironment(
                        shellIntegrationDir: "\(home)/.dashterm2/shell-integration",
                        env: varsToSend,
                        shell: shell,
                    ...
                    let dict = ShellIntegrationInjector.instance.files(
                        destinationBase: URL(fileURLWithPath: "/$HOME/.dashterm2/shell-integration"))
    ...
    @objc(handleLine:depth:) func handle(line: String, depth: Int32) {
        ...
        case .willExecutePipeline(let contexts):
            // BUG-1581: Use guard to safely access .first instead of force unwrap
            guard let firstContext = contexts.first else {
                log("willExecutePipeline has empty contexts")
                return
            }
            let pending = Array(contexts.dropFirst())
            state = .executingPipeline(firstContext, pending)
            try? update(executionContext: firstContext, result: .line(line))
    ...
    @objc func handleUnhook() {
        ...
        case .willExecutePipeline(let contexts):
            ...
            if let firstContext = contexts.first {
                try? update(executionContext: firstContext, result: .abort)
            }
    ...
    @objc func handleCommandEnd(identifier: String, type: String, status: UInt8, depth: Int32) {
        ...
        case let .willExecutePipeline(contexts):
            // BUG-1581: Use guard to safely access .first instead of force unwrap
            guard let firstContext = contexts.first else {
                DLog("willExecutePipeline has empty contexts")
                return
            }
            do {
                try update(executionContext: firstContext, result: .end(status))
                if contexts.count == 1 {
                    ...
                    let pending = Array(contexts.dropFirst())
                    // BUG-f635: Use guard instead of it_assert - assertions stripped in release builds
                    // If pending is unexpectedly empty, go to ground state instead of crashing
                    guard !pending.isEmpty else {
                        DLog("BUG-f635: pending unexpectedly empty after dropFirst - returning to ground state")
                        state = .ground
                        dequeue()
                        return
                    }
        ...
        case let .executingPipeline(context, pending):
            do {
                ...
                if pending.isEmpty {
                    ...
                    // BUG-f635: Removed redundant it_assert - the else branch already guarantees !pending.isEmpty
    ...
    func handleTerminate(_ pid: Int32, code: Int32, depth: Int32) {
        ...
        if pid == framedPID {
            ...
            case let .willExecutePipeline(contexts):
                // BUG-1581: Use guard to safely access .first instead of force unwrap
                if let firstContext = contexts.first {
                    // BUG-f1403: Use truncatingIfNeeded for Int32->UInt8 conversion
                    // Exit codes can be negative (signal) or > 255 on some systems
                    try? update(executionContext: firstContext, result: .end(UInt8(truncatingIfNeeded: code)))
                }
            ...
            case let .executingPipeline(context, _):
                do {
                    // BUG-f1403: Use truncatingIfNeeded for Int32->UInt8 conversion
                    try update(executionContext: context, result: .end(UInt8(truncatingIfNeeded: code)))
                    backgroundJobs.removeValue(forKey: pid)
...
//        DLog("pid \(pid) channel \(channel) produced: \(string)")
        ...
        case let .willExecutePipeline(contexts):
            // BUG-1581: Use guard to safely access .first instead of force unwrap
            if let firstContext = contexts.first {
                state = .executingPipeline(firstContext, Array(contexts.dropFirst()))
            }
        case let .executingPipeline(context, _):
    ...
    private func handleSearchNotif(_ message: String) {
        if #available(macOS 11.0, *) {
            ...
            if let currentSearch, currentSearch.id == id {
                // BUG-f1416: Use index(after:) and check bounds to prevent crash
                // if space is the last character in the string
                let afterSpace = message.index(after: space)
                guard afterSpace < message.endIndex else {
                    DLog("BUG-f1416: Malformed message - space is at end of string")
                    return
                }
                let json = String(message[afterSpace...])
    ...
    private func dequeue() {
        ...
        case .ground, .recovery:
            break
        default:
            // BUG-f540: Log unexpected state instead of crashing.
            // This can happen during rapid state transitions.
            DLog("WARNING: dequeue called in unexpected state: \(state)")
            return
        }
        amendPipeline([])
    }
    ...
    private func takeNextContextPipeline(_ existing: [ExecutionContext]) -> [ExecutionContext] {
        // BUG-f667: Add guard for pipelining check on first existing context
        if let first = existing.first, !first.supportsPipelining {
            DLog("BUG-f667: First context in pipeline does not support pipelining - returning existing")
            return existing
        }
    ...
    private var currentOperationDescription: String {
        ...
        case .willExecutePipeline(let contexts):
            ...
            if contexts.count > 1 {
                return firstContext.command.operationDescription + " (preparation stage) and \(contexts.count - 1) more"
            } else {
                return firstContext.command.operationDescription + " (preparation stage)"
            }
        case .unhooked:
            return "unhooked"
        case .recovery:
