...
- (NSMutableDictionary *)coalescedTree:(NSMutableDictionary *)node {
    ...
    if (nodeType == kVSplitLayoutNode || nodeType == kHSplitLayoutNode) {
        ...
        NSMutableArray *coalescedChildren = [NSMutableArray arrayWithCapacity:children.count];
...
- (NSMutableDictionary *)parsedLayoutFromString:(NSString *)layout {
    // BUG-3815: Validate layout string length before creating range starting at index 5
    if (layout.length < 5) {
        DLog(@"Layout string too short: %@", layout);
        return nil;
    }
    ...
    if ([self parseLayout:layout range:NSMakeRange(5, layout.length - 5) intoTree:temp]) {
        // BUG-f1425: Guard objectAtIndex:0 after parseLayout - array could be empty on parse failure
        if (temp.count == 0) {
            DLog(@"BUG-f1425: parseLayout returned YES but temp array is empty");
            return nil;
        }
        ...
        NSArray *components =
            [layout captureComponentsMatchedByRegex:@"^[0-9a-fA-F]{4},([0-9]+)x([0-9]+),[0-9]+,[0-9]+[\\[{]"];
        if (components.count == 3) {
            tree[kLayoutDictWidthKey] = @([components[1] intValue]);
            tree[kLayoutDictHeightKey] = @([components[2] intValue]);
            DLog(@"%%layout-change reported window size of %@ x %@", tree[kLayoutDictWidthKey],
...
- (id)depthFirstSearchParseTree:(NSMutableDictionary *)parseTree
    ...
    if ([[parseTree objectForKey:kLayoutDictNodeType] intValue] == kLeafLayoutNode) {
        ...
        for (NSMutableDictionary *child in [parseTree objectForKey:kLayoutDictChildrenKey]) {
            id ret = [self depthFirstSearchParseTree:child callingSelector:selector onTarget:target withObject:obj];
            if (ret) {
                return ret;
            }
        }
...
- (LayoutNodeType)nodeTypeInLayout:(NSString *)layout range:(NSRange)range {
    NSRange squareRange = [layout rangeOfString:@"[" options:0 range:range];
    NSRange curlyRange = [layout rangeOfString:@"{" options:0 range:range];
    if (squareRange.location == NSNotFound && curlyRange.location == NSNotFound) {
        return kLeafLayoutNode;
    } else if (squareRange.location != NSNotFound && curlyRange.location != NSNotFound) {
...
- (NSMutableDictionary *)dictForLeafNodeInLayout:(NSString *)layout range:(NSRange)range {
    ...
    if (components.count != 6 && components.count != 5) {
        DLog(@"Matched wrong number of components in singleton layout \"%@\"", [layout substringWithRange:range]);
        return nil;
    }

    NSMutableDictionary *result = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:[components objectAtIndex:1], kLayoutDictWidthKey, [components objectAtIndex:2],
                                     kLayoutDictHeightKey, [components objectAtIndex:3], kLayoutDictXOffsetKey,
                                     [components objectAtIndex:4], kLayoutDictYOffsetKey,
                                     [NSNumber numberWithInt:kLeafLayoutNode], kLayoutDictNodeType, nil];
...
- (NSRange)rangeOfChildrenInLayout:(NSString *)layout
    ...
    // BUG-2150: Check for NSNotFound to avoid infinite loop with huge index
    if (openRange.location == NSNotFound) {
        DLog(@"Open character '%@' not found in layout: %@", openChar, [layout substringWithRange:range]);
        return NSMakeRange(NSNotFound, 0);
    }
    int count = 0;
    for (int i = (int)openRange.location; i < (int)(range.location + range.length); i++) {
    ...
    DLog(@"Unbalanced braces in children in layout: %@", [layout substringWithRange:range]);
    return NSMakeRange(NSNotFound, 0);
}

- (NSMutableDictionary *)splitDictWithType:(LayoutNodeType)nodeType {
    return [NSMutableDictionary dictionaryWithObject:[NSNumber numberWithInt:nodeType] forKey:kLayoutDictNodeType];
}
...
- (NSString *)splitOffFirstLayoutInLayoutArray:(NSString *)layouts rest:(NSMutableString *)rest {
    ...
    NSArray *components =
        [layouts captureComponentsMatchedByRegex:@"^(?:,?)([0-9]+x[0-9]+,[0-9]+,[0-9]+(?:(?:,[0-9]+)?))(.*)"];
    if (components.count != 3) {
        DLog(@"Wrong number of components in layouts array \"%@\"", layouts);
        return nil;
    }
        ...
        if (c == ',') {
            ...
            NSRange childrenRange = [self rangeOfChildrenInLayout:suffix
                                                            range:NSMakeRange(0, suffix.length)
                                                             open:c == '[' ? @"[" : @"{"
                                                            close:c == '[' ? @"]" : @"}"];
            // BUG-7294: Check for NSNotFound before using range
            if (childrenRange.location == NSNotFound) {
                DLog(@"Failed to find children range in layout: %@", suffix);
                return nil;
            }
            int nextItemOffset = childrenRange.location + childrenRange.length + 1;
            [rest setString:[suffix substringWithRange:NSMakeRange(nextItemOffset, suffix.length - nextItemOffset)]];
            ...
            DLog(@"Bad layouts (c=%d): %@", (int)c, layouts);
            return nil;
        }
    }
}
...
- (BOOL)parseLayoutArray:(NSString *)layout range:(NSRange)range intoTree:(NSMutableArray *)tree {
    // layout,layout,...
    NSMutableString *rest = [NSMutableString stringWithCapacity:range.length];
    NSString *first = [self splitOffFirstLayoutInLayoutArray:[layout substringWithRange:range] rest:rest];
    while (first) {
        if (![self parseLayout:first range:NSMakeRange(0, first.length) intoTree:tree]) {
            return NO;
        }
        first = [self splitOffFirstLayoutInLayoutArray:rest rest:rest];
    }
    return YES;
}
...
- (BOOL)parseLayout:(NSString *)layout range:(NSRange)range intoTree:(NSMutableArray *)tree {
    ...
    switch (nodeType) {
        ...
        case kHSplitLayoutNode: {
            NSRange childrenRange = [self rangeOfChildrenInLayout:layout range:range open:openChar close:closeChar];
...
- (NSMutableDictionary *)searchParseTree:(NSMutableDictionary *)parseTree forWindowPane:(NSNumber *)wp {
