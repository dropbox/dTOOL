...
class KittyImageController: NSObject {
    ...
    fileprivate struct Image {
        ...
        var cost: Int {
            ...
            // BUG-901: Calculate cost using Int64 to avoid overflow on large animated images.
            // For a 4K image (3840x2160) with 100 frames: 3840 * 2160 * 4 * 100 = ~3.3 billion
            // which exceeds Int32.max. Use Int64 and cap at Int.max for safety.
            let bytesPerPixel: Int64 = 4
            let width = Int64(image.size.width)
            let height = Int64(image.size.height)
            let frames = Int64(image.images.count)
            let totalCost = width * height * bytesPerPixel * frames
            return totalCost > Int64(Int.max) ? Int.max : Int(totalCost)
        }
    }
    ...
    fileprivate struct Placement {
        ...
        func absRect(cellSize: NSSize, finder: PlacementFinder) -> VT100GridAbsCoordRange? {
            ...
            // BUG-505: Guard against division by zero when cellSize width/height is zero
    ...
    private func dataByAddingAlphaTo3bppData(_ data: Data, _ alpha: UInt8) -> Data {
        // BUG-f1414: Guard against array bounds crash when data.count is not divisible by 3
        // If data has e.g. 5 bytes, stride would give i=0, then i=3, but data[3..<6] would crash
        // since data only has indices 0-4.
        let completePixelCount = data.count / 3
        let safeByteCount = completePixelCount * 3  // Only process complete RGB triplets
        var rgbaData = Data(capacity: completePixelCount * 4)
        for i in stride(from: 0, to: safeByteCount, by: 3) {
    ...
    /// Maximum allowed dimension for Kitty images (10240 pixels)
    private static let maxImageDimension: UInt = 10240

    /// Maximum allowed total image size in bytes (256 MB)
    private static let maxImageBytes: Int = 256 * 1024 * 1024
    ...
    private func image(data rawData: Data, bpp: UInt, width: UInt, height: UInt) -> iTermImage? {
        ...
        // Reject excessively large images to prevent memory exhaustion
        ...
        // Validate total size won't exceed memory limit
        let totalRGBABytes = Int(clamping: width) * Int(clamping: height) * bytesPerPixel
        guard totalRGBABytes <= Self.maxImageBytes else {
            return nil
        }
        ...
        // BUG-1646: Use guard instead of force unwrap for colorSpace creation
    ...
    func inflate(data compressedData: Data) -> Data? {
        // BUG-1681: Guard against empty input - zlib cannot decompress empty data
        // and passing empty data can cause issues (nil baseAddress or infinite loop)
        guard !compressedData.isEmpty else {
            DLog("inflate: empty compressed data")
            return nil
        }
        // BUG-2633: Entire inflate operation must be inside withUnsafeBytes closure
        // to prevent z.next_in from becoming a dangling pointer
        return compressedData.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) -> Data? in
            // BUG-1681: baseAddress can be nil for empty buffers (defensive check)
            guard let baseAddr = ptr.bindMemory(to: Bytef.self).baseAddress else {
                DLog("inflate: baseAddress is nil")
                return nil
            }
            var z = z_stream()
            z.zalloc = nil
            ...
            z.next_in = UnsafeMutablePointer<Bytef>(mutating: baseAddr)
    ...
    func executeTransmitFile(_ command: KittyImageCommand.ImageTransmission,
                             payload: String,
                             query: Bool) -> String? {
        guard let decodedPath = payload.base64Decoded,
              !decodedPath.isEmpty else {
            return "EBADF:Invalid base64 payload"
        }
        let nsPath = decodedPath as NSString
        guard nsPath.isAbsolutePath else {
            return "EBADF:Path must be absolute"
        }
        let fileURL = URL(fileURLWithPath: decodedPath).resolvingSymlinksInPath().standardizedFileURL.absoluteURL
        // Ensure standardized path is still absolute (prevents traversal)
    ...
    private func filenameIsSafe(fileURL: URL) -> Bool {
        ...
        // Allowlist approach: only allow files from user-accessible locations
        ...
        let isInAllowedLocation = allowedPrefixes.anySatisfies { prefix in
        ...
        // Even within allowed locations, block sensitive files/directories
        ...
        return !sensitivePatterns.anySatisfies { pattern in
            lowercasePath.contains(pattern)
        }
    }
    ...
    func readPOSIXSharedMemory(named name: String) throws -> Data {
        ...
        // BUG-7205: Unlink immediately after opening to atomically take ownership
        // This prevents TOCTOU race where another process could ftruncate the shm
        // between our fstat and mmap calls.
        _ = shm_unlink(name)
        ...
        // BUG-7205: Use MAP_PRIVATE to get a private copy that won't be affected
        // if the original shm backing is somehow still modified (defense-in-depth)
        ...
        // BUG-f982: Use guard with precondition instead of force unwrap
        // mmap returns MAP_FAILED (-1) on error, not nil. After the MAP_FAILED check,
        // ptr can never be nil. But we add a guard for defense-in-depth.
        guard let validPtr = ptr else {
            DLog("BUG-f982: mmap returned nil (unexpected after MAP_FAILED check)")
            throw POSIXError(.EINVAL)
        }
        let data = Data(bytes: validPtr, count: size)
    ...
    func executeLoadAnimationFrame(_ command: KittyImageCommand.AnimationFrameLoading) {
        // KITTY_ANIMATION_UNSUPPORTED: Animation frame loading is not yet implemented.
        // The Kitty graphics protocol animation features (frame loading, composition,
        // and control) require significant infrastructure for frame storage, timing,
        // and playback. This is also unimplemented in upstream DashTerm2.
        DLog("Kitty animation frame loading not supported")
    }

    func executeComposeAnimationFrame(_ command: KittyImageCommand.AnimationFrameComposition) {
        // KITTY_ANIMATION_UNSUPPORTED: Animation frame composition is not yet implemented.
        // See executeLoadAnimationFrame for details.
        DLog("Kitty animation frame composition not supported")
    }

    func executeControlAnimation(_ command: KittyImageCommand.AnimationControl) {
        // KITTY_ANIMATION_UNSUPPORTED: Animation control is not yet implemented.
        // See executeLoadAnimationFrame for details.
        DLog("Kitty animation control not supported")
    }
    ...
    func executeDeleteImage(_ command: KittyImageCommand.DeleteImage) {
        ...
        case "n", "N":
            ...
            let identifier = candidates.max { lhs, rhs in
                (_images[lhs]?.sequence ?? 0) < (_images[rhs]?.sequence ?? 0)
            }
            if let identifier {
                if command.placementId == 0 {
        ...
        case "f", "F":
            // KITTY_ANIMATION_UNSUPPORTED: Delete animation frames is not yet implemented.
            // This requires animation support infrastructure. See executeLoadAnimationFrame.
            DLog("Kitty delete animation frames not supported")
            break
...
class iTermKittyImageDraw: NSObject {
    ...
    func gridRect(cellSize: NSSize) -> VT100GridRect {
        // BUG-506: Guard against division by zero when cellSize width/height is zero
        guard cellSize.width > 0 && cellSize.height > 0 else {
            return VT100GridRect(origin: VT100GridCoord(x: 0, y: 0),
                                 size: VT100GridSize(width: 0, height: 0))
        }
