...
- (instancetype)initWithQueue:(dispatch_queue_t)queue {
    ...
    if (self) {
        _registry = [[NSMutableDictionary alloc] initWithCapacity:16];
        _pending = [[NSMutableDictionary alloc] initWithCapacity:8];
    }
    return self;
}
@end
...
- (instancetype)initWithQueue:(dispatch_queue_t)queue client:(iTermFileDescriptorMultiClient *)client {
    ...
    if (self) {
        _client = client;
        _unattachedChildren = [NSMutableArray arrayWithCapacity:16]; // Typical session count
    }
    return self;
}
@end
...
+ (void)getConnectionForSocketNumber:(int)number
    ...
    [self.thread dispatchAsync:^(iTermMultiServerConnectionGlobalState *_Nonnull state) {
        [self connectionForSocketNumber:number createIfPossible:shouldCreate state:state callback:callback];
    }];
}
...
+ (iTermThread<iTermMultiServerConnectionGlobalState *> *)thread {
    ...
    dispatch_once(&onceToken, ^{
        thread = [iTermThread withLabel:@"com.dashterm.dashterm2.multi-server-registry"
...
+ (void)findExistingPrimaryConnection:
    (iTermCallback<iTermMultiServerConnectionGlobalState *, iTermMultiServerConnection *> *)callback {
    ...
    [self connectionForSocketNumber:i
                   createIfPossible:YES
                           ...
                           callback:[self.thread newCallbackWithBlock:^(
                               ...
                               [result
                                   handleObject:^(iTermMultiServerConnection *_Nonnull object) {
...
+ (void)connectionForSocketNumber:(int)number
    ...
    // BUG-f1085: Replace assert with guard - allocation failure should invoke callback with error, not crash
    if (!result) {
        ELog(@"ERROR: Failed to allocate iTermMultiServerConnection for socket %@", @(number));
    ...
    [result.thread dispatchAsync:^(iTermMultiServerPerConnectionState *_Nullable connectionState) {
        if (shouldCreate) {
            ...
            [connectionState.client
                attachToOrLaunchNewDaemonWithCallback:[self.thread newCallbackWithBlock:^(
                    ...
                    [self invokePendingCallbacksForSocketNumber:number state:globalState result:resultObject];
                }]];
            return;
        }
        ...
        [connectionState.client
            attachWithCallback:[self.thread newCallbackWithBlock:^(iTermMultiServerConnectionGlobalState *globalState,
                                                                   NSNumber *statusNumber) {
                if (!statusNumber.boolValue) {
...
+ (NSString *)pathForNumber:(int)number {
    ...
    NSString *normalFilename = [NSString stringWithFormat:@"dashterm2-daemon-%d.socket", number];
    ...
    NSString *dotdir = [homedir stringByAppendingPathComponent:@".dashterm2"];
...
- (instancetype)initWithSocketNumber:(int)number {
    self = [super init];
    if (self) {
        _thread = [iTermThread withLabel:@"com.dashterm.dashterm2.multi-server-conn"
                            stateFactory:^iTermSynchronizedState *_Nullable(dispatch_queue_t _Nonnull queue) {
...
- (void)attachToProcessID:(pid_t)pid callback:(iTermCallback<id, iTermFileDescriptorMultiClientChild *> *)callback {
...
- (void)fileDescriptorMultiClientDidClose:(iTermFileDescriptorMultiClient *)client {
    [[iTermMultiServerConnection thread] dispatchAsync:^(iTermMultiServerConnectionGlobalState *_Nullable state) {
        ...
        [self.thread dispatchAsync:^(iTermMultiServerPerConnectionState *_Nullable state) {
            // BUG-f1112: Replace assert with guard - handle stale client reference gracefully
            if (client != state.client) {
                DLog(@"Warning: fileDescriptorMultiClientDidClose called with mismatched client (expected %p, got %p)",
                     state.client, client);
                // Still proceed with cleanup of the passed client if it matches our expectation
            }
...
- (void)fileDescriptorMultiClient:(iTermFileDescriptorMultiClient *)client
    ...
    [client waitForChild:child removePreemptively:NO callback:nil];
}
