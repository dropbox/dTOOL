...
func matMul(_ mat1: [[Double]], _ mat2: [[Double]]) -> [[Double]] {
    // BUG-354, BUG-360: Guard against empty matrix crash
    // Accessing mat1[0] or mat2[0] on empty arrays causes index out of bounds crash
    guard !mat1.isEmpty, !mat2.isEmpty else {
        return []
    }
    ...
    // BUG-f658: Replace it_assert with guard - mismatched matrix dimensions should not crash
    guard cols1 == rows2 else {
        DLog("BUG-f658: Matrix dimension mismatch in matMul: cols1=\(cols1) != rows2=\(rows2)")
...
class HueVisualizationView: NSView {
    ...
    private struct OKLab: CustomDebugStringConvertible {
        ...
        var xyz: iTermXYZColor {
            ...
            // BUG-f618: Guard against empty matrix result from matMul
            guard xyzVec.count >= 3,
                  xyzVec[0].count >= 1,
                  xyzVec[1].count >= 1,
                  xyzVec[2].count >= 1 else {
                DLog("BUG-f618: matMul returned invalid xyzVec with count \(xyzVec.count)")
                return iTermXYZColor(x: 0, y: 0, z: 0)
            }
        ...
        init(_ xyz: iTermXYZColor) {
            ...
            // BUG-f619: Guard against empty matrix result from matMul
            guard labVec.count >= 3,
                  labVec[0].count >= 1,
                  labVec[1].count >= 1,
                  labVec[2].count >= 1 else {
                DLog("BUG-f619: matMul returned invalid labVec with count \(labVec.count)")
                l = 0
                a = 0
                b = 0
                return
    ...
    func chromaAndHue(for point: NSPoint) -> (Float, Float) {
        ...
        // BUG-f993: Guard against division by zero when radius is 0
        // This can happen if view bounds are (0,0) before layout completes
        let chroma: Float
        if radius > 0 {
            // Calculate the raw chroma based on the distance and reverse the radius scaling
            chroma = Float((distance - pointRadius) / radius) * Float(maxChroma)
        } else {
            chroma = 0
        }
