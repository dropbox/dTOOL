...
- (UTF32Char)codePointForEvent:(NSEvent *)event {
    ...
    if ([self eventIsControlCodeWithOption:event]) {
        ...
        // take this path.
        // RC-010/BUG-3979: Dead keys and some modifier events have empty characters.
        // This is expected macOS behavior, not a bug. Return 0 to signal "no character".
...
- (NSString *)stringForEvent:(NSEvent *)event {
    const NSEventModifierFlags allEventModifierFlags = (NSEventModifierFlagControl | NSEventModifierFlagOption |
    ...
    const NSEventModifierFlags allEventModifierFlagsExShift =
        (NSEventModifierFlagControl | NSEventModifierFlagOption | maybeFunction);
...
- (iTermOptionKeyBehavior)optionKeyBehaviorForEvent:(NSEvent *)event {
    ...
    // BUG-f1027: Guard against neither alt pressed instead of crashing - return OPT_NORMAL
    if (!leftAltPressed && !rightAltPressed) {
        DLog(@"BUG-f1027: optionKeyBehaviorForEvent: called with no alt key pressed - returning OPT_NORMAL");
        return OPT_NORMAL;
    }
...
- (NSString *)stringForCodePoint:(UTF32Char)codePoint modifiers:(NSEventModifierFlags)eventModifiers {
    switch (codePoint) {
        ...
        default:
            ...
            return [NSString
                stringWithFormat:@"\e[27;%d;%d~", [self csiModifiersForEventModifiers:eventModifiers], codePoint];
    }
}

- (NSString *)sequenceForFunctionKeyWithCode:(NSString *)code eventModifiers:(NSEventModifierFlags)eventModifiers {
...
- (NSString *)reallySpecialSequenceWithCode:(UTF32Char)code eventModifiers:(NSEventModifierFlags)eventModifiers {
...
// CSI code ~
// CSI code ; modifier ~
- (NSString *)sequenceForNonUnicodeKeypress:(NSString *)code eventModifiers:(NSEventModifierFlags)eventModifiers {
    const int csiModifiers = [self csiModifiersForEventModifiers:eventModifiers];
    if (csiModifiers == 1) {
...
- (nullable NSString *)keyMapperStringForPreCocoaEvent:(NSEvent *)event {
    ...
    if ([event it_isNumericKeypadKey]) {
        ...
        return [[NSString alloc] initWithData:[output keypadDataForString:event.characters
    ...
    const NSEventModifierFlags allEventModifierFlags = (NSEventModifierFlagControl | NSEventModifierFlagOption |
                                                        NSEventModifierFlagShift | NSEventModifierFlagCommand);
    if (event.keyCode == kVK_Space && (event.it_modifierFlags & allEventModifierFlags) == NSEventModifierFlagShift) {
...
- (BOOL)keyMapperShouldBypassPreCocoaForEvent:(NSEvent *)event {
    ...
    const BOOL leftAltPressed =
        (modifiers & NSEventModifierFlagOption) == NSEventModifierFlagOption && !rightAltPressed;
    iTermOptionKeyBehavior left, right;
    [self.delegate modifyOtherKeys:self getOptionKeyBehaviorLeft:&left right:&right];
    ...
    if (willSendOptionModifiedKey) {
        // Meta+key or Esc+ key
        DLog(@"isNonEmpty=%@ rightAltPressed=%@ leftAltPressed=%@ leftOptionModifiesKey=%@ rightOptionModifiesKey=%@ "
             @"optionModifiesKey=%@ willSendOptionModifiedKey=%@ -> bypass pre-cocoa",
             @(isNonEmpty), @(rightAltPressed), @(leftAltPressed), @(leftOptionModifiesKey), @(rightOptionModifiesKey),
             @(optionModifiesKey), @(willSendOptionModifiedKey));
        return YES;
    }
...
NSDictionary *iTermModifyOtherKeysMapperDictionary(iTermModifyOtherKeysMapper *self,
    ...
    return @{
        @"encoding" : @([delegate modifiyOtherKeysDelegateEncoding:self]),
