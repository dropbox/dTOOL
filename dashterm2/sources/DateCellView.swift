...
class DateCellView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f804: Return nil instead of crashing for unused coder initializer
        DLog("DateCellView init(coder:) is not supported")
        return nil
    }
    ...
    private func updateBubbleColor() {
        ...
        let (lightColor, darkColor) = (NSColor(fromHexString: "#e0e0e0") ?? .lightGray,
                                       NSColor(fromHexString: "#505050") ?? .gray)
