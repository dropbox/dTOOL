...
// These match labels in the profiles tab view. I guess it should be identifiers but I would probably forget to set
// them.
...
- (instancetype)initWithIdentifiers:(NSArray<NSString *> *)identifiers profileTypes:(ProfileType)profileTypes {
...
- (void)awakeFromNib {
    ...
    NSArray<NSView *> *views =
        [[map allValues] sortedArrayUsingComparator:^NSComparisonResult(NSView *lhs, NSView *rhs) {
            return [@(lhs.frame.origin.x) compare:@(rhs.frame.origin.x)];
        }];

    // Pre-allocate for offsets between views (views.count - 1 offsets)
    NSMutableArray<NSNumber *> *offsets = [NSMutableArray arrayWithCapacity:views.count > 0 ? views.count - 1 : 0];
...
- (IBAction)performBulkCopy:(id)sender {
    ...
    for (NSString *destGuid in destGuids) {
        ...
        if (![profileModel bookmarkWithGuid:destGuid]) {
            DLog(@"Selected profile %@ doesn't exist", destGuid);
            continue;
        }
...
- (void)updateLabel {
    ...
    [_bulkCopyLabel
        setStringValue:[NSString stringWithFormat:@"Copy these settings from profile “%@”:", profile[KEY_NAME]]];
}
...
- (void)copyAttributes:(BulkCopySettings)attributes
    ...
    switch (attributes) {
        ...
        case BulkCopyAdvanced:
            ...
            break;
        default:
            DLog(@"Unexpected copy attribute %d", (int)attributes);
            return;
    }
