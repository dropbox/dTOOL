...
//
// BUG-21: Shell utilities strings updated to DashTerm2 branding
// BUG-32: Shell integration window updated to DashTerm2
...
@property (nonatomic, strong)
    IBOutlet NSViewController<iTermShellIntegrationInstallerViewController> *secondPageViewController;
@property (nonatomic, strong)
    IBOutlet iTermShellIntegrationDownloadAndRunViewController<iTermShellIntegrationInstallerViewController>
        *downloadAndRunViewController;
@property (nonatomic, strong)
    IBOutlet iTermShellIntegrationPasteShellCommandsViewController *sendShellCommandsViewController;
@property (nonatomic, strong)
    IBOutlet NSViewController<iTermShellIntegrationInstallerViewController> *finishedViewController;
...
- (void)windowDidLoad {
    ...
    _pendingExpectations = [[NSMutableArray alloc] initWithCapacity:8];
...
- (iTermShellIntegrationInstallationState)nextState {
    switch (self.state) {
        ...
        case iTermShellIntegrationInstallationStateSecondPage:
            // BUG-406: Replace assert(NO) with DLog - SecondPage has no next state
            // (user must choose download or send commands path)
            DLog(@"iTermShellIntegrationWindowController: nextState called from SecondPage (invalid)");
            // Fall through to return Confirmation as a safe fallback
        ...
        case iTermShellIntegrationInstallationStateConfirmation:
            // BUG-406: Replace assert(NO) with DLog - Confirmation has no next state
            DLog(@"iTermShellIntegrationWindowController: nextState called from Confirmation (invalid)");
            return iTermShellIntegrationInstallationStateConfirmation; // Stay on Confirmation
    }
}
...
- (iTermShellIntegrationInstallationState)previousState {
    switch (self.state) {
        case iTermShellIntegrationInstallationStateFirstPage:
            // BUG-407: Replace assert(NO) with DLog - FirstPage has no previous state
            DLog(@"iTermShellIntegrationWindowController: previousState called from FirstPage (invalid)");
            return iTermShellIntegrationInstallationStateFirstPage; // Stay on FirstPage
        case iTermShellIntegrationInstallationStateSecondPage:
        ...
        case iTermShellIntegrationInstallationStateSendShellCommandsConfirm:
            return iTermShellIntegrationInstallationStateSecondPage;
        case iTermShellIntegrationInstallationStateConfirmation:
            // BUG-407: Replace assert(NO) with DLog - Confirmation has no previous state
            DLog(@"iTermShellIntegrationWindowController: previousState called from Confirmation (invalid)");
            return iTermShellIntegrationInstallationStateConfirmation; // Stay on Confirmation
    }
}

- (NSViewController<iTermShellIntegrationInstallerViewController> *)viewControllerForState:
    (iTermShellIntegrationInstallationState)state {
    switch (state) {
        case iTermShellIntegrationInstallationStateFirstPage:
    ...
    // BUG-408: Replace assert(NO) with DLog for unexpected state value
    // (could happen if new enum value added but not handled)
    DLog(@"iTermShellIntegrationWindowController: Unknown state %d, returning firstPageViewController", (int)state);
    return self.firstPageViewController;
}
...
- (NSString *)sendText:(NSString *)text
    ...
    return [self sendText:text reallySend:reallySend afterRegex:regex expectation:expectation completion:nil];
}
...
- (void)cancelAllExpectations {
    [_pendingExpectations
        enumerateObjectsUsingBlock:^(iTermExpectation *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
// BUG-23: Shell integration file names updated to dashterm2_shell_integration for DashTerm2/iTerm2 coexistence
- (NSString *)shellIntegrationPath {
    return [NSString stringWithFormat:@"%@/.dashterm2_shell_integration.%@", self.dotdir,
                                      iTermShellIntegrationShellString(self.shell)];
}

- (NSString *)scriptForCurrentShell {
    NSURL *url = [[NSBundle bundleForClass:self.class] URLForResource:@"dashterm2_shell_integration"
                                                        withExtension:iTermShellIntegrationShellString(self.shell)];
    if (!url) {
        return nil;
    }
    NSString *string = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil];
    // BUG-f1271: Replace assert() with guard - nil string should return nil, not crash
    if (string == nil) {
        DLog(@"WARNING: BUG-f1271 failed to read shell integration snippet from %@", url);
        return nil;
    }
    ...
    // BUG-f1272: Replace assert() with guard - nil names should skip aliases, not crash
    if (names == nil) {
        DLog(@"WARNING: BUG-f1272 failed to read utilities-manifest.txt, skipping aliases");
        return string;
    }
    ...
    NSArray<NSString *> *aliasCommands = [commands mapWithBlock:^id(NSString *command) {
        switch (self.shell) {
            ...
            case iTermShellIntegrationShellBash:
            case iTermShellIntegrationShellFish:
                return [NSString stringWithFormat:@"alias %@=%@/.dashterm2/%@", command, self.dotdir, command];
            case iTermShellIntegrationShellTcsh:
                return [NSString stringWithFormat:@"alias %@ %@/.dashterm2/%@", command, self.dotdir, command];
            case iTermShellIntegrationShellUnknown:
                return nil;
...
- (NSString *)dotfileCommandWithBashDotfile:(NSString *)bashDotFile {
    ...
    switch (self.shell) {
        ...
        case iTermShellIntegrationShellFish:
            ...
            break;
        case iTermShellIntegrationShellUnknown:
            // BUG-409: Replace assert(NO) with DLog - unknown shell should not reach here
            // Return empty command instead of crashing (UI should prevent this state)
            DLog(@"iTermShellIntegrationWindowController: dotfileCommandWithBashDotfile called with unknown shell");
            return @"";
    }
    NSString *relative_filename = [NSString stringWithFormat:@"%@/.dashterm2_shell_integration.%@", self.dotdir,
                                                             iTermShellIntegrationShellString(self.shell)];

    return [NSString stringWithFormat:@"test -e %@%@%@ %@ source %@%@%@ %@ true\n", quote, relative_filename, quote,
                                      shell_and, quote, relative_filename, quote, shell_or];
}
...
- (NSString *)amendDotFileWithExpectation:(inout iTermExpectation **)expectation completion:(void (^)(void))completion {
    const BOOL reallySend = (completion != nil);
    NSMutableArray<NSString *> *strings = [[NSMutableArray alloc] initWithCapacity:16];
    NSArray<NSString *> *parts =
        [[[self launchBashString] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]
    ...
    switch (self.shell) {
        ...
        case iTermShellIntegrationShellBash: {
            NSString *assignment =
                @"IT2_INSTALLER_DOTFILE=$(test -f ~/.bash_profile && echo -n ~/.bash_profile || echo -n ~/.profile)\n";
        ...
        case iTermShellIntegrationShellFish:
            ...
            break;
        case iTermShellIntegrationShellUnknown:
            // BUG-410: Replace assert(NO) with DLog - unknown shell should not reach here
            // Return empty string and skip sending text (UI should prevent this state)
            DLog(@"iTermShellIntegrationWindowController: amendDotFileWithExpectation called with unknown shell");
            return @"";
    }
    ...
    [strings
        addObject:[self sendText:[NSString
                                     stringWithFormat:
                                         @"if ! grep dashterm2_shell_integration %@  > /dev/null 2>&1; then\n", script]
                       reallySend:reallySend
                       afterRegex:@"^>> "
                      expectation:expectation]];
...
- (NSString *)switchToBash:(BOOL)reallySend expectation:(inout iTermExpectation **)expectation {
    return [self sendText:self.launchBashString reallySend:reallySend afterRegex:@"." expectation:expectation];
}
...
- (NSString *)catString:(NSString *)string
    ...
    // Shell commands + script content; 1KB is a reasonable estimate
    ...
    [result appendString:[self sendText:[NSString stringWithFormat:@"cat <<'EOF' > %@\n", path]
                              ...
                              afterRegex:@"."
                             expectation:expectation]];
    [result appendString:[self sendText:string reallySend:reallySend afterRegex:@"^> " expectation:expectation]];
...
- (NSString *)catToScript:(BOOL)reallySend completion:(void (^)(void))completion {
    NSString *scriptForShell = [self scriptForCurrentShell];
    if (!scriptForShell) {
        return @"# Error: your shell could not be determined or is unsupported.";
    }
    // Shell commands + script content; 2KB is a reasonable estimate
...
- (NSString *)catToUtilities:(BOOL)reallySend
    ...
    // Shell commands + base64 encoded utilities; 16KB is a reasonable estimate
    NSMutableString *result = [NSMutableString stringWithCapacity:16384];
    NSString *folder = [self.dotdir stringByAppendingPathComponent:@".dashterm2"];
    [result appendString:[self switchToBash:reallySend expectation:expectation]];
    ...
    [result appendString:[self sendText:encoded reallySend:reallySend afterRegex:@"^> " expectation:expectation]];
    [result appendString:[self sendText:@"\nEOF\n" reallySend:reallySend afterRegex:@"." expectation:expectation]];
    [result appendString:[self sendText:self.exitBashString
                              reallySend:reallySend
                              afterRegex:@"> EOF"
...
- (NSString *)discoverShell:(BOOL)reallySend {
    __weak __typeof(self) weakSelf = self;
    return [self discoverShell:reallySend
                    completion:^(NSString *shell, NSString *dotdir) {
...
- (void)didDiscoverShell:(NSString *)shell dotdir:(NSString *)dotdir {
    NSDictionary<NSString *, NSNumber *> *map = @{
        @"tcsh" : @(iTermShellIntegrationShellTcsh),
...
- (NSString *)installUtilityScripts:(BOOL)reallySend {
    // Shell commands for utility install; 16KB is a reasonable estimate
    NSMutableString *result = [NSMutableString stringWithCapacity:16384];
    iTermExpectation *expectation = nil;
    __weak __typeof(self) weakSelf = self;
...
- (NSString *)modifyStartupScriptsAndProceedTo:(int)nextStage reallySend:(BOOL)reallySend {
    void (^completion)(void) = ^{
        self.sendShellCommandsViewController.stage = nextStage;
    };
    iTermExpectation *expectation = nil;
    return [self amendDotFileWithExpectation:&expectation completion:reallySend ? completion : nil];
}
        ...
        switch (stage) {
            ...
            case 2: {
                if (self.installUtilities) {
                    ...
                    [self modifyStartupScriptsAndProceedTo:stage + 1 reallySend:YES];
                }
                break;
            }
            case 3:
    ...
    switch (stage) {
        ...
        case 2: {
            if (self.installUtilities) {
                ...
                return [self modifyStartupScriptsAndProceedTo:stage + 1 reallySend:NO];
            }
            break;
        }
        case 3:
