...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p docid=%@ identifier=%@ displayName=%@>", self.class, self, self.docid,
                                      self.identifier, self.displayName];
}
...
- (instancetype)initWithKeyword:(NSString *)keyword
                       document:(iTermPreferencesSearchDocument *)document NS_DESIGNATED_INITIALIZER;
...
- (instancetype)initWithCursors:(NSArray<iTermPreferencesSearchCursor *> *)cursors {
    self = [self initWithToken:cursors[0].keyword.keyword index:0];
    if (self) {
        NSMutableSet *docIDs = [[NSMutableSet alloc] initWithCapacity:cursors.count * 4]; // Estimated doc IDs
        [cursors
            enumerateObjectsUsingBlock:^(iTermPreferencesSearchCursor *cursor, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (instancetype)initWithToken:(NSString *)token index:(NSInteger)index {
    ...
    if (self) {
        ...
        _index = index;
        _docIDs = [[NSMutableSet alloc] initWithCapacity:8]; // Doc IDs matching this cursor
    }
    return self;
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _keywordIndex = [[NSMutableArray alloc] initWithCapacity:256]; // Search keywords
        _stemIndex = [[NSMutableArray alloc] initWithCapacity:256];    // Word stems
        _docs = [[NSMutableDictionary alloc] initWithCapacity:128];    // Search documents
    }
    return self;
}
...
- (void)dump {
    NSLog(@"KEYWORDS:");
    [_keywordIndex enumerateObjectsUsingBlock:^(iTermPreferencesSearchIndexEntry *_Nonnull obj, NSUInteger idx,
                                                BOOL *_Nonnull stop) {
        // index: keyword -> ([docid: identifier], ...)
        NSLog(@"%@: %@ -> %@", @(idx), obj.keyword,
              [[obj.documents mapWithBlock:^id(iTermPreferencesSearchDocument *doc) {
                  return [NSString stringWithFormat:@"[%@: %@]", doc.docid, doc.identifier];
              }] componentsJoinedByString:@", "]);
    }];
    NSLog(@"STEMS:");
    [_stemIndex enumerateObjectsUsingBlock:^(iTermPreferencesSearchIndexEntry *_Nonnull obj, NSUInteger idx,
                                             BOOL *_Nonnull stop) {
...
- (NSArray<iTermPreferencesSearchDocument *> *)documentsMatchingQuery:(NSString *)query
    ...
    NSArray<iTermPreferencesSearchCursor *> *stemCursors = [self cursorsForQueryTokens:stemmedTokens
                                                                           ...
                                                                           prefixMatch:YES];
    NSArray<iTermPreferencesSearchCursor *> *cursors =
        [[kwCursors zip:stemCursors] mapWithBlock:^id _Nullable(iTermTuple *tuple) {
...
- (NSSet<NSNumber *> *)documentsWithLiteralPhrases:(NSArray<NSString *> *)phrases
                                        fromDocIDs:(NSSet<NSNumber *> *)docIDs {
...
- (NSArray<iTermPreferencesSearchCursor *> *)cursorsForQueryTokens:(NSArray<NSString *> *)tokens
                                                       searchIndex:
                                                           (NSArray<iTermPreferencesSearchIndexEntry *> *)searchIndex
                                                       prefixMatch:(BOOL)prefixMatch {
    NSArray<iTermPreferencesSearchCursor *> *cursors = [tokens mapWithBlock:^id(NSString *token) {
        iTermPreferencesSearchCursor *cursor = [[iTermPreferencesSearchCursor alloc] initWithToken:token
                                                                                             index:NSNotFound];
        if (token.length == 0) {
            return cursor;
        }
...
- (NSSet<NSNumber *> *)intersectCursorDocIDs:(NSArray<iTermPreferencesSearchCursor *> *)cursors {
    ...
    NSDictionary<NSString *, NSArray<NSNumber *> *> *identifierToDocids =
        [docids.allObjects classifyWithBlock:^id(NSNumber *docid) {
...
- (NSArray<iTermPreferencesSearchDocument *> *)documentsSortedByDisplayNameWithDocIDs:(NSSet<NSNumber *> *)docids {
    ...
    }] sortedArrayUsingComparator:^NSComparisonResult(iTermPreferencesSearchDocument *_Nonnull doc1,
                                                      iTermPreferencesSearchDocument *_Nonnull doc2) {
