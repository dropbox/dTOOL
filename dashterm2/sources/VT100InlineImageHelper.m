...
- (void)broke {
    ...
    // BUG-f1369: Convert assert to guard - create placeholder if broken_image asset is missing
    if (!_image) {
        // Fallback: Create a 1x1 transparent placeholder image
        NSImage *placeholder = [[NSImage alloc] initWithSize:NSMakeSize(1, 1)];
        _image = [iTermImage imageWithNativeImage:placeholder];
    }
}
...
- (instancetype)initWithName:(NSString *)name
    ...
    if (self) {
        ...
        // Base64 images can be large; start with 16KB
        _base64String = [NSMutableString stringWithCapacity:16 * 1024];
    }
    return self;
}

- (instancetype)initWithSixelData:(NSData *)data scaleFactor:(CGFloat)scaleFactor {
...
- (instancetype)initWithNativeImageNamed:(NSString *)name spanningWidth:(int)width scaleFactor:(CGFloat)scaleFactor {
...
- (BOOL)smellsLikeImage {
    ...
    if ([self.type hasPrefix:@"image/"] || [self.type hasPrefix:@"video/"]) {
        return YES;
    }
    return NO;
}
...
- (void)writeImage:(VT100DecodedImage *)decodedImage toGrid:(VT100Grid *)grid {
    ...
    int width;
    int height;
    [self getScreenCharacter:&c width:&width height:&height grid:grid decodedImage:decodedImage];

    [self writeBaseCharacter:c toGrid:grid width:width height:height decodedImage:decodedImage];
    ...
    [self.delegate inlineImageSetMarkOnScreenLine:grid.cursor.y + 1 code:c.code];
...
- (VT100DecodedImage *)decodedImage {
    if (_nativeImage) {
        DLog(@"Image is native");
        // BUG-f1370: Convert asserts to guards - clear conflicting data instead of crash
        if (_base64String.length > 0) {
            DLog(@"WARNING: Native image has unexpected base64 data - clearing");
            _base64String = nil;
        }
        if (_sixelData) {
            DLog(@"WARNING: Native image has unexpected sixel data - clearing");
            _sixelData = nil;
        }
    ...
    if (_sixelData) {
        DLog(@"Image is sixel");
        // BUG-f1370: Convert assert to guard - clear conflicting data instead of crash
        if (_base64String.length > 0) {
            DLog(@"WARNING: Sixel image has unexpected base64 data - clearing");
            _base64String = nil;
        }
        ...
        case kVT100TerminalUnitsAuto:
            if (_widthUnits == kVT100TerminalUnitsAuto) {
                ...
                *requestedHeightInPointsPtr = scaledSize.height;
                ...
                // BUG-5071: Guard against division by zero for aspect ratio
                if (scaledSize.height <= 0) {
                    *heightPtr = 1;
                    *requestedHeightInPointsPtr = cellSize.height;
                } else {
                    double aspectRatio = scaledSize.width / scaledSize.height;
                    // Protect against invalid aspect ratio (NaN/inf from degenerate dimensions)
...
- (void)getRequestedWidthInPoints:(CGFloat *)requestedWidthInPointsPtr
    ...
    if (scaledSize.height <= 0) {
        ...
        aspectRatio = scaledSize.width / scaledSize.height;
        // Protect against invalid aspect ratio (NaN/inf from degenerate dimensions)
        if (aspectRatio <= 0 || !isfinite(aspectRatio)) {
            aspectRatio = 1.0;
        }
    ...
    const NSEdgeInsets inset = [self insetsForWidthInPoints:requestedWidthInPoints
                                             ...
                                             heightInPoints:requestedHeightInPoints
                                              heightInCells:height
                                                   cellSize:cellSize];
    const NSEdgeInsets fractionalInset =
        [self fractionalInsetForInset:inset
                          desiredSize:NSMakeSize(requestedWidthInPoints, requestedHeightInPoints)
                             cellSize:cellSize
                         decodedImage:decodedImage
                                width:width
                               height:height];
    return ImageCharForNewImage(_name, width, height, _preserveAspectRatio, fractionalInset);
}
...
- (void)writeBaseCharacter:(screen_char_t)screenChar
    ...
    DLog(@"Append %d rows of image characters with %d columns. The value of c.image is %@", height, width,
         @(screenChar.image));
