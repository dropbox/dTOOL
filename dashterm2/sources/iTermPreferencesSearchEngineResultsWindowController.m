...
// BUG-10346: Add upper bounds check to prevent array index out of bounds crash
- (iTermPreferencesSearchDocument *)selectedDocument {
    NSInteger row = _tableView.selectedRow;
    if (row < 0 || row >= (NSInteger)self.documents.count) {
        return nil;
    }
...
// BUG-10346: Add bounds check to prevent array index out of bounds crash
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    if (row < 0 || row >= (NSInteger)_documents.count) {
        return nil;
    }
