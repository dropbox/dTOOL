...
- (instancetype)init {
    ...
    if (self) {
        ...
        _images = [[NSMutableDictionary alloc] initWithCapacity:8];
    }
    return self;
}
...
- (NSImage *)freshlyComputedImage {
    DLog(@"Recompute badge self=%p, label=“%@”, color=%@, view size=%@. Called from:\n%@", self, _stringValue,
         _fillColor, NSStringFromSize(_viewSize), [NSThread callStackSymbols]);
...
- (NSImage *)imageWithPointSize:(CGFloat)pointSize {
    ...
    // BUG-7226: FALSE POSITIVE - NSImage lockFocus does not fail in practice on modern macOS.
    // The API returns void and does not provide error indication. The image size is validated
    // above (> 0 width and height) which is the main failure case.
    [image lockFocus];
...
- (NSDictionary *)attributesWithPointSize:(CGFloat)pointSize {
    NSDictionary *attributes = @{
        NSFontAttributeName : [self.delegate badgeLabelFontOfSize:pointSize],
...
- (NSSize)sizeWithAttributes:(NSDictionary *)attributes truncated:(BOOL *)truncated {
    ...
    NSRect bounds = [_stringValue it_boundingRectWithSize:self.maxSize attributes:attributes truncated:truncated];
    return bounds.size;
}
...
- (CGFloat)idealPointSize {
    ...
    while (points != prevPoints) {
        ...
        if (truncated || sizeWithFont.width > maxSize.width || sizeWithFont.height > maxSize.height) {
            max = points;
