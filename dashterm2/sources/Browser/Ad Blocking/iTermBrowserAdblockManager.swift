...
class iTermBrowserAdblockManager: NSObject {
    ...
    private var compiledRulesPath: String {
        // BUG-1692: Use nil-coalescing instead of force unwrap for application support directory
        let appSupport = FileManager.default.applicationSupportDirectory() ?? NSTemporaryDirectory()
    ...
    private func downloadAndUpdateRules() {
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            DispatchQueue.main.async {
                ...
                // BUG-1049: Validate HTTP status code
    ...
    private func compileRules(from jsonString: String) {
        guard let jsonData = jsonString.data(using: .utf8),
            ...
            DLog("Failed to parse ad block rules JSON")
            return
        }
        guard let jsonData = try? JSONSerialization.data(withJSONObject: array, options: []) else {
            DLog("Failed to serialize ad block rules")
            return
        }
        let string = jsonData.lossyString
