...
extension iTermBrowserPermissionManager {
    ...
    func requestPermission(for permissionType: BrowserPermissionType, origin: String) async -> BrowserPermissionDecision {
        ...
        case .audioPlayback:
            // BUG-f554: Return denied for audioPlayback instead of crashing - client should handle it
            DLog("audioPlayback permission request reached manager - returning denied, client should handle")
            return .denied
        }
    ...
    private func handleMediaPermissionRequest(camera: Bool, microphone: Bool, origin: String) async -> BrowserPermissionDecision {
        if camera && !microphone {
            ...
            // BUG-f555: Return denied when neither camera nor microphone is requested
            DLog("handleMediaPermissionRequest called with camera=false, microphone=false - returning denied")
            return .denied
        }
    }
