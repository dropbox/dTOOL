...
extension iTermBrowserWebView {
    ...
    func convertToJavaScriptCoordinates(_ windowPoint: NSPoint) -> NSPoint {
        ...
        // BUG-515: Guard against division by zero when pageZoom or magnification is zero
        let safePageZoom = self.pageZoom > 0 ? self.pageZoom : 1.0
        let safeMagnification = self.magnification > 0 ? self.magnification : 1.0

        let baseJSX = pointInView.x / safePageZoom
        let baseJSY = pointInView.y / safePageZoom

        let jsX = baseJSX / safeMagnification
        let jsY = baseJSY / safeMagnification
    ...
    func allMatches(rules: [SmartSelectRule],
        ...
        for rule in rules {
            for i in 0..<text.utf16.count {
                ...
                // BUG-403: Use .first for safe array access instead of [0] index
                ...
                results.append(.init(beforeCount: 0,
                                     ...
                                     score: rule.weight * Double(firstComponent.length)))
                break
            }
        }
        return results
