...
class iTermBrowserAudioHandler {
    static let messageHandlerName = "DashTerm2AudioHandler"
    ...
    func javascript(world: WKContentWorld) -> String {
        ...
        default:
            // BUG-f553: Return empty string for unexpected world instead of crashing
            DLog("Unexpected world type in javascript(): returning empty string")
            return ""
        }
    }
    ...
    func mute(_ webView: iTermBrowserWebView, frame: WKFrameInfo) async {
        do {
            _ = try await webView.safelyEvaluateJavaScript(iife("window.DashTerm2AudioMuting.mute('\(secret)');"),
                                                     in: frame,
                                                     contentWorld: .page)
    ...
    func unmute(_ webView: iTermBrowserWebView, frame: WKFrameInfo) async {
        do {
            _ = try await webView.safelyEvaluateJavaScript(iife("window.DashTerm2AudioMuting.unmute('\(secret)');"),
                                                     in: frame,
                                                     contentWorld: .page)
