(function() {
    ...
    console.debug('[DashTerm2 Audio] monitor-play.js loaded');

    // Wrap play() method on HTMLMediaElement prototype
    const originalPlay = HTMLMediaElement.prototype.play;
    HTMLMediaElement.prototype.play = function(...args) {
        console.debug('[DashTerm2 Audio] play() method called directly on', this.tagName, 'element');
        console.debug('[DashTerm2 Audio] Element state:', {
            src: this.src,
        ...
        detectAudio(this).then(audioInfo => {
            console.debug('[DashTerm2 Audio] detectAudio resolved with:', audioInfo);
            ...
            if (this.muted || this.volume === 0) {
                ...
                console.debug('[DashTerm2 Audio] Overriding detection due to mute/volume');
            }

            if (hasAudio) {
                console.debug('[DashTerm2 Audio] play() method will produce audio:', reason);
                console.debug('[DashTerm2 Audio] Sending message to handler...');
                try {
                    window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({
                        event: 'play',
                        sessionSecret: sessionSecret
                    });
                    console.debug('[DashTerm2 Audio] Message sent successfully');
                } catch (err) {
                    console.error('[DashTerm2 Audio] Failed to send message:', err);
                }
            } else {
                console.debug('[DashTerm2 Audio] Not sending message, hasAudio=false, reason:', reason);
            }
        }).catch(err => {
            console.debug('[DashTerm2 Audio] detectAudio error:', err);
        });
    ...
    async function detectAudio(mediaElement) {
        console.debug(`[DashTerm2 Audio] detectAudio on ${mediaElement}`);
        // Check cache first
        if (audioDetectionCache.has(mediaElement)) {
            console.debug('[DashTerm2 Audio] Using cached result');
        ...
        console.debug('[DashTerm2 Audio] Checking audioTracks...');
        ...
        if (mediaElement.audioTracks && typeof mediaElement.audioTracks.length === 'number') {
            console.debug(`[DashTerm2 Audio] audioTracks.length = ${mediaElement.audioTracks.length}`);
            if (mediaElement.audioTracks.length > 0) {
                ...
                console.debug(`[DashTerm2 Audio] audioTracks is 0 but readyState=${mediaElement.readyState} (metadata not loaded)`);
                // Keep reason as 'unknown' so the fallback will trigger
                reason = 'unknown';
            }
        } else {
            console.debug('[DashTerm2 Audio] audioTracks not available or not a number');
        }

        console.debug('[DashTerm2 Audio] Checking captureStream...');
        ...
        if (!hasAudio && mediaElement.captureStream) {
            try {
                ...
                console.debug(`[DashTerm2 Audio] captureStream audioTracks.length = ${audioTracks.length}`);
                ...
                console.debug('[DashTerm2 Audio] captureStream detection failed:', err.message);
            }
        } else if (!mediaElement.captureStream) {
            console.debug('[DashTerm2 Audio] captureStream not available');
        }

        console.debug('[DashTerm2 Audio] Checking mozHasAudio...');
        // Method 3: Check mozHasAudio for Firefox
        if (!hasAudio && typeof mediaElement.mozHasAudio === 'boolean') {
            console.debug(`[DashTerm2 Audio] mozHasAudio = ${mediaElement.mozHasAudio}`);
            hasAudio = mediaElement.mozHasAudio;
            reason = hasAudio ? 'mozHasAudio is true' : 'mozHasAudio is false';
        } else {
            console.debug('[DashTerm2 Audio] mozHasAudio not available');
        }
        ...
        if (!hasAudio && reason === 'unknown') {
            console.debug('[DashTerm2 Audio] Using fallback - assuming audio present');
            hasAudio = true;
            reason = 'assuming audio present (metadata not loaded or detection inconclusive)';
        }

        const result = { hasAudio, reason };
        console.debug('[DashTerm2 Audio] Final detection result:', result);
    ...
    document.addEventListener('play', async function(e) {
        ...
        console.debug('[DashTerm2 Audio] Play event:', {
        ...
        if (hasAudio) {
            window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({ event: 'play', sessionSecret: sessionSecret });
        }
    }, true);

    document.addEventListener('pause', function(e) {
        console.debug('[DashTerm2 Audio] Pause event on', window.location.href);
        window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({ event: 'pause', sessionSecret: sessionSecret });
    }, true);
    ...
    function WrappedAudio(...args) {
        console.debug('[DashTerm2 Audio] Audio element created via constructor');
        const audio = new originalAudio(...args);

        // Check if autoplay is set
        if (audio.autoplay) {
            console.debug('[DashTerm2 Audio] Audio element has autoplay');
            window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({
                event: 'play',
                sessionSecret: sessionSecret
            });
        ...
        Object.defineProperty(audio, 'autoplay', {
            ...
            set(value) {
                autoplayValue = value;
                if (value) {
                    console.debug('[DashTerm2 Audio] Audio element autoplay enabled');
                    window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({
                        event: 'play',
                        sessionSecret: sessionSecret
                    });
    ...
    document.createElement = function(tagName, ...args) {
        ...
        if (tagName.toLowerCase() === 'audio' || tagName.toLowerCase() === 'video') {
            console.debug(`[DashTerm2 Audio] ${tagName} element created via createElement`);

            // Monitor autoplay attribute
            const originalSetAttribute = element.setAttribute;
            element.setAttribute = function(name, value) {
                if (name === 'autoplay') {
                    console.debug(`[DashTerm2 Audio] ${tagName} autoplay attribute set`);
                    window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({
                        event: 'play',
                        sessionSecret: sessionSecret
                    });
            ...
            Object.defineProperty(element, 'autoplay', {
                ...
                set(value) {
                    autoplayValue = value;
                    if (value) {
                        console.debug(`[DashTerm2 Audio] ${tagName} autoplay property set to true`);
                        window.webkit.messageHandlers.DashTerm2AudioHandler.postMessage({
                            event: 'play',
                            sessionSecret: sessionSecret
                        });
