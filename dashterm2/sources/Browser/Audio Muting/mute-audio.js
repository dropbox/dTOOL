;(function() {
    ...
    try {
        const secret = '{{SECRET}}';
        console.debug('[DashTerm2-AudioMute] mute-audio.js loading in frame:', window.location.href);
        ...
        function applyStateToElem(e) {
            try {
                ...
                console.warn("[DashTerm2-AudioMute] Failed to apply mute state to element:", err);
            }
        }
        ...
        if (window.AudioContext && canModifyAudioContext) {
            try {
                ...
                console.warn("[DashTerm2-AudioMute] Failed to wrap AudioContext:", err);
            }
        } else if (window.AudioContext) {
            console.warn("[DashTerm2-AudioMute] Cannot modify AudioContext - property is not writable/configurable");
        }
        ...
        if (window.OfflineAudioContext && canModifyOfflineAudioContext) {
            try {
                ...
                console.warn("[DashTerm2-AudioMute] Failed to wrap OfflineAudioContext:", err);
            }
        } else if (window.OfflineAudioContext) {
            console.warn("[DashTerm2-AudioMute] Cannot modify OfflineAudioContext - property is not writable/configurable");
        }

        // API methods
        function mute(sessionSecret) {
            if (sessionSecret != secret) return;
            console.debug('[DashTerm2-AudioMute] Muting audio in frame:', window.location.href);
            muted = true;
            ...
            console.debug('[DashTerm2-AudioMute] Applied mute to', trackedCount, 'tracked audio/video elements');

            // Also check DOM in case we missed any
            const elements = document.querySelectorAll('audio,video');
            console.debug('[DashTerm2-AudioMute] Found', elements.length, 'audio/video elements in DOM');
            elements.forEach(elem => {
                trackElement(elem); // Track them for future
                applyStateToElem(elem);
            });

            console.debug('[DashTerm2-AudioMute] Found', gainNodes.length, 'gain nodes to mute');
            gainNodes.forEach(g => {
        ...
        if (window.hasOwnProperty('DashTerm2AudioMuting')) {
            console.warn("[DashTerm2-AudioMute] DashTerm2AudioMuting already exists on window");
            return;
        }
        
        // Check if we can define the property
        const descriptor = Object.getOwnPropertyDescriptor(window, 'DashTerm2AudioMuting');
        if (descriptor && !descriptor.configurable) {
            console.warn("[DashTerm2-AudioMute] Cannot define DashTerm2AudioMuting - property exists and is not configurable");
            return;
        }
        
        try {
            Object.defineProperty(window, "DashTerm2AudioMuting", {
                value: api,
                writable: false,
                configurable: false,
                enumerable: true
            ...
            console.error("[DashTerm2-AudioMute] Failed to define DashTerm2AudioMuting:", defineError);
            // Fallback: try simple assignment
            try {
                window.DashTerm2AudioMuting = api;
            } catch (assignError) {
                console.error("[DashTerm2-AudioMute] Failed to assign DashTerm2AudioMuting:", assignError);
            }
        }
        ...
        console.error("[DashTerm2-AudioMute] Error while loading mute-audio:", e.toString());
        console.error("[DashTerm2-AudioMute] Stack:", e.stack);
    }
})();
true;
