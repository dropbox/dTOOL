...
class iTermBrowserBookmarkViewHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f539: Use a fallback URL instead of crashing if URL creation fails
    static let bookmarksURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):bookmarks") else {
            DLog("iTermBrowserBookmarkViewHandler: Failed to create bookmarksURL")
