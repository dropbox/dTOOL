...
class iTermBrowserFindManager: NSObject {
    static let messageHandlerName = "DashTerm2CustomFind"
    ...
    fileprivate class State {
        ...
        private func json(_ obj: Any?) -> String {
            ...
            guard let data = try? JSONSerialization.data(withJSONObject: obj, options: [.fragmentsAllowed]) else {
                DLog("Failed to serialize object to JSON")
                return "null"
            }
            return data.lossyString
        }
    ...
    func convertVisibleSearchResultsToContentNavigationShortcuts(action: iTermContentNavigationAction,
        ...
        Task {
            do {
                ...
                // BUG-f549: Handle unknown selection actions gracefully
                ...
                @unknown default:
                    DLog("iTermBrowserFindManager: Unknown selection action, defaulting to open")
                    selectionAction = "open"
                }
