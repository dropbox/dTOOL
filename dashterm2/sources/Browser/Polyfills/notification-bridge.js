...
(function() {
    ...
    try {
        ...
        function iTermNotification(title, options = {}) {
            ...
            if (!window.webkit || !window.webkit.messageHandlers || !window.webkit.messageHandlers.DashTerm2Notification) {
                throw new DOMException("Failed to show notification: native bridge not available.", 'AbortError');
            }

            try {
                window.webkit.messageHandlers.DashTerm2Notification.postMessage({
                    type: 'show',
                    id: notificationId,
                    title: this.title,
                    body: this.body,
