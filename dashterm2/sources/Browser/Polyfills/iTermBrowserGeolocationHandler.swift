...
class iTermBrowserGeolocationHandler: NSObject {
    ...
    static let messageHandlerName = "DashTerm2Geolocation"
    ...
    init(user: iTermBrowserUser) {
        self.user = user
        // BUG-f546: Use a fallback secret instead of crashing
        guard let secret = String.makeSecureHexString() else {
            DLog("iTermBrowserGeolocationHandler: Failed to generate secure token, using UUID fallback")
