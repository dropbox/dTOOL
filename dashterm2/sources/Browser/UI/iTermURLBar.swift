...
class iTermURLBar: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f842: Return nil instead of crashing for unused coder initializer
        DLog("iTermURLBar init(coder:) is not supported")
        return nil
    }
...
class iTermURLBarGuts: NSView {
    ...
    private func setupIcons() {
        // Favicon view
        let newFaviconView = NSImageView()
        newFaviconView.imageScaling = .scaleProportionallyUpOrDown
        newFaviconView.unregisterDraggedTypes()  // Clear default drag behavior
        newFaviconView.setAccessibilityRole(.button)
        newFaviconView.setAccessibilityLabel("Site icon - drag to copy URL")
        faviconView = newFaviconView
        addSubview(newFaviconView)

        // Progress indicator
        let newProgressIndicator = NSProgressIndicator()
        newProgressIndicator.style = .spinning
        newProgressIndicator.controlSize = .small
        newProgressIndicator.isHidden = true
        progressIndicator = newProgressIndicator
        addSubview(newProgressIndicator)
    }
    ...
    private var locationForCompletionsWindow: NSRect {
        ...
        // BUG-1611: Use guard to safely unwrap window instead of force unwrap
