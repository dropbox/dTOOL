...
class iTermBrowserHistoryViewHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f536: Use a fallback URL instead of crashing if URL creation fails
    static let historyURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):history") else {
            DLog("iTermBrowserHistoryViewHandler: Failed to create historyURL")
