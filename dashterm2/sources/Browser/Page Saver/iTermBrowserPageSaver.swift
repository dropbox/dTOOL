            ...
            if let urlRange = Range(match.range(at: 1), in: cssContent) {
                ...
                if let absoluteURL = URL(string: urlString, relativeTo: baseURL) {
                    ...
                    if let localPath = await downloadResource(absoluteURLString) {
                        ...
                        // BUG-1613: Use guard to safely convert Range instead of force unwrap
                        guard let fullMatchRange = Range(match.range(at: 0), in: cssContent) else {
                            continue
                        }
...
extension String {
    var jsonEscaped: String {
        // BUG-1548: Use try? to avoid crash on encoding failure
