...
class iTermBrowserSourceHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f540: Use a fallback URL instead of crashing if URL creation fails
    static let sourceURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):source") else {
            DLog("iTermBrowserSourceHandler: Failed to create sourceURL")
