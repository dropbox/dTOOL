...
extension BrowserBookmarkTags {
    ...
    static func searchBookmarksWithTagsQuery(searchTerms: String, tags: [String], offset: Int = 0, limit: Int = 50) -> (String, [Any?]) {
        ...
        if !tags.isEmpty {
            ...
            let tagPlaceholders = tags.map { _ in "?" }.joined(separator: ", ")
            conditions.append("t.\(Columns.tag.rawValue) IN (\(tagPlaceholders))")
        ...
        // For multiple tags, group by bookmark and ensure all tags are present (intersection)
        if tags.count > 1 {
            query += " GROUP BY b.rowid HAVING COUNT(DISTINCT t.\(Columns.tag.rawValue)) = ?"
            args.append(tags.count)
        }
