...
actor BrowserDatabase {
    ...
    private let databaseQueue = DispatchQueue(label: "com.dashterm.dashterm2.browser-db", qos: .utility)
    let userID: String?
    ...
    static func makeEphemeralInstance() async -> BrowserDatabase? {
        // BUG-1595: Use guard to safely create URL and instance instead of force unwrap
        guard let url = URL(string: "file::memory:?mode=memory&cache=shared") else {
            return nil
        }
        return await BrowserDatabase(url: url, userID: nil)
    }
    ...
    // Note: We capture _db before entering the closure to avoid Swift 6 actor-isolation warnings.
    // This is safe because _db is initialized once in init and never changes, and all
    // database access is serialized through databaseQueue.
    // The iTermDatabase ObjC protocol is not Sendable-aware, but our usage is thread-safe.
    ...
    func clearKeyValueStore(area: String?, extensionId: String?, keys: Set<String>) async throws -> [String: String] {
        return try await withDatabase { db in
            ...
            defer { resultSet.close() }  // BUG-1016: Ensure ResultSet is closed
            var result = [String: String]()
            while resultSet.next() {
