...
class iTermBrowserVisitsFinder: NSObject {
    // nonisolated(unsafe) because cache access is protected by busy counter
    nonisolated(unsafe) private static var cache: (String, [iTermTriple<NSString, NSURL, NSString>])?
    // nonisolated(unsafe) because MutableAtomicObject provides thread-safe access
    nonisolated(unsafe) private static var busy = MutableAtomicObject(0)
    ...
    private static func triple(for visit: BrowserVisits,
        ...
        guard let userID = db.userID else {
            return nil
        }
