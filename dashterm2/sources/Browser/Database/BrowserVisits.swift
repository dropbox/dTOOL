...
extension BrowserVisits {
    ...
    static func searchQuery(terms: String, maxAge: Int, minCount: Int, offset: Int = 0, limit: Int = 50) -> (String, [Any?])? {
        ...
        // BUG-2740: Clamp maxAge to prevent integer overflow when maxAge > now
        // This ensures the threshold timestamp is never negative (Unix epoch is 0)
        let clampedMaxAge = Int64(max(0, maxAge))
        let thresholdTimestamp = max(0, now - clampedMaxAge)
        let maxAgeArg = "\(thresholdTimestamp)"
