...
//    # Adding Static dashterm2-about: Pages
//
//    This document explains how to add simple static HTML pages to DashTerm2's browser system using the built-in template system.
//
...
//    ### 3. Access your page
//
//    Navigate to `dashterm2-about:help` (or whatever urlPath you chose) in the browser.
//
//    ## That's it!
//
...
struct iTermBrowserStaticPageConfig {
    ...
    // BUG-f556: Use fallback URL instead of crashing on invalid path
    init(urlPath: String, templateName: String, substitutions: [String: String] = [:]) {
        if let url = URL(string: "\(iTermBrowserSchemes.about):\(urlPath)") {
            self.url = url
        } else {
            DLog("ERROR: Failed to create URL for static page: \(urlPath) - using fallback")
