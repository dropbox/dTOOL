...
struct iTermBrowserSchemes {
    static let about = "dashterm2-about"
    static let ssh = "dashterm2-ssh"
    static let file = "dashterm2-file"
...
class iTermBrowserLocalPageManager: NSObject {
    ...
    func showErrorPage(for error: Error, failedURL: URL?, webView: iTermBrowserWebView) {
        ...
        // Navigate to dashterm2-about:error which our custom URL scheme handler will serve
    ...
    func showFilePage(for path: String, webView: iTermBrowserWebView) {
        guard let sanitizedPath = sanitizedAbsolutePath(path) else {
        ...
        guard let fileURL = fileSchemeURL(for: sanitizedPath) else {
            showErrorPage(for: iTermError("Invalid file path: \(sanitizedPath)"),
                          failedURL: URL(fileURLWithPath: sanitizedPath),
                          webView: webView)
            return
        }
    ...
    func showSourcePage(htmlSource: String, url: URL, webView: iTermBrowserWebView) {
        ...
        // Store the source HTML to serve when dashterm2-about:source is requested
        sourceHandler.setPendingSourceHTML(sourceHTML)
        
        // Navigate to dashterm2-about:source which our custom URL scheme handler will serve
    ...
    private func sanitizedAbsolutePath(_ rawPath: String) -> String? {
    ...
    private func fileSchemeURL(for sanitizedPath: String) -> URL? {
...
@MainActor
extension iTermBrowserLocalPageManager {
    ...
    fileprivate func handleMessageForContext(_ context: iTermBrowserPageContext, messageURL: String, messageDict: [String: Any], message: WKScriptMessage, webView: iTermBrowserWebView) -> Bool {
        switch messageURL {
        case iTermBrowserSettingsHandler.settingsURL.absoluteString:
