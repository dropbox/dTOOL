...
class iTermBrowserErrorHandler: NSObject, iTermBrowserPageHandler {
    ...
    // BUG-f555: Use hardcoded fallback URL instead of crashing
    static let errorURL: URL = {
        if let url = URL(string: "\(iTermBrowserSchemes.about):error") {
            return url
        }
        DLog("ERROR: Failed to create errorURL - using fallback")
        // Fallback to about:blank if URL creation fails (shouldn't happen)
    ...
    private func errorTitleAndMessage(for error: Error) -> (title: String, message: String) {
        ...
        let tuple: (String, String, String?) = {
            ...
            case NSURLErrorCannotFindHost:
                return ("Server Not Found", "DashTerm2 can't find the server. Check that the web address is correct and try again.", nil)
            ...
            case NSURLErrorCannotConnectToHost:
                return ("Can't Connect to Server", "DashTerm2 can't establish a secure connection to the server. The server may be down or unreachable.", nil)
            ...
            case NSURLErrorHTTPTooManyRedirects:
                return ("Too Many Redirects", "DashTerm2 can't open the page because the server redirected too many times.", nil)

            case NSURLErrorResourceUnavailable:
                return ("Page Unavailable", "The requested page is currently unavailable. Try again later.", nil)

            // Duplicate case removed - NSURLErrorNotConnectedToInternet already handled above

            case NSURLErrorServerCertificateUntrusted, NSURLErrorSecureConnectionFailed:
                return ("Secure Connection Failed", "DashTerm2 can't verify the identity of the website. The connection may not be secure.", sslErrorDetails(from: error))
...
private func secTrust(fromUserInfo ui: [String: Any]) -> SecTrust? {
    ...
    // BUG-1752: The force cast is safe here because we've already verified the type ID,
    // but use unsafeBitCast for explicitness about the intentional bridging
    let cf = any as CFTypeRef
    if CFGetTypeID(cf) == SecTrustGetTypeID() {
        return unsafeBitCast(cf, to: SecTrust.self)
    }
    return nil
}
