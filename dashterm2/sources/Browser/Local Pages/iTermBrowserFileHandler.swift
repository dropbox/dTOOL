...
class iTermBrowserFileHandler: NSObject, iTermBrowserPageHandler {
    ...
    func start(urlSchemeTask: WKURLSchemeTask, url: URL) {
        ...
        // URL format: dashterm2-file:///path/to/file or dashterm2-file:///path/to/folder/
        guard let normalizedPath = normalizedFileSystemPath(from: url.path) else {
            urlSchemeTask.didFailWithError(NSError(domain: "iTermBrowserManager",
                                                  code: -1,
    ...
    private func generateDirectoryHTML(for path: String) throws -> String {
        ...
        for item in contents.sorted(by: { a, b in
            ...
            // Generate absolute dashterm2-file:// URLs using URLComponents
    ...
    private func normalizedFileSystemPath(from rawPath: String) -> String? {
