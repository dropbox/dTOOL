...
//  DashTerm2
//
//  Created for DashTerm2 Browser onboarding flow
//
...
class iTermBrowserOnboardingHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f541: Use a fallback URL instead of crashing if URL creation fails
    static let setupURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):onboarding-setup") else {
            DLog("iTermBrowserOnboardingHandler: Failed to create setupURL")
            return URL(string: "about:blank")!
        }
        return url
    }()
    // BUG-f542: Use a fallback URL instead of crashing if URL creation fails
    static let profileURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):onboarding-profile") else {
            DLog("iTermBrowserOnboardingHandler: Failed to create profileURL")
            return URL(string: "about:blank")!
        }
        return url
    }()
    ...
    init(user: iTermBrowserUser) {
        self.user = user
        // BUG-f543: Use a fallback secret instead of crashing
        guard let secret = String.makeSecureHexString() else {
            DLog("iTermBrowserOnboardingHandler: Failed to generate secure hex string, using UUID fallback")
