...
class iTermBrowserWelcomePageHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f545: Use a fallback URL instead of crashing if URL creation fails
    static let welcomeURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):welcome") else {
            DLog("iTermBrowserWelcomePageHandler: Failed to create welcomeURL")
    ...
    func start(urlSchemeTask: WKURLSchemeTask, url: URL) {
        ...
        if !onboardingCompleted {
            ...
            <html>
            <head>
                <meta http-equiv="refresh" content="0; url=dashterm2-about:onboarding-intro">
            </head>
            <body>
                <script>window.location.href = "dashterm2-about:onboarding-intro";</script>
            </body>
            </html>
            """
