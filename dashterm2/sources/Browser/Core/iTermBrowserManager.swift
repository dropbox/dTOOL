...
class iTermBrowserManager: NSObject, WKURLSchemeHandler, WKScriptMessageHandler {
    ...
    private static let safeSearchQueryCharacters: CharacterSet = {
    ...
    deinit {
        ...
        // BUG-9464: Unregister web view synchronously. Since iTermBrowserManager is @MainActor,
        // deinit runs on MainActor but Swift doesn't automatically recognize this isolation.
        // Use MainActor.assumeIsolated to call the MainActor-isolated method synchronously,
        // preventing async Task use-after-free issues.
    ...
    private func configure(_ configuration: WKWebViewConfiguration,
        ...
        configuration.userContentController.add(handlerProxy, contentWorld: .page, name: "DashTerm2ConsoleLog")
        configuration.userContentController.add(handlerProxy, contentWorld: .defaultClient, name: "DashTerm2ConsoleLog")
        configuration.userContentController.add(handlerProxy, contentWorld: .page, name: "dashterm2-about:permissions")
...
extension iTermBrowserManager: iTermBrowserWebViewDelegate {
    ...
    func urlForWebSearch(query: String) -> URL? {
        guard let escapedQuery = query.addingPercentEncoding(withAllowedCharacters: Self.safeSearchQueryCharacters),
...
extension iTermBrowserManager {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        // Duplicate case removed - iTermBrowserFindManager.messageHandlerName already handled above
...
extension iTermBrowserManager: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        // BUG-f636: Use guard instead of it_assert - assertions stripped in release builds
        guard webView === self.webView else {
            DLog("BUG-f636: didFinish called with wrong webView - ignoring")
            return
        }
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        // Only show error page if this isn't the same URL that already failed
        if failedURL != lastFailedURL && failedURL != nil {
            // Convert regular file:// URLs to our custom dashterm2-file:// scheme
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        // BUG-f636: Use guard instead of it_assert - assertions stripped in release builds
        guard webView === self.webView else {
            DLog("BUG-f636: decidePolicyFor called with wrong webView - cancelling")
        ...
        if navigationAction.targetFrame == nil {
            ...
            case .linkActivated:
                ...
                // BUG-1742: Use as? with fallback to original configuration
                let config = (webView.configuration.copy() as? WKWebViewConfiguration) ?? webView.configuration
...
extension iTermBrowserManager: WKUIDelegate {
    func webView(_ webView: WKWebView,
        ...
        // BUG-f636: Use guard instead of it_assert - assertions stripped in release builds
        guard webView === self.webView else {
            DLog("BUG-f636: createWebViewWith called with wrong webView - returning nil")
            return nil
        }
        localPageManager.unregisterAllMessageHandlers(webView: self.webView, except: nil)
        ...
        panel.beginSheetModal(for: window) { response in
            ...
            group.notify(queue: .main) {
                ...
                if parameters.allowsMultipleSelection {
                    ...
                    // BUG-1632: Use optional binding instead of force unwrap
                    if let firstURL = selectedURLs.first {
                        completionHandler([firstURL])
                    } else {
                        completionHandler(nil)
                    }
                }
    ...
    func webView(_ webView: WKWebView,
        ...
        // BUG-f636: Use guard instead of it_assert - assertions stripped in release builds
        guard webView === self.webView else {
            DLog("BUG-f636: decideMediaCapturePermissionsFor called with wrong webView - denying")
            return .deny
        }
