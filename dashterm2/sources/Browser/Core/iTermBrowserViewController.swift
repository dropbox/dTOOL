...
class iTermBrowserViewController: NSViewController {
    ...
    @objc var zoom: CGFloat {
        get {
            guard let webView = browserManager.webView else { return 100.0 }
            return round(webView.pageZoom * 100.0)
        }
        set {
    ...
    init(configuration: WKWebViewConfiguration?,
        ...
        let user: iTermBrowserUser = if profileObserver.value(KEY_BROWSER_DEV_NULL) == true {
            ...
            .regular(id: UUID(uuidString: "AC0E9812-7F88-478B-B361-5526082EDDB3") ?? UUID())
        }
    ...
    required init?(coder: NSCoder) {
        // BUG-f847: Return nil instead of crashing for unused coder initializer
        DLog("iTermBrowserViewController init(coder:) is not supported")
        return nil
    }
}
...
extension iTermBrowserViewController {
    ...
    // Additional find methods for integration with DashTerm2's find system
...
extension iTermBrowserViewController {
    ...
    func performFindPanelAction(_ sender: Any?) {
        ...
        case .setFindString:
            ...
            // BUG-f548: Handle unknown find modes gracefully instead of crashing
            ...
            case .caseSensitiveRegex, .caseInsensitiveRegex:
                selectedText = (raw as NSString).escapingForRegex() as String
            @unknown default:
                DLog("iTermBrowserViewController: Unknown find mode, using raw text")
                selectedText = raw
            }
