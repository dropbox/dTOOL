...
class iTermBrowserGateway: NSObject {
    ...
    private static let bundleID = "com.dashterm.dashterm2.iTermBrowserPlugin"
    ...
    static func offerPlugin() {
        ...
        if selection == .kiTermWarningSelection0 {
            ...
            if let url = URL(string: "https://dashterm.com/browser-plugin.html") {
    ...
    static func upsell() -> iTermTriState {
        if iTermAdvancedSettingsModel.suppressBrowserPluginUpsell() {
            return .false  // Silently use system browser
        }
        let selection = iTermWarning.show(withTitle: "DashTerm2 can display web pages! But first you must download the Browser Plugin.",
        ...
        case .kiTermWarningSelection2:
            return .other
        default:
            // BUG-f547: Return .other instead of crashing for unknown selection
            DLog("iTermBrowserGateway: Unexpected selection \(selection.rawValue) in upsell()")
            return .other
        }
    }
