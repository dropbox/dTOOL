...
class iTermBrowserTriggerHandler: NSObject {
    ...
    static let messageHandlerName = "DashTerm2Trigger"
...
extension iTermBrowserTriggerHandler {
    func highlightText(matchID: String, textColor: String?, backgroundColor: String?) {
        ...
        Task {
            // BUG-1533: Use try? to avoid crash on encoding failure
            guard let jsonMatchID = (try? JSONEncoder().encode(matchID).lossyString) else {
                DLog("Failed to encode matchID")
                return
            }
            ...
            let script = "window.DashTerm2Triggers?.highlightText(\(jsonMatchID), \(jsonTextColor), \(jsonBackgroundColor));"
    ...
    func makeHyperlink(matchID: String, url: String) {
        ...
        Task {
            // BUG-1533: Use try? to avoid crash on encoding failure
            guard let jsonMatchID = (try? JSONEncoder().encode(matchID).lossyString),
                  let jsonURL = (try? JSONEncoder().encode(url).lossyString) else {
                DLog("Failed to encode hyperlink data")
                return
            }
            let script = "window.DashTerm2Triggers?.makeHyperlink(\(jsonMatchID), \(jsonURL));"
            _ = try? await webView.safelyEvaluateJavaScript(iife(script), contentWorld: .defaultClient)
        }
    }
...
extension iTermBrowserTriggerHandler {
    var currentTriggersJSON: String {
        ...
        // BUG-1533: Use try? to avoid crash on serialization failure
        guard let data = try? JSONSerialization.data(withJSONObject: dicts) else {
            DLog("Failed to serialize triggers to JSON")
            return "{}"
        }
    ...
    func sendTriggersToWebView() {
        ...
        try {
            ...
            window.DashTerm2Triggers.setTriggers(
    ...
    private func handle(match: TriggerMatchEvent.Match,
        ...
        if url {
            ...
            // BUG-f556: Return empty actions for content matches until implemented
            DLog("Content matches not implemented yet - returning empty actions")
            return []
        }
    }
}
