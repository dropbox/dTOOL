    ...
    api.makeHyperlink = function(matchId, url) {
        ...
        if (!matchData) {
            console.error('DashTerm2 Hyperlink Maker: No match data found for ID:', matchId);
            return;
        }
        ...
        if (!matchData.range) {
            console.error('DashTerm2 Hyperlink Maker: No range found in match data');
            return;
        }

        // Validate that it's actually a Range object
        if (!(matchData.range instanceof Range)) {
            console.error('DashTerm2 Hyperlink Maker: Stored range is not a Range object:', typeof matchData.range, matchData.range);
            return;
        }
        ...
        try {
            ...
            if (!range.startContainer || !range.endContainer) {
                console.error('DashTerm2 Hyperlink Maker: Invalid range - containers are null');
                return;
            }
            ...
            try {
                ...
                console.error('DashTerm2 Hyperlink Maker: Error wrapping range:', e);
            ...
            console.error('DashTerm2 Hyperlink Maker: Error during hyperlink creation:', error);
        }
    };
    ...
    function hyperlinkFallback(matchData, matchId, url) {
        ...
        if (!matchText) {
            console.error('DashTerm2 Hyperlink Maker: No match text available for fallback');
            return;
        }
