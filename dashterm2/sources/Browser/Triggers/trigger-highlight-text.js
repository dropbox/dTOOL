    ...
    api.highlightText = function(matchId, textColor, backgroundColor) {
        ...
        if (!matchData) {
            console.error('DashTerm2 Text Highlighter: No match data found for ID:', matchId);
            return;
        }
        ...
        if (!matchData.range) {
            console.error('DashTerm2 Text Highlighter: No range found in match data');
            return;
        }

        // Validate that it's actually a Range object
        if (!(matchData.range instanceof Range)) {
            console.error('DashTerm2 Text Highlighter: Stored range is not a Range object:', typeof matchData.range, matchData.range);
            return;
        }
        ...
        try {
            ...
            if (!range.startContainer || !range.endContainer) {
                console.error('DashTerm2 Text Highlighter: Invalid range - containers are null');
                return;
            }
            ...
            try {
                ...
                console.error('DashTerm2 Text Highlighter: Error wrapping range:', e);
            ...
            console.error('DashTerm2 Text Highlighter: Error during highlighting:', error);
        }
    };
    ...
    function highlightFallback(matchData, matchId, textColor, backgroundColor) {
        ...
        if (!matchText) {
            console.error('DashTerm2 Text Highlighter: No match text available for fallback');
            return;
        }
