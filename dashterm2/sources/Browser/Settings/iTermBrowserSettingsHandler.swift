...
class iTermBrowserSettingsHandler: NSObject, iTermBrowserPageHandler {
    // BUG-f537: Use a fallback URL instead of crashing if URL creation fails
    static let settingsURL: URL = {
        guard let url = URL(string: "\(iTermBrowserSchemes.about):settings") else {
            DLog("iTermBrowserSettingsHandler: Failed to create settingsURL")
    ...
    init(user: iTermBrowserUser) {
        self.user = user
        // BUG-f538: Use a fallback secret instead of crashing
        guard let secret = String.makeSecureHexString() else {
            DLog("iTermBrowserSettingsHandler: Failed to generate secure hex string, using UUID fallback")
            self.secret = UUID().uuidString
            super.init()
            return
        }
        self.secret = secret
    ...
    private func sendAdblockSettings(to webView: iTermBrowserWebView) {
        ...
        do {
            ...
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to convert adblock settings to UTF8 string")
                return
            }
    ...
    private func sendAdblockStats(to webView: iTermBrowserWebView) {
        ...
        do {
            ...
            // BUG-1593: Use guard to safely convert data to string instead of force unwrap
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to convert adblock stats to UTF8 string")
                return
            }
    ...
    private func sendSearchSettings(to webView: iTermBrowserWebView) {
        ...
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: settings)
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to encode search settings to UTF-8 string")
                return
            }
    ...
    private func sendProxySettings(to webView: iTermBrowserWebView) {
        ...
        do {
            ...
            // BUG-1593: Use guard to safely convert data to string instead of force unwrap
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to convert proxy settings to UTF8 string")
                return
            }
    ...
    private func sendExtensionsForMacOS14(to webView: iTermBrowserWebView) {
        ...
        do {
            ...
            // BUG-1593: Use guard to safely convert data to string instead of force unwrap
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to convert extensions data to UTF8 string")
                return
            }
    ...
    private func setLinkOpeningPreference(_ preference: String, webView: iTermBrowserWebView) {
        ...
        case "builtin":
            // Set permanent selection to "Open in DashTerm2" (selection 1)
    ...
    private func sendLinkOpeningPreference(to webView: iTermBrowserWebView) {
        ...
        do {
            ...
            // BUG-1593: Use guard to safely convert data to string instead of force unwrap
            guard let jsonString = String(data: jsonData, encoding: .utf8) else {
                DLog("Failed to convert link opening preference to UTF8 string")
                return
            }
