...
class OnePasswordDataSource: CommandLinePasswordDataSource {
    ...
    // BUG-338: Tag uses DashTerm2 branding for 1Password integration
    init(browser: Bool) {
    ...
    // BUG-501 + CRASH-FIX: Make this a designated initializer (not convenience)
    ...
    @objc override init() {
        tag = "DashTerm2"  // Default to terminal mode (same as browser=false)
        tagToExclude = nil
        super.init()
    }
    ...
    private var listAccountsRecipe: AnyRecipe<Void, [Account]> {
        ...
        // the output:
        //     op item list --tags DashTerm2 --format json | op item get --format=json -
        ...
        let itemsRecipe = OnePasswordDynamicCommandRecipe<Data, [Account]>(
            dataSource: self) { data, token throws -> CommandLinePasswordDataSourceExecutableCommand in
                ...
                // BUG-1598: Use guard to safely convert string to data instead of force unwrap
                ...
                return items.compactMap {
                    ...
                    let otpDisabled = $0.tags?.contains("DashTerm2-no-otp") ?? false
    ...
    private var getPasswordRecipe: AnyRecipe<AccountIdentifier, Password> {
        return AnyRecipe(OnePasswordDynamicCommandRecipe<AccountIdentifier, Password>(dataSource: self) { accountIdentifier, token in
            ...
            // 1password UI and manually tagged it with DashTerm2
...
@objc extension OnePasswordDataSource: PasswordManagerDataSource {
    ...
    func checkAvailability() -> Bool {
        ...
        // BUG-CLI-FIX: Use silent check to avoid showing dialogs during availability checks
        let value = OnePasswordUtils.checkUsabilitySilent()
    ...
    // BUG-f505: Return an error instead of crashing when account type is unexpected
    func toggleShouldSendOTP(context: RecipeExecutionContext,
        ...
        guard let account = pmAccount as? CommandLineProvidedAccount else {
            completion(nil, NSError(domain: "com.dashterm2.onepassword",
                                   code: -1,
                                   userInfo: [NSLocalizedDescriptionKey: "Unexpected account type for OTP toggle"]))
            return
        }
        let recipe = if account.sendOTP {
            mutateTagRecipe(accountID: account.identifier) { tags in
                Array(Set(tags).union(Set(["DashTerm2-no-otp"])))
            }
        } else {
            mutateTagRecipe(accountID: account.identifier) { tags in
                Array(Set(tags).subtracting(Set(["DashTerm2-no-otp"])))
            }
        }
