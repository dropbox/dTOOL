...
class PipelineQueue<T> {
    ...
    func append(_ pipeline: Pipeline<T>) {
        ...
        pipeline.completion = { [weak self] disposition in
            // BUG-f543: Guard against double completion instead of crashing
            guard !completed else {
                DLog("Pipeline completion handler called twice, ignoring")
                return
            }
            completed = true
...
class Pipeline<T>: CustomDebugStringConvertible {
    ...
    init(actions: [Action],
        ...
        dispatchPrecondition(condition: .onQueue(.main))
        // BUG-f510: Clamp maxConcurrentActions to at least 1 instead of crashing
        let safeMaxConcurrent = max(1, maxConcurrentActions)
        if maxConcurrentActions < 1 {
            DLog("Pipeline: maxConcurrentActions \(maxConcurrentActions) < 1, clamping to 1")
        }
        let ids = actions.map { $0.id }
        // BUG-f511: Log duplicate IDs but don't crash - let the pipeline continue with potential issues
        if Set(ids).count != ids.count {  // swiftlint:disable:this empty_count
            DLog("Pipeline: Action IDs are not unique - this may cause unexpected behavior")
        }

        self.actions = actions
        self.completion = completion
        self.maxConcurrentActions = safeMaxConcurrent
        blockedActions = actions
    }

    func begin() {
        dispatchPrecondition(condition: .onQueue(.main))
        // BUG-f545: Guard against double begin instead of crashing
        guard !begun else {
            DLog("Pipeline.begin() called twice, ignoring")
            return
        }
        ...
        // BUG-f544: Handle unsatisfiable dependencies gracefully instead of crashing
        let unsatisfiedDeps = allDeps.subtracting(Set(blockedActions.map(\.id)))
        if !unsatisfiedDeps.isEmpty {
            DLog("ERROR: Pipeline has unsatisfiable dependencies \(unsatisfiedDeps) - canceling pipeline")
            // Cancel the pipeline since it can't complete
            finish(.canceled)
            return
        }
        begun = true
