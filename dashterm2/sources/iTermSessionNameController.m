...
NSString *const iTermSessionNameControllerSystemTitleUniqueIdentifier = @"com.dashterm.dashterm2.system-title";
...
- (NSString *)invocationForTitleProviderID:(NSString *)uniqueIdentifier {
    if ([uniqueIdentifier isEqualToString:iTermSessionNameControllerSystemTitleUniqueIdentifier]) {
        return @"dashterm2.private.session_title(session: session.id)";
    }
...
- (void)evaluateInvocationSynchronously:(BOOL)sync
    ...
    if ([uniqueIdentifier isEqualToString:iTermSessionNameControllerSystemTitleUniqueIdentifier]) {
        ...
        [iTermScriptFunctionCall callFunction:@"dashterm2.private.window_title(session: session.id)"
                                      timeout:0
                           sideEffectsAllowed:YES
                                        scope:scope
                                   retainSelf:YES
...
- (NSString *)presentationWindowTitle {
    ...
    if ([self usingBuiltInTitleProvider] && _cachedBuiltInWindowTitleEvaluation) {
        ...
        // BUG-7903: This fallback handles several edge cases:
        // 1. Builtin title provider with no cached value (timing issue during init)
        // 2. Custom title provider not overridden by OSC 0/OSC 2
        // 3. Unexpected state during recovery
        // Return "Unnamed" as a correlatable fallback for debugging if all else fails.
...
- (void)pushWindowTitle {
    if (!_windowTitleStack) {
        // initialize lazily
        _windowTitleStack = [[NSMutableArray alloc] initWithCapacity:4];  // Typical title stack depth
    }
...
- (void)pushIconTitle {
    if (!_iconTitleStack) {
        // initialize lazily
        _iconTitleStack = [[NSMutableArray alloc] initWithCapacity:4];  // Typical icon title stack depth
    }
...
- (NSString *)formattedName:(NSString *)base {
    ...
    if (descriptor.haveTmuxController) {
        ...
        // equivalent to an DashTerm2 tab. It is reported to us by tmux. We ignore the base name
