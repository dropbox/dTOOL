...
// Optimization: Cache NSNumber objects for equivalence class indices (0-127).
// Class indices are small non-negative integers. In typical tmux usage, there
// are only a handful of equivalence classes (windows/panes), so 128 entries
// more than covers all realistic scenarios.
static const int kCachedEquivalenceClassCount = 128;
static NSNumber *sEquivalenceClassCache[kCachedEquivalenceClassCount];

NS_INLINE NSNumber *iTermEquivalenceClassToNumber(int classIndex) {
    if (classIndex >= 0 && classIndex < kCachedEquivalenceClassCount) {
        return sEquivalenceClassCache[classIndex];
    }
    return @(classIndex);
}

__attribute__((constructor)) static void iTermEquivalenceClassSetInitializeCache(void) {
    for (int i = 0; i < kCachedEquivalenceClassCount; i++) {
        sEquivalenceClassCache[i] = @(i);
    }
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        index_ = [[NSMutableDictionary alloc] initWithCapacity:64];  // Object to equivalence class index
        classes_ = [[NSMutableDictionary alloc] initWithCapacity:32];  // Equivalence classes
    }
    return self;
}
...
- (void)log {
    [classes_ enumerateKeysAndObjectsUsingBlock:^(
                  NSNumber *_Nonnull key, NSMutableSet<NSObject<NSCopying> *> *_Nonnull obj, BOOL *_Nonnull stop) {
...
- (void)addValue:(NSObject<NSCopying> *)value toClass:(NSNumber *)ec {
    ...
    if (!theSet) {
        // Most equivalence classes have 2-8 members
        theSet = [NSMutableSet setWithCapacity:4];
        classes_[ec] = theSet;
    }
...
- (NSNumber *)addEquivalenceClass {
    // Optimization: Use cached NSNumber objects for class index lookups.
    // This avoids boxing on every iteration of the while loop.
    int i = 0;
    while (classes_[iTermEquivalenceClassToNumber(i)]) {
        i++;
    }
    return iTermEquivalenceClassToNumber(i);
}
