...
- (NSString *)description {
    NSDictionary *dict = @{
        @"shouldPlacePromptAtFirstColumn" : @(_shouldPlacePromptAtFirstColumn),
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _dirtyKeyPaths = [[NSMutableSet alloc] initWithCapacity:16];
...
- (void)dealloc {
    [VT100MutableScreenConfiguration it_enumerateDynamicProperties:^(NSString *_Nonnull selectorString) {
        [self removeObserver:self forKeyPath:selectorString];
    }];
}
...
NSDictionary *VT100ScreenConfigKeypressIdentifier(unsigned short keyCode, NSEventModifierFlags flags,
                                                  NSString *characters, NSString *charactersIgnoringModifiers) {
    const NSEventModifierFlags mask =
        (NSEventModifierFlagOption | NSEventModifierFlagCommand | NSEventModifierFlagControl |
         NSEventModifierFlagShift | NSEventModifierFlagFunction | NSEventModifierFlagNumericPad);
    return @{
        @"keyCode" : @(keyCode),
        @"flags" : @(flags & mask),
