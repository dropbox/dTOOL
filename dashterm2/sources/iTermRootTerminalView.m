...
@interface iTermRootTerminalView () <iTermTabBarControlViewDelegate, iTermDragHandleViewDelegate,
                                     iTermGenericStatusBarContainer, iTermStoplightHotboxDelegate>
...
- (instancetype)initWithFrame:(NSRect)frameRect
    ...
    if (self) {
        ...
        NSRect stoplightFrame = NSMakeRect(0, 0, iTermStoplightHotboxWidth, iTermStoplightHotboxHeight);
        ...
        NSColor *borderColor = [NSColor colorWithWhite:0.5 alpha:0.75];
        {
            ...
            dispatch_once(&onceToken, ^{
                ...
                gTopLeftCornerHalfImage = [[NSImage it_imageNamed:halfName
                                                         forClass:self.class] it_verticallyFlippedImage];
                ...
                gTopLeftCornerFullImage = [[NSImage it_imageNamed:fullName
                                                         forClass:self.class] it_verticallyFlippedImage];
        ...
        if (@available(macOS 12.0, *)) {
            _notchMask = [[iTermLayerBackedSolidColorView alloc] initWithFrame:NSMakeRect(0, 0, 0, 0)
...
// Upstream fix 02b57f6: Tweak stoplight button positions for macOS 26 minimal style (Issue 12474)
...
- (NSRect)frameForStandardWindowButtons {
    ...
    NSRect frame = NSMakeRect(insets.left, self.frame.size.height - height + insets.bottom + 1,
                              iTermStandardButtonsViewWidth, iTermStandardButtonsViewHeight);
...
- (NSRect)frameForWindowNumberLabel {
    ...
    const CGFloat tabBarHeight =
        [iTermPreferences intForKey:kPreferenceKeyTabPosition] == PSMTab_LeftTab ? 26.0 : _tabBarControl.height;
    ...
    NSRect rect = NSMakeRect(NSMaxX(standardButtonsFrame) + iTermRootTerminalViewWindowNumberLabelMargin,
                             ...
                             iTermRootTerminalViewWindowNumberLabelWidth, windowNumberHeight);
...
- (NSRect)frameForWindowTitleLabel:(NSTextField *)textField
    ...
    NSRect rect = NSMakeRect([self retinaRound:leftInset], y, ceil(MAX(0, containerWidth - leftInset - rightInset)),
                             ceil(fittingSize.height));
    return [self retinaRoundRect:rect];
}

- (NSWindowButton *)windowButtonTypes {
    static NSWindowButton buttons[] = {NSWindowCloseButton, NSWindowMiniaturizeButton, NSWindowZoomButton};
    return buttons;
}
...
- (void)didChangeCompactness {
    ...
    _standardWindowButtonsView =
        [[iTermStandardWindowButtonsView alloc] initWithFrame:[self frameForStandardWindowButtons]];
    ...
    _standardButtons = [[NSMutableDictionary alloc] initWithCapacity:4];  // Standard window buttons
    for (int i = 0; i < self.numberOfWindowButtons; i++) {
        NSButton *button = [NSWindow standardWindowButton:self.windowButtonTypes[i] forStyleMask:styleMask];
...
- (void)updateTitleAndBorderViews NS_AVAILABLE_MAC(10_14) {
    ...
    if (wantsTitleBackgroundView) {
        if (!_titleBackgroundView) {
            _titleBackgroundView =
                [[iTermLayerBackedSolidColorView alloc] initWithFrame:self.frameForTitleBackgroundView];
...
- (void)updateBorderViews NS_AVAILABLE_MAC(10_14) {
...

    {
        ...
        _leftBorderView.frame = NSMakeRect(0, bottomInset, thickness, self.bounds.size.height - topInset - bottomInset);

        _rightBorderView.frame = NSMakeRect(self.bounds.size.width - thickness, bottomInset, thickness,
                                            self.bounds.size.height - topInset - bottomInset);
        _bottomBorderView.frame = NSMakeRect(leftInset, 0, self.bounds.size.width - leftInset - rightInset, thickness);

        _topBorderView.frame = NSMakeRect(leftInset, self.bounds.size.height - thickness,
                                          self.bounds.size.width - leftInset - rightInset, thickness);
    }
...
- (void)setWindowTitleLabelToString:(NSString *)title subtitle:(NSString *)subtitle icon:(NSImage *)icon {
    [_windowTitleLabel setTitle:title
                       subtitle:subtitle
                           icon:icon
              alignmentProvider:^NSTextAlignment(NSTextField *_Nonnull scratch) {
                  BOOL leftAligned = NO;
                  [self frameForWindowTitleLabel:scratch hasSubtitle:subtitle.length > 0 getLeftAligned:&leftAligned];
...
- (iTermTabBarControlView *)borrowTabBarControl {
    ...
    // BUG-f1159: Replace assert() with guard - double borrow should return existing control, not crash
    if (_tabBarControlOnLoan) {
        DLog(@"WARNING: Tab bar control already on loan, returning current control");
        return _tabBarControl;
    }
...
- (void)returnTabBarControlView:(iTermTabBarControlView *)tabBarControl {
    ...
    // BUG-f1160: Replace assert() with guard - double return should be no-op, not crash
    if (!_tabBarControlOnLoan) {
        DLog(@"WARNING: Tab bar control not on loan, ignoring return");
        return;
    }
...
- (void)windowNumberDidChangeTo:(NSNumber *)number {
    ...
    BOOL deemphasized;
    _windowNumberLabel.stringValue =
        [iTermWindowShortcutLabelTitlebarAccessoryViewController stringForOrdinal:number.intValue
...
- (void)setShowsWindowSize:(BOOL)showsWindowSize {
    ...
    _windowSizeView =
        [[iTermWindowSizeView alloc] initWithDetail:[self.delegate rootTerminalViewWindowSizeViewDetailString]];
...
- (void)updateDivisionViewAndWindowNumberLabel {
    ...
    if (shouldBeVisible) {
        ...
        NSRect divisionViewFrame = NSMakeRect(0, NSMaxY(tabViewFrame), self.bounds.size.width, kDivisionViewHeight);
        ...
        if (!_divisionView) {
            ...
            if (@available(macOS 14.0, *)) {
                ...
                // where using a non-layer-backed division view caused all the other views to disappear, including those
                // over it. I don't remember why I fell back to a non-layer-backed view for non-metal. Probably bugs in
                // old macOS versions.
        ...
        switch ([self.effectiveAppearance it_tabStyle:preferredStyle]) {
            ...
            case TAB_STYLE_COMPACT:
            case TAB_STYLE_MINIMAL:
                // BUG-13012: Replace assert(NO) with DLog and fallthrough to LIGHT style
                // These styles shouldn't reach here but handle gracefully with default color
                DLog(@"Unexpected tab style %d in updateDivisionViewAndWindowNumberLabel, using light colors",
                     (int)preferredStyle);
                // Fallthrough to light style
            case TAB_STYLE_LIGHT:
            case TAB_STYLE_LIGHT_HIGH_CONTRAST:
                _divisionView.color =
                    (self.window.isKeyWindow ? [NSColor colorWithCalibratedHue:1 saturation:0 brightness:0.70 alpha:1]
            ...
            case TAB_STYLE_DARK_HIGH_CONTRAST:
                _divisionView.color =
                    (self.window.isKeyWindow ? [NSColor colorWithCalibratedHue:1 saturation:0 brightness:0.1 alpha:1]
...
- (CGFloat)maximumToolbeltWidthForViewWidth:(CGFloat)viewWidth {
    CGFloat minSize = MIN(kMinimumToolbeltSizeInPoints, viewWidth * kMinimumToolbeltSizeAsFractionOfWindow);
    return MAX(MIN(_toolbeltWidth, viewWidth * kMaximumToolbeltSizeAsFractionOfWindow), minSize);
}
...
- (NSRect)toolbeltFrameInWindow:(NSWindow *)thisWindow {
    ...
    NSRect toolbeltFrame =
        NSMakeRect(self.bounds.size.width - width - right, bottom, width, self.bounds.size.height - top - bottom);
    if ([self shouldLeaveEmptyAreaAtTop]) {
        toolbeltFrame.size.height -= _tabBarControl.height;
    }

    return [self tabViewFrameByShrinkingForFullScreenTabBar:toolbeltFrame window:thisWindow];
}
...
- (CGFloat)tabviewWidth {
    // BUG-f1161: Replace assert() with guard - invalid tab position should return zero width, not crash
    if ([iTermPreferences intForKey:kPreferenceKeyTabPosition] == PSMTab_LeftTab && [self tabBarShouldBeVisible]) {
        DLog(@"WARNING: tabviewWidth called with left tab bar visible - returning 0");
        return 0;
    }
...
- (void)layoutSubviewsWithVisibleTabBarForWindow:(NSWindow *)thisWindow inlineToolbelt:(BOOL)showToolbeltInline {
    // BUG-f1162: Replace assert() with guard - layout while on loan should be no-op, not crash
    if (_tabBarControlOnLoan) {
        DLog(@"WARNING: layoutSubviewsWithVisibleTabBarForWindow called while tab bar on loan, ignoring");
        return;
    }
...
- (void)layoutSubviewsWithHiddenTabBarForWindow:(NSWindow *)thisWindow {
    ...
    const NSRect frame =
        NSMakeRect([self leftBorderInset], decorationHeights.bottom, [self tabviewWidth],
    ...
    if ([self tabBarShouldBeVisibleEvenWhenOnLoan]) {
        tabViewFrame = [self tabViewFrameByShrinkingForFullScreenTabBar:tabViewFrame window:thisWindow];
    }
...
- (void)layoutSubviewsTopTabBarVisible:(BOOL)topTabBarVisible forWindow:(NSWindow *)thisWindow {
    [self removeLeftTabBarDragHandle];
    iTermDecorationHeights decorationHeights = {.bottom = [self bottomBorderInset], .top = [self notchInset]};
    if (!_tabBarControlOnLoan) {
        if (!self.tabBarControl.flashing) {
            decorationHeights.top += _tabBarControl.height;
        }
    ...
    NSRect tabViewFrame = NSMakeRect([self leftBorderInset], temp.bottom, [self tabviewWidth],
    ...
    NSRect tabBarFrame =
        NSMakeRect(tabViewFrame.origin.x, NSMaxY(frame) - tabBarOffset, tabViewFrame.size.width, _tabBarControl.height);
...
- (void)setTabBarFrame:(NSRect)frame {
    // BUG-f1163: Replace assert() with guard - setting frame while on loan should be no-op, not crash
    if (_tabBarControlOnLoan) {
        DLog(@"WARNING: setTabBarFrame called while tab bar on loan, ignoring");
        return;
    }
...
- (void)layoutSubviewsWithVisibleBottomTabBarForWindow:(NSWindow *)thisWindow {
    // BUG-f1164: Replace assert() with guard - layout while on loan should be no-op, not crash
    if (_tabBarControlOnLoan) {
        DLog(@"WARNING: layoutSubviewsWithVisibleBottomTabBarForWindow called while tab bar on loan, ignoring");
        return;
    }
    ...
    NSRect tabBarFrame =
        NSMakeRect([self leftBorderInset], [self bottomBorderInset], [self tabviewWidth], _tabBarControl.height);
    self.tabBarControl.insets = [self.delegate tabBarInsets];
    [self setTabBarFrame:tabBarFrame];
    [self setTabBarControlAutoresizingMask:(NSViewWidthSizable | NSViewMaxYMargin)];
    iTermDecorationHeights decorationHeights = {.top = [self notchInset], .bottom = tabBarFrame.origin.y};
    decorationHeights.top += [self topBorderInset];
    if (_delegate.divisionViewShouldBeVisible) {
        decorationHeights.top += kDivisionViewHeight;
    }
    ...
    NSRect frame =
        NSMakeRect(tabBarFrame.origin.x, decorationHeights.bottom, tabBarFrame.size.width,
                   [thisWindow.contentView frame].size.height - decorationHeights.top - decorationHeights.bottom);
    [self layoutStatusBar:&decorationHeights window:thisWindow frame:frame];
    NSRect tabViewFrame =
        NSMakeRect(tabBarFrame.origin.x, decorationHeights.bottom, tabBarFrame.size.width,
                   [thisWindow.contentView frame].size.height - decorationHeights.top - decorationHeights.bottom);
    DLog(@"repositionWidgets - Set tab view frame to %@", NSStringFromRect(tabViewFrame));
    self.tabView.frame = [self tabViewFrameByShrinkingForFullScreenTabBar:tabViewFrame window:thisWindow];
    [self updateDivisionViewAndWindowNumberLabel];
}

- (NSRect)tabViewFrameByShrinkingForFullScreenTabBar:(NSRect)frame window:(NSWindow *)thisWindow {
    if (!thisWindow) {
        return frame;
    }
    ...
    if (![self.delegate enteringLionFullscreen] && !(thisWindow.styleMask & NSWindowStyleMaskFullScreen)) {
        return frame;
    }
    switch ([iTermPreferences intForKey:kPreferenceKeyTabPosition]) {
        case PSMTab_TopTab:
...
- (void)layoutSubviewsWithVisibleLeftTabBarAndInlineToolbelt:(BOOL)showToolbeltInline forWindow:(NSWindow *)thisWindow {
    // BUG-f1165: Replace assert() with guard - layout while on loan should be no-op, not crash
    if (_tabBarControlOnLoan) {
        DLog(@"WARNING: layoutSubviewsWithVisibleLeftTabBarAndInlineToolbelt called while tab bar on loan, ignoring");
        return;
    }
    [self setLeftTabBarWidthFromPreferredWidth];
    iTermDecorationHeights decorationHeights = {.top = [self notchInset], .bottom = 0};
    ...
    NSRect tabBarFrame =
        NSMakeRect([self leftBorderInset], decorationHeights.bottom, _leftTabBarWidth,
                   [thisWindow.contentView frame].size.height - decorationHeights.bottom - decorationHeights.top);
    self.tabBarControl.insets = [self.delegate tabBarInsets];
    [self setTabBarFrame:tabBarFrame];
    [self setTabBarControlAutoresizingMask:(NSViewHeightSizable | NSViewMaxXMargin)];
    ...
    const NSRect frame =
        NSMakeRect(NSMaxX(tabBarFrame) + xOffset, decorationHeights.bottom,
    ...
    NSRect leftTabBarDragHandleFrame = NSMakeRect(NSMaxX(self.tabBarControl.frame) - dragHandleWidth, 0,
                                                  dragHandleWidth, NSHeight(self.tabBarControl.frame));
...
- (CGFloat)minimumTabBarWidth {
    ...
    // BUG-13013: Replace assert(NO) with DLog and default return for unknown tab style
    DLog(@"Unknown tab style %d in minimumTabBarWidth, returning default 50", (int)preferredStyle);
    return 50;
}
...
- (NSRect)frameForStatusBarInContainingFrame:(NSRect)containingFrame {
    switch ([iTermPreferences unsignedIntegerForKey:kPreferenceKeyStatusBarPosition]) {
        case iTermStatusBarPositionTop:
            return NSMakeRect(NSMinX(containingFrame), NSMaxY(containingFrame) - iTermGetStatusBarHeight(),
                              NSWidth(containingFrame), iTermGetStatusBarHeight());

        case iTermStatusBarPositionBottom:
            return NSMakeRect(NSMinX(containingFrame), NSMinY(containingFrame), NSWidth(containingFrame),
...
- (CGFloat)dragHandleView:(iTermDragHandleView *)dragHandle didMoveBy:(CGFloat)delta {
    ...
    [[NSUserDefaults standardUserDefaults] setDouble:_leftTabBarPreferredWidth forKey:kPreferenceKeyLeftTabBarWidth];
...
- (BOOL)stoplightHotboxCanDrag {
    return ([self.delegate iTermTabBarCanDragWindow] && ![self.delegate iTermTabBarWindowIsFullScreen]);
}
