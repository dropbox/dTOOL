...
- (instancetype)init {
    ...
    if (self) {
        // Silent crash detection - shows non-modal icon in tab bar
        NSString *crashReport = [iTermCrashReportIconController detectLatestCrashReportSilently];
        if (crashReport) {
            [[iTermCrashReportIconController shared] showCrashIconWith:crashReport];
        }
...
- (void)arrangeTerminals:(NSArray *)terminals inFrame:(NSRect)frame {
    ...
    while ([terminalsCopy count] > 0) {
        ...
        // BUG-f916: Guard against nil terminal instead of crashing
        if (!terminal) {
            ELog(@"iTermController: No terminal found in arrangeHorizontally loop");
            continue;
        }
...
- (void)_addBookmark:(Profile*)bookmark
   ...
   alternateSelector:(SEL)alternateSelector {
    ...
    if (withShortcuts) {
        ...
        NSString* shortcut = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:bookmark];
...
- (void)_addBookmarksForTag:(NSString*)tag
    ...
    if (openAllSelector && count > 1) {
        ...
        if ([self respondsToSelector:openAllSelector]) {
            [aMenuItem setTarget:self];
        } else if ([aTarget respondsToSelector:openAllSelector]) {
            // BUG-f917: Guard instead of assert - target may not respond to selector
            [aMenuItem setTarget:aTarget];
        } else {
            ELog(@"iTermController: Neither self nor aTarget responds to openAllSelector");
        }
        ...
        if ([self respondsToSelector:openAllSelector]) {
            ...
            // BUG-f918: Guard instead of assert - target may not respond to selector
            [aMenuItem setTarget:aTarget];
        } else {
            ELog(@"iTermController: Neither self nor aTarget responds to openAllSelector for alternate");
        }
        [subMenu addItem:aMenuItem];
    }
}
    ...
    for (int i = 0; i < N; i++) {
        ...
        [bm.menuController addBookmark:b
                        ...
                        startingAtItem:startingAt
                              withTags:[NSArray castFrom:[b objectForKey:KEY_TAGS]]
                                params:params
                                 atPos:i
                            identifier:nil];
    }
...
- (void)commitAndPopCurrentRestorableSession {
    ...
    // BUG-f919: Guard against nil session instead of crashing
    if (!session) {
        ELog(@"iTermController: commitAndPopCurrentRestorableSession called with no current session");
        return;
    }
...
- (void)killRestorableSessions {
    ...
    // BUG-f920: Guard runJobsInServers check instead of crashing
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
        ELog(@"iTermController: killRestorableSessions called but runJobsInServers is NO");
        return;
    }
