...
struct ParsedSSHArguments: Codable, CustomDebugStringConvertible {
    ...
    init(_ string: String, booleanArgs boolArgsString: String, hostnameFinder: SSHHostnameFinder) {
        ...
        // BUG-1510: Log a warning if port argument is non-numeric instead of silently defaulting to 22
        if let portArg = paramArgs.value(for: .port), Int(portArg) == nil {
            DLog("Warning: Invalid port argument '\(portArg)' is not a valid integer, defaulting to port 22")
        }
        port = preferredPort ?? paramArgs.value(for: .port).flatMap { Int($0) } ?? 22
    }
}
