...
open class MultiCursorTextView: NSTextView {
    ...
    // BUG-1599: Safe accessor for layoutManager to avoid force unwrap crashes
    // Returns the layoutManager or logs an error and returns nil for graceful degradation
    private var safeLayoutManager: NSLayoutManager? {
        guard let lm = layoutManager else {
            DLog("ERROR: layoutManager is nil - text view not properly configured")
            return nil
        }
        return lm
    }

    // BUG-1669: Safe accessor for textStorage to avoid force unwrap crashes
    private var safeTextStorage: NSTextStorage? {
        guard let ts = textStorage else {
            DLog("ERROR: textStorage is nil - text view not properly configured")
            return nil
        }
        return ts
    }

    // BUG-1669: Safe accessor for textContainer to avoid force unwrap crashes
    private var safeTextContainer: NSTextContainer? {
        guard let tc = textContainer else {
            DLog("ERROR: textContainer is nil - text view not properly configured")
            return nil
        }
        return tc
    }
...
extension MultiCursorTextView {
    ...
    open override func mouseUp(with event: NSEvent) {
        if let drag = optionDrag {
            DLog("Finish drag at \(drag)")
            return
        }
...
extension MultiCursorTextView {
    ...
    private func glyphRange(atPoint point: NSPoint) -> NSRange? {
        // BUG-1669: Use guard instead of force unwrap for layout components
        guard let lm = layoutManager, let tc = textContainer else {
            return nil
        }
        let rect = NSRect(origin: point,
                          size: NSSize(width: 1, height: 1))
        let range = lm.glyphRange(forBoundingRect: rect, in: tc)
        if range.location == NSNotFound {
            return nil
        }
        let ranges = split(range, in: rect)
    ...
    private func convertCursor(at point: NSPoint, to kind: NSString.EnumerationOptions) {
        ...
        // BUG-1669: Use guard instead of force unwrap (even though it was checked above)
        guard var temp = _multiCursorSelectedRanges else { return }
        if kind == .byComposedCharacterSequences {
            ...
            // BUG-f551: Gracefully handle unexpected enumeration kind - use original range
            DLog("Unexpected enumeration kind in drag: \(kind.rawValue), using original range")
            temp[index] = originalRange
        }
    ...
    private func safelySetSelectedRanges(_ proposedRanges: [NSRange]) {
        ...
        for range in _multiCursorSelectedRanges ?? [] {
            let boundingRect = lm.boundingRect(forGlyphRange: range, in: tc)
    ...
    private func split(_ range: NSRange, in containingRect: NSRect) -> [NSRange] {
        ...
        let ranges = enumerateLines(in: range) { rect -> NSRange in
            ...
            // BUG-1771: Use nil coalescing with empty range fallback instead of force unwrap
            let value = self.range(for: effectiveRect) ?? NSRange()
    ...
    private func overflowingGlyphIndex(for point: NSPoint, in textContainer: NSTextContainer) -> Int {
        // BUG-1669: Use guard instead of force unwrap for layoutManager
        guard let lm = layoutManager else {
            return 0
        }
        let i = lm.glyphIndex(for: point, in: textContainer)
        if i + 1 < lm.numberOfGlyphs {
            return i
        }
        let bounds = lm.boundingRect(forGlyphRange: NSRange(location: i, length: 1), in: textContainer)
        if bounds.contains(point) {
            return i
        }
        return lm.numberOfGlyphs
    }
    ...
    private func moveLeft(by: NSString.EnumerationOptions) {
        // BUG-1669: Use guard instead of force unwrap for textStorage
    ...
    private func moveRight(by: NSString.EnumerationOptions) {
        // BUG-1669: Use guard instead of force unwrap for textStorage and layoutManager
        guard let storage = textStorage, let lm = layoutManager else { return }
        let numberOfCharacters = storage.string.utf16.count
        modifyRanges { glyphRange in
    ...
    private func underflowingLineFragmentRect(forGlyphAt glyphIndex: Int,
        ...
        let location: Int
        if glyphIndex >= lm.numberOfGlyphs && lm.numberOfGlyphs > 0 {
            location = lm.numberOfGlyphs - 1
        } else {
            location = glyphIndex
        }
        return lm.lineFragmentRect(forGlyphAt: location, effectiveRange: effectiveGlyphRange)
    }
    ...
    private func undoable<T>(force: Bool = false, _ closure: () throws -> T) rethrows -> T {
        ...
        // BUG-1624: Use it_assert instead of precondition - re-entrancy should be debuggable but not crash
        it_assert(!inUndoable)
        inUndoable = true
        defer {
            inUndoable = false
        }
    ...
    private func safelyInsert(_ string: String, at glyphIndex: Int) {
        ...
        if let ranges = _multiCursorSelectedRanges {
            let replacementRanges = ranges.map { range -> NSRange in
                ...
                // BUG-1624: Use it_assert instead of precondition - invariant violations should be debuggable
                it_assert(range.lowerBound < charIndex)
                it_assert(range.upperBound >= charIndex)
    ...
    private func extendLeft(_ range: NSRange, by: NSString.EnumerationOptions) -> NSRange {
        // BUG-1669: Use guard instead of force unwrap for textStorage
        guard let storage = textStorage else { return range }
        var result = range
        let nsstring = storage.string as NSString
    ...
    private func extendLineLeft(_ range: NSRange) -> NSRange {
        // BUG-1669: Use guard instead of force unwrap for layout components
        guard let lm = layoutManager, let tc = textContainer else { return range }
        let rect = underflowingLineFragmentRect(forGlyphAt: max(0, range.location - 1),
                                                effectiveRange: nil)
        let index = lm.glyphIndex(for: rect.origin, in: tc)
    ...
    private func extendLineRight(_ range: NSRange) -> NSRange {
        // BUG-1669: Use guard instead of force unwrap for layout components
        guard let lm = layoutManager, let tc = textContainer else { return range }
        let rect = lm.lineFragmentRect(forGlyphAt: range.location, effectiveRange: nil)
        let index = overflowingGlyphIndex(for: rect.maxPointWithinRect, in: tc)
    ...
    private func deleteRanges(closure: (Range<Int>) -> (Range<Int>)) {
        ...
        for originalRange in sortedRanges {
            ...
            if range.length > 0 {
                ...
                // BUG-1648: Use guard instead of force unwrap for textStorage and Range
                guard let storage = self.textStorage,
                      let swiftRange = Range(range) else {
                    continue
                }
                let count = (storage.string as NSString).length
                let proposed = NSRange(closure(swiftRange))
    ...
    private func didModifySubstringLength(originalCharacterRange: NSRange,
        ...
        // BUG-1624: Use it_assert instead of precondition - mismatched ranges should be debuggable
        it_assert(originalCharacterRange.location == newCharacterRange.location)
        ...
        let replacementCharRanges = originalRanges.compactMap { selectionGlyphRange -> NSRange? in
            let selectionCharRange = lm.characterRange(forGlyphRange: selectionGlyphRange, actualGlyphRange: nil)
        ...
        let replacementGlyphRanges = replacementCharRanges.map {
            return lm.glyphRange(forCharacterRange: $0, actualCharacterRange: nil)
        }.sorted { lhs, rhs in
            return lhs.location < rhs.location
        }.uniq
    ...
    private func modify(glyphRange: NSRange, closure: (String) -> String) -> NSRange {
        // BUG-1669: Use guard instead of force unwrap for layoutManager
        guard let lm = layoutManager else { return glyphRange }
        let characterRange = lm.characterRange(forGlyphRange: glyphRange, actualGlyphRange: nil)
        let newCharacterRange = modify(characterRange: characterRange, closure: closure)
        return lm.glyphRange(forCharacterRange: newCharacterRange, actualCharacterRange: nil)
    }
    ...
    private func modify(characterRange: NSRange, closure: (String) -> String) -> NSRange {
        // BUG-1669: Use guard instead of force unwrap for textStorage
        guard let storage = textStorage else { return characterRange }
        let nsstring = storage.string as NSString
    ...
    private func moveSelection(_ index: Int, to range: NSRange) {
        if index == 0 && _multiCursorSelectedRanges == nil {
            // BUG-1669: Use guard instead of force unwrap for layoutManager
            guard let lm = layoutManager else { return }
            let glyphRange = lm.characterRange(forGlyphRange: range, actualGlyphRange: nil)
            safelySetSelectedRanges([glyphRange])
            return
        }
        // BUG-1669: Use guard instead of force unwrap for _multiCursorSelectedRanges
        guard var temp = _multiCursorSelectedRanges else { return }
        temp[index] = range
    ...
    private func glyphIndexOnLineBelow(glyphIndex: Int) -> Int? {
        // BUG-1649: Use guard instead of force unwrap for layout components
        guard let rect = self.rect(for: NSRange(location: glyphIndex, length: 0)),
              let lm = layoutManager,
              let tc = textContainer else {
            return nil
        }
        let i = lm.glyphIndex(for: rect.neighborBelow, in: tc, fractionOfDistanceThroughGlyph: nil)
        let sanityCheckRect = lm.boundingRect(forGlyphRange: NSRange(location: i, length: 1), in: tc)
        if sanityCheckRect.minY == rect.minY {
            return nil
        }
        if sanityCheckRect.minX < rect.maxX && i + 1 == lm.numberOfGlyphs {
            return i + 1
        }
        return i
    }
    ...
    private func glyphIndexOnLineAbove(glyphIndex: Int) -> Int? {
        ...
        let i = lm.glyphIndex(for: rect.neighborAbove, in: tc, fractionOfDistanceThroughGlyph: nil)
        let sanityCheckRect = lm.boundingRect(forGlyphRange: NSRange(location: i, length: 1), in: tc)
        if sanityCheckRect.minY == rect.minY {
            return nil
        }
...
extension MultiCursorTextView {
    ...
    // DESIGN_LIMITATION: RTL text support is not implemented. moveBackward maps to moveLeft
    // which is correct for LTR text but incorrect for RTL languages like Arabic/Hebrew.
    ...
    func glyphRangeAbove(glyphRange: NSRange) -> NSRange? {
        ...
        // BUG-1669: Use guard instead of force unwrap for layout components
        guard let lm = layoutManager, let tc = textContainer else { return glyphRange }
        let glyphIndex = lm.glyphIndex(for: NSPoint(x: glyphRangeRect.minX,
    ...
    private func handleSpecialKeyDown(_ event: NSEvent) -> Bool {
        ...
        guard let lastExistingRange = existingRanges.last else {
            return false
        }
        let lastExistingCursorLocation = lastExistingRange.location
    ...
    func glyphRangeBelow(glyphRange: NSRange) -> NSRange? {
        ...
        // BUG-1669: Use guard instead of force unwrap for layout components
        guard let lm = layoutManager, let tc = textContainer else { return glyphRange }
        let limit = lm.boundingRect(forGlyphRange: NSMakeRange(0, lm.numberOfGlyphs),
                                    in: tc).maxY
    ...
    open override func pageDownAndModifySelection(_ sender: Any?) {
        ...
        modifyRanges { glyphRange in
            ...
            let index = overflowingGlyphIndex(for: nextPageRect.origin, in: tc)
    ...
    open override func pageUpAndModifySelection(_ sender: Any?) {
        ...
        modifyRanges { glyphRange in
            ...
            let index = overflowingGlyphIndex(for: previousPageRect.origin, in: tc)
...
extension MultiCursorTextView {
    open override func insertText(_ insertString: Any) {
        if hasMarkedText() {
            // BUG-1671: Use as? instead of as! for insertString cast
            guard let stringValue = insertString as? String else { return }
            let stringNSLength = (stringValue as NSString).length
            undoable(force: true) {
                let markedCharacterRange = markedRange()
                safelyReplaceCharacters(in: markedCharacterRange, with: stringValue)
                let maybeCursorsBeforeUnmarkedText = cursorsBeforeMarkedText
                unmarkText()
                if let cursors = maybeCursorsBeforeUnmarkedText,
                   let firstCursor = cursors.first {
                    _multiCursorSelectedRanges = cursors
                    // BUG-1669: Use guard instead of force unwrap for layoutManager
                    guard let lm = layoutManager else { return }
                    undoable {
                        let originalGlyphRange = lm.glyphRange(forCharacterRange: firstCursor,
                                                               actualCharacterRange: nil)
                        let replacementRange = NSRange(location: originalGlyphRange.location,
                                                       length: stringNSLength)
    ...
    private func insert(string insertString: Any, atGlyphRanges ranges: [NSRange]) {
        ...
        if let string = insertString as? String {
            ...
            // BUG-f552: Gracefully handle unexpected string type - return early
            DLog("Unexpected string type in insert: \(type(of: insertString))")
            return
        }
        ...
        let preCharacterRanges = ranges.map {
            lm.characterRange(forGlyphRange: $0, actualGlyphRange: nil)
        }.sorted { lhs, rhs in
            lhs.location < rhs.location
        }
        ...
        undoable {
            ...
            safelySetSelectedRanges(selectionCharacterRanges.map {
                lm.glyphRange(forCharacterRange: $0, actualCharacterRange: nil)
            })
        }
    }
    ...
    open override func transpose(_ sender: Any?) {
        ...
        undoable {
            for range in ranges.reversed() {
                ...
                let rightCharRange = lm.characterRange(forGlyphRange: NSRange(location: range.location, length: 1), actualGlyphRange: nil)
                let leftCharRange = lm.characterRange(forGlyphRange: NSRange(location: range.location - 1, length: 1), actualGlyphRange: nil)
                let rightString = (storage.string as NSString).substring(with: rightCharRange)
                let leftString = (storage.string as NSString).substring(with: leftCharRange)
        ...
        undoable {
            ...
            for oldGlyphIndex in startIndexes {
                ...
                for j in i..<numRanges {
                    ...
                    // BUG-1624: Use it_assert instead of precondition - selection invariants should be debuggable
                    it_assert(selection.length >= 0)
                    guard let intersection = deletedRange.intersection(selection) else {
                        ...
                        // BUG-1624: Use it_assert instead of precondition - selection bounds should be debuggable
                        it_assert(selection.lowerBound >= deletedRange.upperBound)
                    ...
                    // BUG-1624: Use it_assert instead of precondition - modified length should be debuggable
                    it_assert(modified.length >= 0)
...
extension MultiCursorTextView {
    ...
    open override func deleteWordForward(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendWordRight(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func deleteWordBackward(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendWordLeft(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func deleteToBeginningOfLine(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendLineLeft(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func deleteToEndOfLine(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendLineRight(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func deleteToBeginningOfParagraph(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendParagraphLeft(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func deleteToEndOfParagraph(_ sender: Any?) {
        ...
        undoable {
            deleteRanges { range in
                // BUG-1670: Use guard instead of force unwrap for Range conversion
                guard let result = Range(extendParagraphRight(NSRange(location: range.lowerBound, length: 0))) else {
                    return range
                }
                return result
            }
    ...
    open override func paste(_ sender: Any?) {
        ...
        undoable {
            ...
            if ranges.count < groups.count {
                ...
                var glyphIndexAboveLocationToInsert = lastRange.location
                // BUG-1669: Use guard instead of force unwrap for layout components
                guard let storage = textStorage, let lm = layoutManager else { return }
                for string in groups[ranges.count...] {
                    ...
                    if let index = glyphIndexOnLineBelow(glyphIndex: glyphIndexAboveLocationToInsert) {
                        ...
                        multiCursorReplaceCharacters(in: NSRange(location: (storage.string as NSString).length, length: 0),
                                                     with: "\n")
                        let index = lm.numberOfGlyphs
                        multiCursorReplaceCharacters(in: NSRange(location: (storage.string as NSString).length, length: 0),
                                                     with: string)
                        indexes.append((index, stringLength))
                        glyphIndexAboveLocationToInsert = lm.numberOfGlyphs
                    }
                }
                ...
                for (startGlyphIndex, numberOfCharacters) in indexes {
                    let startCharacterIndex = lm.characterIndexForGlyph(at: startGlyphIndex)
                    let glyphIndex = lm.glyphIndexForCharacter(at: startCharacterIndex + numberOfCharacters)
...
extension MultiCursorTextView {
    open override func cancelOperation(_ sender: Any?) {
        guard let ranges = _multiCursorSelectedRanges,
              let lastRange = ranges.last else {
            super.cancelOperation(sender)
            return
        }
        safelySetSelectedRanges([lastRange])
    }
}
