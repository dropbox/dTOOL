...
struct AIExplanationRequest: Codable {
    ...
    private static func content(snapshot: TerminalContentSnapshot,
        ...
        // BUG-1772: Use guard let instead of force unwrap for extractor initialization
        guard let extractor = LocatedStringSelectionExtractor(selection: selection,
                                                               snapshot: snapshot,
                                                               options: .trimWhitespace,
                                                               maxBytes: 0,
                                                               minimumLineNumber: 0) else {
            return iTermLocatedString(string: "", gridCoords: GridCoordArray())
        }
...
extension Result {
    init?<T, U>(_ or: iTermOr<T, U>) {
        ...
        or.whenFirst { value in
            // BUG-1558: Use conditional cast to avoid crash on type mismatch
            if let success = value as? Success {
                actual = .success(success)
            }
        } second: { value in
            // BUG-1558: Use conditional cast to avoid crash on type mismatch
            if let failure = value as? Failure {
                actual = .failure(failure)
            }
        }
        // BUG-490: Return nil with logging instead of crashing if types don't match
        guard let result = actual else {
            DLog("BUG-490: iTermOr type mismatch in Result initializer")
            return nil
        }
        self = result
    }
