...
- (BOOL)it_fontIsOnLigatureBlacklist {
    ...
    dispatch_once(&onceToken, ^{
        blacklist = [NSSet setWithArray:@[
            @"AndaleMono", @"Courier", @"LetterGothicStd", @"Monaco", @"OCRAStd", @"OratorStd", @"Osaka", @"PTMono",
            @"SFMono"
        ]];
    });
...
- (void)setFont:(NSFont *)font {
    // BUG-f963: Use guard instead of assert - nil font should return early with warning
    if (font == nil) {
        DLog(@"BUG-f963: setFont: called with nil font - ignoring");
        return;
    }
    font_ = font;
...
- (CGFloat)computedUnderlineOffset {
    ...
    NSDictionary *attributes =
        @{NSFontAttributeName : font_, NSUnderlineStyleAttributeName : @(NSUnderlineStyleSingle)};
    ...
    return [[layoutManager typesetter] baselineOffsetInLayoutManager:layoutManager glyphIndex:glyphIndex] / -2.0;
}
...
- (NSFont *)boldVersionOfFont:(NSFont *)font {
    ...
    const NSFontTraitMask kImmutableTraits =
        (NSItalicFontMask | NSNarrowFontMask | NSExpandedFontMask | NSCondensedFontMask | NSSmallCapsFontMask |
    ...
    NSMutableArray<NSFont *> *fonts = [NSMutableArray arrayWithCapacity:16]; // Font weight variants
