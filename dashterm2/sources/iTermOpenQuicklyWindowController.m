...
// this window.
// BUG-393: Add bounds check to prevent array index out of bounds crash
- (void)openSelectedRow {
    NSInteger row = [_table selectedRow];

    if (row >= 0) {
        if (row >= (NSInteger)self.model.items.count) {
            return;
        }
...
// BUG-10345: Add bounds check to prevent array index out of bounds crash
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    if (row < 0 || row >= (NSInteger)_model.items.count) {
        return nil;
    }
