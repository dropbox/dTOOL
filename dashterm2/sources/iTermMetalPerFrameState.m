...
// Metal rendering iterates over visible rows per frame. Row indices are typically 0-100.
// Caching 256 values covers all typical use cases with zero allocations during rendering.
static const int kMetalCachedRowKeyCount = 256;
static NSNumber *sMetalRowKeyCache[kMetalCachedRowKeyCount];

NS_INLINE NSNumber *iTermMetalRowKeyToNumber(int row) {
    if (row >= 0 && row < kMetalCachedRowKeyCount) {
        return sMetalRowKeyCache[row];
    }
    return @(row);
}

__attribute__((constructor)) static void iTermMetalPerFrameStateInitializeRowKeyCache(void) {
    for (int i = 0; i < kMetalCachedRowKeyCount; i++) {
        sMetalRowKeyCache[i] = @(i);
    }
}
...
NS_INLINE NSMutableArray<iTermMetalImageRun *> *
iTermEnsureImageRuns(NSMutableArray<iTermMetalImageRun *> *__autoreleasing _Nullable *storage) {
    if (!storage) {
        return nil;
    }
    NSMutableArray<iTermMetalImageRun *> *runs = *storage;
    if (!runs) {
        // Typical lines have 0-4 image runs; pre-allocate to avoid growth
...
NS_INLINE NSMutableArray<iTermKittyImageRun *> *
iTermEnsureKittyImageRuns(NSMutableArray<iTermKittyImageRun *> *__autoreleasing _Nullable *storage) {
    if (!storage) {
        return nil;
    }
    NSMutableArray<iTermKittyImageRun *> *runs = *storage;
    if (!runs) {
        // Typical lines have 0-4 kitty image runs; pre-allocate to avoid growth
        runs = [[NSMutableArray alloc] initWithCapacity:4];
        *storage = runs;
    }
    return runs;
...
@interface iTermMetalPerFrameState () <iTermAttributedStringBuilderDelegate> {
    ...
    NSDictionary<NSNumber *, NSIndexSet *>
        *_rowToAnnotationRanges; // Row on screen to characters with annotation underline on that row.
...
// Synthesize dirty tracking properties for protocol conformance
- (BOOL)allRowsDirty {
    return _allRowsDirty;
}

- (NSIndexSet *)dirtyRowIndices {
    return _dirtyRowIndices;
}
...
- (instancetype)initWithTextView:(PTYTextView *)textView
    ...
    // BUG-f1050: Replace assert with guard - calling from wrong thread should return nil, not crash
    if (![NSThread isMainThread]) {
        ELog(@"iTermMetalPerFrameState initWithTextView called from non-main thread");
        return nil;
    }
    self = [super init];
    if (self) {
        _configuration = [[iTermMetalPerFrameStateConfiguration alloc] init];
        // _rows is lazily allocated in loadLinesWithDrawingHelper: with known capacity
...
- (void)dealloc {
    // Note: Row pooling was evaluated but found to add overhead rather than save time.
    // See benchmarks/perframe_state_row_benchmark.m and comments in newRowForLine:.
    // Rows are simply released by ARC.
...
- (void)loadAllWithTextView:(PTYTextView *)textView
    ...
    [self loadImagesFromTextView:textView drawingHelper:drawingHelper];
...
- (void)loadImagesFromTextView:(PTYTextView *)textView drawingHelper:(iTermTextDrawingHelper *)drawingHelper {
    (void)textView;
    // PTYTextView already asked the data source for kitty draws when building its drawing helper,
    // so reuse that cached array instead of forcing KittyImageController to rebuild placements a
    // second time this frame.
    _kittyImageDraws = drawingHelper.kittyImageDraws;
}

- (void)loadSettingsWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper textView:(PTYTextView *)textView {
...
- (void)loadMetricsWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
    ...
    DLog(@"Visible range for document visible rect %@ is %@", NSStringFromRect(_documentVisibleRect),
         VT100GridCoordRangeDescription(_visibleRange));
...
- (void)loadLinesWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
                          textView:(PTYTextView *)textView
                            screen:(VT100Screen *)screen {
    // Allocate _rows with exact capacity now that we know the visible range
    const NSInteger numRows = _visibleRange.end.y - _visibleRange.start.y;
    _rows = [NSMutableArray arrayWithCapacity:numRows];

    // Query dirty line information from the screen for incremental rendering optimization.
    // The dataSource (VT100Screen) tracks which lines have changed via VT100Grid dirty flags.
    id<PTYTextViewDataSource> dataSource = textView.dataSource;
    if ([dataSource isAllDirty]) {
        // Entire screen is dirty (resize, scroll, first frame, etc.)
        _allRowsDirty = YES;
        _dirtyRowIndices = nil;
    } else {
        // Build index set of dirty screen-relative rows
        NSMutableIndexSet *dirtyIndices = [[NSMutableIndexSet alloc] init];
        const int scrollbackLines = [dataSource numberOfScrollbackLines];
        for (int i = _visibleRange.start.y; i < _visibleRange.end.y; i++) {
            // Convert from line buffer coordinate to grid coordinate
            const int gridY = i - scrollbackLines;
            if (gridY >= 0) {
                // Only grid lines (not scrollback) have dirty tracking
                VT100GridRange dirtyRange = [dataSource dirtyRangeForLine:gridY];
                if (dirtyRange.length > 0) {
                    // This row has at least one dirty character
                    [dirtyIndices addIndex:(NSUInteger)(i - _visibleRange.start.y)];
                }
            }
        }
        _allRowsDirty = (dirtyIndices.count == 0);  // If nothing dirty, treat as all dirty (first frame)
        _dirtyRowIndices = dirtyIndices.count > 0 ? [dirtyIndices copy] : nil;
    }

    iTermMetalPerFrameStateRowFactory *factory =
        [[iTermMetalPerFrameStateRowFactory alloc] initWithDrawingHelper:drawingHelper
                                                                textView:textView
                                                                  screen:screen
...
- (void)loadBadgeWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper textView:(PTYTextView *)textView {
    _badgeImage = drawingHelper.badgeImage;
    if (_badgeImage) {
        _badgeDestinationRect = [iTermTextDrawingHelper
            rectForBadgeImageOfSize:_badgeImage.size
               destinationFrameSize:textView.frame.size
                      sourceRectPtr:&_badgeSourceRect
                            margins:NSEdgeInsetsMake(drawingHelper.badgeTopMargin, 0, 0, drawingHelper.badgeRightMargin)
                     verticalOffset:0];
    }
}

- (void)loadBlinkingCursorWithTextView:(PTYTextView *)textView glue:(id<iTermMetalPerFrameStateDelegate>)glue {
    VT100GridCoord cursorScreenCoord =
        VT100GridCoordMake(textView.dataSource.cursorX - 1, textView.dataSource.cursorY - 1);
...
- (vector_float4)backgroundColorForCharacter:(screen_char_t)c selected:(BOOL)selected findMatch:(BOOL)findMatch {
    ...
    const vector_float4 unprocessedBackgroundColor =
        [self unprocessedColorForBackgroundColorKey:&backgroundKey isDefault:&isDefaultBackgroundColor];
    return [_configuration->_colorMap fastProcessedBackgroundColorForBackgroundColor:unprocessedBackgroundColor];
}
- (void)loadCursorInfoWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper textView:(PTYTextView *)textView {
    ...
    VT100GridCoord cursorCoord = [drawingHelper
        coordinateByTransformingScreenCoordinateForRTL:VT100GridCoordMake(textView.dataSource.cursorX - 1,
    ...
    const BOOL focused = ((_configuration->_isInKeyWindow && _configuration->_textViewIsActiveSession) ||
                          _configuration->_shouldDrawFilledInCursor);

    NSInteger lineWithCursor = textView.dataSource.cursorY - 1 + _numberOfScrollbackLines;
    if ([self shouldDrawCursor] && (!drawingHelper.hideCursorWhenUnfocused || focused) && _cursorVisible &&
        _visibleRange.start.y <= lineWithCursor && lineWithCursor < _visibleRange.end.y) {
        ...
        {
            // BUG-1610: Bounds check _cursorInfo.coord.y before accessing _rows
            if (_cursorInfo.coord.y < 0 || _cursorInfo.coord.y >= (NSInteger)_rows.count) {
                _cursorInfo.cursorVisible = NO;
                return;
            }
            const screen_char_t *const line = _rows[_cursorInfo.coord.y]->_screenCharLine.line;
            const screen_char_t screenChar = _cursorInfo.coord.x < _rows[_cursorInfo.coord.y]->_screenCharLine.length
                                                 ? line[_cursorInfo.coord.x]
                                                 : (screen_char_t){0};
            ...
            if (screenChar.code) {
                if (ScreenCharIsDWC_RIGHT(screenChar)) {
                    ...
                    _cursorInfo.doubleWidth =
                        (column < _configuration->_gridSize.width - 1) && ScreenCharIsDWC_RIGHT(line[column + 1]);
                }
            } else {
                _cursorInfo.doubleWidth = NO;
            }
            iTermMetalPerFrameStateRow *row = _rows[_cursorInfo.coord.y];
            const BOOL cursorSelected =
                row->_selectedIndexSet ? [row->_selectedIndexSet containsIndex:_cursorInfo.coord.x] : NO;
            _cursorInfo.backgroundColor = [self
                backgroundColorForCharacter:screenChar
            ...
            if (_cursorInfo.type == CURSOR_BOX) {
                ...
                if (!focused) {
                    ...
                    _cursorInfo.textColor = [self fastCursorColorForCharacter:screenChar wantBackground:YES muted:NO];
...
- (vector_float4)offscreenCommandLineOutlineColor {
    ...
    return simd_make_float4((float)_configuration->_offscreenCommandLineOutlineColor.redComponent * a,
                            ...
                            (float)_configuration->_offscreenCommandLineOutlineColor.blueComponent * a, a);
}
...
- (vector_float4)offscreenCommandLineBackgroundColor {
    ...
    return simd_make_float4((float)_configuration->_offscreenCommandLineBackgroundColor.redComponent * a,
                            ...
                            (float)_configuration->_offscreenCommandLineBackgroundColor.blueComponent * a, a);
}
...
- (void)loadAnnotationRangesFromTextView:(PTYTextView *)textView {
    const NSInteger firstVisibleRow = _visibleRange.start.y;
    const NSInteger lastVisibleRow = _visibleRange.end.y;

    NSArray<id<PTYAnnotationReading>> *annotations = [textView.dataSource annotationsInRange:_visibleRange];
    if (annotations.count == 0) {
        _rowToAnnotationRanges = nil;
        return;
    }

    NSMutableIndexSet *rowsToQuery = [NSMutableIndexSet indexSet];
    for (id<PTYAnnotationReading> annotation in annotations) {
        const VT100GridCoordRange range = [textView.dataSource coordRangeOfAnnotation:annotation];
        const NSInteger annotationStart = MAX(range.start.y, firstVisibleRow);
        const NSInteger annotationEnd = MIN(range.end.y, lastVisibleRow);
    ...
    NSMutableDictionary<NSNumber *, NSIndexSet *> *rowsWithAnnotations =
        [[NSMutableDictionary alloc] initWithCapacity:rowsToQuery.count];
    [rowsToQuery enumerateIndexesUsingBlock:^(NSUInteger absoluteRow, BOOL *_Nonnull stop) {
        NSArray<NSValue *> *ranges = [textView.dataSource charactersWithNotesOnLine:(int)absoluteRow];
        if (!ranges.count) {
            return;
        }
        ...
        for (NSValue *value in ranges) {
            const VT100GridRange gridRange = value.gridRangeValue;
            if (gridRange.length <= 0) {
                continue;
            }
            [indexes addIndexesInRange:NSMakeRange((NSUInteger)gridRange.location, (NSUInteger)gridRange.length)];
        }
        ...
        const NSInteger relativeRow = (NSInteger)absoluteRow - firstVisibleRow;
        rowsWithAnnotations[iTermMetalRowKeyToNumber((int)relativeRow)] = indexes;
    }];

    _rowToAnnotationRanges = rowsWithAnnotations.count ? rowsWithAnnotations : nil;
}
...
- (void)loadIndicatorsFromTextView:(PTYTextView *)textView drawingHelper:(iTermTextDrawingHelper *)drawingHelper {
    iTermIndicatorsHelper *indicatorsHelper = textView.indicatorsHelper;
    const NSInteger visibleIndicatorCount = indicatorsHelper.numberOfVisibleIndicators;
    if (visibleIndicatorCount == 0) {
        // Most frames display no indicators, so skip descriptor churn entirely.
        _indicators = nil;
        [indicatorsHelper didDraw];
        return;
    }

    _indicators = [[NSMutableArray alloc] initWithCapacity:(NSUInteger)visibleIndicatorCount];
    NSRect frame = drawingHelper.indicatorFrame;
    frame.origin.y -= MAX(0, textView.virtualOffset);

    [indicatorsHelper
        enumerateTopRightIndicatorsInFrame:frame
                                   andDraw:NO
                                     block:^(NSString *identifier, NSImage *image, NSRect rect, BOOL dark) {
                                         // BUG-f1051: Replace assert with guard - missing image should skip indicator,
                                         // not crash
                                         if (!image) {
                                             ELog(@"enumerateTopRightIndicatorsInFrame: indicator %@ has nil image - "
                                                  @"skipping",
                                                  identifier);
                                             return;
                                         }
    ...
    [indicatorsHelper enumerateCenterIndicatorsInFrame:frame
                                                 block:^(NSString *identifier, NSImage *image, NSRect rect,
                                                         CGFloat alpha, BOOL dark) {
                                                     // BUG-f1052: Replace assert with guard - missing image should skip
                                                     // indicator, not crash
                                                     if (!image) {
                                                         ELog(@"enumerateCenterIndicatorsInFrame: indicator %@ has nil "
                                                              @"image - skipping",
                                                              identifier);
                                                         return;
                                                     }
                                                     iTermIndicatorDescriptor *indicator =
                                                         [[iTermIndicatorDescriptor alloc] init];
                                                     indicator.identifier = identifier;
                                                     indicator.image = image;
                                                     indicator.frame = rect;
...
- (void)copyMarkedText:(NSString *)str
    ...
    StringToScreenChars(str, buf, fg, bg, &len, ambiguousIsDoubleWidth, &cursorIndex, NULL, normalization,
                        unicodeVersion, softAlternateScreen, NULL);
    ...
    for (int i = 0; i < len; i++) {
        if (coord.y >= 0 && coord.y < _rows.count) {
            ...
            const BOOL wouldSplit = (i + 1 < len && coord.x == gridWidth - 1 && ScreenCharIsDWC_RIGHT(buf[i + 1]) &&
                                     !buf[i + 1].complexChar);
            if (wouldSplit) {
                ...
                _rows[coord.y]->_screenCharLine = [sca screenCharArrayBySettingCharacterAtIndex:coord.x to:styled];
            }
        }
        justWrapped = NO;
        coord.x++;
...
- (NSColor *)timestampsTextColor {
    // BUG-f1053: Replace assert with guard - return default color if timestamps not enabled
    if (!_configuration->_timestampsEnabled) {
        ELog(@"timestampsTextColor called when timestamps not enabled");
        return [NSColor textColor]; // Return safe default
    }
    return [_configuration->_colorMap colorForKey:kColorMapForeground];
}

- (NSColor *)timestampsBackgroundColor {
    // BUG-f1054: Replace assert with guard - return default color if timestamps not enabled
    if (!_configuration->_timestampsEnabled) {
        ELog(@"timestampsBackgroundColor called when timestamps not enabled");
        return [NSColor textBackgroundColor]; // Return safe default
    }
    return [_configuration->_processedDefaultBackgroundColor
        colorUsingColorSpace:[NSImage colorSpaceForProgramaticallyGeneratedImages]];
}

- (void)enumerateIndicatorsInFrame:(NSRect)frame block:(void (^)(iTermIndicatorDescriptor *_Nonnull))block {
    [_indicators
        enumerateObjectsUsingBlock:^(iTermIndicatorDescriptor *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            block(obj);
        }];
}
...
- (vector_float4)processedDefaultBackgroundColor {
    ...
    return simd_make_float4((float)_configuration->_processedDefaultBackgroundColor.redComponent,
                            ...
                            (float)_configuration->_processedDefaultBackgroundColor.blueComponent, alpha);
}
...
- (BOOL)shouldDrawCursorGuideBelowText {
    return !_configuration->_useNativePowerlineGlyphs ||
           _configuration->_cursorGuideColor.alphaComponent > iTermCursorGuideAlphaThreshold;
}
...
- (vector_float4)processedDefaultTextColor {
    return simd_make_float4((float)_configuration->_processedDefaultTextColor.redComponent,
                            ...
                            (float)_configuration->_processedDefaultTextColor.blueComponent, 1.0);
}
...
- (vector_float4)blockHoverColor {
    return simd_make_float4((float)_configuration->_blockHoverColor.redComponent,
                            ...
                            (float)_configuration->_blockHoverColor.blueComponent, 1.0);
}
...
- (vector_float4)defaultTextColor {
    return simd_make_float4((float)_configuration->_defaultTextColor.redComponent,
                            ...
                            (float)_configuration->_defaultTextColor.blueComponent, 1.0);
}
...
- (VT100GridRect)selectedCommandRect {
    ...
    return VT100GridRectMake(0, minY, _configuration->_gridSize.width, MAX(0, maxY - minY));
}
...
NS_INLINE void iTermGlyphKeySetVisualPosition(iTermCachedGlyphKeysBuffer *buf, int gk, int logicalIndex,
                                              const int *bidiLUT, int bidiLUTLength) {
...
NS_INLINE int iTermGlyphKeyEmitPlaceholder(iTermCachedGlyphKeysBuffer *buf, int i, int logicalIndex, const int *bidiLUT,
                                           int bidiLUTLength) {
...
NS_INLINE int iTermGlyphKeyEmitRegular(iTermCachedGlyphKeysBuffer *buf, int i, int logicalIndex, int width,
                                       BOOL characterIsDrawable, const screen_char_t *line, BOOL isBoxDrawingCharacter,
                                       BOOL thinStrokes, const int *bidiLUT, int bidiLUTLength) {
    iTermCachedGlyphKeysBufferEnsureSize(buf, i);
    ...
    if (logicalIndex + 1 < width && line[logicalIndex + 1].complexChar &&
...
NS_INLINE int iTermGlyphKeyEmitDecomposedFromCheap(iTermCachedGlyphKeysBuffer *buf, BOOL bold, BOOL italic, int gk,
                                                   int logicalIndex, iTermCheapAttributedString *cheapString,
                                                   BOOL thinStrokes, const int *bidiLUT, int bidiLUTLength) {
    ...
    return iTermGlyphKeyEmitDecomposedFromGlyphs(buf, bold, italic, fakeBold, fakeItalic, gk, logicalIndex, font,
                                                 glyphs, cheapString.length, thinStrokes, bidiLUT, bidiLUTLength);
}

NS_INLINE int iTermGlyphKeyEmitDecomposedFromGlyphs(iTermCachedGlyphKeysBuffer *buf, BOOL bold, BOOL italic,
                                                    BOOL fakeBold, BOOL fakeItalic, int gk, int logicalIndex,
                                                    NSFont *font, CGGlyph *glyphs, NSUInteger length, BOOL thinStrokes,
                                                    const int *bidiLUT, int bidiLUTLength) {
    for (NSUInteger i = 0; i < length; i++) {
        iTermGlyphKeyEmitDecomposedForSingleGlyph(buf, bold, italic, fakeBold, fakeItalic, font.it_metalFontID,
                                                  glyphs[i], CGPointZero, gk + i, logicalIndex + i, thinStrokes,
                                                  bidiLUT, bidiLUTLength);
    }
    return gk + length;
}

NS_INLINE void iTermGlyphKeyEmitDecomposedForSingleGlyph(iTermCachedGlyphKeysBuffer *buf, BOOL bold, BOOL italic,
                                                         BOOL fakeBold, BOOL fakeItalic, int fontID, CGGlyph glyph,
                                                         NSPoint glyphPositionRelativeToCellOrigin, int gk,
                                                         CFIndex logicalIndex, BOOL thinStrokes, const int *bidiLUT,
                                                         int bidiLUTLength) {
    iTermCachedGlyphKeysBufferEnsureSize(buf, gk);
    iTermMetalGlyphKey *glyphKeys = buf->buffer;
...
NS_INLINE int iTermGlyphKeyEmitDecomposedFromNSAttributedString(iTermCachedGlyphKeysBuffer *buf, BOOL bold, BOOL italic,
                                                                BOOL fakeBold, BOOL fakeItalic, int gk,
                                                                int logicalIndex, NSAttributedString *attributedString,
                                                                BOOL thinStrokes, const CTVector(CGFloat) * positions,
                                                                const int *bidiLUT, int bidiLUTLength,
    ...
    if (lineRef == nil) {
        ...
        [cache addObjectWithKey:proxy value:(__bridge id)lineRef cost:1];
        CFRelease(lineRef);
    }

    iTermCoreTextLineRenderingHelper *helper =
    ...
    [helper enumerateGridAlignedRunsWithColumnPositions:CTVectorElementsFromIndex(positions, logicalIndex)
                                            alignToZero:YES
                                                closure:^(CTRunRef run, CTFontRef font, const CGGlyph *glyphs,
                                                    ...
                                                    for (int i = 0; i < length; i++) {
                                                        ...
                                                        const int sourceCell =
                                                            characterIndexToSourceCell
                                                                ? characterIndexToSourceCell[characterIndex]
                                                                : (logicalIndex + characterIndex);
                                                        iTermGlyphKeyEmitDecomposedForSingleGlyph(
                                                            buf, bold, italic, fakeBold, fakeItalic, fontID, glyphs[i],
                                                            positions[i], o + i, sourceCell, thinStrokes, bidiLUT,
                                                            bidiLUTLength);
                                                    }
                                                    o += length;
                                                }];
...
NS_INLINE int iTermGlyphKeyEmitDecomposed(iTermCachedGlyphKeysBuffer *buf, BOOL bold, BOOL italic, int gk,
                                          int logicalIndex, id<iTermAttributedString> attributedString,
                                          BOOL thinStrokes, const CTVector(CGFloat) * positions, const int *bidiLUT,
                                          int bidiLUTLength) {
    ...
    if (cheapString) {
        int result = iTermGlyphKeyEmitDecomposedFromCheap(buf, bold, italic, gk, logicalIndex, cheapString, thinStrokes,
                                                          bidiLUT, bidiLUTLength);
        if (result >= 0) {
            return result;
        }
        NSString *string = [NSString stringWithCharacters:cheapString.characters length:cheapString.length];
        nsAttributedString = [NSAttributedString attributedStringWithString:string attributes:cheapString.attributes];
    }
    if (!nsAttributedString) {
        nsAttributedString = [NSAttributedString castFrom:attributedString];
    }
    if (!nsAttributedString) {
        // BUG-420: Log and return instead of crashing when attributed string conversion fails
        // This should not happen but handle gracefully rather than crash
        DLog(@"iTermGlyphKeyEmitDecomposed: Failed to convert attributed string");
        return gk;
    }
    const BOOL fakeBold = [[nsAttributedString attribute:iTermFakeBoldAttribute atIndex:0
                                          effectiveRange:nil] boolValue];
    const BOOL fakeItalic = [[nsAttributedString attribute:iTermFakeItalicAttribute atIndex:0
                                            effectiveRange:nil] boolValue];
    NSData *data = [NSData castFrom:[nsAttributedString attribute:iTermSourceCellIndexAttribute
                                                          atIndex:0
                                                   effectiveRange:nil]];
    const int *characterIndexToSourceCell = (const int *)data.bytes;
    return iTermGlyphKeyEmitDecomposedFromNSAttributedString(buf, bold, italic, fakeBold, fakeItalic, gk, logicalIndex,
                                                             nsAttributedString, thinStrokes, positions, bidiLUT,
                                                             bidiLUTLength, characterIndexToSourceCell);
}

NS_INLINE void
iTermGlyphKeyEmitImage(const screen_char_t *const line, int logicalIndex, int row, iTermExternalAttribute *ea,
                       iTermKittyUnicodePlaceholderState *kittyPlaceholderStatePtr,
                       NSMutableArray<iTermKittyImageRun *> *__autoreleasing _Nullable *kittyImageRunsPtr,
                       NSArray<iTermKittyImageDraw *> *kittyImageDraws, int *previousImageCodePtr,
                       VT100GridCoord *previousImageCoordPtr,
                       NSMutableArray<iTermMetalImageRun *> *__autoreleasing _Nullable *imageRunsPtr) {
    NSMutableArray<iTermKittyImageRun *> *kittyImageRuns = kittyImageRunsPtr ? *kittyImageRunsPtr : nil;
    NSMutableArray<iTermMetalImageRun *> *imageRuns = imageRunsPtr ? *imageRunsPtr : nil;
    if (line[logicalIndex].virtualPlaceholder) {
        ...
        if (iTermDecodeKittyUnicodePlaceholder(&line[logicalIndex], ea, kittyPlaceholderStatePtr, &info)) {
            if (info.runLength > 1) {
                if (!kittyImageRuns) {
                    kittyImageRuns = iTermEnsureKittyImageRuns(kittyImageRunsPtr);
                }
                kittyImageRuns.lastObject.length += 1;
            } else {
                iTermKittyImageDraw *draw =
                    iTermFindKittyImageDrawForVirtualPlaceholder(kittyImageDraws, info.placementID, info.imageID);
                if (draw) {
                    iTermKittyImageRun *run =
                        [[iTermKittyImageRun alloc] initWithDraw:draw
                                                     sourceCoord:VT100GridCoordMake(info.column, info.row)
        ...
        if (line[logicalIndex].code == *previousImageCodePtr &&
            ...
            if (!imageRuns) {
                imageRuns = iTermEnsureImageRuns(imageRunsPtr);
            }
...
static int iTermGetMetalBackgroundColors(iTermMetalPerFrameState *self, const screen_char_t *const line,
                                         iTermMetalBackgroundColorRLE *backgroundRLE,
                                         iTermMetalGlyphAttributes *attributes,
                                         vector_float4 *unprocessedBackgroundColors, int width,
                                         NSIndexSet *selectedIndexes, NSData *findMatches,
                                         id<iTermColorMapReading> colorMap, iTermBidiDisplayInfo *bidiInfo) {
    ...
    for (int logicalX = 0; logicalX < width; logicalX++) {
        ...
        const BOOL selected = selectedIndexes ? [selectedIndexes containsIndex:visualX] : NO;
        BOOL findMatch = NO;
        ...
        if (logicalX > 0 && abs(visualX - prevVisualX) == 1 && backgroundKey.bgColor == lastBackgroundKey.bgColor &&
            backgroundKey.bgGreen == lastBackgroundKey.bgGreen && backgroundKey.bgBlue == lastBackgroundKey.bgBlue &&
            backgroundKey.bgColorMode == lastBackgroundKey.bgColorMode &&
            backgroundKey.selected == lastBackgroundKey.selected &&
            backgroundKey.isMatch == lastBackgroundKey.isMatch && backgroundKey.image == lastBackgroundKey.image) {
            // Extend RLE
...
static void iTermInitializeColorKey(BOOL findMatch, BOOL inUnderlinedRange, BOOL selected, BOOL isBlockCharacter,
                                    vector_float4 bgColor, const screen_char_t *characterPointer,
...
static BOOL iTermColorKeysEqual(const iTermTextColorKey *lhs, const iTermTextColorKey *rhs) {
    return (lhs->isMatch == rhs->isMatch && lhs->inUnderlinedRange == rhs->inUnderlinedRange &&
            lhs->selected == rhs->selected && lhs->foregroundColor == rhs->foregroundColor && lhs->mode == rhs->mode &&
            lhs->fgGreen == rhs->fgGreen && lhs->fgBlue == rhs->fgBlue && lhs->bold == rhs->bold &&
            lhs->faint == rhs->faint && simd_equal(lhs->background, rhs->background) && lhs->isBlock == rhs->isBlock);
}

static void iTermMetalSetUnderline(iTermMetalPerFrameState *self, BOOL annotated, BOOL inUnderlinedRange,
                                   BOOL underlineHyperlinks, const screen_char_t *const line,
                                   const iTermTextColorKey *currentColorKey, int logicalIndex, int visualX,
                                   iTermURL *url, iTermExternalAttribute *ea, iTermMetalGlyphAttributes *attributes) {
...
static int iTermEmitGlyphsAndSetAttributes(
    iTermMetalPerFrameState *self, const screen_char_t *const line, int row, int width,
    NSArray<id<iTermAttributedString>> *attributedStrings, CTVector(CGFloat) positions, iTermBidiDisplayInfo *bidiInfo,
    NSIndexSet *selectedIndexes, NSIndexSet *annotatedIndexes, NSData *findMatches, NSRange underlinedRange,
    iTermExternalAttributeIndex *eaIndex, iTermMetalPerFrameStateConfiguration *_configuration,
    NSMutableArray<iTermKittyImageRun *> *__autoreleasing _Nullable *kittyImageRunsPtr,
    NSArray<iTermKittyImageDraw *> *kittyImageDraws,
    NSMutableArray<iTermMetalImageRun *> *__autoreleasing _Nullable *imageRunsPtr,
    // out parameters:
    iTermCachedGlyphKeysBuffer *buf, iTermMetalGlyphAttributes *attributes, int *drawableGlyphsPtr) {
    const int *bidiLUT = [bidiInfo lut];
    const int bidiLUTLength = bidiInfo.numberOfCells;
    int asIndex = -1;
    int previousVisualX = -1;
    BOOL lastSelected = NO;
    NSCharacterSet *boxCharacterSet = [iTermBoxDrawingBezierCurveFactory
        boxDrawingCharactersWithBezierPathsIncludingPowerline:_configuration->_useNativePowerlineGlyphs];
    ...
    int nextAttributedStringLogicalStartIndex =
        attributedStrings.count > 0 ? [attributedStrings.firstObject sourceColumnRange].location : -1;
    ...
    for (int logicalIndex = 0; logicalIndex < width; logicalIndex++) {
        ...
        const BOOL characterIsDrawable = iTermTextDrawingHelperIsCharacterDrawable(
            &line[logicalIndex], logicalIndex > 0 ? &line[logicalIndex - 1] : NULL,
            line[logicalIndex].complexChar && (complexString != nil), _configuration->_blinkingItemsVisible,
            _configuration->_blinkAllowed, NO /* preferSpeedToFullLigatureSupport */, url != nil);
        const BOOL isBoxDrawingCharacter =
            (characterIsDrawable && ((!line[logicalIndex].complexChar && line[logicalIndex].code > 127 &&
        ...
        iTermInitializeColorKey(findMatch, inUnderlinedRange, selected, isBlockCharacter, bgColor, &line[logicalIndex],
                                currentColorKey);
        ...
        iTermMetalSetUnderline(self, annotated, inUnderlinedRange, underlineHyperlinks, line, currentColorKey,
                               logicalIndex, visualX, url, ea, attributes);
        ...
        if (line[logicalIndex].image) {
            iTermGlyphKeyEmitImage(line, logicalIndex, row, ea, &kittyPlaceholderState, kittyImageRunsPtr,
                                   kittyImageDraws, &previousImageCode, &previousImageCoord, imageRunsPtr);
            ...
            if (!haveEmittedAttributedString) {
                gk = iTermGlyphKeyEmitDecomposed(
                    buf, !!line[logicalIndex].bold, !!line[logicalIndex].italic, gk, logicalIndex, attributedString,
                    [self useThinStrokesWithAttributes:&attributes[visualX]], &positions, bidiLUT, bidiLUTLength);
                haveEmittedAttributedString = YES;
            }
        } else if (attributes[visualX].underlineStyle != iTermMetalGlyphAttributesUnderlineNone ||
                   characterIsDrawable) {
            lastDrawableGlyph = logicalIndex;
            gk = iTermGlyphKeyEmitRegular(
                buf, gk, logicalIndex, width, characterIsDrawable, line, isBoxDrawingCharacter,
                [self useThinStrokesWithAttributes:&attributes[visualX]], bidiLUT, bidiLUTLength);
        } else {
...
- (void)metalGetGlyphKeysData:(iTermGlyphKeyData *)glyphKeysData
                glyphKeyCount:(out NSUInteger *)glyphKeyCountPtr
                   attributes:(iTermMetalGlyphAttributes *)attributes
                    imageRuns:(NSMutableArray<iTermMetalImageRun *> *__autoreleasing _Nullable *)imageRunsPtr
               kittyImageRuns:(NSMutableArray<iTermKittyImageRun *> *__autoreleasing _Nullable *)kittyImageRunsPtr
    ...
    *belongsToBlockPtr = _rows[row]->_belongsToBlock;
    ...
    NSIndexSet *annotatedIndexes = _rowToAnnotationRanges[iTermMetalRowKeyToNumber(row)];
    ...
    *markStylePtr = _rows[row]->_markStyle;
    ...
    *lineStyleMarkRightInsetPtr = _rows[row]->_lineStyleMarkRightInset;
    ...
    int rles = iTermGetMetalBackgroundColors(self, line, backgroundRLE, attributes, unprocessedBackgroundColors, width,
                                             selectedIndexes, findMatches, _configuration->_colorMap, bidiInfo);
    *rleCount = rles;
    ...
    if (bidiInfo || _configuration->_ligaturesEnabled) {
        // Expect rles background color runs * ~4 attributed strings per run
        allAttributedStrings = [NSMutableArray arrayWithCapacity:rles * 4];
        ...
        for (int i = 0; i < rles; i++) {
            ...
            NSColor *bgColor = [NSColor colorWithDisplayP3Red:bgrle->color.x
                                                        green:bgrle->color.y
                                                         blue:bgrle->color.z
                                                        alpha:bgrle->color.w];
            // Compute selected status first to avoid reading uninitialized field in struct initializer
            const BOOL isSelected = [selectedIndexes containsIndex:bgrle->origin];
            iTermBackgroundColorRun run = {
                ...
                .selected = isSelected,
                .isMatch = (findMatches && !isSelected && CheckFindMatchAtIndex(findMatches, bgrle->origin)),
    ...
    NSMutableArray<iTermKittyImageRun *> *kittyImageRuns = nil;
    NSMutableArray<iTermMetalImageRun *> *imageRuns = nil;
    *glyphKeyCountPtr = iTermEmitGlyphsAndSetAttributes(
        self, line, row, width, allAttributedStrings, positions, bidiInfo, selectedIndexes, annotatedIndexes,
        findMatches, underlinedRange, eaIndex, _configuration, &kittyImageRuns, _kittyImageDraws, &imageRuns, &buf,
        attributes, drawableGlyphsPtr);

    if (kittyImageRunsPtr) {
        *kittyImageRunsPtr = kittyImageRuns;
    }
    if (imageRunsPtr) {
        *imageRunsPtr = imageRuns;
    }

    // Tweak the text color for the cell that has a box cursor.
    if (row == _cursorInfo.coord.y && _cursorInfo.type == CURSOR_BOX && _cursorInfo.cursorVisible &&
...
- (vector_float4)selectionColorForCurrentFocus {
    if (_configuration->_isFrontTextView) {
        return VectorForColor(
            [_configuration->_colorMap
                processedBackgroundColorForBackgroundColor:[_configuration->_colorMap colorForKey:kColorMapSelection]],
            _configuration->_colorSpace);
    } else {
...
- (vector_float4)unprocessedColorForBackgroundColorKey:(iTermBackgroundColorKey *)colorKey isDefault:(BOOL *)isDefault {
    ...
    *isDefault = NO;
    if (colorKey->selected) {
        ...
        color = VectorForColor([_configuration->_colorMap colorForKey:kColorMapMatch], _configuration->_colorSpace);
    } else {
        const BOOL defaultBackground =
            (colorKey->bgColor == ALTSEM_DEFAULT && colorKey->bgColorMode == ColorModeAlternate);
...
- (vector_float4)vectorColorForCode:(int)theIndex
    ...
    if (isBackground) {
        ...
        if (isFaint) {
            // Apply faint text alpha using premultiplied alpha (required for Metal blending).
            // Multiply RGB by alpha, then set alpha component.
            const float faintAlpha = _configuration->_colorMap.faintTextAlpha;
            color.x *= faintAlpha;
            color.y *= faintAlpha;
            color.z *= faintAlpha;
            color.w = faintAlpha;
...
- (iTermColorMapKey)colorMapKeyForCode:(int)theIndex
    ...
    switch (theMode) {
        ...
        case ColorModeNormal:
            ...
            // preference.
            if (isBold && _configuration->_brightenBold && (theIndex < 8) &&
...
- (id)metalASCIICreationIdentifierWithOffset:(CGSize)asciiOffset {
    return @{
        @"font" : _configuration->_fontTable.uniqueIdentifier ?: [NSNull null],
...
- (nullable NSDictionary<NSNumber *, iTermCharacterBitmap *> *)metalImagesForGlyphKey:(iTermMetalGlyphKey *)glyphKey
    ...
    switch (glyphKey->type) {
        case iTermMetalGlyphTypeRegular:
            return [self imagesForRegularGlyphKey:glyphKey asciiOffset:asciiOffset size:size scale:scale emoji:emoji];
        case iTermMetalGlyphTypeDecomposed:
            return [self imagesForDecomposedGlyphKey:glyphKey size:size scale:scale emoji:emoji];
    }
}

- (nullable NSDictionary<NSNumber *, iTermCharacterBitmap *> *)imagesForDecomposedGlyphKey:
    ...
    // Pre-allocate for typical part count - most characters have 1-3 parts, max 9 (3x3 grid)
    NSMutableDictionary<NSNumber *, iTermCharacterBitmap *> *result =
        [NSMutableDictionary dictionaryWithCapacity:characterSource.parts.count];
    [characterSource.parts
        enumerateObjectsUsingBlock:^(NSNumber *_Nonnull partNumber, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (nullable NSDictionary<NSNumber *, iTermCharacterBitmap *> *)imagesForRegularGlyphKey:(iTermMetalGlyphKey *)glyphKey
    ...
    // Pre-allocate for typical part count - ASCII uses 1-3 parts, non-ASCII up to parts.count
    NSUInteger capacity = isAscii ? 3 : characterSource.parts.count;
    NSMutableDictionary<NSNumber *, iTermCharacterBitmap *> *result =
        [NSMutableDictionary dictionaryWithCapacity:capacity];
    [characterSource.parts
        enumerateObjectsUsingBlock:^(NSNumber *_Nonnull partNumber, NSUInteger idx, BOOL *_Nonnull stop) {
            int part = partNumber.intValue;
            if (isAscii && part != iTermImagePartFromDeltas(0, 0) && part != iTermImagePartFromDeltas(-1, 0) &&
...
- (void)setDebugString:(NSString *)debugString {
    ...
    for (int i = 0, o = MAX(0, _configuration->_gridSize.width - (int)debugString.length);
         i < debugString.length && o < _configuration->_gridSize.width; i++, o++) {
...
- (CGRect)relativeFrame {
    return NSMakeRect(_relativeFrame.origin.x, 1 - _relativeFrame.size.height - _relativeFrame.origin.y,
                      _relativeFrame.size.width, _relativeFrame.size.height);
}
...
- (vector_float4)textColorForCharacter:(const screen_char_t *const)c
    ...
    const BOOL needsProcessing =
        (colorMap.minimumContrast > 0.001 || colorMap.dimmingAmount > 0.001 || colorMap.mutingAmount > 0.001 ||
    ...
    if (findMatch) {
        ...
        rawColor = VectorForColor(iTermTextDrawingHelperTextColorForMatch(bgColor), _configuration->_colorSpace);
        caches->havePreviousCharacterAttributes = NO;
    } else if (inUnderlinedRange) {
        // Blue link text.
        rawColor = VectorForColor([_configuration->_colorMap colorForKey:kColorMapLink], _configuration->_colorSpace);
        caches->havePreviousCharacterAttributes = NO;
    } else if (selected && _configuration->_useSelectedTextColor) {
        // Selected text.
        rawColor = VectorForColor([colorMap colorForKey:kColorMapSelectedText], _configuration->_colorSpace);
        caches->havePreviousCharacterAttributes = NO;
    } else if (_configuration->_reverseVideo &&
        ...
        // background color.
        rawColor = VectorForColor([colorMap colorForKey:kColorMapBackground], _configuration->_colorSpace);
        caches->havePreviousCharacterAttributes = NO;
               ...
               c->faint != caches->previousCharacterAttributes.faint || !caches->havePreviousForegroundColor) {
    ...
    if (needsProcessing) {
        result = VectorForColor([_configuration->_colorMap
                                    processedTextColorForTextColor:ColorForVector(rawColor)
...
- (iTermCursorNeighbors)cursorNeighbors {
    return [iTermSmartCursorColor
        neighborsForCursorAtCoord:_cursorInfo.coord
                         gridSize:_configuration->_gridSize
...
- (BOOL)shouldDrawCursor {
    ...
    BOOL result = (![self hasMarkedText] && _cursorVisible && !hideCursorBecauseBlinking);
...
- (BOOL)hideCursorBecauseBlinking {
    if (_cursorBlinking && _configuration->_isInKeyWindow && _configuration->_textViewIsActiveSession &&
        _configuration->_textViewIsFirstResponder && _timeSinceCursorMoved > 0.5) {
...
- (NSColor *)unprocessedColorForBackgroundRun:(const iTermBackgroundColorRun *)run enableBlending:(BOOL)enableBlending {
    iTermBackgroundColorKey backgroundKey = {.bgColor = run->bgColor,
                                             .bgGreen = run->bgGreen,
                                             .bgBlue = run->bgBlue,
...
- (NSColor *)colorForCode:(int)theIndex
    ...
    vector_float4 color = [self vectorColorForCode:theIndex
                                         ...
                                         colorMode:theMode
                                             ...
                                             faint:isFaint
                                      isBackground:isBackground];
    return [NSColor colorWithDisplayP3Red:color.x green:color.y blue:color.z alpha:color.w];
}
@end
