...
struct SecureUserDefaultValue<T: SecureUserDefaultStringTranscodable> {
    ...
    var encodedString: String {
        // BUG-1621: Use nil coalescing instead of force unwrap for data encoding
        guard let data = value.sudString.data(using: .utf8) else { return "" }
...
// BUG-1004: Converted from struct to class for thread safety.
// Struct with static var and lazy properties had data races:
// 1. Accessing lazy vars through static var mutates a copy (value semantics)
// 2. Multiple threads could race initializing lazy properties
// 3. hostToOpenURLSUDs dictionary not synchronized
class SecureUserDefaults {
    static let instance = SecureUserDefaults()
    private let lock = NSLock()

    func serializeAll() -> [String: String] {
        lock.lock()
        defer { lock.unlock() }
    ...
    func deserializeAll(dict: [String: String]) {
        lock.lock()
        defer { lock.unlock() }
        for serializable in serializables() {
    ...
    // These are now regular let properties (initialized once, safely)
    let allowPaste = SecureUserDefault<Bool>("AllowPaste", defaultValue: false)
    let requireAuthToOpenPasswordmanager = SecureUserDefault<Bool>("RequireAuthenticationToOpenPasswordManager", defaultValue: true)
    let enableSecureKeyboardEntryAutomatically = SecureUserDefault<Bool>("EnableSecureKeyboardEntryAutomatically", defaultValue: true)
    let enableAI = SecureUserDefault<Bool>("EnableAI", defaultValue: false)
    let browserBundleID = SecureUserDefault<String>("BrowserBundleID", defaultValue: "")
    let aiCompletionsEnabled = SecureUserDefault<Bool>("AICompletions", defaultValue: false)

    private var hostToOpenURLSUDs = [String: SecureUserDefault<Bool>]()
    func openURL(host: String) -> SecureUserDefault<Bool> {
        lock.lock()
        defer { lock.unlock() }
    ...
    private func serializables() -> [any SerializableUserDefault] {
        [allowPaste,
...
fileprivate let FallbackFolder = "/usr/local/DashTerm2-secure-settings"
...
class SecureUserDefault<T: SecureUserDefaultStringTranscodable & Codable & Equatable> {
    ...
    private static func fallbackBaseDirectory(create: Bool) throws -> String {
        ...
        " with prompt "DashTerm2 needs to create \(path) to store secure settings because your home directory is on a network file system." with administrator privileges
        """
        DLog("Will execute:\n\(code)")
    ...
    private static func path(_ key: String,
        ...
        // BUG-1622: Use guard instead of precondition - invalid key should throw
        guard key.rangeOfCharacter(from: CharacterSet.alphanumerics.inverted) == nil else {
            throw iTermError("SecureUserDefaults key '\(key)' contains non-alphanumeric characters")
        }
    ...
    private static func magic(_ key: String, filePath: URL) -> String {
        // BUG-1621: Use guard instead of force unwrap for data encoding
        guard let pathData = filePath.path.data(using: .utf8) as NSData?,
              let keyData = key.data(using: .utf8) as NSData? else {
            return ""
        }
    ...
    private static func store<U: SecureUserDefaultStringTranscodable>(_ key: String, value: U) throws {
        ...
        do shell script "
            ...
            printf '%s\\n%s' '\(magic)' '\(encodedValue)' > \\"$TF\\" && \
                ...
                mv \\"$TF\\" \\"\(path)\\" || \
                rm -f \\"$TF\\"
        " with prompt "DashTerm2 needs to modify a secure setting." with administrator privileges
        """
        let script = NSAppleScript(source: code)
        var error: NSDictionary? = nil
