...
- (instancetype)initWithPath:(NSString *)path {
    ...
    if (self) {
        ...
        _thread = [iTermThread withLabel:@"com.dashterm.dashterm2.multi-client"
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p %@>", NSStringFromClass([self class]), self, _socketPath];
}
...
- (void)attachToOrLaunchNewDaemonWithCallback:(iTermCallback<id, NSNumber *> *)callback {
    [_thread dispatchAsync:^(iTermFileDescriptorMultiClientState *_Nonnull state) {
        [self attachToOrLaunchNewDaemonServerWithState:state callback:callback];
    }];
}
...
- (void)attachWithCallback:(iTermCallback<id, NSNumber *> *)callback {
    ...
    [_thread dispatchAsync:^(iTermFileDescriptorMultiClientState *state) {
        [self tryAttachWithState:state
                        callback:[thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *_Nonnull state,
...
- (void)waitForChild:(iTermFileDescriptorMultiClientChild *)child
    ...
    [_thread dispatchAsync:^(iTermFileDescriptorMultiClientState *_Nullable state) {
        [self waitForChild:child removePreemptively:removePreemptively state:state callback:callback];
    }];
}
...
- (void)didAttachWithState:(iTermFileDescriptorMultiClientState *)state
    ...
    switch (status) {
        ...
        case iTermFileDescriptorMultiClientAttachStatusFatalError:
            ...
            // BUG-f963: Guard readFD state instead of assert - race conditions may leave it open
            if (state.readFD >= 0) {
                ELog(@"BUG-f963: readFD unexpectedly open (%d) after fatal error", state.readFD);
                int fd = state.readFD;
                state.readFD = -1;
                close(fd);
            }
...
- (void)tryAttachWithState:(iTermFileDescriptorMultiClientState *)state
    ...
    [state check];
    // BUG-f964: Guard readFD state instead of assert - close stale fd if needed
    if (state.readFD >= 0) {
        ELog(@"BUG-f964: readFD unexpectedly open (%d) before attach, closing", state.readFD);
        int fd = state.readFD;
        state.readFD = -1;
        close(fd);
    }
    ...
    iTermFileDescriptorMultiClientAttachStatus status =
        iTermConnectToUnixDomainSocket(_socketPath, &temp, 0 /* async */);
    state.readFD = temp;
    if (status != iTermFileDescriptorMultiClientAttachStatusSuccess) {
    ...
    [self readMessageWithState:state
                      callback:[_thread newCallbackWithBlock:^(
                          ...
                          __block BOOL ok = NO;
                          [result
                              handleObject:^(iTermClientServerProtocolMessageBox *_Nonnull messageBox) {
                                  if (messageBox.message.fileDescriptor) {
                                      ...
                                      DLog(@"tryAttachWithState(%@): Failed to get a file descriptor",
                                           strongSelf->_socketPath);
                                  }
                              }
                              error:^(NSError *_Nonnull error) {
                                  DLog(@"tryAttachWithState(%@): Error reading file descriptor: %@",
                                       strongSelf->_socketPath, error);
                                  ok = NO;
                              }];
                          if (!ok) {
                              int fd = state.readFD;
...
- (void)handshakeWithState:(iTermFileDescriptorMultiClientState *)state
    ...
    // BUG-f965: Guard FD state instead of assert - fail gracefully if not connected
    if (state.readFD < 0 || state.writeFD < 0) {
        ELog(@"BUG-f965: Cannot handshake - readFD=%d writeFD=%d", state.readFD, state.writeFD);
        [callback invokeWithObject:@NO];
        return;
    }
    ...
    iTermCallback<id, NSNumber *> *innerCallback =
        [_thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *state, NSNumber *handshakeOK) {
    ...
    [self sendHandshakeRequestWithState:state
                               callback:[_thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *state,
                                                                        NSNumber *sendResult) {
                                   [weakSelf
                                       didSendHandshakeRequestWithSuccess:sendResult.boolValue
                                                                    state:state
...
- (void)sendHandshakeRequestWithState:(iTermFileDescriptorMultiClientState *)state
    ...
    iTermMultiServerClientOriginatedMessage message = {
        ...
        .payload = {.handshake = {.maximumProtocolVersion = iTermMultiServerProtocolVersion2}}};
...
- (void)didSendHandshakeRequestWithSuccess:(BOOL)sendOK
                                  ...
                                  callback:(iTermCallback<id, NSNumber *> *)callback
                       childDiscoveryBlock:
                           (void (^)(iTermFileDescriptorMultiClientState *state, iTermMultiServerReportChild *))block {
    if (!sendOK) {
    ...
    [self readHandshakeResponseWithState:state
                              completion:^(iTermFileDescriptorMultiClientState *state, BOOL ok, int numberOfChildren,
                                           pid_t pid) {
                                  if (!ok) {
                                      [callback invokeWithObject:@NO];
                                      return;
                                  ...
                                  [self readInitialChildReports:numberOfChildren
                                                     ...
                                                     completion:^(iTermFileDescriptorMultiClientState *state, BOOL ok) {
                                                         DLog(@"Done reading child reports for %@ with ok=%@",
                                                              socketPath, @(ok));
                                                         if (!ok) {
                                                             [callback invokeWithObject:@NO];
                                                             return;
                                                         ...
                                                         // Ensure the completion callback runs before the read loop
...
- (void)readHandshakeResponseWithState:(iTermFileDescriptorMultiClientState *)state
                            completion:(void (^)(iTermFileDescriptorMultiClientState *state, BOOL ok,
                                                 int numberOfChildren, pid_t pid))completion {
    NSString *socketPath = _socketPath;
    ...
    [self readMessageWithState:state
                      callback:[_thread newCallbackWithBlock:^(
                          ...
                          [result
                              handleObject:^(iTermClientServerProtocolMessageBox *_Nonnull boxedMessage) {
                                  ...
                                  if (boxedMessage.decoded->payload.handshake.protocolVersion !=
                                      iTermMultiServerProtocolVersion2) {
                                      completion(state, NO, 0, -1);
                                      return;
                                  }
                                  completion(state, YES, boxedMessage.decoded->payload.handshake.numChildren,
...
- (void)readInitialChildReports:(int)numberOfChildren
                          state:(iTermFileDescriptorMultiClientState *)state
                          block:(void (^)(iTermFileDescriptorMultiClientState *state,
    ...
    [self readMessageWithState:state
                      callback:[_thread newCallbackWithBlock:^(
                                            ...
                                            iTermResult<iTermClientServerProtocolMessageBox *> *result) {
                          [result
                              handleObject:^(iTermClientServerProtocolMessageBox *_Nonnull box) {
                                  __strong __typeof(self) strongSelf = weakSelf;
                                  ...
                                  if (!foundLast) {
                                      // BUG-f966: Guard numberOfChildren instead of assert - protocol mismatch should
                                      // not crash
                                      if (numberOfChildren <= 0) {
                                          ELog(@"BUG-f966: numberOfChildren=%d but more children expected",
                                               numberOfChildren);
                                          completion(state, NO);
                                          return;
                                      }
                                      [strongSelf->_thread
                                          dispatchAsync:^(iTermFileDescriptorMultiClientState *_Nullable state) {
...
- (void)didDiscoverChild:(iTermMultiServerReportChild *)report state:(iTermFileDescriptorMultiClientState *)state {
    iTermFileDescriptorMultiClientChild *child =
        [[iTermFileDescriptorMultiClientChild alloc] initWithReport:report thread:self->_thread];
...
- (void)launchChildWithExecutablePath:(const char *)path
    ...
    [self send:&message
           state:state
        callback:[_thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *state, NSNumber *result) {
            DLog(@"called back");
...
static os_unfair_lock gMultiServerLogLock = OS_UNFAIR_LOCK_INIT;

static void iTermMultiServerStringForMessageFromClientLogger(const char *file, int line, const char *func,
                                                             const char *format, ...) {
    va_list args;
...
static NSString *iTermMultiServerStringForMessageFromClient(iTermMultiServerClientOriginatedMessage *message) {
    ...
    dispatch_once(&onceToken, ^{
        // Pre-allocate for typical number of log lines (usually 4-16)
        gCurrentMultiServerLogLineStorage = [NSMutableArray arrayWithCapacity:8];
    });
    os_unfair_lock_lock(&gMultiServerLogLock);
    ...
    os_unfair_lock_unlock(&gMultiServerLogLock);
    return result;
}
...
- (iTermMultiServerClientOriginatedMessage)copyLaunchRequest:(iTermMultiServerClientOriginatedMessage)original {
    ...
    iTermMultiServerClientOriginatedMessage messageCopy;
    {
        ...
        if (status) {
            ...
            NSData *data = [NSData dataWithBytes:temp.ioVectors[0].iov_base length:temp.ioVectors[0].iov_len];
            NSString *description = iTermMultiServerStringForMessageFromClient(&messageCopy);
            ITAssertWithMessage(status == 0, @"On decode: status is %@ for %@ based on %@", @(status),
                                [data debugDescription], description);
        }
    }
...
- (void)handleLaunch:(iTermMultiServerResponseLaunch)launch state:(iTermFileDescriptorMultiClientState *)state {
    ...
    // Happy path
    iTermMultiServerReportChild fakeReport = {.isLast = 0,
                                              .pid = launch.pid,
...
- (void)waitForChild:(iTermFileDescriptorMultiClientChild *)child
    ...
    // BUG-f967: Guard haveWaited state instead of assert - race condition may cause double wait
    if (child.haveWaited) {
        ELog(@"BUG-f967: child.haveWaited became true during wait setup");
        [callback invokeWithObject:[iTermResult withError:[self waitError:2]]];
        return;
    }
    iTermMultiServerClientOriginatedMessage message = {
        .type = iTermMultiServerRPCTypeWait,
        .payload = {.wait = {.pid = child.pid, .removePreemptively = removePreemptively}}};
    ...
    iTermCallback<id, NSNumber *> *sendCallback =
        [_thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *state, NSNumber *value) {
            [weakSelf didWriteWaitRequestWithStatus:value.boolValue child:child state:state];
        }];
...
- (void)handleWait:(iTermMultiServerResponseWait)wait state:(iTermFileDescriptorMultiClientState *)state {
    DLog(@"handleWait for socket %@ pid %@ with error %@, status %@", _socketPath, @(wait.pid), @(wait.resultType),
         @(wait.status));
...
- (void)launchNewDaemonAndHandshakeWithState:(iTermFileDescriptorMultiClientState *)state
                                    callback:(iTermCallback<id, NSNumber *> *)callback {
    // BUG-f968: Guard FD state instead of assert - clean up stale FDs if needed
    if (state.readFD >= 0) {
        ELog(@"BUG-f968: readFD unexpectedly open (%d) before daemon launch", state.readFD);
        int fd = state.readFD;
        state.readFD = -1;
        close(fd);
    }
    if (state.writeFD >= 0) {
        ELog(@"BUG-f968: writeFD unexpectedly open (%d) before daemon launch", state.writeFD);
        int fd = state.writeFD;
        state.writeFD = -1;
        close(fd);
    }
    ...
    if (![self launchNewDaemonWithState:state]) {
        // BUG-f969: Guard FD state after failed launch - clean up if needed
        if (state.readFD >= 0) {
            ELog(@"BUG-f969: readFD open (%d) after failed launch", state.readFD);
            int fd = state.readFD;
            state.readFD = -1;
            close(fd);
        }
        if (state.writeFD >= 0) {
            ELog(@"BUG-f969: writeFD open (%d) after failed launch", state.writeFD);
            int fd = state.writeFD;
            state.writeFD = -1;
            close(fd);
        }
...
- (NSArray<NSString *> *)serverFolders {
    ...
    // Pre-allocate for 2 server folders (app support + dotdir)
...
- (void)showError:(NSError *)error message:(NSString *)message badURL:(NSURL *)url {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            rateLimit = [[iTermRateLimitedUpdate alloc] initWithName:@"Multi client error" minimumInterval:2];
        });
        [rateLimit performRateLimitedBlock:^{
            DLog(@"Called");
            NSAlert *alert = [[NSAlert alloc] init];
            alert.messageText = @"Problem Starting DashTerm2 Daemon";
...
- (NSString *)serverPathCopyingIfNeeded {
    ...
    if (!desiredPath) {
        [self showError:nil
                message:[NSString
                            stringWithFormat:@"Neither ~/Library/Application Support/DashTerm2 nor ~/.dashterm2 are "
                                             @"writable directories. This prevents the session restoration server from "
                                             @"running. Please correct the problem and restart DashTerm2."]
                 badURL:nil];
        return nil;
    }
    NSFileManager *fileManager = [NSFileManager defaultManager];
    ...
    if ([self shouldCopyServerTo:desiredPath]) {
        ...
        if (!sourcePath || ![fileManager fileExistsAtPath:sourcePath]) {
            void (^showAlertAndTerminate)(void) = ^{
                NSAlert *alert = [[NSAlert alloc] init];
                alert.messageText = @"Required File Missing";
                alert.informativeText =
                    @"The iTermServer executable is missing from the application bundle. This indicates DashTerm2 is "
                    @"corrupted or incomplete. Please reinstall DashTerm2 from the official website.";
            ...
            // Avoid deadlocks by never synchronously blocking on the main queue.
            if ([NSThread isMainThread]) {
                showAlertAndTerminate();
            } else {
                dispatch_async(dispatch_get_main_queue(), showAlertAndTerminate);
            }
            return nil;
        }

        NSError *error = nil;
        [fileManager copyItemAtPath:sourcePath toPath:desiredPath error:&error];
        if (error) {
...
- (BOOL)launchNewDaemonWithState:(iTermFileDescriptorMultiClientState *)state {
    // BUG-f970: Guard readFD state instead of assert - clean up stale FD if needed
    if (state.readFD >= 0) {
        ELog(@"BUG-f970: readFD unexpectedly open (%d) before daemon launch", state.readFD);
        int fd = state.readFD;
        state.readFD = -1;
        close(fd);
    }
    ...
    state.daemonProcessSource =
        dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, forkState.pid, DISPATCH_PROC_EXIT, state.queue);
    ...
    // BUG-f971: Guard FD state instead of assert - fork may fail to return valid FDs
    if (readFD < 0 || writeFD < 0) {
        ELog(@"BUG-f971: Invalid FDs after fork - readFD=%d writeFD=%d", readFD, writeFD);
        if (readFD >= 0) {
            close(readFD);
        }
        if (writeFD >= 0) {
...
- (void)closeWithState:(iTermFileDescriptorMultiClientState *)state {
    ...
    NSDictionary<NSNumber *, iTermFileDescriptorMultiClientPendingLaunch *> *pendingLaunches =
        [state.pendingLaunches copy];
    [state.pendingLaunches removeAllObjects];
    [pendingLaunches enumerateKeysAndObjectsUsingBlock:^(
                         NSNumber *_Nonnull uniqueID,
                         iTermFileDescriptorMultiClientPendingLaunch *_Nonnull pendingLaunch, BOOL *_Nonnull stop) {
        [pendingLaunch cancelWithError:[self connectionLostError]];
    }];

    [state.children enumerateObjectsUsingBlock:^(iTermFileDescriptorMultiClientChild *_Nonnull child, NSUInteger idx,
                                                 BOOL *_Nonnull stop) {
...
- (void)readAndDispatchNextMessageWithState:(iTermFileDescriptorMultiClientState *)state {
    ...
    [self readMessageWithState:state
                      callback:[_thread newCallbackWithBlock:^(
                          ...
                          __block BOOL ok = YES;
                          [result
                              handleObject:^(iTermClientServerProtocolMessageBox *_Nonnull object) {
...
- (BOOL)dispatch:(iTermClientServerProtocolMessageBox *)box state:(iTermFileDescriptorMultiClientState *)state {
    ...
    switch (decoded->type) {
        ...
        case iTermMultiServerRPCTypeHello:
            // RC-026: Hello is idempotent - server may re-send it after recovery.
            // Safe to ignore since it carries no payload and just indicates readiness.
            DLog(@"Ignoring unexpected Hello message from %@ - server may have recovered", _socketPath);
            break;

        case iTermMultiServerRPCTypeReportChild:
            // RC-026: Late child report - server may report children after initial enumeration
            // if there was a timing issue or recovery. Handle gracefully by discovering the child.
            DLog(@"Handling late ReportChild from %@ - treating as orphan discovery", _socketPath);
            [self didDiscoverChild:&decoded->payload.reportChild state:state];
            break;

        case iTermMultiServerRPCTypeHandshake:
            // RC-026: Handshake response without a request is a protocol violation.
            // Log it but don't close - the connection may still be usable.
            DLog(@"WARNING: Unexpected Handshake response from %@ - protocol desync, ignoring", _socketPath);
            break;
    }
    return YES;
}
...
- (void)removeChild:(iTermFileDescriptorMultiClientChild *)child state:(iTermFileDescriptorMultiClientState *)state {
    if (!child) {
        return;
    }
...
- (iTermFileDescriptorMultiClientChild *)childWithPID:(pid_t)pid state:(iTermFileDescriptorMultiClientState *)state {
    return [state.children
        objectPassingTest:^BOOL(iTermFileDescriptorMultiClientChild *element, NSUInteger index, BOOL *stop) {
    ...
    [self
        readWithState:state
             ...
             callback:[_thread newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *_Nonnull state,
                 ...
                 [result
                     handleObject:^(iTermMultiServerMessage *_Nonnull lengthMessage) {
                         __strong __typeof(self) strongSelf = weakSelf;
                         if (!strongSelf) {
                         ...
                         // BUG-f972: Guard data length instead of assert - protocol error should not crash
                         if (lengthMessage.data.length != sizeof(size_t)) {
                             ELog(@"BUG-f972: Expected %zu bytes for length, got %lu", sizeof(size_t),
                                  (unsigned long)lengthMessage.data.length);
                         ...
                         [weakSelf
                             readWithState:state
                                    length:length
                                  callback:[strongSelf->_thread newCallbackWithBlock:^(
                                                                    iTermFileDescriptorMultiClientState *_Nonnull state,
                                                                    iTermResult<iTermMultiServerMessage *>
                                                                        *_Nullable result) {
                                      [result
                                          handleObject:^(iTermMultiServerMessage *_Nonnull payload) {
                                                      ...
                                                      invokeWithObject:
                                                          [iTermResult
                                                              withError:
                                                                  [NSError
                                                                      errorWithDomain:
                                                                          iTermFileDescriptorMultiClientErrorDomain
                                                                                 code:
                                              ...
                                              iTermClientServerProtocolMessageBox *encodedBox =
                                                  [iTermClientServerProtocolMessageBox withMessage:payload];

                                              if (!encodedBox) {
                                                  DLog(@"Decoding: recv failed for %@", strongSelf->_socketPath);
                                                  [callback invokeWithObject:[iTermResult
                                                                                 withError:strongSelf.protocolError]];
                                              } else {
...
- (void)partialReadWithState:(iTermFileDescriptorMultiClientState *)state
    ...
    if (bytesRead < 0) {
        if (errno == EAGAIN) {
            ...
            __weak __typeof(self) weakSelf = self;
            [state whenReadable:^(iTermFileDescriptorMultiClientState *state) {
                [weakSelf partialReadWithState:state totalLength:totalLength builder:builder callback:callback];
            }];
            return;
        }
    ...
    if (message.controlBuffer.cm.cmsg_len == CMSG_LEN(sizeof(int)) &&
        message.controlBuffer.cm.cmsg_level == SOL_SOCKET && message.controlBuffer.cm.cmsg_type == SCM_RIGHTS) {
    ...
    if (bytesRead > 0) {
        [builder appendBytes:message.message.msg_iov[0].iov_base length:bytesRead];
    }

    // BUG-f973: Guard builder length instead of assert - overflow should not crash
    if (builder.length > totalLength) {
        ELog(@"BUG-f973: Builder length %ld exceeds totalLength %ld", (long)builder.length, (long)totalLength);
        [callback invokeWithObject:[iTermResult withError:self.ioError]];
        return;
    }
    if (builder.length == totalLength) {
    ...
    __weak __typeof(self) weakSelf = self;
    DLog(@"Have read %@/%@ from %@. Wait for socket to be readable again.", @(builder.length), @(totalLength),
         _socketPath);
    [state whenReadable:^(iTermFileDescriptorMultiClientState *state) {
        [weakSelf partialReadWithState:state totalLength:totalLength builder:builder callback:callback];
    }];
...
static void HexDump(NSData *data) {
    // Each byte becomes "XX " (3 chars), 16 bytes per line
    NSMutableString *dest = [NSMutableString stringWithCapacity:16 * 3];
    ...
    for (int i = 0; i < data.length; i++) {
        if (i % 16 == 0 && i > 0) {
            DLog(@"%4d  %@", addr, dest);
            addr = i;
            dest = [NSMutableString stringWithCapacity:16 * 3];
        }
...
- (void)send:(iTermMultiServerClientOriginatedMessage *)message
    ...
    // BUG-f974: Guard message length instead of assert - empty messages should fail gracefully
    if (clientServerProtocolMessage.ioVectors[0].iov_len == 0) {
        ELog(@"BUG-f974: Encoded message has 0 length (indistinguishable from EOF)");
        iTermClientServerProtocolMessageFree(&clientServerProtocolMessage);
        [callback invokeWithObject:@NO];
        return;
    }
    ...
    NSMutableData *data = [NSMutableData dataWithCapacity:sizeof(length) + length];
    char temp[sizeof(length)];
    memmove(temp, &length, sizeof(length));
    [data appendBytes:temp length:sizeof(temp)];
    [data appendBytes:clientServerProtocolMessage.ioVectors[0].iov_base length:length];
    iTermClientServerProtocolMessageFree(&clientServerProtocolMessage);

    DLog(@"Will send %@ byte header plus %@ byte payload, totaling %@ bytes", @(sizeof(temp)), @(length),
         @(data.length));
#if BETA
    ...
    [state whenWritable:^(iTermFileDescriptorMultiClientState *_Nullable state) {
        DLog(@"called back");
        [weakSelf tryWrite:data state:state callback:callback];
    }];
}
...
- (void)tryWrite:(NSData *)data
           state:(iTermFileDescriptorMultiClientState *)state
        callback:(iTermCallback<id, NSNumber *> *)callback {
    // BUG-f975: Guard data length instead of assert - empty data should fail gracefully
    if (data.length == 0) {
        ELog(@"BUG-f975: Attempted to write empty data");
        [callback invokeWithObject:@NO];
        return;
    }
    if (state.writeFD < 0) {
    ...
    const ssize_t bytesWritten = iTermFileDescriptorClientWrite(state.writeFD, data.bytes, data.length);
    const NSInteger dataLength = data.length;
    const int savedErrno = errno;
    ITAssertWithMessage(bytesWritten <= dataLength, @"Data length is %@ but wrote %@. errno is %d", @(dataLength),
                        @(bytesWritten), savedErrno);
    ...
    __weak __typeof(self) weakSelf = self;
    [state whenWritable:^(iTermFileDescriptorMultiClientState *_Nullable state) {
        ITAssertWithMessage(bytesWritten <= dataLength, @"Data length is %@ but wrote %@.", @(dataLength),
                            @(bytesWritten));
        [weakSelf
            tryWrite:[data
                         subdataFromOffset:MAX(0, bytesWritten)] // NOTE: bytesWritten can be negative if we get EAGAIN
               state:state
            callback:callback];
    }];
}
...
- (NSError *)waitError:(int)errorNumber {
    ...
    return [NSError errorWithDomain:iTermFileDescriptorMultiClientErrorDomain code:code userInfo:nil];
}
