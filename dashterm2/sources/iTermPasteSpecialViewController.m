...
NSString *const kPasteSpecialViewControllerUnicodeDashesRegularExpression = @"[" DASHES @"]";
NSString *const kPasteSpecialViewControllerUnicodeDoubleQuotesRegularExpression = @"[" DOUBLE_QUOTES @"]";
NSString *const kPasteSpecialViewControllerUnicodeSingleQuotesRegularExpression = @"[" SINGLE_QUOTES @"]";
...
- (void)awakeFromNib {
    ...
    [_icuRegexHelpLabel replaceWithHyperlinkTo:[NSURL URLWithString:@"https://dashterm.com/regex"]];
...
- (void)setProfileType:(ProfileType)profileType {
    ...
    (void)[self.view setVisibilityForTerminalEnclosures:!!(profileType & ProfileTypeTerminal)
                               ...
                               sharedProfilesEnclosures:false
                                           stateStorage:[NSMutableDictionary dictionaryWithCapacity:8]
                                              shrinkage:&shrinkage];
...
- (NSString *)descriptionForByteSize:(double)chunkSize {
    ...
    NSString *description =
        [NSString stringWithFormat:@"%@ %@B", [formatter stringFromNumber:@(chunkSize)], units[exponent]];
    return description;
}
...
- (NSString *)descriptionForDuration:(NSTimeInterval)duration {
    ...
    [formatter setNumberStyle:NSNumberFormatterDecimalStyle];
    NSString *description =
        [NSString stringWithFormat:@"%@ %@", [formatter stringFromNumber:@(duration / multiplier)], units];
    return description;
}
...
- (BOOL)controlIsTransform:(NSControl *)control {
    return (control != _bracketedPasteMode && control != _waitForPrompts && control != _chunkSizeSlider &&
            control != _delayBetweenChunksSlider);
}

- (IBAction)settingChanged:(id)sender {
    _spacesPerTab.enabled = (_tabTransform.enabled && _tabTransform.selectedTag == kTabTransformConvertToSpaces);
...
- (void)setShouldEscapeShellCharsWithBackslash:(BOOL)shouldEscapeShellCharsWithBackslash {
    _escapeShellCharsWithBackslash.state =
        shouldEscapeShellCharsWithBackslash ? NSControlStateValueOn : NSControlStateValueOff;
}
...
+ (NSString *)descriptionForCodedSettings:(NSString *)jsonString {
    ...
    // Description components: typically 3-6 items
    ...
    if ([dict[kShouldUseRegexSubstitution] boolValue]) {
        [components addObject:[NSString stringWithFormat:@"s/%@/%@/g", dict[kRegularExpression] ?: @"",
                                                         dict[kSubstitution] ?: @""]];
    }
