...
- (NSInteger)enumerateSwiftySubstringsWithBlock:(void (^_Nullable)(NSUInteger index, NSString *substring,
                                                                   BOOL isLiteral, BOOL *stop))block {
    int parens = 0;
    NSInteger start = 0;
    NSMutableArray<NSNumber *> *parensStack = [NSMutableArray arrayWithCapacity:8]; // Parentheses nesting stack
    NSUInteger index = 0;
    ...
    if (_state == SWIFTY_STATE_EXPR || _state == SWIFTY_STATE_EXPR_STR || _state == SWIFTY_STATE_EXPR_STR_ESC) {
        if (self.tolerateTruncation) {
            _wasTruncated = YES;
