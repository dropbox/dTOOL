...
// YES if the regex compiled successfully (or is empty). NO if there was a compilation error.
@property (nonatomic, readonly) BOOL hasValidRegex;
// If regex compilation failed, contains the error. Otherwise nil.
@property (nullable, nonatomic, readonly) NSError *regexCompilationError;
