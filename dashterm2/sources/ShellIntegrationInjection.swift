...
@objc class ShellIntegrationInjector: NSObject {
    ...
    private enum ShellLauncherInfo {
        ...
        init(_ argv: [String]) {
            ...
            // BUG-356: Guard against argv[5] array bounds crash
            // Even though argv.get(5, ...) is used above, direct argv[5] access below could crash
            guard argv.count > 5,
                  argv.get(5, default: "").hasPrefix("SHELL="),
    ...
    func modifyShellEnvironment(shellIntegrationDir: String,
        ...
        case .command:
            ...
            // BUG-357, BUG-363: Guard against empty argv crash
            guard !argv.isEmpty,
    ...
    func files(destinationBase: URL) -> [URL: URL] {
        ...
        // BUG-1749: Use compactMap to safely convert tuple elements
        let safeTuples = tuples.compactMap { tuple -> (URL, URL)? in
            guard let source = tuple.0 else { return nil }
            return (source, tuple.1)
        }
        return Dictionary(uniqueKeysWithValues: safeTuples)
    }
}
