...
class AICompletion {
    ...
    private static let conversationsLock = NSLock()
                ...
                var completions = content
                    ...
                    .compactMap { line -> CompletionItem? in
                        ...
                        guard parts.count >= 2,
                              ...
                              let suggestion = parts.dropFirst().first else {
                            return nil
                        }
                        ...
                        if completion.value.contains(filePlaceholder) {
                            ...
                            guard let regex = try? NSRegularExpression(pattern: pattern) else {
                                // Static pattern, should never fail - use original value
                                return [CompletionItem(value: completion.value,
                                                       detail: completion.detail,
                                                       kind: .aiSuggestion)]
                            }
