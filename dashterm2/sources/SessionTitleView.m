...
- (instancetype)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        closeButton_ = [[NoFirstResponderButton alloc]
            initWithFrame:NSMakeRect(x, (frame.size.height - kButtonSize) / 2, kButtonSize, kButtonSize)];
        ...
        menuButton_.frame = NSMakeRect(frame.size.width - menuButton_.image.size.width - 6,
                                       ...
                                       menuButton_.image.size.width, menuButton_.image.size.height);
        [menuButton_ setAutoresizingMask:NSViewMinXMargin];
        [self addSubview:menuButton_];

        // AI lock indicator - shown between label and menu button when locked
        if (@available(macOS 11, *)) {
            NSImage *lockImage = [NSImage imageWithSystemSymbolName:@"lock.fill"
                                           accessibilityDescription:@"Locked from AI"];
            aiLockIndicator_ = [[NSImageView alloc] initWithFrame:NSMakeRect(0, 0, kButtonSize, kButtonSize)];
            aiLockIndicator_.image = lockImage;
            aiLockIndicator_.imageScaling = NSImageScaleProportionallyDown;
            aiLockIndicator_.hidden = YES;  // Hidden by default
            aiLockIndicator_.toolTip = @"This terminal is locked from AI access";
            [aiLockIndicator_ setAutoresizingMask:NSViewMinXMargin];
            [self addSubview:aiLockIndicator_];
        }

        label_ = [[NSTextField alloc]
            initWithFrame:NSMakeRect(x, 0, menuButton_.frame.origin.x - x - kMargin, frame.size.height)];
        ...
        [self addCursorRect:NSMakeRect(0, 0, frame.size.width, frame.size.height) cursor:[NSCursor arrowCursor]];
...
- (void)layoutStatusBar {
    if (_statusBarViewController) {
        ...
        _statusBarViewController.view.frame =
            NSMakeRect(minX, 1, NSMinX(menuButton_.frame) - margin - minX, self.frame.size.height);
        label_.hidden = YES;
    } else {
...
- (void)drawRect:(NSRect)insaneRect {
    ...
    if (!self.window.ptyWindow.it_terminalWindowUseMinimalStyle) {
        ...
        NSRectFillUsingOperation(NSMakeRect(dirtyRect.origin.x, 0, dirtyRect.size.width, 1),
...
- (void)viewDidMoveToWindow {
    ...
    [self updateTextColor];
    [self updateLayoutForCurrentScreen];
}
...
#pragma mark - iTermViewScreenNotificationHandling
...
// GitLab #12487: Update layout when screen changes to ensure pixel-aligned frames
// on displays with different backing scale factors.
- (void)updateLayoutForCurrentScreen {
    const double kMargin = 5;
    CGFloat x = kMargin;
    NSRect frame = self.frame;

    // Round all frame coordinates to pixel boundaries for the current screen
    CGFloat buttonY = [self retinaRound:(frame.size.height - kButtonSize) / 2];
    closeButton_.frame = NSMakeRect(x, buttonY, kButtonSize, kButtonSize);

    x += closeButton_.frame.size.width + kMargin;

    CGFloat menuX = [self retinaRound:frame.size.width - menuButton_.image.size.width - 6];
    CGFloat menuY = [self retinaRound:(frame.size.height - menuButton_.image.size.height) / 2];
    menuButton_.frame = NSMakeRect(menuX, menuY, menuButton_.image.size.width, menuButton_.image.size.height);

    // Position AI lock indicator to the left of the menu button
    CGFloat lockX = menuX - kButtonSize - kMargin;
    if (aiLockIndicator_) {
        aiLockIndicator_.frame = NSMakeRect(lockX, buttonY, kButtonSize, kButtonSize);
    }

    // Calculate right edge for label - account for lock indicator if visible
    CGFloat labelRightEdge = aiLockIndicator_ && !aiLockIndicator_.hidden
        ? lockX - kMargin
        : menuX - kMargin;
    ...
    if (_statusBarViewController) {
        ...
        // Recalculate label frame with pixel-aligned coordinates
        NSRect lframe = label_.frame;
        lframe.origin.x = x;
        lframe.origin.y = [self retinaRound:(frame.size.height - lframe.size.height) / 2 + kBottomMargin];
        lframe.size.width = labelRightEdge - x;
        label_.frame = lframe;
    }
...
- (void)updateTextColor {
    ...
    if (self.window.ptyWindow.it_terminalWindowUseMinimalStyle) {
        label_.textColor = [self.window.ptyWindow
            it_terminalWindowDecorationTextColorForBackgroundColor:[delegate_ sessionTitleViewBackgroundColor]];
        [self setNeedsDisplay:YES];
        return;
    }
    switch ([self.effectiveAppearance it_tabStyle:preferredStyle]) {
        ...
        case TAB_STYLE_MINIMAL:
            // BUG-468: Replace assert(NO) with DLog and fallthrough
            // These styles should have been resolved by it_tabStyle
            DLog(@"SessionTitleView updateTextColor got unresolved tab style");
...
- (void)setShowsAILockIndicator:(BOOL)showsAILockIndicator {
    if (_showsAILockIndicator == showsAILockIndicator) {
        return;
    }
    _showsAILockIndicator = showsAILockIndicator;
    aiLockIndicator_.hidden = !showsAILockIndicator;
    [self updateLayoutForCurrentScreen];
}
