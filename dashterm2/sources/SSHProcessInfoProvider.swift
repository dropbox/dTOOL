...
fileprivate struct PSRow {
    ...
    init?(_ line: String) {
        ...
        // BUG-971: Use POSIX locale for parsing English date format from ps output
...
class SSHProcessInfoProvider {
    ...
    private var cpuUtilizationTimer: Timer?  // BUG-979: Store timer for invalidation
    // BUG-8975: Store observer tokens so we can remove them in deinit
    private var notificationObservers: [NSObjectProtocol] = []

    deinit {
        // BUG-979: Invalidate timer to prevent resource leak
        cpuUtilizationTimer?.invalidate()
        // BUG-8975: Remove notification observers to prevent crash after dealloc
    ...
    init(rootPID: pid_t,
        ...
        self.runner = runner
        // BUG-8975: Store observer tokens for removal in deinit
        ...
        notificationObservers.append(becomeActiveObserver)
        ...
        needsUpdate = true
        // BUG-979: Store timer reference for proper cleanup in deinit
        cpuUtilizationTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] timer in
...
fileprivate class SSHProcessDataSource: NSObject, ProcessDataSource {
    ...
    func nameOfProcess(withPid thePid: pid_t,
        ...
        let components = (row.command as NSString).componentsInShellCommand()
        guard let firstComponent = components.first else {
            return nil
        }
        return firstComponent.lastPathComponent
    }
    ...
    func commandLineArguments(forProcess pid: pid_t,
        ...
        execName?.pointee = (firstArg as NSString).removingPrefix("-") as NSString
        return args
    }
