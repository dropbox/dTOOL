...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<%@: %p root=%@ target=%@>", NSStringFromClass([self class]), self, _root, _target];
}
...
- (NSString *)pathByReplacingPrefixWithSyntheticRoot:(NSString *)dir {
    ...
    if ([dir hasPrefix:targetPrefix]) {
        return [dir stringByReplacingCharactersInRange:NSMakeRange(0, _target.length) withString:_root];
    }
...
- (instancetype)initPrivate {
    ...
    if (self) {
        ...
        NSMutableArray<iTermSyntheticDirectory *> *directories =
            [NSMutableArray arrayWithCapacity:8]; // Synthetic directories
        ...
        [[self class]
            enumerateLinesInContent:contents
                              block:^(NSString *line) {
                                  ...
                                  [directories addObject:[[iTermSyntheticDirectory alloc] initWithRoot:root
                                                                                                target:target]];
                              }];
