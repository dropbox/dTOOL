...
class MarkCache: NSObject, MarkCacheReading {
    ...
    override var description: String {
        return "<MarkCache: \(it_addressString)\n" + dict.keys.sorted().map { i in
            "  Line \(i): \(self[i]?.shortDebugDescription ?? "nil")"
        }.joined(separator: "\n") + ">"
    }
    @objc
    override init() {
    ...
    func dump() {
        ...
        for mark in enumerate(from: 0) {
            // BUG-1703: Use as? instead of as! for debug cast safety
            guard let object = mark as? NSObject, let ito = mark as? IntervalTreeObject else {
                continue
            }
