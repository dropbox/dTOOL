...
- (NSFont *)it_fontByAddingToPointSize:(CGFloat)delta {
    ...
    CTFontRef newFont = CTFontCreateCopyWithAttributes(me, newSize, &matrix, myDescriptorCopy);
...
static os_unfair_lock gMetalFontBijectionLock = OS_UNFAIR_LOCK_INIT;
...
- (int)it_metalFontID {
    ...
    os_unfair_lock_lock(&gMetalFontBijectionLock);
    NSNumber *number = [bijection objectForRight:self];
    if (number) {
        os_unfair_lock_unlock(&gMetalFontBijectionLock);
...
+ (instancetype)it_fontWithMetalID:(int)metalID {
    iTermBijection<NSNumber *, NSFont *> *bijection = iTermMetalFontBijection();
    os_unfair_lock_lock(&gMetalFontBijectionLock);
    NSFont *result = [bijection objectForLeft:@(metalID)];
    os_unfair_lock_unlock(&gMetalFontBijectionLock);
    return result;
}
