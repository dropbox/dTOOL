...
static os_unfair_lock gCapturedOutputLock = OS_UNFAIR_LOCK_INIT;
...
+ (instancetype)capturedOutputWithDictionary:(NSDictionary *)dict {
    ...
    if (capturedOutput) {
        ...
        capturedOutput.promisedCommand = [iTermPromise promise:^(id<iTermPromiseSeal>  _Nonnull seal) {
            ...
            [seal reject:[NSError errorWithDomain:@"com.dashterm.dashterm2.captured-output" code:0 userInfo:nil]];
        }];
...
- (id<CapturedOutputReading>)doppelganger {
    os_unfair_lock_lock(&gCapturedOutputLock);
    // BUG-f1371: Replace assert with guard - doppelganger of doppelganger should return nil, not crash
    if (_isDoppelganger) {
        os_unfair_lock_unlock(&gCapturedOutputLock);
        DLog(@"WARNING: Attempted to get doppelganger of a doppelganger (CapturedOutput)");
        return nil;
    }
    if (!_doppelganger) {
    ...
    CapturedOutput *result = _doppelganger;
    os_unfair_lock_unlock(&gCapturedOutputLock);
    return result;
}
