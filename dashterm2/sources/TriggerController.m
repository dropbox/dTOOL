...
- (void)loadTriggers {
    // Pre-allocate based on expected number of trigger classes
    NSArray *triggerClasses = [self.class triggerClassesForTerminal:!_browserMode];
    NSMutableArray *triggers = [NSMutableArray arrayWithCapacity:triggerClasses.count];
    for (Class class in triggerClasses) {
...
- (void)createDetailViewController {
    ...
    _detailViewController = [[iTermAddTriggerViewController alloc]
                  initWithName:@""
                     matchType:_browserMode ? iTermTriggerMatchTypeURLRegex : iTermTriggerMatchTypeRegex
                         regex:@""
                  contentRegex:nil
           interpolatedStrings:[iTermProfilePreferences boolForKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS
...
+ (NSArray<Class> *)triggerClassesForTerminal:(BOOL)terminal {
    ...
    if (terminal) {
        ...
        allClasses = @[
            [ReaderModeBrowserTrigger class], [HighlightBrowserTrigger class], [HyperlinkBrowserTrigger class],
            [ReloadBrowserTrigger class], [InjectJavascriptURLTrigger class], [InjectJavascriptContentTrigger class]
        ];
    }
...
- (void)setTriggerDictionary:(NSDictionary *)triggerDictionary
    ...
    if (rowIndex < 0) {
        // BUG-f1366: Convert assert to guard - nil triggerDictionary with negative index should be no-op
        if (!triggerDictionary) {
            return;
        }
        [triggerDictionaries addObject:triggerDictionary];
    } else {
        // BUG-1252d: Bounds check before accessing array at rowIndex
        if (rowIndex >= (NSInteger)triggerDictionaries.count) {
            return;
        }
        if (triggerDictionary) {
    ...
    const NSInteger row = rowIndex >= 0 ? rowIndex : (NSInteger)triggerDictionaries.count - 1;
...
- (void)moveTriggerOnRow:(int)sourceRow toRow:(int)destinationRow {
    NSMutableArray *triggerDictionaries = [[self triggerDictionariesForCurrentProfile] mutableCopy];
    // BUG-10347: Bounds check before accessing array
    if (sourceRow < 0 || sourceRow >= (int)triggerDictionaries.count) {
        return;
    }
    if (destinationRow > sourceRow) {
        --destinationRow;
    }
    if (destinationRow < 0 || destinationRow > (int)triggerDictionaries.count) {
        return;
    }
...
- (NSDictionary *)defaultTriggerDictionary {
    ...
    // BUG-10348: Bounds check before accessing _triggers array
    if (index < 0 || index >= (int)_triggers.count) {
        return @{kTriggerRegexKey : @"", kTriggerActionKey : @""};
    }
    Trigger *trigger = _triggers[index];
    return @{kTriggerRegexKey : @"", kTriggerActionKey : [trigger action]};
}
...
- (void)updateUseInterpolatedStringParametersState {
    _interpolatedStringParameters.state = [iTermProfilePreferences boolForKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS
                                                                    inProfile:[self bookmark]]
...
+ (NSTextField *)labelWithString:(NSString *)string origin:(NSPoint)origin {
    NSTextField *textField = [[NSTextField alloc] initWithFrame:NSMakeRect(origin.x, origin.y, 0, 0)];
...
- (NSString *)jsonForTriggerAtIndex:(NSInteger)i {
    ...
    // BUG-10349: Bounds check before accessing array
    if (i < 0 || i >= (NSInteger)triggers.count) {
        return nil;
    }
...
- (NSURL *)urlForSelectedTriggers {
    NSArray<NSString *> *parts =
        [_tableView.selectedRowIndexes.it_array mapWithBlock:^id _Nullable(NSNumber *indexNumber) {
            return [self jsonForTriggerAtIndex:indexNumber.integerValue];
        }];
    NSURLComponents *components = [[NSURLComponents alloc] init];
    components.scheme = @"dashterm2";
...
+ (NSString *)importDescriptionForTrigger:(Trigger *)trigger {
    return [NSString stringWithFormat:@"On “%@”, %@%@", [trigger.regex it_sanitized],
                                      [trigger.description it_sanitized], trigger.partialLine ? @"  instantly" : @""];
}
...
+ (NSArray<NSString *> *)guidsForProfilesToImportTriggersInto:(NSArray<Trigger *> *)triggers {
    ...
    NSString *message =
        [NSString stringWithFormat:@"Select the profiles into which these triggers should be imported:\n\n%@", joined];
...
- (BOOL)tableView:(NSTableView *)tableView
    ...
    // Pre-allocate based on number of selected rows
    NSMutableArray *indexes = [NSMutableArray arrayWithCapacity:rowIndexes.count];
    [rowIndexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
        [indexes addObject:@(idx)];
    }];

    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:indexes requiringSecureCoding:NO error:nil];
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    NSError *error = nil;
    NSArray *indexes = [NSKeyedUnarchiver unarchivedObjectOfClass:[NSArray class] fromData:rowData error:&error];
    if (error) {
    ...
    // BUG-10350: Bounds check before accessing indexes array
    if (indexes.count == 0) {
        return NO;
    }
...
+ (NSView *)viewForParameterForTrigger:(Trigger *)trigger
    ...
    if ([trigger paramIsTwoColorWells]) {
        ...
        CPKColorWell *well = wellFactory(NSMakeRect(x, 0, kWellWidth, size.height), trigger.textColor);
        ...
        well = wellFactory(NSMakeRect(x, 0, kWellWidth, size.height), trigger.backgroundColor);
        [container addSubview:well];
    ...
    if ([trigger paramIsTwoStrings]) {
        ...
        iTermTuple<NSString *, NSString *> *pair =
            [iTermTwoParameterTriggerCodec tupleFromString:[NSString castFrom:value]];
        ...
        iTermTwoStringView *container = [[iTermTwoStringView alloc] initWithFirst:nameTextField second:valueTextField];
    ...
    if ([trigger paramIsPopupButton]) {
        ...
        for (NSDictionary *items in [trigger groupedMenuItemsForPopupButton]) {
            ...
            for (id key in [trigger objectsSortedByValueInDict:items]) {
                ...
                if (theTitle) {
                    NSMenuItem *anItem = [[NSMenuItem alloc] initWithTitle:theTitle action:nil keyEquivalent:@""];
    ...
    *delegateOut = [trigger newParameterDelegateWithPassthrough:receiver];
    ...
    if (trigger.helpText) {
        ...
        iTermPopoverHelpButton *helpButton = [[iTermPopoverHelpButton alloc]
            initWithFrame:NSMakeRect(NSMaxX(wrapperFrame) - helpButtonWidth, NSMinY(wrapperFrame), helpButtonWidth,
...
+ (iTermFocusReportingTextField *)newTextFieldOfSize:(NSSize)size
    ...
    iTermFocusReportingTextField *textField =
        [[iTermFocusReportingTextField alloc] initWithFrame:NSMakeRect(0, 0, size.width, size.height)];
    textField.font = [NSFont systemFontOfSize:[NSFont systemFontSize]];
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
    NSArray *triggers = [self triggerDictionariesForCurrentProfile];
    // BUG-10351: Bounds check before accessing arrays
    if (row < 0 || row >= (NSInteger)triggers.count) {
        return 12.0;
    }
    NSDictionary *triggerDictionary = triggers[row];
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    NSArray *triggers = [self triggerDictionariesForCurrentProfile];
    // BUG-10352: Bounds check before accessing array
    if (row < 0 || row >= (NSInteger)triggers.count) {
        return [[NSView alloc] init];
    }
    NSDictionary *triggerDictionary = triggers[row];
    Trigger *trigger = [Trigger triggerFromUntrustedDict:triggerDictionary];
    iTermTableCellViewWithTextField *view =
        [tableView newTableCellViewWithTextFieldUsingIdentifier:@"Trigger Tableview Entry"
...
- (void)updateDetailViewController {
    ...
    NSArray *triggers = [self triggerDictionariesForCurrentProfile];
    // BUG-10353: Bounds check before accessing array
    if (i < 0 || i >= (NSInteger)triggers.count) {
        [self hideDetailView];
        return;
    }
...
- (IBAction)share:(id)sender {
    ...
    [menu addItemWithTitle:@"Copy Trigger as URL to Clipboard"
                    action:^{
...
- (IBAction)toggleUseInterpolatedStrings:(id)sender {
    const BOOL wasEnabled = [iTermProfilePreferences boolForKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS
                                                      inProfile:[self bookmark]];
...
- (void)doubleClick:(id)sender {
    ...
    NSPoint windowLocation =
        [self.window convertRectFromScreen:NSMakeRect(screenLocation.x, screenLocation.y, 0, 0)].origin;
...
- (void)colorWellDidChange:(CPKColorWell *)colorWell {
    ...
    NSMutableDictionary *triggerDictionary = [[self triggerDictionariesForCurrentProfile][row] mutableCopy];
...
- (void)actionDidChange:(NSPopUpButton *)sender {
    ...
    NSArray *triggerDicts = [self triggerDictionariesForCurrentProfile];
    // BUG-10354: Bounds check before accessing arrays
    if (rowIndex >= (NSInteger)triggerDicts.count) {
        return;
    }
    NSInteger indexOfSelectedAction = [sender indexOfSelectedItem];
    if (indexOfSelectedAction < 0 || indexOfSelectedAction >= (NSInteger)_triggers.count) {
        return;
    }

    NSMutableDictionary *triggerDictionary = [triggerDicts[rowIndex] mutableCopy];
    ...
    } else if ([triggerObj triggerOptionalDefaultParameterValueWithInterpolation:_interpolatedStringParameters.state ==
                                                                                 NSControlStateValueOn]) {
        triggerDictionary[kTriggerParameterKey] =
            [triggerObj triggerOptionalDefaultParameterValueWithInterpolation:_interpolatedStringParameters.state ==
                                                                              NSControlStateValueOn];
    }
...
- (void)parameterPopUpButtonDidChange:(NSPopUpButton *)sender {
    ...
    NSArray *triggerDicts = [self triggerDictionariesForCurrentProfile];
    // BUG-10355: Bounds check before accessing array
    if (rowIndex >= (NSInteger)triggerDicts.count) {
        return;
    }
    NSMutableDictionary *triggerDictionary = [triggerDicts[rowIndex] mutableCopy];
...
- (void)detailViewControllerDidChange {
    ...
    NSArray *triggerDicts = [self triggerDictionariesForCurrentProfile];
    // BUG-10356: Bounds check before accessing array
    if (row >= (NSInteger)triggerDicts.count) {
        return;
    }
    NSMutableDictionary *triggerDictionary = [triggerDicts[row] mutableCopy];
...
- (void)triggerDidChangeParameterOptions:(Trigger *)sender {
    NSIndexSet *rowIndexes = [self.triggerDictionariesForCurrentProfile
        indexesOfObjectsPassingTest:^BOOL(NSDictionary *_Nonnull dict, NSUInteger idx, BOOL *_Nonnull stop) {
