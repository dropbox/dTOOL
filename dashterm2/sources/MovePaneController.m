...
@implementation MovePaneController {
    ...
    // The session being moved (weak property, properly zeroed when session deallocates).
...
- (void)moveWindowBy:(NSPoint)point {
    ...
    origin.x += point.x;
    origin.y += point.y;
    DLog(@"Move window from %@ to %@", NSStringFromPoint(window.frame.origin), NSStringFromPoint(origin));
...
- (void)moveSession:(PTYSession *)movingSession toTabInWindow:(NSWindow *)window {
    ...
    // BUG-f964: Use guard instead of assert - nil tab should return early with warning
    if (oldTab == nil) {
        DLog(@"BUG-f964: moveSession:toTabInWindow: called with session not in any tab - ignoring");
        return;
    }
    ...
    if (movingSession.isTmuxClient) {
        [[movingSession tmuxController] breakOutWindowPane:[movingSession tmuxPane] toTabAside:term.terminalGuid];
        return;
    }
...
- (void)moveSessionToNewWindow:(PTYSession *)movingSession atPoint:(NSPoint)point {
    ...
    if ([movingSession isTmuxClient]) {
        [[movingSession tmuxController] breakOutWindowPane:[movingSession tmuxPane] toPoint:point];
        return;
    }
...
+ (void)moveTab:(PTYTab *)tab toWindow:(PseudoTerminal *)term atIndex:(NSInteger)index {
    ...
    // BUG-f1022: Guard against nil sourceTerm instead of crashing
    if (!sourceTerm) {
        DLog(@"BUG-f1022: moveTab:toWindow:atIndex: called with tab not in a PseudoTerminal - ignoring");
        return;
    }
    NSInteger sourceIndex = [sourceTerm.tabs indexOfObject:tab];
    // BUG-f1023: Guard against tab not found instead of crashing
    if (sourceIndex == NSNotFound) {
        DLog(@"BUG-f1023: moveTab:toWindow:atIndex: tab not found in sourceTerm.tabs - ignoring");
        return;
    }
...
- (BOOL)dropTab:(PTYTab *)tab inSession:(PTYSession *)dest half:(SplitSessionHalf)half atPoint:(NSPoint)point {
...
- (BOOL)reallyDropTab:(PTYTab *)tab inSession:(PTYSession *)dest half:(SplitSessionHalf)half atPoint:(NSPoint)point {
    // BUG-395: Guard against empty sessions array (original check was count != 1)
    ...
    if (sessions.count != 1) {
        ...
        DLog(@"Tab has %lu sessions (expected 1). Abort.", (unsigned long)sessions.count);
        return NO;
    }

    session_ = [sessions objectAtIndex:0];
...
- (BOOL)dropInSession:(PTYSession *)dest half:(SplitSessionHalf)half atPoint:(NSPoint)point {
    _dropping = YES;
...
- (BOOL)reallyDropInSession:(PTYSession *)dest half:(SplitSessionHalf)half atPoint:(NSPoint)point {
    ...
    if ([self.session isTmuxClient]) {
        ...
        if (isMove_) {
            ...
            [[self.session tmuxController] swapPane:[self.session tmuxPane] withPane:[dest tmuxPane]];
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:iTermSessionDidChangeTabNotification
                                                            object:self.session];
        return YES;
