...
- (instancetype)initWithScreenChars:(const screen_char_t *)screenChars length:(NSInteger)length {
    self = [super init];
    if (self) {
        _length = length;
        _stringValue = ScreenCharArrayToString(screenChars, 0, length, &_backingStore, &_deltas);
    }
    return self;
}
...
VT100GridCoord GetPositionOfImageInChar(screen_char_t c) {
    return VT100GridCoordMake(c.foregroundColor, c.backgroundColor);
}
...
void AppendToChar(screen_char_t *dest, unichar c) {
    if (dest->complexChar) {
        ...
        SetComplexCharInScreenChar(dest, combined, iTermUnicodeNormalizationNone, NO);
    }
}
...
NSString *ScreenCharArrayToString(const screen_char_t *screenChars, int start, int end, unichar **backingStorePtr,
                                  int **deltasPtr) {
    ...
    *deltasPtr = deltas;
    ...
    for (int i = start; i < end; ++i) {
        ...
        if (screenChars[i].image ||
            (!screenChars[i].complexChar && c >= ITERM2_PRIVATE_BEGIN && c <= ITERM2_PRIVATE_END)) {
...
void DumpScreenCharArray(screen_char_t *screenChars, int lineLength) {
    DLog(@"%@", ScreenCharArrayToStringDebug(screenChars, lineLength));
}

NSString *ScreenCharArrayToStringDebug(const screen_char_t *screenChars, int lineLength) {
...
NSString *DebugStringForScreenChar(screen_char_t c) {
    ...
    return [NSString
        stringWithFormat:@"code=%x (%@) foregroundColor=%@ fgGreen=%@ fgBlue=%@ backgroundColor=%@ bgGreen=%@ "
                         @"bgBlue=%@ foregroundColorMode=%@ backgroundColorMode=%@ complexChar=%@ bold=%@ faint=%@ "
                         @"italic=%@ blink=%@ underline=%@ underlineStyle=%@ strikethrough=%@ image=%@ "
                         @"virtualPlaceholder=%@ invisible=%@ inverse=%@ guarded=%@ rtl=%@ unused=%@",
                         (int)c.code, ScreenCharToStr(&c), @(c.foregroundColor), @(c.fgGreen), @(c.fgBlue),
                         @(c.backgroundColor), @(c.bgGreen), @(c.bgBlue), modes[c.foregroundColorMode],
                         modes[c.backgroundColorMode], @(c.complexChar), @(c.bold), @(c.faint), @(c.italic), @(c.blink),
                         @(c.underline), @(ScreenCharGetUnderlineStyle(c)), @(c.strikethrough), @(c.image),
                         @(c.virtualPlaceholder), @(c.invisible), @(c.inverse), @(c.guarded), @(c.rtlStatus),
                         @(c.unused)];
}

static inline void iTermFillScreenCharBufferWithTemplate(screen_char_t *buffer, screen_char_t templateChar,
                                                         CFIndex length) {
    if (length <= 0 || !buffer) {
        return;
    }
    ...
    CFIndex filled = 1;
    while (filled < length) {
        const CFIndex chunk = MIN(filled, length - filled);
        memcpy(buffer + filled, buffer, chunk * sizeof(screen_char_t));
        filled += chunk;
    }
}

static inline void iTermFillASCIICharsFromBytes(screen_char_t *buffer, const unsigned char *bytes, CFIndex length,
                                                screen_char_t fg, screen_char_t bg) {
    screen_char_t templateChar;
    InitializeScreenChar(&templateChar, fg, bg);
    iTermFillScreenCharBufferWithTemplate(buffer, templateChar, length);
    for (CFIndex i = 0; i < length; i++) {
        const unsigned char code = bytes[i];
        if (code >= ITERM2_PRIVATE_BEGIN && code <= ITERM2_PRIVATE_END) {
            buffer[i].code = UNICODE_REPLACEMENT_CHAR;
        } else {
            buffer[i].code = code;
        }
    }
...
static inline void iTermFillASCIICharsFromUnichars(screen_char_t *buffer, const UniChar *chars, CFIndex length,
                                                   screen_char_t fg, screen_char_t bg) {
    screen_char_t templateChar;
    InitializeScreenChar(&templateChar, fg, bg);
    iTermFillScreenCharBufferWithTemplate(buffer, templateChar, length);
    for (CFIndex i = 0; i < length; i++) {
        const UniChar code = chars[i];
        if (code >= ITERM2_PRIVATE_BEGIN && code <= ITERM2_PRIVATE_END) {
            buffer[i].code = UNICODE_REPLACEMENT_CHAR;
        } else {
            buffer[i].code = code;
...
static BOOL iTermFastPathStringToScreenCharsASCII(NSString *s, screen_char_t *buf, screen_char_t fg, screen_char_t bg,
                                                  int *len, int *cursorIndex) {
    // Root cause fix: CFStringGetLength crashes on NULL pointer.
    // This function must handle nil input gracefully.
    if (!s) {
        if (len) {
            *len = 0;
        }
    ...
    const CFStringRef cfString = (__bridge CFStringRef)s;
    const CFIndex length = CFStringGetLength(cfString);
    if (length == 0) {
        if (len) {
            *len = 0;
        }
        if (cursorIndex && (NSUInteger)*cursorIndex >= (NSUInteger)length) {
            *cursorIndex = 0;
        }
        return YES;
    }

    const char *asciiBytes = CFStringGetCStringPtr(cfString, kCFStringEncodingASCII);
    if (asciiBytes) {
        iTermFillASCIICharsFromBytes(buf, (const unsigned char *)asciiBytes, length, fg, bg);
        if (len) {
            *len = (int)length;
        }
        if (cursorIndex && (NSUInteger)*cursorIndex >= (NSUInteger)length) {
            *cursorIndex = (int)length;
        }
        return YES;
    }

    const UniChar *chars = CFStringGetCharactersPtr(cfString);
    if (chars) {
        for (CFIndex i = 0; i < length; i++) {
            if (chars[i] > 0x7f) {
                return NO;
        ...
        iTermFillASCIICharsFromUnichars(buf, chars, length, fg, bg);
        if (len) {
            *len = (int)length;
        }
        if (cursorIndex && (NSUInteger)*cursorIndex >= (NSUInteger)length) {
    ...
    const CFIndex kStackLimit = 256;
    UniChar stackChars[kStackLimit];
    UniChar *buffer = NULL;
    if (length <= kStackLimit) {
        buffer = stackChars;
    } else {
        buffer = iTermMalloc(length * sizeof(UniChar));
        if (!buffer) {
            return NO;
        }
    }
    ...
    for (CFIndex i = 0; i < length; i++) {
        if (buffer[i] > 0x7f) {
            if (buffer != stackChars) {
                free(buffer);
            }
            return NO;
        }
    }
    iTermFillASCIICharsFromUnichars(buf, buffer, length, fg, bg);
    if (buffer != stackChars) {
        free(buffer);
    }
    if (len) {
...
void StringToScreenChars(NSString *s, screen_char_t *buf, screen_char_t fg, screen_char_t bg, int *len,
                         BOOL ambiguousIsDoubleWidth, int *cursorIndex, BOOL *foundDwc,
                         iTermUnicodeNormalization normalization, NSInteger unicodeVersion,
                         BOOL softAlternateScreenMode, BOOL *rtlFound) {
    // Handle nil string gracefully to prevent CFStringGetLength crash
    if (!s) {
        if (len) {
            *len = 0;
        }
    ...
    if (rtlFound) {
        if ([iTermPreferences boolForKey:kPreferenceKeyBidi]) {
            *rtlFound = ([s rangeOfCharacterFromSet:[NSCharacterSet rtlSmellingCodePoints]].location != NSNotFound);
        } else {
            *rtlFound = NO;
        }
    }

    if (iTermFastPathStringToScreenCharsASCII(s, buf, fg, bg, len, cursorIndex)) {
        return;
    }
    ...
    NSCharacterSet *modifierCharactersForcingFullWidthRendition =
        [NSCharacterSet modifierCharactersForcingFullWidthRendition];
    const BOOL shouldSupportVS16 =
        [iTermAdvancedSettingsModel vs16Supported] ||
        (!softAlternateScreenMode && [iTermAdvancedSettingsModel vs16SupportedInPrimaryScreen]);
    ...
    [s enumerateComposedCharacters:^(NSRange range, unichar baseBmpChar, NSString *composedOrNonBmpChar, BOOL *stop) {
        ...
        if (composedOrNonBmpChar && composedOrNonBmpChar.length > 0) {
            ...
            if (baseChar == 0x10EEEE) {
                ...
                if (baseChar == 0x2018 || baseChar == 0x2019 || baseChar == 0x201C || baseChar == 0x201D) {
                    // Bug fix #11776: Check bounds before accessing characterAtIndex:next
                    // 'next' may be >= composedLength when the quotation mark is alone
                    // or followed by a surrogate pair (next=2, composedLength=2)
                    if (next < (NSInteger)composedLength) {
            ...
            if (!disambiguated) {
                isDoubleWidth = [NSString isDoubleWidthCharacter:baseChar
                                          ambiguousIsDoubleWidth:ambiguousIsDoubleWidth
                                                  unicodeVersion:unicodeVersion
                                                  fullWidthFlags:fullWidthFlags];
                if (!isDoubleWidth && shouldSupportVS16 && [emojiAcceptingVS16 longCharacterIsMember:baseChar] &&
                    [composedOrNonBmpChar rangeOfCharacterFromSet:modifierCharactersForcingFullWidthRendition]
                            .location != NSNotFound) {
                    isDoubleWidth = YES;
                    disambiguated = YES;
                }
...
NSDictionary *ScreenCharEncodedRestorableState(void) {
    return @{
        kScreenCharComplexCharMapKey : [GetComplexCharRegistry() complexCharMap] ?: @{},
...
static NSString *ScreenCharColorDescription(unsigned int red, unsigned int green, unsigned int blue, ColorMode mode,
                                            BOOL fg) {
    switch (mode) {
...
NSString *ScreenCharDescription(screen_char_t c) {
    ...
    NSMutableArray<NSString *> *attrs = [NSMutableArray arrayWithCapacity:12];  // Character attributes count
    if (c.bold) {
    ...
    return [NSString
        stringWithFormat:@"fg=%@ bg=%@%@",
                         ScreenCharColorDescription(c.foregroundColor, c.fgGreen, c.fgBlue, c.foregroundColorMode, YES),
                         ScreenCharColorDescription(c.backgroundColor, c.bgGreen, c.bgBlue, c.backgroundColorMode, NO),
                         style];
}
...
void ScreenCharInvert(screen_char_t *c) {
    ...
    if (saved.backgroundColorMode == ColorModeAlternate && saved.backgroundColor == ALTSEM_DEFAULT) {
        c->foregroundColor = ALTSEM_REVERSED_DEFAULT;
    } else if (saved.backgroundColorMode == ColorModeAlternate && saved.backgroundColor == ALTSEM_REVERSED_DEFAULT) {
    ...
    if (saved.foregroundColorMode == ColorModeAlternate && saved.foregroundColor == ALTSEM_REVERSED_DEFAULT) {
        c->backgroundColor = ALTSEM_DEFAULT;
    } else if (saved.foregroundColorMode == ColorModeAlternate && saved.foregroundColor == ALTSEM_DEFAULT) {
...
BOOL AnnotateRightToLeftInScreenChars(screen_char_t *c, int len) {
    MutableScreenCharArray *msca = [[MutableScreenCharArray alloc] initWithLine:c
                                                                         length:len
...
int CellOffsetFromUTF16Offset(int utf16Offset, const int *deltas) {
