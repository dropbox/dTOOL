...
static void iTermDisplayReconfigurationCallback(CGDirectDisplayID display, CGDisplayChangeSummaryFlags flags,
                                                void *userInfo) {
...
- (void)updateWithSanityCheck:(BOOL)sanityCheck {
    ...
    NSMutableArray<NSScreen *> *screensToHideDock = [NSMutableArray arrayWithCapacity:NSScreen.screens.count];
    NSMutableArray<NSScreen *> *screensToHideMenu = [NSMutableArray arrayWithCapacity:NSScreen.screens.count];
    const BOOL active = NSApp.isActive;
    DLog(@"App active=%@", @(active));
    if (active) {
        [self findScreensToHideDock:screensToHideDock menu:screensToHideMenu];
    }
    ...
    const BOOL shouldHideDock =
        currentScreenWithDock != nil || [self haveFullScreenWindowOnSameScreenWhereDockWasLastHidden];
    ...
    if (sanityCheck && !shouldHideDock && ![self anyScreenHasDock] && [self dockIsCurrentlyHidden] &&
        ...
        DLog(@"Schedule sanity check for next spin of the runlooop. Showing the dock while hidden and no screen has "
             @"the dock and there is a full screen window.");
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)logScreens {
    for (NSScreen *screen in [NSScreen screens]) {
        DLog(@"Screen %@ frame=%@ visibleFrame=%@: has menu bar=%@ has dock=%@", screen, NSStringFromRect(screen.frame),
             NSStringFromRect(screen.visibleFrame), @([self screenHasMenuBar:screen]), @(screen.hasDock));
    }
}
...
- (BOOL)haveFullScreenWindowOnSameScreenWhereDockWasLastHidden {
    DLog(@"Checking if there's still a full screen window on the same screen where we last saw the dock. The last such "
         @"screen had frame %@",
...
NSString *PODescription(NSApplicationPresentationOptions presentationOptions) {
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:12]; // Presentation option flags
...
- (void)setApplicationPresentationFlagsWithHiddenDock:(BOOL)shouldHideDock
    ...
    const NSApplicationPresentationOptions mask =
    ...
    DLog(@"Set presentation options from %@ to %@", PODescription(NSApp.presentationOptions),
...
- (void)findScreensToHideDock:(NSMutableArray<NSScreen *> *)screensToHideDock
    ...
    for (id<iTermPresentationControllerManagedWindowController> windowController in windowControllers) {
        ...
        if (![screensToHideMenu containsObject:screen] && [self shouldHideMenuForWindowController:windowController]) {
...
- (NSScreen *)screenWithDockFromScreens:(NSArray<NSScreen *> *)screens {
    ...
    return [screens objectPassingTest:^BOOL(NSScreen *screen, NSUInteger index, BOOL *stop) {
        ...
        if ([[NSScreen screenWithFrame:screen.frame] hasDock]) {
            DLog(@"Screen with frame %@ - %@ - hasDock", NSStringFromRect(screen.frame),
                 [NSScreen screenWithFrame:screen.frame]);
            result = YES;
        }
        DLog(@"  Screen %@ with frame %@ and visible frame %@ hasdock=%@", screen, NSStringFromRect(screen.frame),
             NSStringFromRect(screen.visibleFrame), @(result));
        return result;
    }];
}
...
- (void)screenParametersDidChange:(NSNotification *)notification {
    ...
    DLog(@"screen parameters did change. Set lastScreen to nil and update. This could cause the dock to spuriously "
         @"appear.");
    _lastScreen = nil;
    [self update];
}
