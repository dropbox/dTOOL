...
// Optimization: Cache NSNumber objects for tmux pane IDs (0-511).
// Pane IDs are small non-negative integers. A tmux session with hundreds
// of panes would be extraordinary, so 512 entries covers all realistic usage.
static const int kCachedPaneIdCount = 512;
static NSNumber *sPaneIdCache[kCachedPaneIdCount];

NS_INLINE NSNumber *iTermTmuxPaneIdToNumber(int paneId) {
    if (paneId >= 0 && paneId < kCachedPaneIdCount) {
        return sPaneIdCache[paneId];
    }
    return @(paneId);
}

__attribute__((constructor)) static void iTermTmuxBufferSizeMonitorInitializePaneCache(void) {
    for (int i = 0; i < kCachedPaneIdCount; i++) {
        sPaneIdCache[i] = @(i);
    }
}
...
- (NSString *)description {
    NSArray<NSString *> *descriptions = [_data mapWithBlock:^NSString *(iTermTmuxBufferSizeDataPoint *point) {
        return point.description;
    }];
    NSString *data = [descriptions componentsJoinedByString:@", "];
    return [NSString stringWithFormat:@"<%@: %p %@>", NSStringFromClass(self.class), self, data];
}
...
- (instancetype)initWithController:(TmuxController *)controller pauseAge:(NSTimeInterval)pauseAge {
    self = [super init];
    if (self) {
        _controller = controller;
        _pauseAge = pauseAge;
        _series = [[NSMutableDictionary alloc] initWithCapacity:16];
...
- (void)setCurrentLatency:(NSTimeInterval)age forPane:(int)wp {
    ...
    NSNumber *paneKey = iTermTmuxPaneIdToNumber(wp);
    iTermTimeSeries *series = _series[paneKey];
    if (!series) {
        series = [[iTermTimeSeries alloc] initWithCapacity:4];
        series.lastTTL = INFINITY;
        _series[paneKey] = series;
    }

    if (now - series.lastUpdateTime > 0.5) {
        iTermTmuxBufferSizeDataPoint *dataPoint = [[iTermTmuxBufferSizeDataPoint alloc] initWithTime:now age:age];
...
- (void)update:(NSTimer *)timer {
    const NSTimeInterval now = [iTermTmuxBufferSizeDataPoint now];
    [_series enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull wp, iTermTimeSeries *_Nonnull series,
                                                 BOOL *_Nonnull stop) {
        if (series.count < 4) {
...
- (void)resetPane:(int)wp {
    [_series removeObjectForKey:iTermTmuxPaneIdToNumber(wp)];
}

- (void)maybeWarnPane:(int)wp ttl:(NSTimeInterval)ttl lastTTL:(NSTimeInterval)lastTTL series:(iTermTimeSeries *)series {
    BOOL redzone = NO;
