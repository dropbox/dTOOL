    ...
    init?(_ llmMessage: LLM.Message) {
        ...
        case .multipart(let subparts):
            ...
            content = .array(subparts.compactMap({ subpart -> ContentPart? in
                ...
                // BUG-f508: Skip unsupported subpart types instead of crashing
                ...
                case .attachment(let attachment):
                    ...
                    case .file(let file):
                        ...
                        if file.mimeType == "application/pdf" {
                            ...
                            var value = "<dashterm2:attachment file=\"\(file.name)\" type=\"\(file.mimeType)\">\n"
                            value += file.content.lossyString
                            value += "\n</dashterm2:attachment>"
        ...
        case .array(let array):
            return array.map { part -> String in
                ...
                case .text(let text):
                    return text.text
                case .file(let file):
                    var value = "<dashterm2:attachment file=\"\(file.filename)\">\n"
