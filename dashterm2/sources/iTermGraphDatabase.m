...
- (instancetype)initWithDatabase:(id<iTermDatabase>)db {
    ...
    if (self) {
        ...
        _thread =
            [[iTermThread alloc] initWithLabel:@"com.dashterm.dashterm2.graph-db"
                                  stateFactory:^iTermSynchronizedState *_Nonnull(dispatch_queue_t _Nonnull queue) {
                                      return [[iTermGraphDatabaseState alloc] initWithQueue:queue database:db];
                                  }];
...
- (BOOL)reallyFinishInitialization:(iTermGraphDatabaseState *)state {
    ...
    DLogCyclic(_log, @"Loaded ok. Root record: key=%@ id=%@ gen=%@ rowid=%@", self.record.key, self.record.identifier,
               @(self.record.generation), self.record.rowid);
...
- (BOOL)updateSynchronously:(BOOL)sync
    ...
    // BUG-f1069: Replace assert with guard - background thread calls should fail gracefully
    if (![NSThread isMainThread]) {
        DLog(@"BUG-f1069: updateSynchronously called from background thread, failing");
    ...
    // up with two root nodes when a second instance of DashTerm2 just quit and this races against loading.
    [_loadCompletePromise wait];
    DLogCyclic(_log, @"Creating encoder. self.record: key=%@ id=%@ rowid=%@ ptr=%p", self.record.key,
               self.record.identifier, self.record.rowid, self.record);
...
- (void)whenReady:(void (^)(void))readyBlock {
    [_loadCompletePromise onQueue:dispatch_get_main_queue()
                             then:^(id value) {
                                 readyBlock();
                             }];
}
...
- (void)trySaveEncoder:(iTermGraphDeltaEncoder *)originalEncoder
    ...
    @try {
        ...
        _recoveryCount += 1;
        DLogCyclic(_log, @"Starting recovery attempt %@. originalEncoder.record rowid=%@ ptr=%p", @(_recoveryCount),
                   originalEncoder.record.rowid, originalEncoder.record);
        ...
        DLogCyclic(_log, @"Recovery: After eraseRowIDs, encoder.record rowid=%@ (should be nil)", encoder.record.rowid);
        ...
        if (ok) {
            ...
            DLogCyclic(_log, @"Save failed. Clearing self.record (was ptr=%p rowid=%@)", self.record,
                       self.record.rowid);
            self.record = nil;
        }
...
- (BOOL)attemptRecovery:(iTermGraphDatabaseState *)state encoder:(iTermGraphDeltaEncoder *)encoder {
    if (!state.db) {
        return NO;
    }
...
- (BOOL)save:(iTermGraphDeltaEncoder *)encoder state:(iTermGraphDatabaseState *)state {
    DLogCyclic(_log, @"save");
    // BUG-f1070: Replace assert with guard - missing db should return NO not crash
    if (!state.db) {
        DLog(@"BUG-f1070: save called with nil state.db");
        return NO;
    }
...
- (BOOL)reallySave:(iTermGraphDeltaEncoder *)encoder state:(iTermGraphDatabaseState *)state {
    DLog(@"Start saving");
    NSDate *start = [NSDate date];
    const BOOL ok =
        [encoder enumerateRecords:^(iTermEncoderGraphRecord *_Nullable before, iTermEncoderGraphRecord *_Nullable after,
                                    NSNumber *parent, NSString *path, BOOL *stop) {
            if (before && !before.rowid) {
                NSString *reason =
                    [NSString stringWithFormat:@"MissingRowID: Before lacking a rowid! path=%@ before: key=%@ id=%@ "
                                               @"gen=%@ ptr=%p after: key=%@ id=%@ gen=%@ rowid=%@ ptr=%p",
                                               path, before.key, before.identifier, @(before.generation), before,
                                               after.key, after.identifier, @(after.generation), after.rowid, after];
                @throw [NSException exceptionWithName:@"MissingRowID" reason:reason userInfo:nil];
            }
            if (before && !after) {
            ...
            if (!before && after) {
                ...
                // BUG-f1071: Replace assert with guard - missing row ID indicates db error
                if (!lastInsertRowID) {
                    DLog(@"BUG-f1071: lastInsertRowID is nil after insert");
                    *stop = YES;
                    return;
                }
                @try {
                    ...
                    @throw [exception
                        it_rethrowWithMessage:@"after.key=%@ after.identifier=%@", after.key, after.identifier];
                }
                return;
            }
            if (before && after) {
                ...
                if (after.rowid == nil) {
                    ...
                    if (before.generation == after.generation && after.generation != iTermGenerationAlwaysEncode) {
                        DLog(@"Don't update rowid %@ %@[%@] because it is unchanged", before.rowid, after.key,
                             after.identifier);
                        return;
                    }
                }
                // BUG-f1072: Replace assert with guard - rowid mismatch indicates database inconsistency
                if (before.rowid.longLongValue != after.rowid.longLongValue) {
                    DLog(@"BUG-f1072: rowid mismatch before=%@ after=%@ path=%@", before.rowid, after.rowid, path);
                    *stop = YES;
                    return;
                }
            ...
            // BUG-402: Replace assert(NO) with ELog for unexpected state
            // This handles edge case where before XOR after is set (not both, not neither)
            // which indicates a database inconsistency - log and continue rather than crash
            ELog(@"iTermGraphDatabase: Unexpected operation state - before=%@ after=%@ path=%@",
                 before ? @"YES" : @"NO", after ? @"YES" : @"NO", path);
            *stop = YES;
        }];
    NSDate *end = [NSDate date];
    DLogCyclic(_log, @"Save result=%@ duration=%.1fms", @(ok),
               (end.timeIntervalSinceNow - start.timeIntervalSinceNow) * 1000);
    return ok;
}
...
- (BOOL)createTables:(iTermGraphDatabaseState *)state {
    ...
    if (![state.db executeUpdate:@"create table if not exists Node (key text not null, identifier text not null, "
    ...
    [state.db executeUpdate:@"delete from Node where "
                            @"  rowid in ("
...
- (iTermEncoderGraphRecord *_Nullable)load:(iTermGraphDatabaseState *)state error:(out NSError **)error {
    DLog(@"load");
    NSMutableArray<NSArray *> *nodes = [NSMutableArray arrayWithCapacity:256]; // Database node rows
    {
        ...
        while ([rs next]) {
            DLog(@"Read row");
            [nodes addObject:@[
                [rs stringForColumn:@"key"], [rs stringForColumn:@"identifier"], @([rs longLongIntForColumn:@"parent"]),
                @([rs longLongIntForColumn:@"rowid"]), [rs dataForColumn:@"data"] ?: [NSData data]
            ]];
        }
        DLog(@"Select done");
        [rs close];
