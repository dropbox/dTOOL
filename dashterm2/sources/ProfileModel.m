...
+ (BOOL)doesProfile:(Profile *)profile matchFilter:(NSArray *)tokens {
    ...
    NSString *command = @"";
    if (![customCommand isEqualToString:kProfilePreferenceCommandTypeLoginShellValue]) {
        command = [iTermProfilePreferences stringForKey:KEY_COMMAND_LINE inProfile:profile];
    }
    // BUG-f1652: Use NSArray castFrom for type-safe access (KEY_TAGS default is NSNull)
    NSArray *tags = [NSArray castFrom:profile[KEY_TAGS]];
...
{
    // BUG-f1652: Use NSArray castFrom for type-safe access (KEY_TAGS default is NSNull)
    NSArray* tags = [NSArray castFrom:[bookmark objectForKey:KEY_TAGS]];
...
- (void)removeBookmarksAtIndices:(NSArray*)indices
{
    ...
    for (int j = [sorted count] - 1; j >= 0; j--) {
        ...
        // BUG-f1502: Bounds check instead of assert - crash-safe for release builds
        if (i < 0 || i >= (int)[bookmarks_ count]) {
            DLog(@"BUG-f1502: removeBookmarksAtIndices: index %d out of bounds (count=%lu)", i, (unsigned long)[bookmarks_ count]);
...
- (void)removeBookmarkAtIndex:(int)i {
    ...
    // BUG-f1500: Bounds check instead of assert - crash-safe for release builds
    if (i < 0 || i >= (int)[bookmarks_ count]) {
        DLog(@"BUG-f1500: removeBookmarkAtIndex: index %d out of bounds (count=%lu)", i, (unsigned long)[bookmarks_ count]);
        return;
    }
    [journal_ addObject:[BookmarkJournalEntry journalWithAction:JOURNAL_REMOVE bookmark:[bookmarks_ objectAtIndex:i] model:self identifier:nil]];
    [[self debugHistoryForGuid:bookmarks_[i][KEY_GUID]] addObject:[NSString stringWithFormat:@"%@: Remove bookmark with guid %@",
...
- (BOOL)bookmark:(Profile*)a differsJournalablyFrom:(Profile*)b
{
    ...
    NSString *shortcutA = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:a];
    NSString *shortcutB = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:b];
    // BUG-f1652: Use NSArray castFrom for type-safe access (KEY_TAGS default is NSNull)
    NSArray *tagsA = [NSArray castFrom:[a objectForKey:KEY_TAGS]];
    NSArray *tagsB = [NSArray castFrom:[b objectForKey:KEY_TAGS]];
    if (![[a objectForKey:KEY_NAME] isEqualToString:[b objectForKey:KEY_NAME]] ||
        ![shortcutA isEqualToString:shortcutB] ||
        ![tagsA isEqualToArray:tagsB] ||
...
- (void)setBookmark:(Profile*)bookmark atIndex:(int)i sideEffects:(BOOL)sideEffects {
    // BUG-f1501: Bounds check before access - prevent crash on invalid index
    if (i < 0 || i >= (int)[bookmarks_ count]) {
        DLog(@"BUG-f1501: setBookmark:atIndex: index %d out of bounds (count=%lu)", i, (unsigned long)[bookmarks_ count]);
        return;
    }
...
- (NSArray*)allTags
{
    ...
    for (Profile *profile in bookmarks_) {
        // BUG-f1652: Use NSArray castFrom for type-safe access (KEY_TAGS default is NSNull)
        NSArray *profileTags = [NSArray castFrom:[profile objectForKey:KEY_TAGS]];
        for (NSString *tag in profileTags) {
            [tags addObject:tag];
        }
    }
