...
struct Plugin {
    ...
    private let bundleID = "com.dashterm.dashterm2.iTermAI"
    ...
    private static func checkSignature(message: Data, signature: Data) throws {
        // BUG-1787: Use guard let instead of force unwrap for base64 decoding
        guard let publicKeyData = Data(base64Encoded: publicKeyB64) else {
            throw PluginError(reason: "Invalid plugin public key encoding")
        }
        let publicKey = try Curve25519.Signing.PublicKey(rawRepresentation: publicKeyData)
        guard publicKey.isValidSignature(signature, for: message) else {
            throw PluginError(reason: "The plugin's signature is invalid. Reinstall the plugin or upgrade DashTerm2.")
        }
...
class iTermAIClient {
    private let executionQueue = DispatchQueue(label: "com.dashterm.dashterm2.ai-execution")
    private let outputQueue = DispatchQueue(label: "com.dashterm.dashterm2.ai-output")
    ...
    func validate() throws {
        ...
        case .success(let plugin):
            guard let pluginVersion = try? plugin.version() else {
                throw PluginError(reason: "Unable to determine version of AI plugin. Reinstall it and upgrade DashTerm2 if possible.")
            }

            guard pluginVersion == Decimal(string: requiredVersion) else {
                throw PluginError(reason: "Plugin has version \(pluginVersion) but DashTerm2 expects \(requiredVersion). Upgrade one or both.")
            }
            return
        case .failure(let error):
            DLog("\(error)")
