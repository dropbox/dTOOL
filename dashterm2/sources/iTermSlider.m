...
- (instancetype)init {
    // BUG-422: Replace assert(NO) with redirect to designated initializer
    // -init should not be used directly; use initWithFrame: or initWithCoder: instead
    // Rather than crashing, redirect to initWithFrame: with a zero frame
    NSLog(@"iTermSlider: -init called directly - use initWithFrame: or initWithCoder: instead");
...
- (void)dealloc {
    // Remove KVO observers to prevent crashes from deallocated observer
    if (_slider) {
