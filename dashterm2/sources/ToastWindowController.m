...
- (void)dealloc {
    // BUG-4096: Invalidate timer to prevent callbacks to deallocated object
    [hideTimer_ invalidate];
    // Cancel pending performSelector:afterDelay: calls
    [NSObject cancelPreviousPerformRequestsWithTarget:visibleToast selector:@selector(removeObject:) object:self];
}

+ (void)initialize
{
    visibleToast = [[NSMutableArray alloc] initWithCapacity:4];  // Visible toast notifications
}
