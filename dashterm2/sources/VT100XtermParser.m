...
// Optimization: Cache NSNumber objects for common OSC mode values.
// The range -2 to 134 covers all standard modes (0, 1, 2, 4, 6-12, 17, 19, 22, 50, 52, 104,
// 110-112, 117, 119, 133, 134) plus the special kAPCMode (-2) and kLinuxSetPaletteMode (-1).
// Mode 1337 (DashTerm2 proprietary) is the only mode outside this range and falls back to boxing.
static const int kModeNumberCacheMin = -2;
static const int kModeNumberCacheMax = 134;
static const int kModeNumberCacheSize = kModeNumberCacheMax - kModeNumberCacheMin + 1;
static NSNumber *sModeNumberCache[137]; // Size is kModeNumberCacheSize = 137

NS_INLINE NSNumber *iTermXtermModeToNumber(int mode) {
    if (mode >= kModeNumberCacheMin && mode <= kModeNumberCacheMax) {
        return sModeNumberCache[mode - kModeNumberCacheMin];
    }
    return @(mode);
}

__attribute__((constructor)) static void iTermVT100XtermParserInitializeModeCache(void) {
    for (int i = 0; i < kModeNumberCacheSize; i++) {
        sModeNumberCache[i] = @(i + kModeNumberCacheMin);
    }
}
            ...
            switch (c) {
                ...
                case ':':
                    if ((mode == 50 || mode == 1337) &&
                        ([data hasPrefixOfBytes:"File=" length:5] || [data hasPrefixOfBytes:"Copy=" length:5])) {
...
+ (VT100TerminalTokenType)tokenTypeForMode:(int)mode {
    ...
    // Optimization: Use cached NSNumber for mode lookup to avoid boxing on every call.
    NSNumber *enumNumber = theMap[iTermXtermModeToNumber(mode)];
    if (enumNumber) {
...
+ (void)emitIncidentalForSetKvpHeaderInVector:(CVector *)vector
    ...
    // BUG-10119: Validate encoding success before emitting token.
    NSString *str = [[[NSString alloc] initWithData:data encoding:encoding] autorelease];
    if (str == nil) {
        DLog(@"BUG-10119: Failed to decode multitoken header data with encoding %lu", (unsigned long)encoding);
        return;
    }
    VT100Token *headerToken = [[VT100TokenPool sharedPool] acquireToken];
    headerToken->type = XTERMCC_MULTITOKEN_HEADER_SET_KVP;
    headerToken.string = str;
...
+ (void)emitIncidentalForMultitokenBodyInVector:(CVector *)vector
    ...
    NSString *str = [[[NSString alloc] initWithData:data encoding:encoding] autorelease];
    if (str == nil) {
        DLog(@"BUG-10119: Failed to decode multitoken body data with encoding %lu", (unsigned long)encoding);
        return;
    }
...
+ (void)decodeFromContext:(iTermParserContext *)context
    ...
    // Initialize the state; 256 bytes is typical for xterm escape sequences
    ...
    if (savedState.count) {
        ...
        iTermParserAdvanceMultiple(context, [savedState[kXtermParserSavedStateBytesUsedKey] intValue]);
        ...
        BOOL apc = NO;
        if (support8BitControlCharacters && (peek == VT100CC_C1_OSC || peek == VT100CC_C1_APC)) {
    ...
    while (1) {
        ...
        switch (state) {
            ...
            case kXtermParserHeaderEndState:
                ...
                // BUG-f1382: Convert assert to guard - unexpected mode should transition to failure
                if ([self tokenTypeForMode:mode] != XTERMCC_SET_KVP) {
                    state = kXtermParserFailingState;
                    break;
                }
                DLog(@"Emitting header incidental with data=%@", data);
                [self emitIncidentalForSetKvpHeaderInVector:incidentals data:data encoding:encoding];
            ...
            case kXtermParserFinishedState:
                if (multitokenHeaderEmitted) {
                    if (data.length) {
                        ...
                        [self emitIncidentalForMultitokenBodyInVector:incidentals data:data encoding:encoding];
                        [data setLength:0];
                    }
                    DLog(@"Multitoken complete, returning MULTITOKEN_END");
                    result->type = XTERMCC_MULTITOKEN_END;
                } else {
                    // BUG-10122: initWithData:encoding: may return nil on invalid encoding.
                    // Handle nil gracefully by skipping the token instead of crashing downstream.
                    NSString *str = [[[NSString alloc] initWithData:data encoding:encoding] autorelease];
                    if (str == nil) {
                        DLog(@"BUG-10122: String conversion failed for data %@ with encoding %lu", data,
