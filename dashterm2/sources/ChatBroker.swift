...
class ChatBroker {
    ...
    nonisolated(unsafe) private static var _instance: ChatBroker?
    ...
    func publish(message: Message, toChatID chatID: String, partial: Bool) throws {
        ...
        // BUG-1005: Copy subs under lock to avoid data race during iteration
        let subsCopy: [Subscription]
        subsLock.lock()
        subsCopy = subs
        subsLock.unlock()
        for sub in subsCopy {
    ...
    func subscribe(chatID: String?,
        ...
        let subscription = Subscription(chatID: chatID,
            ...
            // BUG-1005: Lock subs during mutation to avoid data race
            guard let self else { return }
            self.subsLock.lock()
            self.subs.removeAll(where: { $0 === sub })
            self.subsLock.unlock()
        }
        // BUG-1005: Lock subs during mutation to avoid data race
        subsLock.lock()
