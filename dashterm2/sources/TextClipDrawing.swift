...
class TextClipDrawing: NSObject {
    ...
    static func drawClip(drawingHelper: iTermTextDrawingHelper,
        ...
        SavedState.perform(drawingHelper) {
            guard let instance = TextClipDrawing(drawingHelper: drawingHelper,
    ...
    private init?(drawingHelper: iTermTextDrawingHelper,
        ...
        guard let delegate = drawingHelper.delegate else {
            DLog("TextClipDrawing init failed: drawingHelper.delegate is nil")
            return nil
        }
...
extension TextClipDrawing: iTermTextDrawingHelperDelegate {
    ...
    func drawingHelperAnnotations(in range: VT100GridCoordRange) -> [PTYAnnotationReading]? {
        return nil
    }

    func drawingHelperCoordRange(forAnnotation annotation: PTYAnnotationReading) -> VT100GridCoordRange {
