...
- (void)updateTouchBarIfNeeded:(BOOL)force {
    ...
    if (IsTouchBarAvailable()) {
        ...
        if (force || ![replacement.customizationIdentifier isEqualToString:self.touchBar.customizationIdentifier]) {
            ...
            // BUG-3836: Capture scrubber and use weak self to prevent crash if window deallocated
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [scrubber reloadData];
                NSUInteger index = [strongSelf.tabs indexOfObject:strongSelf.currentTab];
                if (index != NSNotFound) {
                    [scrubber setSelectedIndex:index];
                }
            });
        }
...
// version of DashTerm2. Backward compatibility is preserved because the code to
...
- (NSNumber *)versionFor:(NSString *)identifier withValue:(NSDictionary *)binding {
    // BUG-f1373: Convert assert to guard - return nil for nil binding instead of crash
    if (!binding) {
        return nil;
    }
    ...
    dispatch_once(&onceToken, ^{
        table = [[NSMutableDictionary alloc] initWithCapacity:16];  // Touch bar binding versions
    });
...
- (nullable NSTouchBarItem *)touchBar:(NSTouchBar *)touchBar makeItemForIdentifier:(NSTouchBarItemIdentifier)identifier {
    if ([identifier isEqualToString:iTermTabBarTouchBarIdentifier]) {
        ...
        // tabs.
        // BUG-3837: Use weak self to prevent race condition crash if window/tabs deallocated
        __weak __typeof(self) weakSelf = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong __typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
...
- (void)scrubber:(NSScrubber *)scrubber didSelectItemAtIndex:(NSInteger)selectedIndex {
    // BUG-3838: Add bounds check to prevent index out of bounds crash
...
- (NSString *)scrubber:(NSScrubber *)scrubber labelAtIndex:(NSInteger)index {
    // BUG-3838: Use consistent data source (tabs) and add proper bounds check
    NSArray<PTYTab *> *tabs = self.tabs;
    if (index < 0 || (NSUInteger)index >= tabs.count) {
        return @"";
    }
    PTYTab *tab = tabs[index];
