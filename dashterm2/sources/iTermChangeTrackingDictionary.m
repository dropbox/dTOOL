...
- (instancetype)init {
    self = [super init];
    if (self) {
        _impl = [[NSMutableDictionary alloc] initWithCapacity:32];
        _generations = [[NSMutableDictionary alloc] initWithCapacity:32];
    }
    return self;
}
    ...
    [encoder encodeChildWithKey:@"changeTrackingDictionary"
                     ...
                     generation:_generation
                          block:^BOOL(iTermGraphEncoder *_Nonnull subencoder) {
                              [_impl enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                                                         BOOL *_Nonnull stop) {
...
- (void)loadFromRecord:(iTermEncoderGraphRecord *)record keyClass:(Class)keyClass valueClass:(Class)valueClass {
    ...
    [root.index enumerateKeysAndObjectsUsingBlock:^(iTermTuple<NSString *, NSString *> *_Nonnull key,
                                                    iTermEncoderGraphRecord *_Nonnull subrecord, BOOL *_Nonnull stop) {
