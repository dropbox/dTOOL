...
extension PTYTextView: ExternalSearchResultsController {
    ...
    func text(inRange range: VT100GridCoordRange) -> String {
        ...
        // BUG-1711: Use as? with empty string fallback instead of as! force cast
    ...
    private func replace(range absRange: VT100GridAbsCoordRange,
        ...
        // BUG-1712: Use compactMap with as? instead of map with as! force cast
        porthole.savedLines = (relativeRange.start.y ... relativeRange.end.y).compactMap { i in
    ...
    func addPorthole(_ objcPorthole: ObjCPorthole) {
        ...
        // BUG-1713: Use guard with as? instead of as! force cast
        guard let porthole = objcPorthole as? Porthole else {
            DLog("Failed to cast ObjCPorthole to Porthole")
            return
        }
    ...
    func hidePorthole(_ objcPorthole: ObjCPorthole) {
        ...
        // BUG-1714: Use guard with as? instead of as! force cast
        guard let porthole = objcPorthole as? Porthole else {
            DLog("Failed to cast ObjCPorthole to Porthole")
            return
        }
    ...
    func unhidePorthole(_ objcPorthole: ObjCPorthole) {
        ...
        // BUG-1715: Use guard with as? instead of as! force cast
        guard let porthole = objcPorthole as? Porthole else {
            DLog("Failed to cast ObjCPorthole to Porthole")
            return
        }
        // BUG-f501: Use guard clauses instead of it_assert to prevent crash
        guard portholes.contains(porthole) else {
            DLog("unhidePorthole: Porthole not in portholes set, ignoring")
            return
        }
        guard porthole.view.superview != self else {
            DLog("unhidePorthole: Porthole view already in this view, ignoring")
            return
        }
        addPortholeView(porthole)
    }
    ...
    func removePorthole(_ objcPorthole: ObjCPorthole) {
        ...
        // BUG-1716: Use guard with as? instead of as! force cast
        guard let porthole = objcPorthole as? Porthole else {
            DLog("Failed to cast ObjCPorthole to Porthole")
            return
        }
    ...
    func updatePortholeFrames() {
        ...
        // BUG-1717: Use compactMap with as? instead of as! force cast in loop
        for porthole in portholes.compactMap({ $0 as? Porthole }) {
            updatePortholeFrame(porthole, force: false)
        }
    ...
    func stringForPorthole(range: VT100GridCoordRange) -> String? {
        // BUG-1718: Use compactMap with as? instead of as! force cast in loop
        for porthole in portholes.compactMap({ $0 as? Porthole }) {
    ...
    // BUG-1719: Changed parameter type to Porthole directly instead of using as! force cast
    private func updatePortholeFrame(_ porthole: Porthole, force: Bool) {
        DLog("updatePortholeFrame(\(porthole)")
        guard let dataSource = dataSource else {
            DLog("nil datasource")
            return
        }
    ...
    var hasPortholes: Bool {
        return portholes.count > 0  // swiftlint:disable:this empty_count
    }

    // Because Swift can't cope with forward declarations and I don't want a dependency cycle.
    // BUG-1720: Use compactMap with as? instead of as! force cast
    private var typedPortholes: [Porthole] {
        return portholes.compactMap { $0 as? Porthole }
    }
...
extension PTYTextView: NSViewContentSelectionInfo {
    ...
    func rect(for range: VT100GridAbsCoordRange) -> NSRect {
        ...
        // BUG-1577: Use guard to avoid crash if rects is somehow empty
        guard let initialRect = rects.first else {
            return .null
        }
        return rects.reduce(into: initialRect) { partialResult, rect in
...
extension PTYTextView {
    ...
    func saveArchive(_ sender: Any) {
        if let window = self.window {
            ...
            savePanel.defaultFilename = ((delegate?.textViewVariablesScope().value(forVariableName: iTermVariableKeySessionName) as? String) ?? "DashTerm2 Session") + ".itermarchive"
            savePanel.preferredSSHIdentity = .localhost
            // BUG-1789: Use if let with fallback instead of force unwrap
            if let archiveType = UTType(filenameExtension: "itermarchive") {
