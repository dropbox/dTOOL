...
+ (NSDictionary *)readPresetKeyMappingsFromPlist:(NSString *)thePlist {
    NSString *plistFile = [[NSBundle bundleForClass:[self class]] pathForResource:thePlist ofType:@"plist"];
...
+ (NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *)keystrokeTuplesInAllPresets {
    NSMutableArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *result =
        [NSMutableArray arrayWithCapacity:64]; // All preset keystroke tuples
    ...
    for (NSString *name in builtins) {
        ...
        [dict enumerateKeysAndObjectsUsingBlock:^(id _Nonnull serializedKeystroke, NSDictionary *_Nonnull mapping,
                                                  BOOL *_Nonnull stop) {
...
+ (void)setGlobalKeyMappingsToPreset:(NSString *)presetName byReplacingAll:(BOOL)replaceAll {
    // BUG-f1130: Replace assert with guard - only process factory defaults preset
    if (![presetName isEqualToString:kFactoryDefaultsGlobalPreset]) {
        DLog(@"Warning: setGlobalKeyMappingsToPreset: called with unsupported preset '%@', expected '%@'", presetName,
             kFactoryDefaultsGlobalPreset);
        return;
    }
    if (!replaceAll) {
