...
static void APSWriteToScriptHistory(id<iTermAutomaticProfileSwitcherDelegate> delegate, NSString *format, ...) {
...
- (instancetype)initWithDelegate:(id<iTermAutomaticProfileSwitcherDelegate>)delegate {
    self = [super init];
    if (self) {
        _profileStack = [[NSMutableArray alloc] initWithCapacity:8]; // Profile switch stack
        _delegate = delegate;
        [self commonInit];
    }
    return self;
...
- (void)setHostname:(nullable NSString *)hostname
    ...
    APSLog(@"APS: Updating configuration to hostname=%@, username=%@, path=%@, job=%@, commandLine=%@", hostname,
           username, path, job, commandLine);
    if (_switching) {
        APSLog(@"Already switching so ignore the change");
        return;
    }
    NSString *guid = [self.delegate automaticProfileSwitcherCurrentProfile][KEY_GUID];
    if (!_dirty && [hostname isEqualToString:_lastHostname] && [username isEqualToString:_lastUsername] &&
        [path isEqualToString:_lastPath] && [job isEqualToString:_lastJob] &&
        [commandLine isEqualToString:_lastCommandLine] && [_lastProfileGUID isEqualToString:guid]) {
    ...
    APSLog(@"The current profile is %@ with a score of %0.2f. The highest-ranking profile in the stack is %@, with "
           @"score of %0.2f",
           currentProfile[KEY_NAME], scoreForCurrentProfile, topmostMatchingSavedProfile.profile[KEY_NAME],
           scoreForTopmostMatchingSavedProfile);

    if (topmostMatchingSavedProfile && ![topmostMatchingSavedProfile.originalProfile isEqualToProfile:currentProfile] &&
        scoreForTopmostMatchingSavedProfile > scoreForCurrentProfile) {
        APSLog(@"Profile %@ is in the stack and outranks the current profile. Will switch, but first, remove it and "
               @"subsequent entries from the stack.",
        ...
        APSLog(@"No profile in the stack outranks the current profile. Check if any profile not in the stack outranks "
               @"the current profile.");
        ...
        APSLog(@"The highest scoring profile is %@ with a score of %@", highestScoringProfile[KEY_NAME],
...
- (void)changeToProfile:(iTermSavedProfile *)profile {
    // BUG-f902: Replace assert with guard - prevent re-entrancy but don't crash
    if (_switching) {
        ELog(@"BUG-f902: changeToProfile called while already switching - ignoring to prevent recursion");
        return;
    }
    _switching = YES;
...
- (NSDictionary *)savedState {
    // Stack depth matches _profileStack, typically 1-5 levels
    NSMutableArray *stack = [NSMutableArray arrayWithCapacity:_profileStack.count];
...
- (NSString *)description {
    return
        [NSString stringWithFormat:@"<%@: %p stack=%@>", NSStringFromClass(self.class), self, self.profileStackString];
}

- (NSString *)profileStackString {
    // Estimate ~20 chars per profile name
    NSMutableString *temp = [NSMutableString stringWithCapacity:_profileStack.count * 20];
    for (iTermSavedProfile *savedProfile in _profileStack) {
...
- (nullable Profile *)highestScoringProfileForHostname:(NSString *)hostname
    ...
    // Pre-size based on profile count; each profile can have multiple rules
    NSArray *allProfiles = [_delegate automaticProfileSwitcherAllProfiles];
    NSMutableDictionary<NSString *, Profile *> *ruleToProfileMap =
        [NSMutableDictionary dictionaryWithCapacity:allProfiles.count * 2];
