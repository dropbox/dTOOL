...
static os_unfair_lock gCursorsLock = OS_UNFAIR_LOCK_INIT;

+ (instancetype)mouseCursorOfType:(iTermMouseCursorType)cursorType {
    static NSMutableDictionary *cursors;
    os_unfair_lock_lock(&gCursorsLock);
    if (!cursors) {
        cursors = [[NSMutableDictionary alloc] initWithCapacity:8];  // Mouse cursor cache
    }
    if (!cursors[@(cursorType)]) {
        cursors[@(cursorType)] = [[[self alloc] initWithType:cursorType] autorelease];
    }
    iTermMouseCursor *result = cursors[@(cursorType)];
    os_unfair_lock_unlock(&gCursorsLock);
    return result;
}

- (NSImage *)catalinaIBeamWithCircleImage {
    NSImage *ibeam = [[NSCursor IBeamCursor] image];
    NSImage *overlay = [NSImage it_imageNamed:@"MouseReportingCursorOverlay" forClass:self.class];
    ...
    return [NSImage imageOfSize:size
                      drawBlock:^{
                          ...
                          NSRect dest = NSMakeRect((ibeam.size.width - overlay.size.width) / 2.0,
                                                   (ibeam.size.height - overlay.size.height) / 2.0, overlay.size.width,
                                                   overlay.size.height);
                          [overlay drawInRect:dest
                                     fromRect:NSZeroRect
...
- (instancetype)initWithType:(iTermMouseCursorType)cursorType {
    switch (cursorType) {
        case iTermMouseCursorTypeIBeamWithCircle: {
            ...
            self = [super initWithImage:image hotSpot:NSMakePoint(size.width / 2.0, size.height / 2.0)];
            if (self) {
                _hasImage = YES;
            }
            break;
