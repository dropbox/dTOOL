...
- (instancetype)initWithCharacter:(NSString *)string
    ...
    // BUG-f1159: Convert assert() to guards - invalid descriptor should return nil, not crash
    if (descriptor.glyphSize.width <= 0) {
        DLog(@"BUG-f1159: iTermRegularCharacterSource init with invalid width: %f", descriptor.glyphSize.width);
        return nil;
    }
    if (descriptor.glyphSize.height <= 0) {
        DLog(@"BUG-f1159: iTermRegularCharacterSource init with invalid height: %f", descriptor.glyphSize.height);
        return nil;
    }
    if (descriptor.scale <= 0) {
        DLog(@"BUG-f1159: iTermRegularCharacterSource init with invalid scale: %f", descriptor.scale);
        return nil;
    }
    ...
    if (self) {
        ...
        // Defense in depth: if fontTable becomes nil during app termination, return nil gracefully
        // instead of crashing. The caller handles nil sources safely.
        if (descriptor.fontTable == nil) {
            DLog(@"Nil font table for string=%@ attributes=%@ - returning nil during teardown", string, attributes);
            return nil;
        }
...
- (void)drawBoxInContext:(CGContextRef)context iteration:(int)iteration {
    // BUG-f1160: Convert assert() to guard - nil context should return early, not crash
    if (!context) {
        DLog(@"BUG-f1160: drawBoxInContext called with nil context");
        return;
    }
