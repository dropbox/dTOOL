...
+ (CGFloat)kernForString:(NSString *)string toHaveWidth:(CGFloat)desiredWidth withFont:(NSFont *)font {
...
+ (instancetype)attributedStringWithLinkToURL:(NSString *)urlString string:(NSString *)string {
    ...
    return [[NSAttributedString alloc] initWithString:string attributes:linkAttributes];
}
...
- (CGFloat)heightForWidth:(CGFloat)maxWidth {
    ...
    const CGFloat extraLineFragmentHeight = [layoutManager extraLineFragmentRect].size.height;
...
- (NSArray *)attributedComponentsSeparatedByString:(NSString *)separator {
    ...
    do {
        range = [string rangeOfString:separator options:0 range:nextRange];
    ...
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:indices.count + 1];
...
- (NSAttributedString *)attributedStringByRemovingColor {
    ...
    [self enumerateAttributesInRange:NSMakeRange(0, self.length)
                             options:0
                          usingBlock:^(NSDictionary<NSAttributedStringKey, id> *_Nonnull attrs, NSRange range,
                                       BOOL *_Nonnull stop) {
                              if (attrs[NSAttachmentAttributeName]) {
                                  ...
                                  [result appendAttributedString:[NSAttributedString
                                                                     attributedStringWithAttachment:replacement]];
                                  return;
                              }
