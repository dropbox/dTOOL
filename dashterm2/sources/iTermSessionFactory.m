...
+ (instancetype)launchRequestWithSession:(PTYSession *)aSession
    ...
    request->_name = [[iTermProfilePreferences stringForKey:KEY_NAME inProfile:request.profile] copy];
    NSString *customDirectory = [iTermProfilePreferences stringForKey:KEY_CUSTOM_DIRECTORY inProfile:request.profile];
    if ([customDirectory isEqualToString:@"Yes"]) {
        request->_customWorkingDirectory = [[iTermProfilePreferences stringForKey:KEY_WORKING_DIRECTORY inProfile:request.profile] copy];
    }
    return request;
}
...
- (PTYSession *)newSessionWithProfile:(Profile *)profile
    ...
    // BUG-f1176: Replace assert() with guard - nil profile should return nil, not crash
    if (!profile) {
        DLog(@"WARNING: newSessionWithProfile called with nil profile, returning nil");
        return nil;
    }
    PTYSession *aSession;
...
- (void)attachOrLaunchWithRequest:(iTermSessionAttachOrLaunchRequest *)request {
    DLog(@"attachOrLaunchWithRequest: entered, session=%@", request.session);
    request.delegate = self;
    [request realizeWithCompletion:^(BOOL realized) {
        DLog(@"attachOrLaunchWithRequest: realizeWithCompletion callback, realized=%@", @(realized));
        if (!realized) {
            DLog(@"attachOrLaunchWithRequest: REALIZATION FAILED");
            return;
        }
        DLog(@"attachOrLaunchWithRequest: calling handleRealizedRequest");
        [self handleRealizedRequest:request
                         completion:^(BOOL ok) {
            DLog(@"attachOrLaunchWithRequest: handleRealizedRequest completed ok=%@", @(ok));
...
- (void)handleRealizedRequest:(iTermSessionAttachOrLaunchRequest *)request
    ...
    DLog(@"handleRealizedRequest: entered, request=%@", request);

    if (request.hasServerConnection) {
        // Attach to running server, if possible.
        // BUG-f1177: Replace assert() with guard - server connection without runJobsInServers should fail gracefully
        if (![iTermAdvancedSettingsModel runJobsInServers]) {
            DLog(@"ERROR: hasServerConnection is true but runJobsInServers is false, failing request");
            completion(NO);
            return;
        }
...
- (void)startProgramForRequest:(iTermSessionAttachOrLaunchRequest *)request
                    completion:(void (^)(BOOL))completion {
    DLog(@"startProgramForRequest: calling startProgram with command=%@", request.computedCommand);
