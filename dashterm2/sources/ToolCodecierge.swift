...
class ToolCodecierge: NSView, ToolbeltTool {
    ...
    private class SessionRegistry {
        static let instance = SessionRegistry()
        ...
        func session(forKey key: String) -> Session? {
            sessionsLock.withLock { sessions[key] }
        }

        func setSession(_ session: Session, forKey key: String) {
            sessionsLock.withLock { sessions[key] = session }
        }

        func removeSession(forKey key: String) {
            _ = sessionsLock.withLock { sessions.removeValue(forKey: key) }
        }

        func updateSession(forKey key: String, update: (Session?) -> Void) {
            sessionsLock.withLock { update(sessions[key]) }
        }
    }
    ...
    fileprivate class Session {
        ...
        var goal: String? {
            didSet {
                ...
                if let window = iTermController.sharedInstance().windowForSession(withGUID: guid)?.window(), let goal {
                    ...
                    guard let format = ghostRiding
                        ? iTermAdvancedSettingsModel.codeciergeGhostRidingPrompt()
                        : iTermAdvancedSettingsModel.codeciergeRegularPrompt() else {
                        DLog("codecierge prompt preference is nil")
                        return
                    }
                    let content = format
        ...
        private var hasCommandExitSnapshotSubscription = false

        init(_ guid: String) {
            self.guid = guid
            registerForCommandExitSnapshots()
        ...
        deinit {
            unregisterForCommandExitSnapshots()
            if let observer {
                NotificationCenter.default.removeObserver(observer)
            }
        }

        private func registerForCommandExitSnapshots() {
            guard !hasCommandExitSnapshotSubscription,
                  let session = iTermController.sharedInstance().session(withGUID: guid) else {
                return
            }
            session.addCommandExitSnapshotSubscriber(self)
            hasCommandExitSnapshotSubscription = true
        }

        private func unregisterForCommandExitSnapshots() {
            guard hasCommandExitSnapshotSubscription else {
                return
            }
            if let session = iTermController.sharedInstance().session(withGUID: guid) {
                session.removeCommandExitSnapshotSubscriber(self)
            }
            hasCommandExitSnapshotSubscription = false
        }
    ...
    required init!(frame: NSRect, url: URL!, identifier: String!) {
        // BUG-f806: Return nil instead of crashing for unused URL-based initializer
        DLog("ToolCodecierge URL-based init is not supported")
        return nil
    }
    ...
    required init?(coder: NSCoder) {
        // BUG-f807: Return nil instead of crashing for unused coder initializer
        DLog("CodeciergeOnboardingView init(coder:) is not supported")
        return nil
    }
    ...
    private func currentSession() -> Session? {
        ...
        if let session = SessionRegistry.instance.session(forKey: guid) {
        ...
        SessionRegistry.instance.setSession(session, forKey: guid)
        return session
    }
    ...
    private func setSuggestion(guid: String, suggestion: String) {
        SessionRegistry.instance.updateSession(forKey: guid) { session in
    ...
    private func appendToSuggestion(guid: String, suggestion: String) {
        if let existing = SessionRegistry.instance.session(forKey: guid)?.suggestion {
...
class CodeciergeOnboardingView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f808: Return nil instead of crashing for unused coder initializer
        DLog("CodeciergeGoalSettingView init(coder:) is not supported")
        return nil
    }
...
class CodeciergeGoalView: NSView, NSTextViewDelegate, NSControlTextEditingDelegate {
    ...
    required init?(coder: NSCoder) {
        // BUG-f809: Return nil instead of crashing for unused coder initializer
        DLog("CodeciergeWaitingView init(coder:) is not supported")
        return nil
    }
    ...
    private func layoutTextview() {
        ...
        let textBounds = textView.layoutManager?.usedRect(for: textContainer)
...
class CodeciergeSuggestionView: NSView, NSTextFieldDelegate {
    ...
    required init?(coder: NSCoder) {
        // BUG-f810: Return nil instead of crashing for unused coder initializer
        DLog("CodeciergeQuestionView init(coder:) is not supported")
        return nil
    }
...
extension NSMutableAttributedString {
    func insertButton(withImage dynamicImage: DynamicImage, at index: Int, action: @escaping (NSPoint) -> Void) {
        ...
        // BUG-507: Guard against division by zero when image height is zero
        let aspectRatio = dynamicImage.image.size.height > 0
            ? dynamicImage.image.size.width / dynamicImage.image.size.height
            : 1.0
...
class ClickableTextView: NSTextView, NSTextStorageDelegate {
    ...
    required init?(coder: NSCoder) {
        // BUG-f811: Return nil instead of crashing for unused coder initializer
        DLog("CodeciergeStatusView init(coder:) is not supported")
        return nil
    }
    ...
    private func linkURLAt(event: NSEvent) -> URL? {
        ...
        if let textStorage = self.textStorage,
           ...
           index < textStorage.string.count {
            ...
            if let url = attributes[.link] as? URL, !urlHasDisallowedScheme(url) {
                return url
            }
            if let string = attributes[.link] as? String,
               let url = URL(string: string),
               !urlHasDisallowedScheme(url) {
                return url
            }
        }
        return nil
    ...
    private func retint() {
        ...
        textStorage.enumerateAttribute(
            ...
            in: NSRange(location: 0, length: textStorage.string.count)) { value, range, stopPtr in
                guard let dynamicImage = value as? DynamicImage else { return }
...
class DynamicImage {
    ...
    let dark: NSColor
    let light: NSColor
    private static let darkCacheKey = NSString("DynamicImage.darkCacheKey")
    private static let lightCacheKey = NSString("DynamicImage.lightCacheKey")
