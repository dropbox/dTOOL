...
// Static lock for TmuxGateway class-level synchronization of _tmuxMode
static os_unfair_lock sTmuxModeLock = OS_UNFAIR_LOCK_INIT;
...
static NSString *const __attribute__((unused)) DEPRECATED_SESSION_ARRANGEMENT_DEFAULT_NAME_DEPRECATED =
    @"Session Default Name"; // manually set name
static NSString *const __attribute__((unused)) DEPRECATED_SESSION_ARRANGEMENT_WINDOW_TITLE_DEPRECATED =
    @"Session Window Title"; // server-set window name
static NSString *const __attribute__((unused)) DEPRECATED_SESSION_ARRANGEMENT_NAME_DEPRECATED =
    @"Session Name";                                               // server-set "icon" (tab) name
static NSString *const SESSION_ARRANGEMENT_GUID = @"Session GUID"; // A truly unique ID.
static NSString *const SESSION_ARRANGEMENT_LIVE_SESSION =
    @"Live Session"; // If zoomed, this gives the "live" session's arrangement.
static NSString *const SESSION_ARRANGEMENT_SUBSTITUTIONS = @"Substitutions"; // Dictionary for $$VAR$$ substitutions
static NSString *const SESSION_UNIQUE_ID =
    @"Session Unique ID"; // DEPRECATED. A string used for restoring soft-terminated sessions for arrangements that
                          // predate the introduction of the GUID.
static NSString *const SESSION_ARRANGEMENT_SERVER_PID =
    @"Server PID"; // PID for server process for restoration. Only for monoserver.
// Not static because the ARC category uses it.
NSString *const SESSION_ARRANGEMENT_SERVER_DICT =
    @"Server Dict"; // NSDictionary. Describes server connection. Only for multiserver.
// TODO: Make server report the TTY to us since orphans will end up with a nil tty.
static NSString *const SESSION_ARRANGEMENT_TTY =
    @"TTY"; // TTY name. Used when using restoration to connect to a restored server.
...
// Deprecated in favor of SESSION_ARRANGEMENT_SHOULD_EXPECT_PROMPT_MARKS and
// SESSION_ARRANGEMENT_SHOULD_EXPECT_CURRENT_DIR_UPDATES
static NSString *const SESSION_ARRANGEMENT_SHELL_INTEGRATION_EVER_USED_DEPRECATED =
    @"Shell Integration Ever Used"; // BOOL
...
static NSString *const SESSION_ARRANGEMENT_SHOULD_EXPECT_CURRENT_DIR_UPDATES =
    @"Should Expect Current Dir Updates"; // BOOL

static NSString *const SESSION_ARRANGEMENT_WORKING_DIRECTORY_POLLER_DISABLED =
    @"Working Directory Poller Disabled";                                                  // BOOL
...
static NSString *const SESSION_ARRANGEMENT_LAST_DIRECTORY_IS_UNSUITABLE_FOR_OLD_PWD_DEPRECATED =
    @"Last Directory Is Remote";                                                 // BOOL
...
static NSString *const SESSION_ARRANGEMENT_ENVIRONMENT =
    @"Environment"; // Dictionary of environment vars program was run in
static NSString *const SESSION_ARRANGEMENT_KEYLABELS = @"Key Labels"; // Dictionary string -> string
static NSString *const SESSION_ARRANGEMENT_KEYLABELS_STACK =
    @"Key Labels Stack";                                               // Array of encoded iTermKeyLables dicts
...
static NSString *const SESSION_ARRANGEMENT_FONT_OVERRIDES =
    @"Font Overrides"; // Not saved; just used internally when creating a new tmux session.
static NSString *const SESSION_ARRANGEMENT_KEYBOARD_MAP_OVERRIDES =
    @"Keyboard Map Overrides"; // Not saved; just used internally when creating a new tmux session.
static NSString *const SESSION_ARRANGEMENT_SHORT_LIVED_SINGLE_USE = @"Short Lived Single Use"; // BOOL
static NSString *const SESSION_ARRANGEMENT_HOSTNAME_TO_SHELL =
    @"Hostname to Shell"; // NSString -> NSString (example: example.com -> fish)
static NSString *const SESSION_ARRANGEMENT_CURSOR_TYPE_OVERRIDE =
    @"Cursor Type Override";                                                        // NSNumber wrapping ITermCursorType
...
typedef NS_ENUM(NSUInteger, PTYSessionTurdType) { PTYSessionTurdTypeMouseReporting, PTYSessionTurdTypeDEC2048 };
...
@implementation PTYSession {
    ...
    NSHashTable *_commandExitSnapshotSubscribers;
    os_unfair_lock _commandExitSnapshotSubscribersLock; // Protects _commandExitSnapshotSubscribers
...
+ (NSMapTable<NSString *, PTYSession *> *)sessionMap {
    ...
    dispatch_once(&onceToken, ^{
        ...
        map = [[NSMapTable alloc] initWithKeyOptions:weakWeak valueOptions:weakWeak capacity:1];
    });
    return map;
}
...
+ (void)registerSessionInArrangement:(NSDictionary *)arrangement {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        gRegisteredSessionContents = [[NSMutableDictionary alloc] initWithCapacity:16];
    });
...
- (instancetype)initWithCoder:(NSCoder *)coder {
    // BUG-12005: PTYSession does not support NSCoding - use proper error instead of assert
    // This method exists only to satisfy the NSCoding protocol requirement
    ELog(@"BUG-12005: PTYSession initWithCoder: called - NSCoding is not supported");
...
- (instancetype)initSynthetic:(BOOL)synthetic {
    ...
    if (self) {
        ...
        // Initialize dterm-core integration for parallel processing.
        // Start with default terminal dimensions; resize will update them.
        _dtermCoreIntegration = [[DTermCoreIntegration alloc] initWithRows:24 cols:80];
        // Enable based on advanced setting
        _dtermCoreIntegration.isEnabled = [iTermAdvancedSettingsModel dtermCoreEnabled];
        if (_dtermCoreIntegration.isEnabled) {
            NSLog(@"[dterm-core] Integration enabled for session");
        }
        _dtermGridAdapter = [[DTermGridAdapter alloc] initWithIntegration:_dtermCoreIntegration];

        _overriddenFields = [[NSMutableSet alloc] initWithCapacity:8];

        _variables = [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextSession owner:self];
        // Alias for legacy paths
        [self.variablesScope setValue:_variables forVariableNamed:@"session" weak:YES];
        _userVariables = [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextNone owner:self];
        ...
        // BUG-2728: Use POSIX locale for fixed date formats per Apple TN1480
        ...
        // BUG-f1108: Replace assert with guard - screen.syncDistributor should not be nil during init
        if (!_screen.syncDistributor) {
            ELog(@"BUG-f1108: PTYSession init has nil screen.syncDistributor - attempting to continue");
        }
        _lastOrCurrentlyRunningCommandAbsRange = VT100GridAbsCoordRangeMake(-1, -1, -1, -1);
        _activityCounter = [@0 retain];
        _announcements = [[NSMutableDictionary alloc] initWithCapacity:4];
        _commands = [[NSMutableArray alloc] initWithCapacity:32];
        _directories = [[NSMutableArray alloc] initWithCapacity:16];
        _hosts = [[NSMutableArray alloc] initWithCapacity:8];
        _hostnameToShell = [[NSMutableDictionary alloc] initWithCapacity:8];
        ...
        _keystrokeSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:4];
        _keyboardFilterSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:4];
        _updateSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:4];
        _promptSubscriptions = [[NSMutableDictionary alloc] initWithCapacity:4];
        _commandExitSnapshotSubscribers = [[NSHashTable weakObjectsHashTable] retain];
        _commandExitSnapshotSubscribersLock = OS_UNFAIR_LOCK_INIT;
        _customEscapeSequenceNotifications = [[NSMutableDictionary alloc] initWithCapacity:4];
        _metalDisabledTokens = [[NSMutableSet alloc] initWithCapacity:4];
        ...
        _commandQueue = [[NSMutableArray alloc] initWithCapacity:8];
        _alertOnMarksinOffscreenSessions = [iTermPreferences boolForKey:kPreferenceKeyAlertOnMarksInOffscreenSessions];
        _pendingPublishRequests = [[NSMutableArray alloc] initWithCapacity:4];
        _pendingFilterUpdates = [[NSMutableArray alloc] initWithCapacity:4];
        ...
        [_disableTransparencyInKeyWindowObserver observeKey:kPreferenceKeyDisableTransparencyForKeyWindow
                                                      block:^{
        ...
        _hostStack = [[NSMutableArray alloc] initWithCapacity:8];
        ...
        _channelClients = [[NSMutableArray alloc] initWithCapacity:4];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(autoComposerDidChange:)
                                                   ...
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(aiLockStatusDidChange:)
                                                     name:@"DTermAILockStatusChanged"
                                                   object:nil];
        ...
        _isObservingEffectiveAppearance = YES; // BUG-295: Track KVO registration

        [[iTermFindPasteboard sharedInstance]
            addObserver:self
                  block:^(id sender, NSString *_Nonnull newValue, BOOL internallyGenerated) {
                      if (!weakSelf.view.window.isKeyWindow) {
                          return;
                      }
                      // BUG-12152: Only auto-search in the session that initiated the find pasteboard
                      // change, or in the current session when the change has no specific sender
                      // (e.g., when opening find panel or window becomes key).
                      // This prevents search highlights from appearing in ALL panes when user
                      // searches in just one pane.
                      BOOL isCurrentSession = (weakSelf == [[weakSelf.delegate realParentWindow] currentSession]);
                      if (sender != nil && sender != weakSelf) {
                          // Another session initiated the change - don't auto-search in this session
                          return;
                      }
                      if (sender == nil && !isCurrentSession) {
                          // Refresh notification (e.g., window became key) - only search in current session
                          return;
                      }
...
- (void)dealloc {
    // BUG-295: Only remove KVO observer if we actually registered one
    if (_isObservingEffectiveAppearance) {
        [NSApp removeObserver:self forKeyPath:@"effectiveAppearance"];
    }
    [[[NSWorkspace sharedWorkspace] notificationCenter] removeObserver:self];
    // BUG-1044: Don't use autorelease in dispatch block - let block manage guid's lifetime
    ...
    [_shell release];
    [_screen release];
    [_dtermGridAdapter release];
    [_dtermCoreIntegration release];
    ...
    [_commandExitSnapshotSubscribers release];
    ...
    // BUG-1148: Cancel any delayed selectors (brokenPipe, brokenPipeWithError:,
    // maybeReplaceTerminatedShellWithNewInstance)
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<%@: %p %dx%d metal=%@ id=%@%@%@>", [self class], self, [_screen width],
                                      [_screen height], @(self.useMetal), _guid, synthetic,
                                      _view.isBrowser ? @" WebBrowser" : @""];
}
...
- (void)setGuid:(NSString *)guid {
    ...
    iTermPublisher<NSNumber *> *previousPublisher =
        [[[[iTermCPUUtilization instanceForSessionID:_guid] publisher] retain] autorelease];
    [_guid autorelease];
    _guid = [guid copy];
    [[iTermCPUUtilization instanceForSessionID:_guid] setPublisher:previousPublisher];
    [self sync];
    // BUG-295: Guard against nil guid to prevent crash in setObject:forKey:
    if (_guid) {
        [[PTYSession sessionMap] setObject:self forKey:_guid];
    }
    [self.variablesScope setValue:_guid forVariableNamed:iTermVariableKeySessionID];
...
- (void)setLiveSession:(PTYSession *)liveSession {
    // BUG-f1109: Replace assert with guard - prevent setting self as live session
    if (liveSession == self) {
        ELog(@"BUG-f1109: Attempt to set self as live session - ignoring");
        return;
    }
    if (liveSession) {
        // BUG-f1110: Replace assert with guard - prevent setting multiple live sessions
        if (_liveSession) {
            ELog(@"BUG-f1110: Attempt to set live session when one exists - ignoring");
            return;
        }
        _synthetic = YES;
...
- (long long)irSeekToAtLeast:(long long)timestamp {
    // BUG-f1111: Replace assert with guard - DVR must exist for instant replay
    if (!_dvr) {
        ELog(@"BUG-f1111: irSeekToAtLeast called without DVR - returning 0");
        return 0;
    }
...
- (void)appendLinesInRange:(NSRange)rangeOfLines fromSession:(PTYSession *)source {
    // BUG-f1112: Replace assert with guard - cannot append lines from self
    if (source == self) {
        ELog(@"BUG-f1112: appendLinesInRange called with self as source - ignoring");
        return;
    }
    _modeHandler.mode = iTermSessionModeDefault;

    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                             id<VT100ScreenDelegate> delegate) {
        [source.screen
            enumerateLinesInRange:rangeOfLines
                            block:^(int i, ScreenCharArray *sca, iTermImmutableMetadata metadata, BOOL *stopPtr) {
                                if (i + 1 == NSMaxRange(rangeOfLines)) {
                                    screen_char_t continuation = {0};
                                    continuation.code = EOL_SOFT;
                                    [mutableState appendScreenChars:sca.line
                                                             length:sca.length
                                             externalAttributeIndex:iTermImmutableMetadataGetExternalAttributesIndex(
                                                                        metadata)
...
+ (void)drawArrangementPreview:(NSDictionary *)arrangement frame:(NSRect)frame dark:(BOOL)dark {
    ...
    NSColor *color = [iTermProfilePreferences colorForKey:KEY_BACKGROUND_COLOR dark:dark profile:theBookmark];
    [color set];
    NSRectFill(frame);
}
...
+ (BOOL)arrangement:(NSDictionary *)arrangement passesTest:(BOOL (^NS_NOESCAPE)(NSDictionary *candidate))closure {
    return closure(arrangement);
}

+ (NSDictionary *)modifiedArrangement:(NSDictionary *)arrangement mutator:(NSDictionary * (^)(NSDictionary *))mutator {
...
+ (NSDictionary *)repairedArrangement:(NSDictionary *)arrangement
    ...
    if ([arrangement[SESSION_ARRANGEMENT_GUID] isEqualToString:guid]) {
        return [arrangement dictionaryBySettingObject:replacementOldCWD forKey:SESSION_ARRANGEMENT_WORKING_DIRECTORY];
    }
    return arrangement;
}
...
+ (NSDictionary *)repairedArrangement:(NSDictionary *)arrangement settingCustomLocale:(NSString *)lang {
    DLog(@"Repair arrangement for %@", self);
    Profile *theBookmark =
        [[ProfileModel sharedInstance] bookmarkWithGuid:arrangement[SESSION_ARRANGEMENT_BOOKMARK][KEY_GUID]];
    if (theBookmark) {
...
+ (void)finishInitializingArrangementOriginatedSession:(PTYSession *)aSession
    ...
    if (shortcutDictionary) {
        [[iTermSessionHotkeyController sharedInstance]
            setShortcut:[iTermShortcut shortcutWithDictionary:shortcutDictionary]
             forSession:aSession];
        [aSession setSessionSpecificProfileValues:@{KEY_SESSION_HOTKEY : shortcutDictionary}];
    }

    [aSession.screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                                     id<VT100ScreenDelegate> delegate) {
    ...
    if (didRestoreContents && attachedToServer) {
        [aSession.screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
            ...
            if (arrangement[SESSION_ARRANGEMENT_SHELL_INTEGRATION_EVER_USED_DEPRECATED]) {
                ...
                const BOOL shellIntegrationEverUsed =
                    [arrangement[SESSION_ARRANGEMENT_SHELL_INTEGRATION_EVER_USED_DEPRECATED] boolValue];
                mutableState.shouldExpectPromptMarks = shellIntegrationEverUsed;
            } else {
                mutableState.shouldExpectPromptMarks =
                    [arrangement[SESSION_ARRANGEMENT_SHOULD_EXPECT_PROMPT_MARKS] boolValue];
                aSession->_shouldExpectCurrentDirUpdates =
                    [arrangement[SESSION_ARRANGEMENT_SHOULD_EXPECT_CURRENT_DIR_UPDATES] boolValue];
            }
        }];
    }
    ...
    aSession->_workingDirectoryPollerDisabled =
        [arrangement[SESSION_ARRANGEMENT_WORKING_DIRECTORY_POLLER_DISABLED] boolValue] ||
        aSession->_shouldExpectCurrentDirUpdates;
    ...
    if (didRestoreContents && attachedToServer) {
        ...
        if (arrangement[SESSION_ARRANGEMENT_LAST_DIRECTORY]) {
            ...
            const BOOL isRemote =
                [arrangement[SESSION_ARRANGEMENT_LAST_DIRECTORY_IS_UNSUITABLE_FOR_OLD_PWD_DEPRECATED] boolValue];
            if (!isRemote) {
        ...
        if (arrangement[SESSION_ARRANGEMENT_LAST_LOCAL_DIRECTORY]) {
            ...
            aSession.lastLocalDirectoryWasPushed =
                [arrangement[SESSION_ARRANGEMENT_LAST_LOCAL_DIRECTORY_WAS_PUSHED] boolValue];
        }
    }
    ...
    if (liveArrangement) {
        ...
        [delegate session:aSession
            setLiveSession:[self sessionFromArrangement:liveArrangement
                                                  named:nil
                                                 inView:liveView
    ...
    if (missingProfile) {
        ...
        if (arrangementProfile) {
            if (arrangementProfile[KEY_NAME] == nil || arrangementProfile[KEY_GUID] == nil) {
                [aSession.naggingController
                    offerToFixSessionWithBrokenArrangementProfileIn:arrangementName
    ...
    NSString *path =
        [aSession.screen workingDirectoryOnLine:aSession.screen.numberOfScrollbackLines + aSession.screen.cursorY - 1];
...
- (PTYSession *)newSessionForChannelID:(NSString *)channelID command:(NSString *)command {
    ...
    PTYSession *session =
        [PTYSession sessionFromArrangement:arrangement
                                     named:command
...
+ (PTYSession *)sessionFromArrangement:(NSDictionary *)arrangement
    ...
    NSMutableSet<NSString *> *keysToPreserveInCaseOfDivorce =
        [NSMutableSet setWithArray:@[ KEY_GUID, KEY_ORIGINAL_GUID ]];

    {
        ...
        if (overrides) {
            ...
            [overrides enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key, NSString *_Nonnull obj,
                                                           BOOL *_Nonnull stop) {
                temp[key] = obj;
            }];
    ...
    switch ([iTermMigrationHelper shouldRemoveDeprecatedKeyMappings]) {
        case iTermMigrationHelperShouldRemoveDeprecatedKeyMappingsDefault:
            // BUG-7901: User hasn't been prompted yet - leave mappings as-is until they are prompted
            // This CAN happen during first launch or if askToRemoveDeprecatedKeyMappingsIfNeeded hasn't run yet
            break;
        ...
        case iTermMigrationHelperShouldRemoveDeprecatedKeyMappingsYes: {
            NSDictionary *temp =
                [iTermMigrationHelper keyMappingsByRemovingDeprecatedKeyMappingsFrom:theBookmark[KEY_KEYBOARD_MAP]];
            if (temp) {
                NSMutableDictionary *modifiedProfile = [[theBookmark mutableCopy] autorelease];
    ...
    if ([arrangement objectForKey:SESSION_ARRANGEMENT_TMUX_PANE]) {
        ...
        if (tabColorDict) {
            ...
            NSColor *profileTabColorDict = [iTermProfilePreferences objectForColorKey:KEY_TAB_COLOR
                                                                                 dark:dark
                                                                              profile:theBookmark];
            if (![iTermProfilePreferences boolForColorKey:KEY_USE_TAB_COLOR dark:dark profile:theBookmark] ||
                ...
                // The tmux profile does not specify a tab color or it specifies a different one. Override it and
                // divorce.
    ...
    if ([arrangement[SESSION_ARRANGEMENT_TMUX_FOCUS_REPORTING] boolValue]) {
        ...
        // the terminal.
        [aSession.screen mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                                id<VT100ScreenDelegate> delegate) {
    ...
    if (arrangement[SESSION_ARRANGEMENT_PENDING_JUMPS]) {
        aSession->_pendingJumps = [[[NSArray castFrom:arrangement[SESSION_ARRANGEMENT_PENDING_JUMPS]]
            mapWithBlock:^id _Nullable(id _Nonnull data) {
    ...
    [aSession.variablesScope setValue:[aSession bestGuessAtUserShellWithPath:NO]
                     forVariableNamed:iTermVariableKeyShell];
    ...
    NSDictionary *contents = arrangement[SESSION_ARRANGEMENT_CONTENTS];
    BOOL restoreContents = (!tmuxPaneNumber && (arrangement[SESSION_ARRANGEMENT_BROWSER_STATE] != nil || contents) &&
    ...
    if (!tmuxPaneNumber) {
        ...
        if ([iTermAdvancedSettingsModel runJobsInServers]) {
            ...
            if (isTmuxGateway) {
                ...
                [aSession.screen
                    performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                                    id<VT100ScreenDelegate> delegate) {
            ...
            if (aSession->_conductor) {
                ...
                [aSession.screen
                    performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                        ...
                        if (dict) {
                            [terminal.parser
                                startConductorRecoveryModeWithID:arrangement[SESSION_ARRANGEMENT_CONDUCTOR_DCS_ID]
                                                            tree:dict];
                        }
                    }];
            }
            // DashTerm2 is currently configured to run jobs in servers, but we
            ...
            if ([NSNumber castFrom:arrangement[SESSION_ARRANGEMENT_SERVER_PID]]) {
                ...
                if (serverPid != -1 &&
                    [aSession tryToAttachToServerWithProcessId:serverPid tty:arrangement[SESSION_ARRANGEMENT_TTY]]) {
                    DLog(@"Success!");
                    didAttach = YES;
                }
                ...
                if (partialAttachments) {
                    ...
                    if (partial && [aSession tryToFinishAttachingToMultiserverWithPartialAttachment:partial] != 0) {
            ...
            if (didAttach) {
                ...
                if (aSession->_conductor) {
                    ...
                    // Issue #12360: Restore terminal state before releasing conductor.
                    // Without this, cursor mode, mouse mode, scroll settings etc. remain
                    // in SSH state causing "cursor lost and scrolling broken" symptoms.
                    NSDictionary *config = aSession->_conductor.terminalConfiguration;
                    if (config) {
                        [aSession->_screen restoreSavedState:config];
                    }
        ...
        if (arrangement[SESSION_ARRANGEMENT_GUID]) {
            ...
            if (guid && gRegisteredSessionContents[guid]) {
                ...
                DLog(@"Assign guid %@ to session %@ which will have its contents restored from registered contents",
                     guid, aSession);
            } else if ([[iTermController sharedInstance] startingUp] || arrangement[SESSION_ARRANGEMENT_CONTENTS]) {
                ...
                // session.
                aSession.guid = guid;
                DLog(@"DashTerm2 is starting up or has contents. Assign guid %@ to session %@ (session is loaded from "
                     @"saved arrangement. No content registered.)",
                     guid, aSession);
            }
        }
        ...
        if (restoreContents || [options[PTYSessionArrangementOptionsForDuplication] boolValue]) {
            aSession->_savedBrowserState =
                [[NSDictionary castFrom:arrangement[SESSION_ARRANGEMENT_BROWSER_STATE]] retain];
        }
        ...
        if (runCommand) {
            ...
            if (haveSavedProgramData) {
                ...
                if (oldCWD && [aSession.program isEqualToString:[ITAddressBookMgr standardLoginCommand]]) {
            ...
            runCommandBlock = ^(iTermSessionCreationCompletionBlock completion) {
                // BUG-f1113: Replace assert with guard - completion block must not be nil
                if (!completion) {
                    ELog(@"BUG-f1113: runCommandBlock called with nil completion - ignoring");
                    return;
                }
                iTermSessionAttachOrLaunchRequest *launchRequest = [iTermSessionAttachOrLaunchRequest
                    launchRequestWithSession:aSession
                                   canPrompt:NO
...
- (iTermAsciicastMetadata *)asciicastMetadata {
    ...
    NSArray<NSColor *> *ansi =
        [[NSArray sequenceWithRange:NSMakeRange(0, 16)] mapWithBlock:^id _Nonnull(NSNumber *_Nonnull n) {
            NSString *key = [NSString stringWithFormat:KEYTEMPLATE_ANSI_X_COLOR, n.intValue];
            return [iTermProfilePreferences colorForKey:key dark:dark profile:self.profile];
        }];
    NSString *term = [iTermProfilePreferences stringForKey:KEY_TERMINAL_TYPE inProfile:self.profile];
    NSDictionary *environment = @{@"TERM" : term ?: @"xterm", @"SHELL" : [self userShell]};
    return [[[iTermAsciicastMetadata alloc]
        initWithWidth:_screen.width
               height:_screen.height
              command:_program ?: @""
                title:[[self name] stringByTrimmingTrailingWhitespace] ?: @""
          environment:environment
                   fg:[iTermProfilePreferences colorForKey:KEY_FOREGROUND_COLOR dark:dark profile:self.profile]
                   bg:[iTermProfilePreferences colorForKey:KEY_BACKGROUND_COLOR dark:dark profile:self.profile]
...
- (void)setContentsFromLineBufferDictionary:(NSDictionary *)dict
    ...
    [_screen enumeratePortholes:^(id<PortholeMarkReading> immutableMark) {
        [[PortholeRegistry instance] registerKey:immutableMark.uniqueIdentifier forMark:immutableMark];
        id<Porthole> porthole = [_textview hydratePorthole:immutableMark];
        if (!porthole) {
            [_screen mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                            id<VT100ScreenDelegate> delegate) {
    ...
    if (lastRemoteHost) {
        ...
        [self screenCurrentHostDidChange:lastRemoteHost pwd:pwd ssh:NO];
    }
...
- (BOOL)setScreenSize:(NSSize)size parent:(id<WindowControllerInterface>)parent {
    ...
    _textview =
        [[PTYTextView alloc] initWithFrame:NSMakeRect(0, [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins],
    ...
    [self setTransparencyAffectsOnlyDefaultBackgroundColor:
              [[_profile objectForKey:KEY_TRANSPARENCY_AFFECTS_ONLY_DEFAULT_BACKGROUND_COLOR] boolValue]];

    [_wrapper addSubview:_textview];
    [_textview setFrame:NSMakeRect(0, [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins], aSize.width,
                                   aSize.height - [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins])];
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        // Scrollbar IS accounted for via verticalScrollerClass parameter below
        NSSize contentSize = [PTYScrollView
            contentSizeForFrameSize:aSize
        ...
        int width =
            (contentSize.width - [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2) / [_textview charWidth];
        int height = (contentSize.height - [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) /
                     [_textview lineHeight];
        [_screen destructivelySetScreenWidth:width height:height mutableState:mutableState];
        [self.variablesScope setValuesFromDictionary:@{
            iTermVariableKeySessionColumns : @(width),
            iTermVariableKeySessionRows : @(height)
        }];
    }];

    [_textview setDataSource:_screen];
    _textview.dtermGridAdapter = _dtermGridAdapter;
...
- (BOOL)tryToAttachToServerWithProcessId:(pid_t)serverPid tty:(NSString *)tty {
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
...
- (BOOL)tryToAttachToMultiserverWithRestorationIdentifier:(NSDictionary *)restorationIdentifier {
    const iTermJobManagerAttachResults results =
        [_shell tryToAttachToMultiserverWithRestorationIdentifier:restorationIdentifier];
...
- (void)attachToServer:(iTermGeneralServerConnection)serverConnection completion:(void (^)(void))completion {
    if ([iTermAdvancedSettingsModel runJobsInServers]) {
...
- (void)setSize:(VT100GridSize)size {
    ...
    // Keep dterm-core integration in sync with terminal dimensions
    [_dtermCoreIntegration resizeWithRows:(uint16_t)size.height cols:(uint16_t)size.width];
    ...
    if (!self.delegate || [self.delegate sessionShouldSendWindowSizeIOCTL:self]) {
        [_shell.winSizeController setGridSize:size viewSize:_screen.viewSize scaleFactor:self.backingScaleFactor];
    }
    ...
    [self updateMetalDriver];
    [self.variablesScope setValuesFromDictionary:@{
        iTermVariableKeySessionColumns : @(_screen.width),
...
- (NSArray<iTermSSHReconnectionInfo *> *)sshCommandLineSequence {
    // BUG-f1114: Replace assert with guard - conductor must exist for SSH command line sequence
    if (!_conductor) {
        ELog(@"BUG-f1114: sshCommandLineSequence called without conductor - returning empty array");
        return @[];
    }
    NSMutableArray<iTermSSHReconnectionInfo *> *sequence =
        [NSMutableArray arrayWithCapacity:4]; // Typically few SSH hops
...
- (NSArray<iTermProcessInfo *> *)processInfoForShellAndDescendants {
    // Typical shell has a few child processes
    NSMutableArray<iTermProcessInfo *> *result = [NSMutableArray arrayWithCapacity:8];
    if (_conductor) {
...
- (NSArray<iTermTuple<NSString *, NSString *> *> *)childJobNameTuples {
    ...
    return [allInfos mapWithBlock:^id(iTermProcessInfo *info) {
        ...
        return [iTermTuple tupleWithObject:info.name andObject:info.argv0 ?: info.name];
    }];
}
...
- (iTermPromptOnCloseReason *)promptOnCloseReason {
    ...
    switch ([[_profile objectForKey:KEY_PROMPT_CLOSE] intValue]) {
        ...
        case PROMPT_EX_JOBS: {
            ...
            // Typically few blocking jobs
            NSMutableArray<NSString *> *blockingJobs = [NSMutableArray arrayWithCapacity:4];
...
- (NSSet<NSString *> *)jobsToIgnore {
    NSArray<NSString *> *builtInJobsToIgnore = @[ @"login", @"DashTerm2", @"ShellLauncher" ];
    return [NSSet
        setWithArray:[[_profile objectForKey:KEY_JOBS] ?: @[] arrayByAddingObjectsFromArray:builtInJobsToIgnore]];
}
...
- (NSString *)sessionId {
    return
        [NSString stringWithFormat:@"w%dt%dp%lu:%@", [[_delegate realParentWindow] number],
                                   _delegate.tabNumberForItermSessionId, [_delegate sessionPaneNumber:self], self.guid];
}

- (void)didMoveSession {
    // TODO: Is it really desirable to update this? It'll get out of sync with the environment variable & autolog
    // filename.
...
- (void)profileNameDidChangeTo:(NSString *)name {
    ...
    const BOOL isChangeToLocalName = (self.isDivorced && [_overriddenFields containsObject:KEY_NAME]);
    const BOOL haveAutoNameFormatOverride =
        ([self.variablesScope valueForVariableName:iTermVariableKeySessionIconName] != nil ||
    ...
    [self.variablesScope setValuesFromDictionary:@{
        iTermVariableKeySessionAutoNameFormat : autoNameFormat ?: [NSNull null],
...
- (NSDictionary *)environmentForNewJobFromEnvironment:(NSDictionary *)environment
    ...
    DLog(@"environmentForNewJobFromEnvironment:%@ substitutions:%@", environment, substitutions);
    NSMutableDictionary *env =
        environment ? [[environment mutableCopy] autorelease] : [[NSMutableDictionary alloc] initWithCapacity:32];
    if (sshConfiguration) {
        iTermSSHConfiguration *config =
            [[[iTermSSHConfiguration alloc] initWithDictionary:sshConfiguration] autorelease];
        if (config.pathToSSH) {
    ...
    if ([iTermAdvancedSettingsModel setCookie]) {
        ...
        env[@"DASHTERM2_COOKIE"] = self.cookie;
    }
    ...
    switch ([iTermProfilePreferences unsignedIntegerForKey:KEY_SET_LOCALE_VARS inProfile:_profile]) {
        case iTermSetLocalVarsModeMinimal: {
            iTermLocaleGuesser *localeGuesser =
                [[[iTermLocaleGuesser alloc] initWithEncoding:self.encoding] autorelease];
        ...
        case iTermSetLocalVarsModeSetAutomatically: {
            ...
            if (!localeVars) {
                localeVars = [self promptForLocaleForArrangement:arrangementName fromArrangement:fromArrangement];
            }
            if (!localeVars) {
                DLog(@"Using LC_CTYPE");
    ...
    NSString *trimmed = [env[PWD_ENVNAME]
        stringByTrimmingTrailingCharactersFromCharacterSet:[NSCharacterSet characterSetWithCharactersInString:@"/"]];
    ...
    if (lang && self.encoding == NSUTF8StringEncoding && ![lang containsString:@"UTF-8"]) {
        const iTermWarningSelection selection = [iTermWarning
            showWarningWithTitle:
                @"Warning! This profile uses a custom locale that doesn't use UTF-8 as its character encoding, but "
                @"your profile *is* using UTF-8. This can cause error messages and non-ASCII text to appear wrong."
...
+ (void)setCustomLocale:(NSString *)lang inProfile:(Profile *)originalProfile model:(ProfileModel *)model {
    ...
    if (lang) {
        [iTermProfilePreferences
            setObjectsFromDictionary:@{KEY_CUSTOM_LOCALE : lang, KEY_SET_LOCALE_VARS : @(iTermSetLocalVarsModeCustom)}
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kSessionProfileDidChange object:guid];
}

- (NSDictionary *)promptForLocaleForArrangement:(NSString *)arrangementName fromArrangement:(BOOL)fromArrangement {
    ...
    if (prompt.remember && localeVars != nil && lang != nil) {
        ...
        if (fromArrangement) {
            ...
            NSMutableArray *repairedArrangements = [NSMutableArray arrayWithCapacity:terminalArrangements.count];
...
- (void)arrangementWithName:(NSString *)arrangementName hasBadPWD:(NSString *)pwd {
    ...
    [self.naggingController arrangementWithName:arrangementName hasInvalidPWD:pwd forSessionWithGuid:_arrangementGUID];
}
...
- (void)startProgram:(NSString *)command
    ...
    DLog(@"startProgram: entered with command=%@, browser=%@", command, @(browser));
    DLog(@"startProgram:%@ ssh:%@ browser:%@ environment:%@ customShell:%@ isUTF8:%@ substitutions:%@ "
         @"arrangementName:%@ fromArrangement:%@, self=%@",
         command, @(ssh), @(browser), environment, customShell, @(isUTF8), substitutions, arrangementName,
         @(fromArrangement), self);
    ...
    if (browser) {
        ...
        if (!allowed) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                const iTermWarningSelection selection = [iTermWarning
                    showWarningWithTitle:
                        @"A browser session failed to start because the DashTerm2 Browser Plugin couldn't be found."
                ...
                if (selection == kiTermWarningSelection0) {
                    ...
                    [[NSWorkspace sharedWorkspace]
                        openURL:[NSURL URLWithString:@"https://dashterm.com/browser-plugin.html"]];
                }
            });
        }
        return;
    }
    [self computeArgvForCommand:command
                  substitutions:substitutions
                     completion:^(NSArray<NSString *> *argv) {
                         DLog(@"argv=%@", argv);
                         NSDictionary *env = [self
                             environmentForNewJobFromEnvironment:environment ?: @{}
                                                   substitutions:substitutions
                         ...
                         [self fetchAutoLogFilenameWithCompletion:^(NSString *_Nonnull autoLogFilename) {
                             ...
                             if (!_isArchive) {
                                 [[self loggingHelper]
                                               setPath:autoLogFilename
                                               enabled:autoLogFilename != nil
                                                 style:iTermLoggingStyleFromUserDefaultsValue([iTermProfilePreferences
                                                           unsignedIntegerForKey:KEY_LOGGING_STYLE
                                 ...
                                 if (env[PWD_ENVNAME] && arrangementName && _arrangementGUID) {
                                     ...
                                     [[iTermSlowOperationGateway sharedInstance]
                                         checkIfDirectoryExists:env[PWD_ENVNAME]
                             ...
                             [self
                                 injectShellIntegrationWithEnvironment:env
                                                                  args:argv
                                                            completion:^(NSDictionary<NSString *, NSString *> *env,
                                                                ...
                                                                [_shell
                                                                    ...
                                                                    maybeScaleFactor:_textview.window.backingScaleFactor
                                                                              isUTF8:isUTF8
                                                                          completion:^{
                                                                              id<iTermWindowController> pty =
                                                                                  self.delegate.realParentWindow;
                                                                              if (pty.fullScreenPromise) {
                                                                                  DLog(@"Wait for window to enter full "
                                                                                       @"screen before sending initial "
                                                                                       @"text");
                                                                                  __weak __typeof(self) weakSelf = self;
                                                                                  [pty.fullScreenPromise
                                                                                      then:^(id _Nonnull value) {
                                                                                          DLog(@"Fullscreen promise "
                                                                                               @"fulfilled");
                                                                                          [weakSelf sendInitialText];
                                                                                      }];
                                                                              } else {
                                                                                  DLog(@"Sending initial text "
                                                                                       @"immediately");
...
- (void)injectShellIntegrationWithEnvironment:(NSDictionary<NSString *, NSString *> *)env
    ...
    [injector modifyShellEnvironmentWithShellIntegrationDir:dir env:env argv:argv completion:completion];
}

- (void)setParentScope:(iTermVariableScope *)parentScope {
    iTermVariableScope *scope = self.variablesScope;
    // BUG-f1115: Replace assert with guard - prevent circular scope reference
    if (parentScope == scope) {
        ELog(@"BUG-f1115: Attempt to set parentScope to self - ignoring");
        return;
    }
    ...
    [parentScope.frames enumerateObjectsUsingBlock:^(iTermTuple<NSString *, iTermVariables *> *_Nonnull tuple,
    ...
    // Find non-root frames (e.g., tab) and add their variables as nonterminals to parentSession (becoming, e.g.,
    // parentSession.tab)
    [parentScope.frames enumerateObjectsUsingBlock:^(iTermTuple<NSString *, iTermVariables *> *_Nonnull tuple,
                                                     NSUInteger idx, BOOL *_Nonnull stop) {
        if (!tuple.firstObject) {
...
- (void)sendInitialText {
    ...
    // BUG-f1652: Use iTermProfilePreferences for type-safe access instead of direct dictionary access
    NSString *initialText = [iTermProfilePreferences stringForKey:KEY_INITIAL_TEXT inProfile:_profile];
...
- (void)launchProfileInCurrentTerminal:(Profile *)profile withURL:(NSString *)url style:(iTermOpenStyle)style {
...
- (void)_maybeWarnAboutShortLivedSessions {
    ...
    if ([[NSDate date] timeIntervalSinceDate:_creationDate] < [iTermAdvancedSettingsModel shortLivedSessionDuration]) {
        ...
        if ([self.profile[KEY_CUSTOM_COMMAND] isEqual:kProfilePreferenceCommandTypeLoginShellValue]) {
            ...
            if (!self.isDivorced && shell && profile != nil && !profile.profileIsDynamic &&
                ![[NSFileManager defaultManager] fileExistsAtPath:shell]) {
                NSString *theKey = [NSString stringWithFormat:@"ShellDoesNotExist_%@", guid];
                NSString *theTitle = [NSString stringWithFormat:@"The shell for this account, “%@”, does not exist. "
                                                                @"Change the profile to use /bin/zsh instead?",
                                                                shell];
                ...
                if (selection == kiTermWarningSelection0) {
                    ...
                    [[ProfileModel sharedInstance] setObjectsFromDictionary:change inProfile:profile];
        ...
        NSString *theTitle =
            [NSString stringWithFormat:@"A session ended very soon after starting. Check that the command "
...
- (void)restartSession {
    ...
    // BUG-f1116: Replace assert with guard - session must be restartable
    if (!self.isRestartable) {
        ELog(@"BUG-f1116: restartSession called on non-restartable session - ignoring");
        return;
    }
    _isArchive = NO;
...
// MARK: - dterm-core Validation

/// Perform validation of dterm-core state against iTerm2's screen.
///
/// Compares cursor position and visible cell contents between dterm-core
/// and iTerm2's VT100Screen to detect parsing discrepancies. Logs results.
- (void)performDTermCoreValidation {
    if (!_dtermCoreIntegration.isEnabled) {
        return;
    }

    // Get cursor position from iTerm2's screen (1-based)
    int iTermCursorX = [_screen cursorX];
    int iTermCursorY = [_screen cursorY];

    // Build array of sample cells from visible screen for comparison.
    // Sample a grid of cells instead of all cells for performance.
    NSMutableArray<NSArray<NSNumber *> *> *sampleCells = [NSMutableArray array];
    int height = [_screen height];
    int width = [_screen width];

    // Allocate buffer for screen line
    screen_char_t *buffer = (screen_char_t *)malloc(sizeof(screen_char_t) * (width + 1));
    if (!buffer) {
        NSLog(@"[dterm-core] Validation failed: could not allocate buffer");
        return;
    }

    // Sample every 4th row and every 8th column for quick validation
    for (int row = 0; row < height; row += 4) {
        const screen_char_t *line = [_screen getLineAtIndex:row withBuffer:buffer];
        if (!line) continue;

        for (int col = 0; col < width; col += 8) {
            screen_char_t cell = line[col];
            // Skip complex chars (surrogate pairs, combining marks) - they require special handling
            if (cell.complexChar) continue;

            NSArray<NSNumber *> *cellData = @[
                @(row),
                @(col),
                @(cell.code),
                @(cell.bold ? 1 : 0),
            ...
            [sampleCells addObject:cellData];
        }
    }

    free(buffer);

    // Convert to int arrays for Swift API
    NSMutableArray<NSArray<NSNumber *> *> *intArrays = [NSMutableArray array];
    for (NSArray<NSNumber *> *cell in sampleCells) {
        [intArrays addObject:cell];
    }

    // Perform quick validation
    NSArray<NSString *> *discrepancies = [_dtermCoreIntegration quickValidateWithITermCursorX:iTermCursorX
                                                                                iTermCursorY:iTermCursorY
                                                                                 sampleCells:intArrays];

    if (discrepancies.count > 0) {
        NSLog(@"[dterm-core] VALIDATION FAILED - %lu discrepancies found:", (unsigned long)discrepancies.count);
        // Log first 5 discrepancies
        for (NSUInteger i = 0; i < MIN(5, discrepancies.count); i++) {
            NSLog(@"[dterm-core]   %@", discrepancies[i]);
        }
        if (discrepancies.count > 5) {
            NSLog(@"[dterm-core]   ... and %lu more", (unsigned long)(discrepancies.count - 5));
        }
    } else {
        // Only log periodic success messages to avoid spam
        static NSInteger validationSuccessCount = 0;
        validationSuccessCount++;
        if (validationSuccessCount % 10 == 0) {
            NSLog(@"[dterm-core] Validation passed (%ld checks, cursor at %d,%d)",
                  (long)sampleCells.count, iTermCursorX, iTermCursorY);
        }
    }
}

/// Sync dterm-core display offset to the current scroll position.
- (void)syncDTermCoreDisplayOffset {
    if (!_dtermCoreIntegration.isEnabled ||
        ![iTermAdvancedSettingsModel dtermCoreGridEnabled]) {
        return;
    }
    if (!_textview || _textview.lineHeight <= 0) {
        return;
    }
    ...
    const NSRect visibleRect = scrollView.documentVisibleRect;
    const NSInteger firstVisibleLine = (NSInteger)floor(visibleRect.origin.y / _textview.lineHeight);
    const NSInteger totalLines = _screen.numberOfLines;
    const NSInteger visibleRows = _screen.height;
    const NSInteger maxScrollback = MAX(0, totalLines - visibleRows);

    NSInteger desiredOffset = maxScrollback - firstVisibleLine;
    if (desiredOffset < 0) {
        desiredOffset = 0;
    }

    const NSInteger maxOffset = _dtermCoreIntegration.scrollbackLines;
    if (desiredOffset > maxOffset) {
        desiredOffset = maxOffset;
    }

    const NSInteger currentOffset = _dtermCoreIntegration.displayOffset;
    const NSInteger delta = desiredOffset - currentOffset;
    if (delta != 0) {
        [_dtermCoreIntegration scrollWithLines:(int32_t)delta];
    }
}
...
- (void)terminate {
    ...
    // Log dterm-core performance stats if integration was enabled
    if (_dtermCoreIntegration.isEnabled) {
        NSLog(@"[dterm-core] Session terminating - %@", _dtermCoreIntegration.performanceSummary);
    }
    ...
    if (self.tmuxMode == TMUX_CLIENT) {
        // Defensive check - ensure delegate and tmuxWindow are valid (upstream #12158)
        int tmuxWindowId = _delegate ? [_delegate tmuxWindow] : -1;
        if (tmuxWindowId < 0) {
            DLog(@"TMUX_CLIENT mode but delegate or tmuxWindow is invalid, skipping deregister");
        } else {
            [_tmuxController deregisterWindow:tmuxWindowId windowPane:self.tmuxPane session:self];
        }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermSessionWillTerminateNotification object:self];
...
- (void)makeTerminationUndoable {
    ...
    [self performSelector:@selector(hardStop)
               withObject:nil
               afterDelay:[iTermProfilePreferences intForKey:KEY_UNDO_TIMEOUT inProfile:_profile]];
...
- (void)hardStop {
    if (!self.isTmuxClient && !_isArchive && [iTermProfilePreferences boolForKey:KEY_ARCHIVE inProfile:self.profile]) {
        [self saveArchive];
    }
...
- (BOOL)revive {
    if (_shell.paused) {
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hardStop) object:nil];
        ...
        if ([iTermAdvancedSettingsModel showLocationsInScrollbar] &&
            [iTermAdvancedSettingsModel showMarksInScrollbar]) {
            _screen.intervalTreeObserver = self;
        }
        _shell.paused = NO;
        [_view setFindDriverDelegate:self];

        NSDictionary *shortcutDictionary = [iTermProfilePreferences objectForKey:KEY_SESSION_HOTKEY
                                                                       inProfile:self.profile];
...
- (void)writeTaskImpl:(NSString *)string
    ...
    if (canBroadcast && [[_delegate realParentWindow] broadcastInputToSession:self]) {
        ...
        if ((_buffering || _screen.sendingIsBlocked) && !reporting) {
            ...
            if (!_dataQueue) {
                _dataQueue = [[NSMutableArray alloc] initWithCapacity:16];
            }
...
- (void)writeTaskNoBroadcast:(NSString *)string
    ...
    if (_conductor.handlesKeystrokes) {
        ...
        [[_tmuxController gateway] sendKeys:string toWindowPane:self.tmuxPane];
        return;
    }
...
- (void)performTmuxCommand:(NSString *)command {
    [self.tmuxController.gateway sendCommand:command responseTarget:nil responseSelector:NULL];
}
...
- (void)handleCharacterPressedInTmuxGateway:(unichar)unicode {
    if (unicode == 27) {
        ...
        [self printTmuxMessage:[NSString
                                   stringWithFormat:@"tmux logging %@", (_tmuxGateway.tmuxLogging ? @"on" : @"off")]];
    } else if (unicode == 'C') {
...
- (void)writeTask:(NSString *)string
    ...
    if (self.tmuxMode == TMUX_CLIENT || _conductor.handlesKeystrokes || _connectingSSH) {
        ...
        if ([[_delegate realParentWindow] broadcastInputToSession:self]) {
            ...
            // Send keys to tmux pane (upstream #12158)
            [[_tmuxController gateway] sendKeys:string toWindowPane:self.tmuxPane];
        } else {
            // Unexpected state - log for debugging (upstream #12158)
            DLog(
                @"writeTask:encoding: unexpected state - tmuxMode=%d, conductor.handlesKeystrokes=%d, connectingSSH=%d",
                (int)self.tmuxMode, _conductor.handlesKeystrokes, _connectingSSH);
        }
...
- (void)threadedReadTask:(char *)buffer length:(int)length {
    // Feed data to dterm-core in parallel for comparison testing.
    // DTermCoreIntegration is thread-safe and will only process if enabled.
    [_dtermCoreIntegration processWithBytes:(const uint8_t *)buffer length:length];

    // Continue with normal iTerm2 parsing
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        [terminal setForeground24BitColor:[NSColor colorWithCalibratedRed:70.0 / 255.0
                                                                    green:83.0 / 255.0
                                                                     blue:246.0 / 255.0
                                                                    alpha:1]];
        [terminal setBackgroundColor:ALTSEM_DEFAULT alternateSemantics:YES];
        ...
        if (width > 0) {
            [mutableState appendNativeImageAtCursorWithName:@"BrokenPipeDivider" width:width];
        }
...
- (id)tmuxFormat:(NSString *)tmuxFormat reference:(iTermVariableReference *)ref error:(out NSError **)errorPtr {
    if (!self.tmuxController.gateway) {
        DLog(@"No gateway for %@", tmuxFormat);
        if (errorPtr) {
            *errorPtr = [NSError errorWithDomain:@"com.dashterm.dashterm2.bind-tmux-format"
                                            code:0
                                        userInfo:@{NSLocalizedDescriptionKey : @"Session is not a tmux client"}];
        }
        return nil;
    }
    iTermTmuxOptionMonitor *mon = [_userTmuxOptionMonitors
        objectPassingTest:^BOOL(iTermTmuxOptionMonitor *candidate, NSUInteger index, BOOL *stop) {
    ...
    if (!mon) {
        ...
        if (!_userTmuxOptionMonitors) {
            _userTmuxOptionMonitors = [[NSMutableArray alloc] initWithCapacity:4];
        }
...
- (void)taskDidResizeToGridSize:(VT100GridSize)gridSize pixelSize:(NSSize)pixelSize {
    ...
    NSData *data = [_screen.terminalOutput windowResizeNotificationWithGridSize:gridSize pixelSize:pixelSize];
...
- (void)tmuxDidDisconnect {
    ...
    switch (self.endAction) {
        ...
        case iTermSessionEndActionDefault:
            if (_tmuxWindowClosingByClientRequest || [self.naggingController tmuxWindowsShouldCloseAfterDetach]) {
                [_delegate softCloseSession:self];
                return;
            }
            break;
...
- (void)brokenPipeWithError:(NSString *)message {
    ...
    if ([self shouldPostUserNotification] && [iTermProfilePreferences boolForKey:KEY_SEND_SESSION_ENDED_ALERT
                                                                       inProfile:self.profile]) {
    ...
    switch (self.endAction) {
        ...
        case iTermSessionEndActionRestart:
            if ([self isRestartable]) {
                [self performSelector:@selector(maybeReplaceTerminatedShellWithNewInstance)
                           withObject:nil
                           afterDelay:1];
                return;
            }
...
- (void)replaceTerminatedShellWithNewInstance {
    // BUG-f1117: Replace assert with guard - session must be restartable
    if (!self.isRestartable) {
        ELog(@"BUG-f1117: replaceTerminatedShellWithNewInstance called on non-restartable session - ignoring");
        return;
    }
    // BUG-f1118: Replace assert with guard - session must have exited
    if (!_exited) {
        ELog(@"BUG-f1118: replaceTerminatedShellWithNewInstance called on non-exited session - ignoring");
        return;
    }
    _shouldRestart = NO;
    ...
    _shell = [[PTYTask alloc] init];
    [_shell setDelegate:self];
    [_shell.winSizeController setGridSize:_screen.size viewSize:_screen.viewSize scaleFactor:self.backingScaleFactor];
...
- (NSSize)idealScrollViewSizeWithStyle:(NSScrollerStyle)scrollerStyle {
    NSSize innerSize = NSMakeSize(
        [_screen width] * [_textview charWidth] + [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
...
+ (iTermTimestampsMode)desiredTimestampsModeForProfile:(Profile *)profile {
    ...
    return (iTermTimestampsMode)[iTermProfilePreferences unsignedIntegerForKey:KEY_TIMESTAMPS_STYLE inProfile:profile];
}
...
- (BOOL)shouldPostUserNotification {
    ...
    BOOL windowIsObscured = ([[iTermController sharedInstance] terminalIsObscured:_delegate.realParentWindow]);
...
- (void)open:(NSString *)selection workingDirectory:(NSString *)workingDirectory {
    ...
    // to complete.
    NSString *rawFilename = [semanticHistoryController pathOfExistingFileFoundWithPrefix:selection
                                                                                  suffix:@""
                                                                        workingDirectory:workingDirectory
                                                                    charsTakenFromPrefix:nil
                                                                    charsTakenFromSuffix:nil
                                                                          trimWhitespace:YES];
    if (rawFilename && ![[rawFilename stringByReplacingOccurrencesOfString:@"//"
...
- (void)tryOpenStringAsURL:(NSString *)selection {
    ...
    NSURL *url = [NSURL
        URLWithUserSuppliedString:[selection stringByTrimmingCharactersInSet:[NSCharacterSet
                                                                                 whitespaceAndNewlineCharacterSet]]];
    if (url) {
        [[NSWorkspace sharedWorkspace] it_openURL:url target:nil style:iTermOpenStyleTab window:self.view.window];
        return;
    }
...
- (void)setBell:(BOOL)flag {
    if (flag != _bell) {
        ...
        if (_bell) {
            if ([_textview keyIsARepeat] == NO && [self shouldPostUserNotification] &&
...
- (void)sharedProfileDidChange {
    ...
    [ProfileModel
        log:[NSString stringWithFormat:@"DIVORCE_SHARED_CHANGE: session=%p isDivorced=%d currentProfileGUID=%@ "
                                       @"updatedProfileGUID=%@ originalProfileGUID=%@ overriddenFields=%@",
                                       self, self.isDivorced, self.profile[KEY_GUID], updatedProfile[KEY_GUID],
                                       _originalProfile[KEY_GUID], _overriddenFields]];
    ...
    NSMutableArray *noLongerOverriddenFields = [NSMutableArray arrayWithCapacity:_overriddenFields.count];
    NSMutableSet *keys = [NSMutableSet setWithArray:[updatedProfile allKeys]];
    [keys addObjectsFromArray:[_profile allKeys]];

    [ProfileModel
        log:[NSString stringWithFormat:@"DIVORCE_MERGE_START: tempGUID=%@ updatedGUID=%@ KEY_GUID_is_overridden=%d",
                                       temp[KEY_GUID], updatedProfile[KEY_GUID],
    ...
    for (NSString *key in keys) {
        ...
        if ([_overriddenFields containsObject:key]) {
            ...
            if ([key isEqualToString:KEY_GUID] || [key isEqualToString:KEY_ORIGINAL_GUID]) {
                [ProfileModel
                    log:[NSString stringWithFormat:@"DIVORCE_GUID_UPDATE: key=%@ from=%@ to=%@ (NOT OVERRIDDEN!)", key,
    ...
    for (NSString *key in noLongerOverriddenFields) {
        if ([key isEqualToString:KEY_GUID] || [key isEqualToString:KEY_ORIGINAL_GUID]) {
            [ProfileModel
                log:[NSString stringWithFormat:@"DIVORCE_CRITICAL: Removing %@ from overriddenFields! Value=%@", key,
                                               temp[key]]];
        }
        DLog(@"%p: %@ is no longer overridden because shared profile now matches session profile value of %@", self,
             key, temp[key]);
        [_overriddenFields removeObject:key];
    }
    DLog(@"After shared profile change overridden keys are: %@", _overriddenFields);

    [ProfileModel log:[NSString stringWithFormat:@"DIVORCE_MERGE_END: finalTempGUID=%@ overriddenFields=%@",
                                                 temp[KEY_GUID], _overriddenFields]];
...
- (void)sessionProfileDidChange {
    ...
    NSDictionary *updatedProfile = [[ProfileModel sessionsInstance] bookmarkWithGuid:_profile[KEY_GUID]];
    if (!updatedProfile) {
    ...
    [self setPreferencesFromAddressBookEntry:updatedProfile];
    [self setProfile:updatedProfile];
    [[NSNotificationCenter defaultCenter] postNotificationName:kSessionProfileDidChange object:_profile[KEY_GUID]];
}

- (BOOL)reloadProfile {
    DLog(@"Reload profile for %@", self);

    [ProfileModel log:[NSString stringWithFormat:@"DIVORCE_RELOAD: session=%p isDivorced=%d currentGUID=%@ "
                                                 @"originalProfileGUID=%@ overriddenFields=%@",
                                                 self, self.isDivorced, _profile[KEY_GUID], _originalProfile[KEY_GUID],
                                                 _overriddenFields]];
    ...
    NSMutableDictionary<NSNumber *, id> *colorTable =
        [[[keyMap mapValuesWithBlock:^id(NSNumber *colorKey, NSString *profileKey) {
            if ([profileKey isKindOfClass:[NSString class]]) {
                return [iTermProfilePreferences colorForKey:profileKey dark:dark profile:aDict] ?: [NSNull null];
            } else {
                return [NSNull null];
            }
        }] mutableCopy] autorelease];
    [self load16ANSIColorsFromProfile:aDict darkMode:dark into:colorTable];
    const BOOL didUseSelectedTextColor =
        [iTermProfilePreferences boolForKey:iTermAmendedColorKey(KEY_USE_SELECTED_TEXT_COLOR, self.profile, dark)
                                  inProfile:self.profile];
    const BOOL willUseSelectedTextColor =
        [iTermProfilePreferences boolForKey:iTermAmendedColorKey(KEY_USE_SELECTED_TEXT_COLOR, aDict, dark)
                                  inProfile:aDict];

    [_screen setColorsFromDictionary:colorTable];

    if (didUseSelectedTextColor != willUseSelectedTextColor) {
        [_textview updatePortholeColorsWithUseSelectedTextColor:willUseSelectedTextColor deferUpdate:YES];
    }
    self.cursorGuideColor = [[iTermProfilePreferences
        objectForKey:iTermAmendedColorKey(KEY_CURSOR_GUIDE_COLOR, aDict, dark)
           inProfile:aDict] colorValueForKey:iTermAmendedColorKey(KEY_CURSOR_GUIDE_COLOR, aDict, dark)];
    if (!_cursorGuideSettingHasChanged) {
        _textview.highlightCursorLine =
            [iTermProfilePreferences boolForKey:iTermAmendedColorKey(KEY_USE_CURSOR_GUIDE, aDict, dark)
                                      inProfile:aDict];
    }

    [self setSmartCursorColor:[iTermProfilePreferences
                                  boolForKey:iTermAmendedColorKey(KEY_SMART_CURSOR_COLOR, aDict, dark)
                                   inProfile:aDict]];

    DLog(@"set min contrast to %f using key %@",
         [iTermProfilePreferences floatForKey:iTermAmendedColorKey(KEY_MINIMUM_CONTRAST, aDict, dark) inProfile:aDict],
         iTermAmendedColorKey(KEY_MINIMUM_CONTRAST, aDict, dark));
    [self
        setMinimumContrast:[iTermProfilePreferences floatForKey:iTermAmendedColorKey(KEY_MINIMUM_CONTRAST, aDict, dark)
                                                      inProfile:aDict]];
}
...
- (NSDictionary<NSNumber *, NSString *> *)colorTableForProfile:(Profile *)profile darkMode:(BOOL)dark {
    ...
    NSDictionary *keyMap = @{
        @(kColorMapForeground) : k(KEY_FOREGROUND_COLOR),
...
- (NSDictionary<NSNumber *, id> *)resetColorWithKey:(int)colorKey
    ...
    NSColor *color = [iTermProfilePreferences colorForKey:profileKey dark:dark profile:profile];
    if (!color) {
        return @{};
    }
    if (profileKey) {
...
- (void)load16ANSIColorsFromProfile:(Profile *)aDict
                           darkMode:(BOOL)dark
                               into:(NSMutableDictionary<NSNumber *, id> *)dict {
    for (int i = 0; i < 16; i++) {
...
- (NSColor *)defaultANSIColor:(int)i darkMode:(BOOL)darkMode {
    switch (i) {
        case 0: // Black
            return [NSColor colorWithSRGBRed:0.078431375 green:0.098039217 blue:0.117647059 alpha:1.0];
        case 1: // Red
            return [NSColor colorWithSRGBRed:0.707443237 green:0.236600697 blue:0.163000375 alpha:1.0];
        case 2: // Green
            return [NSColor colorWithSRGBRed:0.0 green:0.760784328 blue:0.0 alpha:1.0];
        case 3: // Yellow
            return [NSColor colorWithSRGBRed:0.780586481 green:0.769594848 blue:0.0 alpha:1.0];
        case 4: // Blue
            return [NSColor colorWithSRGBRed:0.154043003 green:0.264743567 blue:0.782161772 alpha:1.0];
        case 5: // Magenta
            return [NSColor colorWithSRGBRed:0.752197266 green:0.249316841 blue:0.744943619 alpha:1.0];
        case 6: // Cyan
            return [NSColor colorWithSRGBRed:0.0 green:0.774259031 blue:0.781662047 alpha:1.0];
        case 7: // White
            return [NSColor colorWithSRGBRed:0.781039774 green:0.781058252 blue:0.781048298 alpha:1.0];
        case 8: // Bright Black
            return [NSColor colorWithSRGBRed:0.407817602 green:0.407827884 blue:0.407822371 alpha:1.0];
        case 9: // Bright Red
            return [NSColor colorWithSRGBRed:0.865951538 green:0.475240767 blue:0.458332241 alpha:1.0];
        case 10: // Bright Green
            return [NSColor colorWithSRGBRed:0.345007032 green:0.904281616 blue:0.565419376 alpha:1.0];
        case 11: // Bright Yellow
            return [NSColor colorWithSRGBRed:0.92590332 green:0.883377552 blue:0.0 alpha:1.0];
        case 12: // Bright Blue
            return [NSColor colorWithSRGBRed:0.653490782 green:0.670447171 blue:0.948532105 alpha:1.0];
        case 13: // Bright Magenta
            return [NSColor colorWithSRGBRed:0.882156372 green:0.492726654 blue:0.882156372 alpha:1.0];
        case 14: // Bright Cyan
            return [NSColor colorWithSRGBRed:0.375975311 green:0.992632926 blue:1.0 alpha:1.0];
        case 15: // Bright White
            return [NSColor colorWithSRGBRed:0.999996006 green:1.0 blue:1.0 alpha:1.0];
        default:
            return [NSColor colorWithSRGBRed:1.0 green:0.0 blue:0.0 alpha:1.0];
    }
}
- (void)loadANSIColor:(int)i
          fromProfile:(Profile *)aDict
             darkMode:(BOOL)dark
                   to:(NSMutableDictionary<NSNumber *, id> *)dict {
...
- (void)reallySetPreferencesFromAddressBookEntry:(NSDictionary *)aePrefs terminal:(VT100Terminal *)terminal {
    ...
    // background image
    // BUG-f1652: Use iTermProfilePreferences for type-safe access instead of direct dictionary access
    [self setBackgroundImagePath:[iTermProfilePreferences stringForKey:KEY_BACKGROUND_IMAGE_LOCATION
                                                             inProfile:aDict]];
    ...
    [self setTransparencyAffectsOnlyDefaultBackgroundColor:
              [iTermProfilePreferences floatForKey:KEY_TRANSPARENCY_AFFECTS_ONLY_DEFAULT_BACKGROUND_COLOR
                                         inProfile:aDict]];

    // bold
    [self setUseBoldFont:[iTermProfilePreferences boolForKey:KEY_USE_BOLD_FONT inProfile:aDict]];
    ...
    [_screen setPostUserNotifications:[iTermProfilePreferences boolForKey:KEY_BOOKMARK_USER_NOTIFICATIONS
                                                                inProfile:aDict]];
    ...
    _textview.hideCursorWhenUnfocused = [iTermProfilePreferences boolForKey:KEY_CURSOR_HIDDEN_WITHOUT_FOCUS
                                                                  inProfile:aDict];
    _textview.animateMovement = [iTermProfilePreferences boolForKey:KEY_ANIMATE_MOVEMENT inProfile:aDict];
    _textview.animateMovementOnlyInInteractiveApps =
        [iTermProfilePreferences boolForKey:KEY_ANIMATE_MOVEMENT_ONLY_IN_INTERACTIVE_APPS inProfile:aDict];
    [_textview setBlinkingCursor:[iTermProfilePreferences boolForKey:KEY_BLINKING_CURSOR inProfile:aDict]];
    [_textview setCursorType:_cursorTypeOverride ? _cursorTypeOverride.integerValue
                                                 : [iTermProfilePreferences intForKey:KEY_CURSOR_TYPE inProfile:aDict]];
    ...
    [_textview setUseNonAsciiFont:[iTermProfilePreferences boolForKey:KEY_USE_NONASCII_FONT inProfile:aDict]];
    [_textview setAntiAlias:[iTermProfilePreferences boolForKey:KEY_ASCII_ANTI_ALIASED inProfile:aDict]
                   nonAscii:[iTermProfilePreferences boolForKey:KEY_NONASCII_ANTI_ALIASED inProfile:aDict]];
    ...
    [self setEncoding:[iTermProfilePreferences unsignedIntegerForKey:KEY_CHARACTER_ENCODING inProfile:aDict]
             terminal:terminal];
    [self setTermVariable:[iTermProfilePreferences stringForKey:KEY_TERMINAL_TYPE inProfile:aDict] terminal:terminal];
    ...
    [self setXtermMouseReportingEverAllowed:[iTermProfilePreferences boolForKey:KEY_XTERM_MOUSE_REPORTING
                                                                      inProfile:aDict]];
    [self setXtermMouseReportingAllowMouseWheel:[iTermProfilePreferences
                                                    boolForKey:KEY_XTERM_MOUSE_REPORTING_ALLOW_MOUSE_WHEEL
                                                     inProfile:aDict]];
    [self setXtermMouseReportingAllowClicksAndDrags:[iTermProfilePreferences
                                                        boolForKey:KEY_XTERM_MOUSE_REPORTING_ALLOW_CLICKS_AND_DRAGS
                                                         inProfile:aDict]];
    [self setUnicodeVersion:[iTermProfilePreferences integerForKey:KEY_UNICODE_VERSION inProfile:aDict]];
    [terminal setDisableSmcupRmcup:[iTermProfilePreferences boolForKey:KEY_DISABLE_SMCUP_RMCUP inProfile:aDict]];
    [_screen setAllowTitleReporting:[iTermProfilePreferences boolForKey:KEY_ALLOW_TITLE_REPORTING inProfile:aDict]];
    ...
    [terminal setAllowPasteBracketing:[iTermProfilePreferences boolForKey:KEY_ALLOW_PASTE_BRACKETING inProfile:aDict]];
    self.view.enableProgressBars = [iTermProfilePreferences boolForKey:KEY_ENABLE_PROGRESS_BARS inProfile:aDict];
    self.view.progressBarHeight = [iTermProfilePreferences floatForKey:KEY_PROGRESS_BAR_HEIGHT inProfile:aDict];
    ...
    [terminal setAllowKeypadMode:[iTermProfilePreferences boolForKey:KEY_APPLICATION_KEYPAD_ALLOWED inProfile:aDict]];
    if ([iTermProfilePreferences boolForKey:KEY_SHOW_STATUS_BAR inProfile:aDict]) {
        ...
        if (![NSObject object:existing isEqualToObject:layout]) {
            iTermStatusBarLayout *newLayout =
                [[[iTermStatusBarLayout alloc] initWithDictionary:layout scope:self.variablesScope] autorelease];
            if (![NSObject object:existing isEqualToObject:newLayout.dictionaryValue]) {
                [_statusBarViewController release];
                if (newLayout) {
                    _statusBarViewController =
                        [[iTermStatusBarViewController alloc] initWithLayout:newLayout scope:self.variablesScope];
                    _statusBarViewController.delegate = self;
                } else {
                    _statusBarViewController.delegate = nil;
    ...
    if (modifyOtherKeysTerminalSetting == -1) {
        const BOOL profileWantsTickit = [iTermProfilePreferences boolForKey:KEY_USE_LIBTICKIT_PROTOCOL inProfile:aDict];
    ...
    if (self.isTmuxClient) {
        NSDictionary *tabColorDict = [iTermProfilePreferences objectForColorKey:KEY_TAB_COLOR
                                                                           dark:_screen.colorMap.darkMode
                                                                        profile:aDict];
...
- (void)setSubtitleFormat:(NSString *)subtitleFormat {
    ...
    if (!_subtitleSwiftyString) {
        ...
        _subtitleSwiftyString = [[iTermSwiftyString alloc]
            initWithString:@""
                     scope:self.variablesScope
        sideEffectsAllowed:NO
...
- (void)setBadgeFormat:(NSString *)badgeFormat {
    ...
    _badgeSwiftyString = [[iTermSwiftyString alloc]
        initWithString:badgeFormat
                 scope:self.variablesScope
    sideEffectsAllowed:NO
              observer:^NSString *(NSString *_Nonnull newValue, NSError *error) {
                  if (error) {
...
- (BOOL)doesSwiftyString:(iTermSwiftyString *)swiftyString referencePaths:(NSArray<NSString *> *)paths {
...
- (void)setDelegate:(id<PTYSessionDelegate>)delegate {
    if ([self isTmuxClient]) {
        [_tmuxController deregisterWindow:[_delegate tmuxWindow] windowPane:self.tmuxPane session:self];
    }
    BOOL needsTermID = (_delegate == nil);
    _delegate = delegate;
    if ([self isTmuxClient]) {
        [_tmuxController registerSession:self withPane:self.tmuxPane inWindow:[_delegate tmuxWindow]];
    }
    DLog(@"Fit layout to window on session delegate change");
    [_tmuxController fitLayoutToWindows];
    [self useTransparencyDidChange];
    [self.variablesScope setValue:[delegate sessionTabVariables] forVariableNamed:iTermVariableKeySessionTab weak:YES];
    if (needsTermID) {
        [self setTermIDIfPossible];
    }
    // useTransparency may have just changed.
...
- (void)setIconName:(NSString *)theName {
    ...
    [self.variablesScope setValuesFromDictionary:@{
        iTermVariableKeySessionAutoNameFormat : theName ?: [NSNull null],
...
- (void)setBackgroundImagePath:(NSString *)interpolatedString {
    ...
    __weak __typeof(self) weakSelf = self;
    // BUG-f1652: Guard against non-string values from profile dictionary
    // If the profile contains a non-string value (e.g., NSDictionary or NSNumber),
    // calling iTermSwiftyString methods would crash with unrecognized selector.
    if (!interpolatedString || ![interpolatedString isKindOfClass:[NSString class]]) {
        [self reallySetBackgroundImagePath:nil];
        return;
    }
    // BUG-1653: Guard against nil variablesScope during session restoration
    // This can happen when restoring saved state before the session is fully initialized
    iTermVariableScope *scope = self.variablesScope;
    if (!scope) {
        DLog(@"variablesScope is nil, using path directly without interpolation");
        [self reallySetBackgroundImagePath:interpolatedString];
        return;
    }
    _backgroundImageSwiftyString = [[iTermSwiftyString alloc]
        initWithString:interpolatedString
                 scope:scope
    sideEffectsAllowed:NO
...
- (void)reallySetBackgroundImagePath:(NSString *)imageFilePath {
    ...
    if ([imageFilePath length]) {
        ...
        self.backgroundImage = [[iTermSharedImageStore sharedInstance]
            imageWithContentsOfFile:[imageFilePath stringByExpandingTildeInPath]];
    } else {
...
- (void)invalidateBlend {
    ...
    [self.view setTransparencyAlpha:_textview.transparencyAlpha blend:self.effectiveBlend];
}
...
- (void)setUnicodeVersion:(NSInteger)version {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermUnicodeVersionDidChangeNotification object:nil];
}
...
- (BOOL)xtermMouseReporting {
    ...
    if (!_screen.terminalSoftAlternateScreenMode) {
        return ![iTermProfilePreferences boolForKey:KEY_RESTRICT_MOUSE_REPORTING_TO_ALTERNATE_SCREEN_MODE
                                          inProfile:self.profile];
    }
    return YES;
}
...
- (void)logStart {
    __weak __typeof(self) weakSelf = self;
    [iTermSavePanel
        asyncShowWithOptions:kSavePanelOptionAppendOrReplace | kSavePanelOptionLogPlainTextAccessory |
                             kSavePanelOptionLocalhostOnly
...
- (BOOL)shouldSendEscPrefixForModifier:(unsigned int)modmask {
    if ([self optionKey] == OPT_ESC) {
        if ((modmask == NSEventModifierFlagOption) || (modmask & NSLeftAlternateKeyMask) == NSLeftAlternateKeyMask) {
            return YES;
        }
    }
...
- (void)setScrollViewDocumentView {
    ...
    [_view.scrollview setDocumentView:_wrapper];
    NSRect rect = {.origin = NSZeroPoint, .size = _view.scrollview.contentSize};
    _wrapper.frame = rect;
    [_textview refresh];
...
- (void)setProfile:(Profile *)newProfile {
    // BUG-f1119: Replace assert with guard - profile must not be nil
    if (!newProfile) {
        ELog(@"BUG-f1119: setProfile called with nil profile - ignoring");
        return;
    }
    ...
    [ProfileModel log:[NSString stringWithFormat:@"DIVORCE_SET_PROFILE: session=%p isDivorced=%d oldGUID=%@ newGUID=%@ "
                                                 @"originalProfileGUID=%@ overriddenFields=%@",
                                                 self, _divorced, oldGUID, newGUID, _originalProfile[KEY_GUID],
                                                 _overriddenFields]];
    ...
    if (_divorced) {
        ...
        if (newGUIDInShared && !newGUIDInSessions) {
            [ProfileModel
                log:[NSString
                        stringWithFormat:
                            @"DIVORCE_BUG_DETECTED: Divorced session %p being set to shared-only GUID %@! Stack:\n%@",
...
- (BOOL)encodeArrangementWithContents:(BOOL)includeContents encoder:(id<iTermEncoderAdapter>)result {
...
- (BOOL)encodeArrangementWithContents:(BOOL)includeContents
    ...
    if (saveProgram) {
        ...
        if ([programType isEqualToString:kProgramTypeCustomShell]) {
            // The shell launcher command could change from run to run (e.g., if you move DashTerm2).
            // I don't want to use a magic string, so setting program to an empty dict.
            // BUG-f1120: Replace assert with guard - customShell must have length
            if (self.customShell.length) {
                ...
                ELog(@"BUG-f1120: arrangement for custom shell has empty customShell - skipping");
            }
        } else if ([programType isEqualToString:kProgramTypeShellLauncher]) {
            NSDictionary *dict = @{kProgramType : kProgramTypeShellLauncher};
            result[SESSION_ARRANGEMENT_PROGRAM] = dict;
        } else if ([programType isEqualToString:kProgramTypeCommand] && self.program) {
            result[SESSION_ARRANGEMENT_PROGRAM] = @{kProgramType : kProgramTypeCommand, kProgramCommand : self.program};
        }
    }
    if (pendingJumps) {
        result[SESSION_ARRANGEMENT_PENDING_JUMPS] =
            [pendingJumps mapWithBlock:^id _Nullable(iTermSSHReconnectionInfo *_Nonnull info) {
    ...
    NSDictionary *shortcutDictionary =
        [[[iTermSessionHotkeyController sharedInstance] shortcutForSession:self] dictionaryValue];
    if (shortcutDictionary) {
    ...
    if (includeContents) {
        ...
        if (_conductor && [self.profile[KEY_CUSTOM_COMMAND] isEqualTo:kProfilePreferenceCommandTypeSSHValue]) {
            result[SESSION_ARRANGEMENT_PENDING_JUMPS] =
                [self.sshCommandLineSequence mapWithBlock:^id _Nullable(iTermSSHReconnectionInfo *_Nonnull anObject) {
    ...
    if (includeContents && !self.isTmuxClient) {
        DLog(@"Can include restoration info. runJobsInServers=%@ isSessionRestorationPossible=%@",
             @([iTermAdvancedSettingsModel runJobsInServers]), @(_shell.isSessionRestorationPossible));
    ...
    if (_overriddenFields.count > 0) {
        if (replacementProfile) {
            ...
            for (NSString *key in [[NSSet setWithArray:[_profile allKeys]]
                     setByAddingObjectsFromSet:[NSSet setWithArray:[replacementProfile allKeys]]]) {
...
- (void)updateDisplayBecause:(NSString *)reason {
    ...
    // Perform dterm-core validation every 100 updates when validation mode is enabled
    if (_dtermCoreIntegration.isEnabled &&
        [iTermAdvancedSettingsModel dtermCoreValidationEnabled] &&
        _updateCount % 100 == 0) {
        [self performDTermCoreValidation];
    }
...
- (BOOL)shouldShowPasswordManagerAutomatically {
    return [iTermProfilePreferences boolForKey:KEY_OPEN_PASSWORD_MANAGER_AUTOMATICALLY inProfile:self.profile];
}
...
- (void)setCurrentForegroundJobProcessInfo:(iTermProcessInfo *)processInfo {
    ...
    NSNumber *jobPIDNumber = [iTermLSOF cachedNumberForPid:processInfo.processID];
    [self.variablesScope setValue:jobPIDNumber forVariableNamed:iTermVariableKeySessionJobPid];

    NSNumber *effectiveShellPID = _shell.tmuxClientProcessID ?: [iTermLSOF cachedNumberForPid:_shell.pid];
    if (!_exited) {
        if (effectiveShellPID.intValue > 0) {
            [self.variablesScope setValue:effectiveShellPID forVariableNamed:iTermVariableKeySessionChildPid];
        }
...
- (void)setActive:(BOOL)active {
    DLog(@"setActive:%@ timerRunning=%@ updateTimer.isValue=%@ lastTimeout=%f session=%@", @(active), @(_timerRunning),
         @(_cadenceController.updateTimerIsValid), _lastTimeout, self);
    _active = active;
...
- (void)setFontTable:(iTermFontTable *)newFontTable
    ...
    DLog(@"setFontTable:%@ horizontalSpacing:%@ verticalSpacing:%@", newFontTable, @(horizontalSpacing),
         @(verticalSpacing));
    NSWindow *window = [[_delegate realParentWindow] window];
    DLog(@"Before:\n%@", [window.contentView iterm_recursiveDescription]);
    DLog(@"Window frame: %@", window);
    if ([_textview.fontTable isEqual:newFontTable] && [_textview horizontalSpacing] == horizontalSpacing &&
    ...
    [_textview setFontTable:newFontTable horizontalSpacing:horizontalSpacing verticalSpacing:verticalSpacing];
    if (self.isBrowserSession) {
        [_textview configureAsBrowser];
    }
    if (@available(macOS 11, *)) {
...
- (void)aiLockStatusDidChange:(NSNotification *)notification {
    // Only update if this notification is for our session
    if (notification.object == self) {
        BOOL locked = [[notification.userInfo objectForKey:@"locked"] boolValue];
        [_view setShowsAILockIndicator:locked];
    }
}
...
- (NSMutableAttributedString *)kernedAttributedStringForScreenChars:(NSArray<ScreenCharArray *> *)promptText
    ...
    [prompt addAttributes:@{NSKernAttributeName : @(kern)} range:NSMakeRange(0, prompt.length)];
    return prompt;
}

- (void)revealAutoComposerWithPrompt:(NSArray<ScreenCharArray *> *)promptText {
    // BUG-f1121: Replace assert with guard - initialization must be finished
    if (!_initializationFinished) {
        ELog(@"BUG-f1121: revealAutoComposerWithPrompt called before initialization finished - ignoring");
        return;
    }
    ...
    [self.composerManager setPrefix:prompt userData:userData];
}
...
- (NSMutableAttributedString *)attributedStringForScreenChars:(NSArray<ScreenCharArray *> *)promptText
    ...
    NSDictionary *defaultAttributes = [_textview attributeProviderUsingProcessedColors:YES
                                                           elideDefaultBackgroundColor:elideDefaultBackgroundColor](
        (screen_char_t){}, nil);
    NSAttributedString *space = [NSAttributedString attributedStringWithString:@" " attributes:defaultAttributes];
    ...
    NSAttributedString *body = [[promptText mapWithBlock:^id _Nullable(ScreenCharArray *sca) {
        return [sca attributedStringValueWithAttributeProvider:
                        [_textview attributeProviderUsingProcessedColors:YES
                                             elideDefaultBackgroundColor:elideDefaultBackgroundColor]];
...
- (void)profileSessionNameDidEndEditing:(NSNotification *)notification {
    ...
    if (_tmuxTitleOutOfSync && [self isTmuxClient] && [theGuid isEqualToString:_profile[KEY_GUID]]) {
        ...
        if (_tmuxController.canRenamePane) {
            ...
            [_tmuxController renameWindowWithId:_delegate.tmuxWindow inSessionNumber:nil toName:profile[KEY_NAME]];
        }
...
- (void)sessionHotkeyDidChange:(NSNotification *)notification {
    NSString *theGuid = [notification object];
    if ([self isTmuxClient] && [theGuid isEqualToString:_profile[KEY_GUID]]) {
        Profile *profile = [[ProfileModel sessionsInstance] bookmarkWithGuid:theGuid];
...
- (void)apiServerUnsubscribe:(NSNotification *)notification {
    // BUG-295: Guard against nil notification.object to prevent crash
    id key = notification.object;
    if (key == nil) {
        return;
    }
    [_promptSubscriptions removeObjectForKey:key];
    [_keystrokeSubscriptions removeObjectForKey:key];
    [_keyboardFilterSubscriptions removeObjectForKey:key];
    [_updateSubscriptions removeObjectForKey:key];
    [_customEscapeSequenceNotifications removeObjectForKey:key];
}
...
- (void)synchronizeTmuxFonts:(NSNotification *)notification {
    if (!_exited && [self isTmuxClient]) {
        ...
        // BUG-f1418: Guard array bounds before accessing elements 0-4
        if (![args isKindOfClass:[NSArray class]] || args.count < 5) {
            DLog(@"BUG-f1418: synchronizeTmuxFonts notification has insufficient elements (count=%lu)",
...
- (void)notifyTmuxFontChange {
    ...
    if (!fontChangeNotificationInProgress) {
        ...
        [[NSNotificationCenter defaultCenter]
            postNotificationName:kTmuxFontChanged
                          object:@[
                              _textview.fontTable, @(_textview.horizontalSpacing), @(_textview.verticalSpacing),
                              _tmuxController ?: [NSNull null], @(self.delegate.tmuxWindow)
                          ]];
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kPTYSessionTmuxFontDidChange object:self];
    }
}
...
- (void)inheritDivorceFrom:(PTYSession *)parent decree:(NSString *)decree {
    // BUG-f1122: Replace assert with guard - parent must not be nil
    if (!parent) {
        ELog(@"BUG-f1122: inheritDivorceFrom called with nil parent - ignoring");
        return;
    }
    ...
    [ProfileModel
        log:[NSString
                stringWithFormat:@"DIVORCE_INHERIT: session=%p currentGUID=%@ parentGUID=%@ "
                                 @"inheritedOverriddenFields=%@ KEY_GUID_in_overriddenFields=%d originalProfileGUID=%@",
                                 self, self.profile[KEY_GUID], parent.profile[KEY_GUID], _overriddenFields,
                                 [_overriddenFields containsObject:KEY_GUID], _originalProfile[KEY_GUID]]];
}
...
- (void)setIsDivorced:(BOOL)isDivorced withDecree:(NSString *)decree {
    ...
    [self setDivorceDecree:[NSString stringWithFormat:@"isDivorced=%@ Decree=%@ guid=%@ Stack:\n%@", @(isDivorced),
                                                      decree, guid, [NSThread callStackSymbols]]];
}
...
- (NSString *)divorceAddressBookEntryFromPreferences {
    ...
    if (self.isDivorced) {
        ...
        [ProfileModel
            log:[NSString
                    stringWithFormat:
                        @"DIVORCE_ASSERTION_CHECK: guid=%@ inShared=%d inSessions=%d divorced=%d overriddenFields=%@",
                        guid, guidInShared, guidInSessions, _divorced, _overriddenFields]];

        ITAssertWithMessage(
            [[ProfileModel sessionsInstance] bookmarkWithGuid:guid] != nil,
    ...
    NSMutableArray<NSString *> *logs = [NSMutableArray arrayWithCapacity:16]; // Debug divorce logs
    ...
    if (!existingOriginalGuid || ![[ProfileModel sharedInstance] bookmarkWithGuid:existingOriginalGuid] ||
    ...
    [[ProfileModel sessionsInstance] addGuidToDebug:guid];
    [[ProfileModel sessionsInstance] setObject:guid forKey:KEY_GUID inBookmark:bookmark];
    [_overriddenFields removeAllObjects];
...
- (void)setCurrentMarkOrNotePosition:(Interval *)currentMarkOrNotePosition {
    ...
    ITBetaAssert(currentMarkOrNotePosition.limit >= 0, @"Negative limit in current mark or note %@",
                 currentMarkOrNotePosition);
}
...
- (void)setOrAppendComposerString:(NSString *)string {
    [self setComposerString:[(self.composerManager.contents ?: @"") stringByAppendingString:string] forceLarge:NO];
}

- (NSDictionary<NSString *, NSNumber *> *)scrollbackPackingStats {
    id<LineBufferReading> linebuffer = _screen.mutableState.linebuffer;
    if (!linebuffer) {
        return nil;
    }
    LineBufferPackingStats stats = [linebuffer packingStats];
    return @{
        @"totalBlocks" : @(stats.totalBlocks),
        @"packedBlocks" : @(stats.packedBlocks),
        @"currentBytes" : @(stats.currentBytes),
        @"estimatedBytesSaved" : @(stats.estimatedBytesSaved),
        @"estimatedUnpackedBytes" : @(stats.estimatedUnpackedBytes),
        @"totalRawLines" : @(stats.totalRawLines),
        @"packedRawLines" : @(stats.packedRawLines),
        @"totalCharacters" : @(stats.totalCharacters),
        @"packedCharacters" : @(stats.packedCharacters),
    };
}
...
- (void)setComposerString:(NSString *)string forceLarge:(BOOL)forceLarge {
    ...
    _composerClearTurdDetector = nil;
    __weak __typeof(self) weakSelf = self;
    iTermExpectation *composerClearTurdDetector =
        [_expect expectRegularExpression:[self regularExpressionForDataContainingOnlyControlCharacters:data]
                                   after:nil
...
- (void)setFilter:(NSString *)filter {
    ...
    DLog(@"%@: Before updateFindDriver: findDriverType=%@, findDriver=%@", self, @(self.view.findDriverType),
         self.view.findDriver);
    [self.view updateFindDriver];
    DLog(@"%@: After updateFindDriver: findDriverType=%@, findDriver=%@", self, @(self.view.findDriverType),
         self.view.findDriver);
    [self.view createFindDriverIfNeeded];
    DLog(@"%@: After createFindDriverIfNeeded: findDriverType=%@, findDriver=%@", self, @(self.view.findDriverType),
         self.view.findDriver);
    iTermFindDriver *findDriver = self.view.findDriver;
    if (!findDriver) {
        iTermStatusBarViewController *statusBarVC = _statusBarViewController;
        DLog(@"%@: ERROR: No find driver available for filtering. findDriverType=%@, statusBarVC=%@, "
             @"searchViewController=%@, filterViewController=%@, temporaryLeftComponent=%@",
             self, @(self.view.findDriverType), statusBarVC, statusBarVC.searchViewController,
             statusBarVC.filterViewController, statusBarVC.temporaryLeftComponent);
        return;
    }
    ...
    // BUG-f1123: Replace assert with guard - liveSession must exist for filtering
    if (!sourceSession) {
        ELog(@"BUG-f1123: setFilter called without live session - ignoring");
        return;
    }
    ...
    DLog(@"%@: Previous asyncFilter=%@, mode=%@. Current findDriver.filterMode=%@, isRegex=%@", self, refining,
         @(refining.mode), @(findDriver.filterMode), @(iTermFilterModeIsRegularExpression(findDriver.filterMode)));
    if (_asyncFilter.mode != findDriver.filterMode || iTermFilterModeIsRegularExpression(findDriver.filterMode)) {
        DLog(@"%@: Setting refining=nil because %@", self,
    ...
    if (replacingFilter) {
        DLog(
            @"%@: Clear buffer because there is a pre-existing filter (was '%@'). Discarding %@ pending filter updates",
            self, _filter, @(_pendingFilterUpdates.count));
        ...
        [self.screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                                     id<VT100ScreenDelegate> delegate) {
...
- (NSImage *)snapshotCenteredOn:(VT100GridAbsCoord)coord size:(NSSize)size {
    ...
    VT100GridCoord relativeCoord = VT100GridCoordMake(coord.x, coord.y - _screen.totalScrollbackOverflow);
    ...
    NSRect rect = NSMakeRect(MIN(MAX(0, centerPoint.x - size.width / 2), NSWidth(_textview.bounds)),
                             ...
                             MIN(NSWidth(_textview.bounds), size.width), MIN(NSHeight(_textview.bounds), size.height));
...
+ (CGContextRef)onePixelContext {
    ...
    if (context == NULL) {
        context = CGBitmapContextCreate(NULL, 1, 1, 8, 1 * 4, [self metalColorSpace],
...
- (void)setMetalContextSize:(CGSize)size {
    ...
    _metalContext =
        CGBitmapContextCreate(NULL, scaledSize.width, scaledSize.height, 8,
                              scaledSize.width * 4, // bytes per row
                              [PTYSession metalColorSpace], kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrder32Host);
}
...
- (BOOL)usingIntegratedGPU {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // BUG-CRASH-PAC: Use CFArrayRef with NULL check to avoid crashes
        // if MTLCopyAllDevices() returns NULL or a corrupt pointer.
        CFArrayRef devices = (CFArrayRef)MTLCopyAllDevices();
        haveIntegrated = NO;
        if (devices != NULL) {
            const CFIndex count = CFArrayGetCount(devices);
            for (CFIndex i = 0; i < count; i++) {
                id<MTLDevice> device = (__bridge id<MTLDevice>)CFArrayGetValueAtIndex(devices, i);
...
- (BOOL)metalAllowed:(out iTermMetalUnavailableReason *)reason {
    ...
    dispatch_once(&onceToken, ^{
        // BUG-CRASH-PAC: Use CF functions instead of ObjC messaging to avoid
        // pointer authentication failures during early app launch. The Metal
        // framework can return arrays with corrupt isa pointers in edge cases.
        CFArrayRef devices = (CFArrayRef)MTLCopyAllDevices();
        if (devices != NULL) {
            machineSupportsMetal = CFArrayGetCount(devices) > 0;
            CFRelease(devices);
        } else {
            machineSupportsMetal = NO;
        }
    });
    ...
    if ([self usingIntegratedGPU] && [_delegate.realParentWindow.ptyWindow approximateFractionOccluded] > 0.5) {
        if (reason) {
...
- (BOOL)canProduceMetalFramecap {
    DLog(@"useMetal=%@ view.metalView.alphaValue=%@ wrapper.useMetal=%@ textview.suppressDrawing=%@", @(_useMetal),
         @(_view.metalView.alphaValue == 1), @(_wrapper.useMetal), @(_textview.suppressDrawing));
...
- (BOOL)metalViewSizeIsLegal NS_AVAILABLE_MAC(10_11) {
    ...
    return size.width > 0 && size.width < (PTYSessionMaximumMetalViewSize / maxScale) && size.height > 0 &&
           size.height < (PTYSessionMaximumMetalViewSize / maxScale);
}

- (BOOL)idleForMetal {
    return (!_cadenceController.isActive && !_view.verticalScroller.userScroll &&
            !self.overrideGlobalDisableMetalWhenIdleSetting && !_view.driver.captureDebugInfoForNextFrame);
}
            ...
            drawAsynchronouslyInView:_view.metalView
                          completion:^(BOOL ok) {
                              ...
                              if (!ok) {
                                  ...
                                  // Wait 10ms to avoid burning CPU if it failed because it's slow to draw the first
                                  // frame.
...
- (void)showMetalViewImmediately {
    ...
    if (_view.metalView.bounds.size.width == 0 || _view.metalView.bounds.size.height == 0) {
        DLog(@"Declining to show metal view immediately in %@ because the view's size is %@", self,
             NSStringFromSize(_view.metalView.bounds.size));
        return;
    }
    if (_textview == nil) {
    ...
    if (_screen.width == 0 || _screen.height == 0) {
        DLog(@"Declining to show metal view immediately in %@ because the screen's size is %@x%@. Screen is %@", self,
             _textview.dataSource, @(_screen.width), @(_screen.height));
        return;
    }
...
- (void)updateMetalDriver NS_AVAILABLE_MAC(10_11) {
    DLog(@"%@", self);
    // Guard: fontTable can be nil during app termination (window close).
    // This is not an error - just skip the update.
    if (!_textview.fontTable) {
        DLog(@"fontTable is nil, skipping updateMetalDriver");
        return;
    }
...
- (void)launchCoprocessWithCommand:(NSString *)command mute:(BOOL)mute {
    ...
    Coprocess *coprocess = [Coprocess launchedCoprocessWithCommand:command environment:env];
...
- (void)setFocused:(BOOL)focused {
    ...
    if (_screen.terminalReportFocus) {
        ...
        [self writeLatin1EncodedData:[_screen.terminalOutput reportFocusGained:focused]
                    broadcastAllowed:NO
                           reporting:NO];
        _reportingFocus = NO;
    }
...
- (BOOL)wantsContentChangedNotification {
    ...
    return (!_tailFindController.isPerformingContinuousTailFind && !_view.findViewIsHidden &&
...
- (NSString *)preferredTmuxClientName {
    ...
    if (remoteHost) {
        ...
        NSString *name = [_nameController.presentationSessionTitle
...
- (void)setTmuxMode:(PTYSessionTmuxMode)tmuxMode {
    os_unfair_lock_lock(&sTmuxModeLock);
    _tmuxMode = tmuxMode;
    os_unfair_lock_unlock(&sTmuxModeLock);
    ...
    __weak __typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf reallySetTmuxMode:tmuxMode];
    });
}
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        switch (tmuxMode) {
            ...
            case TMUX_CLIENT: {
                ...
                iTermCPUUtilization *gatewayInstance =
                    [iTermCPUUtilization instanceForSessionID:self.tmuxGatewaySession.guid];
...
- (void)setUpTmuxPipe {
    // BUG-f1124: Replace assert with guard - tmux pipe must not already exist
    if (_tmuxClientWritePipe) {
        ELog(@"BUG-f1124: setUpTmuxPipe called when pipe already exists - ignoring");
        return;
    }
    int fds[2];
    if (pipe(fds) < 0) {
    ...
    [_tmuxClientWritePipe release];
    _tmuxClientWritePipe = [[NSFileHandle alloc] initWithFileDescriptor:fds[1] closeOnDealloc:YES];
}
...
typedef NS_ENUM(NSUInteger, PTYSessionTmuxReport) {
    ...
    PTYSessionTmuxReportAll = (PTYSessionTmuxReportForeground | PTYSessionTmuxReportBackground)
};
...
- (void)sendTmuxPerPaneReports:(PTYSessionTmuxReport)reports {
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        if (reports & PTYSessionTmuxReportForeground) {
            fgReport = [[terminal.output reportColor:fgColor atIndex:10 prefix:@""] copy];
        }
        if (reports & PTYSessionTmuxReportBackground) {
            bgReport = [[terminal.output reportColor:bgColor atIndex:11 prefix:@""] copy];
        }
    }];
    if (fgReport) {
...
- (void)didFetchTmuxPid:(NSString *)pidString {
    if (pidString && self.tmuxMode == TMUX_CLIENT && _tmuxController.serverIsLocal) {
        const pid_t parsedPid = (pid_t)pidString.integerValue;
        if (parsedPid > 0) {
            NSNumber *pid = [iTermLSOF cachedNumberForPid:parsedPid];
...
- (void)installTmuxStatusBarMonitor {
    // BUG-f1125: Replace assert with guard - tmux status bar monitor must not already exist
    if (_tmuxStatusBarMonitor) {
        ELog(@"BUG-f1125: installTmuxStatusBarMonitor called when monitor already exists - ignoring");
        return;
    }
        ...
        if ([iTermPreferences boolForKey:kPreferenceKeyUseTmuxStatusBar] ||
            ...
            [self setSessionSpecificProfileValues:@{
                KEY_STATUS_BAR_LAYOUT :
                    [[iTermStatusBarLayout tmuxLayoutWithController:_tmuxController
                                                              scope:nil
...
- (void)installTmuxForegroundJobMonitor {
    ...
    __weak __typeof(self) weakSelf = self;
    _tmuxForegroundJobMonitor =
        [[iTermTmuxOptionMonitor alloc] initWithGateway:_tmuxController.gateway
                                                  scope:self.variablesScope
                                   fallbackVariableName:nil
...
- (PTYSessionTmuxMode)tmuxMode {
    os_unfair_lock_lock(&sTmuxModeLock);
    PTYSessionTmuxMode mode = _tmuxMode;
    os_unfair_lock_unlock(&sTmuxModeLock);
    return mode;
}
...
- (void)startTmuxMode:(NSString *)dcsID {
    ...
    // will ignore the ^C.
    [self writeTaskImpl:[NSString stringWithLongCharacter:3]
...
- (BOOL)isCompatibleWith:(PTYSession *)otherSession {
    ...
    if ((selfIsBrowser && otherSession.tmuxMode == TMUX_CLIENT) || (self.tmuxMode == TMUX_CLIENT && otherIsBrowser)) {
        return NO;
    }
...
- (void)addNoteAtCursor {
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        VT100GridCoordRange rangeAtCursor =
            [self smartSelectionRangeAt:VT100GridCoordMake(_screen.cursorX - 1, _screen.cursorY - 1)];
        VT100GridCoordRange rangeBeforeCursor =
            [self smartSelectionRangeAt:VT100GridCoordMake(_screen.cursorX - 2, _screen.cursorY - 1)];
        VT100GridCoordRange rangeAfterCursor =
            [self smartSelectionRangeAt:VT100GridCoordMake(_screen.cursorX, _screen.cursorY - 1)];
        if (VT100GridCoordRangeLength(rangeAtCursor, _screen.width) > 0) {
            ...
            [_screen addNote:note inRange:VT100GridCoordRangeMake(0, y, _screen.width, y) focus:YES visible:YES];
        }
    }];
}
...
- (void)textViewToggleAnnotations {
    VT100GridCoordRange range =
        VT100GridCoordRangeMake(0, 0, _screen.width, _screen.height + [_screen numberOfScrollbackLines]);
...
- (void)textViewDisconnectSSH {
    if (!_conductor.framing) {
        NSString *title = [NSString
            stringWithFormat:@"Advanced SSH features are unavailable because Python %@ or later was not found on %@",
                             [iTermConductor minimumPythonVersionForFramer],
                             _conductor.sshIdentity.hostname ?: @"remote host"];
...
- (BOOL)markIsNavigable:(id<iTermMark>)mark {
    return ([mark isKindOfClass:[VT100ScreenMark class]] || [mark isKindOfClass:[PTYAnnotation class]]);
}

- (void)selectPreviousCommandMark {
    id<VT100ScreenMarkReading> mark =
        [_screen promptMarkBeforePromptMark:_selectedCommandMark] ?: [_screen lastPromptMark];
    if (mark) {
...
- (void)selectNextCommandMark {
    id<VT100ScreenMarkReading> mark =
        [_screen promptMarkAfterPromptMark:_selectedCommandMark] ?: [_screen firstPromptMark];
    if (mark) {
        [self selectCommandWithMark:mark];
        [self setCurrentMarkOrNote:mark];
    }
...
- (void)tmuxDidOpenInitialWindows {
    if (_hideAfterTmuxWindowOpens) {
        ...
        if (![[NSUserDefaults standardUserDefaults] boolForKey:kAutoBurialKey]) {
            [[iTermNotificationController sharedInstance]
                         notify:@"Session Buried"
                withDescription:
                    @"It can be restored by detaching from tmux, or from the Sessions > Buried Sessions menu."];
...
- (BOOL)tmuxUpdateLayoutForWindow:(int)windowId
                    ...
                    visibleLayout:(NSString *)visibleLayout
                           zoomed:(NSNumber *)zoomed
                             only:(BOOL)only {
    DLog(@"tmuxUpdateLayoutForWindow:%@ layout:%@ zoomed:%@ only:%@", @(windowId), layout, zoomed, @(only));
    PTYTab *tab = [_tmuxController window:windowId];
    if (!tab) {
        DLog(@"* NO TAB, DO NOTHING");
        return NO;
    }
    const BOOL result = [_tmuxController setLayoutInTab:tab toLayout:layout visibleLayout:visibleLayout zoomed:zoomed];
    if (result && only) {
...
- (void)tmuxGatewayDidTimeOutDuringInitialization:(BOOL)duringInitialization {
    if (duringInitialization) {
        const iTermWarningSelection selection = [iTermWarning
            showWarningWithTitle:@"It’s taking a long time for tmux to respond. If this is a old or funky system it "
...
+ (dispatch_queue_t)tmuxQueue {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        tmuxQueue = dispatch_queue_create("com.dashterm.dashterm2.tmuxReadTask", 0);
    });
    return tmuxQueue;
}
...
- (void)handleTmuxData:(NSData *)data {
    ...
    dispatch_async([[self class] tmuxQueue], ^{
        @try {
            ...
            // RC-030: Pipe write failure must be reported, not silently ignored.
            // This typically indicates the tmux session has been disconnected.
            DLog(@"FATAL: %@ while writing to tmux pipe - triggering broken pipe", exception);
            dispatch_async(dispatch_get_main_queue(), ^{
                __strong __typeof(weakSelf) strongSelf = weakSelf;
                if (strongSelf && !strongSelf.exited) {
                    NSString *message = [NSString stringWithFormat:@"Tmux pipe write failed: %@", exception.name];
                    [strongSelf brokenPipeWithError:message];
                }
            });
        }
    });
...
- (void)showTmuxPausedAnnouncement:(BOOL)notification {
    ...
    __weak __typeof(self) weakSelf = self;
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:title
                       ...
                       switch (selection) {
                           ...
                           case 1:
                               [[PreferencePanel sharedInstance]
                                   openToPreferenceWithKey:kPreferenceKeyTmuxPauseModeAgeLimit];
...
- (void)setTmuxHistory:(TmuxHistory *)history altHistory:(TmuxHistory *)altHistory state:(NSDictionary *)state {
    __weak __typeof(self) weakSelf = self;
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                             id<VT100ScreenDelegate> delegate) {
...
- (VT100GridSize)tmuxClientSize {
    ...
    DLog(@"Get size from delegate %@, controller tmuxController %@, window %@", _delegate, _tmuxController,
...
- (void)tmuxDoubleAttachForSessionGUID:(NSString *)sessionGUID {
    ...
    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:@"This instance of DashTerm2 is already attached to this session"
                                   actions:actions
                                 accessory:nil
    ...
    switch (selection) {
        ...
        case kiTermWarningSelection2:
            ...
            return;
        default:
            // BUG-12006: Handle unexpected warning selection gracefully instead of assert(NO)
            DLog(@"BUG-12006: Unexpected warning selection %d in tmux warning handler", (int)selection);
            return;
    }
...
- (BOOL)event:(NSEvent *)event matchesPattern:(ITMKeystrokePattern *)pattern {
    ...
    NSMutableArray *actualModifiers = [NSMutableArray arrayWithCapacity:6]; // At most 6 modifier types
...
- (VT100SyncResult)textViewWillRefresh {
    [self syncDTermCoreDisplayOffset];
    return [self syncCheckingTriggers:VT100ScreenTriggerCheckTypePartialLines resetOverflow:YES];
}
...
- (void)sync {
    ...
    const VT100SyncResult result = [self syncCheckingTriggers:VT100ScreenTriggerCheckTypeNone resetOverflow:NO];
    DLog(@"Sync done");
...
- (VT100SyncResult)syncCheckingTriggers:(VT100ScreenTriggerCheckType)checkTriggers resetOverflow:(BOOL)resetOverflow {
    ...
    [_screen performLightweightBlockWithJoinedThreads:^(VT100ScreenMutableState *mutableState) {
        ...
        result = [self syncCheckingTriggers:checkTriggers resetOverflow:resetOverflow mutableState:mutableState];
    }];
    if (result.namedMarksChanged) {
        [[[[iTermNamedMarksDidChangeNotification alloc] initWithSessionGuid:self.guid] autorelease] post];
    }
...
- (VT100SyncResult)syncCheckingTriggers:(VT100ScreenTriggerCheckType)checkTriggers
                          resetOverflow:(BOOL)resetOverflow
                           mutableState:(VT100ScreenMutableState *)mutableState {
    DLog(@"syncCheckingTriggers:%@ resetOverflow:%@ mutableState:%@ self:%@", @(checkTriggers), @(resetOverflow),
         mutableState, self);
    [self updateConfigurationFields];
    const BOOL expectWasDirty = _expect.dirty;
    [_expect resetDirty];
...
- (NSArray<NSNumber *> *)apiModifiersForModifierFlags:(NSEventModifierFlags)flags {
    NSMutableArray<NSNumber *> *mods = [NSMutableArray arrayWithCapacity:6]; // At most 6 modifier types
...
- (void)sendKeystrokeNotificationForEvent:(NSEvent *)event advanced:(BOOL)advanced {
    ...
    [_keystrokeSubscriptions enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                                                 BOOL *_Nonnull stop) {
        if (advanced && !obj.keystrokeMonitorRequest.advanced) {
            return;
        }
        [[iTermAPIHelper sharedInstance] postAPINotification:notification toConnectionKey:key];
    }];
}
...
- (BOOL)eventAbortsPasteWaitingForPrompt:(NSEvent *)event {
    ...
    const NSEventModifierFlags mask = (NSEventModifierFlagOption | NSEventModifierFlagShift |
...
+ (void)reportFunctionCallError:(NSError *)error
...
- (void)invokeFunctionCall:(NSString *)invocation scope:(iTermVariableScope *)scope origin:(NSString *)origin {
...
- (void)applyAction:(iTermAction *)action {
    ...
    if (self.isBrowserSession) {
        ...
        [self.view.browserViewController performKeyBindingAction:keyBindingAction event:nil];
        return;
    }
    [self.textview.window makeFirstResponder:self.mainResponder];
    [self performKeyBindingAction:keyBindingAction event:nil];
}
...
+ (BOOL)performKeyBindingAction:(iTermKeyBindingAction *)action event:(NSEvent *)event {
    ...
    switch (action.keyAction) {
        ...
        case KEY_ACTION_BYPASS:
            // Not in a terminal so nothing to bypass. This allows it to go to Cocoa (e.g., to launch a Shortcut; see
            // issue 12273)
            return NO;
        ...
        case KEY_ACTION_NEW_WINDOW_WITH_PROFILE: {
            ...
            [iTermSessionLauncher launchBookmark:profile inTerminal:nil respectTabbingMode:NO completion:nil];
            return YES;
        }
    ...
    // BUG-500: Replace assert(false) with ELog - unknown key action should not crash
    ELog(@"PTYSession performKeyBindingAction: unknown action key=%d", (int)action.keyAction);
    return NO;
}
...
- (void)reallyPerformKeyBindingAction:(iTermKeyBindingAction *)action event:(NSEvent *)event {
    ...
    id<iTermWindowController> windowController =
        self.delegate.realParentWindow ?: [[iTermController sharedInstance] currentTerminal];
    ...
    switch (action.keyAction) {
        ...
        case KEY_ACTION_MOVE_TAB_RIGHT:
            ...
            break;
        case KEY_ACTION_NEXT_MRU_TAB:
            [[[_delegate realParentWindow] tabView] cycleKeyDownWithModifiers:[event it_modifierFlags] forwards:YES];
            break;
        case KEY_ACTION_PREVIOUS_MRU_TAB:
            [[[_delegate realParentWindow] tabView] cycleKeyDownWithModifiers:[event it_modifierFlags] forwards:NO];
            break;
        case KEY_ACTION_NEXT_PANE:
        ...
        case KEY_ACTION_PASTE_SPECIAL_FROM_SELECTION: {
            ...
            if (string.length) {
                [_pasteHelper pasteString:string stringConfig:action.parameter];
            }
            break;
        }
        ...
        case KEY_ACTION_TOGGLE_HOTKEY_WINDOW_PINNING: {
            ...
            DLog(@"Getting profile with guid %@ from originalProfile %p", self.originalProfile[KEY_GUID],
                 self.originalProfile);
            Profile *profile = [[ProfileModel sharedInstance] bookmarkWithGuid:self.originalProfile[KEY_GUID]];
            if (profile) {
                DLog(@"Found a profile");
                [iTermProfilePreferences setBool:!autoHid
        ...
        case KEY_ACTION_TOGGLE_MOUSE_REPORTING:
            ...
            break;
        case KEY_ACTION_INVOKE_SCRIPT_FUNCTION:
            [self invokeFunctionCall:action.parameter scope:self.variablesScope origin:@"Key Binding"];
            break;
        case KEY_ACTION_DUPLICATE_TAB:
            ...
            [[[[iTermExpressionEvaluator alloc] initWithStrictInterpolatedString:parameter
                ...
                evaluateWithTimeout:0
                         completion:^(iTermExpressionEvaluator *evaluator) {
                             if (evaluator.error) {
                                 [iTermWarning
                                     showWarningWithTitle:
                                         [NSString
                                             stringWithFormat:
                                                 @"The key-binding action Copy Interpolated String “%@” failed:\n\n%@",
                                 ...
                                 [entry
                                      addOutput:[NSString
                                                    stringWithFormat:@"Error evaluating interpolated string “%@”:\n%@",
                             ...
                             [[NSPasteboard generalPasteboard] setString:evaluator.value
        ...
        case KEY_ACTION_COPY_MODE: {
            ...
            if (error) {
                [self showError:error suppressionKey:@"NoSyncSuppressCopyModeErrors" identifier:@"Copy Mode Error"];
            }
            break;
        }

        case KEY_ACTION_TOGGLE_SETTING: {
            [self toggleSettingWithKey:action.toggleSettingKey isProfile:action.toggleSettingIsProfile];
            break;
        }
...
- (void)toggleSettingWithKey:(NSString *)key isProfile:(BOOL)isProfile {
    if (isProfile) {
...
- (BOOL)eventUsesBuckyBits:(NSEvent *)event {
    ...
    if ([iTermProfilePreferences unsignedIntegerForKey:KEY_LEFT_COMMAND
                                             inProfile:self.profile] != iTermBuckyBitRegular &&
        (event.modifierFlags & NX_DEVICELCMDKEYMASK) != 0) {
        return YES;
    }
    if ([iTermProfilePreferences unsignedIntegerForKey:KEY_RIGHT_COMMAND
                                             inProfile:self.profile] != iTermBuckyBitRegular &&
        (event.modifierFlags & NX_DEVICERCMDKEYMASK) != 0) {
        return YES;
    }
    if ([iTermProfilePreferences unsignedIntegerForKey:KEY_LEFT_CONTROL
                                             inProfile:self.profile] != iTermBuckyBitRegular &&
        (event.modifierFlags & NX_DEVICELCTLKEYMASK) != 0) {
        return YES;
    }
    if ([iTermProfilePreferences unsignedIntegerForKey:KEY_RIGHT_CONTROL
                                             inProfile:self.profile] != iTermBuckyBitRegular &&
...
- (void)textViewSelectionDidChangeToTruncatedString:(NSString *)maybeSelection {
    ...
    [self.variablesScope setValue:[selection substringToIndex:MIN(maxLength, selection.length)]
                 forVariableNamed:iTermVariableKeySessionSelection];
...
- (void)regularKeyDown:(NSEvent *)event {
    ...
    const NSEventModifierFlags mask = (NSEventModifierFlagCommand | NSEventModifierFlagOption |
...
- (BOOL)performTextReplacement {
    ...
    VT100GridCoord coord =
        VT100GridCoordMake(self.screen.cursorX - 1, self.screen.numberOfScrollbackLines + self.screen.cursorY - 1);
    ...
    NSString *word = nil;
    while ([manager hasReplacementsWithSuffix:suffix] && !VT100GridCoordEquals(coord, lastCoord)) {
        word = [suffix copy];
...
- (void)logKeystroke:(NSEvent *)event {
    const unichar unicode = event.characters.length > 0 ? [event.characters characterAtIndex:0] : 0;
    DLog(@"event:%@ (%llx+%x)[%@][%@]:%x(%c) <%lu>", event, (unsigned long long)event.it_modifierFlags, event.keyCode,
         event.characters, event.charactersIgnoringModifiers, unicode, unicode,
...
- (void)restoreStateForZoom:(PTYSessionZoomState *)state {
    ...
    [_textview scrollToAbsoluteOffset:state.firstVisibleAbsoluteLineNumber height:_screen.height];
...
- (void)handleKeypressInInstantReplay:(NSEvent *)event {
    ...
    const unichar character = event.characters.length > 0 ? [event.characters characterAtIndex:0] : 0;
    const unichar characterIgnoringModifiers =
        [event.charactersIgnoringModifiers length] > 0 ? [event.charactersIgnoringModifiers characterAtIndex:0] : 0;
...
- (NSData *)backspaceData {
    iTermKeyBindingAction *action =
        [iTermKeyMappings actionForKeystroke:[iTermKeystroke backspace]
                                 keyMappings:[[self profile] objectForKey:KEY_KEYBOARD_MAP]];
    if (!action) {
        const char del = 0x7f;
...
- (iTermOptionKeyBehavior)optionKey {
    if ([self shouldRespectTerminalMetaSendsEscape] && _screen.terminalMetaSendsEscape &&
...
- (void)menuForEvent:(NSEvent *)theEvent menu:(NSMenu *)theMenu {
    ...
    if ([_delegate realParentWindow] && [[_delegate realParentWindow] respondsToSelector:@selector(menuForEvent:
                                                                                                           menu:)]) {
...
- (void)paste:(id)sender {
    ...
    // If this class is used in a non-DashTerm2 app (as a library), we might not
...
- (BOOL)textViewDrawBackgroundImageInView:(NSView *)view
    ...
    if ([iTermPreferences boolForKey:kPreferenceKeyPerPaneBackgroundImage]) {
        ...
        [_backgroundDrawingHelper
            drawBackgroundImageInView:view
                            container:self.view
                            dirtyRect:dirtyRect
        ...
        NSRect windowVisibleRect =
            [self.view insetRect:container.bounds
                               flipped:YES
...
- (CGRect)textViewRelativeFrame {
    ...
    viewRect = [self.view.metalView.superview convertRect:self.view.metalView.frame toView:container];
    containerBounds = container.bounds;
    // Flip it
    viewRect.origin.y = containerBounds.size.height - viewRect.origin.y - viewRect.size.height;
    return CGRectMake(viewRect.origin.x / containerBounds.size.width, viewRect.origin.y / containerBounds.size.height,
...
- (void)textViewPostTabContentsChangedNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"iTermTabContentsChanged" object:self userInfo:nil];
}
...
- (void)textViewDidFindDirtyRects {
    if (_updateSubscriptions.count) {
        ...
        [_updateSubscriptions enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                                                  BOOL *_Nonnull stop) {
            [[iTermAPIHelper sharedInstance] postAPINotification:notification toConnectionKey:key];
        }];
    }
...
- (BOOL)textViewShouldPlaceCursorAt:(VT100GridCoord)coord verticalOk:(BOOL *)verticalOk {
    DLog(@"coord=%@, _screen.numberOfLines=%@, _screen.height=%@, _screen.width=%@", VT100GridCoordDescription(coord),
         @(_screen.numberOfLines), @(_screen.height), @(_screen.width));
    if (coord.y < _screen.numberOfLines - _screen.height || coord.x < 0 || coord.x >= _screen.width ||
    ...
    if (_screen.commandRange.start.x < 0) {
        ...
        // arrows please.
        NSComparisonResult order = VT100GridCoordOrder(VT100GridCoordRangeMin(_screen.commandRange), coord);
...
- (void)textViewSelectMenuItemWithIdentifier:(NSString *)identifier title:(NSString *)title {
    [[NSApp mainMenu] it_selectMenuItemWithTitle:title identifier:identifier];
}

- (void)textViewPasteSpecialWithStringConfiguration:(NSString *)configuration fromSelection:(BOOL)fromSelection {
    NSString *string = fromSelection ? [[iTermController sharedInstance] lastSelectionPromise].wait.maybeFirst
                                     : [NSString stringFromPasteboard];
    if (string) {
        [_pasteHelper pasteString:string stringConfig:configuration];
    }
}
...
- (void)setReportingMouseDownForEventType:(NSEventType)eventType {
    switch (eventType) {
        ...
        default:
            // BUG-431: Replace assert(NO) with DLog for unexpected event types
            DLog(@"PTYSession: Unexpected event type %ld in setReportingMouseDownForEventType:value:", (long)eventType);
            return;
    }
}
...
- (BOOL)reportingMouseDownForEventType:(NSEventType)eventType {
    switch (eventType) {
        ...
        default:
            // BUG-432: Replace assert(NO) with DLog and NO return for unexpected event types
            DLog(@"PTYSession: Unexpected event type %ld in reportingMouseDownForEventType:", (long)eventType);
            return NO;
    }
}
...
- (BOOL)textViewReportMouseEvent:(NSEventType)eventType
    ...
    DLog(@"Report event type %lu, modifiers=%lu, button=%d, coord=%@ testOnly=%@ terminalMouseMode=%@ "
         @"allowDragBeforeMouseDown%@",
         (unsigned long)eventType, (unsigned long)modifiers, button, VT100GridCoordDescription(coord), @(testOnly),
         @(_screen.terminalMouseMode), @(allowDragBeforeMouseDown));
    ...
    switch (eventType) {
                ...
                case MOUSE_REPORTING_ALL_MOTION:
                    ...
                    if (!testOnly) {
                        ...
                        DLog(@"_lastReportedCoord <- %@, _lastReportedPoint <- %@",
                             VT100GridCoordDescription(_lastReportedCoord), NSStringFromPoint(point));
        ...
        default:
            // BUG-433: Replace assert(NO) with DLog for unexpected event types
            DLog(@"PTYSession: Unexpected event type %ld in textViewReportMouseEvent:", (long)eventType);
            break;
    }
    return NO;
}
...
- (void)detectTurdsForReportData:(NSData *)data type:(PTYSessionTurdType)turdType {
    DLog(@"Will report %@ (%@)", [data stringWithEncoding:NSUTF8StringEncoding],
         [[data stringWithEncoding:NSUTF8StringEncoding] hexEncodedString]);
    ...
    if (!modified) {
        modified = [[NSMutableData alloc] initWithCapacity:data.length];
    }
    DLog(@"Modified is initially %@ (%@)", [modified stringWithEncoding:NSUTF8StringEncoding],
         [[modified stringWithEncoding:NSUTF8StringEncoding] hexEncodedString]);
    if (_turdDetector) {
    ...
    if (modified.length > maxLength) {
        ...
        [modified replaceBytesInRange:NSMakeRange(0, modified.length - maxLength) withBytes:"" length:0];
    }
...
- (VT100GridAbsCoordRange)textViewRangeOfLastCommandOutput {
    ...
    if (![[iTermShellHistoryController sharedInstance] commandHistoryHasEverBeenUsed]) {
        ...
        if (self.isAtShellPrompt || _screen.startOfRunningCommandOutput.x == -1 ||
            ...
            VT100GridAbsCoordRange range =
                VT100GridAbsCoordRangeMake(_screen.startOfRunningCommandOutput.x, _screen.startOfRunningCommandOutput.y,
                                           _screen.cursorX - 1, absCursorY);
...
- (VT100GridAbsCoordRange)textViewRangeOfCurrentCommand {
    ...
    if (![[iTermShellHistoryController sharedInstance] commandHistoryHasEverBeenUsed]) {
        ...
        if (self.isAtShellPrompt) {
            range = VT100GridAbsCoordRangeFromCoordRange(_screen.extendedCommandRange, _screen.totalScrollbackOverflow);
...
- (BOOL)textViewCanSelectCurrentCommand {
    // Return YES if command history has never been used so we can show the informational message.
    return (![[iTermShellHistoryController sharedInstance] commandHistoryHasEverBeenUsed] || self.isAtShellPrompt ||
...
- (BOOL)textViewInPinnedHotkeyWindow {
    ...
    iTermProfileHotKey *profileHotkey = [[iTermHotKeyController sharedInstance]
        profileHotKeyForWindowController:[PseudoTerminal castFrom:_delegate.realParentWindow]];
    if (!profileHotkey) {
        return NO;
    }
...
- (BOOL)textViewResizeFrameIfNeeded {
    ...
    if (fabs(desiredHeight - NSHeight(frame)) >= 0.5) {
        ...
        frame.size.height =
            desiredHeight +
            [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]; // The wrapper is always larger by VMARGIN.
        _wrapper.frame = [self safeFrameForWrapperViewFrame:frame];

        AccLog(@"Post notification: row count changed (PTYSession)");
        NSAccessibilityPostNotification(_textview, NSAccessibilityRowCountChangedNotification);
        return YES;
    }
    return NO;
}
...
- (void)textViewDidRefresh {
    ...
    // It would be nice to support bidi here, but on the other hand that would mean thinking about the touch bar.
    // Revisit this when death is cured.
    NSString *word = [textExtractor
        fastWordAt:VT100GridCoordMake(_screen.cursorX - 1, _screen.cursorY + _screen.numberOfScrollbackLines - 1)];
...
- (void)updateAppearanceForMinimalTheme {
    ...
    if (minimal) {
        NSAppearance *appearance = self.effectiveUnprocessedBackgroundColor.isDark
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == &iTermEffectiveAppearanceKey) {
        ...
        if (minimal && _screen.colorMap.useSeparateColorsForLightAndDarkMode) {
            ...
            NSAppearance *desiredAppearance = [self colorMapShouldBeInDarkMode]
                                                  ? [NSAppearance appearanceNamed:NSAppearanceNameDarkAqua]
                                                  : [NSAppearance appearanceNamed:NSAppearanceNameAqua];
...
- (BOOL)colorMapShouldBeInDarkMode {
    ...
    DLog(@"Not minimal so fall back to view (%@)/app (%@) appearance", self.view.effectiveAppearance,
...
- (void)textViewToggleAILock {
    BOOL isLocked = [DTermAIInterface isAILockedWithSession:self];
    [DTermAIInterface setAILockedWithSession:self locked:!isLocked];
}

- (BOOL)textViewIsAILocked {
    return [DTermAIInterface isAILockedWithSession:self];
}
...
- (BOOL)textViewShowHoverURL:(NSString *)url anchor:(VT100GridWindowedRange)anchor {
    return [_view setHoverURL:url anchorFrame:url ? [self boundingFrameForWindowedRange:anchor] : NSZeroRect];
}
...
- (void)textViewDidSelectPasswordPrompt {
    iTermApplicationDelegate *delegate = [iTermApplication.sharedApplication delegate];
    [delegate openPasswordManagerToAccountName:nil inSession:self];
}
...
- (void)textViewAddContextMenuItems:(NSMenu *)menu {
    ...
    if (_tmuxPaused) {
        NSMenuItem *item = [menu addItemWithTitle:@"Unpause tmux Pane"
                                           action:@selector(toggleTmuxPaused)
                                    keyEquivalent:@""];
        item.target = self;
    } else {
        NSMenuItem *item = [menu addItemWithTitle:@"Pause tmux Pane"
                                           action:@selector(toggleTmuxPaused)
                                    keyEquivalent:@""];
        item.target = self;
    }
...
- (void)openTriggersViewController {
    ...
    __weak __typeof(self) weakSelf = self;
    [self.view.window beginSheet:_triggerWindowController.window
...
- (void)textViewToggleEnableTriggersInInteractiveApps {
    const BOOL value = [iTermProfilePreferences boolForKey:KEY_ENABLE_TRIGGERS_IN_INTERACTIVE_APPS
                                                 inProfile:self.profile];
...
- (void)textViewLiveSelectionDidEnd {
    if (_textview._haveShortSelection) {
        [[iTermChatWindowController instanceShowingErrors:NO] setSelectionText:_textview.selectedText
                                                                    forSession:self.guid];
    }
}
...
- (void)textViewSetClickCoord:(VT100GridAbsCoord)coord
    ...
    self.variablesScope.mouseInfo = @[
        @(coord.x), @(coord.y), @(button), @(count), [self apiModifiersForModifierFlags:modifiers], @(sideEffects),
        @(state)
    ];
}
...
- (void)textViewShowFindIndicator:(VT100GridCoordRange)range {
    ...
    if (visibleRange.start.y > VT100GridRangeMax(visibleLines) || visibleRange.end.y < visibleLines.location) {
        return;
    }
    ...
    const NSSize imageSize =
        NSMakeSize(_screen.width * cellWidth + padding.width * 2, gridSize.height * cellHeight + padding.height * 2);
    NSImage *image =
        [NSImage flippedImageOfSize:imageSize
                          drawBlock:^{
    ...
    const NSRect subrect = NSMakeRect(hmargin + minX * cellWidth, 0, (maxX - minX) * cellWidth + padding.width * 2,
    ...
    NSImage *shadowed =
        [NSImage flippedImageOfSize:NSMakeSize(subrect.size.width + shadowInsets.left + shadowInsets.right,
                          ...
                          drawBlock:^{
                              ...
                              [cropped drawInRect:NSMakeRect(shadowInsets.left, shadowInsets.top, cropped.size.width,
                                                             cropped.size.height)];
...
- (void)sendHexCode:(NSString *)codes {
    ...
    if ([codes length]) {
        [self writeLatin1EncodedData:[self dataForHexCodes:codes] broadcastAllowed:YES reporting:NO];
    }
}
...
- (void)sendText:(NSString *)text escaping:(iTermSendTextEscaping)escaping {
    DLog(@"sendText:%@ escaping:%@", text, @(escaping));
    if (_exited) {
        DLog(@"Already exited");
        return;
    }
...
- (void)setDvrFrame {
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        // BUG-10368: Use NSInteger to support large frame sizes without overflow
        const NSInteger len = [_dvrDecoder screenCharArrayLength];
        ...
        if (info.width != mutableState.width || info.height != mutableState.height) {
            if (![_liveSession isTmuxClient]) {
                [[_delegate realParentWindow] sessionInitiatedResize:self width:info.width height:info.height];
            }
        }
        NSData *data = [NSData dataWithBytes:s length:len];
        NSMutableArray<NSArray *> *metadataArrays =
            [NSMutableArray mapIntegersFrom:0
                                         to:info.height
                                      block:^id(NSInteger i) {
        ...
        if (_dvrDecoder.migrateFromVersion > 0) {
            ...
            NSMutableData *replacement =
                [[NSMutableData alloc] initWithCapacity:lineCount * info.height * sizeof(screen_char_t)];
        ...
        [mutableState setFromFrame:(screen_char_t *)data.bytes len:data.length metadata:metadataArrays info:info];
...
- (iTermTailFindController *)tailFindController {
    if (!_tailFindController) {
        _tailFindController = [[iTermTailFindController alloc] initWithDataSource:_screen
...
- (void)sessionContentsChanged:(NSNotification *)notification {
    if ([notification object] == self && [_delegate sessionBelongsToVisibleTab]) {
...
- (void)printTmuxMessage:(NSString *)message {
    ...
    [_screen mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        screen_char_t savedFgColor = [terminal foregroundColorCode];
        screen_char_t savedBgColor = [terminal backgroundColorCode];
        [terminal setForegroundColor:ALTSEM_DEFAULT alternateSemantics:YES];
        [terminal setBackgroundColor:ALTSEM_DEFAULT alternateSemantics:YES];
        [terminal updateDefaultChar];
        mutableState.currentGrid.defaultChar = terminal.defaultChar;
        [mutableState appendStringAtCursor:message];
...
- (void)screenSizeDidChangeWithNewTopLineAt:(int)newTop {
    if ([(PTYScroller *)([_view.scrollview verticalScroller]) userScroll] && newTop >= 0) {
        const VT100GridRange range = VT100GridRangeMake(newTop, _textview.rangeOfVisibleLines.length);
        [_textview scrollLineNumberRangeIntoView:range];
    } else {
        [_textview scrollEnd];
    }
...
- (void)restoreColorsFromProfile {
    NSMutableDictionary<NSString *, id> *change = [[NSMutableDictionary alloc] initWithCapacity:32];
...
- (void)logCooked:(NSData *)data foreground:(screen_char_t)fg background:(screen_char_t)bg atPrompt:(BOOL)atPrompt {
    if (!_logging.enabled) {
        return;
    }
    if (self.isTmuxGateway) {
...
- (void)screenDidAppendAsciiDataToCurrentLine:(NSData *)asciiData
                                   ...
                                   background:(screen_char_t)bg
                                     atPrompt:(BOOL)atPrompt {
    if (_logging.enabled) {
        [self logCooked:asciiData foreground:fg background:bg atPrompt:atPrompt];
    }
}
...
- (PTYSessionResizePermission)screenShouldInitiateWindowResize {
    ...
    if (!_focused && [iTermProfilePreferences boolForKey:KEY_DISABLE_UNFOCUSED_WINDOW_RESIZING
                                               inProfile:self.profile]) {
...
- (void)askToEnableTerminalInitiatedResizing:(void (^)(void))allowOnce {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:@"A program has tried to resize the window. Allow it?"
                        style:kiTermAnnouncementViewStyleWarning
...
- (void)askToEnableTerminalInitiatedResizingWhenUnfocused:(void (^)(void))allowOnce {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:@"A program has tried to resize the window while this session was not active. Allow it?"
                        style:kiTermAnnouncementViewStyleWarning
                  withActions:@[ @"_Allow Once", @"_Open Settings", @"Don’t Show This Again" ]
                   completion:completion];
    iTermAnnouncementViewController *existing = _announcements[identifier];
...
- (void)screenPrintStringIfAllowed:(NSString *)string completion:(void (^)(void))completion {
...
- (BOOL)shouldBeginPrinting:(BOOL)willPrint {
    ...
    return [_printGuard shouldPrintWithProfile:self.profile inWindow:self.view.window willPrint:willPrint];
}
...
- (NSString *)screenIconTitle {
    return [self.variablesScope valueForVariableName:iTermVariableKeySessionIconName]
               ?: [self.variablesScope valueForVariableName:iTermVariableKeySessionName];
}
...
- (void)screenMoveSelectionUpBy:(int)n inRegion:(VT100GridRect)region {
...
- (void)screenClearCapturedOutput {
    [[NSNotificationCenter defaultCenter] postNotificationName:kPTYSessionCapturedOutputDidChange object:nil];
}
...
- (void)reveal {
    ...
    if ([terminal isHotKeyWindow]) {
        ...
        iTermProfileHotKey *hotKey =
            [[iTermHotKeyController sharedInstance] profileHotKeyForWindowController:(PseudoTerminal *)terminal];
...
- (void)screenDidAddMark:(id<iTermMark>)newMark alert:(BOOL)alert completion:(void (^)(void))completion {
    ...
    if (!alert && [iTermPreferences boolForKey:kPreferenceKeyAlertOnMarksInOffscreenSessions] &&
    ...
    if ([action isEqualToString:kMarkAlertActionRingBell]) {
        ...
        if (previousScreenMark.code) {
            sound =
                [NSSound soundNamed:[iTermAdvancedSettingsModel errorSound]] ?: [NSSound soundNamed:@"error-sound.mp3"];
        } else {
            sound = [NSSound soundNamed:[iTermAdvancedSettingsModel successSound]]
                        ?: [NSSound soundNamed:@"success-sound.mp3"];
        }
        [sound play];
        completion();
        return;
...
- (void)screenPromptDidEndWithMark:(id<VT100ScreenMarkReading>)mark {
    ...
    [_promptSubscriptions enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                                              BOOL *_Nonnull stop) {
    ...
    [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
        ...
        id<iTermMark> mark = [mutableState addMarkStartingAtAbsoluteLine:absLine
                                                                ...
                                                                modifier:^(id<iTermMark> mark) {
                                                                    VT100ScreenMark *screenMark =
                                                                        [VT100ScreenMark castFrom:mark];
...
- (void)renameMark:(id<iTermGenericNamedMarkReading>)genericMark to:(NSString *)newName {
    ...
    if ([[NSObject castFrom:genericMark] conformsToProtocol:@protocol(VT100ScreenMarkReading)]) {
        ...
        [_screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
            ...
            if (newName) {
                [mutableState
                    addNoteWithText:[PTYAnnotation textForAnnotationForNamedMarkWithName:newName]
                    inAbsoluteRange:VT100GridAbsCoordRangeMake(0, actualLine, mutableState.width, actualLine)];
            }
        }];
    }
...
- (void)screenSetProfileToProfileNamed:(NSString *)value {
    ...
    if (newProfile) {
        // GitLab #11854: Respect user preference for window adjustment when
        // changing profile via escape sequence (1337;SetProfile=).
        BOOL adjustWindow = [iTermPreferences boolForKey:kPreferenceKeyAdjustWindowForFontSizeChange];
        [self setProfile:newProfile preservingName:YES adjustWindow:adjustWindow];
    }
}

- (BOOL)setProfile:(NSDictionary *)newProfile preservingName:(BOOL)preservingName {
    return [self setProfile:newProfile preservingName:preservingName adjustWindow:YES];
}

- (BOOL)setProfile:(NSDictionary *)newProfile preservingName:(BOOL)preserveName adjustWindow:(BOOL)adjustWindow {
    ...
    if (preserveName) {
        [self.variablesScope
            setValuesFromDictionary:@{iTermVariableKeySessionProfileName : newProfile[KEY_NAME] ?: [NSNull null]}];
        return YES;
    }
...
- (void)screenDidAddNote:(id<PTYAnnotationReading>)note focus:(BOOL)focus visible:(BOOL)visible {
...
- (void)screenDidReceiveBase64FileData:(NSString *_Nonnull)data
                               confirm:(void (^NS_NOESCAPE)(NSString *name, NSInteger lengthBefore,
    ...
    if (self.download && ![self.download appendData:data]) {
        iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
            announcementWithTitle:@"A file transfer was aborted for exceeding its declared size."
                            style:kiTermAnnouncementViewStyleWarning
                      withActions:@[]
                       completion:^(int selection){
                       }];
...
- (void)screenRequestUpload:(NSString *)args completion:(void (^)(void))completion {
    // Dispatch out of fear that NSOpenPanel might do something funky with ruloops even though it doesn't seem to
    // currently.
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)requestUploadWithFormat:(NSString *)format version:(NSString *)version {
    if (format && ![format isEqualToString:@"tgz"]) {
        ...
        if (![self announcementWithIdentifier:identifier]) {
            iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
                announcementWithTitle:@"An upload with an unsupported archive format was requested. You may need a "
                                      @"newer version of DashTerm2."
                                style:kiTermAnnouncementViewStyleWarning
                          withActions:@[]
                           completion:^(int selection){
                           }];
    ...
    [panel beginWithFallbackWindow:_textview.window
                           handler:^(NSModalResponse result, NSArray<NSURL *> *panelURLs) {
                               if (result == NSModalResponseOK) {
                                   ...
                                   // Get the directories for all the URLs. If a URL was a file, convert it to the
                                   // containing directory, otherwise leave it alone.
                                   ...
                                   NSArray<NSURL *> *directories = [panelURLs mapWithBlock:^id(NSURL *anObject) {
                                       ...
                                       if ([fileManager fileExistsAtPath:anObject.path isDirectory:&isDirectory]) {
                                           if (isDirectory) {
                                               ...
                                               anyFiles = YES;
                                               return [NSURL
                                                   fileURLWithPath:[anObject.path stringByDeletingLastPathComponent]];
                                           }
                                       } else {
                                   ...
                                   NSArray<NSString *> *relativePaths = [panelURLs mapWithBlock:^id(NSURL *anObject) {
                                       ...
                                       NSArray<NSString *> *relativePathComponents = [pathComponents
                                           subarrayWithRange:NSMakeRange(baseComponents.count,
                                                                         pathComponents.count - baseComponents.count)];
                                   ...
                                   if (version) {
                                       NSString *versionString =
                                           [version stringByBase64DecodingStringWithEncoding:NSUTF8StringEncoding];
                                   ...
                                   if (!data && error) {
                                       ...
                                       if (message) {
                                           ...
                                           alert.informativeText =
                                               [NSString stringWithFormat:@"tar failed with this message: %@", message];
                                           [alert runModal];
                                           return;
                                       }
                                   }
                                   ...
                                   if (relativePaths.count == 1) {
                                       ...
                                       label = [NSString stringWithFormat:@"%@ plus %ld more",
                                                                          relativePaths.firstObject.lastPathComponent,
                                                                          relativePaths.count - 1];
                                   }
                                   const NSUInteger size = base64String.length;
                                   self.upload = [[[TerminalFileUpload alloc] initWithName:label
                                   ...
                                   // this introduces a security risk because reports must not contain newlines.
...
- (void)screenDidTryToUseDECRQCRA {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:@"An app tried to read screen contents with DECRQCRA. Enable this feature?"
...
- (void)screenSetBackgroundImageFile:(NSString *)originalFilename {
    NSString *const filename = [[originalFilename stringByBase64DecodingStringWithEncoding:NSUTF8StringEncoding]
        stringByTrimmingTrailingCharactersFromCharacterSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
- (void)screenSetBadgeFormat:(NSString *)base64Format {
    ...
    iTermParsedExpression *parsedExpression =
        [iTermExpressionParser parsedExpressionWithInterpolatedString:theFormat scope:self.variablesScope];
    if ([parsedExpression containsAnyFunctionCall]) {
        ...
        [self showSimpleWarningAnnouncment:
                  @"The application attempted to set the badge to a value that would invoke a function call. For "
                  @"security reasons, this is not allowed and the badge was not updated."
...
- (void)screenSetUserVar:(NSString *)kvpString {
    ...
    if (kvp) {
        ...
        if (self.isTmuxClient) {
            [self.tmuxController setUserVariableWithKey:key value:value pane:self.tmuxPane];
        }
    } else {
        ...
        if (self.isTmuxClient) {
            [self.tmuxController setUserVariableWithKey:key value:nil pane:self.tmuxPane];
        }
    }
}
...
- (NSDictionary<NSNumber *, id> *)screenResetColorWithColorMapKey:(int)key
    ...
    return [self resetColorWithKey:key fromProfile:_originalProfile profileKey:profileKey dark:dark];
}
...
- (void)setTabColor:(NSColor *)color {
    ...
    if (color) {
        dict = @{
            [self amendedColorKey:KEY_USE_TAB_COLOR] : @YES,
...
- (void)screenSetTabColorGreenComponentTo:(CGFloat)color {
    NSColor *curColor = [self tabColor] ?: [NSColor it_colorInDefaultColorSpaceWithRed:0 green:0 blue:0 alpha:0];
    [self setTabColor:[curColor it_colorWithRed:curColor.redComponent green:color blue:curColor.blueComponent alpha:1]];
    [[_delegate parentWindow] updateTabColors];
}
...
- (void)screenCurrentHostDidChange:(id<VT100RemoteHostReading>)host pwd:(NSString *)workingDirectory ssh:(BOOL)ssh {
    ...
    NSDictionary *variablesUpdate = @{
        iTermVariableKeySessionHostname : host.hostname ?: null,
...
- (void)turnOffMouseReportingOrOffer:(VT100Terminal *)terminal {
    DLog(@"begin");
    NSNumber *number =
        [[NSUserDefaults standardUserDefaults] objectForKey:kTurnOffMouseReportingOnHostChangeUserDefaultsKey];
    if ([number boolValue]) {
...
- (void)pushOrPopHostState:(id<VT100RemoteHostReading>)host {
    ...
    const NSInteger i = [_hostStack
        indexOfObjectPassingTest:^BOOL(PTYSessionHostState *_Nonnull state, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)showSimpleWarningAnnouncment:(NSString *)message identifier:(NSString *)identifier {
    iTermAnnouncementViewController *announcement =
        [iTermAnnouncementViewController announcementWithTitle:message
                                                         style:kiTermAnnouncementViewStyleWarning
                                                   withActions:@[ @"OK" ]
...
- (void)offerToStopMakingTurds:(PTYSessionTurdType)turdType {
    ...
    switch (turdType) {
        case PTYSessionTurdTypeDEC2048:
            ...
            title = @"Looks like resize reporting was left on when an ssh session ended unexpectedly or an app "
                    @"misbehaved. Turn it off?";
        ...
        case PTYSessionTurdTypeMouseReporting:
            ...
            title = @"Looks like mouse reporting was left on when an ssh session ended unexpectedly or an app "
                    @"misbehaved. Turn it off?";
                  ...
                  withActions:@[ @"_Yes", @"Always", @"Never" ]
                   completion:^(int selection) {
                       switch (selection) {
                           ...
                           case 2: // Never
                               [[NSUserDefaults standardUserDefaults] setBool:NO forKey:userDefaultsKey];
                       }
                   }];
    [self queueAnnouncement:announcement identifier:identifier];
}
...
- (void)offerToTurnOffFocusReportingRespectingSavedPreference:(VT100Terminal *)terminal {
    NSNumber *number =
        [[NSUserDefaults standardUserDefaults] objectForKey:kTurnOffFocusReportingOnHostChangeUserDefaultsKey];
    if ([number boolValue]) {
        if (terminal) {
...
- (void)offerToTurnOffFocusReporting {
    NSString *title = @"Looks like focus reporting was left on when an ssh session ended unexpectedly or an app "
                      @"misbehaved. Turn it off?";
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:title
                        style:kiTermAnnouncementViewStyleQuestion
...
- (void)tryAutoProfileSwitchWithHostname:(NSString *)hostname
    ...
    [_automaticProfileSwitcher setHostname:hostname username:username path:path job:job commandLine:commandLine];
}
...
- (void)screenDidReceiveCustomEscapeSequenceWithParameters:(NSDictionary<NSString *, NSString *> *)parameters
    ...
    [_customEscapeSequenceNotifications
        enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, ITMNotificationRequest *_Nonnull obj,
                                            BOOL *_Nonnull stop) {
            [[iTermAPIHelper sharedInstance] postAPINotification:notification toConnectionKey:key];
        }];
}
...
- (void)screenReportVariableNamed:(NSString *)name {
    ...
    NSData *data = [_screen.terminalOutput reportVariableNamed:name value:value];
    [self screenSendReportData:data];
}
...
- (BOOL)allowTitleReporting {
    return [iTermProfilePreferences boolForKey:KEY_ALLOW_TITLE_REPORTING inProfile:self.profile];
}
...
- (void)screenReportIconTitle {
    ...
    NSString *s = [NSString stringWithFormat:@"\033]L%@\033\\", [self reportSafeTitle:title]];
...
- (void)screenReportWindowTitle {
    ...
    NSString *s = [NSString stringWithFormat:@"\033]l%@\033\\", [self reportSafeTitle:[self windowTitle]]];
    [self screenSendReportData:[s dataUsingEncoding:NSUTF8StringEncoding]];
}
...
- (NSArray *)autocompleteSuggestionsForCurrentCommand {
    ...
    return [[iTermShellHistoryController sharedInstance] commandHistoryEntriesWithPrefix:trimmedCommand onHost:host];
}
...
- (void)screenCommandDidChangeTo:(NSString *)command
    ...
    DLog(@"FinalTerm: command=%@ atPropt=%@ hadCommand=%@ haveCommand=%@", command, @(atPrompt), @(hadCommand),
         @(haveCommand));
    ...
    if ([[_delegate realParentWindow] wantsCommandHistoryUpdatesFromSession:self]) {
        ...
        if (haveCommand && self.eligibleForAutoCommandHistory) {
            [[_delegate realParentWindow] updateAutoCommandHistoryForPrefix:command inSession:self popIfNeeded:NO];
        }
    }
}
...
- (void)screenCommandDidAbortOnLine:(int)line
    ...
    iTermTerminalContentSnapshot *snapshot = nil;
    if ([self shouldCaptureCommandExitSnapshot]) {
    ...
    NSMutableDictionary *userInfo = [@{
        ...
        // BUG-1679: Handle nullable URL from promptMarkURL
        PTYCommandDidExitUserInfoKeyURL : (id)[self urlForPromptMark:mark] ?: (id)[NSNull null]
    } mutableCopy];
    if (snapshot) {
        [userInfo setObject:snapshot forKey:PTYCommandDidExitUserInfoKeySnapshot];
    }
    NSDictionary *cleanUserInfo = [userInfo dictionaryByRemovingNullValues];
    [userInfo release];
...
- (void)screenCommandDidExitWithCode:(int)code mark:(id<VT100ScreenMarkReading>)maybeMark {
    ...
    if (maybeMark) {
        ...
        NSMutableDictionary *userInfo = [@{
            ...
            PTYCommandDidExitUserInfoKeyLineCount : @(outputRange.end.y - outputRange.start.y + 1),
            // BUG-1679: Handle nullable URL from promptMarkURL
            PTYCommandDidExitUserInfoKeyURL : (id)[self urlForPromptMark:maybeMark] ?: (id)[NSNull null]
        } mutableCopy];
        if (snapshot) {
            [userInfo setObject:snapshot forKey:PTYCommandDidExitUserInfoKeySnapshot];
        }
...
- (void)addCommandExitSnapshotSubscriber:(id)subscriber {
    if (!subscriber) {
        return;
    }
    os_unfair_lock_lock(&_commandExitSnapshotSubscribersLock);
    if (!_commandExitSnapshotSubscribers) {
        _commandExitSnapshotSubscribers = [[NSHashTable weakObjectsHashTable] retain];
    }
    [_commandExitSnapshotSubscribers addObject:subscriber];
    os_unfair_lock_unlock(&_commandExitSnapshotSubscribersLock);
}

- (void)removeCommandExitSnapshotSubscriber:(id)subscriber {
    if (!subscriber || !_commandExitSnapshotSubscribers) {
        return;
    }
    os_unfair_lock_lock(&_commandExitSnapshotSubscribersLock);
    [_commandExitSnapshotSubscribers removeObject:subscriber];
    os_unfair_lock_unlock(&_commandExitSnapshotSubscribersLock);
}

- (BOOL)shouldCaptureCommandExitSnapshot {
    os_unfair_lock_lock(&_commandExitSnapshotSubscribersLock);
    if (!_commandExitSnapshotSubscribers) {
        os_unfair_lock_unlock(&_commandExitSnapshotSubscribersLock);
        return NO;
    }
    [_commandExitSnapshotSubscribers allObjects];
    BOOL result = _commandExitSnapshotSubscribers.count > 0;
    os_unfair_lock_unlock(&_commandExitSnapshotSubscribersLock);
    return result;
}
...
- (void)updateConfigurationFields {
    ...
    if (_profileDidChange || darkModeDidChange) {
        _config.shouldPlacePromptAtFirstColumn = [iTermProfilePreferences boolForKey:KEY_PLACE_PROMPT_AT_FIRST_COLUMN
                                                                           inProfile:_profile];
        _config.enableTriggersInInteractiveApps =
            [iTermProfilePreferences boolForKey:KEY_ENABLE_TRIGGERS_IN_INTERACTIVE_APPS inProfile:self.profile];
        _config.triggerParametersUseInterpolatedStrings =
            [iTermProfilePreferences boolForKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS inProfile:self.profile];
        _config.triggerProfileDicts = [iTermProfilePreferences objectForKey:KEY_TRIGGERS inProfile:self.profile];
        _config.useSeparateColorsForLightAndDarkMode =
            [iTermProfilePreferences boolForKey:KEY_USE_SEPARATE_COLORS_FOR_LIGHT_AND_DARK_MODE inProfile:self.profile];
        ...
        _config.minimumContrast =
            [iTermProfilePreferences floatForKey:iTermAmendedColorKey(KEY_MINIMUM_CONTRAST, self.profile, darkMode)
                                       inProfile:self.profile];
        _config.faintTextAlpha =
            [iTermProfilePreferences floatForKey:iTermAmendedColorKey(KEY_FAINT_TEXT_ALPHA, self.profile, darkMode)
                                       inProfile:self.profile];
        _config.mutingAmount =
            [iTermProfilePreferences floatForKey:iTermAmendedColorKey(KEY_CURSOR_BOOST, self.profile, darkMode)
                                       inProfile:self.profile];
        ...
        _config.appendToScrollbackWithStatusBar = [iTermProfilePreferences boolForKey:KEY_SCROLLBACK_WITH_STATUS_BAR
                                                                            inProfile:self.profile];
        _config.saveToScrollbackInAlternateScreen =
            [iTermProfilePreferences boolForKey:KEY_SCROLLBACK_IN_ALTERNATE_SCREEN inProfile:self.profile];
        _config.unlimitedScrollback = [iTermProfilePreferences boolForKey:KEY_UNLIMITED_SCROLLBACK inProfile:_profile];
        _config.reduceFlicker = [iTermProfilePreferences boolForKey:KEY_REDUCE_FLICKER inProfile:self.profile];
        _config.maxScrollbackLines = [iTermProfilePreferences intForKey:KEY_SCROLLBACK_LINES inProfile:self.profile];
        _config.profileName = [self profileName];
        _config.terminalCanChangeBlink = [iTermProfilePreferences boolForKey:KEY_ALLOW_CHANGE_CURSOR_BLINK
                                                                   inProfile:self.profile];
        _config.optionIsMetaForSpecialChars = ![iTermProfilePreferences boolForKey:KEY_TREAT_OPTION_AS_ALT
                                                                         inProfile:_profile];
    ...
    const BOOL useLineStyleMarks =
        [iTermPreferences boolForKey:kPreferenceAutoComposer] || [iTermAdvancedSettingsModel alwaysUseLineStyleMarks];
...
- (BOOL)shouldAlert {
    ...
    if (!self.view.window.isVisible || self.view.window.isMiniaturized || ![self.view.window isOnActiveSpace] ||
...
- (NSDictionary *)stringForKeypress {
    id (^stringForKeypress)(unsigned short, NSEventModifierFlags, NSString *, NSString *) =
        ^id(unsigned short keyCode, NSEventModifierFlags flags, NSString *characters,
    ...
    return @{
        ...
        @"kD" : stringForKeypress(kVK_ForwardDelete, NSEventModifierFlagFunction, c(NSDeleteFunctionKey),
                                  c(NSDeleteFunctionKey)),
        @"kd" : stringForKeypress(kVK_DownArrow, NSEventModifierFlagFunction, c(NSDownArrowFunctionKey),
        ...
        @"kl" : stringForKeypress(kVK_LeftArrow, NSEventModifierFlagFunction, c(NSLeftArrowFunctionKey),
                                  c(NSLeftArrowFunctionKey)),
        @"kN" : stringForKeypress(kVK_PageDown, NSEventModifierFlagFunction, c(NSPageDownFunctionKey),
                                  c(NSPageDownFunctionKey)),
        @"kP" :
            stringForKeypress(kVK_PageUp, NSEventModifierFlagFunction, c(NSPageUpFunctionKey), c(NSPageUpFunctionKey)),
        @"kr" : stringForKeypress(kVK_RightArrow, NSEventModifierFlagFunction, c(NSRightArrowFunctionKey),
                                  c(NSRightArrowFunctionKey)),
        @"*4" : stringForKeypress(kVK_ForwardDelete, NSEventModifierFlagFunction | NSEventModifierFlagShift,
                                  c(NSDeleteFunctionKey), c(NSDeleteFunctionKey)),
        @"*7" : stringForKeypress(kVK_End, NSEventModifierFlagFunction | NSEventModifierFlagShift, c(NSEndFunctionKey),
                                  c(NSEndFunctionKey)),
        @"#2" : stringForKeypress(kVK_Home, NSEventModifierFlagFunction | NSEventModifierFlagShift,
                                  c(NSHomeFunctionKey), c(NSHomeFunctionKey)),
        @"#4" : stringForKeypress(
            kVK_LeftArrow, NSEventModifierFlagFunction | NSEventModifierFlagNumericPad | NSEventModifierFlagShift,
            c(NSLeftArrowFunctionKey), c(NSLeftArrowFunctionKey)),
        @"%i" : stringForKeypress(
            kVK_RightArrow, NSEventModifierFlagFunction | NSEventModifierFlagNumericPad | NSEventModifierFlagShift,
            c(NSRightArrowFunctionKey), c(NSRightArrowFunctionKey)),
        @"ku" : stringForKeypress(kVK_UpArrow, NSEventModifierFlagFunction, c(NSUpArrowFunctionKey),
                                  c(NSUpArrowFunctionKey))
    };
}

- (BOOL)shouldPostTerminalGeneratedAlert {
    return [iTermProfilePreferences boolForKey:KEY_SEND_TERMINAL_GENERATED_ALERT inProfile:_profile];
}
...
- (void)screenActivateBellAudibly:(BOOL)audibleBell
                    ...
                    showIndicator:(BOOL)showBellIndicator
                            quell:(BOOL)quell {
    if ([self shouldIgnoreBellWhichIsAudible:audibleBell visible:flashBell]) {
        return;
    }
    BOOL notified = NO;
    if (quell) {
...
- (BOOL)shouldIgnoreBellWhichIsAudible:(BOOL)audible visible:(BOOL)visible {
    ...
    // Rate limit bells to prevent flooding the audio system and hanging the GUI.
    // When bells come faster than 10ms apart, ignore them completely.
    // The moving average will still eventually detect rapid bells since we sample every 10ms.
    static const NSTimeInterval kMaximumTimeBetweenSamples = 0.01;
    if (now < _lastBell + kMaximumTimeBetweenSamples) {
        return YES; // Ignore bells that are too rapid - fixes issue #12323
    }
    _lastBell = now;
    ...
    if ([_bellRate value] < kThresholdForBellMovingAverageToInferAnnoyance && !existingAnnouncement &&
        ...
        if (audible) {
            ...
            announcement = [iTermAnnouncementViewController
                announcementWithTitle:@"The bell is ringing a lot. Silence it?"
                                style:kiTermAnnouncementViewStyleQuestion
                          withActions:@[
                              @"_Silence Bell Temporarily", @"Suppress _All Output", @"Don't Offer Again",
                              @"Silence Automatically"
                          ]
                           completion:^(int selection) {
            ...
            announcement = [iTermAnnouncementViewController
                announcementWithTitle:@"The bell is ringing a lot. Want to suppress all output until things calm down?"
                                style:kiTermAnnouncementViewStyleQuestion
                          withActions:@[ @"Suppress _All Output", @"Don't Offer Again" ]
                           completion:^(int selection) {
                               // Release the moving average so the count will restart after the announcement goes away.
                               [_bellRate release];
                               _bellRate = nil;
...
- (BOOL)haveWrittenAnythingBesidesFocusReportVeryRecently {
    return -[self.lastNonFocusReportingWrite timeIntervalSinceNow] <
           PTYSessionFocusReportBellSquelchTimeIntervalThreshold;
}
...
- (void)setLastDirectory:(NSString *)lastDirectory remote:(BOOL)directoryIsRemote pushed:(BOOL)pushed {
    DLog(@"setLastDirectory:%@ remote:%@ pushed:%@\n%@", lastDirectory, @(directoryIsRemote), @(pushed),
         [NSThread callStackSymbols]);
...
- (void)setLastLocalDirectory:(NSString *)lastLocalDirectory {
    DLog(@"lastLocalDirectory goes %@ -> %@ for %@\n%@", _lastLocalDirectory, lastLocalDirectory, self,
         [NSThread callStackSymbols]);
...
- (void)screenLogWorkingDirectoryOnAbsoluteLine:(long long)absLine
    ...
    DLog(@"screenLogWorkingDirectoryOnscreenLogWorkingDirectoryOnAbsoluteLine:%@ remoteHost:%@ withDirectory:%@ "
         @"pushType:%@ accepted:%@",
    ...
    if (pushed) {
        BOOL isSame = ([directory isEqualToString:_lastDirectory] && [remoteHost isEqualToRemoteHost:_lastRemoteHost]);
        [[iTermShellHistoryController sharedInstance] recordUseOfPath:directory onHost:remoteHost isChange:!isSame];
    }
    if (accepted) {
        ...
        const BOOL directoryIsRemote =
            (pushType == VT100ScreenWorkingDirectoryPushTypeStrongPush) && remoteHost && !remoteHost.isLocalhost;
...
- (NSString *)shellIntegrationUpgradeUserDefaultsKeyForHost:(id<VT100RemoteHostReading>)host {
    return [NSString
        stringWithFormat:@"SuppressShellIntegrationUpgradeAnnouncementForHost_%@@%@", host.username, host.hostname];
}
...
- (void)screenSuggestShellIntegrationUpgrade {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:@"This account’s Shell Integration scripts are out of date."
                        style:kiTermAnnouncementViewStyleWarning
...
- (void)restoreKeyLabels:(NSDictionary *)labels updateStatusChangedLine:(BOOL)updateStatusChangedLine {
    ...
    if (!_keyLabels) {
        _keyLabels = [[NSMutableDictionary alloc] initWithCapacity:16];
    }
    [labels
        enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull name, NSString *_Nonnull value, BOOL *_Nonnull stop) {
            if (value.length == 0) {
                return;
            }
...
- (void)screenPushKeyLabels:(NSString *)value {
    ...
    if (!_keyLabelsStack) {
        _keyLabelsStack = [[NSMutableArray alloc] initWithCapacity:8];
    }
...
- (void)screenSendModifiersDidChange {
    ...
    const BOOL allowed = [iTermProfilePreferences boolForKey:KEY_ALLOW_MODIFY_OTHER_KEYS inProfile:self.profile];
    if (!allowed) {
        return;
    }
...
- (void)screenTerminalAttemptedPasteboardAccess {
    ...
    NSString *notice = @"The terminal attempted to access the clipboard but it was denied. Enable clipboard access in "
                       @"“Settings > General > Selection > Applications in terminal may access clipboard”.";
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:notice
                        style:kiTermAnnouncementViewStyleWarning
                  withActions:@[ @"_Open Settings", @"Don't Show This Again" ]
                   completion:^(int selection) {
                       if (selection == 0) {
                           [[PreferencePanel sharedInstance]
                               openToPreferenceWithKey:kPreferenceKeyAllowClipboardAccessFromTerminal];
                       } else if (selection == 1) {
...
- (void)screenSoftAlternateScreenModeDidChangeTo:(BOOL)enabled showingAltScreen:(BOOL)showing {
    _textview.marginColorAllowed = showing;
    [self.processInfoProvider setNeedsUpdate:YES];
    [self.tmuxForegroundJobMonitor updateOnce];
    [self.variablesScope setValue:@(showing) forVariableNamed:iTermVariableKeySessionShowingAlternateScreen];
    [self removeSelectedCommandRange];
}
...
- (void)queueAnnouncement:(iTermAnnouncementViewController *)announcement identifier:(NSString *)identifier {
...
- (BOOL)screenConfirmDownloadNamed:(NSString *)name canExceedSize:(NSInteger)limit {
    ...
    const iTermWarningSelection selection =
        [iTermWarning showWarningWithTitle:[NSString stringWithFormat:@"The download “%@” is larger than %@. Continue?",
                                                                      name, [NSString it_formatBytes:limit]]
                                   actions:@[ @"Allow", @"Deny" ]
                                 accessory:nil
                                identifier:identifier
                               silenceable:kiTermWarningTypePermanentlySilenceable
...
- (BOOL)screenConfirmDownloadAllowed:(NSString *)name
    ...
    *promptIfBig = YES;
    ...
    if (displayInline) {
        title =
            [NSString stringWithFormat:@"The terminal has initiated display of a file named “%@” of size %@. Allow it?",
                                       name, [NSString it_formatBytes:size]];
        heading = @"Allow Terminal-Initiated Display?";
    } else {
        title = [NSString
            stringWithFormat:@"The terminal has initiated transfer of a file named “%@” of size %@. Download it?", name,
                             [NSString it_formatBytes:size]];
        heading = @"Allow Terminal-Initiated Download?";
    }
    const iTermWarningSelection selection = [iTermWarning showWarningWithTitle:title
                                                                       actions:@[ @"Yes", @"No" ]
                                                                     accessory:nil
                                                                    identifier:identifier
                                                                   silenceable:kiTermWarningTypePermanentlySilenceable
...
- (void)screenDidResize {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:PTYSessionDidResizeNotification object:self];
...
- (void)screenAppendScreenCharArray:(ScreenCharArray *)sca
    ...
    [self publishScreenCharArray:sca metadata:metadata lineBufferGeneration:lineBufferGeneration];
}
    ...
    NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                   ...
                   charactersIgnoringModifiers:charactersIgnoringModifiers
                                     isARepeat:NO
                                       keyCode:keycode];
    return
        [_textview.keyboardHandler stringForEventWithoutSideEffects:event
...
- (void)screenRestoreColorsFromSlot:(VT100SavedColorsSlot *)slot {
    ...
    NSMutableDictionary *dict = [[@{
        iTermAmendedColorKey(KEY_FOREGROUND_COLOR, _profile, dark) : slot.text.dictionaryValue,
        ...
        iTermAmendedColorKey(KEY_SELECTION_COLOR, _profile, dark) : slot.selectionBackground.dictionaryValue
    } mutableCopy] autorelease];
...
- (void)screenDidUpdateReturnCodeForMark:(id<VT100ScreenMarkReading>)mark
                              remoteHost:(id<VT100RemoteHostReading>)remoteHost {
    [[iTermShellHistoryController sharedInstance] setStatusOfCommandAtMark:mark onHost:remoteHost to:mark.code];
...
- (void)screenPostUserNotification:(NSString *_Nonnull)message rich:(BOOL)rich {
    ...
    if (rich) {
        NSDictionary<NSString *, NSString *> *dict =
            [[message it_keyValuePairsSeparatedBy:@";"] mapValuesWithBlock:^id(NSString *key, NSString *encoded) {
        ...
        NSString *description =
            [NSString stringWithFormat:@"Session %@ #%d: %@", [[self name] removingHTMLFromTabTitleIfNeeded],
                                       [_delegate tabNumber], message];
...
- (void)screenUpdateCommandUseWithGuid:(NSString *)screenmarkGuid
    ...
    iTermCommandHistoryCommandUseMO *commandUse =
        [[iTermShellHistoryController sharedInstance] commandUseWithMarkGuid:screenMark.guid onHost:lastRemoteHost];
...
- (void)screenDidHookSSHConductorWithToken:(NSString *)token
    ...
    NSDictionary *dict = [NSDictionary castFrom:[iTermProfilePreferences objectForKey:KEY_SSH_CONFIG
                                                                            inProfile:self.profile]];
    const BOOL shouldInjectShellIntegration =
        [iTermProfilePreferences boolForKey:KEY_LOAD_SHELL_INTEGRATION_AUTOMATICALLY inProfile:self.profile];
    iTermSSHConfiguration *config = [[[iTermSSHConfiguration alloc] initWithDictionary:dict] autorelease];
    _conductor =
        [[iTermConductor alloc] init:sshargs
                                boolArgs:boolArgs
                                   dcsID:dcsID
                          clientUniqueID:uniqueID
                              varsToSend:localOrigin
                                             ? [self.screen
                                                   exfiltratedEnvironmentVariables:config.environmentVariablesToCopy]
                                             : @{}
...
- (void)screenDidBeginSSHConductorCommandWithIdentifier:(NSString *)identifier depth:(int)depth {
...
- (void)screenDidEndSSHConductorCommandWithIdentifier:(NSString *)identifier
                                               ...
                                               status:(uint8_t)status
                                                depth:(int)depth {
    [_conductor handleCommandEndWithIdentifier:identifier type:type status:status depth:depth];
}

- (void)screenHandleSSHSideChannelOutput:(NSString *)string pid:(int32_t)pid channel:(uint8_t)channel depth:(int)depth {
...
- (NSInteger)screenEndSSH:(NSString *)uniqueID {
    ...
    // it2ssh waits for a newline before exiting. This is in case ssh dies while DashTerm2 is sending
    // conductor.sh.
    [self writeTaskNoBroadcast:@"\n"];
...
- (void)screenBeginSSHIntegrationWithToken:(NSString *)token
    ...
    NSString *message = [NSString stringWithFormat:@"%@main %@ %@ %@ %@", conductorSH,
    ...
    [self writeTaskNoBroadcast:[@"\n-- BEGIN CONDUCTOR --\n"
                                   stringByAppendingString:[[message base64EncodedWithEncoding:NSUTF8StringEncoding]
                                                               chunkedWithLineLength:80
                                                                           separator:@"\n"]]];
...
- (BOOL)popupWindowShouldAvoidChangingWindowOrderOnClose {
    return [iTermPreferences boolForKey:kPreferenceKeyFocusFollowsMouse] &&
           ![self.delegate sessionBelongsToHotkeyWindow:self];
}
...
- (BOOL)popupHandleSelector:(SEL)selector string:(NSString *)string currentValue:(NSString *)currentValue {
    ...
    if (selector == @selector(deleteBackward:)) {
        [_textview
            keyDown:[NSEvent keyEventWithType:NSEventTypeKeyDown
                                           location:NSZeroPoint
...
- (void)pasteHelperWriteString:(NSString *)string {
    if (_pasteHelper.pasteContext.bytesWritten == 0 &&
        (_pasteHelper.pasteContext.pasteEvent.flags & kPasteFlagsBracket) && _screen.terminalBracketedPasteMode) {
        NSString *prefix = [_pasteHelper.pasteContext.pasteEvent.originalString it_substringToIndex:4];
        [self watchForPasteBracketingOopsieWithPrefix:prefix andWrite:string];
    } else {
        [self writeTask:string];
    }
}
...
- (void)automaticProfileSwitcherLoadProfile:(iTermSavedProfile *)savedProfile {
    Profile *underlyingProfile =
        [[ProfileModel sharedInstance] bookmarkWithGuid:savedProfile.originalProfile[KEY_GUID]];
    ...
    if (![self setProfile:replacementProfile preservingName:NO adjustWindow:NO]) {
        [_view showUnobtrusiveMessage:[NSString stringWithFormat:@"Can’t switch to profile “%@”—wrong profile type.",
                                                                 underlyingProfile[KEY_NAME]]];
        return;
    }
    if (savedProfile.isDivorced) {
        NSMutableDictionary *overrides =
            [[NSMutableDictionary alloc] initWithCapacity:savedProfile.overriddenFields.count];
    ...
    if ([iTermAdvancedSettingsModel showAutomaticProfileSwitchingBanner]) {
        [_view showUnobtrusiveMessage:[NSString
                                          stringWithFormat:@"Switched to profile “%@”.", underlyingProfile[KEY_NAME]]];
    }
}
...
- (NSRect)sessionViewFrameForLegacyView {
    ...
    return NSMakeRect(0, bottomMarginHeight, NSWidth(_textview.bounds), _textview.lineHeight * _screen.height);
}
...
- (NSDragOperation)sessionViewDraggingEntered:(id<NSDraggingInfo>)sender {
    ...
    if (!([[[sender draggingPasteboard] types] indexOfObject:@"com.dashterm.dashterm2.psm.controlitem"] !=
          NSNotFound)) {
...
#pragma mark - DTermCore GPU Renderer Support

- (id)sessionViewDTermCoreIntegration {
    // Return the dterm-core integration if it exists and is enabled
    if (self.dtermCoreIntegration && self.dtermCoreIntegration.isEnabled) {
        return self.dtermCoreIntegration;
    }
    return nil;
}

- (NSFont *)sessionViewFont {
    // Return the ASCII font for glyph rendering
    return _textview.fontTable.asciiFont.font;
}

- (BOOL)textViewOrComposerIsFirstResponder {
    return ((_textview.window.firstResponder == _textview || [_composerManager dropDownComposerIsFirstResponder]) &&
            [NSApp isActive] && _textview.window.isKeyWindow);
}
...
- (BOOL)textViewPasteFiles:(NSArray<NSString *> *)filenames {
    ...
    [eval evaluateWithTimeout:5
                   completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                       ...
                       [weakSelf
                           runCoprocessWithCompletion:^(id output, NSError *error) {
                           }
...
- (void)requestNaturalLanguageQuery:(NSString *)defaultString
    ...
    if (bypassable || reason) {
        ...
        NSMutableArray<NSView *> *views = [NSMutableArray arrayWithCapacity:2]; // At most disable button + reason label
        ...
        NSView *container = [[[NSView alloc]
            initWithFrame:NSMakeRect(0, 0, 400, scrollViewFrame.size.height + extraHeight)] autorelease];
    ...
    [alert beginSheetModalForWindow:self.view.window
                  completionHandler:^(NSModalResponse button) {
                      if (button == NSAlertFirstButtonReturn) {
...
- (iTermSelection *)selectionForOutputToExplainWithAI:(out BOOL *)truncated {
    ...
    if (mark) {
        ...
        VT100GridAbsCoordRange absRange = VT100GridAbsCoordRangeFromCoordRange(range, _screen.totalScrollbackOverflow);
...
- (NSString *)subjectMatterToExplainWithAI {
    if (_textview.selection.hasSelection) {
        return [[_textview selectedText] ellipsizedDescriptionNoLongerThan:16].stringEnclosedInMarkdownInlineCode
                   ?: @"some selected text";
    }
    if (_selectedCommandMark) {
        if (_selectedCommandMark.command.length) {
...
- (void)reallyPerformNaturalLanguageQuery:(NSString *)query completion:(void (^)(BOOL))completion {
    if (!query) {
        return;
    }
    [_aiterm invalidate];
...
- (BOOL)textViewShouldShowOffscreenCommandLineAt:(int)location {
    ...
    if (ocl.mark == lastCommandMark || (ocl.mark == _screen.penultimateCommandMark && lastCommandMark.isRunning == NO &&
...
- (NSSize)sessionViewScrollViewWillResize:(NSSize)proposedSize {
    if ([self isTmuxClient] && ![_delegate sessionBelongsToTmuxTabWhoseSplitsAreBeingDragged]) {
        ...
        NSSize maximumSize =
            NSMakeSize(idealSize.width + _textview.charWidth - 1, idealSize.height + _textview.lineHeight - 1);
        ...
        return NSMakeSize(MIN(proposedSize.width, maximumSize.width), MIN(proposedSize.height, maximumSize.height));
    } else {
        return proposedSize;
    }
}
...
- (BOOL)textViewIsAutoComposerOpen {
    return [_composerManager dropDownComposerViewIsVisible] && _composerManager.isAutoComposer &&
           !_composerManager.temporarilyHidden;
}
...
- (VT100GridRange)textViewLinesToSuppressDrawing {
    if ([_composerManager dropDownComposerViewIsVisible] && _composerManager.isAutoComposer &&
        ...
        const VT100GridCoord bottomRight =
            [_textview coordForPoint:NSMakePoint(NSMaxX(textViewRect) - 1, NSMaxY(textViewRect) - 1)
...
- (NSRect)textViewCursorFrameInScreenCoords {
    ...
    return NSMakeRect(origin.x, origin.y, charWidth, lineHeight);
}
...
- (void)textViewDisableOffscreenCommandLine {
    if (self.isDivorced) {
        [self setSessionSpecificProfileValues:@{KEY_SHOW_OFFSCREEN_COMMANDLINE : @NO}];
        return;
    }
...
- (void)selectCommandWithMarkIfSafe:(id<VT100ScreenMarkReading>)mark {
    if (mark && _screen.terminalSoftAlternateScreenMode) {
        const VT100GridAbsCoordRange absRange = [self rangeOfCommandAndOutputForMark:mark includeSucessorDivider:YES];
        const NSRange markRange = NSMakeRange(absRange.start.y, MAX(0, absRange.end.y - absRange.start.y + 1));
        const NSRange screenRange =
            NSMakeRange(_screen.numberOfScrollbackLines + _screen.totalScrollbackOverflow, _screen.height);
        if (NSIntersectionRange(markRange, screenRange).length > 0) {
            DLog(@"Not allowing selection that includes screen in alternate screen mode. markRange=%@ screenRange=%@",
                 NSStringFromRange(markRange), NSStringFromRange(screenRange));
            return;
        }
    }
    [self selectCommandWithMark:mark];
...
- (VT100GridAbsCoordRange)textViewCoordRangeForCommandAndOutputAtMark:(id<iTermMark>)mark {
    if ([mark conformsToProtocol:@protocol(VT100ScreenMarkReading)]) {
        return [self rangeOfCommandAndOutputForMark:(id<VT100ScreenMarkReading>)mark includeSucessorDivider:YES];
    }
    return [_screen absCoordRangeForInterval:mark.entry.interval];
}

- (void)showCommandSelectionInfo {
    const NSPoint point = [_view convertPoint:NSApp.currentEvent.locationInWindow fromView:nil];
    NSString *html = @"You’ve selected a command by clicking on it. This restricts Find, Filter, and Select All to the "
                     @"content of the command. You can turn this feature off in Settings > General > Selection. <a "
                     @"href=\"dashterm2:disable-command-selection\">Click here to disable this feature.</a>";
    NSAttributedString *attributedString =
        [NSAttributedString attributedStringWithHTML:html
                                                font:[NSFont systemFontOfSize:[NSFont systemFontSize]]
                                      paragraphStyle:[NSParagraphStyle defaultParagraphStyle]];
    [_view it_showWarningWithAttributedString:attributedString rect:NSMakeRect(point.x, point.y, 1, 1)];
}
...
- (void)showHorizontalScrollInfo {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:@"When mouse reporting is on, horizontal scrolling does not switch tabs.\nHold option "
                              @"while swiping or disable horizontal scroll reporting to switch tabs."
                        style:kiTermAnnouncementViewStyleWarning
                   ...
                   completion:^(int selection) {
                       switch (selection) {
                           ...
                           case 0:
                               // Disable reporting
                               [[PreferencePanel sharedInstance]
                                   openToPreferenceWithKey:kPreferenceKeyReportHorizontalScrollEvents];
                               break;
                       }
                       [[NSUserDefaults standardUserDefaults] setBool:YES forKey:warningKey];
                   }];
...
- (void)updateSearchRange {
    ...
    if (_selectedCommandMark.lineStyle) {
        _textview.findOnPageHelper.absLineRange =
            NSMakeRange(MAX(0, range.start.y - 1), range.end.y - range.start.y + 1);
    } else {
...
- (BOOL)sessionHotkeyIsAlreadyFirstResponder {
    return ([NSApp isActive] && [NSApp keyWindow] == self.textview.window &&
            self.textview.window.firstResponder == self.textview);
}

- (BOOL)sessionHotkeyIsAlreadyActiveInNonkeyWindow {
    if ([NSApp isActive] && [NSApp keyWindow] == self.textview.window) {
        return NO;
    }
...
- (void)sessionViewDidChangeWindow {
    [self invalidateBlend];
    // Only update Metal driver if we have a window.
    // When view moves to nil window (during close/teardown), skip the update entirely.
    if (_view.window != nil) {
        [self updateMetalDriver];
    }
...
- (id)temporarilyDisableMetal NS_AVAILABLE_MAC(10_11) {
    // BUG-f1126: Replace assert with guard - metal must be enabled to disable
    if (!_useMetal) {
        DLog(@"BUG-f1126: temporarilyDisableMetal called when metal not enabled - returning nil token");
        return nil;
    }
    _wrapper.useMetal = NO;
    _textview.suppressDrawing = NO;
...
- (void)drawFrameAndRemoveTemporarilyDisablementOfMetalForToken:(id)token NS_AVAILABLE_MAC(10_11) {
    ...
    if ([_metalDisabledTokens containsObject:token]) {
        ...
        if (_metalDisabledTokens.count > 1) {
            ...
            DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: There are still other tokens remaining: %@",
                 _metalDisabledTokens);
            return;
        }
    } else {
    ...
    [_view.driver
        drawAsynchronouslyInView:_view.metalView
                      completion:^(BOOL ok) {
                          DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal drawAsynchronouslyInView finished "
                               @"wtih ok=%@",
                               @(ok));
                          if (![_metalDisabledTokens containsObject:token]) {
                              DLog(
                                  @"drawFrameAndRemoveTemporarilyDisablementOfMetal: Token %@ is gone, not proceeding.",
                                  token);
                              return;
                          }
                          if (!_view.window) {
                              DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: Returning because the view has "
                                   @"no window");
                              return;
                          }
                          if (!_useMetal) {
                              DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: Returning because useMetal is "
                                   @"off");
                              return;
                          }
                          if (!ok) {
                              DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: Schedule "
                                   @"drawFrameAndRemoveTemporarilyDisablementOfMetal to run after a spin of the "
                                   @"mainloop");
                              if (!_delegate) {
                                  [self setUseMetal:NO];
                                  return;
                              }
                              // BUG-1150: Use weakSelf and recheck delegate inside block to avoid stale state
                              __weak __typeof(self) weakSelf = self;
                              dispatch_async(dispatch_get_main_queue(), ^{
                                  PTYSession *strongSelf = weakSelf;
                                  if (!strongSelf) {
                                      return;
                                  }
                                  if (!strongSelf.delegate) {
                                      [strongSelf setUseMetal:NO];
                                      return;
                                  }
                                  if (![strongSelf->_metalDisabledTokens containsObject:token]) {
                                      DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: [after a spin of the "
                                           @"runloop] Token %@ is gone, not proceeding.",
                                           token);
                                      return;
                                  }
                                  [strongSelf drawFrameAndRemoveTemporarilyDisablementOfMetalForToken:token];
                              });
                              return;
                          }

                          // BUG-f1127: Replace assert with guard - token should be in disabled tokens set
                          if (![_metalDisabledTokens containsObject:token]) {
                              DLog(@"BUG-f1127: drawFrameAndRemoveTemporarilyDisablementOfMetal: token %@ not in "
                                   @"_metalDisabledTokens",
                                   token);
                              return;
                          }
                          [_metalDisabledTokens removeObject:token];
                          DLog(@"drawFrameAndRemoveTemporarilyDisablementOfMetal: Remove temporarily disablement. "
                               @"Tokens are now %@",
                               _metalDisabledTokens);
...
- (void)coprocess:(Coprocess *)coprocess didTerminateWithErrorOutput:(NSString *)errors {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:[NSString stringWithFormat:@"Coprocess “%@” terminated with output on stderr.",
                                                         coprocess.command]
                        style:kiTermAnnouncementViewStyleWarning
                  withActions:@[ @"_View Errors", @"Ignore Errors from This Command" ]
                   completion:^(int selection) {
                       if (selection == 0) {
                           NSString *filename =
                               [[NSWorkspace sharedWorkspace] temporaryFileNameWithPrefix:@"coprocess-stderr."
                                                                                   suffix:@".txt"];
...
- (iTermUpdateCadenceState)updateCadenceControllerState {
    ...
    if (self.useMetal) {
        if ([iTermPreferences maximizeThroughput] && !_screen.terminalSoftAlternateScreenMode) {
    ...
    state.slowFrameRate =
        self.useMetal ? [iTermAdvancedSettingsModel metalSlowFrameRate] : [iTermAdvancedSettingsModel slowFrameRate];
...
- (void)addContentSubscriber:(id<iTermContentSubscriber>)contentSubscriber {
    if (!_contentSubscribers) {
        _contentSubscribers = [[NSMutableArray alloc] initWithCapacity:4];
    }
...
- (ITMCellStyle *)protoStyleForCharacter:(screen_char_t)c externalAttributes:(iTermExternalAttribute *)ea {
    ...
    if (ea.blockIDList) {
        style.blockId =
            [[ea.blockIDList componentsSeparatedByString:iTermExternalAttributeBlockIDDelimiter] firstObject];
    }
    if (ea.url) {
...
- (NSString *)stringForLine:(const screen_char_t *)screenChars
    ...
    for (int i = 0; i < length; ++i) {
        ...
        if (style != nil) {
            if (ScreenCharacterAttributesEqual(screenChars[i], prev) && [NSObject object:prevAttr
                                                                            isEqualToObject:eaIndex[i]]) {
                style.repeats += 1;
            } else {
...
- (ITMGetBufferResponse *)handleGetBufferRequest:(ITMGetBufferRequest *)request {
    ...
    const VT100GridWindowedRange range =
        VT100GridWindowedRangeFromVT100GridAbsWindowedRange(windowedRange, _screen.totalScrollbackOverflow);
    iTermTextExtractor *extractor = [iTermTextExtractor textExtractorWithDataSource:_screen];
    ...
    [extractor enumerateCharsInRange:range
        supportBidi:NO
        charBlock:^BOOL(const screen_char_t *currentLine, screen_char_t theChar, iTermExternalAttribute *ea,
                        VT100GridCoord logicalCoord, VT100GridCoord coord) {
            line = currentLine;
            if (firstIndex < 0) {
                firstIndex = coord.x;
            }
...
- (void)handleListPromptsRequest:(ITMListPromptsRequest *)request
                      completion:(void (^)(ITMListPromptsResponse *))completion {
...
- (ITMSetProfilePropertyResponse_Status)
    handleSetProfilePropertyForAssignments:(NSArray<iTermTuple<NSString *, id> *> *)tuples
                        scriptHistoryEntry:(iTermScriptHistoryEntry *)scriptHistoryEntry {
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithCapacity:tuples.count];
    for (iTermTuple<NSString *, id> *tuple in tuples) {
        ...
        if (![iTermProfilePreferences valueIsLegal:value forKey:key]) {
            ...
            [scriptHistoryEntry
                 addOutput:[NSString stringWithFormat:@"Value %@ is not legal type for key %@\n", value, key]
                completion:^{
                }];
...
- (NSString *)sessionNameControllerUniqueIdentifier {
    ...
    iTermTuple<NSString *, NSString *> *tuple =
        [iTermTuple fromPlistValue:[iTermProfilePreferences stringForKey:KEY_TITLE_FUNC inProfile:_profile]];
...
- (void)jobPidDidChange {
    ...
    if ([_graphicSource updateImageForProcessID:self.shell.pid
                                        enabled:[self shouldShowTabGraphic]
                            processInfoProvider:self.processInfoProvider]) {
        [self.delegate sessionDidChangeGraphic:self shouldShow:self.shouldShowTabGraphic image:self.tabGraphic];
    }
    [self.delegate sessionJobDidChange:self];
...
- (void)echoProbe:(iTermEchoProbe *)echoProbe writeString:(NSString *)string {
    ...
    if ([iTermUserDefaults shouldSendReturnAfterPassword]) {
        [_view showUnobtrusiveMessage:@"Password sent." duration:3];
    } else {
        [_view showUnobtrusiveMessage:@"Password sent (press Return)." duration:3];
    }
}
...
- (NSColor *)textColorForStatusBar {
    return [[iTermTheme sharedInstance]
        statusBarTextColorForEffectiveAppearance:_view.effectiveAppearance
...
- (NSColor *)statusBarSeparatorColor {
    ...
    switch ((iTermPreferencesTabStyle)[iTermPreferences intForKey:kPreferenceKeyTabStyle]) {
        ...
        case TAB_STYLE_MINIMAL: // BUG-7902: MINIMAL returns nil early at lines 19358-19360, but handle for completeness
...
- (void)statusBarReportScriptingError:(NSError *)error forInvocation:(NSString *)invocation origin:(NSString *)origin {
    [PTYSession reportFunctionCallError:error
                          forInvocation:invocation
                                 origin:origin
                                 window:self.delegate.realParentWindow.window];
...
- (NSArray<NSString *> *)triggerNames {
    return [_config.triggerProfileDicts mapWithBlock:^id(NSDictionary *dict) {
        ...
        return [NSString
            stringWithFormat:@"%@ — %@", [[[trigger class] title] stringByRemovingSuffix:@"…"], trigger.regex];
    }];
}
...
- (void)maybeOfferToSetOptionAsEscForLeft:(BOOL)left {
    ...
    iTermAnnouncementViewController *announcement = [iTermAnnouncementViewController
        announcementWithTitle:
            [NSString stringWithFormat:@"You seem frustrated. Would you like the %@ option key to send esc+keystroke?",
                                       leftOrRight]
                        style:kiTermAnnouncementViewStyleWarning
                  withActions:actions
                   completion:^(int selection) {
                       ...
                       if (selection == thisProfile) {
                           [iTermProfilePreferences setInt:OPT_ESC
                                                    forKey:profileKey
                                                 inProfile:profileToChange
                                                     model:[ProfileModel sharedInstance]];
...
- (void)workingDirectoryPollerDidFindWorkingDirectory:(NSString *)pwd invalidated:(BOOL)invalidated {
    ...
    if (invalidated && _lastLocalDirectoryWasPushed && _lastLocalDirectory != nil) {
        DLog(@"Ignore local directory poller's invalidated result when we have a pushed last local directory. "
...
- (iTermModernKeyMapperConfiguration *)modernKeyMapperWillMapKey {
    iTermModernKeyMapperConfiguration *configuration =
        [[[iTermModernKeyMapperConfiguration alloc] initWithLeftOptionKey:self.optionKey
...
- (void)standardKeyMapperWillMapKey:(iTermStandardKeyMapper *)standardKeyMapper {
    ...
    iTermStandardKeyMapperConfiguration *configuration =
        [[[iTermStandardKeyMapperConfiguration alloc] init] autorelease];

    configuration.outputFactory = _screen.terminalOutput;
    configuration.encoding = [iTermProfilePreferences unsignedIntegerForKey:KEY_CHARACTER_ENCODING
                                                                  inProfile:self.profile];
...
- (void)shortcutNavigationDidSetPrefix:(NSString *)prefix {
    [[_textview contentNavigationShortcuts]
        enumerateObjectsUsingBlock:^(iTermContentNavigationShortcut *shortcut, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void (^)(NSEvent *))shortcutNavigationActionForKeyEquivalent:(NSString *)characters {
    return
        [[_textview contentNavigationShortcuts] objectPassingTest:^BOOL(iTermContentNavigationShortcut *shortcut,
...
- (id<iTermCopyModeStateProtocol>)copyModeHandlerCreateState:(iTermCopyModeHandler *)handler NOT_COPY_FAMILY {
    ...
    state.coord = VT100GridCoordMake(_screen.cursorX - 1, _screen.cursorY - 1 + _screen.numberOfScrollbackLines);
    ...
    if (_textview.selection.allSubSelections.count == 1) {
        ...
        VT100GridAbsCoordRangeTryMakeRelative(sub.absRange.coordRange, _screen.totalScrollbackOverflow,
                                              ^(VT100GridCoordRange range) {
                                                  [_textview.window makeFirstResponder:self.mainResponder];
                                                  state.selecting = YES;
...
- (iTermBuiltInFunctions *)objectMethodRegistry {
    if (!_methods) {
        ...
        [_methods registerFunction:method namespace:@"dashterm2"];
        ...
        method = [[iTermBuiltInMethod alloc]
                 initWithName:@"set_status_bar_component_unread_count"
                defaultValues:@{}
                        types:@{@"identifier" : [NSString class], @"count" : [NSNumber class]}
            optionalArguments:[NSSet set]
                      context:iTermVariablesSuggestionContextSession
       sideEffectsPlaceholder:nil
                       target:self
        ...
        method =
            [[iTermBuiltInMethod alloc] initWithName:@"run_coprocess"
                                       defaultValues:@{}
                                               types:@{@"commandLine" : [NSString class], @"mute" : [NSNumber class]}
        ...
        method = [[iTermBuiltInMethod alloc]
                 initWithName:@"add_annotation"
                defaultValues:@{}
                        types:@{
                            @"startX" : [NSNumber class],
...
- (void)runCoprocessWithCompletion:(void (^)(id, NSError *))completion
    ...
    if (self.isBrowserSession) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.run-coprocess-command"
                                             code:0
...
- (void)addAnnotationWithCompletion:(void (^)(id, NSError *))completion
                             ...
                             startY:(NSNumber *)startYNumber
                               ...
                               text:(NSString *)text {
    if (self.isBrowserSession) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.add-annotation-command"
                                             code:0
                                         userInfo:@{NSLocalizedDescriptionKey : @"Not supported for browser sessions"}];
        completion(nil, error);
        return;
    }
    const VT100GridAbsCoordRange range = VT100GridAbsCoordRangeMake(startXNumber.intValue, startYNumber.longLongValue,
                                                                    endXNumber.intValue, endYNumber.longLongValue);
    const long long maxY = _screen.totalScrollbackOverflow + _screen.numberOfLines;
    if (startYNumber.integerValue > endYNumber.integerValue || startYNumber.integerValue < 0 ||
        startYNumber.integerValue > maxY || endYNumber.integerValue < 0 || endYNumber.integerValue > maxY ||
...
- (void)fetchTimeOffsetWithCompletion:(void (^)(id, NSError *))completion {
    ...
    [_conductor fetchTimeOffset:^(NSTimeInterval offset, NSString *tz, NSError *error) {
        if (error) {
            ...
            completion(@{@"offset" : @(offset), @"tz" : tz}, nil);
        }
    }];
}
...
- (void)sendTmuxCommandWithCompletion:(void (^)(id, NSError *))completion command:(NSString *)command {
    if (self.tmuxMode == TMUX_NONE || _tmuxController == nil) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.tmux-command"
                                             code:0
...
- (void)sendTmuxCommandMethodDidComplete:(NSString *)result completion:(void (^)(id, NSError *))completion {
    if (result) {
...
- (void)setNameWithCompletion:(void (^)(id, NSError *))completion name:(NSString *)name {
...
#pragma clang diagnostic pop
            // BUG-434: Replace assert(NO) with ELog - we won't get called for global notifications
            ELog(@"PTYSession: Unexpected session-level notification type %d", request.notificationType);
            break;
    }
    if (!subscriptions) {
        response.status = ITMNotificationResponse_Status_RequestMalformed;
...
- (void)loggingHelperStart:(iTermLoggingHelper *)loggingHelper {
    ...
    [loggingHelper
        logWithoutTimestamp:[NSData styleSheetWithFontFamily:self.textview.fontTable.asciiFont.font.familyName
    ...
    switch (loggingHelper.style) {
        ...
        case iTermLoggingStylePlainText: {
            NSDateFormatter *dateFormatter =
                [NSDateFormatter cacheableFormatterWithTemplate:[iTermAdvancedSettingsModel logTimestampFormat]];
            return [[NSString stringWithFormat:@"[%@] ", [dateFormatter stringFromDate:[NSDate date]]]
                stringByReplacingUnicodeSpacesWithASCIISpace];
        }
...
- (void)naggingControllerPrettyPrintJSON {
    iTermSelectionReplacement *replacement = [self.textview.replacementPayloadsForSelection
        objectPassingTest:^BOOL(iTermSelectionReplacement *candidate, NSUInteger index, BOOL *stop) {
            return candidate.kind == iTermSelectionReplacementKindJson;
        }];
    if (replacement) {
        [self.textview replaceSelectionWith:replacement];
        [iTermWarning showWarningWithTitle:@"You can find this feature under Edit > Replace Selection > Replace with "
                                           @"Pretty-Printed JSON if you want to use it again."
                                   actions:@[ @"OK" ]
                                 accessory:nil
                                identifier:nil
                               silenceable:kiTermWarningTypePersistent
        ...
        [iTermWarning
            showWarningWithTitle:
                @"Looks like the selection changed and is no longer a valid JSON object.\nYou can find this feature "
                @"under Edit > Replace Selection > Replace with Pretty-Printed JSON if you want to try again."
                         actions:@[ @"OK" ]
                      identifier:nil
                     silenceable:kiTermWarningTypePersistent
                          window:self.view.window];
...
- (void)naggingControllerSetBackgroundImageToFileWithName:(NSString *)filename {
    [self
        setSessionSpecificProfileValues:@{KEY_BACKGROUND_IMAGE_LOCATION : filename.length ? filename : [NSNull null]}];
}
...
- (void)naggingControllerDisableTriggersInInteractiveApps {
    ...
    [iTermProfilePreferences setObjectsFromDictionary:update
                                            inProfile:self.profile
                                                model:[ProfileModel sharedInstance]];
    [[iTermNotificationController sharedInstance]
                 notify:@"Profile Updated"
        withDescription:
            @"Triggers disabled in interactive apps. You can change this in Settings > Profiles > Advanced."];
}

- (void)naggingControllerAssignProfileToSession:(NSString *)arrangementName guid:(NSString *)guid {
    [self changeProfileInArrangementNamed:arrangementName
                                     guid:guid
...
- (void)changeProfileInArrangementNamed:(NSString *)arrangementName
    ...
    [alert setMessageText:
               @"Select a profile to use for this session. Your selection will be saved back to the arrangement."];
    [alert addButtonWithTitle:@"OK"];
    [alert addButtonWithTitle:@"Cancel"];
...
- (void)reallyChangeProfileInArrangement:(NSString *)arrangementName
    ...
    if (profile) {
        [self setProfileInArrangement:arrangementName withGUID:guid toProfile:profile];
    }
    completion(profile);
}
...
- (BOOL)canChangeProfileInArrangement {
    ...
    for (NSDictionary *windowArrangement in windowArrangements) {
        found = [PseudoTerminal arrangement:windowArrangement
                                 passesTest:^BOOL(NSDictionary *candidate) {
...
- (void)setProfileInArrangement:(NSString *)arrangementName withGUID:(NSString *)guid toProfile:(Profile *)profile {
    ...
    NSArray *modifiedArrangements = [windowArrangements mapWithBlock:^id _Nullable(NSDictionary *arrangement) {
        return [PseudoTerminal modifiedArrangement:arrangement
                                           mutator:^NSDictionary *(NSDictionary *sessionArrangement) {
                                               if ([sessionArrangement[SESSION_ARRANGEMENT_GUID] isEqual:guid]) {
                                                   NSMutableDictionary *fixed =
                                                       [[sessionArrangement mutableCopy] autorelease];
...
- (NSRect)composerManager:(iTermComposerManager *)composerManager
    ...
    if (composerManager.isAutoComposer) {
        ...
        DLog(@"width=%@ actualLinesFree=%@ gridOffsetInRows=%@ lineHeight=%@ titleBarHeight=%@ height=%@ "
             @"gridOffsetInPoints=%@ top=%@ y=%@",
             @(width), @(actualLinesAboveComposer), @(gridOffsetInRows), @(lineHeight), @(titleBarHeight), @(height),
             @(gridOffsetInPoints), @(top), @(y));
    } else {
    ...
    newFrame = NSMakeRect(newFrame.origin.x, y, width, height);
    return newFrame;
}
...
- (void)composerManager:(iTermComposerManager *)composerManager forwardMenuItem:(NSMenuItem *)menuItem {
    [_textview performSelector:menuItem.action withObject:menuItem];
}

- (void)composerManager:(iTermComposerManager *)composerManager preferredOffsetFromTopDidChange:(CGFloat)offset {
    if (_divorced) {
...
- (void)composerManager:(iTermComposerManager *)composerManager
    ...
    if (@available(macOS 11, *)) {
        if ([_conductor framing]) {
            ...
            if (aiSuggest) {
                ...
                [_conductor fetchSuggestions:[limited requestWrappingCompletion:^(
                                                          BOOL _suggestionOnly, NSArray<iTermCompletionItem *> *dumb,
    ...
    [[iTermSlowOperationGateway sharedInstance]
        findCompletionsWithPrefix:request.prefix
                    inDirectories:request.directories
                       ...
                       completion:^(NSArray<NSString *> *_Nonnull completions) {
                           NSArray<iTermCompletionItem *> *fileItems =
                               [completions mapWithBlock:^id _Nullable(NSString *filename) {
                                   return [[[iTermCompletionItem alloc]
                                       initWithValue:filename
...
- (void)sendCommand:(NSString *)command {
    if (_screen.commandRange.start.x < 0) {
        ...
        [[iTermShellHistoryController sharedInstance]
             addCommand:command
                 onHost:host
    ...
    if ([self haveAutoComposer]) {
        ...
        if ([iTermAdvancedSettingsModel smartLoggingWithAutoComposer]) {
            NSString *trimmedCommand =
                [command stringByTrimmingTrailingCharactersFromCharacterSet:[NSCharacterSet newlineCharacterSet]];
        ...
        const BOOL detectedByTrigger =
            [_composerManager.prefixUserData[PTYSessionComposerPrefixUserDataKeyDetectedByTrigger] boolValue];
        [_composerManager setPrefix:nil userData:nil];
        [_screen mutateAsynchronously:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
                                        id<VT100ScreenDelegate> delegate) {
            DLog(@"willSendCommand:%@", command);
            const VT100GridAbsCoord start =
                VT100GridAbsCoordMake(mutableState.currentGrid.cursor.x, mutableState.currentGrid.cursor.y +
                                                                             mutableState.numberOfScrollbackLines +
                                                                             mutableState.cumulativeScrollbackOverflow);
            [mutableState composerWillSendCommand:command startingAt:start];
...
- (void)composerManager:(iTermComposerManager *)composerManager sendToAdvancedPaste:(NSString *)command {
    [self openAdvancedPasteWithText:command escaping:iTermSendTextEscapingNone];
}

- (void)composerManager:(iTermComposerManager *)composerManager sendControl:(NSString *)control {
    [self writeTask:control];
}

- (BOOL)composerManager:(iTermComposerManager *)composerManager wantsKeyEquivalent:(NSEvent *)event {
    const NSEventModifierFlags mask = (NSEventModifierFlagOption | NSEventModifierFlagCommand |
                                       NSEventModifierFlagShift | NSEventModifierFlagControl);
...
- (void)updateAutoComposerSeparatorVisibility {
    ...
    _composerManager.separatorColor =
        [iTermTextDrawingHelper colorForLineStyleMark:iTermMarkIndicatorTypeSuccess
                                      backgroundColor:[_screen.colorMap colorForKey:kColorMapBackground]];
}
...
- (void)initializeMarksMinimap {
    ...
    NSMutableDictionary<NSNumber *, NSMutableIndexSet *> *sets = [[NSMutableDictionary alloc] initWithCapacity:count];
    ...
    for (NSInteger i = 0; i < count; i++) {
        [_view.marksMinimap setLines:sets[@(i)] ?: [NSMutableIndexSet indexSet] forType:i];
    }
}
...
- (void)intervalTreeDidAddObjectOfType:(iTermIntervalTreeObjectType)type onLine:(NSInteger)line {
    [self addMarkToMinimapOfType:type onLine:line];
}

- (void)addMarkToMinimapOfType:(iTermIntervalTreeObjectType)type onLine:(NSInteger)line {
    DLog(@"Add at %@", @(line));
    [iTermGCD assertMainQueueSafe];
    if (![iTermAdvancedSettingsModel showLocationsInScrollbar]) {
        return;
...
- (void)intervalTreeDidRemoveObjectOfType:(iTermIntervalTreeObjectType)type onLine:(NSInteger)line {
    DLog(@"Remove at %@", @(line));
...
- (void)removeMarkFromMinimapOfType:(iTermIntervalTreeObjectType)type onLine:(NSInteger)line {
    if (![iTermAdvancedSettingsModel showLocationsInScrollbar]) {
        return;
    }
    if (![iTermAdvancedSettingsModel showMarksInScrollbar]) {
...
- (void)updateMarksMinimapRangeOfVisibleLines {
    ...
    [_view.marksMinimap setFirstVisibleLine:_screen.totalScrollbackOverflow numberOfVisibleLines:_screen.numberOfLines];
}
...
- (void)tmuxControllerSessionSetTTL:(NSTimeInterval)ttl redzone:(BOOL)redzone {
    ...
    NSString *title = [NSString
        stringWithFormat:@"This session will pause in about %@ second%@ because it is buffering too much data.",
                         @(safeTTL), safeTTL == 1 ? @"" : @"s"];
    iTermAnnouncementViewController *announcement =
        [self announcementWithIdentifier:PTYSessionAnnouncementIdentifierTmuxPaused];
    if (announcement) {
...
- (void)triggersCopyToProfile {
    [ProfileModel updateSharedProfileWithGUID:self.profile[KEY_ORIGINAL_GUID]
                                    newValues:@{
                                        KEY_TRIGGERS : self.profile[KEY_TRIGGERS] ?: @[],
                                        KEY_TRIGGERS_USE_INTERPOLATED_STRINGS :
                                                self.profile[KEY_TRIGGERS_USE_INTERPOLATED_STRINGS]
                                            ?: @NO
                                    }];
}
...
- (void)filterDestinationAppendScreenCharArray:(ScreenCharArray *)sca {
    ...
    DLog(@"%@: filterDestinationAppendScreenCharArray: Add <%@> to pending filter update which now has %@ entries",
         self, sca, @(_pendingFilterUpdates.count));
    if (wasEmpty) {
...
- (void)immutableColorMap:(id<iTermColorMapReading>)colorMap
     didChangeColorForKey:(iTermColorMapKey)theKey
                     from:(NSColor *)before
                       to:(NSColor *)after {
...
- (void)performActionForCapturedOutput:(CapturedOutput *)capturedOutput {
    __weak __typeof(self) weakSelf = self;
    [capturedOutput.promisedCommand onQueue:dispatch_get_main_queue()
                                       then:^(NSString *_Nonnull command) {
                                           [weakSelf reallyPerformActionForCapturedOutput:capturedOutput
                                                                                  command:command];
                                       }];
}

- (void)reallyPerformActionForCapturedOutput:(CapturedOutput *)capturedOutput command:(NSString *)command {
    [self launchCoprocessWithCommand:command identifier:nil silent:NO triggerTitle:@"Captured Output trigger"];
    [self takeFocus];
}
...
- (void)triggerSideEffectShowAlertWithMessage:(NSString *)message
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf reallyShowTriggerAlertWithMessage:message rateLimit:rateLimit disable:disable];
    });
}
...
- (void)triggerSideEffectShowCapturedOutputToolNotVisibleAnnouncementIfNeeded {
    ...
    void (^completion)(int selection) = ^(int selection) {
        switch (selection) {
            ...
            case 1:
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:kSuppressCaptureOutputToolNotVisibleWarning];
                break;
        }
    };
    iTermAnnouncementViewController *announcement =
        [iTermAnnouncementViewController announcementWithTitle:theTitle
                                                   ...
                                                   withActions:@[ @"Show It", @"Silence Warning" ]
                                                    completion:completion];
    announcement.dismissOnKeyDown = YES;
    [self queueAnnouncement:announcement identifier:kSuppressCaptureOutputToolNotVisibleWarning];
}
    ...
    iTermAnnouncementViewController *announcement =
        [iTermAnnouncementViewController announcementWithTitle:theTitle
                                                   ...
                                                   withActions:@[ @"Install", @"Silence Warning" ]
                                                    completion:completion];
    [self queueAnnouncement:announcement identifier:kTwoCoprocessesCanNotRunAtOnceAnnouncementIdentifier];
}
...
- (void)triggerSideEffectOpenPasswordManagerToAccountName:(NSString *_Nullable)accountName {
    [iTermGCD assertMainQueueSafe];
    // Dispatch because you can't have a runloop in a side-effect and the password manager is a bunch of modal UI - why
    // take chances?
    dispatch_async(dispatch_get_main_queue(), ^{
        iTermApplicationDelegate *itad = [iTermApplication.sharedApplication delegate];
        [itad openPasswordManagerToAccountName:accountName inSession:self];
    });
}
...
- (void)triggerSideEffectInvokeFunctionCall:(NSString *_Nonnull)invocation
    ...
    [iTermGCD assertMainQueueSafe];
    iTermVariableScope *scope = [self.variablesScope variableScopeByAddingBackreferences:captureStringArray
                                                                                   owner:trigger];
    [scope setValuesFromDictionary:temporaryVariables];
    [self invokeFunctionCall:invocation scope:scope origin:@"Trigger"];
}

- (void)triggerSideEffectSetValue:(id _Nullable)value forVariableNamed:(NSString *_Nonnull)name {
    [iTermGCD assertMainQueueSafe];
...
- (void)pasteboardReporter:(iTermPasteboardReporter *)sender reportPasteboard:(NSString *)pasteboard {
    NSData *data = [_screen.terminalOutput reportPasteboard:pasteboard contents:[NSString stringFromPasteboard] ?: @""];
    [self screenSendReportData:data];
    [_view showUnobtrusiveMessage:[NSString stringWithFormat:@"Clipboard contents reported"] duration:3];
}
