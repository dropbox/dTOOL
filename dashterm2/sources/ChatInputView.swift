...
class ChatInputView: NSView, NSTextFieldDelegate {
    ...
    init() {
        // BUG-1761: Use nil coalescing with fallback instead of force unwrap
        sendImage = NSImage(systemSymbolName: SFSymbol.paperplaneFill.rawValue, accessibilityDescription: "Send") ?? NSImage()
        stopImage = NSImage(systemSymbolName: SFSymbol.stopCircleFill.rawValue, accessibilityDescription: "Stop") ?? NSImage()
        ...
        // BUG-1762: Use nil coalescing with fallback instead of force unwrap
        var addImage = NSImage.it_image(forSymbolName: SFSymbol.plus.rawValue, accessibilityDescription: "Attach files", fallbackImageName: "plus", for: ChatInputView.self) ?? NSImage()
        if #available(macOS 11.0, *) {
            ...
            addImage = addImage.withSymbolConfiguration(config) ?? addImage
        }
    ...
    required init?(coder: NSCoder) {
        // BUG-f836: Return nil instead of crashing for unused coder initializer
        DLog("ChatInputView init(coder:) is not supported")
        return nil
    }
