...
NSArray<NSString *> *iTermAllFunctionSignaturesFromNamespaceAndNameAndArguments(NSString *namespace, NSString *name,
...
NSString *iTermFunctionSignatureFromNamespaceAndNameAndArguments(NSString *namespace, NSString *name,
                                                                 NSArray<NSString *> *argumentNames) {
    NSString *combinedArguments =
        [[argumentNames sortedArrayUsingSelector:@selector(compare:)] componentsJoinedByString:@","];
    NSString *tail = [NSString stringWithFormat:@"%@(%@)", name, combinedArguments];
    if (!namespace) {
        return tail;
    }
...
- (nullable NSError *)typeCheckParameters:(NSDictionary<NSString *, id> *)parameters {
    for (NSString *name in parameters) {
        ...
        if ([parameters[name] isKindOfClass:[NSNull class]] && [_optionalArguments containsObject:name]) {
            continue;
        }
...
- (NSError *)typeMismatchError:(NSString *)argument wanted:(Class)wanted got:(nullable id)object {
    NSString *reason =
        [NSString stringWithFormat:@"Type mismatch for argument %@. Expected %@ but got %@.", argument,
                                   NSStringFromClass(wanted), object ? NSStringFromClass([object class]) : @"(null)"];
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.bif"
                               code:2
                           userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}

- (NSError *)invalidArgumentError:(NSString *)argument {
    NSString *reason = [NSString stringWithFormat:@"Invalid argument %@ to %@", argument, _name];
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.bif"
                               code:4
                           userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _functions = [[NSMutableDictionary alloc] initWithCapacity:64];
    }
    return self;
}

- (void)registerFunction:(iTermBuiltInFunction *)function namespace:(NSString *)namespace {
    NSArray<NSString *> *signatures = iTermAllFunctionSignaturesFromNamespaceAndNameAndArguments(
        namespace, function.name, function.argumentsAndTypes.allKeys, function.optionalArguments);
    for (NSString *signature in signatures) {
        // BUG-f906: Replace assert with guard - duplicate registration should log not crash
        if (_functions[signature]) {
            ELog(@"BUG-f906: Duplicate function registration for signature: %@", signature);
            continue;
        }
...
- (NSError *)undeclaredIdentifierError:(NSString *)identifier {
    NSString *reason = [NSString stringWithFormat:@"Undeclared identifier %@", identifier];
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.bif"
                               code:1
                           userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
- (NSError *)invalidReferenceError:(NSString *)reference name:(NSString *)name {
    NSString *reason = [NSString stringWithFormat:@"Invalid reference “%@” to %@", reference, name];
    return [NSError errorWithDomain:@"com.dashterm.dashterm2.bif"
                               code:3
                           userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
- (iTermBuiltInMethod *)methodWithSignature:(NSString *)signature {
    ...
    for (NSString *methodSignature in _functions) {
        ...
        // BUG-f907: Replace assert with guard - skip non-method functions instead of crashing
        if (!method) {
            continue;
        }
...
+ (void)registerBuiltInFunction {
    ...
    dispatch_once(&onceToken, ^{
        ...
        iTermBuiltInFunction *func = [[iTermBuiltInFunction alloc] initWithName:@"count"
        ...
        [[iTermBuiltInFunctions sharedInstance] registerFunction:func namespace:@"dashterm2"];
    });
}

+ (void)countOfObject:(id)value completion:(iTermBuiltInFunctionCompletionBlock)completion {
    if (!value) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.array-count"
                                             code:1
        ...
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.array-count"
                                         ...
                                         userInfo:@{
                                             NSLocalizedDescriptionKey : [NSString
                                                 stringWithFormat:@"Argument must be an array (was %@)", [value class]]
                                         }];
        completion(nil, error);
        return;
    }
...
+ (void)registerBuiltInFunction {
    ...
    dispatch_once(&onceToken, ^{
        ...
        iTermBuiltInFunction *func = [[iTermBuiltInFunction alloc] initWithName:@"urlEncode"
...
+ (void)urlEncoded:(id)value completion:(iTermBuiltInFunctionCompletionBlock)completion {
    if (!value) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.url-encode"
                                             code:1
    ...
    if (!string) {
        NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.url-encode"
                                         ...
                                         userInfo:@{
                                             NSLocalizedDescriptionKey : [NSString
                                                 stringWithFormat:@"Argument must be a string (was %@)", [value class]]
                                         }];
        completion(nil, error);
        return;
    }
...
+ (NSArray<iTermReflectionMethodArgument *> *)argumentsFromTarget:(id<iTermObject>)target action:(SEL)action {
...
+ (NSDictionary<NSString *, Class> *)argumentDictionaryFromArray:(NSArray<iTermReflectionMethodArgument *> *)arguments {
    ...
    NSMutableDictionary<NSString *, Class> *result = [[NSMutableDictionary alloc] initWithCapacity:arguments.count];
    for (iTermReflectionMethodArgument *obj in arguments) {
        // BUG-f908: Replace assert with guard - duplicate argument name should log not crash
        if (result[obj.argumentName] != nil) {
            ELog(@"BUG-f908: Duplicate argument name '%@' in method reflection", obj.argumentName);
            continue;
        }
    ...
    self = [super initWithName:name
        ...
        sideEffectsPlaceholder:sideEffectsPlaceholder
                         block:^(NSDictionary *_Nonnull parameters,
                             ...
                             // BUG-13010: Replace assert(NO) with error completion
                             // This block should never be called - use callWithArguments:completion: instead.
                             NSError *error = [NSError
                                 errorWithDomain:@"com.dashterm.dashterm2.built-in-functions"
                                            code:1
                                        userInfo:@{
                                            NSLocalizedDescriptionKey : @"Internal error: direct block invocation not "
                                                                        @"supported for action-based functions"
                                        }];
                             completion(nil, error);
                         }];
    if (self) {
    ...
    for (NSInteger i = 0; i < _args.count; i++) {
        ...
        if ([arg.argumentName hasSuffix:@"WithCompletion"]) {
            ...
            if (!parameters[arg.argumentName]) {
                // BUG-f909: Replace assert with guard - missing required arg should log not crash
                if (![_optionalArguments containsObject:arg.argumentName]) {
                    ELog(@"BUG-f909: Missing required argument '%@' in callWithArguments", arg.argumentName);
                    completion(nil, [NSError errorWithDomain:@"com.dashterm.dashterm2.bif"
                                                        code:4
                                                    userInfo:@{
                                                        NSLocalizedFailureReasonErrorKey :
                                                            [NSString stringWithFormat:@"Missing required argument: %@",
                                                                                       arg.argumentName]
                                                    }]);
                    return;
                }
        ...
        if (requiredClass) {
            ...
            if (isNull && ![_optionalArguments containsObject:arg.argumentName]) {
                completion(nil, [self typeMismatchError:arg.argumentName wanted:requiredClass got:nil]);
                return;
            }
            if (!isNull && ![parameters[arg.argumentName] isKindOfClass:requiredClass] &&
                ![_optionalArguments containsObject:arg.argumentName]) {
                Class actualClass = [parameters[arg.argumentName] class];
                completion(nil, [self typeMismatchError:arg.argumentName wanted:requiredClass got:actualClass]);
                return;
            }
        }
