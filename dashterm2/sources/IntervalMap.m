...
- (IntRange *)intersectionWithRange:(IntRange *)other {
    return [IntRange rangeWithMin:MAX(min, other->min) limit:MIN([self limit], [other limit])];
}

- (NSArray *)rangesAfterSubtractingRange:(IntRange *)other {
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:2]; // At most 2 result ranges
    // Possibilities:
    // 1.      2.      3.       4.       5.       6.
...
- (instancetype)init {
    self = [super init];
    if (self) {
        elements_ = [[NSMutableArray alloc] initWithCapacity:16]; // Interval map elements
    }
    return self;
}
...
- (NSArray *)elementsInRange:(IntRange *)range {
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:elements_.count];
...
- (void)setObject:(id)object forRange:(IntRange *)range {
    ...
    for (IntervalMapElement *elt in overlappingElements) {
        ...
        for (IntRange *fragment in fragments) {
            [self insertElement:[IntervalMapElement elementWithRange:fragment value:elt.value]];
        }
    }
...
- (NSArray *)ranges:(NSArray *)orig bySubtractingRange:(IntRange *)sub {
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:orig.count * 2]; // Up to 2 fragments per range
...
- (NSString *)description {
    // Each element description is ~30 chars; allocate for all elements
    NSMutableString *result = [NSMutableString stringWithCapacity:elements_.count * 30];
    for (IntervalMapElement *e in elements_) {
