...
NSString *const iTermWebViewErrorDomain = @"com.dashterm.dashterm2.webview";
...
- (NSString *)browserName {
    // BUG-3035: Initialize error to NULL and release if populated
    CFErrorRef error = NULL;
    NSURL *URL = self.webView.URL ?: [NSURL URLWithString:@"http://example.com"];
    NSURL *appUrl =
        (__bridge_transfer NSURL *)LSCopyDefaultApplicationURLForURL((__bridge CFURLRef)URL, kLSRolesAll, &error);
    if (appUrl) {
        NSString *name = nil;
        ...
        // BUG-3035: Release error when function fails
        CFRelease(error);
    }
...
- (void)terminateWebView {
    ...
    // BUG-9459: Clean up script message handler to prevent memory leak
    // WKUserContentController strongly retains its message handlers.
    // Removing the handler breaks the retain cycle - don't set userContentController to nil
    // as it's a non-nullable property (causes -Wnonnull warning).
    [_webView.configuration.userContentController removeScriptMessageHandlerForName:@"dashterm2Invoke"];
}
...
- (WKWebView *)webViewWithDelegate:(id<iTermWebViewDelegate>)delegate {
    ...
    if (!webUserAgent.length) {
        // BUG-8: Web view user agent application name updated to DashTerm2
        configuration.applicationNameForUserAgent = @"DashTerm2";
    }
    ...
    [configuration.userContentController it_setWeakAssociatedObject:webView
                                                             forKey:&iTermWebViewFactoryUserControllerWebviewKey];
    webView.UIDelegate = self;

    return webView;
}

- (void)registerUserScriptInConfiguration:(WKWebViewConfiguration *)configuration
                                 delegate:(id<iTermWebViewDelegate>)delegate {
    WKUserContentController *userController = [[WKUserContentController alloc] init];
    [userController it_setWeakAssociatedObject:delegate forKey:&iTermWebViewFactoryUserControllerDelegateKey];
    [userController addScriptMessageHandler:self name:@"dashterm2Invoke"];

    NSURL *url = [[NSBundle bundleForClass:[self class]] URLForResource:@"dashterm2Invoke" withExtension:@"js"];
    NSString *js = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:NULL];
    if (!js) {
        DLog(@"Failed to get dashterm2Invoke.js");
        return;
    }
...
- (void)userContentController:(WKUserContentController *)userContentController
    ...
    if ([message.name isEqualToString:@"dashterm2Invoke"]) {
        __weak id<iTermWebViewDelegate> delegate =
            [userContentController it_associatedObjectForKey:&iTermWebViewFactoryUserControllerDelegateKey];
        ...
        WKWebView *webview =
            [userContentController it_associatedObjectForKey:&iTermWebViewFactoryUserControllerWebviewKey];
        if (!webview) {
            // BUG-462: Replace assert(NO) with ELog - webview can be deallocated
            // This can happen if webview is deallocated before message is processed
            ELog(@"iTermWebViewWrapperViewController received script message but webview is nil");
            return;
        }
        ...
        [iTermScriptFunctionCall callFunction:invocation
                           ...
                           sideEffectsAllowed:YES
                                   ...
                                   completion:^(id value, NSError *error, NSSet<NSString *> *missing) {
                                       if (error) {
                                           [delegate itermWebViewScriptInvocation:invocation didFailWithError:error];
                                           return;
                                       }
                                       [self sendReturnValue:value
                                                  forMessage:dict
                                                   toWebview:webview
...
- (void)sendReturnValue:(id)value
    ...
    if (!callback) {
        if (completion) {
            completion([NSError errorWithDomain:iTermWebViewErrorDomain
                                           code:iTermWebViewErrorCodeMissingCallback
                                       userInfo:nil]);
        }
        return;
    }
    ...
    if (error) {
        script = innerScript = [NSString
            stringWithFormat:@"throw %@", [NSJSONSerialization it_jsonStringForObject:error.localizedDescription]];
    } else {
        innerScript =
            [NSString stringWithFormat:@"%@(%@)", callback, [NSJSONSerialization it_jsonStringForObject:value]];
        script = [NSString stringWithFormat:@"try { %@; } catch(err) { alert('Caught error while sending return value "
                                            @"to javascript: ' + err.message + '\\n' + err.stack); }",
                                            innerScript];
    }
    __weak id<iTermWebViewDelegate> delegate = [webview.configuration.userContentController
        it_associatedObjectForKey:&iTermWebViewFactoryUserControllerDelegateKey];
    ...
    [webview evaluateJavaScript:script
              completionHandler:^(id _Nullable value, NSError *_Nullable error) {
                  ...
                  if (error.userInfo[messageKey]) {
                      description = [NSString stringWithFormat:@"Error evaluating '%@' at line %@: %@", innerScript,
                                                               error.userInfo[lineKey], error.userInfo[messageKey]];
                  }
                  completion([NSError errorWithDomain:iTermWebViewErrorDomain
