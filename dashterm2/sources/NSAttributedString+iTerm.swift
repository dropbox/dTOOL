...
class AttributeToControlSequenceConverter {
    ...
    private func convert(_ attributes: [NSAttributedString.Key : Any]) -> [String] {
        ...
        let url = { () -> iTermURL? in
            ...
            guard !urlHasDisallowedScheme(href) else {
                return nil
            }
        ...
        let ea = iTermExternalAttribute(havingUnderlineColor: underlineColor != nil,
                                        ...
                                        blockIDList: nil,
                                        controlCode: nil)
        // BUG-1748: Use as? with empty array fallback instead of as! force cast
    ...
    class func attributedString(html htmlString: String, font: NSFont, paragraphStyle: NSParagraphStyle) -> NSAttributedString? {
        ...
        while currentIndex < htmlString.endIndex {
            if htmlString[currentIndex] == "<", let endIndex = htmlString[currentIndex...].range(of: ">")?.upperBound, let tagRange = htmlString[currentIndex..<endIndex].range(of: "<a href=\""), tagRange.lowerBound == currentIndex {
                ...
                // BUG-1668: Use limitedBy to safely calculate index offset (expects "> after closing quote)
                guard let textStart = htmlString.index(urlEnd, offsetBy: 2, limitedBy: htmlString.endIndex) else {
                ...
                // BUG-1668: Use limitedBy to safely calculate index offset (expects </a> = 4 chars)
                currentIndex = htmlString.index(textEnd, offsetBy: 4, limitedBy: htmlString.endIndex) ?? htmlString.endIndex
            } else {
