    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(applicationDidBecomeActive:)
                                                   ...
                                                   object:nil];
        _hotKeys = [[NSMutableArray alloc] initWithCapacity:16];  // Registered hotkeys
        _profileHotKeysBirthingWindows = [[NSMutableArray alloc] initWithCapacity:4];  // Profile hotkeys creating windows
    }
    return self;
}
...
- (void)removeHotKey:(iTermBaseHotKey *)hotKey {
    ...
    // BUG-f1015: Gracefully handle removing non-existent hotkey instead of crashing
    if (![_hotKeys containsObject:hotKey]) {
        DLog(@"BUG-f1015: removeHotKey called with hotKey not in _hotKeys: %@", hotKey);
        return;
    }
...
- (BOOL)createHiddenWindowsByDecoding:(iTermEncoderGraphRecord *)record {
    ...
    NSMutableArray<NSDictionary *> *restorableStates = [[NSMutableArray alloc] initWithCapacity:index.keys.count];

    [record enumerateArrayWithKey:@"windows"
                            block:^(NSString *_Nonnull guid, NSInteger i, id obj, BOOL *_Nonnull stop) {
...
- (iTermOrderedDictionary<NSString *, iTermProfileHotKey *> *)profileHotkeyWindowsIndex {
    return [iTermOrderedDictionary byMapping:_hotKeys
                                       block:^id _Nullable(NSUInteger index, __kindof iTermBaseHotKey *hotkey) {
...
- (BOOL)encodeGraphWithEncoder:(iTermGraphEncoder *)encoder {
    ...
    [encoder encodeArrayWithKey:@"windows"
                    ...
                    identifiers:index.keys
                        options:0
                          block:^BOOL(NSString *_Nonnull identifier, NSInteger i,
...
- (BOOL)anyProfileHotkeyWindowHasInvalidState {
    // BUG-7328: Was returning YES when NO windows had invalid state (inverted logic)
    return [[[self profileHotkeyWindowsIndex] values] filteredArrayUsingBlock:^BOOL(iTermProfileHotKey *obj) {
               ...
               return result;
           }].count > 0; // Should return YES when count > 0 (some have invalid state)
}

- (NSArray *)restorableStates {
    NSMutableArray *array = [[NSMutableArray alloc] initWithCapacity:_hotKeys.count];
    for (__kindof iTermBaseHotKey *hotkey in _hotKeys) {
        if ([hotkey isKindOfClass:[iTermProfileHotKey class]]) {
...
- (NSArray<iTermHotKeyDescriptor *> *)descriptorsForProfileHotKeysExcept:(Profile *)profile {
    return [self.profileHotKeys flatMapWithBlock:^id(iTermProfileHotKey *profileHotKey) {
        if (![profileHotKey.profile[KEY_GUID] isEqualToString:profile[KEY_GUID]]) {
            NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:2]; // Key and modifier hotkeys
            if ([iTermProfilePreferences stringForKey:KEY_HAS_HOTKEY inProfile:profileHotKey.profile]) {
                ...
                NSEventModifierFlags modifiers =
                    [iTermProfilePreferences unsignedIntegerForKey:KEY_HOTKEY_MODIFIER_FLAGS
                                                         inProfile:profileHotKey.profile];
                [result addObject:[iTermHotKeyDescriptor descriptorWithKeyCode:keyCode modifiers:modifiers]];
            }

            if ([iTermProfilePreferences boolForKey:KEY_HOTKEY_ACTIVATE_WITH_MODIFIER
                                          inProfile:profileHotKey.profile]) {
...
- (NSArray<PseudoTerminal *> *)siblingWindowControllersOf:(PseudoTerminal *)windowController {
    NSArray<iTermHotKeyDescriptor *> *referenceHotKeyDescriptors =
        [[self profileHotKeyForWindowController:windowController] hotKeyDescriptors];
    iTermHotKeyDescriptor *referenceModifierActivationDescriptor =
        [[self profileHotKeyForWindowController:windowController] modifierActivationDescriptor];
...
- (BOOL)dockIconClicked {
    ...
    if (self.visibleWindowControllers.count > 0) {
        ...
        [self.profileHotKeys enumerateObjectsUsingBlock:^(iTermProfileHotKey *_Nonnull profileHotKey, NSUInteger idx,
                                                          BOOL *_Nonnull stop) {
            if (profileHotKey.hotKeyWindowOpen) {
                [profileHotKey hideHotKeyWindowAnimated:YES
                                        suppressHideApp:NO
                                       otherIsRollingIn:NO
                                       causedByKeypress:YES];
                profileHotKey.wasAutoHidden = NO;
                handled = YES;
            }
        }];
    } else {
        NSUInteger numberOfTerminalWindowsOpen = [[[(iTermApplication *)NSApp orderedWindowsPlusVisibleHotkeyPanels]
            filteredArrayUsingBlock:^BOOL(NSWindow *window) {
                return [window.windowController isKindOfClass:[PseudoTerminal class]] && [window isVisible];
            }] count];
        [self.profileHotKeys enumerateObjectsUsingBlock:^(iTermProfileHotKey *_Nonnull profileHotKey, NSUInteger idx,
                                                          BOOL *_Nonnull stop) {
            switch ([iTermProfilePreferences unsignedIntegerForKey:KEY_HOTKEY_DOCK_CLICK_ACTION
                                                         inProfile:profileHotKey.profile]) {
...
- (BOOL)addRevivedHotkeyWindowController:(PseudoTerminal *)windowController forProfileWithGUID:(NSString *)guid {
...
- (iTermProfileHotKey *)didCreateWindowController:(PseudoTerminal *)windowController withProfile:(Profile *)profile {
    iTermProfileHotKey *profileHotKey =
        [_hotKeys objectOfClass:[iTermProfileHotKey class]
    ...
    if (!profileHotKey.windowController.weaklyReferencedObject) {
        ...
        if ([windowController.window isKindOfClass:[iTermPanel class]]) {
            ...
            // BUG-7331: Normal level means regular window, not floating
            hotkeyWindowType = iTermHotkeyWindowTypeRegular;
        } else {
            // Above-normal level means floating window
...
- (iTermProfileHotKey *)profileHotKeyForGUID:(NSString *)guid {
    ...
    iTermProfileHotKey *profileHotKey =
        [profileHotKeys objectPassingTest:^BOOL(iTermProfileHotKey *element, NSUInteger index, BOOL *stop) {
...
- (BOOL)shouldAutoHide {
    ...
    if ([keyWindow respondsToSelector:@selector(autoHidesHotKeyWindow)] && ![keyWindow autoHidesHotKeyWindow]) {
    ...
    if (keyWindowController != nil && [[[NSBundle bundleForClass:keyWindowController.class] bundlePath]
                                          isEqualToString:[[NSBundle bundleForClass:[SUUpdater class]] bundlePath]]) {
        DLog(@"The key window's controller appears to belong to Sparkle (it is %@), so don't auto-hide.",
             keyWindowController);
        return NO;
    }
    ...
    DLog(@"main window is %@, key terminals are %@, hotkey terminals are %@", [NSApp mainWindow], keyTerminalWindows,
         hotKeyWindowControllers);
...
- (NSArray<PseudoTerminal *> *)hotKeyWindowControllers {
    ...
    NSArray<PseudoTerminal *> *hotKeyWindowControllers =
        [profileHotKeys mapWithBlock:^id(iTermProfileHotKey *profileHotKey) {
...
- (BOOL)willFinishRollingOutProfileHotKey:(iTermProfileHotKey *)profileHotKey causedByKeypress:(BOOL)causedByKeypress {
    // Restore the previous state (key window or active app) unless we switched
    // to another hotkey window.
    DLog(@"Finished rolling out %p. key window is %@.", profileHotKey.windowController,
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
    for (iTermProfileHotKey *profileHotKey in self.profileHotKeys) {
        if (profileHotKey.windowController.weaklyReferencedObject && profileHotKey.wasAutoHidden &&
