...
- (void)awakeFromNib {
    ...
    footerView_.frame =
        NSMakeRect(0, self.footerMargin, self.window.contentView.frame.size.width - footerHeight, footerHeight);
    ...
    _footerBackground.frame =
        NSMakeRect(0, 0, self.window.contentView.frame.size.width, footerHeight + self.footerMargin * 1.5);
...
- (void)popWithDelegate:(id<PopupDelegate>)delegate inWindow:(NSWindow *)owningWindow {
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    for (NSResponder *responder in self.allResponders) {
        if ([responder respondsToSelector:@selector(validateMenuItem:)] && [responder validateMenuItem:menuItem]) {
            return YES;
        }
    }
    return NO;
...
- (NSArray *)allResponders {
    NSMutableArray *responders = [[NSMutableArray alloc] initWithCapacity:8];
...
- (CGFloat)desiredHeight {
    CGFloat height = [[tableView_ headerView] frame].size.height +
                     MIN(20, [model_ count]) * ([tableView_ rowHeight] + [tableView_ intercellSpacing].height);
...
- (void)setPosition:(BOOL)canChangeSide {
    ...
    if (canChangeSide) {
        ...
        float topOverflow =
            p.y + 2 * frame.size.height + NSHeight(cursorRect) - (monitorFrame.origin.y + monitorFrame.size.height);
...
// BUG-390: Add bounds check to prevent array index out of bounds crash
- (BOOL)passKeyEventToDelegateForSelector:(SEL)selector string:(NSString *)string {
    if ([_delegate respondsToSelector:@selector(popupHandleSelector:string:currentValue:)]) {
        ...
        if ([tableView_ selectedRow] >= 0) {
            NSInteger convertedIndex = [self convertIndex:[tableView_ selectedRow]];
            NSArray *model = [self model];
            if (convertedIndex >= 0 && convertedIndex < (NSInteger)model.count) {
                entry = [model objectAtIndex:convertedIndex];
            }
        }
...
- (NSAttributedString *)attributedStringForEntry:(PopupEntry *)entry isSelected:(BOOL)isSelected {
    ...
    for (int i = 0; i < [substring_ length]; ++i) {
        ...
        NSRange r = [temp rangeOfString:[NSString stringWithCharacters:&wantChar length:1]
        ...
        if (prefix.length > 0) {
            ...
            attributedSubstr = [[[NSAttributedString alloc] initWithString:substr
        ...
        attributedSubstr = [[[NSAttributedString alloc] initWithString:[NSString stringWithCharacters:&matchChar
                                                                                               length:1]
    ...
    if ([temp length] > 0) {
        NSAttributedString *attributedSubstr =
            [[[NSAttributedString alloc] initWithString:temp attributes:plainAttributes] autorelease];
        [as appendAttributedString:attributedSubstr];
    }
...
// BUG-389: Add bounds check to prevent array index out of bounds crash
- (id)tableView:(NSTableView *)aTableView
    objectValueForTableColumn:(NSTableColumn *)aTableColumn
                          row:(NSInteger)rowIndex {
    int i = [self convertIndex:rowIndex];
    NSArray *model = [self model];
    if (i < 0 || (NSUInteger)i >= model.count) {
        return nil;
    }
    PopupEntry *e = [model objectAtIndex:i];
    return [self attributedStringForEntry:e isSelected:[aTableView selectedRow] == rowIndex];
}

// BUG-391: Add bounds check to prevent array index out of bounds crash
- (void)tableViewSelectionDidChange:(NSNotification *)aNotification {
    if (!reloading_) {
        ...
        if (rowNum >= 0) {
            ...
            NSInteger convertedIndex = [self convertIndex:rowNum];
            if (convertedIndex >= 0 && convertedIndex < (NSInteger)model_.count) {
                s = [[model_ objectAtIndex:convertedIndex] mainValue];
            }
        }
        if (!s) {
            s = @"";
