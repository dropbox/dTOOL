...
class PlaygroundTextView: ShiftEnterTextView {
    ...
    private func row(_ row: Int) -> String {
        guard let lines = textStorage?.string.components(separatedBy: "\n"),
              row >= 0 && row < lines.count else {
            return ""
        }
        return lines[row]
    }
    ...
    func highlightGridRange(_ range: VT100GridCoordRange) {
        ...
        // BUG-1672: Use guard instead of force unwrap for utf16OffsetOfLine
        guard let lowerBase = s.utf16OffsetOfLine(Int(range.start.y)),
              let upperBase = s.utf16OffsetOfLine(Int(range.end.y)) else {
            return
        }
        let lower = lowerBase + startDelta.cellIndexForUTF16Index(Int(range.start.x))
        let upper = upperBase + endDelta.cellIndexForUTF16Index(Int(range.end.x))
