...
- (void)handleWriteScriptCommand:(NSScriptCommand *)command {
    ...
    if (contentsOfFile != nil) {
        // BUG-1724: Check for file read errors instead of ignoring them
        NSError *fileError = nil;
        aString = [NSString stringWithContentsOfFile:contentsOfFile
                                            encoding:NSUTF8StringEncoding
                                               error:&fileError];
        if (fileError) {
            DLog(@"Failed to read contentsOfFile '%@': %@", contentsOfFile, fileError);
            // Don't proceed if file couldn't be read
            return;
        }
    }
    ...
    if (self.tmuxMode == TMUX_CLIENT) {
        ...
        // Wait for shell to have output before writing, but with timeout and UI responsiveness
        // Maximum wait time: 5 seconds (reduced from 20 to avoid excessive blocking)
        static const NSTimeInterval kMaxWaitTime = 5.0;
        static const NSTimeInterval kPollInterval = 0.05;

        NSDate *startTime = [NSDate date];
        while ([self.shell hasOutput] == NO) {
            NSTimeInterval elapsed = [[NSDate date] timeIntervalSinceDate:startTime];
            if (elapsed >= kMaxWaitTime) {
                DLog(@"Timed out waiting for shell output after %.1f seconds", elapsed);
                break;
            }
            // Use RunLoop instead of usleep to keep the UI responsive during AppleScript execution
...
- (id)handleSplitHorizontallyWithDefaultProfile:(NSScriptCommand *)scriptCommand {
    ...
    [self splitVertically:NO
              withProfile:[[ProfileModel sharedInstance] defaultBookmark]
                  command:args[@"command"]
               completion:^(PTYSession *session) {
        // BUG-1045: Consistent order - restore former session before resuming AppleScript
        [formerSession activateSessionAndTab];
        dispatch_async(dispatch_get_main_queue(), ^{
            [scriptCommand resumeExecutionWithResult:session.objectSpecifier ? session : nil];
        });
