...
class PortholeContainerView: NSView {
    ...
    var backgroundColor = NSColor.textBackgroundColor {
        didSet {
            // BUG-1656: Use guard with optional binding instead of force unwrap for color space conversion
            guard let sRGBColor = backgroundColor.usingColorSpace(.sRGB) else {
                layer?.backgroundColor = backgroundColor.cgColor
                return
            }
            let dimmed = sRGBColor.colorDimmed(by: 0.2, towardsGrayLevel: 0.5)
    ...
    static func closeButtonImage(_ color: NSColor) -> NSImage {
        ...
        // BUG-1656: Use guard with nil coalescing instead of force unwrap for image loading
        guard let image = NSImage.it_imageNamed("closebutton", for: Self.self) else {
            return NSImage()
        }
        return image.it_image(withTintColor: color)
    }
    ...
    // BUG-f583: Return nil instead of crashing for unused coder initializer
    required init?(coder: NSCoder) {
        DLog("PortholeContainerView init(coder:) is not supported")
        return nil
    }
    ...
    func makeWide() {
        ...
        wideButton.state = .on
        toggleWide(nil)
        // BUG-1620: Use guard instead of precondition - if toggle didn't work, just return
        guard wideMode else {
            DLog("Warning: makeWide() called toggleWide() but wideMode is still false")
            return
        }
        _ = layoutSubviews()
    }
