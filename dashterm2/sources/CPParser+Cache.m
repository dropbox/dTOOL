...
+ (NSString *)it_keyWithBNF:(NSString *)bnf start:(NSString *)start {
    ...
    NSString *key =
        [NSString stringWithFormat:@"%@ %@ %@%@", version, NSStringFromClass(self), bnf.it_contentHash, start]
            .it_contentHash;
    return key;
}
...
+ (NSMutableDictionary<NSString *, NSMutableArray *> *)it_cache {
    ...
    dispatch_once(&onceToken, ^{
        // Typically a small number of grammar caches (1-4)
...
+ (instancetype)it_cachedFromFile:(NSString *)file {
    ...
    // Note: requiresSecureCoding must be NO here because the third-party CoreParse
    // library classes (CPShiftReduceParser, CPGrammar, etc.) do not support NSSecureCoding.
    // This is safe because the cache file is written by the app itself to its cache directory,
    // keyed by a hash of the grammar and app version.
...
+ (instancetype)it_parserWithGrammarStart:(NSString *)start bnf:(NSString *)bnf {
    NSError *error = nil;
    CPGrammar *grammar = [CPGrammar grammarWithStart:start backusNaurForm:bnf error:&error];
