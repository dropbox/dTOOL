...
class JSONPortholeRenderer {
    ...
    private static func parse(_ text: String) throws -> Any {
        // BUG-1624: Use guard instead of force unwrap for data encoding
        guard let data = text.data(using: .utf8) else {
            throw NSError(domain: "JSONPortholeRenderer", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to encode text as UTF-8"])
        }
    ...
    init?(_ text: String) {
        do {
            ...
            // BUG-2666: Log parsing errors for debugging before returning nil
            DLog("JSONPortholeRenderer: Failed to parse JSON text (\(text.count) chars): \(error)")
            return nil
        }
    }
}
...
extension NSAttributedString {
    ...
    private static func jsonAttributes(visualAttributes: TextViewPorthole.VisualAttributes,
        ...
        // BUG-f852: Use new value on merge conflict instead of crashing
        return basicJSONAttributes(
            visualAttributes).merging([.foregroundColor: color]) { _, new in new }
    }
