...
class TerminalCommandMessageCellView: MessageCellView {
    ...
    private let icon: NSImageView = {
        // BUG-1596: Use guard/nil coalescing to safely load images instead of force unwrap
        let image: NSImage = {
            if #available(macOS 11, *) {
                guard let baseImage = NSImage(systemSymbolName: SFSymbol.desktopcomputer.rawValue, accessibilityDescription: "Command icon") else {
                    return NSImage()
                }
                if #available(macOS 12, *) {
                    return baseImage.withSymbolConfiguration(.init(paletteColors: [.white, .clear, .black])) ?? baseImage
                } else {
                    return baseImage
                }
            } else {
                return NSImage.it_imageNamed("CommandIcon",
                                             for: TerminalCommandMessageCellView.self) ?? NSImage()
            }
        }()
    ...
    // BUG-f526: Use guard with return instead of crashing for unexpected rendition flavor
    override func configure(with rendition: MessageRendition, maxBubbleWidth: CGFloat) {
        guard case .command(let commandFlavor) = rendition.flavor else {
            DLog("Warning: TerminalCommandMessageCellView received non-command rendition flavor")
            return
        }
    ...
    override func updateColors() {
        ...
        let (lightColor, darkColor) = (NSColor(fromHexString: "p3#448bf7") ?? .systemBlue,
                                       NSColor(fromHexString: "p3#4a93f5") ?? .systemBlue)
