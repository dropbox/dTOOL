...
final class VimKeyParser: NSObject {
    ...
    @objc let errorDomain = "com.dashterm.dashterm2.vim-key-parser"
...
extension VimKeyParser {
    ...
    private func parseToken(_ token: String) -> (characters: String, modifierFlags: NSEvent.ModifierFlags, keyCode: UInt16)? {
        ...
        if parts.count > 1 {
            ...
            baseKey = String(lastPart)
        }

        // This comes from `:help <>` in vim
        // Helper to safely create function key strings
        func functionKeyString(_ key: Int) -> String {
        ...
        let baseMapping: [String: (String, UInt16)] = [
            ...
            "Up":     (functionKeyString(NSUpArrowFunctionKey), UInt16(kVK_UpArrow)),
            "Down":   (functionKeyString(NSDownArrowFunctionKey), UInt16(kVK_DownArrow)),
            "Left":   (functionKeyString(NSLeftArrowFunctionKey), UInt16(kVK_LeftArrow)),
            "Right":  (functionKeyString(NSRightArrowFunctionKey), UInt16(kVK_RightArrow)),
            "F1":     (functionKeyString(NSF1FunctionKey), UInt16(kVK_F1)),
            "F2":     (functionKeyString(NSF2FunctionKey), UInt16(kVK_F2)),
            "F3":     (functionKeyString(NSF3FunctionKey), UInt16(kVK_F3)),
            "F4":     (functionKeyString(NSF4FunctionKey), UInt16(kVK_F4)),
            "F5":     (functionKeyString(NSF5FunctionKey), UInt16(kVK_F5)),
            "F6":     (functionKeyString(NSF6FunctionKey), UInt16(kVK_F6)),
            "F7":     (functionKeyString(NSF7FunctionKey), UInt16(kVK_F7)),
            "F8":     (functionKeyString(NSF8FunctionKey), UInt16(kVK_F8)),
            "F9":     (functionKeyString(NSF9FunctionKey), UInt16(kVK_F9)),
            "F10":    (functionKeyString(NSF10FunctionKey), UInt16(kVK_F10)),
            "F11":    (functionKeyString(NSF11FunctionKey), UInt16(kVK_F11)),
            "F12":    (functionKeyString(NSF12FunctionKey), UInt16(kVK_F12)),
...
fileprivate func currentInputSourceID() -> String {
    ...
    // BUG-1678: TISGetInputSourceProperty returns UnsafeMutableRawPointer?
    // Validate that the property exists before casting
    guard let rawProperty = TISGetInputSourceProperty(source, kTISPropertyInputSourceID) else {
        DLog("TISGetInputSourceProperty returned nil for kTISPropertyInputSourceID")
        return ""
    }
    // Cast to CFString - this is safe because kTISPropertyInputSourceID is documented
    // to return a CFString. The guard above ensures the pointer is not nil.
    let cfSourceID = unsafeBitCast(rawProperty, to: CFString.self)
