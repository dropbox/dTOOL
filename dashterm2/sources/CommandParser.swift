...
extension unichar {
    // BUG-1682: Use nil-coalescing instead of force unwrap for character.unicodeScalars.first
    // BUG-1569: Use truncatingIfNeeded to safely handle Unicode values > 65535 (emoji, etc.)
    init(_ character: Character) {
        // A Character always has at least one unicode scalar, but use safe unwrap pattern
        // Use truncatingIfNeeded to avoid crash on characters outside BMP (> U+FFFF)
        let scalarValue = character.unicodeScalars.first?.value ?? 0
        self = UInt16(truncatingIfNeeded: scalarValue)
    }
}
