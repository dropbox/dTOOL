...
- (instancetype)initWithPath:(nullable NSString *)path
    ...
    if (path) {
        ...
        NSDictionary *knobValues =
            [mergedConfiguration[iTermStatusBarComponentConfigurationKeyKnobValues] ?: @{} mutableCopy];
...
// BUG-7904: Abstract method - subclasses MUST override. doesNotRecognizeSelector raises exception.
// The return @"" is dead code but required to satisfy the compiler.
- (NSString *)statusBarComponentShortDescription {
    [self doesNotRecognizeSelector:_cmd];
    return @""; // Never reached
}

// BUG-7904: Abstract method - subclasses MUST override
...
- (NSArray<NSString *> *)variantsOf:(NSString *)string {
    ...
    NSMutableArray<NSString *> *result = [NSMutableArray arrayWithCapacity:16]; // String variant representations
    NSArray<NSString *> *parts = [string componentsSeparatedByString:@"."];
    if (parts.count > 1) {
        NSString *localname = parts.firstObject;
        if ([localname length] > 1) {
            for (NSInteger numberToOmit = 2; numberToOmit < parts.firstObject.numberOfComposedCharacters - 2;
                 numberToOmit++) {
...
- (nullable NSArray<NSString *> *)stringVariants {
    ...
    if (abbreviation.length && [[NSHost fullyQualifiedDomainName] isEqualToString:self.cached]) {
        return @[ abbreviation ];
    }
...
- (nullable NSString *)stringByCompressingString:(NSString *)source {
    ...
    NSUInteger index =
        [parts indexOfObjectWithOptions:NSEnumerationReverse
                            passingTest:^BOOL(NSString *_Nonnull part, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (nullable NSString *)stringByCompressingString:(NSString *)source {
    ...
    __block NSString *replacement = nil;
    NSUInteger index =
        [parts indexOfObjectPassingTest:^BOOL(NSString *_Nonnull part, NSUInteger idx, BOOL *_Nonnull stop) {
            replacement = [part firstComposedCharacter:nil];
...
- (BOOL)statusBarComponentIsEmpty {
    return [[self.fullString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]
               length] == 0;
}
...
- (void)openMenuWithView:(NSView *)view {
    ...
    for (iTermRecentDirectoryMO *directory in [[[iTermShellHistoryController sharedInstance]
             directoriesSortedByScoreOnHost:remoteHost] it_arrayByKeepingFirstN:10]) {
        NSString *title;
