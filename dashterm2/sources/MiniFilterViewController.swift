...
class MiniFilterViewController: NSViewController, NSTextFieldDelegate, iTermFilterViewController {
    ...
    required init?(coder: NSCoder) {
        // BUG-f814: Return nil instead of crashing for unused coder initializer
        DLog("MiniFilterViewController init(coder:) is not supported")
        return nil
    }

    deinit {
        // BUG-1831: Timer must be invalidated on dealloc to prevent leaks and dangling references
        timer?.invalidate()
        timer = nil
    }
