...
// Window pane IDs are typically small integers.
static const int kCachedTmuxPaneIdCount = 1024;
static NSNumber *sTmuxPaneIdCache[kCachedTmuxPaneIdCount];

NS_INLINE NSNumber *iTermTmuxPaneIdToNumber(int paneId) {
    if (paneId >= 0 && paneId < kCachedTmuxPaneIdCount) {
        return sTmuxPaneIdCache[paneId];
    }
    return @(paneId);
}

__attribute__((constructor)) static void iTermTmuxWindowOpenerInitializePaneIdCache(void) {
    for (int i = 0; i < kCachedTmuxPaneIdCount; i++) {
        sTmuxPaneIdCache[i] = @(i);
    }
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        histories_ = [[NSMutableDictionary alloc] initWithCapacity:8];    // Pane histories
        altHistories_ = [[NSMutableDictionary alloc] initWithCapacity:8]; // Pane alternate histories
        states_ = [[NSMutableDictionary alloc] initWithCapacity:8];       // Pane states
    }
    return self;
}
...
- (BOOL)openWindows:(BOOL)initial {
    ...
    NSMutableArray *cmdList = [NSMutableArray arrayWithCapacity:16]; // Typical pane count
...
- (void)unpauseWindowPanes:(NSArray<NSNumber *> *)windowPanes {
    ...
    NSMutableArray<NSDictionary *> *commands = [NSMutableArray arrayWithCapacity:windowPanes.count];
...
- (BOOL)updateLayoutInTab:(PTYTab *)tab {
    ...
    NSMutableArray *cmdList = [NSMutableArray arrayWithCapacity:8]; // Typical added panes
    for (NSNumber *addedPane in [parser windowPanesInParseTree:self.parseTree]) {
        if (![oldPanes containsObject:addedPane]) {
            [self appendRequestsForWindowPane:addedPane toArray:cmdList];
        }
    }
    if (cmdList.count) {
        tabToUpdate_ = tab;
    ...
    [tab setTmuxLayout:self.parseTree visibleLayout:self.visibleParseTree tmuxController:controller_ zoomed:_zoomed];
    return YES;
}
...
- (void)appendRequestsForWindowPane:(NSNumber *)wp toArray:(NSMutableArray *)cmdList {
...
- (NSDictionary *)dictForGetUserVars:(NSNumber *)wp {
    ++pendingRequests_;
    DLog(@"Increment pending requests to %d", pendingRequests_);
    NSString *command = [NSString stringWithFormat:@"show-options -v -q -p -t %%%d @uservars", [wp intValue]];
    return [gateway_ dictionaryForCommand:command
                           responseTarget:self
...
- (NSDictionary *)dictForDumpStateForWindowPane:(NSNumber *)wp {
    ...
    DLog(@"Increment pending requests to %d", pendingRequests_);
    NSString *command =
        [NSString stringWithFormat:@"list-panes -t \"%%%d\" -F \"%@\"", [wp intValue], [TmuxStateParser format]];
    return [gateway_ dictionaryForCommand:command
                           responseTarget:self
...
- (NSDictionary *)dictForRequestHistoryForWindowPane:(NSNumber *)wp alt:(BOOL)alternate {
    ...
    DLog(@"Increment pending requests to %d", pendingRequests_);
    NSString *maybeN = @"";
    if (self.minimumServerVersion &&
        [self.minimumServerVersion compare:[NSDecimalNumber decimalNumberWithString:@"3.1"]] != NSOrderedAscending) {
        maybeN = @"N";
    }
    NSString *command = [NSString stringWithFormat:@"capture-pane -peqJ%@ %@-t \"%%%d\" -S -%d", maybeN,
                                                   (alternate ? @"-a " : @""), [wp intValue], self.maxHistory];
    return [gateway_ dictionaryForCommand:command
                           responseTarget:self
                         responseSelector:@selector(dumpHistoryResponse:paneAndAlternate:)
                           responseObject:[NSArray arrayWithObjects:wp, [NSNumber numberWithBool:alternate], nil]
                                    flags:kTmuxGatewayCommandShouldTolerateErrors];
}
...
- (void)finishErroneously {
    if (self.target) {
        [self.target it_performNonObjectReturningSelector:self.selector withObject:self];
    }
    if (self.completion) {
...
- (void)dumpHistoryResponse:(NSString *)response paneAndAlternate:(NSArray *)info {
    if (!response) {
        [self didReceiveError];
        return;
    }
    // BUG-f1056: Validate info array has at least 2 elements before accessing indices
    if (info.count < 2) {
        [self didReceiveError];
        return;
    }
...
- (TmuxHistory *)historyLinesForWindowPane:(int)wp alternateScreen:(BOOL)altScreen {
    ...
    return dict[iTermTmuxPaneIdToNumber(wp)];
}
...
- (void)getPendingOutputResponse:(NSData *)response pane:(NSNumber *)wp {
    ...
    // Pending data is approximately same size as response
    NSMutableData *pending = [NSMutableData dataWithCapacity:response.length];
    for (int i = 0; i < response.length; i++) {
        char c = bytes[i];

        // TODO: Fix tmux and update this code.
        if (c == '\\' && response.length >= i + 4) {
...
- (NSDictionary *)stateForWindowPane:(int)wp {
    return states_[iTermTmuxPaneIdToNumber(wp)];
}
...
- (void)requestDidComplete {
    ...
    if (!term) {
        ...
        if (!term && !self.initial && self.anonymous &&
        ...
        if (!term) {
            ...
            term = [[iTermController sharedInstance]
                openTmuxIntegrationWindowUsingProfile:self.profile
    ...
    if (tabToUpdate_) {
        ...
        [tabToUpdate_ setTmuxLayout:parseTree visibleLayout:visibleParseTree tmuxController:controller_ zoomed:@NO];
    ...
    DLog(@"useOriginalWindow=%@ initialTabs=%@ initial=%@ windowPos=%@", @(useOriginalWindow), @(initialTabs),
         @(self.initial), windowPos);
    if (windowPos) {
