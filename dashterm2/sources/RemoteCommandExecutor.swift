...
class RemoteCommandExecutor {
    ...
    // BUG-1937: Thread-safe storage with concurrent queue for reads/barrier writes
    private var storage = [Key: Permission]()
    private let storageQueue = DispatchQueue(label: "com.dashterm2.RemoteCommandExecutor.storage", attributes: .concurrent)
    ...
    func permission(chatID: String,
        ...
        // BUG-1937: Thread-safe read with sync
        return storageQueue.sync {
            storage[Key(chatID: chatID,
                        guid: guid,
    ...
    func setPermission(chatID: String,
        ...
        // BUG-1937: Thread-safe write with barrier
    ...
    func permissionsDict(encoded encodedPermissions: String) -> [Key: Permission]? {
        // BUG-1626: Use guard instead of force unwrap for data encoding
        guard let data = encodedPermissions.data(using: .utf8) else {
            return nil
        }
        return try? JSONDecoder().decode([Key: Permission].self, from: data)
    }
