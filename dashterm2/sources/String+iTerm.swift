...
extension String {
    ...
    var base64Decoded: String? {
        guard let data = base64DecodedData else {
            return nil
        }
...
extension String {
    ...
    public func nsrangeOfUTF16Character(from aSet: CharacterSet,
        ...
        // BUG-1775: Use guard let instead of force unwrap for samePosition
    ...
    // BUG-f539: Returns nil for invalid UTF-16 offsets instead of crashing
...
extension String {
    var mayContainRTL: Bool {
        // BUG-1696: Use nil-coalescing instead of force unwrap for rtlSmellingCodePoints()
        let rtlSmellingCodePoints = iTermPreferences.bool(forKey: kPreferenceKeyBidi) ? (NSCharacterSet.rtlSmellingCodePoints() ?? CharacterSet()) : CharacterSet()
...
extension String {
    ...
    subscript(utf16 range: NSRange) -> String? {
        ...
        // BUG-1594: Use guard to safely convert indices instead of force unwrap
        guard let fromUtf16Index = utf16View.index(
            utf16View.startIndex,
            offsetBy: range.location,
            limitedBy: utf16View.endIndex) else {
            return nil
        }
        guard let toUtf16Index = utf16View.index(
            fromUtf16Index,
            offsetBy: range.length,
            limitedBy: utf16View.endIndex) else {
            return nil
        ...
        guard let startIndex = String.Index(fromUtf16Index, within: self),
              let endIndex = String.Index(toUtf16Index, within: self) else {
            return nil
        }
    ...
    subscript(utf16 range: PartialRangeFrom<Int>) -> String {
        ...
        // BUG-1594: Use nil coalescing instead of force unwrap
