...
+ (void)writeSetupCfgToFile:(NSString *)file
    ...
    // BUG-f1264: Replace assert() with guard - nil pythonVersion should return early, not crash
    if (pythonVersion == nil) {
        DLog(@"WARNING: BUG-f1264 writeSetupCfgToFile called with nil pythonVersion");
        return;
    }
    ...
    if (ensureDashTerm2Present && ![dependencies containsPythonPackage:@"iterm2"]) {
        dependencies = [@[ @"iterm2" ] arrayByAddingObjectsFromArray:dependencies];
    }
    NSString *contents = [NSString
        stringWithFormat:@"# WARNING: This file was generated automatically by DashTerm2. Do not edit it manually.\n"
                         @"[metadata]\n"
                         @"name=%@\n"
                         @"version=1.0\n"
                         @"\n"
...
static int iTermSetupCfgParserIniHandler(void *user, const char *section, const char *name, const char *value) {
...
- (instancetype)initWithPath:(NSString *)path {
    ...
    if (self) {
        ...
        _installRequiresParts = [[NSMutableArray alloc] initWithCapacity:8];
...
- (void)handleSection:(NSString *)section name:(NSString *)name value:(NSString *)value {
...
- (NSString *)pythonPackage {
    ...
    NSString *ws = @"(?: *)";
    NSArray<NSString *> *parts = @[ @"^", packageName, @"(?:", ws, operator, ws, version, @")?", ws, @"$" ];
...
- (NSArray<NSString *> *)arrayBySettingPythonDependency:(NSString *)dependency {
    ...
    const NSInteger index =
        [self indexOfObjectPassingTest:^BOOL(id _Nonnull element, NSUInteger idx, BOOL *_Nonnull stop) {
