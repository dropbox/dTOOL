...
- (instancetype)initWithParameters:(NSArray *)parameters {
    self = [super init];
    if (self) {
        // Initial capacity: escape sequence header + parameters + typical sixel data
        _accumulator = [[NSMutableData alloc] initWithCapacity:256];
        char escp[2] = "\x1bP";
...
- (VT100DCSParserHookResult)handleInput:(iTermParserContext *)context
    ...
    if (_esc) {
        return [self handleInputAfterESC:context token:result] ? VT100DCSParserHookResultUnhook
    ...
    while (iTermParserCanAdvance(context)) {
        ...
        switch (iTermParserPeek(context)) {
            case VT100CC_C1_ST:
                ...
                break;
            case VT100CC_ESC:
                return [self handleInputBeginningWithEsc:context token:result] ? VT100DCSParserHookResultUnhook
                                                                               : VT100DCSParserHookResultCanReadAgain;
        }
        ...
        if (n >= 0) {
            ...
            [self handleInputOfLength:n context:context token:result];
            continue;
        }

        // There is no forthcoming ESC or ST. Handle all the input.
        [self handleInputOfLength:iTermParserLength(context) context:context token:result];
    }
    return VT100DCSParserHookResultCanReadAgain;
}

- (BOOL)handleInputBeginningWithEsc:(iTermParserContext *)context token:(VT100Token *)result {
    iTermParserConsume(context);
    _esc = YES;
...
- (BOOL)handleInputAfterESC:(iTermParserContext *)context token:(VT100Token *)result {
    unsigned char c;
...
- (void)handleInputOfLength:(int)length context:(iTermParserContext *)context token:(VT100Token *)result {
    [_accumulator appendBytes:iTermParserPeekRawBytes(context, length) length:length];
