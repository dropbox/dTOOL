...
struct LRUDictionary<Key: Hashable, Value> {
    ...
    mutating func insert(key: Key, value: Value, cost: Int) -> [(Key, Value)] {
        ...
        for keyToRemove in keysToRemove {
            if let value = dict[keyToRemove] {
                evictions.append((keyToRemove, value))
...
class LRUEvictionPolicy<Element: Hashable> {
    ...
    private func evictIfNeeded() -> Set<Element> {
        ...
        // BUG-1575: Use guard to safely access .first instead of force unwrap
        while totalCost > maximumSize && nodesByKey.count > 1 {
            guard let firstItem = itemsByUse.first else {
                break
            }
            DLog("Evict \(firstItem.value.element) with cost \(firstItem.value.cost)")
            evictions.insert(firstItem.value.element)
            delete(firstItem.value.element)
        }
