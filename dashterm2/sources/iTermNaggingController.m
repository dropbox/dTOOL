...
- (void)offerToFixSessionWithBrokenArrangementProfileIn:(NSString *)arrangementName
                                                   guid:(NSString *)guid {
    NSString *notice = @"This arrangement's profile is missing. This could be due to a bug in DashTerm2 version 3.5.7, which caused profiles to be corrupted in saved arrangements.";
...
- (void)didRestoreOrphan {
    if ([iTermAdvancedSettingsModel suppressOrphanSessionRestoredWarning]) {
        return;
    }
    [self.delegate naggingControllerShowMessage:@"This already-running session was restored but its contents were not saved."
        ...
        if (selection == 0) {
            ...
            NSURL *whyUrl = [NSURL URLWithString:@"https://dashterm.com/why_no_content.html"];
...
- (void)showTmuxSupplementaryPlaneBugHelpPage {
    // BUG-1: Documentation URL updated to dashterm.com
    NSURL *whyUrl = [NSURL URLWithString:@"https://dashterm.com/tmux22bug.html"];
    [[NSWorkspace sharedWorkspace] it_openURL:whyUrl
                                       target:nil
                                        style:iTermOpenStyleTab
                                       window:self.delegate.naggingControllerWindow];
...
- (void)offerToTurnOffBracketedPasteOnHostChange {
    ...
    [self.delegate naggingControllerShowMessage:title
        ...
        switch (selection) {
            ...
            case 3: // Help
                // BUG-1: Documentation URL updated to dashterm.com
                [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/paste_bracketing"]
                                                   target:nil
                                                    style:iTermOpenStyleTab
                                                   window:self.delegate.naggingControllerWindow];
                break;
...
- (void)offerToDisableTriggersInInteractiveAppsWithStats:(NSString *)stats {
    ...
    [self.delegate naggingControllerShowMessage:title
        ...
        switch (selection) {
            ...
            case 4: // Help
                // BUG-1: Documentation URL updated to dashterm.com
                [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/slow_triggers"]
                                                   target:nil
                                                    style:iTermOpenStyleTab
                                                   window:self.delegate.naggingControllerWindow];
                break;
...
- (void)showJSONPromotion {
    [_delegate naggingControllerShowMessage:@"That's a gnarly JSON blob you've got there! DashTerm2 can replace this hard-to-read selection with a pretty-printed value."
                                 isQuestion:NO
                                  important:NO
