...
class iTermModernSavePanel: NSObject {
    ...
    // nonisolated(unsafe) because all access happens on MainActor
    nonisolated(unsafe) static var panels = [iTermModernSavePanel]()
...
extension iTermModernSavePanel {
    ...
    func makePanel() -> SSHFilePanel {
        ...
        sshFilePanel.systemPanelCallback = { [weak self, weak sshFilePanel] in
            ...
            // BUG-1609: Use if-let to safely unwrap window instead of force unwrap
            if let window = sshFilePanel.window, let sheetParent = window.sheetParent {
