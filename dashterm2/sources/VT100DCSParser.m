...
- (VT100StateMachine *)stateMachine {
    ...
    if (!_stateMachineWithout8BitControlCharacters) {
        _stateMachineWithout8BitControlCharacters = [self newStateMachineWith8BitControlCharacters:NO];
...
- (VT100StateMachine *)newStateMachineWith8BitControlCharacters:(BOOL)support8BitControlCharacters {
    ...
    VT100State *groundState = [VT100State stateWithName:@"ground" identifier:@(kVT100DCSStateGround)];
    // Ground can go to escape state when ESC is received.
    VT100State *escapeState = [VT100State stateWithName:@"escape" identifier:@(kVT100DCSStateEscape)];

    // Upon ESC P, enter dcsEntryState.
    VT100State *dcsEntryState = [VT100State stateWithName:@"dcs entry" identifier:@(kVT100DCSStateEntry)];
    ...
    VT100State *dcsParamState = [VT100State stateWithName:@"dcs param" identifier:@(kVT100DCSStateParam)];

    // An illegal character was received; swallow up bytes til ST.
    VT100State *dcsIgnoreState = [VT100State stateWithName:@"dcs ignore" identifier:@(kVT100DCSStateIgnore)];

    // An ESC was received which might lead to termination or something else.
    VT100State *dcsEscapeState = [VT100State stateWithName:@"dcs escape" identifier:@(kVT100DCSStateDCSEscape)];
    ...
    for (VT100State *state in stateMachine.states) {
        ...
        [state addStateTransitionForCharacter:VT100CC_ESC to:dcsEscapeState withAction:nil];
    ...
    [groundState addStateTransitionForCharacter:VT100CC_ESC to:escapeState withAction:nil];
    if (support8BitControlCharacters) {
        [groundState addStateTransitionForCharacter:VT100CC_C1_DCS to:dcsEntryState withAction:nil];
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////
    // Add transitions from escape state.
    [escapeState
        addStateTransitionForCharacterRange:MakeCharacterRange(0, 255)
                                         to:groundState
                                 withAction:^(unsigned char character) {
                                     // BUG-f655: Replace assert(false) with DLog - unexpected character in escape state
                                     // should not crash We shouldn't have been called unless there was a DCS.
                                     DLog(@"BUG-f655: VT100DCSParser escape state received unexpected character 0x%02x",
                                          character);
                                 }];
    [escapeState addStateTransitionForCharacter:'P' to:dcsEntryState withAction:nil];
    ...
    [dcsEntryState addStateTransitionForCharacter:':' to:dcsIgnoreState withAction:nil];
    ...
    [dcsParamState addStateTransitionForCharacter:':' to:dcsIgnoreState withAction:nil];
    [dcsParamState addStateTransitionForCharacterRange:MakeCharacterRange('<', '?') to:dcsIgnoreState withAction:nil];
    ...
    [dcsPassthroughState addStateTransitionForCharacterRange:MakeCharacterRange(VT100CC_NULL, VT100CC_ETB)
                                                          to:dcsPassthroughState
                                                  withAction:^(unsigned char c) {
                                                      [weakSelf put:c];
                                                  }];
    ...
    [dcsPassthroughState addStateTransitionForCharacterRange:MakeCharacterRange(VT100CC_FS, VT100CC_US)
                                                          to:dcsPassthroughState
                                                  withAction:^(unsigned char c) {
                                                      [weakSelf put:c];
                                                  }];
...
- (BOOL)dataLooksLikeBinaryGarbage {
    ...
    if ([_data hasPrefix:@"tmux;\e]1337;File="]) {
        // Allow file downloads to get really big, but cap at 1 GB to prevent memory exhaustion.
        // NSIntegerMax would effectively disable the limit entirely, which is dangerous.
        maxLength = 1024 * 1024 * 1024; // 1 GB
    }
    return (_data.length > maxLength || [_data rangeOfCharacterFromSet:garbageCharacterSet
                                                               options:0
...
- (void)unhook {
    ...
    _hook = nil;
    _uniqueID = nil;
    // BUG-10290: Reset _hookFinished to NO so the next hook can complete properly.
    // Without this, a subsequent hook may think it's already finished when it's just starting.
    _hookFinished = NO;
...
- (int)compactSequence {
    ...
    if (_privateMarkers.length) {
      private
        = [_privateMarkers characterAtIndex:0];
    }
...
- (void)execute {
    ...
    if (_executed) {
        DLog(@"Warning: token already executed %@", token);
        return;
    }
    _executed = YES;
    ...
    switch ([self compactSequence]) {
        ...
        case MAKE_COMPACT_SEQUENCE(0, '$', 't'): // ESC P Param $ t
            ...
            if (token->type != VT100_SKIP && _data.length >= 1) {
                token.string = [_data substringFromIndex:1];
            } else if (token->type != VT100_SKIP) {
                token->type = VT100_SKIP;
            }
            break;
