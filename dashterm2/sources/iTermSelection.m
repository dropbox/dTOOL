...
+ (instancetype)subSelectionWithDictionary:(NSDictionary *)dict
    ...
    return [self subSelectionWithAbsRange:range mode:[dict[kiTermSubSelectionMode] intValue] width:width];
}

- (NSDictionary *)dictionaryValueWithYOffset:(int)yOffset totalScrollbackOverflow:(long long)totalScrollbackOverflow {
    VT100GridAbsWindowedRange absrange = _absRange;
    absrange.coordRange.start.y += yOffset;
    absrange.coordRange.end.y += yOffset;

    const VT100GridWindowedRange range = VT100GridWindowedRangeFromAbsWindowedRange(absrange, totalScrollbackOverflow);
    return @{
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p range=%@ mode=%@>", [self class], self,
...
- (NSArray *)nonwindowedComponentsWithWidth:(int)width {
    if (self.selectionMode == kiTermSelectionModeBox || self.absRange.columnWindow.length <= 0) {
        return @[ self ];
    }
    // Pre-allocate for typical number of lines in windowed range (usually 1-4)
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:4];
    [[self class] enumerateAbsoluteRangesInAbsWindowedRange:self.absRange
                                                      block:^(VT100GridAbsCoordRange subrange) {
                                                          iTermSubSelection *sub = [iTermSubSelection
                                                              subSelectionWithAbsRange:VT100GridAbsWindowedRangeMake(
                                                                                           subrange, 0, 0)
                                                                                  mode:_selectionMode
                                                                                 width:width];
                                                          sub.connected = YES;
...
+ (void)enumerateAbsoluteRangesInAbsWindowedRange:(VT100GridAbsWindowedRange)absWindowedRange
    ...
    if (absWindowedRange.columnWindow.length) {
        ...
        block(VT100GridAbsCoordRangeMake(startX, absWindowedRange.coordRange.end.y,
...
- (BOOL)isEqual:(id)object {
    ...
    return (VT100GridAbsWindowsRangeEqualsAbsWindowedRange(self.absRange, other.absRange) &&
            self.selectionMode == other.selectionMode && self.connected == other.connected);
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Typical selections have 1-4 sub-selections
        _subSelections = [[NSMutableArray alloc] initWithCapacity:4];
    }
    return self;
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p liveRange=%@ initialRange=%@ live=%d extend=%d "
                                      ...
                                      VT100GridAbsCoordRangeDescription(_initialAbsRange.coordRange), _live, _extend,
                                      _resumable, [[self class] nameForMode:_selectionMode], _subSelections, _delegate];
}

- (void)flip {
    _absRange.coordRange = VT100GridAbsCoordRangeMake(_absRange.coordRange.end.x, _absRange.coordRange.end.y,
                                                      _absRange.coordRange.start.x, _absRange.coordRange.start.y);
}
...
- (void)beginExtendingSelectionAt:(VT100GridAbsCoord)absCoord {
    ...
    if ([_subSelections count] == 0) {
        [self beginSelectionAtAbsCoord:absCoord mode:_selectionMode resume:NO append:NO];
        return;
    } else {
    ...
    // BUG-1210: Fixed dead code - fallback to word selection when no range found
    ...
    if (absRange.coordRange.start.x != -1) {
        if ([self absCoord:absCoord isInAbsRange:_absRange]) {
            ...
            long long distanceToStart =
                VT100GridAbsCoordDistance(_absRange.coordRange.start, absRange.coordRange.start, width);
            long long distanceToEnd =
                VT100GridAbsCoordDistance(_absRange.coordRange.end, absRange.coordRange.end, width);
            if (distanceToEnd < distanceToStart) {
                ...
                const VT100GridAbsCoord anchor = [_delegate selectionPredecessorOfAbsCoord:_absRange.coordRange.start];
...
- (VT100GridAbsWindowedRange)absRangeForCurrentModeAtAbsCoord:(VT100GridAbsCoord)rawAbsCoord
    ...
    if (_absRange.columnWindow.length > 0) {
        absCoord.x = MAX(_absRange.columnWindow.location,
                         MIN(_absRange.columnWindow.location + _absRange.columnWindow.length - 1, absCoord.x));
    }
    ...
    switch (_selectionMode) {
        ...
        case kiTermSelectionModeCharacter:
            if (_absRange.columnWindow.length > 0) {
                absCoord.x = MAX(_absRange.columnWindow.location,
                                 MIN(_absRange.columnWindow.location + _absRange.columnWindow.length, rawAbsCoord.x));
            }
        case kiTermSelectionModeBox:
...
- (void)beginSelectionAtAbsCoord:(VT100GridAbsCoord)absCoord
    ...
    _selectionMode = mode;
    _absRange = [self absRangeForCurrentModeAtAbsCoord:absCoord includeParentheticals:YES needAccurateWindow:YES];
...
- (void)endLiveSelectionWithSideEffects:(BOOL)sideEffects {
    ...
    if (_selectionMode == kiTermSelectionModeBox) {
        ...
        _absRange = VT100GridAbsWindowedRangeMake(VT100GridAbsCoordRangeMake(left, top, right, bottom), 0, 0);
        for (long long i = top; i <= bottom; i++) {
            ...
            iTermSubSelection *sub = [iTermSubSelection subSelectionWithAbsRange:theRange
...
- (BOOL)haveLiveSelection {
    return (_live && _absRange.coordRange.start.x != -1 &&
...
- (BOOL)moveSelectionEndpointTo:(VT100GridAbsCoord)coord {
    ...
    if (startLiveSelection) {
        [self beginSelectionAtAbsCoord:coord mode:self.selectionMode resume:NO append:NO];
    }
    switch (_selectionMode) {
        case kiTermSelectionModeBox:
        case kiTermSelectionModeCharacter:
...
- (void)moveSelectionEndpointToAbsRange:(VT100GridAbsWindowedRange)range {
    DLog(@"move selection endpoint to range=%@ selection=%@ initial=%@", VT100GridAbsWindowedRangeDescription(range),
         VT100GridAbsWindowedRangeDescription(_absRange), VT100GridAbsWindowedRangeDescription(_initialAbsRange));
...
- (void)scrollbackOverflowDidChange {
    ...
    if ([self haveLiveSelection]) {
        ...
        // BUG-10309: After clamping start, validate the range is still valid.
        // Clear if end is before overflow, OR if after clamping start is now after end.
        if (_absRange.coordRange.end.y < totalScrollbackOverflow ||
            VT100GridAbsCoordOrder(_absRange.coordRange.start, _absRange.coordRange.end) == NSOrderedDescending) {
            [self clearSelection];
        }
    }

    // Pre-allocate for reasonable size (typically 0-2 subs to remove)
    NSMutableArray<iTermSubSelection *> *subsToRemove = [NSMutableArray arrayWithCapacity:2];
    for (iTermSubSelection *sub in _subSelections) {
        ...
        sub.absRange = range;
        // BUG-10309: Also check if range became inverted after clamping start
        if (range.coordRange.end.y < totalScrollbackOverflow ||
            VT100GridAbsCoordOrder(range.coordRange.start, range.coordRange.end) == NSOrderedDescending) {
...
- (int)width {
    // BUG-1583: Guard against zero width to prevent division by zero
    const int w = [_delegate selectionViewportWidth];
    return w > 0 ? w : 1;
}

- (long long)length {
    // BUG-1206: Use long long accumulator to prevent integer overflow on large selections
...
- (VT100GridAbsWindowedRange)unflippedAbsRangeForAbsRange:(VT100GridAbsWindowedRange)absRange
    ...
    if (mode == kiTermSelectionModeBox) {
        ...
        absRange.coordRange = VT100GridAbsCoordRangeMake(MIN(absRange.coordRange.start.x, absRange.coordRange.end.x),
                                                         MIN(absRange.coordRange.start.y, absRange.coordRange.end.y),
                                                         MAX(absRange.coordRange.start.x, absRange.coordRange.end.x),
                                                         MAX(absRange.coordRange.start.y, absRange.coordRange.end.y));
        ...
        // reading order.
        absRange.coordRange = VT100GridAbsCoordRangeMake(absRange.coordRange.end.x, absRange.coordRange.end.y,
                                                         absRange.coordRange.start.x, absRange.coordRange.start.y);
    }
    return absRange;
}
...
- (VT100GridAbsWindowedRange)absRangeByExtendingRangePastNulls:(VT100GridAbsWindowedRange)range {
    ...
    VT100GridAbsWindowedRange unflippedRange = [self unflippedAbsRangeForAbsRange:range mode:_selectionMode];
    VT100GridRange nulls = [_delegate selectionRangeOfTerminalNullsOnAbsoluteLine:unflippedRange.coordRange.start.y];
...
- (NSArray<iTermSubSelection *> *)allSubSelections {
    if ([self haveLiveSelection]) {
        // Pre-allocate for sub-selections count + 1 for live selection
        NSMutableArray *subs = [NSMutableArray arrayWithCapacity:_subSelections.count + 1];
...
- (VT100GridAbsWindowedRange)lastAbsRange {
    VT100GridAbsWindowedRange best = VT100GridAbsWindowedRangeMake(VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
    for (iTermSubSelection *sub in [self allSubSelections]) {
        VT100GridAbsWindowedRange absRange = sub.absRange;
        if (best.coordRange.start.x < 0 || [self absCoord:absRange.coordRange.end
                                               isAfterAbsCoord:best.coordRange.end]) {
            best = absRange;
        }
    }
    return best;
...
- (VT100GridAbsWindowedRange)firstAbsRange {
    // BUG-1205: Use isBeforeAbsCoord to find the first (earliest) range in reading order
    VT100GridAbsWindowedRange best = VT100GridAbsWindowedRangeMake(VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
    for (iTermSubSelection *sub in [self allSubSelections]) {
        const VT100GridAbsWindowedRange range = sub.absRange;
        if (best.coordRange.start.x < 0 || [self absCoord:range.coordRange.start
                                               isBeforeAbsCoord:best.coordRange.start]) {
            best = range;
        }
    }
    return best;
...
- (void)setLastAbsRange:(VT100GridAbsWindowedRange)lastRange mode:(iTermSelectionMode)mode {
    ...
    if (_live) {
        ...
        [_subSelections removeLastObject];
        [_subSelections addObject:[iTermSubSelection subSelectionWithAbsRange:lastRange mode:mode width:self.width]];
    }
    [_delegate selectionDidChange:[[self retain] autorelease]];
}
...
- (void)removeWindowsWithWidth:(int)width {
    ...
    // Pre-allocate for expected size (usually same or slightly larger)
    NSMutableArray<iTermSubSelection *> *newSubs = [NSMutableArray arrayWithCapacity:_subSelections.count];
    for (iTermSubSelection *sub in _subSelections) {
        if (sub.absRange.columnWindow.location == 0 && sub.absRange.columnWindow.length == width) {
...
- (NSRange)rangeOfIndexesInAbsRange:(VT100GridAbsWindowedRange)range
    ...
    if (mode == kiTermSelectionModeBox) {
        if (range.coordRange.start.y <= line && range.coordRange.end.y >= line) {
            return iTermMakeRange(range.coordRange.start.x, range.coordRange.end.x - range.coordRange.start.x);
        } else {
    ...
    if (range.coordRange.start.y < line && range.coordRange.end.y > line) {
        if (range.columnWindow.length) {
            return iTermMakeRange(range.columnWindow.location, range.columnWindow.length);
        } else {
    ...
    if (range.coordRange.start.y == line) {
        if (range.coordRange.end.y == line) {
            ...
            if (range.columnWindow.length) {
                ...
                return iTermMakeRange(range.coordRange.start.x, [self width] - range.coordRange.start.x);
            }
        }
    }
    if (range.coordRange.end.y == line) {
...
- (NSIndexSet *_Nullable)selectedIndexesOnAbsoluteLine:(long long)line {
    ...
    if (!_live && numberOfSubSelections == 1) {
        ...
        NSRange theRange = [self rangeOfIndexesInAbsRange:sub.absRange onAbsoluteLine:line mode:sub.selectionMode];
    ...
    for (iTermSubSelection *sub in [self allSubSelections]) {
        VT100GridAbsWindowedRange range = sub.absRange;
        NSRange theRange = [self rangeOfIndexesInAbsRange:range onAbsoluteLine:line mode:sub.selectionMode];
        ...
        [indexes enumerateRangesInRange:theRange
                                options:0
                             usingBlock:^(NSRange innerRange, BOOL *stop) {
...
- (NSIndexSet *_Nullable)selectedIndexesIncludingTabFillersInAbsoluteLine:(long long)y {
    NSIndexSet *basicIndexes = [self selectedIndexesOnAbsoluteLine:y];
    if (basicIndexes.count == 0) {
        return nil;
    }
    ...
    NSIndexSet *tabs = [_delegate selectionIndexesOnAbsoluteLine:y containingCharacter:'\t' inRange:range];
    NSIndexSet *tabFillers = [_delegate selectionIndexesOnAbsoluteLine:y containingCharacter:TAB_FILLER inRange:range];
...
- (void)enumerateSelectedAbsoluteRanges:(void (^NS_NOESCAPE)(VT100GridAbsWindowedRange, BOOL *, BOOL))block {
    ...
    // BUG-f1382: Replace assert with _Static_assert - compile-time check for 64-bit platform
    // NOTE: This assumes a 64-bit platform, otherwise the NSUInteger type used by index set would
    // overflow too quickly.
    _Static_assert(sizeof(NSUInteger) >= 8, "Requires 64-bit platform (NSUInteger must be at least 8 bytes)");
    ...
    for (iTermSubSelection *outer in [self allSubSelections]) {
        ...
        [iTermSubSelection
            enumerateAbsoluteRangesInAbsWindowedRange:outer.absRange
                                                block:^(VT100GridAbsCoordRange outerRange) {
                                                    theRange =
                                                        NSMakeRange(outerRange.start.x + outerRange.start.y * width,
                                                                    VT100GridAbsCoordRangeLength(outerRange, width));

                                                    NSMutableIndexSet *indexesToAdd =
                                                        [NSMutableIndexSet indexSetWithIndexesInRange:theRange];
                                                    NSMutableIndexSet *indexesToRemove = [NSMutableIndexSet indexSet];
                                                    [indexes enumerateRangesInRange:theRange
                                                                            options:0
                                                                         usingBlock:^(NSRange range, BOOL *stop) {
                                                                             // range exists in both indexes and
                                                                             // theRange
                                                    ...
                                                    if (outer.absRange.columnWindow.length &&
                                                        !VT100GridAbsCoordEquals(outerRange.end,
                                                                                 outer.absRange.coordRange.end) &&
                                                        theRange.length > 0) {
    ...
    // and then sort it.
    // Pre-allocate for typical range count (usually a few ranges)
    NSMutableArray<NSValue *> *allRanges = [NSMutableArray arrayWithCapacity:8];
    [indexes enumerateRangesUsingBlock:^(NSRange range, BOOL *stop) {
        VT100GridAbsCoordRange coordRange = VT100GridAbsCoordRangeMake(range.location % width, range.location / width,
                                                                       (range.location + range.length) % width,
                                                                       (range.location + range.length) / width);
        [allRanges addObject:[NSValue valueWithGridAbsCoordRange:coordRange]];
    }];

    NSArray<NSValue *> *sortedRanges = [allRanges sortedArrayUsingSelector:@selector(compareGridAbsCoordRangeStart:)];
    for (NSValue *value in sortedRanges) {
        BOOL stop = NO;
        const VT100GridAbsCoordRange theRange = [value gridAbsCoordRangeValue];
        NSUInteger endIndex =
            (theRange.start.x + theRange.start.y * width + VT100GridAbsCoordRangeLength(theRange, width));
        block(VT100GridAbsWindowedRangeMake(theRange, 0, 0), &stop,
...
- (NSDictionary *)dictionaryValueWithYOffset:(int)yOffset totalScrollbackOverflow:(long long)totalScrollbackOverflow {
    ...
    subs = [subs mapWithBlock:^id(id anObject) {
        ...
        return [sub dictionaryValueWithYOffset:yOffset totalScrollbackOverflow:totalScrollbackOverflow];
    }];
...
- (void)setFromDictionaryValue:(NSDictionary *)dict
    ...
    // Pre-allocate for expected number of sub-selections
    NSMutableArray<iTermSubSelection *> *subSelectionsToAdd = [NSMutableArray arrayWithCapacity:subs.count];
