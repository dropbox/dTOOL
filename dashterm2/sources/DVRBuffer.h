...
typedef enum {
    ...
    // Compressed key frame using Apple's Compression framework with LZ4 algorithm.
    // Added in DashTerm2 iteration 127 for memory optimization.
    // Format: [uint32_t uncompressed_size][compressed_data...]
    DVRFrameTypeCompressedKeyFrame
} DVRFrameType;
...
// RC-004: Generation counter incremented on structural changes (allocate/deallocate).
// Readers should capture this before reading and verify it hasn't changed after.
@property (nonatomic, readonly) NSUInteger structuralGeneration;
