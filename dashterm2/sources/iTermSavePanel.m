...
+ (NSString *)nameForFileType:(NSString *)extension {
    ...
    if (range.location != NSNotFound) {
        return [capitalized stringByReplacingCharactersInRange:range withString:extension.uppercaseString];
    }
    return capitalized;
}

+ (iTermSavePanelFileFormatAccessory *)
    newFileFormatAccessoryViewControllerFileWithFileTypes:(NSArray<NSString *> *)fileTypes
                                                  options:(iTermSavePanelOptions)options {
    iTermSavePanelFileFormatAccessory *accessory =
        [[iTermSavePanelFileFormatAccessory alloc] initWithNibName:@"iTermSavePanelFileFormatAccessory"
    ...
    for (NSString *fileType in fileTypes) {
        ...
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:name
                                                      action:@selector(popupButtonDidChange:)
                                               keyEquivalent:@""];
...
+ (iTermModernSavePanel *)newSavePanelWithOptions:(iTermSavePanelOptions)options
    ...
    if (options & (kSavePanelOptionFileFormatAccessory | kSavePanelOptionIncludeTimestampsAccessory)) {
        ...
        [button selectItemWithTag:[[NSUserDefaults standardUserDefaults]
                                      integerForKey:iTermSavePanelLoggingStyleUserDefaultsKey]];
        [button sizeToFit];
...
- (void)presentSavePanel:(iTermModernSavePanel *)savePanel
                 options:(NSInteger)options
                  window:(NSWindow *)window
              completion:(void (^)(iTermModernSavePanel *panel, iTermSavePanel *savePanel))completion {
    [savePanel beginWithFallbackWindow:window
                               handler:^(NSModalResponse response, iTermSavePanelItem *item) {
                                   [self savePanelDidCompleteWithResponse:response
                                                                  options:options
                                                                savePanel:savePanel
                                                               completion:^(iTermModernSavePanel *panel,
                                                                            iTermSavePanel *savePanel, BOOL retry) {
                                                                   if (retry) {
                                                                       [self presentSavePanel:panel
                                                                                      options:options
                                                                                       window:window
...
- (void)savePanelDidCompleteWithResponse:(NSModalResponse)response
                              ...
                              completion:
                                  (void (^)(iTermModernSavePanel *panel, iTermSavePanel *savePanel, BOOL))completion {
...
- (iTermModernSavePanel *)accept:(iTermModernSavePanel *)savePanel {
    if (savePanel.item) {
        ...
        NSDictionary *settings = @{kInitialDirectoryKey : [savePanel.item.filename stringByDeletingLastPathComponent]};
...
- (void)didCheckForExistingFile:(BOOL)exists withCompletion:(void (^)(BOOL retry))completion {
    ...
    } else if (self.item.host.isLocalhost &&
               [directory isEqualToString:[[NSFileManager defaultManager] desktopDirectory]]) {
        location = @" on the Desktop";
    }

    NSString *heading = [NSString stringWithFormat:@"“%@” already exists. Do you want to replace it or append to it?",
    ...
    switch (selection) {
        ...
        default:
            // BUG-434: Replace assert(false) with DLog - unknown warning selection should not crash
            DLog(@"Unknown iTermWarningSelection %d in replaceOrAppendFromSelection:completion:. Defaulting to cancel.",
                 (int)selection);
            completion(YES); // Treat unknown selection as cancel to avoid data loss
            return;
    }
    completion(NO);
}
...
- (void)panel:(iTermModernSavePanel *_Nonnull)sender
    didChangeToDirectory:(iTermSavePanelItem *_Nullable)didChangeToDirectory {
}
...
- (BOOL)panel:(iTermModernSavePanel *)sender validate:(iTermSavePanelItem *)item error:(NSError **)error {
    ...
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            [NSString stringWithFormat:@"You can choose to use both, so that your file name ends in “.%@.%@”.",
                                       proposedExtension, _requiredExtension]
                     actions:@[ [NSString stringWithFormat:@"Use .%@", _requiredExtension], @"Cancel", @"Use both" ]
                   accessory:nil
                  identifier:nil
                 silenceable:kiTermWarningTypePersistent
                     heading:[NSString stringWithFormat:@"You cannot save this document with extension “.%@” at the "
                                                        @"end of the name. The required extension is “.%@”.",
                                                        proposedExtension, _requiredExtension]
                      window:nil];
    switch (selection) {
        case kiTermWarningSelection0:
...
- (BOOL)timestamps {
    iTermSavePanelFileFormatAccessory *accessoryViewController =
        [iTermSavePanelFileFormatAccessory castFrom:self.accessoryViewController];
