...
class AppSignatureValidator: NSObject {
    ...
    static func currentAppTeamID() -> String? {
        ...
        // BUG-1708: Use as? instead of as! for debug print
        DLog("Signing info: \(info as? [String:Any] ?? [:])")
    ...
    static func warn(reason: String) {
        ...
        let message = if team == nil {
            "A required file appears to be missing or corrupted and DashTerm2's code signature could not be verified.\n\nYou should download a fresh copy of the app and reinstall it."
        } else if team == "H7V7XYVQ7D" {
            "A required file appears to be missing or corrupted and DashTerm2's code signature did not match that of the official distribution.\n\nYou should download a fresh copy of the app and reinstall it."
        } else {
            "A required file appears to be missing or corrupted, yet against all odds the code signature for DashTerm2 is valid. Please file a bug at https://github.com/ayates_dbx/dashterm2/issues"
        }
        let alert = NSAlert()
