...
- (instancetype)init {
    self = [super init];
    if (self) {
        // Phase 1 optimization: Pre-size for typical callback count
        _systemCallbacks = [[NSMutableDictionary alloc] initWithCapacity:16];
        dispatch_queue_t queue =
            dispatch_queue_create("com.dashterm.dashterm2.restorable-state", DISPATCH_QUEUE_SERIAL);
        ...
        if ([iTermAdvancedSettingsModel storeStateInSqlite]) {
            ...
            iTermRestorableStateSQLite *sqlite = [[iTermRestorableStateSQLite alloc] initWithURL:url erase:NO];
            sqlite.delegate = self;
            _saver = sqlite;
            _restorer = sqlite;
        } else {
            NSURL *indexURL = [NSURL fileURLWithPath:[savedState stringByAppendingPathComponent:@"Index.plist"]];
            iTermRestorableStateSaver *saver = [[iTermRestorableStateSaver alloc] initWithQueue:queue
                                                                                       indexURL:indexURL];
            _saver = saver;
            saver.delegate = self;
...
- (void)saveRestorableState {
    // BUG-f1166: Replace assert() with guard - off-main-thread call should be dispatched to main, not crash
    if (![NSThread isMainThread]) {
        DLog(@"WARNING: saveRestorableState called off main thread, dispatching to main");
...
- (void)restoreWindowsWithCompletion:(void (^)(void))completion {
    // BUG-f1167: Replace assert() with guard - off-main-thread call should be dispatched to main, not crash
    if (![NSThread isMainThread]) {
        DLog(@"WARNING: restoreWindowsWithCompletion called off main thread, dispatching to main");
        dispatch_async(dispatch_get_main_queue(), ^{
            [self restoreWindowsWithCompletion:completion];
        });
        return;
    }
...
- (void)completeInitialization {
    ...
    // BUG-f1168: Replace assert() with guard - off-main-thread call should be dispatched to main, not crash
    if (![NSThread isMainThread]) {
        DLog(@"WARNING: completeInitialization called off main thread, dispatching to main");
        dispatch_async(dispatch_get_main_queue(), ^{
            [self completeInitialization];
        });
        return;
...
- (void)invokeRemainingCompletionBlocksAsFailure {
    ...
    [temp enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull windowIdentifier,
                                              void (^_Nonnull completion)(NSWindow *, NSError *), BOOL *_Nonnull stop) {
...
- (void)restorableStateRestoreWithRecord:(nonnull iTermEncoderGraphRecord *)record
    ...
    [self.delegate
        restorableStateRestoreWithRecord:record
                              identifier:identifier
                              completion:^(NSWindow *_Nullable window, NSError *_Nullable error) {
                                  DLog(@"iTermRestorableStateController's completion block calling the completion "
                                       @"block for window %@",
                                       window);
                                  completion(window, error);
                                  DLog(@"iTermRestorableStateController's completion block calling didRestoreWindow:%@",
                                       window);
...
- (void)restorableStateEncodeWithCoder:(NSCoder *)coder window:(NSWindow *)window {
