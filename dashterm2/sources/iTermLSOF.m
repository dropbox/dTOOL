...
// Optimization: Cache NSNumber objects for process IDs (0-16383).
// Most PIDs on macOS fall within this range. System processes and user processes
// typically have PIDs under 16384, covering the vast majority of cases.
static const int kCachedPIDCount = 16384;
static NSNumber *sPIDCache[kCachedPIDCount];

NS_INLINE NSNumber *iTermPIDToNumber(int pid) {
    if (pid >= 0 && pid < kCachedPIDCount) {
        return sPIDCache[pid];
    }
    return @(pid);
}

__attribute__((constructor))
static void iTermLSOFInitializePIDCache(void) {
    for (int i = 0; i < kCachedPIDCount; i++) {
        sPIDCache[i] = @(i);
    }
}
...
+ (NSArray<NSString *> *)commandLineArgumentsForProcess:(pid_t)pid execName:(NSString **)execName {
    ...
    NSMutableArray<NSString *> *argv = [NSMutableArray arrayWithCapacity:nargs];
...
+ (NSArray<NSNumber *> *)allPids {
    ...
    if (bufferSize <= 0) {
        // RC-033: Return nil on system call failure, not empty array.
        // This allows callers to distinguish "error" from "no processes".
        DLog(@"proc_listpids returned %d for buffer size query", bufferSize);
        return nil;
    }
    ...
    if (bytesReturned <= 0) {
        // RC-033: Return nil on system call failure.
        DLog(@"proc_listpids returned %d when listing pids", bytesReturned);
        free(pids);
        return nil;
    }
    ...
    NSMutableArray<NSNumber *> *pidsArray = [NSMutableArray arrayWithCapacity:numPids];
    for (int i = 0; i < numPids; i++) {
        [pidsArray addObject:iTermPIDToNumber(pids[i])];
    }
...
// Provides shared access to the PID NSNumber cache for other subsystems.
+ (NSNumber *)cachedNumberForPid:(pid_t)pid {
    return iTermPIDToNumber(pid);
}
...
+ (NSString *)workingDirectoryOfProcess:(pid_t)pid {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("com.dashterm.dashterm2.pwd", DISPATCH_QUEUE_SERIAL);
    });
