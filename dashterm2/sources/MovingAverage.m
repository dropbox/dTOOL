...
- (void)pauseTimer {
    // BUG-f1214: Replace assert with guard - pauseTimer without started timer should be no-op
    if (![self haveStartedTimer]) {
        DLog(@"WARNING: MovingAverage pauseTimer: timer not started");
        return;
    }
    // BUG-f1215: Replace assert with guard - double pause should be no-op
    if (_timePaused != 0) {
        DLog(@"WARNING: MovingAverage pauseTimer: timer already paused");
        return;
    }
...
- (void)resumeTimer {
    // BUG-f1216: Replace assert with guard - resumeTimer without pause should be no-op
    if (_timePaused <= 0) {
        DLog(@"WARNING: MovingAverage resumeTimer: timer not paused");
        return;
    }
    // BUG-f1217: Replace assert with guard - resumeTimer without start should be no-op
    if (_time <= 0) {
        DLog(@"WARNING: MovingAverage resumeTimer: timer not started");
        return;
    }
