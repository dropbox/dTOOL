...
- (void)setAlphaValue:(CGFloat)alphaValue {
    // BUG-423: Replace assert(NO) with DLog - this method is intentionally a no-op
    // to prevent external modification of alpha value. The view manages its own alpha.
    DLog(@"TextViewWrapper: setAlphaValue called with %f but ignored - alpha is managed internally", alphaValue);
}
