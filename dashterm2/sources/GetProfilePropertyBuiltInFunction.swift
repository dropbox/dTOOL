...
extension GetProfilePropertyBuiltInFunction: iTermBuiltInFunctionProtocol {
    private static func error(message: String) -> NSError {
        return NSError(domain: "com.dashterm.dashterm2.get-profile-property",
                       code: 1,
    ...
    static func register() {
        ...
        let builtInFunction = iTermBuiltInFunction(
            ...
            sideEffectsPlaceholder: nil) { parameters, completion in
                ...
                // BUG-1684: Use guard with as? instead of force cast for keyArgName parameter
                guard let key = parameters[keyArgName] as? String else {
                    completion(nil, error(message: "Missing key parameter."))
                    return
                }
                let value = iTermProfilePreferences.object(forKey: key, inProfile: session.profile)
                completion(value, nil)
            }
        iTermBuiltInFunctions.sharedInstance().register(builtInFunction, namespace: "dashterm2")
    }
}
