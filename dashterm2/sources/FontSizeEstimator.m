...
+ (BOOL)useGenerousRoundingForFont:(NSFont *)aFont {
    for (NSString *family in [[iTermAdvancedSettingsModel fontsForGenerousRounding] componentsSeparatedByString:@","]) {
        if ([[[aFont familyName] lowercaseStringWithLocale:[NSLocale localeWithLocaleIdentifier:@"en_US"]]
...
+ (id)fontSizeEstimatorForFont:(NSFont *)aFont {
    // BUG-f690: Replace assert with guard - nil font should return nil not crash
    if (aFont == nil) {
        DLog(@"ERROR: FontSizeEstimator fontSizeEstimatorForFont: nil font passed");
        return nil;
    }
    FontSizeEstimator *fse = [[[FontSizeEstimator alloc] init] autorelease];
    if (fse) {
        // Single attribute dictionary for font measurement
        ...
        if (CGFontGetGlyphAdvances(cgfont, glyphs, 1, advances)) {
            ...
            DLog(@"Improving my guess for width using formula round(%d * %f / %d) giving %f", advances[0],
                 [aFont pointSize], CGFontGetUnitsPerEm(cgfont), size.width);
        }
        ...
        if ([iTermAdvancedSettingsModel useExperimentalFontMetrics]) {
            ...
            NSLayoutManager *layoutManager = [self layoutManagerForFont:aFont textContainer:textContainer];
