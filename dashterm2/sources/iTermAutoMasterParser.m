...
+ (instancetype)entryWithLine:(NSString *)line {
    ...
    if (![scanner scanUpToCharactersFromSet:[NSCharacterSet whitespaceCharacterSet] intoString:&mountpoint]) {
        return nil;
    }
    [scanner scanCharactersFromSet:[NSCharacterSet whitespaceCharacterSet] intoString:nil];
    if (![scanner scanUpToCharactersFromSet:[NSCharacterSet whitespaceCharacterSet] intoString:&map]) {
        return nil;
    }
    [scanner scanCharactersFromSet:[NSCharacterSet whitespaceCharacterSet] intoString:nil];
    // options are optional.
    [scanner scanUpToCharactersFromSet:[NSCharacterSet whitespaceCharacterSet] intoString:&options];
...
- (instancetype)init {
    self = [super init];
    if (self) {
        // /etc/auto_master typically has 5-15 entries
        NSMutableArray<iTermAutoMasterEntry *> *entries = [NSMutableArray arrayWithCapacity:16];
