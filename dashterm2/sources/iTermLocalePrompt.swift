...
extension NSPopUpButton {
    ...
    func populateWithLocales(encoding: String.Encoding) {
        ...
        // BUG-1633: Use safe optional binding with compactMapValues instead of force unwrap
        ...
        for components in good {
            if let quality = index[components.languageCode], quality < defaultIndexQuality {
                defaultIndex = menu?.items.count ?? 0
        ...
        // BUG-f507: Use a placeholder menu item action that logs instead of crashing
        @objc class Placeholder: NSObject, NSMenuItemValidation {
            ...
            @objc func placeholder(_ sender: Any) {
                // This should never be called because validateMenuItem returns false,
                // but if it somehow is called, just log and do nothing
                DLog("Warning: Placeholder action was unexpectedly invoked")
            }
        }
        ...
        if let defaultIndex,
           ...
           defaultIndex >= 0 && defaultIndex < menuItems.count {
            select(menuItems[defaultIndex])
        }
    }
}
...
class iTermLocalePromptAlert {
    ...
    func select(locale: String) {
        if let menu = popup.menu,
           let i = popup.menu?.indexOfItem(withRepresentedObject: locale),
           i >= 0 && i < menu.items.count {
            popup.select(menu.items[i])
        }
    }
