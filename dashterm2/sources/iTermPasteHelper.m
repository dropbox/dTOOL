...
// Timeout for user-provided regex evaluation to prevent ReDoS attacks
static const NSTimeInterval kPasteRegexMatchTimeoutSeconds = 0.5;
...
- (NSRange)makeRangeSafe:(NSRange)range {
    ...
    // Guard against range extending beyond string bounds
...
- (void)removeFirstCharacters:(NSUInteger)count {
    ...
    if (_offset == _string.length) {
        ...
        // BUG-1177: Compact the string periodically to reclaim memory from consumed characters.
        // Compact when we've consumed more than 64KB and the offset is more than half the string.
        static const NSUInteger kCompactionThreshold = 64 * 1024;
        if (_offset > kCompactionThreshold && _offset > _string.length / 2) {
            NSString *remaining = [_string substringFromIndex:_offset];
            _string = [remaining mutableCopy];
            _offset = 0;
        }
    }
}
...
- (void)appendString:(NSString *)string {
    // BUG-1178: Compact before appending if there's significant consumed data.
    // This prevents unbounded growth when repeatedly appending to a partially-consumed queue.
    static const NSUInteger kPreAppendCompactionThreshold = 32 * 1024;
    if (_offset > kPreAppendCompactionThreshold) {
        NSString *remaining = [_string substringFromIndex:_offset];
        _string = [remaining mutableCopy];
        _offset = 0;
    }
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _eventQueue = [[NSMutableArray alloc] initWithCapacity:4];
...
+ (void)sanitizePasteEvent:(PasteEvent *)pasteEvent encoding:(NSStringEncoding)encoding {
    ...
    if (pasteEvent.flags & kPasteFlagsUseRegexSubstitution && pasteEvent.regex.length > 0) {
        // Use NSRegularExpression with timeout protection instead of RegexKitLite
        // to prevent ReDoS attacks from user-provided regex patterns
        NSError *error = nil;
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:pasteEvent.regex
                                                                               options:0
                                                                                 error:&error];
        if (regex && !error) {
            NSString *substitution = [pasteEvent.substitution stringByExpandingVimSpecialCharacters] ?: @"";
            NSMutableString *result = [theString mutableCopy];
            NSDate *startTime = [NSDate date];
            __block BOOL timedOut = NO;

            // Find all matches with timeout protection
            NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:theString
            ...
            for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
                // Check for timeout
                if (-[startTime timeIntervalSinceNow] > kPasteRegexMatchTimeoutSeconds) {
                    DLog(@"Paste regex replacement timed out after %.3fs - possible ReDoS pattern", -[startTime timeIntervalSinceNow]);
                    timedOut = YES;
                    break;
                }

                if (match.range.location != NSNotFound) {
                    // Build replacement string with backreference support
                    NSString *expandedSubstitution = [regex replacementStringForResult:match
                                                                              inString:theString
                                                                                offset:0
                                                                              template:substitution];
                    [result replaceCharactersInRange:match.range withString:expandedSubstitution];
                }
            }
            ...
            if (!timedOut) {
                ...
                DLog(@"Regex substitution timed out for pattern: %@", pasteEvent.regex);
            }
        } else {
            DLog(@"Invalid regex pattern in paste event: %@ (error: %@)", pasteEvent.regex, error);
        }
    }
    if (pasteEvent.flags & kPasteFlagsBase64Encode) {
        // BUG-1184: Limit the size of data that can be base64 encoded to prevent OOM.
        // Base64 expands data by ~33%, so 50MB input → ~67MB output.
        static const NSUInteger kMaxBase64InputSize = 50 * 1024 * 1024;
        NSData *temp = [theString dataUsingEncoding:encoding];
        if (temp.length > kMaxBase64InputSize) {
            DLog(@"Refusing to base64 encode paste larger than %lu bytes (got %lu bytes)",
                 (unsigned long)kMaxBase64InputSize, (unsigned long)temp.length);
            // Truncate to the limit rather than failing completely
            temp = [temp subdataWithRange:NSMakeRange(0, kMaxBase64InputSize)];
        }
...
- (BOOL)maybeWarnAboutMultiLinePaste:(PasteEvent *)pasteEvent {
    ...
    // Count newlines efficiently without creating an array of all lines.
    // This fixes issue #11376 where pasting >1000 lines caused UI freeze
    // due to the overhead of splitting the string into an array.
    NSString *string = pasteEvent.string;
    NSUInteger lineCount = 1;  // Start with 1 since content before first newline is a line
    ...
    // If string ends with newline, we overcounted by 1 (no content after last newline)
    if (length > 0) {
    ...
    NSString *theTitle;
    NSMutableArray<iTermWarningAction *> *actions = [NSMutableArray arrayWithCapacity:4];
    ...
    if (lineCount > 1) {
        if (atShellPrompt) {
            theTitle = [NSString stringWithFormat:@"OK to paste %lu lines at shell prompt?",
            ...
            theTitle = [NSString stringWithFormat:@"OK to paste %lu lines?",
                        (unsigned long)lineCount];
        }
    } else {
