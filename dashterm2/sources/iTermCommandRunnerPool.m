...
- (instancetype)initWithCapacity:(int)capacity
    ...
    if (self) {
        ...
        _idle = [NSMutableArray arrayWithCapacity:capacity];
        _terminating = [NSMutableArray arrayWithCapacity:capacity];
        _busy = [NSMutableArray arrayWithCapacity:capacity];
    }
    return self;
}
...
- (instancetype)initWithCapacity:(int)capacity {
    self = [super initWithCapacity:capacity
                  ...
                  workingDirectory:nil
                       environment:nil];
    if (self) {
        _waiting = [NSMutableArray arrayWithCapacity:capacity];
    }
    return self;
}
...
- (void)commandRunnerDied:(id<iTermCommandRunner>)commandRunner {
    ...
    // BUG-f915: Guard against nil promise instead of crashing
    if (!promise) {
        ELog(@"iTermCommandRunnerPool: promise is nil after count check - race condition?");
        return;
    }
