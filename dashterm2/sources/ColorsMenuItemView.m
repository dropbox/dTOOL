...
- (NSRect)rectForCellIndex:(NSInteger)cellIndex enlarged:(BOOL)enlarged {
    ...
    return NSMakeRect(x, y, kColorAreaDimension + growth * 2, kColorAreaDimension + growth * 2);
}
...
- (void)drawRect:(NSRect)rect {
    ...
    const NSRect noColorRect =
        NSInsetRect([self rectForCellIndex:0 enlarged:(enabled && _selectedIndex == 0)], 0.5, 0.5);
    ...
    for (NSInteger i = 1; i <= colorCount; i++) {
        ...
        if (_mouseDown && _selectedIndex != NSNotFound) {
            ...
            showCheck = [self.currentColor isApproximatelyEqualToColor:[self colorAtIndex:i enabled:YES]
                                                               epsilon:1 / 65535.0];
            if (_mouseDown) {
                showCheck = NO;
            }
        ...
        if (enabled && showCheck) {
            ...
            dispatch_once(&onceToken, ^{
                lightImage =
                    [[NSImage imageNamed:NSImageNameMenuOnStateTemplate] it_imageWithTintColor:[NSColor whiteColor]];
                darkImage =
                    [[NSImage imageNamed:NSImageNameMenuOnStateTemplate] it_imageWithTintColor:[NSColor blackColor]];
            });
            ...
            NSRect rect = NSMakeRect(NSMidX(outlineArea) - checkSize.width / 2,
                                     NSMidY(outlineArea) - checkSize.height / 2, checkSize.width, checkSize.height);
    ...
    if (!enabled) {
        ...
        attributes[NSForegroundColorAttributeName] =
            [attributes[NSForegroundColorAttributeName] colorWithAlphaComponent:alpha];
    }
    NSString *labelTitle = @"Tab Color:";
    const CGFloat x = [self colorXOffset];
    [labelTitle drawAtPoint:NSMakePoint(x, [ColorsMenuItemView preferredSize].height - kMenuLabelOffsetY)
...
- (NSArray<NSColor *> *)colors {
    NSArray<NSColor *> *result = [[[iTermAdvancedSettingsModel tabColorMenuOptions] componentsSeparatedByString:@" "]
        mapWithBlock:^id(NSString *anObject) {
            return [NSColor colorFromHexString:anObject];
        }];
    if (result.count == 0) {
        // Fallback for if the string is totally broken.
        return @[
            [[NSColor colorWithSRGBRed:251.0 / 255.0 green:107.0 / 255.0 blue:98.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:246.0 / 255.0 green:172.0 / 255.0 blue:71.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:240.0 / 255.0 green:220.0 / 255.0 blue:79.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:181.0 / 255.0 green:215.0 / 255.0 blue:73.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:95.0 / 255.0 green:163.0 / 255.0 blue:248.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:193.0 / 255.0 green:142.0 / 255.0 blue:217.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
            [[NSColor colorWithSRGBRed:120.0 / 255.0 green:120.0 / 255.0 blue:120.0 / 255.0
                                 alpha:1] it_colorInDefaultColorSpace],
        ];
    }
    return result;
