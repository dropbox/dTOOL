...
- (instancetype)initWithPath:(NSString *)path image:(out iTermImageWrapper **)imageOut NS_DESIGNATED_INITIALIZER;
...
- (iTermImageWrapper *)imageIfValid {
    ...
    if (image != nil && self.lastModified != nil &&
...
- (void)setupMemoryPressureHandler {
    ...
    dispatch_source_set_event_handler(_memoryPressureSource, ^{
        ...
        dispatch_source_memorypressure_flags_t flags = dispatch_source_get_data(strongSelf->_memoryPressureSource);
        if (flags & (DISPATCH_MEMORYPRESSURE_WARN | DISPATCH_MEMORYPRESSURE_CRITICAL)) {
            DLog(@"Memory pressure detected, clearing image cache");
...
- (instancetype)initWithImage:(NSImage *)unsafeImage {
    self = [super init];
    if (self) {
        _reps = [[NSMutableDictionary alloc] initWithCapacity:8];
        _tilingImages = [[NSMutableDictionary alloc] initWithCapacity:4];
        ...
        if (unsafeImage.size.height > 0 && unsafeImage.size.width > 0) {
            ...
            if (unsafeImage.size.width > maxSize || unsafeImage.size.height > maxSize) {
                ...
                NSImage *downscaled = [unsafeImage
                    it_imageOfSize:NSMakeSize(unsafeImage.size.width * scale, unsafeImage.size.height * scale)];
                image = downscaled;
            }
        }
        _image = image;
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p reps=%@ image=%@ tilingImages=%@ cgimage=%@>",
                                      NSStringFromClass([self class]), self, _reps, _image, _tilingImages, _cgimage];
}
...
- (NSSize)scaledSize {
    NSImageRep *rep =
        [[self.image representations] maxWithComparator:^NSComparisonResult(NSImageRep *a, NSImageRep *b) {
...
- (NSImage *)tilingBackgroundImageForBackingScaleFactor:(CGFloat)scale {
    ...
    NSImageRep *bestRep =
        [self.image.representations maxWithBlock:^NSComparisonResult(NSImageRep *obj1, NSImageRep *obj2) {
    ...
    if ([bestRep isKindOfClass:[NSBitmapImageRep class]]) {
        ...
        if (cgimage) {
            ...
            cookedImage = [[NSImage alloc]
                initWithSize:NSMakeSize(CGImageGetWidth(cgimage) / scale, CGImageGetHeight(cgimage) / scale)];
...
- (NSBitmapImageRep *)bitmapInColorSpace:(NSColorSpace *)colorSpace {
    ...
    if (cgImage == nil) {
        // I've noticed this happens when low on memory. bitmapImageRepByConvertingToColorSpace will die with an
        // assertion, so better to return nil now.
        return nil;
    }
