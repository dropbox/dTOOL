...
// BUG-29: WebSocket protocol identifier updated to DashTerm2 naming
static NSString *const kProtocolName = @"api.dashterm.dashterm2";
...
+ (instancetype)newWebSocketConnectionForRequest:(NSURLRequest *)request
    ...
    NSString *cookie = headers[@"x-dashterm2-cookie"];
    if (cookie) {
        if ([[iTermWebSocketCookieJar sharedInstance] consumeCookie:cookie]) {
            authenticated = YES;
        } else {
            *reason = [NSString stringWithFormat:@"x-dashterm2-cookie of %@ not recognized", cookie];
            cookie = nil;
        }
    }

    {
        NSString *libver = headers[@"x-dashterm2-library-version"];
        if (!libver) {
            *reason = @"The “x-dashterm2-library-version” header was not set. You may need to update your Python runtime to a newer version.";
            return nil;
        }
        NSArray<NSString *> *parts = [libver componentsSeparatedByString:@" "];
        if (parts.count != 2) {
            *reason = [NSString stringWithFormat:@"The “x-dashterm2-library-version” header was malformed: ”%@”", libver];
            return nil;
        }
        ...
        if (result == NSOrderedDescending) {
            *reason = [NSString stringWithFormat:@"%@. %@ library version reported as %@. Minimum supported by this version of DashTerm2 is %@",
    ...
    if (conn) {
        ...
        NSString *key = headers[@"x-dashterm2-key"] ?: [[NSUUID UUID] UUIDString];
        conn->_key = [[iTermAPIConnectionIdentifierController sharedInstance] identifierForKey:key];
        conn->_advisoryName = headers[@"x-dashterm2-advisory-name"];
    }
    return conn;
}
...
- (instancetype)initWithConnection:(iTermHTTPConnection *)connection {
    ...
    if (self) {
        ...
        _queue = dispatch_queue_create("com.dashterm.dashterm2.websocket", NULL);
...
- (void)didUpgradeSuccessfully:(BOOL)upgradeOK {
    ...
    dispatch_async(connection.queue, ^{
        while (YES) {
            ...
            NSMutableData *data = [connection readSynchronously];
            if (!data) {
                DLog(@"Read EOF from connection");
                [strongSelf abortWithCompletion:nil];
                return;
            }
...
- (void)sendData:(NSData *)data {
    // BUG-7212: Explicitly retain data in the destructor block to ensure it lives
    // until dispatch_data is deallocated. The previous pattern of [data length]
    // could theoretically be optimized away by the compiler.
    NSData *strongData = data;
    dispatch_data_t dispatchData = dispatch_data_create(data.bytes, data.length, _queue, ^{
        DLog(@"Disposing of data %p", strongData);
        (void)strongData;  // Explicit capture - ensures ARC retains until block dealloc
    });

    __weak __typeof(self) weakSelf = self;
    [_connection writeAsynchronously:dispatchData queue:_queue completion:^(bool done, dispatch_data_t  _Nullable responseData, int error) {
    ...
    if (CC_SHA1([data bytes], [data length], hash) ) {
        ...
        NSDictionary<NSString *, NSString *> *headers =
            @{
               ...
               @"Sec-WebSocket-Protocol": kProtocolName,
               @"X-DashTerm2-Protocol-Version": @"1.11"
             };
