...
class SavedIntervalTreeObject: NSObject {
    ...
    struct ReflowableCoordinate {
        ...
        // BUG-1686: Add bounds checks for y index access
        init?(x: Int32, y: Int, preprocessedState: PreprocessedState) {
            guard y >= 0 && y < preprocessedState.hardNewlineCounts.count &&
                  y < preprocessedState.cellOffsets.count else {
                return nil
            }
    ...
    init?(intervalTreeObject: IntervalTreeObject,
        ...
        // BUG-1686: Handle failable initializer for bounds-checked coordinates
        guard let startCoord = ReflowableCoordinate(x: absCoordRange.start.x,
                                                    y: Int(absCoordRange.start.y - line),
                                                    preprocessedState: preprocessedState),
              let endCoord = ReflowableCoordinate(x: absCoordRange.end.x,
    ...
    private func horizontalAdvanceToScreenCoord(startY: Int64,
        ...
        // BUG-834: Add iteration limit to prevent infinite loop if cellCountInWrappedLine
        // returns 0 repeatedly (e.g., for invalid or empty lines). The limit is generous
        // enough to handle any reasonable terminal size (Int32.max lines).
        var iterations = 0
        let maxIterations = Int(Int32.max)
        while iterations < maxIterations {
            iterations += 1
            let length = extractor.cellCountInWrappedLine(withAbsY: y)
            // BUG-834: Also break if length is 0 and remaining > 0 to prevent infinite loop
            // when we can't make progress (empty line with content still to traverse).
            if length == 0 && remaining > 0 {
                DLog("horizontalAdvanceToScreenCoord: Breaking due to zero-length line at y=\(y) with remaining=\(remaining)")
                break
            }
        ...
        if iterations >= maxIterations {
            DLog("horizontalAdvanceToScreenCoord: Hit iteration limit, breaking loop")
        }
