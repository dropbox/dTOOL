...
class MultipartMessageCellView: MessageCellView {
    ...
    override func configure(with rendition: MessageRendition,
        ...
        guard case .multipart(let subparts) = rendition.flavor else {
            // BUG-3027: MultipartMessageCellView should only receive multipart content
            DLog("Error: MultipartMessageCellView received non-multipart flavor: \(rendition.flavor)")
            return
        }
        ...
        for (index, subpart) in subparts.enumerated() {
            ...
            case .fileAttachment(id: let id, name: let name, file: let file):
                guard let icon = subpart.icon else {
                    continue
                }
                let view = FileAttachmentSubpartView(icon: icon,
    ...
    private func createStatusUpdateTextView(for subpart: MessageRendition.SubpartContainer,
        ...
        // BUG-1641: Use nil coalescing instead of force unwrap for color parsing
        textView.backgroundColor = NSColor(fromHexString: "#ffffc0") ?? .yellow
    ...
    func backgroundColorPair(_ rendition: MessageRendition) -> (NSColor, NSColor) {
        // BUG-1641: Use nil coalescing instead of force unwrap for color parsing
        rendition.isUser
        ? (NSColor(fromHexString: "p3#448bf7") ?? .systemBlue, NSColor(fromHexString: "p3#4a93f5") ?? .systemBlue)
        : (NSColor(fromHexString: "p3#e9e9eb") ?? .lightGray, NSColor(fromHexString: "p3#3b3b3d") ?? .darkGray)
    }
    ...
    override func forkMenuItemClicked(_ sender: Any) {
        // Call parent implementation which handles the fork callback
        super.forkMenuItemClicked(sender)
    }
}
