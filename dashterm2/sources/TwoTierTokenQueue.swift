...
class TokenArrayGroup {
    ...
    private var gang: VT100Token {
        // BUG-481: Replace it_assert with guard for proper crash prevention
        guard coalescable else {
            DLog("BUG-481: gang property accessed when not coalescable, returning empty token")
            return VT100Token()
        }
        let gang = VT100Token()
    ...
    var peek: VT100Token? {
        ...
        // BUG-482: Replace it_assert with guard + logging for proper crash prevention
        // Non-coalescable should have exactly one array, log warning if unexpected
        guard arrays.count == 1 else {
            DLog("BUG-482: Non-coalescable TokenQueue has \(arrays.count) arrays, expected 1")
            return arrays.first?.peek
        }
    ...
    func consume() -> Bool {
        ...
        // BUG-402: Use safe array access with guard instead of direct index access
        guard let first = arrays.first else {
            return false
        }
...
fileprivate class Queue: CustomDebugStringConvertible {
    ...
    func removeFirst(_ n: Int) {
        mutex.sync {
            // BUG-f1398: Clamp n to actual array count to prevent crash
            // Swift's removeFirst(n) crashes if n > count
            let safeN = min(n, arrays.count)
            guard safeN > 0 else {
                return
            }
            arrays.removeFirst(safeN)
        }
    }
