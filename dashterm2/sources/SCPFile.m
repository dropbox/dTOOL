...
static NSString *const kSCPFileErrorDomain = @"com.dashterm.dashterm2.SCPFile";
...
static NSError *SCPFileError(NSString *description) {
    return [NSError errorWithDomain:kSCPFileErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey : description}];
}
...
@implementation iTermAuthSock {
    NSString *_authSock;     // Protected by _lock
    dispatch_group_t _group; // Protected by _lock
...
- (NSString *)authSock {
    ...
    dispatch_group_wait(group, dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC));
    os_unfair_lock_lock(&_lock);
    NSString *result = _authSock;
    os_unfair_lock_unlock(&_lock);
    DLog(@"Return %@", result);
...
@implementation SCPFile {
    BOOL _okToAdd;
    BOOL _downloading;
    dispatch_queue_t _queue; // Protected by _lock
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _queue = dispatch_queue_create("com.dashterm.dashterm2.SCPFile", NULL);
...
- (dispatch_queue_t)queue {
    os_unfair_lock_lock(&_lock);
    dispatch_queue_t result = _queue;
    os_unfair_lock_unlock(&_lock);
    return result;
}
...
- (NSString *)hostname {
    ...
    } else if (components >= 4 && [hostComponents[0] hasPrefix:@"["] &&
...
- (BOOL)privateKeyIsEncrypted:(NSString *)filename {
    @autoreleasepool {
        NSString *privateKey = [NSString stringWithContentsOfFile:filename encoding:NSUTF8StringEncoding error:nil];
...
- (NSString *)filenameByExpandingMetasyntacticVariables:(NSString *)filename {
    ...
    for (NSString *metavar in substitutions) {
        filename = [filename stringByReplacingOccurrencesOfString:metavar withString:substitutions[metavar]];
    }
    return filename;
}
...
- (NSString *)keyboardInteractiveRequest:(NSString *)prompt {
    ...
    // BUG-11878: Add 5 minute timeout for user input to prevent indefinite hang
    dispatch_time_t timeout = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(300 * NSEC_PER_SEC));
    if (dispatch_group_wait(group, timeout) != 0) {
        DLog(@"keyboardInteractiveRequest timed out after 5 minutes");
        os_unfair_lock_lock(&_lock);
        if (!completed) {
            completed = YES;
            os_unfair_lock_unlock(&_lock);
            // Balance the dispatch_group_enter
            dispatch_group_leave(group);
        } else {
            os_unfair_lock_unlock(&_lock);
        }
...
- (NSURL *)sessionURL {
    // BUG-f1009: Gracefully handle nil session instead of crashing
    if (!self.session) {
        DLog(@"BUG-f1009: sessionURL called with nil session");
        return nil;
    }
...
- (void)performTransfer:(BOOL)isDownload agentAllowed:(BOOL)agentAllowed {
    ...
    DLog(@"performTransfer download=%@ agentAllowed=%@ path=%@ baseName=%@", @(isDownload), @(agentAllowed), self.path,
         baseName);
    if (!baseName) {
    ...
    if (self.session) {
        ...
        DLog(@"Create seession to hostname=%@ configs=%@ port=%@ username=%@", [self hostname], [SCPFile configs],
             @([self port]), self.path.username);
    ...
    if (!self.session.isConnected) {
        ...
        if (!theError) {
            ...
            theError = [NSError errorWithDomain:@"com.dashterm.dashterm2"
                                           code:-1
        ...
        [self performOnMainThread:^{
            [[FileTransferManager sharedInstance] transferrableFile:self didFinishTransmissionWithError:theError];
            iTermWarningSelection selection = [iTermWarning
                showWarningWithTitle:
                    [NSString
                        stringWithFormat:@"Failed to connect to %@:%d. Double-check that the host name is correct.",
            ...
            if (selection == kiTermWarningSelection1) {
                ...
                [[NSWorkspace sharedWorkspace]
                    it_openURL:[NSURL URLWithString:@"https://dashterm.com/troubleshoot-hostname"]
                        target:nil
    ...
    if (!self.session.isAuthorized) {
        ...
        for (NSString *authType in authTypes) {
            ...
            if ([authType isEqualToString:@"password"]) {
                ...
                // Pre-allocate for typical identity file paths (usually 4-8)
                NSMutableArray *keyPaths = [NSMutableArray arrayWithCapacity:8];
                if (self.session.hostConfig.identityFiles.count) {
                    [keyPaths addObjectsFromArray:self.session.hostConfig.identityFiles];
                } else {
                    [keyPaths addObjectsFromArray:@[
                ...
                for (NSString *iteratedKeyPath in keyPaths) {
                    ...
                    if ([self privateKeyIsEncrypted:keyPath]) {
                        NSString *prompt = [NSString stringWithFormat:@"passphrase for private key “%@”:", keyPath];
                    ...
                    if (![[NSFileManager defaultManager] fileExistsAtPath:publicKeyPath]) {
                        XLog(@"Warning: no public key at %@. Trying to authenticate with only a private key.",
                             publicKeyPath);
                        publicKeyPath = nil;
                    }
                    [self.session authenticateByPublicKey:publicKeyPath privateKey:keyPath andPassword:password];
    ...
    if (!self.session.isAuthorized) {
        ...
        [self performOnMainThread:^{
            if (!error) {
                error = [NSError errorWithDomain:@"com.dashterm.dashterm2.SCPFile"
                                            code:0
    ...
    if (_okToAdd) {
        ...
        [self.session addKnownHostName:self.session.host port:[self.session.port intValue] toFile:nil withSalt:nil];
    }
    ...
    if (isDownload) {
        ...
        if (!tempfile) {
            self.error = [NSString stringWithFormat:@"Downloads folder not writable. Tried %@", downloadDirectory];
            [self performOnMainThread:^{
                [[FileTransferManager sharedInstance] transferrableFile:self
        ...
        BOOL ok = [self.session.channel
            downloadFile:self.path.path
                      to:tempfile
        ...
        if (ok) {
            ...
            [self performOnMainThread:^{
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:finalDestination]) {
                    ...
                    [[NSFileManager defaultManager] moveItemAtPath:tempfile toPath:finalDestination error:&error];
                }
            }];
            if (error) {
...
- (void)performOnMainThread:(void (^NS_NOESCAPE)(void))block {
    ...
    // Add 30-second timeout to prevent indefinite hangs if main thread is blocked
    if (dispatch_group_wait(group, dispatch_time(DISPATCH_TIME_NOW, 30 * NSEC_PER_SEC))) {
        DLog(@"performOnMainThread timed out after 30 seconds - main thread may be blocked");
    }
}

- (NSString *)tempFileName {
    NSString *result = [NSString stringWithFormat:@".DashTerm2.%@", [NSString uuid]];
...
- (BOOL)hostnameIsKnown {
    return [[[NSUserDefaults standardUserDefaults] objectForKey:SCPFileKnownHostsUserDefaultsKey]
        containsObject:self.userHostPort];
}
...
- (void)addKnownHost {
    NSArray<NSString *> *hosts =
        [[NSUserDefaults standardUserDefaults] objectForKey:SCPFileKnownHostsUserDefaultsKey] ?: @[];
    ...
    [self performOnMainThread:^{
        ...
        switch (status) {
            ...
            case NMSSHKnownHostStatusMismatch:
                title = @"Warning!";
                message = [NSString
                    stringWithFormat:@"REMOTE HOST IDENTIFICATION HAS CHANGED!\n\n"
            ...
            case NMSSHKnownHostStatusNotFound:
                ...
                message = [NSString stringWithFormat:@"The authenticity of host '%@' can't be established.\n\n"
                                                     @"%@ key fingerprint is %@.\n\nConnect anyway?",
                                                     host, hashName, fingerprint];
                _okToAdd = YES;
                break;
        ...
        if (message) {
            result = [[FileTransferManager sharedInstance] transferrableFile:self title:title confirmMessage:message];
        }
    }];
    return result;
}
