...
class TokenArray: IteratorProtocol, CustomDebugStringConvertible {
    var debugDescription: String {
        ...
        for i in 0..<cvector.count {
            // BUG-1653: Use as? instead of force cast for CVectorGetObject
            guard let token = CVectorGetObject(&cvector, i) as? VT100Token else { continue }
    ...
    static let destroyQueue: DispatchQueue = {
        return DispatchQueue(label: "com.dashterm.dashterm2.token-destroyer")
    }()
    ...
    init(_ cvector: CVector,
        ...
        // BUG-f661: Replace it_assert with guard-like behavior - invalid lengths should not crash
        if lengthTotal <= 0 || lengthExcludingInBandSignaling < 0 {
            DLog("BUG-f661: Invalid token array lengths: total=\(lengthTotal), excluding=\(lengthExcludingInBandSignaling)")
        }
        self.cvector = cvector
        self.lengthTotal = max(lengthTotal, 0)  // Ensure non-negative
        self.lengthExcludingInBandSignaling = max(lengthExcludingInBandSignaling, 0)  // Ensure non-negative
    ...
    func replaceLast(with replacement: VT100Token) {
        ...
        // BUG-480: Replace it_assert with guard to prevent crash on empty array
        guard count > 0 else {
            DLog("BUG-480: replaceLast called on empty TokenArray, ignoring")
            return
        }
