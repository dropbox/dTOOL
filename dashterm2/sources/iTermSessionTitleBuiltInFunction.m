...
+ (void)registerBuiltInFunction {
    ...
    // BUG-f1361: Replace assert() with guard - cyclic reference should log warning, not crash
    // This is a programmer error that should be fixed in code, but shouldn't crash production
    if ([defaults.allValues containsObject:iTermVariableKeySessionName]) {
        ELog(@"iTermSessionTitleBuiltInFunction: Cyclic reference detected - session.name in defaults");
    }
    NSSet *optionalArguments = [NSSet setWithArray:@[
        iTermSessionTitleArgTmuxPane, iTermSessionTitleArgTmuxRole, iTermSessionTitleArgTmuxClientName,
        iTermSessionTitleArgTmuxWindowName, iTermSessionTitleArgTmuxWindowTitle
    ]];
    {
        iTermBuiltInFunction *func = [[iTermBuiltInFunction alloc]
                 initWithName:@"session_title"
       ...
       sideEffectsPlaceholder:nil
        ...
        [[iTermBuiltInFunctions sharedInstance] registerFunction:func namespace:@"dashterm2.private"];
    }
    {
        iTermBuiltInFunction *func = [[iTermBuiltInFunction alloc]
                 initWithName:@"window_title"
                    arguments:@{iTermSessionTitleSession : [NSString class]}
            optionalArguments:optionalArguments
                defaultValues:defaults
                      context:iTermVariablesSuggestionContextSession
       sideEffectsPlaceholder:nil
                        block:^(NSDictionary *_Nonnull parameters,
                            ...
                            // NOTE: iTermSessionNameController assumes that the built-in window_title function
...
+ (NSString *)titleForParameters:(NSDictionary *)parameters isWindow:(BOOL)isWindow {
    ...
    titleComponents = [iTermProfilePreferences unsignedIntegerForKey:KEY_TITLE_COMPONENTS inProfile:session.profile];
...
+ (NSString *)titleForSessionName:(NSString *)rawSessionName
    ...
    DLog(@"sessionName=%@ profileName=%@ job=%@ commandLine=%@ pwd=%@ tty=%@ user=%@ host=%@ tmuxPane=%@ iconName=%@ "
         @"windowName=%@ tmuxWindowName=%@ tmuxWindowTitle=%@ isWindowTitle=%@ rows=%@ columns=%@ titleComponents=%@",
         sessionName, profileName, jobVariable, commandLineVariable, pwdVariable, ttyVariable, userVariable,
         hostVariable, tmuxPaneVariable, iconName, windowName, tmuxWindowName, tmuxWindowTitle, @(isWindowTitle), rows,
         columns, @(titleComponents));

    NSString *name = nil;
    // Session titles are typically short (~64 chars)
    ...
    if (tmuxPaneVariable) {
        if (isWindowTitle) {
            // `tmuxWindowTitle` comes from #{T:set-titles-string} if you've done `set-option -g set-titles on`. Prefer
            // this since it is an explicit opt-in. `windowName` is affected by OSC 0 and OSC 2 and popping the window
            // title stack. It will be unset if there was no OSC. This is the session's `terminalWindowName` variable.
            // `tmuxWindowName` comes from `#{window_name}`. The default is the current process. It can be changed with
            // the rename-window command or ESC k. It comes from the variable `tab.tmuxWindowName`. It is driven by the
            // %window-renamed notification. `tmuxPaneVariable` corresponds to #{pane_title}, which is affected by OSC 0
            // and OSC 2 and popping the window title stack. Its default value is the hostname.
    ...
    if (formatIndex) {
        ...
        // User@host:path typically ~80 chars
        NSMutableString *userHostPWD = [NSMutableString stringWithCapacity:80];
...
+ (NSString *)prettyPWD:(NSString *)absolutePath homeDirectory:(NSString *)home {
    if (!home) {
        return absolutePath;
    }
