...
- (instancetype)initWithContext:(PasteContext *)pasteContext length:(int)length mini:(BOOL)mini {
    self = [super initWithNibName:iTermPasteViewControllerNibName(mini) bundle:[NSBundle bundleForClass:self.class]];
    if (self) {
        [self view];
        _mini = mini;
...
// BUG-405: Cancel delayed selector in dealloc to prevent crash if view is
// deallocated while removeFromSuperview is pending.
- (void)dealloc {
    [NSObject cancelPreviousPerformRequestsWithTarget:self.view];
...
- (void)viewDidAppear {
    // BUG-4069: Must call super to complete view controller lifecycle
    [super viewDidAppear];
...
- (void)updateFrame {
    ...
    newFrame = NSMakeRect(self.view.frame.origin.x, self.view.superview.frame.size.height - self.view.frame.size.height,
