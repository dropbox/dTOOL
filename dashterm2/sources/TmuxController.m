...
// Optimization: Cache NSNumber objects for tmux window/pane IDs (0-1023).
// Window and pane IDs are typically small integers. This cache covers all
// realistic tmux usage scenarios and eliminates boxing in dictionary operations.
static const int kCachedTmuxIdCount = 1024;
static NSNumber *sTmuxIdCache[kCachedTmuxIdCount];

NS_INLINE NSNumber *iTermTmuxIdToNumber(int id) {
    if (id >= 0 && id < kCachedTmuxIdCount) {
        return sTmuxIdCache[id];
    }
    return @(id);
}

__attribute__((constructor)) static void iTermTmuxControllerInitializeIdCache(void) {
    for (int i = 0; i < kCachedTmuxIdCount; i++) {
        sTmuxIdCache[i] = @(i);
    }
}
...
+ (instancetype)trivialInstance {
    return [[self alloc] initWithIndex:nil
                            completion:^(int i){
                            }];
}
...
- (instancetype)initWithIndex:(NSNumber *)index completion:(void (^)(int))completion {
    ...
    self = [super init];
    if (self) {
        // BUG-f1375: Convert assert to guard - ensure index is valid NSNumber or nil
        if (index != nil && [NSNumber castFrom:index] == nil) {
            return nil;
        }
        _index = index;
...
@implementation TmuxController {
    ...
    // 1. > select-window -t @2            // DashTerm2-initiated
    // 2. > select-window -t @3            // DashTerm2-initiated
...
static NSDictionary *iTermTmuxControllerMakeFontOverrides(iTermFontTable *fontTable, CGFloat hs, CGFloat vs) {
    return @{
        KEY_NORMAL_FONT : fontTable.asciiFont.font.stringValue,
...
static NSDictionary *iTermTmuxControllerDefaultFontOverridesFromProfile(Profile *profile) {
    return iTermTmuxControllerMakeFontOverrides([iTermFontTable fontTableForProfile:profile],
                                                [iTermProfilePreferences floatForKey:KEY_HORIZONTAL_SPACING
                                                                           inProfile:profile],
                                                [iTermProfilePreferences floatForKey:KEY_VERTICAL_SPACING
                                                                           inProfile:profile]);
}
...
- (instancetype)initWithGateway:(TmuxGateway *)gateway
    ...
    if (self) {
        ...
        gateway_ = gateway;
        // Tmux sessions typically have a moderate number of panes (8-16 is common)
        _paneIDs = [[NSMutableSet alloc] initWithCapacity:16];
        windowPanes_ = [[NSMutableDictionary alloc] initWithCapacity:16];
        _windowStates = [[NSMutableDictionary alloc] initWithCapacity:8];
        windowPositions_ = [[NSMutableDictionary alloc] initWithCapacity:8];
        origins_ = [[NSMutableDictionary alloc] initWithCapacity:8];
        pendingWindowOpens_ = [[NSMutableSet alloc] initWithCapacity:4];
        hiddenWindows_ = [[NSMutableSet alloc] initWithCapacity:4];
        _hotkeys = [[NSMutableDictionary alloc] initWithCapacity:8];
        _tabColors = [[NSMutableDictionary alloc] initWithCapacity:8];
        self.clientName = [[TmuxControllerRegistry sharedInstance] uniqueClientNameBasedOn:clientName];
        _windowOpenerOptions = [[NSMutableDictionary alloc] initWithCapacity:8];
        _pendingWindows = [[NSMutableDictionary alloc] initWithCapacity:4];
        _currentWindowID = -1;
        _userVars = [[NSMutableDictionary alloc] initWithCapacity:8];
        _when = [[NSMutableDictionary alloc] initWithCapacity:4];
        [[TmuxControllerRegistry sharedInstance] setController:self forClient:_clientName];
        // List windows queue typically holds a few pending commands
        _listWindowsQueue = [[NSMutableArray alloc] initWithCapacity:4];
        _paneToActivateWhenCreated = -1;
        _buriedWindows = [[NSMutableDictionary alloc] initWithCapacity:4];
        _savedFrames = [[NSMutableDictionary alloc] initWithCapacity:8];
        ...
        _windowSizes = [[NSMutableDictionary alloc] initWithCapacity:8];
...
- (Profile *)profileForWindow:(int)window {
    ...
    Profile *original = _windowStates[iTermTmuxIdToNumber(window)].profile;
    if (!original) {
        return [self sharedProfile];
    }
    NSMutableDictionary *temp = [original mutableCopy];
    [temp it_mergeFrom:_windowStates[iTermTmuxIdToNumber(window)].fontOverrides];
    return temp;
}
...
- (NSDictionary *)fontOverridesForWindow:(int)window {
    ...
    return _windowStates[iTermTmuxIdToNumber(window)].fontOverrides ?: self.sharedFontOverrides;
}
...
// Called when listing window finishes. Happens for all new windows/tabs, whether initiated by DashTerm2 or not.
- (void)openWindowWithIndex:(int)windowIndex
    ...
    DLog(@"openWindowWithIndex:%d name:%@ affinities:%@ flags:%@ initial:%@", windowIndex, name, affinities,
         windowFlags, @(initial));
    if (!gateway_) {
    ...
    for (NSString *a in affinities) {
        ...
        [affinities_ setValue:a equalToValue:[NSString stringWithInt:windowIndex]];
    }
    ...
    windowOpener.maxHistory =
        MAX([[gateway_ delegate] tmuxClientSize].height, [[gateway_ delegate] tmuxNumberOfLinesOfScrollbackHistory]);
    ...
    DLog(@"windowGUID=%@ perWindowSettings=%@ perTabSettings=%@", windowOpener.windowGUID,
         windowOpener.perWindowSettings, windowOpener.perTabSettings);
...
- (void)adjustWindowSizeIfNeededForTabs:(NSArray<PTYTab *> *)tabs {
    ...
    if (![tabs anyWithBlock:^BOOL(PTYTab *tab) {
            return [tab updatedTmuxLayoutRequiresAdjustment];
        }]) {
    ...
    const BOOL outstandingResize = [tabs anyWithBlock:^BOOL(PTYTab *tab) {
    ...
    if (outstandingResize) {
        DLog(@"adjustWindowSizeIfNeededForTabs: One of the tabs has a tmux controller with an outstanding window "
             @"resize. Don't update layouts.");
        return;
    }
...
- (void)sessionChangedTo:(NSString *)newSessionName sessionId:(int)sessionid {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerAttachedSessionDidChange object:nil];
}
...
- (void)windowsChanged {
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerWindowsChangeNotification object:self];
}

- (NSArray *)listWindowFields {
    NSArray<NSString *> *basic = @[
        @"session_name", @"window_id", @"window_name", @"window_width", @"window_height", @"window_layout",
        @"window_flags", @"window_active"
    ];
...
// Returns YES if the window has no meaningful affinity (i.e., it's unrecognized
// or only has a placeholder affinity). Such windows should be eligible for
// grouping when "Native tabs in a new window" is enabled.
// Fix for GitLab #11424: Previously, windows with placeholder affinities were
// treated as "recognized" and excluded from grouping when users changed their
// preference to "Native tabs in a new window".
- (BOOL)windowLacksRealAffinity:(NSString *)windowId {
    NSSet<NSObject<NSCopying> *> *affinities = [affinities_ valuesEqualTo:windowId];
    if (!affinities) {
        // No affinity at all - window is unrecognized
        return YES;
    }
    // Check if the affinity class only contains the window and its placeholder
    // (e.g., {"5", "5_ph"}). This is a "placeholder affinity" created when a
    // window was opened alone, and should be treated as unrecognized.
    NSString *placeholder = [windowId stringByAppendingString:@"_ph"];
    for (NSObject<NSCopying> *value in affinities) {
        NSString *valueStr = (NSString *)value;
        if (![valueStr isEqualToString:windowId] && ![valueStr isEqualToString:placeholder]) {
            // Has a real affinity (another window or terminal GUID)
            return NO;
        }
    }
    // Only has self and/or placeholder - treat as unrecognized
    return YES;
}
...
- (void)initialListWindowsResponse:(NSString *)response {
    ...
    if (!doc) {
        ...
        [gateway_ abortWithErrorMessage:[NSString stringWithFormat:@"Bad response for initial list windows request: %@",
                                                                   response]];
        return;
    }
    // Pre-allocate for typical number of tmux windows
    NSMutableArray<NSArray *> *windowsToOpen = [NSMutableArray arrayWithCapacity:doc.records.count];
    BOOL haveHidden = NO;
    ...
    for (NSArray *record in doc.records) {
        ...
        // GitLab #11424: Use windowLacksRealAffinity: to properly detect windows
        // that should be grouped. This correctly treats placeholder-only affinities
        // as "unrecognized" so they can be grouped when the user changes their
        // preference to "Native tabs in a new window".
        if ([self windowLacksRealAffinity:[n stringValue]] && newWindowsInTabs) {
            // Create an equivalence class of all unrecognized windows to each other.
            if (!newWindowAffinity) {
                ...
                [affinities_ setValue:[n stringValue] equalToValue:[newWindowAffinity stringValue]];
            }
        }
    ...
    if (tooMany) {
        [[iTermNotificationController sharedInstance] notify:@"Too many tmux windows!"
                                             withDescription:@"Use the tmux dashboard to select which to open."];
    } else if (haveHidden) {
        [[iTermNotificationController sharedInstance] notify:@"Some tmux windows were hidden."
                                             withDescription:@"Use the tmux dashboard to select which to open."];
    }
...
- (void)handleShowSize:(NSString *)response {
    ...
    int width = 0;
    int height = 0;
    BOOL ok = ([scanner scanInt:&width] && [scanner scanString:@"," intoString:nil] && [scanner scanInt:&height]);
    if (ok) {
...
- (void)openWindowsOfSize:(VT100GridSize)size {
    ...
    // lets us know if a single instance of DashTerm2 is trying to attach to the same
    ...
    // it twice, causing chaos. Or two separate instances of DashTerm2 attaching
    ...
    NSString *getSessionGuidCommand = [NSString stringWithFormat:@"show -v -q -t $%d @iterm2_id", sessionId_];
    ...
    NSString *setSizeCommand = [NSString stringWithFormat:@"refresh-client -C %d,%d", size.width, size.height];
    ...
    NSString *getPerWindowSettingsCommand =
        [NSString stringWithFormat:@"show -v -q -t $%d @per_window_settings", sessionId_];
...
- (NSMutableSet *)attachedSessionGuids {
    ...
    dispatch_once(&onceToken, ^{
        // Typically a few attached sessions at most
        gAttachedSessionGuids = [[NSMutableSet alloc] initWithCapacity:4];
    });
...
- (void)getSessionGuidResponse:(NSString *)sessionGuid {
    if (!sessionGuid.length) {
        ...
        NSString *command = [NSString stringWithFormat:@"set -t $%d @iterm2_id \"%@\"", sessionId_, guid];
...
- (void)registerSession:(PTYSession<iTermTmuxControllerSession> *)aSession
    ...
    ITCriticalError(tab != nil, @"nil tab for session %@ with delegate %@ with realparentwindow %@", aSession,
                    aSession.delegate, aSession.delegate.realParentWindow);
    if (tab) {
        [self retainWindow:window withTab:tab];
        [windowPanes_ setObject:aSession forKey:[self _keyForWindowPane:windowPane]];
        void (^call)(PTYSession<iTermTmuxControllerSession> *) = _when[iTermTmuxIdToNumber(windowPane)];
        if (call) {
            dispatch_async(dispatch_get_main_queue(), ^{
                call(aSession);
            });
            [_when removeObjectForKey:iTermTmuxIdToNumber(windowPane)];
        }
...
- (void)whenPaneRegistered:(int)wp call:(void (^)(PTYSession<iTermTmuxControllerSession> *))block {
    ...
    _when[iTermTmuxIdToNumber(wp)] = [block copy];
}

- (PTYTab *)window:(int)window {
    return _windowStates[iTermTmuxIdToNumber(window)].tab;
}
...
- (void)detach {
    ...
    [self closeAllPanes];
    gateway_ = nil;
    [_when enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
                                               void (^_Nonnull obj)(PTYSession<iTermTmuxControllerSession> *),
                                               BOOL *_Nonnull stop) {
        obj(nil);
    }];
    [_when removeAllObjects];
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerDetachedNotification object:self];
...
- (void)fitLayoutToVariableSizeWindows {
    [self setWindowSizes:[_windowStates.allKeys
                             mapWithBlock:^iTermTuple<NSString *, NSValue *> *(NSNumber *windowNumber) {
...
- (NSArray<NSDictionary *> *)commandListToSetWindowSizes:(NSArray<iTermTuple<NSString *, NSValue *> *> *)windowSizes {
    return [windowSizes mapWithBlock:^NSDictionary *(iTermTuple<NSString *, NSValue *> *tuple) {
        ...
        if ([self refreshClientSupportsWindowArgument]) {
            command = [NSString
                stringWithFormat:@"refresh-client -C @%d:%dx%d", window.intValue, (int)size.width, (int)size.height];
        } else {
            command = [NSString stringWithFormat:@"resize-window -x %@ -y %@ -t @%d", @((int)size.width),
                                                 @((int)size.height), window.intValue];
        }
...
- (NSArray<NSDictionary *> *)commandListToSetSize:(NSSize)size ofWindows:(NSArray<NSString *> *)windows {
    return
        [self commandListToSetWindowSizes:[windows mapWithBlock:^iTermTuple<NSString *, NSValue *> *(NSString *window) {
...
- (int)adjustHeightForStatusBar:(int)height {
    ...
    NSArray *buggyVersions =
        @[ [NSDecimalNumber decimalNumberWithString:@"2.9"], [NSDecimalNumber decimalNumberWithString:@"2.91"] ];
...
- (void)setClientSize:(NSSize)size {
    ...
    // Validate size is positive to prevent invalid tmux commands (upstream #12158)
    if (size.width <= 0 || size.height <= 0) {
        DLog(@"Ignoring invalid client size: %@", NSStringFromSize(size));
        return;
    }
    lastSize_ = size;
    NSString *listStr = [self commandToListWindows];
    NSString *setSizeCommand =
        [NSString stringWithFormat:@"set -t $%d @iterm2_size %d,%d", sessionId_, (int)size.width, (int)size.height];
    const int height = [self adjustHeightForStatusBar:(int)size.height];
    ITBetaAssert(height > 0, @"Invalid size");
    [_windowSizes removeAllObjects];
    NSArray *commands =
        [NSArray arrayWithObjects:[gateway_ dictionaryForCommand:setSizeCommand
                                                  responseTarget:nil
                                                responseSelector:nil
                                                  responseObject:nil
                                                           flags:0],
...
- (void)sendReport:(NSData *)report pane:(int)pane {
    ...
    NSString *escapedReport = [[report stringWithEncoding:NSUTF8StringEncoding] stringByEscapingForTmux];
...
- (void)enablePauseModeIfPossible {
    ...
    if (gateway_.minimumServerVersion &&
        [gateway_.minimumServerVersion compare:[NSDecimalNumber decimalNumberWithString:@"3.2"]] ==
            NSOrderedAscending) {
        DLog(@"min < 3.2");
        return;
    }
    ...
    [gateway_ sendCommand:[NSString stringWithFormat:@"refresh-client -fpause-after=%@", @(age)]
           responseTarget:nil
         responseSelector:nil];
    _tmuxBufferMonitor = [[iTermTmuxBufferSizeMonitor alloc] initWithController:self pauseAge:age];
...
- (void)pausePanes:(NSArray<NSNumber *> *)wps {
    ...
    [self.gateway sendCommand:command
               responseTarget:self
             responseSelector:@selector(didPause:panes:)
               responseObject:wps
                        flags:0];
}
...
- (void)didPause:(NSString *)result panes:(NSArray<NSNumber *> *)wps {
    for (NSNumber *wp in wps) {
        [self.gateway.delegate tmuxWindowPaneDidPause:wp.intValue notification:NO];
    }
}
...
- (void)handleListKeys:(NSString *)response {
    ...
    NSArray<NSDictionary *> *dicts = [lines mapWithBlock:^id(NSString *_Nonnull line) {
        ...
        NSString *flag = nil;
        // Typically 2-4 flags per key binding
        NSMutableDictionary<NSString *, id> *flags = [NSMutableDictionary dictionaryWithCapacity:4];
        ...
        return @{@"key" : key, @"command" : command, @"flags" : flags};
    }];
    NSArray<NSString *> *forbiddenCommands = @[
        @"bind-key",      @"choose-buffer",   @"choose-client",  @"choose-tree",   @"clear-history",
        @"clock-mode",    @"command-prompt",  @"confirm-before", @"copy-mode",     @"customize-mode",
        @"display-menu",  @"display-message", @"display-panes",  @"display-popup", @"find-window",
        @"list-buffers",  @"list-clients",    @"list-commands",  @"list-keys",     @"list-panes",
        @"list-sessions", @"list-windows",    @"show-buffer",    @"show-messages", @"unbind-key",
    ];
...
- (void)guessVersion {
    ...
    NSArray *commands = @[
        [gateway_ dictionaryForCommand:@"display-message -p \"#{version}\""
                        responseTarget:self
...
- (void)decreaseMaximumServerVersionTo:(NSString *)string {
    ...
    if (!gateway_.maximumServerVersion || [gateway_.maximumServerVersion compare:number] == NSOrderedDescending) {
...
- (void)increaseMinimumServerVersionTo:(NSString *)string {
    NSDecimalNumber *number = [NSDecimalNumber decimalNumberWithString:string];
    if (!gateway_.minimumServerVersion || [gateway_.minimumServerVersion compare:number] == NSOrderedAscending) {
...
- (void)checkForUTF8Response:(NSString *)response {
    if ([response containsString:@"_"]) {
        [gateway_ abortWithErrorMessage:@"tmux is not in UTF-8 mode. Please pass the -u command line argument to tmux "
                                        @"or change your LANG environment variable to end with “.UTF-8”."
...
- (void)handleDisplayMessageVersion:(NSString *)response {
    ...
    DLog(@"number=%@", number);
    if (number.doubleValue != number.doubleValue || number.doubleValue < 2.4 || number.doubleValue > 10) {
    ...
    if (gateway_.minimumServerVersion.doubleValue >= 2.9 &&
        [iTermAdvancedSettingsModel tmuxVariableWindowSizesSupported]) {
...
- (void)guessVersion22Response:(NSString *)response {
    ...
    const NSInteger index =
        [response rangeOfCharacterFromSet:[[NSCharacterSet whitespaceAndNewlineCharacterSet] invertedSet]].location;
    if (index == NSNotFound) {
...
- (void)optionValidationFailedForOption:(NSString *)option {
    NSString *message = [NSString stringWithFormat:@"The \"%@\" option is turned on in tmux. "
                                                    "It is not compatible with the DashTerm2-tmux integration. "
                                                    "Please disable it and try again.",
                                                   option];
    [gateway_ abortWithErrorMessage:message title:@"Unsupported tmux option"];
}
...
- (void)windowPane:(int)wp resizedBy:(int)amount horizontally:(BOOL)wasHorizontal {
    NSString *dir;
    if (wasHorizontal) {
        if (amount > 0) {
            dir = @"R";
    ...
    NSString *resizeStr = [NSString stringWithFormat:@"resize-pane -%@ -t \"%%%d\" %d", dir, wp, abs(amount)];
    NSString *listStr = [self commandToListWindows];
    NSArray *commands = [NSArray arrayWithObjects:[gateway_ dictionaryForCommand:resizeStr
                                                                  responseTarget:nil
                                                                responseSelector:nil
                                                                  responseObject:nil
                                                                           flags:0],
...
// The splitVertically parameter uses the DashTerm2 conventions.
        ...
        tmuxSplitWindowCommand:wp
            ...
            recyclingSupported:self.recyclingSupported
                    ...
                    completion:^(NSString *command) {
                        ...
                        NSString *listPanesCommand =
                            [NSString stringWithFormat:@"list-panes -t %%%d -F '#{pane_id}'", wp];
                        // Split state holds completion and pane lists
                        NSMutableDictionary *state = [NSMutableDictionary dictionaryWithCapacity:4];
                        ...
                        NSDictionary *split = [gateway dictionaryForCommand:command
                                                           ...
                                                           responseSelector:nil
                                                             responseObject:nil
                                                                      flags:kTmuxGatewayCommandShouldTolerateErrors];
                        NSDictionary *followupListPanes =
                            [gateway dictionaryForCommand:listPanesCommand
                                           responseTarget:self
...
- (void)selectPane:(int)windowPane {
    ...
    if ([gateway_.minimumServerVersion isEqual:version2_9]) {
        // I presume this will be fixed in whatever verson follows 2.9, so use an isEqual:. I need to remember to
        // revisit this after the bug is fixed!
        return;
    }
...
- (void)didCreateWindowWithCommand:(NSString *)command {
    // Typically 1-2 commands (optional set-size + new-window)
    NSMutableArray *commands = [NSMutableArray arrayWithCapacity:2];
    if (_variableWindowSize) {
        Profile *profile = self.sharedProfile;
        NSSize size = NSMakeSize(MIN(iTermMaxInitialSessionSize, [profile[KEY_COLUMNS] intValue] ?: 80),
                                 MIN(iTermMaxInitialSessionSize, [profile[KEY_ROWS] intValue] ?: 25));
        ITBetaAssert((int)size.width > 0, @"Invalid size");
        const int height = [self adjustHeightForStatusBar:size.height];
        ITBetaAssert(height > 0, @"Invalid size");
        [_windowSizes removeAllObjects];
        NSString *setSizeCommand = [NSString stringWithFormat:@"refresh-client -C %d,%d", (int)size.width, height];
...
- (void)didCreateWindowWithAffinity:(NSString *)windowIdString
    ...
    [commands addObject:[gateway_ dictionaryForCommand:command
                                        responseTarget:self
                                      responseSelector:@selector(newWindowWithAffinityCreated:
                                                                  affinityWindowAndCompletion:)
...
- (void)movePane:(int)srcPane intoPane:(int)destPane isVertical:(BOOL)splitVertical before:(BOOL)addBefore {
    [gateway_ sendCommand:[NSString stringWithFormat:@"move-pane -s \"%%%d\" -t \"%%%d\" %@%@", srcPane, destPane,
                                                     splitVertical ? @"-h" : @"-v", addBefore ? @" -b" : @""]
           responseTarget:nil
         responseSelector:nil];
}
...
- (void)setWindowTitleOverride:(NSString *)title window:(int)windowId {
    [self renameWindowWithId:windowId inSessionNumber:nil toName:title];
    [self savePerTabSettings];
}

- (void)renameWindowWithId:(int)windowId inSessionNumber:(NSNumber *)sessionNumber toName:(NSString *)newName {
    NSString *theCommand;
    if (sessionNumber) {
        theCommand =
            [NSString stringWithFormat:@"rename-window -t \"$%d:@%d\" \"%@\"", sessionNumber.intValue, windowId,
                                       [self stringByEscapingBackslashesAndRemovingNewlines:newName]];
    } else {
        theCommand = [NSString stringWithFormat:@"rename-window -t @%d \"%@\"", windowId,
                                                [self stringByEscapingBackslashesAndRemovingNewlines:newName]];
    }
    [gateway_ sendCommand:theCommand responseTarget:nil responseSelector:nil];
}
...
- (void)renamePane:(int)windowPane toTitle:(NSString *)newTitle {
    ...
    NSString *theCommand = [NSString stringWithFormat:@"select-pane -t %%%d -T \"%@\"", windowPane,
                                                      [self stringByEscapingBackslashesAndRemovingNewlines:newTitle]];
    [gateway_ sendCommand:theCommand responseTarget:nil responseSelector:nil];
}

- (void)setHotkeyForWindowPane:(int)windowPane to:(NSDictionary *)dict {
    _hotkeys[iTermTmuxIdToNumber(windowPane)] = dict;

    // First get a list of existing panes so we can avoid setting hotkeys for any nonexistent panes. Keeps the string
...
- (void)setTabColorString:(NSString *)colorString forWindowPane:(int)windowPane {
    if ([_tabColors[iTermTmuxIdToNumber(windowPane)] isEqualToString:colorString]) {
        return;
    }
    _tabColors[iTermTmuxIdToNumber(windowPane)] = colorString;

    // First get a list of existing panes so we can avoid setting tab colors for any nonexistent panes. Keeps the string
    // from getting too long.
    NSString *getPaneIDsCommand = [NSString stringWithFormat:@"list-panes -s -t $%d -F \"#{pane_id}\"", sessionId_];
    [gateway_ sendCommand:getPaneIDsCommand
           responseTarget:self
...
- (void)getPaneIDsResponseAndSetHotkeys:(NSString *)response {
    ...
    for (NSString *pane in [response componentsSeparatedByString:@"\n"]) {
        if (pane.length > 1) { // BUG-1082: Require at least 2 chars for substringFromIndex:1
...
- (void)sendCommandToSetHotkeys {
    ...
    NSString *command = [NSString stringWithFormat:@"set -t $%d @hotkeys \"%@\"", sessionId_, hexEncoded];
...
- (void)sendCommandToSetTabColors {
    ...
    NSString *command = [NSString stringWithFormat:@"set -t $%d @tab_colors \"%@\"", sessionId_,
                                                   [self encodedString:[self.tabColorsString stringByEscapingQuotes]
...
- (NSDictionary *)hotkeyForWindowPane:(int)windowPane {
    return _hotkeys[iTermTmuxIdToNumber(windowPane)];
}

- (NSString *)tabColorStringForWindowPane:(int)windowPane {
    return _tabColors[iTermTmuxIdToNumber(windowPane)];
}

- (void)killWindow:(int)window {
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermTmuxControllerWillKillWindow
                                                        object:iTermTmuxIdToNumber(window)];
...
- (void)breakOutWindowPane:(int)windowPane toPoint:(NSPoint)screenPoint {
    [windowPositions_ setObject:[NSValue valueWithPoint:screenPoint] forKey:[NSNumber numberWithInt:windowPane]];
...
- (void)windowPaneBrokeOutWithWindowId:(NSString *)windowId setAffinityTo:(NSString *)windowGuid {
...
- (void)setWindowID:(int)wid buriedFromTerminalGUID:(NSString *)terminalGUID tabIndex:(int)tabIndex {
    ...
    if (!wids) {
        // Usually just a few buried windows per terminal
        wids = [NSMutableArray arrayWithCapacity:4];
...
- (void)hideWindows:(NSArray<NSNumber *> *)windowIDs andCloseTabs:(BOOL)closeTabs {
    ...
    if (closeTabs) {
        ...
        [windowIDs enumerateObjectsUsingBlock:^(NSNumber *_Nonnull wid, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            [self setWindowID:wid.intValue
                buriedFromTerminalGUID:terminalGUID
                              tabIndex:[self tabIndexOfWindowID:wid.intValue]];
        }];
    }
...
- (void)openWindowWithId:(int)windowId
    ...
    [_buriedWindows enumerateKeysAndObjectsUsingBlock:^(
                        NSString *_Nonnull terminalGUID,
                        NSMutableArray<iTermTuple<NSNumber *, NSNumber *> *> *_Nonnull tuples, BOOL *_Nonnull stop) {
        const NSInteger i = [tuples indexOfObjectPassingTest:^BOOL(iTermTuple<NSNumber *, NSNumber *> *_Nonnull obj,
    ...
    [gateway_ sendCommand:[NSString
                              stringWithFormat:@"display -p -F %@ -t @%d", [self listWindowsDetailedFormat], windowId]
           responseTarget:self
...
- (void)openWindowWithId:(int)windowId intentional:(BOOL)intentional profile:(Profile *)profile {
    [self openWindowWithId:windowId affinities:@[] intentional:intentional profile:profile];
}

- (void)linkWindowId:(int)windowId inSessionNumber:(int)sessionNumber toSessionNumber:(int)targetSessionNumber {
    [gateway_ sendCommand:[NSString stringWithFormat:@"link-window -s \"$%d:@%d\" -t \"$%d:+\"", sessionNumber,
                                                     windowId, targetSessionNumber]
           responseTarget:nil
         responseSelector:nil];
}

- (void)moveWindowId:(int)windowId inSessionNumber:(int)sessionNumber toSessionNumber:(int)targetSessionNumber {
    [gateway_ sendCommand:[NSString stringWithFormat:@"move-window -s \"$%d:@%d\" -t \"$%d:+\"", sessionNumber,
                                                     windowId, targetSessionNumber]
           responseTarget:nil
         responseSelector:nil];
}
...
- (void)renameSessionNumber:(int)sessionNumber to:(NSString *)newName {
    NSString *renameCommand = [NSString
        stringWithFormat:@"rename-session -t \"$%d\" \"%@\"", sessionNumber, [newName stringByEscapingQuotes]];
...
- (void)addSessionWithName:(NSString *)sessionName {
    NSString *attachCommand =
        [NSString stringWithFormat:@"new-session -s \"%@\"", [sessionName stringByEscapingQuotes]];
    [gateway_ sendCommand:attachCommand responseTarget:nil responseSelector:nil];
    [self listSessions];
}
...
- (void)listWindowsInSessionNumber:(int)sessionNumber target:(id)target selector:(SEL)selector object:(id)object {
    if (detached_ || !object) {
        // This can happen if you're not attached to a session.
        return;
    }
    NSString *listWindowsCommand =
        [NSString stringWithFormat:@"list-windows -F %@ -t \"$%d\"", [self listWindowsDetailedFormat], sessionNumber];
    NSArray *userInfo = @[ listWindowsCommand, object, target, NSStringFromSelector(selector) ];
    ...
    [gateway_ sendCommand:command
         ...
         responseSelector:@selector(didListWindows:userData:)
           responseObject:@[ object, selector, target ]
                    flags:kTmuxGatewayCommandShouldTolerateErrors]; // Tolerates errors because the session may have
                                                                    // been detached by the time we get the notification
                                                                    // or the timer fires.
}
...
- (void)saveHiddenWindows {
    ...
    NSString *command = [NSString stringWithFormat:@"set -t $%d @hidden \"%@\"", sessionId_,
...
- (void)saveWindowOrigins {
    ...
    // One map entry per equivalence class
    NSMutableArray *maps = [NSMutableArray arrayWithCapacity:[affinities_ classes].count];
    for (NSSet *c in [affinities_ classes]) {
        ...
        NSMutableArray *temp = [NSMutableArray arrayWithCapacity:c.count];
        PTYTab *tab = nil;
        ...
        if (tab) {
            ...
            [maps addObject:[NSString stringWithFormat:@"%@:%d,%d", windowIds, (int)origin.x, (int)origin.y]];
        }
    }
    NSString *enc = [maps componentsJoinedByString:@" "];
    DLog(@"Save window origins to %@ called from %@", enc, [NSThread callStackSymbols]);
    NSString *command = [NSString stringWithFormat:@"set -t $%d @origins \"%@\"", sessionId_,
    ...
    if (!lastOrigins_ || ![command isEqualToString:lastOrigins_]) {
        ...
        [gateway_ sendCommand:command responseTarget:self responseSelector:@selector(saveWindowOriginsResponse:)];
    }
...
- (NSString *)windowOptionsForTerminal:(PseudoTerminal *)term {
    if (term.anyFullScreen) {
        return [NSString stringWithFormat:@"%@=%@", kTmuxWindowOpenerWindowOptionStyle,
                                          kTmuxWindowOpenerWindowOptionStyleValueFullScreen];
    } else {
        return @"";
    }
}
...
- (void)saveAffinities {
    ...
    // One affinity entry per terminal
    NSMutableArray *affinities = [NSMutableArray arrayWithCapacity:terminals.count];
    for (PseudoTerminal *term in terminals) {
        // Tabs plus guid plus buried windows
        NSMutableArray *siblings = [NSMutableArray arrayWithCapacity:[term tabs].count + 4];
        ...
        if (siblings.count > 0) {
            NSString *value = [NSString stringWithFormat:@"%@;%@", [siblings componentsJoinedByString:@","],
    ...
    NSString *command = [NSString stringWithFormat:@"set -t $%d @affinities \"%@\"", sessionId_,
                                                   [self encodedString:[arg stringByEscapingQuotes]
...
- (void)saveBuriedIndexes {
    ...
    NSString *command = [NSString stringWithFormat:@"set -t $%d @buried_indexes \"%@\"", sessionId_,
                                                   [self encodedString:[arg stringByEscapingQuotes]
...
- (void)savePerTabSettings {
    iTermController *cont = [iTermController sharedInstance];
    NSArray *terminals = [cont terminals];
    // Estimate total tabs across all terminals
    NSMutableArray<NSString *> *settings = [NSMutableArray arrayWithCapacity:terminals.count * 4];
    for (PseudoTerminal *term in terminals) {
        for (PTYTab *tab in term.tabs) {
            if (!tab.isTmuxTab) {
                continue;
    ...
    NSString *command =
        [NSString stringWithFormat:@"set -t $%d @per_tab_settings \"%@\"", sessionId_,
...
- (void)getPerTabSettingsResponse:(NSString *)result {
    [self setPerTabSettingsFromString:[self decodedString:result
                                           optionalPrefix:iTermTmuxControllerEncodingPrefixPerTabSettings]];
}

- (void)savePerWindowSettings {
    iTermController *cont = [iTermController sharedInstance];
    NSArray *terminals = [cont terminals];
    // One setting per terminal
    NSMutableArray<NSString *> *settings = [NSMutableArray arrayWithCapacity:terminals.count];
    for (PseudoTerminal *term in terminals) {
    ...
    NSString *command =
        [NSString stringWithFormat:@"set -t $%d @per_window_settings \"%@\"", sessionId_,
...
- (void)getPerWindowSettingsResponse:(NSString *)result {
    [self setPerWindowSettingsFromString:[self decodedString:result
                                              optionalPrefix:iTermTmuxControllerEncodingPrefixPerWindowSettings]];
}
...
- (void)changeWindow:(int)window tabTo:(PTYTab *)tab {
    _windowStates[iTermTmuxIdToNumber(window)].tab = tab;
}
...
- (void)listSessions {
    ...
    listSessionsTimer_ = nil;
    NSString *listSessionsCommand = @"list-sessions -F \"#{session_id} #{session_name}\"";
    [gateway_ sendCommand:listSessionsCommand responseTarget:self responseSelector:@selector(listSessionsResponse:)];
}

- (NSString *)listWindowsDetailedFormat {
    NSArray<NSString *> *parts = @[
        @"#{session_name}", @"#{window_id}", @"#{window_name}", @"#{window_width}", @"#{window_height}",
        @"#{window_layout}", @"#{window_flags}", @"#{?window_active,1,0}"
    ];
    if ([self versionAtLeastDecimalNumberWithString:@"2.2"]) {
...
- (void)swapPane:(int)pane1 withPane:(int)pane2 {
    NSString *swapPaneCommand = [NSString stringWithFormat:@"swap-pane -s \"%%%d\" -t \"%%%d\"", pane1, pane2];

    NSArray *commands = @[
        [gateway_ dictionaryForCommand:swapPaneCommand
                        responseTarget:nil
...
- (void)toggleZoomForPane:(int)pane {
    NSArray *commands = @[
        [gateway_ dictionaryForCommand:[NSString stringWithFormat:@"resize-pane -Z -t \"%%%d\"", pane]
                        responseTarget:nil
                      responseSelector:NULL
                        responseObject:nil
                                 flags:0],
...
- (void)setTmuxFontTable:(iTermFontTable *)fontTable hSpacing:(CGFloat)hs vSpacing:(CGFloat)vs window:(int)window {
    NSDictionary *dict = iTermTmuxControllerMakeFontOverrides(fontTable, hs, vs);
    if (_variableWindowSize) {
        _windowStates[iTermTmuxIdToNumber(window)].fontOverrides = dict;
        return;
    }
...
- (void)setLayoutInWindow:(int)window toLayout:(NSString *)layout {
    NSArray *commands = @[
        [gateway_ dictionaryForCommand:[NSString stringWithFormat:@"select-layout -t @%@ %@", @(window), layout]
                        responseTarget:self
...
- (void)getOriginsResponse:(NSString *)encodedResult {
    NSString *result = [self decodedString:encodedResult optionalPrefix:iTermTmuxControllerEncodingPrefixOrigins];
    ...
    if ([result length] > 0) {
        ...
        for (NSString *wstr in windows) {
            ...
            NSPoint origin = NSMakePoint([[xy objectAtIndex:0] intValue], [[xy objectAtIndex:1] intValue]);
            for (NSString *wid in windowIds) {
                [origins_ setObject:[NSValue valueWithPoint:origin] forKey:[NSNumber numberWithInt:[wid intValue]]];
            }
        }
    }
}
...
- (NSString *)hotkeysString {
    NSMutableArray *parts = [NSMutableArray arrayWithCapacity:_hotkeys.count];
    [_hotkeys
        enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, NSDictionary *_Nonnull obj, BOOL *_Nonnull stop) {
...
- (NSString *)tabColorsString {
    NSMutableArray *parts = [NSMutableArray arrayWithCapacity:_tabColors.count];
    [_tabColors
        enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, NSString *_Nonnull obj, BOOL *_Nonnull stop) {
            if ([_paneIDs containsObject:key]) {
...
- (void)getTabColorsResponse:(NSString *)encodedResult {
    ...
    NSString *result = [self decodedString:encodedResult optionalPrefix:iTermTmuxControllerEncodingPrefixTabColors];
    BOOL haveRed = NO;
    ...
    if (haveRed) {
        const iTermWarningSelection selection = [iTermWarning
            showWarningWithTitle:
                @"While attaching to tmux, some tabs were found whose color is pure red. This may be due to a bug in "
                @"early versions of DashTerm2 3.6.x. Would you like to reset those tabs' colors?"
...
- (void)getHiddenWindowsResponse:(NSString *)encodedResponse {
    NSString *response = [self decodedString:encodedResponse optionalPrefix:iTermTmuxControllerEncodingPrefixHidden];
...
- (void)getAffinitiesResponse:(NSString *)result {
    [self setAffinitiesFromString:[self decodedString:result
                                       optionalPrefix:iTermTmuxControllerEncodingPrefixAffinities]];
}
...
- (NSDictionary *)windowOptionsFromString:(NSString *)kvpString {
    ...
    NSMutableDictionary *flags = [NSMutableDictionary dictionaryWithCapacity:kvps.count];
...
- (void)setPerTabSettingsFromString:(NSString *)result {
    ...
    NSMutableDictionary<NSString *, NSString *> *settings = [NSMutableDictionary dictionaryWithCapacity:parts.count];
    for (NSString *part in parts) {
    ...
    for (NSString *theString in affinities) {
        ...
        for (NSString *widString in siblings) {
            if (![widString isEqualToString:exemplar]) {
                ...
                [affinities_ setValue:widString equalToValue:exemplar];
...
- (void)listSessionsResponse:(NSString *)result {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerSessionsWillChange object:nil];
    self.sessionObjects =
        [[result componentsSeparatedByRegex:@"\n"] mapWithBlock:^iTermTmuxSessionObject *(NSString *line) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerSessionsDidChange object:self];
}
...
- (void)listedWindowsToOpenOne:(NSString *)response forWindowIdAndAffinities:(NSArray *)values {
    ...
    if (!doc) {
        [gateway_
            abortWithErrorMessage:[NSString stringWithFormat:@"Bad response for list windows request: %@", response]];
        return;
    }
    for (NSArray *record in doc.records) {
...
// When an DashTerm2 window is resized, a control -s client-size w,h
...
- (void)parseListWindowsResponseAndUpdateLayouts:(NSString *)response {
    ...
    // One tab per layout string
    NSMutableArray<PTYTab *> *tabs = [NSMutableArray arrayWithCapacity:layoutStrings.count];
    ...
    for (NSString *layoutString in layoutStrings) {
        ...
        NSArray *components =
            [layoutString captureComponentsMatchedByRegex:@"^@([0-9]+) ([^ ]+)(?: ([^ ]+)(?: ([^ ]+))?)?"];
        if ([components count] < 3) {
            ...
            if (tab) {
                ...
                const BOOL adjust = [[gateway_ delegate] tmuxUpdateLayoutForWindow:window
                                                                            layout:layout
...
- (void)retainWindow:(int)window withTab:(PTYTab *)tab {
    // Defensive nil check - tab should never be nil but handle gracefully (upstream #12158)
    if (!tab) {
        DLog(@"retainWindow called with nil tab for window %d", window);
        return;
    }
    ...
    if (notify) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerWindowDidOpen object:@[ k, self ]];
    }
}
...
- (void)releaseWindow:(int)window {
    ...
    state.refcount = state.refcount - 1;
    if (!state.refcount) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerWindowDidClose object:@[ k, self ]];
        [_windowStates removeObjectForKey:k];
    }
}

// Called only for DashTerm2-initiated new windows/tabs.
- (void)newWindowWithAffinityCreated:(NSString *)responseStr
    ...
    if ([responseStr hasPrefix:@"@"]) {
        ...
        _pendingWindows[@(intWindowId)] = [iTermTmuxPendingWindow withIndex:tuple.thirdObject completion:completion];
        NSString *affinityWindow = tuple.firstObject;
        if (affinityWindow) {
            [affinities_ setValue:windowId equalToValue:affinityWindow];
        } else {
...
- (void)windowDidOpen:(TmuxWindowOpener *)windowOpener {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kTmuxControllerWindowDidOpen object:nil];
...
- (void)responseForListWindows:(NSString *)response toSetPartialOrder:(NSArray *)partialOrder {
    ...
    NSMutableArray *currentOrder = [NSMutableArray arrayWithCapacity:partialOrder.count];
    ...
    NSMutableArray *desiredOrder = [NSMutableArray arrayWithCapacity:partialOrder.count];
    ...
    NSMutableArray *commands = [NSMutableArray arrayWithCapacity:currentOrder.count];
    for (int i = 0; i < currentOrder.count; i++) {
        if ([currentOrder[i] intValue] != [desiredOrder[i] intValue]) {
            NSInteger swapIndex = [currentOrder indexOfObject:desiredOrder[i]];
            // Skip if object not found - can happen during race conditions (upstream #12158)
            if (swapIndex == NSNotFound) {
                DLog(@"swapIndex not found for desiredOrder[%d]=%@ in currentOrder", i, desiredOrder[i]);
                continue;
            }

            NSString *command =
                [NSString stringWithFormat:@"swap-window -s @%@ -t @%@", currentOrder[i], currentOrder[swapIndex]];
            NSDictionary *dict = [gateway_ dictionaryForCommand:command
                                                 responseTarget:self
...
- (void)setEncodedUserVars:(NSString *)encodedUserVars forPane:(int)paneID {
    NSString *decoded =
        [self decodedString:encodedUserVars optionalPrefix:iTermTmuxControllerEncodingPrefixUserVars] ?: @"";
...
- (NSMutableDictionary<NSString *, NSString *> *)mutableUserVarsForPane:(int)paneID {
    ...
    // Typically a small number of user variables per pane
    dict = [NSMutableDictionary dictionaryWithCapacity:8];
    _userVars[@(paneID)] = dict;
    return dict;
}

- (void)setUserVariableWithKey:(NSString *)key value:(NSString *)value pane:(int)paneID {
    ...
    NSString *command = [NSString stringWithFormat:@"set -p -t %%%d @uservars \"%@\"", paneID,
