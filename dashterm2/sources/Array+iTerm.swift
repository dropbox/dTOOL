...
extension NSArray {
    @objc var longestCommonStringPrefix: String {
        ...
        // BUG-1579: Use guard to safely access .first instead of force unwrap
        guard length > 0, let firstString = converted.first else {
            return ""
        }
        let firstStringAsCollection = AnyCollection(firstString)
...
extension Array where Element: RandomAccessCollection, Element.Index == Int, Element.Element: Comparable {
    var lengthOfLongestCommonPrefix: Int {
        ...
        while true {
            ...
            let prefix = firstElement[0..<trying]
...
extension Array where Element: Collection, Element.Element: Comparable {
    var lengthOfLongestCommonPrefix: Int {
        ...
        let firstCollection = AnyCollection(firstElement)
        var i = 0
        while i < 1024 {
            let trying = i + 1
    ...
    var longestCommonPrefix: [Element.Element] {
        let length = lengthOfLongestCommonPrefix
        // BUG-1579: Use guard to safely access .first instead of force unwrap
        guard length > 0, let firstElement = self.first else {
            return []
        }
        let firstCollection = AnyCollection(firstElement)
...
extension Array {
    subscript (indexes: IndexSet) -> [Element] {
        // BUG-530: Filter out-of-bounds indices to prevent crash
...
extension Array {
    func get(_ index: Index, default defaultValue: Element) -> Element {
        // BUG-f989: Add check for negative index to prevent crash
        // Swift Array indices are >= startIndex (0), so negative is out of bounds
