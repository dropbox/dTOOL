...
- (instancetype)initWithQueue:(dispatch_queue_t)queue indexURL:(NSURL *)indexURL {
    self = [super init];
    if (self) {
        _queue = queue;
...
- (BOOL)saveSynchronously:(BOOL)synchronously withCompletion:(void (^)(void))completion {
    NSMutableArray<iTermRestorableStateRecord *> *recordsToKeepUnchanged =
        [NSMutableArray arrayWithCapacity:_records.count];
    NSMutableArray<iTermRestorableStateRecord *> *recordsNeedingNewContent =
        [NSMutableArray arrayWithCapacity:_records.count];
    if ([iTermRestorableStateController stateRestorationEnabled]) {
        ...
        for (NSWindow *window in [self.delegate restorableStateWindows]) {
            iTermRestorableStateRecord *record =
                [_records objectPassingTest:^BOOL(iTermRestorableStateRecord *element, NSUInteger index, BOOL *stop) {
    ...
    if (synchronously) {
        dispatch_sync(_queue, ^{
            [weakSelf writeUnchanged:recordsToKeepUnchanged update:recordsNeedingNewContent completion:completion];
        });
    } else {
...
- (void)writeUnchanged:(NSArray<iTermRestorableStateRecord *> *)recordsToKeepUnchanged
    ...
    NSArray<NSURL *> *fileURLsToKeep = [[recordsToKeepUnchanged arrayByAddingObjectsFromArray:recordsNeedingNewContent]
        mapWithBlock:^id(iTermRestorableStateRecord *record) {
            return record.url;
        }];
    ...
    [self encryptAndSaveUnchangedRecords:recordsToKeepUnchanged updatedRecords:recordsNeedingNewContent];
...
- (void)eraseFilesExcept:(NSArray<NSURL *> *)fileURLsToKeep {
    ...
    NSDirectoryEnumerator *enumerator =
        [[NSFileManager defaultManager] enumeratorAtURL:[NSURL fileURLWithPath:savedState]
...
- (void)encryptAndSaveUnchangedRecords:(NSArray<iTermRestorableStateRecord *> *)recordsToKeepUnchanged
    ...
    [recordsNeedingNewContent
        enumerateObjectsUsingBlock:^(iTermRestorableStateRecord *_Nonnull record, NSUInteger idx, BOOL *_Nonnull stop) {
            DLog(@"Write new content for %@", @(record.windowNumber));
            [record save];
            [plist addObject:record.indexEntry];
        }];
    [recordsToKeepUnchanged
        enumerateObjectsUsingBlock:^(iTermRestorableStateRecord *_Nonnull record, NSUInteger idx, BOOL *_Nonnull stop) {
            [plist addObject:record.indexEntry];
        }];
    DLog(@"Save index:\n%@", plist);
