...
static NSString *const iTermStatusBarLayoutDictionaryKeySeparatorColor =
    @"separator color 2"; // "separator color" is deprecated after 3.0.0beta1
    ...
    if (self) {
        self.separatorColor =
            [[aDecoder decodeObjectOfClass:[NSDictionary class]
                                    forKey:iTermStatusBarLayoutDictionaryKeySeparatorColor] colorValue];
        self.backgroundColor =
            [[aDecoder decodeObjectOfClass:[NSDictionary class]
                                    forKey:iTermStatusBarLayoutDictionaryKeyBackgroundColor] colorValue];
        self.defaultTextColor =
            [[aDecoder decodeObjectOfClass:[NSDictionary class]
                                    forKey:iTermStatusBarLayoutDictionaryKeyDefaultTextColor] colorValue];
        ...
        self.font = [[aDecoder decodeObjectOfClass:[NSString class] forKey:iTermStatusBarLayoutDictionaryKeyFont]
            fontValueWithLigaturesEnabled:YES];
...
- (NSDictionary *)dictionaryValue {
    NSDictionary *dict = @{
        iTermStatusBarLayoutDictionaryKeySeparatorColor : [self.separatorColor dictionaryValue] ?: [NSNull null],
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:[self.backgroundColor dictionaryValue]
                  forKey:iTermStatusBarLayoutDictionaryKeyBackgroundColor];
    [aCoder encodeObject:[self.defaultTextColor dictionaryValue]
                  forKey:iTermStatusBarLayoutDictionaryKeyDefaultTextColor];
...
+ (NSArray<id<iTermStatusBarComponent>> *)componentsArrayFromDictionary:(NSDictionary *)dictionary
    ...
    NSArray *components = dictionary[iTermStatusBarLayoutKeyComponents];
    NSMutableArray<id<iTermStatusBarComponent>> *result = [NSMutableArray arrayWithCapacity:components.count];
    for (NSDictionary *dict in components) {
        ...
        configuration = [configuration
            dictionaryBySettingObject:dictionary[iTermStatusBarLayoutKeyAdvancedConfiguration] ?: @{}
                               forKey:
                                   iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue];
        id<iTermStatusBarComponent> component = [[theClass alloc] initWithConfiguration:configuration scope:scope];
        [result addObject:component];
    }
    return result;
}

- (instancetype)initWithDictionary:(NSDictionary *)layout scope:(nullable iTermVariableScope *)scope {
    return [self initWithComponents:[iTermStatusBarLayout componentsArrayFromDictionary:layout scope:scope]
              advancedConfiguration:
                  [iTermStatusBarAdvancedConfiguration
                      advancedConfigurationFromDictionary:layout[iTermStatusBarLayoutKeyAdvancedConfiguration]]];
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    return [self initWithComponents:[aDecoder decodeObjectOfClass:[NSArray class] forKey:@"components"]
              advancedConfiguration:[aDecoder decodeObjectOfClass:[iTermStatusBarAdvancedConfiguration class]
                                                           forKey:@"advanced configuration"]];
}

- (void)setComponents:(NSArray<id<iTermStatusBarComponent>> *)components {
    _components = [components copy];
    // BUG-1496: Guard against nil delegate (safe in ObjC but explicit check is cleaner)
    id<iTermStatusBarLayoutDelegate> delegate = self.delegate;
    if (delegate) {
        [delegate statusBarLayoutDidChange:self];
    }
}
...
- (NSArray *)arrayValue {
    return [_components mapWithBlock:^id(id<iTermStatusBarComponent> component) {
        return @{
            iTermStatusBarLayoutKeyClass : NSStringFromClass([component class]),
...
- (NSDictionary *)dictionaryValue {
    return [@{
        iTermStatusBarLayoutKeyComponents : [self arrayValue],
