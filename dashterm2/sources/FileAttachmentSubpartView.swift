...
class FileAttachmentSubpartView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f838: Return nil instead of crashing for unused coder initializer
        DLog("FileAttachmentSubpartView init(coder:) is not supported")
        return nil
    }
    ...
    @objc private func initiateDrag(with timer: Timer) {
        // BUG-1674: Use guard with as? instead of force cast for timer userInfo
        guard let event = timer.userInfo as? NSEvent else {
            return
        }
        ...
        guard let filePath = attachment.localPathCreatingIfNeeded() else {
            DLog("Cannot create local path for attachment")
            return
        }
