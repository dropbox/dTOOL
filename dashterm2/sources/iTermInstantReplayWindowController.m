...
- (void)awakeFromNib {
    _fractions = [[NSMutableArray alloc] initWithCapacity:32];  // Instant replay event fractions
}
...
- (void)updateEvents {
    // BUG-f1067: Replace asserts with guards - nil views/delegate should return early not crash
    if (!_eventsView || !_delegate) {
        DLog(@"BUG-f1067: updateEvents called with nil eventsView or delegate");
        return;
    }
    ...
    // BUG-1269: Guard against division by zero when span is 0 (single frame)
    if (_span == 0) {
        [_eventsView.fractions addObject:@0.0];
    ...
    for (long long i = _firstTimestamp; i > 0 && i <= lastTimestamp; i = [_delegate instantReplayTimestampAfter:i]) {
        NSNumber *fraction = @((double)(i - _firstTimestamp) / (double)_span);
...
- (void)setState:(iTermInstantReplayState)destinationState byCancelling:(BOOL)cancel {
    if (_state == iTermInstantReplayStateSetStart &&
        ...
        // BUG-1269: Guard against division by zero
        _eventsView.startFraction = (_span > 0) ? (double)(_start - _firstTimestamp) / (double)_span : 0.0;
...
- (void)updateInstantReplayView {
    ...
    if (_state == iTermInstantReplayStateSetEnd) {
        // BUG-1269: Guard against division by zero
        _eventsView.endFraction = (_span > 0) ? (double)(timestamp - _firstTimestamp) / (double)_span : 0.0;
        [_eventsView setNeedsDisplay:YES];
