        ...
        init(from decoder: Decoder) throws {
            ...
            if container.contains(.input) {
                ...
                if let dict = inputValue.value as? [String: Any] {
                    ...
                    // BUG-1056: Log warning when input is not a dictionary instead of silent data loss
                    DLog("AnthropicToolUseContent: input field is not a dictionary, using empty dict. Actual type: \(Swift.type(of: inputValue.value))")
                    input = [:]
                }
            } else {
                input = [:]
