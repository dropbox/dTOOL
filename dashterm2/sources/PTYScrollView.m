    ...
    if (self.scrollerStyle != scrollerStyle && scrollerStyle == NSScrollerStyleOverlay) {
        ...
        if (preferredSuperview) {
            ...
            if (preferredSuperview != scrollview && scrollview != nil) {
                DLog(@"Scroller style changing to overlay. Remove self from %@, add to %@", preferredSuperview,
                     scrollview);
...
- (void)setScrollerStyle:(NSScrollerStyle)scrollerStyle {
    DLog(@"%@: set scroller style to %@ from %@:\n%@", self, @(scrollerStyle), @(self.scrollerStyle),
...
- (void)setFrame:(NSRect)frame {
    DLog(@"%@: set frame to %@ from %@\n%@", self, NSStringFromRect(frame), NSStringFromRect(self.frame),
         [NSThread callStackSymbols]);
...
- (void)setFrameSize:(NSSize)newSize {
    DLog(@"%@: set frame size to %@ from %@\n%@", self, NSStringFromSize(newSize), NSStringFromSize(self.frame.size),
         [NSThread callStackSymbols]);
...
- (void)viewDidChangeEffectiveAppearance {
    DLog(@"Appearance of %@ is now %@ from %@", self, self.effectiveAppearance, [NSThread callStackSymbols]);
...
- (instancetype)initWithFrame:(NSRect)frame hasVerticalScroller:(BOOL)hasVerticalScroller {
    ...
    if (self) {
        ...
        // BUG-f1368: Convert assert to guard - contentView should exist but return nil if not
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(it_scrollViewDidScroll:)
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"%@ visibleRect:%@", [super description], [NSValue valueWithRect:[self documentVisibleRect]]];
}
...
- (void)scrollWheel:(NSEvent *)event {
    if (self.hasVerticalScroller && ![iTermAdvancedSettingsModel fastTrackpad]) {
        ...
        DLog(@"Scroll by %@ lines, each with height %@, to scroll from %@ to %@", @(amount), @(self.verticalLineScroll),
             NSStringFromRect(self.documentVisibleRect), NSStringFromRect(scrollRect));
...
+ (NSSize)frameSizeForContentSize:(NSSize)cSize
            hasHorizontalScroller:(BOOL)hFlag
              hasVerticalScroller:(BOOL)vFlag
                       ...
                       rightExtra:(CGFloat)rightExtra
    API_DEPRECATED("Use "
                   "+frameSizeForContentSize:horizontalScrollerClass:verticalScrollerClass:borderType:controlSize:"
                   "scrollerStyle: instead",
                   macos(10.0, 10.7)) {
    NSSize size = [super frameSizeForContentSize:cSize
