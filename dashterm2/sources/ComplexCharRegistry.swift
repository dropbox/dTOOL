...
private class ComplexCharRegistryImpl: NSObject {
    ...
    func convertToGraphics(chars: UnsafeMutablePointer<screen_char_t>, count: Int) {
        ...
        for i in 0..<count {
            // BUG-f507: Skip already-complex characters instead of crashing
            guard chars[i].complexChar == 0 else {
                DLog("convertToGraphics: Character at \(i) already complex, skipping")
                continue
            }
    ...
    func lazilyCreatedCode(for string: NSString,
        ...
        @unknown default:
            // BUG-f848: Handle unknown iTermTriState values gracefully instead of crashing
            DLog("Unknown iTermTriState value in isSpacingCombiningMark, treating as .other")
            if string.rangeOfCharacter(from: NSCharacterSet.spacingCombiningMarks(forUnicodeVersion: 12) as CharacterSet).location != NSNotFound {
                spacingCombiningMarkCodeNumbers.insert(number)
            }
        }
