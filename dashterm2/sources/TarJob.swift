...
struct TarJob: CustomDebugStringConvertible {
    ...
    private func adding(local: URL, destination destinationParent: URL) -> TarJob? {
        ...
        do {
            ...
            guard let firstSourceComponents = splitSources.first,
                  let firstDestinationComponents = splitDestinations.first,
                  let firstDestinationPath = destinations.first?.pathComponents else {
                DLog("Missing components needed to evaluate tar job candidates")
                return nil
            }
            ...
            for (source, dest) in zip(splitSources, splitDestinations) {
                DLog("Check source=\(source), dest=\(dest), preserving source prefix \(firstSourceComponents.prefix(sourcePrefixCount))")
                ...
                if dest.count - sourceSuffix.count > destinationPrefixCount {
                    let prefixSliceEnd = max(0, dest.count - sourceSuffix.count)
                    let preservedPrefix = prefixSliceEnd > 0 ? Array(dest[0..<prefixSliceEnd]) : []
                    let canonicalPrefix = firstDestinationPath.prefix(destinationPrefixCount)
                    DLog("FAIL - destination (\(dest)) after stripping source suffix (\(sourceSuffix)), yielding \(preservedPrefix) is longer than the common destination prefix \(canonicalPrefix)")
                    return nil
                }
                DLog("OK - destination \(dest) ends with source suffix \(sourceSuffix)")
            }
            guard firstSourceComponents.count >= sourcePrefixCount else {
                DLog("FAIL - split sources \(firstSourceComponents) count (\(firstSourceComponents.count)) < sourcePrefixCount \(sourcePrefixCount)")
                return nil
            }
            let replacement = TarJob(sources: sources + [local],
                                     localBase: URL(fileURLWithPath: sourcePrefix),
                                     destinationBase: URL(fileURLWithPath: firstDestinationComponents.dropFirst().dropLast(firstSourceComponents.count - sourcePrefixCount).joined(separator: "/")))
