...
func _SELog(_ messageBlock: () -> String, file: String, line: Int, function: String) {
    ...
    // BUG-1673: Use guard with fallback instead of force unwrap for cString
    guard let fileC = file.cString(using: .utf8),
          let funcC = function.cString(using: .utf8) else {
        return
    }
    DebugLogImpl(fileC, Int32(line), funcC, message)
#endif
}
...
class iTermSearchRequest: NSObject {
    ...
    func realize(dataSource: iTermSearchEngineDataSource) -> SearchRequest {
        ...
        // BUG-2895: Don't crash on unknown mode - use sensible default
        @unknown default:
            .insensitive
        }
...
class SearchOperation: Pausable {
    ...
    struct Positions: CustomDebugStringConvertible {
        ...
        // BUG-f520: Log and continue instead of crashing when stop position is nil
        mutating func updateStopPosition(snapshot: TerminalContentSnapshot, request: SearchRequest) {
            if let position = request.stopPosition(lineBuffer: snapshot.lineBuffer,
                ...
                // BUG-f672: Log warning and continue instead of crashing - stopPosition returning nil is unexpected but not fatal
                DLog("BUG-f672: stopPosition returned nil - this shouldn't happen but continuing")
            }
        }
    }
    ...
    func searchOnce() {
        ...
        case .Searching:
            ...
            // keep looking
            break
        // BUG-f521: Handle unknown search status gracefully
        @unknown default:
            DLog("Warning: Unknown search context status, finishing search")
            finish()
        }
    }
    ...
    private func wrap() {
        SELog("wrap")
        // BUG-1621: Use guard instead of precondition - wrapping twice should just return
        guard !context.hasWrapped else {
            SELog("Warning: wrap() called but context.hasWrapped is true")
            return
        }
        guard !positions.wrapped else {
            SELog("Warning: wrap() called but positions.wrapped is true")
            return
        }
        ...
        // BUG-f522, BUG-f673: Log and finish instead of crashing when wrapped positions fail
        guard let updated else {
            SELog("BUG-f522: Failed to create wrapped positions - finishing search gracefully")
            finish()
            return
        }
    ...
    func updateSnapshot(snapshot: TerminalContentSnapshot) {
        ...
        switch state {
        case .ready, .canceled:
            // BUG-437: Replace it_fatalError with early return - updateSnapshot in invalid state should not crash
            DLog("SearchOperation.updateSnapshot called in invalid state \(state). Ignoring.")
            return
        case .searching:
            break
        case .finished:
    ...
    /// RC-036 ROOT CAUSE FIX: Added assertion when type cast fails.
    /// context.results is declared as NSMutableArray<ResultRange*> in ObjC.
    /// If the cast fails, it's a programming error (array contains wrong type).
    private var results: [ResultRange] {
        guard let contextResults = context.results else {
            return []
        }
        guard let typedResults = contextResults as? [ResultRange] else {
            // BUG-438: Replace it_fatalError with empty return - type mismatch should not crash
            DLog("SearchOperation.results: context.results contains non-ResultRange objects: \(type(of: contextResults.first as Any)). Returning empty array.")
            return []
        }
        return typedResults
    }
...
class SearchEngine: Pausable {
    ...
    private static let regularQueue = DispatchQueue(label: "com.dashterm.dashterm2.search")
    private let queue: MockableQueue
    private(set) var snapshot: TerminalContentSnapshot?
