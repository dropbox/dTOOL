...
+ (NSString *)signatureForFunctionCallInvocation:(NSString *)invocation error:(out NSError *__autoreleasing *)error {
    iTermVariableRecordingScope *permissiveScope =
        [[iTermVariableRecordingScope alloc] initWithScope:[[iTermVariableScope alloc] init]];
    permissiveScope.neverReturnNil = YES;
    iTermParsedExpression *expression = [[iTermExpressionParser callParser] parse:invocation scope:permissiveScope];
    switch (expression.expressionType) {
        ...
        case iTermParsedExpressionTypeArrayOfValues:
            if (error) {
                *error =
                    [NSError errorWithDomain:@"com.dashterm.dashterm2.call"
                                        code:3
        ...
        case iTermParsedExpressionTypeFunctionCalls:
            // callParser returns FunctionCalls (plural) even for a single call.
            // Handle single-call sequences by extracting the signature.
            if (expression.functionCalls.count == 1) {
                return expression.functionCalls.firstObject.signature;
            }
            if (error) {
                *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.call"
                                             code:3
        ...
        case iTermParsedExpressionTypeInterpolatedString:
            if (error) {
                *error = [NSError
                    errorWithDomain:@"com.dashterm.dashterm2.call"
                               code:3
                           userInfo:@{
                               NSLocalizedDescriptionKey : @"Expected function call, not an interpolated string"
                           }];
            }
            return nil;
    }
    // BUG-12003: Replace assert(NO) with proper error return for unknown expression types
    // This handles new enum values gracefully in release builds
    if (error) {
        *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.call"
                                     code:7
                                 userInfo:@{NSLocalizedDescriptionKey : @"Unknown expression type"}];
    }
    return nil;
}
...
- (id)initWithStart:(NSString *)start {
    ...
    if (self) {
        ...
        // BUG-f865: Replace assert with graceful failure - return nil if parser creation fails
        if (!_parser) {
            DLog(@"BUG-f865: Failed to create parser for start symbol '%@'", start);
            return nil;
        }
...
- (void)addSwiftyStringRecognizers {
    iTermSwiftyStringRecognizer *left = [[iTermSwiftyStringRecognizer alloc] initWithStartQuote:@"\""
                                                                                       endQuote:@"\""
                                                                                 escapeSequence:@"\\"
...
- (iTermParsedExpression *)
    parsedExpressionForFunctionCallWithFullyQualifiedName:(NSString *)fqName
    ...
    for (iTermFunctionArgument *arg in argsArray) {
        ...
        [call addParameterWithName:arg.name parsedExpression:arg.expression];
    }
    if (error) {
        *error = nil;
    }
...
- (iTermParsedExpression *)parsedExpressionWithReferenceToPath:(NSString *)path {
    if (![_scope userWritableContainerExistsForPath:path]) {
        return [[iTermParsedExpression alloc]
            initWithErrorCode:3
                       reason:[NSString
                                  stringWithFormat:
                                      @"Can’t form reference to non-existent or read-only container for variable %@",
                                      path]];
    }
    iTermVariableReference *ref = [[iTermVariableReference alloc] initWithPath:path vendor:_scope];
...
- (iTermParsedExpression *)parsedExpressionWithValue:(id)value
    ...
    if ([value conformsToProtocol:@protocol(iTermExpressionParserPlaceholder)]) {
        return [[iTermParsedExpression alloc] initWithPlaceholder:value optional:optional];
    }
    ...
    if (optional) {
        ...
        fallbackError = [NSString
            stringWithFormat:
                @"Reference to undefined variable “%@”. Change it to “%@?” to treat the undefined value as null.", path,
                path];
        return [[iTermParsedExpression alloc] initWithObject:value errorReason:fallbackError];
    }
}

+ (iTermParsedExpression *)parsedExpressionWithInterpolatedStringParts:
    (NSArray<iTermParsedExpression *> *)interpolatedParts {
    NSArray<iTermParsedExpression *> *coalesced = [interpolatedParts
        reduceWithFirstValue:@[]
                       block:^id(NSArray<iTermParsedExpression *> *arraySoFar, iTermParsedExpression *expression) {
                           if (expression.expressionType == iTermParsedExpressionTypeString && arraySoFar.lastObject &&
                               arraySoFar.lastObject.expressionType == iTermParsedExpressionTypeString) {
                               NSString *concatenated =
                                   [arraySoFar.lastObject.string stringByAppendingString:expression.string];
                               iTermParsedExpression *combined =
                                   [[iTermParsedExpression alloc] initWithString:concatenated];
...
+ (iTermParsedExpression *)parsedExpressionWithInterpolatedString:(NSString *)swifty scope:(iTermVariableScope *)scope {
...
+ (iTermParsedExpression *)parsedExpressionWithInterpolatedString:(NSString *)swifty
    ...
    // Interpolated strings typically have 2-10 parts
    NSMutableArray *interpolatedParts = [NSMutableArray arrayWithCapacity:8];
    [swifty enumerateSwiftySubstrings:^(NSUInteger index, NSString *substring, BOOL isLiteral, BOOL *stop) {
        ...
        iTermParsedExpression *expression = [parser parse:substring scope:scope];
        ...
        if (!strict && [iTermAdvancedSettingsModel laxNilPolicyInInterpolatedStrings] &&
            ...
            iTermParsedExpression *expressionWithPlaceholders =
                [parser parse:substring scope:[[iTermVariablePlaceholderScope alloc] init]];
...
- (iTermTriple<id, NSString *, NSString *> *)pathOrDereferencedArrayFromPath:(NSString *)path
    ...
    if (_scope.usePlaceholders) {
        ...
        if (indexNumber) {
            placeholder =
                [[iTermExpressionParserArrayDereferencePlaceholder alloc] initWithPath:path
                                                                                 index:indexNumber.integerValue];
        } else {
            placeholder = [[iTermExpressionParserVariableReferencePlaceholder alloc] initWithPath:path];
        }
        return [iTermTriple tripleWithObject:placeholder andObject:nil object:path];
    }
    ...
    if (!array) {
        NSString *reason = [NSString
            stringWithFormat:@"Variable “%@” is of type %@, not array", path, NSStringFromClass([untypedValue class])];
        return [iTermTriple tripleWithObject:nil andObject:reason object:path];
    }

    NSInteger index = indexNumber.integerValue;
    if (index < 0 || index >= array.count) {
        NSString *reason = [NSString stringWithFormat:@"Index %@ out of range of “%@”, which has %@ value%@", @(index),
                                                      path, @(array.count), array.count == 1 ? @"" : @"s"];
        return [iTermTriple tripleWithObject:nil andObject:reason object:path];
    }
...
- (void)loadRulesAndTransforms {
    ...
    [_grammarProcessor addProductionRule:@"callsequence ::= <callsequence> ';' <call>"
                           treeTransform:^id(CPSyntaxTree *syntaxTree) {
                               ...
                               return [weakSelf callSequenceWithCalls:[callSequence.functionCalls
                                                                          arrayByAddingObject:call.functionCall]];
                           }];
    ...
    [_grammarProcessor
        addProductionRule:@"call ::= <path> <arglist>"
            treeTransform:^id(CPSyntaxTree *syntaxTree) {
                NSError *error = nil;
                iTermParsedExpression *result =
                    [weakSelf parsedExpressionForFunctionCallWithFullyQualifiedName:(NSString *)syntaxTree.children[0]
    ...
    [_grammarProcessor addProductionRule:@"args ::= <arg> ',' <args>"
                           treeTransform:^id(CPSyntaxTree *syntaxTree) {
                               return
                                   [@[ syntaxTree.children[0] ] arrayByAddingObjectsFromArray:syntaxTree.children[2]];
                           }];
    [_grammarProcessor
        addProductionRule:@"arg ::= 'Identifier' ':' '&' <path>"
            treeTransform:^id(CPSyntaxTree *syntaxTree) {
                ...
                if ([path isEqualToString:@"null"]) {
                    NSError *error = [NSError errorWithDomain:@"com.dashterm.dashterm2.parser"
                                                         code:2
    ...
    [_grammarProcessor addProductionRule:@"arg ::= 'Identifier' ':' <expression>"
                           treeTransform:^id(CPSyntaxTree *syntaxTree) {
                               return [weakSelf
                                   newFunctionArgumentWithName:[(CPIdentifierToken *)syntaxTree.children[0] identifier]
                                                    expression:syntaxTree.children[2]
                                               passByReference:NO];
                           }];
    // IMPORTANT: Function call rule MUST come before path_or_dereferenced_array rule
    // to resolve the shift-reduce conflict correctly. Both start with <path>, but
    // function calls should take precedence when followed by '(' (arglist).
    ...
    [_grammarProcessor addProductionRule:@"expression ::= 'Number'"
                           treeTransform:^id(CPSyntaxTree *syntaxTree) {
                               return [[iTermParsedExpression alloc]
                                   initWithNumber:[(CPNumberToken *)syntaxTree.children[0] numberValue]];
                           }];
    ...
    [_grammarProcessor
        addProductionRule:@"comma_delimited_expressions ::= <expression> ',' <comma_delimited_expressions>"
            treeTransform:^id(CPSyntaxTree *syntaxTree) {
    ...
    [_grammarProcessor addProductionRule:@"path_or_dereferenced_array ::= <path>" // -> (value, error string, path);
                                                                                  // value of NSNull means undefined.
                           treeTransform:^id(CPSyntaxTree *syntaxTree) {
                               return [weakSelf pathOrDereferencedArrayFromPath:syntaxTree.children[0] index:nil];
                           }];
    [_grammarProcessor
        addProductionRule:@"path_or_dereferenced_array ::= <path> '[' 'Number' ']'" // -> (value, error string, path);
                                                                                    // value of NSNull means undefined.
            treeTransform:^id(CPSyntaxTree *syntaxTree) {
                CPNumberToken *numberToken = syntaxTree.children[2];
                return [weakSelf pathOrDereferencedArrayFromPath:syntaxTree.children[0] index:numberToken.numberValue];
            }];
    ...
    [_grammarProcessor
        addProductionRule:@"path ::= 'Identifier' '.' <path>"
            treeTransform:^id(CPSyntaxTree *syntaxTree) {
                return [NSString stringWithFormat:@"%@.%@", [(CPIdentifierToken *)syntaxTree.children[0] identifier],
...
- (CPRecoveryAction *)parser:(CPParser *)parser
    ...
    NSString *reason = [NSString stringWithFormat:@"Syntax error at index %@ of “%@”. Expected%@: %@",
                                                  ...
                                                  quotedExpected.count > 1 ? @" one of" : @"", expectedString];
    _error = [NSError errorWithDomain:@"com.dashterm.dashterm2.parser"
                                 code:3
