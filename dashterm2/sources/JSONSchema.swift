    ...
    init<T>(for instance: T,
        ...
        for child in mirror.children {
            ...
            if fieldType == "array" {
                ...
                // BUG-1702: Use guard with as? instead of as! for array cast, and check for empty array
                guard let array = child.value as? [Any], let firstElement = array.first else {
                    continue
                }
                let elementType = JSONSchema.extractFieldType(Swift.type(of: firstElement),
                                                              value: firstElement)
                if elementType == "object" {
                    ...
                    var nested = JSONSchema(for: firstElement, descriptions: innerDescriptions)
                    nested.additionalProperties = false
                    // Use safe decoding - fallback to empty dict on encoding failure
                    if let json = try? JSONEncoder().encode(nested),
                       let obj = try? JSONSerialization.jsonObject(with: json, options: []) as? [String: Any] {
                        ...
                        property.items = AnyCodable(["type": "object"])
                    }
                } else {
