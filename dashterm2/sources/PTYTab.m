...
@implementation PTYTab {
    ...
    __weak NSWindowController<iTermWindowController>
        *realParentWindow_; // non-nil only if parent is PseudoTerminal*. Implements optional methods of protocol.
...
+ (NSImage *)imageForNewOutputWithAppearance:(NSAppearance *)appearance {
    ...
    switch ((iTermPreferencesTabStyle)[appearance it_tabStyle:preferredStyle]) {
        ...
        case TAB_STYLE_MINIMAL:
            // BUG-465: Replace assert(NO) with DLog and fallthrough
            // These styles should have been resolved by it_tabStyle
            DLog(@"imageForNewOutputWithAppearance got unresolved tab style");
...
+ (NSImage *)deadImageWithAppearance:(NSAppearance *)appearance {
    ...
    switch ((iTermPreferencesTabStyle)[appearance it_tabStyle:preferredStyle]) {
        ...
        case TAB_STYLE_COMPACT:
        case TAB_STYLE_MINIMAL:
            // BUG-466: Replace assert(NO) with DLog and fallthrough
            // These styles should have been resolved by it_tabStyle
            DLog(@"deadImageWithAppearance got unresolved tab style");
            // Intentional fallthrough
        case TAB_STYLE_LIGHT:
        case TAB_STYLE_LIGHT_HIGH_CONTRAST:
...
+ (NSSize)cellSizeForBookmark:(Profile *)bookmark {
    ...
    font = [ITAddressBookMgr fontWithDesc:[bookmark objectForKey:KEY_NORMAL_FONT]
                         ligaturesEnabled:[iTermProfilePreferences boolForKey:KEY_ASCII_LIGATURES inProfile:bookmark]];
    ...
    return [PTYTextView charSizeForFont:font horizontalSpacing:hspace verticalSpacing:vspace];
}

+ (NSDictionary *)frameToDict:(NSRect)frame {
    return @{
        TAB_X : @(frame.origin.x),
...
+ (NSRect)dictToFrame:(NSDictionary *)dict {
    return NSMakeRect([[dict objectForKey:TAB_X] doubleValue], [[dict objectForKey:TAB_Y] doubleValue],
                      [[dict objectForKey:TAB_WIDTH] doubleValue], [[dict objectForKey:TAB_HEIGHT] doubleValue]);
}

+ (NSString *)htmlNameForColor:(NSColor *)color {
    return [NSString stringWithFormat:@"%02x%02x%02x", (int)(color.redComponent * 255.0),
                                      (int)(color.greenComponent * 255.0), (int)(color.blueComponent * 255.0)];
}
...
+ (NSSize)sizeForTmuxWindowWithAffinity:(NSString *)affinity controller:(TmuxController *)controller {
    if (affinity != nil) {
    ...
    const int columns = MIN(iTermMaxInitialSessionSize, [iTermProfilePreferences intForKey:KEY_COLUMNS
                                                                                 inProfile:profile]);
    const int rows = MIN(iTermMaxInitialSessionSize, [iTermProfilePreferences intForKey:KEY_ROWS inProfile:profile]);
...
- (instancetype)initWithRoot:(NSSplitView *)root sessions:(NSMapTable<SessionView *, PTYSession *> *)sessions {
    self = [super init];
    if (self) {
...
- (void)commonInit {
    ...
    self.viewToSessionMap =
        [[NSMapTable alloc] initWithKeyOptions:NSPointerFunctionsStrongMemory | NSPointerFunctionsObjectPersonality
                                  valueOptions:NSPointerFunctionsStrongMemory | NSPointerFunctionsObjectPersonality
                                      capacity:1];
    _tabNumberForItermSessionId = -1;
    hiddenLiveViews_ = [[NSMutableArray alloc] initWithCapacity:4]; // Typically few hidden views
    _variables = [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextTab owner:self];
    _variables.primaryKey = @"id";
    _userVariables = [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextTab owner:self];
    ...
    self.tmuxWindow = -1;
    _sessionsWithDeferredFontChanges = [[NSMutableArray alloc] initWithCapacity:4]; // Sessions needing font changes
...
- (void)dealloc {
    // Post a notification
    [[NSNotificationCenter defaultCenter] postNotificationName:@"iTermTabClosing" object:self userInfo:nil];
...
- (NSString *)description {
    if (self.tmuxTab) {
        return [NSString stringWithFormat:@"<%@: %p label=%@ objectCount=%@ tmuxWindow=%@ tmuxController=%@>",
                                          NSStringFromClass([self class]), self, tabViewItem_.label, @(objectCount_),
                                          @(self.tmuxWindow), self.tmuxController];
    }
    return [NSString stringWithFormat:@"<%@: %p label=%@ objectCount=%@>", NSStringFromClass([self class]), self,
                                      tabViewItem_.label, @(objectCount_)];
}
...
- (BOOL)updatePaneTitles {
    ...
    const BOOL statusBarsOnTop =
        (([iTermPreferences unsignedIntegerForKey:kPreferenceKeyStatusBarPosition] == iTermStatusBarPositionTop) &&
         perPaneStatusBars);
    const BOOL anySessionHasTopStatusBar = statusBarsOnTop && [sessions anyWithBlock:^BOOL(PTYSession *session) {
                                               return [iTermProfilePreferences boolForKey:KEY_SHOW_STATUS_BAR
    ...
    for (PTYSession *aSession in sessions) {
        const BOOL shouldShowBottomStatusBar = (perPaneStatusBars && !statusBarsOnTop &&
                                                [iTermProfilePreferences boolForKey:KEY_SHOW_STATUS_BAR
                                                                          inProfile:aSession.profile]);
        const BOOL changedTitle = [[aSession view] setShowTitle:shouldShowTitles adjustScrollView:![self isTmuxTab]];
        ...
        if (changedTitle || changedBottomStatusBar) {
            if (![self isTmuxTab]) {
                ...
                if (size.width != [aSession columns] || size.height != [aSession rows]) {
                    anyChange = YES;
                }
            }
        }
...
- (void)updateIcon {
    ...
    } else if ([iTermPreferences boolForKey:kPreferenceKeyShowNewOutputIndicator] && (_state & kPTYTabIdleState)) {
...
- (void)updateTabTitleForCurrentSessionName:(NSString *)newName {
    ...
    [tabViewItem_ setLabel:[self stringByAppendingSubtitleForActiveSession:value]]; // PSM uses bindings to bind the
                                                                                    // label to its title
...
- (void)setActiveSession:(PTYSession *)session updateActivityCounter:(BOOL)updateActivityCounter {
    ...
    if ([[self realParentWindow] currentTab] == self && [[[self realParentWindow] window] isKeyWindow]) {
...
- (NSPoint)rootRelativeOriginOfSession:(PTYSession *)session {
    if (!isMaximized_) {
        return [root_ convertPoint:session.view.frame.origin fromView:session.view.superview];
    } else {
...
- (void)setActiveSessionPreservingMaximization:(PTYSession *)session {
    ...
    // BUG-1363: Check subviews is non-empty before accessing index 0
    if (isMaximized_ && [[root_ subviews] count] > 0) {
        [root_ replaceSubview:[[root_ subviews] objectAtIndex:0] with:[session view]];
    }
...
- (PTYSession *)sessionInDirection:(int)offset {
    DLog(@"offset=%@", @(offset));
    NSArray *orderedSessions = [self orderedSessions];
    // BUG-3121: Guard against modulo by zero and array access on empty array
...
- (void)updateFlexibleViewColors {
    ...
    if (![realParentWindow_ anyFullScreen] && delta.width >= 0 && delta.width < cellSize.width && delta.height >= 0 &&
        delta.height < cellSize.height) {
        DLog(@"Root is slightly smaller than flexible view so use terminal background color for flexible view's "
             @"background");
...
- (BOOL)isProcessing {
    return (![iTermPreferences hideTabActivityIndicator] && isProcessing_ && ![self isForegroundTab]);
}
...
- (NSArray<PTYSession *> *)sessionsAtTop {
    ...
    return [self.sessions
        minimumsWithComparator:^NSComparisonResult(PTYSession *_Nonnull session1, PTYSession *_Nonnull session2) {
...
- (NSArray<PTYSession *> *)sessionsAtBottom {
    ...
    return [self.sessions
        maximumsWithComparator:^NSComparisonResult(PTYSession *_Nonnull session1, PTYSession *_Nonnull session2) {
...
//     C.right < B.left, AND
//
// To select adjacent-right-of instead of adjacent-left-of, multiply use -1*right where it says left and -1*left where
// it says right. To select adjacent-below instead of adjacent-right-of, swap all axes. To select adjacent-above instead
// of adjacent-right-of, first swap all axes and then use -right and -left in place of left and right.
//
// As an example:
//
// +-------+------+----+
...
- (NSArray<PTYSession *> *)sessionsInProjectionOfSession:(PTYSession *)aSession
    ...
    return [self sessionsSatisfying:^BOOL(PTYSession *b) {
        ...
        return (bBottom >= aTop + safetyMargin && bTop + safetyMargin < aBottom && bLeft >= aRight);
    }];
}
...
- (NSArray<PTYSession *> *)sessionsAdjacentToSession:(PTYSession *)aSession
                                         verticalDir:(BOOL)verticalDir
                                               after:(BOOL)after {
    NSArray<PTYSession *> *bCandidates = [self sessionsInProjectionOfSession:aSession
...
- (BOOL)session:(PTYSession *)c
    sitsBetween:(PTYSession *)a
     andSession:(PTYSession *)b
    verticalDir:(BOOL)verticalDir
          after:(BOOL)after {
    if (c == a || c == b) {
        return NO;
    ...
    return (cBottom >= aTop && cTop < aBottom && cBottom >= bTop && cTop < bBottom && cLeft >= aRight &&
            cRight < bLeft);
}

- (PTYSession *)sessionAdjacentTo:(PTYSession *)session verticalDir:(BOOL)verticalDir after:(BOOL)after {
    ...
    if (sessions.count || ![iTermAdvancedSettingsModel wrapFocus]) {
        ...
        NSArray<PTYSession *> *wraparounds =
            [sessions minimumsWithComparator:^NSComparisonResult(PTYSession *a, PTYSession *b) {
...
- (NSArray<NSNumber *> *)windowPanes {
    NSArray *sessions = [self sessions];
    NSMutableArray *panes = [NSMutableArray arrayWithCapacity:sessions.count];
...
- (NSArray<PTYSession *> *)_recursiveSessions:(NSMutableArray<PTYSession *> *)sessions atNode:(NSSplitView *)node {
...
- (void)session:(PTYSession *)synthetic setLiveSession:(PTYSession *)live {
    ...
    [self setFakeParentWindow:[[FakeWindow alloc] initFromRealWindow:realParentWindow_ session:live]];
...
- (void)setFilter:(NSString *)query inSession:(PTYSession *)oldSession {
    ...
    [oldSession.screen performBlockWithJoinedThreads:^(VT100Terminal *terminal, VT100ScreenMutableState *mutableState,
...
- (void)replaceActiveSessionWithSyntheticSession:(PTYSession *)newSession {
    ...
    // BUG-f1057: Replace assert with guard - same session replacement should return early, not crash
    if (oldSession == newSession) {
        ELog(@"replaceActiveSessionWithSyntheticSession: old and new sessions are the same, returning early");
        return;
    }
    ...
    // window size.
    [self setFakeParentWindow:[[FakeWindow alloc] initFromRealWindow:realParentWindow_ session:oldSession]];
...
- (int)tabNumberForItermSessionId {
    ...
    NSMutableSet *tabNumbersInUse = [NSMutableSet setWithCapacity:realParentWindow_.tabs.count];
...
- (void)checkInvariants:(NSSplitView *)node when:(NSString *)when {
    ...
    if (node != root_) {
        if ([node isKindOfClass:[NSSplitView class]]) {
            // 1. A non-root splitview must have at least two children.
            ITCriticalError([[node subviews] count] > 1, @"A non-root splitview must have at least two children. %@",
                            when);
            NSSplitView *parentSplit = (NSSplitView *)[node superview];
            // 3. A non-root splitview's orientation must be the opposite of its parent's.
            ITCriticalError([node isVertical] != [parentSplit isVertical],
                            @"A non-root splitview's orientation must be the opposite of its parent's. %@", when);
        } else {
            if ([[node subviews] count] == 1) {
                ...
                ITCriticalError([onlyChild isKindOfClass:[SessionView class]],
                                @"The root splitview may have exactly one child iff that child is a SessionView. %@",
                                when);
            }
        }
        ...
        if ([onlyChild isKindOfClass:[NSSplitView class]]) {
            if (splitView == root_) {
                ...
                //     [grandchildren]           |
                {
                    ...
                    [newSubviews replaceObjectsInRange:NSMakeRange(i, 1) withObjectsFromArray:onlyChild.subviews];
...
- (void)fitSubviewsToRoot {
    ...
    for (PTYSession *theSession in [self sessions]) {
        ...
        [[theSession.view scrollview] setFrame:NSMakeRect(0, 0, theSize.width, theSize.height)];
...
- (void)sessionDidChangeFontSize:(PTYSession *)session adjustWindow:(BOOL)adjustWindow {
...
- (void)reallyChangeSessionFontSize:(PTYSession *)session adjustWindow:(BOOL)adjustWindow {
    if (adjustWindow && ![[self parentWindow] anyFullScreen] &&
...
- (SessionView *)nearestNeighborOfSession:(PTYSession *)aSession {
    ...
    NSArray<NSView *> *subviews = [parentSplit subviews];
    if ([subviews count] > 1) {
        // Do a depth-first search to find the first descendent of the neighbor that is a
        // SessionView and make it active.
        NSUInteger theIndex = [subviews indexOfObjectIdenticalTo:[aSession view]];
        // BUG-f1027: Guard against NSNotFound (session view not in parent split)
        if (theIndex == NSNotFound) {
            DLog(@"BUG-f1027: Session view not found in parent split subviews");
            return nil;
        }
        NSUInteger neighborIndex = theIndex > 0 ? theIndex - 1 : theIndex + 1;
        // BUG-f1027: Guard against neighborIndex out of bounds
        if (neighborIndex >= [subviews count]) {
            DLog(@"BUG-f1027: neighborIndex %lu out of bounds (count=%lu)", (unsigned long)neighborIndex,
                 (unsigned long)[subviews count]);
            return nil;
        }
        nearestNeighbor = [subviews objectAtIndex:neighborIndex];
...
- (BOOL)canSplitVertically:(BOOL)isVertical withSize:(NSSize)newSessionSize {
    ...
    if (isVertical == [parentSplit isVertical]) {
        ...
        NSSize minSize = [self _minSessionSize:[activeSession_ view] respectPinning:isVertical];
        if (isVertical && actualSize.width < minSize.width) {
            DLog(@"Not enough width for vertical split");
            return NO;
        }
...
- (void)arrangeSplitPanesEvenlyInSplitView:(NSSplitView *)splitView {
    ...
    const NSUInteger subviewCount = splitView.subviews.count;
    if (subviewCount == 0) {
        return;
    }
    CGFloat size;
    if (splitView.vertical) {
        size = splitView.frame.size.width - subviewCount * splitView.dividerThickness;
    } else {
        size = splitView.frame.size.height - subviewCount * splitView.dividerThickness;
    }
    size /= subviewCount;
    CGFloat offset = 0;
...
- (void)splitVertically:(BOOL)isVertical
    ...
    // BUG-f1058: Replace assert with guard - empty parent split should return early, not crash
    if ([[parentSplit subviews] count] == 0) {
        ELog(@"splitVertically: parentSplit has no subviews, returning early");
        return;
    }
    PtyLog(@"Before:");
    [self dump];
    ...
    if ([[parentSplit subviews] count] == 1) {
        ...
        // BUG-f1059: Replace assert with guard - non-root single child should return early, not crash
        if (parentSplit != root_) {
            ELog(@"splitVertically: parentSplit is not root but has only one child, returning early");
            return;
        }

        // Set its orientation to vertical and add the new view.
        [parentSplit setVertical:isVertical];
        [parentSplit addSubview:newView positioned:before ? NSWindowBelow : NSWindowAbove relativeTo:targetSessionView];
...
+ (NSSize)_sessionSizeWithCellSize:(NSSize)cellSize
    ...
    DLog(@"    term=%@, hasScrollbar=%@, scrollerStyle=%@, NSScroller.preferredScrollerStyle=%@", term, @(hasScrollbar),
         @([term scrollerStyle]), @([NSScroller preferredScrollerStyle]));
...
- (NSSize)_minSessionSize:(SessionView *)sessionView respectPinning:(BOOL)respectPinning {
    ...
    size.width = kVT100ScreenMinColumns * [[session textview] charWidth] +
                 [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2;
    size.height = kVT100ScreenMinRows * [[session textview] lineHeight] +
...
- (NSSize)_recursiveMinSize:(NSSplitView *)node respectPinning:(BOOL)respectPinning {
    NSSize size;
    size.width = 0;
    size.height = 0;
    ...
    for (id subview in [node subviews]) {
        ...
        if ([subview isKindOfClass:[NSSplitView class]]) {
            // Get size of child tree at this subview.
            subviewSize = [self _recursiveMinSize:(NSSplitView *)subview respectPinning:respectPinning];
        } else {
            // Get size of session at this subview.
            SessionView *sessionView = (SessionView *)subview;
            subviewSize = [self _minSessionSize:sessionView respectPinning:respectPinning];
...
- (void)_drawSession:(PTYSession *)session inImage:(NSImage *)viewImage atOrigin:(NSPoint)origin {
    ...
    [viewImage lockFocus];
    [textviewImage drawAtPoint:origin fromRect:NSZeroRect operation:NSCompositingOperationSourceOver fraction:1];
...
- (void)_recursiveDrawSplit:(NSSplitView *)splitView inImage:(NSImage *)viewImage atOrigin:(NSPoint)splitOrigin {
    ...
    for (NSView *subview in [splitView subviews]) {
        if (first) {
            ...
            NSRectFill(NSMakeRect(origin.x, myHeight - origin.y - (dy + hy), dx + hx, dy + hy));
            [viewImage unlockFocus];
...
- (NSSize)_recursiveRecompact:(NSSplitView *)splitView {
    ...
    return (isVertical ? NSMakeSize(offset - dividerThickness, maxAgainstGrain)
...
- (NSSize)sessionSizeForViewSize:(PTYSession *)aSession {
    ...
    [aSession setScrollBarVisible:[parentWindow_ scrollbarShouldBeVisible] style:[parentWindow_ scrollerStyle]];
    ...
    const NSSize cellSize =
        NSMakeSize(MAX(1.0, [[aSession textview] charWidth]), MAX(1.0, [[aSession textview] lineHeight]));
    ...
    PtyLog(@"PTYTab sessionSizeForViewSize: view is %fx%f, set screen to %dx%d", size.width, size.height, width,
           height);
...
- (BOOL)resizeSession:(PTYSession *)aSession toSize:(VT100GridSize)newSize {
    if ([aSession rows] == newSize.height && [aSession columns] == newSize.width) {
...
- (CGFloat)_positionOfDivider:(int)theIndex inSplitView:(NSSplitView *)splitView {
    ...
    // BUG-f1028: Guard against theIndex out of bounds
    // Clamp theIndex to valid range to prevent crash
    int maxIndex = (int)[subviews count] - 1;
    if (theIndex > maxIndex) {
        DLog(@"BUG-f1028: theIndex %d out of bounds (max=%d)", theIndex, maxIndex);
        theIndex = maxIndex;
    }
    if (theIndex < 0) {
        // Nothing to sum for negative index
        return 0;
    }
...
- (NSSize)_minSizeOfView:(NSView *)view respectPinning:(BOOL)respectPinning {
    if ([view isKindOfClass:[SessionView class]]) {
        ...
        return [self _minSessionSize:sessionView respectPinning:respectPinning];
    } else {
        return [self _recursiveMinSize:(NSSplitView *)view respectPinning:respectPinning];
    }
}
...
- (double)blurRadius {
    ...
    for (PTYSession *session in sessions) {
        if ([[[session profile] objectForKey:KEY_BLUR] boolValue]) {
            sum += [[session profile] objectForKey:KEY_BLUR_RADIUS]
                       ? [[[session profile] objectForKey:KEY_BLUR_RADIUS] floatValue]
                       : 2.0;
            ++count;
        }
    }
...
- (void)recheckBlur {
    ...
    if ([realParentWindow_ currentTab] == self && ![[realParentWindow_ window] isMiniaturized]) {
        if ([self blur]) {
...
+ (BOOL)_recursiveEncodeArrangementForView:(NSView *)view
        ...
        [encoder encodeArrayWithKey:SUBVIEWS
                        identifiers:index.keys
                         generation:iTermGenerationAlwaysEncode
                              block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder, NSInteger i,
                                          NSString *_Nonnull identifier, BOOL *stop) {
    ...
    [encoder
        encodeDictionaryWithKey:TAB_ARRANGEMENT_SESSION
                     generation:iTermGenerationAlwaysEncode
                          block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                              return [session
                                  encodeArrangementWithContents:contents
                                                        encoder:encoder
                                             replacementProfile:options[PTYTabArrangementOptionsReplacementProfile]
                                                    saveProgram:[options[PTYTabArrangementOptionsReplacementSaveProgram]
                                                                        ?: @YES boolValue]
                                                   pendingJumps:options[PTYTabArrangementOptionsPendingJumps]
                                                        options:options];
                          }];
    encoder[TAB_ARRANGEMENT_IS_ACTIVE] =
        @(session == activeSession || options[PTYTabArrangementOptionsOnlySessionID] != nil);
...
+ (void)_recursiveDrawArrangementPreview:(NSDictionary *)arrangement frame:(NSRect)frame dark:(BOOL)dark {
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        ...
        if (isVerticalSplitter) {
            ...
            dx = frame.size.width / subviewCount;
            pw = dx;
            ph = yExtent;
        } else {
            xExtent = frame.size.width;
            dy = frame.size.height / subviewCount;
            pw = xExtent;
            ph = dy;
        }
    ...
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        ...
        if (idMap || sessionMap) {
            ...
            if (tabArrangementId && idMap[tabArrangementId]) {
                // Exiting a maximized-pane state, so we can get a session view from theMap, a map from arrangement id
                // -> SessionView* where arrangement IDs are stored in the arrangement dict.
...
+ (NSDictionary *)recursiveModifiedArrangementNode:(NSDictionary *)arrangement
    ...
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        NSArray *subviewArrangements = arrangement[SUBVIEWS];
        NSMutableArray *repairedSubviews = [NSMutableArray arrayWithCapacity:subviewArrangements.count];
        for (NSDictionary<NSString *, id> *subArrangement in subviewArrangements) {
            [repairedSubviews addObject:[PTYTab recursiveModifiedArrangementNode:subArrangement mutator:mutator]];
        }
        NSMutableDictionary *result = [arrangement mutableCopy];
        result[SUBVIEWS] = repairedSubviews;
        return result;
    } else {
        NSDictionary *repairedSession = [PTYSession modifiedArrangement:arrangement[TAB_ARRANGEMENT_SESSION]
                                                                mutator:mutator];
        NSMutableDictionary *result = [arrangement mutableCopy];
...
+ (NSDictionary *)recursiveRepairedArrangementNode:(NSDictionary *)arrangement
    ...
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        ...
        NSDictionary *repairedSession = [PTYSession repairedArrangement:arrangement[TAB_ARRANGEMENT_SESSION]
                                               replacingProfileWithGUID:badGuid
                                                            withProfile:goodProfile];
        NSMutableDictionary *result = [arrangement mutableCopy];
        result[TAB_ARRANGEMENT_SESSION] = repairedSession;
...
+ (NSDictionary *)recursiveFindSessionArrangementWithGUID:(NSString *)sessionGUID node:(NSDictionary *)arrangement {
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        for (NSDictionary<NSString *, id> *subArrangement in arrangement[SUBVIEWS]) {
...
+ (NSDictionary *)recursiveRepairedArrangementNode:(NSDictionary *)arrangement
    ...
    if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        NSArray *subviews = arrangement[SUBVIEWS];
        NSArray *repairedSubviews = [subviews mapWithBlock:^id(NSDictionary<NSString *, id> *subArrangement) {
            return [PTYTab recursiveRepairedArrangementNode:subArrangement profileMutator:profileMutator];
        }];
        return [arrangement dictionaryBySettingObject:repairedSubviews forKey:SUBVIEWS];
    }
...
+ (NSDictionary *)recursiveRepairedArrangementNode:(NSDictionary *)arrangement settingCustomLocale:(NSString *)lang {
    if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        NSArray *subviews = arrangement[SUBVIEWS];
        NSArray *repairedSubviews = [subviews mapWithBlock:^id(NSDictionary<NSString *, id> *subArrangement) {
            return [PTYTab recursiveRepairedArrangementNode:subArrangement settingCustomLocale:lang];
        }];
        return [arrangement dictionaryBySettingObject:repairedSubviews forKey:SUBVIEWS];
    }
...
- (PTYSession *)_recursiveRestoreSessions:(NSDictionary<NSString *, id> *)arrangement
                       ...
                       partialAttachments:(NSDictionary *)partialAttachments
                                  options:(NSDictionary *)options {
    if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        // BUG-f1128: Replace assert with guard - view must be NSSplitView for splitter arrangement
        if (![view isKindOfClass:[NSSplitView class]]) {
            ELog(@"BUG-f1128: _recursiveRestoreSessions expected NSSplitView but got %@",
        ...
        // BUG-f1129: Replace assert with guard - view must be SessionView for session arrangement
        if (![view isKindOfClass:[SessionView class]]) {
            ELog(@"BUG-f1129: _recursiveRestoreSessions expected SessionView but got %@",
                 NSStringFromClass([view class]));
            return nil;
        }
        ...
        NSString *uniqueId = [PTYSession guidInArrangement:arrangement[TAB_ARRANGEMENT_SESSION]];
        PTYSession *session;
        if (uniqueId && [self sessionForSessionView:sessionView]) { // Yes - ensures we only reuse if session exists
...
+ (void)drawArrangementPreview:(NSDictionary *)arrangement frame:(NSRect)frame dark:(BOOL)dark {
    [PTYTab _recursiveDrawArrangementPreview:[arrangement objectForKey:TAB_ARRANGEMENT_ROOT] frame:frame dark:dark];
}

- (NSArray *)_recursiveSplittersFromNode:(NSSplitView *)node splitters:(NSArray<NSSplitView *> *)splitters {
...
- (void)replaceWithContentsOfTab:(PTYTab *)tabToGut {
    ...
    for (SessionView *sessionView in tabToGut.viewToSessionMap) {
        [self.viewToSessionMap setObject:[tabToGut.viewToSessionMap objectForKey:sessionView] forKey:sessionView];
    }
    [tabToGut.viewToSessionMap removeAllObjects];
}

- (void)enableFlexibleView {
    // BUG-f1130: Replace assert with guard - flexibleView must not already exist
    if (flexibleView_) {
        DLog(@"BUG-f1130: enableFlexibleView called when flexibleView already exists - ignoring");
        return;
    }
    // Interpose a vew between the tab and the root so the root can be smaller than the tab.
    flexibleView_ = [[iTermFlexibleView alloc] initWithFrame:root_.frame color:[self flexibleViewColor]];
...
static os_unfair_lock sPartialAttachmentsLock = OS_UNFAIR_LOCK_INIT;

+ (void)_recursiveOpenPartialAttachments:(NSDictionary *)arrangement completion:(void (^)(NSDictionary *))completion {
    if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        ...
        [PTYSession openPartialAttachmentsForArrangement:arrangement[TAB_ARRANGEMENT_SESSION] completion:completion];
    }
}

+ (void)openPartialAttachmentsForArrangement:(NSDictionary *)arrangement
                                  completion:(void (^)(NSDictionary *))completion {
    [self _recursiveOpenPartialAttachments:arrangement[TAB_ARRANGEMENT_ROOT] completion:completion];
}
...
+ (PTYTab *)tabWithArrangement:(NSDictionary *)arrangement
            ...
            partialAttachments:(NSDictionary *)partialAttachments
              reservedTabGUIDs:(NSSet<NSString *> *)reservedTabGUIDs
                       options:(NSDictionary *)options {
    PTYTab *theTab;
    // Small initial capacity - revived sessions are typically few per tab
    NSMutableArray<PTYSession *> *revivedSessions = [NSMutableArray arrayWithCapacity:4];
    // Build a tree with splitters and SessionViews but no PTYSessions.
    NSSplitView *newRoot =
        (NSSplitView *)[PTYTab _recursiveRestoreSplitters:[arrangement objectForKey:TAB_ARRANGEMENT_ROOT]
                                                fromIdMap:viewMap
                                               sessionMap:sessionMap
                                          revivedSessions:revivedSessions];
    ...
    if (guid) {
        if ([[iTermController sharedInstance] tabWithGUID:guid] || [reservedTabGUIDs containsObject:guid]) {
...
+ (NSDictionary *)modifiedArrangement:(NSDictionary *)arrangement mutator:(NSDictionary * (^)(NSDictionary *))mutator {
    NSDictionary *newRoot = [PTYTab recursiveModifiedArrangementNode:arrangement[TAB_ARRANGEMENT_ROOT] mutator:mutator];
    NSMutableDictionary *result = [arrangement mutableCopy];
...
+ (NSDictionary *)arrangementForSessionWithGUID:(NSString *)sessionGUID inArrangement:(NSDictionary *)arrangement {
    return [PTYTab recursiveFindSessionArrangementWithGUID:sessionGUID node:arrangement[TAB_ARRANGEMENT_ROOT]];
}

// This can only be used in conjunction with
// +[tabWithArrangement:inTerminal:hasFlexibleView:viewMap:].
- (void)didAddToTerminal:(NSWindowController<iTermWindowController> *)term withArrangement:(NSDictionary *)arrangement {
    ...
    if (tabColor) {
        ...
        [session setSessionSpecificProfileValues:@{
            [session amendedColorKey:KEY_TAB_COLOR] : [tabColor dictionaryValue],
...
+ (BOOL)encodeArrangementNodeWithContents:(BOOL)includeContents
    ...
    if ([node[TAB_ARRANGEMENT_VIEW_TYPE] isEqual:VIEW_TYPE_SPLITTER]) {
        ...
        iTermOrderedDictionary<NSString *, NSDictionary *> *index = [iTermOrderedDictionary
            byMapping:node[SUBVIEWS]
                block:^id _Nonnull(NSUInteger i, NSDictionary *_Nonnull subnode) {
...
- (NSDictionary *)arrangementConstructingIdMap:(BOOL)constructIdMap contents:(BOOL)contents {
...
- (NSDictionary *)arrangementConstructingIdMap:(BOOL)constructIdMap
                                      contents:(BOOL)contents
                                       options:(NSDictionary *)options {
    // Arrangements typically have ~10-15 top-level keys
...
- (NSDictionary *)arrangementWithOnlySession:(PTYSession *)session
    ...
    const BOOL commit = [self encodeWithContents:NO constructIdMap:NO encoder:adapter options:options];
    if (!commit) {
        return nil;
    }
    return dict;
...
- (BOOL)encodeWithContents:(BOOL)contents encoder:(id<iTermEncoderAdapter>)encoder {
...
+ (BOOL)encodeWithContents:(BOOL)contents
    ...
    if (isMaximized && options[PTYTabArrangementOptionsOnlySessionID] == nil) {
        ...
        // BUG-f1131: Replace assert with guard - constructIdMap should be NO for maximized
        if (constructIdMap) {
            ELog(@"BUG-f1131: arrangeSessionsWithEncoder called with constructIdMap=YES when maximized - ignoring");
            return NO;
        }
        // BUG-f1132: Replace assert with guard - savedArrangement must exist when maximized
        if (!savedArrangement) {
            ELog(@"BUG-f1132: arrangeSessionsWithEncoder called without savedArrangement when maximized - ignoring");
            return NO;
        }
        ...
        NSMutableDictionary *mutableRootNode = [savedArrangement[TAB_ARRANGEMENT_ROOT] mutableCopy];
    ...
    // BUG-f1133: Replace assert with guard - idMap must exist if constructIdMap is YES
    if (constructIdMap && !idMap) {
        ELog(@"BUG-f1133: arrangeSessionsWithEncoder has constructIdMap=YES but idMap is nil - ignoring");
        return NO;
    }
...
- (NSDictionary *)arrangementForDuplication {
    NSMutableDictionary *arrangement =
        [[self arrangementConstructingIdMap:NO contents:NO
                                    options:@{PTYSessionArrangementOptionsForDuplication : @YES}] mutableCopy];
...
+ (BOOL)_recursiveBuildSessionMap:(NSMutableDictionary<NSString *, PTYSession *> *)sessionMap
    ...
    if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        for (NSDictionary *subviewDict in arrangement[SUBVIEWS]) {
            if (![self _recursiveBuildSessionMap:sessionMap withArrangement:subviewDict sessions:sessions]) {
                return NO;
            }
        }
        return YES;
...
+ (NSDictionary<NSString *, PTYSession *> *)sessionMapWithArrangement:(NSDictionary *)arrangement
                                                             sessions:(NSArray *)sessions {
    NSMutableDictionary<NSString *, PTYSession *> *sessionMap =
        [NSMutableDictionary dictionaryWithCapacity:sessions.count];
...
+ (iTermSizeRange)_recursiveSetSizesInTmuxParseTree:(NSMutableDictionary *)parseTree
    ...
    switch ([[parseTree objectForKey:kLayoutDictNodeType] intValue]) {
        case kLeafLayoutNode: {
            ...
            const NSSize maximumSize = NSMakeSize(size.width + cellSize.width, size.height + cellSize.height);
        ...
        case kHSplitLayoutNode: {
            ...
            for (NSMutableDictionary *node in parseTree[kLayoutDictChildrenKey]) {
                const iTermSizeRange sizeRange = [self _recursiveSetSizesInTmuxParseTree:node
                                                                              showTitles:showTitles
                                                                     showBottomStatusBar:showBottomStatusBar
                                                                                bookmark:profile
                                                                              inTerminal:term];
                ...
                SetWithGrainDim(isVertical, &totalSize.minimumSize,
                                (WithGrainDim(isVertical, totalSize.minimumSize) +
                                 WithGrainDim(isVertical, sizeRange.minimumSize) + splitter));
                SetAgainstGrainDim(isVertical, &totalSize.minimumSize,
                                   MAX(AgainstGrainDim(isVertical, totalSize.minimumSize),
                                       AgainstGrainDim(isVertical, sizeRange.minimumSize)));
                SetWithGrainDim(isVertical, &totalSize.maximumSize,
                                (WithGrainDim(isVertical, totalSize.maximumSize) +
                                 WithGrainDim(isVertical, sizeRange.maximumSize) + splitter));
                SetAgainstGrainDim(isVertical, &totalSize.maximumSize,
...
+ (NSDictionary *)_recursiveArrangementForDecoratedTmuxParseTree:(NSDictionary *)parseTree
                                                ...
                                                activeWindowPane:(int)activeWp
                                                  tmuxController:(TmuxController *)tmuxController
                                                          window:(int)window {
    // Typical leaf node has ~6-8 keys, splitter node has ~4-5 keys
    ...
    switch ([[parseTree objectForKey:kLayoutDictNodeType] intValue]) {
        case kLeafLayoutNode: {
            ...
            [dict setObject:[NSNumber numberWithInt:wp] forKey:TAB_ARRANGEMENT_TMUX_WINDOW_PANE];
            if (wp == activeWp) {
        ...
        case kVSplitLayoutNode: {
            ...
            NSMutableArray *subviews = [NSMutableArray arrayWithCapacity:children.count];
...
+ (NSDictionary *)arrangementForDecoratedTmuxParseTree:(NSDictionary *)parseTree
                                      ...
                                      activeWindowPane:(int)activeWp
                                        tmuxController:(TmuxController *)tmuxController
                                                window:(int)window {
    // Top-level arrangement dict typically has just the root entry
    NSMutableDictionary *arrangement = [NSMutableDictionary dictionaryWithCapacity:2];
    ...
    while ([[temp objectForKey:TAB_ARRANGEMENT_VIEW_TYPE] isEqualToString:VIEW_TYPE_SPLITTER]) {
        // BUG-11197: Check SUBVIEWS array is non-empty before accessing [0]
        NSArray *subviews = [temp objectForKey:SUBVIEWS];
        if (subviews.count == 0) {
            break; // No more subviews to traverse
        }
        temp = subviews[0];
    }
...
- (void)setTmuxWindow:(int)window {
    // BUG-f1134: Replace assert with guard - variables must exist
    if (!self.variables) {
        ELog(@"BUG-f1134: setTmuxWindow called when variables is nil - ignoring");
        return;
    }
    // BUG-f1135: Replace assert with guard - variablesScope must exist
    if (![self variablesScope]) {
        ELog(@"BUG-f1135: setTmuxWindow called when variablesScope is nil - ignoring");
        return;
    }
...
- (void)setTmuxFontTable:(iTermFontTable *)fontTable hSpacing:(double)hs vSpacing:(double)vs {
    [self.tmuxController setTmuxFontTable:fontTable hSpacing:hs vSpacing:vs window:self.tmuxWindow];
}
...
+ (void)setSizesInTmuxParseTree:(NSMutableDictionary *)parseTree
    ...
    const BOOL forceTitleBar =
        perPaneTitleBarEnabled && [iTermPreferences boolForKey:kPreferenceKeyShowPaneTitlesEvenIfOnlyOnePane];
...
+ (NSMutableDictionary *)parseTreeWithInjectedRootSplit:(NSMutableDictionary *)parseTree {
    if ([[parseTree objectForKey:kLayoutDictNodeType] intValue] == kLeafLayoutNode) {
        ...
        // Root split node has 7 keys
        NSMutableDictionary *newRoot = [NSMutableDictionary dictionaryWithCapacity:8];
...
- (void)reloadTmuxLayout {
    ...
    if (shouldZoom) {
        [self maximizeAfterApplyingTmuxParseTree:visibleParseTree_ ?: parseTree_ tmuxController:self.tmuxController];
    }
...
+ (PTYTab *)openTabWithTmuxLayout:(NSMutableDictionary *)parseTree
    ...
    [PTYTab setSizesInTmuxParseTree:parseTree inTerminal:term zoomed:NO profile:profile];
    ...
    const BOOL forceTitles =
        titlesEnabled && [iTermPreferences boolForKey:kPreferenceKeyShowPaneTitlesEvenIfOnlyOnePane];
...
- (void)addSplitter:(NSSplitView *)splitter
    ...
    for (NSView *view in [splitter subviews]) {
        ...
        if ([view isKindOfClass:[NSSplitView class]]) {
            ...
            [self addSplitter:sub toIntervalMap:intervalMap forHeight:forHeight origin:origin];
        } else {
            ...
            NSSize contentSize =
                [PTYScrollView contentSizeForFrameSize:sessionViewFrame.size
                               horizontalScrollerClass:nil
                                 verticalScrollerClass:[realParentWindow_ scrollbarShouldBeVisible]
                                                           ? [[session.view.scrollview verticalScroller] class]
                                                           : nil
            ...
            int chars =
                forHeight
                    ? (contentSize.height - [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) /
                          cellSize.height
                    : (contentSize.width - [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2) / cellSize.width;
            [intervalMap incrementNumbersBy:chars inRange:[IntRange rangeWithMin:minPos size:size]];
        }
        if (!first && [splitter isVertical] != forHeight) {
            // Splitters have to be counted and there is a splitter before this view
            [intervalMap incrementNumbersBy:1 inRange:[IntRange rangeWithMin:minPos size:size]];
        }
        first = NO;
        if ([splitter isVertical]) {
...
- (int)tmuxSizeForHeight:(BOOL)forHeight {
    ...
    [self addSplitter:root_ toIntervalMap:intervalMap forHeight:forHeight origin:NSZeroPoint];
...
// Notation for DashTerm2 will be points(cells). Differences are noted with *.
//
//  Thin dividers                        Thick dividers
//  iTerm2             tmux              DashTerm2             tmux
//  ┌──────┬────────┐  ┌──────┬─────┐    ┌──────┬────────┐  ┌──────┬─────┐
//  │      │672(42) │  │      │ 42  │    │      │652(40) │  │      │ 40  │
...
- (NSSize)variableTmuxSize {
    ...
    DLog(@"Margins are %@, divider thickness is %@, cell size is %@", NSStringFromSize(margins), @(dividerThickness),
         NSStringFromSize(cellSize));

    PTYSplitView *root = [PTYSplitView castFrom:root_];
    // BUG-f1136: Replace assert with guard - root must be PTYSplitView
    if (!root) {
        ELog(@"BUG-f1136: variableTmuxSize expected root_ to be PTYSplitView but got %@",
             NSStringFromClass([root_ class]));
        return NSZeroSize;
    }
    ...
    [horizontalInfos
        enumerateObjectsUsingBlock:^(PTYSplitViewDividerInfo *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    [columnSlicePoints enumerateIndexesUsingBlock:^(NSUInteger point, BOOL *_Nonnull stop) {
        ...
        NSArray<NSNumber *> *decorationSizes = [sessionViewsInSlice mapWithBlock:^id(SessionView *sessionView) {
            DLog(@"%@ showTitle=%@ showBottomStatusBar=%@ ", sessionView, @(sessionView.showTitle),
                 @(sessionView.showBottomStatusBar));
    ...
    [rowSlicePoints enumerateIndexesUsingBlock:^(NSUInteger point, BOOL *_Nonnull stop) {
        ...
        NSArray<NSNumber *> *decorationSizes = [sessionViewsInSlice mapWithBlock:^id(SessionView *sessionView) {
            const NSSize scrollViewDecorationSize = [PTYScrollView
                frameSizeForContentSize:NSMakeSize(0, 0)
                horizontalScrollerClass:nil
                  verticalScrollerClass:
                      sessionView.scrollview.hasVerticalScroller ? sessionView.scrollview.verticalScroller.class : nil
    ...
    const NSSize sizeDiff =
        NSMakeSize(targetSizePoints.width - currentSize.width, targetSizePoints.height - currentSize.height);
    ...
    NSSize charsDiff = NSMakeSize(floor(sizeDiff.width / cellSize.width), floor(sizeDiff.height / cellSize.height));
    DLog(@"charsDiff=%@", NSStringFromSize(charsDiff));

    // The character size closest to the target.
    const NSSize tmuxSize =
        NSMakeSize(floor(baseTmuxSize.width + charsDiff.width), floor(baseTmuxSize.height + charsDiff.height));
...
- (NSSize)fixedTmuxSize {
    ...
    //
    // KNOWN_LIMITATION: Rounding error in tmux character size calculation.
    // This rounds off fractional parts. We really need to know the maximum capacity, and fractional
    // parts can add up to more than one whole char. Here's a real world example: When scrollbars
    // appear, every pane interior shrinks by 15px width. With two panes side-by-side [|], 345 and
    // 338px, the sizeDiff is 30px. It should add up to 99 chars:
    //   (345.0 - 10.0)/7.0 + (338.0 - 10.0)/7.0 + 30.0/7.0 = 99
    // But integer rounding shrinks it to 97 chars:
    //   (345 - 10)/7 + (338 - 10)/7 + 30/7 = 97
    // Workaround: We handle scrollbars specially below. A proper fix would require floating-point
    // accumulation until the final rounding step.
    ...
    // The pixel growth (+ for growth, - for shrinkage) needed to attain the target
    NSSize sizeDiff =
        NSMakeSize(targetSizePixels.width - rootSizePixels.width, targetSizePixels.height - rootSizePixels.height);

    // The size of a character
    NSSize charSize = [PTYTab cellSizeForBookmark:[self.tmuxController profileForWindow:self.tmuxWindow]];

    // The characters growth (+ growth, - shrinkage) needed to attain the target
    NSSize charsDiff = NSMakeSize(floor(sizeDiff.width / charSize.width), floor(sizeDiff.height / charSize.height));

    // The character size closest to the target.
    NSSize tmuxSize = NSMakeSize(rootSizeChars.width + charsDiff.width, rootSizeChars.height + charsDiff.height);

    DLog(@"tmuxSize: rootSizeChars=%@, targetSizePixels=%@, rootSizePixels=%@, sizeDiff=%@, charSize=%@, charsDiff=%@, "
         @"tmuxSize=%@",
         NSStringFromSize(rootSizeChars), NSStringFromSize(targetSizePixels), NSStringFromSize(rootSizePixels),
         NSStringFromSize(sizeDiff), NSStringFromSize(charSize), NSStringFromSize(charsDiff),
...
- (void)_recursiveResizeViewsInViewHierarchy:(NSView *)view forArrangement:(NSDictionary *)arrangement {
    if (!arrangement) {
        DLog(@"Missing arrangement in _recursiveResizeViewsInViewHierarchy");
        return;
    }
    ...
    if ([view isKindOfClass:[NSSplitView class]]) {
        ...
        // BUG-f1029: Guard against subarrangements/subviews count mismatch
        NSInteger subarrangementCount = [subarrangements count];
        for (NSView *child in [view subviews]) {
            if (i >= subarrangementCount) {
                DLog(@"BUG-f1029: More subviews than subarrangements at index %ld", (long)i);
                break;
            }
            [self _recursiveResizeViewsInViewHierarchy:child forArrangement:[subarrangements objectAtIndex:i]];
            i++;
        }
    } else {
        SessionView *sessionView = (SessionView *)view;
        ...
        NSDictionary *frameDict = [arrangement objectForKey:TAB_ARRANGEMENT_SESSIONVIEW_FRAME];
        if (!frameDict) {
            DLog(@"Missing TAB_ARRANGEMENT_SESSIONVIEW_FRAME in arrangement");
            return;
        }
        NSRect aFrame = [PTYTab dictToFrame:frameDict];
...
- (int)nodeSize:(ITMSplitTreeNode *)node width:(BOOL)sumWidths {
    ...
    const BOOL perpindicular = ((sumWidths && !node.vertical) || (!sumWidths && node.vertical));
    for (ITMSplitTreeNode_SplitTreeLink *link in node.linksArray) {
        switch (link.childOneOfCase) {
            ...
            case ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_GPBUnsetOneOfCase:
                // BUG-467: Replace assert(NO) with DLog - unset case should be skipped
                DLog(@"PTYTab nodeSize encountered GPBUnsetOneOfCase - skipping");
                break;
        }
        if (perpindicular) {
            return sum;
...
- (iTermTmuxLayoutBuilderLeafNode *)layoutBuilderLeafNodeForLink:(ITMSplitTreeNode_SplitTreeLink *)link {
    ...
    return [[iTermTmuxLayoutBuilderLeafNode alloc]
        initWithSessionOfSize:VT100GridSizeMake(link.session.gridSize.width, link.session.gridSize.height)
...
- (iTermTmuxLayoutBuilderNode *)layoutBuilderNodeForSplitTreeNode:(ITMSplitTreeNode *)node {
    ...
    iTermTmuxLayoutBuilderInteriorNode *result =
        [[iTermTmuxLayoutBuilderInteriorNode alloc] initWithVerticalDividers:node.vertical];
    for (ITMSplitTreeNode_SplitTreeLink *link in node.linksArray) {
        switch (link.childOneOfCase) {
            case ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_Node: {
...
- (void)setTmuxSizesFromSplitTreeNode:(ITMSplitTreeNode *)node {
    ...
    if (!self.realParentWindow.anyFullScreen) {
        ...
        [self.tmuxController setSize:NSMakeSize(clientSize.width, clientSize.height) window:self.tmuxWindow];
    }
...
- (CGSize)setSizesFromSplitTreeNode:(ITMSplitTreeNode *)node splitView:(NSSplitView *)splitView {
    ...
    for (NSInteger i = 0; i < splitView.subviews.count; i++) {
        ...
        if (splitView) {
            ...
            newSubviewSize =
                [PTYTab _sessionSizeWithCellSize:[PTYTab cellSizeForBookmark:session.profile]
                                      dimensions:NSMakeSize(link.session.gridSize.width, link.session.gridSize.height)
                                      showTitles:sessionView.showTitle
                             showBottomStatusBar:sessionView.showBottomStatusBar
                                      rightExtra:session.desiredRightExtra
                                      inTerminal:realParentWindow_];
        } else {
            // BUG-f648: Replace assert(false) with ELog and fallback - unknown subview type should not crash
            ELog(@"BUG-f648: setSizesFromSplitTreeNode encountered unknown subview type: %@",
                 NSStringFromClass([subview class]));
            newSubviewSize = subview.frame.size; // Preserve existing size as fallback
        }
...
- (void)installTmuxTitleMonitor {
    // BUG-f1137: Replace assert with guard - tmux title monitor must not already exist
    if (_tmuxTitleMonitor) {
        DLog(@"BUG-f1137: installTmuxTitleMonitor called when monitor already exists - ignoring");
        return;
    }
    ...
    __weak __typeof(self) weakSelf = self;
    _tmuxTitleMonitor =
        [[iTermTmuxOptionMonitor alloc] initWithGateway:tmuxController_.gateway
    ...
    if (self.titleOverride.length == 0) {
        ...
        self.variablesScope.tabTitleOverrideFormat =
            [NSString stringWithFormat:@"%@\\(%@?)", [iTermAdvancedSettingsModel tmuxTitlePrefix],
...
- (void)uninstallTmuxTitleMonitor {
    // BUG-f1138: Replace assert with guard - tmux title monitor must exist to uninstall
    if (!_tmuxTitleMonitor) {
        DLog(@"BUG-f1138: uninstallTmuxTitleMonitor called when monitor doesn't exist - ignoring");
        return;
    }
...
+ (NSMutableDictionary *)tweakedParseTree:(NSDictionary *)parseTree fillingRootOfSize:(NSSize)desiredSize {
    NSMutableDictionary *dict = [parseTree mutableCopy];

    const NSSize actualSize =
        NSMakeSize([parseTree[kLayoutDictPixelWidthKey] intValue], [parseTree[kLayoutDictPixelHeightKey] intValue]);
    ...
    switch ((LayoutNodeType)[parseTree[kLayoutDictNodeType] intValue]) {
        ...
        case kVSplitLayoutNode:
            isVertical = YES;
        case kHSplitLayoutNode: {
            NSArray *childNodes = parseTree[kLayoutDictChildrenKey];
            NSMutableArray *replacementChildren = [NSMutableArray arrayWithCapacity:childNodes.count];
            NSSize desiredGrowth =
                NSMakeSize(desiredSize.width - actualSize.width, desiredSize.height - actualSize.height);
            NSSize tweakedSize = actualSize;
            for (NSMutableDictionary *node in childNodes) {
                // Figure out how much this node would need to grow by and set `exaggeratedSize` to that size.
                const NSSize originalSize =
                    NSMakeSize([node[kLayoutDictPixelWidthKey] intValue], [node[kLayoutDictPixelHeightKey] intValue]);
                ...
                NSMutableDictionary *replacement = [self tweakedParseTree:node fillingRootOfSize:exaggeratedSize];
                ...
                const NSSize growth =
                    NSMakeSize(updatedSize.width - originalSize.width, updatedSize.height - originalSize.height);
                if (isVertical) {
...
- (void)replaceViewHierarchyWithParseTree:(NSMutableDictionary *)parseTree
    ...
    // Single entry for TAB_ARRANGEMENT_ROOT
    NSMutableDictionary *arrangement = [NSMutableDictionary dictionaryWithCapacity:2];
    parseTree = [PTYTab parseTreeWithInjectedRootSplit:parseTree];
    ...
    [arrangement setObject:[PTYTab _recursiveArrangementForDecoratedTmuxParseTree:parseTree
                                                                         bookmark:[self.tmuxController
                                                                                      profileForWindow:self.tmuxWindow]
                                                                           origin:NSZeroPoint
                                                                 activeWindowPane:[activeSession_ tmuxPane]
                                                                   tmuxController:tmuxController
                                                                           window:self.tmuxWindow]
    ...
    NSArray<PTYSession *> *allSessions = [self sessions];
    NSMutableDictionary<NSNumber *, SessionView *> *idMap =
        [NSMutableDictionary dictionaryWithCapacity:allSessions.count];
    for (PTYSession *aSession in allSessions) {
    ...
    for (PTYSession *aSession in [self sessions]) {
        ...
        if (![preexistingPanes containsObject:n]) {
            ...
            [tmuxController_ registerSession:aSession withPane:[aSession tmuxPane] inWindow:self.tmuxWindow];
...
- (void)maximizeAfterApplyingTmuxParseTree:(NSMutableDictionary *)parseTree
                            tmuxController:(TmuxController *)tmuxController {
    DLog(@"Maximizing");
    [self maximize];
    ...
    DLog(@"After resizing views in maximize, root_.width=%f, flexibleView_.width=%f", root_.frame.size.width,
...
- (void)setTmuxLayout:(NSMutableDictionary *)parseTree
    ...
    if ([self parseTree:parseTree matchesViewHierarchy:root_]) {
        ...
        DLog(@"PTYTab setTmuxLayout %@ visible parse tree yielding width of %@px. Will resize views. root_ width "
             @"before=%f, flexibleView_ width before=%f",
             visibleParseTree ? @"with" : @"without", parseTree[kLayoutDictMaximumPixelWidthKey],
        ...
        [self replaceViewHierarchyWithParseTree:parseTree tmuxController:tmuxController];
        shouldZoom = NO;
    }
    [self updateFlexibleViewColors];
    [[root_ window] makeFirstResponder:[[self activeSession] mainResponder]];
    ...
    if (shouldZoom) {
        [self maximizeAfterApplyingTmuxParseTree:visibleParseTree ?: parseTree tmuxController:tmuxController];
    }
...
- (BOOL)updatedTmuxLayoutRequiresAdjustment {
    ...
    if ([iTermAdvancedSettingsModel disableTmuxWindowResizing]) {
        ...
        DLog(@"updatedTmuxLayoutRequiresAdjustment: dx=%@ dy=%@ cellSize=%@ root.frame=%@ flexibleView.frame=%@", @(dx),
             @(dy), NSStringFromSize(cellSize), NSStringFromRect(root_.frame), NSStringFromRect(flexibleView_.frame));
    ...
    DLog(@"updatedTmuxLayoutRequiresAdjustment: Using too-large check only. root %@ has size %@ vs flexible view %@ "
         @"with size %@",
         root_, NSStringFromSize(root_.frame.size), flexibleView_, NSStringFromSize(flexibleView_.frame.size));
...
- (void)maximize {
    ...
    // BUG-f1139: Replace assert with guard - savedArrangement must be nil before maximize
    if (savedArrangement_) {
        ELog(@"BUG-f1139: maximize called when savedArrangement_ already exists - ignoring");
        return;
    }
    // BUG-f1140: Replace assert with guard - idMap must be nil before maximize
    if (idMap_) {
        ELog(@"BUG-f1140: maximize called when idMap_ already exists - ignoring");
        return;
    }
    // BUG-f1141: Replace assert with guard - isMaximized must be NO before maximize
    if (isMaximized_) {
        ELog(@"BUG-f1141: maximize called when already maximized - ignoring");
        return;
    }
    ...
    idMap_ = [[NSMutableDictionary alloc] initWithCapacity:8]; // Session ID map for arrangement
...
- (void)resizeMaximizedTmuxSessionView:(SessionView *)sessionView toGridSize:(VT100GridSize)gridSize {
    ...
    NSSize size = [PTYTab
        _sessionSizeWithCellSize:[PTYTab cellSizeForBookmark:[self.tmuxController profileForWindow:self.tmuxWindow]]
                      dimensions:NSMakeSize(gridSize.width, gridSize.height)
                      showTitles:showTitles
             showBottomStatusBar:NO
                      rightExtra:sessionView.desiredRightExtra
                      inTerminal:self.realParentWindow];
    NSRect frame = {.origin = sessionView.frame.origin, .size = size};
    sessionView.frame = frame;
}

- (void)unmaximize {
    // BUG-f1142: Replace assert with guard - savedArrangement must exist to unmaximize
    if (!savedArrangement_) {
        ELog(@"BUG-f1142: unmaximize called without savedArrangement_ - ignoring");
        return;
    }
    // BUG-f1143: Replace assert with guard - idMap must exist to unmaximize
    if (!idMap_) {
        ELog(@"BUG-f1143: unmaximize called without idMap_ - ignoring");
        return;
    }
    // BUG-f1144: Replace assert with guard - isMaximized must be YES to unmaximize
    if (!isMaximized_) {
        ELog(@"BUG-f1144: unmaximize called when not maximized - ignoring");
        return;
    }

    // Pull the formerly maximized sessionview out of the old root.
    // BUG-f1145: Replace assert with guard - root must have exactly 1 subview when maximized
    if ([[root_ subviews] count] != 1) {
        ELog(@"BUG-f1145: unmaximize expected 1 subview but got %lu", (unsigned long)[[root_ subviews] count]);
        // BUG-1363: Guard against empty subviews
        if ([[root_ subviews] count] == 0) {
            return;
        }
    }
    SessionView *formerlyMaximizedSessionView = [[root_ subviews] objectAtIndex:0];

    // I'm not convinced this is necessary but I'm afraid to remove it. idMap_ should hold refs to all SessionViews that
    // matter.
...
- (void (^)(void))blockToMoveView:(NSView *)view
                     ...
                     horizontally:(BOOL)horizontally
                               by:(int)direction {
    if (![possibleSplit isKindOfClass:[PTYSplitView class]] || possibleSplit.subviews.count == 1) {
        return NULL;
    }

    // Compute the index of the passed-in view and divider that are affected.
    PTYSplitView *split = (PTYSplitView *)possibleSplit;
    // BUG-f1146: Replace assert with guard - split orientation must match movement direction
    if (!(([split isVertical] && horizontally) || (![split isVertical] && !horizontally))) {
        ELog(@"BUG-f1146: blockToMoveView called with mismatched split orientation and direction");
        return NULL;
    }
    ...
    NSSize movement = NSMakeSize(horizontally ? direction * step : 0, horizontally ? 0 : direction * step);
    ...
    if (direction > 0) {
        ...
        const CGFloat constraint = [self splitView:split constrainMinCoordinate:proposed ofSubviewAt:splitterIndex];
        if (constraint > proposed) {
            return NULL;
        }
    }
...
- (void)moveView:(NSView *)view inSplit:(NSView *)possibleSplit horizontally:(BOOL)horizontally by:(int)direction {
    void (^block)(void) = [self blockToMoveView:view inSplit:possibleSplit horizontally:horizontally by:direction];
    if (block) {
        block();
    }
}

- (BOOL)canMoveView:(NSView *)view inSplit:(NSView *)possibleSplit horizontally:(BOOL)horizontally by:(int)direction {
    void (^block)(void) = [self blockToMoveView:view inSplit:possibleSplit horizontally:horizontally by:direction];
    return block != nil;
}
...
- (void)swapSession:(PTYSession *)session1 withSession:(PTYSession *)session2 {
    ...
    // BUG-f1147: Replace assert with guard - session1 must be in this tab
    if (session1.delegate != self) {
        ELog(@"BUG-f1147: swapSession called with session1 not in this tab - ignoring");
        return;
    }
    if (isMaximized_) {
        [self unmaximize];
    ...
    if (session1.isTmuxClient && session2.isTmuxClient && session1.tmuxController == session2.tmuxController) {
        DLog(@"Use tmux to swap");
        [session1.tmuxController swapPane:session1.tmuxPane withPane:session2.tmuxPane];
        return;
    }
    if ([session1 isTmuxClient] || [session2 isTmuxClient] || [session1 isTmuxGateway] || [session2 isTmuxGateway]) {
        return;
    }

    DLog(@"Before swap, %@ has superview %@ and %@ has superview %@", session1.view, session1.view.superview,
    ...
    DLog(@"After swap, %@ has superview %@ and %@ has superview %@", session1.view, session1.view.superview,
         session2.view, session2.view.superview);
...
- (void)_recursivePopulateSplitTreeNode:(ITMSplitTreeNode *)node from:(NSSplitView *)splitview {
...
- (void)updateTabTitle {
    NSString *sessionName =
        [self.activeSession.variablesScope valueForVariableName:iTermVariableKeySessionPresentationName];
...
- (void)setTitleOverride:(NSString *)titleOverride {
    ...
    if (self.tmuxTab) {
        if (titleOverride) {
            [self.tmuxController setWindowTitleOverride:titleOverride window:self.tmuxWindow];
        }
    }
}
...
- (NSArray<PTYSession *> *)sessionsAdjacentToSplitter:(int)splitterIndex of:(PTYSplitView *)splitView {
...
- (void)splitView:(PTYSplitView *)splitView draggingDidEndOfSplit:(int)splitterIndex pixels:(NSSize)pxMoved {
    ...
    NSView *theView =
        [subviews uncheckedObjectAtIndex:splitterIndex]; // the view right of or below the dragged splitter.
    if (theView) {
        while ([theView isKindOfClass:[NSSplitView class]]) {
            NSSplitView *subSplitView = (NSSplitView *)theView;
            // BUG-f1030: Guard against empty subviews in nested split view
            if ([[subSplitView subviews] count] == 0) {
                DLog(@"BUG-f1030: subSplitView has no subviews, cannot descend further");
                theView = nil;
                break;
            }
        ...
        // it finishes.
        if (amount != 0) {
            [tmuxController_ windowPane:[session tmuxPane] resizedBy:amount horizontally:[splitView isVertical]];
        }
    }
    [self updateUseMetal];
}
...
- (CGFloat)splitView:(NSSplitView *)splitView
    ...
    // BUG-f1031: Guard against dividerIndex out of bounds
    NSArray<NSView *> *subviews = [splitView subviews];
    if (dividerIndex < 0 || dividerIndex >= (NSInteger)[subviews count]) {
        DLog(@"BUG-f1031: dividerIndex %ld out of bounds (count=%lu)", (long)dividerIndex,
             (unsigned long)[subviews count]);
        return proposedMin;
    }
    CGFloat dim;
    NSSize minSize = [self _minSizeOfView:[subviews objectAtIndex:dividerIndex] respectPinning:YES];
    if ([splitView isVertical]) {
        dim = minSize.width;
    } else {
        dim = minSize.height;
    }
    return [self _positionOfDivider:(int)dividerIndex - 1 inSplitView:splitView] + dim;
}
...
- (CGFloat)splitView:(NSSplitView *)splitView
    ...
    // BUG-f1032: Guard against dividerIndex+1 out of bounds
    NSArray<NSView *> *subviews = [splitView subviews];
    NSInteger nextIndex = dividerIndex + 1;
    if (nextIndex < 0 || nextIndex >= (NSInteger)[subviews count]) {
        DLog(@"BUG-f1032: dividerIndex+1=%ld out of bounds (count=%lu)", (long)nextIndex,
             (unsigned long)[subviews count]);
        return proposedMax;
    }
    CGFloat dim;
    NSSize minSize = [self _minSizeOfView:[subviews objectAtIndex:nextIndex] respectPinning:YES];
    if ([splitView isVertical]) {
        dim = minSize.width;
    } else {
        dim = minSize.height;
    }
    return [self _positionOfDivider:(int)nextIndex inSplitView:splitView] - dim - [splitView dividerThickness];
}
...
- (void)_recursiveLockedSize:(NSView *)theSubview
    ...
    if ([theSubview isKindOfClass:[SessionView class]]) {
        // BUG-f1010: Gracefully handle unexpected subview instead of crashing
        if (theSubview != [lockedSession_ view]) {
            DLog(@"BUG-f1010: _recursiveLockedSize called with unexpected SessionView (not locked session)");
            *minSize = *maxSizeOut = NSZeroSize;
            return;
        }
...
- (void)_redistributeQuantizationError:(const double)targetSize
    ...
    ITCriticalError(sizes.count == minSizes.count && sizes.count == maxSizes.count,
                    @"Mismatch in sizes array. sizes=%@ minSizes=%@ maxSizes=%@ self=%@ root=%@", sizes, minSizes,
                    maxSizes, self, [root_ iterm_recursiveDescription]);
    // BUG-f1011: Gracefully handle empty sizes arrays instead of crashing
    if (sizes.count == 0 || minSizes.count == 0 || maxSizes.count == 0) {
        DLog(@"BUG-f1011: _redistributeQuantizationError called with empty arrays: sizes=%lu, minSizes=%lu, "
             @"maxSizes=%lu",
             (unsigned long)sizes.count, (unsigned long)minSizes.count, (unsigned long)maxSizes.count);
        return;
    }
    ...
    while (error != 0) {
        ...
        for (int i = 0; i < [sizes count] && error != 0; ++i) {
            ...
            ITCriticalError(
                sizes.count == minSizes.count && sizes.count == maxSizes.count,
...
- (BOOL)resizeSubviewsOfSplitView:(NSSplitView *)splitView oldSize:(NSSize)oldSize respectPinning:(BOOL)respectPinning {
    ...
    const double targetSize =
        WithGrainDim(isVertical, [splitView frame].size) - ([splitView dividerThickness] * (n - 1));
    ...
    if (!ignoreConstraints) {
        ...
        for (NSView *aSubview in [splitView subviews]) {
            ...
            PtyLog(@"splitView:resizeSubviewsWithOldSize - initial guess of %p is %lf (based on size of %lf), clamped "
                   @"is %lf",
                   aSubview, initialGuess, (double)WithGrainDim(isVertical, [aSubview frame].size), size);
        ...
        while (fabs(currentSumOfSizes - targetSize) > kEpsilon) {
            PtyLog(@"splitView:resizeSubviewsWithOldSize - refining. currentSumOfSizes=%lf vs target %lf",
                   currentSumOfSizes, targetSize);
            ...
            PtyLog(@"splitView:resizeSubviewsWithOldSize - coeff %lf to bring current %lf unclamped size to %lf", coeff,
                   currentSumOfUnclamped, desiredNewSizeForUnclamped);
            ...
            for (int i = 0; i < [sizes count]; ++i) {
                ...
                PtyLog(@"splitView:resizeSubviewsWithOldSize - change %lf to %lf (would be %lf unclamped)",
                       [[sizes objectAtIndex:i] doubleValue], size, preferredSize);
            ...
            if (!anyChanges) {
                ...
                if (fabs(currentSumOfSizes - targetSize) > [[splitView subviews] count]) {
                    ...
                    SetPinnedDebugLogMessage(@"Current split view for redistribution",
                                             @"oldTotalSize=%@ ignoreConstraints=%@ splitView.recursiveDescription=%@",
                                             @(oldTotalSize), @(ignoreConstraints),
                                             [splitView iterm_recursiveDescription]);
    ...
    if (ignoreConstraints) {
        ...
        if (sizeChangeCoeff == 0) {
            ...
            // BUG-f1033: Guard against sizes/subviews count mismatch
            NSArray<NSView *> *currentSubviews = [splitView subviews];
            NSInteger minCount = MIN([sizes count], [currentSubviews count]);
            for (int i = 0; i < minCount; ++i) {
                NSView *aSubview = [currentSubviews objectAtIndex:i];
        ...
        SetPinnedDebugLogMessage(@"Current split view for redistribution",
                                 @"oldTotalSize=%@ ignoreConstraints=%@ splitView.recursiveDescription=%@",
                                 @(oldTotalSize), @(ignoreConstraints), [splitView iterm_recursiveDescription]);
        [self _redistributeQuantizationError:targetSize
                           currentSumOfSizes:currentSumOfSizes
                                       sizes:sizes
                                    minSizes:minSizes
    ...
    if (allSubviewsAreLeafNodes) {
        // BUG-f1034: Guard against sizes/subviews count mismatch
        NSArray<NSView *> *currentSubviews = [splitView subviews];
        NSInteger minCount = MIN(sizes.count, [currentSubviews count]);
        // index -> overage (typically few views have overage)
        NSMutableDictionary *over = [NSMutableDictionary dictionaryWithCapacity:minCount];
        // index -> underage (typically few views have underage)
        NSMutableDictionary *under = [NSMutableDictionary dictionaryWithCapacity:minCount];
        for (int i = 0; i < minCount; i++) {
            NSSize size;
            SetWithGrainDim(isVertical, &size, [[sizes objectAtIndex:i] doubleValue]);
            SessionView *sessionView = (SessionView *)[currentSubviews objectAtIndex:i];
        ...
        NSMutableArray *overKeys =
            [NSMutableArray arrayWithArray:[[over allKeys] sortedArrayUsingSelector:@selector(compare:)]];
        NSMutableArray *underKeys =
            [NSMutableArray arrayWithArray:[[under allKeys] sortedArrayUsingSelector:@selector(compare:)]];
        while (overKeys.count && underKeys.count) {
            ...
            // BUG-f1421: Guard against mostOverIndex/mostUnderIndex out of bounds
            if (mostOverIndex < 0 || mostOverIndex >= (int)[sizes count] || mostUnderIndex < 0 ||
                mostUnderIndex >= (int)[sizes count]) {
                DLog(@"BUG-f1421: mostOverIndex=%d or mostUnderIndex=%d out of bounds (sizes.count=%lu)", mostOverIndex,
                     mostUnderIndex, (unsigned long)[sizes count]);
                break;
            }
    ...
    // BUG-f1035: Guard against sizes/subviews count mismatch when setting frames
    NSArray<NSView *> *finalSubviews = [splitView subviews];
    NSInteger finalMinCount = MIN([sizes count], [finalSubviews count]);
    for (int i = 0; i < finalMinCount; ++i) {
        SetWithGrainDim(isVertical, &frame.size, [[sizes objectAtIndex:i] doubleValue]);
        [[finalSubviews objectAtIndex:i] setFrame:frame];
        if (isVertical) {
...
- (void)_splitViewDidResizeSubviews:(NSSplitView *)splitView {
    ...
    for (NSView *subview in [splitView subviews]) {
        if ([subview isKindOfClass:[SessionView class]]) {
            ...
            if (session) {
                PtyLog(@"splitViewDidResizeSubviews - view is %fx%f, ignore=%d", [subview frame].size.width,
                       [subview frame].size.height, (int)[session ignoreResizeNotifications]);
...
- (void)adjustSplitSubviewSizesForPinnedSizes:(NSSplitView *)splitView {
    ...
    NSArray<SessionView *> *sessionViews =
        [splitView.subviews mapWithBlock:^id _Nullable(__kindof NSView *_Nonnull anObject) {
...
- (CGFloat)stepForMovementOfDividerIndex:(NSInteger)dividerIndex ofSplitView:(NSSplitView *)splitView {
    NSArray<NSView *> *subviews = [splitView subviews];
...
- (CGFloat)splitView:(NSSplitView *)splitView
    ...
    NSArray<NSView *> *subviews = [splitView subviews];
    if (dividerIndex < 0 || subviews.count < dividerIndex + 2) {
                ...
                if (!session.havePostedIdleNotification && [session shouldPostUserNotification]) {
                    NSString *theDescription =
                        [NSString stringWithFormat:@"Session %@ in tab #%d became idle.",
                                                   [[session name] removingHTMLFromTabTitleIfNeeded], [self tabNumber]];
...
- (void)setLabelAttributesForActiveTab:(BOOL)notify {
    ...
    if (![[self activeSession] havePostedNewOutputNotification] &&
        [[self realParentWindow] broadcastMode] == BROADCAST_OFF && notify &&
        [[NSDate date] timeIntervalSinceDate:[SessionView lastResizeDate]] > POST_WINDOW_RESIZE_SILENCE_SEC) {
        if ([iTermProfilePreferences boolForKey:KEY_SEND_NEW_OUTPUT_ALERT inProfile:self.activeSession.profile]) {
            [[iTermNotificationController sharedInstance]
                         notify:NSLocalizedStringFromTableInBundle(
                                    @"New Output", @"iTerm", [NSBundle bundleForClass:[self class]], @"User Alerts")
                withDescription:[NSString
                                    stringWithFormat:@"New output was received in %@, tab #%d.",
...
- (void)resetLabelAttributesIfAppropriate {
    ...
    for (PTYSession *aSession in [self sessions]) {
        if (!amProcessing && !aSession.havePostedNewOutputNotification && !aSession.newOutput) {
    ...
    if (shouldResetLabel && [self isForegroundTab]) {
        DLog(@"Reset label");
        [self setIsProcessing:NO];
        [self setState:0 reset:(kPTYTabIdleState | kPTYTabNewOutputState | kPTYTabDeadState)];
    }
}
...
- (void)updateUseMetal NS_AVAILABLE_MAC(10_11) {
    ...
    const BOOL dtermCoreRenderer = [iTermAdvancedSettingsModel dtermCoreRendererEnabled];
    const BOOL resizing = self.realParentWindow.windowIsResizing;
    // dterm-core: skip power check (performance-focused, not power-saving)
    const BOOL powerOK = dtermCoreRenderer ? YES : [[iTermPowerManager sharedInstance] metalAllowed];
    ...
    // dterm-core: skip idle check (performance-focused, not power-saving)
    const BOOL allSessionsIdle = (!dtermCoreRenderer &&
                                  allSessionsAllowMetal && [iTermAdvancedSettingsModel disableMetalWhenIdle] &&
...
- (BOOL)session:(PTYSession *)session shouldAllowDrag:(id<NSDraggingInfo>)sender {
    if ([[[sender draggingPasteboard] types] indexOfObject:@"com.dashterm.dashterm2.psm.controlitem"] != NSNotFound) {
        // Dragging a tab handle. Source is a PSMTabBarControl.
        PTYTab *theTab =
            (PTYTab *)[[[[PSMTabDragAssistant sharedDragAssistant] draggedCell] representedObject] identifier];
    ...
    if ([[[sender draggingPasteboard] types] indexOfObject:iTermMovePaneDragType] != NSNotFound) {
        if ([[MovePaneController sharedInstance] isMovingSession:session]) {
            if (self.sessions.count == 1 && !self.realParentWindow.anyFullScreen &&
                self.realParentWindow.movesWhenDraggedOntoSelf) {
...
- (NSUInteger)sessionPaneNumber:(PTYSession *)session {
    ...
    if (index == NSNotFound) {
        // Session not yet in the array (being added), return the next available index
        return self.sessions.count;
    } else {
        // Return the actual index of the session in the array
        return index;
    }
}
...
- (VT100GridSize)sessionTmuxSizeWithProfile:(Profile *)profile {
    ...
    if ([iTermPreferences useTmuxProfile] && !useExistingWindowSize) {
        ...
        NSSize contentSize =
            [PTYScrollView contentSizeForFrameSize:frameSize
                           horizontalScrollerClass:nil
                             verticalScrollerClass:[realParentWindow_ scrollbarShouldBeVisible]
                                                       ? [[anySession.view.scrollview verticalScroller] class]
                                                       : nil
...
- (void)sessionDidChangeGraphic:(PTYSession *)session shouldShow:(BOOL)shouldShow image:(NSImage *)image {
    if (session == self.activeSession) {
        [self.delegate tabDidChangeGraphic:self shouldShow:shouldShow image:image];
    }
}
...
- (void)sessionExecDidFail {
    // Stub implementation for protocol conformance.
    // This method is declared in PTYSessionDelegate but never called.
}
...
- (void)setTitleWithCompletion:(void (^)(id, NSError *))completion title:(NSString *)title {
    [self setTitleOverride:title];
    completion(nil, nil);
}

- (void)selectPaneInDirectionWithCompletion:(void (^)(id, NSError *))completion direction:(NSString *)direction {
    PTYSession *activeSession = [self activeSession];
    PTYSession *session;
    if ([direction isEqualToString:@"left"]) {
        ...
        NSError *error =
            [NSError errorWithDomain:@"com.dashterm.dashterm2.select-pane-in-direction"
                                code:0
                            userInfo:@{
                                NSLocalizedDescriptionKey : @"Invalid direction. Should be left, right, above or below."
                            }];
