...
- (NSArray<Class> *)commands {
    ...
    dispatch_once(&onceToken, ^{
        commands = @[
            [iTermOpenQuicklyInTabsWindowArrangementCommand class], [iTermOpenQuicklyWindowArrangementCommand class],
            [iTermOpenQuicklySearchSessionsCommand class], [iTermOpenQuicklySwitchProfileCommand class],
            [iTermOpenQuicklySearchWindowsCommand class], [iTermOpenQuicklyCreateTabCommand class],
            [iTermOpenQuicklyColorPresetCommand class], [iTermOpenQuicklyScriptCommand class],
            [iTermOpenQuicklyActionCommand class], [iTermOpenQuicklySnippetCommand class],
...
- (id<iTermOpenQuicklyCommand>)commandForQuery:(NSString *)queryString {
    if ([queryString hasPrefix:@"/"] && queryString.length > 1) {
        ...
        NSString *text = [[queryString substringFromIndex:2]
...
- (NSArray *)sessions {
    ...
    // Estimate ~4 sessions per terminal window on average
    NSMutableArray *sessions = [[NSMutableArray alloc] initWithCapacity:terminals.count * 4];
...
// Returns a function PTYSession -> (Feature name, Feature value) that gives the value which most distinguishes sessions
// from one another.
- (iTermTuple<NSString *, NSString *> * (^)(PTYSession *))detailFunctionForSessions:(NSArray<PTYSession *> *)sessions {
    ...
    NSArray<iTermTriple<NSString *, NSString *, NSNumber *> * (^)(PTYSession *)> *functions =
        @[ pwd, command, hostname, badge ];
    if ([iTermBrowserGateway browserAllowedCheckingIfNot:NO]) {
        functions = [functions arrayByAddingObject:webSite];
    }
    NSMutableArray<NSMutableArray *> *functionValues = [[NSMutableArray alloc] initWithCapacity:functions.count];
    [functions
        enumerateObjectsUsingBlock:^(iTermTriple<NSString *, NSString *, NSNumber *> * (^_Nonnull obj)(PTYSession *),
                                     NSUInteger idx, BOOL *_Nonnull stop) {
            [functionValues addObject:[[NSMutableArray alloc] initWithCapacity:sessions.count]];
        }];
    [sessions enumerateObjectsUsingBlock:^(PTYSession *_Nonnull session, NSUInteger sessionIndex, BOOL *_Nonnull stop) {
        [functions
            enumerateObjectsUsingBlock:^(iTermTriple<NSString *, NSString *, NSNumber *> * (^_Nonnull f)(PTYSession *),
                                         NSUInteger functionIndex, BOOL *_Nonnull stop) {
    ...
    NSMutableArray<iTermTriple<NSString *, NSString *, NSNumber *> * (^)(PTYSession *)> *filteredFunctions =
        [[NSMutableArray alloc] initWithCapacity:indexes.count];
    NSMutableArray<NSMutableArray *> *filteredFunctionValues = [[NSMutableArray alloc] initWithCapacity:indexes.count];
    [indexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
        [filteredFunctions addObject:functions[idx]];
        [filteredFunctionValues addObject:functionValues[idx]];
    }];
    double (^variance)(NSArray<iTermTriple<NSString *, NSString *, NSNumber *> *> *) =
        ^double(NSArray<iTermTriple<NSString *, NSString *, NSNumber *> *> *tuples) {
            NSSet<NSString *> *set = [NSSet
                setWithArray:[tuples mapWithBlock:^id _Nonnull(iTermTuple<NSString *, NSString *> *_Nonnull tuple) {
                    return tuple.secondObject;
                }]];
            return set.count;
        };

    // Indexes into filteredFunction[Value]s sorted by variance. The functions at the end of the list discriminate
    // sessions better.
    ...
    sortedIndexes = [sortedIndexes sortedArrayUsingComparator:^NSComparisonResult(NSNumber *li, NSNumber *ri) {
        NSMutableArray<iTermTriple<NSString *, NSString *, NSNumber *> *> *obj1 =
            filteredFunctionValues[li.unsignedIntegerValue];
        NSMutableArray<iTermTriple<NSString *, NSString *, NSNumber *> *> *obj2 =
            filteredFunctionValues[ri.unsignedIntegerValue];
    ...
    return ^id(PTYSession *session) {
        for (NSNumber *i in sortedIndexes.reversed) {
            iTermTriple<NSString *, NSString *, NSNumber *> *candidate =
                filteredFunctions[i.unsignedIntegerValue](session);
            if (candidate.thirdObject.boolValue) {
                // The value for this session is valid.
                return [iTermTuple tupleWithObject:candidate.firstObject andObject:candidate.secondObject];
            }
        }
        return nil;
    };
...
- (void)addSessionLocationToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    iTermTuple<NSString *, NSString *> * (^detailFunction)(PTYSession *) =
        [self detailFunctionForSessions:self.sessions];

    for (PTYSession *session in self.sessions) {
        // Typically 4-6 features per session (name, badge, command, directory, hostname, profile)
        NSMutableArray *features = [[NSMutableArray alloc] initWithCapacity:8];
        ...
        item.score = [self scoreForSession:session matcher:matcher features:features attributedName:attributedName];
        if (item.score > 0) {
            ...
            if (detailFunction) {
                ...
                if (detailTuple) {
                    // "Feature: value" giving identifying info about this session to distinguish it from others.
                    // Query-independent.
                    ...
                    if (![detailTuple.firstObject isEqual:detail.firstObject] && item.detail != nil) {
                        ...
                        NSAttributedString *emdash = [[NSAttributedString alloc]
                            initWithString:@" — "
...
- (iTermOpenQuicklyMenuItem *)itemForMenuItem:(NSMenuItem *)menuItem
    ...
    NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
    item.score = [self scoreForMenuItem:menuItem matcher:matcher attributedName:attributedName];
    if (item.score <= 0) {
        return nil;
    }
    item.title = attributedName;
    ...
    NSMutableAttributedString *detail = [[NSMutableAttributedString alloc]
        initWithString:menuItem.alternate ? @"Alternate menu item under " : @"Menu item under "
...
- (iTermOpenQuicklyNamedMarkItem *)itemForNamedMark:(id<iTermGenericNamedMarkReading>)namedMark
    ...
    item.detail = [_delegate
        openQuicklyModelDisplayStringForFeatureNamed:nil
    ...
    for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
        {
            ...
            NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
            item.score = [self scoreForWindow:term matcher:matcher attributedName:attributedName];
            if (item.score > 0) {
                // Features: display name, miniaturized, full screen, other space, hotkey
                NSMutableArray<NSString *> *features = [[NSMutableArray alloc] initWithCapacity:4];
                if (multipleDisplays) {
                ...
                if (!term.window.isOnActiveSpace &&
                    !(term.window.collectionBehavior & NSWindowCollectionBehaviorCanJoinAllSpaces)) {
                    [features addObject:@"On other Space"];
                }
                if (term.isHotKeyWindow) {
                    iTermProfileHotKey *profileHotkey =
                        [[iTermHotKeyController sharedInstance] profileHotKeyForWindowController:term];
...
- (void)addBookmarkToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    for (iTermTriple<NSString *, NSURL *, NSString *> *triple in
         [iTermBrowserBookmarkFinder bookmarksMatchingSubstring:matcher.query]) {
        if ([urls containsObject:triple.secondObject]) {
            continue;
        }
        iTermOpenQuicklyBookmarkItem *item = [[iTermOpenQuicklyBookmarkItem alloc] init];
        NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
        item.score = [self scoreForBookmarkTitle:triple.firstObject
                                             url:triple.secondObject
                                         matcher:matcher
                                  attributedName:attributedName];
        if (item.score > 0) {
            item.detail = [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil
    ...
    for (iTermTriple<NSString *, NSURL *, NSString *> *triple in
         [iTermBrowserVisitsFinder historyMatchingSubstring:matcher.query]) {
        if ([urls containsObject:triple.secondObject]) {
            continue;
        }
        iTermOpenQuicklyBookmarkItem *item = [[iTermOpenQuicklyBookmarkItem alloc] init];
...
- (void)addChangeColorPresetToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    for (NSString *name in allPresets) {
        ...
        item.logoGenerator.textColor = iTermColorPresetGet(preset, KEY_FOREGROUND_COLOR, dark)
                                           ?: [NSColor colorWithRed:0.75 green:0.75 blue:0.75 alpha:1];
        item.logoGenerator.backgroundColor = iTermColorPresetGet(preset, KEY_BACKGROUND_COLOR, dark)
                                                 ?: [NSColor colorWithRed:0.25 green:0.25 blue:0.25 alpha:1];
...
- (void)addChangeProfileToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
        ...
        if (changeProfileItem.score > 0) {
            changeProfileItem.detail =
                [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil
...
- (void)addActionsToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
              withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
    [[[iTermActionsModel sharedInstance] actions]
        enumerateObjectsUsingBlock:^(iTermAction *_Nonnull action, NSUInteger idx, BOOL *_Nonnull stop) {
            iTermOpenQuicklyActionItem *actionItem = [self actionItemForAction:action matcher:matcher];
            if (actionItem) {
...
- (void)addInvocation:(NSString *)invocation
    ...
    item.score = 1;
    item.detail = [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil value:@"Invoke" highlightedIndexes:nil];
    NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
...
- (void)addNamedMarksToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    for (PTYSession *session in [[iTermController sharedInstance] allSessions]) {
        for (id<iTermGenericNamedMarkReading> namedMark in session.namedMarks) {
            ...
            iTermOpenQuicklyNamedMarkItem *item = [self itemForNamedMark:namedMark inSession:session matcher:matcher];
            if (item) {
                [items addObject:item];
            }
        }
...
- (void)addMenuItemsInMenu:(NSMenu *)menu
    ...
    for (NSMenuItem *menuItem in menu.itemArray) {
        if (menuItem.title.length > 0 && menuItem.action != nil && !menuItem.submenu) {
            iTermOpenQuicklyMenuItem *item = [self itemForMenuItem:menuItem matcher:matcher path:path];
            if (item) {
                [items addObject:item];
            }
        }
        if (menuItem.submenu) {
            [self addMenuItemsInMenu:menuItem.submenu
                             toItems:items
                         withMatcher:matcher
                                path:[path arrayByAddingObject:menuItem.title]];
        }
    }
}
...
- (void)addSnippetsToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
    ...
    [[[iTermSnippetsModel sharedInstance] snippets]
        enumerateObjectsUsingBlock:^(iTermSnippet *_Nonnull snippet, NSUInteger idx, BOOL *_Nonnull stop) {
            if (![snippet hasTags:tags]) {
                return;
            }
            iTermOpenQuicklySnippetItem *snippetItem = [self snippetItemForSnippet:snippet matcher:matcher];
            if (snippetItem) {
...
- (iTermOpenQuicklySnippetItem *)snippetItemForSnippet:(iTermSnippet *)snippet
    ...
    snippetItem.detail = [_delegate
        openQuicklyModelDisplayStringForFeatureNamed:nil
                                               value:[NSString
                                                         stringWithFormat:@"Send snippet “%@”. Press ⌥ to edit first.",
...
- (iTermOpenQuicklyArrangementItem *)arrangementItemWithName:(NSString *)arrangementName
    ...
    NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
    item.score = [self scoreForArrangementWithName:arrangementName matcher:matcher attributedName:attributedName];
    if (item.score > 0) {
        item.inTabs = inTabs;
        item.detail =
            [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil
                                                              value:inTabs ? @"Restore window arrangement in tabs"
...
- (iTermOpenQuicklyScriptItem *)scriptItemWithName:(NSString *)scriptName
    ...
    NSMutableAttributedString *attributedName = [[NSMutableAttributedString alloc] init];
    item.score = [self scoreForScriptWithName:scriptName matcher:matcher attributedName:attributedName];
    if (item.score > 0) {
        item.detail = [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil
                                                                        value:@"Run Script"
                                                           highlightedIndexes:nil];
...
- (void)addScriptToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
             withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
    NSArray<NSString *> *allScripts =
        [[[[iTermApplication sharedApplication] delegate] scriptsMenuController] allScripts];
...
- (void)updateWithQuery:(NSString *)queryString {
    ...
    iTermMinimumSubsequenceMatcher *matcher = [[iTermMinimumSubsequenceMatcher alloc] initWithQuery:command.text];

    // Pre-allocate for expected number of items (profiles + sessions + arrangements + etc.)
    NSMutableArray *items = [[NSMutableArray alloc] initWithCapacity:64];
...
#endif
        if ([iTermBrowserGateway browserAllowedCheckingIfNot:NO]) {
            // Typical number of bookmark/URL results
            NSMutableSet<NSURL *> *urls = [[NSMutableSet alloc] initWithCapacity:16];
    ...
    [items sortUsingComparator:^NSComparisonResult(iTermOpenQuicklyItem *obj1, iTermOpenQuicklyItem *obj2) {
...
- (iTermVariableScope *)scopeForValidInvocation:(NSString *)invocation {
    ...
    if (scope) {
        iTermParsedExpression *expression = [[iTermExpressionParser callParser] parse:invocation scope:scope];
        if (expression) {
...
- (double)scoreForInvocation:(NSString *)invocation
    ...
    NSMutableArray *invocationFeature = [[NSMutableArray alloc] initWithCapacity:2];
...
- (double)scoreForArrangementWithName:(NSString *)arrangementName
                              matcher:(iTermMinimumSubsequenceMatcher *)matcher
                       attributedName:(NSMutableAttributedString *)attributedName {
    NSMutableArray *nameFeature = [[NSMutableArray alloc] initWithCapacity:2];
    double score = [self scoreUsingMatcher:matcher
    ...
    if (score > 0 && [[WindowArrangements defaultArrangementName] isEqualToString:arrangementName]) {
...
- (double)scoreForSnippet:(iTermSnippet *)snippet
    ...
    NSMutableArray *nameFeature = [[NSMutableArray alloc] initWithCapacity:2];
    double score = [self scoreUsingMatcher:matcher
                                 documents:@[ snippet.title ?: @"", [snippet trimmedValue:80] ?: @"" ]
...
- (double)scoreForProfile:(Profile *)profile
    ...
    if (score > 0 && [[[ProfileModel sharedInstance] defaultBookmark][KEY_GUID] isEqualToString:profile[KEY_GUID]]) {
...
- (double)scoreForSession:(PTYSession *)session
    ...
    NSDictionary<NSString *, NSString *> *userVariablesDict =
    ...
    NSDictionary<NSString *, NSString *> *variables =
        [@{@"tty" : tty ?: [NSNull null], @"pid" : pid.stringValue ?: [NSNull null]}
...
- (iTermTuple<NSString *, NSAttributedString *> *)detailForSession:(PTYSession *)session features:(NSArray *)features {
...
- (double)scoreUsingMatcher:(iTermMinimumSubsequenceMatcher *)matcher
    ...
    // Index set capacity based on typical query length matching document chars
    NSMutableIndexSet *indexSet = [[NSMutableIndexSet alloc] init];
    for (NSString *document in documents) {
        [indexSet removeAllIndexes];
        double value = [self qualityOfMatchWithMatcher:matcher document:[document lowercaseString] indexSet:indexSet];
...
- (NSArray *)hostnamesInHosts:(NSArray<id<VT100RemoteHostReading>> *)hosts {
    NSMutableArray *names = [[NSMutableArray alloc] initWithCapacity:hosts.count];
