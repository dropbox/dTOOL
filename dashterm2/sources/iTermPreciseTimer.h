...
// "_locked" versions for use when caller already holds the timer lock (via iTermPreciseTimerLock).
// These avoid recursive lock attempts on os_unfair_lock which would cause a crash.
...
NSString *iTermPreciseTimerLogString(NSString *identifier,
                                     ...
                                     NSArray *histograms,
                                     BOOL reset);
// "_locked" version for use when caller already holds the timer lock
NSString *iTermPreciseTimerLogString_locked(NSString *identifier,
                                            iTermPreciseTimerStats stats[],
                                            size_t count,
                                            NSArray *histograms,
                                            BOOL reset);
