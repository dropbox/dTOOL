...
class FontTable: NSObject, FontProviderProtocol {
    ...
    private class FontBox: NSObject {
        ...
        static var defaultFont: PTYFontInfo {
            // BUG-1791: Use nil coalescing with fallback instead of force unwrap
        ...
        func dictionaryValue() -> [AnyHashable : Any] {
            // BUG-1728: Use nil coalescing for familyName instead of force unwrap
            var dict: [AnyHashable : Any] = [ "name": font.font.familyName ?? "Unknown", "size": font.font.pointSize ]
            if let delta {
                dict["delta"] = delta
            }
            return dict
        ...
        func copyOf() -> Self {
            // BUG-1726: Use unsafeDowncast which is explicit about the cast being intentional
            // This cast is safe because FontBox is the only implementation of Self for this class
            return unsafeDowncast(FontBox(font, delta: delta), to: Self.self)
        }
    ...
    struct Config: Codable, Equatable, Hashable {
        ...
        init?(string: String) {
            ...
            // BUG-1623: Use guard instead of force unwrap for data encoding
    ...
    private func font(for c: Character,
                      remapped: UnsafeMutablePointer<UTF32Char>) -> PTYFontInfo {
        // BUG-1637: Use nil coalescing to handle missing unicode scalars
        guard let firstScalar = c.unicodeScalars.first else {
            return asciiFont
        }
        return font(for: UTF32Char(firstScalar.value), remapped: remapped)
    }
    ...
    func font(for codePoint: UTF32Char,
        ...
        return cachedRangeMapEntry.mutableAccess { cachedRangeMapEntry in
            ...
            if let cachedRangeMapEntry, cachedRangeMapEntry.range.contains(Int(codePoint)) {
                ...
                // BUG-1638: Use guard instead of force unwrap for range map access
                guard let entry = rangeMap[Int(codePoint)] else {
                    return asciiFont
                }
...
class DualFontProvider: NSObject, FontProviderProtocol {
    ...
    func cloneFontProvider() -> any FontProviderProtocol {
        // BUG-1727: Use as? with fallback to original if copy fails
        return DualFontProvider(asciiFontInfo: (asciiFontInfo.copy() as? PTYFontInfo) ?? asciiFontInfo,
                                nonASCIIFontInfo: (nonASCIIFontInfo.copy() as? PTYFontInfo) ?? nonASCIIFontInfo)
    }
}
