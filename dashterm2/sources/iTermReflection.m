...
- (instancetype)initWithObjectHavingClassName:(NSString *)className argumentName:(NSString *)argumentName {
    self = [super init];
    if (self) {
...
- (instancetype)initWithType:(iTermReflectionMethodArgumentType)type argumentName:(NSString *)argumentName {
    self = [super init];
    if (self) {
        _argumentName = argumentName.copy;
        _type = type;
...
+ (iTermReflectionMethodArgument *)argumentForTypeString:(NSString *)typeString argumentName:(NSString *)argumentName {
    ...
    if (type) {
        return [[iTermReflectionMethodArgument alloc] initWithType:type.unsignedIntegerValue argumentName:argumentName];
    }

    if ([typeString hasPrefix:@"@"]) {
        return [self argumentForObjectTypeString:[typeString substringFromIndex:1] argumentName:argumentName];
    }
    if ([typeString hasPrefix:@"^"]) {
...
+ (iTermReflectionMethodArgument *)argumentForObjectTypeString:(NSString *)typeString
    ...
    // BUG-f1162: Convert assert() to guard - malformed type string should return fallback, not crash
    if (closeQuote.location == NSNotFound) {
        DLog(@"BUG-f1162: Malformed type string missing close quote: %@", typeString);
        return [[iTermReflectionMethodArgument alloc] initWithObjectHavingClassName:@"NSObject"
                                                                       argumentName:argumentName];
    }
    NSString *className = [typeString substringWithRange:NSMakeRange(1, closeQuote.location - 2)];
    return [[iTermReflectionMethodArgument alloc] initWithObjectHavingClassName:className argumentName:argumentName];
}
...
- (instancetype)initWithClass:(Class)theClass selector:(SEL)selector {
    self = [super init];
    if (self) {
        _class = theClass;
        _selector = selector;
...
- (NSArray<iTermReflectionMethodArgument *> *)reflectedArguments {
    ...
    // Pre-allocate based on number of method arguments (minus self and _cmd)
    NSMutableArray<iTermReflectionMethodArgument *> *const args =
        [NSMutableArray arrayWithCapacity:numberOfArguments > 2 ? numberOfArguments - 2 : 0];
    ...
    // BUG-f1163: Convert assert() to guard - argument count mismatch should return empty, not crash
    if (parts.count + 2 != numberOfArguments) {
        DLog(@"BUG-f1163: Argument count mismatch: parts.count=%lu + 2 != numberOfArguments=%u for selector %@",
             (unsigned long)parts.count, numberOfArguments, selector);
        return @[];
    }
