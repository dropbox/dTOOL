...
CGFloat PerceivedBrightness(CGFloat r, CGFloat g, CGFloat b) {
    return (kRedComponentBrightness * r + kGreenComponentBrightness * g + kBlueComponentBrightness * b);
}
...
iTermRGBColor iTermLinearizeSRGB(iTermSRGBColor srgb) {
    ...
    return (iTermRGBColor){.r = pivot(srgb.r), .g = pivot(srgb.g), .b = pivot(srgb.b)};
}
...
iTermSRGBColor iTermCompressRGB(iTermRGBColor rgb) {
    ...
    return (iTermSRGBColor){.r = pivot(rgb.r), .g = pivot(rgb.g), .b = pivot(rgb.b)};
}
...
static iTermXYZColor iTermCompressXYZ(iTermXYZColor compressed) {
    ...
    return (iTermXYZColor){.x = pivot(compressed.x), .y = pivot(compressed.y), .z = pivot(compressed.z)};
}
...
static iTermXYZColor iTermLinearizeXYZ(iTermXYZColor linear) {
    ...
    return (iTermXYZColor){.x = pivot(linear.x), .y = pivot(linear.y), .z = pivot(linear.z)};
}
...
iTermLABColor iTermLABFromSRGB(iTermSRGBColor srgb) {
    ...
    return (iTermLABColor){.l = (116.0 * xyz.y) - 16.0, .a = 500.0 * (xyz.x - xyz.y), .b = 200.0 * (xyz.y - xyz.z)};
}

iTermSRGBColor iTermSRGBFromLAB(iTermLABColor lab) {
    const CGFloat tempY = (lab.l + 16.0) / 116.0;
    iTermXYZColor xyz = {.x = lab.a / 500.0 + tempY, .y = tempY, .z = tempY - lab.b / 200.0};
    ...
    iTermRGBColor rgb = {.r = MAX(MIN(1, xyz.x * 3.2406 + xyz.y * -1.5372 + xyz.z * -0.4986), 0),
...
CGFloat iTermLABBrightnessDistance(iTermLABColor lhs, iTermLABColor rhs) {
    ...
    return fabs(PerceivedBrightness(lsrgb.r, lsrgb.g, lsrgb.b) - PerceivedBrightness(rsrgb.r, rsrgb.g, rsrgb.b));
}
...
CGFloat iTermLABDistance(iTermLABColor lhs, iTermLABColor rhs) {
    ...
    return sqrt(pow(lhs.l - rhs.l, 2) + pow(lhs.a - rhs.a, 2) + pow(lhs.b - rhs.b, 2)) / 100.0;
}
...
CGFloat iTermLABDeltaE2000(iTermLABColor lab1, iTermLABColor lab2) {
    ...
    CGFloat HBarPrime =
        (fabs(h1Prime - h2Prime) > M_PI) ? (h1Prime + h2Prime + 2.0 * M_PI) / 2.0 : (h1Prime + h2Prime) / 2.0;
    ...
    CGFloat deltaE =
        (sqrt(pow(deltaLPrime / (kL * SL), 2.0) + pow((C2Prime - C1Prime) / (kC * SC), 2.0) +
              pow(deltaHPrime / (kH * SH), 2.0) + RT * (C2Prime - C1Prime) * deltaHPrime / (kC * SC * kH * SH)));
...
iTermXYZColor iTermP3ToXYZ(iTermP3Color p3) {
    ...
    const CGFloat P3ToXYZ[3][3] = {{0.4865709486482162, 0.26566769316909306, 0.1982172852343625},
                                   {0.2289745640697488, 0.6917385218365064, 0.079286914093745},
                                   {0.0000000000000000, 0.0451133818589026, 1.0439443689009760}};
    iTermXYZColor xyz;
    xyz.x = P3ToXYZ[0][0] * iTermLinearizeP3(p3.r) + P3ToXYZ[0][1] * iTermLinearizeP3(p3.g) +
            P3ToXYZ[0][2] * iTermLinearizeP3(p3.b);
    xyz.y = P3ToXYZ[1][0] * iTermLinearizeP3(p3.r) + P3ToXYZ[1][1] * iTermLinearizeP3(p3.g) +
            P3ToXYZ[1][2] * iTermLinearizeP3(p3.b);
    xyz.z = P3ToXYZ[2][0] * iTermLinearizeP3(p3.r) + P3ToXYZ[2][1] * iTermLinearizeP3(p3.g) +
            P3ToXYZ[2][2] * iTermLinearizeP3(p3.b);
...
iTermP3Color iTermXYZToLinearP3(iTermXYZColor xyz) {
    ...
    const CGFloat XYZToP3[3][3] = {{2.493496911941425, -0.9313836179191239, -0.40271078445071684},
...
- (iTermSRGBColor)itermSRGBColor {
    ...
    // BUG-1591: colorUsingColorSpace: can return nil for pattern/catalog colors
    if (!srgb) {
        return (iTermSRGBColor){0, 0, 0};
    }
    return (iTermSRGBColor){srgb.redComponent, srgb.greenComponent, srgb.blueComponent};
}
...
+ (NSColor *)it_blue {
    return [NSColor colorWithName:@"iTermBlueTextColor"
...
- (iTermLABColor)labColor {
    ...
    // BUG-3725: Fix swapped green and blue components
    const iTermSRGBColor srgb =
        (iTermSRGBColor){.r = color.redComponent, .g = color.greenComponent, .b = color.blueComponent};
        ...
        case NSColorTypeComponentBased: {
            ...
            switch (colorSpace.colorSpaceModel) {
                ...
                case NSColorSpaceModelGray: {
                    ...
                    return [NSString stringWithFormat:@"%@#%02X", colorSpaceDescription, (int)(white * 255)];
                }
                ...
                case NSColorSpaceModelRGB: {
                    ...
                    return [NSString stringWithFormat:@"%@#%02X%02X%02X", colorSpaceDescription, (int)(red * 255),
                ...
                case NSColorSpaceModelCMYK: {
                    ...
                    return [NSString stringWithFormat:@"%@#%02X%02X%02X%02X", colorSpaceDescription, (int)(cyan * 255),
                                                      (int)(magenta * 255), (int)(yellow * 255), (int)(black * 255)];
                }
...
+ (NSColor *)colorWith8BitRed:(int)red green:(int)green blue:(int)blue {
    if ([iTermAdvancedSettingsModel p3]) {
        return [NSColor colorWithDisplayP3Red:red / 255.0 green:green / 255.0 blue:blue / 255.0 alpha:1];
    } else {
        return [NSColor colorWithSRGBRed:red / 255.0 green:green / 255.0 blue:blue / 255.0 alpha:1];
    }
}

+ (NSColor *)it_dynamicColorForLightMode:(NSColor *)light darkMode:(NSColor *)dark {
    return [NSColor colorWithName:[NSString stringWithFormat:@"iTerm%@_%@DynamicColor", light.shortDescription,
                                                             dark.shortDescription]
                  dynamicProvider:^NSColor *_Nonnull(NSAppearance *appearance) {
                      if (appearance.it_isDark) {
                          return dark;
                      } else {
                          return light;
...
+ (NSColor *)it_automaticDynamicColorForLightModeWhite:(CGFloat)white alpha:(CGFloat)alpha {
...
- (NSColor *)it_darkModeCounterpart {
    ...
    return [NSColor colorWithHue:hue saturation:saturation brightness:invertedBrightness alpha:alpha];
}
...
+ (void)getComponents:(CGFloat *)result
    ...
    CGFloat p = ((t - PerceivedBrightness(c1, c2, c3)) / (PerceivedBrightness(e1 - c1, e2 - c2, e3 - c3)));
...
+ (NSColor *)colorForAnsi256ColorIndex:(int)index {
    ...
    if ([iTermAdvancedSettingsModel p3]) {
        return [NSColor colorWithDisplayP3Red:r green:g blue:b alpha:1];
    } else {
...
+ (void)getComponents:(CGFloat *)result
    ...
    if (brightnessDiff < minimumContrast) {
        ...
        [self getComponents:result forColorWithRed:r green:g blue:b alpha:a perceivedBrightness:targetBrightness];
    } else {
...
- (CGFloat)perceivedBrightness {
    ...
    return PerceivedBrightness([safeColor redComponent], [safeColor greenComponent], [safeColor blueComponent]);
}
...
- (NSDictionary *)dictionaryValue {
    if ([iTermAdvancedSettingsModel p3]) {
        ...
        return @{
            kEncodedColorDictionaryColorSpace : kEncodedColorDictionaryP3ColorSpace,
...
- (NSDictionary *)dictionaryValuePreservingColorSpace {
    ...
    [self getRed:&red green:&green blue:&blue alpha:&alpha];
    return @{
        kEncodedColorDictionaryColorSpace : colorSpaceName,
        kEncodedColorDictionaryRedComponent : @(red),
        kEncodedColorDictionaryGreenComponent : @(green),
        kEncodedColorDictionaryBlueComponent : @(blue),
        kEncodedColorDictionaryAlphaComponent : @(alpha)
...
- (NSColor *)colorByPremultiplyingAlphaWithColor:(NSColor *)background {
    // BUG-1598: Handle nil background - return self unchanged
    if (!background) {
        return self;
    }
    CGFloat a[4];
    CGFloat b[4] = {0, 0, 0, 1}; // Initialize to opaque black as safety default
...
+ (instancetype)colorFromHexString:(NSString *)fullString {
    ...
    if (p3) {
        return [[NSColor colorWithDisplayP3Red:red / 65535.0 green:green / 65535.0 blue:blue / 65535.0
                                         alpha:1] it_colorInDefaultColorSpace];
    } else {
        return [[NSColor colorWithSRGBRed:red / 65535.0 green:green / 65535.0 blue:blue / 65535.0
                                    alpha:1] it_colorInDefaultColorSpace];
    }
}
...
- (NSColor *)it_colorWithAppearance:(NSAppearance *)appearance {
    ...
    // BUG-f1430: Use NSCache for automatic memory pressure eviction
    static NSCache<NSColor *, NSColor *> *darkCache, *lightCache;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        darkCache = [[NSCache alloc] init];
        darkCache.name = @"iTermDarkColorCache";
        darkCache.countLimit = 128;

        lightCache = [[NSCache alloc] init];
        lightCache.name = @"iTermLightColorCache";
        lightCache.countLimit = 128;
    });

    NSCache<NSColor *, NSColor *> *cache;
    NSString *closest =
        [appearance bestMatchFromAppearancesWithNames:@[ NSAppearanceNameDarkAqua, NSAppearanceNameAqua ]];
    ...
    NSColor *result = [cache objectForKey:self];
    if (result) {
        return result;
    }
...
- (NSColorSpace *_Nullable)it_colorSpace {
    @try {
        // JFC apple. colorSpace throws an exception but it's a property so it's a nice little landmine they leave for
        // you.
...
+ (NSColor *)it_colorInDefaultColorSpaceWithRed:(CGFloat)red
                                          green:(CGFloat)green
                                           blue:(CGFloat)blue
                                          alpha:(CGFloat)alpha {
    if ([iTermAdvancedSettingsModel p3]) {
...
- (BOOL)isApproximatelyEqualToColor:(NSColor *)other epsilon:(double)e {
    ...
    if (fabs(self.redComponent - color.redComponent) > e || fabs(self.greenComponent - color.greenComponent) > e ||
        fabs(self.blueComponent - color.blueComponent) > e || fabs(self.alphaComponent - color.alphaComponent) > e) {
        return NO;
    }
    return YES;
}
...
- (vector_float4)vector {
    // BUG-1596: Convert to sRGB first to handle grayscale/pattern colors
    // that don't have redComponent/greenComponent/blueComponent
    NSColor *rgbColor = [self colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
    if (!rgbColor) {
        // Fallback for unconvertible colors (patterns, etc.)
        return simd_make_float4(0, 0, 0, 1);
    }
    return simd_make_float4(rgbColor.redComponent, rgbColor.greenComponent, rgbColor.blueComponent,
...
- (NSColor *)colorByShiftingTowardsColor:(NSColor *)targetColor amount:(CGFloat)amount {
    ...
    iTermSRGBColor sourceSRGBColor = {
        .r = sourceSRGB.redComponent, .g = sourceSRGB.greenComponent, .b = sourceSRGB.blueComponent};

    iTermSRGBColor targetSRGBColor = {
        .r = targetSRGB.redComponent, .g = targetSRGB.greenComponent, .b = targetSRGB.blueComponent};
    ...
    iTermLABColor resultLab = {.l = sourceLab.l + (targetLab.l - sourceLab.l) * amount,
    ...
    NSColor *result = [NSColor colorWithSRGBRed:resultSRGB.r green:resultSRGB.g blue:resultSRGB.b alpha:resultAlpha];
