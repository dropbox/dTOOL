...
- (NSString *)stringWithBase64EncodingWithLineBreak:(NSString *)lineBreak {
    ...
    NSMutableString *string = [[NSMutableString alloc] initWithCapacity:length + (length / 77 + 1) * lineBreak.length];
...
+ (NSData *)dataWithTGZContainingFiles:(NSArray<NSString *> *)files
    ...
    if (errorMessage.length) {
        ...
        if (errorString) {
            if (error) {
                *error = [NSError errorWithDomain:@"com.dashterm.dashterm2" code:1 userInfo:@{ @"errorMessage": errorString }];
            }
        } else {
...
- (id)it_unarchivedObjectOfClasses:(NSArray<Class> *)allowedClasses {
    ...
    // Use secure coding since we're specifying allowed classes
...
- (NSString *)shortDebugString {
    ...
    NSMutableString *result = [[NSMutableString alloc] initWithCapacity:self.length * 4];
...
- (NSString * _Nullable)humanFriendlyStringRepresentation {
    ...
    if (utf8String && containsNonWhitespace) {
        NSMutableString *sanitizedString = [[NSMutableString alloc] initWithCapacity:utf8String.length * 2];
    ...
    NSMutableString *hexString = [[NSMutableString alloc] initWithCapacity:length * 3];
...
- (NSData *)aesCBCEncryptedDataWithPCKS7PaddingAndKey:(NSData *)key
                                                   iv:(NSData *)iv {
    // BUG-f1336: Replace assert() with guard - invalid key/iv should return nil, not crash
    if (iv.length != 16) {
        ELog(@"aesCBCEncryptedData: Invalid IV length %lu (expected 16)", (unsigned long)iv.length);
        return nil;
    }
    if (key.length != 16) {
        ELog(@"aesCBCEncryptedData: Invalid key length %lu (expected 16)", (unsigned long)key.length);
        return nil;
    }
...
- (NSData *)decryptedAESCBCDataWithPCKS7PaddingAndKey:(NSData *)key
                                                   iv:(NSData *)iv {
    // BUG-f1337: Replace assert() with guard - invalid key/iv should return nil, not crash
    if (iv.length != 16) {
        ELog(@"decryptedAESCBCData: Invalid IV length %lu (expected 16)", (unsigned long)iv.length);
        return nil;
    }
    if (key.length != 16) {
        ELog(@"decryptedAESCBCData: Invalid key length %lu (expected 16)", (unsigned long)key.length);
        return nil;
    }
...
+ (NSData *)randomAESKey {
    ...
    const int result = SecRandomCopyBytes(kSecRandomDefault,
                                          length,
                                          data.mutableBytes);
    // BUG-f1338: Replace assert() with guard - return nil if random bytes cannot be generated
    if (result != 0) {
        ELog(@"randomAESKey: SecRandomCopyBytes failed with error %d", result);
        return nil;
    }
...
- (void)writeReadOnlyToURL:(NSURL *)url {
    ...
    // Create it exclusively. If another instance of DashTerm2 is trying to create it, back off.
    int fd = -1;
    do {
