...
class TerminalButton: NSObject {
    ...
    init(id: Int, backgroundImage: NSImage, foregroundImage: NSImage, mark: iTermMarkProtocol?, tooltip: String) {
        ...
        self.mark = mark
        // BUG-516: Guard against division by zero when image width is zero
        aspectRatio = foregroundImage.size.width > 0 ? foregroundImage.size.height / foregroundImage.size.width : 1.0;
    ...
    required init?(_ original: TerminalButton) {
        ...
        // BUG-517: Guard against division by zero when image width is zero
        let originalWidth = original.tintedForegroundImage.original.size.width
        aspectRatio = originalWidth > 0 ? original.tintedForegroundImage.original.size.height / originalWidth : 1.0;
    ...
    @objc func clone() -> Self {
        guard let cloned = Self(self) else {
            // BUG-f560, BUG-f680: Log error and return self as fallback instead of crashing
            DLog("BUG-f560: Clone failed for TerminalButton - init should never return nil for existing instance - returning self")
            // Return self as fallback - not ideal but better than crashing
            return self
        }
        return cloned
    }
    ...
    @objc func size(cellSize: NSSize) -> NSSize {
        ...
        // BUG-518: Guard against division by zero when result height is zero
        guard result.height > 0 else {
...
class GenericBlockButton: TerminalButton {
    ...
    required init?(_ original: TerminalButton) {
        // BUG-1654: Use guard with as? instead of force cast
        guard let downcast = original as? GenericBlockButton else { return nil }
    ...
    override func clone() -> Self {
        guard let cloned = Self(self) else {
            // BUG-f561, BUG-f681: Log error and return self as fallback instead of crashing
            DLog("BUG-f561: Clone failed for GenericBlockButton - init should never return nil for existing instance - returning self")
            // Return self as fallback - not ideal but better than crashing
            return self
        }
        return cloned
...
class TerminalRevealChannelButton: TerminalButton {
    ...
    required init?(_ original: TerminalButton) {
        // BUG-1654: Use guard with as? instead of force cast
        guard let downcast = original as? TerminalRevealChannelButton else { return nil }
    ...
    override func clone() -> Self {
        guard let cloned = Self(self) else {
            // BUG-f562, BUG-f682: Log error and return self as fallback instead of crashing
            DLog("BUG-f562: Clone failed for TerminalRevealChannelButton - init should never return nil for existing instance - returning self")
            // Return self as fallback - not ideal but better than crashing
            return self
        }
        return cloned
...
class TerminalFoldBlockButton: GenericBlockButton {
    ...
    required init?(_ original: TerminalButton) {
        // BUG-1654: Use guard with as? instead of force cast
        guard let downcast = original as? TerminalFoldBlockButton else { return nil }
        absLineRange = downcast.absLineRange
        folded = downcast.folded
        super.init(original)
    }
}
...
class TerminalMarkButton: TerminalButton {
    ...
    required init?(_ original: TerminalButton) {
        // BUG-1654: Use guard with as? instead of force cast
        guard let downcast = original as? TerminalMarkButton else { return nil }
    ...
    override func clone() -> Self {
        guard let cloned = Self(self) else {
            // BUG-f563, BUG-f683: Log error and return self as fallback instead of crashing
            DLog("BUG-f563: Clone failed for TerminalMarkButton - init should never return nil for existing instance - returning self")
            // Return self as fallback - not ideal but better than crashing
            return self
        }
        return cloned
