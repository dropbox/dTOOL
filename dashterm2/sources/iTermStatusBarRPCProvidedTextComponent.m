...
static NSString *const iTermStatusBarRPCRegistrationRequestKey_Deprecated =
    @"registration request"; // legacy, NSData value which cannot be JSON encoded for profile export or Python API
static NSString *const iTermStatusBarRPCRegistrationRequestV2Key =
    @"registration request v2"; // use this instead, has a base64-encoded string
...
- (NSDictionary *)statusBarConfiguration {
    return @{
        iTermStatusBarRPCRegistrationRequestV2Key : [self.data base64EncodedStringWithOptions:0],
...
- (instancetype)latestStatusBarRequest {
    return
        [iTermAPIHelper registrationRequestForStatusBarComponentWithUniqueIdentifier:self.statusBarComponentAttributes
                                                                                         .uniqueIdentifier]
            ?: self;
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        ...
        _savedRegistrationRequest = [[ITMRPCRegistrationRequest alloc] initWithData:data error:&parseError];
        if (!_savedRegistrationRequest) {
            if (parseError) {
                XLog(@"Failed to decode RPC registration request: %@", parseError);
            }
            return nil;
        }
    }
...
- (NSDictionary *)defaultKnobs {
    NSMutableDictionary *knobs =
        [[NSMutableDictionary alloc] initWithCapacity:_savedRegistrationRequest.latestStatusBarRequest
                                                          .statusBarComponentAttributes.knobsArray.count];
    [_savedRegistrationRequest.latestStatusBarRequest.statusBarComponentAttributes.knobsArray
        enumerateObjectsUsingBlock:^(ITMRPCRegistrationRequest_StatusBarComponentAttributes_Knob *_Nonnull obj,
...
- (instancetype)initWithRegistrationRequest:(ITMRPCRegistrationRequest *)registrationRequest
    ...
    return [self initWithConfiguration:@{
        iTermStatusBarRPCRegistrationRequestV2Key : [registrationRequest.data base64EncodedStringWithOptions:0],
...
- (instancetype)initWithConfiguration:(NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)configuration
    ...
    if (!registrationRequest) {
        if (parseError) {
            XLog(@"Failed to parse RPC registration request from configuration: %@", parseError);
        }
        return nil;
    }
    self = [super init];
...
- (void)finishInitialization {
    ...
    [_dependencies enumerateObjectsUsingBlock:^(iTermVariableReference *_Nonnull obj, NSUInteger idx,
                                                BOOL *_Nonnull stop) {
        obj.onChangeBlock = ^{
...
- (NSString *)invocation {
    NSArray<ITMRPCRegistrationRequest_RPCArgument *> *defaults =
        _savedRegistrationRequest.latestStatusBarRequest.defaultsArray ?: @[];
    ITMRPCRegistrationRequest_RPCArgument *knobs = [[ITMRPCRegistrationRequest_RPCArgument alloc] init];
    knobs.name = @"knobs";
    knobs.path = @"__knobs";
    return [iTermAPIHelper
        invocationWithFullyQualifiedName:_savedRegistrationRequest.latestStatusBarRequest.it_fullyQualifiedName
...
- (id<iTermStatusBarComponentFactory>)statusBarComponentFactory {
    return [[iTermStatusBarRPCComponentFactory alloc]
        initWithRegistrationRequest:_savedRegistrationRequest.latestStatusBarRequest];
}
...
- (nullable NSImage *)statusBarComponentIcon {
    ...
    NSArray<NSBitmapImageRep *> *reps =
        [_savedRegistrationRequest.latestStatusBarRequest.statusBarComponentAttributes.iconsArray
            mapWithBlock:^id(ITMRPCRegistrationRequest_StatusBarComponentAttributes_Icon *proto) {
...
- (iTermStatusBarComponentKnobType)knobTypeFromDescriptorType:
...
- (NSArray<iTermStatusBarComponentKnob *> *)amendedStatusBarComponentKnobs:
    (NSArray<iTermStatusBarComponentKnob *> *)superKnobs {
    NSArray<iTermStatusBarComponentKnob *> *knobs;
    knobs = [_savedRegistrationRequest.latestStatusBarRequest.statusBarComponentAttributes.knobsArray
                mapWithBlock:^id(ITMRPCRegistrationRequest_StatusBarComponentAttributes_Knob *descriptor) {
                    return [[iTermStatusBarComponentKnob alloc]
                        initWithLabelText:descriptor.name
                                     type:[self knobTypeFromDescriptorType:descriptor.type]
                              placeholder:descriptor.hasPlaceholder ? descriptor.placeholder : nil
                             defaultValue:descriptor.hasJsonDefaultValue
                                              ? [NSJSONSerialization it_objectForJsonString:descriptor.jsonDefaultValue]
                                              : nil
                                      key:descriptor.key];
                }]
                ?: @[];
...
- (void)updateWithKnobValues:(NSDictionary<NSString *, id> *)knobValues {
    ...
    DLog(@"Update status bar component %@ instance %p for session %@\n%@",
         _savedRegistrationRequest.statusBarComponentAttributes.uniqueIdentifier, self,
         [scope valueForVariableName:iTermVariableKeySessionID], [NSThread callStackSymbols]);
    ...
    [iTermScriptFunctionCall
        callFunction:self.invocation
             timeout:_savedRegistrationRequest.latestStatusBarRequest.timeout
...
- (void)maybeOfferToMoveScriptToAutoLaunch {
    ...
    iTermScriptsMenuController *menuController =
        [[[iTermApplication sharedApplication] delegate] scriptsMenuController];
    ...
    if ([iTermWarning showWarningWithTitle:@"This will move the script into the AutoLaunch folder."
                               ...
                               silenceable:kiTermWarningTypePermanentlySilenceable
                                   heading:@"Always launch this script when DashTerm2 starts?"
...
- (void)handleSuccessfulEvaluationWithValue:(id)value {
    ...
    if (stringValue) {
        ...
        _errorMessage = [NSString stringWithFormat:@"Return value from %@ invalid.\n\nIt should have returned a string "
                                                   @"or a list of strings.\n\nInstead, it returned:\n\n%@",
                                                   self.invocation, value];
        [[iTermAPIHelper sharedInstance]
            logToConnectionHostingFunctionWithSignature:_savedRegistrationRequest.latestStatusBarRequest
                                                            .it_stringRepresentation
...
- (void)handleEvaluationError:(NSError *)error missingFunctions:(NSSet<NSString *> *)missingFunctions {
    _errorMessage = [NSString
        stringWithFormat:@"Status bar component “%@” (%@) failed.\n\nThis function call had an error:\n\n%@\n\nThe "
                         @"error was:\n\n%@\n\n%@",
                         ...
                         self.invocation, error.localizedDescription,
...
- (BOOL)scriptIsNotRunningButCouldBeLaunched {
    ...
    return [[[[iTermApplication sharedApplication] delegate] scriptsMenuController]
        couldLaunchScriptWithAbsolutePath:fullPath];
}
...
- (nullable NSString *)fullPathOfScript {
    ...
    return [iTermAPIHelper
        nameOfScriptVendingStatusBarComponentWithUniqueIdentifier:_savedRegistrationRequest.latestStatusBarRequest
...
- (void)onClick:(id)sender {
    if (_errorMessage) {
        ...
        if ([self scriptIsNotRunningButCouldBeLaunched]) {
            iTermWarningAction *launch = [iTermWarningAction warningActionWithLabel:@"Launch Script"
                                                                              block:^(iTermWarningSelection selection) {
                                                                                  [self launchScript];
                                                                              }];
            iTermWarningAction *reveal = [iTermWarningAction warningActionWithLabel:@"Reveal in Finder"
                                                                              block:^(iTermWarningSelection selection) {
                                                                                  [self revealInFinder];
                                                                              }];
            actions = [actions arrayByAddingObjectsFromArray:@[ launch, reveal ]];

            warning.title = [NSString
                stringWithFormat:@"%@It looks like the script is not running. Launching it might fix the problem.",
                                 _errorMessage];
        }
    ...
    NSString *identifier = [[self.statusBarComponentIdentifier
                               ...
                               completion:^(id result, NSError *error, NSSet<NSString *> *mutations) {
                                   if (error) {
                                       NSString *message = [NSString
                                           stringWithFormat:@"Error in onclick handler: %@\n%@",
...
- (void)itermWebViewJavascriptError:(NSString *)errorText {
    NSError *error = nil;
    NSString *signature = [iTermExpressionParser signatureForFunctionCallInvocation:self.invocation error:&error];
    if (!signature && error) {
        signature = error.localizedDescription;
    }
    [[iTermAPIHelper sharedInstance] logToConnectionHostingFunctionWithSignature:signature string:errorText];
    [[iTermAPIHelper sharedInstance]
        logToConnectionHostingFunctionWithSignature:signature
                                             string:@"Right-click in the webview and choose Inspect Element to open "
                                                    @"the Web Inspector."];
}
...
@interface iTermStatusBarRPCProvidedTextComponent () <iTermObject, iTermStatusBarRPCProvidedTextComponentCommonDelegate,
                                                      iTermStatusBarRPCProvidedTextComponent>
@end
...
- (instancetype)initWithConfiguration:(NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)configuration
                                scope:(nullable iTermVariableScope *)realScope {
    iTermStatusBarRPCProvidedTextComponentCommon *common =
        [[iTermStatusBarRPCProvidedTextComponentCommon alloc] initWithConfiguration:configuration scope:realScope];
    if (!common) {
        return nil;
    }
...
@interface iTermStatusBarRPCProvidedAttributedTextComponent () <
    iTermObject, iTermStatusBarRPCProvidedTextComponentCommonDelegate, iTermStatusBarRPCProvidedTextComponent>
@end
