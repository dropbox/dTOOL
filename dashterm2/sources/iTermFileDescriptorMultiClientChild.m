...
- (instancetype)initWithReport:(iTermMultiServerReportChild *)report thread:(iTermThread *)thread {
    self = [super init];
    if (self) {
        _thread = thread;
        _pid = report->pid;
        _executablePath = [[NSString alloc] initWithUTF8String:report->path];
        NSMutableArray<NSString *> *args = [NSMutableArray arrayWithCapacity:report->argc];
        ...
        NSMutableDictionary<NSString *, NSString *> *environment = [NSMutableDictionary dictionaryWithCapacity:report->envc];
        for (int i = 0; i < report->envc; i++) {
            ...
            if (equals == NSNotFound) {
                // BUG-409: Skip malformed environment variable instead of crashing
                // Environment variables should always have = but handle gracefully if not
                continue;
            }
        ...
        _fd = report->fd;
        // BUG-f976: Guard fd validity instead of assert - invalid fd should return nil
        if (_fd < 0) {
            return nil;
        }
        ...
        _haveWaited = NO;
        _callbacks = [NSMutableArray arrayWithCapacity:4];  // Typical callback count
    }
    return self;
}
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<%@: %p pid=%@ fd=%@>", NSStringFromClass(self.class), self, @(self.pid), @(self.fd)];
}
...
- (void)willWaitPreemptively {
    [_thread dispatchRecursiveSync:^(id _Nonnull state) {
        // BUG-f977: Guard preemptive wait state instead of assert - double call should be no-op
        if (_haveSentPreemptiveWait) {
            return;
        }
...
- (void)setTerminationStatus:(int)status {
    [_thread dispatchRecursiveSync:^(id _Nonnull state) {
        // BUG-f978: Guard terminated state instead of assert - set terminated if not already
        if (!_hasTerminated) {
            _hasTerminated = YES;
        }
        _haveWaited = YES;
