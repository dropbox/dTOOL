...
static dispatch_queue_t iTermPathFinderQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("com.dashterm.dashterm2.path-finder", DISPATCH_QUEUE_SERIAL);
    });
    return queue;
}
...
- (void)searchSynchronously {
    ...
    if (!workingDirectoryIsOk) {
        DLog(@"Working directory %@ is a network share or doesn't exist. Not using it for context.", _workingDirectory);
    }

    DLog(@"Brute force path from prefix <<%@>>, suffix <<%@>> directory=%@", _beforeStringIn, _afterStringIn,
         _workingDirectory);
    ...
    NSMutableString *left = [[NSMutableString alloc] initWithCapacity:_beforeStringIn.length];
    int iterationsBeforeQuitting = 100; // Bail after 100 iterations if nothing is still found.
    NSMutableSet *paths = [[NSMutableSet alloc] initWithCapacity:32];
    ...
    for (NSInteger i = [beforeChunks count]; i >= 0; i--) {
        ...
        NSMutableString *right = [[NSMutableString alloc] initWithCapacity:_afterStringIn.length];
        ...
        for (int j = 0; j < MAX(1, afterChunks.count) && j < 10; j++) {
            ...
            for (NSString *modifiedPossiblePath in [self pathsFromPath:trimmedPath
                                                 byRemovingBadSuffixes:questionableSuffixes]) {
                if (self.canceled) {
                    _path = nil;
                    return;
                }
                ...
                if (exists) {
                    ...
                    if (_trimWhitespace) {
                        n = (int)(
                            [[right stringByTrimmingTrailingCharactersFromCharacterSet:whitespaceCharset] length] -
                            lengthOfBadSuffix);
                    } else {
...
- (NSArray<NSString *> *)splitString:(NSString *)string {
    NSMutableArray<NSString *> *parts = [NSMutableArray arrayWithCapacity:16]; // Path string parts
...
- (NSArray *)pathsFromPath:(NSString *)source byRemovingBadSuffixes:(NSArray *)badSuffixes {
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:badSuffixes.count + 1];
...
- (NSString *)columnAndLineNumberFromChunks:(NSArray<NSString *> *)afterChunks {
    ...
    NSArray<NSString *> *regexes = @[
        @"^(:\\d+:\\d+)", @"^(:\\d+)", @"^(\\[\\d+, ?\\d+])", @"^(\", line \\d+, column \\d+)", @"^(\", line \\d+, in)",
        @"^(\\(\\d+, ?\\d+\\))", @"^(\\(\\d+\\))", @"^( line \\d+:$)"
    ];
