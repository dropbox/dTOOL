...
static NSString *const iTermToolActionsPasteboardType = @"com.dashterm.dashterm2.iTermToolActionsPasteboardType";
...
// BUG-3126: Cancel delayed fixCursor and scrollToEndOfDocument selectors to
// prevent crash if tool is deallocated while an update is pending.
- (void)dealloc {
    [NSObject cancelPreviousPerformRequestsWithTarget:self];
}

static NSButton *iTermToolActionsNewButton(NSString *imageName, NSString *title, id target, SEL selector,
                                           NSRect frame) {
...
- (void)relayout {
    ...
    [_applyButton
        setFrame:NSMakeRect(0, frame.size.height - kButtonHeight, _applyButton.frame.size.width, kButtonHeight)];
    ...
    for (NSButton *button in @[ _addButton, _removeButton, _editButton ]) {
        ...
        x -= width + margin;
        button.frame = NSMakeRect(x, frame.size.height - kButtonHeight, width, kButtonHeight);
    }
...
// BUG-10343: Add bounds check to prevent array index out of bounds crash
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    NSTableCellView *cell =
        [tableView newTableCellViewWithTextFieldUsingIdentifier:identifier
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    NSMutableArray<NSNumber *> *identifiers = [NSMutableArray arrayWithCapacity:4]; // Action identifiers being dragged
    [info
    ...
    [[iTermActionsModel sharedInstance] moveActionsWithIdentifiers:identifiers toIndex:row];
    return YES;
}
...
- (void)pushUndo {
    // BUG-1200: Deep copy actions array to prevent undo state corruption from in-place mutations
