...
- (NSArray<NSNumber *> *)desiredSeparatorOffsets {
    NSMutableArray<NSNumber *> *offsets = [NSMutableArray arrayWithCapacity:_visibleContainerViews.count];
...
- (void)updateViews {
    NSMutableArray<iTermStatusBarContainerView *> *updatedContainerViews = [NSMutableArray arrayWithCapacity:_layout.components.count + 3];  // + temp components and placeholder
...
- (void)updateAutoRainbowColorsIfNeeded {
    ...
    // BUG-1490: Capture array locally to protect against concurrent mutation
    NSArray<iTermStatusBarContainerView *> *views = [_visibleContainerViews copy];
    const NSUInteger viewCount = views.count;
    [_autoRainbowController enumerateColorsWithCount:viewCount block:^(NSInteger i, NSColor * _Nonnull color) {
        // BUG-1490: Bounds check before array access
        if (i < 0 || (NSUInteger)i >= viewCount) {
            return;
        }
