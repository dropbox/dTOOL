        ...
        [[iTermBuiltInFunction alloc] initWithName:@"alert"
                                 ...
                                 optionalArguments:[NSSet setWithObject:window_id]
                                     defaultValues:@{window_id : @""}
                                           context:iTermVariablesSuggestionContextNone sideEffectsPlaceholder:nil
...
+ (NSDictionary *)polyModalCompletion:(iTermPolyModalItems *)polyModalItems tag:(NSModalResponse)tag {
    // Result contains button, tf_text, checks, combo - at most 4 keys
    ...
    if (polyModalItems.comboBox) {
        ...
        if (comboBoxIndex >= 0) {
            result[@"combo"] = [polyModalItems.comboBox itemObjectValueAtIndex:comboBoxIndex];
        } else {
            result[@"combo"] = @"";
        }
    }
...
+ (void)showPolyModalAlert:(NSString *)title
    ...
    // Stack views: combobox + checkboxes + text field = typically 1-5 items
    NSMutableArray *stackViews = [NSMutableArray arrayWithCapacity:4];
    if ([comboboxItems count] > 0) {
    ...
    if ([checkboxes count] > 0) {
        // Pre-size based on checkbox count
        NSMutableArray *checkboxViews = [NSMutableArray arrayWithCapacity:[checkboxes count]];
        NSUInteger idx = 0;
