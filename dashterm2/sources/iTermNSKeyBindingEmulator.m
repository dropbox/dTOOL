...
static struct {
    unichar c;
    NSUInteger mask;
} gModifiers[] = {{'^', NSEventModifierFlagControl},
...
+ (NSString *)userFile {
    ...
    NSString *bindPath = [paths[0] stringByAppendingPathComponent:@"KeyBindings/DefaultKeyBinding.dict"];
    return bindPath;
}
...
- (void)loadKeyBindingsDictionary:(NSString *)bindPath allowedActions:(NSSet<NSString *> *)allowedActions {
    // Typical key binding files have a modest number of entries
    NSMutableSet<NSString *> *temp = [NSMutableSet setWithCapacity:64];
...
+ (NSDictionary *)keyBindingDictionaryByPruningUselessBranches:(NSDictionary *)node
    ...
    // key bindings at all.
    NSMutableDictionary *replacement = [NSMutableDictionary dictionaryWithCapacity:node.count];
...
+ (NSDictionary *)keyBindingDictionaryByNormalizingModifiersInKeys:(NSDictionary *)input {
    ...
    for (NSString *key in input) {
        ...
        NSString *normalizedKey = [self dictionaryKeyForCharacters:characters andFlags:flags];
        if (!normalizedKey) {
            DLog(@"Bogus key in key bindings dictionary: %@", [key dataUsingEncoding:NSUTF16BigEndianStringEncoding]);
...
+ (NSString *)dictionaryKeyForCharacters:(NSString *)nonNormalChars andFlags:(NSUInteger)nonNormalFlags {
    ...
    // Modifiers (max 4 chars) + characters
    NSMutableString *theKey = [NSMutableString stringWithCapacity:4 + characters.length];
    for (int i = 0; i < sizeof(gModifiers) / sizeof(gModifiers[0]); i++) {
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _savedEvents = [[NSMutableArray alloc] initWithCapacity:8]; // Key binding events
    }
    return self;
}
...
- (BOOL)handlesEvent:(NSEvent *)event
    ...
    *pointlessly = NO;
    if (!_root && [_pointlessFirstKeystrokes count] == 0) {
        DLog(@"Short-circuit DefaultKeyBindings handling because no bindings are defined and there are no pointless "
             @"leaders");
    ...
    if (possibleKeys.count == 0) {
        ...
        DLog(@"WARNING: Unexpected charactersIgnoringModifiers=%@ in event %@", event.charactersIgnoringModifiers,
             event);
        [_savedEvents removeAllObjects];
        return NO;
    }
...
- (NSString *)charactersIgnoringAllModifiersInEvent:(NSEvent *)event {
    ...
    const OSStatus status = UCKeyTranslate(
        keyboardLayout, keyCode, kUCKeyActionDisplay, 0, LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &deadKeyState,
        sizeof(unicodeString) / sizeof(unicodeString[0]), &actualStringLength, unicodeString);
    CFRelease(keyboard);
    ...
    NSString *theString =
        (__bridge_transfer NSString *)CFStringCreateWithCharacters(kCFAllocatorDefault, unicodeString, 1);
...
- (NSString *)fallbackCharactersIgnoringAllModifiersInEvent:(NSEvent *)event {
    ...
    const CGEventFlags mask = (kCGEventFlagMaskHelp | kCGEventFlagMaskSecondaryFn | kCGEventFlagMaskNumericPad |
...
- (NSArray *)dictionaryKeysForEvent:(NSEvent *)event {
    ...
    // Pre-allocate for typical case (1-2 keys)
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:2];

    NSString *theKey = [self.class dictionaryKeyForCharacters:charactersIgnoringModifiersExceptShift andFlags:flags];
    if (theKey) {
        [result addObject:theKey];
    }

    NSString *charactersIgnoringAllModifiers = [self charactersIgnoringAllModifiersInEvent:event];
    if (charactersIgnoringAllModifiers && (flags & NSEventModifierFlagShift) &&
        ...
        theKey = [self.class dictionaryKeyForCharacters:charactersIgnoringModifiersExceptShift
