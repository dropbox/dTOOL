...
class RangeArray: NSObject {
    ...
    subscript(_ i: Int) -> NSRange {
        // BUG-f992: Guard against out of bounds access
        // Return empty range for invalid indices instead of crashing
        guard i >= 0 && i < ranges.count else {
            DLog("WARNING: RangeArray subscript: index \(i) out of bounds (count=\(ranges.count))")
            return NSRange(location: NSNotFound, length: 0)
        }
        return NSRange(ranges[i])
    }
}
...
fileprivate struct IntermediateLookupTable {
    ...
    init(line: CTLine, deltas: UnsafePointer<Int32>, count: Int) {
        self.count = count
        // BUG-1710: Use as? with empty array fallback instead of as! for CTLineGetGlyphRuns
...
class BidiDisplayInfoObjc: NSObject {
    ...
    // BUG-2781: Use ContiguousArray for stable memory layout when exposing pointers to ObjC.
    // The previous implementation returned pointers from withUnsafeBufferPointer closures,
    // which is undefined behavior as the pointer is only valid inside the closure.
    // ContiguousArray guarantees contiguous storage and the pointer remains valid as long
    // as this class instance exists (which matches the expected lifetime from ObjC callers).
    private lazy var _lutStorage: ContiguousArray<Int32> = {
        ContiguousArray(guts.lut)
    }()

    private lazy var _inverseLUTStorage: ContiguousArray<Int32> = {
        let lut = guts.lut
        guard let max = lut.max() else {
            return ContiguousArray()
        }
        var result = ContiguousArray(0..<Int32(max + 1))
    ...
    // Raw pointer to storage. Safe to access from ObjC as ContiguousArray guarantees
    // stable storage that won't be relocated while this object exists.
    private var _lutPointer: UnsafeMutablePointer<Int32>?
    private var _inverseLUTPointer: UnsafeMutablePointer<Int32>?
    ...
    @objc var lut: UnsafePointer<Int32> {
        // Ensure storage is initialized and get stable pointer
        _ = _lutStorage  // Force lazy initialization
        if _lutPointer == nil {
            _lutPointer = _lutStorage.withUnsafeMutableBufferPointer { buffer in
                buffer.baseAddress
            }
        }
        guard let ptr = _lutPointer else {
            // BUG-440: Replace it_fatalError with fallback - return pointer to empty array instead of crashing
            DLog("BidiDisplayInfo.lut: buffer has no base address. Returning fallback pointer.")
            return UnsafePointer(&Self.fallbackLUTValue)
        }
        return UnsafePointer(ptr)
    }

    // BUG-440: Fallback value for when buffer pointer is nil
    private static var fallbackLUTValue: Int32 = 0
    private static var fallbackInverseLUTValue: Int32 = 0

    @objc var inverseLUT: UnsafePointer<Int32> {
        // Ensure storage is initialized and get stable pointer
        _ = _inverseLUTStorage  // Force lazy initialization
        if _inverseLUTPointer == nil {
            _inverseLUTPointer = _inverseLUTStorage.withUnsafeMutableBufferPointer { buffer in
                buffer.baseAddress
            }
        }
        guard let ptr = _inverseLUTPointer else {
            // BUG-441: Replace it_fatalError with fallback - return pointer to empty array instead of crashing
            DLog("BidiDisplayInfo.inverseLUT: buffer has no base address. Returning fallback pointer.")
            return UnsafePointer(&Self.fallbackInverseLUTValue)
        }
        return UnsafePointer(ptr)
    }

    @objc var inverseLUTCount: Int32 {
        Int32(_inverseLUTStorage.count)
    }
    ...
    func logicalForVisual(_ visual: Int32) -> Int32 {
        ...
        if visual >= _inverseLUTStorage.count {
            return visual
        }
        return _inverseLUTStorage[Int(visual)]
    }
...
fileprivate struct Chunk {
    ...
    init?(_ value: Any) {
        if let i = value as? Int32 {
            ...
            // BUG-355, BUG-361: Guard against array bounds crash
            // If plist/serialization provides a malformed array with < 3 elements, crash occurs
    ...
    // BUG-f506: Handle unexpected stride values gracefully instead of crashing
    var plistValues: [Any] {
        switch stride {
        case 0:
            return [start]
        ...
        default:
            // BUG-f687: For any other stride, encode as [start, count, stride] array and log
            DLog("BUG-f687: Unexpected stride value \(stride) in BidiDisplayInfo - encoding as array")
            return [[start, count, stride]]
        }
    }
...
struct BidiDisplayInfo: CustomDebugStringConvertible, Equatable {
    ...
    init?(_ sca: ScreenCharArray) {
        ...
        // BUG-1757: Use guard let instead of force unwrap for ScreenCharArrayToString result
        guard let string = ScreenCharArrayToString(sca.line, 0, nonEmptyCount, &buffer, &deltas) else {
            return nil
        }
        defer {
            free(deltas)
        ...
        // BUG-1758: Use guard let instead of force unwrap for deltas
        guard let safeDeltas = deltas else {
            return nil
        }
    ...
    private static func pad(lut: [Int32], width: Int32, paragraphIsRTL: Bool) -> [Int32] {
        ...
        // BUG-f506: Return unchanged if width is not greater than base length
        guard width > baseLength else {
            DLog("pad: width \(width) <= baseLength \(baseLength), returning original lut")
            return lut
        }
    ...
    func subInfo(range nsrange: NSRange) -> BidiDisplayInfo? {
        // BUG-1664: Use guard instead of force unwrap for Range conversion
        guard let swiftRange = Range(nsrange) else { return nil }
        let range = swiftRange.clamped(to: 0..<lut.count)
        ...
        // BUG-1807: Use compactMap with nil coalescing instead of force unwrap
