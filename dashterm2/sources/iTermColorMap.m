...
// Optimization: Cache NSNumber objects for common color map keys (0-265) to avoid boxing.
// Keys 0-9 are special colors, 10-265 are 8-bit ANSI colors. Extended colors are rare.
static const int kColorMapCachedKeyCount = kColorMap8bitBase + kColorMapNumberOf8BitColors; // 266
static NSNumber *sColorMapCachedKeys[kColorMapCachedKeyCount];

// Returns a cached NSNumber for common keys, or creates a new one for rare keys.
NS_INLINE NSNumber *iTermColorMapKeyToNumber(int key) {
    if (key >= 0 && key < kColorMapCachedKeyCount) {
        return sColorMapCachedKeys[key];
    }
    return @(key);
}

__attribute__((constructor)) static void iTermColorMapInitializeCachedKeys(void) {
    for (int i = 0; i < kColorMapCachedKeyCount; i++) {
        sColorMapCachedKeys[i] = @(i);
    }
}
...
+ (iTermColorMapKey)keyFor8bitRed:(int)red green:(int)green blue:(int)blue {
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _map = [[NSMutableDictionary alloc] initWithCapacity:32];
        _fastMap = [[NSMutableDictionary alloc] initWithCapacity:32];
        _faintTextAlpha = 0.5;
    }
    return self;
}
...
- (void)setColor:(NSColor *)colorInArbitrarySpace forKey:(iTermColorMapKey)theKey {
    ...
    if (!colorInArbitrarySpace) {
        NSNumber *keyNum = iTermColorMapKeyToNumber(theKey);
        [_map removeObjectForKey:keyNum];
        [_fastMap removeObjectForKey:keyNum];
        return;
    }

    NSNumber *keyNum = iTermColorMapKeyToNumber(theKey);
    NSColor *theColor = [colorInArbitrarySpace colorUsingColorSpace:[NSColorSpace it_defaultColorSpace]];
    // BUG-1593: colorUsingColorSpace: can return nil for pattern/catalog colors
    if (!theColor) {
        // Fallback: try sRGB, then give up
        theColor = [colorInArbitrarySpace colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
        if (!theColor) {
            return;
        }
    }
    NSColor *oldColor = _map[keyNum];
    {
    ...
    _map[keyNum] = theColor;

    // Get components again, now in the default color space (which might be the same)
    CGFloat components[4];
    [theColor getComponents:components];
    vector_float4 value = {(float)components[0], (float)components[1], (float)components[2], (float)components[3]};
    _fastMap[keyNum] = [NSData dataWithBytes:&value length:sizeof(value)];
...
- (NSColor *)colorForKey:(iTermColorMapKey)theKey {
    if (theKey == kColorMapInvalid) {
        ...
        return _map[iTermColorMapKeyToNumber(theKey)];
    }
}
...
- (vector_float4)fastColorForKey:(iTermColorMapKey)theKey {
    if (theKey == kColorMapInvalid) {
        ...
        int green = (n >> 8) & 0xff;
        int red = (n >> 16) & 0xff;
        return simd_make_float4(red / 255.0, green / 255.0, blue / 255.0, 1);
    } else {
        NSData *data = _fastMap[iTermColorMapKeyToNumber(theKey)];
        if (!data) {
            return simd_make_float4(1, 0, 0, 1);
        }
        vector_float4 value;
...
- (vector_float4)fastAverageComponents:(vector_float4)rgb1 with:(vector_float4)rgb2 alpha:(float)alpha {
    vector_float4 result = {rgb1.x * (1 - alpha) + rgb2.x * alpha, rgb1.y * (1 - alpha) + rgb2.y * alpha,
                            rgb1.z * (1 - alpha) + rgb2.z * alpha, rgb1.w};
    return result;
}
...
- (NSColor *)processedTextColorForTextColor:(NSColor *)textColor
    ...
    [_map[iTermColorMapKeyToNumber(kColorMapBackground)] getComponents:defaultBackgroundComponents];
    ...
    [iTermColorMap getComponents:dimmedRgb byAveragingComponents:mutedRgb withComponents:grayRgb alpha:_dimmingAmount];
    ...
    if (_lastTextColor && !memcmp(_lastTextComponents, dimmedRgb, sizeof(CGFloat) * 3)) {
        ...
        _lastTextColor = [NSColor colorWithColorSpace:textColor.colorSpace components:dimmedRgb count:4];
        return _lastTextColor;
    }
}
...
- (NSColor *)colorByMutingColor:(NSColor *)color {
    ...
    vector_float4 colorVector = simd_make_float4(components[0], components[1], components[2], components[3]);
    vector_float4 v = [self commonColorByMutingColor:colorVector];

    CGFloat mutedRgb[4] = {v.x, v.y, v.z, v.w};
    return [NSColor colorWithColorSpace:color.colorSpace components:mutedRgb count:4];
}
...
+ (NSColor *)dimmedTextColor:(NSColor *)color
    ...
    [iTermColorMap getComponents:dimmedRgb byAveragingComponents:components withComponents:grayRgb alpha:dimmingAmount];
    dimmedRgb[3] = components[3];

    return [NSColor colorWithColorSpace:color.colorSpace components:dimmedRgb count:4];
}
// There is an issue where where the passed-in color can be in a different color space than the
// default background color. It doesn't make sense to combine RGB values from different color
// spaces. The effects are generally subtle.
...
- (vector_float4)fastProcessedBackgroundColorForBackgroundColor:(vector_float4)backgroundColor {
    ...
    const vector_float4 mutedRgb = [self fastAverageComponents:backgroundColor
                                                          with:defaultBackgroundComponents
                                                         alpha:_mutingAmount];
    ...
    if (_dimOnlyText) {
        const BOOL isDefaultBackgroundColor = (fabs(backgroundColor.x - defaultBackgroundComponents.x) < 0.01 &&
        ...
        if (!isDefaultBackgroundColor) {
            grayRgb = (vector_float4){(float)_backgroundBrightness, (float)_backgroundBrightness,
                                      (float)_backgroundBrightness, 1};
            shouldDim = YES;
        }
    }
...
- (NSColor *)processedBackgroundColorForBackgroundColor:(NSColor *)backgroundColor {
    ...
    // If dimOnlyText is set then text and non-default background colors get dimmed toward black.
    if (_dimOnlyText) {
        const BOOL isDefaultBackgroundColor = (fabs(backgroundRgb[0] - defaultBackgroundComponents[0]) < 0.01 &&
    ...
    if (!memcmp(_lastBackgroundComponents, dimmedRgb, sizeof(CGFloat) * 4)) {
        ...
        _lastBackgroundColor = [NSColor colorWithColorSpace:backgroundColor.colorSpace components:dimmedRgb count:4];
        return _lastBackgroundColor;
    }
}
...
- (NSString *)baseProfileKeyForColorMapKey:(int)theKey {
    return self.colormapKeyToProfileKeyDictionary[iTermColorMapKeyToNumber(theKey)];
}
...
- (NSDictionary<NSNumber *, NSString *> *)colormapKeyToProfileKeyDictionary {
    ...
    dispatch_once(&onceToken, ^{
        // Use cached NSNumber keys for consistent dictionary lookups
        dict = @{
            iTermColorMapKeyToNumber(kColorMapForeground) : KEY_FOREGROUND_COLOR,
            iTermColorMapKeyToNumber(kColorMapBackground) : KEY_BACKGROUND_COLOR,
            iTermColorMapKeyToNumber(kColorMapBold) : KEY_BOLD_COLOR,
            iTermColorMapKeyToNumber(kColorMapLink) : KEY_LINK_COLOR,
            iTermColorMapKeyToNumber(kColorMapMatch) : KEY_MATCH_COLOR,
            iTermColorMapKeyToNumber(kColorMapSelection) : KEY_SELECTION_COLOR,
            iTermColorMapKeyToNumber(kColorMapSelectedText) : KEY_SELECTED_TEXT_COLOR,
            iTermColorMapKeyToNumber(kColorMapCursor) : KEY_CURSOR_COLOR,
            iTermColorMapKeyToNumber(kColorMapCursorText) : KEY_CURSOR_TEXT_COLOR,
            iTermColorMapKeyToNumber(kColorMapUnderline) : KEY_UNDERLINE_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiBlack) : KEY_ANSI_0_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiRed) : KEY_ANSI_1_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiGreen) : KEY_ANSI_2_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiYellow) : KEY_ANSI_3_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiBlue) : KEY_ANSI_4_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiMagenta) : KEY_ANSI_5_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiCyan) : KEY_ANSI_6_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiWhite) : KEY_ANSI_7_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiBlack + kColorMapAnsiBrightModifier) : KEY_ANSI_8_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiRed + kColorMapAnsiBrightModifier) : KEY_ANSI_9_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiGreen + kColorMapAnsiBrightModifier) : KEY_ANSI_10_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiYellow + kColorMapAnsiBrightModifier) : KEY_ANSI_11_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiBlue + kColorMapAnsiBrightModifier) : KEY_ANSI_12_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiMagenta + kColorMapAnsiBrightModifier) : KEY_ANSI_13_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiCyan + kColorMapAnsiBrightModifier) : KEY_ANSI_14_COLOR,
            iTermColorMapKeyToNumber(kColorMapAnsiWhite + kColorMapAnsiBrightModifier) : KEY_ANSI_15_COLOR,
        };
    });
    return dict;
}
...
- (iTermColorMapKey)keyForColor:(int)color
    ...
    switch (mode) {
        ...
        case ColorModeNormal:
            ...
            // preference.
            if (isBold && brightenBold && (color < 8) && !isBackground) { // Only colors 0-7 can be made "bright".
