...
// 4: Advisory lock on dashterm2-daemon-1.socket.lock. Remains open for the lifetime of the process.
typedef enum {
...
static void AcceptAndReject(int socket) {
    ...
    if (rc) {
        ...
        // BUG-2755: Free obj before goto to prevent memory leak on encode failure
        iTermClientServerProtocolMessageFree(&obj);
        goto done;
    }
    int error;
...
static int MakeAndSendPipe(int unixDomainSocketFd) {
    ...
    if (encodeRC) {
        ...
        // BUG-2746: Close both pipe ends on encode failure to prevent leak
        close(readPipe);
        close(writePipe);
        return -1;
    }
...
static void CheckIfBootstrapPortIsDead(void) {
    ...
    kern_return_t type_result = mach_port_type(mach_task_self(), port, &type);
    // BUG-3394: task_get_bootstrap_port returns a send right reference that must be deallocated.
    // This function is called periodically, so not deallocating causes a port leak over time.
...
static int iTermFileDescriptorMultiServerRun(char *path, int socketFd, int writeFD, int readFD) {
    ...
    SetRunningServer();
    // If DashTerm2 dies while we're blocked in sendmsg we get a deadly sigpipe.
