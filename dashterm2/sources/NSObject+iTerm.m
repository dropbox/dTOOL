...
+ (instancetype)forceCastFrom:(id)object {
    // BUG-f1354: Replace assert() with guard - nil object or cast failure should return nil, not crash
    if (!object) {
        ELog(@"forceCastFrom: object is nil");
        return nil;
    }
    id result = [self castFrom:object];
    if (!result) {
        ELog(@"forceCastFrom: Failed to cast %@ to %@", object, NSStringFromClass(self));
        return nil;
    }
    return result;
}
...
+ (instancetype)it_fromKeyValueCodedData:(NSData *)data {
    ...
    if (!unarchiver) {
        DLog(@"Failed to create unarchiver from data: %@", error);
        return nil;
    }
    NSArray *classes = @[
        [NSArray class],
