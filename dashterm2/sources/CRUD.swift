...
class CRUDTableViewController<Delegate: CRUDTableViewControllerDelegate>: NSObject, CompetentTableViewDelegate, NSTableViewDataSource {
    ...
    func undoable<T>(_ closure: () throws -> (T)) rethrows -> T {
        ...
        // BUG-f508: The assertion here was unreachable (dead code) since we already checked inUndoable above
        inUndoable = true
        defer {
            inUndoable = false
        }
        ...
        // BUG-3124: Only begin undo grouping if we have a saved state to restore
        // This ensures balanced begin/end grouping calls
        if savedState != nil {
        ...
        undoManager?.registerUndo(withTarget: self, handler: { [weak self] crud in
            ...
            DLog("\(self?.tableView.visibleRect ?? .zero) -> \(fullState.visibleRect)")
        ...
        defer {
            // BUG-3124: Only end undo grouping if we began one
            if savedState != nil {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        // BUG-f612: Add bounds check before accessing dataProvider
        guard row >= 0 && row < schema.dataProvider.count else {
            DLog("CRUDTableViewController: Invalid row index \(row) for count \(schema.dataProvider.count)")
            return nil
        }
