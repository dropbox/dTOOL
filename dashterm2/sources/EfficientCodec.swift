...
struct EfficientDecoder {
    ...
    mutating func getScalar<T: DefaultInitializable>() throws -> T {
        ...
        withUnsafeMutableBytes(of: &result) { destBuffer in
            data.withUnsafeBytes { rawBuffer in
                guard let srcBase = rawBuffer.baseAddress,
                      let destBase = destBuffer.baseAddress else {
                    return
                }
                let src = srcBase.advanced(by: offset)
                destBase.copyMemory(from: src, byteCount: size)
            }
        }
        offset += size
        return result
    ...
    mutating func getArray<T: DefaultInitializable>() throws -> [T] {
        ...
        // BUG-2651: Check for overflow in length calculation
        guard count >= 0 else {
            throw EfficientDecoderError()
        }
        let stride = MemoryLayout<T>.stride
        let (lengthInBytes, overflow) = Int(count).multipliedReportingOverflow(by: stride)
        if overflow {
            throw EfficientDecoderError()
        }
        if offset < 0 {
        ...
        // Also check for overflow in offset + lengthInBytes
        let (endOffset, addOverflow) = offset.addingReportingOverflow(lengthInBytes)
        if addOverflow || endOffset > data.count {
            throw EfficientDecoderError()
        }
        defer {
    ...
    mutating func getData() throws -> Data {
        ...
        // BUG-f1411: Check for negative length before using it
        // A negative Int32 would cause invalid range calculations (e.g., offset - large_value)
        guard length >= 0 else {
            throw EfficientDecoderError()
        }
        // BUG-f1411: Check for overflow in offset + length calculation
