...
extension iTermLineStringReading {
    func _screenCharsData(withEOL: Bool) -> Data {
        ...
        if withEOL {
            ...
            // BUG-1737: Use guard with as? instead of as! force cast
            guard let m = lms.screenCharArray.mutableCopy() as? MutableScreenCharArray else {
                return lms.screenCharArray.data
            }
            ...
            // BUG-1738: Use guard with as? instead of as! force cast
            guard let m = lms.screenCharArray.mutableCopy() as? MutableScreenCharArray else {
                return lms.screenCharArray.data
            }
...
class iTermLineString: NSObject, iTermLineStringReading {
    ...
    var externalImmutableMetadata: iTermImmutableMetadata {
        ...
        var newMetadata = iTermImmutableMetadataDefault()
        iTermImmutableMetadataInit(&newMetadata,
                                   metadata.timestamp,
                                   metadata.rtlFound.boolValue,
                                   content.externalAttributesIndex() as? iTermExternalAttributeIndex)
        _externalMetadata = newMetadata
        return newMetadata
    }
...
class iTermMutableLineString: NSObject, iTermLineStringReading {
    ...
    var externalMetadata: iTermMetadata {
        ...
        var newMetadata = iTermMetadataDefault()
        iTermMetadataInit(&newMetadata,
                          timestamp,
                          rtlFound,
                          content.externalAttributesIndex() as? iTermExternalAttributeIndex)
        _externalMetadata = newMetadata
    ...
    init(content: iTermMutableStringProtocol,
         ...
         continuation: screen_char_t,
         metadata: iTermLineStringMetadata) {
        // BUG-f564, BUG-f684: Log error and use fallback instead of crashing
        guard let swiftContent = content as? iTermMutableStringProtocolSwift else {
            DLog("BUG-f564: content must conform to iTermMutableStringProtocolSwift - got \(type(of: content)) - using empty mutable rope as fallback")
            // Create an empty mutable rope as fallback
            self._content = iTermMutableRope()
            self.eol = eol
    ...
    func eraseCharacterAt(_ i: Int32) {
        // BUG-1252g: Bounds check to prevent crash on invalid index
        guard i >= 0 && i < Int32(content.cellCount) else {
            return
        }
        let lms = ensureLegacy()
        let sca = lms.mutableScreenCharArray
    ...
    func substringWithRange(_ range: NSRange) -> iTermLineStringReading {
        // BUG-1663: Handle optional from failable init
        guard let subString = iTermSubString(base: content, range: range) else {
            return self
        }
        return iTermLineString(content: subString,
                               eol: eol,
                               continuation: continuation,
                               metadata: metadata,
                               bidi: bidi,
