    ...
    static func ==(lhs: SubData, rhs: SubData) -> Bool {
        ...
        return lhs._data.withUnsafeBytes { (urbp_l: UnsafeRawBufferPointer) in
            return rhs._data.withUnsafeBytes { (urbp_r: UnsafeRawBufferPointer) in
                guard let baseL = urbp_l.baseAddress,
                      let baseR = urbp_r.baseAddress else {
                    return false
                }
                return memcmp(baseL.advanced(by: lhs.range.lowerBound),
                              baseR.advanced(by: rhs.range.lowerBound),
                              lhs.range.count) == 0
            }
        }
    }
