...
class iTermASCIIString: iTermBaseString, iTermString {
    ...
    func stringBySettingRTL(in nsrange: NSRange,
        ...
        if let rtlIndexes {
            guard let membershipRange = Range(nsrange) else {
                return self
            }
            for (range, rtl) in rtlIndexes.membership(in: membershipRange) {
    ...
    func efficientlyEncodedData(range: NSRange, type: UnsafeMutablePointer<Int32>) -> Data {
        ...
        guard let swiftRange = Range(range) else {
            tlvEncoder.put(tag: .data, value: Data())
            tlvEncoder.put(tag: .style, value: style)
            tlvEncoder.put(tag: .ea, value: ea)
            return tlvEncoder.data
        }
        tlvEncoder.put(tag: .data, value: data.data.subdata(in: swiftRange))
        tlvEncoder.put(tag: .style, value: style)
        tlvEncoder.put(tag: .ea, value: ea)
        return tlvEncoder.data
    }
...
extension iTermASCIIString: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set([.data, .style, .ea]))
        guard var dataDecoder = dict[.data],
              var styleDecoder = dict[.style],
              var eaDecoder = dict[.ea] else {
            throw EfficientDecoderError("Missing required keys in iTermASCIIString decoder")
        }
        return Self(data: try Data.create(efficientDecoder: &dataDecoder),
