...
class iTermLegacyStyleString: iTermBaseString, iTermString {
    ...
    func usedLength(range: NSRange) -> Int32 {
        line.withUnsafeBufferPointer { ubp in
            guard let baseAddr = ubp.baseAddress else {
                return 0
            }
            return iTermUsedLength(chars: baseAddr.advanced(by: range.location),
    ...
    func character(at i: Int) -> screen_char_t {
        // BUG-1252e: Bounds check to prevent crash on invalid index
    ...
    func buildString(range: NSRange, builder: DeltaStringBuilder) {
        line.withUnsafeBufferPointer { ubp in
            ...
            builder.append(chars: baseAddr.advanced(by: range.location),
    ...
    func efficientlyEncodedData(range: NSRange, type: UnsafeMutablePointer<Int32>) -> Data {
        ...
        guard let swiftRange = Range(range) else {
            tlvEncoder.put(tag: .line, value: [screen_char_t]())
            tlvEncoder.put(tag: .eaIndex, value: nil as iTermExternalAttributeIndex?)
            return tlvEncoder.data
        }
        tlvEncoder.put(tag: .line, value: Array(line[swiftRange]))
...
extension iTermLegacyStyleString: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set([.line, .eaIndex]))
        guard var lineDecoder = dict[.line],
              var eaIndexDecoder = dict[.eaIndex] else {
            throw EfficientDecoderError("Missing required keys in iTermLegacyStyleString decoder")
        }
        return Self(
            line: try Array<screen_char_t>.create(efficientDecoder: &lineDecoder),
            eaIndex: try Optional<iTermExternalAttributeIndex>.create(efficientDecoder: &eaIndexDecoder))
    }
...
extension iTermExternalAttributeIndex: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        // BUG-1746: Use guard with as? instead of as! force cast
