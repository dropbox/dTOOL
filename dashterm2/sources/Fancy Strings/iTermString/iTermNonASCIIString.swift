...
class iTermNonASCIIString: iTermBaseString, iTermString {
    ...
    override var description: String {
        return "<iTermNonASCIIString: cells=\(cellCount) value=\(deltaString(range: fullRange).string.trimmingTrailingNulls.escapingControlCharactersAndBackslash())>"
    }
    ...
    func isEqual(lhsRange lhsNSRange: NSRange, toString rhs: iTermString, startingAtIndex startIndex: Int) -> Bool {
        ...
        guard let lhsRange = Range(lhsNSRange) else {
            return false
        }
    ...
    func stringBySettingRTL(in nsrange: NSRange,
        ...
        if let rtlIndexes {
            guard let rangeSwift = Range(nsrange) else {
                return self  // Invalid range - return unchanged
            }
            for (range, rtl) in rtlIndexes.membership(in: rangeSwift) {
    ...
    func efficientlyEncodedData(range: NSRange, type: UnsafeMutablePointer<Int32>) -> Data {
        ...
        guard let rangeSwift = Range(range) else {
            return tlvEncoder.data  // Return empty data for invalid range
        }
        tlvEncoder.put(tag: .codes, value: codes[rangeSwift].array)
        tlvEncoder.put(tag: .complex, value: complex[rangeSwift].shifted(by: -range.lowerBound))
...
extension iTermNonASCIIString: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set([.codes, .complex, .style, .ea]))
        guard var codesDecoder = dict[.codes] else {
            throw EfficientDecoderError("Missing .codes in iTermNonASCIIString")
        }
        guard var complexDecoder = dict[.complex] else {
            throw EfficientDecoderError("Missing .complex in iTermNonASCIIString")
        }
        guard var styleDecoder = dict[.style] else {
            throw EfficientDecoderError("Missing .style in iTermNonASCIIString")
        }
        guard var eaDecoder = dict[.ea] else {
            throw EfficientDecoderError("Missing .ea in iTermNonASCIIString")
        }
        return Self(
            codes: try [UInt16].create(efficientDecoder: &codesDecoder),
            complex: try IndexSet.create(efficientDecoder: &complexDecoder),
            style: try screen_char_t.create(efficientDecoder: &styleDecoder),
            ea: try iTermExternalAttribute?.create(efficientDecoder: &eaDecoder))
    }
