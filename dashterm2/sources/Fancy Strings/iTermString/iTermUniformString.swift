...
class iTermUniformString: iTermBaseString, iTermString {
        ...
        if let rtlIndexes {
            ...
            if let firstRange = rtlIndexes.rangeView.first {
                if firstRange.contains(0..<cellCount) {
                    ...
                    guard let swiftRange = Range(nsrange) else {
                        return self
                    }
                    for (range, isMember) in rtlIndexes.membership(in: swiftRange) {
                        for i in range {
    ...
    func doubleWidthIndexes(range: NSRange, rebaseTo newBaseIndex: Int) -> IndexSet {
        if ScreenCharIsDWC_RIGHT(char) {
            ...
            return IndexSet(integersIn: swiftRange.shifted(by: newBaseIndex - range.location))
        }
        return IndexSet()
    }
...
extension iTermUniformString: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set([.char, .length]))
        guard var charDecoder = dict[.char], var lengthDecoder = dict[.length] else {
            throw EfficientDecoderError("Missing .char or .length in iTermUniformString")
        }
        return Self(char: try screen_char_t.create(efficientDecoder: &charDecoder),
                    length: try Int.create(efficientDecoder: &lengthDecoder))
    }
