...
class iTermRope: iTermBaseString {
    ...
    final class Guts: Cloning {
        ...
        func insert(segment: Segment, at i: Int) {
            // BUG-f629: Use guard instead of it_assert - assertions stripped in release builds
            if i == 0 && deletedHeadCellCount != 0 {
                DLog("Warning: inserting segment at index 0 when deletedHeadCellCount=\(deletedHeadCellCount)")
                // Reset deletedHeadCellCount to allow insertion at head
                deletedHeadCellCount = 0
            }
...
extension iTermRope: EfficientDecodable, EfficientEncodable {
    ...
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set<CodingKeys>([.segments,
                                                                       .deletedHeadCount,
                                                                       .mayHaveExternalAttributes]))
        guard var segmentsDecoder = dict[.segments] else {
            throw EfficientDecoderError("Missing .segments in iTermRope")
        }
        guard var deletedHeadCountDecoder = dict[.deletedHeadCount] else {
            throw EfficientDecoderError("Missing .deletedHeadCount in iTermRope")
        }
        guard var mayHaveExternalAttributesDecoder = dict[.mayHaveExternalAttributes] else {
            throw EfficientDecoderError("Missing .mayHaveExternalAttributes in iTermRope")
        }
        let segments = try Array<Segment>.create(efficientDecoder: &segmentsDecoder)
        let deletedHeadCount: Int = try Int.create(efficientDecoder: &deletedHeadCountDecoder)
        let mayHaveExternalAttributes: Bool = try Bool.create(efficientDecoder: &mayHaveExternalAttributesDecoder)
...
extension iTermRope.Segment: EfficientDecodable, EfficientEncodable {
    ...
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try decoder.decodeAll(required: Set<CodingKeys>([.stringType,
                                                                    .string,
                                                                    .cumulativeCellCount]))
        guard var stringTypeDecoder = dict[.stringType] else {
            throw EfficientDecoderError("Missing .stringType in Segment")
        }
        guard var stringDecoder = dict[.string] else {
            throw EfficientDecoderError("Missing .string in Segment")
        }
        guard var cumulativeCellCountDecoder = dict[.cumulativeCellCount] else {
            throw EfficientDecoderError("Missing .cumulativeCellCount in Segment")
        }
        let stringType = try Int32.create(efficientDecoder: &stringTypeDecoder)
        let stringData = try Data.create(efficientDecoder: &stringDecoder)
        let cumulativeCellCount = try Int.create(efficientDecoder: &cumulativeCellCountDecoder)
...
extension iTermRope: iTermString {
    ...
    func isEqual(lhsRange: NSRange, toString rhs: iTermString, startingAtIndex startIndex: Int) -> Bool {
        guard let fullRangeSwift = Range(fullRange),
              let lhsRangeSwift = Range(lhsRange) else {
            return false
        }
        if !fullRangeSwift.contains(lhsRangeSwift) {
            return false
        }
        ...
        for (_, substr, localRange) in segmentIterator(inRange: lhsRangeSwift) {
    ...
    func usedLength(range: NSRange) -> Int32 {
        ...
        guard let rangeSwift = Range(range) else {
            return 0
        }
        for (_, seg, localRange) in segmentIterator(inRange: rangeSwift).reversed() {
            if found {
    ...
    func stringBySettingRTL(in range: NSRange,
        ...
        enumerateSegments(inRange: rangeSwift) { i, substring, localRange in
    ...
    func doubleWidthIndexes(range nsrange: NSRange,
        ...
        var result = IndexSet()
        var offset = 0
        guard let rangeSwift = Range(nsrange) else {
            return result
        }
        enumerateSegments(inRange: rangeSwift) { i, string, localRange in
    ...
    func isEmpty(range: NSRange) -> Bool {
        guard let rangeSwift = Range(range) else {
            return true  // Invalid range is considered empty
        }
        return segmentIterator(inRange: rangeSwift).allSatisfy { (_, seg, localRange) in
    ...
    func hydrate(into msca: MutableScreenCharArray,
                 ...
                 sourceRange: NSRange) {
        // BUG-f628: Use guard instead of it_assert - assertions stripped in release builds
        guard fullRange.contains(sourceRange) else {
            DLog("Source range \(sourceRange) out of bounds in rope of length \(cellCount)")
            return
        }
        guard let rangeSwift = Range(sourceRange) else {
            // BUG-f566: Return early for invalid sourceRange instead of crashing
            DLog("Invalid sourceRange \(sourceRange) in hydrate - returning early")
            return
        }
        var o = 0
        enumerateSegments(inRange: rangeSwift) { i, seg, localRange in
    ...
    func buildString(range: NSRange, builder: DeltaStringBuilder) {
        guard let rangeSwift = Range(range) else {
            return  // Invalid range - nothing to build
        }
        enumerateSegments(inRange: rangeSwift) { i, seg, segmentRange in
            seg.buildString(range: NSRange(segmentRange), builder: builder)
        }
    }

    /// BUG-f563: Use DeltaString.empty as fallback when builder fails
    ...
    func mayContainDoubleWidthCharacter(in nsrange: NSRange) -> Bool {
        guard let rangeSwift = Range(nsrange) else {
            return false  // Invalid range cannot contain double-width characters
        }
        for (_, string, localRange) in segmentIterator(inRange: rangeSwift) {
    ...
    func hasExternalAttributes(range: NSRange) -> Bool {
        guard let rangeSwift = Range(range) else {
            return false  // Invalid range cannot have external attributes
        }
        for (_, string, localRange) in segmentIterator(inRange: rangeSwift) {
...
extension iTermRope {
    private func segmentsClipped(toRange clippingRange: NSRange) -> [Segment] {
        var result = [Segment]()
        guard let rangeSwift = Range(clippingRange) else {
            return result
        }
        var ccc = 0
        for (_, string, localRange) in segmentIterator(inRange: rangeSwift) {
