...
extension iTermMutableRope: iTermMutableStringProtocol {
    ...
    func objcDelete(range: NSRange) {
        guard let swiftRange = Range(range) else { return }
        delete(range: swiftRange)
    }

    @objc
    func objcReplace(range: NSRange, with replacement: iTermString) {
        guard let swiftRange = Range(range) else { return }
        replace(range: swiftRange, with: replacement)
    }
    ...
    /// BUG-f534: Use guard clauses instead of precondition crash for invalid count
    @objc
    func deleteFromStart(_ count: Int) {
        guard count > 0 else {
            if count < 0 {
                DLog("iTermMutableRope.deleteFromStart: negative count \(count), ignoring")
            }
            return
        }
        guts.stringCache.clear()
        if count >= cellCount {
            // Delete everything if count exceeds cellCount
            if count > cellCount {
                DLog("iTermMutableRope.deleteFromStart: count \(count) exceeds cellCount \(cellCount), deleting all")
            }
            guts.set(segments: [])
            return
        }
    ...
    /// BUG-f535: Use guard clauses instead of precondition crash for invalid count
    @objc
    func deleteFromEnd(_ count: Int) {
        guard count > 0 else {
            if count < 0 {
                DLog("iTermMutableRope.deleteFromEnd: negative count \(count), ignoring")
            }
            return
        }
        if count >= cellCount {
            // Delete everything if count exceeds cellCount
            if count > cellCount {
                DLog("iTermMutableRope.deleteFromEnd: count \(count) exceeds cellCount \(cellCount), deleting all")
            }
            guts.set(segments: [])
            return
        }
...
extension iTermMutableRope: iTermMutableStringProtocolSwift {
    ...
    func delete(range: Range<Int>) {
        ...
        // BUG-f536: Use guard clause instead of precondition crash for invalid range
        guard endIdx >= startIdx else {
            DLog("iTermMutableRope.delete: endIdx \(endIdx) < startIdx \(startIdx), aborting delete")
            return
        }
...
extension iTermMutableRope {
    ...
    /// BUG-f537: Use guard clause instead of precondition crash for invalid offset
    private func partition(segmentIndex: Int, atOffset offset: Int) {
        guard segmentIndex >= 0 && segmentIndex < guts.segments.count else {
            DLog("iTermMutableRope.partition: segmentIndex \(segmentIndex) out of bounds [0, \(guts.segments.count))")
            return
        }
        let original = guts.segments[segmentIndex]
        guard offset > 0 && offset < original.string.cellCount else {
            DLog("iTermMutableRope.partition: offset \(offset) out of bounds (0, \(original.string.cellCount))")
            return
        }
