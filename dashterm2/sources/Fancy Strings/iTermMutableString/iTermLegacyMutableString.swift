...
extension iTermLegacyMutableString: iTermLegacyString {
    ...
    func character(at i: Int) -> screen_char_t {
        // BUG-1252f: Bounds check to prevent crash on invalid index
        guard i >= 0 && i < Int(sca.length) else {
...
extension iTermLegacyMutableString: EfficientDecodable, EfficientEncodable {
    static func create(efficientDecoder decoder: inout EfficientDecoder) throws -> Self {
        ...
        let dict = try tlvDecoder.decodeAll(required: Set([
            .line,
            .eaIndex,
            .metadata,
            .continuation]))

        guard var metadataDecoder = dict[.metadata],
              var lineDecoder = dict[.line],
              var continuationDecoder = dict[.continuation] else {
            throw EfficientDecoderError("Missing required keys in iTermLegacyMutableString decoder")
        }

        let encodedMetadata = try Data.create(efficientDecoder: &metadataDecoder)

        let msca = MutableScreenCharArray(
            data: try Data.create(efficientDecoder: &lineDecoder),
            includingContinuation: true,
            metadata: iTermMetadataMakeImmutable(iTermMetadataDecodedFromData(encodedMetadata)),
            continuation: try screen_char_t.create(efficientDecoder: &continuationDecoder))
        return Self(msca)
    }
