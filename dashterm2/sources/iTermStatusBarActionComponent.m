...
- (nullable NSArray<NSString *> *)stringVariants {
    NSMutableArray<NSString *> *result = [NSMutableArray arrayWithCapacity:8]; // Status bar action string variants
    [_value
        enumerateStringsSeparatedByRegex:@"\\h+"
...
- (void)updateTitleInButton {
    ...
    _swiftyString = [[iTermSwiftyString alloc]
        initWithString:expression
                 scope:self.scope
    sideEffectsAllowed:NO
              observer:^(NSString *_Nonnull newValue, NSError *error) {
                  if (error != nil) {
                      [[iTermScriptHistoryEntry globalEntry]
                           addOutput:[NSString
                                         stringWithFormat:@"Error while evaluating %@ in status bar action button: %@",
                                                          expression, error]
                          completion:^{
                          }];
...
- (NSString *)statusBarComponentDetailedDescription {
    return @"When clicked, opens a menu of actions. Actions are like custom key bindings, but without a keystroke "
           @"attached.";
}
...
- (void)openMenuWithView:(NSView *)view {
    ...
    NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:@"Edit Actions…"
                                                  action:@selector(editActions:)
                                           keyEquivalent:@""];
    item.target = self;
    [menu addItem:item];
