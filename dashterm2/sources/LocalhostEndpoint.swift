...
class LocalhostEndpoint: SSHEndpoint {
    ...
    func downloadChunked(remoteFile: RemoteFile,
        ...
        // BUG-f638: Use guard instead of it_assert - assertions stripped in release builds
        // If the file is not a regular file (e.g., symlink, socket, etc.), throw an error
        guard remoteFile.kind.isRegularFile else {
            DLog("BUG-f638: downloadChunked called with non-file kind: \(remoteFile.kind)")
            throw SSHEndpointException.notAFile
        }
        progress?.fraction = 0
    ...
    // BUG-1507: Only open FileHandle when chunked download is requested
