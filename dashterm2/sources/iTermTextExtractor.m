...
+ (NSCharacterSet *)wordSeparatorCharacterSet {
    ...
    [complement addCharactersInString:[iTermPreferences
                                          stringForKey:kPreferenceKeyCharactersConsideredPartOfAWordForSelection]];
...
- (NSURL *)urlOfHypertextLinkAt:(VT100GridCoord)coord urlId:(out NSString **)urlId target:(out NSString **)target {
...
- (VT100GridWindowedRange)rangeOfCoordinatesAround:(VT100GridCoord)origin
                                   maximumDistance:(int)maximumDistance
                                       passingTest:(BOOL (^)(screen_char_t *c, iTermExternalAttribute *ea,
...
- (int)startOfIndentationOnLine:(int)line {
    ...
    [self enumerateCharsInRange:VT100GridWindowedRangeMake(VT100GridCoordRangeMake(0, line, [_dataSource width], line),
        ...
        charBlock:^BOOL(const screen_char_t *currentLine, screen_char_t theChar, iTermExternalAttribute *ea,
            ...
            if (!theChar.complexChar && !theChar.image &&
...
- (int)rowCountForRawLineEncompassingWithAbsY:(long long)absY {
    ...
    const VT100GridCoordRange lineRange = [self rangeForWrappedLineEncompassing:VT100GridCoordMake(0, absY - offset)
...
- (VT100GridWindowedRange)rangeForWordAt:(VT100GridCoord)location maximumLength:(NSInteger)maximumLength {
...
- (VT100GridWindowedRange)rangeForBigWordAt:(VT100GridCoord)unsafeLocation maximumLength:(NSInteger)maximumLength {
...
- (VT100GridWindowedRange)rangeForWordAt:(VT100GridCoord)visualLocation
    ...
    if (bidi) {
        // BUG-659: Known limitation - when a word wraps in RTL text, the selection should
        // include characters from the left side of the start line and the right side of the
        // end line. The current implementation returns a simplified visual range that may
        // not correctly highlight all characters in wrapped RTL words. A proper fix would
        // require extending iTermSelection to support non-contiguous visual selections.
...
- (void)performBlockWithLineCache:(void (^NS_NOESCAPE)(void))block {
    // BUG-f968: Use guard instead of assert - re-entrancy should return early with warning
    if (_shouldCacheLines) {
        DLog(@"BUG-f968: performBlockWithLineCache: called while already caching - executing without nested caching");
        block();
        return;
    }
...
// Instance method wrapper that calls the class method implementation.
// Required by iTermWordExtractorDataSource protocol.
- (NSInteger)indexInSortedArray:(NSArray<NSNumber *> *)indexes
     withValueLessThanOrEqualTo:(NSInteger)maximumValue
          searchingBackwardFrom:(NSInteger)start {
    return [[self class] indexInSortedArray:indexes
                 withValueLessThanOrEqualTo:maximumValue
                      searchingBackwardFrom:start];
}

// Returns indexes.count if no value can be found greater or equal to `minimumValue`.
// Optimization: Use binary search for O(log n) instead of O(n) linear search.
// This method is called during word/semantic selection (double-click to select).
- (NSInteger)indexInSortedArray:(NSArray<NSNumber *> *)indexes
    ...
    const NSUInteger searchRangeLength = indexes.count - startIndex;
    NSUInteger index = [indexes indexOfObject:@(minimumValue)
                                inSortedRange:NSMakeRange(startIndex, searchRangeLength)
                                      options:NSBinarySearchingInsertionIndex
                              usingComparator:^NSComparisonResult(id _Nonnull obj1, id _Nonnull obj2) {
                                  return [obj1 compare:obj2];
                              }];
    // indexOfObject with NSBinarySearchingInsertionIndex returns the index where minimumValue
    // would be inserted to maintain sorted order. This is the first index >= minimumValue.
    return index;
}
...
// Note: User-defined smart selection regexes could theoretically cause ReDoS, but the input
// is bounded by smartSelectionRadius (default 2 lines) which limits practical exposure.
// The operation is also user-initiated (triple-click/smart selection) not automatic.
- (SmartMatch *)smartSelectionAt:(VT100GridCoord)location
                ...
                if (i + temp.location <= targetOffset && i + temp.location + temp.length > targetOffset) {
                    ...
                    if (!oldMatch || score > oldMatch.score) {
                        ...
                        VT100GridCoord endCoord =
                            [coords[MIN(numCoords - 1, i + temp.location + temp.length - 1)] gridCoordValue];
                        ...
                        if (debug) {
                            NSLog(@"Regex matched. Add result %@ at %d,%lld -> %d,%lld with score %lf", result,
                                  match.startX, match.absStartY, match.endX, match.absEndY, match.score);
                        }
                    }
    ...
    if ([matches count]) {
        ...
        VT100GridCoordRange theRange =
            VT100GridCoordRangeMake(bestMatch.startX, bestMatch.absStartY - [_dataSource totalScrollbackOverflow],
                                    bestMatch.endX, bestMatch.absEndY - [_dataSource totalScrollbackOverflow]);
        *range = [self windowedRangeWithRange:theRange];
        ...
        if (actionRequired) {
            ...
            *range = VT100GridWindowedRangeMake(VT100GridCoordRangeMake(-1, -1, -1, -1), _logicalWindow.location,
...
- (VT100GridWindowedRange)rangeOfParentheticalSubstringAtLocation:(VT100GridCoord)location {
    ...
    if (left == 0) {
        ...
        return [self windowedRangeWithRange:VT100GridCoordRangeMake(location.x, location.y, end.x + 1, end.y)];
    } else {
        return [self windowedRangeWithRange:VT100GridCoordRangeMake(end.x, end.y, location.x + 1, location.y)];
    }
}
...
- (VT100GridCoord)predecessorOfCoord:(VT100GridCoord)coord {
    DLog(@"Want predecessor of %@.  logicalWindow.location=%@", VT100GridCoordDescription(coord),
...
- (int)numberOfCoordsInIndexSet:(NSIndexSet *)coords between:(VT100GridCoord)coord1 andCoord:(VT100GridCoord)coord2 {
...
- (VT100GridCoord)coord:(VT100GridCoord)coord
    ...
    while (n) {
        ...
        if (coord.x >= left && coord.x < right) {
            int extra = [self numberOfCoordsInIndexSet:coordsToSkip between:coord andCoord:prevCoord];
            if (forward) {
                coord.x += extra;
            } else {
                coord.x -= extra;
        ...
        if (coord.x < left) {
            while (coord.x < left) {
                coord.y--;
                coord.x += span;
                n -= [self numberOfCoordsInIndexSet:coordsToSkip between:coord andCoord:prevCoord];
                prevCoord = coord;
            }
        } else {
...
- (NSString *)textAround:(VT100GridCoord)coord
    ...
    VT100GridCoordRange theRange = VT100GridCoordRangeMake(0, coord.y - radius, coord.x, coord.y);
    NSMutableString *joinedLines = [NSMutableString stringWithCapacity:radius * _logicalWindow.length];
    VT100GridWindowedRange windowedRange =
        VT100GridWindowedRangeMake(theRange, _logicalWindow.location, _logicalWindow.length);
    [self enumerateInReverseCharsInRange:windowedRange
        charBlock:^BOOL(screen_char_t theChar, VT100GridCoord logicalCoord, VT100GridCoord charCoord) {
            if (!theChar.code) {
                return YES;
            }
            if (ignoreContinuations && windowedRange.columnWindow.length > 0 &&
                charCoord.x == windowedRange.columnWindow.location + windowedRange.columnWindow.length - 1 &&
                theChar.code == '\\' && !theChar.complexChar) {
                // Is a backslash at the right edge of a window.
                // no-op
            } else if (theChar.image) {
                // Treat images as nulls.
                return YES;
            } else if (theChar.complexChar || theChar.code < ITERM2_PRIVATE_BEGIN ||
    ...
    theRange = VT100GridCoordRangeMake(coord.x, coord.y, [_dataSource width], coord.y + radius);
    windowedRange = VT100GridWindowedRangeMake(theRange, _logicalWindow.location, _logicalWindow.length);

    [self enumerateCharsInRange:windowedRange
        supportBidi:NO
        charBlock:^BOOL(const screen_char_t *currentLine, screen_char_t theChar, iTermExternalAttribute *ea,
                        VT100GridCoord logicalCoord, VT100GridCoord charCoord) {
            if (!theChar.code) {
                return YES;
            }
            if (ignoreContinuations && windowedRange.columnWindow.length > 0 &&
...
- (VT100GridWindowedRange)rangeForWrappedLineEncompassing:(VT100GridCoord)coord
    ...
    int end = [self lineNumberWithEndOfWholeLineIncludingLine:coord.y
                                         respectContinuations:respectContinuations
                                                     maxChars:maxChars];
    return [self windowedRangeWithRange:VT100GridCoordRangeMake(_logicalWindow.location, start, [self xLimit], end)];
}
...
- (NSAttributedString *)attributedStringForSnippetForRange:(VT100GridAbsCoordRange)range
    ...
    NSString *match =
        [self contentInRange:VT100GridWindowedRangeMake(relativeRange, _logicalWindow.location, _logicalWindow.length)
            ...
            trimTrailingWhitespace:NO
                 ...
                 continuationChars:nil
                            coords:nil];
    NSAttributedString *matchString = [[NSAttributedString alloc] initWithString:match attributes:matchAttributes];
    ...
    if (suffix.length > maximumSuffixLength) {
        suffix = [[[suffix stringByDroppingLastCharacters:1] stringByTrimmingOrphanedSurrogates]
...
- (id)contentInRange:(VT100GridWindowedRange)windowedRange
    ...
    __kindof iTermLocatedString *locatedString = [self locatedStringInRange:windowedRange
...
- (id)locatedStringInRange:(VT100GridWindowedRange)windowedRange
    ...
    void (^appendString)(NSString *, screen_char_t, iTermExternalAttribute *, VT100GridCoord) =
        ^void(NSString *string, screen_char_t theChar, iTermExternalAttribute *ea, VT100GridCoord coord) {
            if (attributeProvider) {
                [locatedAttributedString appendString:string withAttributes:attributeProvider(theChar, ea) at:coord];
            } else {
    ...
    [self enumerateCharsInRange:windowedRange
        supportBidi:YES
        charBlock:^BOOL(const screen_char_t *currentLine, screen_char_t theChar, iTermExternalAttribute *ea,
                        VT100GridCoord logicalCoord, VT100GridCoord visualCoord) {
            if (needsTimestamps) {
            ...
            if (theChar.image) {
                ...
                if (attributeProvider && theChar.foregroundColor == 0 && theChar.backgroundColor == 0 &&
                    ...
                    if (image) {
                        ...
                        [locatedAttributedString appendAttributedString:attributedStringWithAttachment at:logicalCoord];
                    }
                }
            ...
            } else if (theChar.complexChar || (theChar.code != DWC_RIGHT && theChar.code != DWC_SKIP)) {
                // Normal character. Add it unless it's a backslash at the right edge
                // of a window.
                if (continuationChars && windowedRange.columnWindow.length > 0 &&
        ...
        eolBlock:^BOOL(unichar code, int numPrecedingNulls, int line) {
            if (needsTimestamps) {
                ...
                appendString([self formattedTimestampForLine:coord.y], [self defaultChar], nil, coord);
            }
            needsTimestamps = self.addTimestamps;
            self.progress.fraction = (double)(line - windowedRange.coordRange.start.y) /
                                     (double)(windowedRange.coordRange.end.y - windowedRange.coordRange.start.y + 1);
            ...
            if (pad) {
                for (int i = 0; i < numPrecedingNulls; i++) {
                    VT100GridCoord coord = VT100GridCoordMake(right - numPrecedingNulls + i, line);
                ...
                switch (nullPolicy) {
                    ...
                    case kiTermTextExtractorNullPolicyFromStartToFirst:
                        return YES;
                    case kiTermTextExtractorNullPolicyTreatAsSpace:
                        appendString(@" ", [self defaultChar], nil, VT100GridCoordMake(right - 1, line));
                        break;
                    case kiTermTextExtractorNullPolicyMidlineAsSpaceIgnoreTerminal:
                        break;
                }
            }
            if (code == EOL_HARD && shouldAppendNewline &&
                (includeLastNewline || line < windowedRange.coordRange.end.y)) {
                if (trimSelectionTrailingSpaces) {
                    [locatedString trimTrailingWhitespace];
                }
                appendString(@"\n", [self defaultChar], nil, VT100GridCoordMake(right, line));
            }
            if (truncateTail) {
                return locatedString.length >= maxBytes;
            } else if (locatedString.length > maxBytes + kMaximumOversizeAmountWhenTruncatingHead) {
...
- (VT100GridAbsCoordRange)rangeByTrimmingWhitespaceFromRange:(VT100GridAbsCoordRange)range {
    return [self rangeByTrimmingWhitespaceFromRange:range
                                            leading:YES
...
- (VT100GridAbsCoordRange)rangeByTrimmingWhitespaceFromRange:(VT100GridAbsCoordRange)range
    ...
    VT100GridCoordRange localRange = VT100GridCoordRangeMake(range.start.x, range.start.y - totalScrollbackOverflow,
                                                             range.end.x, range.end.y - totalScrollbackOverflow);
    ...
    if (trailing != iTermTextExtractorTrimTrailingWhitespaceNone) {
        ...
        [self enumerateInReverseCharsInRange:windowedRange
            charBlock:^BOOL(screen_char_t theChar, VT100GridCoord logicalCoord, VT100GridCoord coord) {
                NSString *string = ScreenCharToStr(&theChar);
                BOOL result = NO;
...
- (int)lineNumberWithEndOfWholeLineIncludingLine:(int)y
    ...
    NSInteger count = 0;
    while (count < maxChars + width && i < maxY &&
           [self lineHasSoftEol:i - 1 respectContinuations:respectContinuations]) {
        i++;
        count += width;
    }
    return i - 1;
...
- (BOOL)lineHasSoftEol:(int)y respectContinuations:(BOOL)respectContinuations {
    ...
    if (respectContinuations) {
        return (sca.eol != EOL_HARD || (theLine[width - 1].code == '\\' && !theLine[width - 1].complexChar));
    } else {
...
- (iTermLocatedString *)wrappedLocatedStringAt:(VT100GridCoord)coord
    ...
    if (respectHardNewlines) {
        ...
        VT100GridCoordRange coordRange =
            VT100GridCoordRangeMake(_logicalWindow.location, MAX(0, coord.y - 10), [self xLimit],
    ...
    if (forward) {
        ...
        if (VT100GridCoordOrder(range.coordRange.start, range.coordRange.end) != NSOrderedAscending) {
    ...
    iTermLocatedString *locatedString = [self locatedStringInRange:range
                                                 attributeProvider:nil
                                                        nullPolicy:nullPolicy
                                                               pad:NO
                                                includeLastNewline:NO
...
- (void)enumerateCharsInRange:(VT100GridWindowedRange)range
                  supportBidi:(BOOL)supportBidi
                    charBlock:(BOOL (^NS_NOESCAPE)(const screen_char_t *currentLine, screen_char_t theChar,
                                                   iTermExternalAttribute *, VT100GridCoord logicalCoord,
    ...
    int endx =
        range.columnWindow.length ? range.columnWindow.location + range.columnWindow.length : [_dataSource width];
    if (endx > width) {
        endx = width;
    }
    int bound = [_dataSource numberOfLines] - 1;
    BOOL fullWidth =
        ((range.columnWindow.location == 0 && range.columnWindow.length == width) || range.columnWindow.length <= 0);
    ...
    for (int y = MAX(0, range.coordRange.start.y); y <= MIN(bound, range.coordRange.end.y); y++) {
        ...
        if (y == range.coordRange.end.y) {
            ...
            const int reducedEndX =
                range.columnWindow.length ? VT100GridWindowedRangeEnd(range).x : range.coordRange.end.x;
        ...
        for (int x = endx - 1; x >= range.columnWindow.location; x--) {
            ITAssertWithMessage(x >= 0 && x < width, @"Counting number of nulls. x=%@ range=%@ width=%@", @(x),
                                VT100GridWindowedRangeDescription(range), @(width));
            BOOL isNull;
        ...
        if (charBlock) {
            if (supportBidi && bidi) {
                const NSRange visualRange =
                    NSMakeRangeFromHalfOpenInterval(MIN(width - 1, MAX(range.columnWindow.location, startx)), endx);
                [bidi enumerateLogicalRangesIn:visualRange
                                       closure:^(NSRange logicalRange, int visualStart, BOOL *stop) {
                                           for (int i = 0; i < logicalRange.length; i++) {
                                               int x = logicalRange.location + i;
                                               if (charBlock(theLine, theLine[x], eaIndex[x], VT100GridCoordMake(x, y),
                                                             VT100GridCoordMake(visualStart + i, y))) {
                                                   *stop = YES;
                                                   return;
                                               }
                                           }
                ...
                for (int x = MIN(width - 1, MAX(range.columnWindow.location, startx)); x < endx - numNulls; x++) {
                    ITAssertWithMessage(x >= 0 && x < width,
                                        @"Iterating terminal nulls. x=%@ range=%@ width=%@ numNulls=%@", @(x),
                                        VT100GridWindowedRangeDescription(range), @(width), @(numNulls));
                    if (charBlock(theLine, theLine[x], eaIndex[x], VT100GridCoordMake(x, y),
                                  VT100GridCoordMake(x, y))) {
                        return;
                    }
                }
            }
...
static NSRange NSMakeRangeFromHalfOpenInterval(NSUInteger lowerBound, NSUInteger openUpperBound) {
    // BUG-f1350: Replace assert() with guard - invalid bounds should return empty range, not crash
    if (lowerBound > openUpperBound) {
        ELog(@"NSMakeRangeFromHalfOpenInterval: lowerBound %lu > openUpperBound %lu, returning empty range",
             (unsigned long)lowerBound, (unsigned long)openUpperBound);
...
- (void)enumerateInReverseCharsInRange:(VT100GridWindowedRange)range
                             charBlock:(BOOL (^NS_NOESCAPE)(screen_char_t theChar, VT100GridCoord logicalCoord,
                                                            VT100GridCoord visualCoord))charBlock
                              eolBlock:(BOOL (^NS_NOESCAPE)(unichar code, int numPrecedingNulls, int line))eolBlock {
    ...
    for (int y = MIN([_dataSource numberOfLines] - 1, range.coordRange.end.y); y >= yLimit; y--) {
        if (self.stopAsSoonAsPossible) {
            DLog(@"Aborted");
            break;
        }
...
- (NSIndexSet *)possibleColumnDividerIndexesAround:(VT100GridCoord)coord {
    NSMutableIndexSet *indexes = [NSMutableIndexSet indexSet];
    VT100GridCoordRange theRange = VT100GridCoordRangeMake(0, coord.y, [_dataSource width], coord.y);
    NSCharacterSet *columnDividers = [self columnDividers];
    [self
        enumerateCharsInRange:VT100GridWindowedRangeMake(theRange, 0, 0)
                  supportBidi:NO
                    charBlock:^BOOL(const screen_char_t *currentLine, screen_char_t theChar, iTermExternalAttribute *ea,
                                    VT100GridCoord logicalCoord, VT100GridCoord theCoord) {
                        if (!theChar.complexChar && [columnDividers characterIsMember:theChar.code]) {
...
- (VT100GridCoord)coordLockedToWindow:(VT100GridCoord)coord {
    ...
    coord.x = MIN(MAX(coord.x, _logicalWindow.location), _logicalWindow.location + _logicalWindow.length - 1);
    return coord;
}
