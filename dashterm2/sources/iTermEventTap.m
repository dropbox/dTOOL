...
- (instancetype)initWithEventTypes:(CGEventMask)types {
    ...
    if (self) {
        _types = types;
        _weakObservers = [[NSMutableArray alloc] initWithCapacity:4];  // Event tap observers
...
- (void)dealloc {
    ...
    // BUG-3036: Release _eventSource and _machPort to avoid resource leaks
    if (_eventSource) {
        CFRunLoopRemoveSource(CFRunLoopGetCurrent(), _eventSource, kCFRunLoopCommonModes);
...
- (void)stopEventTap {
    ...
    // BUG-f939: Guard against stopping already disabled tap instead of crashing
    if (!self.isEnabled) {
        ELog(@"iTermEventTap: stopEventTap called but tap is not enabled");
        return;
    }
...
- (BOOL)startEventTap {
    ...
    // BUG-f940: Guard against starting already enabled tap instead of crashing
    if (self.isEnabled) {
        ELog(@"iTermEventTap: startEventTap called but tap is already enabled");
        return NO;
    }
