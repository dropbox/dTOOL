...
- (BOOL)finishInitialization {
    ...
    _records = [[NSMutableDictionary alloc] initWithCapacity:256];  // Shell history records
    _expandedCache = [[NSMutableDictionary alloc] initWithCapacity:64];  // Expanded entry cache
...
- (BOOL)initializeCoreDataWithRetry:(BOOL)retry vacuum:(BOOL)vacuum {
    ...
    // BUG-f1268: Replace assert() with guard - nil modelURL should return NO, not crash
    if (modelURL == nil) {
        XLog(@"BUG-f1268: Failed to find Model.momd resource");
        return NO;
    }
    ...
    // BUG-f1269: Replace assert() with guard - nil coordinator should return NO, not crash
    if (persistentStoreCoordinator == nil) {
        XLog(@"BUG-f1269: Failed to create persistent store coordinator");
        return NO;
    }

    _managedObjectContext = [[NSManagedObjectContext alloc] initWithConcurrencyType:NSMainQueueConcurrencyType];
    // BUG-f1270: Replace assert() with guard - nil context should return NO, not crash
    if (_managedObjectContext == nil) {
        XLog(@"BUG-f1270: Failed to create managed object context");
        return NO;
    }
...
- (void)makeDatabaseReadableOnlyByUser {
    ...
    NSDirectoryEnumerator<NSURL *> *enumerator =
        [[NSFileManager defaultManager] enumeratorAtURL:[NSURL fileURLWithPath:basePath]
                             includingPropertiesForKeys:nil
                                                options:NSDirectoryEnumerationSkipsSubdirectoryDescendants
                                           errorHandler:nil];
    for (NSURL *fileURL in enumerator) {
        // BUG-7270: Use lastPathComponent instead of path to get just the filename,
        // not the full path. The hasPrefix check was failing because fileURL.path
        // returns "/full/path/ShellHistory.sqlite" which doesn't start with "ShellHistory.sqlite".
        ...
        // Use fileURL.path directly since we already have the full path from the enumerator
...
- (void)deleteObjectsWithEntityName:(NSString *)entityName {
    // BUG-9493: Ensure Core Data access is on the correct queue
...
- (NSArray *)managedObjects {
    ...
    [_managedObjectContext performBlockAndWait:^{
        NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:[iTermHostRecordMO entityName]];
        NSError *error = nil;
...
+ (void)showInformationalMessageInWindow:(NSWindow *)window {
    ...
    alert.informativeText = @"To use shell integration features such as "
                            @"Command History, "
                            @"Recent Directories, "
                      ...
                      switch (returnCode) {
                          case NSAlertFirstButtonReturn:
                              ...
                              [[NSWorkspace sharedWorkspace]
                                  it_openURL:[NSURL URLWithString:@"https://dashterm.com/shell_integration.html"]
                                      target:nil
...
- (void)backingStoreTypeDidChange {
    NSPersistentStore *store = _managedObjectContext.persistentStoreCoordinator.persistentStores.firstObject;
...
- (void)eraseCommandHistory:(BOOL)commandHistory directories:(BOOL)directories {
    ...
    if (directories) {
        [[NSFileManager defaultManager] removeItemAtPath:self.pathToDeprecatedDirectoriesPlist error:NULL];
    }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kDirectoriesDidChangeNotificationName object:nil];
...
- (void)setStatusOfCommandAtMark:(id<VT100ScreenMarkReading>)mark
                          onHost:(id<VT100RemoteHostReading>)remoteHost
                              to:(int)status {
    iTermCommandHistoryCommandUseMO *commandUse = [self commandUseWithMarkGuid:mark.guid onHost:remoteHost];
...
- (BOOL)commandHistoryHasEverBeenUsed {
    return (_records.count > 0 || [[NSUserDefaults standardUserDefaults] boolForKey:kCommandHistoryHasEverBeenUsed]);
}
...
- (NSArray<iTermCommandHistoryEntryMO *> *)commandHistoryEntriesWithPrefix:(NSString *)partialCommand
    ...
    iTermHostRecordMO *hostRecord = [self recordForHost:host];
    NSMutableArray<iTermCommandHistoryEntryMO *> *result = [NSMutableArray arrayWithCapacity:hostRecord.entries.count];
    for (iTermCommandHistoryEntryMO *entry in hostRecord.entries) {
...
- (NSArray<iTermCommandHistoryCommandUseMO *> *)autocompleteSuggestionsWithPartialCommand:(NSString *)partialCommand
                                                                                   onHost:(id<VT100RemoteHostReading>)
                                                                                              host {
    NSArray<iTermCommandHistoryEntryMO *> *temp = [self commandHistoryEntriesWithPrefix:partialCommand onHost:host];
    NSMutableArray<iTermCommandHistoryCommandUseMO *> *result = [NSMutableArray arrayWithCapacity:temp.count];
...
- (BOOL)removeOldData {
    ...
    [fetchRequest setEntity:[NSEntityDescription entityForName:[iTermCommandHistoryCommandUseMO entityName]
                                        inManagedObjectContext:_managedObjectContext]];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"time < %f", [self now] - kMaxTimeToRememberCommands];
    [fetchRequest setPredicate:predicate];
    NSError *error = nil;
    NSArray<iTermCommandHistoryCommandUseMO *> *results = [_managedObjectContext executeFetchRequest:fetchRequest
                                                                                               error:&error];
    ...
    [fetchRequest setEntity:[NSEntityDescription entityForName:[iTermRecentDirectoryMO entityName]
                                        inManagedObjectContext:_managedObjectContext]];
    predicate =
        [NSPredicate predicateWithFormat:@"lastUse < %f and starred == 0", [self now] - kMaxTimeToRememberDirectories];
    [fetchRequest setPredicate:predicate];
    error = nil;
    NSArray<iTermRecentDirectoryMO *> *directories = [_managedObjectContext executeFetchRequest:fetchRequest
                                                                                          error:&error];
...
- (void)loadObjectGraphIntoDictionary:(NSMutableDictionary *)records {
    // BUG-FIX: All Core Data access must happen within performBlockAndWait: to avoid
    // memory issues with managed objects being accessed outside their context's queue.
    if (!_managedObjectContext) {
        NSLog(@"Warning: loadObjectGraphIntoDictionary called with nil managedObjectContext");
        return;
    }
    [_managedObjectContext performBlockAndWait:^{
        NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:[iTermHostRecordMO entityName]];
        ...
        if (error) {
            NSLog(@"Error fetching shell history records: %@", error);
            return;
        }
        if (!managedObjects) {
            return;
        ...
        for (iTermHostRecordMO *hostRecord in managedObjects) {
            @try {
                NSString *key = hostRecord.hostKey;
                if (key) {
                    records[key] = hostRecord;
                }
            } @catch (NSException *exception) {
                NSLog(@"Exception loading host record: %@", exception);
            }
        }
    }];
}
...
- (void)loadObjectGraph {
    ...
    [_expandedCache removeAllObjects];
    // BUG-FIX: Access Core Data managed object properties within performBlockAndWait:
    if (!_managedObjectContext) {
        return;
    }
    [_managedObjectContext performBlockAndWait:^{
        for (NSString *hostKey in _records) {
            ...
            @try {
                ...
                NSLog(@"Exception loading directories for host %@: %@", hostKey, exception);
            }
        }
    }];
}
...
- (NSMutableArray<iTermCommandHistoryCommandUseMO *> *)commandUsesByExpandingEntries:
    (NSArray<iTermCommandHistoryEntryMO *> *)array {
    NSMutableArray<iTermCommandHistoryCommandUseMO *> *result =
        [NSMutableArray arrayWithCapacity:array.count * 2]; // ~2 uses per entry average
    ...
    [result sortWithOptions:0
            usingComparator:^NSComparisonResult(iTermCommandHistoryCommandUseMO *obj1,
...
- (void)loadExpandedCacheForHost:(id<VT100RemoteHostReading>)host {
    ...
    NSArray<iTermCommandHistoryEntryMO *> *temp = [self commandHistoryEntriesWithPrefix:@"" onHost:host];
    NSMutableArray<iTermCommandHistoryCommandUseMO *> *expanded = [self commandUsesByExpandingEntries:temp];
...
- (NSArray<iTermRecentDirectoryMO *> *)directoriesForHost:(id<VT100RemoteHostReading>)host {
    NSArray *directories = [[self recordForHost:host] directories];
    NSMutableArray<iTermRecentDirectoryMO *> *results = [NSMutableArray arrayWithCapacity:directories.count];
    NSMutableArray<iTermRecentDirectoryMO *> *starred = [NSMutableArray arrayWithCapacity:4]; // Small subset
    for (iTermRecentDirectoryMO *directory in directories) {
