...
// Optimization: Cache NSNumber objects for character codes and indices.
// Character codes 0-255 cover ASCII which handles most searches.
// Index cache of 4096 covers typical document lengths in Open Quickly searches.
static const int kCachedCharCodeCount = 256;
static const int kCachedIndexCount = 4096;
static NSNumber *sCharCodeCache[kCachedCharCodeCount];
static NSNumber *sIndexCache[kCachedIndexCount];

NS_INLINE NSNumber *iTermCharCodeToNumber(unichar c) {
    if (c < kCachedCharCodeCount) {
        return sCharCodeCache[c];
    }
    return @(c);
}

NS_INLINE NSNumber *iTermIndexToNumber(NSInteger index) {
    if (index >= 0 && index < kCachedIndexCount) {
        return sIndexCache[index];
    }
    return @(index);
}

__attribute__((constructor)) static void iTermMinimumSubsequenceMatcherInitializeCache(void) {
    for (int i = 0; i < kCachedCharCodeCount; i++) {
        sCharCodeCache[i] = @(i);
    }
    for (int i = 0; i < kCachedIndexCount; i++) {
        sIndexCache[i] = @(i);
    }
}
...
- (instancetype)initWithQuery:(NSString *)query {
    ...
    if (self) {
        ...
        NSMutableArray *temp = [[NSMutableArray alloc] initWithCapacity:normalized.length];
        for (int i = 0; i < normalized.length; i++) {
            [temp addObject:iTermCharCodeToNumber([normalized characterAtIndex:i])];
        }
        _queryChars = temp; // Already retained by alloc/init
    }
    return self;
}
...
- (NSDictionary *)postingListsForDocument:(NSString *)document {
    NSMutableDictionary *postingLists = [[NSMutableDictionary alloc] initWithCapacity:_queryChars.count];
    ...
    for (int i = 0; i < queryLength; i++) {
        if (!postingLists[_queryChars[i]]) {
            postingLists[_queryChars[i]] = [[NSMutableArray alloc] initWithCapacity:8]; // Document positions
        }
    }
    ...
    for (NSInteger i = 0; i < documentLength; i++) {
        ...
        NSMutableArray *indexArray = postingLists[iTermCharCodeToNumber(c)];
        if (indexArray) {
            [indexArray addObject:iTermIndexToNumber(i)];
        }
    }
...
- (BOOL)advanceIndexes {
    ...
    for (NSInteger i = 0; i < queryLength; i++) {
        ...
        _indexes[i] = iTermIndexToNumber(currentIndex);
        previousDocOffset = docOffset;
    }
    return YES;
}
...
- (NSRange)range {
    // BUG-10203: Guard against empty _queryChars or _indexes to prevent nil subscript crash
    if (_queryChars.count == 0 || _indexes.count == 0) {
...
- (NSMutableArray *)initialIndexes {
    ...
    NSMutableArray *indexes = [[NSMutableArray alloc] initWithCapacity:_queryChars.count];
    const NSInteger queryLength = _queryChars.count;
    for (int i = 0; i < queryLength; i++) {
        ...
        [indexes addObject:iTermIndexToNumber(index)];
    }
...
- (NSArray *)bestIndexes {
    ...
    NSMutableArray *bestIndexes = [[NSMutableArray alloc] initWithCapacity:_queryChars.count];
    const NSInteger queryLength = _queryChars.count;
    while (1) {
