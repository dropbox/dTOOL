...
fileprivate let favoriteReorderPasteboardType =
    NSPasteboard.PasteboardType("com.dashterm.dashterm2.ssh-favorite-reorder")

@available(macOS 11, *)
class SSHFilePanelSourceList: NSOutlineView {
    static let sshFileNodePasteboardType = NSPasteboard.PasteboardType("com.dashterm.dashterm2.ssh-file-node")
    static let sshFileNodeDirectoryPasteboardType = NSPasteboard.PasteboardType("com.dashterm.dashterm2.ssh-file-node-directory")
    ...
    required init?(coder: NSCoder) {
        // BUG-f816: Return nil instead of crashing for unused coder initializer
        DLog("SidebarOutlineView init(coder:) is not supported")
        return nil
    }
...
class SSHFilePanelSidebar: NSView {
    ...
    @MainActor
    required init?(coder: NSCoder) {
        // BUG-f817: Return nil instead of crashing for unused coder initializer
        DLog("SSHFilePanelSidebarViewController init(coder:) is not supported")
        return nil
    }
    ...
    private func loadFavoritesFromFinder(_ path: String) -> [Favorite]? {
        ...
        // BUG-1550: Use [AnyClass] array instead of NSSet with force cast
        ...
        guard let plist = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: allowedClasses, from: data),
...
extension SSHFilePanelSidebar: NSOutlineViewDataSource {
    ...
    func outlineView(
        ...
        if item == nil {
            // BUG-f608: Safely handle index out of bounds for section nodes
            guard index >= 0 && index < SidebarSectionNode.allCases.count else {
                DLog("SSHFilePanelSidebar: Invalid section index \(index) for count \(SidebarSectionNode.allCases.count)")
                return SidebarSectionNode.favorites
            }
        ...
        if let section = item as? SidebarSectionNode {
            switch section.identifier {
            case .favorites:
                // BUG-f528: Safely handle index out of bounds
                guard index >= 0 && index < filteredFavorites.count else {
                    DLog("SSHFilePanelSidebar: Invalid favorites index \(index) for count \(filteredFavorites.count)")
                    return SidebarSectionNode.favorites
                }
                return filteredFavorites[index]
            case .connectedHosts:
                // BUG-f528: Safely handle index out of bounds
                guard index >= 0 && index < connectedHosts.count else {
                    DLog("SSHFilePanelSidebar: Invalid connectedHosts index \(index) for count \(connectedHosts.count)")
                    return SidebarSectionNode.connectedHosts
                }
                return connectedHosts[index]
            }
        }

        // BUG-f528: Return a safe fallback instead of crashing for unexpected item types
        DLog("SSHFilePanelSidebar: Unexpected item type \(type(of: item)) in outlineView(_:child:ofItem:)")
        return SidebarSectionNode.favorites
    }
...
extension SSHFilePanelSidebar: NSOutlineViewDelegate {
    func outlineView(_ outlineView: NSOutlineView,
            ...
            if cell == nil {
                ...
                NSLayoutConstraint.activate([
                    textField.leadingAnchor.constraint(
                        equalTo: newCell.leadingAnchor,
                        constant: 0
                    ),
                    textField.trailingAnchor.constraint(
                        equalTo: newCell.trailingAnchor,
                        constant: -8
                    ),
        ...
        if cell == nil {
            ...
            newCell.addSubview(imageView)
            newCell.addSubview(textField)
            newCell.imageView = imageView
            newCell.textField = textField

            NSLayoutConstraint.activate([
                imageView.leadingAnchor.constraint(equalTo: newCell.leadingAnchor, constant: 3),
                ...
                textField.trailingAnchor.constraint(lessThanOrEqualTo: newCell.trailingAnchor),
                textField.topAnchor.constraint(equalTo: newCell.topAnchor, constant: 4),
                textField.bottomAnchor.constraint(equalTo: newCell.bottomAnchor, constant: -4)
            ])
            cell = newCell
        }
