...
- (void)setMouseInfo:(NSArray *)mouseInfo {
    // BUG-f1218: Replace asserts with validation guards - malformed mouse info should be ignored not crash
    if (mouseInfo.count != 7) {
        DLog(@"WARNING: iTermVariableScope setMouseInfo: invalid mouseInfo count %lu, expected 7", (unsigned long)mouseInfo.count);
        return;
    }
    if (![mouseInfo[0] isKindOfClass:[NSNumber class]] ||  // x
        ![mouseInfo[1] isKindOfClass:[NSNumber class]] ||  // y
        ![mouseInfo[2] isKindOfClass:[NSNumber class]] ||  // button
        ![mouseInfo[3] isKindOfClass:[NSNumber class]] ||  // count
        ![mouseInfo[4] isKindOfClass:[NSArray class]] ||   // modifiers
        ![mouseInfo[5] isKindOfClass:[NSNumber class]] ||  // side effects
        ![mouseInfo[6] isKindOfClass:[NSNumber class]]) {  // state
        DLog(@"WARNING: iTermVariableScope setMouseInfo: invalid mouseInfo element types");
        return;
    }
    for (id obj in mouseInfo[4]) {
        if (![obj isKindOfClass:[NSNumber class]]) {  // modifier
            DLog(@"WARNING: iTermVariableScope setMouseInfo: invalid modifier type in mouseInfo[4]");
            return;
        }
    }
