...
enum { kUseSystemWindowRestorationSettingTag = 0, kOpenDefaultWindowArrangementTag = 1, kDontOpenAnyWindowsTag = 2 };
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        [_observer observeKey:@"NSQuitAlwaysKeepsWindows"
                        block:^{
        ...
        [gRemotePrefsObserver observeKey:kPreferenceKeyCustomFolder
                                   block:^{
                                       DLog(@"Remote prefs changed from\n%@", [NSThread callStackSymbols]);
                                   }];
        [gRemotePrefsObserver observeKey:kPreferenceKeyLoadPrefsFromCustomFolder
                                   block:^{
...
- (void)awakeFromNib {
    ...
    __weak __typeof(self) weakSelf = self;
    [self defineControl:_openBookmark key:kPreferenceKeyOpenBookmark relatedView:nil type:kPreferenceInfoTypeCheckbox];
    ...
    info = [self defineControl:_openWindowsAtStartup
        ...
        update:^BOOL {
            ...
            } else if ([WindowArrangements count] && [self boolForKey:kPreferenceKeyOpenArrangementAtStartup]) {
    ...
    info.hasDefaultValue = ^BOOL {
        return [weakSelf boolForKey:kPreferenceKeyOpenArrangementAtStartup] == NO &&
               [weakSelf boolForKey:kPreferenceKeyOpenNoWindowsAtStartup] == NO;
    };
    ...
    info.syntheticSetter = ^(NSNumber *newValue) {
        ...
        [iTermAPIHelper setRequireApplescriptAuth:useApplescript window:self.view.window];
    ...
    _advancedGPUWindowController = [[iTermAdvancedGPUSettingsWindowController alloc]
        initWithWindowNibName:@"iTermAdvancedGPUSettingsWindowController"];
    ...
    info.onChange = ^{
        [iTermWarning showWarningWithTitle:@"You must restart DashTerm2 for this change to take effect."
                                   actions:@[ @"OK" ]
                                identifier:nil
    ...
    [self addViewToSearchIndex:_advancedGPUPrefsButton
                   displayName:@"Advanced GPU settings"
                       phrases:@[
                           _advancedGPUWindowController.viewController.disableWhenDisconnected.title,
    ...
    info = [self defineControl:_loadPrefsFromCustomFolder
                   ...
                   relatedView:nil
                          type:kPreferenceInfoTypeCheckbox];
    info.onChange = ^() {
        [self loadPrefsFromCustomFolderDidChangeByUI:YES];
    };
    info.observer = ^() {
    ...
    info.observer = ^{
        ...
        strongSelf->_wordCharsLabel.labelEnabled =
            ![strongSelf boolForKey:kPreferenceKeyDoubleClickPerformsSmartSelection];
    };
    ...
    // BUG-7200: Third parameter is nullable per Swift definition, so declare _Nullable
    [AIMetadata.instance enumerateModels:^(NSString *_Nonnull name, NSInteger context, NSString *_Nullable url) {
    ...
    PreferenceInfo *tokenLimitInfo = [self defineControl:_aiTokenLimit
                                             ...
                                             relatedView:_aiTokenLimitLabel
                                                    type:kPreferenceInfoTypeIntegerTextField];
    PreferenceInfo *responseTokenLimitInfo = [self defineControl:_aiResponseTokenLimit
    ...
    NSMutableArray<PreferenceInfo *> *aiFeatureInfos = [NSMutableArray arrayWithCapacity:6]; // AI feature checkboxes
    ...
    info = [self defineControl:_aiVendor key:kPreferenceKeyAIVendor relatedView:nil type:kPreferenceInfoTypePopup];
    info.onChange = ^{
    ...
    info = [self defineControl:_enableAI key:kPreferenceKeyEnableAI relatedView:nil type:kPreferenceInfoTypeCheckbox];
    info.syntheticGetter = ^id {
    ...
    // ---------------------------------------------------------------------------------------------
    [self defineControl:_enableRTL key:kPreferenceKeyBidi relatedView:nil type:kPreferenceInfoTypeCheckbox];
...
- (void)aiModelDidChange:(PreferenceInfo *)tokenLimitInfo
    ...
    if ([AIMetadata.instance modelHasDefaults:model]) {
        ...
        [self setInteger:[AIMetadata.instance vectorStoreForModel:model] forKey:kPreferenceKeyAIVectorStore];
...
- (void)updateEnabledState {
    ...
    const BOOL systemRestorationEnabled =
...
- (IBAction)changeAPIKey:(id)sender {
    ...
    alert.messageText = [NSString
        stringWithFormat:@"Enter the API key for your AI provider. The key will be stored securely in the Keychain."];
...
- (IBAction)installPlugin:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/ai-plugin.html"]
                                       target:nil
...
- (void)revealPlugin:(id)sender {
    NSURL *url =
        [[NSWorkspace sharedWorkspace] URLForApplicationWithBundleIdentifier:@"com.dashterm.dashterm2.iTermAI"];
    if (!url) {
        NSBeep();
        return;
    }
...
- (IBAction)exportAllSettingsAndData:(id)sender {
    [self showMessage:[DashTerm2ImportExport exportAll] title:@"Problem Exporting"];
}

- (IBAction)importAllSettingsAndData:(id)sender {
    [self showMessage:[DashTerm2ImportExport importAll] title:@"Problem Importing"];
}
...
- (IBAction)warning:(id)sender {
    ...
    if (@available(macOS 13, *)) {
        message = @"System window restoration has been disabled, which prevents DashTerm2 from respecting this "
                  @"setting. Disable ”System Settings > Desktop & Dock > Close windows when quitting an application“ "
                  @"to enable window restoration.";
        action = @"Open System Settings";
        path = @"/System/Library/PreferencePanes/Dock.prefPane";
    } else {
        message =
            @"System window restoration has been disabled, which prevents DashTerm2 from respecting this setting. "
            @"Disable System Settings > General > Close windows when quitting an app to enable window restoration.";
...
- (IBAction)advancedGPU:(NSView *)sender {
    [self.view.window beginSheet:_advancedGPUWindowController.window
               completionHandler:^(NSModalResponse returnCode){
               }];
}
...
- (IBAction)resetAIPrompt:(id)sender {
    ...
    [self setString:[iTermPreferences defaultObjectForKey:key] forKey:key];
    [_aiPrompt.textStorage
        setAttributedString:[NSAttributedString attributedStringWithString:iTermDefaultAIPrompt
                                                                attributes:_aiPrompt.typingAttributes]];
}

- (IBAction)aiPromptHelp:(id)sender {
    NSString *text = [NSString
        stringWithContentsOfFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"ai-prompt-help" ofType:@"md"]
...
- (void)updateCustomScriptsFolderViews {
    ...
    const BOOL locationIsValid =
        [[NSFileManager defaultManager] customScriptsFolderIsValid:_customScriptsFolder.stringValue];
    _customScriptsFolderWarning.image = locationIsValid ? [NSImage it_imageNamed:@"CheckMark" forClass:self.class]
                                                        : [NSImage it_imageNamed:@"WarningSign" forClass:self.class];
}
...
- (void)updateRemotePrefsViews {
    ...
    _prefsDirWarning.image = remoteLocationIsValid ? [NSImage it_imageNamed:@"CheckMark" forClass:self.class]
                                                   : [NSImage it_imageNamed:@"WarningSign" forClass:self.class];
    BOOL isValidFile = (shouldLoadRemotePrefs && remoteLocationIsValid &&
