...
- (IBAction)help:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/status-bar-layout"]
                                       target:nil
...
- (NSImage *)collectionView:(NSCollectionView *)collectionView draggingImageForItemsAtIndexPaths:(NSSet<NSIndexPath *> *)indexPaths
    ...
    // BUG-f1276: Replace assert() with guard - nil item should return nil image, not crash
    if (item == nil) {
        DLog(@"WARNING: BUG-f1276 newItemWithIndexPath returned nil for %@", indexPaths);
        return nil;
    }
    item.hideDetail = YES;
    [item sizeToFit];
