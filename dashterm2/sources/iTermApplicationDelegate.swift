    ...
    func registerMenuTips() {
        ...
        let tips = [
            ...
            Tip(identifier: "Edit.Actions",
                ...
                text: "Actions are saved instructions for DashTerm2. For example, you could create an action that opens a new window and then creates a split pane."),
            ...
            Tip(identifier: "Toolbelt.Paste History",
                text: "**Paste History** shows text that you have copied and pasted in DashTerm2. You can configure it to be saved long term."),
            ...
            // BUG-2: Python REPL branding updated to DashTerm2
            Tip(identifier: "Open Interactive Window",
                text: "The Python REPL opens a window running a special Python interpreter that lets you experiment with DashTerm2's Python API. You can use `await` at the top level of the interpreter."),
            ...
            Tip(identifier: "Install Python Runtime",
                text: "DashTerm2's Python Runtime is a large binary package (hundreds of MBs) that enables the Python API by installing a pre-built Python environment that scripts can use."),
            ...
            Tip(identifier: "Install Shell Integration",
                text: "**Shell Integration** consists of shell scripts that run when you log in. They inform DashTerm2 of where your shell prompt is. This enables dozens of useful features such as command history, directory history, AI features, and more."),
        ...
        for tip in tips {
            if let item = index[tip.identifier] {
                // BUG-1776: Use nil coalescing with fallback instead of force unwrap
                let tipAttributedString = NSAttributedString.attributedString(markdown: tip.text,
                                                                              font: NSFont.systemFont(ofSize: NSFont.systemFontSize),
                                                                              paragraphStyle: NSParagraphStyle.default) ?? NSAttributedString(string: tip.text)
                controller.registerTip(forMenuItem: item,
                                       image: tip.imageName.compactMap { NSImage.it_imageNamed($0, for: Self.self) },
                                       attributedString: tipAttributedString)
            } else {
                #if(DEBUG)
                // BUG-f581, BUG-f677: Log warning instead of crashing for missing menu tip identifiers
                // This is a developer check that shouldn't crash any builds
                DLog("BUG-f581: Menu tip index missing identifier '\(tip.identifier)' - skipping")
                #endif
            }
        }
    }
