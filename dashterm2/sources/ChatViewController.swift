...
class ChatViewController: NSViewController {
    ...
    private var commandExitSnapshotSessionGuid: String?
    ...
    private(set) var streaming = false {
        didSet {
            ...
            updateCommandExitSnapshotSubscription()
        }
    }
    ...
    init(listModel: ChatListModel, client: ChatClient) {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               object: nil)
        metadataObserver = NotificationCenter.default.addObserver(forName: ChatListModel.metadataDidChange,
                                                                  object: nil,
                                                                  queue: .main) { [weak self] _ in
            self?.updateCommandExitSnapshotSubscription()
        }
    ...
    required init?(coder: NSCoder) {
        // BUG-f819: Return nil instead of crashing for unused coder initializer
        DLog("ChatViewController init(coder:) is not supported")
        return nil
    }

    deinit {
        if let observer = _commandDidExitObserver {
        ...
        tearDownCommandExitSnapshotSubscription()
    ...
    override func loadView() {
        ...
        // BUG-1643: Use nil coalescing instead of force unwrap for color parsing
        let divider = ChatViewControllerDividerView(
            gradient: .init(
                stops: [
                    .init(
                        color: .it_dynamicColor(
                            forLightMode: .init(fromHexString: "#f2f2f2") ?? .lightGray,
                            darkMode: .init(fromHexString: "#161616") ?? .darkGray), location: 0.25),
                    .init(
                        color: .it_dynamicColor(
                            forLightMode: .init(fromHexString: "#e3e3e3") ?? .lightGray,
                            darkMode: .init(fromHexString: "#0b0b0b") ?? .black), location: 0.75)]))
...
extension ChatViewController {
        ...
        if newPermission == .always,
           let autopopulationWarningText = category.autopopulationWarningText {
            ...
            case .kiTermWarningSelection2:
                newPermission = .never
            default:
                // BUG-f601: Handle unexpected/cancel selection gracefully instead of crashing
                DLog("BUG-f601: Unexpected warning selection \(sel.rawValue), keeping existing permission")
                return
            }
        }
        do {
            ...
            // BUG-2672: Show error to user when permission setting fails
            DLog("\(error)")
            presentErrorAlert(title: "Permission Update Failed", error: error)
        }
    }
    ...
    // BUG-2672/BUG-2673: Helper to present error alerts to users instead of failing silently
...
extension ChatViewController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        // BUG-830: Bounds check to prevent crash if model changes between numberOfRows and this call
        guard row >= 0 && row < model.items.count else {
            DLog("tableView(_, viewFor:, row: \(row)) out of bounds (count=\(model.items.count))")
            return nil
        }
    ...
    private func view(forItem item: ChatViewControllerModel.Item, isLastMessage: Bool) -> NSView {
        ...
        case .message(let message):
            ...
            case .userCommand:
                // BUG-f529: Return a placeholder cell instead of crashing
                // User messages should not be in model but handle gracefully
                DLog("ChatViewController: Unexpected userCommand message in model")
                let cell = RegularMessageCellView()
                configure(cell: cell, for: message.message, isLast: isLastMessage)
                return cell
            case .append, .appendAttachment:
                // BUG-f530: Return a placeholder cell instead of crashing
                // Append-type messages should not be in model but handle gracefully
                DLog("ChatViewController: Unexpected append-type message in model")
                let cell = RegularMessageCellView()
                configure(cell: cell, for: message.message, isLast: isLastMessage)
                return cell
    ...
    private func configure(cell: RegularMessageCellView,
        ...
        case .remoteCommandRequest(let remoteCommand, safe: _):
            ...
            cell.buttonClicked = { [client, listModel] identifier, messageID in
                ...
                if allowed {
                    // BUG-2675: Log errors from AI command execution instead of silent try?
                    do {
                        ...
                        DLog("Failed to perform remote command: \(error)")
                        // Attempt to notify AI that command failed
                        try? self.client.respondSuccessfullyToRemoteCommandRequest(
                            inChat: chatID,
                            requestUUID: messageID,
                            ...
                            functionCallName: functionCallName,
                            functionCallID: functionCallID,
                            userNotice: "AI command execution failed: \(error.localizedDescription)")
                    }
                } else {
                    // BUG-2675: Log errors from decline response instead of silent try?
                    do {
                        try self.client.respondSuccessfullyToRemoteCommandRequest(
                            inChat: chatID,
                            requestUUID: messageID,
                        ...
                        DLog("Failed to send decline response to AI: \(error)")
                    }
                }
            }
        case .plainText, .markdown, .explanationRequest, .explanationResponse,
                .remoteCommandResponse, .renameChat, .append, .commit, .setPermissions,
                .appendAttachment, .multipart, .terminalCommand:
            // These message types don't have interactive buttons
            cell.buttonClicked = nil
        }
    }
    ...
    func tableView(_ tableView: NSTableView, heightOfRow row: Int) -> CGFloat {
        ...
        // BUG-830: Bounds check to prevent crash if model changes between numberOfRows and this call
        guard row >= 0 && row < model.items.count else {
            DLog("tableView(_, heightOfRow: \(row)) out of bounds (count=\(model.items.count))")
            return 0
        }
...
extension LLM.Message.Attachment {
    func localPathCreatingIfNeeded() -> String? {
        ...
        guard let path = proposedLocalPath() else {
            return nil
        }
        switch type {
        case .code(let text):
            do {
                ...
                // BUG-2671: Return nil when write fails instead of returning path
                DLog("Failed to write to \(path): \(error)")
                return nil
            }
        case .statusUpdate:
            // Status updates are ephemeral UI elements, not files - nothing to write
            DLog("statusUpdate attachment type has no file content to write")
            return nil
        case .file(let file):
            do {
        ...
        case .fileID(id: let fileID, name: _):
            // fileID represents a file stored on OpenAI's servers that hasn't been downloaded yet.
            // We cannot synchronously download it here. Return nil to indicate file not available locally.
            DLog("fileID attachment '\(fileID)' requires async download - not available locally yet")
            return nil
        }
        return path
    }
    ...
    private var basePathForAttachments: String {
        NSTemporaryDirectory() + "DashTerm2ChatAttachments/"
    }
    ...
    private func possibleLocalPaths() -> [String] {
        ...
        case .statusUpdate:
            // Status updates are ephemeral UI elements, not files - no local paths
            []
        case .fileID(id: _, name: let name):
            // fileID represents a file on OpenAI's servers. Return where it would be stored if downloaded.
            [basePathForAttachments.appendingPathComponent(id).appendingPathComponent(name.lastPathComponent)]
        }
    }
    ...
    private func proposedLocalPath() -> String? {
        guard let path = possibleLocalPaths().first else {
            DLog("No possible local paths for attachment type \(type)")
            return nil
        }
        do {
...
extension ChatViewController: ChatInputViewDelegate {
    ...
    func sendButtonClicked(text: String) {
        ...
        do {
            ...
            // BUG-2673: Show error to user when message fails to send
            DLog("\(error)")
            presentErrorAlert(title: "Message Send Failed", error: error)
        }
    }
    ...
    private func tearDownCommandExitSnapshotSubscription() {
        if let currentGuid = commandExitSnapshotSessionGuid,
           let session = iTermController.sharedInstance().session(withGUID: currentGuid) {
            session.removeCommandExitSnapshotSubscriber(self)
        }
        commandExitSnapshotSessionGuid = nil
    }

    private func updateCommandExitSnapshotSubscription() {
        guard streaming,
              let guid = model?.terminalSessionGuid,
              let session = iTermController.sharedInstance().session(withGUID: guid) else {
            tearDownCommandExitSnapshotSubscription()
        ...
        if commandExitSnapshotSessionGuid == guid {
            return
        }
        tearDownCommandExitSnapshotSubscription()
        session.addCommandExitSnapshotSubscriber(self)
        commandExitSnapshotSessionGuid = guid
    }
...
extension ChatViewController {
    func streamLastCommand(_ userInfo: [AnyHashable: Any]) {
        // BUG-476: Handle non-streaming state gracefully instead of crashing
        guard streaming else {
            DLog("Warning: streamLastCommand called when not streaming - ignoring")
            return
        }
        ...
        guard let startLine = userInfo[PTYCommandDidExitUserInfoKeyStartLine] as? Int32,
            ...
            DLog("Missing required userInfo keys in command did exit notification")
            return
        }
...
extension ChatViewController: ChatViewControllerModelDelegate {
    ...
    func chatViewControllerModel(didInsertItemAtIndex i: Int) {
        ...
        estimatedCount += 1
        // BUG-477: Handle invalid index gracefully instead of crashing
        guard i <= estimatedCount else {
            DLog("Warning: Insert index \(i) exceeds estimated count \(estimatedCount) - adjusting")
            estimatedCount = i
            return
        }
    ...
    func chatViewControllerModel(didRemoveItemsInRange range: Range<Int>) {
        ...
        // BUG-478: Handle invalid range gracefully instead of crashing
        guard range.upperBound <= estimatedCount else {
            DLog("Warning: Remove range upper bound \(range.upperBound) exceeds estimated count \(estimatedCount) - adjusting")
            estimatedCount = max(0, estimatedCount - range.count)
            return
        }
...
extension Message.Content {
    func attributedStringValue(linkColor: NSColor,
        ...
        case .multipart(let subparts, vectorStoreID: _):
            // Combine all subparts into a single attributed string
            ...
            for subpart in subparts {
                ...
                case .markdown(let markdown):
                    result.append(AttributedStringForGPTMarkdown(markdown,
                                                                  linkColor: linkColor,
                                                                  textColor: textColor) { })
                case .attachment(let attachment):
                    let paragraphStyle = NSMutableParagraphStyle()
                    ...
                    case .statusUpdate(let update):
                        ...
                        case .webSearchStarted:
                            description = "[Web search started...]"
                        case .webSearchFinished(let result):
                            description = result ?? "[Web search finished]"
                        case .codeInterpreterStarted:
                            description = "[Code interpreter started...]"
                        case .codeInterpreterFinished:
                            description = "[Code interpreter finished]"
                        case .reasoningSummaryUpdate(let summary):
                            description = summary
                        case .multipart(let updates):
                            description = updates.map { update -> String in
                                switch update {
                                case .webSearchStarted: return "[Web search started...]"
                                case .webSearchFinished(let r): return r ?? "[Web search finished]"
                                case .codeInterpreterStarted: return "[Code interpreter started...]"
                                case .codeInterpreterFinished: return "[Code interpreter finished]"
                                case .reasoningSummaryUpdate(let s): return s
                                case .multipart: return "[Status updates]"
                                }
                            }.joined(separator: "\n")
                        }
                    case .file(let file):
                    ...
                    result.append(NSAttributedString(string: description, attributes: attributes))
                case .context(let context):
                    ...
                    result.append(NSAttributedString(string: context, attributes: attributes))
                }
            }
            return result
        ...
        case .remoteCommandResponse(let response, _, _, _):
            ...
            case .success(let output):
                // Render successful command output as markdown (code block style)
                let formattedOutput = "```\n\(output)\n```"
                return AttributedStringForGPTMarkdown(formattedOutput,
                                                      linkColor: linkColor,
                                                      textColor: textColor) {}
        ...
        case .clientLocal(let clientLocal):
            ...
            case .streamingChanged(let state):
                ...
                case .stoppedAutomatically:
                    AttributedStringForSystemMessageMarkdown("Terminal commands will no longer be sent to AI automatically. Automatic sending always terminates when DashTerm2 restarts or the current chat changes.") {}
                }
