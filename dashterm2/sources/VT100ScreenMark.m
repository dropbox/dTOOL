...
// Lock protecting the registry - replaces @synchronized([VT100ScreenMark class])
static os_unfair_lock sRegistryLock = OS_UNFAIR_LOCK_INIT;
...
+ (id<VT100ScreenMarkReading>)markWithGuid:(NSString *)guid
    ...
    os_unfair_lock_lock(&sRegistryLock);
    VT100ScreenMark *mark = [self.registry objectForKey:guid];
    os_unfair_lock_unlock(&sRegistryLock);
...
- (instancetype)initWithDictionary:(NSDictionary *)dict shouldRegister:(BOOL)shouldRegister {
    ...
    if (self) {
        ...
        NSArray *capturedOutputDicts = dict[kMarkCapturedOutputKey];
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:capturedOutputDicts.count];
        _capturedOutput = array;
        for (NSDictionary *capturedOutputDict in capturedOutputDicts) {
...
- (instancetype)copyWithZone:(NSZone *)zone {
    // BUG-f1376: Replace assert with guard - copying a doppelganger should return nil, not crash
    // BUG-1202: Use isDoppelgangerLocked instead of self.isDoppelganger.
    // copyWithZone: is called from iTermMark's doppelganger method which holds sMarkLock.
    // The regular isDoppelganger property would try to acquire the lock again (deadlock).
    if ([self isDoppelgangerLocked]) {
        DLog(@"WARNING: Attempted to copy a doppelganger (VT100ScreenMark)");
        return nil;
    }
...
- (NSArray *)capturedOutputDictionaries {
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:_capturedOutput.count];
...
- (void)addCapturedOutput:(CapturedOutput *)capturedOutput {
    if (!_capturedOutput) {
        _capturedOutput = [[NSMutableArray alloc] initWithCapacity:8];  // Captured output entries
