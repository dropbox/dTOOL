...
static NSString *const iTermActionsEditingPasteboardType = @"com.dashterm.dashterm2.iTermActionsEditingPasteboardType";
...
// BUG-3106: Add bounds check to prevent array index out of bounds crash
// The data source may change between table view selection and action execution.
- (NSArray<iTermAction *> *)selectedActions {
    ...
    return [[_tableView.selectedRowIndexes it_array] mapWithBlock:^id(NSNumber *indexNumber) {
        const NSInteger index = indexNumber.integerValue;
...
- (void)pushUndo {
    // BUG-1199: Deep copy actions array to prevent undo state corruption from in-place mutations
    NSArray *actionsCopy = [[NSArray alloc] initWithArray:_actions copyItems:YES];
    [[self undoManager] registerUndoWithTarget:self selector:@selector(setActions:) object:actionsCopy];
}
...
- (void)actionsDidChange:(iTermActionsDidChangeNotification *)notif {
    ...
    switch (notif.mutationType) {
        ...
        case iTermActionsDidChangeMutationTypeDeletion: {
            [_tableView it_performUpdateBlock:^{
                [_tableView removeRowsAtIndexes:notif.indexSet withAnimation:YES];
            }];
            break;
        }
        case iTermActionsDidChangeMutationTypeInsertion: {
            [_tableView it_performUpdateBlock:^{
                [_tableView insertRowsAtIndexes:[NSIndexSet indexSetWithIndex:notif.index] withAnimation:YES];
            }];
            break;
        }
        case iTermActionsDidChangeMutationTypeMove: {
            [_tableView it_performUpdateBlock:^{
                ...
                [_tableView insertRowsAtIndexes:insertionIndexes withAnimation:YES];
            }];
            break;
        }
...
- (void)exportToItem:(iTermSavePanelItem *)item {
    ...
    // Pre-size based on selection count
    NSMutableArray<NSDictionary *> *array = [NSMutableArray arrayWithCapacity:indexes.count];
    [indexes enumerateIndexesUsingBlock:^(NSUInteger i, BOOL *_Nonnull stop) {
        // BUG-10364: Bounds check - selection indices may be stale after data changes
        if (i >= _actions.count) {
            return;
        }
...
- (NSView *)viewForTitleColumnOnRow:(NSInteger)row {
    // BUG-10365: Bounds check - row may be stale after data changes
...
- (NSView *)viewForActionColumnOnRow:(NSInteger)row {
    // BUG-10366: Bounds check - row may be stale after data changes
    if (row < 0 || row >= (NSInteger)_actions.count) {
        return nil;
    }
...
- (id<NSPasteboardWriting>)tableView:(NSTableView *)tableView pasteboardWriterForRow:(NSInteger)row {
    // BUG-10367: Bounds check - row may be stale during drag operations
    if (row < 0 || row >= (NSInteger)_actions.count) {
        return nil;
    }
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    [self pushUndo];
    // Typically dragging 1-10 items
    NSMutableArray<NSNumber *> *allIdentifiers = [NSMutableArray arrayWithCapacity:8];
    [info
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *_Nonnull draggingItem, NSInteger idx, BOOL *_Nonnull stop) {
                                   ...
                                   NSArray<NSNumber *> *identifiers =
                                       [item propertyListForType:iTermActionsEditingPasteboardType];
                                   [allIdentifiers addObjectsFromArray:identifiers];
                               }];
    [[iTermActionsModel sharedInstance] moveActionsWithIdentifiers:allIdentifiers toIndex:row];
    return YES;
}
