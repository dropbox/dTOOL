...
+ (instancetype)withPODs:(NSDictionary<NSString *, id> *)pod
    ...
    // BUG-f934: Guard against nil identifier instead of crashing
    if (!identifier) {
        ELog(@"iTermEncoderGraphRecord: withPODs called with nil identifier");
        return nil;
    }
...
- (instancetype)initWithPODs:(NSDictionary<NSString *, id> *)pods
    ...
    // BUG-f935: Guard against nil key instead of crashing
    if (!key) {
        ELog(@"iTermEncoderGraphRecord: initWithPODs called with nil key");
        return nil;
    }
    self = [super init];
    if (self) {
        ...
        [graphRecords
            enumerateObjectsUsingBlock:^(iTermEncoderGraphRecord *_Nonnull child, NSUInteger idx, BOOL *_Nonnull stop) {
                child->_parent = self;
            }];
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<iTermEncoderGraphRecord: rowid=%@ key=%@ gen=%@ id=%@ pod=%@ graphs=%@>",
                                      self.rowid, self.key, @(self.generation), self.identifier, self.pod,
                                      [[self.graphRecords mapWithBlock:^id(iTermEncoderGraphRecord *anObject) {
                                          return [NSString stringWithFormat:@"<graph rowid=%@ key=%@ id=%@>",
                                                                            anObject.rowid, anObject.key,
                                                                            anObject.identifier];
                                      }] componentsJoinedByString:@", "]];
}

- (void)setRowid:(NSNumber *)rowid {
    // BUG-f936: Use ELog instead of exception for duplicate rowid - prevent crash
    if (_rowid != nil) {
        ELog(@"iTermEncoderGraphRecord: setRowid called with existing rowid=%@ new=%@", _rowid, rowid);
        return;
    }
    _rowid = rowid;
}
...
- (BOOL)isEqual:(id)object {
    ...
    if (![[other.graphRecords sortedArrayUsingSelector:@selector(compareGraphRecord:)]
            isEqual:[self.graphRecords sortedArrayUsingSelector:@selector(compareGraphRecord:)]]) {
        return NO;
    }
...
- (iTermEncoderGraphRecord *_Nullable)childRecordWithKey:(NSString *)key identifier:(NSString *)identifier {
    if (_index) {
        iTermTuple<NSString *, NSString *> *tuple = [iTermTuple tupleWithObject:key andObject:identifier];
        return _index[tuple];
    }
    return [_graphRecords objectPassingTest:^BOOL(iTermEncoderGraphRecord *element, NSUInteger index, BOOL *stop) {
        return ([element.key isEqualToString:key] && [identifier isEqualToString:element.identifier]);
    }];
}
...
- (void)ensureIndexOfGraphRecords {
    ...
    _index = [[NSMutableDictionary alloc] initWithCapacity:_graphRecords.count];
    for (iTermEncoderGraphRecord *element in _graphRecords) {
        iTermTuple<NSString *, NSString *> *key = [iTermTuple tupleWithObject:element.key andObject:element.identifier];
        _index[key] = element;
    }
}
...
- (void)enumerateArrayWithKey:(NSString *)key
                        block:(void (^NS_NOESCAPE)(NSString *identifier, NSInteger index, id obj, BOOL *stop))block {
...
- (NSArray *)arrayWithKey:(NSString *)key {
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:16]; // Graph record array elements
    [self
        enumerateArrayWithKey:key
                        block:^(NSString *_Nonnull identifier, NSInteger index, id _Nonnull obj, BOOL *_Nonnull stop) {
...
- (id)objectWithKey:(NSString *)key
              class:(Class)desiredClass
              error:(out NSError *__autoreleasing _Nullable *_Nullable)error {
    id instance = [desiredClass castFrom:_pod[key]];
    if (!instance) {
        if (error) {
            *error = [[NSError alloc] initWithDomain:@"com.dashterm.dashterm2.graph-record"
                                                code:1
                                            userInfo:@{NSLocalizedDescriptionKey : @"No such record or wrong type"}];
        }
        return nil;
    }
    return instance;
...
- (NSArray *)arrayValue {
    // BUG-f937: Guard against invalid key instead of crashing
    if (![self.key isEqualToString:@"__array"]) {
        ELog(@"iTermEncoderGraphRecord: arrayValue called but key is '%@' not '__array'", self.key);
        return @[];
    }
...
- (NSDictionary *)dictionaryValue {
    // BUG-f938: Guard against invalid key instead of crashing
    if (![self.key isEqualToString:@"__dict"]) {
        ELog(@"iTermEncoderGraphRecord: dictionaryValue called but key is '%@' not '__dict'", self.key);
        return @{};
    }
