...
- (void)appendStringToComposer:(NSString *)string {
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
...
- (void)terminalAppendAsciiData:(AsciiData *)asciiData {
    ...
    if (![self appendAsciiDataToTriggerLine:asciiData] && self.config.loggingEnabled) {
        const screen_char_t foregroundColorCode = self.terminal.foregroundColorCode;
        const screen_char_t backgroundColorCode = self.terminal.backgroundColorCode;
        NSData *data = [NSData dataWithBytes:asciiData->buffer length:asciiData->length];
        const BOOL atPrompt = _promptStateMachine.isAtPrompt;
        [self
            addSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
                DLog(@"begin side-effect");
...
- (void)terminalAppendMixedAsciiCRLFData:(AsciiData *)asciiData crlfs:(CTVector(int) *)crlfs {
    [self appendMixedAsciiCRLFData:asciiData crlfs:crlfs startOffset:0];
}

- (int)appendMixedAsciiCRLFData:(AsciiData *)asciiData crlfs:(CTVector(int) *)crlfs startOffset:(int)startOffset {
    int numLines = 0;
    int start = 0;
    ...
    for (NSInteger i = 0; i < count + 1; i += 2) {
        ...
        if (control > start) {
            ScreenChars screenChars = {.buffer = asciiData->screenChars->buffer + start, .length = control - start};
            AsciiData temp = {
                .buffer = asciiData->buffer + start, .length = control - start, .screenChars = &screenChars};
            numLines += temp.length / width;
            DLog(@"Append ascii data at %@: %.*s", VT100GridCoordDescription(self.currentGrid.cursor), temp.length,
                 temp.buffer);
...
- (VT100ScreenMutableStateAppendInfo)appendInfoForTokens:(NSArray<VT100Token *> *)tokens {
    ...
    for (VT100Token *token in tokens) {
        ...
        WrappedLineInfo info = {.tokenIndex = tokenIndex};
        int start = 0;
        ...
        for (int j = 0; j < count + 2; j += 2) {
            ...
            DLog(@"Check range %@..≤%@ on segment %d/%@: %.*s", @(start), @(control), j, @(count + 1), length,
    ...
    for (int j = CTVectorCount(&wrappedLineInfos) - 1; j >= 0; j--) {
        ...
        if (wli.startY <= cutoff && wli.endY >= cutoff) {
            const int distanceToGrid = VT100GridCoordDistance(VT100GridCoordMake(wli.startX, wli.startY),
                                                              VT100GridCoordMake(0, cutoff), width);
...
- (void)terminalAppendMixedAsciiGang:(NSArray<VT100Token *> *)tokens {
    ...
    BOOL canTakeFastPath =
        (!self.collectInputForPrinting && ![self shouldEvaluateTriggers] &&
         ![self shouldConvertCharactersToGraphicsCharacterSetInTerminal:terminal] && !self.config.publishing &&
         self.commandStartCoord.x == -1 && tokens.count > 0 && self.wraparoundMode && !self.ansi && !self.insert &&
         self.currentGrid == self.primaryGrid && self.currentGrid.canTakeFastPath);
    if (!canTakeFastPath) {
...
- (void)appendGangFastPath:(NSArray<VT100Token *> *)tokens {
    ...
    ITAssertWithMessage(info.tokenCount >= 0, @"Negative token count. cursor=%d, height=%d, tokens=%@", coord.y,
                        self.height, tokens);
    ...
    iTermImmutableMetadataInit(&metadata, self.currentGrid.currentDate, NO,
    ...
    for (int ti = 0; ti <= lastTokenIndexToSendToLineBuffer; ti++) {
        ...
        for (NSInteger i = 0; i < count + 2; i += 2) {
            ...
            if (!stop || screenChars.length > 0) {
                iTermAppendItem item = {.buffer = screenChars.buffer,
                ...
                if (!item.partial || item.length > 0) {
                    DLog(@"Append direct to line buffer %d chars starting with %.*s", screenChars.length,
                         MIN(screenChars.length, 40), asciiData->buffer + start);
    ...
    [self.linebuffer appendLines:&items width:width];
    ...
    for (int ti = info.tokenCount; ti < tokens.count; ti++) {
        ...
        [self appendMixedAsciiCRLFData:token.asciiData crlfs:token.crlfs startOffset:startOffset];
        startOffset = 0;
    }
...
- (void)terminalBackspace {
    ...
    if (self.commandStartCoord.x != -1 &&
        (self.currentGrid.cursorX != cursorX || self.currentGrid.cursorY != cursorY)) {
...
- (BOOL)terminalShouldSendReport:(BOOL)tmuxAllowed {
    ...
    [self
        addJoinedSideEffect:^(id<VT100ScreenDelegate> delegate) {
...
- (VT100OutputOptionalDeviceAttributes)terminalOptionalDeviceAttributes {
    return (VT100OutputOptionalDeviceAttributes){.osc52 = self.config.osc52};
}
...
- (void)terminalShowTestPattern {
    ...
    [self.currentGrid setCharsFrom:VT100GridCoordMake(0, 0)
                                to:VT100GridCoordMake(self.currentGrid.size.width - 1, self.currentGrid.size.height - 1)
                            toChar:ch
...
- (int)terminalRelativeCursorX {
    DLog(@"begin");
    // BUG-10059: Clamp to 1 minimum to prevent negative values when cursor is outside left margin.
    // Per VT100 spec, relative position should be 1-based and never negative.
    const int relative = self.currentGrid.cursorX - self.currentGrid.leftMargin + 1;
    return MAX(1, relative);
}

- (int)terminalRelativeCursorY {
    DLog(@"begin");
    // BUG-10059: Clamp to 1 minimum to prevent negative values when cursor is outside top margin.
    // Per VT100 spec, relative position should be 1-based and never negative.
    const int relative = self.currentGrid.cursorY - self.currentGrid.topMargin + 1;
    return MAX(1, relative);
}
...
- (void)terminalSetCursorType:(ITermCursorType)cursorType {
    ...
    [self
        addDeferredSideEffect:^(id<VT100ScreenDelegate> delegate) {
            DLog(@"begin side-effect");
...
- (void)terminalSetCursorBlinking:(BOOL)blinking {
    ...
    [self
        addDeferredSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
            DLog(@"begin side-effect");
...
- (void)terminalResetCursorTypeAndBlink {
    DLog(@"begin");
    [self
        addSideEffect:^(id<VT100ScreenDelegate> delegate) {
            DLog(@"begin side-effect");
...
- (BOOL)terminalLineDrawingFlagForCharset:(int)charset {
    ...
    return [self lineDrawingModeEnabledForCharset:charset];
}
...
- (void)terminalSetWidth:(int)width
    ...
    DLog(@"begin width=%@ preserveScreen=%@ updateRegions=%@ newCursorCoord=%@", @(width), @(preserveScreen),
         @(updateRegions), VT100GridCoordDescription(newCursorCoord));
    __weak __typeof(self) weakSelf = self;
    const int height = self.currentGrid.size.height;
    dispatch_queue_t queue = _queue;
    [self
        addUnmanagedPausedSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate,
                                       iTermTokenExecutorUnpauser *_Nonnull unpauser) {
            DLog(@"side effect starts");
            const BOOL shouldResize =
                ([delegate screenShouldInitiateWindowResize] == PTYSessionResizePermissionAllowed &&
                 ![delegate screenWindowIsFullscreen]);
            if (shouldResize) {
                DLog(@"shouldResize is true");
                [delegate screenResizeToWidth:width height:height];
            }
            dispatch_async(queue, ^{
                DLog(@"begin async");
...
- (void)terminalSetRows:(int)rows andColumns:(int)columns {
    ...
    [self
        addUnmanagedPausedSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate,
                                       iTermTokenExecutorUnpauser *_Nonnull unpauser) {
            // BUG-10125: VT100GridSizeMake takes (width, height), not (rows, columns)
            // columns = width, rows = height
            [delegate screenSetSize:VT100GridSizeMake(columns, rows)];
            [unpauser unpause];
        }
...
- (void)finishAfterResizing:(BOOL)didResize
             preserveScreen:(BOOL)preserveScreen
              updateRegions:(BOOL)updateRegions
               moveCursorTo:(VT100GridCoord)newCursorCoord
                 ...
                 completion:(void (^)(void))completion {
    DLog(@"begin didResize=%@ preserveScreen=%@ updateRegions=%@ newCursorCoord=%@", @(didResize), @(preserveScreen),
         @(updateRegions), VT100GridCoordDescription(newCursorCoord));
    ;
    ...
    if (updateRegions) {
        ...
        [self setScrollRegionTop:0 bottom:self.height - 1];
    }
...
- (void)terminalSetWindowTitle:(NSString *)title {
    ...
    // Note: Known limitation - remote host can scroll off history end, causing working directory
    // to fall back to PTYTask (when nil is passed here). This is acceptable because unlimited
    // scrollback typically prevents it, and there's no practical fix without storing unbounded
    // working directory history outside the line buffer.
    //
...
- (void)terminalReportPasteboard:(NSString *)pasteboard {
    ...
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
            DLog(@"running");
            [delegate screenReportPasteboard:pasteboard
                                  completion:^{
                                      DLog(@"unpausing");
                                      [unpauser unpause];
                                      [weakSelf didSendReport:delegate];
...
- (void)terminalDidFinishReceivingPasteboard {
    ...
    if (self.pasteboardString && self.config.clipboardAccessAllowed) {
        ...
        if (data) {
            DLog(@"data=%@", data);
            NSString *string = [[NSString alloc] initWithData:data encoding:self.terminal.encoding];
            if (!string) {
                DLog(@"nil string");
...
- (void)terminalDeleteLinesAtCursor:(int)n {
    ...
    if (cursorInScrollRegion) {
        DLog(@"In scroll region");
        [self.currentGrid scrollRect:VT100GridRectMake(self.currentGrid.leftMargin, self.currentGrid.cursorY,
...
- (void)terminalScrollUp:(int)n {
    ...
    BOOL anySent = NO;
    for (int i = 0; i < MIN(self.currentGrid.size.height, n); i++) {
        BOOL sent = NO;
...
- (void)terminalAddNote:(NSString *)value show:(BOOL)show {
    ...
    if (parts.count == 1) {
        ...
        VT100GridCoord limit = {.x = self.width - 1, .y = self.height - 1};
    ...
    if (length > 0 && message.length > 0 && endVal <= maxVal) {
        PTYAnnotation *note = [[PTYAnnotation alloc] init];
        note.stringValue = message;
        [self addAnnotation:note
                    inRange:VT100GridCoordRangeMake(location.x, location.y + self.numberOfScrollbackLines, end.x,
...
- (void)terminalSetProfileProperty:(NSString *)kvps {
    NSArray<iTermTuple<NSString *, NSString *> *> *tuples =
        [[kvps componentsSeparatedByString:@";"] mapWithBlock:^id _Nullable(NSString *kvp) {
    ...
    [tuples enumerateObjectsUsingBlock:^(iTermTuple<NSString *, NSString *> *tuple,
                                         NSUInteger idx,
                                         BOOL *_Nonnull stop) {
...
- (void)terminalPromptDidStart:(BOOL)wasInCommand {
    DLog(@"begin");
    [self promptDidStartAt:VT100GridAbsCoordMake(self.currentGrid.cursor.x, self.currentGrid.cursor.y +
...
- (NSArray<NSNumber *> *)terminalTabStops {
    DLog(@"begin");
    return [[self sortedTabStops] mapWithBlock:^NSNumber *(NSNumber *ts) {
...
- (void)terminalReturnCodeOfLastCommandWas:(int)returnCode {
    DLog(@"begin");
    // BUG-12382: OSC 133;D marks the end of command execution. Assign the end date
    // to the command mark so it's properly considered "ended" by shell history tracking.
...
- (void)terminalSetShellIntegrationVersion:(NSString *)version {
    ...
    if (parts.count >= 2) {
        NSMutableDictionary *params = [[NSMutableDictionary alloc] initWithCapacity:parts.count - 1];
    ...
    if ([iTermPreferences boolForKey:kPreferenceKeyNotifyOnlyForCriticalShellIntegrationUpdates]) {
        latestVersionByShell = @{@"tcsh" : @2, @"bash" : @5, @"zsh" : @5, @"fish" : @5};
    } else {
...
- (int)terminalChecksumInRectangle:(VT100GridRect)rect {
    ...
    for (int y = rect.origin.y; y < rect.origin.y + rect.size.height; y++) {
        ...
        for (int x = rect.origin.x; x < rect.origin.x + rect.size.width && x < self.width; x++) {
            ...
            // BUG-7246: Fixed logic - character is private if it's >= BEGIN AND <= END
            // Previous code used impossible condition (< BEGIN && > END)
            BOOL isPrivate = (code >= ITERM2_PRIVATE_BEGIN && code <= ITERM2_PRIVATE_END);
            if (code && !isPrivate) {
...
- (NSArray<NSString *> *)terminalSGRCodesInRectangle:(VT100GridRect)screenRect {
    ...
    [self enumerateLinesInRange:NSMakeRange(rect.origin.y, rect.size.height)
                          block:^(int y, ScreenCharArray *sca, iTermImmutableMetadata metadata, BOOL *stop) {
...
- (void)terminalWillReceiveFileNamed:(NSString *)name ofSize:(NSInteger)size completion:(void (^)(BOOL ok))completion {
...
- (void)terminalWillReceiveInlineFileNamed:(NSString *)name
    ...
    DLog(@"begin name=%@ size=%@ width=%@ widthUnits=%@ height=%@ heightUnits=%@ preserveAR=%@ inset=%f,%f,%f,%f "
         @"type=%@",
         name, @(size), @(width), @(widthUnits), @(height), @(heightUnits), @(preserveAspectRatio), inset.top,
         inset.bottom, inset.left, inset.right, type);
    __weak __typeof(self) weakSelf = self;
    dispatch_queue_t queue = _queue;
    [self
        addUnmanagedPausedSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate,
        ...
        [self
            addUnmanagedPausedSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate,
                ...
                [delegate screenDidReceiveBase64FileData:data
                                                 confirm:^void(NSString *name, NSInteger lengthBefore,
...
- (void)terminalCustomEscapeSequenceWithParameters:(NSDictionary<NSString *, NSString *> *)parameters
    ...
    [self
        addSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
            DLog(@"begin side-effect");
            [delegate screenDidReceiveCustomEscapeSequenceWithParameters:parameters payload:payload];
        }
...
- (void)terminalRepeatPreviousCharacter:(int)times {
    ...
    if (self.lastCharacter.code) {
        ...
        for (int i = 0; i < times; i++) {
            [self
                appendScreenCharArrayAtCursor:chars
                                       length:length
...
- (void)terminalFillRectangle:(VT100GridRect)rect withCharacter:(unichar)inputChar {
    ...
    screen_char_t c = {.code = inputChar};
    if ([self lineDrawingModeEnabledForCharset:self.terminal.charset]) {
    ...
    [self fillRectangle:rect with:c externalAttributes:[self.terminal externalAttributes]];
}
...
- (void)terminalDidChangeSendModifiers {
    ...
    [self
        addJoinedSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
            DLog(@"begin side-effect");
...
- (void)terminalSelectiveEraseInLine:(int)mode {
    ...
    switch (mode) {
        case 0:
            [self selectiveEraseRange:VT100GridCoordRangeMake(self.currentGrid.cursorX, self.currentGrid.cursorY,
                                                              self.currentGrid.size.width, self.currentGrid.cursorY)
                      eraseAttributes:YES];
            return;
        case 1:
            [self selectiveEraseRange:VT100GridCoordRangeMake(0, self.currentGrid.cursorY, self.currentGrid.cursorX + 1,
                                                              self.currentGrid.cursorY)
                      eraseAttributes:YES];
            return;
        case 2:
            [self selectiveEraseRange:VT100GridCoordRangeMake(0, self.currentGrid.cursorY, self.currentGrid.size.width,
                                                              self.currentGrid.cursorY)
                      eraseAttributes:YES];
    }
}
...
- (void)terminalDidBeginSSHConductorCommandWithIdentifier:(NSString *)identifier depth:(int)depth {
    [self
        addSideEffect:^(id<VT100ScreenDelegate> _Nonnull delegate) {
            [delegate screenDidBeginSSHConductorCommandWithIdentifier:identifier depth:depth];
        }
...
- (void)terminalDidEndSSHConductorCommandWithIdentifier:(NSString *)identifier
    ...
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
            [delegate screenDidEndSSHConductorCommandWithIdentifier:identifier type:type status:status depth:depth];
...
- (void)terminalDidReadRawSSHData:(NSData *)data pid:(int)pid channel:(int)channel {
    if (channel != -1) {
...
- (void)terminalSendConductor:(NSString *)args {
    ...
    if (!v || [v integerValue] < 3) {
        ...
        _sshIntegrationFlags = nil;
        [self appendBannerMessage:@"Future version of it2ssh detected. Please upgrade DashTerm2."];
        return;
    }
    ...
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
            [delegate screenBeginSSHIntegrationWithToken:token uniqueID:uniqueID encodedBA:encodedBA sshArgs:sshArgs];
            [unpauser unpause];
        }
                       name:@"send conductor"];
}
...
- (void)terminalUpdateEnv:(NSString *)value {
    ...
    self.exfiltratedEnvironment =
        [[decodedPayload componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]]
            mapWithBlock:^id _Nullable(NSString *_Nonnull string) {
                return [string keyValuePair] ?: [iTermTuple tupleWithObject:string andObject:@""];
            }];
}
...
- (void)terminalEndSSH:(NSString *)uniqueID {
    ...
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
            ...
            NSString *banner = [weakSelf sshEndBannerTerminatingCount:count newLocation:[delegate screenSSHLocation]];
            dispatch_async(queue, ^{
    ...
    [self
        addPausedSideEffect:^(id<VT100ScreenDelegate> delegate, iTermTokenExecutorUnpauser *unpauser) {
            ...
            if (recovery) {
                ...
                weakSelf.terminal.framerBoundaryNumber =
                    [weakSelf.terminal.parser startConductorRecoveryModeWithID:recovery.dcsID tree:recovery.tree];
...
- (void)terminalBlock:(NSString *)blockID start:(BOOL)start type:(NSString *)type render:(BOOL)render {
    ...
    if (!start && [[_currentBlockIDList componentsSeparatedByString:iTermExternalAttributeBlockIDDelimiter]
                      containsObject:blockID]) {
...
- (void)terminalInsertCopyButtonForBlock:(NSString *)blockID {
    iTermButtonMark *mark =
        (iTermButtonMark *)[self addMarkOnLine:self.numberOfScrollbackLines + self.currentGrid.cursorY
                                        column:self.currentGrid.cursorX
                                       ofClass:[iTermButtonMark class]];
    [self.mutableIntervalTree mutateObject:mark
                                     block:^(id<IntervalTreeObject> _Nonnull obj) {
...
- (void)terminalStartWrappedCommand:(NSString *)command channel:(NSString *)uid {
    ...
    iTermButtonMark *mark =
        (iTermButtonMark *)[self addMarkOnLine:self.numberOfScrollbackLines + self.currentGrid.cursor.y
                                       ofClass:[iTermButtonMark class]];
    [self.mutableIntervalTree mutateObject:mark
                                     block:^(id obj) {
