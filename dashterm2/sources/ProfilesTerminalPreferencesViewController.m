...
- (void)awakeFromNib {
    ...
    info.onUpdate = ^BOOL() {
        ...
        // BUG-f1371: Convert assert to guard - weakInfo could be nil if info was deallocated
        if (!weakInfo) {
            return NO;
        }
    ...
    info.observer = ^() {
        ...
        [strongSelf->_xtermMouseReportingAllowClicksAndDrags
            setEnabled:[strongSelf boolForKey:KEY_XTERM_MOUSE_REPORTING]];
    };
    ...
    [self defineControl:_disablePrinting key:KEY_DISABLE_PRINTING relatedView:nil type:kPreferenceInfoTypeCheckbox];

    [self defineControl:_disableAltScreen key:KEY_DISABLE_SMCUP_RMCUP relatedView:nil type:kPreferenceInfoTypeCheckbox];
    ...
    [info addShouldBeEnabledDependencyOnSetting:KEY_DISABLE_WINDOW_RESIZING controller:self];
    ...
    [self defineControl:_silenceBell key:KEY_SILENCE_BELL relatedView:nil type:kPreferenceInfoTypeCheckbox];
    ...
    [self defineUnsearchableControl:_bellAlert key:KEY_SEND_BELL_ALERT type:kPreferenceInfoTypeCheckbox];
    [self defineUnsearchableControl:_idleAlert key:KEY_SEND_IDLE_ALERT type:kPreferenceInfoTypeCheckbox];
    [self defineUnsearchableControl:_newOutputAlert key:KEY_SEND_NEW_OUTPUT_ALERT type:kPreferenceInfoTypeCheckbox];
    ...
    [self defineControl:_flashingBell key:KEY_FLASHING_BELL relatedView:nil type:kPreferenceInfoTypeCheckbox];

    [self defineControl:_bellIconInTabs key:KEY_VISUAL_BELL relatedView:nil type:kPreferenceInfoTypeCheckbox];

    info = [self defineControl:_localeVars key:KEY_SET_LOCALE_VARS relatedView:nil type:kPreferenceInfoTypePopup];
    ...
    [self defineControl:_tmuxNewline key:KEY_TMUX_NEWLINE relatedView:nil type:kPreferenceInfoTypeCheckbox];
...
- (NSAttributedString *)attributedStringForLocale:(NSString *)lang {
    ...
    NSAttributedString *titleAS = [[NSAttributedString alloc]
...
- (void)updateCustomLocaleControls {
    ...
    switch ((iTermSetLocalVarsMode)[self unsignedIntegerForKey:KEY_SET_LOCALE_VARS]) {
        case iTermSetLocalVarsModeMinimal: {
            iTermLocaleGuesser *guesser =
                [[iTermLocaleGuesser alloc] initWithEncoding:[self unsignedIntegerForKey:KEY_CHARACTER_ENCODING]];
            ...
            if (ctype) {
                ...
                DLog(@"enc=%@ %@", @([self unsignedIntegerForKey:KEY_CHARACTER_ENCODING]),
                     [guesser dictionaryWithLC_CTYPE]);
        ...
        case iTermSetLocalVarsModeCustom: {
            ...
            if ([self stringForKey:KEY_CUSTOM_LOCALE].length) {
                _localeName.attributedStringValue =
                    [self attributedStringForLocale:[self stringForKey:KEY_CUSTOM_LOCALE]];
            } else {
...
- (NSArray *)sortedEncodings {
    ...
    // Pre-allocate for typical number of encodings (usually 50-100)
    NSMutableArray *encodings = [NSMutableArray arrayWithCapacity:64];
...
- (IBAction)showFilterAlertsPanel:(id)sender {
    __weak __typeof(self) weakSelf = self;
    [self.view.window beginSheet:_filterAlertsPanel
...
- (IBAction)changeCustomLocale:(id)sender {
    iTermLocalePrompt *prompt =
        [[iTermLocalePrompt alloc] initWithEncoding:[self unsignedIntegerForKey:KEY_CHARACTER_ENCODING]];
    ...
    if (locale && [self unsignedIntegerForKey:KEY_CHARACTER_ENCODING] == NSUTF8StringEncoding &&
        ![locale containsString:@"UTF-8"]) {
...
- (IBAction)shellIntegrationRequired:(id)sender {
    ...
    NSString *html = @"This feature requires shell integration to be installed. <a "
                     @"href=\"https://dashterm.com/documentation-shell-integration.html\">Learn more.</a>";
