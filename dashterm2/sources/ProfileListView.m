...
#define kProfileTableViewDataType @"com.dashterm.dashterm2.DashTerm2ProfileGuid"

// NSAttributedString attribute keys
static NSString *const iTermProfileListViewRestorableStateTagsVisible =
    @"iTermProfileListViewRestorableStateTagsVisible";
static NSString *const iTermProfileListViewRestorableStateTagsFraction =
    @"iTermProfileListViewRestorableStateTagsFraction";
...
- (instancetype)initWithFrame:(NSRect)frameRect
    ...
    if (self) {
        ...
        // Cache row heights for each profile
        _savedHeights = [[NSMutableDictionary alloc] initWithCapacity:32];
        ...
        // Split view ------------------------------------------------------------------------------
        NSRect splitViewFrame = NSMakeRect(0, 0, frame.size.width, frame.size.height - kSearchWidgetHeight - margin_);
        ...
        tableViewFrame.size = [NSScrollView contentSizeForFrameSize:scrollViewFrame.size
        ...
        tableColumn_ = [[NSTableColumn alloc] initWithIdentifier:@"name"];
        ...
        // Typically one or a few selected guids
        selectedGuids_ = [[NSMutableSet alloc] initWithCapacity:4];
...
// BUG-404: Cancel delayed selector in dealloc to prevent crash if view is
// deallocated while reloadData is pending (from dataChangeNotification:).
- (void)dealloc {
...
- (id<NSPasteboardWriting>)tableView:(NSTableView *)tableView pasteboardWriterForRow:(NSInteger)row {
    ...
    [pbItem setData:[self dataForRow:row] forType:kProfileTableViewDataType];
    return pbItem;
}

- (NSData *)dataForRow:(NSInteger)rowIndex {
    // Copy guid to pboard - typically just one guid per row
    NSMutableSet *guids = [[[NSMutableSet alloc] initWithCapacity:1] autorelease];
    ...
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:guids requiringSecureCoding:NO error:nil];
    return data;
}
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    [[self undoManager] registerUndoWithTarget:self selector:@selector(setRowOrder:) object:[self rowOrder]];

    // Typically a few items being dragged
    NSMutableSet<NSString *> *guids = [NSMutableSet setWithCapacity:4];
    [info
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *_Nonnull draggingItem, NSInteger idx, BOOL *_Nonnull stop) {
                                   ...
                                   NSKeyedUnarchiver *unarchiver =
                                       [[[NSKeyedUnarchiver alloc] initForReadingFromData:rowData
                                                                                    error:&error] autorelease];
                                   if (error) {
                                       return;
                                   }
                                   NSSet<NSString *> *itemGUIDs = [unarchiver
                                       decodeObjectOfClasses:[NSSet setWithArray:@[ [NSSet class], [NSString class] ]]
                                                      forKey:NSKeyedArchiveRootObjectKey];
                                   // BUG-1589: Check itemGUIDs (inner variable) not guids (outer variable)
                                   if (!itemGUIDs) {
                                       return;
                                   }
    ...
    NSMutableDictionary *map = [[[NSMutableDictionary alloc] initWithCapacity:guids.count] autorelease];
...
- (void)_addTag:(id)sender {
    ...
    NSArray *allTags =
        [[[dataSource_ underlyingModel] allTags] sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
    // BUG-394: Guard against out-of-bounds itemTag from sender.tag
    if (itemTag < 0 || (NSUInteger)itemTag >= allTags.count) {
        return;
    }
    ...
    [searchField_
        setStringValue:[searchStringPlusTag stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
...
- (void)_addTags:(NSArray *)tags toSearchField:(NSSearchField *)searchField {
    ...
    item = [[[NSMenuItem alloc] initWithTitle:@"Tags" action:nil keyEquivalent:@""] autorelease];
    [item setTarget:self];
    [item setTag:-1];
...
- (void)openHowToSearchHelp:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/search_syntax.html"]
                                       target:nil
...
- (NSArray *)orderedGuids {
    // One guid per row
    NSMutableArray *result = [[[NSMutableArray alloc] initWithCapacity:[tableView_ numberOfRows]] autorelease];
...
- (NSDictionary *)rowOrderWithSortDescriptors:(NSArray *)descriptors {
    // Row order contains descriptors and guids
    NSMutableDictionary *rowOrder = [NSMutableDictionary dictionaryWithCapacity:2];
    if (descriptors) {
...
- (void)tableView:(NSTableView *)aTableView sortDescriptorsDidChange:(NSArray *)oldDescriptors {
    ...
    if ([sortDescriptors count] > 0) {
        ...
        [aTableView setIndicatorImage:([primarySortDesc ascending] ? [NSImage imageNamed:@"NSAscendingSortIndicator"]
...
- (NSAttributedString *)attributedStringForName:(NSString *)name
    ...
    NSDictionary *highlightedNameAttributes = @{
        NSParagraphStyleAttributeName : paragraphStyle,
    ...
    NSDictionary *highlightedSmallAttributes = @{
        NSParagraphStyleAttributeName : paragraphStyle,
        NSBackgroundColorAttributeName : highlightedBackgroundColor,
        NSFontAttributeName : self.tagFont
    };
    ...
    if (isDynamic) {
        if (@available(macOS 12.0, *)) {
            ...
            NSAttributedString *space = [[[NSAttributedString alloc] initWithString:@" "
                                                                         attributes:plainAttributes] autorelease];
            NSMutableAttributedString *spaceWithIcon =
                [[[NSMutableAttributedString alloc] initWithAttributedString:space] autorelease];
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    if ([value isKindOfClass:[NSAttributedString class]]) {
        result = [[tableView newTableCellViewWithTextFieldUsingIdentifier:identifier
                                                         attributedString:value] autorelease];
        result.textField.toolTip = [value string];
    } else {
        result = [[tableView newTableCellViewWithTextFieldUsingIdentifier:identifier font:_font
                                                                   string:value] autorelease];
...
- (id)stringOrAttributedStringForColumn:(NSTableColumn *)aTableColumn
    ...
    *multilinePtr = 0;
    if (aTableColumn == tableColumn_) {
        DLog(@"Getting name of profile at row %d. The dictionary's address is %p. Its name is %@", (int)rowIndex,
             bookmark, bookmark[KEY_NAME]);
        ...
        NSArray *tags = [NSArray castFrom:bookmark[KEY_TAGS]];
        *multilinePtr = tags.count > 0;
        return [self attributedStringForName:bookmark[KEY_NAME] ?: @""
                                   ...
                                   isDefault:[bookmark[KEY_GUID] isEqualToString:defaultProfile[KEY_GUID]] ||
                                             [bookmark[KEY_GUID] isEqualToString:defaultBrowserProfile[KEY_GUID]]
                                   isBrowser:[bookmark[KEY_CUSTOM_COMMAND]
                                                 isEqualToString:kProfilePreferenceCommandTypeBrowserValue]
        ...
        NSString *key = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:bookmark];
        if ([key length]) {
            NSString *theString = [NSString stringWithFormat:@"^⌘%@", key];
...
- (NSArray *)orderedSelectedGuids {
    ...
    // One guid per selected row
    NSMutableArray *result = [[[NSMutableArray alloc] initWithCapacity:indexes.count] autorelease];
...
- (BOOL)control:(NSControl *)control textView:(NSTextView *)textView doCommandBySelector:(SEL)commandSelector {
    ...
    if (commandSelector == @selector(insertNewline:) && (self.numberOfRows == 1 || tableView_.selectedRow != -1) &&
...
- (void)turnOnDebug {
    DLog(@"Debugging object at %p. Current count is %d", (void *)self, (int)[self retainCount]);
    debug = YES;
}
...
- (NSDictionary *)restorableState {
    return @{
        iTermProfileListViewRestorableStateTagsVisible : @(self.tagsVisible),
...
- (void)restoreFromState:(NSDictionary *)state {
    ...
    // BUG-1585: Use logical && instead of bitwise & for boolean condition
...
- (void)profileTagsViewSelectionDidChange:(ProfileTagsView *)profileTagsView {
    searchField_.stringValue = [[profileTagsView.selectedTags mapWithBlock:^id(NSString *tag) {
