...
class CursorSmearView: NSView {
    ...
    required init?(coder: NSCoder) {
        // BUG-f833: Return nil instead of crashing for unused coder initializer
        DLog("CursorSmearView init(coder:) is not supported")
        return nil
    }
    ...
    private func setShapeLayer(_ layer: CAShapeLayer) {
        // BUG-9481: Remove animation before removing layer to ensure completion block fires.
        // When a layer is removed mid-animation, CATransaction completion may not fire.
        if let existingLayer = shapeLayer {
            existingLayer.removeAllAnimations()
