...
class PortholeMark: iTermMark, PortholeMarkReading {
    ...
    init(_ uniqueIdentifier: String) {
        ...
        // BUG-1705: Use guard with as? instead of as! for doppelganger cast
        if let mark = doppelganger() as? PortholeMarkReading {
            PortholeRegistry.instance.set(uniqueIdentifier, mark: mark)
        }
    }
        ...
        mutex.sync {
            _ = encoder.encodeChild(withKey: key,
                ...
                subencoder.encodeArray(withKey: "portholes",
                    ...
                    // BUG-f614: Add bounds check before accessing keys array
                    guard index >= 0 && index < keys.count else {
                        DLog("PortholeRegistry: Invalid index \(index) for keys count \(keys.count)")
                        return true
                    }
                    let key = keys[index]
                    // BUG-1634: Use optional binding instead of force unwrap for dictionary access
                    guard let porthole = portholes[key] else { return true }
                    adapter.merge(porthole.dictionaryValue)
                    return true
                }
                return true
            }
