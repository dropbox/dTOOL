...
class ExternalSearchResult: NSObject {
    ...
    // BUG-f580, BUG-f675: Log error and return false instead of crashing when subclass doesn't override isEqual
    @objc override func isEqual(_ object: Any?) -> Bool {
        DLog("BUG-f580: ExternalSearchResult subclass \(type(of: self)) must override isEqual(_:) - using default reference equality")
        // Fall back to reference equality if subclass didn't override
