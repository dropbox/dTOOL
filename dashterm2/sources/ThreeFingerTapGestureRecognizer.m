...
- (void)touchesEndedWithEvent:(NSEvent *)ev {
    ...
    if (numTouches_ == 0 &&
        ...
        // BUG-1021: Store strong reference to weak target before performSelector
        // to prevent target from being deallocated between check and call
        NSView *strongTarget = target_;
        if (strongTarget) {
            [strongTarget performSelector:selector_ withObject:ev];
            DLog(@"fired->YES");
            fired_ = YES;
        }
    }
