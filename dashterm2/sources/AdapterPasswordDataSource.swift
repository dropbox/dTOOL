...
class AdapterPasswordDataSource: CommandLinePasswordDataSource {
    enum AdapterError: LocalizedError {
        ...
        var reason: String? {
            ...
            case .incompatibleProtocol:
                return "Incompatible protocol. Please update DashTerm2."
            case .adapterNotFound:
...
@objc extension AdapterPasswordDataSource: PasswordManagerDataSource {
    ...
    // BUG-f501: Return an error instead of crashing when OTP toggling is not supported
    func toggleShouldSendOTP(context: RecipeExecutionContext,
                             account: PasswordManagerAccount,
                             completion: @escaping (PasswordManagerAccount?, Error?) -> ()) {
        completion(nil, NSError(domain: "com.dashterm2.adapter",
                               code: -1,
                               userInfo: [NSLocalizedDescriptionKey: "OTP toggling is not supported by this password manager adapter"]))
    }
