...
class AutoResizingTextView: NSTextView {
    ...
    func adjustFontSizes() {
        ...
        if originalAttributedString == nil {
            // BUG-1657: Use as? instead of force cast for copy()
            originalAttributedString = attributedString.copy() as? NSAttributedString
        }
        // BUG-1657: Use guard let instead of force unwrap
        guard let currentUserFont = userFont,
              let currentOriginalString = originalAttributedString else {
            return
        }
        DLog("attributedString is \(attributedString.string)")
        let maxFontSize = currentUserFont.pointSize
        ...
        while maxFontSize + delta >= minFontSize {
            // BUG-1657: Use guard with as? instead of force cast for mutableCopy()
            guard let newAttributedString = currentOriginalString.mutableCopy() as? NSMutableAttributedString else {
                break
            }
        ...
        // BUG-1657: Use guard with as? instead of force cast for copy()
        guard let copiedString = currentOriginalString.copy() as? NSAttributedString else {
            return
        }
        attributedString.setAttributedString(copiedString)
    ...
    func sizeThatFits(_ attributedString: NSAttributedString,
        ...
        // BUG-1731: Use as? with fallback to create NSMutableAttributedString from original
        let mutableAttrString = (attributedString.mutableCopy() as? NSMutableAttributedString) ?? NSMutableAttributedString(attributedString: attributedString)
        let textStorage = NSTextStorage(attributedString: mutableAttrString)
        textStorage.addLayoutManager(layoutManager)
        layoutManager.addTextContainer(textContainer)
        layoutManager.ensureLayout(for: textContainer)
        let size = layoutManager.usedRect(for: textContainer).size
        // BUG-1594: Use guard to safely find index instead of force unwrap
