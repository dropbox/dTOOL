...
extension TextViewPorthole: Porthole {
    ...
    func snippet(for result: ExternalSearchResult,
        ...
        // BUG-1729: Use as? with fallback to NSMutableAttributedString initializer
        let multiLineSubstring = (textStorage.attributedSubstring(from: expandedRange).mutableCopy() as? NSMutableAttributedString)
            ?? NSMutableAttributedString(attributedString: textStorage.attributedSubstring(from: expandedRange))
    ...
    static func config(fromDictionary dict: [String: AnyObject],
        ...
        if let url = dict[Self.baseDirectoryKey], let urlString = url as? String {
            // BUG-4112: Validate baseDirectory from state restoration
            // Only allow file:// URLs with absolute paths in user-accessible locations
            if let parsedURL = URL(string: urlString),
               parsedURL.isFileURL,
               parsedURL.path.hasPrefix("/") {
                let path = parsedURL.standardized.path
                let homeDir = NSHomeDirectory()
                let tempDir = FileManager.default.temporaryDirectory.path
                // Only allow paths under user home, /tmp, or per-user temp directories
                if path.hasPrefix(homeDir) || path.hasPrefix("/tmp/") || path.hasPrefix(tempDir) || path.hasPrefix("/var/folders/") {
                    baseDirectory = parsedURL.standardized
                } else {
                    baseDirectory = nil
                }
            } else {
...
extension NSAttributedString {
    ...
    func rewritingRelativeFileLinks(baseDirectory: URL) -> NSAttributedString {
        // BUG-1730: Use as? with fallback to NSMutableAttributedString initializer
        let result = (mutableCopy() as? NSMutableAttributedString) ?? NSMutableAttributedString(attributedString: self)
...
class TopRightAvoidingTextContainer: NSTextContainer {
    ...
    // BUG-f586: Use fallback initialization instead of crashing for coder init
    required init(coder: NSCoder) {
        DLog("TextViewPorthole.CutOutTextContainer init(coder:) is not supported - using fallback")
        self.cutOutSize = .zero
...
extension NSString {
    // BUG-3348: ReDoS protection timeout for user-provided search regex (0.5 seconds)
    private static let regexTimeoutSeconds: TimeInterval = 0.5
    ...
    func ranges(substring: String, mode: iTermFindMode) -> [NSRange] {
        ...
        @unknown default:
            // BUG-2896: Don't crash on unknown mode - use sensible defaults
            DLog("Unknown find mode \(mode), using case-insensitive substring search")
            regex = false
            caseSensitive = false
        }

        // BUG-3348: For regex searches, use enumerateMatches with timeout protection
        // to prevent ReDoS attacks from user-provided search patterns
        if regex {
            var options: NSRegularExpression.Options = [.anchorsMatchLines]
            if !caseSensitive {
                options.insert(.caseInsensitive)
            }
            guard let compiledRegex = try? NSRegularExpression(pattern: substring, options: options) else {
                return []
            }
            ...
            compiledRegex.enumerateMatches(
                in: self as String,
                ...
                if flags.contains(.progress) {
                    ...
                    if elapsed > NSString.regexTimeoutSeconds {
                        stop.pointee = true
                        timedOut = true
                        DLog("TextViewPorthole regex search timeout after \(elapsed)s for pattern: \(substring)")
                    }
                    return
                }
            ...
            if timedOut {
                DLog("TextViewPorthole regex search timed out, returning partial results (\(result.count) matches)")
            }

            return result
        }

        // Non-regex substring search (no timeout needed - built-in methods are safe)
        var start = 0
        let length = self.length
        var result = [NSRange]()
        while true {
            let currentRange = range(of: substring,
...
extension TextViewPorthole: ExternalSearchResultOwner {
    ...
    func select(_ result: ExternalSearchResult) {
        // BUG-1586: Use as? to avoid crash on type mismatch
        guard let searchResult = result as? SearchResult else {
            return
        }
    ...
    func searchResultIsVisible(_ result: ExternalSearchResult) -> Bool {
        ...
        // BUG-1602: Use guard to safely access layoutManager instead of force unwrap
    ...
    func remove(_ result: ExternalSearchResult) {
        // BUG-f600: Use guard with early return instead of it_fatalError for type mismatch
        // If result is wrong type, log and return - don't crash
        guard let myResult = result as? SearchResult else {
            DLog("BUG-f600: TextViewPorthole.remove() received non-SearchResult type: \(Swift.type(of: result))")
            return
        }
