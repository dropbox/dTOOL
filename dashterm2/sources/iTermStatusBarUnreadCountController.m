...
- (instancetype)init {
    self = [super init];
    if (self) {
        _sessionAndIdentifierToCount = [[NSMutableDictionary alloc] initWithCapacity:16];
        _identifierToCount = [[NSMutableDictionary alloc] initWithCapacity:16];
    }
    return self;
}

- (void)setUnreadCountForComponentWithIdentifier:(NSString *)identifier count:(NSInteger)count {
    _identifierToCount[identifier] = @(count);
    [[NSNotificationCenter defaultCenter] postNotificationName:iTermStatusBarUnreadCountDidChange object:identifier];
}
...
- (NSInteger)unreadCountForComponentWithIdentifier:(NSString *)identifier sessionID:(NSString *)sessionID {
