...
- (instancetype)initWithShortcut:(iTermShortcut *)shortcut {
    ...
    if (self) {
        ...
        _weakSessions = [[NSMutableArray alloc] initWithCapacity:4];  // Sessions with this hotkey
...
- (void)dealloc {
    // BUG-f967: Unregister hotkey if still registered instead of asserting
    if (_hotKey != nil) {
        DLog(@"BUG-f967: dealloc called with _hotKey still registered - unregistering now");
...
- (NSArray<iTermHotKey *> *)hotkeyPressed:(NSDictionary *)userData siblings:(NSArray *)siblings {
    NSUInteger index = [self.weakSessions
        indexOfObjectPassingTest:^BOOL(iTermWeakReference<id<iTermHotKeyNavigableSession>> *_Nonnull obj,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    // BUG-3120: Guard against modulo by zero when weakSessions array is empty
    if (index != NSNotFound && self.weakSessions.count > 0) {
        NSUInteger next = (index + 1) % self.weakSessions.count;
        session = [self.weakSessions[next] weaklyReferencedObject];
    } else {
        index = [self.weakSessions
            indexOfObjectPassingTest:^BOOL(iTermWeakReference<id<iTermHotKeyNavigableSession>> *_Nonnull obj,
                                           NSUInteger idx, BOOL *_Nonnull stop) {
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _registrations = [[NSMutableArray alloc] initWithCapacity:8];  // Typical hotkey registrations
    }
    return self;
}
...
- (iTermSessionHotkeyRegistration *)registrationForSession:(id<iTermHotKeyNavigableSession>)session
