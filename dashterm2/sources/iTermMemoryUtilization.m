...
- (long long)pageSize {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // BUG-3391: mach_host_self() returns a send right that must be deallocated
...
- (long long)memoryUsage {
    // BUG-3299: mach_host_self() returns a new send right each call that must be deallocated.
    ...
    // BUG-3299: Always deallocate the host port to prevent mach port leak
    mach_port_deallocate(mach_task_self(), host_port);
