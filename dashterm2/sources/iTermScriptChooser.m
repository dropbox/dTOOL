...
- (SIGIdentity *)selectedSigningIdentity {
    ...
    // BUG-1725: Validate array bounds before access
    NSInteger selectedTag = _identityButton.selectedTag;
    if (selectedTag < 0 || (NSUInteger)selectedTag >= _identities.count) {
        DLog(@"BUG-1725: selectedTag %ld out of bounds for identities count %lu",
             (long)selectedTag, (unsigned long)_identities.count);
        return nil;
    }
    return _identities[selectedTag];
}
