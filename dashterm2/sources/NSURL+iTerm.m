...
+ (nullable NSURL *)urlByReplacingFormatSpecifier:(NSString *)formatSpecifier
    ...
    NSString *urlString = [string stringByReplacingOccurrencesOfString:formatSpecifier withString:placeholder];
    NSURLComponents *components = [[NSURLComponents alloc] initWithString:urlString];

    // Query item value?
    {
        NSMutableArray<NSURLQueryItem *> *queryItems = [components.queryItems mutableCopy];
        const NSUInteger i = [queryItems
            indexOfObjectPassingTest:^BOOL(NSURLQueryItem *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
+ (NSURL *)URLWithUserSuppliedStringImpl:(NSString *)string {
    ...
    NSMutableIndexSet *nonReservedSymbolIndices =
        [[string indicesOfCharactersInSet:[reservedSymbols invertedSet]] mutableCopy];
    ...
    if (rangeOfFirstOpenSquareBracket.location != NSNotFound) {
        [[string indicesOfCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@":"]]
    ...
    NSMutableDictionary<NSString *, NSString *> *map = [[NSMutableDictionary alloc] initWithCapacity:16];
    NSMutableDictionary<NSString *, NSString *> *originalValuesMap = [[NSMutableDictionary alloc] initWithCapacity:16];
    [nonReservedSymbolIndices
        enumerateRangesWithOptions:NSEnumerationReverse
        ...
        if (components.port) {
            ...
            if (rangeOfLastColon.location == NSNotFound) {
                ...
                if ([possiblePort isNumeric]) {
                    ...
                    components.host =
                        [NSURL IDNEncodedHostname:[compositeString substringToIndex:rangeOfLastColon.location]];
                } else {
                    components.host = [NSURL IDNEncodedHostname:compositeString];
                }
            }
    ...
    NSMutableDictionary<NSString *, NSURLQueryItem *> *uuidToQueryItem =
        [[NSMutableDictionary alloc] initWithCapacity:components.queryItems.count];
    ...
    [uuidToQueryItem enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull uuid, NSURLQueryItem *_Nonnull queryItem,
                                                         BOOL *_Nonnull stop) {
        NSString *queryParam;
    ...
    [urlString replaceOccurrencesOfString:semicolonUUID
                               withString:@";"
                                  options:0
                                    range:NSMakeRange(0, urlString.length)];
...
- (nullable NSData *)zippedContents {
    ...
    NSURL *destination = [[NSURL fileURLWithPath:NSTemporaryDirectory()]
        URLByAppendingPathComponent:[uuid stringByAppendingString:@".zip"]];
...
- (BOOL)saveContentsOfPathToZip:(NSURL *)destination {
    ...
    if (isDir) {
        ...
        NSError *error;
        [fileManager createDirectoryAtURL:sourceDir withIntermediateDirectories:YES attributes:nil error:&error];
        if (error) {
            return NO;
        }
