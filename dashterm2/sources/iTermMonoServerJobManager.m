...
- (void)setChildPid:(pid_t)childPid {
    // BUG-3271: Replace assert(NO) with DLog - setChildPid should never be called
    // on mono server job manager because childPid is read-only and always returns -1.
    // This method exists to satisfy protocol conformance but calling it is an error.
    DLog(@"iTermMonoServerJobManager: setChildPid:%d called but is not supported - childPid is read-only", childPid);
}
...
- (NSString *)pathToNewUnixDomainSocket {
    ...
    NSString *tempPath = [[NSWorkspace sharedWorkspace] temporaryFileNameWithPrefix:@"DashTerm2-temp-socket."
                                                                             suffix:@""];
    return tempPath;
}
...
- (void)forkAndExecWithTtyState:(iTermTTYState)ttyState
    ...
    void (^wrapper)(iTermJobManagerForkAndExecStatus, NSNumber *) =
...
- (void)queueForkAndExecWithTtyState:(iTermTTYState)ttyState
    ...
    self.fd = iTermForkAndExecToRunJobInServer(&forkState, &ttyState, unixDomainSocketPath, argpath.UTF8String, cArgv,
    ...
    [self queueDidForkParent:&forkState ttyState:ttyState task:task completion:completion];
}
...
- (void)queueDidForkParent:(const iTermForkState *)forkStatePtr
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(self.queue, ^{
            iTermJobManagerForkAndExecStatus status = [self queueDidCompleteHandshakeWithForkState:forkState
                                                                                          ttyState:ttyState
...
- (iTermJobManagerForkAndExecStatus)queueDidCompleteHandshakeWithForkState:(iTermForkState)state
    ...
    if (serverConnection.ok) {
        // We intentionally leave connectionFd open. If DashTerm2 stops unexpectedly then its closure
...
- (void)queueAttachToServer:(iTermGeneralServerConnection)serverConnection task:(id<iTermTask>)task {
    // BUG-f1055: Replace assert with guard - wrong connection type should return early, not crash
    if (serverConnection.type != iTermGeneralServerConnectionTypeMono) {
        ELog(@"queueAttachToServer called with non-mono connection type: %lu", (unsigned long)serverConnection.type);
        return;
    }
    // BUG-f1056: Replace assert with guard - servers not enabled should return early, not crash
    if (![iTermAdvancedSettingsModel runJobsInServers]) {
        ELog(@"queueAttachToServer called but runJobsInServers is disabled");
        return;
    }
...
- (void)attachToServer:(iTermGeneralServerConnection)serverConnection
    ...
    completion([self attachToServer:serverConnection withProcessID:pidNumber task:task]);
}
...
- (iTermJobManagerAttachResults)attachToServer:(iTermGeneralServerConnection)serverConnection
    ...
    const iTermJobManagerAttachResults results =
        iTermJobManagerAttachResultsAttached | iTermJobManagerAttachResultsRegistered;
    dispatch_sync(self.queue, ^{
...
// Sends a signal to the server. This breaks it out of accept()ing forever when DashTerm2 quits.
- (void)killServerIfRunning {
    ...
    dispatch_sync(self.queue, ^{
        ...
        // Don't want to leak these. They exist to let the server know when DashTerm2 crashes, but if
