...
+ (void)setArrangement:(NSArray *)arrangement withName:(NSString *)name {
    ...
    NSMutableDictionary *arrangements =
        [NSMutableDictionary dictionaryWithDictionary:[WindowArrangements arrangements]];
    ...
    @try {
        ...
        ITCriticalError(NO, @"Exception %@ while saving arrangement. Invalid path is %@\n%@", e, oops,
                        [arrangement debugDescription]);
        return;
    }
...
// BUG-3245: Add bounds check to prevent array index out of bounds crash
- (NSString *)nameAtIndex:(NSUInteger)rowIndex {
    NSArray *keys = [WindowArrangements allNames];
    if (rowIndex >= keys.count) {
        return nil;
    }
...
+ (void)showAlertWithText:(NSString *)prompt
    ...
    if (button == okButton) {
        ...
        [iTermSavePanel
            asyncShowWithOptions:kSavePanelOptionDefaultToLocalhost
                      identifier:@"NoSyncExportWindowArrangements"
                initialDirectory:NSHomeDirectory()
                 defaultFilename:[[input stringValue] stringByAppendingPathExtension:@"dashterm2arrangement"]
                allowedFileTypes:@[ @"dashterm2arrangement" ]
                          window:nil
...
+ (NSString *)nameForNewArrangement {
    ...
    NSArray<NSString *> *templates =
        @[ @"MM d, yyyy", @"MM d, yyyy, j:mm", @"MM d, yyyy, j:mm:ss", @"MM d, yyyy, j:mm:ss.SSS" ];
    ...
    NSString *msFormat = [NSDateFormatter dateFormatFromTemplate:@"EEEE, MMMM d, yyyy, j:mm:ss.SSS"
                                                         options:0
                                                          locale:locale];
    ...
    // BUG-1226: Add safety guard to prevent infinite loop if arrangements are somehow corrupted.
    // 10000 is a reasonable upper bound - no user would have this many arrangements with the
    // exact same millisecond timestamp.
    static const NSInteger kMaxAttempts = 10000;
    for (NSInteger i = 2; i < kMaxAttempts; i++) {
    ...
    // If we exhausted attempts, use a UUID-based fallback (guaranteed unique).
    return [NSString stringWithFormat:@"Arrangement %@", [[NSUUID UUID] UUIDString]];
}
                               ...
                               completion:^(NSString *name, iTermSavePanelItem *saveItem) {
                                   ...
                                   if ([WindowArrangements hasWindowArrangement:name]) {
                                       ...
                                       alert.informativeText =
                                           @"There is an existing saved window arrangement with this name. Would you "
                                           @"like to replace it with the current arrangement?";
...
+ (void)selectNameAndWhetherToIncludeContentsWithCompletion:(void (^)(NSString *name,
                                                                      iTermSavePanelItem *saveItem))completion {
...
- (void)controlTextDidEndEditing:(NSNotification *)obj {
    ...
    NSDictionary *existingArrangements = [[NSUserDefaults standardUserDefaults] objectForKey:WINDOW_ARRANGEMENTS];
    if (!existingArrangements) {
        return;
    }
    NSMutableDictionary *dict = [existingArrangements mutableCopy];
    NSDictionary *value = [dict[oldName] copy];
    if (dict[newName]) {
        NSAlert *alert = [[NSAlert alloc] init];
        alert.messageText = @"Replace Arrangement?";
        alert.informativeText = [NSString
            stringWithFormat:@"An arrangement named “%@” already exists. Would you like to replace it?", newName];
        [alert addButtonWithTitle:@"OK"];
        [alert addButtonWithTitle:@"Cancel"];
    ...
    if (index != NSNotFound) {
        [tableView_ selectRowIndexes:[NSIndexSet indexSetWithIndex:index] byExtendingSelection:NO];
    }
}
...
- (void)setArrangements:(NSDictionary *)saved {
    ...
    // BUG-1194: Handle NSNull stored when default was nil during undo registration
    id defaultValue = saved[@"default"];
    NSString *defaultArrangementName = nil;
    if (defaultValue && ![defaultValue isKindOfClass:[NSNull class]]) {
        defaultArrangementName = [NSString castFrom:defaultValue];
    }
    ...
    [[self undoManager]
        registerUndoWithTarget:self
                      selector:@selector(setArrangements:)
                        object:@{
                            @"default" : [WindowArrangements defaultArrangementName] ?: [NSNull null],
