...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    self = [super init];
    if (self) {
        NSString *className = [aDecoder decodeObjectOfClass:[NSString class] forKey:@"class"] ?: @"";
...
- (id<iTermStatusBarComponent>)newComponentWithKnobs:(NSDictionary *)knobs
    ...
    return [[_class alloc] initWithConfiguration:@{
        ...
        iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue :
            advancedConfiguration.dictionaryValue
    }
                                           scope:scope];
}
...
- (instancetype)initWithConfiguration:(NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)configuration
    ...
    if (self) {
        ...
        _advancedConfiguration = [iTermStatusBarAdvancedConfiguration
            advancedConfigurationFromDictionary:
                configuration[iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue]];
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSDictionary<iTermStatusBarComponentConfigurationKey, id> *configuration =
        [aDecoder decodeObjectOfClass:[NSDictionary class] forKey:@"configuration"];
    if (!configuration) {
        return nil;
    }
    return [self initWithConfiguration:configuration scope:nil];
}

- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p priority=%@>", NSStringFromClass([self class]), self,
                                      @(self.statusBarComponentPriority)];
}
...
+ (NSString *)statusBarComponentIdentifier {
    return [NSString stringWithFormat:@"com.dashterm.dashterm2.%@", NSStringFromClass(self.class)];
}
...
+ (NSDictionary *)defaultMinMaxWidthKnobValues {
    return @{iTermStatusBarMaximumWidthKey : @(INFINITY), iTermStatusBarMinimumWidthKey : @0};
}
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
    ...
    if ([self statusBarComponentCanStretch]) {
        ...
        compressionResistanceKnob = [[iTermStatusBarComponentKnob alloc]
            initWithLabelText:title
                         type:iTermStatusBarComponentKnobTypeDouble
                  placeholder:@""
...
+ (NSDictionary *)statusBarComponentDefaultKnobs {
    return @{
        iTermStatusBarCompressionResistanceKey : @1,
...
- (void)statusBarComponentSetKnobValues:(NSDictionary *)knobValues {
    ...
    NSDictionary *replacement =
        [_configuration dictionaryBySettingObject:knobValues forKey:iTermStatusBarComponentConfigurationKeyKnobValues];
    NSMutableSet<NSString *> *updatedKeys = [NSMutableSet setWithCapacity:keys.count];
    ...
    [self.delegate statusBarComponentKnobsDidChange:self updatedKeys:updatedKeys];
}
...
- (double)statusBarComponentPriority {
    NSNumber *number =
        self.configuration[iTermStatusBarComponentConfigurationKeyKnobValues][iTermStatusBarPriorityKey] ?: @5;
    if (number) {
...
- (CGFloat)statusBarComponentSpringConstant {
    NSNumber *value =
        self.configuration[iTermStatusBarComponentConfigurationKeyKnobValues][iTermStatusBarCompressionResistanceKey]
            ?: @1;
...
- (CGFloat)statusBarComponentMaximumWidth {
    NSNumber *value =
        self.configuration[iTermStatusBarComponentConfigurationKeyKnobValues][iTermStatusBarMaximumWidthKey]
            ?: @(INFINITY);
