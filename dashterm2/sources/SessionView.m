...
@interface SessionView () <iTermAnnouncementDelegate, iTermFindDriverDelegate, iTermGenericStatusBarContainer,
                           iTermLegacyViewDelegate, iTermSearchResultsMinimapViewDelegate, NSDraggingSource,
                           PTYScrollerDelegate, SplitSelectionViewDelegate>
...
@implementation SessionView {
    ...
    BOOL _useMetal;
    BOOL _usingDTermCoreRenderer;  // True when using DTermMetalView instead of iTermMTKView
    NSView *_dtermMetalView;  // dterm-core GPU renderer (when dtermCoreRendererEnabled). Type is DTermMetalView, loaded dynamically.
    NSView *_fpsOverlay;  // Development FPS overlay (when DtermCoreFPSOverlayEnabled). Type is DTermFPSOverlay, loaded dynamically.
...
- (instancetype)initWithFrame:(NSRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self registerForDraggedTypes:@[ iTermMovePaneDragType, @"com.dashterm.dashterm2.psm.controlitem" ]];
        lastResizeDate_ = [NSDate date];
        _announcements = [[NSMutableArray alloc] initWithCapacity:4];  // Session announcements
        ...
        _scrollview = [[PTYScrollView alloc] initWithFrame:NSMakeRect(0, 0, aRect.size.width, aRect.size.height)
        ...
        if ([iTermAdvancedSettingsModel showLocationsInScrollbar]) {
            ...
            iTermTuple<NSColor *, NSColor *> * (^tuple)(NSColor *) =
                ^iTermTuple<NSColor *, NSColor *> *(NSColor *color) {
                NSColor *saturated = [NSColor colorWithHue:color.hueComponent saturation:1 brightness:1 alpha:1];
            ...
            NSArray<iTermTuple<NSColor *, NSColor *> *> *colors = @[
                ...
                [iTermTuple tupleWithObject:[iTermTextDrawingHelper otherMarkColor]
                                  andObject:[[iTermTextDrawingHelper otherMarkColor] colorDimmedBy:0.2
                                                                                  towardsGrayLevel:1]],
...
- (void)setBrowserViewController:(iTermBrowserViewController *)browserViewController
    ...
    NSRect initialFrame = NSMakeRect(0, reservedSpaceOnBottom, self.frame.size.width,
                                     self.frame.size.height - titleHeight - reservedSpaceOnBottom);
    _browserViewController.view.frame = initialFrame;

    // This magic incantation prevents auto layout from virally eating everything in the window preventing it from
    // resizing. I am so unbelievably lucky that this works. It seems like the only real fix is to make *everything* use
    // auto layout and I would sooner die.
    __weak __typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        __typeof(self) strongSelf = weakSelf;
        [strongSelf finishInstallingBrowserWithInitialURL:initialURL restorableState:restorableState];
    });
}

- (void)finishInstallingBrowserWithInitialURL:(NSString *)initialURL restorableState:(NSDictionary *)restorableState {
...
- (void)setTerminalViewsHidden:(BOOL)hidden {
    ...
    if (_dtermMetalView) {
        _dtermMetalView.hidden = hidden;
    }
    if (_legacyView) {
        // When DTermMetalView is active, legacy view must stay hidden
        // to avoid double-rendering corruption
        _legacyView.hidden = hidden || _usingDTermCoreRenderer;
    }
...
- (void)setTerminalBackgroundColor:(NSColor *)color {
    ...
    CGFloat activeMetalViewAlpha = _usingDTermCoreRenderer ? _dtermMetalView.alphaValue : _metalView.alphaValue;
    if (color && activeMetalViewAlpha < 1) {
        DLog(@"setTerminalBackgroundColor: Set background color view hidden=%@ because metalview is not opaque",
        ...
        DLog(@"setTerminalBackgroundColor: Set background color view hidden=YES because bg color (%@) is nil or "
             @"metalView.alphaValue (%@) == 1",
             color, @(activeMetalViewAlpha));
...
- (void)setTransparencyAlpha:(CGFloat)transparencyAlpha blend:(CGFloat)blend {
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
...
- (void)dealloc {
    ...
    // Note: DTermMetalView doesn't have a delegate to clear
}
...
- (iTermDropDownFindViewController *)newDropDownFindView {
    ...
    iTermDropDownFindViewController *dropDownViewController =
        [[iTermDropDownFindViewController alloc] initWithNibName:nibName bundle:[NSBundle bundleForClass:self.class]];
...
- (void)updateDropDownFrame:(iTermDropDownFindViewController *)dropDownViewController {
    ...
    const NSPoint origin = NSMakePoint(aRect.size.width - size.width - 30, aRect.size.height - size.height);
...
- (BOOL)findViewHasKeyboardFocus {
    ...
    // BUG-491: Replace assert(false) with ELog - unknown find driver type should not crash
    ELog(@"SessionView canInstantiateFindDriver: unknown _findDriverType=%d", (int)_findDriverType);
    return YES;
}
...
- (BOOL)findViewIsHidden {
    ...
    // BUG-492: Replace assert(false) with ELog - unknown find driver type should not crash
    ELog(@"SessionView findViewIsHidden: unknown _findDriverType=%d", (int)_findDriverType);
    return YES;
}
...
- (iTermFindDriver *)findDriver {
    ...
    // BUG-493: Replace assert(false) with ELog - unknown find driver type should not crash
    ELog(@"SessionView findDriver: unknown _findDriverType=%d", (int)_findDriverType);
    return nil;
}
    ...
    switch (_findDriverType) {
        ...
        case iTermSessionViewFindDriverTemporaryStatusBar:
            if (!_temporaryStatusBarFindDriver) {
                [self loadTemporaryStatusBarFindDriverWithStatusBarViewController:
                          [self.delegate sessionViewStatusBarViewController]];
            }
    }
...
- (void)loadTemporaryStatusBarFindDriverWithStatusBarViewController:
    ...
    NSDictionary *knobs =
        @{iTermStatusBarPriorityKey : @(INFINITY), iTermStatusBarSearchComponentIsTemporaryKey : @YES};
    ...
    _temporaryStatusBarFindDriver =
        [[iTermFindDriver alloc] initWithViewController:component.statusBarComponentSearchViewController
...
- (iTermStatusBarFilterComponent *)temporaryFilterComponent {
    ...
    NSDictionary *knobs =
        @{iTermStatusBarPriorityKey : @(INFINITY), iTermStatusBarFilterComponent.isTemporaryKey : @YES};
    NSDictionary *configuration = @{
        ...
        iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue :
            self.delegate.sessionViewStatusBarAdvancedConfigurationDictionary
    };
...
- (void)showFilter {
    ...
    if (statusBarViewController) {
        iTermStatusBarFilterComponent *filterComponent =
            (iTermStatusBarFilterComponent *)[statusBarViewController.visibleComponents
                objectPassingTest:^BOOL(id<iTermStatusBarComponent> candidate, NSUInteger index, BOOL *stop) {
    ...
    switch (self.findDriverType) {
        ...
        case iTermSessionViewFindDriverPermanentStatusBar: {
            ...
            _permanentStatusBarFindDriver =
                [[iTermFindDriver alloc] initWithViewController:statusBarViewController.searchViewController
                                           filterViewController:statusBarViewController.filterViewController];
        ...
        case iTermSessionViewFindDriverTemporaryStatusBar:
            ...
            _temporaryStatusBarFindDriver =
                [[iTermFindDriver alloc] initWithViewController:statusBarViewController.temporaryLeftComponent
...
- (void)showFindUI {
    ...
    if (_findDriverType == iTermSessionViewFindDriverPermanentStatusBar) {
        ...
        // BUG-f1303: Replace asserts with guards - these can be nil in edge cases
        // Return early if we don't have the required components
        if (!statusBarViewController) {
            DLog(@"BUG-f1303: statusBarViewController is nil, cannot create find driver");
            return;
        }
        if (!statusBarViewController.temporaryLeftComponent) {
            DLog(@"BUG-f1303: temporaryLeftComponent is nil, loading it first");
            [self loadTemporaryStatusBarFindDriverWithStatusBarViewController:statusBarViewController];
            if (!statusBarViewController.temporaryLeftComponent) {
                DLog(@"BUG-f1303: Failed to load temporaryLeftComponent");
                return;
            }
        }
        _temporaryStatusBarFindDriver = [[iTermFindDriver alloc]
            initWithViewController:statusBarViewController.temporaryLeftComponent.statusBarComponentSearchViewController
              filterViewController:statusBarViewController.filterViewController];
        _temporaryStatusBarFindDriver.delegate = _dropDownFindDriver.delegate;
        [_temporaryStatusBarFindDriver open];
    }
...
- (void)setUseMetal:(BOOL)useMetal dataSource:(id<iTermMetalDriverDataSource>)dataSource NS_AVAILABLE_MAC(10_11) {
    if (useMetal != _useMetal) {
        ...
        // Legacy view should be hidden when:
        // - Metal is disabled (we're using legacy drawing)
        // - In browser mode (no terminal rendering)
        // - Using DTermMetalView (replaces both legacy and iTermMetalView)
        _legacyView.hidden = !useMetal || self.isBrowser || _usingDTermCoreRenderer;
...
- (void)installLegacyView {
    // BUG-f1304: Replace assert with guard - handle double-install gracefully
    // This can happen in edge cases during view reconfiguration
    if (_legacyView) {
        DLog(@"BUG-f1304: Legacy view already installed, skipping");
        return;
    }
...
- (void)sanityCheckSubviewOrder {
    NSInteger l = [self.subviews
        indexOfObjectPassingTest:^BOOL(__kindof NSView *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            return obj == _legacyView;
        }];
    NSInteger s = [self.subviews
        indexOfObjectPassingTest:^BOOL(__kindof NSView *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            return obj == _scrollview;
        }];
    if (l != NSNotFound && s != NSNotFound && l > s) {
        NSString *message =
            [NSString stringWithFormat:@"Wrong subview order.\n%@\n%@", [self subviews], [NSThread callStackSymbols]];
#if BETA
...
- (void)installMetalViewWithDataSource:(id<iTermMetalDriverDataSource>)dataSource NS_AVAILABLE_MAC(10_11) {
    ...
    // Check if we should use dterm-core renderer
    Class dtermMetalViewClass = NSClassFromString(@"DTermMetalView");
    BOOL dtermCoreRendererEnabled = [iTermAdvancedSettingsModel dtermCoreRendererEnabled];
    DLog(@"dtermCoreRendererEnabled=%d, dtermMetalViewClass=%@", dtermCoreRendererEnabled, dtermMetalViewClass);
    if (dtermCoreRendererEnabled && dtermMetalViewClass != nil) {
        // DTermMetalView.isAvailable is a Swift static Bool property exposed via @objc.
        // We need to use NSInvocation to call it properly since performSelector: doesn't
        // handle BOOL return values correctly.
        SEL isAvailableSel = NSSelectorFromString(@"isAvailable");
        if ([dtermMetalViewClass respondsToSelector:isAvailableSel]) {
            NSMethodSignature *sig = [dtermMetalViewClass methodSignatureForSelector:isAvailableSel];
            if (sig && strcmp(sig.methodReturnType, @encode(BOOL)) == 0) {
                NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
                [inv setSelector:isAvailableSel];
                [inv setTarget:dtermMetalViewClass];
                [inv invoke];
                BOOL isAvailable = NO;
                [inv getReturnValue:&isAvailable];
                DLog(@"DTermMetalView.isAvailable = %d", isAvailable);
                if (isAvailable) {
                    DLog(@"Installing DTermMetalView (hybrid renderer)");
                    if ([self installDTermMetalView]) {
                        // Successfully installed DTermMetalView
                        return;
                    }
                    // Installation failed - fall through to legacy renderer
                    DLog(@"DTermMetalView installation failed, falling back to legacy renderer");
                }
            }
        }
    }

    // Fall back to legacy iTermMTKView
    [self installLegacyMetalViewWithDataSource:dataSource];
}

/// Install the dterm-core GPU renderer (DTermMetalView).
/// Uses safe Rust frame synchronization instead of dispatch_group.
/// Returns YES if installation succeeded, NO if we should fall back to legacy.
- (BOOL)installDTermMetalView {
    DLog(@"Installing DTermMetalView (dterm-core GPU renderer)");
    _usingDTermCoreRenderer = YES;

    // CRITICAL: Hide legacy view IMMEDIATELY to prevent double-rendering.
    // Must be done before any code that might trigger metalViewVisibilityDidChange.
    _legacyView.hidden = YES;

    // Allocate new DTermMetalView using simple frame-based init.
    // DTermMetalView is a Swift class with initWithFrame: that takes CGRect.
    Class dtermMetalViewClass = NSClassFromString(@"DTermMetalView");
    NSRect frame = _scrollview.contentView.frame;
    _dtermMetalView = [[dtermMetalViewClass alloc] initWithFrame:frame];

    if (!_dtermMetalView) {
        DLog(@"Failed to create DTermMetalView - falling back to legacy renderer");
        _usingDTermCoreRenderer = NO;
        _legacyView.hidden = NO;  // Restore legacy view on failure
        return NO;
    }

    // Configure the DTermMetalView with terminal state from the delegate
    // This validates that we have a working integration and font
    if (![self configureDTermMetalView]) {
        // Configuration failed - clean up and indicate we should fall back
        DLog(@"DTermMetalView configuration failed - will fall back to legacy renderer");
        [_dtermMetalView removeFromSuperview];
        _dtermMetalView = nil;
        _usingDTermCoreRenderer = NO;
        _legacyView.hidden = NO;  // Restore legacy view on failure
        return NO;
    ...
    if (iTermTextIsMonochrome()) {
        _dtermMetalView.layer.opaque = NO;
    } else {
        _dtermMetalView.layer.opaque = YES;
    }
#else
    _dtermMetalView.layer.opaque = YES;
#endif

    // Note: DTermMetalView doesn't need colorspace setting - it uses the layer's default

    // Tell the clip view about it (using the metalView property since DTermMetalView is an NSView)
    // For now, metalClipView won't work with DTermMetalView since it expects iTermMTKView
    // TODO: Update iTermMetalClipView to support DTermMetalView

    // Image view and background color view go under it.
    [self insertSubview:_dtermMetalView atIndex:_contentViewIndex];

    // Configure DTermMetalView. Unlike legacy iTermMTKView, DTermMetalView starts unpaused
    // and uses its own display link for vsync. We just control visibility via alphaValue.
    // Start rendering immediately - it renders to an invisible (alpha=0) surface.
    [_dtermMetalView setValue:@NO forKey:@"paused"];
    // In browser mode, keep metal view hidden
    _dtermMetalView.hidden = self.isBrowser;
    // Start at full alpha since DTermMetalView handles its own first-frame logic
    _dtermMetalView.alphaValue = 1.0;

    // DTermMetalView handles its own rendering via dterm-core - no iTermMetalDriver needed
    _driver = nil;

    // Install FPS overlay if enabled (development feature)
    [self installFPSOverlayIfEnabled];
...
/// Install the FPS overlay for development monitoring.
/// Enabled via: defaults write com.dashterm.dashterm2 DtermCoreFPSOverlayEnabled -bool YES
- (void)installFPSOverlayIfEnabled {
    if (!_usingDTermCoreRenderer || !_dtermMetalView) {
        return;
    }

    // Check if FPS overlay is enabled via user defaults
    BOOL fpsOverlayEnabled = [[NSUserDefaults standardUserDefaults] boolForKey:@"DtermCoreFPSOverlayEnabled"];
    if (!fpsOverlayEnabled) {
        return;
    }

    // Load DTermFPSOverlay class dynamically
    Class fpsOverlayClass = NSClassFromString(@"DTermFPSOverlay");
    if (!fpsOverlayClass) {
        DLog(@"DTermFPSOverlay class not found");
        return;
    }

    // Create the overlay using the factory method
    SEL createSelector = NSSelectorFromString(@"createOverlayFor:");
    if (![fpsOverlayClass respondsToSelector:createSelector]) {
        DLog(@"DTermFPSOverlay.createOverlay(for:) not found");
        return;
    }

    NSMethodSignature *sig = [fpsOverlayClass methodSignatureForSelector:createSelector];
    NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
    [inv setSelector:createSelector];
    [inv setTarget:fpsOverlayClass];
    NSView *metalView = _dtermMetalView;
    [inv setArgument:&metalView atIndex:2];
    [inv invoke];

    __unsafe_unretained NSView *overlay = nil;
    [inv getReturnValue:&overlay];
    _fpsOverlay = overlay;

    if (_fpsOverlay) {
        _fpsOverlay.frame = self.bounds;
        _fpsOverlay.autoresizingMask = NSViewWidthSizable | NSViewHeightSizable;
        [self addSubview:_fpsOverlay positioned:NSWindowAbove relativeTo:_dtermMetalView];
        DLog(@"FPS overlay installed");
    }
}

/// Configure the DTermMetalView with terminal state from the delegate.
/// Called after creating the view and when terminal state changes.
/// Returns YES if configuration succeeded, NO if it failed and we should fall back.
- (BOOL)configureDTermMetalView {
    if (!_dtermMetalView || !_usingDTermCoreRenderer) {
        return NO;
    }

    // Get DTermCoreIntegration from delegate (optional method)
    if ([_delegate respondsToSelector:@selector(sessionViewDTermCoreIntegration)]) {
        id integration = [_delegate sessionViewDTermCoreIntegration];
        if (integration) {
            SEL setIntegrationSelector = NSSelectorFromString(@"setIntegration:");
            if ([_dtermMetalView respondsToSelector:setIntegrationSelector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [_dtermMetalView performSelector:setIntegrationSelector withObject:integration];
#pragma clang diagnostic pop
                DLog(@"DTermMetalView: Connected to DTermCoreIntegration");
            }
        } else {
            // No integration available - can't render without terminal state
            DLog(@"DTermMetalView configuration failed: no DTermCoreIntegration available");
            return NO;
        }
    }

    // Get font from delegate (optional method) - REQUIRED for GPU renderer
    if ([_delegate respondsToSelector:@selector(sessionViewFont)]) {
        NSFont *font = [_delegate sessionViewFont];
        if (font) {
            SEL setFontSelector = NSSelectorFromString(@"setFont:");
            if ([_dtermMetalView respondsToSelector:setFontSelector]) {
                // Use NSInvocation to properly get the BOOL return value
                NSMethodSignature *sig = [_dtermMetalView methodSignatureForSelector:setFontSelector];
                if (sig && strcmp(sig.methodReturnType, @encode(BOOL)) == 0) {
                    NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
                    [inv setSelector:setFontSelector];
                    [inv setTarget:_dtermMetalView];
                    [inv setArgument:&font atIndex:2];  // Index 0=self, 1=_cmd, 2=first arg
                    [inv invoke];
                    BOOL fontSetSucceeded = NO;
                    [inv getReturnValue:&fontSetSucceeded];

                    if (!fontSetSucceeded) {
                        // Font setup failed (likely system font without file URL)
                        DLog(@"DTermMetalView font setup failed for %@ - falling back to legacy renderer", font.fontName);
                        return NO;
                    }
                    DLog(@"DTermMetalView: Set font to %@", font.fontName);
                } else {
                    // Fallback if signature doesn't match expected type
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                    [_dtermMetalView performSelector:setFontSelector withObject:font];
#pragma clang diagnostic pop
                    DLog(@"DTermMetalView: Set font to %@ (unchecked)", font.fontName);
                }
            }
        } else {
            // No font available - can't render text
            DLog(@"DTermMetalView configuration failed: no font available");
            return NO;
        }
    }
...
/// Install the legacy iTermMTKView renderer.
- (void)installLegacyMetalViewWithDataSource:(id<iTermMetalDriverDataSource>)dataSource {
    DLog(@"Installing iTermMTKView (legacy renderer)");
    _usingDTermCoreRenderer = NO;

    // Allocate a new metal view
    _metalView = [[iTermMTKView alloc] initWithFrame:_scrollview.contentView.frame device:[self metalDevice]];
...
- (void)removeMetalView NS_AVAILABLE_MAC(10_11) {
    if (_usingDTermCoreRenderer) {
        // Remove FPS overlay if present
        if (_fpsOverlay) {
            // Call detach method via selector to clean up timer
            SEL detachSelector = NSSelectorFromString(@"detach");
            if ([_fpsOverlay respondsToSelector:detachSelector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [_fpsOverlay performSelector:detachSelector];
#pragma clang diagnostic pop
            }
            [_fpsOverlay removeFromSuperview];
            _fpsOverlay = nil;
        }
        // Remove DTermMetalView
        [_dtermMetalView removeFromSuperview];
        _dtermMetalView = nil;
    } else {
        // Remove legacy iTermMTKView
        _metalView.delegate = nil;
...
- (void)setMetalViewNeedsDisplayInTextViewRect:(NSRect)textViewRect NS_AVAILABLE_MAC(10_11) {
    if (_useMetal) {
        ...
        if (_usingDTermCoreRenderer) {
            [_dtermMetalView setNeedsDisplay:YES];
        } else {
...
- (void)metalViewVisibilityDidChange {
    // ASSERTION: Prevent double-rendering bug.
    // If DTermMetalView is active and visible, legacy view MUST be hidden.
    // This caught a critical bug where both renderers were drawing simultaneously.
    if (_usingDTermCoreRenderer && _dtermMetalView && _dtermMetalView.alphaValue > 0 && !_dtermMetalView.hidden) {
        NSAssert(_legacyView.hidden, @"DOUBLE-RENDERING BUG: Legacy view must be hidden when DTermMetalView is active. "
                 "Both renderers are drawing, causing ghost text and corrupted graphics.");
    }
    [self updateImageAndBackgroundViewVisibility];
}
...
- (void)updateImageAndBackgroundViewVisibility {
    ...
    [CATransaction setDisableActions:YES];
    CGFloat activeMetalViewAlpha = _usingDTermCoreRenderer ? _dtermMetalView.alphaValue : _metalView.alphaValue;
    if (activeMetalViewAlpha == 0) {
        _imageView.hidden = (_imageView.image == nil);
        DLog(
            @"updateImageAndBackgroundViewVisibility: set backgroundColorView.hidden=%@ because metalView.alphaValue=0",
            @(!iTermTextIsMonochrome()));
        _backgroundColorView.hidden = !iTermTextIsMonochrome();
        _legacyScrollerBackgroundView.hidden = iTermTextIsMonochrome();
    } else {
        _imageView.hidden = YES;
        DLog(@"updateImageAndBackgroundViewVisibility: Set backgroundColorView.hidden=YES because metalView.alphaValue "
             @"(%@) != 0",
             @(activeMetalViewAlpha));
        _backgroundColorView.hidden = YES;
        _legacyScrollerBackgroundView.hidden = YES;
    }
    [self setNeedsDisplay:YES];
...
- (NSColor *)it_backgroundColorOfEnclosingTerminalIfBackgroundColorViewHidden {
    if (_backgroundColorView.isHidden) {
        return
            [_backgroundColorView.backgroundColor colorWithAlphaComponent:[self.delegate sessionViewTransparencyAlpha]];
    }
    return nil;
}
...
- (void)addSubviewBelowFindView:(NSView *)aView {
    ...
    if ([aView isKindOfClass:[PTYScrollView class]]) {
        NSIndexSet *indexes = [self.subviews
            indexesOfObjectsPassingTest:^BOOL(__kindof NSView *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                return [obj isKindOfClass:[iTermMTKView class]] ||
                       (dtermMetalViewClass != nil && [obj isKindOfClass:dtermMetalViewClass]) ||
                       obj == _legacyView;
            }];
        if (indexes.count) {
    ...
    if ([aView isKindOfClass:[iTermMTKView class]] || (dtermMetalViewClass != nil && [aView isKindOfClass:dtermMetalViewClass])) {
        NSInteger i = [self.subviews
            indexOfObjectPassingTest:^BOOL(__kindof NSView *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (NSRect)frameForLegacyScroller {
    ...
    return [_scrollview.verticalScroller convertRect:_scrollview.verticalScroller.bounds toView:self];
}
...
- (void)updateLayout {
    ...
    if ([_delegate sessionViewShouldUpdateSubviewsFramesAutomatically]) {
        ...
        if (self.showTitle) {
            ...
            [_title setFrame:NSMakeRect(0, maxY, _title.frame.size.width, _title.frame.size.height)];
        ...
        if (_showBottomStatusBar) {
            _genericStatusBarContainer.frame =
                NSMakeRect(0, 0, self.frame.size.width, _genericStatusBarContainer.frame.size.height);
        }
    ...
    _legacyScrollerBackgroundView.frame = [self frameForLegacyScroller];
    _progressBar.frame = NSMakeRect(0, self.bounds.size.height - _progressBar.desiredHeight, self.bounds.size.width,
    ...
    if (_hoverURLView) {
        ...
        NSArray<NSValue *> *proposedFrames =
            [self framesForURLPreviewWithPadding:NSMakeSize(horizontalPadding, verticalPadding)];
        NSValue *bestFrameValue = [proposedFrames maxWithBlock:^NSComparisonResult(NSValue *obj1, NSValue *obj2) {
            ...
            DLog(@"SessionView: while searching for best frame, comparing %@ (area=%@) to %@ (area=%@)",
                 NSStringFromRect(lhs), @(leftArea), NSStringFromRect(rhs), @(rightArea));
...
- (NSArray<NSValue *> *)framesForURLPreviewWithPadding:(NSSize)padding {
    ...
    DLog(@"SessionView: Proposing frames with minX=%@, minY=%@, maxX=%@, maxY=%@, width=%@, height=%@", @(minX),
         @(minY), @(maxX), @(maxY), @(frame.size.width), @(frame.size.height));
    return @[
...
- (void)reallyUpdateMetalViewFrame {
    if (_usingDTermCoreRenderer) {
        _dtermMetalView.frame = self.bounds;
        // DTermMetalView handles resize internally via setFrameSize:
    } else {
...
- (void)getDesiredTrackingRectFrames:(iTermTrackingAreaSpec *)specs {
    ...
    const iTermTrackingAreaSpec value[SessionViewNumberOfTrackingAreas] = {
        {.rect = self.bounds, .options = trackingOptions},
...
- (SessionViewTrackingMode)desiredTrackingMode {
    ...
    const NSRect firstResponderRect = [firstResponderView convertRect:firstResponderView.bounds toView:nil];
    const BOOL mouseInFirstResponder = NSPointInRect(mouseLocation, firstResponderRect);
    NSView *hitTest = [self.window.contentView hitTest:[self.window.contentView convertPoint:mouseLocation
                                                                                    fromView:nil]];
...
- (void)mouseDown:(NSEvent *)event {
    ...
    DLog(@"Point in screen coords=%@, point in window coords=%@, point in session view=%@", NSStringFromPoint(p),
         NSStringFromPoint(windowRect.origin), NSStringFromPoint(pointInSessionView));
...
- (void)setFrameSize:(NSSize)frameSize {
    ...
    if (frameSize.width < 340) {
        [findView setFrameSize:NSMakeSize(MAX(150, frameSize.width - 50), [findView frame].size.height)];
    } else {
        [findView setFrameSize:NSMakeSize(290, [findView frame].size.height)];
    }
    [self updateFindViewFrame];
}
...
- (void)createSplitSelectionViewWithMode:(SplitSelectionViewMode)mode session:(id)session {
    ...
    _splitSelectionView =
        [[SplitSelectionView alloc] initWithMode:mode
                                       withFrame:NSMakeRect(0, 0, [self frame].size.width, [self frame].size.height)
                                         session:session
                                        delegate:delegate];
...
- (NSColor *)backgroundColorForDecorativeSubviews {
    return [[iTermTheme sharedInstance]
        backgroundColorForDecorativeSubviewsInSessionWithTabColor:self.tabColor
...
- (void)createSplitSelectionView {
    ...
    _splitSelectionView =
        [[SplitSelectionView alloc] initWithFrame:NSMakeRect(0, 0, frame.size.width, frame.size.height)];
    _splitSelectionView.wantsLayer = [iTermPreferences boolForKey:kPreferenceKeyUseMetal];
    [self addSubviewBelowFindView:_splitSelectionView];
...
- (void)setSuppressLegacyDrawing:(BOOL)suppressLegacyDrawing {
    // In browser mode or when using DTermMetalView, always keep legacy view hidden
    // DTermMetalView replaces the legacy renderer entirely
    _legacyView.hidden = suppressLegacyDrawing || self.isBrowser || _usingDTermCoreRenderer;
}
...
- (BOOL)setShowTitle:(BOOL)value adjustScrollView:(BOOL)adjustScrollView {
    ...
    if (_showTitle) {
        ...
        _title = [[SessionTitleView alloc]
            initWithFrame:NSMakeRect(0, self.frame.size.height - iTermGetSessionViewTitleHeight(),
                                     self.frame.size.width, iTermGetSessionViewTitleHeight())];
...
- (BOOL)setShowBottomStatusBar:(BOOL)value adjustScrollView:(BOOL)adjustScrollView {
    ...
    if (_showBottomStatusBar) {
        ...
        _genericStatusBarContainer = [[iTermGenericStatusBarContainer alloc]
            initWithFrame:NSMakeRect(0, 0, self.frame.size.width, iTermGetStatusBarHeight())];
...
- (void)updateFindDriver {
    iTermStatusBarViewController *statusBarViewController = [self.delegate sessionViewStatusBarViewController];
    DLog(@"updateFindDriver: statusBarVC=%@, searchVC=%@, filterVC=%@, temporaryLeft=%@", statusBarViewController,
         statusBarViewController.searchViewController, statusBarViewController.filterViewController,
    ...
    if (statusBarViewController.searchViewController && statusBarViewController.temporaryLeftComponent == nil) {
        ...
        DLog(@"updateFindDriver: permanent status bar type. existing driver=%@, needsRecreate=%@",
             _permanentStatusBarFindDriver, @(needsRecreate));
        if (needsRecreate) {
            DLog(@"updateFindDriver: Creating new permanent driver (old=%@, mode=%@)", _permanentStatusBarFindDriver,
                 @(_permanentStatusBarFindDriver.filterMode));
            _permanentStatusBarFindDriver =
                [[iTermFindDriver alloc] initWithViewController:statusBarViewController.searchViewController
                                           filterViewController:statusBarViewController.filterViewController];
            _permanentStatusBarFindDriver.delegate = self.findDriverDelegate;
            DLog(@"updateFindDriver: Created new permanent driver %@ with mode=%@", _permanentStatusBarFindDriver,
                 @(_permanentStatusBarFindDriver.filterMode));
        } else {
            DLog(@"updateFindDriver: Keeping existing permanent driver %@ with mode=%@", _permanentStatusBarFindDriver,
                 @(_permanentStatusBarFindDriver.filterMode));
        }
...
- (NSSize)compactFrame {
    ...
    NSSize innerSize =
        NSMakeSize(cellSize.width * dim.width + [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
    ...
    NSSize size = [PTYScrollView frameSizeForContentSize:innerSize
                                 horizontalScrollerClass:nil
                                   verticalScrollerClass:(hasScrollbar ? [PTYScroller class] : nil)borderType:NSNoBorder
...
- (void)updateTitleFrame {
    ...
    NSRect aRect = [self frame];
    if (_showTitle) {
        [_title setFrame:NSMakeRect(0, aRect.size.height - iTermGetSessionViewTitleHeight(), aRect.size.width,
...
- (void)updateBottomStatusBarFrame {
    NSRect aRect = [self frame];
    if (_showBottomStatusBar) {
        _genericStatusBarContainer.frame = NSMakeRect(0, 0, aRect.size.width, iTermGetStatusBarHeight());
...
- (void)updateScrollViewFrame {
    ...
    CGFloat reservedSpaceOnBottom = _showBottomStatusBar ? iTermGetStatusBarHeight() : 0;
    NSSize proposedSize =
        NSMakeSize(self.frame.size.width, self.frame.size.height - titleHeight - reservedSpaceOnBottom);
    NSSize size = [_delegate sessionViewScrollViewWillResize:proposedSize];
    NSRect rect = NSMakeRect(0, reservedSpaceOnBottom + proposedSize.height - size.height, size.width, size.height);
    DLog(@"titleHeight=%@ bottomStatusBarHeight=%@ proposedSize=%@ size=%@ rect=%@", @(titleHeight),
         @(reservedSpaceOnBottom), NSStringFromSize(proposedSize), NSStringFromSize(size), NSStringFromRect(rect));
...
- (void)updateBrowserViewFrame {
    ...
    NSRect browserFrame = NSMakeRect(0, reservedSpaceOnBottom, self.frame.size.width,
                                     self.frame.size.height - titleHeight - reservedSpaceOnBottom);
...
- (void)updateMinimapFrameAnimated:(BOOL)animated {
    ...
    NSRect frame = [self convertRect:_scrollview.verticalScroller.bounds fromView:_scrollview.verticalScroller];
        ...
        [NSView animateWithDuration:5.0 / 60.0
                         animations:^{
                             [[NSAnimationContext currentContext]
                                 setTimingFunction:[CAMediaTimingFunction functionWithName:@"easeOut"]];
...
- (void)setShowsAILockIndicator:(BOOL)showsAILockIndicator {
    _title.showsAILockIndicator = showsAILockIndicator;
}

- (NSString *)description {
    return [NSString stringWithFormat:@"<%@:%p frame:%@ size:%@>", [self class], self,
                                      [NSValue valueWithRect:[self frame]],
                                      VT100GridSizeDescription([_delegate sessionViewGridSize])];
}
...
- (void)showNextAnnouncement {
    ...
    if (_announcements.count) {
        ...
        // Animate in
        NSRect finalRect = NSMakeRect(0, self.frame.size.height - _currentAnnouncement.view.frame.size.height,
                                      self.frame.size.width, _currentAnnouncement.view.frame.size.height);
...
- (void)didSelectDestinationSession:(PTYSession *)session half:(SplitSessionHalf)half {
    [[NSNotificationCenter defaultCenter] postNotificationName:SessionViewWasSelectedForInspectionNotification
                                                        object:self];
}
