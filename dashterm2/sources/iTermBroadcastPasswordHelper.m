...
// BUG-1520: Use dispatch queue for thread-safe access to static array
static NSMutableArray<iTermBroadcastPasswordHelper *> *sBroadcastPasswordHelpers;
static dispatch_queue_t sBroadcastPasswordHelpersQueue;

+ (void)tryToSendPassword:(NSString *)password
               toSessions:(NSArray<PTYSession *> *)sessions
               completion:(NSArray<PTYSession *> *_Nonnull (^)(NSArray<PTYSession *> *_Nonnull,
                                                               NSArray<PTYSession *> *_Nonnull))completion {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sBroadcastPasswordHelpers = [NSMutableArray arrayWithCapacity:4]; // Active password helpers
        sBroadcastPasswordHelpersQueue =
            dispatch_queue_create("com.dashterm2.BroadcastPasswordHelpers", DISPATCH_QUEUE_SERIAL);
    });
    iTermBroadcastPasswordHelper *helper = [[self alloc] initWithPassword:password
                                                                 sessions:sessions
                                                               completion:completion];
    if (helper) {
        dispatch_sync(sBroadcastPasswordHelpersQueue, ^{
...
- (instancetype)initWithPassword:(NSString *)password
                      ...
                      completion:
                          (NSArray<PTYSession *> * (^)(NSArray<PTYSession *> *, NSArray<PTYSession *> *))completion {
    if (sessions.count == 0) {
        return nil;
    }
    ...
    if (self) {
        ...
        _failures = [NSMutableArray arrayWithCapacity:sessions.count];
        _successes = [NSMutableArray arrayWithCapacity:sessions.count];
...
- (void)cleanup {
    ...
    _cleaningUp = YES;
    [_proxies
        enumerateObjectsUsingBlock:^(iTermEchoProbeDelegateProxy *_Nonnull proxy, NSUInteger idx, BOOL *_Nonnull stop) {
            PTYSession *session = proxy.session;
            if (session) {
    ...
    _completion = nil;
    // BUG-1520: Use synchronized access to static array
    dispatch_sync(sBroadcastPasswordHelpersQueue, ^{
...
- (void)echoProbeDidFail:(iTermEchoProbe *)echoProbe {
    ...
    if (session) {
        ...
        // BUG-1515: Only call resetEchoProbe when session is non-nil
