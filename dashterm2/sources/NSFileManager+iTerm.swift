        ...
        mutating func next() -> URL? {
            while let fileURL = enumerator.nextObject() as? URL {
                do {
                    ...
                    // BUG-2663: Log the error instead of silently ignoring
                    // This can happen if file is deleted mid-enumeration or permissions denied
                    DLog("Failed to get resource values for \(fileURL.path): \(error.localizedDescription)")
                    // Continue to next file - we can't return this one but shouldn't stop iteration
                }
            }
