...
- (void)prependSuggestions:(NSArray<NSString *> *)suggestions maxCount:(NSUInteger)maxCount {
    ...
    for (NSInteger index = (NSInteger)suggestions.count - 1; index >= 0; index--) {
        [_comboBox insertItemWithObjectValue:suggestions[(NSUInteger)index] atIndex:0];
    }
    while (_comboBox.numberOfItems > (NSInteger)maxCount) {
        [_comboBox removeItemAtIndex:_comboBox.numberOfItems - 1];
    }
}
...
- (NSRect)cursorFrameInScreenCoordinates {
    NSTextView *const textEditor =
        [_comboBox.currentEditor isKindOfClass:[NSTextView class]] ? (NSTextView *)_comboBox.currentEditor : nil;
    if (!textEditor) {
...
- (void)drawWithFrame:(NSRect)originalFrame inView:(NSView *)controlView {
    if (PSMShouldExtendTransparencyIntoMinimalTabBar()) {
        if (@available(macOS 10.16, *)) {
            ...
            // BUG-421: Replace assert(NO) with legacy drawing fallback
            // PSMShouldExtendTransparencyIntoMinimalTabBar should only return YES on 10.16+
            // but if the system state is inconsistent, fall back to legacy drawing instead of crashing
            DLog(@"iTermComposerComboBoxCell: PSMShouldExtendTransparencyIntoMinimalTabBar returned YES on pre-10.16");
...
- (void)drawModernWithFrame:(NSRect)originalFrame inView:(NSView *)controlView NS_AVAILABLE_MAC(10_16) {
    ...
    NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:cellFrame xRadius:4 yRadius:4];
    ...
    path = [NSBezierPath bezierPathWithRoundedRect:cellFrame xRadius:4 yRadius:4];
    [path setLineWidth:0.5];
    if ([controlView.effectiveAppearance it_isDark]) {
...

    {
        ...
        dispatch_once(&onceToken, ^{
            lightImage = [NSImage imageWithSystemSymbolName:SFSymbolGetString(SFSymbolChevronDown)
        ...
        const NSSize size = [self sizeWithAspectRatio:image.size.width / image.size.height height:imageHeight];
        const CGFloat topOffset = (NSHeight(cellFrame) - imageHeight) / 2.0;
        const CGFloat rightMargin = 1;
        NSRect buttonRect = NSMakeRect(NSMaxX(cellFrame) - size.width - rightMargin, NSMinY(cellFrame) + topOffset,
...
- (void)reallyUpdateKeyboardClipView:(NSTextField *)textField {
    NSView *keyboardClipView =
        [self.controlView.subviews objectPassingTest:^BOOL(__kindof NSView *element, NSUInteger index, BOOL *stop) {
