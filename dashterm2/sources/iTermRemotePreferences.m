...
respectingTimeoutSetting:(BOOL)respectingTimeoutSetting
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:req completionHandler:^(NSData * _Nullable taskData,
        ...
        if (taskError) {
            ...
            // BUG-1046: Validate HTTP status code - treat non-2xx responses as errors
            NSHTTPURLResponse *httpResponse = [taskResponse isKindOfClass:[NSHTTPURLResponse class]] ? (NSHTTPURLResponse *)taskResponse : nil;
            NSInteger statusCode = httpResponse.statusCode;
            if (httpResponse && (statusCode < 200 || statusCode >= 300)) {
                error = [NSError errorWithDomain:NSURLErrorDomain
                                            code:NSURLErrorBadServerResponse
                                        userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Server returned HTTP %ld", (long)statusCode]}];
            } else {
                data = taskData;
            }
        }
    ...
    // Add a maximum wait time as a safety net, even though the NSURLRequest has its own timeout.
    // Use 5 minutes (300 seconds) as an absolute maximum - this covers cases where:
    // 1. User set an extremely long timeout in settings
    // 2. "Try Again Without Timeout" was selected (timeout = INFINITY)
    // This prevents indefinite blocking while still allowing reasonable download times.
    static const NSTimeInterval kMaxWaitSeconds = 300.0;
    dispatch_time_t maxWait = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kMaxWaitSeconds * NSEC_PER_SEC));
    long waitResult = dispatch_semaphore_wait(sema, maxWait);
    if (waitResult != 0) {
        DLog(@"Network download safety timeout after %.0f seconds", kMaxWaitSeconds);
        [task cancel];
        if (errorPtr && !*errorPtr) {
            *errorPtr = [NSError errorWithDomain:NSURLErrorDomain
                                            code:NSURLErrorTimedOut
                                        userInfo:@{NSLocalizedDescriptionKey: @"Download timed out (safety limit reached)"}];
        }
        return nil;
    }
...
- (void)saveLocalUserDefaultsToRemotePrefs
{
    ...
    if ([self remotePrefsHaveChanged]) {
        NSString *theTitle =
            [NSString stringWithFormat:@"Settings at %@ changed since DashTerm2 started. "
                                       @"Overwrite it?",
                                       [self customFolderOrURL]];
    ...
    if ([folder stringIsUrlLike]) {
        NSString *informativeText =
            @"To make it available, first quit DashTerm2 and then manually "
            @"copy ~/Library/Preferences/com.dashterm.dashterm2.plist to "
...
// BUG-1140: This method is main-thread-only. The flag check/set is safe because:
// 1. All callers are on the main queue (UI callbacks, notification observers)
// 2. The check and set happen synchronously before dispatch_after
// 3. Multiple dispatch_after blocks coalesce via the _needsSave flag in saveIfNeeded
// If threading becomes a concern, use @synchronized or atomic property.
- (void)setNeedsSave {
    if (_needsSave) {
        return;
    }
...
- (void)saveIfNeeded {
    ...
    if (!_queue) {
        _queue = dispatch_queue_create("com.dashterm.dashterm2.save-prefs", DISPATCH_QUEUE_SERIAL);
    }
    NSString *filename = [self prefsFilenameWithBaseDir:folder];
