...
@interface ProfilesGeneralPreferencesViewController () <iTermImageWellDelegate, iTermShortcutInputViewDelegate,
                                                        NSMenuDelegate, NSTabViewDelegate, ProfileListViewDelegate,
                                                        NSMenuItemValidation, NSControlTextEditingDelegate>
@end
...
- (void)awakeFromNib {
    ...
    info.customSettingChangedHandler = ^(id sender) {
        ...
        // BUG-f1347: Replace assert() with guard - missing info should not crash
        if (!weakInfo) {
            ELog(@"ProfilesGeneralPreferencesViewController: weakInfo is nil in customSettingChangedHandler");
            return;
        }
    ...
    _profileNameFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
    ...
    _profileNameFieldForEditCurrentSessionDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    info = [self defineControl:_icon key:KEY_ICON displayName:@"Profile icon" type:kPreferenceInfoTypePopup];
    info.onChange = ^{
    ...
    [self defineControl:_profileShortcut
        ...
        update:^BOOL {
            [weakSelf updateShortcutTitles];
            return YES;
        }];

    [self defineControl:_tagsTokenField key:KEY_TAGS displayName:@"Profile tags" type:kPreferenceInfoTypeTokenField];
    ...
    _commandDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    _sendTextAtStartDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    [self defineControl:_initialDirectoryType
        ...
        update:^BOOL {
            [weakSelf updateDirectoryType];
            return YES;
        }];

    // Remove paths that need the session fully initialized.
    NSSet<NSString *> *exclusions = [NSSet setWithArray:@[
        iTermVariableKeySessionTTY, iTermVariableKeySessionUsername, iTermVariableKeySessionHostname,
        iTermVariableKeySessionName, iTermVariableKeySessionJob, iTermVariableKeySessionProcessTitle,
        iTermVariableKeySessionPath, iTermVariableKeySessionJobPid
    ]];
    _customDirectoryTextFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession
    ...
    [self addViewToSearchIndex:_editBadgeButton
                   displayName:@"Edit badge appearance"
                       phrases:@[ @"Badge font", @"Badge minum and maximum width", @"Badge right and top margins" ]
                           key:nil];
    [self defineControl:_badgeText
                    key:KEY_BADGE_FORMAT
            displayName:@"Profile badge"
                   type:kPreferenceInfoTypeStringTextField];
    _badgeTextFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    [self defineUnsearchableControl:_badgeTextForEditCurrentSession
                                key:KEY_BADGE_FORMAT
                               type:kPreferenceInfoTypeStringTextField];
    _badgeTextForEditCurrentSessionFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    _tabTitleTextFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextTab]
               passthrough:_tabTitle.delegate
             functionsOnly:NO];
    _tabTitle.delegate = _tabTitleTextFieldDelegate;

    _windowTitleTextFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextWindow]
    ...
    [self defineControl:_subtitleText
            ...
            relatedView:_subtitleLabel
                   type:kPreferenceInfoTypeStringTextField];
    _subtitleTextDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:[iTermVariableHistory pathSourceForContext:iTermVariablesSuggestionContextSession]
    ...
    [_commandWarningImageView
        addGestureRecognizer:[[NSClickGestureRecognizer alloc] initWithTarget:self
                                                                       action:@selector(handleCommandWarningClick:)]];
    [self commitControls];
}
...
- (void)updateSubtitlesAllowed {
    ...
    if (@available(macOS 26, *)) {
        ...
        subtitlesAllowed =
            ((iTermPreferencesTabStyle)[iTermPreferences intForKey:kPreferenceKeyTabStyle] == TAB_STYLE_MINIMAL ||
             [iTermAdvancedSettingsModel defaultTabBarHeight] >= 28);
    }
...
- (BOOL)onUpdateTitle {
    ...
    if ([_profileNameFieldForEditCurrentSession textFieldIsFirstResponder] &&
        _profileNameFieldForEditCurrentSession.window.isKeyWindow) {
...
- (void)openFilePicker {
    ...
    void (^completion)(NSInteger) = ^(NSInteger result) {
        if (result == NSModalResponseOK) {
            // BUG-f1043: Guard against empty URLs array
            if ([panel URLs].count == 0) {
                DLog(@"BUG-f1043: NSOpenPanel returned OK but URLs array is empty");
                return;
            }
...
- (void)updateImageWellHidden {
    _imageWell.hidden = ([self unsignedIntegerForKey:KEY_ICON] != iTermProfileIconCustom || _imageWell.image == nil);
}
...
- (void)reloadProfile {
    ...
    _sessionHotkeyInputView.shortcut =
        [iTermShortcut shortcutWithDictionary:(NSDictionary *)[self objectForKey:KEY_SESSION_HOTKEY]];
...
- (void)ensureSessionNameVisible {
    ...
    if (components & iTermTitleComponentsProfileName) {
        ...
        [self setUnsignedInteger:updated forKey:KEY_TITLE_COMPONENTS];
        return;
    }
    [self setUnsignedInteger:(components | iTermTitleComponentsSessionName) forKey:KEY_TITLE_COMPONENTS];
}
...
- (void)updateTitleSettingsMenuForView:(NSPopUpButton *)titleSettings {
    ...
    NSIndexSet *indexSet = [titleSettings.menu.itemArray
        indexesOfObjectsPassingTest:^BOOL(NSMenuItem *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            return obj.tag == -1;
        }];
...
- (void)updateEnabledState {
    ...
    _customDirectory.enabled =
        ([[self stringForKey:KEY_CUSTOM_DIRECTORY] isEqualToString:kProfilePreferenceInitialDirectoryCustomValue]);
    NSString *reason;
...
- (IBAction)configureSSH:(id)sender {
    _sshConfigurationWindowController =
        [[iTermSSHConfigurationWindowController alloc] initWithWindowNibName:@"SSHConfigurationWindow"];
    __weak typeof(self) weakSelf = self;
    [_sshConfigurationWindowController load:[NSDictionary castFrom:[self objectForKey:KEY_SSH_CONFIG]]];
    [self.view.window beginSheet:_sshConfigurationWindowController.window
...
- (IBAction)configureBadge:(id)sender {
    iTermBadgeConfigurationWindowController *badgeConfigurationWindowController =
        [[iTermBadgeConfigurationWindowController alloc]
            initWithProfile:[self.delegate profilePreferencesCurrentProfile]];
    [badgeConfigurationWindowController window]; // force the window to load
    __weak typeof(self) weakSelf = self;
    [self.view.window
               beginSheet:badgeConfigurationWindowController.window
        completionHandler:^(NSModalResponse returnCode) {
            __strong __typeof(weakSelf) strongSelf = self;
            if (!strongSelf) {
                return;
...
- (BOOL)profileHandlesScheme:(NSString *)scheme {
    ...
    return (handler && [[handler objectForKey:KEY_GUID] isEqualToString:guid] &&
...
- (IBAction)showAdvancedWorkingDirConfigPanel:(id)sender {
    ...
    __weak typeof(self) weakSelf = self;
    [self.view.window beginSheet:_advancedWorkingDirWindowController.window
               completionHandler:^(NSModalResponse returnCode) {
                   __strong __typeof(weakSelf) strongSelf = self;
                   if (!strongSelf) {
                       return;
...
- (void)handleCommandWarningClick:(NSClickGestureRecognizer *)recognizer {
    ...
    NSString *message = [NSString
        stringWithFormat:@"Command not found. You may need to specify the full path because your shell is not used "
                         @"when running this command. The search path contains the following folders: %@.",
                         [paths componentsJoinedWithOxfordComma]];
...
- (void)checkIfCommandLineIsValid {
    ...
    [[iTermSlowOperationGateway sharedInstance]
        checkIfExecutableRegularFile:filename
                         searchPaths:paths
...
- (void)setShortcutValueToSelectedItem {
    ...
    if (shortcut) {
        ...
        // Pre-allocate for potential profiles to modify (typically 0-4)
        NSMutableArray *guidsOfProfilesToModify = [NSMutableArray arrayWithCapacity:4];
        for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
            // BUG-f1652: Use iTermProfilePreferences for type-safe access (KEY_SHORTCUT default is NSNull)
            NSString *existingShortcut = [iTermProfilePreferences stringForKey:KEY_SHORTCUT inProfile:profile];
            if ([shortcut length] > 0 && [existingShortcut isEqualToString:shortcut] && profile != currentProfile) {
...
- (void)updateShortcutTitles {
    ...
    for (Profile *profile in [profileModel bookmarks]) {
        ...
        if (tag != -1) {
            ...
            NSString *newTitle = [NSString stringWithFormat:@"⌃⌘%@ (%@)", existingShortcut, profile[KEY_NAME]];
...
- (void)toggleSelectedTitleComponentForView:(NSPopUpButton *)titleSettings {
    ...
    if (selectedTag == iTermTitleComponentsCustom && originalValue != iTermTitleComponentsCustom) {
    ...
    if ((newValue & iTermTitleComponentsJob) && (newValue & iTermTitleComponentsCommandLine)) {
    ...
    NSUInteger nameTagsMask =
        (iTermTitleComponentsProfileName | iTermTitleComponentsSessionName | iTermTitleComponentsProfileAndSessionName);
...
- (void)updateSelectedTitleComponentsForView:(NSPopUpButton *)titleSettings {
    ...
    // Pre-allocate for typical number of title components (usually 2-6)
    ...
    titleSettings.title = customName
                              ?: [iTermSessionTitleBuiltInFunction titleForSessionName:@"Name"
                                                                           profileName:@"Profile"
                                                                                   job:@"Job"
...
- (IBAction)titleHelp:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace]
        it_openURL:[NSURL URLWithString:@"https://dashterm.com/documentation-session-title.html"]
            target:nil
...
- (NSArray *)tokenField:(NSTokenField *)tokenField
    ...
    // Pre-allocate for tag completions (typically 0-20 matches)
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:MIN(allTags.count, 16)];
...
- (void)controlTextDidChange:(NSNotification *)aNotification {
    ...
    if (control == _profileNameFieldForEditCurrentSession) {
        ...
        if ([self boolForKey:KEY_ALLOW_TITLE_SETTING] && [iTermAdvancedSettingsModel autoLockSessionNameOnEdit]) {
            [self toggleLock];
        }
        return;
    }
...
- (void)removeWhitespaceFromCustomCommand {
    while (1) {
        const NSRange range =
            [_customCommand.stringValue rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
        if (range.location == NSNotFound) {
            return;
        }
        _customCommand.stringValue = [_customCommand.stringValue stringByReplacingCharactersInRange:range
                                                                                         withString:@""];
    }
}
...
- (void)sessionNameDidChange {
    ...
    [_rateLimit performRateLimitedSelector:@selector(postUpdateSessionNameNotification) onTarget:self withObject:nil];
}
...
- (void)tabTitleDidChange {
    ...
    iTermCallMethodByIdentifier(self.scope.tab.tabID, @"iterm2.set_title", @{@"title" : value},
