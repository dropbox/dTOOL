...
static dispatch_queue_t iTermPathCleanerQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("com.dashterm.dashterm2.path-cleaner", DISPATCH_QUEUE_SERIAL);
    });
    return queue;
}
...
// /Users/gnachman/git/iterm2 + git/iterm2/DashTerm2.xcodeproj -> /Users/gnachman/git/iterm2/DashTerm2.xcodeproj
...
- (NSString *)getFullPath:(NSString *)pathExLineNumberAndColumn
    ...
    // BUG-1453: Standardize path BEFORE forbidden prefix check to catch path traversal.
    // e.g., ~/../../etc/passwd expands to /Users/x/../../etc/passwd, which must be
    // standardized to /etc/passwd before checking forbidden prefixes.
    // NOTE: stringByStandardizingPath does NOT access the filesystem, unlike fileURLWithPath:.
    path = [path stringByStandardizingPath];
    DLog(@"[%d]  Standardized path: %@", _reqid, path);
    ...
    // Also check fallback path if available
    if (fallbackPath) {
        fallbackPath = [fallbackPath stringByStandardizingPath];
        DLog(@"[%d]  Standardized fallback path: %@", _reqid, fallbackPath);

        if ([self fileHasForbiddenPrefix:fallbackPath]) {
            DLog(@"[%d]    NO: Fallback path has forbidden prefix.", _reqid);
        } else if ([self fileExistsAtPathLocally:fallbackPath]) {
