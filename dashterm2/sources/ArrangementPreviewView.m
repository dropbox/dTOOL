...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    // BUG-3135: Guard against division by zero when screen dimensions are degenerate
    double xRange = xMax - xMin;
    double yRange = yMax - yMin;
    if (xRange <= 0 || yRange <= 0) {
        return;
    }
    double xScale = [self frame].size.width / xRange;
    double yScale = [self frame].size.height / yRange;
    double scale = MIN(xScale, yScale);
    double yCorrection = MAX(0, ([self frame].size.height - yRange * scale) / 2);
    double xCorrection = MAX(0, ([self frame].size.width - xRange * scale) / 2);

    NSMutableArray *screenFrames = [NSMutableArray arrayWithCapacity:screens.count];
    for (NSScreen *screen in screens) {
        NSRect frame = [screen frame];
