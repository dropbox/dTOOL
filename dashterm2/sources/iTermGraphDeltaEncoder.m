...
- (instancetype)initWithKey:(NSString *)key
    ...
    // BUG-f1038: Replace assert(identifier) with guard - nil identifier should return nil, not crash
    if (!identifier) {
        ELog(@"iTermGraphDeltaEncoder initialized with nil identifier for key: %@", key);
        return nil;
    }
    self = [super initWithKey:key identifier:identifier generation:generation];
    if (self) {
        if (previousRevision && !previousRevision.rowid) {
            DLog(@"WARNING: Previous revision lacks a rowID: key=%@ id=%@ gen=%@ ptr=%p", previousRevision.key,
                 previousRevision.identifier, @(previousRevision.generation), previousRevision);
...
- (void)encodeChildrenWithKey:(NSString *)key
                  identifiers:(NSArray<NSString *> *)identifiers
                   generation:(NSInteger)generation
                        block:(BOOL (^)(NSString *_Nonnull, NSUInteger, iTermGraphEncoder *_Nonnull,
                                        BOOL *_Nonnull))block {
...
- (BOOL)encodeChildWithKey:(NSString *)key
    ...
    iTermEncoderGraphRecord *record = [_previousRevision childRecordWithKey:key identifier:identifier];
    if (!record) {
    ...
    // BUG-f1079: Replace assert with guard - generation regression should log warning, not crash
    ...
    if (record.generation >= generation) {
        DLog(@"WARNING: Generation regression detected: record.generation=%@ >= generation=%@ for key=%@ id=%@",
             @(record.generation), @(generation), key, identifier);
    }
...
- (BOOL)enumerateRecords:(void (^)(iTermEncoderGraphRecord *_Nullable before, iTermEncoderGraphRecord *_Nullable after,
                                   NSNumber *parent, NSString *path, BOOL *stop))block {
    BOOL stop = NO;
    @try {
        block(_previousRevision, self.record, @0, @"root", &stop);
    } @catch (NSException *exception) {
        [exception it_rethrowWithMessage:@"(1) %@ vs %@", _previousRevision.compactDescription,
...
- (BOOL)enumerateBefore:(iTermEncoderGraphRecord *)preRecord
                 ...
                 parent:(NSNumber *)parent
                   path:(NSString *)path
                  block:(void (^)(iTermEncoderGraphRecord *_Nullable before, iTermEncoderGraphRecord *_Nullable after,
                                  NSNumber *parent, NSString *path, BOOL *stop))block {
    iTermOrderedDictionary<iTermTuple<NSString *, NSString *> *, iTermEncoderGraphRecord *> *beforeDict =
        [iTermOrderedDictionary byMapping:preRecord.graphRecords ?: @[]
                                    block:^id _Nonnull(NSUInteger index, iTermEncoderGraphRecord *_Nonnull record) {
    ...
    iTermOrderedDictionary<iTermTuple<NSString *, NSString *> *, iTermEncoderGraphRecord *> *afterDict =
        [iTermOrderedDictionary byMapping:postRecord.graphRecords
                                    block:^id _Nonnull(NSUInteger index, iTermEncoderGraphRecord *_Nonnull record) {
                                        return [iTermTuple tupleWithObject:record.key andObject:record.identifier];
                                    }];
    __block BOOL ok = YES;
    void (^handle)(iTermTuple<NSString *, NSString *> *, iTermEncoderGraphRecord *, NSString *, BOOL *) = ^(
        iTermTuple<NSString *, NSString *> *key, iTermEncoderGraphRecord *record, NSString *path, BOOL *stop) {
        ...
        @try {
            ...
            ok = NO;
            [exception it_rethrowWithMessage:@"(2) %@ [%@] vs %@ [%@]", before.compactDescription,
                                             beforeDict.debugString, after.compactDescription, afterDict.debugString];
        } @
        try {
            ...
            ok = NO;
            [exception it_rethrowWithMessage:@"(3) %@ [%@] vs %@ [%@]", before.compactDescription,
                                             beforeDict.debugString, after.compactDescription, afterDict.debugString];
        }
    };
    NSMutableSet<iTermTuple<NSString *, NSString *> *> *seenKeys = [NSMutableSet setWithCapacity:beforeDict.keys.count];
    [beforeDict.keys enumerateObjectsUsingBlock:^(iTermTuple<NSString *, NSString *> *_Nonnull key, NSUInteger idx,
                                                  BOOL *_Nonnull stop) {
        handle(key, beforeDict[key], [NSString stringWithFormat:@"%@.%@[%@]", path, key.firstObject, key.secondObject],
               stop);
    ...
    [afterDict.keys enumerateObjectsUsingBlock:^(iTermTuple<NSString *, NSString *> *_Nonnull key, NSUInteger idx,
                                                 BOOL *_Nonnull stop) {
        if ([seenKeys containsObject:key]) {
            return;
        }
        handle(key, afterDict[key], [NSString stringWithFormat:@"%@.%@[%@]", path, key.firstObject, key.secondObject],
               stop);
        if (!ok) {
            *stop = YES;
        }
