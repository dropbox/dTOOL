...
class EventuallyConsistentIntervalTree: IntervalTree {
    ...
    override init(dictionary: [AnyHashable : Any]) {
        // BUG-f854: This init is unavailable - use init(sideEffectPerformer:dictionary:derivative:) instead
        // Since this is marked @available(*, unavailable), Swift prevents calling it at compile time
        // The it_fatalError is retained only as a final fallback for ObjC interop
        it_fatalError("JournalingIntervalTree.init(dictionary:) is unavailable")
    }

    @objc
    override func add(_ object: IntervalTreeObject, with interval: Interval) {
        super.add(object, with: interval)
        // BUG-1721: Use guard with as? instead of as! for interval copy
        guard let copyOfInterval = interval.copy(with: nil) as? Interval else {
            DLog("Failed to copy interval")
            return
        }
    ...
    func mutate(_ object: IntervalTreeImmutableObject,
        ...
        // BUG-1722: Use guard with as? instead of as! force cast
        guard let ito = object as? IntervalTreeObject else {
            DLog("Failed to cast IntervalTreeImmutableObject to IntervalTreeObject")
            return
        }
#if DEBUG
        super.sanityCheck()
        ...
        super.sanityCheck()
#endif
        addSideEffect(ito, closure: { doppelganger, _ in
    ...
    func bulkMutate(_ objects: [IntervalTreeImmutableObject],
                    closure: @escaping (IntervalTreeObject) -> ()) {
        // BUG-1723: Use compactMap with as? instead of as! force cast
        let itos = objects.compactMap { $0 as? IntervalTreeObject }
        for ito in itos {
#if DEBUG
            super.sanityCheck()
#endif
        ...
        addBulkSideEffect(itos, closure: { _, doppelganger, _ in
            closure(doppelganger)
    ...
    func bulkRemoveObjects(_ objects: [IntervalTreeImmutableObject]) {
        ...
        // BUG-1724: Use compactMap with as? instead of as! force cast
        let itos = objects.compactMap { $0 as? IntervalTreeObject }
#if DEBUG
        sanityCheck()
#endif
        ...
        sanityCheck()
#endif
        addBulkSideEffect(itos, closure: { i, ito, tree in
    ...
    func bulkMoveObjects(_ objects: [IntervalTreeImmutableObject],
        ...
        // BUG-1725: Use compactMap with as? instead of as! force cast
        let itos = objects.compactMap { $0 as? IntervalTreeObject }
        let newIntervals = itos.map { closure($0) }
        for (ito, interval) in zip(itos, newIntervals) {
#if DEBUG
super.sanityCheck()
#endif
            super.remove(ito)
