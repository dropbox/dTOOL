...
#if __has_include("DashTerm2-Swift.h")
#import "DashTerm2-Swift.h"
#endif
...
- (void)setTerminalGuid:(NSString *)terminalGuid {
    // BUG-f1148: Replace assert with guard - _scope must exist
    if (!_scope) {
        ELog(@"BUG-f1148: setTerminalGuid called when _scope is nil - ignoring");
        return;
    }
...
- (void)toggleAILock:(id)sender {
    PTYSession *session = self.currentSession;
    if (!session) {
        return;
    }
    BOOL currentlyLocked = [DTermAIInterface isAILockedWithSession:session];
    [DTermAIInterface setAILockedWithSession:session locked:!currentlyLocked];
}
...
- (IBAction)duplicateSession:(id)sender {
    ...
    if (session.sshIdentity) {
        ...
        if ([profile[KEY_CUSTOM_COMMAND] isEqualToString:kProfilePreferenceCommandTypeSSHValue]) {
            ...
            // BUG-f1149: Replace assert with guard - sequence must have at least one element
            if (sequence.count == 0) {
                ELog(@"BUG-f1149: SSH sequence is empty - cannot set SSH profile");
                return;
            }
...
- (void)restartSessionWithConfirmation:(PTYSession *)aSession {
    // BUG-f1150: Replace assert with guard - session must be restartable
    if (!aSession.isRestartable) {
        ELog(@"BUG-f1150: restartSessionWithConfirmation called with non-restartable session - ignoring");
        return;
    }
    [[self retain] autorelease];
    if (aSession.exited) {
...
- (void)setWindowTitle:(NSString *)title subtitle:(NSString *)subtitle {
    ...
    // BUG-f1151: Replace assert with guard - subtitle must not be nil
    if (!subtitle) {
        ELog(@"BUG-f1151: setWindowTitle:subtitle: called with nil subtitle - using empty string");
        subtitle = @"";
    }
...
+ (instancetype)terminalWithArrangement:(NSDictionary *)arrangement
    ...
    for (PTYSession *session in sessions) {
        // BUG-f1152: Replace assert with guard - session revive may fail
        if (![session revive]) {
            ELog(@"BUG-f1152: Failed to revive session %@ - continuing with other sessions", session);
        }
    }
...
+ (NSDictionary *)modifiedArrangement:(NSDictionary *)arrangement
    ...
    for (NSDictionary* tabArrangement in [arrangement objectForKey:TERMINAL_ARRANGEMENT_TABS]) {
        [mutableTabs addObject:[PTYTab modifiedArrangement:tabArrangement
                                                   mutator:mutator]];
    }
...
- (void)returnTabBarToContentView {
    ...
    if (![_titlebarAccessoryNanny has:_titleBarAccessoryTabBarViewController]) {
        // BUG-f1153: Replace assert with warning - tabBarControlOnLoan should be NO
        if (_contentView.tabBarControlOnLoan) {
            ELog(@"BUG-f1153: returnTabBarToContentView: nanny doesn't have VC but tabBarControlOnLoan is YES");
        }
        return;
    }
    // BUG-f1154: Replace assert with warning - tabBarControlOnLoan should be YES
    if (!_contentView.tabBarControlOnLoan) {
        ELog(@"BUG-f1154: returnTabBarToContentView: nanny has VC but tabBarControlOnLoan is NO");
    }
...
- (void)recreateTab:(PTYTab *)tab
    ...
    if (revive) {
        for (PTYSession *session in sessions) {
            // BUG-f1155: Replace assert with guard - session revive may fail
            if (![session revive]) {
                ELog(@"BUG-f1155: Failed to revive session %@ during undo close", session);
            }
        }
    }
...
#if 0  // TODO: macOS 26 - PSMTahoeTabStyle
        if (@available(macOS 26, *)) {
...
- (void)insertTab:(PTYTab*)aTab atIndex:(int)anIndex {
    ...
    // BUG-f1156: Replace assert with guard - aTab must not be nil
    if (!aTab) {
        ELog(@"BUG-f1156: insertTab:atIndex: called with nil tab - ignoring");
        return;
    }
    if ([_contentView.tabView indexOfTabViewItemWithIdentifier:aTab] == NSNotFound) {
        for (PTYSession* aSession in [aTab sessions]) {
        ...
        // BUG-f1157: Replace assert with guard - aTabViewItem must not be nil
        if (!aTabViewItem) {
            ELog(@"BUG-f1157: insertTab:atIndex: failed to create tab view item - ignoring");
            return;
        }
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
    ...
    } else if ([item action] == @selector(toggleAILock:)) {
        PTYSession *session = self.currentSession;
        if (!session) {
            return NO;
        }
#if __has_include("DashTerm2-Swift.h")
        BOOL locked = [DTermAIInterface isAILockedWithSession:session];
        item.state = locked ? NSControlStateValueOn : NSControlStateValueOff;
#endif
        return YES;
...
- (id)swipeHandlerBeginSessionAtOffset:(CGFloat)offset identifier:(nonnull id)identifier {
    ...
    // BUG-f1158: Replace assert with guard - _swipeContainerView must not already exist
    if (_swipeContainerView) {
        ELog(@"BUG-f1158: swipeHandlerBeginSessionAtOffset: _swipeContainerView already exists - cleaning up");
