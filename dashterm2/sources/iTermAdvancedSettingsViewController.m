...
+ (NSDictionary *)settingsDictionary {
    ...
    dispatch_once(&onceToken, ^{
        NSArray *advSettings = [self advancedSettings];
        // Pre-size based on number of advanced settings (~500)
        NSMutableDictionary *temp = [NSMutableDictionary dictionaryWithCapacity:advSettings.count];
        for (NSDictionary *setting in advSettings) {
...
// BUG-f1040: Guard against missing colon in description to prevent substringFromIndex crash
+ (NSArray *)groupedSettingsArrayFromSortedArray:(NSArray *)sorted {
    NSString *previousCategory = nil;
    // Result includes settings + category headers
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:sorted.count + 20];
    for (NSDictionary *dict in sorted) {
        NSString *description = dict[kAdvancedSettingDescription];
        NSRange colonRange = [description rangeOfString:@": "];
        if (colonRange.location == NSNotFound) {
            // No category separator found - use the whole description as-is with "Other" category
            if (![@"Other" isEqualToString:previousCategory]) {
                previousCategory = @"Other";
                [result addObject:previousCategory];
            }
            [result addObject:[dict mutableCopy]];
            continue;
        }
        NSString *thisCategory = [description substringToIndex:colonRange.location];
        NSString *remainder = [description substringFromIndex:colonRange.location + colonRange.length];
...
+ (NSArray<NSDictionary *> *)advancedSettings {
    ...
    dispatch_once(&onceToken, ^{
        ...
        settings = [NSMutableArray arrayWithCapacity:500];
...
- (iTermTableViewTextField *)viewForImmutableAttributedString:(NSAttributedString *)attributedString
    ...
    iTermTableViewTextField *textField =
        [_tableView makeViewWithIdentifier:@"attributedstring" owner:self] ?: [[iTermTableViewTextField alloc] init];
...
- (void)toggleTristate:(NSPopUpButton *)sender {
    ...
    if (value == 0) {
        ...
        [[NSUserDefaults standardUserDefaults] setBool:value == 2 forKey:identifier];
    }
}

- (iTermTableViewTextFieldWrapper *)viewForMutableString:(NSString *)string row:(int)row {
    iTermTableViewTextFieldWrapper *wrapper =
        [_tableView makeViewWithIdentifier:@"mutablestring" owner:self]
            ?: [[iTermTableViewTextFieldWrapper alloc] initWithFrame:NSMakeRect(0, 0, 100, 100)];
    ...
    [textField sizeToFit];
    textField.frame = NSMakeRect(0, (wrapper.frame.size.height - textField.frame.size.height) / 2.0,
                                 wrapper.frame.size.width, textField.frame.size.height);
...
- (NSArray *)filteredAdvancedSettings {
    if (!_filteredAdvancedSettings) {
        ...
        if (_searchField.stringValue.length == 0) {
            ...
            // Filter results typically return a subset of all settings
            NSMutableArray *result = [NSMutableArray arrayWithCapacity:sortedSettings.count];
...
- (NSAttributedString *)attributedStringForSettingWithName:(NSString *)description
                                                  ...
                                                  selected:(BOOL)selected {
    NSMutableAttributedString *attributedDescription = [self attributedStringForString:description
    ...
    if (subtitle) {
        ...
        NSDictionary *attributes =
            @{NSForegroundColorAttributeName : color, NSFontAttributeName : [NSFont systemFontOfSize:11]};
        NSAttributedString *attributedSubtitle = [[NSAttributedString alloc] initWithString:subtitle
...
static void iTermAdvancedSettingsSaveSecureString(NSDictionary *dict, NSString *value) {
    NSString *selectorName = dict[kAdvancedSettingSetter];
    // BUG-f898: Replace assert with guard - missing selector name should not crash
    if (!selectorName) {
        ELog(@"BUG-f898: iTermAdvancedSettingsSaveSecureString: missing selectorName in dict: %@", dict);
        return;
    }
    SEL selector = NSSelectorFromString(selectorName);
    [iTermAdvancedSettingsModel performSelector:selector withObject:value];
}
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    if (tableColumn == _settingColumn) {
        ...
        iTermTableViewTextField *textField =
            [self viewForImmutableAttributedString:[self attributedStringForSettingAtRow:row selected:NO]
        ...
        switch ([dict advancedSettingType]) {
            ...
            case kiTermAdvancedSettingTypeInteger: {
                iTermTableViewTextFieldWrapper *wrapper =
                    [self viewForMutableString:[NSString stringWithFormat:@"%@", value] row:row];
                NSTableRowView *rowView = [tableView rowViewAtRow:row makeIfNecessary:NO];
                if (rowView) {
...
- (void)controlTextDidChange:(NSNotification *)aNotification {
    if ([aNotification object] == _searchField) {
        ...
        switch ([dict advancedSettingType]) {
            case kiTermAdvancedSettingTypeBoolean:
            case kiTermAdvancedSettingTypeOptionalBoolean:
                // BUG-13011: Replace assert(NO) with DLog - these types use toggle, not text field
                // This code path shouldn't be reached for boolean types as they use toggles
                DLog(@"Unexpected boolean type in text field delegate - ignoring");
                break;

            case kiTermAdvancedSettingTypeFloat:
                [[NSUserDefaults standardUserDefaults] setFloat:string.floatValue forKey:identifier];
                break;

            case kiTermAdvancedSettingTypeInteger:
                [[NSUserDefaults standardUserDefaults] setInteger:string.integerValue forKey:identifier];
                break;
    ...
    if (!_docs) {
        _docs = [[iTermAdvancedSettingsViewController sortedAdvancedSettings] mapWithBlock:^id(NSDictionary *dict) {
            iTermPreferencesSearchDocument *doc = [iTermPreferencesSearchDocument
                documentWithDisplayName:@"Advanced Preferences…" // dict[kAdvancedSettingDescription]
...
- (NSInteger)indexOfIdentifier:(NSString *)identifier {
    return [self.filteredAdvancedSettings
        indexOfObjectPassingTest:^BOOL(NSDictionary *_Nonnull dict, NSUInteger idx, BOOL *_Nonnull stop) {
