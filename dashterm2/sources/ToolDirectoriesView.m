...
- (instancetype)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        if (@available(macOS 10.16, *)) {
            ...
            clear_.bordered = NO;
            clear_.image = [NSImage it_imageForSymbolName:SFSymbolGetString(SFSymbolTrash)
        ...
        _scrollView = [NSScrollView
            scrollViewWithTableViewForToolbeltWithContainer:self
                                                     insets:NSEdgeInsetsZero
                                                  rowHeight:[NSTableView
                                                                heightForTextCellUsingFont:[NSFont it_toolbeltFont]]
        ...
        NSMenuItem *item;
        item = [[NSMenuItem alloc] initWithTitle:@"Toggle Star" action:@selector(toggleStar:) keyEquivalent:@""];
        [_tableView.menu addItem:item];

        boldFont_ = [[NSFontManager sharedFontManager] convertFont:[NSFont it_toolbeltFont]
...
// BUG-399: Cancel delayed selectors in dealloc to prevent crash if view is
// deallocated while updateDirectories or fixCursor is pending.
- (void)dealloc {
...
- (void)relayout_bigSur {
    ...
    NSRect searchFieldFrame =
        NSMakeRect(0, 0, frame.size.width - help_.frame.size.width - clear_.frame.size.width - 2 * kMargin,
    ...
    // Help button
    {
        ...
        help_.frame = NSMakeRect(frame.size.width - help_.frame.size.width, fudgeFactor, help_.frame.size.width,
    ...
    // Clear button
    {
        ...
        clear_.frame = NSMakeRect(help_.frame.origin.x - clear_.frame.size.width - kMargin, fudgeFactor,
                                  clear_.frame.size.width, clear_.frame.size.height);
    }

    // Scroll view
    [_scrollView setFrame:NSMakeRect(0, searchFieldFrame.size.height + kMargin, frame.size.width,
...
- (void)relayout_legacy {
    ...
    help_.frame = NSMakeRect(frame.size.width - help_.frame.size.width,
                             frame.size.height - help_.frame.size.height -
                                 ceil((clear_.frame.size.height - help_.frame.size.height) / 2) + 2,
                             help_.frame.size.width, help_.frame.size.height);
    [clear_ setFrame:NSMakeRect(0, frame.size.height - kButtonHeight,
                                frame.size.width - help_.frame.size.width - kHelpMargin, kButtonHeight)];
    _scrollView.frame = NSMakeRect(0, searchField_.frame.size.height + kMargin, frame.size.width,
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    // BUG-10340: Add bounds check before accessing filteredEntries_
    if (row < 0 || row >= (NSInteger)filteredEntries_.count) {
        return nil;
    }
...
- (id)stringOrAttributedStringForColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
    // BUG-10340: Add bounds check before accessing filteredEntries_
    if (rowIndex < 0 || rowIndex >= (NSInteger)filteredEntries_.count) {
        return @"";
    }
    iTermRecentDirectoryMO *entry = filteredEntries_[rowIndex];
    NSIndexSet *indexes = [[iTermShellHistoryController sharedInstance] abbreviationSafeIndexesInRecentDirectory:entry];
    return [entry attributedStringForTableColumn:aTableColumn abbreviationSafeComponents:indexes];
}
...
- (void)doubleClickOnTableView:(id)sender {
    ...
    // BUG-10340: Add upper bounds check before accessing filteredEntries_
    if (selectedIndex < 0 || selectedIndex >= (NSInteger)filteredEntries_.count) {
        return;
    }
...
- (void)computeFilteredEntries {
    ...
    if (searchField_.stringValue.length == 0) {
        ...
        NSMutableArray *array = [[NSMutableArray alloc] initWithCapacity:entries_.count];
        for (iTermRecentDirectoryMO *entry in entries_) {
            if ([entry.path rangeOfString:searchField_.stringValue options:NSCaseInsensitiveSearch].location !=
                NSNotFound) {
...
// BUG-3005: Add bounds check to prevent array index out of bounds crash
...
- (void)toggleStar:(id)sender {
    ...
    // Take a snapshot of entries to avoid race condition where
    // the array could change between checking index and accessing it.
    NSArray<iTermRecentDirectoryMO *> *entries = filteredEntries_;
    if (index < 0 || index >= (NSInteger)entries.count) {
        return;
    }
    iTermRecentDirectoryMO *entry = entries[index];
    [[iTermShellHistoryController sharedInstance] setDirectory:entry starred:!entry.starred.boolValue];
    [self updateDirectories];
}

- (void)help:(id)sender {
    // BUG-1: Documentation URL updated to dashterm.com
    [[NSWorkspace sharedWorkspace] it_openURL:[NSURL URLWithString:@"https://dashterm.com/shell_integration.html"]
                                       target:nil
