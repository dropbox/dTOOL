...
struct MessageToPromptStateMachine {
    ...
    private mutating func body(agentMessage: Message) -> LLM.Message.Body {
        ...
        // BUG-f515: Return uninitialized for unsupported content types instead of crashing
        ...
        case .multipart(let subparts, _):
            return .multipart(subparts.compactMap { subpart -> LLM.Message.Body? in
                ...
                case .markdown(let string), .plainText(let string):
                    return .text(string)
                case .context(let contextString):
                    // Context is extra info in user-sent messages - include it as text
    ...
    private mutating func body(userMessage: Message) -> LLM.Message.Body {
        ...
        // BUG-f516: Return uninitialized for unsupported user message content types instead of crashing
        case .explanationResponse, .append, .appendAttachment, .remoteCommandRequest,
            ...
            DLog("Warning: Unexpected user message content type in body()")
            return .uninitialized
        }
    }
}
...
extension LLM.Message.Body {
    ...
    mutating func prepend(_ string: String)  {
        // BUG-f633: Use guard instead of it_assert - assertions stripped in release builds
        guard isText else {
            DLog("BUG-f633: prepend() called on non-text body - ignoring")
            return
        }
