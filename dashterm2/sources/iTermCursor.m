...
- (void)drawWithRect:(NSRect)rect
     ...
     foregroundColor:(NSColor *)foregroundColor
             ...
             outline:(BOOL)outline
       virtualOffset:(CGFloat)virtualOffset {
    // BUG-f922: Guard against unexpected outline parameter instead of crashing
    if (outline) {
        ELog(@"iTermCursor: drawWithRect called with outline=YES unexpectedly");
        // Fall through and draw non-outline version
    }
