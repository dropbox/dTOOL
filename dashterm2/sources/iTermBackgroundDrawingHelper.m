    ...
    if (backgroundImage) {
        ...
        switch ([self.delegate backgroundDrawingHelperBackgroundImageMode]) {
            ...
            case iTermBackgroundImageModeScaleAspectFit:
                image = backgroundImage;
                // Note: dirtyRectInContainerCoords is correct here (not dirtyRectInAdjustedContainerCoords).
                // The scaleAspectFitSourceRectForForImageSize method expects dirtyRect in the same coordinate
                // system as destinationRect (windowVisibleAreaRect). Both are in container coordinates.
                // The getPillarBoxes/getLetterBoxes helpers intersect dirtyRect with destinationRect,
                // so they must share the same coordinate space. The commented-out intersection was dead
                // code because dirtyRectInAdjustedContainerCoords is never used in this case branch.
...
- (NSRect)sourceRectForImageSize:(NSSize)imageSize
    ...
    // BUG-531: Guard against division by zero when viewSize width/height is zero
...
- (NSRect)scaleAspectFillSourceRectForImageSize:(NSSize)imageSize
    ...
    // BUG-532: Guard against division by zero when imageSize or viewSize width/height is zero
    ...
    if (imageAspectRatio > viewAspectRatio) {
        ...
        // BUG-533: Guard against division by zero when scale is zero
        const CGFloat crop = scale > 0 ? (scaledWidth - viewSize.width) / scale : 0;
        ...
        // BUG-534: Guard against division by zero when scale is zero
        const CGFloat crop = scale > 0 ? (scaledHeight - viewSize.height) / scale : 0;
...
+ (NSRect)scaleAspectFitRectWithImageOfSize:(NSSize)imageSize
    ...
    // BUG-535: Guard against division by zero when imageSize or destinationRect size is zero
    if (imageSize.width <= 0 || imageSize.height <= 0 ||
        destinationRect.size.width <= 0 || destinationRect.size.height <= 0) {
        if (scale) {
            *scale = 1.0;
        }
        return NSZeroRect;
    ...
    if (imageAspectRatio > viewAspectRatio) {
        ...
        // BUG-536: Guard against division by zero when imageAspectRatio is zero
        rectWithImage.size.height = imageAspectRatio > 0 ? destinationRect.size.width / imageAspectRatio : 0;
...
+ (void)getClippedScaleAspectFitSourceRect:(inout NSRect *)sourceRectInOut
    ...
    *destinationRectInOut = clippedDestinationRect;
    ...
    // BUG-537: Guard against division by zero when originalDestinationRect width/height is zero
    const CGFloat destWidth = NSWidth(originalDestinationRect);
    const CGFloat destHeight = NSHeight(originalDestinationRect);
    if (destWidth <= 0 || destHeight <= 0) {
        return;
    }
    const CGFloat leftLost = (NSMinX(clippedDestinationRect) - NSMinX(originalDestinationRect)) / destWidth;
    const CGFloat topLost = (NSMinY(clippedDestinationRect) - NSMinY(originalDestinationRect)) / destHeight;
    const CGFloat rightLost = (NSMaxX(originalDestinationRect) - NSMaxX(clippedDestinationRect)) / destWidth;
    const CGFloat bottomLost = (NSMaxY(originalDestinationRect) - NSMaxY(clippedDestinationRect)) / destHeight;
...
+ (NSRect)scaleAspectFitSourceRectForForImageSize:(NSSize)imageSize
                                  destinationRect:(NSRect)destinationRect
                                        dirtyRect:(NSRect)dirtyRect
                                         ...
                                         boxRect1:(out NSRect *)boxRect1
                                         boxRect2:(out NSRect *)boxRect2 {
    // BUG-538: Guard against division by zero when imageSize or destinationRect size is zero
    if (imageSize.width <= 0 || imageSize.height <= 0 ||
        destinationRect.size.width <= 0 || destinationRect.size.height <= 0) {
        if (drawRect) {
            *drawRect = NSZeroRect;
