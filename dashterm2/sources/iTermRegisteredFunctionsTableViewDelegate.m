...
- (instancetype)initWithSignature:(NSString *)signature role:(NSString *)role script:(NSString *)script {
    self = [super init];
    if (self) {
...
- (void)loadRows {
    NSDictionary<NSString *, iTermTuple<id, ITMNotificationRequest *> *> *subs =
        [[iTermAPIHelper sharedInstance] serverOriginatedRPCSubscriptions];
    _rows = [subs.allKeys mapWithBlock:^id(NSString *signature) {
        ...
        iTermScriptHistoryEntry *entry =
            [[iTermAPIHelper sharedInstance] scriptHistoryEntryForConnectionKey:connectionKey];
...
- (NSString *)stringForProxy:(iTermRegisteredFunctionProxy *)proxy columnIdentifier:(NSString *)identifier {
    ...
    // BUG-405: Replace assert(NO) with DLog for unknown column identifier
    // This handles case where a new column is added to the table but not handled here
    DLog(@"iTermRegisteredFunctionsTableViewDelegate: Unknown column identifier '%@'", identifier);
    return nil;
}
