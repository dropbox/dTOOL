...
class TailFindController: NSObject {
    ...
    func beginOneShotTailFind() {
        ...
        performingOneShotTailFind = true  // BUG-976: Was false, should be true when starting
...
extension TailFindController {
    ...
    private func continueTailFind() {
        ...
        for r in results {
            guard let searchResult = r as? SearchResult else {
                DLog("Unexpected object in tail find results: \(type(of: r))")
                continue
            }
            delegate.tailFindControllerAdd(searchResult: searchResult)
        }
        if results.count > 0 {  // swiftlint:disable:this empty_count
    ...
    private func beginTailFindImpl() -> Bool {
        ...
        guard mainSearchEngine.hasRequest, let query = mainSearchEngine.query else {
            return false
        }
