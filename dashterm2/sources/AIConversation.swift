...
struct AIConversation {
    ...
    mutating func add(_ aiMessage: AITermController.Message) {
        // BUG-f984: Use guard with isEmpty check before removeLast() to prevent crash
        // The original while loop could crash if messages became empty during iteration
        while let lastRole = messages.last?.role, lastRole == aiMessage.role, !messages.isEmpty {
            messages.removeLast()
        }
            ...
            if let registrationProvider {
                registrationProvider.registrationProviderRequestRegistration() { registration in
                    if let registration {
                        ...
                        // BUG-3: AI error messages updated to DashTerm2 branding
                        completion(AIError("You must provide a valid API key to use AI features in DashTerm2."))
                    }
                }
            } else {
    ...
    mutating func complete(streaming: ((LLM.StreamingUpdate, String?) -> ())?,
        ...
        // BUG-1626: Use guard instead of precondition - empty messages should fail gracefully
        guard !messages.isEmpty else {
            completion(.failure(AIError("Cannot complete AI conversation with no messages")))
            return
        }
