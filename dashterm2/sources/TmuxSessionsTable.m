...
- (instancetype)init {
    self = [super init];
    if (self) {
        _model = [[NSMutableArray alloc] initWithCapacity:16]; // Tmux sessions
    }
    return self;
}
...
- (void)selectSessionNumber:(int)number {
    ...
    NSInteger i = [_model
        indexOfObjectPassingTest:^BOOL(iTermTmuxSessionObject *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            return obj.number == number;
        }];
    if (i != NSNotFound) {
        [tableView_ selectRowIndexes:[NSIndexSet indexSetWithIndex:i] byExtendingSelection:NO];
    }
...
- (id)tableView:(NSTableView *)aTableView
    ...
    // BUG-1080: Add bounds check before accessing _model
    if (rowIndex < 0 || rowIndex >= (NSInteger)_model.count) {
        return nil;
    }
...
- (void)tableView:(NSTableView *)aTableView
    ...
    [delegate_ renameSessionWithNumber:_model[rowIndex].number toName:(NSString *)anObject];
}
...
- (NSNumber *)selectedSessionNumber {
    // BUG-1104: Use NSInteger for selectedRow to handle -1 correctly across platforms
    NSInteger i = [tableView_ selectedRow];
    if (i >= 0 && i < (NSInteger)_model.count) {
...
- (BOOL)tableView:(NSTableView *)aTableView
    ...
    NSMutableArray<NSArray *> *draggedItems = [NSMutableArray arrayWithCapacity:4]; // Dragged items
    [info
        enumerateDraggingItemsWithOptions:0
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *_Nonnull draggingItem, NSInteger idx, BOOL *_Nonnull stop) {
                                   ...
                                   // BUG-f1057: Validate array has at least 2 elements before accessing indices
                                   if (array.count >= 2) {
                                       sessionNumber = array[0];
                                       id arrayElement = array[1];
                                       if ([arrayElement isKindOfClass:[NSArray class]]) {
                                           [draggedItems addObjectsFromArray:arrayElement];
                                       }
                                   }
                               }];
    ...
    for (NSArray *tuple in draggedItems) {
        // BUG-f1058: Validate tuple has at least 2 elements before accessing index 1
        if (tuple.count < 2) {
            continue;
        }
...
- (void)updateEnabledStateOfButtons {
    if ([tableView_ selectedRow] < 0) {
        ...
        BOOL isAttachedSession =
            (selected != nil && [[delegate_ numberOfAttachedSession] isEqual:@(selected.intValue)]);
