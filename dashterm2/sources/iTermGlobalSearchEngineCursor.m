...
- (instancetype)initWithQuery:(NSString *)query mode:(iTermFindMode)mode session:(PTYSession *)session {
...
- (instancetype)initWithQuery:(NSString *)query
    ...
    if (self) {
        ...
        iTermSearchRequest *request =
            [[iTermSearchRequest alloc] initWithQuery:query
                                                 mode:mode
...
- (long long)expectedLines {
    if (self.session.screen.showingAlternateScreen) {
        const long long lastLineY =
            [self.session.screen absLineNumberOfLastLineInLineBuffer] - self.session.screen.totalScrollbackOverflow;
...
- (iTermGlobalSearchEngineCursorSearchOutput)search {
    NSMutableArray<SearchResult *> *results = [NSMutableArray arrayWithCapacity:64]; // Global search results
    ...
    NSArray<iTermExternalSearchResult *> *externals = [esrc externalSearchResultsForQuery:query mode:mode];
    NSArray<SearchResult *> *wrapped =
        [externals mapEnumeratedWithBlock:^id(NSUInteger i, iTermExternalSearchResult *external, BOOL *stop) {
    ...
    const long long gridStartAbsY =
        self.session.screen.numberOfScrollbackLines + self.session.screen.totalScrollbackOverflow;
    NSArray<id<iTermGlobalSearchResultProtocol>> *mapped = [results mapWithBlock:^id(SearchResult *anObject) {
        if (self.currentScreenIsAlternate && self.pass == iTermGlobalSearchEngineCursorPassMainScreen &&
        ...
        if (anObject.isExternal) {
            result.snippet = [esrc snippetFromExternalSearchResult:anObject.externalResult
    ...
    return (iTermGlobalSearchEngineCursorSearchOutput){
        .results = mapped, .more = more, .retiredLines = more ? 0 : self.session.screen.numberOfLines};
}
...
- (NSDictionary *)regularAttributes {
    return @{NSFontAttributeName : [NSFont systemFontOfSize:[NSFont systemFontSize]]};
}

- (NSAttributedString *)snippetFromExtractor:(iTermTextExtractor *)extractor result:(SearchResult *)result {
    // BUG-f1068: Replace assert with guard - external results should return nil not crash
    if (result.isExternal) {
        DLog(@"BUG-f1068: snippetFromExtractor called with external result, returning nil");
        return nil;
    }

    const VT100GridAbsCoordRange range = VT100GridAbsCoordRangeMake(result.internalStartX, result.internalAbsStartY,
                                                                    result.internalEndX + 1, result.internalAbsEndY);
...
- (instancetype)initWithQuery:(NSString *)query mode:(iTermFindMode)mode session:(PTYSession *)session {
    ...
    if (self) {
        ...
        self.mode = mode;
        self.session = session;
        _stream = [self.session.view.browserViewController executeGlobalSearch:self.query mode:self.mode];
    }
    return self;
}
...
- (void)drainFully:(void (^NS_NOESCAPE)(NSArray<id<iTermGlobalSearchResultProtocol>> *, NSUInteger))handler {
    ...
    NSArray<id<iTermGlobalSearchResultProtocol>> *results =
        [_stream.consume mapWithBlock:^id _Nullable(iTermBrowserFindResult *findResult) {
