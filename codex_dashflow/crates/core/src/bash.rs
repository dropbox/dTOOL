//! Bash command parsing using tree-sitter.
//!
//! This module provides utilities for parsing shell commands using tree-sitter-bash,
//! allowing analysis of command structure for safety checks and approval decisions.
...
/// Extract shell and script from a command like `["bash", "-c", "ls"]`.
...
mod tests {
    ...
    #[test]
    fn parse_bash_c_plain_commands() {
        let command = vec!["bash".to_string(), "-c".to_string(), "pwd".to_string()];
        let parsed = parse_shell_lc_plain_commands(&command).unwrap();
        assert_eq!(parsed, vec![vec!["pwd".to_string()]]);
    }

    #[test]
    fn extract_bash_command_valid() {
        let cmd = vec![
            "bash".to_string(),
            "-c".to_string(),
    ...
    #[test]
    fn extract_bash_command_invalid_flag() {
        let cmd = vec![
            "bash".to_string(),
            "-x".to_string(),
            "echo hello".to_string(),
    ...
    #[test]
    fn extract_bash_command_invalid_shell() {
        let cmd = vec![
            "fish".to_string(),
            "-c".to_string(),
            "echo hello".to_string(),
    ...
    #[test]
    fn extract_bash_command_wrong_length() {
    ...
    #[test]
    fn try_parse_shell_returns_tree() {
        let tree = try_parse_shell("ls -la").unwrap();
        assert!(!tree.root_node().has_error());
    }

    #[test]
    fn try_parse_shell_handles_complex_commands() {
        let tree = try_parse_shell("for i in 1 2 3; do echo $i; done").unwrap();
        // This should parse, but won't pass word-only check
        assert!(!tree.root_node().has_error());

        // But it should fail word-only sequence check
        assert!(
            try_parse_word_only_commands_sequence(&tree, "for i in 1 2 3; do echo $i; done")
                .is_none()
        );
    }
}
