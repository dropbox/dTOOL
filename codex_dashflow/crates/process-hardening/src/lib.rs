//! Process hardening for security applications.
//!
//! This module provides functions to harden a process against debugging and memory
//! inspection. It is designed to be called pre-main() (using `#[ctor::ctor]`) to
//! perform various process hardening steps, such as:
...
pub(crate) fn pre_main_hardening_linux() {
    ...
    for key in ld_keys {
        // Safety: We collected the keys above and remove them here.
        // This is safe because we're removing environment variables we own.
        unsafe {
...
pub(crate) fn pre_main_hardening_windows() {
    // Windows hardening is more complex and depends on specific requirements.
