...
//! Fireworks AI chat model implementation

use crate::FIREWORKS_API_BASE;
...
/// Fireworks AI chat model configuration and client
///
/// Fireworks AI provides fast LLM inference with an OpenAI-compatible API.
///
/// # Example
/// ```no_run
...
///
/// # Available Models
/// - `accounts/fireworks/models/llama-v3p1-8b-instruct` (default) - Fast Llama 3.1 8B
...
pub struct ChatFireworks {
    /// Fireworks client (using OpenAI-compatible interface)
...
impl ChatFireworks {
    /// Create a new `ChatFireworks` instance with default settings
    ///
    /// Uses `FIREWORKS_API_KEY` environment variable for authentication
    #[deprecated(
        since = "1.0.1",
        note = "Use `dashflow_fireworks::build_chat_model(&config)` for config-driven instantiation"
    )]
    #[must_use]
    pub fn new() -> Self {
        let config = OpenAIConfig::new().with_api_base(FIREWORKS_API_BASE);
        Self::with_config(config)
    }

    /// Create a new `ChatFireworks` instance with custom configuration
    #[must_use]
    ...
    /// });
    ///
    /// let model = ChatFireworks::new().with_tools(vec![tool]);
    /// ```
    #[deprecated(
        since = "1.9.0",
    ...
    /// Create a `ChatFireworks` instance from a configuration
    ///
    /// This method constructs a `ChatFireworks` model from a `ChatModelConfig::Fireworks` variant,
    /// resolving environment variables for API keys and applying all configuration parameters.
    ///
    /// # Arguments
    ///
    /// * `config` - A reference to a `ChatModelConfig` (must be Fireworks variant)
    ///
    /// # Returns
    ///
    /// Returns a `Result<Self>` with the constructed `ChatFireworks` instance.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    /// - The config is not a Fireworks variant
    ...
    pub fn from_config(
        ...
        match config {
            ChatModelConfig::Fireworks {
                ...
                // Build the Fireworks config with custom API key
                let openai_config = OpenAIConfig::new()
                    .with_api_key(&resolved_api_key)
                    .with_api_base(FIREWORKS_API_BASE);

                // Create the ChatFireworks instance
            ...
            _ => Err(dashflow::core::error::Error::Configuration(format!(
                "Expected Fireworks config, got {} config",
                config.provider()
            ))),
        }
    }
...
impl Default for ChatFireworks {
    fn default() -> Self {
        Self::new()
    }
}
...
/// Convert `DashFlow` `ToolDefinition` to `OpenAI` `ChatCompletionTool` (Fireworks uses `OpenAI` format)
...
/// Convert `DashFlow` `ToolChoice` to `OpenAI` `ChatCompletionToolChoiceOption` (Fireworks uses `OpenAI` format)
...
#[async_trait]
impl ChatModel for ChatFireworks {
    async fn _generate(
        &self,
        ...
        // Fireworks requires n=1 (does not support multiple completions)
        request_builder.n(1);
        ...
        if let Some(tool_defs) = tools {
            let fireworks_tools: Vec<ChatCompletionTool> =
                tool_defs.iter().map(convert_tool_definition).collect();
            request_builder.tools(fireworks_tools);
        ...
        let response_result = with_retry(&self.retry_policy, move || {
            ...
            async move {
                client
                    ...
                    .create(request)
                    .await
                    .map_err(|e| Error::api(format!("Fireworks API error: {e}")))
            }
        })
        .await;
    ...
    async fn _stream(
        ...
        let mut stream = self
            ...
            .await
            .map_err(|e| Error::api(format!("Fireworks streaming API error: {e}")))?;
...
mod tests {
    ...
    #[test]
    fn test_chat_fireworks_builder() {
        let model = ChatFireworks::new()
            .with_model("gpt-4")
            .with_temperature(0.7)
    ...
    fn test_with_tools_multiple() {
        ...
        let model = ChatFireworks::new().with_tools(tools);
    ...
    fn test_with_tools_empty() {
        let model = ChatFireworks::new().with_tools(vec![]);
    ...
    fn test_with_tool_choice_none() {
        let model = ChatFireworks::new().with_tool_choice(Some("none".to_string()));
    ...
    fn test_with_tool_choice_auto() {
        let model = ChatFireworks::new().with_tool_choice(Some("auto".to_string()));
    ...
    fn test_with_tool_choice_required() {
        let model = ChatFireworks::new().with_tool_choice(Some("required".to_string()));
    ...
    fn test_with_tool_choice_named() {
        let model = ChatFireworks::new().with_tool_choice(Some("get_weather".to_string()));
    ...
    fn test_with_tool_choice_unset() {
        let model = ChatFireworks::new().with_tool_choice(None);
    ...
    fn test_with_json_mode() {
        let model = ChatFireworks::new().with_json_mode();
    ...
    fn test_with_structured_output() {
        ...
        let model = ChatFireworks::new().with_structured_output(
            "user_info",
            schema.clone(),
    ...
    fn test_json_mode_and_tools_incompatible() {
        ...
        let model = ChatFireworks::new().with_tools(vec![tool]).with_json_mode();
    ...
    fn test_builder_default_values() {
        let model = ChatFireworks::new();
        assert_eq!(model.model, "accounts/fireworks/models/llama-v3p1-8b-instruct");
    ...
    fn test_builder_with_model_various_names() {
        // Test various Fireworks model names
        let model_names = [
        ...
        for name in model_names {
            let model = ChatFireworks::new().with_model(name);
    ...
    fn test_builder_with_model_string_ownership() {
        ...
        let model1 = ChatFireworks::new().with_model("llama-v3p1-8b-instruct");
        let model2 = ChatFireworks::new().with_model(String::from("llama-v3p1-8b-instruct"));
    ...
    fn test_builder_temperature_boundary_values() {
        ...
        let model = ChatFireworks::new().with_temperature(0.0);
        assert_eq!(model.temperature, Some(0.0));

        let model = ChatFireworks::new().with_temperature(1.0);
        assert_eq!(model.temperature, Some(1.0));

        let model = ChatFireworks::new().with_temperature(2.0);
        assert_eq!(model.temperature, Some(2.0));

        let model = ChatFireworks::new().with_temperature(0.5);
    ...
    fn test_builder_max_tokens_various_values() {
        ...
        for val in values {
            let model = ChatFireworks::new().with_max_tokens(val);
    ...
    fn test_builder_top_p_boundary_values() {
        let model = ChatFireworks::new().with_top_p(0.0);
        assert_eq!(model.top_p, Some(0.0));

        let model = ChatFireworks::new().with_top_p(0.5);
        assert_eq!(model.top_p, Some(0.5));

        let model = ChatFireworks::new().with_top_p(1.0);
    ...
    fn test_builder_frequency_penalty_boundary_values() {
        ...
        let model = ChatFireworks::new().with_frequency_penalty(-2.0);
        assert_eq!(model.frequency_penalty, Some(-2.0));

        let model = ChatFireworks::new().with_frequency_penalty(0.0);
        assert_eq!(model.frequency_penalty, Some(0.0));

        let model = ChatFireworks::new().with_frequency_penalty(2.0);
        assert_eq!(model.frequency_penalty, Some(2.0));

        let model = ChatFireworks::new().with_frequency_penalty(-0.5);
        assert_eq!(model.frequency_penalty, Some(-0.5));

        let model = ChatFireworks::new().with_frequency_penalty(1.5);
    ...
    fn test_builder_presence_penalty_boundary_values() {
        ...
        let model = ChatFireworks::new().with_presence_penalty(-2.0);
        assert_eq!(model.presence_penalty, Some(-2.0));

        let model = ChatFireworks::new().with_presence_penalty(0.0);
    ...
    fn test_builder_chaining_all_options() {
        ...
        assert_eq!(model.model, "accounts/fireworks/models/llama-v3p3-70b-instruct");
        assert_eq!(model.temperature, Some(0.7));
    ...
    fn test_builder_retry_policy() {
        ...
        let model = ChatFireworks::new().with_retry_policy(policy);
    ...
    fn test_builder_override_values() {
        ...
        let model = ChatFireworks::new()
            .with_model("llama-v3p1-8b-instruct")
            .with_model("llama-v3p3-70b-instruct");
        assert_eq!(model.model, "llama-v3p3-70b-instruct");
    }

    #[test]
    fn test_default_trait() {
        let model: ChatFireworks = Default::default();
        assert_eq!(model.model, "accounts/fireworks/models/llama-v3p1-8b-instruct");
        assert!(model.temperature.is_none());
    }
    ...
    fn test_model_clone() {
        ...
        assert_eq!(cloned.model, "accounts/fireworks/models/llama-v3p3-70b-instruct");
    ...
    fn test_model_debug_format() {
        let model = ChatFireworks::new().with_model("test-model");
        let debug_str = format!("{:?}", model);
        assert!(debug_str.contains("ChatFireworks"));
    ...
    fn test_identifying_params_with_temperature() {
        use dashflow::core::language_models::ChatModel;
        let model = ChatFireworks::new().with_temperature(0.7);
        let params = model.identifying_params();
        assert_eq!(
            params.get("model").unwrap(),
            "accounts/fireworks/models/llama-v3p1-8b-instruct"
        );
        // Temperature is stored as f32, so use approx comparison
        let temp: f64 = params.get("temperature").unwrap().as_f64().unwrap();
    ...
    fn test_identifying_params_with_max_tokens() {
        use dashflow::core::language_models::ChatModel;
        let model = ChatFireworks::new().with_max_tokens(1000);
        let params = model.identifying_params();
        assert_eq!(
            params.get("model").unwrap(),
            "accounts/fireworks/models/llama-v3p1-8b-instruct"
    ...
    fn test_identifying_params_full() {
        ...
        // Temperature is stored as f32, so use approx comparison
        let temp: f64 = params.get("temperature").unwrap().as_f64().unwrap();
        assert!((temp - 0.5).abs() < 0.001, "temperature was {}", temp);
    ...
    #[test]
    fn test_llm_type_returns_static_identifier() {
        use dashflow::core::language_models::ChatModel;
        let model = ChatFireworks::new().with_model("custom-model");
        // llm_type returns a static identifier, not the configured model name
    ...
    fn test_as_any_downcast() {
        ...
        let downcast = any.downcast_ref::<ChatFireworks>().unwrap();
        assert_eq!(downcast.model, "test-model");
    }
    ...
    #[test]
    fn test_with_structured_output_empty_schema() {
        let schema = serde_json::json!({});
        ...
        match model.response_format.unwrap() {
            ResponseFormat::JsonSchema { json_schema } => {
                assert_eq!(json_schema.name, "empty_output");
    ...
    #[test]
    fn test_structured_output_overrides_json_mode() {
        let schema = serde_json::json!({"type": "object"});

        // First set JSON mode, then structured output - structured output should win
        let model = ChatFireworks::new()
            .with_json_mode()
            .with_structured_output("test", schema.clone(), None, true);

        match model.response_format.unwrap() {
            ResponseFormat::JsonSchema { json_schema } => {
                assert_eq!(json_schema.name, "test");
            }
            _ => panic!("Expected JsonSchema, not JsonObject"),
        }
    }

    #[test]
    fn test_json_mode_overrides_structured_output() {
        let schema = serde_json::json!({"type": "object"});

        // First set structured output, then JSON mode - JSON mode should win
        let model = ChatFireworks::new()
            .with_structured_output("test", schema.clone(), None, true)
            .with_json_mode();
        ...
        match model.response_format.unwrap() {
            ResponseFormat::JsonObject => {}
            _ => panic!("Expected JsonObject, not JsonSchema"),
        }
    }

    // ========================================================================
    // TOOLS BUILDER EDGE CASES
    // ========================================================================

    #[test]
    fn test_with_tools_name_only() {
        let tool = serde_json::json!({"name": "simple_tool"});
    ...
    fn test_with_tools_all_invalid_filtered() {
        // Tools without names should all be filtered out
        let tools = vec![
            serde_json::json!({"description": "No name 1"}),
            serde_json::json!({"description": "No name 2"}),
        ];

        let model = ChatFireworks::new().with_tools(tools);

        // All invalid, so tools should be None
        assert!(model.tools.is_none());
    }
    ...
    fn test_with_tools_mixed_valid_invalid() {
        let tools = vec![
            serde_json::json!({"description": "No name"}),
            serde_json::json!({"name": "valid_tool_1"}),
            serde_json::json!({"not_a_name": "wrong_field"}),
            serde_json::json!({"name": "valid_tool_2", "description": "Has description"}),
        ];
        ...
        let bound_tools = model.tools.unwrap();
        assert_eq!(bound_tools.len(), 2);
        assert_eq!(bound_tools[0].function.name, "valid_tool_1");
        assert_eq!(bound_tools[1].function.name, "valid_tool_2");
    }
    ...
    #[test]
    fn test_with_tools_special_characters() {
        let tool = serde_json::json!({
            "name": "get-weather_v2.0",
        ...
        assert!(model.tools.is_some());
        let tools = model.tools.unwrap();
        assert_eq!(tools[0].function.name, "get-weather_v2.0");
    }
    ...
    #[test]
    fn test_message_conversion_human_text_only_blocks() {
        use dashflow::core::messages::{ContentBlock, MessageContent};
        ...
        match converted {
            ChatCompletionRequestMessage::User(user_msg) => match user_msg.content {
                ...
                _ => panic!("Expected text content (optimized from single block)"),
            },
            _ => panic!("Expected user message"),
        }
    }
    ...
    #[test]
    fn test_message_conversion_human_empty_blocks() {
        ...
        match converted {
            ChatCompletionRequestMessage::User(user_msg) => match user_msg.content {
                ChatCompletionRequestUserMessageContent::Array(parts) => {
                    // Empty blocks result in empty array
                    assert_eq!(parts.len(), 0);
                }
                ChatCompletionRequestUserMessageContent::Text(text) => {
                    // Also valid if optimized to empty string
                    assert_eq!(text, "");
                }
            },
            _ => panic!("Expected user message"),
    ...
    #[test]
    fn test_message_conversion_tool_use_filtering() {
        use dashflow::core::messages::{ContentBlock, MessageContent};

        // ToolUse blocks should be filtered out from user messages
        let msg = Message::Human {
            content: MessageContent::Blocks(vec![
                ContentBlock::Text {
        ...
        match converted {
            ChatCompletionRequestMessage::User(user_msg) => match user_msg.content {
                ChatCompletionRequestUserMessageContent::Array(parts) => {
                    // ToolUse should be filtered, only text parts remain
                    assert_eq!(parts.len(), 2);
                }
                ChatCompletionRequestUserMessageContent::Text(_) => {
                    // Also valid if optimized
                }
            },
            _ => panic!("Expected user message"),
        }
...
///
/// These tests verify that ChatFireworks behaves consistently with other
...
mod standard_tests {
    ...
    use dashflow_test_utils::{fireworks_credentials, init_test_env};

    /// Helper function to create a test model with standard settings
    ///
    /// Uses accounts/fireworks/models/llama-v3p1-8b-instruct for testing
    fn create_test_model() -> ChatFireworks {
        ChatFireworks::new()
    ...
    fn check_credentials() {
        init_test_env().ok();
        fireworks_credentials().expect("FIREWORKS_API_KEY must be set");
    }
    ...
    async fn test_invoke_standard() {
        check_credentials();
        if fireworks_credentials().is_err() {
            return;
        }
