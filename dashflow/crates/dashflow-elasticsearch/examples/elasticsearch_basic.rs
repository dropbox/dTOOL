...
//!
//! This example demonstrates how to use ElasticsearchVectorStore for storing
//! and searching document embeddings in Elasticsearch.
//!
//! **Prerequisites:**
...
//! **Run this example:**
//! ```bash
//! cargo run --package dashflow-elasticsearch --example elasticsearch_basic
//! ```
//!
//! Covers:
//! - Creating an Elasticsearch vector store with index
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Elasticsearch Vector Store Example ===\n");
    println!("Note: This requires a running Elasticsearch server at http://localhost:9200");
    println!("Start with: docker run -p 9200:9200 -e \"discovery.type=single-node\" \\");
    println!("  -e \"xpack.security.enabled=false\" docker.elastic.co/elasticsearch/elasticsearch:8.11.0\n");

    // Create embeddings and connect to Elasticsearch
    let embeddings: Arc<dyn Embeddings> = Arc::new(DemoEmbeddings);
    let index_name = "dashflow_demo";

    println!(
        "Connecting to Elasticsearch and creating index '{}'...",
        index_name
    );
    let mut store =
        ElasticsearchVectorStore::new(index_name, Arc::clone(&embeddings), "http://localhost:9200")
            .await?;
