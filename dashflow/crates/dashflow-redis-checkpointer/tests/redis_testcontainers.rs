...
//
//! Integration tests for RedisCheckpointer using testcontainers.
//! These tests automatically start Redis in Docker and clean up afterward.
//!
//! Run these tests with:
//! ```bash
//! # On macOS with Colima, set DOCKER_HOST:
//! export DOCKER_HOST=unix://$HOME/.colima/default/docker.sock
//! cargo test -p dashflow-redis-checkpointer --test redis_testcontainers
//!
...
/// Build Redis connection string from container
...
async fn test_redis_checkpointer_save_load_with_testcontainers() {
    // Start Redis in Docker (automatically cleaned up when test ends)
    ...
    let state = TestState {
        value: 42,
        message: "Hello, Redis with testcontainers!".to_string(),
    };
    ...
    assert_eq!(loaded.state.message, "Hello, Redis with testcontainers!");
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_get_latest_with_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_list_with_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_delete_with_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_delete_thread_with_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_with_metadata_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn test_redis_checkpointer_concurrent_saves_testcontainers() {
    let container = Redis::default().start().await.unwrap();
    let host = container.get_host().await.unwrap();
    let port = container.get_host_port_ipv4(6379).await.unwrap();
    let connection_string = build_connection_string(&host.to_string(), port);
    ...
    let thread_id = "thread_concurrent_test_tc";
