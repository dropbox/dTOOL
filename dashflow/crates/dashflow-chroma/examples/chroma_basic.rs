...
//!
//! This example demonstrates how to use ChromaVectorStore for storing
//! and searching document embeddings in Chroma.
//!
//! **Prerequisites:**
...
//!
//! Covers:
//! - Creating a Chroma vector store with collection
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Chroma Vector Store Example ===\n");
    println!("Note: This requires a running Chroma server at http://localhost:8000");
    println!("Start with: docker run -p 8000:8000 chromadb/chroma\n");

    // Create embeddings and connect to Chroma
    ...
    let mut store = ChromaVectorStore::new(
        ...
        Some("http://localhost:8000"), // Chroma server URL
    )
    .await?;
    ...
    println!("Example 4: Metadata Filtering");
    let mut filter = HashMap::new();
    filter.insert("category".to_string(), serde_json::json!("science"));

    let filtered_results = store
        .similarity_search_with_score(query, 5, Some(&filter))
        .await?;

    println!("Results filtered by category='science':");
    ...
    store.add_texts(&similar_texts, None, None).await?;
    ...
    let regular_results = store._similarity_search(ml_query, 3, None).await?;
    ...
    let mmr_results = store
        .max_marginal_relevance_search(ml_query, 3, 8, 0.5, None)
        .await?;
    ...
    let mmr_high_lambda = store
        .max_marginal_relevance_search(ml_query, 3, 8, 1.0, None)
        .await?;
    ...
    let mmr_low_lambda = store
        .max_marginal_relevance_search(ml_query, 3, 8, 0.0, None)
        .await?;
    ...
    store.delete(Some(&[ids[0].clone()])).await?;
    ...
    let added_ids = store
        .add_texts(&custom_texts, None, Some(&custom_ids))
        .await?;
    ...
    let vector_results = store
        .similarity_search_by_vector(&query_vector, 2, None)
        .await?;
    ...
    // Example 10: Add Document Objects
    println!("Example 10: Adding Full Document Objects");
    let mut doc_meta = HashMap::new();
    doc_meta.insert("source".to_string(), serde_json::json!("example"));
    doc_meta.insert("importance".to_string(), serde_json::json!("high"));

    let documents = vec![
        Document {
            id: Some("doc-obj-1".to_string()),
            page_content: "Documents can contain structured metadata".to_string(),
            metadata: doc_meta.clone(),
        },
        Document {
            id: None, // Auto-generated ID
            page_content: "This document will receive an auto-generated UUID".to_string(),
            metadata: doc_meta,
        },
    ];

    let doc_ids = store.add_documents(&documents, None).await?;
    println!("Added Document objects with IDs: {:?}", doc_ids);
    println!("First has custom ID: {}", doc_ids[0] == "doc-obj-1");
    println!();

    // Final stats
    println!("=== Summary ===");
    println!(
        "Chroma collection '{}' now contains multiple documents",
        collection_name
    );
    println!("Demonstrated: CRUD operations, similarity search, metadata filtering, and MMR");
    println!("\nExample complete!");
    println!("\nNote: Collection persists in Chroma. To reset, restart the Chroma container.");
